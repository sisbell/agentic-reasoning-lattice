...
8. **`gate_vspec`'s ordinal-level requirement is a completeness guard, not a discretionary narrowing; depth-compatibility is deliberately *not* gated.** ASN-0077 WF_V(iv) requires only `actionPoint(ℓ) ≤ #u`, and ASN-0122 X12 requires only T12-well-formedness + a content-subspace start — *not* strict ordinal-level. M6 nonetheless gates all four resolve-based ops (RETRIEVEV, SHOWORIGIN_V, COMPARE, FINDDOCSCONTAINING) to **ordinal-level, level-uniform** spans. **Resolution:** for RETRIEVEV this is exactly ASN-0115 VSpec well-formedness (which *does* require ordinal-level + level-uniform), so rejecting is faithful by the note's own definition. For SHOWORIGIN_V, COMPARE, and FINDDOCSCONTAINING it is a narrowing beyond the note's T12-only precondition — and the binding reason to *reject* such a span rather than accept it is **completeness, not convenience**: M5's only resolution primitives, `resolve`/`resolve_coverage`, **force-empty** any span that is not depth-2 ordinal-level, so accepting a WF_V/X12-admissible-but-non-ordinal-level span and pushing it through M5 would silently drop its real contribution and return an **under-complete** answer — a violation of COMPARE **R2-completeness** / FINDDOCSCONTAINING **FD-COMPLETE** (and the analogous origin-set completeness for SHOWORIGIN_V). The gate is therefore a *completeness guard* against M5's narrow resolve domain: the spans it rejects are **exactly the ones M6 cannot answer correctly with the upstream as given**, so turning M5's silent upstream empty into an explicit typed rejection (`MalformedSpec`/`MalformedSpan`) is the faithful behavior — not merely a narrowing that happens to lose no region. (Every selection these notes name is in any case expressible as an ordinal-level span in the `m_S ≡ 2` model, so no admissible query is lost.) **What `gate_vspec` does *not* do is gate the depth:** it admits any well-formed `#start ≥ 2` start and lets a depth-incompatible (`#start ≥ 3`) span pass to M5's `resolve`, which force-empties it. The distinction from the non-ordinal-level case is *what the contract says the answer should be*: a depth-incompatible span is **contractually empty** — ASN-0115 R6 mandates a depth-incompatible spec yield `act = ∅` *without failure*, and X12 reports a depth-incompatible content span's empty region as success — so force-emptying it is the *correct, complete* answer, whereas a non-ordinal-level span has real results M5 cannot produce. Gating depth-2 *as well-formedness* would be an over-reach — ASN-0115 is explicit that depth-compatibility is a consulting-state predicate, **not** a well-formedness condition. So depth-incompatibility stays a silent ⟨⟩ (RETRIEVEV R6 / COMPARE empty region), surfacing as a typed rejection only where the operation's *own* precondition demands the common depth — SHOWORIGIN_V's WF_V(v), discharged via `RangeNotPresent`. Recorded here as the deliberate split: ordinal-level/level-uniform gated (a completeness guard, faithful); depth-compatibility *not* gated (consulting-state).
