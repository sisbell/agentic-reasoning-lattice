...
/// MUST be `BuildHasher + Default + Clone` so `ContentStore`'s `Default`/`Clone`/`Deserialize`
/// derives hold. The *specific* hasher is Open decision #5; this alias is the one place it is named.
