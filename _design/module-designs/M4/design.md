...
- **SD store-disjointness** (`dom(C) ∩ dom(L) = ∅`, ASN-0093, from L0 + SC-NEQ + T7): a cross-store guarantee M4 cannot enforce alone. M4 upholds its half by storing only the content-subspace (`s_C`) addresses M5 routes to it, with M7 the disjoint `L` value store. The optional `NotContentAddress` assertion is M4's only local guard against a mis-routed `s_L` address ever landing in `dom(C)`.

## Dependencies & seams

**Upstream — concrete use:**

- **M1.** `Tumbler` (slice/record key — M4 relies only on its `Eq + Hash` for the `im::HashMap`, never `Ord`; see Conflict #3 for why ordering is deliberately unused), `Address` (the value `stage_write`/`write` accept — fresh from M3's `checked_inc`), `document_of` (derive the lock key for the standalone op), and — *only if* the optional content-address assertion is on — `subspace`/`classify`/`Level`. No span/span-set use: M4 stores scalar values, not spans.
  - **Upstream serde precondition (firm — a build-blocking M1 obligation, not an M4 hedge).** M4's slice (`ContentStore`) and record (`ContentWrite`) both `#[derive(Serialize, Deserialize)]`, which requires `Tumbler: Serialize + DeserializeOwned` — and transitively `Nat = BigUint: Serialize`, i.e. `num-bigint`'s `serde` feature enabled in the crate that owns `Tumbler`. M1 *as given* derives only `Clone, PartialEq, Eq, Hash` on `Tumbler`/`Address`; serde is absent, so M4's derives **do not compile against M1 today**, and M4 **cannot** repair it locally (orphan rule: both `serde::Serialize` and `Tumbler` are foreign to M4's crate). This is therefore a hard obligation on **M1 / the shared base crate that owns `Tumbler`**: derive `Serialize + DeserializeOwned` on `Tumbler` (M1's "the storage/journal key" designation implies it, but a prose designation is not a derive). Without it **no store can journal or checkpoint at all** — M4 merely surfaces the precondition first, being the thinnest store to hit it. Escalate it as a cross-module precondition, not an M4-internal choice.
- **M2.** Provides `apply`-driven commit + replay (engine wires `apply_write`), `Snapshot`/`world()` (readers obtain `&ContentStore`), `Kernel::transact` (standalone `write` only), `LockKey`, `Seq`. **M4 owns no journal, replay, snapshot, or recovery** — all M2's. The per-(document, content-subspace) `LockKey` must be byte-identical across M3 alloc / M4 write / M5 placement, so both the `key(document_of(addr), s_C)` *constructor* and the `s_C` constant live in the single shared base crate below the stores — the composition contract's home for "shared low-level types every store must name" (`skep-kernel`, or a shared base crate), alongside the central 1-byte space-tag enum the key draws its tag from. M4 imports `key(...)` and `s_C` from there; it does not re-spell `s_C = 1` or re-define `key(...)` locally, and neither do M3 or M5 — the three modules emit identical bytes precisely because there is exactly one source.
