...
| `idem: parking_lot::Mutex<LruCache<(SessionId, ReqId), Cached>>` | **hint** (best-effort *committed-write* retry memo, **keyed per-session** so a replayed `ReqId` from another session cannot read a prior session's commit coordinate/address — §7/item-4; purged on `close_session` — §6; `Cached` = op-kind-tagged ack essence, not the whole `Response`) | lost — a post-restart retry re-executes (duplicate, by design — ASN-0134 §A7) |
...
The `Durability` arm threads the underlying `io::Error`'s text into `Rejection.detail` via `with_detail`, so an operator sees the I/O cause behind the `Retry` hint. `MintError::Gate`'s inner `GateViolation` remains dropped — it is a fieldless struct (M1) carrying no message, so there is nothing to thread.
...
|---|---|---|
| **Permanent** | reissuing identically cannot succeed | `Malformed`, `MalformedSpan`, `BadRegion`, `BadPosition`, `BadCutCount`, `NotAscending`, `Empty*`, `NotOwner`, `NotAuthorized`, `SelfSupersession`, `NodeTierCrossOwner`, `DcViolation`, `Unauthenticated`, **`NotNextForm`**, **`NotFresh`**, **`NotRegistered`** (type) |
...
Two reclassifications keep the table honest against upstream invariants — both classed `Permanent`, where a "future committed state may satisfy it" is factually false:
