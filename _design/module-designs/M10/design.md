...
/// the site. `Operand`/`SpecFault` are M6's (named over the M10→M6 edge). M5's/M8's errors carry no
/// index/operand/region, so they fill only `code` (and, for M5, never `addr`).
