...
M7's raw reads (`ReadLink`/`FollowLink`) follow the same shape with **no** driver handle — read `snap.world().links().readlink(&a)` / `.followlink(&a, slot)` off the one snapshot, marshaling `Response::LinkValue` / `Response::Follow`. The two **namespace reads** (`NextAccountPrefix`/`PrincipalPrefix`, item-1) likewise read off the one snapshot — `snap.world().m3().next_account_prefix(&parent)` / `.principal_prefix(id)`, each returning `Option<Address>` (no fault path) — and marshal `Response::MaybeAddr { addr, as_of: snap.seq() }`. They hand a client the M3-internal frontier/registry values `Op::Delegate` and `Op::CreateNewDocument` demand but cannot otherwise compute: the next-form delegable prefix (M3's `NotNextForm` precondition, which a flat-`Permanent` disposition cannot recover from by retry — §5), and a principal's own account `Address`.
...
Three reclassifications keep the table honest against upstream invariants — all classed `Permanent`, where a "future committed state may satisfy it" was factually false:
...
- **`NotNextForm` (a `delegate` prefix not in next-form) is `Permanent`** — the prefix is wrong *for the frontier as it stands*, and M3's frontier is monotone, so the *identical* prefix never becomes next-form later. Recovery is, again, to fetch the demanded value via `NextAccountPrefix` and reissue with a *different* prefix. A flat `code → disposition` table classes all three as `Permanent`, with the delegate-side recovery (re-derive via `NextAccountPrefix`) owned by the caller.
