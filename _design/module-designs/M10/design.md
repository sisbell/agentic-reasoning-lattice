...
///
/// REQUIRED UPSTREAM INTERFACE AMENDMENT (Conflicts resolved #6): M3/M5/M7 MUST expose engine-facing
/// constructors for their transact-driving handles — `Namespace::new(Arc<Kernel<W>>)`,
/// `Vstream::new(&Kernel<W>)`, `LinkStore::new(&Kernel<W>, Arc<TypeRegistry>)` — which are ABSENT from
/// their dependent-facing interfaces and must be ADDED to their published interfaces. Under Rust's
/// crate model the binary is an external dependent of each store crate exactly as M10 is, with NO
/// construction privilege M10 lacks; without these `::new`s NO dependent can wire the write path, so
/// this is load-bearing for WHOLE-SYSTEM assembly, not merely for M10. The amendment is trivially
/// satisfiable — each constructor mirrors the handle's documented fields. Given them, the binary
/// (which holds the recovered kernel + M7's registry from M2/M7 recovery) builds `Stores`; M10 takes
/// it INJECTED for decoupling/testability (an in-memory-kernel-backed `Stores` exercises the whole
/// lifecycle with no disk/recovery), not because the constructors are unreachable.
    ...
    W::Record: From<M3Rec> + From<M5Rec> + From<LinkRec> + From<ContentWrite>,
{
    ...
    /// genesis-immutable `Arc<TypeRegistry>` via the store-driver `::new`s of the required interface
    /// amendment above). The binary calls Kernel::open (M2 recovery), handles OpenError::{Corruption,
    ...
    /// `idem_capacity` bounds the idempotency LRU (§7) — an explicit construction knob the binary
    /// sizes; there is no implicit default.
    pub fn new(stores: Box<dyn Stores<W>>, idem_capacity: usize) -> Self;

    // ── session binding (M10-owned, ephemeral) ──
    pub fn open_session(&self, principal: PrincipalId) -> SessionId;
    pub fn close_session(&self, s: SessionId);             // drops the binding AND purges the session's idem entries (§6/§7)
...
/// EditLink's successor is assembled by M10 from content V-specs (see Internal design §4).
/// from/to are content-resolved ONLY — a deliberate narrowing (§4): an address-denoting successor
/// is not constructible here; supersession of managed tuples goes via Emit + AssertSup.
...
pub enum Response {
    ...
    Follow    { result: Result<SpanSet, Invalid>, as_of: Seq },  // in-band by design: ⟨⟩ ≠ ⊥ is a defined FOLLOWLINK answer (§2)
...
/// Where in a multi-part request a fault landed — threaded from M6's variant-carried localization
...
/// the site. `Operand`/`SpecFault` are M6's (named over the M10→M6 edge). M6 is the ONLY producer of
/// a populated site: M5's and M8's error variants carry no localization at all (M8's
/// `DocNotRegistered` is fieldless, unlike M6's `DocNotRegistered(Address)`), so every M5/M8 variant
/// lowers with `site = None` — a `Lower` impl for them fills only `code`.
...
| `sessions: parking_lot::Mutex<HashMap<SessionId, PrincipalId>>` | **ephemeral authoritative** connection state (retired only by `close_session` — the transport calls it on connection drop, §6) | lost — clients re-authenticate |
| `next_session: AtomicU64` | **ephemeral** session-id counter | reset (ids unique within one uptime) |
| `idem: parking_lot::Mutex<LruCache<(SessionId, ReqId), Cached>>` | **hint** (best-effort *committed-write* retry memo, **keyed per-session** so a replayed `ReqId` from another session cannot read a prior session's commit coordinate/address — §7/item-4; purged on `close_session` — §6; capacity fixed at construction — `Operation::new`'s `idem_capacity`, §7; `Cached` = op-kind-tagged ack essence, not the whole `Response`) | lost — a post-restart retry re-executes (duplicate, by design — ASN-0134 §A7) |
| `poisoned: AtomicBool` | **hint** (recomputable by attempting `transact`) | re-derived on first `TxnError::Poisoned` |

The dispatch table is a `match`, not data. The `idem` cache stores a small op-kind-tagged `Cached` committed-write essence (§7), keyed on **`(SessionId, ReqId)`** so a replayed key from another session cannot read a prior session's commit, and never a whole `Response`, so M10 needs **no** (transitively heavy) `Response: Clone` bound. The `sessions`/`idem` state lives in a plain `HashMap`/`LruCache` behind a **`parking_lot::Mutex`** (or a sharded map) — non-poisoning, so a panic while the lock is held cannot poison it and break `execute`'s `Total` contract (§7) — and pointedly **not `im`**, because no field is ever snapshotted or replayed; the system-wide "persistent collection" discipline is for journaled slices, which M10 has none of. This is the one module that legitimately departs from the `im`-everywhere convention, and saying so is the point.
...
/// no `.expect()`, no non-local "the gate guaranteed it" reasoning. Misfiling protection is exactly
/// this strong and no stronger: a PRINCIPAL-CONSUMING write (Version, the M3 ops) misfiled into
/// `is_read` finds no principal in `dispatch_read` — a build-time prod toward the write path — but a
/// principal-FREE write (e.g. Insert) misfiled there compiles fine and surfaces only at runtime, as
/// the typed `Malformed` from `dispatch_read`'s defensive complementary-half arm. The universal
/// guarantee is the weaker one: a misfiled op yields a typed rejection, never a panic.
...
impl Op {
    ...
    /// don't), and it selects the dispatch function. EXHAUSTIVE match with NO `_` arm — a new `Op`
    /// variant fails to compile here (and at both dispatch functions) until classified, never
    /// silently defaulting into either half.
    fn is_read(&self) -> bool { match self {
        ...
        | Op::DiscoverableFrom {..} | Op::DeleteOrphans {..} | Op::InClaims {..} | Op::OutClaims {..} => true,
          Op::CreateNewDocument {..} | Op::Delegate {..} | Op::RegisterNode {..} | Op::Fork
        | Op::Insert {..} | Op::Delete {..} | Op::Copy {..} | Op::Rearrange {..} | Op::Version {..}
        | Op::MakeLink {..} | Op::Emit {..} | Op::Nullify {..} | Op::AssertSup {..} | Op::EditLink {..} => false,
    }}
...
Dispatch is split by the same partition the gate uses, so neither half carries authority it shouldn't: `dispatch_write(&self, wc: WriteCtx, op: Op) -> Result<Response, Rejection>` hosts the ⊕ rows (the four ownership-checked ops read `wc.principal` — a `PrincipalId`, never an `Option`), and `dispatch_read(&self, op: Op) -> Result<Response, Rejection>` hosts the read rows (no principal, no session). Each is the static table for its half; every arm is one of the four shapes below. Each is also *exhaustive over `Op`* **with no `_` wildcard**: the compiler-required arms for the complementary half (unreachable under the `is_write` partition that selected the function) are written as one **explicit `|`-list of the complementary variants**, collapsing into a single defensive arm returning a typed `Rejection` (`RejectCode::Malformed`), **never a panic** — so `execute`'s `Total` contract holds regardless of the partition's correctness, and there is **no fallthrough that turns an error into a success**. The explicit lists (rather than `_`) are load-bearing for maintenance: a newly added `Op` variant is a **compile-time non-exhaustiveness error at all three sites** — `is_read`'s exhaustive match and both dispatch functions — never a silent runtime `Malformed`. The never-silent invariant is enforced by this exhaustiveness over every *parsed* `Op`; a malformed *frame* never produces an `Op` and is the transport's to surface (§Public interface/Codec, Invariants).
...
| ⊕ `AssertSup{home,old,new}` | `linkstore().assert_sup(&home, &old, &new)` | `AckAddr` |
| ⊕ `EditLink{original,successor,d_s,d_a}` | assemble (§4) → `linkstore().editlink(&original, link, &d_s, &d_a)` | `AckEdit` |
| `ReadLink{a}` | `snap.world().links().readlink(&a)` † | `LinkValue` |
| `FollowLink{a,slot}` | `snap.world().links().followlink(&a, slot)` † (carries its own `Result` in the value) | `Follow` |
| `RetrieveV{specs}` | `Query::new(&snap).retrieve_v(&specs)` | `Delivery` |
| `RetrieveDocVSpan{doc}` | `Query::new(&snap).doc_vspan(&doc)` | `SpanSet` |
| `RetrieveDocVSpanSet{doc}` | `Query::new(&snap).doc_vspanset(&doc)` | `SpanSet` |
| `ShowOrigin{doc,span}` | `Query::new(&snap).show_origin_v(&doc, &span)` | `Addrs` |
| `ShowDeletions{d_a,d_b}` | `Query::new(&snap).show_deletions(&d_a, &d_b)` | `Deletions` |
| `Compare{rho1,rho2}` | `Query::new(&snap).compare(&rho1, &rho2)` | `Compare` |
| `FindDocsContaining{regions}` | `Query::new(&snap).find_docs_containing(&regions)` | `Addrs` |
| `Image{d,region}` | `image_on(&snap, &d, &region)` | `Runs` |
| `FindLinksV{d,region}` | `findlinks_v_on(&snap, &d, &region)` | `Addrs` |
| `FindLinksFtt{q}` | `findlinks_ftt_on(&snap, &q)` † | `Addrs` |
| `CountV{d,region}` | `count_v_on(&snap, &d, &region)` | `Count` |
| `CountFtt{q}` | `count_ftt_on(&snap, &q)` † | `Count` |
| `WindowV{d,region,cur,n}` | `window_v_on(&snap, &d, &region, cur, n)` | `Page` |
| `WindowFtt{q,cur,n}` | `window_ftt_on(&snap, &q, cur, n)` † | `Page` |
| `RetrieveEndsets{d,region}` | `retrieve_endsets_on(&snap, &d, &region)` | `Endsets` |
| `Project{a,slot,d}` | `project_on(&snap, &a, slot, &d)` | `SpanSet` |
| `DiscoverableFrom{a,d}` | `discoverable_from_on(&snap, &a, &d)` | `Bool` |
| `DeleteOrphans{d,p,width}` | `delete_orphans_on(&snap, &d, &p, &width)` | `Orphans` |
| `InClaims{y,view}` | `in_claims_on(&snap, &y, view)` † | `Claims` |
| `OutClaims{x,view}` | `out_claims_on(&snap, &x, view)` † | `Claims` |
...
M7's raw reads (`ReadLink`/`FollowLink`) follow the same shape with **no** driver handle — read `snap.world().links().readlink(&a)` / `.followlink(&a, slot)` off the one snapshot, marshaling `Response::LinkValue` / `Response::Follow`. **`Response::Follow` is the one response carrying a `Result` in-band, and the asymmetry is deliberate:** M7 defines ⟨⟩ ≠ ⊥ as two *answers* of FOLLOWLINK — `Ok(SpanSet::empty())` a valid-but-empty endset, `Err(Invalid)` link/slot absent — so `Invalid` is a defined query result, not a lifecycle failure, and lowering it to a `Rejection` would erase a distinction M7 calls unforgeable. Contrast `Project`, where M8's `NotALink` *is* a precondition failure and *is* lowered to a `Rejection`. A builder must not "normalize" the two in either direction. The two **namespace reads** (`NextAccountPrefix`/`PrincipalPrefix`, item-1) likewise read off the one snapshot — `snap.world().m3().next_account_prefix(&parent)` / `.principal_prefix(id)`, each returning `Option<Address>` (no fault path) — and marshal `Response::MaybeAddr { addr, as_of: snap.seq() }`. They hand a client the M3-internal frontier/registry values `Op::Delegate` and `Op::CreateNewDocument` demand but cannot otherwise compute: the next-form delegable prefix (the recovery path for M3's `NotNextForm` rejection — §5 classes that rejection `Permanent` precisely to steer clients here, to re-derivation, rather than to reissue-polling), and a principal's own account `Address`.

**`PrincipalPrefix` takes an explicit wire `id`, not the session's bound principal — and that is deliberate.** A principal's prefix is *public, immutable* registry data: any account `Address` is derivable by anyone holding the principal's `PrincipalId` (M3's `principal_prefix` is value-stable across snapshots), so an arbitrary-id lookup discloses nothing confidential and is independently useful. M3's seam frames the *typical* use as "resolve a *bound session's* prefix"; a client serves that case by passing **its own** `PrincipalId`, which the transport surfaces to it at session-open (the same connection→`PrincipalId` authentication result the transport already holds to call `open_session` — §6/downstream seam). The design keeps the id explicit rather than a session-relative no-id form because the read path is principal-free *by construction* (§1 step (b) hands a principal only to *write* arms), and arbitrary public-prefix lookup costs nothing extra. The common account-creation flow needs none of this — `Op::Delegate` returns the freshly-minted account `Address` directly.
...
**`SuccessorSpec.ty` is the one slot offering two forms.** `TypeArg::Addrs(a)` builds an *address-denoting* (managed-relation) type slot via `enc(a)`; `TypeArg::Resolve(v)` builds a *content-resolved* one via the same `endset_from_vspecs` path the from/to slots use — mirroring `makelink`, whose type slot is content `Vec<VSpec>` only. Both feed M7's type-slot gate *inside* `editlink`: M7 rejects an empty or non-level-uniform type slot (`IllFormedSuccessor`) and a retraction- or claim-schema-violating one (`DcViolation`). So M10's helper pre-checks only the *content well-formedness* of any `Resolve` VSpecs (the silent-clip guard below) and lets M7 own the slot-shape/schema verdict — M10 does not duplicate the type gate.

**`SuccessorSpec.from`/`.to` are content-resolved only (`Vec<VSpec>`) — a deliberate expressiveness narrowing, not a gap.** An *address-denoting* successor — e.g. a claim-schema `[K_sup]`-typed link, which M7's DC guard explicitly *admits* (it schema-conforms a claim-typed successor rather than rejecting it) — is not constructible from the FEBE surface: only the type slot offers an address form (`TypeArg::Addrs`), never the endpoints. This is intentional. M7's own seam frames M10's editlink job as forming a **content** successor (`resolve` + `Run::iextent` + `Endset::from_spans`/`enc` + `Link::new`), and supersession of *managed* tuples is reached through the managed surface instead: `Op::Emit` the successor tuple, then `Op::AssertSup { old, new }` — the same supersession claim `editlink` would have emitted. A builder must neither widen `SuccessorSpec` to admit address-denoting endpoints ad hoc nor read the narrowing as an accident.
...
/// The content-V well-formedness predicate makelink applies (M5/M7 reject ⟨⟩ otherwise): a depth-2,
/// content-subspace, ordinal-level V-span. Length checks PRECEDE every `get` — M1's `Tumbler::get`
/// PANICS on i > #t, and `execute` is contracted `Total`, so an out-of-range `get` would breach it.
fn is_content_vspan(span: &Span) -> bool {
    let start = span.start();
    let width = span.width();
    start.len() == 2 && width.len() == 2                     // depth-2 BEFORE any get(1)/get(2)
...
(The same `is_content_vspan` guard is the one a `WindowV`/`FindLinksV` region must satisfy upstream in M8; M8 applies it internally and returns `BadRegion`, so M10 forwards the region verbatim and lets M8 gate — only `editlink`'s helper needs the explicit M10-side check, because M5's `resolve` would otherwise swallow a malformed span silently.)
...
Every upstream failure is lowered to a `Rejection`; neither converter ever returns `Ok`. Each store error enum implements a mechanical `lower(self) -> (RejectCode, Option<FaultSite>)` (the `From`-equivalent, extended so the localized **M6** variants thread their fault site — span `index`/`fault`, COMPARE's `operand`/`region`, and the offending document `Address` of the multi-document `DocNotRegistered(Address)` variants — through `FaultSite`; M5's and M8's errors carry no localization fields at all — M8's `DocNotRegistered`, unlike M6's, is fieldless — so every M5/M8 variant gives `None`).

**Nested-error lowering rule (fully determining the ~15 impls).** Most store error enums are *flat* — each variant maps to the same-named `RejectCode` leaf (`EmitError::ShapeViolation → RejectCode::ShapeViolation`, `RearrangeError::NotAscending → RejectCode::NotAscending`, …). The *wrapper* variants recurse: a leaf enum (`MintError`, `SeatError`) gets its own flat, same-named lowering, and the wrapper delegates to it. So the **8 enums** carrying `Mint(MintError)` (Insert/Version/MakeLink/Emit/Nullify/AssertSup/EditLink errors and `OpError`) lower their `Mint` arm by `MintError`'s own impl — `MintError::{HomeNotRegistered, SourceNotRegistered, NotAnAccount} → RejectCode::{…}` and `MintError::Gate(GateViolation) → RejectCode::Gate` (the inner `GateViolation` is dropped); `MakeLinkError::Seat(SeatError)` lowers by `SeatError::{NotHomeLink, AlreadySeated} → RejectCode::{…}`; `OpError::NotOwner → RejectCode::NotOwner`, else recurses into `MintError`. Every leaf thus has exactly one `RejectCode`, so the impls are mechanical with no judgment left to the builder. The lone wrapper that does **not** preserve inner structure is `InsertError::Content(ContentError)` (next paragraph).

**The `ContentError` collapse is a flagged best-effort, pending M4's (out-of-scope) interface.** `InsertError::Content(ContentError)` lowers *wholesale* to a single `RejectCode::Content`, discarding whatever structure M4's `ContentError` carries; `disposition_of` then defaults `Content` to `Permanent`. M4's error surface is out of scope for M10 — the `M10 → M4` edge is type-only (Conflicts resolved #4) — so M10 cannot today inspect a `ContentError` to tell a *permanent* fault (e.g. a malformed write) from a *transient* one (e.g. a backing-store hiccup), and a transient `ContentError` is therefore **misclassified `Permanent`**. This is acceptable for v1 (M5's `insert` is the only path that surfaces it, and a content write under an already-validated M3 mint rarely fails transiently), but it is a real imprecision to revisit if/when M4's edge is ratified and its error enum is in scope — at which point `Content` would split into per-cause codes with their own dispositions (§ Open build decisions 8).

`map_txn` is a **`&self` method** so it can latch the poison flag on the spot; `map_read` stays a free function (a read can never poison the kernel):
...
impl<W> Operation<W> /* … */ {
    fn map_txn<E: Lower>(&self, op: OpKind, e: TxnError<E>) -> Rejection { match e {
        ...
        TxnError::Durability(io)  => Rejection::classified(op, RejectCode::Durability, None)
                                         .with_detail(io.to_string()),                              // operators see the I/O cause
...
impl Rejection {
    fn classified(op: OpKind, code: RejectCode, site: Option<FaultSite>) -> Rejection {
        // Gate is the one code carrying a FIXED M10 detail: it signals store corruption (an operator
        // condition), never client error — see the disposition table.
        let detail = matches!(code, RejectCode::Gate)
            .then(|| "inc-gate tripped: corrupted frontier — operator condition".to_string());
        Rejection { op, code, disposition: disposition_of(code), site, detail }   // disposition recomputed from the flat code
    }
    fn with_detail(mut self, d: String) -> Rejection { self.detail = Some(d); self }
}
```

The `Durability` arm threads the underlying `io::Error`'s text into `Rejection.detail` via `with_detail`, so an operator sees the I/O cause behind the `Retry` hint. `MintError::Gate`'s inner `GateViolation` remains dropped — it is a fieldless struct (M1) carrying no message, so there is nothing to thread; the corruption `detail` a `Gate` rejection carries is M10's own fixed string, attached in `classified`, not threaded from M1.
...
Three codes are named explicitly rather than left to the catch-all — all classed `Permanent`, the first two forced by upstream invariants (a "future committed state may satisfy it" is factually false), the third named for *what it means* rather than where it lands:
...
- **`NotFresh` is `Permanent`** — allocations are append-only and never freed (M3), so reissuing the *identical* request can never succeed. `register_node`'s `NotFresh` is unconditionally terminal; a `delegate` client recovers *not by reordering* but by re-deriving a **fresh** prefix via `NextAccountPrefix` (item-1; the read op that exposes M3's `next_account_prefix`) — a *different* request, not a hinted retry of this one.
- **`Gate` is `Permanent` — and it signals store corruption, not client error.** M3 documents `MintError::Gate` as *defensive*: it fires only when M1's TA5a inc-gate trips on a corrupted frontier, never on a well-formed request against a healthy store. Reissuing identically cannot succeed (a corrupt frontier never self-heals), so `Permanent` is the right hint — but the remedy is operational, not a request change, which is why `classified` attaches the fixed `detail` ("inc-gate tripped: corrupted frontier — operator condition"): the rejection must read as the store-health signal it is, not as a client fault.

**`NotNextForm` (a `delegate` prefix not in next-form) is *also* classed `Permanent` — but as a documented recovery-steering heuristic, not an invariant-forced fact.** It fires exactly when the prefix is *fresh* yet ≠ `next_in` (an already-allocated prefix yields `NotFresh` instead) — the guess sits *ahead* on the account chain — and M3's monotone frontier advance is precisely what can make `next_in` reach it: after intervening successful delegations under the same parent, the *identical* `delegate(delegator, p, new_id)` can succeed (freshness, ancestry, top-down, and authorization all still hold for a sibling-advanced frontier). Like `NotArranged`, the precondition is satisfiable only through other actors' intervening commits, so a `Reorder` hint inviting blind reissue-polling would be bad advice; `Permanent` steers the client to the right recovery — fetch the demanded value via `NextAccountPrefix` (item-1) and reissue with the *current* next-form prefix, a *different* request.

A flat `RejectCode → Disposition` table classifies by code alone and therefore **cannot see operation context**, so several codes that are *sometimes* future-satisfiable are conservatively classed **`Permanent`**: **`NotArranged`** (a later INSERT could arrange the position, making a re-DELETE succeed); **`OutOfBounds`** (a later INSERT extends `n_C`, bringing an identical re-DELETE/REARRANGE into range); **`EmptySource`** and **`EmptyContentSubspace`** (a later INSERT populates the source document or subspace); M6's **`RangeNotPresent`** and **`EmptySubspace`** (later placements bind the range or populate the subspace); **`fork`'s `NotOwner`** when it arises from an *unknown* session id (a later `delegate` could register that id) — indistinguishable, code-only, from `create_new_document`'s genuinely-permanent `NotOwner`; and likewise **`DelegatorUnknown`** (`delegate`) and **`NotAPrincipal`** (`version`), each satisfiable by a later `delegate` registering the missing principal id. This named list is **representative of the pattern, not exhaustive** (COPY's `EmptyResult` sits in the same boat); the named codes are the ones a builder would most plausibly mis-file as `Reorder`, so none falls through the catch-all silently. These err on the safe side: the disposition is an advisory hint and the `code` is authoritative, so a client that knows its own op-context may still reissue. The imprecision is **documented, not accidental** (§ Open build decisions 7).

This is the surfaced-typed-re-orderable contract (ASN-0134 rejection path): the canonical out-of-order retraction — `nullify` whose target isn't present — returns `Rejection { code: BadTarget, disposition: Reorder }`, so a coordination layer or client can reissue once the target exists. M10 **surfaces, it does not reorder** — buffering/reordering is policy that belongs above M10. Ambiguous codes (`DocNotRegistered` could be a typo *or* an out-of-order create) are classed optimistically as `Reorder`; the hint is advisory and the `code` lets the client decide. The two **conflated registration codes** are classed the same optimistic way, and named here so neither slips through silently: **`SourceNotRegistered`** (M3 folds *unregistered source* and *registered-non-document* into one code) and **`NotAnAccount`** (M3 folds *unregistered account* and *non-account-tier* into one code). Each has a future-satisfiable sub-case (merely-unregistered, which a later `delegate`/registration may admit) beside a permanent one (a genuinely wrong-tier address); a flat code-keyed table cannot separate them, so `disposition_of` returns `Reorder` for **both** — by parity with the policy above, at worst costing one doomed reissue the client may suppress. (`NotArranged`, `OutOfBounds`, the `EmptySource`/`EmptyContentSubspace`/`RangeNotPresent`/`EmptySubspace` state-dependent cases, `fork`'s unknown-id `NotOwner`, `DelegatorUnknown`, and `NotAPrincipal`, by contrast, the table sends the *other* way — conservatively `Permanent`; that the two buckets are split by a code-only guess is the heuristic refined in § Open build decisions 7.)

### 6. Session binding & authorization pass-through

M10 owns the *policy* "which principal does this connection speak for" (`sessions` map); the stores own the *mechanism* "is this principal the effective owner" — the `ω` check, **atomic inside M3's own ops** (`create_new_document`/`fork`/`delegate`) and, for M5's `version`, a **stability-justified snapshot pre-read** (ω of an existing `d_src` is stable, so M5 pre-reads it off a snapshot to pick the branch-dependent namespace lock — M5's seam as given). M10 passes `wc.principal` down (resolved at step (b), which has proven the session bound) and **never duplicates the `ω` check** — an M10-side duplicate would race against the state the owning store evaluates (atomically in M3; ω-stable pre-read in M5). The only pre-check M10 does is "is there a principal at all" (`Unauthenticated`), which avoids opening a doomed transaction.
...
`open_session(principal)` records the binding and returns a fresh `SessionId` minted from an atomic counter (`next_session.fetch_add(1, Ordering::Relaxed)`); `bootstrap_session()` does the same bound to `BOOTSTRAP_PRINCIPAL` so the first `delegate`/`create`/`register_node` can happen — and its confinement is transport policy, an explicit obligation beside the non-forgeability precondition: **the transport must not expose `bootstrap_session()` beyond provisioning** (it mints `BOOTSTRAP_PRINCIPAL` authority ungated; nothing in M10 restricts who calls it). `close_session` drops the binding and then purges every `(s, _)` entry from the `idem` cache — the two locks taken sequentially, never nested; a linear sweep of the bounded LRU — so the cache's cross-session confinement is enforced *locally*, not only by the transport's contract (a retired id's cached acks are gone, readable by no later holder of the id even if a transport ever violated the never-reissue rule), and LRU capacity held by dead sessions is reclaimed. **Calling `close_session` on connection drop is a transport obligation:** nothing else retires a binding, so a missed close leaks the `sessions` entry — and its idem entries, until LRU pressure evicts them — for the rest of the uptime. Session ids are unique within one M10 uptime (reset on restart — clients re-authenticate). The authentication *mechanism* that yields a `PrincipalId` for a connection is the transport's seam (not specified in the corpus); M10 records the result and trusts the §6 non-forgeability precondition above. The transport, holding that `PrincipalId`, may also surface it to the client at session-open, so a client can name its own account in `Op::PrincipalPrefix` (§2).
...
impl<W> Operation<W> /* … */ {
    fn idem_put(&self, s: SessionId, id: &ReqId, op: OpKind, resp: &Response) {   // call-site guards resp.is_committed_write()
    ...
    fn idem_get(&self, s: SessionId, id: &ReqId, op: OpKind) -> Option<Response> {   // miss on session OR op-kind mismatch
        let mut g = self.idem.lock();
        let c = g.get(&(s, id.clone()))?;                  // (SessionId, ReqId) key; a foreign session simply misses; bumps LRU recency
...
M10 has no journaled state, so it has nothing to recover. The binary calls `Kernel::open` (M2 replays the world), handles `OpenError::{Corruption, BadCheckpoint}` (operator intervention, not auto-retry), builds the store-driver handles via their engine-facing `::new`s (the Conflicts-resolved #6 required interface amendment), and assembles the `Stores` factory *before* constructing `Operation` (sizing the idempotency LRU via `idem_capacity` at the same time). At runtime, the first `TxnError::Poisoned` latches `self.poisoned` (relaxed) inside `map_txn` (§5); thereafter writes fail fast with `Disposition::Halt` at step (c) while **reads keep being served** (M2 snapshots survive a poisoned kernel) — a clean degraded mode until an operator restarts.
...
self.stores.kernel().transact(&[M3State::document_lock_key(acct)], |stg| {
    ...
    /* … then M5's pure content-place step over stg.working() (mint_content + M4 write + splice).
       NO arrangement-registration step exists or is wanted: a fresh d's arrangement stays
       implicit/lazy until first placement (the settled M3/M5 eager-lazy split). … */  Ok(d)
})
```
...
- **Session-principal binding & write authorization gate.** A write on an unbound session is rejected `Unauthenticated` before any transaction — *enforced in `execute` step (b)*, which resolves the bound principal once and hands it to the write arm as `wc.principal` (no arm unwraps an `Option`); fine-grained `ω` is the owning store's check (atomic in M3's ops; a stability-justified snapshot pre-read in M5's `version`), passed through verbatim. The binding M10 trusts to name the caller is sound only under §6's **SessionId non-forgeability precondition** (the transport injects each id, never the wire). `register_node` ignores its principal but still requires a bound (e.g. bootstrap) session — no exemption (§6).
- **Best-effort exactly-once for retried writes (M3 boundary; ASN-0134 §A7).** The idempotency cache short-circuits a repeated `ReqId`, **keyed `(SessionId, ReqId)`** so a replay under another session cannot read a prior commit's coordinate/address (sound under §6's non-forgeability precondition, and enforced locally by `close_session`'s purge of the session's entries — §6), and **caching only committed-write acks** (`Ack`/`AckAddr`/`AckEdit`, as a small op-kind-tagged `Cached` essence bounded at the construction-time `idem_capacity`; a `ReqId` reused across op-kinds misses and re-executes, a same-kind/different-args reuse serves the original ack — a client-contract breach the cache cannot detect, §7 — and *concurrent* same-session-same-`ReqId` requests are not serialized — sequential retries only) — rejections and reads are never memoized, so a `Reorder`/`Retry` reissue re-executes — *enforced in `execute` steps (a)/(d)*.
...
The contract neighbors (the transport in the binary) build against is `Operation<W>::execute(SessionId, Request) -> Response` plus a `Codec`. Guarantees the client may rely on: commit-before-acknowledge; `committed_at`/`as_of` on every response; typed `Rejection` with an advisory `disposition` and a structured fault `site` (including the offending document `Address` for the multi-document `DocNotRegistered` cases); per-session idempotency-key honoring within M10's uptime (sequential retries, op-kind-matched); session→principal binding; the two namespace-structure reads (`NextAccountPrefix`/`PrincipalPrefix`) needed to obtain the prefixes `delegate`/`create_new_document` demand. The transport supplies: a concrete `Codec`, the connection→`PrincipalId` authentication (and surfaces that `PrincipalId` to the client at session-open, so a client can name its own account in `PrincipalPrefix` — §2), the **per-connection `SessionId` injection** (drawn from the connection binding, never read off the wire — the non-forgeability precondition §6 rests on), the **`close_session` call on connection drop/teardown** (binding retirement is transport-driven — §6; a missed close leaks the binding, and its idem entries up to LRU pressure, for the uptime), the **confinement of `bootstrap_session()` to provisioning** (§6), the concurrency policy (sequential vs pipelined), the **request↔response correlation** (M10 returns a bare `Response` carrying no `ReqId`, and the marshaled frame carries no correlation id — the transport's envelope pairs each reply with the in-flight `Request`'s `id` it still holds — §8), the parse-failure `Response::Rejected` (stamped `OpKind::Unparseable`), and — at startup — the `Stores` factory passed to `Operation::new` (built via the store-driver `::new`s of the Conflicts-resolved #6 required interface amendment) plus the `idem_capacity` sizing. **M10 ⟂ M9:** M9's rule fires reach M7's gated write path directly; M10 never sees them except as committed state in later snapshots — no edge, no shared lifecycle.
...
6. **Store-driver constructor gap (write-path buildability) — a required upstream interface amendment.** *Resolved: an explicit amendment to M3/M5/M7's published interfaces plus an injected `Stores` factory, justified by decoupling/testability — not by any construction privilege M10 lacks.* M3/M5/M7's dependent-facing interfaces publish their *read* constructors (`Query::new`, `LinkQuery::new`/`*_on`, `Link::new`, `Endset::from_spans`, `Run::new`, `TypeRegistry::build`) but say nothing about how their **transact-driving** handles (`Namespace`/`Vstream`/`LinkStore`) are constructed. Under Rust's crate model the binary (`binary → skep-engine → store crates`) is an *external dependent* of each store crate exactly as `skep-operation` (M10) is — it has **no** construction privilege M10 lacks — so the resolution cannot rest on "the binary can construct them, M10 cannot." **Flag this as a required amendment to M3/M5/M7's *published* interfaces:** they MUST expose engine-facing constructors — `Namespace::new(Arc<Kernel<W>>)`, `Vstream::new(&Kernel<W>)`, `LinkStore::new(&Kernel<W>, Arc<TypeRegistry>)` — absent from their current dependent-facing interfaces. The amendment is trivially satisfiable (each constructor mirrors the handle's documented fields: `Namespace` holds an `Arc<Kernel<W>>`, `Vstream`/`LinkStore` borrow a `&Kernel<W>`, `LinkStore` additionally caches the genesis `Arc<TypeRegistry>`) but **load-bearing for whole-system assembly, not merely for M10** — without these `::new`s *no* dependent (binary or M10) can build a `Stores` impl and the write path is unbuildable. Given those constructors, M10 *could* call them directly; it nonetheless takes a `Box<dyn Stores<W>>` injected at `Operation::new` for **decoupling and testability** — M10 holds neither the recovered `Arc<Kernel<W>>` nor M7's genesis-immutable `Arc<TypeRegistry>` (both produced by the binary during M2/M7 recovery), so it receives a `Stores` that bundles them and vends the drivers, and an in-memory-kernel-backed `Stores` exercises M10's whole lifecycle with no disk/recovery. M10 names only its own `Stores` trait, the published handle *types*, and `stores.{kernel,namespace,vstream,linkstore}()`. (If M3/M5/M7 surface those constructors as ordinary `pub` items, M10 could drop the factory and construct directly.)
...
7. **Disposition refinement** — the `Reorder`/`Permanent` split for context-ambiguous codes is a heuristic hint; tune it (or expose both the raw code and let the client decide entirely) under real client traffic. A flat code-keyed table cannot see op-context, so for the *sometimes*-satisfiable codes it splits two ways by guess: classed optimistically **`Reorder`** (`DocNotRegistered`, and the conflated registration codes `SourceNotRegistered`/`NotAnAccount`) vs conservatively **`Permanent`** (`NotArranged`, `OutOfBounds`, `EmptySource`/`EmptyContentSubspace`, M6's `RangeNotPresent`/`EmptySubspace`, `fork`'s unknown-id `NotOwner`, `DelegatorUnknown`, `NotAPrincipal`, and the recovery-steering `NotNextForm` — a representative list, §5); refine if the hint proves too coarse. (Of §5's explicitly-named codes, `NotFresh`/`NotRegistered` are *not* heuristic — forced by upstream append-only / registry-immutable invariants — and `Gate` is non-heuristic too, named for its operator-condition meaning (store corruption) rather than its bucket. `NotNextForm` is heuristic after all — the identical prefix *can* become next-form after intervening same-parent delegations — and is kept `Permanent` as recovery-steering toward `NextAccountPrefix` re-derivation, item-1.)
8. **`ContentError` granularity** — `InsertError::Content(ContentError)` is lowered *wholesale* to `RejectCode::Content` (→ `Permanent`), discarding M4's error structure, because the `M10 → M4` edge is type-only and M4's error surface is out of scope (Conflicts resolved #4/#5). A *transient* content fault is thereby misclassified `Permanent` (§5). Revisit when/if M4's edge is ratified and its `ContentError` enters scope: split `Content` into per-cause codes with their own dispositions — the `lower`/disposition machinery ports unchanged, exactly as for the #6 `RejectCode` compaction.
