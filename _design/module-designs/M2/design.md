...
impl<W: WorldState> Kernel<W> {
    ...
    /// op whose `base` is the not-yet-durable working tip first WAITS to be resolved at its
    /// base batch's serialized resolution (§3) — woken once that base is durable+installed —
    ...
    /// in-flight batched incumbent — §1/§3). If that base is instead ROLLED BACK by a barrier
    /// failure, the op is woken to retry and returns `TxnError::Conflict` (caller re-invokes),
    /// NEVER its provisional value (§3). Under per-commit fsync `base` is already durable, so
    /// no wait.
    ///
...
|---|---|---|---|
| **Journal** | append-only file(s) of magic-anchored, CRC-framed records + per-txn commit markers (+ a durable poison sentinel written only in §1's failed-truncation corner) | **authoritative, durable** | record-of-record; replay reconstructs everything; append-only makes immutability of committed steps free (ASN-0047 P0/P1/P2 at the log level); the per-frame magic word anchors recovery resync (§1/§7); the poison sentinel bounds the trustworthy prefix when a failed-barrier truncation cannot itself complete (§1/§7) |
...
| **Working tip** | `Committed<W>` held by the applier across a group-commit batch | volatile (internal to the applier; never externally visible; rebuilt fresh on recovery) | lets a batched txn read its predecessors' not-yet-published records (correct dependent reads); the leader captures it at batch close and the batch's final state reaches **Root** via the leader's serialized, **monotonic** install *after* the barrier (or a later batch's superseding install) — never a fresh post-barrier `tip.load()` (§3) — **single-applier `durable_before_visible:true` only; per-key/CAS install before the barrier and need no tip (§8)** |
...
**Group commit (the throughput realization of the canonical order).** `FsyncBatch{window, max_delay}` amortizes the barrier across a batch, closing it on whichever of *window-fill* or *max_delay* fires first — a window-only trigger would stall a low-traffic batch (and any zero-step op waiting on its barrier, §3) indefinitely, so the timeout is mandatory; `flush()` forces an immediate close. Because install must stay *after* durability, no `ArcSwap` root update happens until the batch's barrier — yet a batched txn must still read the state its predecessor left: a COPY pipelined after an INSERT into the same document must see the inserted content, and an allocation must read the frontier its predecessor advanced (H0). The applier therefore threads an **in-memory working tip** — a `Committed<W>` it advances with each batched txn's staged records — and draws each batched txn's `base` from that tip rather than from `root.load()`. The working tip is internal to the applier and never externally visible. **The full leader-serialized commit protocol — batch-final capture, the single unlocked fsync, the re-serialized *monotonic* install, the *post-install* watermark raise, the waiter-wake, and the zero-step watermark wait — is the single source of truth in §3;** the barrier-failure handling (truncate + Seq-rollback, or poison + `Poison` sentinel) is exactly the canonical-ordering bullets above, performed under §3's re-acquired serialization point. (The dependent-*write* hazard — a batched dependent txn reading a stale pre-batch root yet landing at a higher `Seq` — is specific to *this* durable-before-visible default combined with group commit, and the working tip is its fix; the zero-step watermark wait is its read-side companion.)
...
base ← tip.load()                              // working tip: == root for per-commit fsync or a batch's
                                               //   FIRST member; == predecessors' fold within a batch (§1)
...
match f(&mut stg) {                            // store logic; f MUST NOT call transact/critical_section (#NR — deadlock)
    ...
    Ok(v) if stg empty →                       // zero-step (A1); V1 referent = base.seq()
        if base is a not-yet-durable working tip (durable-before-visible group commit):
            enroll in base's batch waiter set keyed on base IDENTITY     // so a burned-then-reused Seq can't
            release applier lock; park until that batch is RESOLVED       //   false-wake it; resolved by the
              at its serialized resolution (protocol step 5/6 below):     //   LEADER, deterministically:
    ...
    Ok(v) ⇒
        ...
        tip.advance(Committed{ last_seq, stg.working })           // PER-TXN — advance the working tip   (proto step 1)
        ...
        // FsyncBatch — append + advance done above (step 1); this txn now follows the leader-serialized
        //   group-commit protocol (steps 2–6 below). A non-leader parks and returns when the leader resolves
        //   the batch; the leader captures batch_final under the lock (2), fsyncs UNLOCKED (3), re-acquires the
        //   serialization point (4), installs MONOTONICALLY → raises the watermark → wakes waiters (5),
        //   or truncates/rolls back/poisons (6).
}
```

**Group-commit commit protocol (durable-before-visible) — the single source of truth for the ordering.** Under `FsyncBatch`, the per-txn `barrier`+`install` shown above for per-commit fsync is replaced by this leader-serialized batch protocol; every other section refers *here* for the ordering:

1. **Append + advance, under the applier lock.** Each batched txn, in turn under the applier lock, appends its record+marker frames and advances the working tip. A **non-leader** then enrolls in the batch's waiter set, releases the lock, and parks (step 5/6 wakes it).
2. **Capture, under the lock.** The txn that closes the batch (window-fill / `max_delay`) is the **leader**; *still holding the applier lock* it captures `batch_final ← tip.load()` (seq == its own `last_seq` == the batch `last_seq`), then releases the lock.
3. **Barrier, unlocked — the only unlocked step.** The leader issues one `durability.barrier()` covering every batched txn's frames. Nothing else runs unlocked.
4. **Re-acquire the serialization point.** After the fsync the leader re-acquires the applier lock (the §Core-data-model sequencer's *install* point) — so install, watermark, waiter-wake, and the barrier-failure handling are all serialized; installs and failure handling from different batches can neither run concurrently nor reorder, nor race the appenders that advance the tip/file.
5. **On barrier success: install → raise watermark → wake.** Install `batch_final` **iff `batch_final.seq > root.seq()`** (**monotonic** — a later batch that resolved first must not be regressed; this is also what keeps `current_seq()` monotone, §5); *then* raise the durable watermark to the batch `last_seq`; *then* wake the batch's members (→ `Ok`) and its zero-step waiters (→ `confirmed`). Because the watermark is raised *after* install (both under this serialized point), any woken waiter's effect is already in `root` — the read-your-writes A7 requires, which a pre-install watermark would violate.
6. **On barrier failure: truncate/rollback/wake, under the lock.** If `journal.truncate` of the batch's un-acked frames completes durably: roll the `Seq` high-water back to the last committed marker, reset the tip to the last durable state, and wake the batch's members (→ `Durability`, a true no-op) and its zero-step waiters (→ `Conflict`, never acking a value off the erased state). If that truncation itself fails to complete durably: durably write `Poison{ trusted_through = last committed last_seq }`, poison the kernel, wake all (→ `Poisoned`) — the surviving failed marker plus reused burned Seqs would otherwise corrupt (§1/§7). Release the lock.

The leader returns only after step 5/6 — i.e. post-install — commit-before-acknowledge (A7); non-leaders and zero-step waiters return when the leader wakes them at step 5/6, deterministically `confirmed` vs `Conflict` (the wake-on-`base`-identity set is resolved at this one serialized point, not by racing the watermark). Only step 3 runs unlocked, so the leader's fsync proceeds while later txns append under the lock, yet the install that publishes its result — and any rollback that erases it — is re-serialized.
...
**Lock discipline (group commit).** Under per-commit `Fsync` the barrier and install sit *inside* the applier-locked region (single applier, single core — nothing to gain by releasing). Under `FsyncBatch`, **only the leader's fsync (protocol step 3) runs unlocked**; the applier lock is released after a txn's appends + `tip.advance` (the leader having first captured `batch_final` under the lock, step 2), and the leader **re-acquires** the serialization point (step 4) to perform install + watermark-raise + waiter-wake (step 5) or the truncate/rollback/reset + waiter-wake (step 6). This is what lets (a) the leader's fsync proceed while later txns append under the lock, while (b) keeping every install/rollback serialized through the one point the Sequencer row promises — so two leaders can never install out of order, and a failed batch's truncate/tip-reset never races concurrent appenders. The durable watermark is the post-install completion signal: a monotone "max-`Seq`-known-durable-and-installed" the leader raises *after* install, with waiting `transact`s and zero-step ops parked on a condvar until it covers their `last_seq`/`base.seq()` — or, on a batch rollback, woken to *retry* (zero-step ops return `Conflict`, §1). Because the lock is dropped *before* any waiter parks and *before* the unlocked fsync, a waiter can never block the very barrier it awaits — no deadlock.
...
The barrier shown is the canonical per-txn durable-before-visible commit; group commit amortizes it across a batch and publishes the batch's captured final state *after* its barrier, via the leader's re-serialized monotonic install (steps 4–5). Either way **install follows durability**, so `TxnError::Durability` is a sound true no-op — *provided the un-acked-tail truncation completes durably; if it does not, the kernel poisons (returning `Poisoned`, and writing the `Poison` sentinel for recovery) rather than no-ops* (§1). The optional visible-before-durable variant (§1) — and, necessarily, the per-key-concurrent and CAS realizations (§8) — move the install above the barrier and convert a barrier failure into a kernel poison (`Poisoned`).
...
- **at or after the committed tip** (a CRC/marker failure with no valid marker following the corrupt run): the never-acked **torn tail** → discard + truncate. It was never acked, so loss is correct, and in the **normal** failure path a barrier-failed txn's record+marker frames were already truncated at failure time, so no failed marker survives to be misread as the tip.
...
- **Commit-before-acknowledge** — `transact` returns only post-durable+install; under group commit a non-leader is woken by the leader's serialized resolution *after* the install (§3 step 5), so a returning write's effect is already in `root`. A zero-step op reading off a not-yet-durable working tip likewise waits to be resolved at its base batch's serialized resolution before returning, and on a barrier-failure rollback of that base is woken to **retry** (returns `Conflict`), never acking a value off erased state §1/§3 (A7, MIC-3).
- **Composite atomicity (none-or-all to readers)** — single atomic install §3 + commit-marker recovery discard §7 (ASN-0047).
- **Durability before ack, with a sound failure path** — canonical durable-before-visible (`SingleApplier { durable_before_visible: true }`): append records+marker → barrier → install → raise watermark → return; under group commit the leader captures `batch_final` under the lock, fsyncs **unlocked**, then re-acquires the serialization point to install **monotonically** (no root regression / `current_seq()` backslide) and raise the watermark before waking waiters (§3). A barrier failure truncates the txn's un-acked frames and rolls the `Seq` high-water back, so `TxnError::Durability` is a **true no-op** and no failed marker can be promoted by a later fsync — *but only if that truncation itself completes durably; if it does not, the kernel **poisons** (the poisoning call returning `TxnError::Poisoned`, not `Durability`) **and durably writes a `Poison{ trusted_through }` sentinel so the next `open()` refuses the surviving failed marker's tail (`OpenError::PoisonedTail`) instead of replaying it as a phantom commit**_, since a surviving failed marker plus the reused burned Seqs would corrupt §1/§3/§7 (ASN-0047). The visible-before-durable ordering — the optional `durable_before_visible: false` single-applier variant **and the necessary ordering of the per-key-concurrent and CAS realizations (§8)** — instead **poisons** on barrier failure (returning `Poisoned`) and exposes a not-yet-durable state to an external M10-acked read (resolved conflict — see *Conflicts resolved*).
- **Recovery faithfulness** — load the latest valid *retained* checkpoint (falling back to an older retained one or genesis on `BadCheckpoint`, §6/§7); resynchronize past interior bad frames via the per-frame `magic` word to locate the **committed tip** (§1/§7); replay committed records **filtered to `Seq > S_load`** (skipping any `≤ S_load` frames surviving §6's segment-granular reclamation) in `Seq` order **without requiring Seq-contiguity** (a `TolerateGap` gap folds harmlessly); a CRC/marker failure *at or after* the committed tip is the never-acked torn tail (discard + truncate — in the normal path a failed-barrier txn already truncated its frames, so no marker survives); one in the genuinely-replayed range **`(S_load, committed tip]`** is corruption of durable acked data (halt, `OpenError::Corruption` — never drop); one at `Seq ≤ S_load` is **harmless** (the `Seq > S_load` filter skips it; the checkpoint already embodies it — never fatal). The **poison corner** (§1: a failed-barrier truncation that itself failed) is the sole case where a never-acked marker DOES survive on disk, byte-indistinguishable from a committed one; recovery handles it not by re-deriving an ack-status the bare marker lacks but by honoring the durable `Poison{ trusted_through }` sentinel §1 wrote — reconstructing through `trusted_through` and **refusing the tail** (`OpenError::PoisonedTail`) §6/§7 (A6; ASN-0047).
