...
impl<W: WorldState> Kernel<W> {
    ...
    /// Seq no reader can observe). A verdict reported "as of `current_seq()`" is thus a
    /// retrospective statement about a real committed state (V1). Infallible, including when poisoned.
...
impl<W: WorldState> Kernel<W> {
    ...
    /// evaluated against (V1). UNDER DURABLE-BEFORE-VISIBLE GROUP COMMIT a zero-step op whose
    /// `base` is the not-yet-durable working tip first WAITS on the durable watermark (§3) to
    /// cover `base.seq()` before returning: it journals nothing, but must not acknowledge a
    /// value read off a state a batch rollback could erase (e.g. an idem-hit deduplicated
    /// against a still in-flight batched incumbent — §1/§3). If that base is instead ROLLED
    /// BACK by a barrier failure, the op is woken to retry and returns `TxnError::Conflict`
    /// (caller re-invokes), NEVER its provisional value (§3). Under per-commit fsync `base` is
...
pub enum TxnError<E> {
    ...
    Conflict,               // the caller must re-make the staged decision with a FRESH closure (f is FnOnce, so
                            ...
                            //       whose `base` was ROLLED BACK by a barrier failure before its watermark covered
                            //       it — the provisional value was decided against a now-erased state and acking it
                            //       would violate V1 / clause 7, so the op is woken to retry (§3).
...
| **Sequencer** | `Seq` high-water + the install serialization point (a `Mutex` or actor mailbox) | volatile, recovered as `max committed Seq` | assigns the total order; serializes journal-append + install (`current_seq()` reports the installed *root*'s seq, not this in-flight high-water — §5) |
...
**Group commit (the throughput realization of the canonical order).** `FsyncBatch{window, max_delay}` amortizes the barrier across a batch, closing it on whichever of *window-fill* or *max_delay* fires first — a window-only trigger would stall a low-traffic batch (and any zero-step op waiting on its barrier, below) indefinitely, so the timeout is mandatory; `flush()` forces an immediate close. Because install must stay *after* durability, no `ArcSwap` root update happens until the batch's barrier — yet a batched txn must still read the state its predecessor left: a COPY pipelined after an INSERT into the same document must see the inserted content, and an allocation must read the frontier its predecessor advanced (H0). The applier therefore threads an **in-memory working tip** — a `Committed<W>` it advances with each batched txn's staged records — and draws each batched txn's `base` from that tip rather than from `root.load()`. So: under the serializer, for each batched txn append its record+marker frames and advance the tip (releasing the applier lock right after — see §3); then *one* durability barrier; the leader's fsync then raises the **durable watermark** to the batch's `last_seq`; then publish the batch's **final committed state** — the `Committed` the leader **captured at batch close** (seq == the batch's `last_seq`, the point the barrier covered), *not* a fresh post-barrier `tip.load()`, which the next batch's concurrent appends may already have advanced *past* the watermark — to the root in a single atomic install performed by the batch **leader**; each `transact` returns only after the watermark covers *its* `last_seq`. A barrier failure aborts the batch's un-acked tail (truncate + Seq rollback, **or poison — returning `Poisoned`, and writing the `Poison` sentinel — if that truncation fails**, see above), resets the tip to the last durable state, and **wakes any zero-step op parked on a now-burned base to *retry* rather than ack a rolled-back value (§3)**, so no partial marker survives and the root never reflected the aborted work. Install stays *after* durability — external readers never see pre-durable state; the working tip is internal to the applier and never externally visible. A **zero-step op** (idem-hit, nullify-hit, or read) that decides off the working tip is the one path that journals nothing yet still acknowledges a value read from the tip: it must therefore *also* wait on the durable watermark to cover its `base.seq()` before returning, because acking an idem-hit deduplicated against a still-in-flight batched incumbent — one a barrier failure would roll back — is exactly the not-yet-durable external-read window the canonical ordering forbids (§3); reading the durable *root* instead is no fix (it would miss the batched predecessor and double-deposit, breaking clause 7), so the zero-step op reads off the tip but waits for its durability — and **if that batch instead rolls back on a barrier failure, the parked op is woken to *retry*: it returns `TxnError::Conflict` for the caller to re-invoke with a fresh closure, never returning its provisional value against the erased state. The wait keys on `base` *identity*, not merely `watermark ≥ base.seq()`, so a burned-then-reused `Seq` cannot wake it with a false positive (§3).** Under per-commit fsync `base` is the durable root, so no such wait arises. (The dependent-*write* bug — a batched dependent txn reading a stale pre-batch root yet landing at a higher `Seq` — is specific to *this* canonical default combined with group commit, and the working tip is its fix; the zero-step wait is its read-side companion.)
...
match f(&mut stg) {                            // store logic: reads stg.base()/working(), pushes records.
                                               //   f MUST NOT call transact/critical_section — the applier
                                               //   lock is held, so a nested kernel write DEADLOCKS (#NR);
                                               //   a composite composes neighbors' PURE math (M3's
                                               //   recompute_max/inc against stg.base()), never a nested txn.
    ...
    Ok(v) if stg empty  →                              // zero-step (A1); V1 referent = base.seq()
        ...
        if base is the not-yet-durable working tip:    //   (durable-before-visible group commit only)
            park on the durable watermark, keyed on base IDENTITY (NOT merely watermark ≥ base.seq(),
                so a burned-then-reused Seq cannot false-wake it):
              base CONFIRMED durable                    → fall through → return Ok((v, base.seq()))
              base ROLLED BACK (its batch aborted)      → return Conflict   // provisional value decided
                                                        //   against erased state; f is FnOnce ⇒ caller
                                                        //   re-invokes with a fresh closure (§1)
    ...
    Ok(v) ⇒
        ...
        // GROUP COMMIT: if THIS txn closes the batch (window-fill / max_delay) it is the LEADER and,
        //   STILL UNDER THE LOCK, captures  batch_final ← tip.load()  (seq == this txn's last_seq ==
        //   the batch's last_seq) to install post-barrier — it must NOT re-tip.load() after the
        //   unlocked barrier, where the NEXT batch may have advanced the tip past last_seq (§1).
        ...
        durability.barrier()                            // PER-COMMIT fsync: per-txn, lock still held.
            ...
            on Err →                                    // sound failure path (§1):
                if journal.truncate(seqs) durably succeeds:
                    rollback_seq_hi; tip.reset_to_last_durable
                    wake rolled-back zero-step waiters → Conflict   // never ack a value off erased state (§1)
                    return Durability                              // TRUE no-op
                else:                                              // truncation failed → failed marker survives
                    durably write Poison{ trusted_through = last committed last_seq }   // bounds the prefix (§1/§7)
                    poison_kernel(); wake all waiters → retry; return Poisoned          //   reused Seqs ⇒ corruption; HALT

        // ATOMIC INSTALL — AFTER durability.
        //   PER-COMMIT fsync:  per-txn, lock still held → root.store(Committed{ last_seq, stg.working }).
        //   GROUP COMMIT:      LEADER-ONLY, UNLOCKED   → root.store(batch_final), the value captured at
        //                      close (seq == last_seq == watermark), so root.seq() never exceeds the
        //                      durable watermark (§1). Non-leader batched txns do NOT install; they wait.
        root.store( <per-commit: Committed{ last_seq, stg.working }> | <group-commit leader: batch_final> )
...
**Lock discipline (group commit).** The pseudocode's `durability.barrier()` and `root.store()` sit *inside* the applier-locked region only under per-commit `Fsync` (single applier, single core — there is no concurrency to lose by holding the lock through the per-txn barrier). Under `FsyncBatch`, the applier lock is released immediately after a txn's appends + `tip.advance` (the leader having first captured `batch_final` under the lock) — the shared barrier and the leader's single install run **unlocked**. This is what lets (a) the leader's fsync proceed while later txns append under the lock, and (b) a zero-step op block on `base.seq()`'s durability without holding the applier lock. Both the leader's post-fsync signal and the waiters use one **durable watermark**: a monotone "max-`Seq`-known-durable" the leader raises after a successful barrier, with waiting `transact`s and zero-step ops parked on a condvar until it covers their `last_seq`/`base.seq()` — or, on a batch rollback, woken to *retry* (zero-step ops return `Conflict`, §1). Because the lock is dropped *before* any wait on the watermark, a waiter can never block the very barrier it awaits — no deadlock.
...
- **No torn read / per-call single-state** — immutable `Committed<W>` + atomic `ArcSwap` install + lock-free load (A0/A3/A4/V0, MIC-4). Under group commit, external readers see only the post-barrier root, never the applier's working tip and never a tip past the watermark — the leader installs the state it **captured at batch close**, so `root.seq() == watermark` (§1/§3). `current_seq()` reports that installed root's seq, not the in-flight high-water (§5).
