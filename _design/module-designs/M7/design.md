...
impl LinkState {
    ...
    pub fn targets_keyed(&self, source: &Address) -> im::HashMap<CoverageClass, Address>;  // BH3: target_of joined over all BH3 Binary types; key via the public `coverage_class(ty)` (§Core data model)
...
/// key, the DC guard, and the walk-class checks.
...
editlink(original, successor, d_s, d_a):                      // successor : Link, supplied (M10 built it via Link::new)
  ...
  transact(keys, |stg| {                                      // two home alloc keys only — no dedup key (claim key carries fresh a')
     ...
     let b  = emit_core(stg, d_a,                             // claim: "original superseded by a'"
                        Link::new([enc(&[original]), enc(&[a']), reserved(Supersedes).clone()]).unwrap(),
...
- **`match_links(constraints, view)`** — findlinks' core: per constrained slot, OR (`stab`-union, which already unions overlap across the constraint endset's spans) over the slot's query spans, then AND (intersect by link identity) across slots. M8's findlinks = this + cursor/count/window. (Each slot's `stab` excludes `Adjacent`, so findlinks never returns an abutment-only false positive.)
...
- **Observe is exact membership** (ASN-0086): `observe`/`is_k`, and the BH3 reverse family, test `⊆ coverage(·)` / exact denotation via `denotes`/`addrs`/prefix, using `stab` overlap only as a prefilter — never as the match; an empty pattern is no constraint and its prefilter is skipped.
