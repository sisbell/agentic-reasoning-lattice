...
where W: WorldState + HasLinks + HasM3, W::Record: From<LinkRec> + From<M3Rec>
{
    ...
    /// (emitter, Seq) Vec (a target already nullified by a prior *same-`d_retr`* retraction dedups to a
    /// hit + base Seq; one nullified by a different `d_retr` deposits a redundant but harmless `[R]`
    /// tuple — the dedup key carries `d_retr`); on first TxnError returns Err, leaving earlier nullifies
    /// committed and durable — a re-run with the same `d_retr` is safe (§7).
