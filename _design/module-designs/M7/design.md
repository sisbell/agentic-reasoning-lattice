...
impl LinkState {
    ...
    /// iterate its spans for `classify_spans` overlap.
    pub fn stab(i: usize, query: &Endset, v: View) -> im::OrdSet<Tumbler>;
...
- **→ M8** (`M8→M7` edge): `stab(i, Q, view)` and `match_links(constraints, view)` — **the query `Q`/constraint regions are M7's readable `Endset`** (M8 builds them via `enc` over resolved I-addresses, or `Endset::from_spans`), so M7 can iterate the query spans for overlap (`ProperOverlap | Containment | Equal`, never abutment); plus `type_slice(K, view)`, `is_active`/`is_nullified`, the BH3 reverse family, and `readlink` — M8 layers cursors, counting, windowed pagination, projection (via M5's `project`), RETRIEVEENDSETS, and archival `in/out` (composing BH3) on top. M8 owns no index and writes nothing.
