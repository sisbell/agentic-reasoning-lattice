...
// longer than 2 components are minted via raw `shift` + `validate`.
...
/// `LockKey`, or ever journals/checkpoints the canonical form itself, derive
/// `Serialize + Deserialize` here too (the inner `SpanSet` already carries them). Invariant:
/// the inner SpanSet is normalized.
...
/// general `|Σ| = |P|` unit-span cover for arbitrary (possibly non-uniform) finite P (S7) is the
/// separate `cover` (below), not this single-span hull.
pub fn hull(points: &[Tumbler]) -> Option<Span>;

/// S7 unit-span cover of a finite point set: one unit span `from_endpoints(t, inc(t,0))` per
/// point, `union`-joined (concatenation). |Σ| = |P| and ⟦Σ⟧ ⊇ P — a COVER, not exact (S7's
/// binding fact: no span-set denotes an arbitrary finite P exactly). Total: each `inc(t,0) > t`
/// is length-preserving so `WF` always fires per point, and `union` never fails; admits a
/// MIXED-length P (the per-point spans are never merged, so no level gate). NOT normalized
/// (normalizing could coalesce and break |Σ| = |P|) — caller normalizes for a minimal form.
/// `cover(&[]) = empty()`.
...
- **Whether to content-address span-sets** (hash the `CanonicalForm`) for dedup/memoization — directly feeds the shape of M7's coverage-class policy keyed on `canonical_key`. If M7 needs `CanonicalForm` to back a collision-free coverage-class `LockKey`, or ever journals it, `CanonicalForm` must additionally derive `Serialize + Deserialize` (it does not today — it is an in-memory dedup key, rebuilt by replay).
