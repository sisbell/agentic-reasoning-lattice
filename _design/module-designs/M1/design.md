...
pub fn zeros(t: &Tumbler) -> u8;                       // separator count
pub fn is_t4_valid(t: &Tumbler) -> bool;
pub fn classify(t: &Tumbler) -> Class;                 // total, never faults (ASN-0045)

/// Admission constructor: the validate-and-classify front door.
pub fn validate(t: Tumbler) -> Result<Address, T4Error>;   // T4Error carries the violated clause(s)

/// A T4-valid, classified tumbler. Minted only by `validate`. `level` is a derived
/// constant (immutable ⇒ never stale; not a hint).
#[derive(Clone, PartialEq, Eq, Hash)]
...
impl Address {
    ...
    pub fn account_field(&self) -> Option<&[Nat]>;    // zeros ≥ 1
    pub fn document_field(&self) -> Option<&[Nat]>;   // zeros ≥ 2
    pub fn element_field(&self) -> Option<&[Nat]>;    // zeros = 3
...
pub fn same_node(a: &Address, b: &Address) -> bool;       // T6(a)
pub fn same_account(a: &Address, b: &Address) -> bool;     // T6(b)
pub fn same_document(a: &Address, b: &Address) -> bool;    // T6(c)
pub fn under_document(a: &Address, b: &Address) -> bool;   // T6(d): a falls under b's document prefix
```
...
```rust
pub fn shift(v: &Tumbler, n: &Nat) -> Tumbler;   // v ⊕ δ(n,#v): advance the last component (TS1–TS5)

// Recommended subspace-safe packaging — makes TA7a un-violable by callers:
pub struct ElemPos { pub doc: Address, pub subspace: Nat, pub ordinal: Nat }
pub fn elem_addr(p: &ElemPos) -> Address;                 // doc · 0 · subspace · ordinal
pub fn shift_ordinal(p: &ElemPos, n: &Nat) -> ElemPos;    // ordinal += n only
```
...
```rust
#[derive(Clone)] pub struct SpanSet(/* im::Vector<Span> (default) */);
impl SpanSet {
...
pub fn union(a: &SpanSet, b: &SpanSet) -> SpanSet;                 // ⊕-free join; concat (+ normalize)
...
pub fn equiv(a: &SpanSet, b: &SpanSet) -> bool;                   // normalize both, compare (S9)
pub fn canonical_key(s: &SpanSet) -> CanonicalForm;              // normalized form as dedup/cache key
pub fn hull(points: &[Tumbler]) -> Option<Span>;                // covering span for finite P (S7)
```
...
**`SpanSet` — an unordered union-denoting collection.** Default backing is `im::Vector<Span>`: structural sharing makes each derived span-set (a difference, an edit) a new value cheaply sharing the old, which is exactly the cheap-versioned-immutable-value shape Xanadu wants. This is where `im` earns its keep (not on the tumbler). The incremental-maintenance alternative — back it with `im::OrdMap<Tumbler, Span>` keyed by `start`, holding N1/N2 on every coalescing insert — is an Open decision for large, long-lived, in-place-edited span-sets. `canonical_key` returns the normalized form (unique by S9) as a content-addressed identity — the dedup/cache-key primitive M7 builds its link-dedup coverage-class on.
...
`shift(v,n) = v ⊕ δ(n,#v)` advances `v`'s last component by `n` (TumblerAdd at the final position). It inherits `⊕`'s structure: order-preserving (TS1), injective (TS2), additively-composing (TS3), strict and amount-monotone (TS4/TS5) — exactly what lets the I-stream stay sorted and distinct under shift. The hazard (TA7a): the *last component is the ordinal only for a full element position* (`doc·0·subspace·ordinal`); shifting a bare subspace base (`doc·0·subspace`, whose last component **is** the subspace id) would silently advance text→link. The recommended packaging — `ElemPos { doc, subspace, ordinal }` with `shift_ordinal` touching only `ordinal` — strips and reattaches the subspace as structural context, making the bug un-representable. Raw `shift` is exposed for callers already holding a verified full position; the wrapper is the safe default. **Tradeoff:** the wrapper costs a reconstruct on each shift but removes an entire silent-corruption class — take it.
...
### 7. Span-sets
**`normalize`** is the central primitive: sort by `start`, linear left-to-right sweep coalescing any pair with `reach_i ≥ start_{i+1}` (overlap or adjacency), producing the unique N1∧N2 canonical form — O(n log n), dominated by the sort. Uniqueness (S9) makes the canonical form an **equality oracle and cache key**: `equiv` = normalize both, compare; `canonical_key` = the normalized form, content-addressable for dedup/memoization (the seam M7's coverage-class dedup key rides). **`union`** is concatenation; because union is commutative, associative, and idempotent (S10, within one tumbler length), span-sets form a join-semilattice — workers can accumulate and merge in any order to a deterministic canonical result, coordination-free. **`intersect_sets`/`difference_sets`** are one parametrized sweep-line over two already-normalized inputs; difference at the set level has **no proven output bound** (open), so its result is not size-promised. **`hull`** gives a covering span for finite P (convexity, S0); note the binding S7 fact — *no* span-set denotes an arbitrary finite P *exactly*, because every span denotes an infinite set (deeper sub-extensions fill every interval). The algebra is interval arithmetic over a hierarchical ordered space — not finite-set manipulation, not byte counting. **Default to eager normalization for stored/handed-off span-sets, lazy in-flight**; default to batch sort-sweep, graduate to the `im::OrdMap` incremental form only for large long-lived edited sets. Keep the commutative merge path **union-only** — difference is not order-free.
...
- *Contiguous subtrees* (T5, ASN-0034) — every prefix's subtree is a contiguous interval; holds from the order alone, no field parse, so spans capture subtrees for free.
- *Exactly-one-level / off-domain vacuity / never-faults* (Partition, OffDomainVacuity, ASN-0045) — from the five-way sum classifier: a function is single-valued and `Invalid` is disjoint.
...
- *T4 well-formedness* (T4/ASN-0045) — the single scan in `validate`/`classify`; the depth ceiling and every `T6/T7` field-read rest on the `zeros ≤ 3` check. *Where:* §2.
...
- *Level-compatibility* (S6/WF, ASN-0053) — gate before all span/span-set construction. *Where:* §6/§7.
...
- *Arithmetic preconditions* — `⊕`: `Pos(w)∧actionPoint(w)≤#a`; `⊖`: `a≥w`. *Where:* `add`/`sub`.
...
- **`Tumbler`/`Address`** — the universal key/endpoint/classification type for every module (M3–M10). Flat `Tumbler` is the storage/journal key; `Address` is the past-the-door carried value.
...
- **`classify`/`Level`/field projections/containment → M3, M5, M6, M8.** M3's `ω` longest-prefix owner resolver composes `same_account`/`under_document` over its principal registry (M1 gives the per-address predicates; the resolver is M3's). M6/M8's registered-empty-vs-unallocated query distinctions, and M5's S3★ content-vs-link V-position routing, all key on `classify`/`subspace`.
- **`subspace` → M5/M7/M8.** Drives content-subspace (M4-targeted) vs link-subspace (M7-targeted) referential-integrity routing.
- **`Span`/`SpanSet`/`normalize`/`classify_spans`/`intersect`/`difference`/`canonical_key` → M5, M6, M7, M8.** M7's endsets/link values are span-sets; its link-dedup **coverage-class key** is derived from `canonical_key` (M1 computes the canonical form; M7 decides the class policy and supplies it to M2's keyed critical section — M2 never computes it). M8's coverage/discovery and M6's extent queries consume `classify_spans`/`intersect`/`difference`/membership.
- **`shift`/`ElemPos` → M3/M5.** I-stream element allocation and V-enfilade traversal are driven by the ordinal shift; M1 supplies the pure value tool, the stateful allocation/traversal is upstream.
...
- **Whether to content-address span-sets** (hash the canonical form) for dedup/memoization — directly feeds the shape of M7's coverage-class key.
- **Pairwise return shape:** uniform `SpanSet` vs a `None | One | Two` sum, given the proven fan-outs.
- **`Invalid` diagnostic:** bare rejection vs the failing clause(s); since clauses co-occur (`[0]` violates leading *and* trailing; `[0,0]` adds adjacent), first-failure vs full-set is itself a sub-decision.
- **Subspace-context plumbing:** the `ElemPos` strip/reattach wrapper (recommended — makes TA7a un-violable) vs carrying the subspace id alongside ordinals everywhere.
