...
impl M3State {
    ...
    // apart.) (And there is NO shared base-crate `key(home, …)` for namespace keys — these constructors
    // supersede that composition-contract sketch; §1.) The constructor is called BEFORE the closure for
    // transact's `keys` arg (M2 holds keys for the txn's duration); the mint runs INSIDE the closure and
    // hands back the matching `Allocate` to stage.
...
```rust
pub struct Namespace<W: WorldState> { kernel: Arc<Kernel<W>> }   // M3's transact-driving op handle over M2

impl<W: WorldState + HasM3> Namespace<W> where W::Record: From<M3Rec> { // the From bound lets each op
                 // LIFT its M3Rec deltas to W::Record and stage them via stg.push(rec.into()) (the
                 // write-path mirror of HasM3's read accessor). Each op opens ONE `transact`.
                 // `delegate`'s race-prone gates (freshness, top-down, id-freshness, next-form,
                 // authorization) are evaluated INSIDE the closure against stg.base().m3() under the
                 // held locks (a pre-transaction snapshot() read is at most a fail-fast, NEVER
    ...
    /// only; §6 / Conflicts §7) and HOISTED to pure pre-work, right after the validate-lift and
...
impl M3State {
    ...
    /// account chain; an account ⇒ the (parent, 1) sub-account chain (a chain family ASN-0042
    /// licenses though K.δ's vocabulary omits it — Conflicts §8). Both yield zeros=1 (account-tier,
    /// consistent with (iii)==1). Pure frontier read off any snapshot; `None` unless `parent` is a
    /// REGISTERED node or account — entity_level(parent) ∈ {Some(Node), Some(Account)}, which
    /// pre-discharges the one monotone gate a peek can answer honestly (delegate's parent-reg: an
    /// unregistered parent's peek would only ever earn ParentNotRegistered; and E is append-only,
    /// so a Some answer never regresses). After that pre-gate the TA5a namespace gate CANNOT trip
    /// on any reachable input — both arms pass unconditionally ((N,2) at zeros(N)=0 meets the k=2
    /// budget; (A,1)'s k=1 always passes) — so a gate-caused None is DEFENSIVE-ONLY (corrupted
    /// frontier state), never a live error path a builder should wire for. The returned prefix still
    /// faces delegate's full in-closure gate, so two racing peeks of the same value still leave
    /// exactly one winner.
    pub fn next_account_prefix(&self, parent: &Address) -> Option<Address>;
    /// Containment test (O1): prefix ≼ a — pure, total, decidable from the two addresses alone.
    /// NEVER an authorization check: several principals' prefixes contain the same address (a node
    /// operator's prefix contains every delegated account); authorization resolves the ONE effective
    /// owner via `effective_owner` (ω, longest match) — §5.
...
```rust
impl M3State { pub fn genesis() -> M3State; }   // pub — the engine seeds Kernel::open(cfg, genesis-World) with it.
                                                // nodes={[1]}, frontiers={}, Π={ [1] → Principal{BOOTSTRAP_PRINCIPAL,[1]} }  (Σ₀, O14)
```
...
`apply_ns` (M3's fold) is deterministic, total, side-effect-free — and `pub`, since the engine's `World::apply` dispatches into it. A bare journal `Tumbler` must be lifted to an `Address` before `parent`/projection (M1 types those over `&Address`/`&Tumbler` respectively); minted and journaled addresses are always T4-valid, so the lift cannot fail. The engine's `World::apply` routes the `Record::Ns` variant (M3's lifted `M3Rec`, M3 being the engine's namespace slice) into this fold:

```rust
/// M3's fold — pub: the engine crate wires World::apply to this. TOTALITY DOMAIN (M2's total-apply
/// obligation, stated here at the seam the engine wires): total — deterministic, side-effect-free,
/// panic-free — over every record staged by M3's own paths, the only record source (every mint
...
/// Recover (parent, g, ordinal) from a T4-valid address of ≥ 2 COMPONENTS — pure M1. The ≥ 2 bound
/// is a caller obligation both call paths discharge: every MINT extends a registered parent
/// (#addr ≥ #parent + 1 ≥ 2), and `delegate` calls only AFTER its hoisted tier check (iii) —
/// zeros(new_prefix) == 1 forces the N·0·U parse, hence ≥ 3 components (§6). A bare 1-component
/// node (e.g. [7]) is T4-valid but PARENTLESS — M1's `parent` returns None exactly there — so the
/// validate-lift alone does NOT establish the bound, and such an address must never reach here.
/// `parent` takes `&Address`, so the bare `Tumbler` is `validate`-lifted first (total — the caller
/// passes a T4-valid tumbler); `zeros` and `ordinal` take `&Tumbler`, so they read `.tumbler()`
/// projections.
fn decompose(addr: &Tumbler) -> (Tumbler, u8, Nat) {
    let ad  = validate(addr.clone()).expect("T4-valid by caller contract");
    let par = parent(&ad).expect("≥ 2 components by caller contract — mints extend a parent; delegate tier-checks first");
    let g   = if zeros(addr) == zeros(par.tumbler()) { 1 } else { 2 };
    (par.tumbler().clone(), g, ordinal(addr).clone())
}

/// The NsKey a minted address advances under — its namespace. Pure M1 via `decompose`, inheriting
/// its ≥ 2-component caller obligation. `delegate` uses it for BOTH the next-form check and the held
/// namespace LockKey, so the checked key, the locked key, and the key the staged `Allocate` advances
/// are one and the same key — and it calls this only after the validate-lift AND the hoisted (iii)
/// tier check (the lift alone is INSUFFICIENT: a 1-component node is T4-valid but parentless — §6).
...
```rust
use num_traits::Zero;   // Nat::zero()/is_zero() are num-traits trait fns, not inherent — must be in scope
...
// REUSE an address).
//
// PLACEMENT vs the Engine Composition Contract's `key(home, …)` sketch: the contract (and M2's seam
// sketch) places a shared `key(home: &Address, …) -> LockKey` constructor in the shared base crate
// over skep-address + skep-kernel, with "every store building a namespace lock key calls that one
// constructor." M3's `ns_lock_key` + the `*_lock_key` constructors SUPERSEDE that sketch for
// namespace keys — there is deliberately NO base-crate `key(d, s_C)`; dependents call
// M3State::*_lock_key and never hand-roll a key from the sketch. The contract's INTENT — ONE
// injective constructor, living below every builder, so every builder's key bytes agree by
// construction — is satisfied, and more strongly: (a) the true key is the NsKey (parent, g), M3's
// own frontier key, which the sketch's (home, subspace) shape cannot even express for
// version/document/account chains — and a bare (home_doc, g) under-distinguishes the three g=1
// chains under one document (the fatal aliasing); (b) M3's crate sits BELOW every store that builds
// a namespace lock key (M5, M7, and M10 all depend on M3; M4 builds no namespace keys — its writes
// ride M5's placement composite), so the acyclicity the base-crate placement exists to protect
// holds unchanged; (c) byte-identity across builders comes from SINGLE OWNERSHIP of the one code
// path — every builder calls the same *_ns + ns_lock_key the mint's frontier advances under — which
// is strictly stronger than shared placement of a constructor each store re-parameterizes.
...
```rust
use num_traits::{One, Zero};   // Nat::one()/Nat::zero() are trait fns — in scope for the range checks
...
fn entity_level(&self, a: &Address) -> Option<Level> {
    match a.level() {
        ...
        Level::Account | Level::Document => {
            ...
            (n >= &Nat::one() && n <= &m).then_some(a.level())
        }
    ...
    match a.level() {
        ...
        Level::Account | Level::Document | Level::Element => {
            ...
            let key = NsKey{ parent: p.tumbler().clone(),
                             g: if zeros(a.tumbler())==zeros(p.tumbler()) {1} else {2} };
            let m = self.frontiers.get(&key).cloned().unwrap_or_else(Nat::zero);
            let n = ordinal(a.tumbler());                      // &Nat — by reference, as above
            n >= &Nat::one() && n <= &m
        }
    }
}
...
(iii) account-tier  zeros(new_prefix) == 1                           O1a (narrowed ≤1→==1)   [monotone — pure; HOISTED to pre-work: guards namespace()/lock-key construction; ⇒ NotAccountTier]
(iv)  top-down      no existing principal strictly under new_prefix  OrdMap single probe (below) [NON-MONOTONE → in-closure]
(v)   fresh-valid   T4-valid(new_prefix) ∧ !is_allocated(new_prefix) O1b prefix-injectivity  [T4 monotone (pre-work lift); !is_allocated NON-MONOTONE → in-closure]
...
Delegation always produces an **account** (zeros=1: condition (iii)'s strict `== 1` forces it directly — see the narrowing above; internal baptism never mints a zeros=0 node, and nodes arrive only via `register_node`), advancing an account-chain frontier — a node's `(N, 2)` account chain or, for a sub-account, an account's own `(A, 1)` chain (licensed by ASN-0042 though outside K.δ's case vocabulary — [Conflicts resolved](#conflicts-resolved) §8) — and adding one principal; it is the *only* non-bootstrap entry for principals. Because both `LockKey`s are computed from the validate-lifted, tier-checked prefix *before* `transact` opens (M2 holds `keys` for the txn's duration), the held namespace lock and the in-closure `next_in(namespace(new_prefix))` advance the **same** `(parent, g)` key, and the held `principals_lock_key()` covers the in-closure `id-fresh`/`top-down`/authorization reads against every concurrent delegation (§8).

### 7. CREATENEWDOCUMENT, fork, node baptism, genesis

- **`create_new_document`** locks `[document_lock_key(account), principals_lock_key()]`; inside the closure, against `stg.base().m3()`, it evaluates `matches!(stg.base().m3().effective_owner(account), Some(p) if p.id == caller)` (`NotOwner` if ω is absent — `None` — or names another principal), then mints via `mint_document` off `stg.working().m3()`, stages one `Allocate` (lifted `rec.into()`), and returns the address with its commit `Seq` post-commit. **ω(account) is stable** for an existing account — from delegation freshness (v) plus O1a: every `zeros ≤ 1` prefix that could cover `account` is closed to new principals (its account-tier prefixes — itself and, for a sub-account, its covering user-field chain — are already baptized by the delegation induction and P8; the node tier is closed to delegation outright by (iii); and a sub-account sits *below* `account`, never covering it), so no concurrent `delegate` (which requires a *fresh* target) can introduce a longer coverer. The read is therefore stale-safe; it is evaluated in-closure for uniformity with the rest of the op (and could equally be a pre-read), and the held `principals_lock_key()` is **defensive/harmless-redundant, not load-bearing** (§8). It registers d and **stops** — no `M(d)=∅` write into M5. This is the deliberate divergence from ASN-0103's eager recommendation ([Conflicts resolved](#conflicts-resolved)). No idempotency key (identity is the address, ASN-0103): a retried lost-ack yields a harmless orphan empty document; exactly-once lives at M10's session layer if wanted.
...
**Upstream — M2 (calls):** `transact(&[<namespace LockKey>], …)` — the namespace key from `content`/`link`/`version`/`document_lock_key` (plus `principals_lock_key()` for `delegate` (load-bearing) and `create_new_document` (defensive), and `node_lock_key()` for `register_node`, §8) — for every mutating op (atomic + serialized + durable), receiving each op's commit `Seq` from `transact`'s `(T, Seq)` return; `snapshot()` for query reads; M3 provides its fold `M3State::apply_ns` (`pub` — the engine's `World::apply` dispatches the `Record::Ns` variant into it; its totality domain is stated at the function itself for the engine implementer wiring the seam) and relies on M2's **default `rebuild_derived`** (its slice is fully `Serialize`d, nothing to re-seed). M3 stages `M3Rec` **lifted to `W::Record` via the `W::Record: From<M3Rec>` bound** (`stg.push(rec.into())` — Public interface B); M2 owns ordering/durability/recovery. The 1-byte space tags (`Space::Namespace`/`Principals`/`Nodes`) are **declared in** M2's single central `Space` enum, which lives in `skep-kernel` below every store (not the engine assembler crate — that would cycle); M3 is the one store drawing *three* tags from it (three serialization domains). The contract's one-tag-*per-store* line is *guidance*; its hard rule is tag **uniqueness**, which three distinct `Space` variants satisfy — and those variants live in skep-kernel's enum, not "reserved" by skep-engine (which only assembles `World`/`Record`). Likewise, the contract's shared-base-crate `key(home, …)` constructor sketch is **superseded for namespace keys by M3's `ns_lock_key`/`*_lock_key`** (§1): the true key is M3's `NsKey` `(parent, g)` — inexpressible as `(home, subspace)` for version/document/account chains — M3 sits below every namespace-key builder (M5/M7/M10; M4 builds no namespace keys), and single ownership of the one encoding path delivers exactly the byte-identity the base-crate placement exists for. There is no base-crate `key(d, s_C)`; dependents call M3's constructors.
...
6. **O10 node-tier fork (ASN‑0042 O10) vs the account ≡ principal model.** O10 (DenialAsFork) lets *any* principal fork a self-owned address one structural tier below its prefix — including a **node-tier** principal minting a self-owned *account* (zeros=1) with **no** new principal. M3 deliberately scopes `fork` to the **account-tier** case only: a fork mints a *document* (zeros=2) under an account-tier caller's prefix. **Resolution / consequence:** the node-tier O10 case is **dropped, not relocated** — and the drop is a deliberate **API-scoping decision, not a correctness necessity**. A node-tier caller is rejected `Mint(NotAnAccount)`; this is *not* silently equivalent to "delegate an account first," because `delegate` mints a **new** principal and moves effective ownership to it (O7), whereas O10's node-tier fork would leave the new account owned by the *same* node principal. To be precise about what the drop does *not* protect: a node-tier fork would register a principal-less account — covered only by the node principal via ω — and **that would still authorize correctly**. `create_new_document`'s gate reads `ω(account)` and admits `p.id == caller`; for such an account ω resolves to exactly the node principal, which is O10's intent. And ω-stability of an existing account is **independent** of any "every registered account carries its own account principal" invariant — it follows from delegation freshness (v) plus O1a: every account-tier prefix of a registered account is already baptized (its user-field chain by the delegation induction, its node parent by P8), the node tier is closed to delegation outright ((iii) `== 1`), and freshness requires an unbaptized target, so no concurrent delegation can introduce a new coverer. What the drop buys is a single, simple model: the account-tier prefix is the unit of principal identity, and a node operator provisions accounts *via delegation* (new principals) — one path, not two. Re-admitting node-tier `fork` is flagged under [Open decisions](#open-build-decisions).
...
8. **Sub-account delegation on `(A, 1)` (ASN‑0042) vs ASN‑0047's K.δ case vocabulary.** ASN‑0042 explicitly licenses a delegate prefix on an *account's own* depth-1 chain: O7(c) admits any next-reachable `next(B, p, d)` for B6-valid `(p, d)` — `(A, 1)` qualifies (`zeros(A)=1`, `g=1`, budget `1+0 ≤ 3`), its emissions extend the user field one component and stay `zeros=1` (account-tier, so condition (iii) `==1` still passes), and `acct` explicitly parses a multi-component user field. This design admits it: `next_account_prefix`'s account arm peeks `(parent, 1)`, and `delegate`'s gate passes such a prefix. But ASN‑0047's K.δ has **no admissible case** for the chain's *first* emission — `e = inc(A, 1)` falls under case (ii) k=1, whose precondition is `t ∈ E_doc` (a *version* step; an account operand has no k=1 sub-case) — and AllocatorHierarchy names five chain families (`A_C(d)`, `A_L(d)`, `A_v(d)`, `A_doc(A)`, `A_account(N)`), none of them account-under-account. Resolution: **ASN‑0042 governs delegation.** Delegation is not a K.δ instance — it is the principal-introducing transition of O15/O17b (one baptism *plus* one principal registration, a pairing K.δ cannot express either) — so K.δ's case vocabulary does not constrain the prefixes `delegate` may baptize; O15/O17c do, and they admit `(A, 1)`. M3's uniform `(parent, g)` registry simply carries `(A, 1)` as a **sixth chain family** with nothing special: same counter, same `ns_lock_key`, same membership decomposition (M1's longest-valid-prefix `parent` of a sub-account is the covering account at equal zeros ⇒ `g=1` — the sub-account case already verified in §2's membership invariant), and P8 discharged by `delegate`'s parent-reg gate (the parent *account* must be registered).
...
- **Lock-key constructor placement:** M3-owned `ns_lock_key` + `*_lock_key` (current — supersedes the composition contract's base-crate `key(home, …)` sketch; §1 states why the contract's one-constructor intent is met, and more strongly, by single ownership below every builder) vs. moving `NsKey` + `ns_lock_key` into the shared base crate over `skep-address`+`skep-kernel` (literal contract conformance; byte-equivalent, but M3's frontier-map key type then lives outside M3). Either way there is exactly one encoding path and no per-store re-parameterization.
...
- **Node-tier `fork` (O10):** dropped today as API scoping ([Conflicts resolved](#conflicts-resolved) §6); re-admit a node-principal forking a *self-owned* account (no new principal, ownership stays at the node) only if that provisioning path is needed — it requires a node-tier "mint a self-owned account" branch distinct from `delegate` (which moves ownership to a new principal). No invariant blocks it: a principal-less account authorizes correctly under ω, and ω-stability is independent of principal placement (Conflicts §6).
