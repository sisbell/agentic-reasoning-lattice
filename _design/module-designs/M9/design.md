...
- **Fire atomicity/finiteness/home; domain re-check; benign two-transaction race** — one M7 `emit` *or* `nullify` per fire; M7 checks the home (Q's H-ATOM/H-FIN/H-HOME). `fire` re-checks `x ∈ [D_ρ]` at its own snapshot before the trigger (ASN-0133's fire relation is defined only on domain members; an out-of-domain argument NoOps — the fairness *removed* discharge). The membership + trigger check and the deposit are two transactions; an idem⊤ dedup hit in the gap is reported `FireOutcome::Deduped` (commits nothing) and **excluded from the divergence count**, which counts only real `Fired` (item 5); a fire *error* surfaces from the `step` driver as `StepOutcome::Failed` (rotate-past, §7), never a silent skip.
