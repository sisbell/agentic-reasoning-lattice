...
impl<W> Coordinator<W> {
    ...
    /// The catalog projection is VALIDATE-ONCE-OR-FAIL (mirroring genesis): each
    /// enc(&[reserved.X]) must be coverage-equal to the injected registry's own reserved(X), and
    /// each decls key must hold a registration in that registry — a miss fails construction with
    /// CatalogError, so residual drift of the twice-passed (reserved, decls) is caught at
    /// assembly, never as a spurious UnregisteredType at type-check.
    pub fn new(
        kernel:        Arc<Kernel<W>>,
...
impl<W> Coordinator<W> {
    ...
    /// insert's). REJECTS a `Tup`-sorted Γ_D parameter UP FRONT, before any insert
    /// (`DefineError::TupParameter` — stored-def params are Codom-only, ASN-0130 SignedTerm; only a
    /// `type_check_trigger`-built term can carry one; the codec also refuses to encode `Sort::Tup`).
    /// A concurrent INSERT/DELETE may move/invalidate the computed append position — a
    ...
    /// breach) answers None, memoized as a PERMANENT POISONED entry — a deliberate FREEZE-ON-BREACH
    /// POLICY (§Internal 4): a parse failure cannot heal (content immutable), while a WT failure
    /// via a DanglingReference to a never-registered referent could in principle heal on that
    /// referent's later registration — the freeze declines to re-check (safe: a poisoned start
    /// merely stays signature-less; never a wrong Some). A never-registered None is NOT memoized
    /// (transient — a later registration must surface). No snapshot parameter.
    ...
    /// Register `new_term` (define-style, sharing `define_predicate`'s up-front `TupParameter`
    /// rejection) and record old→new via the shipped `supersedes` type (content-address
    /// endpoints, NOT M7::assert_sup). GATES `old_start` UP FRONT, before any transaction: it
    /// must be EVER-registered (`is_ever_pred` off a snapshot pinned at entry) — else
    /// `DefineError::OldStartNotEverRegistered` — so a typo'd non-def address can never seed a
    /// `supersedes` lineage that `current_version` would then walk. 3 non-atomic txns; no
    /// idempotency key — a lost-ack retry branches the lineage. Retry-dedup is the DRIVING
    /// coordination caller's (this is a Coordinator method reaching M5/M7 directly, NOT via M10);
    /// `create_new_document` is the *pattern* analogue, not the same dispatcher. Returns the
    /// successor's identity (its content start) and the `supersedes` EMIT's commit Seq — the
    /// third transaction's, not the insert's and not the successor's `pdef` emit's.
...
impl<W> Coordinator<W> {
    ...
    /// trigger cannot serve a `Tup` domain); a Marker action's `ty` is a cataloged idem⊤ Unary type
    /// (idem⊥ ⇒ `NonIdemMarkerType` — the gap dedup-absorption and the Q3/I1a analysis lean on
    /// idem⊤) that is NOT a PredLayer class (`pdef`/`pd_stable` ⇒ `PredLayerMarkerType` — PR-DISC
    /// reserves those slices for register_pred/certify_stable). REJECTS a ref-bearing
    ...
    /// SF + Marker + grow-only lint. INPUT CONTRACT: takes the RAW submission and RE-RUNS
    /// register_rule's normalization internally (the one shared validation path — domain →
    /// TypedDom, trigger checks), then lints the CHECKED artifacts; a malformed rule returns the
    /// same typed RuleError register_rule would (never a panic). Callable pre-registration.
    pub fn certify_rule(&self, rule: &Rule) -> Result<RuleCertification, RuleError>;

    /// Q0. SNAPSHOT-FRESHNESS PRECONDITION (here and on quiescent_scoped / next_enabled / step —
    /// every rule-engine entry point taking a caller Snapshot): `snap` must not predate the
    /// registration commit of any admitted `TriggerRef::Def` trigger's def — ever-registration is
    /// monotone, so any snapshot pinned after the last Def-trigger `register_rule` qualifies; a
    /// staler snapshot can make `evaluate_def` err `NotEverRegistered` inside a bool/StepOutcome-
    /// returning method, and is a PRECONDITION VIOLATION (panics, like `decide`). `fire` is
    /// exempt: it pins its own fresh snapshot.
    ...
    pub fn fire(&self, e: &Enabled) -> Result<FireOutcome, FireError>;          // domain+trigger re-check, then ≤1 deposit (emit|nullify), atomic, H-*
    pub fn step(&mut self, snap: &Snapshot<W>) -> StepOutcome;                  // pick+fire driver; fire errors → StepOutcome::Failed (rotate-past, §7)
...
| `FireCounters` | `(RuleId, x) → u64` | replay of M7 deposits under §8's attribution key — `(ty coverage-class, home, F-coverage)` for Marker, `([R], retracting home, G-coverage)` for Nullify — plus re-evaluation; the key **collides** across rules sharing `(ty, home)` and with non-rule same-type writers, so the recompute **over-counts only** (never under-counts a genuine rule fire — §8) |
...
**Interior mutability of the hint caches.** `signature`, `define_predicate`, and `register_pred` take `&self` yet populate the `DefMemo` (memoize a freshly-derived `Some` signature / resolved body / expanded term), so the `DefMemo` is held behind **interior mutability** (a `RwLock` or a concurrent map — **not** a `RefCell`: a `Coordinator<W>` over the `Send + Sync` factories and `Arc<Kernel<W>>` is naturally `Sync`, and `RefCell` is `!Sync`, so it would not compile across the `&self` `signature`/`define_predicate`/`register_pred` API of a shared `Coordinator`), never `&mut self`. This is sound precisely because every memo entry is an *immutable-once-defined hint* (a `Some` — or a poisoned marker — is permanent; §4): concurrent fills race only to write the same value. **`FireCounters` are journal-recomputable, not cache-authoritative:** a per-`(ρ,x)` fire count is recomputed by replaying M7's deposits under §8's attribution key (the counter table is a recomputable hint — see the table; the key collides in §8's documented cases, so the recompute is an over-approximation that never under-counts a genuine rule fire), so a direct `&self` `fire` that does *not* touch the cached counter is still covered by the divergence monitor's recompute (a `Deduped`/`NoOp` — or erroring — fire deposits nothing, so it leaves no journal record and is correctly excluded). The cached counter is only a latency optimization over that recompute.
...
pub enum TypeError {
    ...
    UnservedWalkClass(TypeKey),                    // a BH2 atom (Succs/Chain/Tip/IsInChain) at a cataloged Walk class other than
                                                   //   shipped Supersedes — M7 v1 serves the walk only for Supersedes (empty/
                                                   //   trivial otherwise), so admitting it would silently denote ∅/[]; lifts
                                                   //   when M7 serves app Walk classes
    ...
    NoReverseLookupClass,                          // the `targets_keyed` atom used (bare or under MapGet) with no cataloged
                                                   //   class attaching BH3 (V-atom)
...
pub enum DefineError    { TupParameter(VarId),     // a Tup-sorted Γ_D parameter — rejected UP FRONT, before any insert (no
                                                   //   orphan content); stored-def params are Codom-only (ASN-0130 SignedTerm).
                                                   //   Checked by define_predicate AND supersede (the define-style path)
                          OldStartNotEverRegistered(Address),  // supersede only: `old_start` is not an ever-registered def —
                                                   //   gated UP FRONT, before any transaction (a typo'd non-def address must
                                                   //   not seed a `supersedes` lineage; PR4 presupposes the superseded
                                                   //   address IS a definition)
                          Insert(TxnError<InsertError>), Register(RegisterError), Supersede(TxnError<EmitError>) }
...
pub enum CertifyError   { NotEverRegistered,
                          UndisciplinedDef,      // ever-registered start with NO defined signature — the permanent poisoned
                                                 //   memo entry (PR-DISC breach, §Internal 4); mirrors EvalError's variant, so
                                                 //   CVALID (0)'s two `None` causes surface distinctly
                          NotBoolean, NotActive, ViewDependent, NotStable, Emit(TxnError<EmitError>) }
...
pub enum RuleError      { RefBearingInlineTrigger,                                // Inline must be ref-free; persist-as-def + `Def` works only for a Codom-param trigger — a tuple-domained trigger must inline the helper
                          ...
                          NonIdemMarkerType(TypeKey),                             // Marker.ty cataloged Unary but idem = ⊥ — the fire executor's
                                                                                  //   gap dedup-absorption and the Q3/I1a extinction analysis
                                                                                  //   both require idem⊤
                          PredLayerMarkerType(TypeKey) }                          // Marker.ty ∈ {pdef, pd_stable} — PR-DISC reserves
                                                                                  //   those slices for register_pred/certify_stable
pub enum CatalogError   { ReservedMismatch(ShippedType),                          // enc(&[reserved.X]) not coverage-equal to the injected
                                                                                  //   registry's own reserved(X) — the twice-passed pair
                                                                                  //   drifted from the genesis pair
                          DeclNotInRegistry(TypeKey) }                            // a decls key (pre-checked address-denoting, keeping the
                                                                                  //   coverage_class probe total) with no registration in the
                                                                                  //   injected registry — drift caught at construction

pub struct RuleId(u64);
pub struct Enabled { pub rule: RuleId, pub arg: Value }      // (ρ, x); Value::{Addr | Tuple} per the domain sort
pub enum FireOutcome { NoOp,                                  // arg out of [D_ρ] (removed) or trigger false (falsified) at fire
                                                              //   time — Q1; the fire relation is defined only on domain members
...
// StepOutcome::{Fired, Deduped} carry FireOutcome's `effect` (the deposited resp. incumbent tuple's
// address) through, so a driver can log the deposit and reconcile the divergence monitor against
// the journal without re-deriving the deposited tuple's address.
pub enum StepOutcome { Fired   { rule: RuleId, arg: Address, effect: Address, seq: Seq },
                       Deduped { rule: RuleId, arg: Address, effect: Address, seq: Seq },  // a `fire` that dedup-hit (no real fire)
                       Failed  { rule: RuleId, arg: Address, err: FireError },    // a fire error, surfaced (never swallowed);
                                                                                  //   nothing committed; driver rotates past (§7)
                       NoOp, Quiescent }
...
| `succs`, `chain`, `tip`, `is_in_chain` | `succs`, `chain`, `tip`, `chain(K,x).contains(t)` — v1: admissible only at the shipped `Supersedes` key (the catalog guard rejects other Walk classes, `UnservedWalkClass` — M7 v1 serves the walk only for `Supersedes`) |
| `sources_to`, `target_of`, `targets_keyed` | identical M7 BH3 reads (`targets_keyed`'s map keyed by `CoverageClass`; `·[K]` indexes via the catalog's precomputed class for `K` — an absent key, non-BH3 `K` included, reads ⊥ / `OptAddr(None)`) |
...
**Set-element lift (Tumbler→Address).** `Value::AddrSet` holds raw `im::OrdSet<Tumbler>` (the folds' native key); the lift to M1's `Address` happens at the **binding sites** where a set element crosses into address position — a quantifier/`Filter`/`BigUnion` binding over a `SetTerm` (or `Reflect`-materialized) domain produces `Value::Addr(Address)`, and a T1-extremum over a materialized set produces `OptAddr` — via M1's `validate`, infallible on store-minted addresses.
...
- **Stability** (`Stability`, the 4-point lattice `StSf`/`StOnly`/`SfOnly`/`Neither`) by the PD0 mutual induction. Call an address argument **step-constant** iff it is a Γ- or binder-bound `Var`, a literal, or a bound-tuple V-TUP projection — equivalently, a subterm with empty footprint (PD0's "literal or already-bound address" proviso, transcribed structurally). The *grow-only* domain closure the certifier transcribes (its completeness reach): `L_K`; `L_dom`; `M_K` *in an audit-view term* (`v = audit`; V-AUD's union over the growing `L_K`); a `Filter{D, P}` with `D` grow-only and the body `P ∈ ST` for every binding; a *step-constant domain* (a set-valued term reading no state beyond bound parameters); plus the two derived set-valued grow-only forms — `targets_of(x, audit)` at a step-constant argument (an `Atom::TargetsOf` at `v = audit`, or its explicit `observe`-rebuild spelling), and a `⋃(D, f)` fold with `D` grow-only and `f` step-constant per binding. On that closure: step-constants in `ST∩SF`; `is_doc(e)` and audit `is_K(e)` (`IsK` at `v = audit`, or its explicit `∃`-over-`L_K` spelling at any view) in `ST` **only when the argument `e` is step-constant** — a state-reading argument (e.g. an `IfSome`-narrowed value) lands the node in `Neither`, its value free to change across steps and flip the verdict; grow-only membership (`t ∈ D`, incl. `SetMem(t, Reflect(D))` over a grow-only `D`) in `ST` under the same step-constant proviso on `t`; emptiness (`D = ∅`, incl. `IsEmpty(Reflect(D))` over a grow-only `D`) and upper-bound counts (`count(D) ≤ c`) in `SF`; lower-bound counts (`count(D) ≥ c`) over a grow-only `D` in `ST`; `¬` swaps, `∧`/`∨` preserve, `⇒` combines `SF⇒ST`; quantifiers/aggregates per grow-only/step-constant domain (a step-constant `D` strengthens both quantifier directions and puts every comparison in `ST∩SF`). T1-extrema are in neither, and `count(D) = c` is unclassified (so the certifier lands it in `Neither` — never over-certified).
...
**Snapshot precision for (iii).** `signature(r)` on a `DefMemo` miss pins its **own** snapshot (§Resolution bullets below), so the validation is not literally single-snapshot. It is sound as if it were: (iii)'s ever-registration check at σ gates first, ever-registration is monotone, and every fact the signature derivation consults is content-intrinsic — the immutable run plus PR-SIG-fixed referent signatures — so the later pin cannot answer differently than a σ-only derivation would.
...
**Discipline (PR-DISC).** PR1/PR2/PR-SIG and their consequences hold on **registration-disciplined derivations** — every `L_pdef`-growing step `register_pred`'s deposit branch, every `L_pd_stable`-growing step `certify_stable`'s. In-module, `register_rule` closes the one internal route: a `FireAction::Marker` into either PredLayer class is rejected (`PredLayerMarkerType`), so no rule fire can grow those slices. Out-of-module, M7's `emit` is reachable by other holders (M10 dispatches §D ops; M7's gate rejects only R-class), so **PR-DISC is a standing assembly obligation**: no writer routes a typed emit with `ty ∈ {pdef, pd_stable}` except M9's `register_pred`/`certify_stable`. On a breach the degradation is **per-start and defined**: an ever-registered start whose immutable content fails the PR-ENC parse/WT has **no defined signature** — `signature` answers `None` and memoizes a **permanent poisoned entry** (distinct from the transient, unmemoized never-registered `None`), `evaluate_def` returns `EvalError::UndisciplinedDef`, `certify_stable` rejects `CertifyError::UndisciplinedDef`, and a `Ref` to it has no typing judgment (`DanglingReference` — WT-ref's domain condition, read as "no defined signature"). The permanence of the poison is a **deliberate freeze-on-breach policy, not a pure immutability consequence**: a *parse* failure genuinely cannot heal (content immutable), but a *WT* failure whose `DanglingReference` names a never-registered referent could in principle heal if that referent were later legitimately registered — the freeze declines to re-check, which is safe (a poisoned start merely stays signature-less; no wrong `Some` ever arises) and reachable only under a breach in the first place.
...
- `sig(a)` — `(Γ_D, C_D)`; `C_D` re-derives identically forever (PR-SIG); defined on ever-registered-and-disciplined starts, by induction on registration order (well-founded — references name strictly-earlier defs, PR2). `signature(start)` answers from the `DefMemo`; on a **miss** it pins its own snapshot and checks ever-registration (via `is_K(pdef, start)@audit`): **if ever-registered**, it parses + `Reg`-expands + derives `C_D` — which, by WT-ref, **recurses through each referent's `signature(r)`** (well-founded: every referent is strictly-earlier-registered and ever-registered whenever `start` is, PR2; the recursion bottoms out at ref-free bodies) — and **memoizes the `Some`** (permanent — content immutable, ever-registration monotone, so a once-`Some` answer stays valid); **if ever-registered but the parse/WT of the immutable content fails** (an undisciplined deposit — a PR-DISC breach), it answers `None` and memoizes a **permanent poisoned entry** (freeze-on-breach, per the Discipline paragraph: never re-checked — exact for a parse failure, a deliberate freeze for a dangling-referent WT failure a later registration could in principle heal; the start thereafter behaves as having no defined signature); **if not ever-registered**, it returns `None` and **memoizes nothing** (a never-registered `None` is transient — a later registration must be observed).
- `expand(a)` — **for evaluation, never materialized** (the *reference*-expansion, distinct from `Reg`-expansion): `evaluate_def` drives the denotation by **DAG-recursion** — its driver is `eval`'s ref-free walk (§2) *plus the one `Ref` arm `eval` lacks*: at a `Ref{addr,args}` node, resolve `addr` (M4 content read), evaluate `args` to *values* (recursively, so nested refs are handled), and denote the referent's `Reg`-expanded body in a fresh env bound to those values (denotation is compositional → sound, no flat string built). Content reads (resolution) and structural reads (denotation) stay distinct passes, so the denotation remains content-free — *this* is why self-hosting doesn't re-open the structural-reads-only surface, and why `eval` itself can keep its ref-free precondition. **For certification, the flat `expand` is built** (§3) with a **content-deterministic fresh-name counter**, **advanced along the fixed traversal order PR3 states: reference nodes processed bottom-up, siblings left to right (each `Ref`'s arguments expanded before the node itself), and at each reference node the fresh names assigned to `expand(r)`'s parameters first, in signature order, then to its binding sites depth-first, left to right** — one counter per top-level expand, advanced across the whole traversal, never reset per sub-call, never global; a **crate-private minting path**, not `VarId::new` — drawing names from the reserved `VarId ≥ EXPANSION_NAME_BASE` sub-range (§Core data model), which no recorded parameter nor body binder can inhabit (the guarded `VarId::new` plus the codec's reserved-range rejection), so the renamed expansion names are disjoint from every host binder *by construction* (PR-ENC's reserved supply); two evaluators thus obtain the same concrete term **because the traversal order is fixed here, not left to convention** (the cross-evaluator agreement that licenses shared caching). De Bruijn / locally-nameless + hash-consing is the alternative if a flat form must be cached.

**Versioning.** `supersede(d, old_start, new_term)` **gates `old_start` up front, before any transaction**: it pins a snapshot at entry and requires `is_ever_pred(old_start, snap)` — else **`DefineError::OldStartNotEverRegistered`** — so a typo'd/non-def address can never seed a `supersedes` lineage that `current_version` would then happily walk (ever-registration, not *active* registration, is the right level: superseding a retracted-but-ever-registered def is legitimate lineage, PR4 presupposing only that the superseded address *is* a definition). It then registers `new_term` (define-style: insert + `register_pred`, sharing the up-front `TupParameter` rejection), and finally records `old→new` via **`emit(d, reserved_type(Supersedes), old_start, slice::from_ref(&new_start))`** — content-address endpoints (`from` the single `&Address`, `to` the one-element slice), the shipped `Supersedes` class reused — **not** M7's `assert_sup` (which requires resident *links*; def starts are content addresses). **The returned pair is the successor's identity (its content start) and the `supersedes` emit's commit `Seq`** — the third transaction's, not the insert's and not the successor's `pdef` emit's (the analogue of `define_predicate`'s Seq convention). `current_version(start)` = `tip(reserved_type(Supersedes), start)` → `Sink(head)` / `Indeterminate` (branch/cycle). The reference DAG is acyclic by registration order, so the lineage is a strict order with no cycle check. **Retry hazard (handed to the driving caller):** `supersede` is **three non-atomic transactions** (the successor's insert + `register_pred`, then the `supersedes` emit) — M7/M5 expose only transact-wrapped ops, so M9 cannot fuse them. A lost-ack retry re-inserts a *fresh* successor and emits a *second* `old→new'` edge, branching the lineage so `current_version` returns `Indeterminate` — a worse failure than `define_predicate`'s harmless-orphan retry. M9 offers no idempotency key; **retry-dedup / exactly-once is the duty of the driving coordination caller** invoking `supersede` (M9's upward consumer, out of corpus) — *not* M10, which never sees this call (M9 ∥ M10; M9's writes reach M5/M7's transact-wrapped ops directly). This mirrors the `create_new_document` *pattern* (no key; the dispatcher dedups), with the dispatcher being M10 there and the coordination caller here.

**Certification / retraction.** `certify_stable(d, start)` checks CVALID(0..iii) (§3): a `None` from `signature(start)` (CVALID (0)'s failure) maps to **`NotEverRegistered`** when `start` is not ever-registered and to **`UndisciplinedDef`** when it is ever-registered with a permanent poisoned memo entry (PR-DISC breach — the memo distinguishes the two `None`s, per the Discipline paragraph); the remaining rejections are `NotBoolean`/`NotActive`/`ViewDependent`/`NotStable`; then `emit(d, reserved_type(PredStable), start, &[])`. `retract_pred(d, start)` finds the active `pdef` tuple with **`observe(pdef, slice::from_ref(start), &[], Active).first().ok_or(RetractError::NotActive)?.addr`** (never `[0]`) and `nullify(d, that_addr)` — content untouched, audit retains it, re-registration after nullify deposits afresh (idem class now empty). De-registration **does not cascade** to referents.
...
pub enum FireAction {
    ...
    /// `ty` must be a cataloged idem⊤ Unary type — register_rule rejects idem⊥ (`NonIdemMarkerType`;
    /// the gap dedup-absorption and the Q3/I1a extinction analysis lean on idem⊤) — and NOT a
    /// PredLayer class (`pdef`/`pd_stable`) — PR-DISC (register_rule rejects).
    Marker { home: Address, ty: TypeKey },
    /// Single retraction: `nullify(home, a)` on the bound argument `a` — one atomic M7 `nullify`
    /// transact (H-ATOM/H-FIN exactly as Marker). idem⊤ (dedup keyed on (retracting home, target)
    /// — S3's from-fill; see the gap accounting below). NOT SF-certifiable (active-state trigger),
...
- **Marker shape** — a `FireAction::Marker { ty, … }` whose `ty` is not a **cataloged Unary** type is rejected here (`BadMarkerType(ty)`), turning a fire-time `Emit(ShapeViolation)` into a registration-time typed rejection; a cataloged Unary `ty` with **`idem = ⊥`** is rejected as **`NonIdemMarkerType(ty)`** — one finite catalog check that makes the gap analysis's dedup absorption and the certifier's Q3/I1a idem⊤ hypothesis *checked rather than assumed* (an idem⊥ Marker would double-deposit on a gap-planted witness — a redundant `Fired`, never a `Deduped`); and a `ty` that **is one of the two PredLayer classes** (`pdef`/`pd_stable` — both cataloged idem⊤ Unary, so the first two guards would admit them) is rejected as **`PredLayerMarkerType(ty)`**: PR-DISC reserves those slices for `register_pred`/`certify_stable`, and this guard closes the one in-module route by which a rule fire could deposit an unvalidated `pdef`/`pd_stable` tuple (§4 Discipline).
...
**Trigger evaluation** `T_ρ(x, snap)` = `eval`/`evaluate_def` of the trigger at `rule.view` with `x` bound (`Enabled.arg` — a `Value::Addr` for an `Addr` domain, a `Value::Tuple` for a `Tup` domain), off the snapshot — `eval` for the ref-free `Inline`, `evaluate_def` for `Def`. (Caller-supplied snapshots obey the **snapshot-freshness precondition** — §Public interface C: a snapshot handed to `quiescent`/`quiescent_scoped`/`next_enabled`/`step` must not predate any admitted `Def` trigger's registration commit, else `evaluate_def`'s `NotEverRegistered` is reachable inside a `bool`/`StepOutcome`-returning method — a precondition violation that panics like `decide`. `fire` pins its own fresh snapshot, which — registration having been validated against an earlier-observed commit, ever-registration monotone — always satisfies it.)

**Fire executor** `fire(Enabled{rule, x})`:
1. snapshot; **re-check domain membership** — `x ∈ [D_ρ]_snap`, the stored `TypedDom` enumerated off the fire snapshot; for a `Tup` argument membership keys on the tuple's address (R1 AddressInjectivity — the bound tuple's stored value is immutable, so an address hit is a value hit). Out ⇒ **`FireOutcome::NoOp`**: ASN-0133 defines a fire of `(ρ, x)` only at states with `x ∈ [D_ρ]_Σ`, so an argument removed in the peek→fire gap must not deposit — this NoOp *is* the fairness account's **removed** discharge. Then evaluate `T_ρ(x)`. False ⇒ **`FireOutcome::NoOp`** (absorption Q1 / **falsified-in-place** — never fire on a false trigger).
...
**Two-transaction race, exactly accounted (item 5).** The membership + trigger check (step 1, off `fire`'s own snapshot) and the deposit (M7's *separate* `transact` — M9 cannot nest a write inside its own read) are **two transactions**; the trigger — and, identically, domain membership — can flip in the gap (a concurrent writer plants the witness, or retracts the target). The gap never errors and never changes semantic outcome, and the `Deduped`/`Fired` accounting is exact against M7's actual contract:
- **Marker** — `ty` is idem⊤ **by `register_rule`'s guard** (`NonIdemMarkerType` rejects an idem⊥ catalog type, so the dedup argument is checked, not assumed): a gap-planted witness makes the emit a dedup **hit** — M7 returns the incumbent and commits nothing — reported **`Deduped`**.
- **Nullify, same-home gap retraction** — S3's hit branch keys on `coverage(F′) = subtree(d_retr)` ∧ `coverage(G′) = subtree(a)`, i.e. on **(retracting home, target)**: a prior gap retraction of `a` from the *same* `home` dedup-hits — **`Deduped`**, commits nothing.
- **Nullify, different-home gap retraction** — the target is already nullified yet still **resident** (the store is append-only; P-tgt is residence), so no `BadTarget`; but the emit **misses** (the hit key includes the retracting home) and deposits a **redundant second `[R]` tuple** — a real deposit, correctly reported **`Fired`** and counted. Benign in effect (the target was already nullified; the extra tuple only thickens the retraction audit), but *not* dedup-absorbed.

`fire` **reports the dedup distinctly**: an idem⊤ dedup hit (M7 returns an incumbent address and commits nothing) is **`FireOutcome::Deduped { effect, seq }`**, a fresh deposit **`Fired { effect, seq }`**. `fire` distinguishes them by the fire-snapshot status of the returned `effect` — an incumbent is already active (resp. already a live retraction) in `snap`, a fresh deposit's address is newly minted and absent from `snap` (exact absent concurrency; safe-direction under concurrency — at worst a gap-deposited witness is miscounted as a real fire, and the monitor is only a backstop). **Only `Fired` advances the divergence monitor's count** (§8); `Deduped` and `NoOp` do not — matching the journal truth, where a deduped emit leaves *no* record (the different-home redundant `[R]` deposit *does* leave a record and *is* counted, consistently).

Either real deposit is **one M2 transaction (m=1) ⇒ H-ATOM by M2's per-transact atomicity, H-FIN by single-deposit**; M9 takes the fire's `Seq` from M7's return. The `LinkStore` handle is minted off `Arc<Kernel<W>>` via the engine-injected factory. M7 checks the home is registered ⇒ **H-HOME**: an unregistered home maps to `FireError::HomeNotRegistered` (the shared variant — from `emit` for Marker, from `nullify`'s `NullifyError::HomeNotRegistered` for Nullify), never a silent skip; other emit/nullify rejections wrap in `FireError::Emit`/`FireError::Nullify` respectively. For a **Marker** (`ty` idem⊤ by `register_rule`'s guard), absent a concurrent depositor the emit is necessarily a *miss* (the true trigger certifies no covering audit tuple in `snap`), so it grows `L_K` and flips the audit-read trigger — extinction by construction; a concurrent witness instead yields a benign `Deduped` (the slice already grew). A **Nullify** fire instead *shrinks* an active slice, flipping an active-state trigger that PD0 does **not** make ⊥-stable, so it is uncertified (§8) and re-armable. v1 ships single-emit (Marker) and single-nullify; only multi-deposit *atomic* fires await M7's pure `stage_emit` (open).

*Recovery.* Fire effects are durable in M7's journal; the in-memory registry is re-registered on restart by the coordination layer. **SF/Marker** semantics make re-evaluation **idempotent** — already-fired arguments read their audit trigger as false, so no double-fire; a **Nullify** rule (uncertified) carries no such guarantee — its replay-time re-fire is bounded only by the divergence monitor (§8). `FireCounters` rebuild by replay (a `Deduped`/`NoOp` — or erroring — fire deposits nothing, so it leaves no journal record and is correctly excluded from the replay-recomputed count).
...
`next_enabled(&self, snap)` **peeks** an enabled `(ρ, x)` — some `x∈[D_ρ]` with `T_ρ(x)` true — but, taking `&self`, it **cannot advance a rotation cursor**, so it is not itself "fair": it is a pure candidate query. The `&mut self` **`step` driver owns the rotation** (round-robin/FIFO over the agenda, de-duplicating re-armed occurrences), and **weak fairness is a property of the `step` loop**, not of a single `next_enabled` call. A `fire` error surfaces from `step` as **`StepOutcome::Failed { rule, arg, err }`** — never a silent swallow (the design's own no-silent-skip demand: `HomeNotRegistered`, a Nullify rule's `BadTarget`, and M2-level failures all land here) — and the driver's rotation policy for a failing occurrence is **rotate past**: the cursor advances beyond it, so a persistently failing occurrence cannot starve the rest of the agenda, while the occurrence itself remains enabled and undischarged (a `Failed` fire committed nothing, so it is neither real-fired nor removed nor falsified in place); restoring the fairness account over it is the caller's — deregister or repair the rule on the `Failed` signal. `Failed` never advances the divergence count (nothing deposited, no journal record). Weak fairness *suffices to reach and hold* quiescence for the structural route: an all-SF, extinction-disciplined registry over **grow-only** domains on bounded input (Q6 regime (ii)-grow-only, Q5a). **Strong fairness** (turn-fairness machinery) is needed only for *non-grow-only* domains under an adversarial environment that cycles arguments out of phase; it is an open lever, not built in v1. **Design guidance to callers: keep domains grow-only and weak fairness is enough.** The scheduler/violation policy and the *driving* of the loop are explicitly handed upward (ASN-0133) — `step` is a default driver the coordination layer may replace.

### 8. Lint / certifier & divergence monitor

`certify_rule(rule)` is a static, sound-but-incomplete lint. **Input contract (its legs read checked artifacts, not the raw submission):** it re-runs `register_rule`'s normalization internally — the same domain-check + `Reg`-expansion into a `TypedDom` and the same trigger checks, one shared validation path — returning the same typed `RuleError` on a malformed rule (never a panic) and otherwise `Ok(RuleCertification)` computed over the checked forms (the `TypedDom`, the ref-free trigger):
...
- (b) action is the **Marker pattern** — a syntactic match that the emitted tuple's slot-coverage is exactly the witness the trigger's negated-existential quantifies over (canonical: trigger `¬is_K(a)` @ audit ⟺ `Marker{_, K}`); a Marker rule surviving the re-run validation already carries a cataloged-Unary, **idem⊤**, non-PredLayer `ty` (`BadMarkerType`/`NonIdemMarkerType`/`PredLayerMarkerType` reject otherwise), so leg (b) only verifies the witness-coverage match;
- (c) domain **grow-only**.
...
The **divergence monitor**: `FireCounters` per `(ρ, x)`. For an SF/Marker rule Q-EXT bounds each argument to one *real* fire (domain growth adds *new* arguments, never re-fires an existing one), so **count > 1 flags misbehavior for investigation — it does *not* certify it**: the flag is exact only in the race-free, single-writer regime, and a builder must wire it as a watchdog (surface, inspect, optionally quarantine the rule), **never an automated kill on count > 1**. Two documented routes over-count, and neither under-counts a genuine rule fire: (a) *live-side*, the `Fired`/`Deduped` discrimination is safe-direction under concurrency — a gap-deposited witness can be miscounted as a real fire (§Internal 5); (b) *replay-side*, journal attribution is a key match, and the key collides. **Replay attribution rule** (how the `FireCounters` recompute maps raw `LinkRec::Emit` deposits to `(ρ, x)`): a deposit is attributed to a Marker rule `ρ` at argument `x` when its type slot's coverage-class equals `coverage_class(ρ.ty)`, its home equals `ρ`'s `Marker.home`, and its F denotes exactly `{x}`; to a Nullify rule when it is `[R]`-classed with from-fill denoting `{ρ's home}` (equivalently homed at `ρ.home`) and G denoting `{x}`. The key is **not injective to `(ρ, x)`**: it collides (i) across two registered rules sharing `(ty, home)` — one deposit matches both — and (ii) with any non-rule writer emitting the same `(ty, home, x)` — e.g. a legitimate resurrection re-emission after an environment nullify: two genuine deposits under one key, zero rule misbehavior. Both routes err toward false alarms — the right direction for a backstop, the wrong one for a kill switch. Paired with the static cycle check, this is the cheap livelock watchdog. **Only real `Fired` outcomes increment the live count** — a `Deduped` (idem⊤ dedup hit in the trigger-check↔commit gap, no deposit), a `NoOp`, and an erroring fire (surfaced as `StepOutcome::Failed`; nothing committed) do not — so the live count tracks the journal's deposit record (a deduped emit leaves no record), and the journal recompute agrees with the cached count up to the collision routes above.
...
- **Expansion deterministic & well-typed at `C_D`** — immutable content + content-deterministic naming from the reserved `VarId ≥ EXPANSION_NAME_BASE` supply, minted along the fixed traversal order (reference nodes bottom-up, siblings left-to-right; at each node parameters first in signature order, then binding sites depth-first, left-to-right) (PR3, PR3a).
...
- **One shared type registry; M9 builds none** — M9 receives the engine-built `Arc<TypeRegistry>` behind the genesis-sealed config and projects its static catalog from it; no second deterministic build, no divergence trap (engine-injection, §Core data model). The projection is **validate-once-or-fail at `Coordinator::new`** (`CatalogError`: `ReservedMismatch`/`DeclNotInRegistry`), so residual drift of the twice-passed `(reserved, decls)` against the injected registry is caught at assembly, never as a spurious `UnregisteredType` at type-check.

**By active enforcement** (M9 must guard, at the named site):

- **Γ_D is part of the checking judgment** — `type_check`/`type_check_trigger` are *given* the ordered Γ_D and seed Γ with it; a free `Var` outside it is `UnboundVariable`; every **define-path** Γ_D parameter sort must be a codomain (a `Tup`-sorted parameter is `TupParameter`, since `evaluate_def` binds values, never a tuple), while the **trigger-path `type_check_trigger`** admits one `Tup` parameter (a tuple-domained rule binds a `Value::Tuple`, ASN-0133 ρ_R — so a `Tup`-domained rule needs an `Inline` trigger, a `Def` signature being Codom-only); the `TypedTerm` carries Γ_D so `define_predicate`/`evaluate_def`/ST⁺ have their context; and `define_predicate`/`supersede` **re-reject a `Tup`-parameter `TypedTerm` up front, before any insert** (`DefineError::TupParameter` — a `type_check_trigger`-built term can carry one, and a `register_pred`-only catch would first commit orphan content), with the def codec refusing to *encode* `Sort::Tup` in a parameter context, so the Codom-only invariant holds at encode time as well as at registration (ASN-0129 WT; ASN-0130 SignedTerm/PR5).
...
- **`Reg` admissible under exactly two formers:** `Forall`/`Exists` (where `type_check` expands it away) and `Count` (which folds to a `Lit`) — every other `Dom`-accepting former (`Reflect`, `MaxT1`, `MinT1`, `BigUnion`, `Filter`) rejects a `Reg` domain at the element-sort check, and `Reg` has no term form so never reaches `SetTerm` or any value position (ASN-0129 V-IDX/PC2a; §Internal 1).
...
- **Single-coherent-pre-state for every multi-read verdict** — read all constituents off one M2 `Snapshot`; sites: `eval`, `quiescent[_scoped]`, `register_pred`/`certify_stable` validation (PC4/ASN-0134 clause 6, ASN-0130 single-coherent-pre-state). One stated refinement: the signature legs (`register_pred` (iii), CVALID (0)) may recurse into memo-missing `signature(r)` calls that pin their **own** snapshots — sound because the σ-pinned ever-registration gate runs first, ever-registration is monotone, and signature facts are content-intrinsic (§Internal 4).
- **Rule-engine caller-snapshot freshness (`Def` triggers)** — a `Snapshot` handed to `quiescent`/`quiescent_scoped`/`next_enabled`/`step` must not predate any admitted `Def` trigger's registration commit (ever-registration is monotone, so any snapshot pinned after the last Def-trigger `register_rule` qualifies; `fire` pins its own fresh snapshot and is exempt); a staler snapshot makes `evaluate_def`'s `NotEverRegistered` reachable inside a `bool`/`StepOutcome`-returning method — a precondition violation, panics like `decide` (§Public interface C; §Internal 5).
...
- **`signature` memoizes `Some` and poisoned entries only** — ever-registration is monotone and content immutable, so a `Some` is permanent and cacheable forever, and an ever-registered-but-unparseable start (PR-DISC breach) is memoized as a **permanent poisoned entry** — a deliberate **freeze-on-breach policy**: exact for a parse failure (content immutable, unhealable), a chosen freeze for a `DanglingReference`-caused WT failure whose never-registered referent could later be registered (safe — the start merely stays signature-less, never a wrong `Some`); a never-registered `None` is transient and never memoized, else a later registration goes unobserved. The miss-path derivation recurses through referent signatures (WT-ref, well-founded by PR2). The `DefMemo` is interior-mutable behind this `&self` API (a `RwLock`/concurrent map — never `RefCell`, which is `!Sync`).
...
- **Fire atomicity/finiteness/home; domain re-check; two-transaction race exactly accounted** — one M7 `emit` *or* `nullify` per fire; M7 checks the home (Q's H-ATOM/H-FIN/H-HOME). `fire` re-checks `x ∈ [D_ρ]` at its own snapshot before the trigger (ASN-0133's fire relation is defined only on domain members; an out-of-domain argument NoOps — the fairness *removed* discharge). The membership + trigger check and the deposit are two transactions; the gap accounting is exact (§Internal 5): a Marker gap-witness dedup-hits (`ty` idem⊤ by the `NonIdemMarkerType` guard) and a *same-home* gap retraction dedup-hits (S3's hit key is (retracting home, target)) — each reported `FireOutcome::Deduped` (commits nothing) and **excluded from the divergence count**, which counts only real `Fired` (item 5); a *different-home* gap retraction leaves the target resident yet misses, depositing a redundant real `[R]` tuple — reported `Fired` and counted, benign (the target was already nullified); a fire *error* surfaces from the `step` driver as `StepOutcome::Failed` (rotate-past, §7), never a silent skip.
- **At-most-once per argument** — by construction for SF+Marker (Q-EXT); for non-SF rules (every `Nullify` among them), an obligation the divergence monitor watches (recomputable from the M7 journal under §8's attribution key — over-counting only, in its documented collision cases — where `Deduped`/`NoOp`/`Failed` leave no record, so a cache-untouching `&self` `fire` is still covered). For a `Tup`-domain rule the bookkeeping key/report is the tuple's `t.addr` (R1 AddressInjectivity; §Core data model).
...
- **M1** — address/span value ops behind `Endset`/coverage handling and the def-identity (`shift` for any byte-granular run; `validate` to lift a raw set-element `Tumbler` to `Address` at the evaluator's binding sites — quantifier/fold binding over materialized `AddrSet` domains and T1-extrema, §Internal 2; infallible on store-minted addresses. M9 never calls M7's §G discovery reads, so no §G lift arises).
...
**Downstream seam (the coordination surface M9 exposes upward, out of corpus):** the three capability groups of §Public interface. The contracts a consumer codes against: (1) `type_check` once with the def's Γ_D (or `type_check_trigger` for a tuple-domained rule's trigger — the only PL context that binds a `Tup`), then `eval`/`decide` are pure ref-free verdicts "as of `snap.seq()`" (`classify` per-view); (2) `define_predicate`→identity-by-start (an up-front `TupParameter` rejection; a benign retryable `BadPosition`/orphan-content under concurrency), `evaluate_def` keyed on ever-registration (an undisciplined start is a defined `UndisciplinedDef`), `supersede`/`current_version` for lineage (`old_start` gated ever-registered up front — `OldStartNotEverRegistered`; the returned `Seq` is the `supersedes` emit's; 3 non-atomic txns — retry-dedup is the **driving coordination caller's**, not M10's: `supersede` reaches M5/M7 directly, never via M10), `certify_stable`/`is_certified_stable` for the cached stability verdict; (3) `register_rule` (validated: the domain checked + `Reg`-expanded to a stored `TypedDom`, ref-free — `RefBearingDomain`; ref-free `Inline` via `type_check_trigger` or ever-registered `Def`, one-`Bool`-param trigger sort-matched to the domain element sort — a `Tup` domain requires an `Inline` trigger; a `FireAction::Nullify` rule must be tuple-domained or `Addr`-over-`L_dom`; cataloged-Unary idem⊤, non-PredLayer `Marker.ty`) + `certify_rule` (re-runs the same validation; typed `RuleError` on malformed), `quiescent[_scoped]` as a decidable done-verdict any party can run from state+registry alone (scoped: exact for a sort-homogeneous scoped set, else a safe-direction over-approximation; caller snapshots obey the `Def`-trigger freshness precondition), `next_enabled`/`fire`/`step` as the (replaceable) driver whose weak-fairness guarantee lives in `step`'s rotation, with fire failures surfaced as `StepOutcome::Failed` (rotate-past — deregister/repair is the caller's) and the explicit "reaching quiescence is conditional" boundary. The activation binding (who may register rules), bounded-input workloads, the scheduler/violation policy, and stochastic rule bodies are *handed further up* — M9 supplies the mechanism, not the policy.
...
7. **Type-registry build site: M9-built (`TypeRegistry::build`) vs engine-built shared instance.** Resolve to **one engine-built `Arc<TypeRegistry>`** injected at `Coordinator::new` and shared with the genesis-sealed config and M9's `LinkStore` handle factory; M9 builds **no** second registry, only its static `TypeCatalog` as a projection of the injected one. This removes the redundant deterministic build and the divergence trap if `(reserved, decls)` ever drift, and the same injection supplies the `Vstream`/`LinkStore` op-handle factories so M9 names no unpublished upstream constructor. Residual drift — the `(reserved, decls)` pair is still passed twice, once to `LinkState::genesis` by the engine and once to `Coordinator::new` — is caught at assembly: the catalog build validates the pair against the injected registry and fails construction (`CatalogError`) on a mismatch, mirroring genesis's validate-once-or-fail.

8. **BH2 atom admission (ASN-0128: any registered Walk class) vs M7's v1 walk service (`succs`/`chain`/`tip` served only for shipped `Supersedes` — empty/trivial for any other `ty`).** An app `TypeDecl` may attach Walk at genesis, so registration-driven admission would let PL terms type-check whose walk atoms silently denote `∅`/`[]` at every state — ASN-0128's BH2 semantics contradicted without a signal. Resolved M9-locally (the cheapest honest fix): the catalog guard admits BH2 atoms only at the `Supersedes` key, rejecting any other Walk class with the typed `UnservedWalkClass` naming the v1 limitation; the guard lifts when M7 serves app-declared Walk classes.

---

## Open build decisions

- **The def byte format** — length-prefixed envelope (recommended) vs self-terminating grammar; must be prefix-free, self-delimiting, decidable, **injective** (PR-ENC), reject reserved-range `VarId`s on decode, refuse `Sort::Tup` in a parameter context on encode, and encode the **compact** pre-`Reg`-expansion body (item 4). **And n=1 single-`Val` (recommended here) vs a byte-granular multi-address run** if a future content model wants token-level addresses.
