...
| `age(K,a)` / `stale(K,h)` | `is_active(a) ∧ is_k(K,a) ? M7::age(a) : ⊥` (ASN-0129 BH4 totalization — M7's `age` is type-agnostic) / `M7::stale(K, h as u64)` (Nat→u64 at the seam). Dormant in v1: no BH4 type cataloged ⇒ `Age`/`Stale` cannot type-check |
...
- **BH4 totalization at the dispatch** — `age(K, a)` returns `M7::age(a)` only when `is_active(a) ∧ is_k(K, a)`, else `⊥`; a `stale` horizon narrows `Nat → u64` at the seam (ASN-0129 BH4; dormant in v1).
...
- **`L_dom` membership/emptiness spelled through the `Dom` formers** — `t ∈ L_dom` is `∃ over LinkDom`, emptiness `¬∃` (or `count ≤ 0`), **never `count = 0`** (PD0 leaves `count = c` unclassified, so `= 0` silently loses the `SF` precision a `¬∃` keeps) (§Internal 1; PD0).
