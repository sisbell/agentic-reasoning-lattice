...
/// inhabit it, so PR-ENC's body-binder-disjointness ("no recorded parameter name and no body binder
/// may inhabit" the expansion supply) is enforced structurally, not merely intended.
...
- **(iii)** WT+WT-ref under σ (`IllTyped`); each referent `r` ever-registered via `is_K(pdef, r)@audit` (= `!observe(pdef, slice::from_ref(r), &[], Audit).is_empty()`; else `ReferentNotEverRegistered`); sigs via the resolver.
...
- `expand(a)` — **for evaluation, never materialized** (the *reference*-expansion, distinct from `Reg`-expansion): `evaluate_def` drives the denotation by **DAG-recursion** — its driver is `eval`'s ref-free walk (§2) *plus the one `Ref` arm `eval` lacks*: at a `Ref{addr,args}` node, resolve `addr` (M4 content read), evaluate `args` to *values* (recursively, so nested refs are handled), and denote the referent's `Reg`-expanded body in a fresh env bound to those values (denotation is compositional → sound, no flat string built). Content reads (resolution) and structural reads (denotation) stay distinct passes, so the denotation remains content-free — *this* is why self-hosting doesn't re-open the structural-reads-only surface, and why `eval` itself can keep its ref-free precondition. **For certification, the flat `expand` is built** (§3) with a **content-deterministic fresh-name counter** — one counter per top-level expand, advanced across the whole traversal, never reset per sub-call, never global — drawing names from the reserved `VarId ≥ EXPANSION_NAME_BASE` sub-range (§Core data model), which no recorded parameter nor body binder inhabits, so the renamed expansion names are disjoint from every host binder *by construction* (PR-ENC's reserved supply); two evaluators thus obtain the same concrete term (the cross-evaluator agreement that licenses shared caching). De Bruijn / locally-nameless + hash-consing is the alternative if a flat form must be cached.
...
pub enum FireAction {
    ...
    /// (`a = t.addr`, itself a resident link) or `Addr`-over-`L_dom` (link addresses); an
    /// `Addr`-over-`M_K` domain binds from-side MEMBER addresses (not links), every fire then
    /// tripping `FireError::Nullify(Rejected(BadTarget))` — a DOCUMENTED CONTRACT, not a
    /// `register_rule` check (see register_rule below).
...
- **Nullify domain (documented contract, not enforced)** — a `FireAction::Nullify` rule's domain must yield **resident-link** addresses: a `Tup` domain (`a = t.addr`) or an `Addr` domain over `L_dom`. An `Addr`-over-`M_K` (member-address) domain is *not* rejected here — "yields a link address" is not statically decidable for an arbitrary `Filter` domain — but every fire then trips M7's `BadTarget`, surfaced as `FireError::Nullify(Rejected(BadTarget))`. Keep `Nullify` rules tuple-domained (or address-over-`L_dom`) so the rejection is structurally impossible.
...
- **Ref-free for every non-def evaluator** — `eval`/`decide`/`classify` and `TriggerRef::Inline` require `is_ref_free`; `register_rule` rejects a ref-bearing `Inline`; all ref-bearing terms route only through `define_predicate → evaluate_def`'s resolve-then-denote passes, keeping the denotation content-free (the persist-as-def escape for a ref-bearing trigger is Codom-param-only; a tuple-domained ref-bearing trigger must inline its helper, §Internal 5) (ASN-0130; PC4).
...
- **`Nullify` rule domain yields resident links (documented contract)** — a `FireAction::Nullify` rule must be tuple-domained (`a = t.addr`) or `Addr`-over-`L_dom`, so the bound argument is a resident link; an `Addr`-over-`M_K` (member-address) domain is *not* rejected at `register_rule` ("yields a link address" is undecidable for arbitrary `Filter` domains) but every fire then trips M7's `BadTarget`, surfaced as `FireError::Nullify(Rejected(BadTarget))` (M7 BadTarget; §Internal 5).
...
- **Reserved expansion-name range** — reference-expansion fresh names are drawn only from `VarId ≥ EXPANSION_NAME_BASE`, a range no recorded parameter name and no body binder inhabits, so PR-ENC's body-binder-disjointness is structural (PR-ENC; §Internal 4).
...
- **The def byte format** — length-prefixed envelope (recommended) vs self-terminating grammar; must be prefix-free, self-delimiting, decidable, **injective** (PR-ENC), and encode the **compact** pre-`Reg`-expansion body (item 4). **And n=1 single-`Val` (recommended here) vs a byte-granular multi-address run** if a future content model wants token-level addresses.
