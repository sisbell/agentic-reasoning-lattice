...
- **The result-constructing algebra is partial: level-uniform, level-compatible only.** Every theorem that *builds* a span — intersection, merge, split, difference, normalization-existence — requires `#start = #width` per span and a shared tumbler length across operands, because each routes through WF, which needs `#s = #r`. Comparison, convexity (S0), the five-case classification (SC), and normalized-form uniqueness (S9) rest on the total order alone and carry no level precondition. Cross-level operations are *out of scope*, a hard boundary, not a convenience the implementer may relax. The builder consequence: the level gate belongs *after* classification, *before* construction — the classifier itself doesn't need it. *(Forced.)*
