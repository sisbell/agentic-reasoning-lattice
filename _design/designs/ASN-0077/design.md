...
- The *granularity* of the report — the enumerated set itself is fixed by the postcondition (O1.2's multi-origin diagnostic needs to know *which* documents are present, so a "mixed" sentinel would not satisfy it); the only genuine choice is whether to layer a summary on top of that set.
...
Recommendation: enumerate for narrow, boundary-walk for wide; treat any 2-D index as recomputable from the address and reconcilable against it (throwaway hint or trusted-but-validatable cache, your call). The single-origin weakest-precondition characterises exactly the narrow, document-scoped queries that stay within one document, so a fast path — "do the span's endpoints share a document prefix, with known single-document allocation between?" — short-circuits to a singleton without enumeration in those cases.
...
**Permanence / recovery — falls out, isn't built.** Origin permanence is not a feature; it is a consequence of append-only content allocation with no address reuse (P0/P3). If content writes go to an append-only store — Green's **permascroll/granfilade**, where content addresses (cf. Q19's `2.1.0.x.x.x` permascroll addresses) are assigned at write time and never reused — every address is stable and origin (a function of the address) is automatically permanent. The only promise to actively keep is *never reassign or reuse a content address*. No SHOWORIGIN-specific durability machinery is needed.
...
- **Single-origin fast path.** Whether to detect and short-circuit a single-document span — typical of narrow, scoped queries — before enumerating.
