...
- Optionally maintain an aggregate frontier at the home node as a *cache* — a *hint* in Lampson's sense, check-and-recompute (the reference keeps an O(1) maintained width aggregate at the apex, albeit whole-store, not per-type); recompute on a miss. Add only if allocation latency demands it.
...
- Clause 5 buys the writer-side half of run-atomicity and no more: a perfectly contiguous run still *passes through* every interior state, each canonical (A6) yet carrying a strict prefix of the run's atoms (A5/W2), so a reader can land mid-run on a valid-looking partial state. Making the run appear all-or-nothing *to a reader* is the separate, harder contract of OQ4, above the substrate.
- Hold a per-`(d, s_C)` critical section for the run, or — cheaper — reserve `m` consecutive slots in one frontier-advance under the per-home lock, then fill them; on the reserve path the fill must commit atomically (no reader may observe the interval until it is full), else a reader sees reserved-but-empty addresses — states not on `𝔼`, contradicting A0/A6. Keep content and link subspaces on *disjoint* allocators so a link emit cannot fragment a content run (the tight exclusion scope); fusing them, as the reference does, inherits a coarser whole-home exclusion.
