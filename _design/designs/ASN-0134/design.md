...
- Optionally maintain an aggregate frontier at the home node, but be clear which kind. An *eagerly-maintained authoritative-derived* aggregate — propagated on every deposit, so always exact and never stale (the reference keeps exactly this: an O(1) width aggregate at the enfilade apex, maintained after each insert, albeit whole-store rather than per-type) — buys O(1) reads at the cost of the maintenance write on every allocation. A *Lampson hint* — a possibly-stale value verified against the store on use and recomputed when wrong — is a different thing and does *not* fit a frontier aggregate, since the coupled allocator needs the exact maximum at deposit time, not a check-and-recompute guess. Add the eager aggregate only if allocation latency demands it.
