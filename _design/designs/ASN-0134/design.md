...
The decisive payoff is **clause 7**. The note frames the multi-read verdict as needing a *global* reader-side critical section — but that blocking is an artifact of a per-type read API that re-reads live state between calls. **Hand the reader a snapshot handle (a captured immutable root) and run all `p` constituent reads against it; clause 7 collapses into clause 4 and costs no *coordination*** — the persistent-structure and version-retention overhead remains (it is exactly the cost Green's mutable route avoids, below). This is the biggest Lampson win in the design: the "global" *blocking* was misplaced in the API, not inherent in the contract.
...
- Multi-read verdict soundness (V2) — clause 7 (cross-type joins **and** `stale`, even single-home); *free* under root-capture, else enforced.
