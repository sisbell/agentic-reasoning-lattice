...
- **Ownership is a theorem about addresses, not stored state.** The two-place predicate `owns(π, a)` is decided by comparing two tumblers (`pfx(π) ≼ a`) — no registry, table, or history consulted (O1). *Forced.* Consequence: there is no ownership table to keep consistent, migrate, replicate, or corrupt. The *containment check* `owns()` can be evaluated anywhere — at the edge, in any layer — without a round trip to authoritative state; effective-ownership *authority* (`ω(a) = π`), by contrast, does consult the principal registry and stays central.
...
- **Bootstrap initialization (O14)** — seed an initial principal set that is nonempty, finite, account-tier, injective, valid, pairwise non-nesting, covers the initially-baptized addresses, and whose every prefix is itself baptized (in Σ₀.B).
...
- *Option A — label the baptism journal.* Since every prefix is already a baptized address, mark the baptism records that are delegations/bootstrap (carrying the delegator id if you want it) and treat the in-memory principal set as an *index recovered by replay*. This is the journal→derived-index pattern (an append-only event log replayed into an in-memory registry), and it matches udanax-green, where account creation inserts the prefix into the granfilade *in the same operation* (`insertseq`) — there is no separate registry.
...
- **Session↔principal binding, and whether a session may name only its own prefix.** This decision sizes the *subdivision-authority* surface (O5), not the existence of the resolver. If a session may name only its own prefix as the allocation hint, **O5** holds by construction (Green's model), and a principal can authorize writes *within its own undelegated subtree* by containment alone. It does **not** make `ω` unnecessary: any principal sitting above a delegated account still needs longest-match — a node session with `pfx = [1]` gets `owns([1], [1,0,2,0,3]) = TRUE` by containment, yet `ω` of that address is the delegate `π_A` (worked example, Σ₁), and authorizing by containment there reproduces the divergence the note exists to prevent. Containment ≈ `ω` only in a flat, account-only hierarchy (Green's actual reality, not this note's multi-tier model). The live fork is therefore the resolver structure *for `ω`* (its own decision below) plus this prefix-naming policy that collapses O5 — not "`ω` vs. no-`ω`." Decide the prefix policy early.
...
- **Concurrency / serialization point:** Green serialized via a single-threaded loop, making baptize+register trivially atomic and "≤ 1 principal per transition" automatic. A concurrent build must choose a serialization point — the journal append is the natural one.
