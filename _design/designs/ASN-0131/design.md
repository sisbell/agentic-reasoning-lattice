...
- **Identity is withheld; the answer is non-followable.** *(Forced — this is the operation's whole reason for being.)* The link address never reaches the answer, so a surfaced from-endset cannot be paired with its link's to-endset. *(Conventional — provisional, Open Question 2:)* the adopted answer further *deduplicates* — two distinct links bearing the same endset value in the same slot collapse to one `(role, endset)` pair, which makes link multiplicity unrecoverable. OQ2 reopens whether multiplicity must be preserved; the incremental-refcount tension noted below is contingent on this dedup and dissolves if multiplicity is kept.
...
- **The region must lie in the content subspace (`∀ v ∈ W : subspace(v) = s_C`).** *(Forced — a caller obligation, **not** a check the operation performs.)* It places the image in content (`I ⊆ dom(Σ.C)`, S3★), and that is the premise the retraction from-set/to-set disjointness and the net-removal-only result below rest on: those hold *because* the image is content. Link-subspace regions (`W ⊆ s_L`) are **out of scope (Open Question 7)**; green accepts them but returns an *empty* result rather than crashing, so a builder must either trust the caller or document the empty/undefined behavior.
...
On the growth side, RE-ADDR settles emission cleanly: a freshly-emitted *non-retraction* link is immediately addressable in its post-state (`ℓ_new ∉ nullified(Σ')`), so ordinary emission can only *add* `(role, endset)` pairs — the answer is monotone under non-retraction emission, and only retraction can remove a pair.
