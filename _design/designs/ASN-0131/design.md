...
And RE-EDIT/RE-CWP are the cache's **invalidation rules** — they tell you, *before* recomputing, whether an edit can move the answer at all. RE-EDIT classifies every transition: edits to other documents, content allocation `K.α`, entity creation `K.δ`, provenance writes `K.ρ`, and link-subspace-confined edits on `d` (`K.μ⁺_L` and content-retaining `K.μ⁻`) leave `RE` *fixed* — skip the recompute outright. RE-CWP handles the one hard case, a contraction, with a *checkable, pre-edit* precondition: `∀ available (i,e): coverage(e) ∩ Δ ≠ ∅ ⟹ coverage(e) ∩ I_R ≠ ∅` (with `Δ` the dropped I-addresses and `I_R` the retained image) — when it holds, the deletion provably leaves `RE` unchanged and the recompute can be skipped without touching the post-state.
