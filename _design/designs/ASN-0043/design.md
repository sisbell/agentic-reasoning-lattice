...
- **Identity is by address, never by value.** A link's identity is the address the allocator minted for it. There is no find-or-create, no deduplication, no injectivity constraint: two links with identical from/to/type endsets are distinct objects, independently owned and removable. The payload never participates in identity — the allocator assigns it, and that is deliberate.
- **Links live in a distinct, disjoint address subspace `s_L`.** The address is self-describing: the subspace field of a link's own tumbler suffices to tell it apart from content, making a side flag *unnecessary in principle*. Disjointness from content's `s_C` is then automatic (T7) — though only over content the content layer actually keeps in `s_C` (see *Guarantees to uphold*). A redundant leaf-type tag is nonetheless permitted, and recommended until the global `s_C`-residence contract lands (Physical layout, decision 8). Subspace separation supplies the *disjointness* half of the dual-primitive architecture (L14); the "no third stored category" half is a separate claim.
...
- **Links are immutable and append-only.** Creation is the only write; the store grows monotonically, and no entry is ever changed or removed — the per-transition invariants here are L12/L12a (links), with S0/S1 and document monotonicity carrying the same discipline for the other stores. The whole-state `StateExtension` relation `⊒` is the *definition* that names this append-only discipline across all three stores — the lemmas build on it; it is not itself an invariant.
- **A link is a slot-indexed sequence of ≥ 3 endsets, with a non-empty type endset; an endset is a *set* of spans.** Slot index (from/to/type) is a primitive of the model; span *order within* an endset is not — access is by membership only. Link equality is component-wise. The type slot must carry at least one classifying address: a conforming store rejects an empty or absent type endset at create. This `e₃ ≠ ∅` conjunct is the conformance boundary between the model and the reference implementation, which silently stores a two-endset link when handed an empty type specset (see decision 6).
...
- **Links are non-transcludable.** Content is shareable across arrangements; links are not. In any `s_C`-resident system, no arrangement may map a position to a link address — non-transcludability inherits the same content-side `s_C`-residence contract as subspace disjointness (discharged via S3 + L1d(b) over the `s_C`-slice).
...
- The standard triple and slot numbering (from=1, to=2, type=3); the model admits arity > 3 (the reference *API* takes three specsets, though it stores fewer when the type specset is empty — see decision 6).
...
- A **link search index**: given a slot and a query span, find links whose endset in that slot *overlaps* the query, and intersect across constrained slots by link identity. It must also report match *cardinality* cheaply (count) and enumerate matches in a *stable order* with a resumable cursor (paginate) — the five-operation FEBE set the note names includes count and paginate, not just find. The note specifies no ordering over link-match results, so the result order is a builder choice (decision 9); an address-derived order is a reasonable, stable default, but it is not a grounded commitment.
...
- A **conformance suite** is already supplied by the note: its worked example — a grounded single-link state plus a six-step extension — exercises every state-local L-invariant that carries a per-state check, non-vacuously after the six-step extension (non-injectivity, link-to-link addressing, arity-4, type discrimination, multi-span coverage-equality); the META properties L4 and L7 carry no per-state check and are omitted. Use it directly as the acceptance vector for the store, allocator, and index.
...
The tempting wrong turn is to **key the store by a hash of the endset value** (value-addressing). It is forbidden here: identity is by address, and the store must not *enforce* injectivity (non-injectivity is permitted — L11b), so hashing the endset value would collapse distinct links into one. Links are precisely the case where you must *not* derive the key from the payload — the allocator assigns it.

**Address allocator — stateless query-and-increment by default; cache a high-water hint only when it proves hot.** Green's proven approach is a *stateless* query-and-increment: the next address is `max existing sibling under home.0.s_L.* + 1` (first link seated at the document's link-subspace child), computed fresh each time, with no counter object. Make this the default: its virtue is Lampsonian — the store *is* the counter, so there is no duplicate state to drift — and its only cost is a "max-under-prefix" query per allocation. If that query proves hot at scale, add a per-home high-water map as the optimization — but as a **hint rebuilt from the journal on load** rather than authoritative state, making the common case (allocate) O(1) while the journal remains the source of truth: an index/registry as a recomputable hint over the authoritative journal. Either way:
...
| **No orphaned links** (L12b) — a link's home stays allocated forever | *Construction* — documents are append-only. |
...
| **Non-transcludability** (L14a, under `s_C`-residence) — no arrangement points at a link | *Active enforcement, **and not here*** — see below. |
...
9. **Pagination order over link-match results.** The note fixes no ordering on the search/paginate results, so the order in which `FINDNEXTNLINKS` enumerates matches is yours to choose — an address-derived order is a reasonable, stable default, but it is a builder choice, not a grounded spec commitment.
