...
- **Links live in a distinct, disjoint address subspace `s_L`.** The address is self-describing — a link is told apart from content by the subspace field of its own tumbler, not by a side flag. Disjointness from content's `s_C` is then automatic (T7). This *is* the dual-primitive architecture.
...
| **Subspace disjointness** (L1d, L14) — links and content never collide | *Construction* — if the allocator respects subspaces, T7 gives it for free. |
...
6. **Arity policy and where the conformance gate lives.** The model admits N ≥ 3; the reference path fixes 3 — *and can silently emit a two-endset / empty-type link* when the client sends an empty type specset (verified). A conforming store excludes these, so decide explicitly to reject a missing or empty type endset **at the create boundary** (the right place to put the check) rather than letting non-conforming links into the store.
