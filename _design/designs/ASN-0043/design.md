...
- A **conformance suite** is already supplied by the note: its worked example — a grounded single-link state plus a six-step extension — exercises every L-invariant non-vacuously (non-injectivity, link-to-link addressing, arity-4, type discrimination, multi-span coverage-equality). Use it directly as the acceptance vector for the store, allocator, and index.
