...
The note *derives* "append-only log" from S0 + S1, so the structure chooses itself: **an append-only journal of content writes, recovered by replay on load.** Atomicity and crash recovery come from the journal for free (write the record, then it's durable; a torn tail is discarded on replay). The shape is proven on both sides: Green's permascroll *is* this, and this repo's own spec-authoring tooling runs the same append-and-replay shape — though its `links.jsonl` journal is a pipeline artifact, offered as an analogy, not a component of the engine being specced. Take it.
...
- **`Val` is heterogeneous and untyped in storage.** The note leaves `Val` opaque and asks (open question 1) whether entries are uniform; the Green evidence settles the engineering answer — text content and link objects both sit in the store as plain untyped tumblers, with the *kind* of content carried by the referencing V-position's subspace (`1.x` text, `2.x` links), not by any tag on the value. Store no runtime type discriminator on content; this is the other half of why the store stays value-oblivious (S4) — it cares about neither a value's bytes nor its type.
...
- **Store arrangements as run-lists, not per-character maps.** Storage cost then scales with the number of *distinct contiguous transclusions*, not with character count: pasting a long quote is one run. Concretely, give each run `(I-start, length)` and hold the runs with *implicit* V-positions — a run's V-start is the prefix sum of the preceding run lengths, not a stored key — in a balanced tree carrying subtree length-sums. That gives `O(log #runs)` positional lookup *and* `O(log #runs)` splice with no renumbering of the positions that follow. A plain per-position persistent map is simpler and makes S2 trivially true, but it is wasteful at scale — pick it only for a prototype.
...
- **Journal arrangement edits too,** alongside the content journal. The same edit-journal-plus-registry shape that this repo's spec-authoring tooling uses (`links.jsonl` as edit journal, `paths.json` as registry — pipeline artifacts, offered as a proven-here analogy, not as parts of the Xanadu engine) generalizes directly. Recover everything by replay.
...
- **Content typing:** untyped values with type carried by the V-subspace (recommended — Green's choice, keeps the store value-oblivious) vs. a tagged `Val` that carries its own type. The note leaves `Val` open; the subspace-carries-type answer is the one the evidence supports.
