...
- **Address-decoded derivation and fork-rank hold only under VD (version-namespace discipline).** [forced conditional] The address always encodes *allocation lineage*; reading it as *derivation* (or the final component as a *fork* rank) requires that nothing but VERSION allocate into a version namespace. ASN-0047's J4 is a live counter-client — itself a version-producing composite, not *this* operation, allocating into the very same version sub-allocator — and any discipline-conforming allocation may take the frontier, so one interleaved non-VERSION allocation gives the k-th fork a rank > k and silently corrupts the decoding. This is OQ1.
...
- **VERSION mints exactly one identity; the node-tier cross-owner case is scoped out.** [conventional / scoping] P-tier admits the cross-owner branch *only* for an account-tier forker. A node-tier non-owner would have to mint both an account and a document under it, so the note pushes that case outside VERSION's domain — a revisable choice to keep the operation single-mint, not a physical impossibility (VERSION *could* be defined to mint the pair).
...
- **Link carry-through (refraction)** — "a link to one version is a link to all versions" holds by construction from the shared content addresses, with no fork-time work. It is **arrangement-driven and continuous**: discoverability is computed at *query time* from `ran(M(v))=A` via LP12 against the **unchanged** link store, not captured at the fork. So it is **total over content anchors** (V2b excludes link-subspace anchors), it **covers links created after the fork**, and it is **bidirectional** (a shared anchor makes the link discoverable from source and version at once). It is **conditional**: if the version later contracts off the anchor, the link ceases to be discoverable *from the version* while persisting in the store and remaining discoverable from the source. Build no fork-time link capture or link-migration — carry-through is a consequence of shared addresses, not an action of the fork.
...
- **Hands to** link operations (the link store, projection LP12): the fork supplies the shared anchors that make carry-through total over content anchors, but does *zero per-link work* — carry-through is a consequence of the shared addresses, not a migration.
