...
**Certification** is a second validated surface depositing a `pd_stable` certificate — a cached, permanent verdict that turns "is this stable?" into a one-atom downstream query. The stability checker is a syntax-directed walk over the *expanded* term; its internal algorithm is explicitly uncommitted, and its soundness is an inherited open question. Treat it as sound-but-incomplete: an uncertified definition is *unknown*, not unstable. One part of the certified *meaning* (not the open algorithm) the checker must honor: ST⁺'s per-parameter reading widens the dynamics aggregate rule's count-threshold from "ℕ literal" to "ℕ literal *or* environment-bound ℕ parameter" — a checker built to literal PD0 would silently *refuse* a certifiable term such as a view-independent `count(L_W) ≥ x` with `x : ℕ` a parameter.
