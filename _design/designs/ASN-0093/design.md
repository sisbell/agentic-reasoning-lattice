...
- The specific identifiers `s_C=1, s_L=2` are pinned for Green-compatibility; load-bearing are their *distinctness*, their *fixedness*, and that they are *consecutive integers each reachable by `inc(·,0)` off the previous anchor* (`s_L = s_C+1`, so `b_L(d) = inc(b_C(d),0)`) — the property that lets the allocator produce every anchor with its own inc-steps and underwrites the C1c/L1c conformance witnesses. Exactly two subspaces is an axiom — a third (`s_C+2`, anchor `inc(b_L,0)`) needs it revisited.
...
- **A document registry** — records which document addresses are allocated, answers "is `d` registered?", admits fresh well-formed registrations (enforcing the document-level shape: `T4-valid`, `zeros(d)=2`), never forgets; reserves a (currently empty) arrangement slot per document.
...
- **Hands up to:** the **arrangement-mutation layer** that will populate `M(d)` (the reserved, currently-empty slot); **entity allocation** layered on `K.σ` (which originates the document addresses `K.σ` currently only validates); **provenance** (a new `R` component recording who allocated what); **coupling constraints** binding allocation to arrangement — Green's `CREATELINK` (`docreatelink`) *fuses* the link I-space allocation with the POOM/arrangement insertion (`docopy`) in one operation, whereas `K.λ` here does only the allocation and holds `M` in frame (`M(d)=∅`), so a builder porting from Green must not re-fuse them or assume `K.λ` touches `M`; and **link withdrawal** — a retraction mechanism that must revisit L12's value-equality clause. (Green has no withdrawal at all: its spanfilade is write-only and there is no `DELETELINK`, so this is genuinely new ground above the substrate.)
