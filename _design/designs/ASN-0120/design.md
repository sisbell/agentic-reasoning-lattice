...
- *Persistent ordered map keyed by V-position* (e.g. an `im` balanced-tree map). A range scan over the span interval yields the active positions and their I-addresses directly, in V-order. Cheap, simple, and exactly enough for MAKELINK, which only ever needs the V→I direction at creation. (It does *not* group the results into I-chain-adjacent runs — that costs the adjacency test noted under packaging below.)
...
Pick by where the reverse mapping belongs. The arrangement is **shared document state** — one `Σ.M(d)` per document, mutated by insert/delete/rearrange and read by both MAKELINK (V→I) and the read side (I→V) — so MAKELINK builds nothing and the representation choice belongs to the document subsystem. The write side needs only the **V→I direction**, for which an ordered map suffices; forcing an enfilade just to resolve at creation drags a read-side mechanism (the I→V direction) into a write-side operation for no gain. Let the read side own whatever bidirectional structure FOLLOWLINK and discovery need over that same arrangement. (Verified Green behavior worth inheriting: resolved I-widths trace *exactly* allocated content and never reach past the allocation frontier — your range scan should clip to active positions, never round up to chain boundaries.)
