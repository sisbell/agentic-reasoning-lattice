...
- **Discoverability is by content identity and is decoupled from residence — and it is not a separate action.** A link is findable from any region any endset references, regardless of home. Critically, the note is emphatic that discovery is *the standing meaning of having content-identity endsets in the store*, not an indexing step MAKELINK performs.
...
Merely *conventional* (not forced): the first-link V-position depth (the note fixes `m=2` but says any `m≥2` serves); the choice of span decomposition; the *semantic meaning* of from/to in any given case (Nelson: "what 'from' and 'to' mean depend on the specific case"); and, for a one-sided link, *which* slot is left empty (both `(∅, e₂, e₃)` and `(e₁, ∅, e₃)` are legal). The slot→role *binding* is forced, not conventional — the recorded order fixes which region is the from-side (the ordered-endsets commitment above).

## What must be built

Functionally, an implementation honoring this note must provide six MAKELINK-internal components:
...
- **A link writer.** Record `link-address ↦ (from, to, type)` permanently — keyed by the home-scoped link address just minted, not by the home document (`Σ'.L = Σ.L ∪ {a ↦ (e₁, e₂, e₃)}`; one home document accumulates a growing *set* of links).
...
Discovery is *not* a seventh component. The link must be findable from any content region any endset references, with home ignored — but this is a **read-side capability the recorded endsets enable for free**, the standing meaning of having content-identity endsets in the store, not an indexing step MAKELINK performs. Writing the endsets *is* discharging discoverability; the discovery subsystem (covered under Implementation approaches) realizes the query as a view over what MAKELINK already recorded.
...
- *Persistent ordered map keyed by V-position* (e.g. an `im` balanced-tree map). A range scan over the span interval yields the active positions and their I-addresses directly, in V-order — cheap, simple, and all the V→I direction MAKELINK ever touches at creation. (It does *not* group the results into I-chain-adjacent runs — that costs the adjacency test noted under packaging below.)
...
But the choice is not MAKELINK's to make. The arrangement is **shared document state** — one `Σ.M(d)` per document, mutated by insert/delete/rearrange and read by both MAKELINK (V→I) and the read side (I→V) — so **MAKELINK imposes no representation of its own**: it inherits whatever bidirectional structure FOLLOWLINK and discovery already require over that same arrangement, and consumes only its V→I direction. The "ordered map" above is the *floor of MAKELINK's requirement* — a V→I range query is all the write side demands — not a structure to stand up beside the read side's. Standing up a separate write-side V→I map would duplicate authoritative state for no gain; forcing an enfilade just to resolve at creation would drag a read-side mechanism into a write-side operation for equally no gain. The representation belongs to the document subsystem, settled by the read side's bidirectional need; MAKELINK just reads through it. (Verified Green behavior worth inheriting: resolved I-widths trace *exactly* allocated content and never reach past the allocation frontier — your range scan should clip to active positions, never round up to chain boundaries.)
...
- *Merged canonical spans* — fold *I-chain-adjacent* resolved addresses (each `a_{k+1} = inc(a_k, 0)`) into one wider span to save room. Legal **only** when every chain address in the run is resolved; one unresolved or frontier member would make the span cover unresolved content and break the recovery equation.

Recommendation: if you merge, group by maximal *I-chain-adjacent* resolved runs — break at every I-discontinuity and never extend onto the unallocated frontier. Two cautions: the break is on **I-adjacency, not V-scan order** — a single fully-active V-span over content transcluded from two origins resolves to two non-chain-adjacent I-runs (the note's M16 CrossOriginMergeImpossibility), which cannot be one canonical span and must split at the discontinuity; and with the plain V→I ordered map this grouping is *not* free — it costs a sibling-step (sig-valid) test between consecutive resolved I-addresses (only the enfilade does the run-grouping for free, one region per contiguous I-run). If that bookkeeping isn't worth it, default to the **reference decomposition**, which is immune to all of this. Either way, document that decomposition is observable on raw read-back, so two coverage-equal links are not value-equal. (Green's `vspanset2sporglset` is deterministic given arrangement state but explicitly *non*-canonical — fragmentation reflects insertion history. That is fine here and is consistent with decomposition being unobservable to the coverage-determined views.)
...
- *Recompute the high-water mark on recovery* — the next address is `max(existing links homed at d) + 1`, derivable by replaying the link journal. This makes the counter a **hint**, not authoritative state — preferred. Green effectively does this (scan for the previous homed link's I-address and increment).
- *Persist a per-home counter* in a registry. Faster, but now it is authoritative state you must keep consistent with the journal across crashes.

Prefer the hint: the link journal is already the source of truth, so derive the counter from it.

**Link write + home seating (atomicity).** The link store never mutates entries, so writing the link is an **append to a journal** (whatever append-only store the repo provides). The home seating is the *one* in-place arrangement edit, and it is append-at-the-link-subspace-extent, so it never shifts existing positions (Green confirms this shift is structurally a no-op). Derive the seat from the *arrangement's* link-subspace extent, never from the allocation counter — the address selector (store/journal high-water mark) and the seat selector (arrangement extent) are independent. They agree at a link-virgin home but **diverge at a contracted home** — links still homed while the link subspace has been emptied by a retention-0 contraction — where the address takes the subsequent-emission branch and the seat takes the first-position branch; collapsing the two would mis-seat the link there. The composite has an intermediate state, so make the two effects crash-atomic with **one journal record** capturing both the link value and the home binding; apply to in-memory state, then fsync; recover by replay. This is plain write-ahead logging and needs nothing fancier.
...
Either way the index is a **hint recomputable from the link journal** — so it needs no durability of its own, a miss can fall back to a scan, and rebuild-on-load is free. Key it by the *resolved I-addresses* and answer by **point-membership**: the discovery test reduces to the exact-set intersection `ρ(R_i,Σ) ∩ ran(Σ.M(d'))` — a querying document's arrangement can never reach a *proper descendant* of a resolved root, so every match lands on the root itself and a span-overlap query never arises (interval-vs-exact representation is a compactness tradeoff, not a query-capability one). And **never key it by home** — that single discipline is what upholds discoverability-decoupled-from-residence. The crucial architectural stance: MAKELINK's contract is discharged by writing the endsets; index maintenance lives in the discovery subsystem, eager or lazy, as a view.
...
- **Frame (no collateral mutation)** — by construction. MAKELINK creates no content, entity, or provenance, and edits *only* the home document's link subspace; linking into a region modifies nothing of that region. With residence/application orthogonality, this is what makes annotation of published, un-owned content possible — and it forbids the conventional-hyperlink reflex of writing a back-reference into the *target* to make the link findable: discovery is via the content-identity index, never by mutating the target.
