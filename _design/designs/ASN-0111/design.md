...
- Whether to allow arity > 3: the model *permits* it; Green caps at 3. Capping is a legitimate simplification but a real narrowing of the note. Green diverges on the *lower* bound too — it permits 2-endset links and an absent or empty type slot, not merely a ceiling of 3 — so honoring the note's mandatory non-empty type is a conscious model-fidelity choice, not something the reference gives you for free.
