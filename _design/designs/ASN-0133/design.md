...
- **Termination is conditional, with hypotheses named.** Recognizability and absorption are unconditional; *reaching* (and holding) quiescence follows from any of **three sufficient packages**, not one conjunction — **(i)** the environment eventually idles (*any* registry, no extinction or SF); **(ii)** grow‑only domains + all‑SF + extinction + bounded growth, under *weak fairness alone* — the structural route, *no* environment hypothesis; or **(iii)** non‑grow‑only + all‑SF + extinction + bounded growth, which still needs H‑SFAIR or per‑rule environment‑idle *to reach* (Q5a bounds the work regardless). The note pins exactly which buys "registry inert," "reached and held," or nothing.
...
- **Reaching quiescence** — *route‑dependent*. The engine always guarantees the *registry‑side* half — finitely many real fires, inert past the last — under bounded input + fairness. *Reaching and holding* a quiescent state then follows from regime (i) (environment idle, any registry), regime (ii) (grow‑only + all‑SF + extinction + bounded growth, under *weak fairness alone* — **no** environment hypothesis), or regime (iii) (non‑grow‑only + all‑SF + extinction + bounded growth, needing H‑SFAIR or per‑rule environment‑idle *to reach*). Only regime (i) and regime (iii)'s extra demand sit outside the engine; regime (ii) is structural.

So recognizability, absorption, and observer‑uniformity are free; at‑most‑once, atomicity, and finiteness need enforcement; reaching quiescence is route‑dependent — structural (regime ii) for a grow‑only all‑SF registry under weak fairness, environmental otherwise.
