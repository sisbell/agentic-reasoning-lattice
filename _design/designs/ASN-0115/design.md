...
- **Span width is a tumbler boundary, not a count.** A span names a half-open V-interval, not a quantity of positions or bytes; the delivered quantity for a spec is `|act|` — the number of *active* positions — so delivered byte-length is *not* the sum of the spec-set's V-widths (Green's boundary clip makes the two diverge). The count falls out of the interval meeting the arrangement, never from the width.
...
**Repeatability — free against an immutable snapshot.** Because the arrangement is the sole mutable input and the stores never change what they hold, persistent maps make repeatability a non-theorem: a "version's arrangement" is an immutable map *value*, and re-running delivery against the same value is bit-identical by construction. Against *live* state, repeatability holds only while the consulted slice is unedited — so offer callers a way to **pin an arrangement snapshot** (a cheap handle to a persistent value) when they need stable re-delivery, while the default reads current state per the spec. One caveat on the direction of the guarantee: R7 is a *sufficiency, not a biconditional* — equal arrangement restriction ⟹ identical delivery, but identical delivery does *not* imply an unchanged arrangement, since S4 permits rebinding to a distinct, equal-valued address that leaves the bytes identical while the restriction changed. So key any delivery cache on the consulted arrangement *value*; never infer cache validity from, or invalidate it by, output byte-identity.
...
- **The one legitimate failure** is a precondition, not a delivery fault: the named document must be *allocated* (`d ∈ dom(Σ.M)`). Everything past that gate degrades to partial delivery, never to failure. Green *additionally* fails when the document is not *open* — a session/access gate that lives outside the note's model; treat it as an implementation choice (see "Decisions for the builder"), not a contract guarantee.
