...
- Fixing `m_1 = 2` — *load-bearing for the recommended implementation, not a throwaway scoping point.* The foundation permits deeper text subspaces; this ASN scopes them out, and a builder may honor only depth 2 and still be complete for 0084 (the Green evidence confirms this is the real-world reality — text content is fixed at depth 2 by `findisatoinsertmolecule`, never deeper). Why it matters downstream: the note's identification of singleton tumblers `[k]` with naturals `k`, together with D-SEQ giving `V_1(d) = {[S, 1..N]}`, is exactly what collapses a text V-position to a single integer ordinal — the precondition for the prefix-sum/sequence/rope model recommended below. The out-of-scope `m_1 > 2` case (multi-component ordinals, no flat `1..N` range) would not admit that clean prefix-sum representation without rework.
