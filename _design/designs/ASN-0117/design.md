...
- **Backtrack support**: retained arrangement history beyond the content store, sufficient to reconstruct a prior placement — the bytes (but not the old mapping) persist in the store, so the old mapping is exactly the state that must be retained elsewhere.
...
- *Relative-displacement balanced tree (the udanax enfilade).* Store each node's V-displacement relative to its parent; absolute position is the cumulative sum down the path. A uniform suffix shift then adjusts the displacement of one boundary subtree rather than every leaf — Green uses exactly this representation (the shift is applied to boundary crums, then rebalance), and it makes "carry the I-address along" automatic: the leaf is untouched, only a V-coordinate offset moves. Under a *persistent* version of this tree the shift is O(log N) work and O(log N) new nodes — but that is efficiency to build in, not behavior Green demonstrates: Green's intersection-finder is stubbed to always return the root (so it never makes the O(log N) descent) and it mutates in place with height-collapse disabled (so it is not persistent). **Pick this** at scale, especially with frequent edits near the front, and because it makes retained-version snapshots cheap (below). Cost: substantially more machinery (cut/slice at boundaries, recombine, the displacement algebra).
...
- **Within-document sharing:** decide whether the model allows one document to arrange the same I-address at multiple V-positions; if so, the discovery post-filter and any index retraction must reason about "last witness" (the `A_del^{excl}` wp, Approach §3), not per-pair removal.
