...
- **Classification is at I-address-set granularity.** **Forced** by `R`'s granularity, not by the predicate forms: the foundation's provenance relation is over element×document pairs and carries no occurrence-level record, so SHOWDELETIONS cannot see per-occurrence removal even in principle. Only `a ∈ ran(M(d))` — set membership, never V-position multiplicity — is therefore available to read. Per-occurrence removal stays invisible while any occurrence of `a` survives, and the note deliberately scopes that finer operation out as a Vstream concern needing finer state.
...
| **Faithful DELETED semantics** (DELETED means "was present and removed") | inherited (P4a) — every `(a,d) ∈ R` witnesses a real prior arrangement; active downstream — `R` must be appended only for genuine placements, never speculatively, or the word DELETED stops meaning what it says |
