...
What is merely conventional: the specific numbering `s_C = 1`, `s_L = 2`, and the common working depth of 2. What is forced: the span-set result type, purity, two-kinds-only, arranged semantics, the allocation gate, and reliance on contiguity for exactness.
...
**Option 3 — A maintained per-document extent summary, as a recomputable hint.** Carry `(n_{s_C}, n_{s_L}, m_{s_C}, m_{s_L})` and have every arrangement-mutating operation update it; the query reads it in O(1). This is exactly udanax-green's proven approach — `setwispnd` propagates a bounding-box aggregate to the POOM root on every edit, and the query reads the root with no cache layer, so deletions are reflected immediately. The Lampson discipline is essential here: treat the summary strictly as a **hint** — never journaled as authoritative, **recomputed from the arrangement on load** — so it carries no recovery burden and a drift bug degrades performance, not correctness. Pick this only when the query is hot *and* you control every editing path. The cost is coupling: this note's summary now rides along with every editor.

**Option 4 — Free-ride an existing aggregate.** If the arrangement is itself stored as an enfilade-style structure that already maintains per-node V-extent aggregates for other operations (Green's case — the bounding box serves many callers, so its maintenance cost is amortized), then the extent is already computed; just read it. This is the best option *if you were going to pay for the aggregate anyway*; it is over-engineering to introduce such a structure solely for this query.

My recommendation: ship Option 1, reach for Option 4 if your arrangement representation already carries the aggregate, and only escalate to 2 or 3 under measurement.

**The allocation gate.** Test membership against the document registry — the analog of this repo's `paths.json` registry and of Green's granfilade registration (Green's `findorgl`/`checkforopen` path returns failure, the `?` marker, for an unregistered id, while an existing-but-empty document returns the empty result). The load-bearing requirement: **registration must be a journaled event separate from content writes**, so "no arrangement entries for `d`" (allocated-empty → `⟨⟩`) is distinguishable from "`d` was never allocated" (→ fail). Conflating them by treating "no journal entries" as "doesn't exist" is the tempting wrong move.

**Recovery.** The query itself has no recovery story — it is a pure read. Its inputs ride the standard substrate: an append-only arrangement journal plus a document registry, recovered by replay on load (the `links.jsonl` + `paths.json` pattern). Any extent hint (Option 3) is rebuilt during that replay, never replayed as truth.
