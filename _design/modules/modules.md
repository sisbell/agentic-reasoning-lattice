...
- **Seams:** Hands fresh, T4-valid, owned addresses plus "is allocated?"/"who owns?" answers to every store and operation; persists registry + frontier via M2; deliberately decoupled from content (ghost elements). CREATENEWDOCUMENT registers a document in the entity registry but does **not** materialize its (empty) arrangement — that lives in M5 and is left implicit/lazy (M5, via `M5 → M3`, distinguishes registered-empty from unallocated). This diverges from ASN-0103's eager-`M(d)=∅` recommendation, which would write M5 from M3 and close an `M3 → M5` cycle.
...
- **Responsibility:** Own each document's mutable V→I arrangement (the POOM) — the only layer where *destructive* change lives — together with the append-only content provenance relation R (permanent history, not part of the mutable presentation, co-located with its writers), and provide the editing/versioning operations.
...
- **Key components:** link store (L) + endset/link value; type-by-coverage matching; MAKELINK / READLINK / FOLLOWLINK; Emit / Observe / Nullify + active vs audit slices; nullified/tombstone set; shape gate + type registry (shape/idem/behaviors); de-duplication; behavior *declarations* and the forward behaviors (determinate-walk over forward adjacency, read-filter, age); supersession claims.
- **Seams:** Allocates link addresses via M3, resolves V-region endset args & seats home links via M5's arrangement API, commits via M2; hands the canonical store, active/audit slices, and behavior declarations to discovery (M8) — which executes typed-reverse-lookup over its spanfilade — and the typed-relation read surface (Observe) plus the forward behaviors to coordination (M9).
...
- **Key components:** spanfilade content-coverage index (per-slot, hint); project / coverage / discoverability; findlinks (four-set match); typed-reverse-lookup execution (BH3, over the spanfilade); count; windowed enumeration (identity cursor); RETRIEVEENDSETS; the V→I image primitive (via M5).
...
- **M7/M8 link boundary.** Type registry, shapes, dedup, behavior *declarations*, and the forward behaviors (determinate-walk, read-filter, age) are in M7; indexed *query execution* — findlinks/count/window plus typed-reverse-lookup (BH3) — is in M8. `Observe` (the by-pattern read of the typed-relation surface) sits in M7 and still overlaps `findlinks` (M8) conceptually. This remains the softest seam in the design; the whole link subsystem could also collapse to one module.
