...
Distinctness from every *other* document address — present or future — splits into a same-chain and a cross-chain case. *Same-chain:* `A_doc(A) = S(A, 2)` is a SiblingStream whose enumeration is strictly increasing under T1, hence injective (S0, StreamOrdering; ASN-0040), so distinct stream positions name distinct addresses and no document baptised under `A` — earlier or later — coincides with `d`. *Cross-chain:* we must show `d` differs from every document-level address (`zeros = 2`) not on `S(A, 2)`, and this requires first establishing that every such address lies on *some* SiblingStream other than `S(A, 2)` — an exhaustiveness fact that the B7 discharge presupposes but does not itself supply. We derive it. Let `b` be any document-level entity with `b ∉ S(A, 2)`. Since `Document(b)` holds, `b` is non-node, so ActivatedEmission (ASN-0047) places `b ∈ dom(A_b)` for some activated entity-level sub-allocator `A_b`. By the AllocatorHierarchy taxonomy (ASN-0047) the entity-level sub-allocators are exactly `A_account(N)`, `A_doc(A'')`, and the version chain `A_v(d_src)`, emitting `zeros = 1`, `zeros = 2`, and (as members of `E_doc`) `zeros = 2` respectively — these are the only entity-level allocators, and only the latter two emit `zeros = 2`. Since `zeros(b) = 2`, `A_b` cannot be an account allocator, so it is `A_doc(A'')` for some account `A'' ≠ A` or `A_v(d_src)` for some document-level `d_src`. Each is a SiblingStream — `A_doc(A'') = S(A'', 2)`, `A_v(d_src) = S(d_src, 1)` — with a B6-valid parent–depth pair `(p', d')`: document chains satisfy `zeros(A'') + (2 − 1) = 2 ≤ 3`, version chains `zeros(d_src) + (1 − 1) = 2 ≤ 3`. With exhaustiveness in hand, we discharge all of them with a single B7 instantiation: for *every* B6-valid `(p', d') ≠ (A, 2)`, `S(A, 2) ∩ S(p', d') = ∅` (B7, NamespaceDisjointness; ASN-0040), since `(A, 2)` differs from every other parent–depth pair. As `d ∈ S(A, 2)`, it collides with none. The same taxonomy governs *future* baptisms: any later document-level allocation is again an `A_doc` or `A_v` emission (`K.δ` case (ii)), so it lands on a stream disjoint from `S(A, 2)` and cannot reuse `d`.
