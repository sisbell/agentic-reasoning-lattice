...
The foundations state SubAllocatorBundle only for the *document* tier, so `Activated(A_doc(A))` is not derivable from them; we take it as owed by out-of-scope account provisioning.
