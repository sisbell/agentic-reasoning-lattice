...
Existing addresses, meanwhile, remain valid: the allocation only adjoins `d` and reuses nothing (`E ⊆ E'`). An empty document is still in `E`, so the next allocation reads the live high-water mark and steps over it; there is no mechanism by which a later document is baptised onto an already-occupied position, since same-chain injectivity (S0 above) already forbids any two `S(A, 2)` emissions from coinciding. This is the abstract content of Nelson's permanence guarantee:
