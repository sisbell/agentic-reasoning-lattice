...
This is the account-level analogue of SubAllocatorBundle (which activates a document's `A_C(d)` and `A_L(d)` at its own allocation event; ASN-0047). The foundations state SubAllocatorBundle only for the *document* tier, so `Activated(A_doc(A))` is not derivable from them; we take it as owed by account provisioning — structurally, since owning an account *is* the authority to fork documents beneath it ("full control over its subdivision forevermore", 4/29). CND.A-act is what lets Effect One's `d ∈ S(A, 2)` discharge ActivatedEmission for `d` below.
...
*Freshness.* The address is new: `d ∉ E`. The clean closure is uniform across *every* entity type and needs no case split. Effect One placed `d` on the stream — `d ∈ S(A, 2)` in both branches (the first emission `inc(A, 2)` is the stream's head; the subsequent emission `inc(d_prev, 0)` lands on it with `d_prev = max(D_A) ∈ S(A, 2)`) — and gave `d > max(D_A)` when `D_A ≠ ∅` (vacuously `d ∉ D_A` when `D_A = ∅`), so in either branch `d ∉ D_A`. Since `D_A = E ∩ S(A, 2)` (proven above), `d ∈ S(A, 2) \ D_A = S(A, 2) \ E`, whence `d ∉ E` — against nodes (`zeros = 0`), accounts (`zeros = 1`), documents, and versions at once. (Equivalently and more coarsely, `zeros(d) = 2` already excludes every node and account; the stream membership closes the residual document and version cases.) What remains — the permanent distinctness of `d` from every *future* allocation — is the separate argument we now give from S0 and B7. The cited freshness lemmas of ASN-0093 (FirstEmission, ChainEnumerationInjectivity) are stated only for the content and link sub-allocators `A_C(d)`, `A_L(d)`; the document sub-allocator `A_doc(A)` is not in their scope. But `A_doc(A) = S(A, 2)` is a SiblingStream, so it inherits the requisite properties directly from ASN-0040. Its enumeration is strictly increasing under T1 (S0, StreamOrdering; ASN-0040): for a subsequent emission `d = inc(d_prev, 0) > d_prev ≥ e` for every prior `e ∈ D_A`, placing `d` strictly past the entire existing document chain; for the first emission `D_A = ∅`, so `d = inc(A, 2)` is the stream's first element and no document under `A` precedes it. Distinctness from the version chains is structural rather than ordinal: each `A_v(d_i) = S(d_i, 1)` is a *distinct namespace* from `A_doc(A) = S(A, 2)` — the parent–depth pairs `(d_i, 1)` and `(A, 2)` differ, and both are B6-valid (`d_i` is a T4-valid document, so `zeros(d_i) + (1 − 1) = 2 ≤ 3`; `A` an account, `zeros(A) + (2 − 1) = 2 ≤ 3`) — so `S(A, 2) ∩ S(d_i, 1) = ∅` by namespace disjointness (B7, ASN-0040). Hence `d` collides with no version address, present or future. Distinctness from every *other* document-chain emission — present and future — is likewise unconditional, but its source is S0, not B8: both `d` and every other document baptised under `A` are elements of the single stream `S(A, 2)`, and S0 (StreamOrdering; ASN-0040) makes that enumeration strictly increasing, hence injective, so distinct stream positions name distinct addresses. We deliberately do *not* route this through B8 (Uniqueness; ASN-0040): B8's same-namespace branch carries the precondition "committed under a single baptismal authority (so B-Seq applies)", which this state model never explicitly discharges; S0's injectivity over `S(A, 2)` needs no single-authority premise at all. Cross-account collisions are excluded directly by namespace disjointness — the same lemma that separates `d` from the version chains. For any other account `A' ≠ A`, the parent–depth pairs `(A, 2)` and `(A', 2)` differ, and both are B6-valid (each account is T4-valid with `zeros = 1`, so the depth-2 bound `zeros(·) + (2 − 1) = 2 ≤ 3` holds), so `S(A, 2) ∩ S(A', 2) = ∅` by namespace disjointness (B7, ASN-0040). Every document baptised under a distinct account inhabits a disjoint stream, so `d` collides with none — unconditionally, without any global appeal. In particular we do not need GlobalUniqueness (ASN-0034), whose precondition demands that the entity allocations form a T10a-conforming allocator tree, a premise this state model never discharges; B7 delivers cross-namespace distinctness with no such obligation. Nor do we route through partition independence (T10): its non-nesting premise `A ⋠ A' ∧ A' ⋠ A` does not follow from `Account(A') ∧ A' ≠ A` alone — `Account` fixes only `zeros = 1` and T4-validity, so e.g. `[N, 0, 5]` and `[N, 0, 5, 3]` are both accounts yet nest. B7 sidesteps that premise entirely.

*Strict advance over every prior address under `A`.* We claim `d` strictly exceeds every document address ever baptised *on `A`'s own document chain* — every emission of `A_doc(A)` and every version forked from one (precisely the `v_{#A+1} = 0` entities, as we shall see) — including documents created but never written into. Document-level entities that extend `A` but lie *off* that chain are deliberately excluded from the dominance claim; we show separately that they are distinct from `d` by divergence at position `#A+1`, which is all that freshness and uniqueness require. For same-allocator (document-chain) emissions the dominance is S0 above.

We isolate the fact both the empty-chain and non-empty-chain cases consume:

  **(FirstForkRoot)** *Let `v` be an on-chain version under `A`: `Document(v)`, `A ≼ v`, `v_{#A+1} = 0`, `#v > #A + 2`. Then `v` carries `0, i` at positions `#A+1, #A+2` for some `i ≥ 1`, and the root document `d_i = [A, 0, i]` lies in `D_A`.*

*Proof.* `v` is created by a `K.δ` case-(ii) `k=1` fork off a document operand (ASN-0047), and its chain may iterate further `k=1` forks and `k=0` sibling steps. Every such step freezes positions `1..#A+2`: a `k=1` fork appends one component and agrees with its operand on all prior positions (TA5(b), ASN-0034), while a `k=0` sibling step modifies only position `sig(·)`. The equality `sig(·) = #(·)` is not TA5(c) but TA5-SigValid (SigOnValidAddresses, ASN-0034): every chain operand is a baptized entity of `E`, hence T4-valid (S7d/M0; B10/B6, ASN-0040), so its final component is non-zero and `sig = length`. Once a version has been forked, each operand has length `≥ #A+3`, so each `k=0` step modifies `sig(o) = #o ≥ #A+3 > #A+2` — positions `#A+1, #A+2` are never touched. Those two positions therefore hold throughout the chain the values the chain's *root* document carries.

That root is a true document `t`, reached by isolating the *first* `k=1` fork in `v`'s ancestry; `t` is its operand, with `Document(t)` since `t ∈ E_doc`. By the same freezing argument applied from `inc(t, 1)` onward (each step agreeing with its operand on prior positions, each `k=0` step touching only `sig(o) = #o ≥ #t+1 > #t`), `v` agrees with `t` on positions `1..#t`. Were `#t < #A`, then since `A ≼ v`, `t` would agree with `A` on `1..#t`, i.e. `t ≼ A`, forcing `zeros(t) ≤ zeros(A) = 1` and contradicting `zeros(t) = 2`. Hence `#t ≥ #A`, and agreement on `1..#A` gives `A ≼ t`. Being the first such fork, no `k=1` step precedes `t`, so `t`'s derivation from `A` uses only the depth-2 descent and `k=0` sibling steps. Reaching `zeros(t) = 2` from `A` (`zeros(A) = 1`) requires exactly one `k=2` step (`K.δ-ID.zeros-2`, ASN-0047; `k=0` preserves zero-count, B5a, ASN-0040), and it is off `A` itself — a `k=0` step off `A` would differ from `A` at position `#A`, breaking `A ≼ t` — so it is `inc(A, 2) = [A, 0, 1]` of length `#A+2` with parent `A` (`K.δ-ID.parent-2`), the trailing `k=0` steps preserving length (TA5(c)) and parent (`K.δ-ID.parent-0`). Hence `#t = #A+2 ∧ parent(t) = A ∧ Document(t)`, placing `t ∈ D_A`; by `D_A = E ∩ S(A, 2)`, `t = [A, 0, i]` with `i ≥ 1`. Entity permanence (P1, ASN-0047) keeps `d_i = t` in `E`, so `d_i ∈ D_A`; and since positions `#A+1, #A+2` are frozen at the root's values, `v` carries `0, i` there. ∎

The version case now splits on whether `A` already has documents. *First case (`D_A = ∅`).* On-chain version dominance holds vacuously: were `v` an on-chain version under `A`, FirstForkRoot would give a root document `d_i = [A, 0, i] ∈ D_A`, contradicting `D_A = ∅`. So no on-chain version exists, and `d = inc(A, 2)` dominates them all vacuously. Off-chain entities (`v_{#A+1} ≥ 1`) are distinct from `d = [A, 0, 1]` by divergence at `#A+1` (`0 < v_{#A+1}`), exactly as in the subsequent case, and fall outside the dominance scope. *Subsequent case (`D_A ≠ ∅`).* For a version `v` we cannot appeal to T9 (ForwardAllocation), which orders only same-allocator pairs: `v` lives in `A_v(d_i)`, a different allocator from `A_doc(A)`. We argue directly by lexicographic order. Every `A_doc(A) = S(A, 2)` emission has the canonical SiblingStream form `[A, 0, j]` (SiblingStream postcondition `cₙ = [A, 0, n]`; ASN-0040). Write `d = [A, 0, p]`. Take any document-level entity `v` — `Document(v)` (so `zeros(v) = 2`) with `A ≼ v` — and split on its component at position `#A+1`.

*Off-chain (`v_{#A+1} ≠ 0`).* Since `d_{#A+1} = 0`, `d` and `v` agree on positions `1..#A` and first diverge at `#A+1` with `0 = d_{#A+1} < v_{#A+1}`; hence `d < v` by T1 case (i) (ASN-0034), so `d ≠ v`. CND.monotone makes *no* dominance claim over such `v`: distinctness by divergence at `#A+1` already discharges freshness, the only property the operation needs from these entities.

*On-chain (`v_{#A+1} = 0`).* This is the scope of CND.monotone's dominance. With `A ≼ v`, `zeros(A) = 1`, and `zeros(v) = 2`, the inherited zeros of `A` occupy positions `1..#A` and the lone additional zero sits at `#A+1`; the remainder is zero-free, so `v = [A, 0, w]` with `w` zero-free and `#w ≥ 1` — exactly the form of an `A_doc(A)` document (`#w = 1`) or a version forked from one (`#w > 1`). If `#v = #A + 2` then `v = [A, 0, j]` is itself a document-chain emission, dominated by `d` via S0 (same allocator). If `#v > #A + 2`, then `v` is an on-chain version, and FirstForkRoot gives `v_{#A+2} = i` with `d_i = [A, 0, i] ∈ D_A` for some `i ≥ 1`. Then `d_i ≤ max(D_A) = d_prev = [A, 0, p−1]` gives `i ≤ p − 1 < p`. So `d` and `v` agree on positions `1..#A+1` and at position `#A+2` carry `p` versus `i < p`; hence `d > v` by T1 case (i) (ASN-0034), regardless of how deep `v` nests. The frontier counter at position `#A+2` dominates every version counter beneath an earlier document. The high-water mark is read from the live entity set — an empty document is still in `E`, so the next allocation steps over it — and there is no mechanism by which a later document is baptised onto an already-occupied position: a later document is a higher-index emission of `S(A, 2)`, which S0 (StreamOrdering; ASN-0040) places strictly above every earlier emission, so it cannot coincide with one. This is the abstract content of Nelson's permanence guarantee:
...
**State preconditions** (against pre-state `Σ`):
  ...
  - the invoking principal `π` satisfies `pfx(π) ≼ A` — it owns the account (O1, PrefixDetermination; ASN-0042). Subdivision authority (O5; ASN-0042) is a stated modeling assumption deferred to the registry-carrying ASN (see Open Questions).
...
*Address permanence (T8, ASN-0034) and distinctness (S0, B7, ASN-0040).* Every previously valid address remains valid (`E ⊆ E'`, `dom(C) ⊆ dom(C')`) by T8, and `d` collides with no existing or future address: same-chain emissions by S0 (StreamOrdering; ASN-0040 — the `S(A, 2)` enumeration is strictly increasing, hence injective), version chains and cross-account documents by B7 (NamespaceDisjointness, disjoint streams `S(A, 2)` vs `S(d_i, 1)` and `S(A', 2)`), and content/link addresses by zero-count (`d` carries `zeros = 2`, they carry `zeros = 3`). The document's identity is permanently distinct from every other document, including ones created later. The proof route is S0 + B7, as established in Effect One.
...
| CND.pre | Preconditions: A ∈ E ∧ Account(A); the invoking principal π owns the account (pfx(π) ≼ A, O1; ASN-0042). Subdivision authority (O5; ASN-0042) is a stated modeling assumption deferred to the registry-carrying ASN (see Open Questions). No content argument | introduced |
...
| CND.own | Ownership is structural (derivable over (C,L,E,M,R)): parent(d)=A and A ≼ d (every A_doc(A) emission has form [A,0,j]), so with pfx(π) ≼ A (CND.pre) and prefix transitivity, owns(π,d) ≡ pfx(π) ≼ d (O1; ASN-0042) — d ∈ odom(π). Effective-owner ω_{Σ'}(d)=ω_Σ(A) and subdivision authority O5 quantify over ASN-0042's registry B, absent from this state; both deferred — see Open Questions | introduced |
