...
**State preconditions** (against pre-state `Σ`):
  ...
  - the invoking principal `π` satisfies `pfx(π) ≼ A` — it owns the account (O1, PrefixDetermination; ASN-0042); the authority to allocate `d` beneath `A` is then O5 (SubdivisionAuthority; ASN-0042).
...
| CND.pre | Preconditions: A ∈ E ∧ Account(A); the invoking principal π owns the account (pfx(π) ≼ A, O1; ASN-0042), with allocation authority by O5 (ASN-0042). No content argument | introduced |
