...
*Address permanence (T8, ASN-0034) and distinctness (B7, B8, ASN-0040).* Every previously valid address remains valid (`E ⊆ E'`, `dom(C) ⊆ dom(C')`) by T8, and `d` collides with no existing or future address: same-chain emissions by B8 (Uniqueness), version chains and cross-account documents by B7 (NamespaceDisjointness, disjoint streams `S(A, 2)` vs `S(d_i, 1)` and `S(A', 2)`), and content/link addresses by zero-count (`d` carries `zeros = 2`, they carry `zeros = 3`). The document's identity is permanently distinct from every other document, including ones created later. We do not invoke GlobalUniqueness, whose T10a-conformance premise this state model leaves undischarged; B7 and B8 suffice.
