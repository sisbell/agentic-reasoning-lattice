...
*Source: ASN-0068-compareversions-operation.md (revised 2026-05-24) — Extracted: 2026-05-25*

## CV-IN — CvIn (PRE, predicate)
...
`Result := P(T × T × ℕ⁺)` — a set of triples `(v_a, v_b, n)` with `n ≥ 1`, the *correspondence runs* defined below.

## Definition — CorrRelation
...
## CV-PROV-FORGOTTEN — CvProvForgotten (LEMMA, lemma)
...
## CV-LINK-DEGEN — CvLinkDegen (LEMMA, lemma)
...
## CV-LINK-SELF — CvLinkSelf (LEMMA, lemma)
...
## CV-PRED — CvPred (DEF, definition)

For a V-position `v ∈ V_S(d)` (so by D-SEQ★, ASN-0047, `v = [S, 1, ..., 1, v_m]` of depth `m` with `v_m ≥ 1`) and `j ≥ 0`, the *j-th iterated V-predecessor* `v − j` is the unique V-position `v'` of depth `m` satisfying `v' + j = v` under the OrdinalShiftBase convention of ASN-0058, with five clauses:
...
## CV-DETERM — CvDeterm (LEMMA, lemma)
