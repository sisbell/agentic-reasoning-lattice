...
- *Postconditions:* (a) `(A a ∈ dom(π') ∩ Σ'.B : ω_{Σ'}(a) = π')`; (b) `π'` satisfies O5 for allocations within `dom(π')`; (c) the delegation relation is satisfiable with `π'` as delegator for any sub-prefix `p''` (with `pfx(π') ≺ p''`) at every state at which O15 conditions (ii), (vi), and (vii) hold for `p''`. All three trivially satisfied at `Σ'`; checkable at later states.
