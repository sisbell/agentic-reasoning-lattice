...
**Freshness-(v) (derived).** Condition (v) entails that the delegate prefix is unbaptized at `Σ`: `pfx(π') ∉ Σ.B`. Since (v) fixes `pfx(π') = c_{hwm(Σ.B,p,d)+1}` for a B6-valid `(p, d)`, B2 (HighWaterMarkSufficiency, ASN-0040) identifies it as the next element of the stream `S(p, d)`, and B1 (ContiguousPrefix) gives `children(Σ.B, p, d) = {c₁, …, c_{hwm(Σ.B,p,d)}}`, so `c_{hwm(Σ.B,p,d)+1} ∉ Σ.B`. We cite this consequence as *Freshness-(v)* at its use sites below.
