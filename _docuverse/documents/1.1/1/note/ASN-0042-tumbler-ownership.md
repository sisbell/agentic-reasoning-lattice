...
**O14 (BootstrapPrincipal).** The initial principal set is non-empty and finite, every initially allocated address is covered by at least one initial principal, and the initial principals satisfy the structural constraints that O1a, O1b, T4, and pairwise non-nesting require of all bootstrap principals:
...
*Inductive step:* Suppose the invariant holds at `Σ_n` and `Σ_n → Σ_{n+1}` via some delegation `delegated_{Σ_n}(π_d, π')` introducing `π'` (by O15, at most one new principal per step; if none is introduced, the invariant is preserved trivially). Consider any pair `π₁, π₂ ∈ Π_{Σ_{n+1}}` with `π₁ ≠ π₂`. If both lie in `Π_{Σ_n}`, the IH applies (with `delegated_Σ*` extended to `Σ_{n+1}` by inclusion of the prior path). Otherwise one of them is `π'`; without loss of generality let `π₂ = π'` and `π₁ ∈ Π_{Σ_n}` (`π₁ = π'` would force `π₁ = π₂`). Compare `pfx(π₁)` and `pfx(π')`:
...
By O15 (PrincipalClosure), `π' ∈ Π_{Σ'} ∖ Π_Σ` arrived through bootstrap or through delegation. The reachability hypothesis (`Σ` reachable from `Σ₀` — see the reachability convention above) supplies a finite-length transition sequence `Σ₀ → Σ_1 → ... → Σ`; iterated application of O12 (PrincipalPersistence) along that sequence gives `Π₀ ⊆ Π_Σ`. Combined with `π' ∉ Π_Σ`, the bootstrap case is excluded. Hence `π'` was delegated by some `π_d ∈ Π_Σ` — every change to `ω(a)` is attributable to a delegation act in the transition `Σ → Σ'`.

We conclude: `ω_{Σ'}(a) ≠ ω_Σ(a)` implies `(E π' ∈ Π_{Σ'} ∖ Π_Σ : pfx(π') ≼ a ∧ #pfx(π') > #pfx(ω_Σ(a)))`, and that `π'` arrived via delegation rather than bootstrap. ∎
...
We check that O14's bootstrap clauses are satisfied: `Π₀ ≠ ∅`; each `pfx` has `zeros ≤ 1` (both have `zeros = 0`); `pfx` is injective on `Π₀` (`[1] ≠ [2]`); each prefix satisfies T4 (HierarchicalParsing — single positive component, zero-count `0 ≤ 3`) and T4a (SyntacticEquivalence — no adjacent zeros, no leading or trailing zero — vacuously, since there are no zeros); and the pair is non-nesting (`[1] ⋠ [2]` and `[2] ⋠ [1]`, since component 1 differs). `|Π₀| = 2 < ∞`. ✓
...
**Self-ownership at the prefix.** Suppose now that the principal's prefix itself is an allocated address — for instance, `[1, 0, 2] ∈ Σ.B`, perhaps allocated as `π_A`'s account-root index entry by `π_A` immediately upon delegation. We check `ω` at the boundary `a₆ = pfx(π_A) = [1, 0, 2]`. The covering set is `C(a₆) = {π ∈ Π_{Σ} : pfx(π) ≼ [1, 0, 2]}`. Candidates: `π_N` (prefix `[1]`, `[1] ≼ [1, 0, 2]` since the first component matches and `#[1] ≤ #[1, 0, 2]`), `π_M` (prefix `[2]`, fails: `2 ≠ 1`), `π_A` (prefix `[1, 0, 2]`, `[1, 0, 2] ≼ [1, 0, 2]` reflexively — every component matches and lengths are equal). So `C(a₆) = {π_N, π_A}` with prefix lengths `1` and `3`. The longest match is `π_A`, hence `ω(a₆) = π_A`. The principal owns its own prefix. The general fact: whenever `pfx(π) ∈ Σ.B`, we have `ω(pfx(π)) = π` — the address that names the namespace root is owned by the principal that holds the namespace. The argument is structural: `pfx(π) ≼ pfx(π)` reflexively, so `π ∈ C(pfx(π))`; any other covering principal `π''` has `pfx(π'') ≼ pfx(π)` with `pfx(π'') ≠ pfx(π)` (by O1b, since `π'' ≠ π`), forcing `#pfx(π'') < #pfx(π)`, so `π` achieves the strict longest match. The boundary case verifies as expected.
...
**Fork (O10).** Suppose `π_A` wishes to modify the content at `a₃ = [1, 0, 7, 0, 1, 0, 1]`. Since `ω(a₃) = π_N ≠ π_A`, the system does not grant modification. Instead, `π_A` creates a fork: a new address `a' = [1, 0, 2, 0, 6, 0, 1]` within `dom(π_A)`. We trace the baptismal trajectory and verify O10's conditions.

*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`, so the O10 proof's `zeros(pfx(π)) = 1` branch applies — a chain of exactly `2` baptisms, both by `π_A` (no cooperation required, since no sub-delegate of `π_A` can cover a document-level extension of `[1, 0, 2]` by the saturation argument). Assume the pre-fork state `Σ_pre` is reached after `π_A`'s earlier allocations `a₁ = [1, 0, 2, 0, 3, 0, 1]` and `a₂ = [1, 0, 2, 0, 5, 0, 1]` have established `hwm(Σ_pre.B, [1, 0, 2], 2) = 5` (the user-field child stream `S([1, 0, 2], 2)` has its high-water mark at index 5, the largest user-field index used so far by `π_A`'s allocations under `[1, 0, 2]`). Set `m = hwm + 1 = 6`.

The two baptisms:
- **Step 1 — document-level slot.** `b_1 = next(Σ_pre.B, [1, 0, 2], 2)`. Since `hwm = 5 > 0`, the sibling-advance branch applies: `b_1 = inc([1, 0, 2, 0, 5], 0) = [1, 0, 2, 0, 6]`. This produces a document-level prefix with `zeros = 2`. *O5 check at `Σ_pre`:* the most-specific covering principal of `[1, 0, 2, 0, 6]` in `Π_{Σ_pre} = {π_N, π_M, π_A}` is `π_A` (matches first three components; `π_N` matches only `[1]`; `π_M` does not cover). `π_A` is O5-authorized. Result: `Σ_pre → Σ_1` with `b_1 ∈ Σ_1.B`.
- **Step 2 — element-level slot.** `b_2 = next(Σ_1.B, b_1, 2)`. Since `b_1` was freshly baptized in `Σ_1` with no children in `S(b_1, 2)`, the field-opening branch applies: `b_2 = inc([1, 0, 2, 0, 6], 2) = [1, 0, 2, 0, 6, 0, 1]`. This produces an element-level prefix with `zeros = 3`. *O5 check at `Σ_1`:* `[1, 0, 2, 0, 6, 0, 1]` has component `0` at position `4` (the user-document separator); every sub-delegate of `π_A` (none exist yet in `Π_{Σ_1}`, but if any did, they'd have `zeros(pfx(π_i)) = 1` with the zero already consumed at position 4 of `pfx(π_A)`'s own user-field separator, so the prefix would have positive at position 4) — vacuously no sub-delegate covers. `π_A` is most-specific. `π_A` is O5-authorized. Result: `Σ_1 → Σ_2` with `b_2 = a' ∈ Σ_2.B`.

Set `Σ' = Σ_2`. The trajectory `Σ_pre → Σ_1 → Σ_2` is fully unilateral (both baptisms by `π_A`) and exhibits `a' ∈ dom(π_A) ∩ Σ'.B`. Step count: 2.

*Verifying O10's postconditions at `Σ'`:*

- **O10(a)**: `pfx(π_A) = [1, 0, 2] ≼ [1, 0, 2, 0, 6, 0, 1] = a'`, and `π_A` has the longest matching prefix in `Π_{Σ'}`, so `ω_{Σ'}(a') = π_A`. ✓
- **O10(a) corollary**: by (a), `pfx(π_A) = [1, 0, 2] ≼ a'`; the O6 biconditional gives `pfx(π_A) ≼ acct(a') = [1, 0, 2]`. ✓
- **O10(b)**: `a₃ ∈ Σ_pre.B ⊆ Σ_1.B ⊆ Σ_2.B = Σ'.B` (by T8 AllocationPermanence: allocation is monotone). `Π_{Σ'} = Π_{Σ_pre}` (baptism introduces no principals by O15). Hence `ω_{Σ'}(a₃) = ω_{Σ_pre}(a₃) = π_N` as before — no content modified, no ownership transferred. ✓

The fork transforms the ownership boundary into a creative act: `π_A` now has a fully owned address `a'` whose content identity may relate to `a₃`'s content (through the content model), but whose ownership is entirely independent. The trajectory above also illustrates Unilateral O10★'s `zeros = 1` clause — for an account-level principal, the fork is always unilateral regardless of `Σ_pre`'s state, because the user-field separator at position `#pfx(π_A) + 1 = 4` of any document-level extension structurally defeats every potential sub-delegate prefix (each of which must have a positive component there, by O1a's `zeros ≤ 1` saturation).
...
**O8 (IrrevocableDelegation).** Once principal `π` delegates to `π'`, the delegating parent never regains effective ownership of addresses in the delegate's domain:
  ...
  `(A π, π', a, Σ, Σ' : Σ reachable from Σ₀ ∧ delegated_Σ(π, π') ∧ a ∈ dom(π') ∩ Σ'.B ∧ Σ →⁺ Σ' : ω_{Σ'}(a) ≠ π)`

The formulation captures irrevocability without overclaiming. It says the *parent* can never recover the addresses, while permitting the delegate `π'` to sub-delegate (via O7(c)): if `π'` delegates to `π''` with `pfx(π') ≺ pfx(π'')`, then `ω(a) = π''` for `a ∈ dom(π'')` — the address leaves `π'`'s effective ownership but does not return to `π`. The domain restriction `dom(π') ∩ Σ'.B` ensures `ω` is applied only to addresses where it is defined (grounded by O4). The reachability hypothesis on `Σ` propagates to every `Σ'` with `Σ →⁺ Σ'` (each transition extends the witnessing sequence from `Σ₀`), so the iterated O12 argument used by the proof body remains available throughout the trajectory.
...
*Formal Contract:*
- *Preconditions:* `Σ` reachable from `Σ₀`, `delegated_Σ(π, π')`, `a ∈ dom(π') ∩ Σ'.B`, `Σ →⁺ Σ'`.
...
When `zeros(pfx(π)) = 0` (node-level principal): `π`'s prefix is entirely within the node field. By O1a, every sub-delegate `π_i` of `π` (i.e., every `π_i ∈ Π_Σ` with `pfx(π) ≺ pfx(π_i)`) satisfies `zeros(pfx(π_i)) ≤ 1`. We construct the candidate address `a' = pfx(π).0.u.0.1.0.1` for a value `u ≥ 1` chosen below, and analyze sub-delegates by the form of their prefix immediately after `pfx(π)`.
  ...
  - *Form B (`pfx(π).0.Y`):* the component of `pfx(π_i)` at position `#pfx(π) + 1` is `0` — the user-field separator falls immediately after `pfx(π)`. By T4a (SyntacticEquivalence — non-empty user-field segment requires a positive component immediately after the separator), T4's positive-component constraint, and O1a (`zeros(pfx(π_i)) ≤ 1`), `pfx(π_i)` then continues with strictly positive user-field components, and its first user-field component sits at position `#pfx(π) + 2`. Denote this component `U^{(i)}_1 = pfx(π_i)_{#pfx(π) + 2}`. Coverage of `a' = pfx(π).0.u.0.1.0.1` by a Form B sub-delegate splits on length. A Form B sub-delegate of length *exactly* `#pfx(π) + 2` (so `pfx(π_i) = pfx(π).0.U^{(i)}_1` with no further components) covers `a'` iff `u = U^{(i)}_1` — the only component requiring agreement beyond the matched leading segment is at position `#pfx(π) + 2`. A *longer* Form B sub-delegate has additional positive user-field components `U^{(i)}_2, U^{(i)}_3, ...` (positive by T4; they cannot include another zero without violating O1a, since `pfx(π_i)`'s single allowed zero is already consumed at the user-field separator). But `a'` has `0` at position `#pfx(π) + 3` (the user-document separator). The prefix relation requires `pfx(π_i)_{#pfx(π) + 3} = U^{(i)}_2 = 0`, contradicting `U^{(i)}_2 > 0`. Coverage fails at position `#pfx(π) + 3` regardless of `u`. Hence Form B coverage of `a'` is possible only for Form B sub-delegates of length exactly `#pfx(π) + 2`, and only when `u = U^{(i)}_1`.
...
In both cases, `π` can always produce an address it effectively owns. We make the baptism trajectory explicit, anchoring it in ASN-0040's `next(B, p, d) = inc(p, d)` when `B ∩ S(p, d) = ∅`, else `inc(max(B ∩ S(p, d)), 0)` — strictly sequential within each child-stream `S(p, d)`. Two operational branches appear: *field-opening* (`inc(p, 2)`, extending the tumbler by two positions to descend a level, by TA5(d)) and *sibling advance* (`inc(max_child, 0)`, same length and zeros count, by TA5(c)). Both are needed: field-opening for new descents, sibling advance for traversing existing sibling streams when the chosen target lies beyond the current `hwm`.

*Baptismal coupling of delegation.* The trajectory analysis turns on O18 (DelegationBaptizes): whenever a sub-delegate `π_k ∈ Π_Σ` of `π` has prefix `pfx(π_k) = pfx(π).0.k`, the prefix `pfx(π).0.k` is itself in `Σ.B`. Define `S' := { U^{(i)}_1 : π_i \text{ is Form B with } #pfx(π_i) = #pfx(π) + 2 } ⊆ S` — the first user-field component of *those* Form B sub-delegates whose entire prefix is `pfx(π).0.U^{(i)}_1` (the only sub-delegates capable of being the most-specific covering principal of any user-level slot `pfx(π).0.k`). The coupling yields a sharp consequence: every such prefix `pfx(π).0.{U^{(i)}_1}` lies in `Σ.B` by O18, hence `U^{(i)}_1 ≤ hwm_0 := hwm(Σ.B, pfx(π), 2)` for every Form B (=length) sub-delegate. Therefore `S' ⊆ {1, ..., hwm_0}`, and in particular `hwm_0 + 1 ∉ S'` *in every reachable state*. The next slot beyond the current high-water mark is never a slot already pre-claimed by an immediate sub-delegate of `π`.

*Trajectory for `zeros(pfx(π)) = 0` (node-level principal).* Set `u = hwm_0 + 1`. By the baptismal coupling above, `u ∉ S'`. The non-coverage analysis for `a' = pfx(π).0.u.0.1.0.1` from the existence derivation applies verbatim with this `u`: no Form A sub-delegate covers `a'` (failing at position `#pfx(π) + 1`); no Form B (>length) sub-delegate covers `a'` (failing at position `#pfx(π) + 3`); no Form B (=length) sub-delegate covers `a'` (since `u ∉ S'`). Hence `π` is the most-specific covering principal of `a'` and of every intermediate point on the trajectory.

`π` performs the trajectory in exactly `3` baptisms, each O5-authorized for `π`:

- `b_1 = next(Σ.B, pfx(π), 2) = pfx(π).0.{hwm_0 + 1}` (user-level, `zeros = 1`). By ASN-0040's `next` semantics: when `hwm_0 = 0`, `Σ.B ∩ S(pfx(π), 2) = ∅` and `next` reduces to the field-opening branch `inc(pfx(π), 2) = pfx(π).0.1`; when `hwm_0 ≥ 1`, the sibling-advance branch yields `inc(pfx(π).0.{hwm_0}, 0) = pfx(π).0.{hwm_0 + 1}`. In neither case does any sub-delegate prefix occupy `b_1`, by baptismal coupling.
- `b_2 = next(Σ_1.B, b_1, 2) = inc(b_1, 2) = pfx(π).0.{hwm_0 + 1}.0.1` (document-level, `zeros = 2`). `b_1` was freshly baptized in `Σ_1` and has an empty child stream in `S(b_1, 2)`, so the field-opening branch applies.
- `b_3 = next(Σ_2.B, b_2, 2) = inc(b_2, 2) = pfx(π).0.{hwm_0 + 1}.0.1.0.1 = a'` (element-level, `zeros = 3`). Same reasoning: `b_2` has an empty child stream in `Σ_2`.

All three baptisms are performed by `π`. The trajectory `Σ → Σ_1 → Σ_2 → Σ_3 = Σ'` is wholly π-driven; no sub-delegate cooperation is required.

*Unilateral feasibility (unconditional).* The existence claim is witnessed in every reachable state by a chain of `3` baptisms performed entirely by `π` — no cooperative gap remains. The baptismal coupling closes what might have appeared to be one: sub-delegates of `π` whose prefixes pre-claim user-field slots have, by their existence as registered principals, already been baptized into `Σ.B`, and the high-water mark `hwm_0` already reflects them. `π`'s sibling-advance call therefore lands on slot `hwm_0 + 1`, strictly beyond every pre-claimed slot, and `π` is the most-specific covering principal of that slot. The trajectory then descends into `π`'s own depth via two field-openings on freshly baptized parents.

Both lines of confirmation from the consultation agree. Nelson's design intent rules out any forking mode that requires sub-delegate participation: "once assigned a User account, the user will have full control over its subdivision forevermore" (LM 4/29). The "forevermore" clause forbids an ongoing cooperative role for the parent tier within a delegate's subtree, and by symmetry forbids an ongoing cooperative role for sub-delegates within the parent's free user-field. The parent's fork at `hwm_0 + 1` is structurally outside every sub-delegate's authority and structurally inside `π`'s. Gregory's allocator behaves identically: `findpreviousisagr` advances unilaterally past delegated slots, treating the granfilade as the sole source of truth — no inter-session signaling, no shared counter, no lock. The allocator finds the highest existing entry in the parent's subtree (delegated account slots included) and returns the next slot. The abstract baptismal coupling captures exactly this implementation property.

> *Unilateral O10★.* In every reachable state `Σ` and for every `π ∈ Π_Σ`, there exists a chain `Σ →⁺ Σ'` of `2` baptisms (when `zeros(pfx(π)) = 1`) or `3` baptisms (when `zeros(pfx(π)) = 0`), *all performed by π alone*, witnessing the fork postcondition.

No condition on `Σ`, on `S'`, or on sub-delegate timing remains. "Denial as fork" is unconditional and unilateral, as the architecture intends.

*Trajectory for `zeros(pfx(π)) = 1` (account-level principal).* No sub-delegate of `π` covers any document-level extension of `pfx(π)`: every sub-delegate has `zeros(pfx(π_i)) = 1` (the available zero is already consumed by `pfx(π)`'s own user-field separator) with no further zero, so `pfx(π_i)` extends `pfx(π)` only by positive user-field components — position `#pfx(π) + 1` of every sub-delegate prefix is positive, but the same position in any document-level address `pfx(π).0.X.…` is the user-document separator `0`. Hence `π` is the most-specific covering principal of every document-level and deeper extension of `pfx(π)`, and is O5-authorized for every baptism in this trajectory.

Let `m = hwm(Σ.B, pfx(π), 2) + 1` (the next document-level index produced by `next`). Two baptisms by `π`:
- `b_1 = next(Σ.B, pfx(π), 2) = pfx(π).0.m` (document-level, `zeros = 2`; this is `inc(pfx(π), 2) = pfx(π).0.1` when `m = 1`, else the sibling advance `inc(pfx(π).0.{m−1}, 0) = pfx(π).0.m`),
- `b_2 = next(Σ_1.B, b_1, 2) = inc(b_1, 2) = pfx(π).0.m.0.1` (element-level, `zeros = 3`; `b_1` is freshly baptized in `Σ_1` with no children in `S(b_1, 2)`, so the field-opening branch applies).

Take `a' = b_2 = pfx(π).0.m.0.1`. The earlier non-coverage argument transfers verbatim with `m` in place of the literal `1` at position `#pfx(π) + 2`: the user-document separator at position `#pfx(π) + 1` is what defeats every sub-delegate prefix (each has positive there); the specific user-field index `m` plays no role. Step count `k = 2`; both baptisms by `π`.

*Per-baptism authorization.* At each step `Σ_{j-1} → Σ_j` of the trajectory, the baptism is performed by the most-specific covering principal of its result, satisfying O5.

For the `zeros(pfx(π)) = 0` trajectory, apply the Form A/Form B classification to the slot `b_1 = pfx(π).0.u` with `u = hwm_0 + 1`: Form A sub-delegates have a positive component at position `#pfx(π) + 1` but `b_1` has `0` there — no Form A covers. Form B sub-delegates of length `> #pfx(π) + 2` have length exceeding `b_1` and so are not prefixes of it. Form B sub-delegates of length exactly `#pfx(π) + 2` cover `b_1` iff `U^{(i)}_1 = u`; by baptismal coupling `S' ⊆ {1, ..., hwm_0}` and `u = hwm_0 + 1 ∉ S'`, so no Form B (=length) sub-delegate covers `b_1` either. Hence `π` is the unique most-specific covering principal of `b_1`. For the deeper `b_2 = pfx(π).0.u.0.1` and `b_3 = a'`: the same classification at each depth rules out coverage — Form A (`0` at position `#pfx(π) + 1`), Form B (>length) of pfx length `≥ #pfx(π) + 3` having positive `U^{(i)}_2` at position `#pfx(π) + 3` where `b_2` and `b_3` carry the user-document separator `0`, and Form B (=length) ruled out by `u ∉ S'`. `π` is the most-specific covering principal of all three baptisms.

For the `zeros(pfx(π)) = 1` trajectory, sub-delegates `π_i` satisfy `pfx(π) ≺ pfx(π_i)` and `zeros(pfx(π_i)) = 1` (the available zero is consumed at `pfx(π)`'s own user-field separator). Hence `pfx(π_i)` extends `pfx(π)` only by additional positive user-field components: `pfx(π_i) = pfx(π).U'_1.U'_2....` with every `U'_k > 0` and no further zero. Both `b_1 = pfx(π).0.m` and `b_2 = pfx(π).0.m.0.1 = a'` have `0` at position `#pfx(π) + 1`, where every sub-delegate prefix has `U'_1 > 0`. Hence no sub-delegate covers either intermediate; `π` is most-specific for both.

*Trajectory closure.* By O15, baptism does not introduce new principals — only delegation does — and the trajectory contains no delegation acts (only baptisms by `π`). Hence `Π_{Σ_{j-1}} = Π_Σ` for every `j`, and the Form A/B classification (with its derived sets `S, S'`) computed at `Σ` remains valid throughout. The composition of authorized baptisms yields `Σ →⁺ Σ'` with `a' ∈ Σ'.B` and `ω_{Σ'}(a') = π`. The fork operation's postcondition is satisfiable in every reachable state via this unilateral trajectory. ∎

*Formal Contract:*
- *Preconditions:* `Σ` reachable from `Σ₀`, `π ∈ Π_Σ`, `a ∈ Σ.B`, `ω(a) ≠ π`.
- *Postconditions:* `(E Σ', a' : Σ →⁺ Σ' ∧ a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ a ∈ Σ'.B)` — there exists a reachable successor state `Σ'` (reached via a finite sequence of authorized baptismal transitions from `Σ`) and a new address `a' ∈ dom(π)` such that `a'` is effectively owned by `π` in `Σ'`, and the original address `a` remains allocated and unmodified throughout the sequence. The body's construction exhibits, in every reachable state, a unilateral trajectory:
  - For `zeros(pfx(π)) = 1`: a chain of `2` baptisms by `π` (`pfx(π) → pfx(π).0.m → pfx(π).0.m.0.1 = a'`, where `m = hwm(Σ.B, pfx(π), 2) + 1`), each O5-authorized for `π` because no sub-delegate covers any document-level extension of `pfx(π)` (the `zeros(pfx(π_i)) = 1` saturation argument).
  - For `zeros(pfx(π)) = 0`: a chain of `3` baptisms by `π`. Setting `u = hwm_0 + 1` (where `hwm_0 := hwm(Σ.B, pfx(π), 2)`), `π` calls `next(Σ.B, pfx(π), 2)` to produce `pfx(π).0.u`, then performs two field-openings (`pfx(π).0.u.0.1`, `pfx(π).0.u.0.1.0.1 = a'`). All three baptisms are O5-authorized for `π`: by baptismal coupling, the set `S' = { U^{(i)}_1 : π_i \text{ is Form B (=length) sub-delegate of } π }` satisfies `S' ⊆ {1, ..., hwm_0}`, so `u = hwm_0 + 1 ∉ S'` and the Form A/B classification rules out sub-delegate coverage at every step.

  In both cases, no sub-delegate covers `a'` (verified by the Form A/B classification and the O1a `zeros ≤ 1` constraint on sub-delegate prefixes), so `ω_{Σ'}(a') = π`. The trajectory is performed entirely by `π`; no sub-delegate cooperation is required in any reachable state.
- *Unilateral postcondition (Unilateral O10★):* In every reachable state `Σ` and for every `π ∈ Π_Σ`, the existence claim is witnessed by a chain `Σ →⁺ Σ'` of `2` (when `zeros(pfx(π)) = 1`) or `3` (when `zeros(pfx(π)) = 0`) baptisms, *all performed by π alone*. The unilateral guarantee is unconditional: O18 (DelegationBaptizes) ensures that every sub-delegate's prefix is already in `Σ.B`, so the next user-field slot beyond `hwm_0` is never pre-claimed by a sub-delegate, and `π`'s sibling-advance call always lands on a slot it is O5-authorized for. This matches Nelson's design intent (forking is the architectural answer to non-ownership; "full control... forevermore" rules out cooperation as a forking mode) and Gregory's allocator behavior (`findpreviousisagr` advances past delegated slots without coordination).
- *Invariant:* In every reachable state, every principal can unilaterally produce an address it effectively owns — the fork postcondition is satisfiable by `π` alone, with `2` or `3` baptisms, in every reachable state.
...
| AccountLevelPermanence | No external delegation can alter effective ownership within `dom(π)` — changes to `ω(a)` inside a principal's domain arise only from that principal's own acts or its sub-delegates' acts | from Delegation, O1b, O3, O14, O15, Prefix |
...
| O8 | `Σ reachable ∧ delegated_Σ(π, π') ∧ a ∈ dom(π') ∩ Σ'.B ∧ Σ →⁺ Σ' ⟹ ω_{Σ'}(a) ≠ π` — delegating parent never regains ownership | from Delegation, O2, O12, O13, T8 |
| O9 | `(A π ∈ Π, a ∈ Σ.B : owns(π, a) ⟹ N(pfx(π)) ≼ N(a))` — ownership bounded by node field | from O1, O1a, T4, Prefix |
| O10 | Non-ownership of target yields a fork: new address under the requesting principal's domain | from O1a, O6, O18, FiniteRegistry, T0a, TA5(c), TA5(d), ASN-0040 `next`, ASN-0040 `hwm` |
...
| `acct(a)` | When `zeros(a) = 0`: `acct(a) = a`; when `zeros(a) ≥ 1`: truncation through user field | from T4b, T3 |
