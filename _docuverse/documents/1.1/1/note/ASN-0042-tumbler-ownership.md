...
- *Postconditions:* (a) `(A a ∈ odom(π') ∩ Σ'.B : ω_{Σ'}(a) = π')`; (b) `π'` satisfies O5 for allocations within `odom(π')`; (c) the delegation relation is satisfiable with `π'` as delegator for a sub-prefix `p''` (with `pfx(π') ≺ p''`) at every state at which O15 condition (v) holds for `p''`. Conditions (i), (ii), and (iv) are discharged at `Σ'` independent of `p''`; condition (v) remains the per-state obligation on `p''`, as established in the proof of postcondition (c) above.
