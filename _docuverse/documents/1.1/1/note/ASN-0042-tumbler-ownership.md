...
**Self-ownership at the prefix.** O18 (DelegationBaptizes) mandates `pfx(π_A) = [1, 0, 2] ∈ Σ_1.B` as an immediate consequence of the delegation transition introducing `π_A` into `Π` — the delegate's prefix enters the baptismal registry in the same act that enters `π_A` into the principal registry. We check `ω` at the boundary `a₆ = pfx(π_A) = [1, 0, 2]`. The covering set is `C(a₆) = {π ∈ Π_{Σ_1} : pfx(π) ≼ [1, 0, 2]}`. Candidates: `π_N` (prefix `[1]`, `[1] ≼ [1, 0, 2]` since the first component matches and `#[1] ≤ #[1, 0, 2]`), `π_M` (prefix `[2]`, fails: `2 ≠ 1`), `π_A` (prefix `[1, 0, 2]`, `[1, 0, 2] ≼ [1, 0, 2]` reflexively — every component matches and lengths are equal). So `C(a₆) = {π_N, π_A}` with prefix lengths `1` and `3`. The longest match is `π_A`, hence `ω(a₆) = π_A`. The principal owns its own prefix. The general fact holds unconditionally in every reachable state for every principal: `(A Σ, π ∈ Π_Σ : ω_Σ(pfx(π)) = π)`. Combining O18's inductive clause (every newly introduced principal's prefix enters `Σ.B` at the delegating transition) with O14's seventh clause (`(A π ∈ Π₀ : pfx(π) ∈ Σ₀.B)`, the O18 bootstrap base case), every principal's prefix is in `Σ.B`; the longest-match argument is structural: `pfx(π) ≼ pfx(π)` reflexively, so `π ∈ C(pfx(π))`; any other covering principal `π''` has `pfx(π'') ≼ pfx(π)` with `pfx(π'') ≠ pfx(π)` (by O1b, since `π'' ≠ π`), forcing `#pfx(π'') < #pfx(π)`, so `π` achieves the strict longest match. Self-ownership at the prefix is a corollary of O18 and O1b, not a hypothetical scenario.
...
*Non-coverage analysis.* We show `ω_{Σ'}(a') = π` by ruling out sub-delegate coverage of `a'`. Every sub-delegate `π_i` of `π` (i.e., `π_i ∈ Π_Σ` with `pfx(π) ≺ pfx(π_i)`) satisfies `zeros(pfx(π_i)) ≤ 1` by O1a. Classify by the component of `pfx(π_i)` at position `#pfx(π) + 1`:
  ...
  Now invoke O18 (DelegationBaptizes): every Form B sub-delegate's prefix `pfx(π_i) = pfx(π).0.U^{(i)}_1` lies in `Σ.B`, hence `U^{(i)}_1 ∈ S(pfx(π), 2) ∩ Σ.B`, hence `U^{(i)}_1 ≤ hwm_0`. The set `S' := { U^{(i)}_1 : π_i \text{ is a Form B sub-delegate} }` satisfies `S' ⊆ {1, …, hwm_0}`, so `hwm_0 + 1 ∉ S'`. No Form B sub-delegate covers `a'`.
...
- *Unilateral postcondition (Unilateral O10★):* In every reachable state `Σ` and for every `π ∈ Π_Σ`, the existence claim is witnessed by a single baptism `Σ → Σ'` performed by `π` alone, producing `a' = pfx(π).0.{hwm_0 + 1}` ∈ `dom(π) ∩ Σ'.B` with `ω_{Σ'}(a') = π`. The unilateral guarantee is unconditional: O18 ensures every sub-delegate's prefix lies in `Σ.B`, so `S' ⊆ {1, …, hwm_0}` and `hwm_0 + 1 ∉ S'` in every reachable state.
...
| O10 | Non-ownership of target yields a fork: new address under the requesting principal's domain | from O1a, O6, O18, TA5(c), TA5(d), ASN-0040 `next`, ASN-0040 `hwm`, ASN-0040 B6 |
