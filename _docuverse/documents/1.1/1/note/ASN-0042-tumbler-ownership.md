...
The ownership model rests on seven structural axioms (O12, O13, O14, O15, O5, O16, O18) together with the primitive allocation relation `allocated_by_Σ`, from which we derive four further properties (FiniteRegistry, NestingByDelegation, O17, PrefixBaptismCoupling) by induction over the transition sequence from `Σ₀`. The axioms constrain transition-induced changes; the initial state `Σ₀` is governed by O14 (for the principal registry `Π₀`) and by ASN-0040's bootstrap clause B₀ conf. (for the baptismal registry `Σ₀.B`). The constraints O5 (SubdivisionAuthority) and O16 (AllocationClosure) apply to transition-induced allocations only — addresses entering `Σ.B` via a `→` step — and not to bootstrap-seeded addresses, whose well-formedness is the responsibility of ASN-0040. We state them explicitly.
...
- *`pfx(π') = pfx(π₁)`:* Forbidden by O1b. O1b is preserved across transitions: by O15 the only new principal is `π'`, and condition (i) requires `pfx(π_d) ≺ pfx(π')` (strict), so `pfx(π') ≠ pfx(π_d)`; combined with O1b at `Σ_n` (which rules out duplication among `Π_{Σ_n}` and in particular between `π_d` and any other existing principal), `pfx(π') = pfx(π₁)` would force `π_d = π₁` (impossible, since `pfx(π_d) ≺ pfx(π')` strict) or violate O1b at `Σ_{n+1}`. This case is impossible.
...
**ω(a) (EffectiveOwner).** The *effective owner* of an allocated address `a` is the principal with the longest matching prefix:

  `ω(a) = π  ≡  pfx(π) ≼ a  ∧  (A π' ∈ Π : π' ≠ π ∧ pfx(π') ≼ a : #pfx(π) > #pfx(π'))`

This is a partial definition until we show that the right-hand side picks out exactly one principal in every reachable state. That is the content of O2.

*Notation (state-relativization of `ω` and `Π`).* The defining equivalence above quantifies over `Π`, which is itself a state-indexed object (`Π_Σ`). Every evaluation of `ω` is therefore *state-relativized*: at state `Σ`, the function `ω_Σ : Σ.B → Π_Σ` selects the longest-match principal from `Π_Σ` against an address in `Σ.B`. We write `ω_Σ(a)` when the state must be disambiguated — typically when comparing two states (e.g., `ω_{Σ'}(a) ≠ ω_Σ(a)` in O3) or invoking the function under a specific reachable state. We write bare `ω(a)` when the ambient state is fixed by context — typically inside a paragraph operating at a single state `Σ`, or in informal prose where no inter-state comparison is at stake. Both forms denote the same function under the relativization; the subscript is a notational convenience for readability, not a distinct operator. The same convention applies to bare `Π` versus `Π_Σ`. When a property's formal contract restates the state explicitly, the formal-contract form prevails over the prose's abbreviated form. Whenever the proof body invokes `ω` at a state different from the one currently in scope, the subscript is supplied explicitly.

**O2 (OwnershipExclusivity).** For every allocated address `a`, there exists exactly one principal that effectively owns `a` — equivalently, `ω : Σ.B → Π` is a total well-defined function:
...
- *Postconditions:* `(E! π ∈ Π_Σ : ω(a) = π)` — exactly one principal satisfies the defining equivalence.
...
We check that O14's bootstrap clauses are satisfied: `Π₀ ≠ ∅`; each `pfx` has `zeros ≤ 1` (both have `zeros = 0`); `pfx` is injective on `Π₀` (`[1] ≠ [2]`); each prefix satisfies T4 (HierarchicalParsing — single positive component, zero-count `0 ≤ 3`) and T4a (SyntacticEquivalence — no adjacent zeros, no leading or trailing zero — vacuously, since there are no zeros); the pair is non-nesting (`[1] ⋠ [2]` and `[2] ⋠ [1]`, since component 1 differs); and each principal's prefix lies in `Σ₀.B` (we assume the bootstrap state was seeded with `[1], [2] ∈ Σ₀.B`, satisfying O14's seventh clause; we additionally seed the following addresses into `Σ_0.B`, treating them as bootstrap data rather than as Bop products of transitions reaching Σ_0 — Σ_0 is the source state of the worked-example trajectory, so its baptismal registry is given as a single bootstrap snapshot consistent with O14, B1, and O18: (i) `[1, 0, 1] ∈ Σ_0.B` — covered by `π_N` per O14's first clause, since `[1] ≼ [1, 0, 1]` — so that B1 (ContiguousPrefix of ASN-0040) holds when `pfx(π_A) = [1, 0, 2] = c_2 ∈ S([1], 2)` is baptized at the delegation transition below, B1 requiring `c_1 = [1, 0, 1] ∈ Σ_0.B` prior; (ii) `[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3] ∈ Σ_0.B` — three siblings of `S([1, 0, 2], 2)` covered by `π_N` (the most-specific covering principal of each in `Π_0`), so that B1 within `S([1, 0, 2], 2)` is satisfied as `children(Σ_0.B, [1, 0, 2], 2) = {[1, 0, 2, 0, k] : 1 ≤ k ≤ 3}` (a contiguous prefix); (iii) `a_1 = [1, 0, 2, 0, 3, 0, 1] ∈ Σ_0.B` — the document-element address under `[1, 0, 2, 0, 3]`, first member of `S([1, 0, 2, 0, 3], 2)`, so B1 within that stream is vacuous; (iv) `a_3 = [1, 0, 7, 0, 1, 0, 1] ∈ Σ_0.B` — the document-element address used in the Fork scenario below, covered by `π_N` (the most-specific covering principal in `Π_0`, since `[1] ≼ a_3` while `[2] ⋠ a_3`); `a_3` is the first member of `S([1, 0, 7, 0, 1], 2)`, so B1 within that stream is vacuous, and `a_3` does not enter `children(Σ_0.B, [1], 2)` (a length-3 stream) or any other stream populated by seeds in (i)–(iii), so contiguity within those streams is unaffected. All seeds in (ii)–(iv) are consistent with O18's freshness conjunct for the delegated prefix, which constrains only `[1, 0, 2]` itself — and `[1, 0, 2] ∉ Σ_0.B` since it will be baptized exactly at the delegation transition — not other slots of any sibling stream. No corresponding seed is required under `π_M`: the bootstrap state contains no `[2, 0, k]` addresses (only `[2]` per O14's seventh clause), so when the later delegation `delegated(π_M, π_C)` of `[2, 0, 1] = c_1 ∈ S([2], 2)` is baptized as `pfx(π_C)`, B1's contiguous-prefix requirement is vacuously satisfied — `[2, 0, 1]` is the first baptism in its stream and has no B1 predecessor). `|Π₀| = 2 < ∞`. ✓
...
| O2 | Every allocated address has exactly one effective owner `ω(a)`, determined by longest matching prefix | from O4, O1b, Prefix, T3 |
...
| O14 | `Π₀ ≠ ∅`, initial principals cover all initially allocated addresses, `\|Π₀\| < ∞`, `zeros ≤ 1`, `pfx` injective on `Π₀`, `T4(pfx(π))`, pairwise non-nesting, and every initial principal's prefix lies in `Σ₀.B` — bootstrap with finiteness/O1a/O1b/T4/non-nesting/O18 base cases | design requirement |
...
| `ω(a)` | `effectiveOwner : Σ.B → Principal` — the effective owner function (defined only for allocated addresses) | from O4, O1b, Prefix, T3 |
