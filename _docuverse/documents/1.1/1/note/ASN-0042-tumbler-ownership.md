...
*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`. The pre-fork state `Σ_pre` is reached by an explicit baptism history under `[1, 0, 2]`, with each address established as a direct `Bop` target rather than derived by cross-stream inference. *Pre-delegation (by `π_N` as most-specific covering principal of `[1, 0, 2]` in `Σ₀`, per `delegated_{Σ₀}(π_N, π_A)`'s condition (ii)):* successive `Bop([1, 0, 2], 2)` calls baptize `[1, 0, 2, 0, 1]`, then `[1, 0, 2, 0, 2]`, then `[1, 0, 2, 0, 3]` in `S([1, 0, 2], 2)` (the first call is a field-opening from `hwm = 0`, the next two are sibling-advances); then `Bop([1, 0, 2, 0, 3], 2)` baptizes element `a₁ = [1, 0, 2, 0, 3, 0, 1]` (per State Σ₁ above). *Delegation:* `Σ₀ → Σ_1` introduces `π_A` with `pfx(π_A) = [1, 0, 2]`. *Post-delegation (by `π_A` as most-specific covering principal of `[1, 0, 2]` in `Σ_1`):* successive `Bop([1, 0, 2], 2)` calls baptize `[1, 0, 2, 0, 4]`, then `[1, 0, 2, 0, 5]` (sibling-advances continuing `S([1, 0, 2], 2)`); then `Bop([1, 0, 2, 0, 5], 2)` baptizes element `a₂ = [1, 0, 2, 0, 5, 0, 1]` (per the *Allocation* paragraph). By B0 (Irrevocability) of ASN-0040 iterated along this trajectory, every baptized target persists, so `{[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3], [1, 0, 2, 0, 4], [1, 0, 2, 0, 5], a₁, a₂} ⊆ Σ_pre.B`. Each of `[1, 0, 2, 0, 1], …, [1, 0, 2, 0, 5]` is a direct `Bop([1, 0, 2], 2)` target; none is inferred from an element baptism by cross-stream contiguity (B1 ContiguousPrefix establishes intra-stream contiguity within a single `S(p, d)` — it cannot bridge `S([1, 0, 2, 0, 3], 2)`'s element `a₁` to `S([1, 0, 2], 2)`'s document `[1, 0, 2, 0, 3]`). B1 applied intra-stream to `S([1, 0, 2], 2)` is then consistent with the trajectory: `children(Σ_pre.B, [1, 0, 2], 2) = {[1, 0, 2, 0, k] : 1 ≤ k ≤ 5}`. Hence `hwm(Σ_pre.B, [1, 0, 2], 2) = 5`. Set `m = hwm + 1 = 6`.
