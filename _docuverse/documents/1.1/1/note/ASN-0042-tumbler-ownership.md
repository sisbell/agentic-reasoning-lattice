...
In both cases, `π` can always produce an address it effectively owns. We make the baptism trajectory explicit, anchoring it in the actual baptism operations of ASN-0040 rather than in single-component extensions (which would be ill-typed: appending one zero to a node-level prefix yields a trailing-zero tumbler that violates T4). The relevant operation is *child baptism* via `inc(·, 2)` — by TA5(d), each `inc(·, 2)` extends the tumbler by exactly two positions (a zero separator followed by a positive component) and produces a valid tumbler at the next deeper field level. Sibling baptism (`inc(·, 0)`) is not used here, since it does not extend the tumbler in length and so cannot move the trajectory toward `a'`. The `inc(·, 1)` operation (extending the current field by one positive component) is also not needed: every step of our trajectory crosses a field boundary.

*Trajectory for `zeros(pfx(π)) = 0` (node-level principal).* The constructed `a' = pfx(π).0.u.0.1.0.1` (with `u` chosen as above) has `zeros(a') = 3` (element level); reaching it from `pfx(π)` requires three child baptisms. Let `b₀ = pfx(π)` and define
- `b₁ = inc(b₀, 2) = pfx(π).0.u` (account-level, `zeros = 1`),
- `b₂ = inc(b₁, 2) = pfx(π).0.u.0.1` (document-level, `zeros = 2`),
- `b₃ = inc(b₂, 2) = pfx(π).0.u.0.1.0.1 = a'` (element-level, `zeros = 3`).

Hence `k = 3` baptism steps, each adding two positions; the total position count `#a' - #pfx(π) = 6` is recovered as `k · 2`, but the *step count* is `k = 3`, not `6`. The intermediates `b_1, b_2` are valid tumblers (T4) at their respective field levels.

*Trajectory for `zeros(pfx(π)) = 1` (account-level principal).* The constructed `a' = pfx(π).0.1.0.1` has `zeros(a') = 3` (element level); reaching it from `pfx(π)` requires two child baptisms:
- `b₀ = pfx(π)` (account-level, `zeros = 1`),
- `b₁ = inc(b₀, 2) = pfx(π).0.1` (document-level, `zeros = 2`),
- `b₂ = inc(b₁, 2) = pfx(π).0.1.0.1 = a'` (element-level, `zeros = 3`).

Hence `k = 2` baptism steps, each adding two positions; the total position count `#a' - #pfx(π) = 4` is recovered as `k · 2`, with step count `k = 2`.

*Per-intermediate sub-delegate non-coverage.* At each step `Σ_{j-1} → Σ_j` the target `b_j` lies within `dom(π)` (since `pfx(π) = b₀ ≼ b_j` along the chain). We must additionally verify that no sub-delegate `π_i` of `π` covers `b_j` at any intermediate, since the `u`-selection / boundary arguments above only addressed the final `a'`.

For the `zeros(pfx(π)) = 0` trajectory, recall the Form-A/Form-B classification and the choice `u ∉ S = { U^{(i)}_1 : π_i \text{ is Form B} }`. Every `b_j` (j ≥ 1) has a `0` at position `#pfx(π) + 1` (each `b_j` is an extension of `pfx(π).0.u`, so the user-field separator sits there). Form A sub-delegates have a positive component at position `#pfx(π) + 1`, so the prefix relation fails immediately — no Form A sub-delegate covers any `b_j`. Form B sub-delegates have zero at position `#pfx(π) + 1` and positive `U^{(i)}_1` at position `#pfx(π) + 2`; covering `b_j` requires `U^{(i)}_1 = u`, but `u ∉ S` excludes this for every `j ≥ 1` (each `b_j`'s component at position `#pfx(π) + 2` is `u`). Form B sub-delegates whose prefixes extend further into the user field (with `U^{(i)}_2, U^{(i)}_3, ...`) cannot cover `b_2` or `b_3` either: `b_2` has a `0` at position `#pfx(π) + 3` (the user-document separator), but every Form B prefix's component at that position is a positive `U^{(i)}_2` (Form B prefixes have `zeros = 1`; introducing a second zero would violate O1a, so additional components must remain in the user field). Hence no sub-delegate covers any `b_j` in this trajectory.

For the `zeros(pfx(π)) = 1` trajectory, sub-delegates `π_i` satisfy `pfx(π) ≺ pfx(π_i)` and `zeros(pfx(π_i)) ≤ 1`. Since `pfx(π)` already has `zeros = 1` (the node-user separator), `pfx(π_i)` must have exactly `zeros = 1` and extends `pfx(π)` only by additional positive user-field components: `pfx(π_i) = pfx(π).U'_1.U'_2....` with every `U'_k > 0` and no further zero. Every intermediate `b_j` (j ≥ 1) has a `0` at position `#pfx(π) + 1` (the user-document separator). Coverage of `b_j` by `π_i` would require `pfx(π_i)`'s component at position `#pfx(π) + 1` to equal `0`, but it is `U'_1 > 0`. Hence no sub-delegate covers `b_1` or `b_2 = a'`.

*Per-step authorization and trajectory closure.* At each step `Σ_{j-1} → Σ_j`, the per-intermediate analysis just given establishes that `π` is the most-specific covering principal for `b_j` in `Π_{Σ_{j-1}}`. By O5 (SubdivisionAuthority), `π` has authority to baptize `b_j`. The set of sub-delegates of `π` does not change across the trajectory: by O15, a baptism (allocation) does not introduce a new principal — only a delegation does, and the trajectory consists exclusively of `π`'s own baptisms. Hence `S` and the Form A/B classification computed at `Σ` remain valid in every `Σ_{j-1}`, and the per-step authorization is uniform across `j = 1, ..., k`. The composition of these `k` baptisms yields a multi-step transition `Σ →⁺ Σ_k` with `a' = b_k ∈ Σ_k.B` and `ω_{Σ_k}(a') = π`. The fork operation's postcondition is satisfiable in every reachable state via this multi-step trajectory. ∎

*Formal Contract:*
- *Preconditions:* `Σ` reachable from `Σ₀`, `π ∈ Π_Σ`, `a ∈ Σ.B`, `ω(a) ≠ π`.
- *Postconditions:* `(E Σ', a' : Σ →⁺ Σ' ∧ a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ a ∈ Σ'.B)` — there exists a reachable successor state `Σ'` (reached through a finite sequence of child baptisms from `Σ`) and a new address `a' ∈ dom(π)` such that `a'` is effectively owned by `π` in `Σ'`, and the original address `a` remains allocated and unmodified throughout the sequence. The body's construction (the `u`-selection argument for `zeros(pfx(π)) = 0` and the immediate document-level construction for `zeros(pfx(π)) = 1`) exhibits a chain `b₀ = pfx(π) ⊏ b₁ ⊏ ... ⊏ b_k = a'` of `k = 3 - zeros(pfx(π))` baptism steps, each invoking `inc(b_{j-1}, 2)` to descend one field level, along which each `b_j` lies in `dom(π)` and is not covered by any sub-delegate (verified per intermediate by the Form A/B classification and the O1a `zeros ≤ 1` constraint on sub-delegate prefixes); O5 (SubdivisionAuthority) supplies `π`'s right to baptize each `b_j` in turn, yielding the witnessing trajectory `Σ →⁺ Σ'`.
