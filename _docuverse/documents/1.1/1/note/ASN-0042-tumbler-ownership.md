...
*Field-opening boundary case.* The trajectory above used the sibling-advance branch of `next` (`hwm_0 = 5`). The complementary field-opening branch arises when `hwm_0 = 0` — no depth-2 child of `pfx(π_A)` has yet been baptized in `S([1, 0, 2], 2)`. This boundary case is incompatible with the running scenario's `Σ_1`: the pre-delegation trajectory of `π_N` (per the *Trajectory* paragraph above) directly baptizes `[1, 0, 2, 0, 1]`, `[1, 0, 2, 0, 2]`, `[1, 0, 2, 0, 3]` via successive `Bop([1, 0, 2], 2)` calls — each a direct member of `S([1, 0, 2], 2)`. By B0 (Irrevocability) of ASN-0040, these persist from `Σ₀.B` into `Σ_1.B` across the delegation transition, so `hwm(Σ_1.B, [1, 0, 2], 2) ≥ 3 ≠ 0`. (Note that we do not derive these from the element baptism `a₁ ∈ Σ_1.B`: B1 ContiguousPrefix governs intra-stream contiguity within a single `S(p, d)`, and `a₁ ∈ S([1, 0, 2, 0, 3], 3)` cannot witness any membership in `S([1, 0, 2], 2)` by B1 alone — the document parents must be established as direct `Bop` targets in the trajectory.) We therefore consider an alternative scenario in which `π_N` performed no pre-delegation baptism under `[1, 0, 2]` — neither `a₁` nor any of its document parents was allocated before delegation. Let `Σ_alt` denote the post-delegation state in this scenario — `π_A` has just entered `Π`, no document-level baptism has occurred under `[1, 0, 2]`, and `Π_{Σ_alt} = {π_N, π_M, π_A}` matches `Π_{Σ_1}`'s principal set without `a₁` having been pre-allocated. Then `hwm(Σ_alt.B, [1, 0, 2], 2) = 0`, and the single baptism reduces to the field-opening branch: `b_1 = next(Σ_alt.B, [1, 0, 2], 2) = inc([1, 0, 2], 2) = [1, 0, 2, 0, 1]` (TA5(d), appending the document-field separator and first document index). The resulting `a' = [1, 0, 2, 0, 1]` has `zeros = 2` (document level), with `pfx(π_A) = [1, 0, 2] ≼ a'`. *O5 check at `Σ_alt`:* `π_A` is the most-specific covering principal in `Π_{Σ_alt} = {π_N, π_M, π_A}` (no sub-delegate of `π_A` exists yet — `Π_{Σ_alt} ∖ {π_A}` contains only `π_N` with prefix `[1]` and `π_M` with prefix `[2]`, neither extending `[1, 0, 2]`). ✓ *B6 check:* `d = 2`, `zeros(pfx(π_A)) + (d − 1) = 1 + 1 = 2 ≤ 3`. ✓ *O10(a):* `pfx(π_A) ≼ a'` and `π_A` has the longest matching prefix in `Π_{Σ'}`, so `ω_{Σ'}(a') = π_A`. ✓ *O10(b):* every pre-existing baptized address persists in `Σ.B` (B0 Irrevocability of ASN-0040) and `Π` is unchanged (O15). ✓ The field-opening branch exhibits the unilateral fork at the boundary case where no prior document baptism exists — `π_A` can fork from the very moment of delegation, requiring no preceding allocation history under `pfx(π_A)`.
...
| O3 | `ω(a)` changes only through delegation introducing a longer matching prefix — monotonic refinement | from B0 (ASN-0040), O12, O13, O1b, O14, O15 |
