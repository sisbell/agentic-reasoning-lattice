...
*Case `zeros(a) = 3`.* By T4b (UniqueParse), `fields(a)` decomposes `a` uniquely into `N(a)`, `U(a)`, `D(a)`, and `E(a)`, with the structure `N(a) ++ [0] ++ U(a) ++ [0] ++ D(a) ++ [0] ++ E(a)`. Define `acct(a) = N(a) ++ [0] ++ U(a)`. By the same reasoning as the `zeros(a) = 2` case — `N(a)` and `U(a)` each have at least one strictly positive component (non-emptiness from T4a, positivity from T4) — the constructed tumbler has exactly one zero, no adjacent zeros, no leading or trailing zero (T4a), and all non-separator components positive (T4). It satisfies T4 and has `zeros(acct(a)) = 1`.
...
- *Postconditions:* (a) `acct(a)` is a valid tumbler satisfying T4 and T4a. (b) `zeros(acct(a)) ≤ 1`: specifically, `zeros(acct(a)) = 0` when `zeros(a) = 0`, and `zeros(acct(a)) = 1` when `zeros(a) ≥ 1` — the two branches together yielding the ≤ 1 bound. (c) When `zeros(a) ≤ 1`: `acct(a) = a`. (d) When `zeros(a) ≥ 2`: `acct(a)` is a proper prefix of `a` with `zeros(acct(a)) = 1`.
