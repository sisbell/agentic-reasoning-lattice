...
We define `acct(a)`, the *account field* of a valid tumbler `a` — the node-and-user portion that fixes ownership, with defining property `zeros(acct(a)) ≤ 1` for every valid tumbler. The construction is given once in the Formal Contract below.
