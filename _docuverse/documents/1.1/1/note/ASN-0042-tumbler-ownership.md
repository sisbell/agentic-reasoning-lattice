...
The coupling is sharpened for principal-introducing transitions: every transition that admits a new principal `π'` takes the baptism branch — never the frame branch — and the element it baptizes is exactly `pfx(π')`. (That this prefix is the next-reachable `next(Σ.B, p, d)` for a B6-valid `(p, d)` is owned by condition (v) of the delegation predicate, not restated here.)
...
*Proof.* By O17b's principal-introduction coupling, the introducing transition takes the baptism branch with `Σ'.B = Σ.B ∪ {pfx(π')}`, so `pfx(π') ∈ Σ'.B`. Freshness has a single source of truth: condition (v) fixes `pfx(π') = next(Σ.B, p, d)` for a B6-valid `(p, d)`, and ASN-0040's `Bop` postcondition `next(s.B, p, d) ∉ s.B` gives `pfx(π') ∉ Σ.B`. Hence `pfx(π') ∈ Σ'.B ∖ Σ.B`. ∎
...
| O18 | `Σ → Σ' ∧ π' ∈ Π_{Σ'} ∖ Π_Σ ⟹ pfx(π') ∈ Σ'.B ∖ Σ.B` — delegation materially baptizes the delegate's prefix as a fresh registration | derived from O17b coupling and condition (v) `next`-semantics |
