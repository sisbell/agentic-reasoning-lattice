...
Should `π_B` fork against content it does not own — say `a₃ = [1, 0, 7, 0, 1, 0, 1]`, with `ω_{Σ_3}(a₃) = π_N` — the single baptism `next(Σ_3.B, [1, 0, 2, 3], 2)` takes the field-opening branch: `inc([1, 0, 2, 3], 2) = [1, 0, 2, 3, 0, 1]` (TA5(d), appending the document-field separator and first document index), a document-level address (`zeros = 2`) with `pfx(π_B) ≼ a'`. That `ω(a') = π_B`, that the baptism is `π_B`-authorized, and that `a₃` is untouched are the `hwm_0 = 0` case of the O10 *Construction*, *B6 verification*, and *Non-coverage analysis*, discharged there in full generality; no re-derivation is repeated here. With `π_A`'s sibling-advance fork and `π_B`'s field-opening fork both exhibited on concrete addresses, the two branches of Unilateral O10★ are witnessed.
...
This is not a deficiency in the ownership *model* — it is a gap in the ownership *enforcement*. That `validaccount` returns TRUE unconditionally locates enforcement outside the model: O1 through O10 hold regardless of how principal identity is established (the binding is treated in the *Summary of the Model*). The structural predicate `tumbleraccounteq` gives the correct answer for any two tumblers; whether the *right* tumblers are being compared — whether the session's claimed account tumbler is the one the principal is actually entitled to — is a separate concern.
