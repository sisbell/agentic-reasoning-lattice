...
*Non-coverage analysis.* We show `ω_{Σ'}(a') = π` by ruling out sub-delegate coverage of `a'`. Every sub-delegate `π_i` of `π` (i.e., `π_i ∈ Π_Σ` with `pfx(π) ≺ pfx(π_i)`) satisfies `zeros(pfx(π_i)) ≤ 1` by O1a. Classify by the component of `pfx(π_i)` at position `#pfx(π) + 1`:
  ...
  Now invoke PrefixBaptismCoupling: every Form B sub-delegate `π_i ∈ Π_Σ` has `pfx(π_i) = pfx(π).0.U^{(i)}_1 ∈ Σ.B` in the reachable state `Σ`, hence `U^{(i)}_1 ∈ S(pfx(π), 2) ∩ Σ.B`, hence `U^{(i)}_1 ≤ hwm_0`. The set `S' := { U^{(i)}_1 : π_i \text{ is a Form B sub-delegate} }` satisfies `S' ⊆ {1, …, hwm_0}`, so `hwm_0 + 1 ∉ S'`. No Form B sub-delegate covers `a'`.
