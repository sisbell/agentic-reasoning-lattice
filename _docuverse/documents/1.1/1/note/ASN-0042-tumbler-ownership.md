...
O1a is a reachable-state invariant, proved in the *Delegation* section.
...
where `delegated_Σ*` is the reflexive-transitive closure of the parent relation `R_Σ` defined above. Equality `pfx(π₁) = pfx(π₂)` is excluded by O1b.
...
- *Postconditions:* `(E π_d ∈ Π_Σ, π' ∈ Π_{Σ'} ∖ Π_Σ : pfx(π') ≼ a ∧ #pfx(π') > #pfx(ω_Σ(a)) ∧ delegated_Σ(π_d, π'))` — the change is witnessed by both the new principal `π'` (with a strictly longer matching prefix) and the delegator `π_d` (the existing principal whose authority condition (ii) admits `π'`). The monotonic-refinement corollary `#pfx(ω_{Σ'}(a)) ≥ #pfx(ω_Σ(a))` (given `a ∈ Σ.B`) is stated and proved in the body above.
...
- *Postconditions:* (a) `(A a ∈ odom(π') ∩ Σ'.B : ω_{Σ'}(a) = π')`; (b) `π'` satisfies O5 for allocations within `odom(π')`; (c) the delegation relation is satisfiable with `π'` as delegator for a sub-prefix `p''` (with `pfx(π') ≺ p''`) at every prospective delegation state at which O15 conditions (ii), (iv), and (v) hold for `p''`. At the entry state `Σ'`, conditions (i), (ii), and (iv) hold for `π'` independent of `p''`; at any later prospective delegation state, conditions (ii) and (iv) become per-state obligations re-checked alongside (v), since `π'` may itself have sub-delegated — as established in the proof of postcondition (c) above.
