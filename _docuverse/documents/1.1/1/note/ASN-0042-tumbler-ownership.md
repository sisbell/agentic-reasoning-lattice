...
The field-opening branch is therefore exhibited via a *delegation-induced* boundary case rather than a hypothetical scenario substitution: every freshly delegated account-level principal `π_B` faces a virgin granfilade `S(pfx(π_B), 2)` at the moment of its arrival, and its first fork — whether immediate or deferred — invokes the field-opening branch with `hwm_0 = 0`. The boundary case is not an exception in a different timeline; it is the structural inheritance of every fresh delegate, propagating along the lattice of sub-delegations.
