...
O1a is a reachable-state invariant; its inductive proof is given in the *Delegation* section (base case O14's third clause, preserved by delegation condition (iii)). The uses of O1a in O6, O9, and O10 below are therefore not circular — they invoke an invariant established by that induction.
...
- *Postconditions* (all four follow from FieldStructure: when `zeros(a) ≤ 1` the construction reproduces `a`; when `zeros(a) ≥ 2` the node and user segments give a length-`α + 1 + β` tumbler with one separator at position `α + 1`): (a) `acct(a)` is a valid tumbler satisfying T4. (b) `zeros(acct(a)) ≤ 1`. (c) When `zeros(a) ≤ 1`: `acct(a) = a`. (d) When `zeros(a) ≥ 2`: `acct(a)` is a proper prefix of `a` with `zeros(acct(a)) = 1`.
...
**Definition (delegated).** We name the conjunction of conditions (i)–(v) above the *delegation predicate*, with a four-place signature: `delegated(Σ, Σ', π, π')` holds iff `Σ → Σ'`, `π ∈ Π_Σ`, `π' ∈ Π_{Σ'} ∖ Π_Σ`, and conditions (i)–(v) hold for `(π, π')` at `Σ` (the subscript form `delegated_Σ(π, π')` abbreviates `delegated(Σ, Σ', π, π')` for the `Σ'` bound by the surrounding formula). Evaluation-state convention: the delegator prefix `pfx(π)` is read at `Σ`, the delegate prefix `pfx(π')` at `Σ'` (fixed for all subsequent states by O13, PrefixImmutability).
...
- **(v)** `pfx(π_A) = [1, 0, 2] = next(Σ₀.B, [1], 2)` with `(p, d) = ([1], 2)` B6-valid (`[1]` satisfies T4, `d = 2 ∈ {1, 2}`, `zeros([1]) + (2 − 1) = 1 ≤ 3`): the stream `S([1], 2)` has `c₁ = inc([1], 2) = [1, 0, 1]` and `c₂ = inc([1, 0, 1], 0) = [1, 0, 2]`. The seed `[1, 0, 1] ∈ Σ₀.B` is `c₁`, so `children(Σ₀.B, [1], 2) = {[1, 0, 1]}`, `hwm = 1`, and `next = inc(c₁, 0) = [1, 0, 2] = c₂`. The bootstrap deliberately seeds `[1, 0, 1]` precisely so `[1, 0, 2]` is next-reachable; without it the stream-predecessor would be missing and (v) would fail. `T4([1, 0, 2])` and `[1, 0, 2] ∉ Σ₀.B` then follow (by Freshness-(v)). ✓
