...
This is not a deficiency in the ownership *model* — it is a gap in the ownership *enforcement*. The model itself is clean: O1 through O10 hold regardless of how principal identity is established. The binding `session.account = pfx(π)` is an axiom of the session, not a theorem derivable within O1–O10 — any conforming implementation must supply *some* mechanism for it (authentication, delegation verification, cryptographic binding), but the ownership properties are independent of which mechanism is chosen, provided the mapping is consistent with the delegation structure. The structural predicate `tumbleraccounteq` gives the correct answer for any two tumblers; whether the *right* tumblers are being compared — whether the session's claimed account tumbler is the one the principal is actually entitled to — is a separate concern.
