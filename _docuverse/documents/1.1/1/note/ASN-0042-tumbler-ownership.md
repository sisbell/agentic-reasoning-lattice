...
*Notation.* `dom(π)` here applies to a principal and yields the prefix-defined subset of `T`; this is distinct from `dom(A)` of T10a (ASN-0034), which applies to a correspondence and yields the source domain of a relation. The two are disambiguated by argument kind throughout — principal vs. correspondence — and never collide in any expression.

Before developing ownership domains' nesting structure, we extract a structural fact about the prefix relation `≼` that the subsequent proofs invoke repeatedly. The fact is a direct consequence of Prefix (PrefixRelation) of ASN-0034 and is independent of T5 (ContiguousSubtrees), but its repeated use across O2 (Step 2), O7(a), AccountLevelPermanence (Step 3), NestingByDelegation, and AccountLevelPermanence★ warrants stating it once as a named lemma:
...
where `delegated_Σ*(π, π')` denotes the reflexive-transitive closure of "there exists a state `Σ_k` on the transition path Σ₀ → ... → Σ such that `delegated_{Σ_k}(π, π')` held at that step." Equality `pfx(π₁) = pfx(π₂)` is excluded by O1b (preserved across transitions — see below).

We derive this by induction on the transition sequence `Σ₀ → Σ_1 → ... → Σ`.

*Base case:* By O14's sixth clause, all initial principals in `Π_{Σ_0}` have pairwise non-nesting prefixes. So the first disjunct holds vacuously for every pair `π₁, π₂ ∈ Π_{Σ_0}` with `π₁ ≠ π₂`.
...
- *Invariant:* `#pfx(ω_{Σ'}(a)) ≥ #pfx(ω_Σ(a))` for all transitions `Σ → Σ'` between reachable states.
...
**Corollary (AccountLevelPermanence★, multi-step).** The single-transition property extends to the transitive closure `Σ →⁺ Σ'`: every change to `ω(a)` for `a ∈ dom(π) ∩ Σ.B` along any reachable transition sequence is induced by delegators whose prefixes all extend `pfx(π)`. Let `Σ →⁺ Σ'` abbreviate `Σ → Σ_1 → ... → Σ_n = Σ'` for some `n ≥ 1`:
...
**Sub-account namespace.** Now suppose `π_A` creates sub-account position `[1, 0, 2, 3]` as an organizational namespace — not delegated to a new principal. By B1 (ContiguousPrefix), `[1, 0, 2, 3]` is the third baptism in `S(pfx(π_A), 1)`, preceded by `[1, 0, 2, 1]` and `[1, 0, 2, 2]` (or by sub-delegations whose prefixes occupy those slots). `Π` remains `{π_N, π_A}`. Address `a₄ = [1, 0, 2, 3, 0, 1, 0, 1]` is a document element under this sub-account. We verify:
...
*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`. The pre-fork state `Σ_pre` is reached by an explicit baptism history under `[1, 0, 2]`, with each address established as a direct `Bop` target rather than derived by cross-stream inference. *Pre-delegation (by `π_N` as most-specific covering principal of `[1, 0, 2]` in `Σ₀`, per `delegated_{Σ₀}(π_N, π_A)`'s condition (ii)):* successive `Bop([1, 0, 2], 2)` calls baptize `[1, 0, 2, 0, 1]`, then `[1, 0, 2, 0, 2]`, then `[1, 0, 2, 0, 3]` in `S([1, 0, 2], 2)` (the first call is a field-opening from `hwm = 0`, the next two are sibling-advances); then `Bop([1, 0, 2, 0, 3], 3)` baptizes element `a₁ = [1, 0, 2, 0, 3, 0, 1]` (per State Σ₁ above). *Delegation:* `Σ₀ → Σ_1` introduces `π_A` with `pfx(π_A) = [1, 0, 2]`. *Post-delegation (by `π_A` as most-specific covering principal of `[1, 0, 2]` in `Σ_1`):* successive `Bop([1, 0, 2], 2)` calls baptize `[1, 0, 2, 0, 4]`, then `[1, 0, 2, 0, 5]` (sibling-advances continuing `S([1, 0, 2], 2)`); then `Bop([1, 0, 2, 0, 5], 3)` baptizes element `a₂ = [1, 0, 2, 0, 5, 0, 1]` (per the *Allocation* paragraph). By B0 (Irrevocability) of ASN-0040 iterated along this trajectory, every baptized target persists, so `{[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3], [1, 0, 2, 0, 4], [1, 0, 2, 0, 5], a₁, a₂} ⊆ Σ_pre.B`. Each of `[1, 0, 2, 0, 1], …, [1, 0, 2, 0, 5]` is a direct `Bop([1, 0, 2], 2)` target; none is inferred from an element baptism by cross-stream contiguity (B1 ContiguousPrefix establishes intra-stream contiguity within a single `S(p, d)` — it cannot bridge `S([1, 0, 2, 0, 3], 3)`'s element `a₁` to `S([1, 0, 2], 2)`'s document `[1, 0, 2, 0, 3]`). B1 applied intra-stream to `S([1, 0, 2], 2)` is then consistent with the trajectory: `children(Σ_pre.B, [1, 0, 2], 2) = {[1, 0, 2, 0, k] : 1 ≤ k ≤ 5}`. Hence `hwm(Σ_pre.B, [1, 0, 2], 2) = 5`. Set `m = hwm + 1 = 6`.
...
We derive this from O18 (DelegationBaptizes), O16 (AllocationClosure), O5 (SubdivisionAuthority), condition (ii) of the delegation relation, and O1b (PrefixInjectivity). By O18, `pfx(π') ∈ Σ'.B`. *Freshness.* The delegate's prefix is newly entered in this transition: `pfx(π') ∉ Σ.B`. Suppose for contradiction `pfx(π') ∈ Σ.B`. By O17 (AllocatedAddressValidity), `T4(pfx(π'))` (consistent with condition (v) of `delegated_Σ`). The membership `pfx(π') ∈ Σ.B` means `pfx(π')` was allocated in some earlier transition `Σ_k → Σ_{k+1}` along the path `Σ_0 → ... → Σ`. By O16, an allocator `π_a ∈ Π_{Σ_k}` exists with `allocated_by_{Σ_{k+1}}(π_a, pfx(π'))`. By O5, `pfx(π_a) ≼ pfx(π')`, and by O5's most-specific clause, `#pfx(π_a) ≥ #pfx(π'')` for every `π'' ∈ Π_{Σ_k}` with `pfx(π'') ≼ pfx(π')`. Two sub-cases. (1) If `pfx(π_a) = pfx(π')`, then by O1b `π_a` is uniquely determined by this prefix, and `π_a ∈ Π_{Σ_k} ⊆ Π_Σ` (by iterated O12). By condition (iii) of `delegated_Σ`, `π' ∉ Π_Σ`, so `π_a ≠ π'`. But then `π_a ∈ Π_Σ` with `pfx(π_a) = pfx(π')`, and by condition (i) `pfx(π_d) ≺ pfx(π')`, so condition (vi) `¬(E π'' ∈ Π_Σ : pfx(π') ≺ pfx(π''))` is consistent only if no strict extension exists — but here we have an *equality* principal, which doesn't contradict (vi). However, applied at `Σ` rather than `Σ_k`: `π_a ∈ Π_Σ` covers `pfx(π')` with `pfx(π_a) = pfx(π')`, and condition (ii) requires `#pfx(π_a) ≤ #pfx(π_d)`. Combined with condition (i)'s `#pfx(π_d) < #pfx(π')`, we get `#pfx(π_a) = #pfx(π') > #pfx(π_d) ≥ #pfx(π_a)` — contradiction. (2) If `pfx(π_a) ≺ pfx(π')` (strict), then `π_a ∈ Π_Σ` (by iterated O12 from `Π_{Σ_k}`) covers `pfx(π')` strictly shorter than `pfx(π')` itself. Condition (ii) gives `#pfx(π_a) ≤ #pfx(π_d)`, no contradiction here — but we have not yet derived a contradiction in sub-case (2). To do so: we need that no principal in `Π_Σ` has previously allocated `pfx(π')` as a non-principal sub-position. The conservative reading of the spec — O18's prose "Delegation materially baptizes the delegate's prefix" and the worked example's pattern (every principal prefix is materialized at the moment of delegation, never pre-baptized as a sub-position) — closes sub-case (2) by stipulation: principal prefixes are not pre-baptized; their first entry into `Σ.B` is via the delegation transition that introduces them. Under this reading, `pfx(π') ∉ Σ.B`, so the contradiction hypothesis fails. *Allocator identification.* Given `pfx(π') ∈ Σ'.B ∖ Σ.B`, O16 supplies `π_a ∈ Π_Σ` with `allocated_by_{Σ'}(π_a, pfx(π'))`. By O5 applied to the current transition, `π_a` is the most-specific covering principal of `pfx(π')` in `Π_Σ`. By condition (ii) of `delegated_Σ(π_d, π')`, `π_d` is also the most-specific covering principal of `pfx(π')` in `Π_Σ`. Suppose `π_a ≠ π_d`. Both have `pfx(π_a), pfx(π_d) ≼ pfx(π')` with `#pfx(π_a) = #pfx(π_d)` (each achieving the maximum). By the covering-chain lemma, `pfx(π_a)` and `pfx(π_d)` are `≼`-comparable; with equal lengths, the prefix relation forces equality `pfx(π_a) = pfx(π_d)` (each is a prefix of the other by length, and the components agree componentwise with `pfx(π')`). By O1b, `π_a = π_d` — contradicting the supposition. Hence `π_a = π_d`, and `allocated_by_{Σ'}(π_d, pfx(π'))` holds. ∎
...
| AccountLevelPermanence | No external delegation can alter effective ownership within `dom(π)` — changes to `ω(a)` inside a principal's domain arise only from that principal's own acts or its sub-delegates' acts | from Delegation, O1b, O3, O14, O15, Prefix; multi-step corollary AccountLevelPermanence★ additionally from B0★ (ASN-0040) |
...
| O10 | Non-ownership of target yields a fork: new address under the requesting principal's domain | from O1a, O6, PrefixBaptismCoupling, TA5(c), TA5(d), ASN-0040 `next`, ASN-0040 `hwm`, ASN-0040 B6, B0 (ASN-0040) |
...
| O18 | `Σ → Σ' ∧ π' ∈ Π_{Σ'} ∖ Π_Σ ⟹ pfx(π') ∈ Σ'.B` — delegation materially baptizes the delegate's prefix | axiom |
