...
*Design confirmation.* O8 instantiates the refinement-only reading of Nelson's "forevermore" (established at OwnershipDomainPermanence) at the parent–delegate boundary. There is no revocation command, no forced reclamation: Gregory's `validaccount` is a stub that unconditionally returns TRUE, confirming only the *absence of any revocation mechanism* — the premise that no transition removes a principal or shortens a prefix — not the exclusivity conclusion itself. The irrevocability O8 establishes rests on the longest-match rule: the delegate's strictly longer prefix permanently outranks the parent's, a model theorem the stub neither computes nor witnesses.
