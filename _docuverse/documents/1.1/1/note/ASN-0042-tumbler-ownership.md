...
*Case 1 — `π ∈ Π_{Σ_n}` (carried forward).* By O13 (PrefixImmutability), `pfx_{Σ_{n+1}}(π) = pfx_{Σ_n}(π)`, and the IH gives `pfx_{Σ_n}(π) ∈ Σ_n.B`. By T8 (AllocationPermanence), `Σ_n.B ⊆ Σ_{n+1}.B`, so `pfx_{Σ_{n+1}}(π) ∈ Σ_{n+1}.B`.
...
The named derived property collects the four foundation steps — O14's seventh clause (bootstrap base case), O15 (closure of `Π` by bootstrap or delegation, which makes the case split exhaustive), O18 (the delegation-step inductive case), and T8 (carrying baptized prefixes forward across transitions) — so subsequent properties may cite the single load-bearing fact rather than reassemble its base/step/closure structure each time. O10's non-coverage analysis and the worked example's self-ownership scenario both rely on the conclusion: every principal's prefix sits in the baptismal registry of every reachable state in which the principal exists.
...
*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`. Assume the pre-fork state `Σ_pre` is reached after document-level baptisms have populated slots 1 through 5 under `[1, 0, 2]` — the document addresses `[1, 0, 2, 0, 1]`, `[1, 0, 2, 0, 2]`, `[1, 0, 2, 0, 3]`, `[1, 0, 2, 0, 4]`, `[1, 0, 2, 0, 5]` are all in `Σ_pre.B` (B1 ContiguousPrefix forces the slots between any used document indices to be present). In particular, the document parents of the earlier element baptisms are in `Σ_pre.B`: `[1, 0, 2, 0, 3]` (the document parent of `a₁ = [1, 0, 2, 0, 3, 0, 1]`, allocated by `π_N` pre-delegation, per State Σ₁) and `[1, 0, 2, 0, 5]` (the document parent of `a₂ = [1, 0, 2, 0, 5, 0, 1]`, allocated by `π_A` post-delegation, per the *Allocation* paragraph). Hence `hwm(Σ_pre.B, [1, 0, 2], 2) = 5` (the largest document-field index used so far under `[1, 0, 2]`). Set `m = hwm + 1 = 6`.
...
- **O10(b)**: `a₃ ∈ Σ_pre.B ⊆ Σ'.B` (by T8 AllocationPermanence). `Π_{Σ'} = Π_{Σ_pre}` (baptism introduces no principals by O15). Hence `ω_{Σ'}(a₃) = ω_{Σ_pre}(a₃) = π_N` as before — no content modified, no ownership transferred. ✓

*Field-opening boundary case.* The trajectory above used the sibling-advance branch of `next` (`hwm_0 = 5`). The complementary field-opening branch arises when `hwm_0 = 0` — no document-field child of `pfx(π_A)` has yet been baptized. This boundary case is incompatible with the running scenario's `Σ_1`: the pre-delegation allocation `a₁ = [1, 0, 2, 0, 3, 0, 1] ∈ Σ₀.B` (per State Σ₁ above) persists into `Σ_1.B` by T8 (AllocationPermanence), and B1 (ContiguousPrefix) of ASN-0040 forces `[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3] ∈ Σ_1.B`, so `hwm(Σ_1.B, [1, 0, 2], 2) ≥ 3 ≠ 0`. We therefore consider an alternative scenario in which `a₁` was not pre-allocated by `π_N` before delegation. Let `Σ_alt` denote the post-delegation state in this scenario — `π_A` has just entered `Π`, no document-level baptism has occurred under `[1, 0, 2]`, and `Π_{Σ_alt} = {π_N, π_M, π_A}` matches `Π_{Σ_1}`'s principal set without `a₁` having been pre-allocated. Then `hwm(Σ_alt.B, [1, 0, 2], 2) = 0`, and the single baptism reduces to the field-opening branch: `b_1 = next(Σ_alt.B, [1, 0, 2], 2) = inc([1, 0, 2], 2) = [1, 0, 2, 0, 1]` (TA5(d), appending the document-field separator and first document index). The resulting `a' = [1, 0, 2, 0, 1]` has `zeros = 2` (document level), with `pfx(π_A) = [1, 0, 2] ≼ a'`. *O5 check at `Σ_alt`:* `π_A` is the most-specific covering principal in `Π_{Σ_alt} = {π_N, π_M, π_A}` (no sub-delegate of `π_A` exists yet — `Π_{Σ_alt} ∖ {π_A}` contains only `π_N` with prefix `[1]` and `π_M` with prefix `[2]`, neither extending `[1, 0, 2]`). ✓ *B6 check:* `d = 2`, `zeros(pfx(π_A)) + (d − 1) = 1 + 1 = 2 ≤ 3`. ✓ *O10(a):* `pfx(π_A) ≼ a'` and `π_A` has the longest matching prefix in `Π_{Σ'}`, so `ω_{Σ'}(a') = π_A`. ✓ *O10(b):* every pre-existing address persists unchanged (T8) and `Π` is unchanged (O15). ✓ The field-opening branch exhibits the unilateral fork at the boundary case where no prior document baptism exists — `π_A` can fork from the very moment of delegation, requiring no preceding allocation history under `pfx(π_A)`.
...
*Trajectory closure.* The baptism does not modify `Π` (by O15) or any pre-existing address (by T8 AllocationPermanence). The original address `a` remains in `Σ'.B` with its ownership unchanged: `ω_{Σ'}(a) = ω_Σ(a) ≠ π`. The fork postcondition is satisfied: `a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π`, with `a ∈ Σ'.B` unchanged. ∎
...
- *Unilateral postcondition (Unilateral O10★):* In every reachable state `Σ` and for every `π ∈ Π_Σ`, the existence claim is witnessed by a single baptism `Σ → Σ'` performed by `π` alone, producing `a' = pfx(π).0.{hwm_0 + 1}` ∈ `dom(π) ∩ Σ'.B` with `ω_{Σ'}(a') = π`. The unilateral guarantee is unconditional: PrefixBaptismCoupling ensures every sub-delegate's prefix lies in `Σ.B`, so `S' ⊆ {1, …, hwm_0}` and `hwm_0 + 1 ∉ S'` in every reachable state.
...
| O3 | `ω(a)` changes only through delegation introducing a longer matching prefix — monotonic refinement | from T8, O12, O13, O1b, O14, O15 |
...
| O8 | `Σ_d reachable ∧ delegated_{Σ_d}(π, π') ∧ Σ_d →⁺ Σ' ∧ π' ∈ Π_{Σ'} ∧ a ∈ dom(π') ∩ Σ'.B ⟹ ω_{Σ'}(a) ≠ π` — delegating parent never regains ownership | from Delegation, O2, O12, O13, O15, T8 |
| O9 | `(A π ∈ Π, a ∈ Σ.B : owns(π, a) ⟹ N(pfx(π)) ≼ N(a))` — ownership bounded by node field | from O1, O1a, T4, Prefix |
| O10 | Non-ownership of target yields a fork: new address under the requesting principal's domain | from O1a, O6, PrefixBaptismCoupling, TA5(c), TA5(d), ASN-0040 `next`, ASN-0040 `hwm`, ASN-0040 B6 |
...
| PrefixBaptismCoupling | `(A Σ reachable, π ∈ Π_Σ : pfx(π) ∈ Σ.B)` — every principal's prefix is itself baptized in every reachable state | from O13, O14(vii), O15, O18, T8 |
