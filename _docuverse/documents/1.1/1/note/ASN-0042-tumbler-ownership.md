...
**Sub-lemma (FirstDelegatorIsπ).** Let `Σ` be reachable from `Σ₀`, `π ∈ Π_Σ`, and let `π'' ∈ Π_{Σ'}` with `Σ →⁺ Σ'` and `pfx(π) ≺ pfx(π'')` be any sub-delegate of `π` introduced after `Σ`. Then there exists a finite chain `π = π^{(0)}, π^{(1)}, ..., π^{(k)} = π''` (`k ≥ 1`) such that for each `0 ≤ j < k`, the delegation `delegated_{Σ^{(j)}}(π^{(j)}, π^{(j+1)})` holds at some intermediate state `Σ^{(j)}` along `Σ →⁺ Σ'`.

*Proof.* Let `S_π(Σ̃) = { π* ∈ Π_{Σ̃} : pfx(π) ≺ pfx(π*) }` denote the set of strict sub-delegates of `π` present in state `Σ̃`. By FiniteRegistry, `|S_π(Σ̃)| < ∞` for every reachable `Σ̃`; by O12, `S_π` grows monotonically along `Σ →⁺ Σ'`. We induct on `|S_π(Σ̃)|` at the transition `Σ̃ → Σ̃'` that introduces `π''` (the unique transition along `Σ →⁺ Σ'` with `π'' ∈ Π_{Σ̃'} ∖ Π_{Σ̃}`, well-defined by O15).
...
*Inductive step `|S_π(Σ̃)| = m + 1`.* By O15, `π''` was delegated by some `π_d ∈ Π_{Σ̃}`. Two cases on `π_d`. (a) `π_d = π`: chain `π → π''` of length `k = 1`. (b) `π_d ≠ π` and `pfx(π) ≺ pfx(π_d)` (the latter from condition (ii) and the AccountLevelPermanence step as in the base case): then `π_d ∈ S_π(Σ̃)`. The state `Σ̃_d` at which `π_d` itself was introduced lies along `Σ →⁺ Σ̃` — specifically, `π_d ∈ Π_{Σ̃}` and `π_d ∉ Π_Σ` (else `π_d ∈ S_π(Σ)` would contradict the induction setup unless `S_π(Σ) ≠ ∅`; in either case, walking back to the first transition introducing a sub-delegate of `π` yields a state `Σ̃_d` reachable from `Σ` with `|S_π(Σ̃_d^-)| < m + 1` where `Σ̃_d^-` is the predecessor of `Σ̃_d`). Apply the inductive hypothesis to `π_d` in place of `π''`: a chain `π = π^{(0)}, ..., π^{(k')} = π_d` exists. Concatenate with `π_d → π''`: the chain `π = π^{(0)}, ..., π^{(k')} = π_d, π^{(k'+1)} = π''` of length `k = k' + 1` witnesses the claim.
