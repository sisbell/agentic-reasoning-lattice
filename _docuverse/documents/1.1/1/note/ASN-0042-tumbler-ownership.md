...
*Reading.* The notation `delegated_Σ(π, π')` does double duty: as a witness to the actual delegation transition `Σ → Σ'` introducing `π'` (the historical reading, invoked when reasoning about a transition that has occurred — e.g., O8's anchoring of `Σ_d^{post}`) and as the satisfaction of the six conditions at state `Σ` (the structural reading, invoked when verifying that a prospective delegation is admissible). The two readings coincide on actual transitions — condition (iii) fixes the successor state via `π' ∈ Π_{Σ'} ∖ Π_Σ` — so we use a single notation throughout this ASN; context disambiguates.
