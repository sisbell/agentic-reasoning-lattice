...
`findpreviousisagr` issues each new account slot as a fresh granfilade entry, never re-purposing a previously baptized tumbler as a new principal's prefix — confirming the freshness conjunct.
...
In udanax-green the new account slot is entered through `findpreviousisagr` under the session's own account-tumbler authority, so the delegator and the allocator are necessarily the same process.
...
The construction instantiates the refinement-only regime of O3 and the irrevocability of O8: the parent's fork at `hwm_0 + 1` is structurally outside every sub-delegate's authority and structurally inside `π`'s. Gregory's allocator behaves identically: each new tumbler issues from the single granfilade allocation point, which advances unilaterally past delegated slots, treating the granfilade as the sole source of truth. The abstract baptismal coupling captures exactly this implementation property.
...
For an account-level principal (`zeros(pfx(π)) = 1`), the single baptism produces a document-level address (`zeros(a') = 2`), one tier above content-bearing element depth, in one allocation call. A node-level principal's fork instead yields a user-level namespace slot (`zeros(a') = 1`); since Nelson confines the node operator's role to account allocation ("the host computer allocates accounts," LM 4/19), content depth there requires further baptism (O10(c)).

*Formal Contract:*
- *Preconditions:* `Σ` reachable from `Σ₀`, `π ∈ Π_Σ`, `a ∈ Σ.B`, `ω(a) ≠ π`.
- *Postconditions:* `(E Σ', a' : Σ → Σ' ∧ a' ∈ odom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ zeros(a') = zeros(pfx(π)) + 1 ∧ a ∈ Σ'.B)`, where `a' = pfx(π).0.{hwm_0 + 1}` and `hwm_0 := hwm(Σ.B, pfx(π), 2)`.
