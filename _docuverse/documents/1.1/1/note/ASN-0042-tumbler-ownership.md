...
Condition (ii) is the *authorization* clause — the delegator `π` must be the most-specific covering principal of `pfx(π')` in `Π_Σ`, so no principal may delegate within a sub-domain it has already delegated away. Condition (vi) enforces *top-down delegation order* — a parent prefix must be delegated before any child prefix strictly within it, so the delegate has full authority over its domain from the moment of creation. Condition (vii) is the *freshness* gate — `pfx(π') ∉ Σ.B`. Condition (viii) is the *next-reachability* gate — `pfx(π') = next(Σ.B, p, d)` for some B6-valid namespace `(p, d)`.

Conditions (v) and (vii) are not independent admission conditions: condition (viii) entails both. For the witnessing `(p, d)`, `next(Σ.B, p, d) = c_{hwm+1} ∈ S(p, d)` (B2), so B6 sufficiency gives `T4(pfx(π'))` — condition (v); and B1/B2 give `c_{hwm+1} ∉ Σ.B ∩ S(p, d)`, whence `pfx(π') ∉ Σ.B` — condition (vii). We retain (v) and (vii) in the list for readability, but they are consequences of (viii) via B6 sufficiency and B1/B2. Condition (iv) (`zeros(pfx(π')) ≤ 1`) is *not* implied — B6(iii) bounds only `zeros(p) + (d − 1) ≤ 3` — and is genuinely needed. Nelson's design contains no concept of principals appearing outside the delegation hierarchy, and Gregory's codebase provides no mechanism for it.

*Remark (next-reachability discipline).* Condition (viii) restricts delegation to the single address an ASN-0040 baptism would issue: O18's material baptism adds exactly `next(Σ.B, p, d)` (O17b) in the introducing transition, so the admissible delegate prefix is the `(hwm(Σ.B, p, d) + 1)`-th stream element `c_{hwm+1}`, whose predecessors `c₁, …, c_hwm` are already in `Σ.B` (B1, B2).

**Definition (delegated).** We name the conjunction of conditions (i)–(viii) above the *delegation predicate*, with a four-place signature: `delegated(Σ, Σ', π, π')` holds iff `Σ → Σ'`, `π ∈ Π_Σ`, `π' ∈ Π_{Σ'} ∖ Π_Σ` (condition (iii)), and conditions (i), (ii), (iv), (v), (vi), (vii), (viii) hold for `(π, π')` at `Σ`. Where a formula already binds a transition `Σ → Σ'`, we write `delegated_Σ(π, π')` as an abbreviation for `delegated(Σ, Σ', π, π')` with that same `Σ'`; the subscript form is used only when `Σ'` is named in the surrounding formula.
...
The reachability premise that the inductions below rely on is structural: states are reached by composing finitely many `→` transitions starting from `Σ₀`. Induction over reachable states does not require an external "finitely many transitions" axiom — it is induction over the path length, which is by definition a natural number.
...
Gregory's implementation corroborates the coupling: every registry write in udanax-green funnels through the single ISA-allocation point `findisatoinsertgr`, of which ASN-0040's `Bop` is the abstract image.
...
**NamespacePrincipalExclusivity (derived).** For any prefix `p`, namespace baptism and principal baptism are mutually exclusive futures: once `p ∈ Σ.B` (baptized as an organizational namespace by a non-delegating baptism, or already consumed by a prior delegation), no later transition can adopt `p` as a new principal's prefix. Proof: delegation of `p` requires freshness `p ∉ Σ.B` (condition (vii), itself a consequence of next-reachability (viii) via B1/B2) and, by O18, materially baptizes `p` at the introducing transition. Since `Σ.B` only grows (B0 of ASN-0040), `p ∈ Σ.B` makes (vii) fail permanently — so `p` can never become a principal's prefix, and a delegation that does adopt `p` consumes it against any second adoption.
...
The proof of O6 proceeds in two directions, under the precondition that `Σ` is reachable from `Σ₀` — the condition that licenses the appeal to O1a (a reachable-state invariant; the consolidated induction is in the Delegation section). *Forward:* we must show that for any principal `π` — by O1a (AccountOwnershipBoundary), which holds because `Σ` is reachable, every principal satisfies `zeros(pfx(π)) ≤ 1` — the relation `pfx(π) ≼ a` implies `pfx(π) ≼ acct(a)`. Two cases arise from the zero count.
...
- *Postconditions:* (a) `(A a ∈ dom(π') ∩ Σ'.B : ω_{Σ'}(a) = π')`; (b) `π'` satisfies O5 for allocations within `dom(π')`; (c) the delegation relation is satisfiable with `π'` as delegator for a sub-prefix `p''` (with `pfx(π') ≺ p''`) at every state at which O15 conditions (ii), (vi), (vii), and (viii) hold for `p''`. Conditions (ii) and (vi) are discharged at `Σ'` (derived in the proof from the original delegation's condition (vi)); conditions (vii) and (viii) are *not* asserted at `Σ'` for arbitrary `p''` but remain per-state obligations on `p''` (the recursive right is bounded by condition (viii), as stated in postcondition (c) above).
