...
The case distinction is exhaustive: T4 constrains `zeros(a) ∈ {0, 1, 2, 3}`, and each value is handled. In every case, `acct(a)` is a valid tumbler satisfying T4, and `zeros(acct(a)) ≤ 1`. The field extraction functions `N(a)` and `U(a)` are uniquely determined by T4(b) (UniqueParse), and component-wise access is decidable from T3 (CanonicalRepresentation), so the construction is computable from `a` alone. ∎
...
- *Definition:* `acct(a) = a` when `zeros(a) = 0`; `acct(a) = N(a) ++ [0] ++ U(a)` when `zeros(a) ≥ 1`, where `N(a)` and `U(a)` are the node and user fields extracted by `fields(a)` (T4(b) UniqueParse), with component-wise access decidable from T3 (CanonicalRepresentation).
...
The corollary's content: every delegator that participates in a chain of changes to `ω(a)` within `dom(π)` has a prefix extending `pfx(π)`. No delegator outside `dom(π)` can induce a change. We sharpen this to a *first-delegator* claim — the chain of delegations introducing principals into `dom(π)` after `π`'s own arrival traces back to `π`'s own act.

**Sub-lemma (FirstDelegatorIsπ).** Let `Σ` be reachable from `Σ₀`, `π ∈ Π_Σ` with `S_π(Σ) = ∅` (where `S_π(Σ̃) = { π* ∈ Π_{Σ̃} : pfx(π) ≺ pfx(π*) }` denotes the set of strict sub-delegates of `π` present in `Σ̃`), and let `π'' ∈ Π_{Σ'}` with `Σ →⁺ Σ'` and `pfx(π) ≺ pfx(π'')` be any sub-delegate of `π` introduced after `Σ`. Then there exists a finite chain `π = π^{(0)}, π^{(1)}, ..., π^{(k)} = π''` (`k ≥ 1`) such that for each `0 ≤ j < k`, the delegation `delegated_{Σ^{(j)}}(π^{(j)}, π^{(j+1)})` holds at some intermediate state `Σ^{(j)}` along `Σ →⁺ Σ'`.

The hypothesis `S_π(Σ) = ∅` makes the chain anchor at `π` definitive: every link in the chain falls strictly between `Σ` and `Σ'`. The hypothesis is satisfied, for instance, when `Σ` is the state at which `π` itself was introduced, or any state at which `π` has not yet exercised delegation authority — the canonical setting in which the first-delegator claim is invoked.

*Proof.* By FiniteRegistry, `|S_π(Σ̃)| < ∞` for every reachable `Σ̃`; by O12, `S_π` grows monotonically along `Σ →⁺ Σ'`. We induct on `|S_π(Σ̃)|` at the transition `Σ̃ → Σ̃'` that introduces `π''` (the unique transition along `Σ →⁺ Σ'` with `π'' ∈ Π_{Σ̃'} ∖ Π_{Σ̃}`, well-defined by O15).

*Base case `|S_π(Σ̃)| = 0`.* No sub-delegate of `π` exists in `Σ̃`. By O15, `π''` was delegated by some `π_d ∈ Π_{Σ̃}`. We derive `pfx(π) ≼ pfx(π_d)` directly from the delegation conditions, without invoking AccountLevelPermanence (whose `a ∈ dom(π) ∩ Σ.B` precondition would be unsatisfiable when `π''`'s domain contains no allocated address yet — a legitimate scenario, since delegation may create a namespace before any allocation occurs in it). Condition (ii) of `delegated_{Σ̃}(π_d, π'')` asserts that `π_d` is the most-specific covering principal of `pfx(π'')` in `Π_{Σ̃}`, so `pfx(π_d) ≼ pfx(π'')`. By the sub-lemma's hypothesis `pfx(π) ≺ pfx(π'')`, `π ∈ Π_{Σ̃}` itself covers `pfx(π'')`; by (ii)'s most-specific clause, `#pfx(π) ≤ #pfx(π_d)`. Both `pfx(π)` and `pfx(π_d)` are prefixes of the same tumbler `pfx(π'')`; by the covering-chain lemma (cited from O2's proof — any two tumbler prefixes of a common address are `≼`-comparable), they are `≼`-comparable. With `#pfx(π) ≤ #pfx(π_d)`, the comparison forces `pfx(π) ≼ pfx(π_d)`. By the empty-`S_π` hypothesis, no principal in `Π_{Σ̃}` strictly extends `pfx(π)`. Hence `#pfx(π_d) ≤ #pfx(π)`, giving `#pfx(π_d) = #pfx(π)`. Combined with `pfx(π) ≼ pfx(π_d)`, equal length forces `pfx(π_d) = pfx(π)`; by O1b (PrefixInjectivity), `π_d = π`. The chain `π → π''` has length `k = 1`.

*Inductive step `|S_π(Σ̃)| = m + 1`.* By O15, `π''` was delegated by some `π_d ∈ Π_{Σ̃}`. Two cases on `π_d`. (a) `π_d = π`: chain `π → π''` of length `k = 1`. (b) `π_d ≠ π` and `pfx(π) ≺ pfx(π_d)` (the latter derived as in the base case — condition (ii) plus the covering-chain lemma yields `pfx(π) ≼ pfx(π_d)`; strictness follows from `π_d ≠ π` via O1b, which forbids equal prefixes for distinct principals): then `π_d ∈ S_π(Σ̃)`. We show that `π_d` was introduced strictly after `Σ`, so that the inductive hypothesis applies to it. By the sub-lemma's hypothesis, `S_π(Σ) = ∅`; combined with `π_d ∈ S_π(Σ̃)`, monotonicity of `S_π` along `Σ →⁺ Σ̃` (from O12) gives `π_d ∈ S_π(Σ̃) ∖ S_π(Σ)`. Hence `π_d ∉ Π_Σ`, and there is a unique transition `Σ̃_d^- → Σ̃_d` along `Σ →⁺ Σ̃` that introduces `π_d` (well-defined by O15, with `Σ̃_d^-` denoting its predecessor and `π_d ∈ Π_{Σ̃_d} ∖ Π_{Σ̃_d^-}`). At the predecessor `Σ̃_d^-`, `π_d ∉ S_π(Σ̃_d^-)` while `π_d ∈ S_π(Σ̃) \ S_π(Σ̃_d^-)`; by O12 monotonicity `S_π(Σ̃_d^-) ⊆ S_π(Σ̃)`, so `|S_π(Σ̃_d^-)| ≤ |S_π(Σ̃)| - 1 = m`. The premises of the inductive hypothesis hold for `π_d` (replace `π''` by `π_d`, `Σ'` by `Σ̃_d`): `Σ` reachable, `S_π(Σ) = ∅`, `π_d ∈ Π_{Σ̃_d}` with `Σ →⁺ Σ̃_d` and `pfx(π) ≺ pfx(π_d)`. The inductive measure at the introducing transition `Σ̃_d^- → Σ̃_d` is `|S_π(Σ̃_d^-)| ≤ m < m + 1`. Apply the inductive hypothesis: a chain `π = π^{(0)}, ..., π^{(k')} = π_d` exists, each link `delegated_{Σ^{(j)}}` at some state along `Σ →⁺ Σ̃_d`, which is a sub-path of `Σ →⁺ Σ'`. Concatenate with the final link `delegated_{Σ̃}(π_d, π'')`: the chain `π = π^{(0)}, ..., π^{(k')} = π_d, π^{(k'+1)} = π''` of length `k = k' + 1` witnesses the claim.

The chain is finite because each link strictly increases the prefix length (condition (i) of `delegated`), and prefix lengths are bounded above by `#pfx(π'')`. ∎

The sub-lemma yields the *first-delegator* form: every principal newly introduced into `dom(π)` after `Σ` is reached by a chain of delegations whose initial step is `π`'s own delegation act. This is Nelson's "forevermore" in its multi-step form — the chain of delegators inducing changes inside `dom(π)` always begins with `π`'s own act, and no external party can ever insert into the chain.
...
*Case 1: `zeros(pfx(π)) = 0` (node-level principal).* Every component of `pfx(π)` is strictly positive — T4(a) requires that every non-separator component be positive, and the absence of zeros means every component is a non-separator. By T4(c) (LevelDetermination), a tumbler with no zeros is a node-level address, and by T4(b) its node field is the tumbler itself: `N(pfx(π)) = pfx(π)`, with `#N(pfx(π)) = #pfx(π)`.

Since `pfx(π) ≼ a`, the first `#pfx(π)` components of `a` match those of `pfx(π)` and are therefore all strictly positive. By T4(b), the node field `N(a)` consists of the components of `a` preceding the first zero-valued component (or all components of `a` if no zero occurs). Since positions `1` through `#pfx(π)` of `a` are all positive, the first zero of `a` — if it exists — occurs at position `#pfx(π) + 1` or later. Therefore `#N(a) ≥ #pfx(π) = #N(pfx(π))`. The first `#N(pfx(π))` components of `N(a)` are `a₁, ..., a_{#pfx(π)}`, which equal `pfx(π)₁, ..., pfx(π)_{#pfx(π)}` by the prefix relation, and these are exactly the components of `N(pfx(π))`. Hence `N(pfx(π)) ≼ N(a)`.
...
*Case 2: `zeros(pfx(π)) = 1` (account-level principal).* By T4(b) and T4(a), the prefix has the form `N₁. ... .Nₐ . 0 . U₁. ... .Uᵦ` with `α ≥ 1` and `β ≥ 1` (non-empty field constraint from T4(a)), where every `Nᵢ > 0` (positive-component constraint from T4(a)) and every `Uⱼ > 0`. The node field is `N(pfx(π)) = [N₁, ..., Nₐ]`, and the single zero sits at position `α + 1`.
...
By T4(b), the node field `N(a)` consists of the components of `a` before `a`'s first zero. Since positions `1` through `α` are all positive and position `α + 1` is zero, the first zero of `a` is at position `α + 1`. Hence `N(a) = [a₁, ..., aₐ] = [N₁, ..., Nₐ] = N(pfx(π))`. The prefix relation holds with equality: `N(pfx(π)) = N(a)`, which implies `N(pfx(π)) ≼ N(a)`.
...
- *Postconditions:* `(E Σ', a' : Σ →⁺ Σ' ∧ a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ a ∈ Σ'.B)` — there exists a reachable successor state `Σ'` (reached via a finite sequence of authorized baptismal transitions from `Σ`) and a new address `a' ∈ dom(π)` such that `a'` is effectively owned by `π` in `Σ'`, and the original address `a` remains allocated and unmodified throughout the sequence. The body's construction exhibits:
  ...
  - For `zeros(pfx(π)) = 0`: a cooperative chain of `u + 2` baptisms, with `u ∈ ℕ_{>0} ∖ S'` chosen via the sharpened `S' = { U^{(i)}_1 : π_i \text{ is Form B with } #pfx(π_i) = #pfx(π) + 2 }` analysis. The first `u` baptisms allocate `pfx(π).0.1, ..., pfx(π).0.u` sequentially within `S(pfx(π), 2)` (`next` advances one slot per call); each slot `pfx(π).0.k` is baptized by its most-specific covering principal — `π_k` when `k ∈ S'`, `π` when `k ∉ S'`. The final two baptisms (`pfx(π).0.u.0.1`, `pfx(π).0.u.0.1.0.1 = a'`) are field-openings by `π`, both O5-authorized via the Form A/B classification at the respective depths.
...
- *Invariant:* In every reachable state, every principal can produce an address it effectively owns — the fork postcondition is universally satisfiable up to a finite sequence of baptisms.
...
| `acct(a)` | When `zeros(a) = 0`: `acct(a) = a`; when `zeros(a) ≥ 1`: truncation through user field | from T4(b), T3 |
