...
| O1a | `(A π ∈ Π : zeros(pfx(π)) ≤ 1)` — ownership principals exist only at node or account level | axiom (`pfx` signature constraint) |
| O1b | `pfx` is injective — distinct principals have distinct prefixes | axiom (`pfx` signature constraint) |
...
| MostSpecificCoveringUnique | Whenever two covering principals of a tumbler tie for maximum prefix length, they are equal — uniqueness of the most-specific covering principal | from Covering-chain lemma, O1b, Prefix |
