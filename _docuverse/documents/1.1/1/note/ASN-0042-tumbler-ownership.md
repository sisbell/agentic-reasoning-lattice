...
- *Preconditions:* `π ∈ Π`.
- *Postconditions:* (a) `pfx(π) ∈ T`. (b) `T4(pfx(π))` — the prefix is a valid tumbler satisfying FieldSeparatorConstraint.
...
*Trajectory for `zeros(pfx(π)) = 0` (node-level principal).* Sharpen the `u`-selection. Define `S' = { U^{(i)}_1 : π_i \text{ is Form B with } #pfx(π_i) = #pfx(π) + 2 } ⊆ S` — the first user-field component of *those* Form B sub-delegates whose entire prefix is `pfx(π).0.U^{(i)}_1` (the only sub-delegates capable of being the most-specific covering principal of any user-level slot `pfx(π).0.k`). The non-coverage analysis for `a' = pfx(π).0.u.0.1.0.1` only ever required `u ∉ S'` (longer Form B prefixes fail at position `#pfx(π) + 3` regardless of `u`); choose `u ∈ ℕ_{>0} ∖ S'` with `u > hwm_0` (where `hwm_0 := hwm(Σ.B, pfx(π), 2)` is the current high-water mark of the user-field child stream — such `u` exists by T0a since `|S'| ≤ |Π_Σ| < ∞` and `S' ∪ {1, ..., hwm_0}` remains finite).

Baptism within `S(pfx(π), 2) = (pfx(π).0.1, pfx(π).0.2, ...)` is strictly sequential: each `next` call advances the namespace by exactly one slot — `π` cannot leap directly to `pfx(π).0.u` for `u > hwm_0 + 1` in a single baptismal step. By ASN-0040's `next` semantics, given `Σ.B` already containing baptisms up to `pfx(π).0.{hwm_0}` (and no slots beyond, by definition of `hwm`), the next call to `next(Σ.B, pfx(π), 2)` produces `pfx(π).0.{hwm_0 + 1}` via sibling advance (or via field-opening when `hwm_0 = 0`). The trajectory therefore traverses `pfx(π).0.{hwm_0 + 1}, pfx(π).0.{hwm_0 + 2}, ..., pfx(π).0.u` in order, with each new slot baptized by its most-specific covering principal: `π_k` when `k ∈ S'` (the sub-delegate with `pfx(π_k) = pfx(π).0.k`), `π` when `k ∉ S'`. Both assignments are O5-authorized — `π_k` is maximally specific for its own prefix (by injectivity of `pfx` and the Form B (=length) classification); `π` is maximally specific when no Form B (=length) sub-delegate has `U^{(i)}_1 = k`. By induction on the step index `j ∈ {1, ..., u − hwm_0}`, after `j` such authorized baptisms `Σ → Σ_1 → ... → Σ_j`, `B_{Σ_j} ∩ S(pfx(π), 2)` contains `pfx(π).0.{hwm_0 + 1}, ..., pfx(π).0.{hwm_0 + j}` in addition to whatever subset of `{pfx(π).0.1, ..., pfx(π).0.{hwm_0}}` was already present in `Σ.B`, and the next call to `next(B_{Σ_j}, pfx(π), 2)` produces `pfx(π).0.{hwm_0 + j + 1}` via sibling advance. After `u − hwm_0` steps, `b_{u − hwm_0} := pfx(π).0.u ∈ Σ_{u − hwm_0}.B` with `ω(b_{u − hwm_0}) = π` (since `u ∉ S'`).

`π` then completes the trajectory with two further field-opening baptisms on its own (children-empty) parents:
- `b_{(u − hwm_0) + 1} = inc(b_{u − hwm_0}, 2) = pfx(π).0.u.0.1` (document-level, `zeros = 2`),
- `b_{(u − hwm_0) + 2} = inc(b_{(u − hwm_0) + 1}, 2) = pfx(π).0.u.0.1.0.1 = a'` (element-level, `zeros = 3`).

Both are O5-authorized for `π` by the same Form A / Form B (>length) / Form B (=length with `u ∉ S'`) classification applied at the respective depths. Total trajectory: `(u − hwm_0) + 2` baptisms — `π` performs `((u − hwm_0) − |S' ∩ {hwm_0 + 1, ..., u}|) + 2`, and the immediate sub-delegates contribute `|S' ∩ {hwm_0 + 1, ..., u}|` cooperative baptisms within the user-field range traversed. When this intersection is empty (e.g., when `S' = ∅`, or when all of `S'` lies at or below `hwm_0`), the trajectory is wholly π-driven; otherwise each cooperative baptism is an act the corresponding `π_k` is independently O5-authorized to perform, so every transition in the chain `Σ →⁺ Σ_{(u − hwm_0) + 2}` is valid under the abstract spec's reachability relation. The smallest valid `u` is `hwm_0 + 1` when `hwm_0 + 1 ∉ S'`, yielding the minimal trajectory of `3` baptisms.

*Unilateral feasibility.* The existence claim above is satisfied by *some* `Σ →⁺ Σ'` in the abstract reachability relation, which the spec admits for any composition of authorized acts. We sharpen this to a *unilateral* claim — one in which `π` alone suffices to drive the trajectory — under a precisely characterized condition. Specifically:

> *Unilateral O10★.* When `hwm_0 + 1 ∉ S'` (equivalently, no Form B (=length) sub-delegate of `π` has its prefix at the very next slot), there exists a chain `Σ →⁺ Σ'` of exactly `3` baptisms, all performed by `π`, witnessing the fork postcondition with `u = hwm_0 + 1`.

The proof is the trajectory above instantiated with `u = hwm_0 + 1`: `π` calls `next(Σ.B, pfx(π), 2)` once to produce `pfx(π).0.{hwm_0 + 1}` (a slot `π` is most-specific covering principal of, by `hwm_0 + 1 ∉ S'`), then two field-openings.

When `hwm_0 + 1 ∈ S'`, unilateral feasibility from `Σ` fails: `π`'s first `next` call produces a slot `π` is not O5-authorized for. `π` must wait for the corresponding `π_{hwm_0 + 1}` to advance the namespace (an act that sub-delegate is independently authorized to perform). Once `π_{hwm_0 + 1}` baptizes its own slot, `hwm` advances to `hwm_0 + 1`, and the situation repeats from `Σ̃` with `hwm_0' = hwm_0 + 1`. Since `S'` is finite, after at most `|S'|` sub-delegate baptisms the namespace reaches a state `Σ̃` where `hwm_0' + 1 ∉ S'` — at that point the unilateral trajectory applies. This decomposition exposes the *cooperative gap* as a finite delay rather than a permanent obstruction.

The cooperative gap has a precise operational interpretation. Nelson's "denial as fork" promise — π can always create a new owned address — is unconditionally true at the abstract reachability level: the existential `Σ →⁺ Σ'` holds always. But the gap is real: if a sub-delegate `π_k` with `pfx(π_k) = pfx(π).0.{hwm_0 + 1}` refuses to baptize, `π` cannot proceed unilaterally through the user-field child stream. The ownership model treats this gap as a *liveness* concern — `π_k`'s refusal is a behavior the ownership model has no authority to constrain — separate from the *safety* guarantee of O10 (π can fork). The cooperative case never *violates* O10; it merely defers π's exercise of the right until the sub-delegate acts. In practice, when `S' = ∅` (no immediate sub-delegates at user level) the unilateral trajectory always applies — this is the canonical setting in which "denial as fork" is purely π's unilateral act, and the architecture's design intent is realized without qualification.
...
For the `zeros(pfx(π)) = 0` trajectory, apply the Form A/Form B classification to a user-level slot `pfx(π).0.k`: Form A sub-delegates have a positive component at position `#pfx(π) + 1` but `pfx(π).0.k` has `0` there — no Form A covers. Form B sub-delegates of length `> #pfx(π) + 2` have length exceeding `pfx(π).0.k` and so are not prefixes of it. Form B sub-delegates of length exactly `#pfx(π) + 2` cover `pfx(π).0.k` iff `U^{(i)}_1 = k`, i.e., iff `k ∈ S'`. Hence the most-specific covering principal of `pfx(π).0.k` is `π_k` (when `k ∈ S'`) or `π` (otherwise), matching the trajectory's actor assignment. For the deeper `b_{u+1} = pfx(π).0.u.0.1` and `b_{u+2} = a'`: the same classification at each depth rules out coverage — Form A (`0` at position `#pfx(π) + 1`), Form B (>length) of pfx length `≥ #pfx(π) + 3` having positive `U^{(i)}_2` at position `#pfx(π) + 3` where `b_{u+1}` and `b_{u+2}` carry the user-document separator `0`, and Form B (=length) ruled out by `u ∉ S'`. `π` is the most-specific covering principal of both.
...
*Trajectory closure.* By O15, baptism does not introduce new principals — only delegation does — and the trajectory contains no delegation acts (only baptisms by `π` and, in the cooperative case, by immediate sub-delegates `π_k` of their own prefixes). Hence `Π_{Σ_{j-1}} = Π_Σ` for every `j`, and the Form A/B classification (with its derived sets `S, S'`) computed at `Σ` remains valid throughout. The composition of authorized baptisms yields `Σ →⁺ Σ'` with `a' ∈ Σ'.B` and `ω_{Σ'}(a') = π`. The fork operation's postcondition is satisfiable in every reachable state via this trajectory. ∎
...
- *Postconditions:* `(E Σ', a' : Σ →⁺ Σ' ∧ a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ a ∈ Σ'.B)` — there exists a reachable successor state `Σ'` (reached via a finite sequence of authorized baptismal transitions from `Σ`) and a new address `a' ∈ dom(π)` such that `a'` is effectively owned by `π` in `Σ'`, and the original address `a` remains allocated and unmodified throughout the sequence. The body's construction exhibits:
  - For `zeros(pfx(π)) = 1`: a chain of `2` baptisms by `π` (`pfx(π) → pfx(π).0.m → pfx(π).0.m.0.1 = a'`), each O5-authorized for `π` because no sub-delegate covers any document-level extension of `pfx(π)` (the `zeros(pfx(π_i)) = 1` saturation argument).
  - For `zeros(pfx(π)) = 0`: a cooperative chain of `(u − hwm_0) + 2` baptisms (where `hwm_0 := hwm(Σ.B, pfx(π), 2)`), with `u ∈ ℕ_{>0} ∖ S'` and `u > hwm_0` chosen via the sharpened `S' = { U^{(i)}_1 : π_i \text{ is Form B with } #pfx(π_i) = #pfx(π) + 2 }` analysis. The first `u − hwm_0` baptisms allocate `pfx(π).0.{hwm_0 + 1}, ..., pfx(π).0.u` sequentially within `S(pfx(π), 2)` starting from the current high-water mark (`next` advances one slot per call); each slot `pfx(π).0.k` for `k ∈ {hwm_0 + 1, ..., u}` is baptized by its most-specific covering principal — `π_k` when `k ∈ S'`, `π` when `k ∉ S'`. The final two baptisms (`pfx(π).0.u.0.1`, `pfx(π).0.u.0.1.0.1 = a'`) are field-openings by `π`, both O5-authorized via the Form A/B classification at the respective depths. The minimal valid trajectory has length `3` (achieved when `hwm_0 + 1 ∉ S'`).

  In both cases, no sub-delegate covers `a'` (verified by the Form A/B classification and the O1a `zeros ≤ 1` constraint on sub-delegate prefixes), so `ω_{Σ'}(a') = π`. The reachability `Σ →⁺ Σ'` is the reflexive-transitive closure of all authorized transitions, including baptisms by sub-delegates of their own prefixes (each independently O5-authorized by injectivity of `pfx`); hence the trajectory's mixed actor assignment poses no obstacle to the existence claim.
- *Unilateral postcondition (Unilateral O10★):* When `zeros(pfx(π)) = 1`, or when `zeros(pfx(π)) = 0 ∧ hwm_0 + 1 ∉ S'` (where `hwm_0 = hwm(Σ.B, pfx(π), 2)` and `S' = { U^{(i)}_1 : π_i \text{ is Form B (=length) sub-delegate of } π }`), there exists a chain `Σ →⁺ Σ'` of `2` or `3` baptisms respectively, *all performed by π*, witnessing the existence claim. When `zeros(pfx(π)) = 0 ∧ hwm_0 + 1 ∈ S'`, the witnessing trajectory requires cooperation from a finite, identifiable set of immediate sub-delegates `{π_k : k ∈ S' ∩ {hwm_0 + 1, ..., u}}` — each performing a baptism it is independently O5-authorized to perform. The cooperative gap is a *liveness* concern (sub-delegate may delay its baptism); O10 itself is a *safety* statement and holds unconditionally.
- *Invariant:* In every reachable state, every principal can produce an address it effectively owns — the fork postcondition is universally satisfiable up to a finite sequence of baptisms. Unilateral satisfaction is guaranteed for `zeros(pfx(π)) = 1` principals always, and for `zeros(pfx(π)) = 0` principals whenever the next user-field slot is not pre-claimed by an immediate sub-delegate.
