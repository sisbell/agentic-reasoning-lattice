...
*Base case `|S_π(Σ̃)| = 0`.* No sub-delegate of `π` exists in `Σ̃`. By O15, `π''` was delegated by some `π_d ∈ Π_{Σ̃}`. Apply the single-transition AccountLevelPermanence (whose reachability and `π ∈ Π_{Σ̃}` premises hold by hypothesis and iterated O12) to any `a ∈ dom(π'') ⊆ dom(π)` whose effective owner changed in this transition: it yields `pfx(π) ≼ pfx(π_d)`. Combined with condition (ii) of `delegated_{Σ̃}(π_d, π'')` — which asserts `π_d` is the most-specific covering principal of `pfx(π'')` in `Π_{Σ̃}` — and the fact that `π ∈ Π_{Σ̃}` itself covers `pfx(π'')` (since `pfx(π) ≺ pfx(π'')`), we have `#pfx(π) ≤ #pfx(π_d)`. By the empty-`S_π` hypothesis, no principal in `Π_{Σ̃}` strictly extends `pfx(π)`. Hence `#pfx(π_d) ≤ #pfx(π)`, giving `#pfx(π_d) = #pfx(π)`. Both prefixes lie within `pfx(π'')` and have equal length, so `pfx(π_d) = pfx(π)`; by O1b (PrefixInjectivity), `π_d = π`. The chain `π → π''` has length `k = 1`.

*Inductive step `|S_π(Σ̃)| = m + 1`.* By O15, `π''` was delegated by some `π_d ∈ Π_{Σ̃}`. Two cases on `π_d`. (a) `π_d = π`: chain `π → π''` of length `k = 1`. (b) `π_d ≠ π` and `pfx(π) ≺ pfx(π_d)` (the latter from condition (ii) and the AccountLevelPermanence step as in the base case): then `π_d ∈ S_π(Σ̃)`. We show that `π_d` was introduced strictly after `Σ`, so that the inductive hypothesis applies to it. By the sub-lemma's hypothesis, `S_π(Σ) = ∅`; combined with `π_d ∈ S_π(Σ̃)`, monotonicity of `S_π` along `Σ →⁺ Σ̃` (from O12) gives `π_d ∈ S_π(Σ̃) ∖ S_π(Σ)`. Hence `π_d ∉ Π_Σ`, and there is a unique transition `Σ̃_d^- → Σ̃_d` along `Σ →⁺ Σ̃` that introduces `π_d` (well-defined by O15, with `Σ̃_d^-` denoting its predecessor and `π_d ∈ Π_{Σ̃_d} ∖ Π_{Σ̃_d^-}`). At the predecessor `Σ̃_d^-`, `π_d ∉ S_π(Σ̃_d^-)` while `π_d ∈ S_π(Σ̃) \ S_π(Σ̃_d^-)`; by O12 monotonicity `S_π(Σ̃_d^-) ⊆ S_π(Σ̃)`, so `|S_π(Σ̃_d^-)| ≤ |S_π(Σ̃)| - 1 = m`. The premises of the inductive hypothesis hold for `π_d` (replace `π''` by `π_d`, `Σ'` by `Σ̃_d`): `Σ` reachable, `S_π(Σ) = ∅`, `π_d ∈ Π_{Σ̃_d}` with `Σ →⁺ Σ̃_d` and `pfx(π) ≺ pfx(π_d)`. The inductive measure at the introducing transition `Σ̃_d^- → Σ̃_d` is `|S_π(Σ̃_d^-)| ≤ m < m + 1`. Apply the inductive hypothesis: a chain `π = π^{(0)}, ..., π^{(k')} = π_d` exists, each link `delegated_{Σ^{(j)}}` at some state along `Σ →⁺ Σ̃_d`, which is a sub-path of `Σ →⁺ Σ'`. Concatenate with the final link `delegated_{Σ̃}(π_d, π'')`: the chain `π = π^{(0)}, ..., π^{(k')} = π_d, π^{(k'+1)} = π''` of length `k = k' + 1` witnesses the claim.
