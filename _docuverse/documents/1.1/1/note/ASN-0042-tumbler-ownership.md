...
A delegation transition falls in the *baptism* branch, not the frame branch: O18 (DelegationBaptizes) records `pfx(π') ∈ Σ'.B ∖ Σ.B` for the newly introduced principal, and condition (v) fixes that prefix as `next(Σ.B, p, d)`. The frame branch therefore covers only genuine no-op-on-`B` operations. (Implementation provenance: ASN-0040's `Bop` is the abstract image of udanax-green's single allocation point for registry writes.)
