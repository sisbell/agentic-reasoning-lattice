...
- *Postconditions:* (a) `(A a ∈ odom(π') ∩ Σ'.B : ω_{Σ'}(a) = π')`; (b) `π'` satisfies O5 for allocations within `odom(π')`; (c) immediately upon entry at `Σ'`, `π'` may delegate to a new principal whose prefix is a next-reachable first child `p'' = next(Σ'.B, p, d)` of an already-baptized prefix (the form imposed by O17b), with binding obligations (iii) [structural-tier] and (v) [fresh-valid: `T4(p'') ∧ p'' ∉ Σ'.B`]. The admitted `p''` is thus a `next`-reachable single-step stream extension, not an arbitrary strict descendant.
...
| Delegation | `π'` introduced into `Π` by act of `π`, with `pfx(π) ≺ pfx(π')`, `π` most-specific covering principal, no existing principal extends `pfx(π')`, `zeros(pfx(π')) ≤ 1`, and `T4(pfx(π')) ∧ pfx(π') ∉ Σ.B` (condition (v), fresh-valid) — the conditions form the complete admission gate; the next-reachable baptism form `pfx(π') = next(Σ.B, p, d)` is supplied by O17b, not by (v) | introduced |
