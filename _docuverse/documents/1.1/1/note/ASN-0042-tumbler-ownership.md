...
*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`. Assume the pre-fork state `Σ_pre` is reached after earlier baptisms `a₁ = [1, 0, 2, 0, 3, 0, 1]` (allocated by `π_N` pre-delegation, per State Σ₁) and `a₂ = [1, 0, 2, 0, 5, 0, 1]` (allocated by `π_A` post-delegation, per the *Allocation* paragraph) have established `hwm(Σ_pre.B, [1, 0, 2], 2) = 5` (the largest document-field index used so far under `[1, 0, 2]`). Set `m = hwm + 1 = 6`.
...
*The delegate persists with an unchanged prefix.* Let `Σ_d^{post}` denote the successor state of the delegation transition `Σ_d → Σ_d^{post}` at which `π'` enters `Π`; by condition (iii) of the delegation relation, `π' ∈ Π_{Σ_d^{post}} ∖ Π_{Σ_d}`. The hypothesis `π' ∈ Π_{Σ'}` is given directly, and combined with O15 (PrincipalClosure) — `π'` can enter `Π` only via the delegation transition that witnesses `delegated_{Σ_d}(π, π')` — it forces the trajectory `Σ_d →⁺ Σ'` to pass through `Σ_d^{post}`. By O12 (PrincipalPersistence) iterated along `Σ_d^{post} →^* Σ'`, `Π_{Σ_d^{post}} ⊆ Π_{Σ'}`, consistent with the hypothesis. By O13 (PrefixImmutability) iterated along the same sub-trajectory, `pfx_{Σ'}(π') = pfx_{Σ_d^{post}}(π')`. The delegate is present at `Σ'` with the prefix it received at the delegation transition.
...
| O8 | `Σ_d reachable ∧ delegated_{Σ_d}(π, π') ∧ Σ_d →⁺ Σ' ∧ π' ∈ Π_{Σ'} ∧ a ∈ dom(π') ∩ Σ'.B ⟹ ω_{Σ'}(a) ≠ π` — delegating parent never regains ownership | from Delegation, O2, O12, O13, T8 |
...
| PrefixBaptismCoupling | `(A Σ reachable, π ∈ Π_Σ : pfx(π) ∈ Σ.B)` — every principal's prefix is itself baptized in every reachable state | from O14(vii), O15, O18, T8 |
