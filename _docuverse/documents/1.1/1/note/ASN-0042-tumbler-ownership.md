...
*Baptismal coupling of delegation.* The trajectory analysis turns on a structural fact bridging the principal registry and the baptismal registry: whenever a sub-delegate `π_k ∈ Π_Σ` of `π` has prefix `pfx(π_k) = pfx(π).0.k`, the prefix `pfx(π).0.k` is itself in `Σ.B`. Delegation establishes a new principal, and a principal cannot enter the registry without an allocated prefix — the act of delegation by `π` to a sub-account at slot `k` materially baptizes `pfx(π).0.k` and binds the new principal to it. This is the abstract counterpart to an implementation invariant: delegated account slots reside in the granfilade tree, where `findpreviousisagr` scans the entire subtree under the parent's prefix without distinguishing delegated from non-delegated entries, because the granfilade *is* the global baptismal registry and the principal registry's prefixes occupy it. Define `S' := { U^{(i)}_1 : π_i \text{ is Form B with } #pfx(π_i) = #pfx(π) + 2 } ⊆ S` — the first user-field component of *those* Form B sub-delegates whose entire prefix is `pfx(π).0.U^{(i)}_1` (the only sub-delegates capable of being the most-specific covering principal of any user-level slot `pfx(π).0.k`). The coupling yields a sharp consequence: every such prefix `pfx(π).0.{U^{(i)}_1}` lies in `Σ.B`, hence `U^{(i)}_1 ≤ hwm_0 := hwm(Σ.B, pfx(π), 2)` for every Form B (=length) sub-delegate. Therefore `S' ⊆ {1, ..., hwm_0}`, and in particular `hwm_0 + 1 ∉ S'` *in every reachable state*. The next slot beyond the current high-water mark is never a slot already pre-claimed by an immediate sub-delegate of `π`.
...
- *Unilateral postcondition (Unilateral O10★):* In every reachable state `Σ` and for every `π ∈ Π_Σ`, the existence claim is witnessed by a chain `Σ →⁺ Σ'` of `2` (when `zeros(pfx(π)) = 1`) or `3` (when `zeros(pfx(π)) = 0`) baptisms, *all performed by π alone*. The unilateral guarantee is unconditional: the baptismal coupling between delegation and baptism ensures that the next user-field slot beyond `hwm_0` is never pre-claimed by a sub-delegate, so `π`'s sibling-advance call always lands on a slot it is O5-authorized for. This matches Nelson's design intent (forking is the architectural answer to non-ownership; "full control... forevermore" rules out cooperation as a forking mode) and Gregory's allocator behavior (`findpreviousisagr` advances past delegated slots without coordination).
