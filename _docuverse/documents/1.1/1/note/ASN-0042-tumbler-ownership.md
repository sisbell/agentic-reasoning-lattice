...
- *Postconditions:* `(E Σ', a' : Σ → Σ' ∧ a' ∈ dom(π) ∩ Σ'.B ∧ ω_{Σ'}(a') = π ∧ a ∈ Σ'.B)` — there exists a successor state `Σ'` (reached via a single authorized baptism from `Σ`) and a new address `a' = pfx(π).0.{hwm_0 + 1}` (where `hwm_0 := hwm(Σ.B, pfx(π), 2)`) such that `a'` is effectively owned by `π` in `Σ'`, and the original address `a` remains allocated and unmodified. The construction satisfies O5 (`π` is the most-specific covering principal of `a'` by the Form A/B classification and O18 baptismal coupling) and ASN-0040's B6 (with `d = 2`, `zeros(pfx(π)) + (d − 1) = zeros(pfx(π)) + 1 ∈ {1, 2} ≤ 3`); `a'` is at user level (`zeros = 1`) when `zeros(pfx(π)) = 0` and at document level (`zeros = 2`) when `zeros(pfx(π)) = 1`.
...
| O10 | Non-ownership of target yields a fork: new address under the requesting principal's domain | from O1a, O1b, O6, PrefixBaptismCoupling, TA5(c), TA5(d), ASN-0040 `next`, ASN-0040 `hwm`, ASN-0040 B6, B0 (ASN-0040) |
