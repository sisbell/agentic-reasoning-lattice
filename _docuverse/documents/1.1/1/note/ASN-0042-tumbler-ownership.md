...
*Field-opening boundary case.* The trajectory above used the sibling-advance branch of `next` (`hwm_0 = 5`). The complementary field-opening branch arises when `hwm_0 = 0` — no document-field child of `pfx(π_A)` has yet been baptized. Consider the alternative pre-fork state `Σ_pre^0 = Σ_1` (the state immediately after `π_A`'s delegation, before any document-level baptism under `[1, 0, 2]`). Then `hwm(Σ_1.B, [1, 0, 2], 2) = 0`, and the single baptism reduces to the field-opening branch: `b_1 = next(Σ_1.B, [1, 0, 2], 2) = inc([1, 0, 2], 2) = [1, 0, 2, 0, 1]` (TA5(d), appending the document-field separator and first document index). The resulting `a' = [1, 0, 2, 0, 1]` has `zeros = 2` (document level), with `pfx(π_A) = [1, 0, 2] ≼ a'`. *O5 check at `Σ_1`:* `π_A` is the most-specific covering principal in `Π_{Σ_1} = {π_N, π_M, π_A}` (no sub-delegate of `π_A` exists yet — `Π_{Σ_1} ∖ {π_A}` contains only `π_N` with prefix `[1]` and `π_M` with prefix `[2]`, neither extending `[1, 0, 2]`). ✓ *B6 check:* `d = 2`, `zeros(pfx(π_A)) + (d − 1) = 1 + 1 = 2 ≤ 3`. ✓ *O10(a):* `pfx(π_A) ≼ a'` and `π_A` has the longest matching prefix in `Π_{Σ'}`, so `ω_{Σ'}(a') = π_A`. ✓ *O10(b):* every pre-existing address persists unchanged (T8) and `Π` is unchanged (O15). ✓ The field-opening branch exhibits the unilateral fork at the boundary case where no prior document baptism exists — `π_A` can fork from the very moment of delegation, requiring no preceding allocation history under `pfx(π_A)`.
...
*Non-coverage analysis.* We show `ω_{Σ'}(a') = π` by ruling out sub-delegate coverage of `a'`. Every sub-delegate `π_i` of `π` (i.e., `π_i ∈ Π_Σ` with `pfx(π) ≺ pfx(π_i)`) satisfies `zeros(pfx(π_i)) ≤ 1` by O1a. Classify by the component of `pfx(π_i)` at position `#pfx(π) + 1`:
  ...
  Restricting attention to length-(#pfx(π) + 2) Form B sub-delegates — the only ones that can cover `a'` by length, per the length analysis above — apply PrefixBaptismCoupling: for each such `π_i ∈ Π_Σ`, the entire prefix is `pfx(π_i) = pfx(π).0.U^{(i)}_1`, and PrefixBaptismCoupling places this prefix in `Σ.B`. Hence `pfx(π).0.U^{(i)}_1 ∈ Σ.B`, so `U^{(i)}_1 ∈ S(pfx(π), 2) ∩ Σ.B` (where `S(pfx(π), 2)` denotes the set of admissible depth-2 child indices under `pfx(π)`), and therefore `U^{(i)}_1 ≤ hwm_0` by definition of `hwm`. So `U^{(i)}_1 ≠ hwm_0 + 1`, and no length-(#pfx(π) + 2) Form B sub-delegate covers `a'`. Combined with the prior exclusion of longer Form B sub-delegates by length, no Form B sub-delegate covers `a'`. (We make no claim about the first user-field components `U^{(i)}_1` of *longer* Form B sub-delegates, whose full prefixes — not the truncation `pfx(π).0.U^{(i)}_1` — are what PrefixBaptismCoupling places in `Σ.B`; coverage of `a'` is decided by length and the length-(#pfx(π) + 2) case alone.)

In both `zeros(pfx(π)) = 0` and `zeros(pfx(π)) = 1` cases, no sub-delegate covers `a'`. Hence `π` itself achieves the unique longest matching prefix in `Π_Σ` for `a'`, and `ω_{Σ'}(a') = π` (where `Σ'` is the post-baptism state; `Π_{Σ'} = Π_Σ` by O15, since baptism introduces no principals).
