...
Here `delegated_{Σ_d}(π, π')` abbreviates the four-place `delegated(Σ_d, Σ_d^{post}, π, π')`, so the hypothesis asserts that the specific transition `Σ_d → Σ_d^{post}` occurred and introduced `π'`; the proof's identification of `π'`'s unique introduction event with that transition is immediate from the definition.
