...
**Definition (delegated).** We name the conjunction of conditions (i)–(v) above the *delegation predicate*, with a four-place signature: `delegated(Σ, Σ', π, π')` holds iff `Σ → Σ'`, `π ∈ Π_Σ`, `π' ∈ Π_{Σ'} ∖ Π_Σ`, and conditions (i)–(v) hold for `(π, π')` at `Σ` (the subscript form `delegated_Σ(π, π')` abbreviates `delegated(Σ, Σ', π, π')` for the `Σ'` bound by the surrounding formula).

**Freshness-(v) (derived).** Condition (v) entails two facts about the delegate prefix at `Σ`: (T4) it is a valid address — `T4(pfx(π'))` — and (fresh) it is unbaptized — `pfx(π') ∉ Σ.B`. Both follow from (v) fixing `pfx(π') = c_{hwm(Σ.B,p,d)+1}` for a B6-valid `(p, d)`. For (T4): B6 (ValidDepth, ASN-0040) sufficiency gives that every element of the stream `S(p, d)` — hence `c_{hwm(Σ.B,p,d)+1} = pfx(π')` — satisfies T4 (no adjacent zeros, no leading or trailing zero, every present field non-empty). For (fresh): B2 (HighWaterMarkSufficiency, ASN-0040) identifies `pfx(π')` as the next element of `S(p, d)`, and B1 (ContiguousPrefix) gives `children(Σ.B, p, d) = {c₁, …, c_{hwm(Σ.B,p,d)}}`, so `c_{hwm(Σ.B,p,d)+1} ∉ Σ.B`.
...
Gregory's implementation corroborates the coupling, and we anchor the allocation-site evidence here for reuse: every registry write in udanax-green funnels through a single allocation point — `findisatoinsertgr` for the ISA store, and `findpreviousisagr` for account slots — which issues each new tumbler as a fresh granfilade entry under the session's own account-tumbler authority, advancing unilaterally past existing (and delegated) slots with no inter-session signaling, shared counter, or lock. ASN-0040's `Bop` is the abstract image of this single allocation point.
...
The single-allocation-point evidence anchored at O17b confirms the freshness conjunct: `findpreviousisagr` never re-purposes a previously baptized tumbler as a new principal's prefix.
...
By the single-allocation-point evidence anchored at O17b, the new account slot is entered under the session's own account-tumbler authority, so the delegator and the allocator are necessarily the same process.
...
Condition (a) entails a structural consequence: since `ω(a') = π` gives `pfx(π) ≼ a'`, and the O6 biconditional (`pfx(π) ≼ a' ≡ pfx(π) ≼ acct(a')`, holding for all principals with `zeros(pfx(π)) ≤ 1` — i.e., all principals by O1a) yields `pfx(π) ≼ acct(a')`. The address structure necessarily records the fork within the requesting principal's account domain. This holds for both `zeros = 0` and `zeros = 1`; no case distinction is needed. Condition (c) is enforced by the construction `a' = pfx(π).0.{hwm_0 + 1}` (verified below), not by an additional axiom: `a' ∈ S(pfx(π), 2)`, every element of which has the form `pfx(π).0.k` with exactly one zero separator beyond `pfx(π)` (the field-opening step `inc(·, 2)` introduces it; the sibling step `inc(·, 0)` preserves it by B5a), hence `zeros(a') = zeros(pfx(π)) + 1`. The clause makes formal the namespace-vs-content distinction: O10 guarantees the next structural tier (user from node, document from account), and no more.
...
The ownership model's static layer is one ownership predicate (prefix containment) resolved by one longest-match rule under one structural invariant (exclusivity). This static layer, together with the state-dynamics axioms governing how `Π` and `Σ.B` evolve — O5, O12, O13, O14, O15, O16, O17b, and O18 (subdivision authority, persistence, prefix immutability, bootstrap, delegation closure, allocation closure, registry coupling, delegation-baptizes) — jointly yields the derived properties O1–O18 catalogued in the table below; the Status column marks which entries are axioms and which are derived.
