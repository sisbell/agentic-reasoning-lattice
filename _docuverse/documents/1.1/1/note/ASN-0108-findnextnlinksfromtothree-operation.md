...
*Offset cursor.* The resume step stores an integer `j` = the count of links already delivered, and delivers ranks `[j+1, j+N]` of `Match(q, Σ')`. For that to coincide with the identity continuation `R`, the first `j` ranks of `Σ'` must be exactly the `j` links already seen — otherwise rank `j+1` of `Σ'` is not the successor of the last delivered link. Hence

> `wp(resume_offset, R)  ≡  ranks 1..j of Match(q, Σ') = the j links already delivered`

— *no membership change at or below the cut* between the two calls. Over a mutable set this precondition is not discharged: any insertion or deletion at a rank `≤ j` falsifies it. The two weakest preconditions are not merely different in degree — the identity obligation (`κ(c)` recoverable) is met for free, while the offset obligation (no churn below the cut) is exactly the frozen-prefix assumption a mutable set denies.
...
- *Offset resume* takes ranks `[3, 4]` of `M_{Σ'} = {a_2, a_3}`. But `a_2` was already delivered in window 1 — a **duplicate (overlap)**. Its precondition fails: ranks `1..2` of `Σ'` are now `{a_0, a_1}`, not the seen `{a_1, a_2}`. (Symmetrically, a deletion below the offset slides ranks down and **omits** a link.)
