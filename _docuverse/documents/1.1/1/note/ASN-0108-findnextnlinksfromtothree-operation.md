...
*Proof.* Let `M = {a_1 ≺ a_2 ≺ … ≺ a_m}` (W0). We show by induction that `W_i` is the block of ranks `[iN+1, … , min((i+1)N, m)]`. Base: `After(⊥) = M`, and `W_0` is its `≺`-least `min(N, m)` elements — ranks `1 … min(N, m)`; its `≺`-max is `a_{min(N,m)}`, so `c_1 = a_{min(N,m)}`. Step: assume `c_i = a_{iN}` (when `iN ≤ m`). Then `After(c_i) = {a_{iN+1}, …, a_m}` — exactly the links whose key exceeds `κ(a_{iN})`, by W0's strict order — and `W_i` takes its least `min(N, m-iN)` elements, ranks `iN+1 … min((i+1)N, m)`. Consecutive blocks share no rank (disjoint) and abut (consecutive). The union over `i` is ranks `1 … m` (cover, each once). Termination: the bound function `t_i = |After(c_i, Σ)| = m - iN` strictly decreases by `N ≥ 1` per non-final call and reaches a value `< N` after `⌈m/N⌉` windows, after which one more call yields the empty window. ∎
...
> **W5 (OrderStability).** Resumption is well-defined across `Σ → Σ'` only if the ordering key is *state-stable*: for every link present and matching in both states, `κ_{Σ'}(a) = κ_Σ(a)`, and hence for any two such links their relative `≺`-order agrees in both states. Without state-stability, the cursor's key shifts and the successor set `After(c, Σ')` no longer continues the enumeration the reader was traversing.

W5 is a requirement, and it discriminates the two candidate keys sharply. An **address-based key** `κ(a) = a` is state-stable *unconditionally*: a link's address is permanent — never changed, never reused (foundation T8, allocation permanence) — so the order of any two surviving links is frozen for the lifetime of the search, exactly as Nelson requires ("links keep their creation-order addresses permanently"; deleting one "doesn't renumber" the others). A **content-position key** is state-stable only while the matched content keeps its position; an edit that moves the endpoint moves the key, and the enumeration the reader was walking is silently re-laid. State-stability is therefore the first concrete advantage of ordering by link identity over ordering by matched-content position: it is free for the former and conditional for the latter.
...
> **W6 (MonotoneAppendRequiresAllocationMonotoneKey).** A newly created matching link is `≺`-greater than every previously enumerated link — append-at-tail — *iff* the ordering key is *allocation-monotone*: a link allocated later receives a key strictly greater than every key already issued. Under an address-based key this holds within a single home document's link allocator, whose successive links carry strictly increasing addresses (foundation T9, forward allocation; a single allocator chain is strictly increasing). Under a content-position key it does not hold at all: a new link's key is the position of *its endpoint content*, which may sort anywhere — before, between, or after existing keys.
...
The reconciliation: Nelson's append-at-tail is the *intended* behaviour, and it is *attained* exactly when the key is the link's permanent arrival-order address. An implementation that orders by matched-content position satisfies W0–W5 but forfeits W6's append guarantee, admitting the blind spot. An alternative implementation seeking Nelson's guarantee must adopt an allocation-monotone key.
...
> **W9 (ExhaustionByShortWindow).** A window returning fewer than `N` links signals exhaustion: every matching link reachable past the cursor has been delivered. Formally, `|Window(q, c, N, Σ)| < N ⟹ After(next-cursor, Σ) = ∅`. The reader detects the end by comparing the batch size against the requested `N`; a short — possibly empty — batch is the terminal signal.

*Derivation.* `Window` returns `min(N, |After(c, Σ)|)` links. If this is `< N`, then `|After(c, Σ)| < N`, the whole successor set fit in the batch, and the new successor set is empty: nothing remains. Contrapositive: a *full* batch of exactly `N` means the reader must call again — possibly to receive more, possibly to learn there is no more.
...
| W5 | Coherent resumption requires a state-stable key; an address-based key is state-stable unconditionally (T8), a content-position key only conditionally | introduced |
| W6 | New matching link appends at the tail iff the key is allocation-monotone; a content-position key admits a permanent blind spot (silent skip) | introduced |
...
| W9 | A window of fewer than `N` links (zero included) signals exhaustion; no terminal marker is needed | introduced |
