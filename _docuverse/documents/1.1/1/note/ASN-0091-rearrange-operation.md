...
REARRANGE_K (the cut-sequence operation of ASN-0084) is one concrete realisation of the abstract Vstream-only class. Per the case split established above, its realiser is ASN-0047's K.μ~ in the non-trivial case and the identity composite in the collapse case. The clause-by-clause argument below treats the non-trivial case; the collapse case needs no clause discharge, since the transition is the identity.
...
With the unification fixed above, the abstract class's defining clauses map to their REARRANGE_K sources, and K.μ~'s own admissibility clauses (i)–(v) of ASN-0047 map to their discharge. The substantive RA-adm work follows in the per-invariant layers; the two correspondences are tabulated rather than re-narrated.

*Abstract class clause ← REARRANGE_K source.*

| Clause | Source |
|--------|--------|
| RA-reg | K.μ~'s precondition `d ∈ E_doc`, via the unification `E_doc = dom(M)` |
...
This identity is well-defined across the freedom in choosing π: when `Σ.M(d_tgt)` carries shared I-addresses (allowed by S5), multiple bijections satisfy RA-π, yet the set image `π(project(e, d_tgt, Σ))` equals the state-determined object `project(e, d_tgt, Σ')` regardless of which witness is used — the set-image invariance proved concretely in the "Worked Example — Bijection Non-Uniqueness Under Shared I-Addresses" below. In the non-target branch the freedom is absent — `π̂_d` collapses to the identity, fully determined by `dom(Σ.M(d))`.
...
*Verification.* The routine claims — RE-C, RE-L, RE-dom, RE-ran, RE-μ, RE-cov, RE-disc, RE-other, RE-sub, RE-origin, RE-R — discharge as in the first Worked Example, against the five-position arrangement `Σ'.M(d) = {[1, 1] ↦ a₃, [1, 2] ↦ a₄, [1, 3] ↦ a₂, [1, 4] ↦ a₁, [2, 1] ↦ a_link}`. The genuine deltas under the 4-cut swap — all turning on the μ-region displacement `Δ(μ) = w_β − w_α = +1` and the R-SPERM image — are:
