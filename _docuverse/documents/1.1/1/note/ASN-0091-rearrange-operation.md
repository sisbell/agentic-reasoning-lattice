...
**Reverse witness (coalescence).** Take pre-state `Σ.M(d)` with V-positions `[1, 1] ↦ a + 1`, `[1, 2] ↦ c`, `[1, 3] ↦ a`, where `a + 1` and `a` are consecutive content addresses (both produced by the same sub-allocator chain `A_X` of some document `d_X` in subspace `s_X`) and `c` is an I-address allocated from a different sub-allocator chain `A_Y` of some `(d_Y, s_Y) ≠ (d_X, s_X)`. The structural forms differ: `a − 1` and `a + 2`, if they exist, are chain elements of `A_X` and have the form `[d_X, 0, s_X, k_X − 1]` and `[d_X, 0, s_X, k_X + 2]` respectively (positions 1–5 give `d_X`, position 7 gives `s_X`); whereas `c`, as a chain element of `A_Y`, has the form `[d_Y, 0, s_Y, k_Y]`. If `d_X ≠ d_Y`, then the two forms disagree on at least one of positions 1–5; if `d_X = d_Y` and `s_X ≠ s_Y`, then by SC-NEQ (`s_C ≠ s_L`, ASN-0093) the two forms disagree at position 7. In either case `c` differs as a tumbler from both `a − 1` and `a + 2`, so `c ∉ {a − 1, a + 2}` — the two values that would make `c` adjacent to `a + 1` on the right or to `a` on the left within a single chain. The pre-state maximal runs are `([1, 1], a + 1, 1)`, `([1, 2], c, 1)`, `([1, 3], a, 1)` — three singletons, since `(a + 1) + 1 = a + 2 ≠ c` (excluding the right-extension of the first run) and `c + 1 ≠ a` (excluding the right-extension of the second run, since this requires `c ≠ a − 1`). Pre-state run cardinality: 3.
...
**Equality witness.** Take pre-state `Σ.M(d)` populated only on the content subspace with V-positions `[1, 1] ↦ a` and `[1, 2] ↦ c`, where `a` and `c` are I-addresses from different sub-allocator chains — concretely, `a ∈ A_C(d_X)` and `c ∈ A_C(d_Y)` with `d_X ≠ d_Y`. By the same chain-structural argument used in the coalescence witness above (`d_X ≠ d_Y` forces disagreement on positions 1–5 of the chain elements), `c ∉ {a − 1, a + 1}` and symmetrically `a ∉ {c − 1, c + 1}` — neither address is chain-adjacent to the other on either side. The pre-state maximal runs are the two singletons `([1, 1], a, 1)` and `([1, 2], c, 1)`, since `a + 1 ≠ c` rules out the right-extension of the first run. Pre-state run cardinality: 2.
