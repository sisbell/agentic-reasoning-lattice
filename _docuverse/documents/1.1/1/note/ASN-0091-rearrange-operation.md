...
- **Clause (i) — induced post-state satisfies the shape package (S8a, S8-depth, D-CTG★, D-MIN★).** Discharged by the constructive shape-package layer of the per-invariant discharges below, which derives each of these at Σ' from RA-dom alone.
...
The *abstract* property — subspace preservation as a property of any admissible π — is captured by RE-subpres (derived earlier in the "REARRANGE as Vstream-Only Operation" section): no V-position crosses from one subspace to another (`subspace(π(v)) = subspace(v)`). RE-sub adds, on the complement of the cut subspace, the pointwise form — that non-S V-positions are not permuted at all (`π(v) = v`), not merely kept within their subspace.
...
*Verification.* The routine claims — RE-C, RE-L, RE-dom, RE-ran, RE-μ, RE-cov, RE-disc, RE-other, RE-sub, RE-origin, and RE-R — discharge exactly as in the first Worked Example, against the five-position arrangement `Σ'.M(d) = {[1, 1] ↦ a₃, [1, 2] ↦ a₄, [1, 3] ↦ a₂, [1, 4] ↦ a₁, [2, 1] ↦ a_link}` with the obvious value substitutions: `ran(·)` and the per-address multiplicities cover `{a₁, a₂, a₃, a₄, a_link}` (each multiplicity 1 in both states); RE-disc holds because `coverage(e₁) ∩ ran(·) ⊇ {a₂, a₃} ≠ ∅` at both states; RE-sub fixes the link-subspace entry `[2, 1] ↦ a_link` verbatim via R-SPERM's non-S branch and R-FRAME-S(a); and RE-origin gives `origin(aᵢ) = d'`, `origin(a_link) = d`. The genuine deltas under the 4-cut swap — all turning on the μ-region displacement `Δ(μ) = w_β − w_α = +1` and the R-SPERM image — are:

- **RE-proj (R-SPERM image).** `project(e₁, d, Σ) = {v : Σ.M(d)(v) ∈ coverage(e₁)} = {[1, 2], [1, 3]}` (V-positions holding `a₂` and `a₃`). `project(e₁, d, Σ') = {[1, 3], [1, 1]}` (the same content under the new arrangement). The image `π({[1, 2], [1, 3]}) = {[1, 3], [1, 1]}` matches — exercising R-SPERM's μ-branch (`[1, 2] ↦ [1, 3]`) and β-branch (`[1, 3] ↦ [1, 1]`).
- **RE-frag (μ-region asymmetry).** Pre-state runs on the content subspace: `([1, 1], a₁, 4)` — a single maximal run, since `a₁, a₂, a₃, a₄` are consecutive in `A_C(d')`. Post-state runs: `([1, 1], a₃, 2)` (since `a₃ + 1 = a₄`, but `a₄ + 1 ≠ a₂` so the run stops at [1, 2]), `([1, 3], a₂, 1)`, `([1, 4], a₁, 1)` — cardinality 3. With the shared link-subspace run `([2, 1], a_link, 1)`, total cardinalities are 2 (pre) and 4 (post). The strict increase confirms RE-frag; the `w_α ≠ w_β` asymmetry is structurally distinctive — the μ-region's content `a₂` migrates between the β and α images, breaking run continuity in a pattern not realizable under any 3-cut pivot.
- **RE-trans (μ-region transclusion).** All four `a₁, a₂, a₃, a₄` have `origin(·) = d' ≠ d`, so each is a transclusion in `d`; range, multiplicity, and `origin` are preserved as in the first Worked Example. The distinctive case is `(a₂, d)`: pre-state at [1, 2], post-state at [1, 3] — migrated by `Δ(μ)` to a position between the β image (at [1, 1]–[1, 2]) and the α image (at [1, 4]), with the relationship to `d'` intact across the displacement.
- **Admissibility (RA-adm) and composite-boundary properties.** The per-state RA-adm invariants and the frame-inherited state-component-only invariants discharge exactly as in the first Worked Example, with five V-positions in place of four. The one structurally distinctive RA-adm clause is *S8★*: the content subspace decomposes into three correspondence runs `([1, 1], a₃, 2), ([1, 3], a₂, 1), ([1, 4], a₁, 1)` (identified under RE-frag above), where the μ-region's `a₂` — displaced to `[1, 3]` by the `w_α ≠ w_β` asymmetry — heads its own singleton run; the link-subspace clause `{[2, 1] ↦ a_link}` carries over verbatim via RE-sub. The composite-boundary properties P4★ and P4a (outside RA-adm's scope) discharge exactly as in the first Worked Example.
...
**Document-parameterised chaining lemma.** Let `X(Σ, d)` be any quantity that is a function of `Σ.M(d)` alone (a domain, range, multiset, or subspace frame at `d`), and suppose each single REARRANGE step targeting `d` preserves `X(·, d)`. Then `X(Σ_n, d) = X(Σ_0, d)` across the whole sequence: at each step `Σᵢ₋₁ →_R Σᵢ` targeting `dᵢ`, either `dᵢ = d` — the per-step claim preserves `X` directly — or `dᵢ ≠ d`, in which case RE-other gives `Σᵢ.M(d) = Σᵢ₋₁.M(d)` and any function of `Σ.M(d)` is trivially preserved. Each ★ claim below that fixes a document parameter cites this lemma rather than re-deriving the case split.

The multi-step (★) forms:
...
- **RE-dom★, RE-ran★, RE-sub★ at a fixed `d`:** each is the chaining lemma instantiated with `X(Σ, d)` = the domain, range, or non-S subspace frame of `Σ.M(d)` respectively, whose single-step preservation is RE-dom, RE-ran, RE-sub. Composing yields `X(Σ_n, d) = X(Σ₀, d)`.
