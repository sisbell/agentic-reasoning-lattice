...
The subspace-preservation half is structurally necessary at the abstract level — the same RA-adm + S3★ + L14 consequence already noted in the RA-adm exposition of "REARRANGE as Vstream-Only Operation" above, restated here for emphasis on its role in distinguishing pointwise fixity from mere within-subspace constraint. (An admissible bijection cannot carry a content-subspace V-position to a link-subspace V-position, since the post-state image would land in `dom(Σ'.C)` while S3★'s link-subspace clause demands a `dom(Σ'.L)` value, and these stores are disjoint by L14.) The pointwise-fixity strengthening, however, is REARRANGE_K-specific: a different concrete realization of the abstract class could non-trivially permute the link subspace while preserving subspace identity, and would still satisfy RA-adm. Under such an alternate realization the arrangement-preservation half of RE-sub could fail in a more subtle way too: if π permuted link-subspace V-positions whose images shared an I-address (S5/UnrestrictedSharing), the arrangement values would still match position-by-position, but distinct V-positions would have swapped identities. REARRANGE_K's cut sequence operates in one subspace at a time and leaves the others verbatim — this is the joint content of R-PPERM/R-SPERM (for π) and R-FRAME-P/S(a) (for the arrangement).
