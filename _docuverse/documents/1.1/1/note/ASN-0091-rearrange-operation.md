...
Together, RE-frag and RE-coal record that the maximal-run-decomposition cardinality is *neither monotonically non-decreasing nor monotonically non-increasing nor invariant* under REARRANGE — every relation between pre- and post-state cardinality (strict increase, strict decrease, equality) is realizable.
...
ASN-0084's R-FRAME-P/S(a) restricts the cut sequence's effect to the content subspace S = s_C. V-positions in any other subspace are pointwise fixed:
```
...
The subspace-preservation half is structurally necessary at the abstract level: an admissible bijection cannot carry content-subspace V-positions into link-subspace V-positions or vice versa, because the typed referential integrity invariant (foundation S3★: content-subspace V-positions map to `dom(C)`, link-subspace V-positions map to `dom(L)`) would be violated at Σ' — and RA-adm requires Σ' to satisfy it. The pointwise-fixity strengthening, however, is REARRANGE_K-specific: a different concrete realization of the abstract class could non-trivially permute the link subspace while preserving subspace identity, and would still satisfy RA-adm. REARRANGE_K's cut sequence operates in one subspace at a time and leaves the others verbatim — this is the content of R-FRAME-P/S(a).
...
- **RE-sub.** `[2, 1] ∈ dom(Σ.M(d))` has `subspace([2, 1]) = 2 ≠ s_C = S`. By RE-sub, `Σ'.M(d)([2, 1]) = Σ.M(d)([2, 1]) = a_link`, matching the post-state arrangement exhibited above.
...
- **RE-frag★ / RE-coal★:** no per-step monotonicity is available — each step in the sequence may independently increase (RE-frag) or decrease (RE-coal) run-decomposition cardinality, as the single-step fragmentation and coalescence witnesses above demonstrate. The composite claim is the negation of any uniform per-step direction; we do not assert anything stronger about the net cardinality change `|runs(Σ_n.M(d))| − |runs(Σ₀.M(d))|` across the full sequence.
