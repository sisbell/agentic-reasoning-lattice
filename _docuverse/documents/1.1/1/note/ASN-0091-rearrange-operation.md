...
A single witness establishes that REARRANGE_K admits such a degenerate no-op with π ≠ id. Foundation S5 (UnrestrictedSharing) admits a 3-cut pivot with `w_α = w_β = 2`, cuts `([1, 1], [1, 3], [1, 5])`, and pre-state `{[1, 1] ↦ a, [1, 2] ↦ b, [1, 3] ↦ a, [1, 4] ↦ b}` (`a ≠ b`): R-P1 gives `M'([1, 1]) = M([1, 3]) = a`, `M'([1, 2]) = M([1, 4]) = b`; R-P2 gives `M'([1, 3]) = M([1, 1]) = a`, `M'([1, 4]) = M([1, 2]) = b`, so `M'(d) = M(d)` even though the affected range carries two distinct I-addresses, while π is the non-identity rotation (π ≠ id). We therefore split the realisation by whether the cut-driven π produces a net change `M'(d) ≠ M(d)`:

- *Non-trivial case* — `M'(d) ≠ M(d)`. Here K.μ~'s admissibility clause (ii) `M'(d) ≠ M(d)` holds, and K.μ~'s own precondition — `M(d)|_{dom_C}` takes at least two distinct values — holds a fortiori: a value-uniform affected range is fixed by any cut-induced permutation, so `M'(d) ≠ M(d)` forces the range to exhibit at least two distinct I-addresses. K.μ~ is the realiser: it supplies π via R-PPERM/R-SPERM and the realisation proceeds clause-by-clause as below.

- *Collapse case* — `M'(d) = M(d)` with π ≠ id. The cut-driven permutation is non-identity but its net effect is vacuous: RA-π under this π together with RA-dom gives `Σ'.M(d) = Σ.M(d)` as partial functions, and RA-frame fixes every other state component, so `Σ' = Σ`. This is a degenerate no-op — every RE-* claim below holds trivially as an identity of Σ with itself. K.μ~ is *not* the realiser here, since its admissibility clause (ii) `M'(d) ≠ M(d)` fails; but no realiser is needed, because the transition is the identity composite (the reflexive case `Σ' = Σ` of ASN-0093's SequentialTransitionAxiom). REARRANGE_K is *not* given an additional non-triviality precondition — it remains defined wherever R-PRE holds, collapsing to the identity transition precisely on those affected ranges fixed by the cut-induced permutation.
...
- **Clause (ii) — non-trivial net effect `M'(d) ≠ M(d)`.** This is exactly the defining condition of the non-trivial case above; in the collapse case it fails and K.μ~ is not the realiser.
...
RE-ext is exercised concretely in the third Worked Example ("Interior Cuts (R-EXT Exercised)") below, where in-subspace exterior positions `[1, 1]` and `[1, 5]` are pointwise preserved while interior content-subspace positions migrate under the cut-driven permutation.
