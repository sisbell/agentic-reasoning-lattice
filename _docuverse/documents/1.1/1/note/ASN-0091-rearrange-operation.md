...
Fragmentation is exhibited concretely in the Worked Example below (the RE-frag bullet), where a 3-cut pivot raises the content-subspace run cardinality from 2 to 3.
...
- **Composite-boundary properties (P4★, P4a, P7a).** By the "Composite-Boundary Properties" section, these hold at Σ' whenever the pre-state Σ sits at a composite boundary. Concretely one may read off the witnesses: `Contains_C(Σ') = {(a₂, d), (b₁, d), (a₁, d)} = Contains_C(Σ) ⊆ Σ.R = Σ'.R` for P4★, and `(b₁, d), (a₁, d'), (a₂, d') ∈ Σ'.R` covering the three displayed content addresses for P7a.
