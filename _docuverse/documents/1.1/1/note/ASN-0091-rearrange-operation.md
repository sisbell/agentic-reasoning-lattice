...
- *Collapse case* — `M'(d) = M(d)` with π ≠ id. The cut-driven permutation is non-identity but its net effect is vacuous: the case is defined by `M'(d) = M(d)`, which is already `Σ'.M(d) = Σ.M(d)`, and RA-frame fixes every other state component, so `Σ' = Σ`. This is a degenerate no-op — every RE-* claim below holds trivially as an identity of Σ with itself. K.μ~ is *not* the realiser here, since its admissibility clause (ii) `M'(d) ≠ M(d)` fails; but no realiser is needed, because the transition is the identity composite (the reflexive case `Σ' = Σ` of ASN-0093's SequentialTransitionAxiom). REARRANGE_K is *not* given an additional non-triviality precondition — it remains defined wherever R-PRE holds, collapsing to the identity transition precisely on those affected ranges fixed by the cut-induced permutation.
...
REARRANGE_K's cut-induced π must satisfy each of K.μ~'s own admissibility clauses (i)–(v) of ASN-0047. We close each:
...
The subspace-preservation half is structurally necessary at the abstract level — this is exactly the content of RE-subpres established earlier from RA-adm + S3★ + L14. The arrangement-preservation half of RE-sub could in principle fail under an alternate realization in a subtle way: if π permuted link-subspace V-positions whose images shared an I-address (S5/UnrestrictedSharing), the arrangement values would still match position-by-position, but distinct V-positions would have swapped identities. REARRANGE_K's cut sequence operates in one subspace at a time and leaves the others verbatim — this is the joint content of R-PPERM/R-SPERM (for π) and R-FRAME-P/S(a) (for the arrangement).
...
The pointwise-fixity strengthening — both RE-sub's fixity on non-S V-positions and RE-ext's fixity on in-subspace exterior V-positions — is REARRANGE_K-specific, not abstract: a different concrete realisation of the abstract class could non-trivially permute the link subspace or the in-subspace exterior while preserving subspace identity and the affected range's image, and would still satisfy RA-adm. REARRANGE_K's cut sequence operates only on the affected range within a single subspace and leaves everything else verbatim — this is the joint content of R-PPERM/R-SPERM (for π) and R-FRAME-P/S(a)/R-EXT (for the arrangement).
