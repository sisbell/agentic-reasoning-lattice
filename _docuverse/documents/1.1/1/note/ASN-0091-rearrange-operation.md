...
REARRANGE_K (the cut-sequence operation of ASN-0084) realizes this class. ASN-0047's K.μ~ precondition `d ∈ E_doc` discharges RA-reg (in the unified state, `E_doc` coincides with `dom(M)` for documents). ASN-0084's R-PPERM and R-SPERM construct π explicitly for 3-cut pivot and 4-cut swap respectively. ASN-0047's K.μ~-FIX (DomainFixity) discharges RA-dom: `dom(Σ'.M(d)) = dom(Σ.M(d))` follows from π's bijectivity together with ASN-0047's D-SEQ★ at both endpoints (per-subspace V-position enumeration matches across the transition). ASN-0084's R-SP (RearrangeSufficientPrecondition) discharges RA-adm with respect to the ASN-0036 foundation invariants at the cut-sequence level by deriving that every such invariant holds at Σ' under the precondition `R-PRE(K) ∧ ASN-0036-invariants(Σ, d)`. The ASN-0047 *extended* invariants — S3★ (generalised referential integrity), S3★-aux (subspace exhaustiveness), CL-OWN (link-subspace ownership), CL-UNIQ (link-subspace position uniqueness), P4★ (content-subspace provenance bound), and S8★ (per-subspace span decomposition) — lie outside R-SP's named scope and are discharged separately by REARRANGE_K's structural properties. Because CS3 fixes the cut subspace at S = s_C, the link-subspace portion of `M(d)` is preserved pointwise (RE-sub below): `Σ'.M(d)|_{V_{s_L}(d)} = Σ.M(d)|_{V_{s_L}(d)}` carries over verbatim, so CL-OWN (every link-subspace V-position still maps to a link with `origin = d`) and CL-UNIQ (the partial-injection property of `M(d)|_{dom_L}`) are preserved unchanged at Σ'. S3★ holds at Σ' by constructive subspace-preservation of π: R-PPERM/R-SPERM build π from a non-S identity branch (writing `π(v) = v` for every `v ∈ dom(Σ.M(d))` with `subspace(v) ≠ S`) and in-S branches (R-P1/R-P2 for 3-cut, R-S1/R-S2/R-S3 for 4-cut) whose domain and codomain are confined to V-positions in the cut subspace S = s_C, so by construction `subspace(π(v)) = subspace(v)` for every `v ∈ dom(Σ.M(d))`. Pre-state S3★ at Σ supplies each content-subspace `v` with an image in `dom(Σ.C)` and each link-subspace `v` with an image in `dom(Σ.L)`. By RA-π, `Σ'.M(d)(π(v)) = Σ.M(d)(v)`; by RE-C and RE-L, `dom(Σ'.C) = dom(Σ.C)` and `dom(Σ'.L) = dom(Σ.L)`; and by π's subspace-preservation, `π(v)` lies in the same subspace as `v`. So each post-state image `Σ'.M(d)(π(v))` lands in the store correct for `π(v)`'s subspace, discharging S3★ at Σ' from constructive premises rather than from circular appeal to RA-adm. S3★-aux holds because `dom(Σ'.M(d)) = dom(Σ.M(d))` (RA-dom) and the subspace of every V-position is a structural projection on its first component (not on state). P4★ holds because RE-R gives `Σ'.R = Σ.R` while the bijection π restricted to `V_{s_C}(d)` bijects each I-address's pre-image set onto its post-state pre-image set within the content subspace, so `Contains_C(Σ') = Contains_C(Σ)` as a set of (a, d)-pairs and the inclusion `Contains_C ⊆ R` carries over from the pre-state. S8★ holds at Σ' by per-subspace decomposition. The content-subspace clause `M'(d)|_{V_{s_C}(d)} : V_{s_C}(d) → dom(Σ'.C)` is a direct application of ASN-0036's S8 to the restricted arrangement: under R-SP's discharge of ASN-0036's S8 at Σ' (which covers the foundation per-document arrangement decomposition), the restriction to the content subspace inherits a finite partial function with uniform depth `m_{s_C}` (S8-depth at Σ') and S3 holds with target `dom(Σ'.C)` (S3★'s content-subspace clause at Σ', already discharged above), so ASN-0036's S8 applies and yields the content-subspace correspondence-run decomposition. The link-subspace clause follows from RE-sub: `Σ'.M(d)|_{V_{s_L}(d)} = Σ.M(d)|_{V_{s_L}(d)}` pointwise (since RE-sub fixes every link-subspace V-position and its arrangement value), so the pre-state trivial length-1 decomposition `{(v, Σ.M(d)(v), 1) : v ∈ V_{s_L}(d)}` carries over verbatim to Σ' — every link-subspace V-position constitutes its own length-1 correspondence run unchanged. All remaining state-component-only foundation invariants — those depending only on Σ.C, Σ.L, Σ.E, Σ.R, or dom(Σ.M) — are trivially preserved across REARRANGE, since RA-frame fixes each of these components verbatim. In particular, P0, P1, P2, P3, P6, P7, P7a, P8, NodeLineage, L0–L14, L12, L-fin, C0–C2, and C-fin hold at Σ' iff they hold at Σ. The ASN-0036 dom(M(d))-ranging structural invariants — S8a (VPositionWellFormedness), S8-fin (FiniteArrangement), and S8-depth (FixedDepthVPositions) — are not state-component-only (they quantify over `dom(Σ.M(d))`), but they are nevertheless preserved at Σ' because `dom(Σ'.M(d)) = dom(Σ.M(d))` (RA-dom) and each invariant's predicate depends only on state-independent structural projections of V-positions (the first-component `subspace(v)`, the length `#v`, the zero-count `zeros(v)`, and the componentwise positivity check) — properties of tumblers as values, not of the state. Domain-equality together with structural invariance discharges each from its pre-state form at Σ. For REARRANGE_K specifically, the same conclusion is supplied independently by ASN-0084's R-SP (RearrangeSufficientPrecondition), which discharges all ASN-0036 invariants including S8a, S8-fin, and S8-depth at the cut-sequence level. P4a (HistoricalFidelity) is the one foundation invariant excluded from this class: its existential witness `Σ_k` ranges over the prior transition trace rather than the current state, so frame inheritance does not discharge it directly. P4a is preserved by a separate argument — the transition trace is append-only as a corollary of ASN-0093's SequentialTransitionAxiom, and `Σ'.R = Σ.R` by RE-R, so every pre-existing historical witness `Σ_k ∈ {Σ_0, ..., Σ_n}` for a given `(a, d) ∈ Σ.R` remains a valid witness at Σ' for the same pair in `Σ'.R`. The append-only corollary unfolds as follows. SequentialTransitionAxiom declares each transition `Σ_k → Σ_{k+1}` atomic, uninterruptible, and totally ordered, with `Σ_{k+1}` produced from `Σ_k` in a single indivisible step and no intermediate state in which a transition has begun but not yet committed. The state-trace `Σ_0, Σ_1, ..., Σ_n` is thereby the sequence of committed inputs to the prior transitions in their total order; each `Σ_k` is the *input* to the (k+1)-th transition (and the *output* of the k-th, if k ≥ 1) and is fixed at the moment of that transition's commit. A subsequent REARRANGE step extends the trace by appending `Σ_{n+1}` as the output of the (n+1)-th transition; SequentialTransitionAxiom admits no transition mechanism that re-writes any earlier `Σ_k` for `k ≤ n`, since each `Σ_k` is by construction the committed input to a transition whose total-order index precedes the REARRANGE step's index `n+1`. So the prior states `Σ_0, ..., Σ_n` are concrete tuples of state components fixed once their transitions commit, and the REARRANGE step alters none of them — only `Σ_{n+1}` is produced. This is the precise content of "the transition trace is append-only" as used below. The worked-example admissibility traces below elaborate this in concrete form. REARRANGE_K is the concrete realization of ASN-0047's K.μ~ operation, whose frame supplies RA-frame in full. K.μ~'s ASN-0047 frame reads `C' = C; E' = E; R' = R; L' = L; (A d' : d' ≠ d : M'(d') = M(d'))`, which matches each RA-frame conjunct *explicitly* except `dom(Σ'.M) = dom(Σ.M)`. That last conjunct is implicit rather than explicit in K.μ~'s frame: K.μ~ is the named composite K.μ⁻ + K.μ⁺ from ASN-0047, both components are arrangement-modifying operations whose effect clauses change only `Σ.M(d)` for the targeted document, and neither one adds to or removes from the document registry — document registration is the exclusive province of K.σ (ASN-0093) and K.δ-IsDocument (ASN-0047). So `dom(Σ'.M) = dom(Σ.M)` follows structurally from K.μ~'s composite definition rather than appearing as an explicit frame clause. The remaining matched conjuncts are explicit: notably `L' = L` (the source of the link-store invariance derived below, RE-L), `E' = E`, and `R' = R` (the source of RE-R). ASN-0084's R-FRAME-P/S, by contrast, supply only the within-document, within-subspace clauses at the cut-sequence level (clause (a): non-S V-positions preserved; clause (b): other documents' arrangements preserved; clause (c): `C' = C`); link-store, entity-set, and provenance preservation enter the picture through K.μ~ at the ASN-0047 layer, not through R-FRAME-P/S. The cut sequence further restricts the bijection — π acts as identity on V-positions outside the affected range `[c₀, c_{n−1})` and on V-positions in subspaces other than the cut subspace S. Most of the abstract claims below are derivable from RA-dom, RA-π, RA-frame, and RA-adm alone, independent of how π was generated; the exception is RE-sub (subspace frame), which is *pointwise* fixity on non-S V-positions and requires the cut-subspace restriction supplied by ASN-0084's R-FRAME-P/S(a). RA-adm together with S3★ + L14 forces only the weaker property that π preserves subspaces (no V-position crosses from one subspace to another) — a content-subspace V-position must remain in the content subspace, a link-subspace V-position must remain in the link subspace — but admissibility leaves room for non-identity permutations *within* each subspace. The pointwise fixity in RE-sub, which says non-S V-positions are *not permuted at all*, is genuinely REARRANGE_K-specific.
...
Where a link's coverage strikes the arrangement is the set `project(e, d, Σ) = {v ∈ dom(M(d)) : M(d)(v) ∈ coverage(e)}`. The bijection π carries this set faithfully to the post-state. For each `v ∈ project(e, d, Σ)`, the V-position `π(v)` holds the same I-address (RA-π), and that address remains in `coverage(e)` — which is a state-independent function of the endset `e` alone (per the coverage definition in ASN-0098, which consults no state component) — so `π(v) ∈ project(e, d, Σ')`. For the reverse inclusion, take any `v' ∈ project(e, d, Σ')`. By the projection definition, `v' ∈ dom(Σ'.M(d))`; by RA-dom, `dom(Σ'.M(d)) = dom(Σ.M(d))`, so `v' ∈ dom(Σ.M(d))`; and π is a bijection of `dom(Σ.M(d))` onto itself (RA-π), so `v := π⁻¹(v') ∈ dom(Σ.M(d))` is well-defined as the unique pre-image of `v'` under π. The computation `Σ.M(d)(v) = Σ'.M(d)(π(v)) = Σ'.M(d)(v') ∈ coverage(e)` then proceeds by RA-π together with the state-independence of coverage, so `v ∈ project(e, d, Σ)` and `v' = π(v) ∈ π(project(e, d, Σ))`. Therefore
```
project(e, d, Σ') = π(project(e, d, Σ))                                                 (RE-proj)
```

This identity is well-defined across the freedom in choosing π. When `Σ.M(d)` carries shared I-addresses (allowed by S5), multiple bijections satisfy RA-π — any two witnesses agree on the arrangement pre-image partition `{Σ.M(d)⁻¹(a)}_{a ∈ ran(Σ.M(d))}` but may permute freely within each pre-image (and symmetrically within the post-state pre-images `{Σ'.M(d)⁻¹(a)}_{a ∈ ran(Σ'.M(d))}`). The set-image `π(project(e, d, Σ))` is invariant under this freedom: for any `v ∈ project(e, d, Σ)`, every valid π carries `v` to some `v' ∈ dom(Σ'.M(d))` with `Σ'.M(d)(v') = Σ.M(d)(v) ∈ coverage(e)`, hence `v' ∈ project(e, d, Σ')`. Conversely, every `v' ∈ project(e, d, Σ')` is `π⁻¹(v')`-imaged from some `v ∈ project(e, d, Σ)`. So both inclusions hold for any valid π, and `π(project(e, d, Σ)) = project(e, d, Σ')` is a state-determined identity — the RHS depends only on Σ' (not on π), and RE-proj's equation constrains the LHS via the RHS, with the conclusion uniform across witnesses.
...
RE-sub is the one consequence in this ASN that does not flow from the abstract class alone. RA-adm together with foundation S3★ + L14 already forces *subspace preservation* — no V-position may cross from one subspace to another under π — but admissibility allows non-identity permutations *within* each subspace. The pointwise form of RE-sub — that non-S V-positions are not permuted at all, not merely kept within their subspace — is what the cut-sequence structure supplies and what admissibility alone does not.
...
The subspace-preservation half is structurally necessary at the abstract level — the same RA-adm + S3★ + L14 consequence already noted in the RA-adm exposition of "REARRANGE as Vstream-Only Operation" above, restated here for emphasis on its role in distinguishing pointwise fixity from mere within-subspace constraint. We argue both directions, since each is required to establish that "no V-position may cross from one subspace to another." (Content-to-link direction: an admissible bijection cannot carry a content-subspace V-position to a link-subspace V-position, since the post-state image would land in `dom(Σ'.C)` while S3★'s link-subspace clause demands a `dom(Σ'.L)` value, and these stores are disjoint by L14. In detail: let `v ∈ dom(Σ.M(d))` with `subspace(v) = s_C`; pre-state S3★ gives `Σ.M(d)(v) ∈ dom(Σ.C)`; suppose for contradiction `subspace(π(v)) = s_L`; RA-π combined with RE-C gives `Σ'.M(d)(π(v)) = Σ.M(d)(v) ∈ dom(Σ.C) = dom(Σ'.C)`; but S3★'s link-subspace clause at Σ' demands `Σ'.M(d)(π(v)) ∈ dom(Σ'.L)`; L14 (`dom(Σ'.C) ∩ dom(Σ'.L) = ∅`) yields the contradiction. Link-to-content direction (by symmetric reasoning, applying RE-L in place of RE-C): an admissible bijection cannot carry a link-subspace V-position to a content-subspace V-position. Let `v ∈ dom(Σ.M(d))` with `subspace(v) = s_L`; pre-state S3★ gives `Σ.M(d)(v) ∈ dom(Σ.L)`; suppose for contradiction `subspace(π(v)) = s_C`; RA-π combined with RE-L gives `Σ'.M(d)(π(v)) = Σ.M(d)(v) ∈ dom(Σ.L) = dom(Σ'.L)`; but S3★'s content-subspace clause at Σ' demands `Σ'.M(d)(π(v)) ∈ dom(Σ'.C)`; L14 again yields the contradiction. Both directions establish the full statement that no V-position may cross subspaces under any admissible π.) The pointwise-fixity strengthening, however, is REARRANGE_K-specific: a different concrete realization of the abstract class could non-trivially permute the link subspace while preserving subspace identity, and would still satisfy RA-adm. Under such an alternate realization the arrangement-preservation half of RE-sub could fail in a more subtle way too: if π permuted link-subspace V-positions whose images shared an I-address (S5/UnrestrictedSharing), the arrangement values would still match position-by-position, but distinct V-positions would have swapped identities. REARRANGE_K's cut sequence operates in one subspace at a time and leaves the others verbatim — this is the joint content of R-PPERM/R-SPERM (for π) and R-FRAME-P/S(a) (for the arrangement).

## In-Subspace Exterior Frame (REARRANGE_K-specific)

A second pointwise-fixity property complements RE-sub on the cut subspace itself. RE-sub covers V-positions in subspaces other than the cut subspace; RE-ext (below) covers V-positions *within* the cut subspace S that lie *outside* the affected range `[c₀, c_{n−1})`. Under the abstract class alone, admissibility forces only that π respect subspace identity (RA-adm + S3★ + L14); within the cut subspace, admissibility leaves room for π to permute every V-position freely. REARRANGE_K's cut-sequence structure delivers strict pointwise fixity for in-subspace V-positions outside the affected range.
...
- change the cardinality or content-identity of any link's projection onto d (RE-proj transports the projection set along π — the specific V-positions in the set may migrate, but the cardinality and the underlying I-addresses they identify are preserved);
...
- **RE-frag★ / RE-coal★ / RE-eq★:** no per-step monotonicity is available — each step in the sequence may independently increase (RE-frag), decrease (RE-coal), or preserve (RE-eq) run-decomposition cardinality, as the single-step witnesses above demonstrate. The composite claim is the negation of any uniform per-step direction; we do not assert anything stronger about the net cardinality change `|runs(Σ_n.M(d))| − |runs(Σ₀.M(d))|` across the full sequence.
...
| RE-trans | Transclusion preservation: for every (a, d) with a ∈ ran(Σ.M(d)) and origin(a) ≠ d, the transclusion relationship and its multiplicity persist at Σ', and origin(a)'s arrangement is unchanged | abstract (from RE-ran + RE-μ + RE-other + RE-C + RE-origin; foreign-address routing via CL-OWN + S3★ + C2) | introduced |
| RE-sub | Subspace frame: for every v ∈ dom(M(d)) with subspace(v) ≠ S, π(v) = v and Σ'.M(d)(v) = Σ.M(d)(v) | REARRANGE_K (π-fixity from R-PPERM/R-SPERM; arrangement preservation from R-FRAME-P/S(a)) | introduced |
...
| RE-frag★ / RE-coal★ / RE-eq★ | Existential composite cardinality variations: any combination of strict increase, strict decrease, or equality is realisable across multi-step sequences; no per-step monotonicity is available | none (existential) | abstract (from RE-frag/coal/eq witnesses) | introduced |
