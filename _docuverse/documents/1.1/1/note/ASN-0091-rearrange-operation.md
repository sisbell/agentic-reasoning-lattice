...
*Reachability scope of the realisation.* The discharge of RA-adm for the REARRANGE_K realiser below routes through reachability: it shows Σ' reachable and reads off the per-state foundation invariants from ASN-0047's ExtendedReachableStateInvariants. This argument therefore establishes RA-adm only for a *reachable* pre-state Σ, and we scope the realisation theorem accordingly — REARRANGE_K realises the abstract Vstream-only class on every Σ reachable from Σ₀ by a finite sequence of elementary transitions drawn from valid composites. The abstract RA-adm clause as defined quantifies over arbitrary Σ; for a Σ that satisfies every per-state invariant but is not reachable, the reachability route supplies nothing, and the realiser's admissibility is not claimed there. (A reachability-independent, invariant-by-invariant preservation argument would lift the realisation to all invariant-satisfying Σ; we do not pursue it, since every state REARRANGE_K is applied to in this development arises along a trace from Σ₀ and is reachable by construction.)
...
Several of the run-decomposition witnesses and worked examples below turn on the same structural fact: two I-addresses drawn from *different* sub-allocator chains can never be chain-consecutive. Because the result is invoked from three separate places — the coalescence witness (RE-coal), the equality witness (RE-eq), and the shared-I-address worked example — we state it once as a named lemma rather than inline.
...
A REARRANGE is itself a composite (K.μ~ for the REARRANGE_K realiser, the empty composite in the collapse case), so its endpoints are composite boundaries. For Σ' to be a *valid* composite boundary it must satisfy not only the per-state foundation invariants (discharged by RA-adm) but also ASN-0047's composite-boundary properties **P4★ ∧ P4a ∧ P7a**. RA-adm covers only per-state invariants, so we discharge these three at the abstract level here, each from RE-ran, RE-C, or RE-R. Throughout, Σ is a reachable pre-state, so each property holds at Σ as a composite-boundary property of ASN-0047's ExtendedReachableStateInvariants.

**P7a (ProvenanceCoverage), `(A a ∈ dom(Σ'.C) :: (E d :: (a, d) ∈ Σ'.R))`.** By RE-C, `dom(Σ'.C) = dom(Σ.C)`; by RE-R, `Σ'.R = Σ.R`. P7a holds at Σ, so it transfers to Σ' verbatim — the quantifier ranges over the identical content domain and is witnessed by the identical relation.

**P4★ (ProvenanceBounds), `Contains_C(Σ') ⊆ Σ'.R`.** The content containment `Contains_C(Σ) = {(a, d) : d ∈ E_doc ∧ (E v : v ∈ dom(Σ.M(d)) ∧ subspace(v) = s_C : Σ.M(d)(v) = a)}` selects content-subspace images. By S3★ a content-subspace V-position maps into `dom(Σ.C)` and a link-subspace one into `dom(Σ.L)`, and these are disjoint (L14/SD), so the content-subspace images of d are exactly `ran(Σ.M(d)) ∩ dom(Σ.C)`; hence `Contains_C(Σ) = {(a, d) : d ∈ E_doc ∧ a ∈ ran(Σ.M(d)) ∩ dom(Σ.C)}`, a function of the per-document ranges and the content domain alone. RE-ran fixes every `ran(Σ'.M(d)) = ran(Σ.M(d))` and RE-C fixes `dom(Σ'.C) = dom(Σ.C)`, so `Contains_C(Σ') = Contains_C(Σ)`; with `Σ'.R = Σ.R` (RE-R), the pre-state inclusion `Contains_C(Σ) ⊆ Σ.R` carries to `Contains_C(Σ') ⊆ Σ'.R`.
...
| RA-adm | Rearrangement admissibility: every per-state foundation invariant satisfied by Σ is satisfied by Σ' | abstract (definition); REARRANGE_K realisation *scoped to reachable Σ*: the K.μ~-validity / empty-composite argument makes Σ' reachable, whence the per-state foundation invariants follow at Σ' (the realiser's admissibility is not claimed at non-reachable invariant-satisfying Σ) |
