...
The per-state foundation invariants hold at every reachable state (ASN-0047's ExtendedReachableStateInvariants), so RA-adm reduces to establishing that Σ' is reachable, given Σ reachable from Σ₀ by a finite sequence of elementary transitions drawn from valid composites.
