...
- **RA-adm ← per-invariant discharges below.** Discharged in three layers: a constructive shape package from RA-dom, the abstract RE-subpres, and the remaining per-state invariants from ASN-0047's ExtendedReachableStateInvariants once K.μ~ is a valid composite.
...
- **Clause (i) — induced post-state satisfies the shape package (S8a, S8-depth, D-CTG★, D-MIN★).** Discharged by the constructive shape-package layer of the per-invariant discharges below, which derives each of these at Σ' from RA-dom alone (independent of K.μ~, so no circularity against this clause).
...
P4a (TraceWitnessing) is the one foundation invariant excluded from the frame-inheritance class above. Its existential witness `Σ_k` ranges over the prior transition trace rather than the current state, so RA-frame does not discharge it directly. By ASN-0093's SequentialTransitionAxiom the trace is append-only, so any pre-existing witness `Σ_k ∈ {Σ_0, ..., Σ_n}` for `(a, d) ∈ Σ.R` survives, and RE-R's `Σ'.R = Σ.R` preserves `(a, d) ∈ Σ'.R`.
...
The *abstract* property — subspace preservation as a property of any admissible π — is captured by RE-subpres (derived earlier in the "REARRANGE as Vstream-Only Operation" section). RE-subpres asserts only that no V-position crosses from one subspace to another (`subspace(π(v)) = subspace(v)`); admissibility allows non-identity permutations *within* each subspace under the abstract class. The pointwise form of RE-sub — that non-S V-positions are not permuted at all (`π(v) = v`), not merely kept within their subspace — is what the cut-sequence structure supplies and what admissibility (and RE-subpres) alone does not.
...
What rearrangement does is exactly one thing: it permutes which V-positions hold which I-addresses, via a bijection π that exhausts d's V-stream domain. Everything else is invariant — the content store, link store, populated V-positions, I-address multiset, link coverage and discoverability, projections (transported along π̂_d), other documents' arrangements, origin, and provenance — each catalogued with its RE-* label and provenance in the Claims Introduced table. Everything else follows — including the cost (run-decomposition cardinality can grow under fragmentation) and the guarantees (link survivability, transclusion preservation, content permanence).
...
- **Admissibility (RA-adm).** The post-state invariants hold against `Σ'.M(d) = {[1, 1] ↦ a₃, [1, 2] ↦ a₄, [1, 3] ↦ a₂, [1, 4] ↦ a₁, [2, 1] ↦ a_link}` by the per-invariant discharge of the first Worked Example, with five V-positions in place of four. The only structurally distinctive clause under the 4-cut swap is *S8★*: the content subspace decomposes into three correspondence runs `([1, 1], a₃, 2), ([1, 3], a₂, 1), ([1, 4], a₁, 1)` (identified under RE-frag above), where the μ-region's `a₂` — displaced to `[1, 3]` by the `w_α ≠ w_β` asymmetry — heads its own singleton run; the link-subspace clause `{[2, 1] ↦ a_link}` carries over verbatim via RE-sub. All other clauses (including S3★ and P4★) and P4a discharge exactly as in the first Worked Example.
...
*Admissibility (RA-adm).* The post-state invariants hold against `Σ'.M(d) = {[1, 1] ↦ b, [1, 2] ↦ a, [1, 3] ↦ a}`. The load-bearing distinction from the first three traces is the shared I-address `a` at two V-positions, so we show only the clauses whose witness turns on sharing. *S5 (UnrestrictedSharing):* the post-state assignment of `a` at both `[1, 2]` and `[1, 3]` is admitted by S5; the multiset shape `(a → 2, b → 1)` is the pre-state shape preserved by RE-μ. *S2 (functionality):* each of `[1, 1], [1, 2], [1, 3]` appears once on the left of the map, so each V-position has a unique image even though distinct V-positions share an image (which S5 admits but S2 does not exclude). *S8★:* the content subspace decomposes into three singletons `([1, 1], b, 1), ([1, 2], a, 1), ([1, 3], a, 1)` — the shared `a` at adjacent V-positions fails chain-adjacency (a length-2 run would need `Σ'.M(d)([1, 3]) = a + 1`, but it is `a`, and `a + 1 ≠ a` by TA-strict). All remaining clauses (S3★, P4★; CL-OWN and CL-UNIQ vacuous, no link-subspace V-position populated) and P4a discharge by the per-invariant pattern of the first three traces.
