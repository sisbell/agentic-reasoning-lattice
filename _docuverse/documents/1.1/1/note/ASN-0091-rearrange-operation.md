...
| RA-adm | discharged in two constructive layers: shape package (from RA-dom) and the remaining per-state invariants (via ExtendedReachableStateInvariants). RE-subpres is a downstream consequence of RA-adm, not a layer that establishes it |
...
| (ii) non-trivial net effect `M'(d) ≠ M(d)` | the net-effect case split established in "REARRANGE as Vstream-Only Operation" (the S5 witness that `π ≠ id` and `M'(d) ≠ M(d)` come apart): K.μ~ is the realiser exactly in the non-trivial case, where clause (ii) holds by hypothesis |
...
*Remaining per-state invariants (from ExtendedReachableStateInvariants).* With K.μ~'s admissibility clauses (i)–(v) closed above, K.μ~ is a valid composite, so ASN-0047's ExtendedReachableStateInvariants establishes that it preserves the full per-state invariant package at its boundary. This discharges the arrangement-dependent invariants outside the shape package — S3★, S3★-aux, CL-OWN, CL-UNIQ, S8★ — together with the co-occurring ASN-0036 foundation and ASN-0093 substrate invariants and the composite-boundary properties P4★, P4a, and P7a, without re-deriving each from scratch. With this layer the full per-state invariant package holds at Σ', so RA-adm is established.

*RE-subpres as a downstream consequence.* RE-subpres is not a layer that establishes RA-adm — it is a consequence of it. Once the two layers above secure RA-adm (in particular post-state S3★ and S3★-aux), the abstract derivation in "REARRANGE as Vstream-Only Operation" yields RE-subpres for any admissible π. For REARRANGE_K, RE-subpres is moreover available constructively and independently of RA-adm: K.μ~ admissibility clause (iv), discharged above from the R-PPERM/R-SPERM branch structure alone, is exactly `subspace(π(v)) = subspace(v)`, confining each branch of π to a single subspace.
