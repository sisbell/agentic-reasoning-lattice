...
*S2 derivation at the abstract level.* The post-state arrangement `Σ'.M(d)` is a partial function — foundation invariant S2 — derived directly from RA-π. Since π is a bijection (RA-π), each `v' ∈ dom(Σ'.M(d)) = dom(Σ.M(d))` (RA-dom) is the image of a unique `v = π⁻¹(v') ∈ dom(Σ.M(d))` — π's surjectivity supplies the existence of `v` and its injectivity supplies the uniqueness; the inverse `π⁻¹` is itself well-defined only because π is a bijection. RA-π then assigns `Σ'.M(d)(v') = Σ.M(d)(v)`, a function value determined uniquely by `v'`. So `Σ'.M(d)` is a partial function. The derivation is abstract — it relies only on RA-dom, RA-π (bijection), and S2 at Σ — independent of how π was generated, so every concrete realization of the class (including REARRANGE_K) inherits S2 at Σ' without case-specific verification.
...
The subspace-preservation half is structurally necessary at the abstract level: an admissible bijection cannot carry content-subspace V-positions into link-subspace V-positions or vice versa, because the typed referential integrity invariant (foundation S3★: content-subspace V-positions map to `dom(C)`, link-subspace V-positions map to `dom(L)`) would be violated at Σ' — and RA-adm requires Σ' to satisfy it. The pointwise-fixity strengthening, however, is REARRANGE_K-specific: a different concrete realization of the abstract class could non-trivially permute the link subspace while preserving subspace identity, and would still satisfy RA-adm. Under such an alternate realization the arrangement-preservation half of RE-sub could fail in a more subtle way too: if π permuted link-subspace V-positions whose images shared an I-address (S5/UnrestrictedSharing), the arrangement values would still match position-by-position, but distinct V-positions would have swapped identities. REARRANGE_K's cut sequence operates in one subspace at a time and leaves the others verbatim — this is the joint content of R-PPERM/R-SPERM (for π) and R-FRAME-P/S(a) (for the arrangement).
