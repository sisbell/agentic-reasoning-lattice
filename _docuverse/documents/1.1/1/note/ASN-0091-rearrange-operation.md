...
| (iv) subspace-preserving `subspace(π(v)) = subspace(v)` | Directly from the R-PPERM/R-SPERM branch structure: non-S and in-S-exterior positions are fixed (`π(v) = v`), so subspace is trivially preserved; every affected-range position `v` has the cut subspace S (by CS3), and R-PPERM/R-SPERM map it to a position of the form `c₀ + (offset)`, which shares c₀'s subspace S, so `subspace(π(v)) = S = subspace(v)` (ASN-0036's OrdShiftHom(a)). Discharged from the cut-sequence construction alone — not via the abstract RE-subpres, which is a downstream consequence of RA-adm |
...
*Target case.* For `d = d_tgt`, the bijection π carries the projection set faithfully to the post-state. This is ASN-0098's LP11 (ReorderingBijection) instantiated at the REARRANGE realiser `Σ' = K.μ~(Σ)`:
```
project(e, d_tgt, Σ') = π(project(e, d_tgt, Σ))
```
LP11's hypotheses are exactly RA-π (the witnessing bijection with `Σ'.M(d)(π(v)) = Σ.M(d)(v)`) and the state-independence of coverage; the abstract Vstream-only class supplies both, so the conclusion holds for every realisation, not only REARRANGE_K.
...
Every ★ form whose single-step claim is a preserved equality follows from this lemma by induction: RE-C★, RE-L★, RE-R★, RE-dom★, RE-ran★, RE-μ★, RE-cov★, RE-disc★, RE-proj★, RE-sub★, and RE-origin★ are catalogued with their composition conditions in the ★ table of Claims Introduced. Three ★ forms are not bare equalities and need separate treatment: two carry substantive side conditions, and one is an existence claim.
