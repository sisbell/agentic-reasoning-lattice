...
In the non-trivial case, K.μ~'s admissibility clauses (i)–(v) are closed above, so K.μ~ is a valid composite; its K.μ⁻ + K.μ⁺ elementary decomposition (ASN-0047) is then a finite sequence of elementary transitions drawn from a valid composite. Appending that sequence to any trace witnessing reachability of Σ yields a trace witnessing reachability of Σ' — the K.μ~ composite extends reachability by one composite step, and Σ' sits at the resulting composite boundary. ExtendedReachableStateInvariants applied to the now-reachable Σ' delivers the full per-state invariant package at Σ', discharging S3★, S3★-aux, CL-OWN, CL-UNIQ, S8★; the composite-boundary clause of the same theorem additionally delivers P4★, P4a, and P7a at Σ'. In the collapse case `M'(d) = M(d)`, so the REARRANGE_K transition is the identity `Σ' = Σ`; no K.μ~ composite is owed (clause (ii) excludes `M'(d) = M(d)`, so K.μ~ is unavailable), and none is needed. The realising composite is the *empty* sequence of elementary transitions — admitted by ASN-0047's SequentialTransitionAxiom, whose reflexive-transitive closure `Σ →* Σ'` ranges over finite, possibly empty, sequences. The empty composite satisfies every elementary precondition vacuously and carries `Σ' = Σ` by definition; reachability of Σ' = Σ is immediate from reachability of Σ (the witnessing trace is unchanged), and every invariant holds at Σ' = Σ trivially. We therefore do not appeal to a bare K.μ⁻ + K.μ⁺ pair — which would interpose a strictly contracted intermediate state whose elementary preconditions are not discharged — but to the empty composite, which returns `Σ' = Σ` directly. The remaining per-state invariants — those depending only on components RA-frame fixes verbatim — are discharged by the frame-inheritance and transition-satisfaction arguments of "State-Component-Only Invariants" below. With all three layers the full per-state invariant package holds at Σ', so RA-adm is established for the REARRANGE_K realiser.

### State-Component-Only Invariants

All remaining foundation invariants that depend only on Σ.C, Σ.L, Σ.E, Σ.R, or `dom(Σ.M)` — and not on per-document arrangements — are discharged across REARRANGE by RA-frame, which fixes each of these components verbatim. Two categories are gathered here and discharge by different routes: single-state predicates by **frame inheritance**, and binary transition invariants by **transition-satisfaction**.
