...
written `compareversions(d_a, R_a, d_b, R_b)`. The restricting span-sets `R_a, R_b` select which portions of each arrangement participate. Without them, comparison would implicitly span the entire arrangement of each side; with them, the caller confines the operation to a passage of `d_a` against a passage of `d_b`.
...
Under the exact constraint `actionPoint(width(σ)) = m_σ`, the span's V-extent at depth `m_σ` contains exactly `n_σ` consecutive depth-`m_σ` tumblers starting at `start(σ)` — a property of the span alone, independent of any arrangement. CV-IN does not require the span to fit within the arrangement; the operation accommodates a span that exceeds `V_S(d)` via the run conditions on `dom(M(d))` membership.
...
*The shared I-address is not recorded in the triple.* For a run `(v_a, v_b, n)` at offset `0 ≤ k < n`, the shared I-address is derivable as `M(d_a)(v_a + k)` (equivalently `M(d_b)(v_b + k)`, by condition (iii)), so the triple omits it.
