...
> **CV-IN**: `d_a, d_b ∈ E_doc`. `R_a, R_b` are normalized V-span-sets (ASN-0053). For every span `σ ∈ R_a ∪ R_b`: `start(σ) ∈ V_S(d)` for the relevant document `d` and a common subspace identifier `S ∈ {s_C, s_L}` (the same `S` for both `R_a` and `R_b`); `σ` is level-uniform (S6, ASN-0053) at the document's V-position depth `m_a := m_{d_a, S}` for `σ ∈ R_a` or `m_b := m_{d_b, S}` for `σ ∈ R_b` (S8-depth, ASN-0036); and `actionPoint(width(σ)) ≥ 2` — equivalently, `width(σ)` is an ordinal displacement at the document's depth (ASN-0034, ASN-0058 C0). The depth bound `m ≥ 2` is itself supplied by S8a (ASN-0036) on V-positions.

The action-point clause `actionPoint(width(σ)) ≥ 2` is load-bearing. Level-uniformity (S6) alone requires only `#start(σ) = #width(σ)`; it does not constrain the action point of the width. When `actionPoint(width(σ)) = 1`, TumblerAdd (ASN-0034) makes the first component of `reach(σ)` equal to `start(σ)₁ + width(σ)₁ > S`, so `⟦σ⟧` extends into tumblers with first component greater than `S` — leaving the subspace `S`. The minimum action point of 2 confines `⟦σ⟧` to the depth-`m` tumblers sharing first component `S`, which is the in-subspace property the restriction is meant to enforce. We make the constraint explicit at the input level rather than rely on the downstream relation `corr_{a,b}` to filter cross-subspace positions via L0 + L14 (ASN-0047); structurally, the input is what the operation contracts on.
...
|-------|-----------|--------|
| CV-IN | Admissibility: `d_a, d_b ∈ E_doc` (with `d_a = d_b` admissible); `R_a, R_b` normalized V-span-sets lying within a single common subspace `S`; each span level-uniform at the document's V-position depth in `S` with `actionPoint(width(σ)) ≥ 2` (ordinal displacement at depth `m`) | introduced |
