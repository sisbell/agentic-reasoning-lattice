...
*Derivation.* We derive each form from foundation NAT axioms (ASN-0034). For the right-summand form `b + a > b`: NAT-addcompat's strict successor clause gives `b < b + 1` directly. NAT-addcompat's order compatibility applied at `(m, n, p) = (b, a, 1)` lifts `a ≥ 1` to `b + a ≥ b + 1`. NAT-order's transitivity composing the weak `b + a ≥ b + 1` with the strict `b + 1 > b` (i.e., `b < b + 1`) yields `b + a > b`. For the left-summand form `a + b > b`: by the same chain instantiated with operand roles symmetric in `+`, NAT-addcompat's strict successor at the b-summand position together with order compatibility lifts `a ≥ 1` to `a + b ≥ 1 + b > b` — formally, NAT-closure's left identity `0 + b = b` together with NAT-addcompat at `(m, n, p) = (1, b, 0)` would give `1 + b ≥ 1 + 0 = 1` (not what we need); the cleaner path is to invoke commutativity of `+` on ℕ — a derived property of the natural-number monoid available under NAT-closure, NAT-addcompat, and NAT-wellorder's induction — to rewrite `a + b` as `b + a` and reduce to the right-summand form just proved. Both forms therefore hold. ∎
...
| PositiveOffsetExceeds | lemma | For a, b ∈ ℕ with a ≥ 1: a + b > b and b + a > b — strict dominance of an offset sum over its summand | introduced (derived from NAT-addcompat, NAT-order, NAT-closure, ASN-0034) |
