...
Each non-trivial obligation discharges via a foundation invariant or precondition: TS2 (ShiftInjectivity, ASN-0034) for (1); TS4 (ShiftStrictlyAdvances, ASN-0034) combined with the I3 preconditions `n ≥ 1` and the quantifier ranges `u₁ ≥ p, u₂ < p` for (2); subspace preservation derived from the I3 precondition `#p ≥ 2` for (3); pre-state S2 for (4) and (6); subspace-identifier disjointness for (5); I3-V's exclusion clause for (7).
...
Compute reach(σ'ₛ) = σ(s) ⊕ ℓ. By definition, σ(s) = vpos(1, ord(s) ⊖ w_ord) = [1, s₂ − c] (TumblerSub at depth 1: divergence at position 1, result s₂ − c, well-defined since s₂ ≥ p₂ + c and p₂ + c > c by PositiveOffsetExceeds — chaining via NAT-order's transitivity gives s₂ > c). Then σ(s) ⊕ ℓ = [1, s₂ − c] ⊕ [0, c'] = [1, s₂ − c + c'] = [1, s₂ + c' − c] by TumblerAdd at action point 2.

The two are equal. ✓ ∎

*Derivation of (b).* The span σ'ₛ is level-uniform: #σ(s) = 2 = #ℓ by vpos's result-length identity. By D2 (WidthRecovery, ASN-0053), width(σ'ₛ) = reach(σ'ₛ) ⊖ start(σ'ₛ) = [1, s₂ + c' − c] ⊖ [1, s₂ − c]. The two tumblers agree at position 1 (both equal to 1) and differ at position 2 by (s₂ + c' − c) − (s₂ − c) = c'. By TumblerSub at depth 2 with divergence at position 2, the result is [0, c'] = ℓ. ✓ ∎
