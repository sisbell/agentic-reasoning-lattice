...
The remaining post-state lemmas — S2-post (functionality), S8-depth-post, D-CTG-post, D-MIN-post, D-SEQ-post (contiguity triple), S8-fin-post, S3-post, and S7-post — are established by the preservation lemmas above, exactly as the insertion-half remainder is subsumed by I3-VP. Their operation-specific discharges: S2-post's cross-region no-conflict obligation is the region disjointness of D-DP(a) (`L ∩ Q₃ = ∅`) and D-BJ(b) (injectivity of σ) plus pre-state S2; the contiguity triple discharges against the closed form V_1(d') = {[1, k] : 1 ≤ k ≤ N − c}; S3-post and S7-post against D-I; and S8-fin-post against |L ∪ Q₃| = |L| + |R|.
