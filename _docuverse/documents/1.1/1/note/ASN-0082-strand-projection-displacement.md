...
The example exercises I3 against a pre-state where the active-subspace region V_2(d) lacks D-CTG: gaps at [2, 1], [2, 2], [2, 4], [2, 5], [2, 6], [2, 8], [2, 9] in the pre-state are durable tombstone structure, not violations of foundation invariants (the foundation's D-CTG is text-subspace only). The shifted positions {[2, 9], [2, 12]} inherit S8a from the pre-state via TumblerAdd's action-point arithmetic — at depth m = 2, the position-1 (subspace identifier) is copied from the operand and remains ≥ 1, and the position-2 (ordinal) is the strict sum of two positive naturals and remains ≥ 1 by NAT-addbound's dominance clauses. Note that one pre-state tombstone gap, [2, 9], is now occupied in the post-state (by the shifted image of [2, 7]); this is permitted — the link subspace's gap structure is not preserved across a link-side insertion, only its sparsity discipline. The wp derivation for I3 (specifically the chain I3-VV → S8a) carries through identically at S = 2 as it does at S = 1: it routes through S8a, S8-depth, and TumblerAdd's action-point identity, all of which are subspace-agnostic. ∎
...
The shift is well-defined. For any v ∈ R, ord(v) ≥ ord(r) (since v ≥ r, by OrdinalOrderEquivalence). Since r = p ⊕ w, OrdAddHom (a) gives ord(r) = ord(p) ⊕ w_ord. The subtraction ord(v) ⊖ w_ord is well-defined by TA2 (SubtractionWellDefined, ASN-0034), which requires ord(v) ≥ w_ord. We establish this step by step. At depth #p = 2: ord(v) = [v₂], ord(p) = [p₂], w_ord = [c] with c = w₂ ≥ 1, and ord(r) = ord(p) ⊕ w_ord = [p₂ + c] (TumblerAdd). From v ≥ r and OrdinalOrderEquivalence, ord(v) ≥ ord(r) = [p₂ + c], i.e., v₂ ≥ p₂ + c (T1 at depth 1). Since p ∈ V_1(d), S8a gives p₂ ≥ 1, hence p₂ + c > c by PositiveOffsetExceeds (a = p₂, b = c). NAT-order's transitivity composing the weak `v₂ ≥ p₂ + c` with the strict `p₂ + c > c` yields v₂ > c, and NAT-order's strict-implies-weak (`a > b ⟹ a ≥ b`) gives v₂ ≥ c, equivalently ord(v) = [v₂] ≥ [c] = w_ord (T1 at depth 1). So TA2 applies and `[v₂] ⊖ [c] = [v₂ − c]` is well-defined. The shifted ordinal is positive: the minimum shifted ordinal is ord(r) ⊖ w_ord = ord(p) (verified in D-SEP below). Since p ∈ V_1(d) and S8a's componentwise positivity gives p₂ ≥ 1, ord(p) = [p₂] is positive. The reconstructed V-position σ(v) = vpos(S, ord(v) ⊖ w_ord) then satisfies S8a by vpos's S8a-closure postcondition (ASN-0036): S = 1 ≥ 1 and the ordinal is componentwise positive.
...
We derive `ord(v) ≥ w_ord` step by step for every v ∈ R. At depth #p = 2 we have ord(p) = [p₂], w_ord = [c] with c = w₂ ≥ 1 (since Pos(w) and w₁ = 0 imply w₂ ≥ 1), and ord(r) = ord(p ⊕ w) = ord(p) ⊕ w_ord = [p₂ + c] by OrdAddHom(a). For any v ∈ R, v ≥ r (definition of R), so by OrdinalOrderEquivalence ord(v) ≥ ord(r) = [p₂ + c]. The ordinal comparison `[v₂] ≥ [p₂ + c]` at depth 1 is the natural-number comparison `v₂ ≥ p₂ + c` (by T1 at depth 1, which reduces to component-wise comparison). From p₂ ≥ 1 (S8a's componentwise positivity applied to p ∈ V_1(d) at position 2), PositiveOffsetExceeds (a = p₂, b = c) gives `p₂ + c > c`. Combining with `v₂ ≥ p₂ + c` through NAT-order's transitivity (composing the weak `v₂ ≥ p₂ + c` with the strict `p₂ + c > c`) yields `v₂ > c`, hence `v₂ ≥ c` (NAT-order's strict-implies-weak). The depth-1 ordinal comparison `[v₂] ≥ [c]` is exactly this natural-number inequality, so `ord(v) = [v₂] ≥ [c] = w_ord`. ∎(derivation of `ord(v) ≥ w_ord`)
...
*Derivation of (a).* We have reach(σₛ) = s ⊕ ℓ and reach(σ'ₛ) = σ(s) ⊕ ℓ (SpanReach, ASN-0053). At depth 2 with ℓ = [0, c'] (since actionPoint(ℓ) = 2 and Pos(ℓ) force ℓ₁ = 0 and ℓ₂ = c' ≥ 1), OrdAddHom(a) (ASN-0036) applied to s ⊕ ℓ gives ord(reach(σₛ)) = ord(s) ⊕ ℓ_ord = [s₂] ⊕ [c'] = [s₂ + c'] by TumblerAdd. From s ∈ R, ord(s) ≥ w_ord (the same derivation that establishes D-BJ's well-definedness: s₂ ≥ p₂ + c by OrdinalOrderEquivalence on s ≥ r, and p₂ + c > c by PositiveOffsetExceeds with a = p₂, b = c, where p₂ ≥ 1 is S8a on p; NAT-order's transitivity then composes the weak ≥ with the strict > to give s₂ > c, hence s₂ ≥ c by strict-implies-weak), and adding [c'] (componentwise positive) only widens the inequality, so ord(reach(σₛ)) = [s₂ + c'] ≥ [c] = w_ord. Hence σ extended to reach(σₛ) is well-defined by TA2.
