...
**NAT-CA** — *CarrierAdditionCommutativityAssociativity* (introduced locally). For all m, n, k ∈ ℕ: `m + n = n + m` (commutativity) and `(m + n) + k = m + (n + k)` (associativity). *Justification.* ℕ is the standard set of finite cardinals (the Peano naturals) over which T0 (ASN-0034) builds the carrier T; commutativity and associativity of their addition are defining properties of finite-cardinal arithmetic (witnessed by the canonical bijections `A ⊎ B ≅ B ⊎ A` and `(A ⊎ B) ⊎ C ≅ A ⊎ (B ⊎ C)` of disjoint unions). ASN-0034's minimal NAT-* extraction (NAT-addcompat, NAT-closure, NAT-discrete, NAT-order, NAT-wellorder) names only the order-and-monotonicity facts its own proofs required — and, as the order-monotone but non-commutative model of ordinal addition shows, commutativity is *not* derivable from them — so we supply these two carrier facts here, where the span-algebra corollaries I3-S(a) and D-S(a) require reordering scalar ℕ sums. This discharges, rather than merely flags, the reordering steps below.
...
Without D-DOM, D-L and D-SHIFT alone would constrain only the surviving L and R positions, leaving dom(M'(d)) ∩ subspace 1 underdetermined; D-DOM closes it from above to exactly L ∪ Q₃.
