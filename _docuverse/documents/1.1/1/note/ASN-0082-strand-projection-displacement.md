...
**NAT-CA** — *CarrierAdditionCommutativityAssociativity* (introduced locally). For all m, n, k ∈ ℕ: `m + n = n + m` (commutativity) and `(m + n) + k = m + (n + k)` (associativity). *Justification.* These are defining properties of finite-cardinal arithmetic on ℕ, the carrier over which T0 (ASN-0034) builds T. ASN-0034's minimal NAT-* extraction (NAT-addcompat, NAT-closure, NAT-discrete, NAT-order, NAT-wellorder) names only the order-and-monotonicity facts its own proofs required; commutativity is *not* derivable from them, as the order-monotone but non-commutative model of ordinal addition shows, so we supply these two carrier facts here.
