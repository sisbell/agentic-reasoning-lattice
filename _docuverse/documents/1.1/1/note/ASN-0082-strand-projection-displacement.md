...
- *S8a-post (post-state well-formedness, the key non-frame-style invariant exercised by I3 over the active link subspace):* every post-state position is zero-free, has depth 2 ≥ 2, and has each component ≥ 1. Text positions [1, 1] and [1, 2] inherit S8a from the pre-state via I3-X. Left-region link position [2, 3] inherits via I3-L. For shifted link positions [2, 9] and [2, 12], TumblerAdd's prefix-copy at position 1 preserves the subspace identifier (2 ≥ 1), and the action-point sum at position 2 produces 7 + 2 = 9 ≥ 1 and 10 + 2 = 12 ≥ 1 (both positive since the pre-state values 7, 10 are positive by S8a on M(d) and the shift amount 2 ≥ 1, and NAT-addbound's left-dominance carries positivity through `+`). Zeros count is 0 at each shifted position. ✓
- *I3-C:* dom(C') = dom(C) and values unchanged. ✓

The example exercises I3 against a pre-state where the active-subspace region V_2(d) lacks D-CTG: gaps at [2, 1], [2, 2], [2, 4], [2, 5], [2, 6], [2, 8], [2, 9] in the pre-state are durable tombstone structure, not violations of foundation invariants (the foundation's D-CTG is text-subspace only). The shifted positions {[2, 9], [2, 12]} inherit S8a from the pre-state via TumblerAdd's action-point arithmetic — at depth m = 2, the position-1 (subspace identifier) is copied from the operand and remains ≥ 1, and the position-2 (ordinal) is the strict sum of two positive naturals and remains ≥ 1 by NAT-addbound's dominance clauses. Note that one pre-state tombstone gap, [2, 9], is now occupied in the post-state (by the shifted image of [2, 7]); this is permitted — the link subspace's gap structure is not preserved across a link-side insertion, only its sparsity discipline. The wp derivation for I3 (specifically the chain I3-VP → S8a) carries through identically at S = 2 as it does at S = 1: it routes through S8a, S8-depth, and TumblerAdd's action-point identity, all of which are subspace-agnostic. ∎
...
*Derivation of (a).* We have reach(σ) = s ⊕ ℓ and reach(σ') = shift(s, n) ⊕ ℓ (SpanReach, ASN-0053). Expanding: reach(σ') = (s ⊕ δₙ) ⊕ ℓ. Both δₙ and ℓ have action point m, and m ≤ #s = m, m ≤ #δₙ = m, so TA-assoc (ASN-0034) applies: (s ⊕ δₙ) ⊕ ℓ = s ⊕ (δₙ ⊕ ℓ). By TumblerAdd, δₙ ⊕ ℓ = [0, …, 0, n + ℓₘ] since both operands have all zeros before position m; similarly ℓ ⊕ δₙ = [0, …, 0, ℓₘ + n]. Tumbler equality requires componentwise agreement at every position; positions 1..m − 1 agree (both zero), and position m requires the natural-number identity `n + ℓₘ = ℓₘ + n`. We derive this identity by a NAT-sub chain (ASN-0034), with no appeal to commutativity of `+`. NAT-sub's left-telescoping `(A m, n ∈ ℕ :: (n + m) − n = m)` instantiated at `(n, m) := (n, ℓₘ)` yields `(n + ℓₘ) − n = ℓₘ`. NAT-sub's right-telescoping `(A m, n ∈ ℕ :: (m + n) − n = m)` instantiated at `(m, n) := (ℓₘ, n)` yields `(ℓₘ + n) − n = ℓₘ`. Hence `(n + ℓₘ) − n = (ℓₘ + n) − n` by transitivity of equality through ℓₘ. NAT-addbound's left-dominance clause `(A m, n ∈ ℕ :: m + n ≥ m)` instantiated at `(m, n) := (n, ℓₘ)` gives `n + ℓₘ ≥ n`, and right-dominance `(A m, n ∈ ℕ :: m + n ≥ n)` instantiated at `(m, n) := (ℓₘ, n)` gives `ℓₘ + n ≥ n`. NAT-sub's right-inverse `(A m, n ∈ ℕ : m ≥ n : (m − n) + n = m)` applied at `(m, n) := (n + ℓₘ, n)` and `(m, n) := (ℓₘ + n, n)` yields `((n + ℓₘ) − n) + n = n + ℓₘ` and `((ℓₘ + n) − n) + n = ℓₘ + n`. Adding `+ n` to both sides of `(n + ℓₘ) − n = (ℓₘ + n) − n` via indiscernibility of equality gives `((n + ℓₘ) − n) + n = ((ℓₘ + n) − n) + n`; substituting through the right-inverses delivers `n + ℓₘ = ℓₘ + n`. The tumbler equality δₙ ⊕ ℓ = ℓ ⊕ δₙ follows componentwise. The reverse TA-assoc application (a = s, b = ℓ, c = δₙ) requires actionPoint(ℓ) ≤ #s and actionPoint(δₙ) ≤ #ℓ; both are m ≤ m. Applying: s ⊕ (ℓ ⊕ δₙ) = (s ⊕ ℓ) ⊕ δₙ = reach(σ) ⊕ δₙ. By S6 (LevelConstraint, ASN-0053), #reach(σ) = #s = m for level-uniform σ, so δₙ = δ(n, m) = δ(n, #reach(σ)) and reach(σ) ⊕ δₙ = shift(reach(σ), n). ∎

*Derivation of (b).* The span σ' = (shift(s, n), ℓ) is level-uniform: #shift(s, n) = m = #ℓ by the result-length identity of TumblerAdd. By D2 (WidthRecovery, ASN-0053), width(σ') = reach(σ') ⊖ start(σ') = (shift(s, n) ⊕ ℓ) ⊖ shift(s, n). We compute this directly from the action-point components of the two endpoints, without invoking (a) or the rewritten form `shift(reach(σ), n)`.

By TumblerAdd at action point m (Pos(ℓ) localises ℓ's nonzero mass at position m, and #shift(s, n) = m): the sum shift(s, n) ⊕ ℓ has positions 1..m−1 copied from shift(s, n) (since ℓᵢ = 0 there), and position m equal to shift(s, n)ₘ + ℓₘ = (sₘ + n) + ℓₘ (using TumblerAdd's action-point identity shift(s, n)ₘ = sₘ + n from the inner δₙ-addition). The two operands shift(s, n) ⊕ ℓ and shift(s, n) therefore agree at positions 1..m−1 and differ at position m by `((sₘ + n) + ℓₘ) − (sₘ + n)`. By NAT-sub's left-telescoping `(A a, b ∈ ℕ :: (a + b) − a = b)` (ASN-0034, NAT-sub, stated with bound variables a, b to avoid clash with the local shift amount n) instantiated at `(a, b) := (sₘ + n, ℓₘ)`, this equals ℓₘ. By TumblerSub at depth m with divergence at position m, the result is [0, …, 0, ℓₘ] = ℓ. ∎
...
**Lemma — PositiveOffsetExceeds** (LEMMA, introduced). For natural numbers `a, b ∈ ℕ` with `a ≥ 1`: `a + b > b` and `b + a > b`. A natural number strictly exceeds any of its strictly positive offsets, regardless of which side the offset is added to.

*Derivation.* We derive each form from foundation NAT axioms (ASN-0034). For the right-summand form `b + a > b`: NAT-addcompat's strict successor clause gives `b < b + 1` directly. NAT-addcompat's order compatibility applied at `(m, n, p) = (b, a, 1)` lifts `a ≥ 1` to `b + a ≥ b + 1`. NAT-order's transitivity composing the weak `b + a ≥ b + 1` with the strict `b + 1 > b` (i.e., `b < b + 1`) yields `b + a > b`. For the left-summand form `a + b > b`, the foundation offers no commutativity-of-`+` axiom on ℕ — its NAT-* axioms are stated as independent left/right pairs (left/right additive identity, additive cancellation, order compatibility, additive dominance), structurally signalling that left/right symmetry must be re-derived rather than assumed. We therefore proceed by a chain that draws on the right-side clauses of NAT-addbound and NAT-cancel directly, with no appeal to commutativity. (i) NAT-addbound's right-dominance clause `(A m, n ∈ ℕ :: m + n ≥ n)` instantiated at `(m, n) := (a, b)` yields the weak inequality `a + b ≥ b`. (ii) NAT-cancel's mirror form `(A m, n ∈ ℕ : n + m = m : n = 0)` instantiated at `(m, n) := (b, a)` yields the implication `a + b = b ⟹ a = 0`. From the hypothesis `a ≥ 1` we extract `a ≠ 0` as follows: NAT-closure provides `0 < 1`; combining with `1 ≤ a` (the `≥`-form of `a ≥ 1`) by NAT-order's transitivity gives `0 < a`; were `a = 0` to hold, substitution would yield `0 < 0`, contradicting NAT-order's irreflexivity, so `a ≠ 0`. Contraposing (ii) against `a ≠ 0` yields `a + b ≠ b`. (iii) NAT-order's `≤`-defining clause `m < n ⟺ m ≤ n ∧ m ≠ n` applied to the conjunction of `b ≤ a + b` (the `≤`-form of (i)) and `a + b ≠ b` (the contrapositive output of (ii)) yields the strict inequality `b < a + b`, i.e., `a + b > b`. Both forms therefore hold. ∎

This lemma packages a recurring step in the contraction proofs below — establishing strict dominance of an offset sum over its summand — into a single citation, replacing repeated ad-hoc chains. We use it at D-SHIFT's well-definedness, D-BJ's order-preservation derivation, D-S's span-level derivation, and the S8a-post wp analysis.
...
The shift is well-defined. For any v ∈ R, ord(v) ≥ ord(r) (since v ≥ r, by OrdinalOrderEquivalence). Since r = p ⊕ w, OrdAddHom (a) gives ord(r) = ord(p) ⊕ w_ord. The subtraction ord(v) ⊖ w_ord is well-defined by TA2 (SubtractionWellDefined, ASN-0034), which requires ord(v) ≥ w_ord. We establish this step by step. At depth #p = 2: ord(v) = [v₂], ord(p) = [p₂], w_ord = [c] with c = w₂ ≥ 1, and ord(r) = ord(p) ⊕ w_ord = [p₂ + c] (TumblerAdd). From v ≥ r and OrdinalOrderEquivalence, ord(v) ≥ ord(r) = [p₂ + c], i.e., v₂ ≥ p₂ + c (T1 at depth 1). Since p ∈ V_1(d), S8a gives p₂ ≥ 1, hence p₂ + c > c by PositiveOffsetExceeds (a = p₂, b = c). NAT-order's transitivity composing the weak `v₂ ≥ p₂ + c` with the strict `p₂ + c > c` yields v₂ > c; we then extract the weak form v₂ ≥ c by NAT-order's `≤`-defining clause `m < n ⟺ m ≤ n ∧ m ≠ n` (ASN-0034, NAT-order) applied to `c < v₂` (the `<`-form of `v₂ > c`), whose first conjunct delivers `c ≤ v₂`, i.e., `v₂ ≥ c`. Equivalently ord(v) = [v₂] ≥ [c] = w_ord (T1 at depth 1). So TA2 applies and `[v₂] ⊖ [c] = [v₂ − c]` is well-defined. The shifted ordinal is positive: the minimum shifted ordinal is ord(r) ⊖ w_ord = ord(p) (verified in D-SEP below). Since p ∈ V_1(d) and S8a's componentwise positivity gives p₂ ≥ 1, ord(p) = [p₂] is positive. The reconstructed V-position σ(v) = vpos(S, ord(v) ⊖ w_ord) then satisfies S8a by vpos's S8a-closure postcondition (ASN-0036): S = 1 ≥ 1 and the ordinal is componentwise positive.
...
We derive `ord(v) ≥ w_ord` step by step for every v ∈ R. At depth #p = 2 we have ord(p) = [p₂], w_ord = [c] with c = w₂ ≥ 1 (since Pos(w) and w₁ = 0 imply w₂ ≥ 1), and ord(r) = ord(p ⊕ w) = ord(p) ⊕ w_ord = [p₂ + c] by OrdAddHom(a). For any v ∈ R, v ≥ r (definition of R), so by OrdinalOrderEquivalence ord(v) ≥ ord(r) = [p₂ + c]. The ordinal comparison `[v₂] ≥ [p₂ + c]` at depth 1 is the natural-number comparison `v₂ ≥ p₂ + c` (by T1 at depth 1, which reduces to component-wise comparison). From p₂ ≥ 1 (S8a's componentwise positivity applied to p ∈ V_1(d) at position 2), PositiveOffsetExceeds (a = p₂, b = c) gives `p₂ + c > c`. Combining with `v₂ ≥ p₂ + c` through NAT-order's transitivity (composing the weak `v₂ ≥ p₂ + c` with the strict `p₂ + c > c`) yields `v₂ > c`; we then extract the weak form `v₂ ≥ c` by NAT-order's `≤`-defining clause `m < n ⟺ m ≤ n ∧ m ≠ n` (ASN-0034, NAT-order) applied to `c < v₂` (the `<`-form of `v₂ > c`), whose first conjunct delivers `c ≤ v₂`, i.e., `v₂ ≥ c`. The depth-1 ordinal comparison `[v₂] ≥ [c]` is exactly this natural-number inequality, so `ord(v) = [v₂] ≥ [c] = w_ord`. ∎(derivation of `ord(v) ≥ w_ord`)
...
2. *`v₂ − c > 0` — strict positivity of the shifted ordinal.* From `v ∈ R` (D-SHIFT's quantifier), OrdinalOrderEquivalence gives `v₂ ≥ p₂ + c` (the depth-1 ordinal comparison equivalent of `v ≥ r`). Combined with `p₂ ≥ 1` (S8a on p, from the precondition `p ∈ V_1(d)`), PositiveOffsetExceeds (a = p₂, b = c) yields `p₂ + c > c`; NAT-order's transitivity then composes `v₂ ≥ p₂ + c` with `p₂ + c > c` into `v₂ > c`, hence `v₂ − c > 0`. The wp clarifies that the `v ∈ R` precondition combined with `p ∈ V_1(d)` (which delivers S8a on p) is doing essential work — without `v ≥ r`, the shifted ordinal could be zero or negative; without `p₂ ≥ 1`, even `v ≥ r` would not suffice (a `p₂ = 0` p with `c = v₂` would still admit `v ≥ r` with `v₂ − c = 0`).
...
4. *`ord(v) ≥ w_ord` — subtraction well-definedness.* Discharged by the same chain as conjunct 2: `v ≥ r` plus S8a on p gives `[v₂] ≥ [c] = w_ord`. The wp surfaces TA2 (SubtractionWellDefined, ASN-0034) as the precise foundation lemma that the assignment relies on; without TA2, `ord(v) ⊖ w_ord` would be undefined and σ(v) would not exist as a tumbler.
...
This is the same recipe as S8a-post (substitute the post-state predicate, push it backwards through the assignments, read each conjunct as a discharged obligation), but the *content* of the obligations is different: S8a-post's wp surfaces TA2 well-definedness and componentwise positivity of `v₂ − c`; S2-post's wp surfaces D-BJ injectivity and D-DP disjointness. The two together cover the structural roles of the foundation properties (S8a, TA2, D-BJ, D-DP, depth-2 subspace preservation) that the contract promises.

The remaining post-state lemmas — S8-depth-post (depth uniformity), D-CTG-post, D-MIN-post, D-SEQ-post (contiguity triple), S8-fin-post (finiteness), S3-post (referential integrity), and S7-post (allocation invariants) — admit wp derivations of the same form and discharge against the closed-form characterization V_1(d') = {[1, k] : 1 ≤ k ≤ N − c} (for the contiguity triple), D-I (for S3-post and S7-post), and the bijectivity-induced cardinality bound |L ∪ Q₃| = |L| + |R| (for S8-fin-post); we have not worked them in detail because the obligations they surface are subsumed by those already exposed for S8a-post and S2-post.
...
*Derivation of (a).* We have reach(σₛ) = s ⊕ ℓ and reach(σ'ₛ) = σ(s) ⊕ ℓ (SpanReach, ASN-0053). At depth 2 with ℓ = [0, c'] (since actionPoint(ℓ) = 2 and Pos(ℓ) force ℓ₁ = 0 and ℓ₂ = c' ≥ 1), OrdAddHom(a) (ASN-0036) applied to s ⊕ ℓ gives ord(reach(σₛ)) = ord(s) ⊕ ℓ_ord = [s₂] ⊕ [c'] = [s₂ + c'] by TumblerAdd. From s ∈ R, ord(s) ≥ w_ord (the same derivation that establishes D-BJ's well-definedness: s₂ ≥ p₂ + c by OrdinalOrderEquivalence on s ≥ r, and p₂ + c > c by PositiveOffsetExceeds with a = p₂, b = c, where p₂ ≥ 1 is S8a on p; NAT-order's transitivity then composes the weak ≥ with the strict > to give s₂ > c, and NAT-order's `≤`-defining clause `m < n ⟺ m ≤ n ∧ m ≠ n` applied to `c < s₂` extracts `c ≤ s₂`, i.e., s₂ ≥ c). NAT-addbound's left-dominance `(A m, n ∈ ℕ :: m + n ≥ m)` (ASN-0034) instantiated at `(m, n) := (s₂, c')` gives `s₂ + c' ≥ s₂`; chaining with the established `s₂ ≥ c` via NAT-order's `≤`-transitivity Consequence (ASN-0034) yields `s₂ + c' ≥ c`. Hence ord(reach(σₛ)) = [s₂ + c'] ≥ [c] = w_ord. Hence σ extended to reach(σₛ) is well-defined by TA2.

Compute σ(reach(σₛ)) = vpos(1, ord(reach(σₛ)) ⊖ w_ord) = vpos(1, [s₂ + c'] ⊖ [c]) = vpos(1, [s₂ + c' − c]) = [1, s₂ + c' − c].

Compute reach(σ'ₛ) = σ(s) ⊕ ℓ. By definition, σ(s) = vpos(1, ord(s) ⊖ w_ord) = [1, s₂ − c] (TumblerSub at depth 1: divergence at position 1, result s₂ − c, well-defined since s₂ ≥ p₂ + c and p₂ + c > c by PositiveOffsetExceeds — chaining via NAT-order's transitivity gives s₂ > c). Then σ(s) ⊕ ℓ = [1, s₂ − c] ⊕ [0, c'] = [1, (s₂ − c) + c'] by TumblerAdd at action point 2.

Tumbler equality of σ(reach(σₛ)) = [1, (s₂ + c') − c] and reach(σ'ₛ) = [1, (s₂ − c) + c'] requires componentwise agreement: position 1 (both equal 1) agrees, and position 2 requires the natural-number identity `(s₂ + c') − c = (s₂ − c) + c'`. We derive this identity by a NAT-sub chain (ASN-0034), with no appeal to commutativity of `+`. NAT-sub's right-inverse `(A m, n ∈ ℕ : m ≥ n : (m − n) + n = m)` applied at `(m, n) := (s₂, c)` (using s₂ ≥ c established above) yields `(s₂ − c) + c = s₂`. Right-addition of c' to both sides (indiscernibility of equality under NAT-addcompat) gives `((s₂ − c) + c) + c' = s₂ + c'`, and NAT-addassoc rewrites the left side as `(s₂ − c) + (c + c') = s₂ + c'`.

We separately derive the specific commutativity instance `c + c' = c' + c` by the same NAT-sub chain pattern established in I3-S(a) (applied at operand pair `(c, c')` in place of `(n, ℓₘ)`). NAT-sub's left-telescoping `(A m, n ∈ ℕ :: (n + m) − n = m)` instantiated at `(n, m) := (c', c)` yields `(c' + c) − c' = c`, and NAT-sub's right-telescoping `(A m, n ∈ ℕ :: (m + n) − n = m)` instantiated at `(m, n) := (c, c')` yields `(c + c') − c' = c`. Hence `(c' + c) − c' = (c + c') − c'` by transitivity of equality through c. NAT-addbound's left-dominance `(A m, n ∈ ℕ :: m + n ≥ m)` at `(m, n) := (c', c)` gives `c' + c ≥ c'`, and right-dominance `(A m, n ∈ ℕ :: m + n ≥ n)` at `(m, n) := (c, c')` gives `c + c' ≥ c'`. NAT-sub's right-inverse applied at `(m, n) := (c' + c, c')` and `(m, n) := (c + c', c')` yields `((c' + c) − c') + c' = c' + c` and `((c + c') − c') + c' = c + c'`. Right-adding `+ c'` to both sides of `(c' + c) − c' = (c + c') − c'` (indiscernibility of equality) and substituting through the right-inverses delivers `c' + c = c + c'`.

Substituting `c + c' = c' + c` into `(s₂ − c) + (c + c') = s₂ + c'` (indiscernibility of equality, applied at the inner +-expression) gives `(s₂ − c) + (c' + c) = s₂ + c'`. NAT-addassoc rewrites the left side as `((s₂ − c) + c') + c = s₂ + c'`. NAT-sub's right-telescoping `(A m, n ∈ ℕ :: (m + n) − n = m)` instantiated at `(m, n) := ((s₂ − c) + c', c)` then yields `(((s₂ − c) + c') + c) − c = (s₂ − c) + c'`; substituting `((s₂ − c) + c') + c = s₂ + c'` on the left side via indiscernibility of equality delivers `(s₂ + c') − c = (s₂ − c) + c'`.

The two tumblers agree componentwise. Hence σ(reach(σₛ)) = reach(σ'ₛ). ✓ ∎

*Derivation of (b).* The span σ'ₛ = (σ(s), ℓ) is level-uniform: #σ(s) = 2 = #ℓ by vpos's result-length identity. By D2 (WidthRecovery, ASN-0053), width(σ'ₛ) = reach(σ'ₛ) ⊖ start(σ'ₛ) = (σ(s) ⊕ ℓ) ⊖ σ(s). We compute this directly from the action-point components, without invoking (a) or the rewritten form `(s₂ + c') − c`.

By TumblerAdd at action point 2 (ℓ = [0, c'] has action point 2, and #σ(s) = 2): σ(s) ⊕ ℓ = [1, s₂ − c] ⊕ [0, c'] has position 1 copied from σ(s) (since ℓ₁ = 0), giving 1, and position 2 equal to (s₂ − c) + c'. Hence σ(s) ⊕ ℓ = [1, (s₂ − c) + c']. The two operands σ(s) ⊕ ℓ and σ(s) = [1, s₂ − c] agree at position 1 (both equal 1) and differ at position 2 by `((s₂ − c) + c') − (s₂ − c)`. By NAT-sub's left-telescoping `(A a, b ∈ ℕ :: (a + b) − a = b)` (ASN-0034, stated with bound variables a, b to avoid clash with the local subtraction operand c) instantiated at `(a, b) := (s₂ − c, c')`, this equals c'. By TumblerSub at depth 2 with divergence at position 2, the result is [0, c'] = ℓ. ✓ ∎
...
| I3-S7 | lemma | S7a, S7b, S7c, S7d preserved post-insertion (and S7 as a corollary) — trivially by I3-C (dom(C') = dom(C), per-address values unchanged) and I3-D (document set unchanged) | derived |
...
| PositiveOffsetExceeds | lemma | For a, b ∈ ℕ with a ≥ 1: a + b > b and b + a > b — strict dominance of an offset sum over its summand | introduced (derived from NAT-addcompat, NAT-addbound, NAT-cancel, NAT-order, NAT-closure, ASN-0034) |
...
| S7-post | lemma | Post-state satisfies S7a, S7b, S7c, S7d (and S7 as a corollary) — trivially by D-I (Σ'.C = Σ.C) and D-CD (other documents unchanged) | introduced |
