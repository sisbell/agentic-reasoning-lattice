...
*Disciplined-domain simplification.* At a layer-reachable state ASN-0086's third conjunct holds vacuously (its unit-depth retraction discipline with R0a forces `a_emit(Σ, d) ∉ coverage(G')` for every pre-existing retraction tuple), so the wp reduces to `K registered ∧ Sh-conf(K, F, G) ∧ d ∈ dom(Σ.M) ∧ (K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))`. For a non-retraction conforming emit (`K ≁ R`) this collapses further to `K registered ∧ Sh-conf(K, F, G) ∧ d ∈ dom(Σ.M)` — the two-conjunct shape gate plus an allocated home, exactly what `K.λ_sh` checks.
...
**P4 (Sh-confWellFormedness).** No `→_sh`-step extends `dom(Σ.L)` with a tuple `(F, G, K)` whose K is unregistered, nor with one for which `Sh-conf(K, F, G)` fails. The only store-of-links step is `K.λ_sh`, whose preconditions are (i) K registered and (ii) `Sh-conf(K, F, G)`: an unregistered K is rejected by (i) before `Sh-conf` is consulted; a registered but non-conforming tuple is rejected by (ii). P4 is the *enablement* half of the gate, and falls out of the weakest-precondition derivation (The shape-gated emit): the wp of the gated emit against `(a, F, G) ∈ A_K^{Σ'}` carries `K registered ∧ Sh-conf(K, F, G)` as its leading conjuncts, so no firing emit escapes the gate. This is a definitional refinement of ASN-0086's relation, not a property of the unmodified `→` — which, having only L3 on `K.λ`, admits both unchecked-type and non-conforming tuples.
