...
The one place this commitment bites against ASN-0086's own vocabulary is retraction. ASN-0086 defines `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})` — an *empty* from-set, with the target carried in G; its RetractionDirectionality convention reserves the from-set "for attribution-bearing endset content or [leaves it] empty for unattributed retractions." Under `|F| = 1` the unattributed branch (`F = ∅`, `|F| = 0`) fails every shape, so ASN-0086's literal `F = ∅` Nullify is **not** expressible under `→_sh`. This is a deliberate exclusion, not an oversight, and Nelson's design intent endorses it: an *unattributed* retraction "is not an intended, essential capability ... everything in the system is part of a document. No free-floating materials exist," and "every retraction legitimately — indeed unavoidably — carries a single attributing source," via the home document and, when made explicit in the endsets, via one from-span. The framework re-expresses retraction as that attributed form: `Emit_R(Σ, d_retr, [r], {(a, δ(1, #a))})` where `[r]` is the single attributing source span (`|F| = 1`) and the single target stays in G as one unit-depth to-span (`|G| = 1`). This shape — `|F| = 1`, `|G| = 1` — is exactly **Binary**, and R is registered Binary. The single-target form is not a narrowing we impose for convenience; it is what retraction *is*. Nelson confirms retraction "was designed as a single-target (single-span) operation per invocation": his DELETEVSPAN command takes "the given span" (singular), in pointed contrast to COPY, MAKELINK, and REARRANGE, which he explicitly generalized to sets/regions; multi-region retraction falls to the front-end / request-set layer, not to the retraction primitive. Gregory confirms the implementation is hard-wired to a single `(stream, width)` span at every layer — protocol parse (`getspan`, not `getspanset`), FEBE handler (one stack-allocated `typespan`), core dispatch, and POOM execution (`deletevspanpm` reads `->stream`/`->width` once, no loop over `->next`) — with no mechanism at any layer to carry multiple targets in one tuple. To retract a discontiguous region one issues several separate retraction emits. Registering R Binary therefore matches both authorities, and the attributed retraction form is Binary-conformant (`|F| = 1`, `|G| = 1`). Binary registration alone, however, does **not** entail ASN-0086's UnitDepthRetractionDiscipline, and we are careful not to overclaim that it does. Binary requires only that G carry a single *span*; a single span `(t, ℓ)` of non-unit length — say `ℓ = δ(2, #t)`, covering a contiguous multi-address range — is equally Binary-conformant. Binary is thus strictly weaker than the discipline's unit-depth requirement. The unit-depth property — that every `L_R` tuple carries a to-span of the canonical form `{(a, δ(1, #a))}` — is supplied not by the shape registration but by the retraction *operation's construction*, which the framework inherits from ASN-0086 as a layer/operational commitment: the re-expressed retraction emit `Emit_R(Σ, d_retr, [r], {(a, δ(1, #a))})` writes a unit-depth to-span by construction. R-Scope's single-tuple-scope result `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}` depends on that operational unit-depth — it needs `coverage(G) = {t : a ≼ t}`, which a non-unit Binary G would exceed — so it is preserved by the construction, not by the registration. Binary registration is therefore *compatible* with the discipline (it admits the unit-depth form) but does not by itself enforce it; the discipline remains exactly where ASN-0086 placed it, at the operation that constructs the retraction emit. Because the target remains in G as that single span, ASN-0086's `nullified`/`L_R`/active-subset machinery, all of which read `coverage(G')`, carry over unchanged; only the from-slot of the emit moves from `∅` to a one-span attribution. (One contiguous span may still cover a whole subtree, so a single Binary retraction can withdraw a large *contiguous* region; it is only *discontiguous* multi-target retraction that the primitive declines and the front end composes.)

Two qualifications bound this expressibility claim. First, it is conditional on R being *registered* at all: under `K.λ_sh` every emit — the re-expressed retraction included — requires K registered (precondition (i) below). Whether the substrate ships R as a standard registration or leaves each app to register its own retraction type is exactly the open standardization question (Open questions #4); Nelson names no retraction type and treats his standard set as "tentative" and "provisional," with type definition deliberately left "open-ended [so that] any user may define his or her link types." So we claim only that *when R is registered Binary*, the attributed form is conformant and emittable under `→_sh`; R's registration itself is deferred. Second, the framework rejects the substrate's raw `F = ∅` unattributed Nullify — and that exclusion is precisely what Nelson's "no free-floating materials" rule already forbids. With those two qualifications, the single-source commitment rejects nothing the substrate is *legitimately* asked to express whose type is registered.
...
One edge follows from counting spans rather than coverage, and we state it explicitly rather than leave it implicit. Types are keyed by *coverage class* (Registration entries) — a coverage-invariant notion — but F-conformance counts spans, a coverage-variant one. So a source presenting one contiguous extent as two abutting spans `(a, ℓ₁)`, `(a ⊕ ℓ₁, ℓ₂)` has `|F| = 2` and fails every shape, even though its coverage equals that of the conformant one-span F over the same extent. We resolve this on the side of the literal emission: **single-source means a single span as emitted**, and coalescing abutting spans to that canonical one-span form is the app's responsibility before emit, not the substrate's at the gate. This is forced by implementation ground truth — Gregory confirms udanax-green performs *no* endset coalescing: spans are stored per-emit (`spanf1.c`'s `insertspanf`) and returned per-sporgl with the consolidation hook commented out (`orglinks.c`'s `permute`), so a coverage-contiguous source is represented as one span only when the client supplies it as one. Nelson's span/span-set distinction agrees that contiguous coverage *has* a canonical single-span form — span-sets are reserved for genuinely *discontiguous* selection ("a separated series of items") — so the app coalescing to that canonical form is a normalization the design already expects, merely one the substrate does not perform automatically. The framework therefore admits the canonical one-span F and rejects the un-coalesced multi-span F of equal coverage; that rejection is a representation requirement on the app, not a coverage judgement. (Genuinely discontiguous multi-span F — a gap between spans, hence a *different* coverage — is rejected on its own merits as the multi-source case the substrate does not provide; see Single-source.) Counting spans-as-emitted keeps the measure intrinsic to the value and thus state-independent (P5).
...
where `K.λ_sh` is `K.λ` with three added preconditions: (0) *the emitted value is a standard triple* — `|value| = 3`, so it carries exactly the two content slots `(F, G)` that `Sh-conf` reads; (i) *K is registered* — the registry records a shape for K; and (ii) `Sh-conf(K, F, G)`. The three are ordered: (0) and (i) jointly discharge the domain condition for (ii). Precondition (0) is the gate that makes (ii) well-defined: `K.λ` inherited from ASN-0086 admits any arity `N ≥ 3` (its sole arity constraint is L3), but `Sh-conf(K, F, G)` is defined over a standard triple `(F, G, K)` and reads exactly two content slots, so for a higher-arity value `(e₁, e₂, e₃, e₄, K)` it has no defined reading. Rather than give `Sh-conf` an ad-hoc reading on such values, we restrict `→_sh` to arity-3 emits: a value with `|value| ≠ 3` fails (0) and is simply not a `→_sh`-step. This is not a refusal by the substrate — an app needing higher arity reaches the link store directly (Single-source), outside `→_sh`. An emit of value `(F, G, K)` is thus a `→_sh`-step only when it is a standard triple, K names a registered type, and the tuple conforms to K's shape. K.σ and K.α are unchanged.

**Weakest precondition of the shape-gated emit.** The refinement lives in the emit's *precondition*, so the proper depth artifact is the weakest precondition of the gated emit against a non-trivial postcondition — not the assertion that it deposits no non-conforming tuple (that is P4, a corollary). We take the postcondition `(a, F, G) ∈ A_K^{Σ'}` — the emitted tuple lands in the *active* subset of its type at the post-state — and reason backward, refining ASN-0086's Case-2 wp.
...
This weakest precondition *coincides with `K.λ_sh`'s own precondition*. `K.λ_sh`'s precondition is `K.λ`'s precondition — which, for landing in the active subset, ASN-0086 distilled to exactly the three inherited conjuncts — conjoined with (i) and (ii). The added guards introduce no slack between "enabled" and "achieves the postcondition": every legal `→_sh` emit attains active-subset membership of its tuple, and the gate rejects exactly the calls whose tuples are unregistered/non-conforming or, by the inherited conjuncts, would fail to land active. This is the analytic content behind P4's "by construction" claim — P4 records the *Sh-conf* half of the gate; the wp records what the whole gate buys against a real postcondition.

*Disciplined-domain simplification (conditional).* One might hope to drop ASN-0086's third conjunct, but the projection `π` (below) establishes only that every `→_sh`-reachable state projects to a `→*`-reachable ASN-0086 state — **not** to a *layer-reachable* one. Layer-reachability (ASN-0086) is strictly stronger than `→*`-reachability: it additionally requires every `→`-step that grows `L_R` to be a `Nullify` carrying a unit-depth to-span. As established in Single-source, `→_sh`'s gate enforces only Binary conformance on R, which is weaker than unit-depth, so `→_sh`-reachable states are *not* known to be layer-reachable and we cannot import the vacuity of the third conjunct at a general `→_sh`-reachable state. We therefore state the simplification strictly conditionally. *If* the substrate is additionally operated under ASN-0086's retraction discipline — so that its reachable states are layer-reachable — then the third conjunct holds vacuously (the unit-depth discipline with R0a forces `a_emit(Σ, d) ∉ coverage(G')` for every pre-existing retraction tuple), and the wp reduces to `K registered ∧ Sh-conf(K, F, G) ∧ d ∈ dom(Σ.M) ∧ (K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))`, collapsing for a non-retraction conforming emit (`K ≁ R`) to `K registered ∧ Sh-conf(K, F, G) ∧ d ∈ dom(Σ.M)` — the two-conjunct shape gate plus an allocated home. This characterizes what `K.λ_sh` checks only on the layer-disciplined sub-domain; at a general `→_sh`-reachable state the full inherited wp of the preceding paragraph — all three ASN-0086 conjuncts intact — stands.

This note's state carries a fourth component, so `→_sh` is not literally a subrelation of ASN-0086's three-component `→` — the two relate different state types, and "every `→_sh*`-reachable state is `→*`-reachable" cannot be read as a set inclusion between four-tuples and three-tuples. We make the bridge explicit through the projection `π(Σ) = (Σ.C, Σ.M, Σ.L)` that forgets the registry. Each `→_sh`-step preserves the registry in its frame (Registry permanence) and acts on the C/M/L components exactly as the corresponding ASN-0086 step: a K.σ-step as `K.σ`, a K.α-step as `K.α`, and a `K.λ_sh`-step as a `K.λ` step — its two added preconditions only *restrict* when it fires, leaving its C/M/L effect and frame identical to `K.λ`'s. Hence whenever `Σ →_sh Σ'`, we have `π(Σ) → π(Σ')` in ASN-0086's relation. By induction on derivation length, `π` maps every `→_sh*`-reachable state to a `→*`-reachable state: the base `π(Σ_init)` is `→*`-reachable from itself, and each step extends a `→`-derivation by the projected step just exhibited. ASN-0086's structural lemmas — R0 (fresh-address emission), `a_emit` totality, L-ContiguousPrefix, PrefixSpanCoverage — are quantified over `→*`-reachable three-component states, so they hold at `π(Σ)` for every state Σ this note reasons about; since they constrain only the C/M/L components, which Σ and `π(Σ)` share, their conclusions transfer to Σ directly. All reachability in this note is with respect to `→_sh`, and this projection is what licenses importing those `→`-domain results. Under this definition P4 holds *by construction* of `K.λ_sh`, not as a derived property of the unmodified ASN-0086 relation.
...
**P4 (Sh-confWellFormedness).** No `→_sh`-step extends `dom(Σ.L)` with a tuple `(F, G, K)` whose K is unregistered, nor with one for which `Sh-conf(K, F, G)` fails. The only store-of-links step is `K.λ_sh`, whose preconditions are (0) `|value| = 3`, (i) K registered, and (ii) `Sh-conf(K, F, G)`: a non-triple value fails (0) and is not a `→_sh`-step at all, so the tuples P4 quantifies over are exactly the standard triples on which `Sh-conf` is well-defined; among those, an unregistered K is rejected by (i) before `Sh-conf` is consulted, and a registered but non-conforming tuple is rejected by (ii). P4 is the *enablement* half of the gate, and falls out of the weakest-precondition derivation (The shape-gated emit): the wp of the gated emit against `(a, F, G) ∈ A_K^{Σ'}` carries `K registered ∧ Sh-conf(K, F, G)` as its leading conjuncts, so no firing emit escapes the gate. This is a definitional refinement of ASN-0086's relation, not a property of the unmodified `→` — which, having only L3 on `K.λ`, admits both unchecked-type and non-conforming tuples.
