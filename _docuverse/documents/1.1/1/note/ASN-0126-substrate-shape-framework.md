...
The *operation set* an app invokes refines ASN-0086's `{Emit_K, Observe_K, Nullify}` in step with the transition relation. `Emit_K` and `Observe_K` carry over unchanged; the empty-from `Nullify`, excluded above, is *superseded* by the attributed-Binary wrapper `Nullify_Binary` — itself an instance of `Emit_K` that an app routes retraction through. The set an app actually invokes is therefore `{Emit_K, Observe_K, Nullify_Binary}`.
