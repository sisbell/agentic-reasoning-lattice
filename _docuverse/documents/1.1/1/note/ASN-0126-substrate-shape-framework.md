...
The registry records, for each type an app declares, the shape its tuples must take. It is a partial function `T_admissible/~ ⇀ shape`: a key is a coverage class `[K_j]`, stored concretely as a representative `K_j ∈ T_admissible` against which CoverageEqualityDecidable (ASN-0086) settles membership; the value is the type's **shape** — one of `Unary`, `Binary`, `Multi` (Three shapes by G span count). Keying by coverage class rather than raw endset honours ASN-0086's TypeEquivalence (lifting L8, ASN-0043), which identifies type endsets by coverage — `K ~ K' ≡ coverage(K) = coverage(K')` — and treats the type subscript as a coverage-class index, so `L_K = L_{K'}` whenever `K ~ K'`.

The registry value is the shape alone — *not* a type name. A type's identity is purely positional: Nelson makes the type an address the search mechanism matches blindly, "completely arbitrary" in what it points to, and a type may even be a *ghost* with nothing stored under it [LM 4/44–4/45]. A capability that read a type's name back could not survive such a ghost, so none is part of the model — the substrate neither carries nor interprets a type's name. Whatever human-readable label a type bears ("citation," "comment," "counterpart") is an app-side convention over addresses, retrieved — if stored at all — by the ordinary content path, never by a substrate name-lookup. We therefore key the registry by coverage class and record only the shape, leaving naming to the layer above. The permanence guarantees below (P1 registry-invariance, P2 shape-stability) concern that shape, which — unlike a name — *is* observable, through emit success or failure under the gate.
...
*Existence-of-successor results are excluded.* An ASN-0086 result whose conclusion has the form `∃ Σ' : Σ → Σ' ∧ …` does not transfer via B2: transferring such a conclusion through `π` yields a `→`-successor of `π(Σ)`, whose witnessing `K.λ` step need not satisfy (0)/(i)/(ii) and so need not be a `K.λ_sh` step — a `→`-successor of `π(Σ)` is not automatically a `→_sh`-successor of Σ. The one such result this note needs, R0 (TupleAddressFreshness), is therefore obtained not through B2 but by lifting (P5), which re-derives a gated emission by applying `Emit_K` at `π(Σ)` and manually lifting its `K.λ` step to `K.λ_sh`.
