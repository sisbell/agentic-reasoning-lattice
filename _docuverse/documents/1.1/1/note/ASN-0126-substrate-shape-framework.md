...
An app *declares* a type by placing its `[K_j] ↦ shape` entry in `Σ_init.registry` before the dynamics begin — the construction of `Σ_init` is the only act that writes the registry. Two distinct objects carry the dynamics, kept apart exactly as in ASN-0086. The *operation set* — the methods an app invokes against the link store — *refines* ASN-0086's `{Emit_K, Observe_K, Nullify}`. `Emit_K` and `Observe_K` carry over unchanged; ASN-0086's empty-from `Nullify`, however, has no `→_sh` image — its `|F| = 0` source fails the gate (The shape-gated emit) — and is *superseded* by the attributed-Binary wrapper `Nullify_Binary` (defined in Retraction as an attributed Binary), itself an instance of `Emit_K` that an app routes retraction through. The set an app actually invokes is therefore `{Emit_K, Observe_K, Nullify_Binary}`. The *transition relation* is *refined*: ASN-0086's `→ ≡ K.σ ∪ K.α ∪ K.λ` becomes `→_sh ≡ K.σ ∪ K.α ∪ K.λ_sh` (The shape-gated emit), the shape-gated step `K.λ_sh` replacing `K.λ`. Neither these operations nor any of `→_sh`'s three steps writes the registry; though carried as a component of `Σ`, it is therefore a fixed *input* that no reachable state revises (P1, Registry permanence).
