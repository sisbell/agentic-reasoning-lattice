...
Every typed relation the framework gates has a single-span source: `|F| = 1`, where for an endset `e` we write `|e|` for its *span count* — since `Endset = 𝒫_fin(Span)` (ASN-0043), `|e|` is the cardinality of `e` as a finite set of spans. The one F span may itself cover a contiguous range or a whole subtree, not merely one address. In particular `F = ∅` has `|F| = 0`: the rule admits no empty-from source. At the other extreme `|F| ≥ 2` is excluded too: the gate measures *span count*, not coverage, so it excludes *every* multi-span source. A single span covers a *contiguous* T1-interval `{t : s ≤ t < s ⊕ ℓ}` (by the `coverage` definition, ASN-0043, and the T1 ordering, ASN-0034), so a source spanning *disjoint* passages — three scattered sentences jointly citing one target, say — needs `|F| ≥ 2`; but a source presenting one *contiguous* extent as two abutting spans is `|F| = 2` and excluded just the same, though its coverage equals that of a conformant one-span F. A multi-span source is deferred to Open Question 6.
...
An app *declares* a type by placing its `[K_j] ↦ shape` entry in `Σ_init.registry` before the dynamics begin — the construction of `Σ_init` is the only act that writes the registry, and no operation revises it thereafter, so the registry is a fixed *input* carried in `Σ` that no reachable state revises (P1, Registry permanence).

A registry is well-formed when shape values lie in `{Unary, Binary, Multi}` and *coverage-class keys are unique*: no two entries have `~`-equal keys.
...
ASN-0086's `K.λ` step has precondition L3 only (arity ≥ 3, non-empty type slot); it does not inspect span counts, so the bare `→` of ASN-0086 *admits* shape-non-conforming tuples. This framework **refines** the emit step, and with it the *transition relation*: ASN-0086's `→ ≡ K.σ ∪ K.α ∪ K.λ` becomes

`→_sh ≡ K.σ ∪ K.α ∪ K.λ_sh`,

the shape-gated step `K.λ_sh` replacing `K.λ`. The step `K.λ_sh` is `K.λ` with three added preconditions: (0) *the emitted value is a standard triple* (StandardTriple, ASN-0043) — arity 3; (i) *K is registered*; and (ii) `Sh-conf(K, F, G)`. The first two make the third well-defined: (0) fixes `F = e₁` and `G = e₂` as the value's only two content slots — the pair `Sh-conf` reads — and (i) supplies `shape(K)` (defined for registered K, The registry), so `Sh-conf(K, F, G)` (partial on unregistered K, Shape-conformance) is well-defined wherever (ii) is reached. The refinement is confined to the guard: `K.λ_sh` adds only preconditions to `K.λ`, so its C/M/L effect, its fresh emission address `a_emit(Σ, d)`, and its C/M/L frame are identical to `K.λ`'s — added preconditions restrict *when* a step fires, not *what* it does. Call this **effect-identity**.
...
Consider five registered coverage classes. The registry records only their shapes; the readable labels `approved`, `succession`, `citation`, `touched`, `retract` are the app's own (The registry):
