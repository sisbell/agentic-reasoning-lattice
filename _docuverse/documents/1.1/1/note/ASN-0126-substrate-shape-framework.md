...
The *operation set* an app invokes refines ASN-0086's `{Emit_K, Observe_K, Nullify}` in step with the transition relation. The two operations carry over asymmetrically. `Observe_K`, a pure read, carries over genuinely unchanged: it inspects state without extending `dom(Σ.L)`, so the emit refinement does not touch it. `Emit_K` carries over but is now *gated* — it realizes a `K.λ_sh` step rather than the ungated `K.λ` (The shape-gated emit), so its successful invocation is strictly tightened, requiring K registered and `Sh-conf(K, F, G)`. The empty-from `Nullify`, excluded above (The shape-gated emit), is *superseded* by the attributed-Binary wrapper `Nullify_Binary` defined here — itself a gated `Emit_R` that an app routes retraction through. The set an app actually invokes is therefore `{Emit_K, Observe_K, Nullify_Binary}`.
