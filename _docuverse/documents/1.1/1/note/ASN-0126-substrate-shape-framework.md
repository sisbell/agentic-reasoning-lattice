...
So the framework supplies the from-slot rather than leaving it empty or open. The retraction wrapper is `Emit_R(Σ, d_retr, [r], {(a, δ(1, #a))})`: a one-span source (`|F| = 1`) and the single target as one unit-depth to-span in G (`|G| = 1`). The from-slot carries no fixed semantic role. So when the app has a case-specific source span it supplies that; for the case ASN-0086 wrote as `F = ∅`, the canonical fill is the home document's own unit-depth span, `r = (d_retr, δ(1, #d_retr))` — a legitimate case-specific use of the slot that designates the party the retraction issues from, not a derivation assertion. The from-slot is thus never an unbound parameter: it is either the app's case span or this canonical home-document span. This shape — `|F| = 1`, `|G| = 1` — is exactly **Binary**, so R is registered Binary. Because the target remains in G as that single span, ASN-0086's `nullified`/`L_R`/active-subset machinery, all of which read `coverage(G')` and ignore F, carry over unchanged; what changes is only the from-slot, which moves from the inexpressible `∅` to a one-span attribution — the app's source, or canonically the home document.
...
The first two conjuncts are this note's contribution; the remaining three are inherited verbatim. The conjunction is read left-to-right under the domain-discharge ordering: `K registered` precedes and licenses `Sh-conf`.
