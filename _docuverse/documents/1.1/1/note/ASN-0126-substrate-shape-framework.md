...
Among ASN-0086's *named* operations, only Nullify hardcodes `F = ∅`; but the `|F| = 1` rule is more general — it excludes *every* empty-from emit ASN-0086 admits. `Emit_K` is total over `Endset × Endset` and `∅ ∈ Endset`, so `Emit_K(Σ, d, ∅, G)` is a legitimate ASN-0086 invocation, yet — carrying no F span at all — it falls outside the shape-gated transition `→_sh` we define below (The shape-gated emit): it has **no** `→_sh` image. ASN-0086's `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})` is one such empty-from emit, so by the general claim it too has no `→_sh` image. Retraction must therefore be re-expressed as a single-source emit before the framework can gate it; we supply that re-expression once `→_sh` and the projection bridge are in hand (Retraction as an attributed Binary).
...
Registry permanence introduced the forgetful projection `π`. We now show it is a *bridge*: it carries this framework's gated dynamics onto ASN-0086's ungated dynamics, so that every ASN-0086 result holds, suitably projected, here. The bridge is load-bearing — Retraction as an attributed Binary, the weakest precondition of the gated emit, P5, and P6 all rest on it — so we establish it once, as a lemma, and cite its two consequences thereafter rather than re-deriving them at each site.
...
The bridge has two consequences, cited throughout the sequel.
...
R-Scope (ASN-0086) is proven for the empty-from Nullify over ASN-0086's three-component `→*`-reachable states, not for this Binary wrapper; its conclusion `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}` nonetheless transfers verbatim to our four-component `→_sh*`-reachable Σ. By the projection bridge, R-Scope holds at the `→*`-reachable `π(Σ)` (B2), and — constraining only the post-state link-address set `A_rel^{Σ'}` and the fixed target subtree `{t : a ≼ t}` — its conclusion transfers to Σ (B1, `A_rel^{π(Σ')} = A_rel^{Σ'}`). A frame argument then carries it from Nullify to the wrapper: the wrapper and the Nullify it re-expresses both call `a_emit` on the *same* `(Σ, d_retr)`, and `a_emit` is blind to F (B1), so the two emit at the *identical* fresh address and yield the *same* post-state link domain `dom(Σ'.L)` — hence the same `A_rel^{Σ'}` — differing only in the value stored at that address (`F = ∅` versus the wrapper's `F = {r}`). R-Scope's conclusion reads only that address set and the fixed target subtree, neither of which sees the stored F, so the equation it proves for Nullify's post-state holds unchanged at the wrapper's.
