...
*Multi-block (p ≥ 2) attainment — open.* This ASN does not exhibit an attainment witness for p ≥ 2; only the p = 1 case above is realised. SV11's structural biconditional ("attainment ⇔ every (j, k) yields a non-empty term and within each block the m terms are pairwise non-adjacent and non-overlapping") is established at any (m, p) by the (⇒)/(⇐) proof above and remains valid; what is left open is the *existence* of a concrete (B, e) configuration realising the biconditional at p ≥ 2. The constraint is topological: a span whose denotation crosses one or more block boundaries deposits non-empty terms in each block it intersects, so arranging m such spans against p blocks to satisfy the non-adjacency-and-non-overlap condition in every block simultaneously requires controlling cross-block-spanning coverage intervals against the block extents — a joint condition on (B, e) that this ASN does not characterise. The m · p = 2 case above is therefore the SV11 attainment claim's witnessed scope; the multi-block tight bound is recorded as an open question against this ASN.
