...
This observation is definitional — locate is defined as `{v ∈ dom(M(d)) : M(d)(v) ∈ coverage(e)}`, so the input dependency is built into the function itself. The substantive claim worth stating is about *what the algebra forbids*: no state component external to (coverage(e), M(d)) participates in resolution, because no such component exists in the link store. This is an *architectural remark* (NoStaleResolutionState), not a derivation — a reading of the foundation schema rather than a transition-induced property, and so not assigned an SV label.

*Schema closure.* Three foundation facts conspire to preclude stale V-position state. (i) *Link-store signature [L3, ASN-0043; K.λ, ASN-0047].* The link value Σ.L(a) = (F, G, Θ) stores I-space content only — endsets are sets of spans (s, ℓ) over T (L3, TripleEndsetStructure); no V-address, no per-document arrangement, no creation-time snapshot is recorded in the link value at allocation (K.λ). (ii) *State-schema closure [Σ = (C, L, E, M, R), ASN-0047].* M(d) is the *current* arrangement; no component carries a historical M_k. R holds per-mapping provenance over I-addresses only (J0/J1/J1★, ASN-0047), not over V-addresses. The schema admits no auxiliary V-cache field. (iii) *Operational closure [K = {K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.μ~, K.ρ}, ASN-0047].* No elementary transition writes a V-address into Σ.L, into a link value, or into any historical-M slot — direct inspection of each transition's effect (ASN-0047) confirms this; there is consequently no operation that *could* establish a stale V-position field.

A naive implementation might intend to cache V-positions at link creation time, leaving stale entries when the document is rearranged. The remark records that this is *structurally precluded* by (i)–(iii): there is no field in which a stale V-position could persist, and no transition that could populate one. The corresponding functional fact about locate is the immediate definitional reading — `Σ₁.M(d) = Σ₂.M(d) ⇒ locate_{Σ₁}(e, d) = locate_{Σ₂}(e, d)` for any endset e, since `locate(e, d) = {v ∈ dom(M(d)) : M(d)(v) ∈ coverage(e)}` references only M(d) and the supplied endset. No L-equality precondition is needed; adding one would carry no force, since locate does not consult Σ.L. The architectural content above is what guarantees this functional triviality is not undermined by some schema field overlooked.
...
| SV11 | PartialSurvivalDecomposition: the text-subspace projection equals the union of *exactly* m·p decomposition terms (one per span-block pair, some possibly empty), equivalently the union of *at most* m·p maximal ordinal-contiguous fragments (strict when terms coalesce within a block); the collection is a cover, not necessarily a partition, under non-injective arrangements | introduced |
