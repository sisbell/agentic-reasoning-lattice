...
`π_{Σ'}(e, d) = coverage(Σ'.L.e) ∩ ran(Σ'.M(d)) = coverage(Σ.L.e) ∩ ran(Σ.M(d)) = π_Σ(e, d)`

The first and third equalities are the definition of π (π(e, d) = coverage(e) ∩ ran(M(d)), unfolded with the state-dependent endset and arrangement readings made explicit). The middle equality has two factors. *Coverage invariance:* K.μ~ holds L in frame (it is a composite of K.μ⁻ + K.μ⁺, both of which hold L in frame per ASN-0047), so L12 (LinkImmutability, ASN-0043) gives Σ'.L = Σ.L and therefore coverage(Σ'.L.e) = coverage(Σ.L.e). *Range invariance:* K.μ~'s ran-preservation corollary (ASN-0047) records that K.μ~ preserves the multiset of I-addresses in M(d) — ran(M'(d)) = ran(M(d)) — because K.μ~ is a bijection on V-positions that holds the V↦I assignment's image fixed (each I-address removed by the K.μ⁻ stage is restored by the K.μ⁺ stage at its new V-position). Together: both arguments to the set intersection are identical at Σ and Σ', so the intersections are equal. ∎
