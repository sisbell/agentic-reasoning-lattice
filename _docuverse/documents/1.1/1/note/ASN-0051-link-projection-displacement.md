...
*Proof of the π-invariance claim.* The π-equality unfolds directly from the definition of π and K.μ~'s effect on its two inputs:

`π_{Σ'}(e, d) = coverage(e) ∩ ran(Σ'.M(d)) = coverage(e) ∩ ran(Σ.M(d)) = π_Σ(e, d)`

The first and third equalities are the definition of π (π(e, d) = coverage(e) ∩ ran(M(d))). Here e is the endset value being projected (a set of spans, bound at link creation); coverage(e) depends only on e and is state-independent in this proof — L12 (LinkImmutability, ASN-0043) holds the slot value e fixed across every transition for any e drawn from a link in dom(L), so the symbol e refers to the same endset value at Σ and Σ' and we do not re-tag it with a state subscript. The only state-dependent input to π is therefore ran(M(d)). The middle equality is range invariance alone: K.μ~'s ran-preservation corollary (ASN-0047) records that K.μ~ preserves the multiset of I-addresses in M(d) — ran(M'(d)) = ran(M(d)) — because K.μ~ is a bijection on V-positions that holds the V↦I assignment's image fixed (each I-address removed by the K.μ⁻ stage is restored by the K.μ⁺ stage at its new V-position). Both arguments to the set intersection therefore coincide at Σ and Σ', so the intersections are equal. ∎
...
**SV6 (CrossOriginExclusion).** For a span (s, ℓ) in an existing endset where s is element-level (zeros(s) = 3), the span's action point lies strictly within the element field (k > p₃, where k = actionPoint(ℓ) and p₃ is the position of the third zero component in s), and a newly allocated address b with zeros(b) = 3 and origin(b) ≠ origin(s):
...
*Sub-claim (i) — early-divergence shape.* Suppose the first position where tⱼ ≠ sⱼ is some j with j < k. Then #t ≥ j and tⱼ > sⱼ.

*Proof of (i).* For #t ≥ j: T1(i) requires both t and s to have a component at position j. Span well-formedness (ASN-0034, span precondition `actionPoint(ℓ) ≤ #s`) gives k ≤ #s, so j < k ≤ #s and sⱼ is well-defined. For tⱼ, suppose for contradiction that #t < j. Then t and s agree on every position 1 through #t (because j is the *first* position of divergence and #t < j), and #t < j ≤ #s, so t is a proper prefix of s. By T1(ii) (a proper prefix is strictly less in the lex order), t < s — contradicting s ≤ t. Hence #t ≥ j, so tⱼ is well-defined.

For tⱼ > sⱼ: since t ≥ s and t agrees with s on positions 1 through j−1, T1(i) gives tⱼ > sⱼ. □
...
We now lift the no-early-divergence fact to (a) and (b):

(a) *#t ≥ k.* Suppose #t < k. The no-early-divergence fact excludes any first-divergence at j ≤ #t < k, so t agrees with s on all positions 1 through #t — making t a proper prefix of s, hence t < s by T1(ii), contradicting s ≤ t.

(b) *t agrees with s on positions 1 through k−1.* The no-early-divergence hypothesis presupposes that *some* position of divergence exists, i.e., t ≠ s. We therefore split on whether t = s:
- *t = s.* Trivially, t agrees with s on every position, in particular on positions 1 through k−1.
- *t ≠ s.* Then t has a first position of divergence from s. If that position lay in [1, k−1], it would be a first divergence at some j < k — excluded by the no-early-divergence fact. So the first position of divergence lies at or beyond k, and t agrees with s on positions 1 through k−1.
...
*Attainment witness — p = 1 case; multi-block tight bound left open.* The m · p bound is realised concretely *for p = 1*. Reusing the Worked Example's initial state — `M(d) = {v₁ ↦ a₁, v₂ ↦ a₂, v₃ ↦ a₃, v₄ ↦ a₄, v₅ ↦ a₅}` with five sibling addresses of equal tumbler length sharing one origin, forming a single maximally merged block β = (v₁, a₁, 5) with I-extent I(β) = {a₁, a₂, a₃, a₄, a₅} (so p = 1) — take the two-span endset `e = {(s₁, ℓ₁), (s₂, ℓ₂)}` with s₁ = a₁, ℓ₁ = a₂ ⊖ a₁ (well-defined by D0; reach a₂ by D1 inverse, coverage `⟦(a₁, ℓ₁)⟧ = {t : a₁ ≤ t < a₂}`) and s₂ = a₄, ℓ₂ = a₅ ⊖ a₄ (well-defined by D0; reach a₅, coverage `⟦(a₄, ℓ₂)⟧ = {t : a₄ ≤ t < a₅}`). Then m = 2 and the m · p = 2 decomposition terms are:

- ⟦(s₁, ℓ₁)⟧ ∩ I(β) = {a₁}
- ⟦(s₂, ℓ₂)⟧ ∩ I(β) = {a₄}

Both terms are non-empty, ruling out mechanism (a). Within β their ordinal positions are a₁ (offset 0) and a₄ (offset 3), separated by a₂, a₃ at offsets 1, 2 — non-adjacent (the gap between offsets 0 and 3 has width 2 ≥ 1) and non-overlapping (disjoint singletons), ruling out mechanism (b). The maximal-fragment count equals the non-empty-term count equals m · p = 2; both bounds are saturated simultaneously. ✓ The same construction extended to m spans of the form `(a_{2j-1}, a_{2j} ⊖ a_{2j-1})` over 2m − 1 sibling addresses in a single block witnesses attainment at arbitrary m with p = 1.

*Multi-block (p ≥ 2) attainment — provably non-attained for m ≥ 2.* The m · p bound is *not* attained at any (m, p) with m ≥ 2 and p ≥ 2. We give a structural argument that closes the question rather than leaving it open.

*Disjoint-extent case (the generic injective configuration).* Suppose the p blocks have pairwise-disjoint I-extents. Order them by the T-position of their first ordinal element, so β_{k₁} precedes β_{k₂} in T when β_{k₁}'s first element is < β_{k₂}'s first element. By S0 (Convexity), each span (s_j, ℓ_j) has coverage `⟦(s_j, ℓ_j)⟧ = [s_j, s_j ⊕ ℓ_j)`, a half-open interval in T. For span j to deposit non-empty terms in both β_{k₁} and β_{k₂} — necessary for the (j, k₁) and (j, k₂) decomposition terms to both be non-empty — its coverage interval must intersect both; by S0 convexity the interval then contains the entire T-region between β_{k₁}'s last contributing element and β_{k₂}'s first contributing element. In particular, span j's contribution to β_{k₁} is a *suffix* of β_{k₁}'s ordinal sequence ending at β_{k₁}'s last element, and its contribution to β_{k₂} is a *prefix* of β_{k₂}'s ordinal sequence starting at β_{k₂}'s first element. For *every* span j ∈ {1, ..., m} to contribute non-empty to both blocks (m ≥ 2), the m contributions to β_{k₁} are all suffixes that share β_{k₁}'s last element — hence pairwise overlap at that shared endpoint and coalesce into a single maximal fragment, violating the within-block non-overlap requirement of the attainment biconditional. Attainment fails.

*Non-injective case (overlapping I-extents, S5 sharing).* When two blocks β_{k₁}, β_{k₂} have overlapping I-extents — admitted under S5 (UnrestrictedSharing) and exhibited by the two-span Worked Example below with β₂'s extent {a₂, a₃} ⊆ β₁'s extent {a₁..a₅} — the within-block non-adjacency requirement carries an independent block-size barrier. Within a block β_k with ordinal sequence length n_k, m pairwise non-adjacent singletons need at least 2m − 1 ordinal positions to fit (m occupied positions plus m − 1 gap positions). If `min_k n_k < 2m − 1`, no arrangement of m within-block contributions can satisfy non-adjacency in the smallest block, regardless of the spans. The two-span Worked Example below has n_{β₂} = 2 < 2·2 − 1 = 3, so attainment at (m = 2, p = 2) is already excluded by the block-size constraint alone — independently of the suffix-overlap argument of the disjoint-extent case.

*Conclusion.* The witnessed attainment scope is precisely (m ≥ 1, p = 1); at every (m, p) with m ≥ 2 and p ≥ 2 the m · p bound is structurally loose. This sharpens SV11: the biconditional ("attainment ⇔ every (j, k) yields a non-empty term and within each block the m terms are pairwise non-adjacent and non-overlapping") remains valid as the characterisation of attainment, but the antecedent is jointly satisfiable by some concrete (B, e) only at p = 1.
