...
*Witness.* Extend the SV10 witness. Σ already contains link a with F = {(i₁, ℓ_span)} and coverage(F) ⊇ {i₁, i₂, i₃}; let d₁ = d with Σ.M(d₁) = {v₁ ↦ i₂}, so the SV10 discovery clause supplies A = {i₂} ⊆ ran(Σ.M(d₁)) and a ∈ discover_from(A). Introduce a second document d₂ ≠ d₁ with Σ.M(d₂) = {v₁ ↦ j} where j is any element-level T4-valid I-address satisfying origin(j) ≠ O — admissible by SV6, which gives j ∉ ⟦(i₁, ℓ_span)⟧ since the span's action point lies in the element field (k = 8 > 6 = p₃), so coverage(F) ∩ {j} = ∅. Then π(Σ.L(a).from, d₂) = coverage(F) ∩ ran(Σ.M(d₂)) = coverage(F) ∩ {j} = ∅. ∎
