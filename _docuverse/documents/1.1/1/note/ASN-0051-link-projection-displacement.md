...
(d) *Non-permanence witness — strict shrinkage under contraction.* The inclusion (b) is strict when the contraction removes the last V-position whose I-address is in some link's endset coverage: `(E Σ →_{K.μ⁻} Σ', d, s, a ∈ dom(Σ.L) :: a ∈ discover_through_s(d) in Σ ∧ a ∉ discover_through_s(d) in Σ')`. *Witness.* Take the Worked Example's post-removal state (preceding the "After reordering" subsection): from the pre-removal arrangement `{v₁↦a₁, v₂↦a₂, v₃↦a₃, v₄↦a₄, v₅↦a₅}` the composite K.μ~ + K.μ⁻ removes the V-position carrying a₃, yielding `M'(d) = {v₁↦a₁, v₂↦a₂, v₃↦a₄, v₄↦a₅}`. Consider any link a' with coverage(Σ.L(a').s) = {a₃} (a singleton endset on a₃). Pre-state: coverage ∩ ran(Σ.M(d)) = {a₃} ≠ ∅, so a' ∈ discover_through_s(d) in Σ. Post-state: a₃ ∉ ran(Σ'.M(d)) = {a₁, a₂, a₄, a₅}, so coverage ∩ ran(Σ'.M(d)) = ∅ and a' ∉ discover_through_s(d) in Σ'. The exit is local to d: a' remains in discover_s({a₃}) (the fixed I-address set is unchanged, SV8), and remains in discover_through_s(d'') for any d'' ≠ d whose arrangement still maps a₃. ∎
