...
- *t₁ ≠ 0.* Position 1 lies in [1, k − 1] (since k > p₃ ≥ 6, so k ≥ 7 ≥ 2). (`p₃ ≥ 6` follows from T4-validity: `t₁ ≠ 0` gives `p₁ ≥ 2`, no adjacent zeros gives `p₂ ≥ 4` and `p₃ ≥ 6`.) Conclusion (b) gives t₁ = s₁, and s is T4-valid, so t₁ = s₁ ≠ 0.
