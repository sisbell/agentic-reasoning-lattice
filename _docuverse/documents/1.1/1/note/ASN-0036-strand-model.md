...
*Formal Contract (S3):*
- *Axiom (well-formedness invariant):* In every observable state `Σ`, `(A d, v : v ∈ dom(Σ.M(d)) : Σ.M(d)(v) ∈ dom(Σ.C))` — equivalently, `ran(Σ.M(d)) ⊆ dom(Σ.C)`.
...
- *Postconditions:* (a) `subspace_I(a) = E(a)₁` and the content ordinal `[E(a)₂, ..., E(a)_δ]` occupy distinct components. (b) For any `k ≥ 1`, the displacement `δ(k, #a)` has action point `#a`, which falls strictly after the position of `subspace_I(a)` in the full address — so `shift(a, k)` preserves `subspace_I(a)` by TumblerAdd's prefix rule. (c) The within-subspace ordinal `[E(a)₂, ..., E(a)_δ]` is a non-empty tumbler in `S` — non-empty by S7c's own axiom `#E(a) ≥ 2`, and componentwise positive by T4's positive-component constraint on present fields — satisfying TA7a's operand precondition `o ∈ S` so that `⊕` and `⊖` are directly applicable.
- *Depends:* S7b (element-level I-addresses) — provides `E(a)`; T4b (UniqueParse, ASN-0034) — defines element-field projection; TA7a (ASN-0034) — `⊕` and `⊖` on the within-subspace ordinal require operands in `S`, i.e. non-empty (`#o ≥ 1`) with positive components, which `#E(a) ≥ 2` together with componentwise positivity supplies; T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — supplies the componentwise positivity of `E(a)` (no zero components within the element field, so `E(a)₂, …, E(a)_δ` are all positive) that TA7a's operand precondition `o ∈ S` requires beyond mere non-emptiness; S0 (content immutability) — discharges the persistence step that connects `a ∈ dom(Σ.C)` in the current state to the T10a allocation event under which T10a.4 originally established T4-validity (and hence componentwise positivity of `E(a)`).
...
- *Postconditions:* (a) `subspace_I(a) ∈ ℕ` — the projected component inherits T0's ℕ-valued carrier (ASN-0034). (b) `subspace_I(a) ≥ 1` — T4's positive-component constraint on present fields, refined by T10a.4's componentwise positivity within `E(a)`, delivers `E(a)₁ ≥ 1`. (c) *Subspace preservation under shift:* for `k ≥ 1`, `subspace_I(shift(a, k)) = subspace_I(a)` — supplied by S7c's own postcondition (b) (the action point of `δ(k, #a)` is `#a`, which falls strictly after the position of `subspace_I(a)` precisely because `#E(a) ≥ 2`, so TumblerAdd's prefix rule copies the `subspace_I` component unchanged), with the full derivation given in S8's auxiliary lemma's conclusion (i).
- *Depends:* T0 (ASN-0034) — ℕ-valued component carrier underwriting postcondition (a); T4b (UniqueParse, ASN-0034) — supplies `E(a)` once S7b holds; T4 (HierarchicalParsing, ASN-0034) — positive-component constraint on present fields underwriting postcondition (b); T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — componentwise positivity within the element field, reinforcing postcondition (b); S7b — provides `E(a)` via `zeros(a) = 3`; S7c — provides `#E(a) ≥ 2` and postcondition (b)'s shift-preservation premise underwriting postcondition (c); S8's auxiliary lemma conclusion (i) — supplies the full derivation behind postcondition (c). The function itself depends only on S7b and S7c; the shift-preservation postcondition (c) is a conditional refinement that strengthens the conclusion in the contexts where the lemma's premises hold.
