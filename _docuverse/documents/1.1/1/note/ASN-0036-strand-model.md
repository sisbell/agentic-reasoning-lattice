...
*S0-persistence bridge.* Several premises below connect an address `a` in the *current* state's `dom(Σ.C)` to the T10a allocation event under which T10a.4 (ASN-0034) originally established its T4-validity. S0 (content immutability) discharges this connection once and for all: every `a ∈ dom(Σ.C)` is identity-fixed for all successor states, so any structural property T10a.4 established at allocation persists. We cite this as the *S0-persistence bridge* in the contracts below rather than restating it.
...
- *Postconditions (derived):* `(A v ∈ dom(Σ.M(d)) :: zeros(v) = 0 ∧ #v ≥ 2 ∧ (A i : 1 ≤ i ≤ #v : vᵢ > 0))`.
...
**Existence.** By S8-fin, `dom(M(d))` is finite. By S2 (ArrangementFunctionality), `M(d)` is a function, so each `v ∈ dom(M(d))` has a uniquely determined image `a = M(d)(v)`. By S3 (referential integrity), `a ∈ dom(Σ.C)`. For each such `v`, form the singleton run `(v, a, 1)`. Conjunct (b) requires `M(d)(shift(v, k)) = shift(a, k)` for all `k` with `0 ≤ k < 1` — the only such `k` is `0`, where `shift(v, 0) = v` and `shift(a, 0) = a` by convention, so the identity reduces to `M(d)(v) = a`, which holds by construction. The singleton witness thus exercises `shift` only at `k = 0` (the identity), so neither S7b (`zeros(a) = 3`) nor S7c (`#E(a) ≥ 2`) is invoked here; these structural facts become load-bearing only for coarser decompositions (run lengths `nⱼ ≥ 2`), through ShiftPreservation in the run-corollary below where the shift is exercised at `k ≥ 1`. We state this distinction once; the contract slots below cite their dependencies without re-litigating which `k` exercises them. Since `dom(M(d))` is finite, the collection of singletons is finite.
