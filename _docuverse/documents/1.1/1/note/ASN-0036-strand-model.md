...
**S4 (OriginBasedIdentity).** For I-addresses `a₁`, `a₂` produced by distinct allocation events:
...
This allocation is an event of T10a's discipline (ASN-0034), not an informal act: the document owner baptises element addresses through an *element allocator* `A_element` — a node of the global allocator tree `𝒯`, spawned under the document's own allocator by T10a's child-spawning rule — whose domain holds exactly the element addresses issued under that document's prefix. Consequently every `a ∈ dom(Σ.C)` satisfies `a ∈ dom(A_element) ⊆ 𝒯` for a T10a-conforming element allocator `A_element` — the element-level counterpart of S7d's document-level allocation assertion. This membership is the premise that, in S7's well-definedness step, licenses T10a.4's preservation invariant to be instantiated at `a`.
...
**`succ` stays within a subspace and preserves well-formedness.** Let `v ∈ dom(M(d))`, so `v` satisfies S8a; write `m = #v`, which by S8-depth is the common depth shared by every active position in `v`'s subspace. By OrdShiftHom (a), `subspace(shift(v, 1)) = subspace(v)`, and by OrdShiftHom (b) `shift(v, 1)` again satisfies S8a; its depth is fixed by OrdShiftHom's frame, `#shift(v, 1) = #v = m` (ultimately TA0's `#(a ⊕ w) = #w`) — an unconditional property of `shift` that does not presuppose `shift(v, 1) ∈ dom(M(d))`, so we may invoke it here, before the lockstep link is known to land back in the domain. Thus every lockstep link stays inside one subspace at depth `m`, and `succ` is a partial function on the well-formed V-positions of that subspace. On the image side, when `v` is lockstep-linked we have `shift(v, 1) ∈ dom(M(d))`, so `shift(a, 1) = M(d)(shift(v, 1)) ∈ ran(M(d)) ⊆ dom(Σ.C)` by S3.
...
| S8 | Correspondence-run partition: `dom(M(d))` is the disjoint union of finitely many maximal runs `(vⱼ, aⱼ, nⱼ)` with `M(d)(shift(vⱼ, k)) = shift(aⱼ, k)` for `0 ≤ k < nⱼ`; maximal decomposition is unique | theorem from S2, S3, S8-fin, S8a, S8-depth, OrdShiftHom, T1, TS2, TS3, TS4, OrdinalShift (ASN-0034) |
