...
*Conclusion (i): preserved zero-count.* By T4's field-segment constraint (ASN-0034) applied to `a`, `a_{#a} ≠ 0`, so `a_{#a} ≥ 1` by **Nat-pos** — the elementary fact that for `n ∈ ℕ`, `n ≠ 0 ⟹ n ≥ 1` (immediate from NAT-discrete at `m = 0`), which we name here and cite at later sites. Since `a_{#a} ≥ 1 > 0` and `k ≥ 1 > 0`, NAT-closure gives `a_{#a} + k > 0`. By TumblerAdd's prefix rule, every position `i < #a` of `shift(a, k)` is copied unchanged from `a`, preserving the zero/nonzero status at every such position: the three field-separator zeros of `a` (between `N`, `U`, `D`, and `E`, all at positions `< #a`) remain zero in `shift(a, k)` at the same positions, and every non-separator position `i < #a` (each of which is nonzero in `a` since the three field separators account for all of `zeros(a) = 3` per S7b) remains nonzero in `shift(a, k)`. Combined with `a_{#a} + k > 0` at position `#a` from the chain above, `shift(a, k)` has exactly the three field-separator zeros at the same positions as `a` and no other zeros, so `zeros(shift(a, k)) = zeros(a) = 3` — establishing conclusion (i).
...
*Proof.* A V-position is, by definition, an isolated element field of depth at least 2 (paralleling S7c's element field for I-addresses), so `zeros(v) = 0` and `#v ≥ 2` hold by definition. Componentwise positivity follows: every component of `v` lies in T0's carrier ℕ, and `zeros(v) = 0` forces each to be `≠ 0`, hence `≥ 1` by Nat-pos; in particular the subspace identifier `v₁ ≥ 1`. ∎
*Formal Contract:*
...
- *Depends:* T0 (ASN-0034) — supplies the ℕ-valued component carrier on which `vᵢ ∈ ℕ` for every component; NAT-discrete (NatDiscreteness, ASN-0034) — underwrites Nat-pos (the `n ≠ 0 ⟹ n ≥ 1` fact named at ShiftPreservation), which discharges the positivity step: `vᵢ ≠ 0` (delivered by `zeros(v) = 0`) gives `vᵢ ≥ 1`, hence `(A i : 1 ≤ i ≤ #v : vᵢ > 0)`.
...
- *Postconditions:* (*Existence.*) There exists a finite set of correspondence runs `{(vⱼ, aⱼ, nⱼ)}` satisfying (a) `(A v ∈ dom(M(d)) :: (E! j :: vⱼ ≤ v < shift(vⱼ, nⱼ)))` and (b) `(A j, k : 0 ≤ k < nⱼ : M(d)(shift(vⱼ, k)) = shift(aⱼ, k))`. The proof exhibits the singleton decomposition (every `nⱼ = 1`), for which conjunct (b) reduces to the base case `M(d)(vⱼ) = aⱼ` at `k = 0`. (Structure-preservation of `shift` on images — the fact that coalescing into multi-element runs will require — is not a postcondition of this singleton-existence theorem; it is a standalone consequence of ShiftPreservation and S3, recorded with the deferred coalescing work in the open questions.)
