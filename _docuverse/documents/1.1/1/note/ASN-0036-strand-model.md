...
For `m ≥ 2`, `δ(n, m)` has action point `m > 1`, so TumblerAdd copies component 1 unchanged and the subspace identifier is preserved. This is the canonical minimum position required by D-MIN. Basic INSERT typically commits to `m = 2`. Once any position is placed, S8-depth fixes the depth at the chosen `m` for all subsequent positions in the text subspace — transitioning the document into the non-empty regime governed by the binary `ValidInsertionPosition(d, v)`.
