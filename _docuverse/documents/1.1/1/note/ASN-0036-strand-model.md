...
- *Postconditions:* (*Existence.*) There exists a finite set of correspondence runs `{(vⱼ, aⱼ, nⱼ)}` satisfying (a) `(A v ∈ dom(M(d)) :: (E! j :: vⱼ ≤ v < shift(vⱼ, nⱼ)))` and (b) `(A j, k : 0 ≤ k < nⱼ : M(d)(shift(vⱼ, k)) = shift(aⱼ, k))`. The proof exhibits the singleton decomposition (every `nⱼ = 1`), for which conjunct (b) reduces to the base case `M(d)(vⱼ) = aⱼ` at `k = 0`. (*Corollary (subspace and field-structure preservation across a run), conditional on a hypothesized non-trivial run.*) The existence claim above establishes only the singleton decomposition (`nⱼ = 1`), for which this corollary is vacuous beyond `k = 0`. *Hypothesize* a non-trivial run `(vⱼ, aⱼ, nⱼ)` with `nⱼ > 1` satisfying conjunct (b) — a structure this theorem does not construct (its occurrence is operations-layer-determined; see the Non-canonicality remark). For any such hypothesized run, every image `shift(aⱼ, k)` with `1 ≤ k < nⱼ` preserves the structural properties of `aⱼ` (subspace identifier, element-level zero-count, element-field depth) established by ShiftPreservation, applied pointwise to `aⱼ ∈ dom(Σ.C)` (S3 supplies this membership); at `k = 0` preservation is trivial.
