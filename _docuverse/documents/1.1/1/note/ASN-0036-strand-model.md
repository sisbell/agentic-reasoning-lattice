...
- *Depends:* T4 (HierarchicalParsing, ASN-0034) — defines the prefix structure; T4b (UniqueParse, ASN-0034) — defines projections `N`, `U`, `D` (the projection `D` is partial, well-defined only when `zeros(a) ≥ 2`); S7b (Element-level I-addresses) — supplies `zeros(a) = 3 ≥ 2`, which is the structural premise on which `D(a)` (and hence S7a's statement) is well-defined for every `a ∈ dom(Σ.C)`; T10a (AllocatorDiscipline, ASN-0034) — establishes the baptism principle; T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — supplies the *surrounding* T4-validity (no adjacent zeros, positive endpoint components `a₁ ≠ 0 ∧ a_{#a} ≠ 0`) that T4b's projections require beyond the zero-count condition, so `N(a)`, `U(a)`, `D(a)` are well-defined on every `a ∈ dom(Σ.C)`; S0 (content immutability) — discharges the persistence step that connects `a ∈ dom(Σ.C)` in the current state to the T10a allocation event under which T10a.4 originally established T4-validity (the tumbler is identity-fixed thereafter). (Note on textual order: S7b is stated *after* S7a in this ASN for expository reasons — the axiomatic premise is introduced before the structural premise it implicitly relies on — but logically S7a's statement presupposes S7b. Equivalently, S7a could be conditioned on `zeros(a) ≥ 2`, which S7b then discharges universally.)
...
*Conclusion (ii): preserved zero-count.* By T4's field-segment constraint applied to `aⱼ` (ASN-0034), `aⱼ_{#aⱼ} ≠ 0`, so `aⱼ_{#aⱼ} ∈ ℕ` with `aⱼ_{#aⱼ} ≥ 1` (NAT-zero's disjunction `0 < aⱼ_{#aⱼ} ∨ 0 = aⱼ_{#aⱼ}` combined with `aⱼ_{#aⱼ} ≠ 0` forces `0 < aⱼ_{#aⱼ}`, which NAT-discrete at `(0, aⱼ_{#aⱼ})` promotes to `1 ≤ aⱼ_{#aⱼ}`); combined with `k ≥ 1`, NAT-addcompat (left and right order compatibility) together with NAT-order's ≤-transitivity Consequence yields the chain `1 + 1 ≤ aⱼ_{#aⱼ} + 1 ≤ aⱼ_{#aⱼ} + k`: right order compatibility at `(m, n, p) = (1, aⱼ_{#aⱼ}, 1)` lifts `1 ≤ aⱼ_{#aⱼ}` to `1 + 1 ≤ aⱼ_{#aⱼ} + 1`, then left order compatibility at `(m, n, p) = (aⱼ_{#aⱼ}, k, 1)` lifts `1 ≤ k` to `aⱼ_{#aⱼ} + 1 ≤ aⱼ_{#aⱼ} + k`, and ≤-transitivity chains the two into `aⱼ_{#aⱼ} + k ≥ 1 + 1 > 0`. Hence position `#aⱼ` of `shift(aⱼ, k)` contributes no new zero. The three field-separator zeros of `aⱼ` (between `N`, `U`, `D`, and `E`, sitting at positions `< #aⱼ`) are copied unchanged into `shift(aⱼ, k)` by the prefix rule, and no additional zero is introduced at position `#aⱼ`, so `zeros(shift(aⱼ, k)) = zeros(aⱼ) = 3` — establishing conclusion (ii) and S7b's element-level zero-count for `shift(aⱼ, k)`.
...
*Proof.* Let V_1(d) be non-empty with common depth m ≥ 3 (S8-depth). Suppose for contradiction that V_1(d) contains two positions v₁ and v₂ with v₁ < v₂ (both depth m) whose first point of disagreement is at component j with 2 ≤ j ≤ m − 1 — that is, (v₁)ᵢ = (v₂)ᵢ for all i < j, and (v₁)ⱼ < (v₂)ⱼ (the inequality follows from v₁ < v₂ by T1(i), since j is the first disagreeing component and j ≤ min(m, m)).
