...
- *Postconditions:* (a) `subspace_I(a) ∈ ℕ` — the projected component inherits T0's ℕ-valued carrier (ASN-0034). (b) `subspace_I(a) ≥ 1` — T4's positive-component constraint on present fields, refined by T10a.4's componentwise positivity within `E(a)`, delivers `E(a)₁ ≥ 1`. (c) *Subspace preservation under shift:* for `k ≥ 1`, `subspace_I(shift(a, k)) = subspace_I(a)` — ShiftPreservation conclusion (iv) below.
- *Depends:* T0 (ASN-0034) — ℕ-valued component carrier underwriting postcondition (a); T4b (UniqueParse, ASN-0034) — supplies `E(a)` once S7b holds; T4 (HierarchicalParsing, ASN-0034) — positive-component constraint underwriting postcondition (b); T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — componentwise positivity within the element field, reinforcing postcondition (b); S7b — provides `E(a)` via `zeros(a) = 3`; S7c — provides `#E(a) ≥ 2`; ShiftPreservation (below) — supplies postcondition (c). The function itself depends only on S7b and S7c; postcondition (c) is a conditional refinement valid wherever `k ≥ 1` and S7c's premise hold.
...
*Proof.* By S3 (referential integrity) applied to `vⱼ ∈ dom(M(d))`, the image `aⱼ = M(d)(vⱼ) ∈ dom(Σ.C)`, so S7b and S7c apply to `aⱼ`. At `k = 0`, `shift(aⱼ, 0) = aⱼ` by convention and all three conclusions are immediate. For `k ≥ 1` — which is vacuous for the singleton decomposition exhibited in the existence argument and arises only in coarser decompositions with run lengths exceeding 1 — ShiftPreservation (above) applied pointwise to `aⱼ` and `k` yields (i) `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)`, (ii) `zeros(shift(aⱼ, k)) = 3`, and (iii) `#E(shift(aⱼ, k)) = #E(aⱼ) = δⱼ`. The conclusions are independent of the value of `k`, so they hold for every `k` with `1 ≤ k < nⱼ` regardless of run length. ∎
