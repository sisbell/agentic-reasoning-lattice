...
- *Preconditions:* T4 (HierarchicalParsing, ASN-0034) — every non-separator component is strictly positive, every present field has at least one component; S7b — addresses in `dom(Σ.C)` are element-level tumblers with `zeros(a) = 3`.
...
**Coverage.** Each `v ∈ dom(M(d))` lies in its own singleton's interval: `v ≤ v < v + 1`, where the right inequality holds because `v + 1 = shift(v, 1) = v ⊕ δ(1, m) > v` by TumblerAdd's strictness postcondition (`a ⊕ w > a` whenever `w > 0`, applied to `w = δ(1, m)`). So every V-position falls in at least one run.
...
By S8a, `zeros(v) = 0`, so every component of `v` is nonzero. By definition, `v + 1 = shift(v, 1) = v ⊕ δ(1, m)`, where `δ(1, m)` is the unit displacement of depth `m` with action point at position `m` (OrdinalShift, ASN-0034). By TumblerAdd's length postcondition, `#(v + 1) = m`. By TumblerAdd's three-region component formula, components before the action point are copied from `v`: `(v + 1)ᵢ = vᵢ` for all `i < m`; at the action point, `(v + 1)_m = v_m + 1` (no carry, since `δ(1, m)` is zero beyond position `m`).

Suppose for contradiction that `t ≠ v` satisfies `#t = m` and `v ≤ t < v + 1`. Since `#t = #v = m`, the sequences diverge at some first position `j ≤ m`. For the case of interest `t = w`, shared subspace `w₁ = v₁ = S` gives `t₁ = v₁`, forcing `j ≥ 2`; at `m = 2` this further forces `j = m = 2`, leaving only the *j = m* case below. The two-case argument that follows treats `j` generically for any `2 ≤ j ≤ m`.
...
*Case j = m.* Then `tᵢ = vᵢ` for `i < m`, so `tᵢ = (v + 1)ᵢ` for `i < m` as well. Since `tᵢ = vᵢ` for `i < m` and `t ≠ v` (with `#t = #v = m`), the divergence at `j = m` is real: `t_m ≠ v_m`. Combined with `v ≤ t`, this gives `v < t`, and T1(i) applied to `v < t` with first divergence at `m` yields strict `t_m > v_m`, i.e., `t_m ≥ v_m + 1`. From `t < v + 1` with first divergence at `m`: `t_m < (v + 1)_m = v_m + 1` by T1(i). But `t_m ≥ v_m + 1` and `t_m < v_m + 1` are incompatible — contradiction.
