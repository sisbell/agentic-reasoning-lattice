...
*Check S0*: no prior content existed, so the implication holds vacuously. *Check S3*: every V-reference resolves — `ran(M(d₁)) ⊆ dom(C)`. *Check S7*: for `a = 1.0.1.0.1.0.1.3`, `origin(a) = 1.0.1.0.1 = d₁` — the document-level prefix directly identifies the allocating document. *Check S8*: the arrangement decomposes into a single correspondence run `(v₁, a₁, n₁) = (1.1, 1.0.1.0.1.0.1.1, 5)` — a *non-singleton* run that exhibits the index-arithmetic identity at every `k ∈ {0, 1, 2, 3, 4}`, not just the trivial base case. The decomposition is non-singleton precisely because the five characters were typed sequentially, receiving consecutive I-addresses by T10a (allocator discipline) — exactly the condition under which `M(d)(shift(v, k)) = shift(a, k)` holds for `k ≥ 1` rather than only at `k = 0`. We verify the identity explicitly at `k = 3` (the auxiliary lemma's structural-preservation argument is non-trivial here, since the shift is exercised at `k ≥ 1`):
