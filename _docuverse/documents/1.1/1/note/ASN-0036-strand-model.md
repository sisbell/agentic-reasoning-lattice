...
Several premises below connect an address `a` in the *current* state's `dom(Σ.C)` to the T10a allocation event under which T10a.4 (ASN-0034) originally established its T4-validity. S0 (content immutability) discharges this connection: every `a ∈ dom(Σ.C)` is identity-fixed for all successor states, so any structural property T10a.4 established at allocation persists. The contracts below cite S0 directly for this persistence step.
