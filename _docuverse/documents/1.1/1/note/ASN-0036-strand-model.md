...
- `N + 1` documents `d₁, …, d_{N+1}`, with `M_N(dᵢ) = {vᵢ ↦ a}` where each `vᵢ = [1, 1]` is the depth-2 V-position in subspace 1 — the `vᵢ` are equal across documents (not pairwise distinct), but sharing multiplicity counts `(d, v)` pairs and the documents `dᵢ` are pairwise distinct by hypothesis, so the pairs `(dᵢ, [1, 1])` are pairwise distinct. (Within-document distinctness does not arise here, as each `M_N(dᵢ)` has a singleton domain.) Each `vᵢ = [1, 1]` satisfies S8a: `zeros([1, 1]) = 0`, `#[1, 1] = 2`, and both components are strictly positive — strengthening the construction beyond the S0–S3 scope of S5 to also satisfy strand-model V-position well-formedness.
...
- *Postconditions:* (a) `subspace_I(a) = E(a)₁` and the content ordinal `[E(a)₂, ..., E(a)_δ]` occupy distinct components. (b) For any `k ≥ 1`, the displacement `δ(k, #a)` has action point `#a`, which falls strictly after the position of `subspace_I(a)` in the full address — so `shift(a, k)` preserves `subspace_I(a)` by TumblerAdd's prefix rule. (c) TA7a's ordinal-only formulation can be applied to the within-subspace ordinal `[E(a)₂, ..., E(a)_δ]`.
- *Depends:* S7b (element-level I-addresses) — provides `E(a)`; T4b (UniqueParse, ASN-0034) — defines element-field projection; TA7a (ASN-0034) — ordinal-only formulation requires `#E(a) ≥ 2`.
...
**The subspace-preservation postcondition.** Conjunct (b) of S8 is the equality `M(d)(shift(vⱼ, k)) = shift(aⱼ, k)` — not subspace preservation. We separately establish, as an auxiliary property of correspondence runs, that for every run `(vⱼ, aⱼ, nⱼ)` and every `k` with `0 ≤ k < nⱼ`, the image `shift(aⱼ, k)` preserves the I-address subspace identifier — `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)`. This is a property of correspondence runs as such, independent of the specific decomposition chosen; we record the derivation here so that any decomposition arising from S8 or its operational refinements inherits it. For the singleton decomposition `(v, a, 1)` constructed above, the only valid `k` is `0`, where `shift(a, 0) = a` by convention makes the property trivial. For `k ≥ 1` — which arises only in coarser decompositions with run lengths exceeding 1 — we derive the claim explicitly. Let `δⱼ = #E(aⱼ)`. By S7b, `zeros(aⱼ) = 3`, so T4 partitions `aⱼ` as `N(aⱼ).0.U(aⱼ).0.D(aⱼ).0.E(aⱼ)` and the element field `E(aⱼ)` occupies the last `δⱼ` components of `aⱼ`. The first component of `E(aⱼ)` — the I-address subspace identifier `subspace_I(aⱼ) = E(aⱼ)₁` — sits at *position* `#aⱼ − δⱼ + 1` within `aⱼ` (counting from 1). The displacement `δ(k, #aⱼ) = [0, …, 0, k]` of length `#aⱼ` has `actionPoint(δ(k, #aⱼ)) = #aⱼ` (its only nonzero component is the last, by OrdinalShift, ASN-0034). The strict inequality `#aⱼ − δⱼ + 1 < #aⱼ` is equivalent to `δⱼ > 1`, i.e. `δⱼ ≥ 2` — exactly the bound supplied by S7c. Hence the position of `subspace_I(aⱼ)` lies strictly before the action point of `δ(k, #aⱼ)`. By TumblerAdd's three-region component formula (ASN-0034), every component of `aⱼ` at a position strictly before the action point is copied unchanged into `shift(aⱼ, k) = aⱼ ⊕ δ(k, #aⱼ)`. In particular, the component at position `#aⱼ − δⱼ + 1` is copied unchanged, so `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)` for every `k ≥ 1`. The claim is independent of the value of `k`, so it holds for every `k` with `1 ≤ k < nⱼ` regardless of run length.
