...
- `N + 1` documents `d₁, …, d_{N+1}` with explicit witnesses `dᵢ = [1, 0, 1, 0, i]` for `i = 1, …, N + 1` (the natural numbers `1, …, N + 1` exist in ℕ by T0, ASN-0034). Each `dᵢ` is a valid document-level tumbler: `zeros(dᵢ) = 2` with no adjacent zeros, positive endpoint components, and the three fields `N(dᵢ) = [1]`, `U(dᵢ) = [1]`, `D(dᵢ) = [i]` populated by strictly positive natural numbers (T4, HierarchicalParsing, ASN-0034). The `dᵢ` are pairwise distinct by T3 (CanonicalRepresentation, ASN-0034) since they have distinct last components. Each arrangement is `M_N(dᵢ) = {vᵢ ↦ a}` where each `vᵢ = [1, 1]` is the depth-2 V-position in subspace 1 — the `vᵢ` are equal across documents (not pairwise distinct), but sharing multiplicity counts `(d, v)` pairs and the documents `dᵢ` are pairwise distinct by the explicit witness, so the pairs `(dᵢ, [1, 1])` are pairwise distinct. (Within-document distinctness does not arise here, as each `M_N(dᵢ)` has a singleton domain.) Each `vᵢ = [1, 1]` satisfies S8a: `zeros([1, 1]) = 0`, `#[1, 1] = 2`, and both components are strictly positive — strengthening the construction beyond the S0–S3 scope of S5 to also satisfy strand-model V-position well-formedness.
