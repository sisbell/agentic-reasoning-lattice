...
We verify each invariant. S2 (arrangement functionality): each `M_N(dᵢ)` contains a single entry `{v ↦ a}` — the domain has one element, so uniqueness of the image is immediate; `M_N(dᵢ)` is a function. S3 (referential integrity): the sole I-address referenced by any arrangement is `a`, and `a ∈ dom(C_N)` by construction. S7b (element-level I-addresses): `a = [1, 0, 1, 0, 1, 0, 1, 1]` has exactly three zero components (positions 2, 4, 6), so `zeros(a) = 3`, and `dom(C_N) = {a}` contains only this address. S7a (document-scoped allocation): `a`'s document-level prefix is `origin(a) = 1.0.1.0.1`, and we stipulate `a` allocated under that document — the prefix names its allocating document, as S7a requires. S7d (document allocation discipline): each `dᵢ = [1, 0, 1, 0, i]` has zeros at positions 2 and 4, so `zeros(dᵢ) = 2`, and the `dᵢ` are pairwise distinct (distinct last components, T3). Domain-restriction axiom on `M_N(dᵢ)`: the sole active V-position `v = [1, 1]` satisfies `zeros(v) = 0` and `#v = 2 ≥ 2`; equivalently (S8a) both components are positive and `#v ≥ 2`. S8-depth: each arrangement has the single V-position `v = [1, 1]` in subspace 1 at depth 2, so the common-depth condition holds trivially. D-MIN: `V_1(dᵢ) = {[1, 1]}`, whose minimum `[1, 1]` is the all-ones tumbler of depth 2. D-CTG: each `V_1(dᵢ)` is the singleton `{[1, 1]}`, a contiguous ordinal range vacuously (no intermediate positions). S8-fin: each `dom(M_N(dᵢ))` is the singleton `{v}` and `dom(C_N)` is the singleton `{a}`, all finite. Thus `Σ_N` is a genuine strand state.
...
We verify each invariant. S2 (arrangement functionality): the `vᵢ` are pairwise distinct by construction (distinct last components, T3 — CanonicalRepresentation, ASN-0034), so each V-position maps to exactly one I-address (namely `a`); `M'_N(d)` is a well-defined function. S3 (referential integrity): the sole referenced I-address `a` satisfies `a ∈ dom(C'_N)` by construction. S7b: `a = [1, 0, 1, 0, 1, 0, 1, 1]` has `zeros(a) = 3` and `dom(C'_N) = {a}`. S7a: `origin(a) = 1.0.1.0.1`, and we stipulate `a` allocated under that document; the prefix names its allocating document. S7d: the sole document `d = [1, 0, 1, 0, 1]` has `zeros(d) = 2`. Domain-restriction axiom on `M'_N(d)`: every `vₖ = [1, k]` with `k ≥ 1` has `zeros(vₖ) = 0` and `#vₖ = 2 ≥ 2` (equivalently S8a: both components positive, depth ≥ 2). S8-depth: every `vₖ` is in subspace 1 at depth 2, so the common-depth condition holds. D-MIN: `V_1(d) = {[1, k] : 1 ≤ k ≤ N + 1}` has minimum `[1, 1]`, the all-ones tumbler of depth 2. D-CTG: `V_1(d)` occupies the contiguous ordinal range `1, …, N + 1` with no gaps. S8-fin: `dom(M'_N(d)) = {v₁, …, v_{N+1}}` and `dom(C'_N) = {a}` are both finite. Thus `Σ'_N` is a genuine strand state.

The within-document sharing multiplicity is `|{v : v ∈ dom(M'_N(d)) ∧ M'_N(d)(v) = a}| = N + 1 > N`.

**Conclusion.** Since both constructions yield genuine strand states for arbitrary `N ∈ ℕ`, sharing multiplicity exceeding any given finite bound is consistent with the full state model — S0–S3 together with the complete always-on invariant set (S7a, S7b, S7d, the domain-restriction axiom, S8-fin, S8-depth, D-CTG, D-MIN). No finite cap on `|{(d, v) : v ∈ dom(Σ.M(d)) ∧ Σ.M(d)(v) = a}|` is entailed by these invariants — neither across documents nor within a single document. ∎

*Formal Contract:*
- *Preconditions:* `N ∈ ℕ` arbitrary.
- *Postconditions:* There exists a genuine strand state `Σ` — satisfying every always-on state-level requirement: S2 (arrangement functionality), S3 (referential integrity), S7a (document-scoped allocation), S7b (element-level I-addresses), S7d (document allocation discipline), the domain-restriction axiom on `Σ.M(d)`, S8-fin (finite arrangements), S8-depth (common depth per subspace), D-CTG and D-MIN (text-subspace contiguity); and trivially consistent with the transition invariants S0, S1 — such that for some `a ∈ dom(Σ.C)`, `|{(d, v) : v ∈ dom(Σ.M(d)) ∧ Σ.M(d)(v) = a}| > N`. The construction works both across documents (multiplicity `N + 1` over `N + 1` documents) and within a single document (multiplicity `N + 1` at `N + 1` distinct V-positions).
- *Depends:* S0, S1, S2, S3, S7a, S7b, S7d, S8-fin, S8-depth, D-CTG, D-MIN, the domain-restriction axiom, S8a, T0 (ASN-0034), T3 (ASN-0034), T4 (ASN-0034).
