...
**Auxiliary lemma (subspace preservation across a correspondence run).** This is not part of the existence argument and constructs no new run; it is a generic property of any correspondence run `(vⱼ, aⱼ, nⱼ)` satisfying S8's conjunct (b), recorded here so that decompositions arising from S8 or its operational refinements inherit it. Claim: for every `k` with `0 ≤ k < nⱼ`, the image `shift(aⱼ, k)` preserves the I-address subspace identifier — `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)`. At `k = 0`, `shift(aⱼ, 0) = aⱼ` by convention and the claim is immediate; the singleton runs constructed in Existence above use only this case. For `k ≥ 1` — which is vacuous for the singleton decomposition and arises only in coarser decompositions with run lengths exceeding 1 — we derive the claim as follows. Let `δⱼ = #E(aⱼ)`. By S7b, `zeros(aⱼ) = 3`, so T4 partitions `aⱼ` as `N(aⱼ).0.U(aⱼ).0.D(aⱼ).0.E(aⱼ)` and the element field `E(aⱼ)` occupies the last `δⱼ` components of `aⱼ`. The first component of `E(aⱼ)` — the I-address subspace identifier `subspace_I(aⱼ) = E(aⱼ)₁` — sits at *position* `#aⱼ − δⱼ + 1` within `aⱼ` (counting from 1). The displacement `δ(k, #aⱼ) = [0, …, 0, k]` of length `#aⱼ` has `actionPoint(δ(k, #aⱼ)) = #aⱼ` (its only nonzero component is the last, by OrdinalShift, ASN-0034). The strict inequality `#aⱼ − δⱼ + 1 < #aⱼ` is equivalent to `δⱼ > 1`, i.e. `δⱼ ≥ 2` — exactly the bound supplied by S7c. Hence the position of `subspace_I(aⱼ)` lies strictly before the action point of `δ(k, #aⱼ)`. By TumblerAdd's three-region component formula (ASN-0034), every component of `aⱼ` at a position strictly before the action point is copied unchanged into `shift(aⱼ, k) = aⱼ ⊕ δ(k, #aⱼ)`. In particular, the component at position `#aⱼ − δⱼ + 1` is copied unchanged, so `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)` for every `k ≥ 1`. By the same prefix-copying argument, the field structure of `aⱼ` is preserved through the shift: the action point of `δ(k, #aⱼ)` is at the last position `#aⱼ`, so every component of `aⱼ` at a position strictly less than `#aⱼ` is copied unchanged — including the three field-separator zeros that T4 places between `N(aⱼ)`, `U(aⱼ)`, `D(aⱼ)`, and `E(aⱼ)`. Hence `zeros(shift(aⱼ, k)) = zeros(aⱼ) = 3`, and `shift(aⱼ, k)` remains an element-level I-address (S7b) with the same element-field depth `#E(shift(aⱼ, k)) = #E(aⱼ) = δⱼ ≥ 2` (S7c). The claim is independent of the value of `k`, so both the subspace-preservation and field-structure-preservation conclusions hold for every `k` with `1 ≤ k < nⱼ` regardless of run length.
...
*Formal Contract:*
- *Preconditions:* `dom(M(d))` finite (S8-fin); `M(d)` a function (S2); referential integrity (S3); `(A v ∈ dom(M(d)) :: zeros(v) = 0 ∧ #v ≥ 2 ∧ (A i : 1 ≤ i ≤ #v : vᵢ > 0))` (S8a); within each subspace, all V-positions share a common depth (S8-depth); I-addresses are element-level with `zeros(a) = 3` (S7b). S7c (`#E(a) ≥ 2`) is *not* a precondition of the existence claim — the singleton witness exhibited in the proof invokes `shift(aⱼ, k)` only at `k = 0` (the identity), so I-address subspace preservation is never appealed to. S7c is a precondition of the auxiliary subspace-preservation lemma below, which becomes load-bearing only for coarser decompositions with run lengths `nⱼ ≥ 2`.
- *Postconditions:* (*Existence.*) There exists a finite set of correspondence runs `{(vⱼ, aⱼ, nⱼ)}` satisfying (a) `(A v ∈ dom(M(d)) :: (E! j :: vⱼ ≤ v < shift(vⱼ, nⱼ)))` and (b) `(A j, k : 0 ≤ k < nⱼ : M(d)(shift(vⱼ, k)) = shift(aⱼ, k))`. The proof exhibits the singleton decomposition (every `nⱼ = 1`), for which conjunct (b) reduces to the base case `M(d)(vⱼ) = aⱼ` at `k = 0` only — both shifts are identities and no I-address subspace preservation argument is invoked. (*Auxiliary lemma (subspace preservation across a run).*) For any correspondence run `(vⱼ, aⱼ, nⱼ)` satisfying conjunct (b), every image `shift(aⱼ, k)` with `0 ≤ k < nⱼ` preserves the I-address subspace `subspace_I(aⱼ)`. At `k = 0` the shift is the identity and the claim is immediate; for `k ≥ 1` — which arises only when `nⱼ ≥ 2`, i.e. in coarser-than-singleton decompositions — S7c supplies the strict inequality `#aⱼ − #E(aⱼ) + 1 < #aⱼ` that places `subspace_I(aⱼ)` strictly before the action point of `δ(k, #aⱼ)`, and TumblerAdd's prefix rule copies the subspace identifier unchanged. The lemma is therefore vacuous on the singleton witness and load-bearing only for coarser decompositions.
- *Depends:* (*Local properties*) S2 (ArrangementFunctionality) — each `v ∈ dom(M(d))` has a uniquely determined image `a = M(d)(v)` for the singleton-run construction; S3 (referential integrity) — places `M(d)(v) ∈ dom(Σ.C)`, enabling S7b/S7c to apply to `a`; S7b (Element-level I-addresses) — supplies `zeros(a) = 3` so that T4 partitions `a` with a non-empty element field; S7c (Element-field depth) — required only by the auxiliary subspace-preservation lemma at `k ≥ 1`; vacuous on the singleton witness exhibited in the existence proof (where only `k = 0` arises and the shift is the identity). When invoked, S7c supplies `#E(a) ≥ 2` — equivalently the strict inequality `#a − #E(a) + 1 < #a` that places `subspace_I(a)` strictly before the action point of `δ(k, #a)` so that `subspace_I(shift(a, k)) = subspace_I(a)`. S8a — supplies `zeros(v) = 0` and `#v ≥ 2` for the within-subspace incompatibility lemma (the lemma's two-case analysis on the first divergence position needs `m ≥ 2` and `v_m > 0`); S8-depth — gives a common depth `m` to every V-position in a fixed subspace, enabling the uniqueness-within-a-subspace argument; S8-fin — finite `dom(M(d))` gives a finite singleton family. (*Foundation claims, ASN-0034*) T1 (TumblerOrdering) case (i) — first-divergence comparison powering the within- and across-subspace incompatibility lemmas; T3 (CanonicalRepresentation) — equates tumblers with their canonical component sequences; T5 (ContiguousSubtrees) — shows that every element of `[v, shift(v, 1))` extends `[v₁]`, sealing the cross-subspace uniqueness step; T10 — non-nesting prefixes generate disjoint tumbler subtrees, ruling out cross-subspace overlap; TS4 (ShiftStrictIncrease) — `v < shift(v, 1)`, witnessing both the singleton-interval inclusion and the existence of a proper successor; TumblerAdd, OrdinalShift, OrdinalDisplacement — supply the action-point semantics of `δ(k, m)` and the three-region component formula used throughout: prefix copying gives subspace preservation; the action-point identity `shift(v, 1)_m = v_m + 1` underwrites the divergence step in the Case `j = m` branch of the lemma.
...
*Case m = 2.* Every position has exactly two components: the subspace identifier 1 at component 1, and a single ordinal at component 2. There are no intermediate components (components 2 through m − 1 is the empty range 2 through 1), so the shared-prefix condition holds vacuously. Every position is [1, k] for some k, which is [1, 1, …, 1, k] with zero intervening 1s.
...
**Step 3: contiguity of k-values.** Let k₁ < k₂ be two values attained by positions v₁ = [1, 1, …, 1, k₁] and v₂ = [1, 1, …, 1, k₂] in V_1(d). Both have subspace 1 and depth m. By T1(i) (TumblerOrdering, ASN-0034), v₁ < v₂ since they agree on components 1 through m − 1 and differ first at component m where k₁ < k₂. For any integer k with k₁ < k < k₂, the tuple w = [1, 1, …, 1, k] satisfies subspace(w) = 1, #w = m, and v₁ < w < v₂ (again by T1(i), since w agrees with both on components 1 through m − 1 and k₁ < k < k₂ at component m). By D-CTG (VContiguity), w ∈ V_1(d). Therefore every integer between any two attained k-values is itself attained — the k-values form a contiguous range.
