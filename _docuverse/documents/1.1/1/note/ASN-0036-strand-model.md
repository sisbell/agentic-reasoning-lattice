...
- *Postconditions:* (a) `subspace_I(a) ∈ ℕ` — the projected component inherits T0's ℕ-valued carrier (ASN-0034). (b) `subspace_I(a) ≥ 1` — T4's positive-component constraint on present fields, refined by T10a.4's componentwise positivity within `E(a)`, delivers `E(a)₁ ≥ 1`. (c) *Subspace preservation under shift:* for `k ≥ 1`, `subspace_I(shift(a, k)) = subspace_I(a)`. *Derivation (two-line):* The action point of `δ(k, #a)` is `#a` (OrdinalShift, ASN-0034); the position of `subspace_I(a) = E(a)₁` within `a` is `#a − #E(a) + 1`, which satisfies `#a − #E(a) + 1 < #a` because S7c's axiom `#E(a) ≥ 2` gives `#a − #E(a) + 1 ≤ #a − 1 < #a` via NAT-sub (`#a − #E(a) ∈ ℕ` since `#a ≥ #E(a)`) and NAT-order. Hence the position of `subspace_I(a)` lies strictly before the action point, so TumblerAdd's prefix rule copies this component unchanged into `shift(a, k)`. Re-expressing via T4b on `shift(a, k)` (whose T4-validity follows from S8's auxiliary lemma's conclusions (ii) and (iii), which are independent of the present postcondition — the lemma's proof does not invoke `subspace_I`'s postcondition (c)), this is `subspace_I(shift(a, k)) = subspace_I(a)`. *Note on non-circularity:* S8's auxiliary lemma's conclusion (i) is an export of this same two-line argument applied inside a correspondence run — it consumes S7c directly, not `subspace_I`'s postcondition (c). The dependency chain is `S7c → subspace_I Postcondition (c)` and `S7c → S8 auxiliary lemma conclusion (i)` in parallel, not in series.
- *Depends:* T0 (ASN-0034) — ℕ-valued component carrier underwriting postcondition (a); T4b (UniqueParse, ASN-0034) — supplies `E(a)` once S7b holds, and applies to `shift(a, k)` via S8's auxiliary lemma conclusions (ii) and (iii); T4 (HierarchicalParsing, ASN-0034) — positive-component constraint on present fields underwriting postcondition (b); T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — componentwise positivity within the element field, reinforcing postcondition (b); NAT-sub, NAT-order (ASN-0034) — license the position-arithmetic step `#a − #E(a) + 1 < #a` in postcondition (c); OrdinalShift (ASN-0034) — fixes the action point of `δ(k, #a)` at `#a`; TumblerAdd (ASN-0034) — prefix rule copies the component strictly before the action point; S7b — provides `E(a)` via `zeros(a) = 3`; S7c — provides `#E(a) ≥ 2`, which is the sole non-arithmetic premise of postcondition (c)'s derivation; S8's auxiliary lemma conclusions (ii) and (iii) — license T4b's application to `shift(a, k)` (these conclusions are derived from S7b, S7c, and TumblerAdd's prefix rule, without invoking postcondition (c) of `subspace_I`, so no circularity arises). The function itself depends only on S7b and S7c; postcondition (c) is a conditional refinement valid in any context where `k ≥ 1` and S7c's premise holds.
