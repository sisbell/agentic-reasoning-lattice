...
- *Postconditions:* (a) `subspace_I(a) = E(a)₁` and the content ordinal `[E(a)₂, ..., E(a)_δ]` occupy distinct components. (b) For any `k ≥ 1`, the displacement `δ(k, #a)` has action point `#a`, which falls strictly after the position of `subspace_I(a)` in the full address — so `shift(a, k)` preserves `subspace_I(a)` by TumblerAdd's prefix rule. (c) When `#E(a) ≥ 2`, the within-subspace ordinal `[E(a)₂, ..., E(a)_δ]` is a non-empty tumbler that lies in `S` whenever all its components are positive, satisfying TA7a's operand precondition `o ∈ S` so that `⊕` and `⊖` are directly applicable.
