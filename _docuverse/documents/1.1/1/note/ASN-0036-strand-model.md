...
- *Consequence (b) — shift action-point separation:* For any `k ≥ 1`, the displacement `δ(k, #a)` has action point `#a`, which falls strictly after the position of `subspace_I(a)` in the full address — so `shift(a, k)` preserves `subspace_I(a)` by TumblerAdd's prefix rule: `subspace_I(shift(a, k)) = subspace_I(a)`. *Derivation:* This conclusion is established in full by **ShiftPreservation** conclusion (iv) below, whose proof carries out the position-arithmetic step `#a − δ + 1 < #a` from `δ = #E(a) ≥ 2` (S7c) via NAT-sub, NAT-addcompat, NAT-order, and NAT-cancel, then applies TumblerAdd's prefix rule. ShiftPreservation is generic in `a ∈ dom(Σ.C)` and `k ≥ 1`, independent of correspondence runs; it stands as the single canonical source for this identity, with subspace_I's Postcondition (c) and S8's run-corollary clause (i) both citing it.
...
- *Postconditions:* (a) `subspace_I(a) ∈ ℕ` — the projected component inherits T0's ℕ-valued carrier (ASN-0034). (b) `subspace_I(a) ≥ 1` — T4's positive-component constraint on present fields, refined by T10a.4's componentwise positivity within `E(a)`, delivers `E(a)₁ ≥ 1`. (c) *Subspace preservation under shift:* for `k ≥ 1`, `subspace_I(shift(a, k)) = subspace_I(a)`. *Derivation:* This is **ShiftPreservation** conclusion (iv) below, established for any `a ∈ dom(Σ.C)` and any `k ≥ 1`. *Note on non-circularity:* ShiftPreservation's proofs of conclusion (ii) (T4-validity of `shift(a, k)`, on which the re-expression via T4b depends) and conclusion (iv) (the subspace identity itself, with its position-arithmetic step `#a − #E(a) + 1 < #a` carried out from `#E(a) ≥ 2` via NAT-sub, NAT-addcompat, NAT-order, NAT-cancel) are derived from S7b, S7c, T10a.4, T4, and TumblerAdd's prefix rule alone — neither invokes `subspace_I`'s postcondition (c) or any output of S8.
...
*Formal Contract:*
- *Preconditions:* `dom(M(d))` finite (S8-fin); `M(d)` a function (S2); referential integrity (S3); `(A v ∈ dom(M(d)) :: zeros(v) = 0 ∧ #v ≥ 2 ∧ (A i : 1 ≤ i ≤ #v : vᵢ > 0))` (S8a); within each subspace, all V-positions share a common depth (S8-depth); I-addresses are element-level with `zeros(a) = 3` (S7b) and have element-field depth `#E(a) ≥ 2` (S7c). Neither S7b nor S7c is a precondition of the existence claim — the singleton witness exhibited in the proof invokes `shift(aⱼ, k)` only at `k = 0` (the identity), so neither I-address element-level structure nor field-structure preservation is appealed to. Both S7b and S7c are preconditions of ShiftPreservation, which the corollary below cites pointwise at each `k`; the corollary's `k ≥ 1` content is therefore load-bearing only for coarser decompositions with run lengths `nⱼ ≥ 2`.
