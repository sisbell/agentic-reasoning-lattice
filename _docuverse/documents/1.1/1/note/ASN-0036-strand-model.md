...
We verify the construction-specific invariants. S2 (arrangement functionality): each `M_N(dᵢ)` contains a single entry `{v ↦ a}` — the domain has one element, so `M_N(dᵢ)` is a function. S7d (document allocation discipline): each `dᵢ = [1, 0, 1, 0, i]` has zeros at positions 2 and 4, so `zeros(dᵢ) = 2`, and the `dᵢ` are pairwise distinct (distinct last components, T3). D-CTG: each `V_1(dᵢ)` is the singleton `{[1, 1]}`, a contiguous ordinal range vacuously (no intermediate positions). S8-fin: each `dom(M_N(dᵢ))` is the singleton `{v}`, finite. With the shared facts, `Σ_N` is a genuine strand state.
...
We verify the construction-specific invariants. S2 (arrangement functionality): the `vₖ` are pairwise distinct by construction (distinct last components, T3 — CanonicalRepresentation, ASN-0034), so each V-position maps to exactly one I-address (namely `a`); `M'_N(d)` is a well-defined function. S7d: the sole document `d = [1, 0, 1, 0, 1]` has `zeros(d) = 2`. D-CTG: `V_1(d)` occupies the contiguous ordinal range `1, …, N + 1` with no gaps. S8-fin: `dom(M'_N(d)) = {v₁, …, v_{N+1}}` is finite. With the shared facts, `Σ'_N` is a genuine strand state.
...
- *Depends:* S0, S1, S2, S3, S7a, S7b, S7d, S8-fin, S8-depth, D-CTG, D-MIN, the domain-restriction axiom (equivalently S8a), T0 (ASN-0034), T3 (ASN-0034), T4 (ASN-0034).
