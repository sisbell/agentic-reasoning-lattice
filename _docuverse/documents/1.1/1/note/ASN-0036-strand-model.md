...
**Auxiliary lemma (subspace and field-structure preservation across a correspondence run).** This is not part of the existence argument and constructs no new run; it is a generic property of any correspondence run `(vⱼ, aⱼ, nⱼ)` satisfying S8's conjunct (b), recorded here so that decompositions arising from S8 or its operational refinements inherit it. Claim: for every `k` with `0 ≤ k < nⱼ`, the image `shift(aⱼ, k)` preserves three structural properties of `aⱼ` — (i) the I-address subspace identifier `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)`; (ii) the element-level zero-count `zeros(shift(aⱼ, k)) = zeros(aⱼ) = 3` (so `shift(aⱼ, k)` remains an element-level I-address, S7b); and (iii) the element-field depth `#E(shift(aⱼ, k)) = #E(aⱼ) = δⱼ ≥ 2` (so the depth bound of S7c is preserved). At `k = 0`, `shift(aⱼ, 0) = aⱼ` by convention and all three conclusions are immediate; the singleton runs constructed in Existence above use only this case. For `k ≥ 1` — which is vacuous for the singleton decomposition and arises only in coarser decompositions with run lengths exceeding 1 — we derive the conclusions as follows. Let `δⱼ = #E(aⱼ)`. By S7b, `zeros(aⱼ) = 3`, so T4 partitions `aⱼ` as `N(aⱼ).0.U(aⱼ).0.D(aⱼ).0.E(aⱼ)` and the element field `E(aⱼ)` occupies the last `δⱼ` components of `aⱼ`. The first component of `E(aⱼ)` — the I-address subspace identifier `subspace_I(aⱼ) = E(aⱼ)₁` — sits at *position* `#aⱼ − δⱼ + 1` within `aⱼ` (counting from 1). The displacement `δ(k, #aⱼ) = [0, …, 0, k]` of length `#aⱼ` has `actionPoint(δ(k, #aⱼ)) = #aⱼ` (its only nonzero component is the last, by OrdinalShift, ASN-0034). The strict inequality `#aⱼ − δⱼ + 1 < #aⱼ` is equivalent to `δⱼ > 1`, i.e. `δⱼ ≥ 2` — exactly the bound supplied by S7c. Hence the position of `subspace_I(aⱼ)` lies strictly before the action point of `δ(k, #aⱼ)`. By TumblerAdd's three-region component formula (ASN-0034), every component of `aⱼ` at a position strictly before the action point is copied unchanged into `shift(aⱼ, k) = aⱼ ⊕ δ(k, #aⱼ)`. In particular, the component at position `#aⱼ − δⱼ + 1` is copied unchanged, establishing conclusion (i): `subspace_I(shift(aⱼ, k)) = subspace_I(aⱼ)`. By the same prefix-copying argument, every component of `aⱼ` at a position strictly less than `#aⱼ` is copied unchanged — including the three field-separator zeros that T4 places between `N(aⱼ)`, `U(aⱼ)`, `D(aⱼ)`, and `E(aⱼ)` — and the length identity `#shift(aⱼ, k) = #aⱼ` (TumblerAdd's length postcondition, ASN-0034) means `shift(aⱼ, k)` has the same positions to count. The only remaining position is the last one, `#aⱼ`, which is overwritten by TumblerAdd's action-point clause: `shift(aⱼ, k)_{#aⱼ} = aⱼ_{#aⱼ} + k`. By T4's field-segment constraint applied to `aⱼ` (ASN-0034), `aⱼ_{#aⱼ} ≠ 0`, so `aⱼ_{#aⱼ} ∈ ℕ` with `aⱼ_{#aⱼ} ≥ 1` (NAT-zero's disjunction `0 < aⱼ_{#aⱼ} ∨ 0 = aⱼ_{#aⱼ}` combined with `aⱼ_{#aⱼ} ≠ 0` forces `0 < aⱼ_{#aⱼ}`, which NAT-discrete at `(0, aⱼ_{#aⱼ})` promotes to `1 ≤ aⱼ_{#aⱼ}`); combined with `k ≥ 1`, NAT-addcompat's left compatibility gives `aⱼ_{#aⱼ} + k ≥ 1 + 1 ≥ 1 > 0`. Hence position `#aⱼ` of `shift(aⱼ, k)` contributes no new zero. The shifted tumbler therefore carries exactly the three field-separator zeros inherited from `aⱼ` and no others, establishing conclusion (ii): `zeros(shift(aⱼ, k)) = zeros(aⱼ) = 3`, and `shift(aⱼ, k)` satisfies S7b's element-level zero-count. Conclusion (iii) follows directly: since `#shift(aⱼ, k) = #aⱼ` and the three field-separator zeros sit at exactly the same positions (all strictly less than `#aⱼ`), T4's partition `N(shift(aⱼ, k)).0.U(shift(aⱼ, k)).0.D(shift(aⱼ, k)).0.E(shift(aⱼ, k))` has the same element-field boundary, so `#E(shift(aⱼ, k)) = #E(aⱼ) = δⱼ ≥ 2`, preserving S7c's depth bound. The three conclusions are independent of the value of `k`, so they hold for every `k` with `1 ≤ k < nⱼ` regardless of run length.
