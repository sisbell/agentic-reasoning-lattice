...
**`succ` stays within a subspace and preserves well-formedness.** Let `v ∈ dom(M(d))`, so `v` satisfies S8a. By OrdShiftHom (b), `subspace(shift(v, 1)) = subspace(v)`, and by OrdShiftHom (c) `shift(v, 1)` again satisfies S8a; by S8-depth it shares `v`'s depth `m`. Thus every lockstep link stays inside one subspace, and `succ` is a partial function on the well-formed V-positions of that subspace. On the image side, `a = M(d)(v) ∈ dom(Σ.C)` by S3, so by ShiftPreservation `shift(a, 1)` is again an element-level, T4-valid I-address with `subspace_I(shift(a, 1)) = subspace_I(a)`; the lockstep condition `M(d)(shift(v, 1)) = shift(a, 1)` therefore equates two structurally valid I-addresses.
...
| S7c | Element-field depth: `(A a ∈ dom(C) :: #E(a) ≥ 2)` — subspace identifier and content ordinal occupy distinct components | design requirement; uses S7b, T4b (ASN-0034) |
| subspace_I(a) | I-address subspace identifier: `subspace_I(a) = E(a)₁`, first component of the element field | introduced; uses S7b, S7c, T4b (ASN-0034) |
| ShiftPreservation | Element-level shift preserves structure: for `a ∈ dom(C)`, `k ≥ 1`, `shift(a, k)` has `zeros = 3`, is T4-valid, `#E(shift(a,k)) = #E(a)`, `subspace_I(shift(a,k)) = subspace_I(a)` | lemma; uses S7b, S7c, T4, T4b, T10a.4, OrdinalShift, TumblerAdd, NAT-discrete, NAT-closure, NAT-addcompat, NAT-order (ASN-0034) |
