...
K.μ⁺_L's effect clause (ASN-0047) asserts `dom(Σ'.M(d)) = dom(Σ.M(d)) ∪ {v_ℓ}`; for this to constitute strict domain extension (E1) rather than mere set union with a possibly-existing element, we additionally need `v_ℓ ∉ dom(Σ.M(d))`. We discharge this freshness clause here by subspace decomposition, owning the derivation rather than citing it to ASN-0047. By S3★-aux (SubspaceExhaustiveness), every `v ∈ dom(Σ.M(d))` has `subspace(v) ∈ {s_C, s_L}`, so the freshness check reduces to two sub-cases:

(i) Against `V_{s_C}(d) = {v ∈ dom(Σ.M(d)) : subspace(v) = s_C}`: K.μ⁺_L's precondition places `v_ℓ` with `subspace(v_ℓ) = s_L`, so by SC-NEQ (ASN-0093, `s_C ≠ s_L`), `v_ℓ` differs from every element of `V_{s_C}(d)` at position 1.

(ii) Against `V_{s_L}(d)`: K.μ⁺_L's precondition selects `v_ℓ` either as the D-MIN★ minimum `[s_L, 1, ..., 1]` (when `V_{s_L}(d) = ∅`, vacuously excluding every element of `V_{s_L}(d)`) or as `shift(max(V_{s_L}(d)), 1)` (when `V_{s_L}(d) ≠ ∅`, with TS4 (ShiftStrictIncrease, ASN-0034) supplying `v_ℓ > max(V_{s_L}(d))` and thus excluding every element of `V_{s_L}(d)`).

Together, `v_ℓ ∉ V_{s_C}(d) ∪ V_{s_L}(d) = dom(Σ.M(d))`, so the effect clause `dom(Σ'.M(d)) = dom(Σ.M(d)) ∪ {v_ℓ}` is in fact a strict containment `⊃`, yielding (E1). The agreement clause `(A v ∈ dom(Σ.M(d)) : Σ'.M(d)(v) = Σ.M(d)(v))` holds because the effect adds a new mapping at `v_ℓ` without disturbing existing entries — (E2) holds. The constraints (a)–(c) affect which `ℓ` and which `v_ℓ` may appear in any specific K.μ⁺_L event, but they do not modify the way `Σ'.M(d)` relates to `Σ.M(d)` on the prior domain or how the domain grows. LP9's argument consumes only (E1) and (E2); it is invariant under these constraint-level differences. (The per-subspace dependence of D-CTG★/D-MIN★ in ASN-0047 governs *which* V-positions K.μ⁺_L may select — placing the new link-subspace V-position contiguously after the current `V_{s_L}(d)` maximum, or at its minimum if the subspace is empty — but does not affect the structural form of (E1)/(E2).)
...
`F` is countably infinite. By T0(a) and T0(b) of ASN-0034, the set of T4-valid document tumblers is itself infinite (component values are unbounded and tumbler length is unbounded), and each contributes a countably infinite chain in each of the two subspaces. The universal quantifier `(A t ∈ F : s ≤ t < s ⊕ ℓ : …)` in the tightness predicate therefore ranges over an infinite domain. Decidability of this quantifier rests on a finitude lemma about the interval's interaction with `F`'s structural form.

**LP-Fin — IntervalFinitude**: For every well-formed span `(s, ℓ)` (T12, ASN-0034), the set `F ∩ [s, s ⊕ ℓ)` is finite.

The argument proceeds by bounding both the length and component values of admissible candidates. Every candidate `a ∈ F` has structural form `a = [d, 0, s', k]` for some T4-valid `d` with `zeros(d) = 2`, subspace `s' ∈ {s_C, s_L}`, and chain index `k ≥ 1`; the triple `(d, s', k)` determines `a` uniquely and `#a = #d + 3`.

*Length bound on `#a`.* By T1 (ASN-0034), `a < s ⊕ ℓ` admits two cases. Case (ii) places `a` as a proper prefix of `s ⊕ ℓ` with `#a < #(s ⊕ ℓ)`; at each length `n ∈ {1, ..., #(s ⊕ ℓ) - 1}` there is at most one such prefix, and for that prefix to also inhabit `F` the structural form `[d, 0, s', k]` must align with `s ⊕ ℓ`'s components at the prescribed positions — at most one matching candidate per `n`, contributing finitely many in total. Case (i) places the divergence at some position `j ≤ min(#a, #(s ⊕ ℓ))` with `a_j < (s ⊕ ℓ)_j` and `a, s ⊕ ℓ` agreeing on positions `1..j-1`. Here `j ≤ #(s ⊕ ℓ)` is bounded by the right endpoint's length, independent of `#a`.

*Bound on `#d` via the zeros(d) = 2 constraint.* The agreement on positions `1..j-1` forces `a`'s first `j-1` components to equal `(s ⊕ ℓ)`'s first `j-1` components. By `a`'s structural form, positions `1..#d` of `a` are exactly `d`, so the first `min(j-1, #d)` components of `d` are determined by the prefix of `s ⊕ ℓ`. The constraint `zeros(d) = 2` then bounds `#d`: any zero component of `d` must appear at a position consistent with the prefix-agreement, and `d`'s T4 structure (no adjacent zeros, non-zero endpoints) forces the two zeros of `d` to fall at positions admissible under that agreement. The structural rigidity of `d` as a document tumbler (separators only at the two field-boundary positions, with non-empty fields between and around) admits only finitely many candidate placements of `d`'s two zeros relative to the prefix-agreement structure. For each candidate placement, the trailing positions of `d` (those beyond the prefix-agreement's reach) must continue `d`'s T4 structure under `zeros(d) = 2`, which combined with the requirement that the divergence at position `j` lie strictly within `d`'s admissible value range (`a_j < (s ⊕ ℓ)_j`, finite) bounds `#d` to a finite range.

*Bound on component values.* At the divergence position `j`, `a_j ∈ {0, 1, ..., (s ⊕ ℓ)_j - 1}` — a finite set. At positions where `d`'s structural form is fixed by the prefix-agreement, no freedom remains. At positions where `d`'s components are unconstrained by the prefix-agreement, the T4 structure of `d` (combined with `zeros(d) = 2`) and the lower-bound constraint `a ≥ s` collectively pin down a finite set of admissible values: any component of `d` must be either zero (at one of the two separator positions) or strictly positive but constrained by the requirement that `a ≥ s` (so that `a`'s components at positions where `s` has non-zero values do not undercut them).

*Bound on `s'` and `k`.* The subspace `s' ∈ {s_C, s_L}` is a 2-element set. The chain index `k ≥ 1` is bounded above by the constraint `a_{#d + 3} < (s ⊕ ℓ)_{#d + 3}` when the divergence falls at position `#d + 3`, or by `a ≥ s` and `a < s ⊕ ℓ` together when the divergence falls elsewhere; either way, `k` ranges over a finite set determined by `s` and `s ⊕ ℓ`.

Combining: `F ∩ [s, s ⊕ ℓ)` is a finite union of finite sets, hence finite. The achievability arguments below proceed by structural case analysis on the relation between candidate `d` and the span's home document `d_0`, exhausting the partition of `F` across `d`-cases; the count within each case is finite by LP-Fin, and the case analysis is decidable from `s`, `ℓ`, and the structural form of `F`-candidates without enumerating `F`.
...
| LP-Fin | Interval finitude: `(A s, ℓ : (s, ℓ) satisfies T12 : |F ∩ [s, s ⊕ ℓ)| < ∞)` — only finitely many `F`-candidates fall within any well-formed span's reach | introduced |
