...
*Boundary case — content-subspace empty, link-subspace retained.* When `n'_{s_C} = 0` but `n'_{s_L} > 0`, the retention set reduces to `R = {[s_L, 1, ..., 1, k] : 1 ≤ k ≤ n'_{s_L}} ⊆ V_{s_L}(d)` — only link-subspace V-positions are retained. For a link `a` whose endsets reach only content-subspace I-addresses (every span `(s, ℓ) ∈ Σ.L(a).eᵢ` for every slot `i` satisfies `subspace_I(s) = s_C` and similarly along the span, ensured by canonical construction in content-subspace coverage), the pre-state projection `project(a, i, d, Σ)` is contained in `V_{s_C}(d)` by S3★ of ASN-0047 (content-subspace V-positions map to content I-addresses, so only content-subspace V-positions can satisfy `Σ.M(d)(v) ∈ coverage(Σ.L(a).eᵢ)`). Hence `project(a, i, d, Σ) ∩ R ⊆ V_{s_C}(d) ∩ V_{s_L}(d) = ∅` (by S3★-aux and SC-NEQ, the two subspace V-position sets are disjoint), and the wp again evaluates to false. The boundary case exhibits the wp's *per-subspace sensitivity*: the retention set's subspace partition — not merely its total cardinality — determines whether the wp is satisfiable for a given link.
...
*Finiteness from the bound.* The sub-case (ii) argument, applied symmetrically to `#d ≤ #d_0`, shows that `d` must agree with `d_0` on positions `1..#d` (any disagreement would yield `a < s` or `a > s ⊕ ℓ` at the disagreement position). Hence `d` is the length-`#d` prefix of `d_0`, uniquely determined by `#d`. With `#d ∈ {1, …, #d_0}`, there are at most `#d_0` candidate lengths; admissibility further requires `d` to be T4-valid with `zeros(d) = 2`, restricting `#d` to a finite subset. For each admissible `#d`, the remaining components `s'' ∈ {s_C, s_L}` (a 2-element set) and `k ≥ 1` (bounded above by the comparison constraints `a ≥ s` and `a < s ⊕ ℓ`, hence a finite range) yield finitely many candidates per `#d`. Total: `F ∩ [s, s ⊕ ℓ)` is finite.

*Non-canonical spans yield infinite intersections.* For `(s, ℓ)` satisfying T12 but with `#ℓ < #s`, TumblerAdd's result-length identity gives `#(s ⊕ ℓ) = #ℓ < #s`. The T1 comparison `a < s ⊕ ℓ` via case (i) requires divergence at some position `j ≤ min(#a, #(s ⊕ ℓ)) = min(#a, #ℓ)`, which lies at most at position `#ℓ`. When `#ℓ ≤ #d_0`, position `#d_0 + 1` exceeds `#(s ⊕ ℓ)` and so does not enter the T1 comparison at all. The trap that drives sub-case (i)'s contradiction — `a_{#d_0 + 1} ≥ 1 > 0 = (s ⊕ ℓ)_{#d_0 + 1}` — cannot be sprung, because `(s ⊕ ℓ)_{#d_0 + 1}` is undefined. Candidates with `#d > #d_0` whose tails beyond `#ℓ` are unconstrained by the comparison can then inhabit `F ∩ [s, s ⊕ ℓ)` in infinite supply.

Concrete witness: take `s = [1, 0, 1, 0, 1, 0, 1, 1]` (so `#s = 8`, `d_0 = [1, 0, 1, 0, 1]`, `zeros(d_0) = 2`) and `ℓ = [0, 0, 0, 1]` (so `#ℓ = 4` and `actionPoint(ℓ) = 4 ≤ #s = 8`, satisfying T12). Then `s ⊕ ℓ = [1, 0, 1, 1]` (length 4, via TumblerAdd's prefix-copy on positions 1–3 and sum at position 4: `0 + 1 = 1`). For each `n ≥ 1`, define `d_n = [1, 0, 1, 0, 1, 1, …, 1]` of length `5 + n` (the first 5 components are `d_0`; positions 6..5 + n are all `1`). By inspection, `zeros(d_n) = 2`, no two zeros of `d_n` are adjacent (zeros lie at positions 2 and 4), `d_n[1] = 1 ≠ 0`, and `d_n[5 + n] = 1 ≠ 0`, so `d_n` is T4-valid. Set `a_n = [d_n, 0, 1, 1]` of length `5 + n + 3 = 8 + n`. Verification that `a_n ∈ F`: `zeros(a_n) = 2 + 1 = 3`, no adjacent zeros (the separator at position `6 + n` is preceded by `d_n`'s non-zero tail and followed by `1`), `a_n[1] = 1 ≠ 0`, and `a_n[8 + n] = 1 ≠ 0`. Verification that `a_n ∈ [s, s ⊕ ℓ)`: at position 4, `a_n[4] = d_n[4] = 0 < 1 = (s ⊕ ℓ)[4]`, with positions 1–3 agreeing, so `a_n < s ⊕ ℓ` by T1 case (i); at position 6, `a_n[6] = d_n[6] = 1 > 0 = s[6]`, with positions 1–5 agreeing, so `a_n > s` by T1 case (i). Hence `{a_n : n ≥ 1} ⊆ F ∩ [s, s ⊕ ℓ)`, an infinite set. The canonical assumption `ℓ = δ(n, #s)` is therefore load-bearing for LP-Fin's finitude conclusion — the witness exhibited above breaks finitude in the `#ℓ < #s` direction, and the proof's reliance on the action-point identification `actionPoint(ℓ) = #s` (OrdinalDisplacement, ASN-0034) means non-canonical forms with `#ℓ = #s` but `ℓ` not an ordinal displacement (some nonzero component before position `#s`) also fall outside the argument's scope.
...
| LP-Fin | Interval finitude (canonical): `(A s, ℓ : s ∈ F ∧ ℓ = δ(n, #s) for some n ≥ 1 : |F ∩ [s, s ⊕ ℓ)| < ∞)` — only finitely many `F`-candidates fall within any canonical span's reach. Non-canonical spans (`#ℓ < #s`) have `|F ∩ [s, s ⊕ ℓ)| = ℵ₀` and are unconditionally non-tight. | introduced |
