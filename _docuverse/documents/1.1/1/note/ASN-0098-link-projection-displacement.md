...
**LP-Comp — CompositionalCoverage** (documentation note, not a load-bearing lemma): The per-step lemmas LP4 through LP14 form a covering case-analysis on the operation kinds of the working frame. Every atomic transition `Σ → Σ'` admitted by the ASN-0047 + ASN-0093 vocabulary is governed by exactly one of them: K.σ and K.δ-IsDocument by LP8; K.δ-IsNode and K.δ-IsAccount by LP4 via their `(A d :: M'(d) = M(d))` frames; K.α by LP6; K.λ by LP7; K.ρ by LP14; K.μ⁺ and K.μ⁺_L on document `d` by LP9 (and on `d' ≠ d` by LP5); K.μ⁻ on `d` by LP10 (and on `d' ≠ d` by LP5); K.μ~ on `d` by LP11 (and on `d' ≠ d` by LP5). Since reachable sequences `Σ →* Σ'` decompose into finite chains of atomic transitions (SequentialTransitionAxiom of ASN-0093), the per-step lemmas jointly characterise the projection's evolution across any such sequence. We make no separate claim about the *cumulative* behaviour of mixed-kind chains: same-operation chains (LP9-only, LP10-only, LP11-only) compose immediately by set-containment, set-containment, and function composition respectively, but mixed chains have no uniform characterisation worth naming. The multi-step claims of this ASN — LP18 (resurrection) and LP19 (tight boundary exclusion across `Σ_e →* Σ_n`) — are stated and proved as self-contained results that consume state-invariant facts (LP3★ coverage, LP19a freshness on a single allocation step) and per-step lemmas as their proofs require, without leaning on any cumulative-displacement lemma. LP-Comp is the assertion that no such lemma is missing: the per-step coverage is exhaustive on operation kinds, so any multi-step argument decomposes structurally into per-step applications. This is the principle the present ASN relies on, nothing more.
...
The argument turns on the absence of link addresses from coverage: `coverage(Σ.L(a).eᵢ) ∩ dom(Σ.L) = ∅`. We derive `dom(Σ.L) ⊆ F` via an explicit three-step citation chain. First, by ChainMembershipForOrigin (ASN-0093), every `ℓ' ∈ dom(Σ.L)` is a chain element of the link sub-allocator `A_L(d')` where `d' = origin(ℓ')` — link-store membership is *populated* from sub-allocator chains, not from arbitrary tumblers. Second, by SubAllocatorAxiom.FirstEmission and SubAllocatorAxiom.ChainDiscipline (ASN-0093), every chain element of `A_L(d')` has structural form `[d', 0, s_L, k']` for some `k' ≥ 1`: FirstEmission fixes the chain's first element at `[d', 0, s_L, 1]`, and ChainDiscipline supplies the recurrence `t_{n+1} = inc(t_n, 0)`, which by TA5(c) of ASN-0034 advances only the final component, preserving the prefix `[d', 0, s_L, ·]` across all subsequent elements. Third, by SubAllocatorAxiom's restriction that sub-allocator chains are activated only at document-level tumblers (`d' ∈ dom(Σ.M)` per the activation discipline) combined with M0 of ASN-0093 (every allocated document tumbler is T4-valid with `zeros(d') = 2`), the document parameter `d'` is T4-valid with `zeros(d') = 2`. Substituting into `F`'s set-builder (with `d = d'`, `s = s_L`, `k = k'`), every `ℓ' ∈ dom(Σ.L)` satisfies the membership condition of `F`, so `dom(Σ.L) ⊆ F`. (StoreT4Validity additionally supplies that each `ℓ' ∈ dom(Σ.L)` is T4-valid, which is consistent with — but not required for — the chain-derived structural form to inhabit `F`.) For a single span `(s, ℓ) ∈ Σ.L(a).eᵢ` with `s = [d_s, 0, s_C, k_s]` and `ℓ = δ(n, #s)`, the coverage is the interval `[s, s ⊕ ℓ)`; rewriting via `dom(Σ.L) ⊆ F`:
```
...
*Scope restriction — link-canonical companion case is OUT_OF_SCOPE.* LP12b addresses only the **content-canonical** class of links — those whose every span is canonical with `s = [d_s, 0, s_C, k_s]`. The symmetric **link-canonical** class — every span canonical with `s = [d_s, 0, s_L, k_s]`, under the same retention pattern `n'_{s_C} = 0, n'_{s_L} > 0` — is explicitly *out of scope* for this ASN. The case is not artificial: L4(c) of ASN-0043 admits link-subspace endsets (endsets whose spans reference addresses of other links). For such a link, LP-Fin Corollary applied at `X = s_L` gives `F ∩ [s, s ⊕ ℓ) = {[d_s, 0, s_L, k] : k_s ≤ k < k_s + n}` for each canonical span, an interval whose F-candidates all carry subspace identifier `s_L`. By L0 of ASN-0093, every element of `dom(L)` carries `s_L`, so this F-interval is *not* disjoint from `dom(L)` — it may intersect coverage with link-subspace addresses, and the projection may then contain link-subspace V-positions (by S3★, link-subspace V-positions in the arrangement map to `dom(L)`). The retention set `R ⊆ V_{s_L}(d)` admits non-empty intersection with such a projection, so the wp in LP12a *could* evaluate to true rather than false on this retention pattern. The structural argument that closes LP12b for content-canonical links (route every F-candidate in coverage to a different subspace than the retained V-positions) inverts in the link-canonical case (every F-candidate in coverage shares the retained subspace) and so does not extend. Characterising the wp's exact value for the link-canonical class would require analysing when the link-subspace projection through `d` non-trivially intersects the link-subspace D-SEQ★-prefix `R` — a structural question we leave to future work. We flag the omission rather than silently restricting LP12b's scope, so the asymmetry between content-canonical (wp = false on this retention pattern) and link-canonical (wp value unsettled) is visible to readers.
...
*Non-canonical spans are unconditionally non-tight.* The tightness predicate's domain is *defined* to require canonical form `ℓ = δ(n, #s)` — no evaluation of the universal quantifier is attempted on non-canonical spans, which the predicate rejects by construction. The structural finitude argument that follows is therefore motivational rather than load-bearing for non-canonical decidability: it explains *why* the canonical restriction is the right structural cut, by exhibiting concrete obstructions to discharging the universal quantifier on the non-canonical regime even were one to try. The grounds split into three sub-cases by the relation between `#ℓ` and `#s`. (i) `#ℓ < #s`: LP-Fin's non-canonical remark (above) gives `|F ∩ [s, s ⊕ ℓ)| = ℵ₀` via the within-chain construction, while `dom(Σ_e.C) ∪ dom(Σ_e.L)` is finite at every reachable state (by C-fin and L-fin of ASN-0093) — the universal quantifier cannot be discharged at any state, since there always exist substrate-emittable addresses in the span's reach that no construction discipline could have pre-allocated. (ii) `#ℓ = #s` with `ℓ` not an ordinal displacement (some nonzero component before position `#s`, equivalently `actionPoint(ℓ) < #s`): the within-chain construction extension immediately above establishes `|F ∩ [s, s ⊕ ℓ)| = ℵ₀` uniformly across the sub-case — finitude fails identically to (i). (iii) `#ℓ > #s`: the finitude question is not analysed here and is genuinely unsettled by this ASN; specific structural forms of `ℓ` may admit finite or infinite intersections. The tightness predicate's purposes do not require an answer: because the predicate's definitional canonical-form requirement rejects the entire `#ℓ > #s` regime at the type-level — before any quantifier evaluation is attempted — the predicate's value on such spans is fixed at false by definition, and the question of whether `|F ∩ [s, s ⊕ ℓ)|` is finite or infinite is irrelevant to evaluating the predicate. In grounds (i) and (ii) the finitude failure additionally exhibits that the canonical restriction is structurally necessary for predicate decidability on those sub-ranges; in ground (iii) the definitional rejection alone suffices. No state, and no editing trajectory leading to a state, can render a non-canonical span tight. The distinction between "could be made tight with discipline" and "cannot be tight at any state" is decided by the structural form `ℓ = δ(n, #s)` alone — i.e., by the full canonical form, not merely by `#ℓ = #s` — and not by the contents of the stores.

*Achievability (under canonical-ℓ assumption).* **The analysis below restricts to canonical spans** — those with `ℓ = δ(n, #s)` for some `n ≥ 1`, equivalently `#ℓ = #s` with `ℓ` an ordinal displacement (OrdinalDisplacement, ASN-0034). Non-canonical spans — any `ℓ ≠ δ(n, #s)`, including the `#ℓ < #s`, `#ℓ = #s` non-ordinal, and `#ℓ > #s` cases enumerated above — fall outside the tight-endset domain by the structural non-tightness result just established above, so achievability of tight construction addresses only canonical spans.
...
*Relationship to LP-Fin Corollary.* The four cross-chain interference arguments below — LP19-Achiev-CrossSub-C and LP19-Achiev-CrossSub-L (same-document cross-subspace, both directions), LP19-Achiev-NonNest (non-nesting documents), LP19-Achiev-Desc (descendant documents), LP19-Achiev-Anc (ancestor documents) — are direct consequences of LP-Fin Corollary, which already establishes `F ∩ [s, s ⊕ ℓ) = {[d_0, 0, X, k] : k_s ≤ k < k_s + n}` (every F-candidate in the interval shares the span's subspace identifier `X` and origin `d_0`, so cross-chain candidates contribute nothing). We retain the per-case verification under individual sub-labels for two reasons. First, motivational clarity: each labelled case exhibits the specific T1 divergence position that rules out one structurally distinct family of interfering candidates (`#d_0 + 2` for the two CrossSub cases, `j ≤ min(#d_0, #d')` for NonNest, `#d_0 + 1` for Desc, `#d' + 1` for Anc), and these positional arguments are exactly the per-family sub-arguments that combine to discharge the LP-Fin Corollary's structural exclusion. The individual sub-labels make each per-family argument citable in downstream work without having to refer to the LP-Fin Corollary's collective form. Second, this section additionally establishes tightness against `A_X(d_0)`'s own *future* emissions via the emission-frontier choice `s ⊕ ℓ ≤ inc(t_m^X(d_0), 0)` — a condition that constrains `n` against the chain-index frontier `m` and is not implied by LP-Fin Corollary alone (the corollary characterises interval membership but does not say which of those indices are allocated at `Σ_e`). Readers who accept the corollary may skim the LP19-Achiev-CrossSub-*, LP19-Achiev-NonNest, LP19-Achiev-Desc, and LP19-Achiev-Anc sub-cases as concrete instances of its structural exclusion clause; the within-chain emission-frontier argument carries content beyond the corollary and is load-bearing for achievability.
...
*Each `x_i ≥ 1`.* By F's structural definition (the conjunct `zeros(d) = 2 ∧ d satisfies T4` in F's set-builder formula above) applied to both documents — `d_0` via the canonical-span hypothesis `s ∈ F`, and `d'` via its role as the document parameter of any chain element of `A_C(d')` or `A_L(d')` that could lie in `F ∩ [s, s ⊕ ℓ)` — `d_0` and `d'` are T4-valid with `zeros(d_0) = zeros(d') = 2`. We perform the zero-count decomposition explicitly. By Prefix (ASN-0034) applied to `d_0 ≺ d'`, `d'[i] = d_0[i]` for every `1 ≤ i ≤ #d_0`, so the length-`#d_0` prefix of `d'` is component-wise identical to `d_0`. Counting zeros position-by-position over `d_0`'s positions, `zeros(d'|_{1..#d_0}) = #{i : 1 ≤ i ≤ #d_0 ∧ d'[i] = 0} = #{i : 1 ≤ i ≤ #d_0 ∧ d_0[i] = 0} = zeros(d_0) = 2`. Now split `d'`'s zero count into the prefix-positions (`1..#d_0`) and the extension-positions (`#d_0 + 1, …, #d_0 + q`): `zeros(d') = zeros(d'|_{1..#d_0}) + #{i : 1 ≤ i ≤ q ∧ x_i = 0} = 2 + #{i : 1 ≤ i ≤ q ∧ x_i = 0}`. The constraint `zeros(d') = 2` forces `#{i : 1 ≤ i ≤ q ∧ x_i = 0} = 0`, so every `x_i ≠ 0`. T0's carrier ℕ forces `x_i ≥ 1` from `x_i ≠ 0`. (We do not need the no-adjacent-zeros conjunct of T4 here; the zero-count balance alone suffices.) In particular `x_1 ≥ 1` — the only structural fact the proof below depends on; the values of `x_2, ..., x_q` and `q` itself drop out of the argument.
...
*LP19-Achiev-Anc — Ancestor documents.* Consider documents `d'` standing in a proper prefix relation `d' ≺ d_0`. As in the descendant case, the argument is structural: it depends on the form `d_0` must take given that `d' ≺ d_0` and both are T4-valid documents, not on the specific K.δ chain by which `d_0` was reached from `d'`. (For context: descendants of `d'` are reachable by interleaved `inc(_, 0)` and `inc(_, 1)` chains starting with `inc(d', 1)`, never `inc(_, 2)` once `zeros = 2` is achieved; this is symmetric to the descendant case above.)

By Prefix (ASN-0034) applied to `d' ≺ d_0`, `#d' < #d_0` and `d_{0, i} = d'_i` for `1 ≤ i ≤ #d'`. Set `r := #d_0 - #d' ≥ 1` and write `d_0 = [d', y_1, y_2, …, y_r]` where `y_i := d_{0, #d' + i}`. The zero-count balance argument is the symmetric counterpart of the descendant case, performed explicitly: by F's structural definition applied to both documents (`d_0` via the canonical-span hypothesis `s ∈ F`, `d'` via its role as the document parameter of any chain element of `A_C(d')` or `A_L(d')` that could lie in `F ∩ [s, s ⊕ ℓ)`), `zeros(d') = zeros(d_0) = 2`. The Prefix-derived position-wise agreement `d_{0, i} = d'_i` for `1 ≤ i ≤ #d'` gives `zeros(d_0|_{1..#d'}) = #{i : 1 ≤ i ≤ #d' ∧ d_0[i] = 0} = #{i : 1 ≤ i ≤ #d' ∧ d'[i] = 0} = zeros(d') = 2`. Splitting `d_0`'s zero count into the prefix-positions (`1..#d'`) and the extension-positions (`#d' + 1, …, #d' + r`): `zeros(d_0) = zeros(d_0|_{1..#d'}) + #{i : 1 ≤ i ≤ r ∧ y_i = 0} = 2 + #{i : 1 ≤ i ≤ r ∧ y_i = 0}`. The constraint `zeros(d_0) = 2` forces `#{i : 1 ≤ i ≤ r ∧ y_i = 0} = 0`, so every `y_i ≠ 0`. T0's carrier ℕ forces `y_i ≥ 1` from `y_i ≠ 0`. In particular `y_1 ≥ 1` — the structural fact the proof needs.
...
| LP-Comp | Documentation note: per-step lemmas LP4–LP14 form a covering case-analysis on the operation kinds of the working frame, so any reachable `Σ →* Σ'` decomposes into atomic steps each governed by exactly one of these lemmas. No cumulative-displacement lemma is asserted; LP18 and LP19 carry self-contained proofs. | introduced |
...
| LP-Fin Corollary | Canonical interval characterisation: for canonical `(s, ℓ)` with `s = [d_0, 0, X, k_s]` and `ℓ = δ(n, #s)`, `F ∩ [s, s ⊕ ℓ) = {[d_0, 0, X, k] : k_s ≤ k < k_s + n}` — every F-candidate in the interval inherits the span's subspace identifier `X` and origin `d_0`. Used in LP12b to derive `coverage ∩ dom(L) = ∅` from content-subspace canonical construction (specialisation at `X = s_C`); LP12b in turn discharges LP12a's deferred content-canonical-link-subspace boundary case. The citation chain is LP12a → LP12b → LP-Fin Corollary. | introduced |
| LP19-Achiev-CrossSub-C | Tight achievability sub-case: for span on `A_C(d_0)`, interfering chain elements of `A_L(d_0)` are excluded above `[s, s ⊕ ℓ)` by T1 divergence at position `#d_0 + 2` | introduced |
| LP19-Achiev-CrossSub-L | Tight achievability sub-case: for span on `A_L(d_0)`, interfering chain elements of `A_C(d_0)` are excluded below `[s, s ⊕ ℓ)` by T1 divergence at position `#d_0 + 2` | introduced |
| LP19-Achiev-NonNest | Tight achievability sub-case: chain elements of `A_sub'(d')` for non-nesting `d' ≠ d_0` are excluded from `[s, s ⊕ ℓ)` by T1 divergence at position `j ≤ min(#d_0, #d')` | introduced |
| LP19-Achiev-Desc | Tight achievability sub-case: chain elements of `A_sub'(d')` for descendant `d_0 ≺ d'` are excluded above `[s, s ⊕ ℓ)` by T1 divergence at position `#d_0 + 1` | introduced |
| LP19-Achiev-Anc | Tight achievability sub-case: chain elements of `A_sub'(d')` for ancestor `d' ≺ d_0` are excluded below `[s, s ⊕ ℓ)` by T1 divergence at position `#d' + 1` | introduced |
