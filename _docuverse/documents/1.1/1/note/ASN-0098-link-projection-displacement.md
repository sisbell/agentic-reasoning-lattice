...
`F` is countably infinite — by T0(a) and T0(b) of ASN-0034 the T4-valid document tumblers are unbounded in both component value and length, and each contributes a countably infinite chain in each of the two subspaces. The tightness predicate's universal `(A t ∈ F : s ≤ t < s ⊕ ℓ : …)` therefore ranges over an infinite domain; LP-Fin's finiteness of `F ∩ [s, s ⊕ ℓ)` for a canonical span is the fact that renders that predicate decidable.
...
*Finiteness from the bound — case decomposition on `#d`.* We now show, under the established bound `#d ≤ #d_0`, that `d` must agree with `d_0` on positions `1..#d`. Suppose not: `d` disagrees with `d_0` at some position `j` with `1 ≤ j ≤ #d`, and let `j` be the first such position. Positions `1..j − 1` of `a` (which are `d_1..d_{j-1}`) equal positions `1..j − 1` of both `s` and `s ⊕ ℓ`: they equal `d_{0,1}..d_{0,j-1}` by `s`'s structural form, and the agreement transfers to `s ⊕ ℓ` through TumblerAdd's prefix-copy region, since `j − 1 < j ≤ #d ≤ #d_0 < #s = actionPoint(ℓ)`. At position `j`, `a_j = d_j`, while `s_j = d_{0,j}` (structural form) and `(s ⊕ ℓ)_j = s_j = d_{0,j}` (prefix-copy, as `j ≤ #d_0 < #s`); the disagreement gives `a_j = d_j ≠ d_{0,j} = s_j = (s ⊕ ℓ)_j`. By T1 case (i) at position `j`, either `d_j < d_{0,j}` (yielding `a < s`, contradicting `a ≥ s`) or `d_j > d_{0,j}` (yielding `a > s ⊕ ℓ`, contradicting `a < s ⊕ ℓ`). Contradiction either way. Hence `d` agrees with `d_0` on positions `1..#d`, so `d` is the length-`#d` prefix of `d_0`, uniquely determined by `#d`.
