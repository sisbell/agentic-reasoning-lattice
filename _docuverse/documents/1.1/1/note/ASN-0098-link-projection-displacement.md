...
*Non-canonical spans are unconditionally non-tight.* For spans satisfying T12 but with `#ℓ < #s`, LP-Fin's non-canonical remark (below) gives `|F ∩ [s, s ⊕ ℓ)| = ℵ₀`, while `dom(Σ_e.C) ∪ dom(Σ_e.L)` is finite at every reachable state (by C-fin and L-fin of ASN-0093). The universal quantifier therefore cannot be discharged at any state — there always exist substrate-emittable addresses in the span's reach that no construction discipline could have pre-allocated. No state, and no editing trajectory leading to a state, can render a non-canonical span tight. The distinction between "could be made tight with discipline" and "cannot be tight at any state" is decided by the structural form `#ℓ = #s` alone, not by the contents of the stores.
