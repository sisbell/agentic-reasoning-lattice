...
**LP19 — TightEndsetBoundaryExclusion**: For any endset `e` tight at `Σ_e`, any reachable state sequence `Σ_e →* Σ_n` whose prefix contains a K.α (or K.λ) transition allocating a fresh address `a_new`, and any K.μ⁺ (or K.μ⁺_L) transition `Σ_n → Σ_{n+1}` operating on `d` — and for each specific newly-added mapping `(v_new, a_new) ∈ dom(Σ_{n+1}.M(d)) ∖ dom(Σ_n.M(d))` in that transition whose I-address `a_new` is the address freshly allocated by the K.α (or K.λ) step on the prefix `Σ_e →* Σ_n`:
```
v_new ∉ project(e, d, Σ_{n+1})
```

The lemma applies *per pair* `(v_new, a_new)`. ASN-0047's K.μ⁺ may add several mappings `{v_1, …, v_k}` in a single step (the operation's precondition explicitly names this set as the newly added V-positions), and the lemma's conclusion is asserted only for those specific pairs whose I-address was K.α/K.λ-allocated on the prefix; pairs added at the same step whose I-address is a pre-existing (e.g., transcluded) address fall outside this lemma's hypothesis and are governed by LP9's general growth analysis. The K.α (or K.λ) step that allocated `a_new` lies on the prefix `Σ_e →* Σ_n`, so LP19a applied at that step yields `a_new ∉ coverage(e)`. Since `coverage(e)` is a deterministic function of `e`'s spans (per the coverage definition of ASN-0043) and `e` is a fixed endset value across the entire sequence — `coverage` consults no state component — the membership `a_new ∉ coverage(e)` carries through unchanged to `Σ_{n+1}`. The K.μ⁺ (or K.μ⁺_L) transition `Σ_n → Σ_{n+1}` adds the mapping `(v_new, a_new)`, giving `v_new ∈ dom(Σ_{n+1}.M(d))` and `Σ_{n+1}.M(d)(v_new) = a_new ∉ coverage(e)`. The projection definition then excludes `v_new` from `project(e, d, Σ_{n+1})`. Other mappings `(v_j, a_j)` added in the same K.μ⁺ step with `a_j ≠ a_new` and `a_j ∈ dom(Σ_n.C) ∪ dom(Σ_n.L)` (the transclusion case) are not covered by LP19; their interaction with `project(e, d, ·)` is governed by LP9's general characterisation, which admits growth when `a_j ∈ coverage(e)`.
