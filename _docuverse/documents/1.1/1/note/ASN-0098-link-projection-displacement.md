...
*Working reference frame.* This ASN operates in the ASN-0047 transition-model frame layered over the ASN-0093 allocation substrate. The layered frame supplies the full operation vocabulary the projection responds to — K.σ (ASN-0093) together with K.α, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.μ~, K.δ, K.ρ (ASN-0047). The extended-state invariants this ASN consumes — S3★ (GeneralizedReferentialIntegrity), S3★-aux (SubspaceExhaustiveness), and the per-subspace amendments D-CTG★, D-MIN★, D-SEQ★ — are cited at the proof steps that use them.
...
*Remark on K.δ.* ASN-0047 includes K.δ as a unified entity-creation operation spanning nodes, accounts, and documents. The K.δ-IsNode and K.δ-IsAccount cases have frame `(A d :: M'(d) = M(d))` — no arrangement is modified — so LP4 applied to every `d ∈ dom(Σ.M) = dom(Σ'.M)` yields projection invariance. The K.δ-IsDocument case is covered directly by LP8 above, whose hypothesis explicitly admits both K.σ and K.δ-IsDocument as document-registration operations. Each K.δ kind therefore reduces to either LP4 or LP8.
...
*Scope restriction — link-canonical companion case is OUT_OF_SCOPE.* LP12b addresses only the **content-canonical** class of links — those whose every span is canonical with `s = [d_s, 0, s_C, k_s]`. The symmetric **link-canonical** class — every span canonical with `s = [d_s, 0, s_L, k_s]`, under the same retention pattern `n'_{s_C} = 0, n'_{s_L} > 0` — is explicitly *out of scope* for this ASN. The case is not artificial: L4(c) of ASN-0043 admits link-subspace endsets (endsets whose spans reference addresses of other links). For such a link, LP-Fin Corollary applied at `X = s_L` gives `F ∩ [s, s ⊕ ℓ) = {[d_s, 0, s_L, k] : k_s ≤ k < k_s + n}` for each canonical span, an interval whose F-candidates all carry subspace identifier `s_L`. By L0 of ASN-0093, every element of `dom(L)` carries `s_L`, so this F-interval is *not* disjoint from `dom(L)` — it may intersect coverage with link-subspace addresses, and the projection may then contain link-subspace V-positions (by S3★, link-subspace V-positions in the arrangement map to `dom(L)`). The retention set `R ⊆ V_{s_L}(d)` admits non-empty intersection with such a projection, so the wp in LP12a *could* evaluate to true rather than false on this retention pattern. The structural argument that closes LP12b for content-canonical links (route every F-candidate in coverage to a different subspace than the retained V-positions) inverts in the link-canonical case (every F-candidate in coverage shares the retained subspace) and so does not extend. Characterising the wp's exact value for the link-canonical class would require analysing when the link-subspace projection through `d` non-trivially intersects the link-subspace D-SEQ★-prefix `R` — a structural question we leave to future work.

*Non-canonical spans yield infinite intersections.* The finitude conclusion of LP-Fin rests on the canonical action-point identification `actionPoint(ℓ) = #s` (OrdinalDisplacement, ASN-0034). Whenever a span is non-canonical with `actionPoint(ℓ) < #s` — which covers both `#ℓ < #s` (there `actionPoint(ℓ) ≤ #ℓ < #s` by ActionPoint) and `#ℓ = #s` with `ℓ` not an ordinal displacement (there `actionPoint(ℓ) < #s` by definition of non-ordinal) — the chain `A_X(d_0)` itself, same document and subspace as `s`, supplies infinitely many F-candidates in `[s, s ⊕ ℓ)`. We give the construction once, parametric in `k_ℓ := actionPoint(ℓ)`; a single divergence at position `k_ℓ` discharges both sub-ranges. The descendant trap of LP-Fin's positive proof (sub-case (i) above) — the contradiction at position `#d_0 + 1` — is not invoked; the argument uses only within-chain elements and needs no descendant candidates.

By ActionPoint (ASN-0034), `1 ≤ k_ℓ ≤ #ℓ`, and the non-canonical hypothesis gives `k_ℓ < #s = #d_0 + 3`. Writing `X ∈ {s_C, s_L}` for the subspace component of `s` (so `s = [d_0, 0, X, k_s]`), the chain `A_X(d_0)` has elements `t_k^X(d_0) = [d_0, 0, X, k]` for `k ≥ 1`, each of length `#s` and each agreeing with `s` on positions `1..#s − 1` by structural form (they differ from `s` only at position `#s`). Fix `k ≥ k_s` and compare `t_k^X(d_0)` with `s ⊕ ℓ`. *Upper bound:* the T1 case (i) comparison range is `≤ min(#t_k^X(d_0), #(s ⊕ ℓ)) = min(#s, #ℓ)`, and `k_ℓ ≤ min(#s, #ℓ)` since `k_ℓ ≤ #ℓ` and `k_ℓ < #s`. At positions `1..k_ℓ − 1`, TumblerAdd's prefix-copy region gives `(s ⊕ ℓ)_i = s_i` and structural agreement gives `t_k^X(d_0)_i = s_i`, so the two agree. At position `k_ℓ < #s`, TumblerAdd's action-point step gives `(s ⊕ ℓ)_{k_ℓ} = s_{k_ℓ} + ℓ_{k_ℓ}` with `ℓ_{k_ℓ} ≥ 1` (ActionPoint), while structural agreement gives `t_k^X(d_0)_{k_ℓ} = s_{k_ℓ}`; hence `t_k^X(d_0)_{k_ℓ} < (s ⊕ ℓ)_{k_ℓ}`. By T1 case (i) at divergence position `k_ℓ`, `t_k^X(d_0) < s ⊕ ℓ` — the order is decided at `k_ℓ`, so positions beyond `k_ℓ` (including the last position `#s`, when it lies in range under `#ℓ = #s`) need no consultation. *Lower bound:* `t_k^X(d_0) ≥ s`, with equality at `k = k_s` by structural identity (`t_{k_s}^X(d_0) = [d_0, 0, X, k_s] = s`) and strict inequality at `k > k_s` by ChainEnumerationInjectivity (ASN-0093). Hence `{t_k^X(d_0) : k ≥ k_s} ⊆ F ∩ [s, s ⊕ ℓ)`, an infinite subset within a single sub-allocator chain, so `|F ∩ [s, s ⊕ ℓ)| = ℵ₀` for every non-canonical span with `actionPoint(ℓ) < #s`.

Concrete witness: take `s = [1, 0, 1, 0, 1, 0, 1, 1]` (so `#s = 8`, `d_0 = [1, 0, 1, 0, 1]`, `zeros(d_0) = 2`, `X = 1`, `k_s = 1`) and `ℓ = [0, 0, 0, 1]` (so `#ℓ = 4` and `actionPoint(ℓ) = 4 ≤ #s = 8`, satisfying T12). Then `s ⊕ ℓ = [1, 0, 1, 1]` (length 4, via TumblerAdd's prefix-copy on positions 1–3 and sum at position 4: `0 + 1 = 1`). For each `k ≥ 1`, the chain element `t_k^1(d_0) = [1, 0, 1, 0, 1, 0, 1, k]` is in F by direct inspection: `zeros = 3` (the two zeros of `d_0` at positions 2 and 4, plus the separator at position 6), no adjacent zeros (each separator is bordered by nonzero components), `t_k^1(d_0)_1 = 1 ≠ 0`, and `t_k^1(d_0)_8 = k ≥ 1 ≠ 0`. Verification that `t_k^1(d_0) ∈ [s, s ⊕ ℓ)`: at position 4 (the action point of `ℓ`), `t_k^1(d_0)_4 = 0 < 1 = (s ⊕ ℓ)_4`, with positions 1–3 agreeing, so `t_k^1(d_0) < s ⊕ ℓ` by T1 case (i); at position 8, `t_k^1(d_0)_8 = k ≥ 1 = s_8`, with positions 1–7 agreeing, so `t_k^1(d_0) ≥ s` (equality at `k = 1`, strict above) by T1 case (i). Hence `{t_k^1(d_0) : k ≥ 1} ⊆ F ∩ [s, s ⊕ ℓ)`, an infinite set within a single sub-allocator chain — an instance of the parametric construction above at `k_ℓ = 4`.
...
*Non-canonical spans are unconditionally non-tight.* The tightness predicate's domain is *defined* to require canonical form `ℓ = δ(n, #s)`; any non-canonical span is rejected at the type level and fixed at false before any quantifier evaluation, so no state and no editing trajectory leading to a state can render a non-canonical span tight. For the two sub-ranges with `actionPoint(ℓ) < #s` — `#ℓ < #s`, and `#ℓ = #s` with `ℓ` not an ordinal displacement — the within-chain construction above independently establishes `|F ∩ [s, s ⊕ ℓ)| = ℵ₀`, so finitude (and hence decidability by enumeration) fails there even setting the definitional rejection aside, while `dom(Σ_e.C) ∪ dom(Σ_e.L)` is finite at every reachable state (C-fin, L-fin of ASN-0093). The remaining sub-range, (iii) `#ℓ > #s`, is genuinely unsettled by this ASN: specific structural forms of `ℓ` may admit finite or infinite intersections. The tightness predicate's purposes do not require an answer — the definitional canonical-form requirement rejects the entire `#ℓ > #s` regime at the type level, before any quantifier evaluation, so the predicate's value is fixed at false regardless of whether `|F ∩ [s, s ⊕ ℓ)|` is finite or infinite. The distinction between "could be made tight with discipline" and "cannot be tight at any state" is decided by the full canonical form `ℓ = δ(n, #s)` alone — not merely by `#ℓ = #s` — and not by the contents of the stores.
