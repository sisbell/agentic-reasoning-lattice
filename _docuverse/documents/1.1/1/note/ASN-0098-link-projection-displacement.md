...
*Ancestor documents.* The third case is documents `d'` standing in a proper prefix relation `d' ≺ d_0`. By the K.δ rules for document allocation (symmetric to the descendant case), `d_0` was obtained from `d'` by a chain of `inc(_, 1)` version steps, each appending a single non-zero component (by TA5(d) of ASN-0034). Concretely, `d_0 = [d'_1, …, d'_{#d'}, y_1, y_2, …, y_r]` with `r ≥ 1` and each `y_i ≥ 1`. Chain elements `b` of `A_C(d')` (resp. `A_L(d')`) have the form `[d'_1, …, d'_{#d'}, 0, s_C, k]` (resp. with `s_L`) for `k ≥ 1`, so `#b = #d' + 3 < #d_0 + 3 = #s`. Positions `1..#d'` agree between `b` and `s` (both carry `d'` as a common prefix — `b` by the chain structure of `A_sub'(d')`, `s` because it carries `d_0` as prefix and `d_0` extends `d'`). At the divergence position `#d' + 1`: `b_{#d' + 1} = 0` (the separator immediately after `d'`'s prefix in any chain element of `A_sub'(d')`), while `s_{#d' + 1} = d_{0, #d' + 1} = y_1 ≥ 1`. The image `s ⊕ ℓ` agrees with `s` at position `#d' + 1`, since `ℓ` has action point `k_ℓ = #s > #d' + 1` so this position falls in TumblerAdd's prefix-copy region where `(s ⊕ ℓ)_{#d' + 1} = s_{#d' + 1}`. By T1 case (i) of ASN-0034 at the divergence position `#d' + 1` with `b_{#d'+1} = 0 < 1 ≤ y_1 = s_{#d'+1} = (s ⊕ ℓ)_{#d'+1}`, we have `b < s`, so `b` lies below the interval `[s, s ⊕ ℓ)`. This argument extends inductively to ancestors of `d'` (further back along the version chain that produced `d_0`): for any `d'' ≺ d'`, chain elements of `A_sub''(d'')` have a separator zero at position `#d'' + 1` while `s` has a non-zero version-step component there, yielding the same T1 case (i) inequality. Tightness is therefore preserved against every ancestor at every depth in the version sub-allocator tree from which `d_0` descends.
