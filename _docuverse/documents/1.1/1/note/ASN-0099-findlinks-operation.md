...
*Any other refinement.* The general clause is the contrapositive of F1's structural minimality: if a candidate predicate `P` fails on any `(eᵢ, I)` pair whose intersection (under F1's coverage-and-overlap form) is a singleton, then `P` excludes some link that F1 includes, since F1's predicate is satisfied by exactly that singleton-intersection instance. The three enumerated cases are particular shapes of this general phenomenon — each names a specific way to fail on a singleton-intersection pair — and any further refinement is governed by the same witness pattern, realized through the canonical-span construction used in the three cases above.
...
- `d_a` is a content-bearing document. Its content sub-allocator `A_C(d_a)` (ASN-0093) has produced three I-addresses: `α₁ = [d_a.0.s_C.1]`, `α₂ = [d_a.0.s_C.2]`, `α₃ = [d_a.0.s_C.3]`, each placed into `dom(Σ.C)` by successive K.α steps with values `v₁, v₂, v₃ ∈ Val`. Its arrangement, by D-SEQ★ (ASN-0047), is `Σ.M(d_a) = {v_a^1 ↦ α₁, v_a^2 ↦ α₂, v_a^3 ↦ α₃}`, where `v_a^k = [s_C, 1, ..., 1, k]` is the canonical depth-`m_C` text-subspace V-position.
