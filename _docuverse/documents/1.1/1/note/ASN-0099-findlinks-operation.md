...
`R` is unconstrained beyond `R ⊆ T` — single position, contiguous V-span, or any subset. When `R` is a contiguous V-span in subspace `s_C`, ASN-0058's mapping-block decomposition gives the image as a union of I-runs, one per maximal correspondence run. The image of the span is the union of the I-extents of the mapping blocks whose V-extents meet `R`, each restricted to `V(βⱼ) ∩ R` — these restricted V-extents partition `R ∩ dom(Σ.M(d))` (B1+B2, ASN-0058); the corresponding I-extents may coincide where content is shared (S5, ASN-0036; M13/M14, ASN-0058), and the set union absorbs any overlap. When `v ∈ R` has `subspace(v) = s_L`, S3★ (ASN-0047) routes `Σ.M(d)(v) ∈ dom(Σ.L)` and the image picks up a link address. The match predicate accepts this without modification: endsets may reference any addresses in `T` (L4, ASN-0043), so the link subspace is admissible as a coverage target.
