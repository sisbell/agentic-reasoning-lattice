...
F4 (PartialOverlapSuffices):
   (A a ∈ dom(Σ.L), i : 1 ≤ i ≤ |Σ.L(a)| :
       coverage(Σ.L(a).eᵢ) ∩ I ≠ ∅ ⟹ matches(a, I, Σ)).
```

This is immediate from the definition of `matches`: the slot-existential in F1 is witnessed by the very `i` whose coverage meets `I`, so the implication discharges by F1's definitional content. We name F4 as a separate claim because the consequence — that the implementation cannot require full overlap, near-overlap, majority overlap, or any other strengthened condition — is a load-bearing design choice that propagates through the rest of the operation's semantics.
...
Operation effects/frames in this specification family are stated by ASN-0093 and ASN-0047 in side-by-side fashion: an *effect* clause names what changes, and a *frame* clause names what does not. ASN-0093's operation specifications list every preserved component explicitly — K.σ, for instance, ends its frame with `L' = L`. Several operations in ASN-0047 omit `L` from their published frames: K.μ⁺ (its frame names only `C`, `E`, `R`, and per-document arrangement), K.μ⁻ (likewise), and K.ρ (whose frame names only `C`, `E`, and per-document arrangement). We therefore assert as a structural axiom:

```
A1 (EffectClauseExhaustivity):
   Premise (vocabulary closure): The operation vocabulary of the substrate
   is exactly the published set V = {K.σ, K.α, K.λ, K.δ, K.μ⁺, K.μ⁻,
   K.μ~, K.μ⁺_L, K.ρ} as enumerated across ASN-0047 and ASN-0093 at the
   time this ASN was written. No operation outside V exists in the
   substrate.

   Conclusion: For every operation in V, the operation's stated effect
   clause names every state-component modification the operation makes.
   Equivalently: any state component not appearing in either the effect
   clause or the frame clause is unchanged across the transition.
```

A1 is a meta-axiom about the published specifications: it asserts that the effect clauses as written are *exhaustive* with respect to the modifications each operation performs. Its validity rests on the closure premise above; if the substrate vocabulary is later extended, every new operation must publish its `L`-behaviour explicitly for A1 to remain applicable. ASN-0093's K.σ frame lists `L' = L` directly, demonstrating that authors do name `L` in frame clauses when they track the dependency; ASN-0047's omission for K.μ⁺, K.μ⁻, and K.ρ is a documentation gap that an ASN-0047 revision adding `L' = L` to those frames would close, eliminating A1's load-bearing role in this ASN's derivations.

*A1 dependency surface.* A1 is load-bearing for exactly three operations of V: K.μ⁺, K.μ⁻, and K.ρ. F9's derivation invokes A1 at the K.μ⁺ and K.μ⁻ cases; the K.ρ case enters via the unified corollary to F9 (below) and is reused for K.ρ in any composite that records provenance. Query 4 of the Worked Example exercises K.μ⁻. Outside these uses A1 does not propagate into the rest of this ASN.
...
**Implicit verifications in Queries 1–3 (F4, F7(a)).** Queries 1–3 each rely on the fact that any non-empty slot-coverage / I-set intersection forces a match (F4): in Query 1, slot 1's coverage at `ℓ` meets `{α₂}` in the singleton `{α₂}`, and that singleton overlap suffices to put `ℓ` in the result without examining slot 2 or slot 3. Queries 1–3 also exercise F7(a)'s slot symmetry: the match predicate's existential ranges over every slot of every link uniformly, so when Query 3 finds `ℓ'` via its slot 1 (whose coverage extends `α₃`), no slot is privileged over any other — the same uniformity that lets Query 1 find `ℓ` via slot 1 alone. Both observations are intrinsic to the existential structure of `matches` and require no separate verification step.
