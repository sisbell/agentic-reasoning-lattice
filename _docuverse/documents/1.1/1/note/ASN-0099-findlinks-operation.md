...
F1's slot-existential together with its intersection (rather than containment) form is exactly the minimal sufficient match condition: any non-empty intersection `coverage(Σ.L(a).eᵢ) ∩ I ≠ ∅` at any slot `i` already witnesses the existential and forces `matches(a, I, Σ)` to hold. This is not an auxiliary claim — it is the immediate reading of F1's definition under existential introduction. We surface it here as a *design constraint on conforming implementations*: the match cannot require full overlap, near-overlap, majority overlap, or any other strengthened condition on either side. A predicate that demanded, say, `coverage(Σ.L(a).eᵢ) ⊆ I` or `|coverage(Σ.L(a).eᵢ) ∩ I| ≥ k` for any fixed threshold `k > 1` would not be a refinement of F1 — it would be a different match predicate, returning a different (and incomplete) result set. The reader's promise rests on the singleton-overlap reading: a link is about every byte its endset names, and one shared byte is one shared byte.
...
A1 (EffectClauseExhaustivity):
   Vocabulary closure (scope of the contract): The operation vocabulary
   of the substrate is exactly the published set V = {K.σ, K.α, K.λ,
   K.δ, K.μ⁺, K.μ⁻, K.μ~, K.μ⁺_L, K.ρ} as enumerated across ASN-0047 and
   ASN-0093 at the time this ASN was written. No operation outside V
