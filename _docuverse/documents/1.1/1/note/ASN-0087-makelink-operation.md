...
For the V-arrangement entry `v_ℓ ↦ ℓ`:
  ...
  D-CTG★:   extension is contiguous                               discharged below by the interior-component argument over the full depth-m_L slice, not merely read off the last component
...
For D-CTG★: the D-SEQ★ set computed above exhibits contiguity only in the *last* component, whereas D-CTG★ (ASN-0047) quantifies over the *full* depth-`m_L^{Σ'}(d)`, subspace-`s_L` slice — every positive-component tuple `z` with `v_lo ≤ z ≤ v_hi` (extremes drawn from `V_{s_L}^{Σ'}(d)`) must be a member. MAKELINK commits the canonical depth `m_L^{Σ'}(d) = 2` for every first link it places (M-DepthConv), and S8-depth then pins `m_L(d) = 2` for all later link V-positions of `d`; at that depth the slice is exactly `{[s_L, k]}` and last-component contiguity is the whole claim. We nonetheless discharge the interior-component step for arbitrary `m_L^{Σ'}(d) ≥ 2`, keeping the argument independent of how `m_L(d)` was established. The post-state set is `V_{s_L}^{Σ'}(d) = {[s_L, 1, ..., 1, k] : 1 ≤ k ≤ K}` with `K = 1` (empty case) or `K = n_L + 1` (non-empty case), of common depth `m_L^{Σ'}(d)`, so its extremes are `v_lo = [s_L, 1, ..., 1, 1]` and `v_hi = [s_L, 1, ..., 1, K]`. Let `z = [s_L, z₂, ..., z_{m_L}]` be any slice tuple (all components `> 0`) with `v_lo ≤ z ≤ v_hi`. Suppose `z_j > 1` for some interior position `2 ≤ j ≤ m_L^{Σ'}(d) − 1`, and take the least such `j`. Then `z` agrees with `v_hi` at positions `1` (both `s_L`) through `j − 1` (both `1`, by minimality of `j` and `v_hi`'s interior `1`s), and at position `j` has `z_j > 1 = (v_hi)_j`; by T1 case (i) this gives `z > v_hi`, contradicting `z ≤ v_hi`. Hence every interior component of `z` equals `1`, so `z = [s_L, 1, ..., 1, k]` for some `k`; the bound `v_lo ≤ z ≤ v_hi` then forces `1 ≤ k ≤ K`, making `z ∈ V_{s_L}^{Σ'}(d)`. This is the D-CTG★ contiguity conjunct over the full slice, at every reachable `m_L^{Σ'}(d)`. ✓
