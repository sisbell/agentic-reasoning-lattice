...
*Canonical link-subspace depth (M-DepthConv).* When `V_{s_L}(d) = ∅`, the substrate operation K.μ⁺_L (ASN-0047) admits *any* `m ≥ 2` for the first link's V-position via `ValidFirstLinkPosition(d, v_ℓ, m)`; the state `Σ` does not determine `m` (m_L(d), ASN-0047, is well-defined only while `V_{s_L}(d) ≠ ∅`). MAKELINK therefore commits, normatively, to the *minimal admissible* depth `m = 2` for the first link of every document. This is not an arbitrary tie-break: it is the canonical link-subspace depth of the design. Nelson fixes the first link at serial position 1 within the document's `0.2` link subspace — a local V-position of the form `[s_L, serial]`, i.e. depth 2 — placed in permanent order of arrival; the depth is structurally determined, not chosen per document. Gregory's `findnextlinkvsa` (do2.c:151–167) realizes exactly this, assembling the first-link position `2.1` from bare integer literals with no per-document variation. Once the first link is placed at depth 2, S8-depth (ASN-0047) pins `m_L(d) = 2` for all later link V-positions, so every subsequent `v_ℓ` *is* fully state-determined.
...
We must be precise about where the depth comes from. The substrate does *not* fix it for the *first* link: when `V_{s_L}(d) = ∅`, K.μ⁺_L's `ValidFirstLinkPosition(d, v_ℓ, m)` admits any `m ≥ 2`, and `m_L(d)` (ASN-0047) is undefined while the link subspace is empty — so the depth is *not* recoverable from `Σ` in this boundary case. MAKELINK closes the gap by convention (M-DepthConv): it commits to the canonical minimal depth `m = 2` for every document's first link. Once that first link is placed, S8-depth (ASN-0047) pins `m_L(d) = 2` for all later link V-positions. By D-SEQ★ (ASN-0047), `V_{s_L}(d) = {[s_L, 1, ..., 1, k] : 1 ≤ k ≤ n_L}` of common depth `m_L(d)` when non-empty (with `n_L = |V_{s_L}(d)|`), so the non-empty case yields `v_ℓ = shift(max(V_{s_L}(d)), 1) = [s_L, 1, ..., 1, n_L + 1]` at that same depth `m_L(d)`.

*The depth is universally 2.* We can close the residual freedom rather than hedge against it. The depth recorded by S8-depth for `d` is the depth at which `d`'s *first* link V-position was placed, so the question reduces to: by what operation can a document's first link-subspace V-position be seeded, and at what depth? In the substrate vocabulary (ASN-0047) exactly one operation writes a link-subspace V-position: K.μ⁺_L. Its sibling K.μ⁺ is amended to the content-subspace restriction `subspace(v) = s_C` (ASN-0047, ContentSubspaceRestriction), so K.μ⁺ cannot place any `v` with `subspace(v) = s_L`. The fork composite J4/ForkComposite (ASN-0047) populates `M(d_new)` solely through the order-preserving bijection `φ : V_{s_C}(d_op) → V_{s_C}(d_new)` over the *content* subspace, never seeding a link V-position; a freshly forked version therefore begins with `V_{s_L}(d_new) = ∅`. No other transition kind touches `M(d)` in the link subspace. Hence every document's first link V-position is placed by a K.μ⁺_L step, and within this ASN K.μ⁺_L is invoked only as the second half of MAKELINK — which fixes that placement at `m = 2` by M-DepthConv. Consequently `m_L(d) = 2` holds for *every* document whose link subspace is non-empty: the universal claim of M-DepthConv is exact, not merely typical. We therefore drop the earlier "regardless of its value" hedge; the depth is always 2, and `v_ℓ = shift(max(V_{s_L}(d)), 1) = [s_L, 1, ..., 1, n_L + 1]` of depth 2 in the non-empty case. (Implementation observation, not an abstract claim: Gregory's udanax-green admits two convention-violating paths — INSERT via `acceptablevsa` and REARRANGE pivot arithmetic — that can deposit *non-link* content at a `2.x` V-address. These are subspace-convention violations, not link placements, and they fall outside ASN-0047's abstract vocabulary, where K.μ⁺ is restricted to `s_C`. They do not place a proper link orgl entry and so cannot seed `m_L(d) ≠ 2` for any link; the abstract universal claim is unaffected.) Thus for every document the caller never supplies `v_ℓ`: in the non-empty case it is computed from `Σ` (the link subspace's current cardinality at the recorded depth `m_L(d) = 2`), and in the empty (first-link) case its serial component is computed from `Σ` (cardinality 0, giving serial 1) while its depth is supplied by M-DepthConv rather than by `Σ`.
...
| M-DepthConv | Normative convention: MAKELINK fixes the first link's V-position depth at the canonical minimal `m = 2`. When `V_{s_L}(d) = ∅`, the substrate K.μ⁺_L admits any `m ≥ 2` and `Σ` does not determine `m`; MAKELINK supplies `m = 2`. Once placed, S8-depth pins `m_L(d) = 2` for all subsequent link V-positions of `d`. The convention is *universal*, not merely typical: K.μ⁺_L is the sole substrate operation that seeds a link-subspace V-position (K.μ⁺ is restricted to `s_C`; J4/ForkComposite copies only `V_{s_C}`), and within this ASN K.μ⁺_L is invoked only as MAKELINK's second half — so every document's first link is placed at depth 2, giving `m_L(d) = 2` for every document with `V_{s_L}(d) ≠ ∅`. Grounded in Nelson's canonical link-subspace structure (`version.0.2.serial`, depth 2) and Gregory's `findnextlinkvsa` (first link hardcoded at `2.1`). | introduced |
