...
- *Prior entries:* K.λ's frame on `L` preserves every prior `ℓ' ∈ dom(Σ.L)` with `Σ_mid.L(ℓ') = Σ.L(ℓ')`, so all per-entry invariants (L0, L1, L1a, L1b, L1c, L3) on prior entries hold at `Σ_mid` by inheritance from `Σ`.
