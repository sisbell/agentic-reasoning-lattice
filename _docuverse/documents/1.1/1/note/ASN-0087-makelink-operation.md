...
*Canonical link-subspace depth (M-DepthConv).* When `V_{s_L}(d) = ∅`, the substrate operation K.μ⁺_L (ASN-0047) admits *any* `m ≥ 2` for the first link's V-position via `ValidFirstLinkPosition(d, v_ℓ, m)`; the state `Σ` does not determine `m` (m_L(d), ASN-0047, is well-defined only while `V_{s_L}(d) ≠ ∅`). MAKELINK therefore commits, normatively, to the *minimal admissible* depth `m = 2` for every first link *it* places. This is not an arbitrary tie-break: it is the canonical link-subspace depth of the standard creation path. Nelson fixes the first link at serial position 1 within the document's `0.2` link subspace — a local V-position of the form `[s_L, serial]`, i.e. depth 2 — placed in permanent order of arrival; the depth is the canonical placement of that path, not chosen per document. Nelson does *not*, however, declare depth 2 a closed invariant: he explicitly anticipates "the subdivision of links by further digits (after '2' and the position)", i.e. link addresses deeper than depth 2, so M-DepthConv records MAKELINK's normative commitment rather than a structural prohibition on other depths. Gregory's `findnextlinkvsa` (do2.c:151–167) realizes the canonical commitment exactly, assembling the first-link position `2.1` from bare integer literals with no per-document variation. Once MAKELINK has placed the first link at depth 2, S8-depth (ASN-0047) pins `m_L(d) = 2` for all later link V-positions of that document, so every subsequent `v_ℓ` MAKELINK places *is* fully state-determined.

*Notation convention — `dom(M)` and `E_doc`.* ASN-0093 uses `dom(M)` and ASN-0047 uses `E_doc` for the set of allocated documents. These are not two distinct notions to be reconciled by a hypothetical future ASN: ASN-0047's M1 (ArrangementMonotonicity) establishes the identification directly, constraining the document set as `dom(M) = E_doc` (the allocated documents, which grow only via K.δ's `Document(e)` case). Since MAKELINK operates in ASN-0047's combined state — it composes K.λ and K.μ⁺_L and references `Σ.E`, `Σ.R` — the equality `dom(M) = E_doc` holds here by M1. We use `dom(M)` throughout, and discharge ASN-0047 preconditions stated against `E_doc` (notably K.μ⁺_L's `d ∈ E_doc`) from membership in `dom(M)` by this M1 equality. The discharge is complete: it rests on the foundation, not on an assumption.
...
We must be precise about where the depth comes from. The substrate does *not* fix it for the *first* link: when `V_{s_L}(d) = ∅`, K.μ⁺_L's `ValidFirstLinkPosition(d, v_ℓ, m)` admits any `m ≥ 2`, and `m_L(d)` (ASN-0047) is undefined while the link subspace is empty — so the depth is *not* recoverable from `Σ` in this boundary case. MAKELINK closes the gap by convention (M-DepthConv): it commits to the canonical minimal depth `m = 2` for the first link *it* places. Once MAKELINK has placed that first link, S8-depth (ASN-0047) pins `m_L(d) = 2` for all later link V-positions of `d`. By D-SEQ★ (ASN-0047), `V_{s_L}(d) = {[s_L, 1, ..., 1, k] : 1 ≤ k ≤ n_L}` of common depth `m_L(d)` when non-empty (with `n_L = |V_{s_L}(d)|`), so the non-empty case yields `v_ℓ = shift(max(V_{s_L}(d)), 1) = [s_L, 1, ..., 1, n_L + 1]` at that same depth `m_L(d)`.

*The depth is 2 for every link MAKELINK places.* We can pin the depth down precisely, but only within the scope its premises license — we must not overreach to a system-wide universal. The depth recorded by S8-depth for `d` is the depth at which `d`'s *first* link V-position was placed, so the question reduces to: by what operation is a document's first link-subspace V-position seeded, and at what depth? In the substrate vocabulary (ASN-0047) exactly one operation writes a link-subspace V-position: K.μ⁺_L. Its sibling K.μ⁺ is amended to the content-subspace restriction `subspace(v) = s_C` (ASN-0047, ContentSubspaceRestriction), so K.μ⁺ cannot place any `v` with `subspace(v) = s_L`. The fork composite J4/ForkComposite (ASN-0047) populates `M(d_new)` solely through the order-preserving bijection `φ : V_{s_C}(d_op) → V_{s_C}(d_new)` over the *content* subspace, never seeding a link V-position; a freshly forked version therefore begins with `V_{s_L}(d_new) = ∅`. No other transition kind touches `M(d)` in the link subspace. Hence every document's first link V-position is placed by *some* K.μ⁺_L step — and whenever that step is the second half of a MAKELINK invocation, M-DepthConv fixes the placement at `m = 2`.

We claim no more than this. K.μ⁺_L is a standalone substrate primitive (ASN-0047): its `ValidFirstLinkPosition(d, v_ℓ, m)` admits *any* `m ≥ 2`, and it may be invoked independently of MAKELINK — by an operation in another ASN, or by a direct substrate call. Nothing in this self-contained ASN, nor in the cited foundations, establishes that every link-subspace V-position in every execution is placed by MAKELINK; so we cannot license the leap from "MAKELINK is the only K.μ⁺_L caller *within this ASN*" to "`m_L(d) = 2` for every document." Nelson's design confirms this is deliberate latitude, not an oversight: he fixes the *canonical* link address at depth 2 (`version.0.2.serial`, in permanent order of arrival) but explicitly anticipates "the subdivision of links by further digits (after '2' and the position)" — i.e. link addresses deeper than depth 2 are contemplated by design. Depth 2 is therefore the canonical placement of the standard link-creation path, not a system-wide invariant pinning every link at depth 2. Accordingly we *retain* the general `m_L(d)` reading downstream and assert only the scoped universal: for any document `d` whose every link V-position was placed by MAKELINK, `m_L(d) = 2`, and in that case `v_ℓ = shift(max(V_{s_L}(d)), 1) = [s_L, 1, ..., 1, n_L + 1]` of depth 2 in the non-empty case. (Implementation observation, not an abstract claim: Gregory's `findnextlinkvsa` — the standard link-creation path used by `domakelink`/`docreatelink` — hardcodes the first-link position at `2.1`, with no per-document variation. But udanax-green also admits paths that bypass `findnextlinkvsa`: INSERT via the unconditional `acceptablevsa` stub and REARRANGE pivot arithmetic can deposit *non-link* content at a `2.x` V-address, and the FEBE `copy` command can place a caller-supplied I-span at *any* V-depth, because `acceptablevsa` performs no depth check. These are convention violations, not proper link placements, and they fall outside ASN-0047's abstract vocabulary; they confirm that the depth-2 placement is a discipline of the creation path, not a structural guarantee the substrate enforces.) Thus whenever MAKELINK is the placing operation the caller never supplies `v_ℓ`: in the non-empty case it is computed from `Σ` (the link subspace's current cardinality at the recorded depth `m_L(d)`), and in the empty (first-link) case its serial component is computed from `Σ` (cardinality 0, giving serial 1) while its depth is supplied by M-DepthConv rather than by `Σ`.
...
*Reflexive variant.* We exercise M-Reflexive concretely. Consider a variant of the above scenario in which the caller deliberately constructs a reflexive endset: replace `e₁` with `e₁' = {(ℓ, δ(1, #ℓ))}` while keeping `e₂` and `e₃` as before. (The caller can compute `ℓ = [d, 0, 2, 1]` from `Σ` via `A_L(d)`'s deterministic first-emission rule, so this is a permitted construction.) By PrefixSpanCoverage (ASN-0043), `coverage(e₁') = {t ∈ T : ℓ ≼ t}`, which contains `ℓ` itself. This input violates StandardAuthoring at `Σ`: since `dom(Σ.L) = ∅`, we have `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)`, hence `coverage(e₁') ⊄ dom(Σ.C) ∪ dom(Σ.L)`. After MAKELINK, `Σ'.M(d) = {[1, 1] ↦ a₁, [1, 2] ↦ a₂, [2, 1] ↦ ℓ}` and `Σ'.L(ℓ).e₁ = e₁'`. Compute `project(ℓ, 1, d, Σ')`: we want `{v ∈ dom(Σ'.M(d)) : Σ'.M(d)(v) ∈ coverage(e₁')}`. Prefix-test each image of `dom(Σ'.M(d))` against `ℓ`: `ℓ ⋠ a₁` (different position-7 component), `ℓ ⋠ a₂` (different position-7 component), `ℓ ≼ ℓ` trivially. The only V-position whose image satisfies the prefix relation is `v_ℓ = [2, 1]` itself, since `Σ'.M(d)(v_ℓ) = ℓ ∈ coverage(e₁')`. Hence `project(ℓ, 1, d, Σ') = {v_ℓ}`, so `discoverable_from(ℓ, d, Σ')` holds via the reflexive route — forced regardless of `Σ.M(d)`'s pre-existing arrangement. The witness is the very mapping `v_ℓ ↦ ℓ` that K.μ⁺_L places, validating M-Reflexive's claim that LP12 covers the reflexive case uniformly without special-case rules.
...
Unless `enabled(MAKELINK)` holds at `Σ`, no post-state `Σ'` exists (K.λ's precondition fails), and `discoverable_from(ℓ, d_target, Σ')` is not assertable. Every wp expression below conjoins this predicate explicitly; it concerns the *home* document `d` and the endset inputs, and is logically independent of the target document `d_target`.

*Membership precondition.* `discoverable_from(a, d, Σ)` is defined in ASN-0098 only when `a ∈ dom(Σ.L) ∧ d ∈ dom(Σ.M)`. At the post-state, `ℓ ∈ dom(Σ'.L)` by K.λ's effect, so the left conjunct is automatic. The right conjunct `d_target ∈ dom(Σ'.M)` requires that the target document was already registered at the pre-state. M1 (ASN-0093) supplies only the inclusion `dom(Σ.M) ⊆ dom(Σ'.M)`; equality `dom(Σ'.M) = dom(Σ.M)` at MAKELINK comes from the K.λ frame and K.μ⁺_L's effect, neither of which extends `dom(M)` (K.λ's frame holds `M` entirely fixed; K.μ⁺_L extends `dom(M(d))` for an already-registered `d` without adding any new document). Combining the M1 inclusion with these two frame consequences gives the equality, so `d_target ∈ dom(Σ'.M) ⟺ d_target ∈ dom(Σ.M)`. This membership clause keeps `discoverable_from` *defined* at the post-state; it is distinct from `enabled(MAKELINK)`, which keeps the post-state from existing at all. For `d_target = d` the membership clause is subsumed by `enabled(MAKELINK)`'s `d ∈ dom(Σ.M)` conjunct; for `d_target ≠ d` it is an independent obligation. Every wp expression below carries both explicitly.
...
where `enabled(MAKELINK) ≡ d ∈ dom(Σ.M) ∧ N ≥ 3 ∧ (A i : eᵢ ∈ Endset) ∧ e₃ ≠ ∅` is MAKELINK's home-document-and-inputs applicability predicate. For `d_target ≠ d` the two membership obligations are independent: `enabled(MAKELINK)` constrains the home document `d` (so a post-state exists), while `d_target ∈ dom(Σ.M)` constrains the target document (so `discoverable_from` is defined). The allocation of `ℓ` contributes nothing to discoverability from documents other than `d`. The wp depends only on the chosen endsets, `d_target`'s pre-existing arrangement, `d_target`'s membership in `dom(Σ.M)`, and MAKELINK's own enabledness.

*Case 2: d_target = d.* Here the membership clause `d ∈ dom(Σ.M)` is not a separate obligation: it is one of the conjuncts of `enabled(MAKELINK)`, so it is discharged by MAKELINK's own input precondition. The post-state arrangement gains `ℓ`: `ran(Σ'.M(d)) = ran(Σ.M(d)) ∪ {ℓ}`. By LP12:
...
Stated with MAKELINK's enabledness held explicit (its `d ∈ dom(Σ.M)` conjunct subsuming the membership clause, since `d_target = d`):

  wp(MAKELINK, discoverable_from(ℓ, d, ·))
    ≡  enabled(MAKELINK)  ∧  [(E i :: coverage(eᵢ) ∩ ran(Σ.M(d)) ≠ ∅) ∨ (E i :: ℓ ∈ coverage(eᵢ))]

*Reduction under standard authoring.* Suppose every input endset satisfies the standard-authoring predicate at `Σ`: `StandardAuthoring(eᵢ, Σ)` for every `i ∈ {1, ..., N}`. Then the reflexive route's disjunct is unreachable: by K.λ's freshness precondition, `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)`; since `StandardAuthoring(eᵢ, Σ)` gives `coverage(eᵢ) ⊆ dom(Σ.C) ∪ dom(Σ.L)`, we have `ℓ ∉ coverage(eᵢ)` for every `i`. The wp collapses:

  wp(MAKELINK, discoverable_from(ℓ, d, ·))
    ≡  enabled(MAKELINK)  ∧  (E i :: coverage(eᵢ) ∩ ran(Σ.M(d)) ≠ ∅)

— the same shape as Case 1 (with `d_target := d`; the two enabledness-and-membership conjuncts coincide there into `enabled(MAKELINK)`, since the home and target documents are one). Under standard authoring, home-document discoverability requires `d`'s arrangement to reach into some endset's coverage; there is no automatic "self-discovery" of `ℓ` from `d`. The home-document privilege of MAKELINK is structural (placement of `v_ℓ`), not semantic (privileged discoverability). The collapse depends only on the structural predicate `StandardAuthoring(eᵢ, Σ)`; any conditions on the caller's epistemic access to `ℓ` are orthogonal and play no role in the wp computation.
...
*Reflexive coverage is structurally excluded under standard authoring.* When every input endset satisfies `StandardAuthoring(eᵢ, Σ)`, K.λ's freshness gives `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)`, and standard authoring confines every `coverage(eᵢ)` to `dom(Σ.C) ∪ dom(Σ.L)`, so `ℓ ∉ coverage(eᵢ)` for every `i`. The defense rests on a structural invariant of the substrate applied to the predicate — not on what the caller does or does not know. `A_L(d)`'s next emission is fully deterministic from the current state (`[d, 0, s_L, 1]` for the first emission, `inc(max{ℓ' : origin(ℓ') = d}, 0)` otherwise), so a client with state access can compute `ℓ` in advance; the substrate provides no architectural barrier against deliberately constructed reflexive coverage. The structural defense above is the only substrate-level guarantee. (As a parenthetical about typical protocol-layer practice: many callers form endsets without consulting `A_L(d)`'s state, and so do not produce reflexive coverage by accident; this is a property of typical client behavior, not a substrate constraint.)
...
*Cross-document discovery cascade across composite MAKELINK sequences.* The two independent effects of a single MAKELINK — (i) the new link `ℓ` becoming discoverable from documents `d_target` whose arrangement reaches `coverage(eᵢ)` for some endset `eᵢ` of `ℓ` (M-WP Case 1), and (ii) a prior link `ℓ'` whose endset covers `ℓ` becoming newly discoverable from the home document `d` of `ℓ` (M-PriorLinkDisc) — compose under sequential MAKELINK invocations: a later MAKELINK whose endsets reach into one of `ℓ`'s coverages can then trigger M-PriorLinkDisc against `ℓ`, and so on. We claim this cascade is *invariant-preserving* — no chain of MAKELINK invocations, however long, can violate any per-state invariant of the substrate. (The claim concerns invariant preservation under composition, not termination of any chain.) The argument is structural rather than inductive: `discoverable_from` is a *derived predicate* over `(L, M)`, not a state component, so the invariant-preservation obligations of the substrate apply to `(L, M)`, not to `discoverable_from`. By LP9 (ExtensionMonotonicity, ASN-0098), each MAKELINK step is monotone in `project(·, ·, d, ·)`: the projection set can only grow, never shrink, at any document whose arrangement is extended. By LP13 (unconditional link persistence, ASN-0098) and L12 (link immutability, ASN-0043), each MAKELINK preserves every prior link's identity, value, and coverage. Hence each composite step of a MAKELINK sequence is independently verified by the per-step analysis already established for a single MAKELINK invocation — there is no joint invariant of the cascade that must be discharged separately, because the substrate carries no joint invariant of discoverability across multiple links. Discoverability is computed afresh from `(L, M)` at every state; the cascade redistributes which links are discoverable from which documents but does not corrupt any state component.
...
*Supplementary observation — chain uniqueness (strengthening beyond L1c's existential).* L1c only demands existence of a structural inc-chain; the existence verification above is sufficient to discharge it. The remainder of this paragraph records a stronger observation, not load-bearing for L1c, that the chain just exhibited is the *unique* chain meeting L1c's structural constraints. We retain it because it locates the chain canonically — useful for downstream reasoning about MAKELINK's emission discipline — but a reader content with the existential may skip ahead to the V-arrangement entry below.

Both TA5a admissibility bounds in the existence chain *saturate exactly* — the `k = 2` step at position 1 with `zeros(d) = 2 = 2` (TA5a's `k = 2` limit) and the `k = 1` step at position 3 with `zeros(b_L(d)) = 3 = 3` (TA5a's `k = 1` limit) — leaving no slack in either direction. From the post-t₁ state with `zeros = 3`, no further `k = 2` step is admissible (would require `zeros ≤ 2`); from the post-t₃ state with `zeros = 3`, no further `k = 1` step is admissible (would require `zeros ≤ 3` — this bound is also at saturation, but additional `k = 1` steps would drive `#E` and length beyond the structural target).

The constraints on the chain — `t₀ = origin(ℓ) = d`, `tₙ = ℓ`, `k₁ = 2` (built into L1c's existential), each `kᵢ ∈ {0, 1, 2}`, TA5a's per-step admissibility (zero-count bounds), and `#tᵢ > #d` for every `i ≥ 1` — combined with two structural facts about `inc` (TA5, ASN-0034): (a) `inc(t, 0)` modifies the component at `sig(t)` (the rightmost nonzero position) and never decreases any component value; (b) `inc(t, k)` for `k > 0` preserves positions `1..#t` (TA5(b)) and extends length by `k` (TA5(d)), while `inc(t, 0)` preserves length (TA5(c)) — jointly force a unique `kᵢ` at each step. We first record the *chain-length fact* used in the final-step exclusion: by FirstEmission (ASN-0093), `t_1^L(d) = inc(b_L(d), 1) = [d, 0, s_L, 1]`, of length `#d + 3`; by ChainDiscipline (ASN-0093), every subsequent element of `A_L(d)` advances by `inc(·, 0)`, which preserves length (TA5(c), ASN-0034); hence every element of `A_L(d)` has length `#d + 3`, and by ChainMembershipForOrigin (ASN-0093) every `ℓ ∈ dom(L)` with `origin(ℓ) = d` lies in `A_L(d)`, so `#ℓ = #d + 3`. The non-target exclusions tabulate against the downstream link-address invariants `origin(ℓ) = d` (L1c), `E(ℓ)₁ = s_L` (L0), and `#ℓ = #d + 3` (the chain-length fact just derived):

| Step | Entering `tᵢ₋₁` (zeros) | TA5a-admissible `k` | Non-target `k` excluded by | Forced `k` |
|------|-------------------------|----------------------|-----------------------------|------------|
| 1 | `d` (zeros = 2) | `{0, 1, 2}` | `k = 0`: violates `#tᵢ > #d` (TA5(c) preserves length). `k = 1`: produces `[d, 1]` whose every continuation has position `#d + 1` non-zero (facts (a), (b)), so the third zero of any descendant lies at position `≥ #d + 2`; by T4b, `D(·)` properly extends `D(d)`, hence `origin(·) ≠ d`, contradicting L1c's `t₀ = origin(ℓ)`. | `k = 2` |
| 2 | `b_C(d) = [d, 0, 1]` (zeros = 3) | `{0, 1}` (`k = 2` excluded: needs zeros ≤ 2) | `k = 1`: produces `[d, 0, 1, 1]` with `E(·)₁ = 1 = s_C`; by fact (a), `sig` moves to position `#d + 3` and position `#d + 2` freezes at `1`, so every continuation has `E(·)₁ = 1 ≠ s_L`, contradicting L0. | `k = 0` |
| 3 | `b_L(d) = [d, 0, 2]` (zeros = 3) | `{0, 1}` (`k = 2` excluded: needs zeros ≤ 2) | `k = 0`: produces `[d, 0, 3]`; by fact (a), position `#d + 2` cannot decrease, so every continuation has `E(·)₁ ≥ 3 ≠ s_L = 2`, contradicting L0. | `k = 1` |
| `j ≥ 4` | `[d, 0, 2, m]` (zeros = 3, length `#d + 3`) | `{0, 1}` (`k = 2` excluded: needs zeros ≤ 2) | `k = 1`: extends length to `#d + 4`; by fact (b), no descendant has length `#d + 3`, contradicting the chain-length fact `#ℓ = #d + 3` derived above (every `ℓ ∈ dom(L)` with `origin(ℓ) = d` lies in `A_L(d)` by ChainMembershipForOrigin, and every element of `A_L(d)` has length `#d + 3`). | `k = 0` |

Each row's non-target exclusion uses facts (a) and (b) to propagate the violation forward through every reachable continuation, so a single wrong choice at any step is incompatible with `ℓ`'s downstream invariants regardless of subsequent `k` choices. The number of `k = 0` steps from `t₃` is fixed by ChainEnumerationInjectivity (ASN-0093): `k_s − 1` repetitions yield `[d, 0, 2, k_s] = ℓ`, where `k_s` is `ℓ`'s chain index in `A_L(d)`, giving total chain length `n = k_s + 2`. Hence the chain `(d, b_C(d), b_L(d), t_1^L(d), …, ℓ)` is the unique structural inc-derivation of any `ℓ ∈ dom(L)` from its home document — a strengthening, not a substitute, of L1c's existential.
...
Note that J0 and J1'★ are vacuous because their quantification universes are empty under MAKELINK's frames on `C` and `R`; J1★ is vacuous because every new V-position MAKELINK introduces is link-subspace (`subspace(v_ℓ) = s_L`), structurally outside J1★'s `subspace = s_C` filter. A hypothetical future variant of MAKELINK that also placed something in the content subspace would still discharge J0 and J1'★ trivially (provided `Σ'.C = Σ.C` and `Σ'.R = Σ.R`) but would leave J1★ to verify against the new content-subspace witnesses.
...
`Σ_mid` is a fully reachable state, not a transitional artifact: by SequentialTransitionAxiom (ASN-0093), every atomic step commits before the next begins, so K.λ on `Σ` yields a complete state `Σ_mid` against which K.μ⁺_L's precondition is evaluated. The per-state invariants hold at `Σ_mid`.

We organise the verification by what K.λ changes and what it leaves alone. K.λ's effect is `Σ_mid.L = Σ.L ∪ {ℓ ↦ (e₁, ..., eₙ)}`; K.λ's frame is `Σ_mid.C = Σ.C` and `(A d' :: Σ_mid.M(d') = Σ.M(d'))` (so `dom(Σ_mid.M) = dom(Σ.M)`), with `Σ_mid.E = Σ.E` and `Σ_mid.R = Σ.R`. The full per-state invariant set partitions into three classes by what they quantify over:

*(α) Invariants quantifying only over unchanged components (`C`, `M`, `E`, `R`).* By the frame, these are *inherited verbatim from `Σ`*: every invariant the reachability hypothesis on `Σ` discharges over `(C, M, E, R)` holds identically over `(Σ_mid.C, Σ_mid.M, Σ_mid.E, Σ_mid.R) = (Σ.C, Σ.M, Σ.E, Σ.R)`. This class covers: S2 (each `Σ_mid.M(d')` remains a partial function), S8a, S8-depth, S8-fin, S8★, CL-OWN, CL-UNIQ, D-MIN★, D-CTG★, D-SEQ★, S3★-aux (subspace exhaustiveness over `dom(M(d))`), M0 (document tumblers in `dom(M)`), S4, S7a, S7b, S7d, C1b, C1c (origin, structural attribution, and allocator conformance for `dom(C)`), C-fin, P6, P7, P8, NodeLineage, ActivatedEmission. Each is inherited because none of its quantifiers names `L`.

*(β) Invariants quantifying over `L` — restricted to prior entries.* For every `ℓ' ∈ dom(Σ.L)`, `Σ_mid.L(ℓ') = Σ.L(ℓ')` by K.λ's frame on prior entries, so per-entry invariants (L0, L1, L1a, L1b, L1c, L3) on prior entries hold at `Σ_mid` by inheritance from `Σ`.

*(γ) Invariants whose witnesses involve the new entry or relate `L` and `M`/`C`.* These require explicit verification:

- *S3★ (referential integrity):* K.λ extends `dom(L)` from `dom(Σ.L)` to `dom(Σ.L) ∪ {ℓ}` while preserving `M` entirely (K.λ frame: `Σ_mid.M = Σ.M`), so every `v ∈ dom(Σ_mid.M(d'))` images consistently — content-subspace V-positions still point into the unchanged `dom(Σ_mid.C) = dom(Σ.C)`, and link-subspace V-positions still point into `dom(Σ_mid.L) ⊇ dom(Σ.L)` (referential integrity cannot break under growth).
- *L0, L1, L1a, L1b, L3 on the new entry `ℓ`:* discharged directly by K.λ's precondition — `E(ℓ)₁ = s_L`, `zeros(ℓ) = 3`, `origin(ℓ) = d ∈ dom(Σ.M) = dom(Σ_mid.M)` (K.λ preserves `dom(M)`), `#E(ℓ) ≥ 2`, and `N ≥ 3 ∧ (A i : eᵢ ∈ Endset) ∧ e₃ ≠ ∅`.
- *L1c at `Σ_mid`:* the same chain `(d, b_C(d), b_L(d), t_1^L(d), …, ℓ)` constructed for `Σ'` in "Per-State Invariants at Σ'" applies here — the chain depends on `(d, ℓ)` alone, not on which other state components have been modified.
- *L14:* K.λ's freshness `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)` combined with L14 at `Σ` gives `dom(Σ_mid.C) ∩ dom(Σ_mid.L) = dom(Σ.C) ∩ (dom(Σ.L) ∪ {ℓ}) = ∅`.
...
| M-Reflexive | If `ℓ ∈ coverage(eᵢ)` for some `i` (the reflexive endset case), then `v_ℓ ∈ project(ℓ, i, d, Σ')` and `discoverable_from(ℓ, d, Σ')` is forced true regardless of `Σ.M(d)`'s pre-existing arrangement. When `(A i : StandardAuthoring(eᵢ, Σ))` holds for the input endsets, the reflexive case is structurally excluded by K.λ's freshness: `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)` combined with `coverage(eᵢ) ⊆ dom(Σ.C) ∪ dom(Σ.L)` gives `ℓ ∉ coverage(eᵢ)` for every `i`. | introduced |
| M-PriorLinkDisc | For every prior link `ℓ' ∈ dom(Σ.L)` and every document `d_target ∈ dom(Σ.M)`: if `d_target = d` (the home document of the new link `ℓ`), then `discoverable_from(ℓ', d, Σ') ⟺ discoverable_from(ℓ', d, Σ) ∨ (E i :: ℓ ∈ coverage(Σ.L(ℓ').eᵢ))` — a prior link is newly discoverable from `d` precisely when some endset of `ℓ'` covers `ℓ`; if `d_target ≠ d`, then `discoverable_from(ℓ', d_target, Σ') = discoverable_from(ℓ', d_target, Σ)` by K.μ⁺_L's frame on `M` (and K.λ's frame on `M`), so prior-link discoverability is unchanged. The discoverability relation is derived from `(L, M)` and is not preserved by the frame on `L` alone; the side-effect window is confined to the home document. Cross-document discovery cascades across sequences of MAKELINK invocations preserve every per-state invariant by LP9 (monotone projection) + LP13 (link persistence) + L12 (link immutability); the cascade redistributes discoverability but corrupts no state component. | introduced |
| M-WP | Post-MAKELINK discoverability has explicit weakest preconditions on `Σ`, each conjoining MAKELINK's enabledness `enabled(MAKELINK) ≡ d ∈ dom(Σ.M) ∧ N ≥ 3 ∧ (A i : eᵢ ∈ Endset) ∧ e₃ ≠ ∅` (the applicability predicate that guarantees a post-state exists, matching LP12a's `enabled(op) ∧ …` convention) with the membership clause that keeps `discoverable_from` defined at the post-state: for `d_target ≠ d`, `wp ≡ enabled(MAKELINK) ∧ d_target ∈ dom(Σ.M) ∧ (E i :: coverage(eᵢ) ∩ ran(Σ.M(d_target)) ≠ ∅)`; for `d_target = d`, `wp ≡ enabled(MAKELINK) ∧ [(E i :: coverage(eᵢ) ∩ ran(Σ.M(d)) ≠ ∅) ∨ (E i :: ℓ ∈ coverage(eᵢ))]` (here `enabled(MAKELINK)`'s `d ∈ dom(Σ.M)` conjunct subsumes the membership clause). This is the weakest precondition for *total* correctness. By M1 combined with the K.λ and K.μ⁺_L frames on `dom(M)` (which together give equality `dom(Σ'.M) = dom(Σ.M)` at MAKELINK), the membership clause is equivalent to `d_target ∈ dom(Σ'.M)`. When `(A i : StandardAuthoring(eᵢ, Σ))`, the home and non-home wp shapes coincide (the reflexive disjunct collapses). | introduced |
