...
*Fresh-address exclusion (M-FreshExcl).* The fresh link address `ℓ ∈ F` (it is an `A_L(d)` emission of the form `[d, 0, s_L, k]`, LP-Sub). When `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)`, standard authoring yields `ℓ ∉ coverage(eᵢ)` for every standardly authored `eᵢ`: were `ℓ ∈ coverage(eᵢ)`, then `ℓ ∈ coverage(eᵢ) ∩ F ⊆ dom(Σ.C) ∪ dom(Σ.L)`, contradicting freshness. We cite this derivation downstream as M-FreshExcl.
...
The frame is a direct consequence of the composite's structure: K.λ modifies only `L`, and K.μ⁺_L modifies only `M(d)`. Neither operation touches `C`. The link's endsets *reference* I-addresses in `dom(C)`, but referencing is read-only — the endset stores spans (start, length pairs), not the bytes at those addresses.
