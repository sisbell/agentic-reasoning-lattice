...
*Notation convention — `dom(M)` and `E_doc`.* ASN-0093 uses `dom(M)` for the set of allocated documents; ASN-0047 uses `E_doc` for the same set. The two foundation ASNs define structurally different registration operations: ASN-0093's K.σ has effect `dom(M') = dom(M) ∪ {d}` with frame `C' = C; L' = L` and does not mention `E` (ASN-0093 carries no entity component); ASN-0047's K.δ in the IsDocument case extends both `E` and `dom(M)`. The combined substrate (ASN-0093 + ASN-0047) reconciles these by imposing the bidirectional coupling

  `d ∈ dom(M) ⟺ d ∈ E_doc`

as an *additional invariant of the combined model* — a constraint not present in either foundation ASN alone. The combined model must therefore either (a) supersede K.σ by K.δ-IsDocument, or (b) extend K.σ's effect with the entity update `E' = E ∪ {d}` to preserve the coupling. In either reading, no transition in the combined vocabulary removes a document from either set. We use `dom(M)` throughout this ASN; ASN-0047 preconditions stated against `E_doc` — notably K.μ⁺_L's `d ∈ E_doc` — are discharged equivalently by membership in `dom(M)` via the bidirectional coupling.
...
*Note on caller knowledge.* The caller does not know `ℓ` at endset-formation time — the substrate determines `A_L(d)`'s next emission only when K.λ fires. This is a separate observation about the *caller's epistemic state*, not the structural standard-authoring predicate above. The reduction relies only on the structural predicate; caller knowledge is irrelevant to the wp computation.
...
*The caller cannot construct a reflexive endset deliberately.* The substrate does not expose `A_L(d)`'s next emission to the caller. Endsets are chosen without knowledge of `ℓ`. Reflexive coverage arises only by accident — when an endset span chosen for other reasons happens to contain the address `ℓ` that `A_L(d)` will emit. When every input endset satisfies `StandardAuthoring(eᵢ, Σ)`, the reflexive case is structurally excluded: K.λ's freshness gives `ℓ ∉ dom(Σ.C) ∪ dom(Σ.L)`, and standard authoring confines every `coverage(eᵢ)` to `dom(Σ.C) ∪ dom(Σ.L)`, so `ℓ ∉ coverage(eᵢ)` for every `i`.
...
We strengthen the L1c existential to a uniqueness claim by ruling out alternative `kᵢ` choices at each step. The argument rests on two structural facts about `inc` (TA5, ASN-0034): (a) `inc(t, 0)` modifies the component at `sig(t)` (the rightmost nonzero position) and never decreases any component value; (b) `inc(t, k)` for `k > 0` extends length by `k`, and `inc(t, 0)` preserves length — so length is monotonically non-decreasing across the chain, and a component value at any fixed position can never decrease once `sig` has moved past that position. We use these facts to force each `kᵢ`:

- *k₁ = 2 is forced* directly by L1c's clause (built into the existential). For completeness, `k₁ = 0` violates the `#tᵢ > #d` clause (TA5(c) preserves length, so `#inc(d, 0) = #d`). For `k₁ = 1`: the step produces `[d, 1]` of length `#d + 1` with position `#d + 1` set to value `1` and no new zero introduced. By facts (a) and (b) — `inc(t, 0)` increments only at `sig(t)` and never decreases any component value, and `inc(t, k)` for `k > 0` preserves positions `1..#t` (TA5(b)) — every reachable continuation from `[d, 1]` has position `#d + 1` non-zero (initially `1`, possibly higher after `k = 0` steps targeting that position, but never zero). By M0 (ASN-0093), `zeros(d) = 2` with both zeros at positions `≤ #d`; by TA5(b)/(c), positions `1..#d` are preserved across every subsequent `inc` step, so the 1st and 2nd zeros of any continuation remain at those same positions `≤ #d`. Hence for any reachable address with three zeros, the third zero must appear at position `≥ #d + 2` (position `#d + 1` is non-zero). By T4b's parse, the document field reads from the position after the 2nd zero up to the position before the 3rd zero, so `D(·)` of any such continuation properly extends `D(d)` — specifically, `D(·)` begins with `D(d)` and continues with at least the non-zero value at position `#d + 1`. Consequently `origin(·) = N(·).0.U(·).0.D(·)` properly extends `d` (it has the shape `[d, m, …]` for some `m ≥ 1`), so `origin(·) ≠ d`. L1c's clause `t₀ = origin(ℓ)` combined with our root `t₀ = d` forces `origin(ℓ) = d`, which the `k₁ = 1` branch cannot satisfy. Hence `k₁ ≠ 1`.

- *k₂ = 0 is forced.* From `b_C(d) = [d, 0, 1]` with `zeros = 3`, TA5a admits `k ∈ {0, 1}` and excludes `k = 2` (requires `zeros ≤ 2`). The `k = 0` branch increments position `#d + 2` from `1` to `2`, yielding `b_L(d) = [d, 0, 2]` with element-field first component `2 = s_L`. The `k = 1` branch appends, yielding `[d, 0, 1, 1]` with element-field first component `1 = s_C` and `sig = #d + 3`. From this point, subsequent inc steps modify only position `sig ≥ #d + 3` or append further; by fact (a), position `#d + 2` is permanently fixed at `1`. Every reachable continuation has `E(·)₁ = 1 ≠ s_L`, contradicting `E(ℓ)₁ = s_L` (L0). Hence `k₂ = 0`.

- *k₃ = 1 is forced.* From `b_L(d) = [d, 0, 2]` with `zeros = 3`, TA5a admits `k ∈ {0, 1}` and excludes `k = 2`. The `k = 1` branch appends `1`, yielding `t_1^L(d) = [d, 0, 2, 1]`, the first emission of `A_L(d)`. The `k = 0` branch increments position `#d + 2` from `2` to `3`, yielding `[d, 0, 3]`; by fact (a), no continuation decreases position `#d + 2` below `3`, so every reachable address has element-field first component `≥ 3 ≠ s_L = 2`, contradicting L0. Hence `k₃ = 1`.

- *kⱼ = 0 for j ≥ 4 is forced.* Each `tⱼ₋₁` in the sibling tail of `A_L(d)` has the form `[d, 0, 2, m]` for some `m ≥ 1`, with `zeros = 3` and length `#d + 3`. TA5a admits `k ∈ {0, 1}` and excludes `k = 2`. The `k = 0` branch yields `tⱼ = [d, 0, 2, m + 1]`, the next sibling, preserving length `#d + 3`. The `k = 1` branch extends length to `#d + 4`; by fact (b), no subsequent address has length `#d + 3`. The structural target `#ℓ = #d + 3` is *not* forced by L0 and L1 alone (a tumbler such as `[d, 0, 2, 1, 1]` satisfies L0's `E(·)₁ = s_L` and L1's `zeros(·) = 3` with length `#d + 4`); it is forced operationally by K.λ's emission precondition, which requires `ℓ` to be produced by `A_L(d)` — `ℓ = inc(s_{n_d}, 0)` in the subsequent-emission case or `ℓ = [d, 0, s_L, 1]` in the first-emission case. By ChainUniformLength (ASN-0093), every element of `A_L(d)` has length `#t_1^L(d) = #d + 3`, and by ChainMembershipForOrigin every `ℓ ∈ dom(L)` with `origin(ℓ) = d` is one such element. Hence `#ℓ = #d + 3`, and the `k = 1` branch is excluded. `kⱼ = 0` for every `j ≥ 4`. The number of `k = 0` steps from `t₃ = [d, 0, 2, 1]` is uniquely determined as `k_s − 1`, where `k_s` is the chain index of `ℓ` in `A_L(d)` — the chain terminates at `tₙ = [d, 0, 2, k_s] = ℓ` exactly when the sibling counter equals `k_s`, giving `n = k_s + 2`.

Combined with TA5a's branching constraint (each step fixes one of `k ∈ {0, 1, 2}` with the precondition-permitted values determined by the current zero count), the chain `(d, b_C(d), b_L(d), t_1^L(d), …, ℓ)` is the *unique* structural inc-derivation of any `ℓ ∈ dom(L)` from its home document. L1c's existential conclusion ("there exists a chain") therefore tightens, for link addresses, to a canonical witness — every `ℓ ∈ dom(L)` has exactly one structural inc-derivation from `origin(ℓ)`.
...
| M-WP | Post-MAKELINK discoverability has explicit weakest preconditions on `Σ`: for `d_target ≠ d`, `wp ≡ (E i :: coverage(eᵢ) ∩ ran(Σ.M(d_target)) ≠ ∅)`; for `d_target = d`, `wp ≡ (E i :: coverage(eᵢ) ∩ ran(Σ.M(d)) ≠ ∅) ∨ (E i :: ℓ ∈ coverage(eᵢ))`. When `(A i : StandardAuthoring(eᵢ, Σ))`, the home and non-home wp shapes coincide (the reflexive disjunct collapses). | introduced |
