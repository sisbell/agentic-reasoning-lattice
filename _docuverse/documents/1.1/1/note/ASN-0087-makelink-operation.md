...
*Notation convention — `dom(M)` and `E_doc`.* ASN-0093 uses `dom(M)` and ASN-0047 uses `E_doc` for the set of allocated documents. ASN-0047's M1 (ArrangementMonotonicity) identifies the two, constraining the document set as `dom(M) = E_doc` (the allocated documents, which grow only via K.δ's `Document(e)` case). We use `dom(M)` throughout, and discharge ASN-0047 preconditions stated against `E_doc` (notably K.μ⁺_L's `d ∈ E_doc`) from membership in `dom(M)` by this M1 equality.
...
Accordingly we assert the *scoped universal*: for any document `d` whose every link V-position was placed by MAKELINK, `m_L(d) = 2`, and in that case `v_ℓ = shift(max(V_{s_L}(d)), 1) = [s_L, 1, ..., 1, n_L + 1]` of depth 2 in the non-empty case. We *retain* the general `m_L(d)` reading downstream: K.μ⁺_L is a standalone substrate primitive whose `ValidFirstLinkPosition(d, v_ℓ, m)` admits any `m ≥ 2` and may be invoked outside MAKELINK (ASN-0047), and Nelson contemplates deeper link addresses by design — fixing the canonical link address at depth 2 (`version.0.2.serial`) while anticipating "the subdivision of links by further digits (after '2' and the position)". (Implementation observation, not an abstract claim: Gregory's `findnextlinkvsa` hardcodes the first-link position at `2.1`.) Whenever MAKELINK is the placing operation the caller never supplies `v_ℓ`: in the non-empty case it is computed from `Σ` (the link subspace's current cardinality at the recorded depth `m_L(d)`), and in the empty (first-link) case its serial component is computed from `Σ` (cardinality 0, giving serial 1) while its depth is supplied by M-DepthConv rather than by `Σ`.
...
*Cross-document discovery cascade across composite MAKELINK sequences.* A single MAKELINK has two independent discovery effects — the new link `ℓ` becoming discoverable from documents reaching `coverage(eᵢ)` (M-WP Case 1), and a prior link `ℓ'` whose endset covers `ℓ` becoming newly discoverable from `d` (M-PriorLinkDisc) — which compose under sequential MAKELINK invocations. Because `discoverable_from` is derived from `(L, M)` rather than a state component, invariant preservation reduces to the per-step guarantees on `(L, M)`: LP9 (projection monotone under arrangement extension, ASN-0098), LP13 (link persistence, ASN-0098), and L12 (link immutability, ASN-0043). These close composition; the cascade redistributes which links are discoverable from which documents but corrupts no state component.
...
*Supplementary observation — chain uniqueness (not load-bearing; L1c needs only existence).* Given the downstream invariants `origin(ℓ) = d` (L1c), `E(ℓ)₁ = s_L` (L0), and `#ℓ = #d + 3` (every `ℓ ∈ dom(L)` with `origin(ℓ) = d` lies in `A_L(d)`, whose elements all have length `#d + 3`, by ChainMembershipForOrigin + ChainDiscipline, ASN-0093), the chain `(d, b_C(d), b_L(d), t_1^L(d), …, ℓ)` exhibited above is the *unique* structural inc-derivation meeting L1c's constraints.
...
| M-DepthConv | MAKELINK fixes the V-position depth of every first link it places at the canonical minimal `m = 2` (`Σ` leaves `m` free, since K.μ⁺_L admits any `m ≥ 2`); thereafter S8-depth pins `m_L(d) = 2`. Scoped universal: for any `d` whose every link V-position was placed by MAKELINK, `m_L(d) = 2`. Not a system-wide invariant; the general `m_L(d)` reading is retained downstream. | introduced |
...
| M-WP | Post-MAKELINK discoverability has explicit weakest preconditions (total correctness, each conjoining `enabled(MAKELINK) ≡ d ∈ dom(Σ.M) ∧ N ≥ 3 ∧ (A i : eᵢ ∈ Endset) ∧ e₃ ≠ ∅`): for `d_target ≠ d`, `wp ≡ enabled(MAKELINK) ∧ d_target ∈ dom(Σ.M) ∧ (E i :: coverage(eᵢ) ∩ ran(Σ.M(d_target)) ≠ ∅)`; for `d_target = d`, `wp ≡ enabled(MAKELINK) ∧ [(E i :: coverage(eᵢ) ∩ ran(Σ.M(d)) ≠ ∅) ∨ (E i :: ℓ ∈ coverage(eᵢ))]`. Under `(A i : StandardAuthoring(eᵢ, Σ))` the reflexive disjunct collapses and the two shapes coincide. | introduced |
