...
*Reduction under standard authoring.* When every input endset satisfies `StandardAuthoring(eᵢ, Σ)`, the reflexive route's disjunct `(E i :: ℓ ∈ coverage(eᵢ))` is unreachable — this structural exclusion is derived in *Reflexive Endsets* (M-Reflexive). The wp collapses:
