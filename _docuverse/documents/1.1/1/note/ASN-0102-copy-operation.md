...
COPY does not name bytes; it names *positions* that already hold bytes. Its source argument is a content reference sequence `R = ⟨r₁, …, r_q⟩` (ASN-0058), each `rᵢ = (d_i, σ_i)` a well-formed content reference into some source document's arrangement. Resolution flattens these into an I-address sequence. Because the source may include the target itself (`d_s = d`), the evaluation point matters, and we settle it here once for the whole operation: **resolution is pinned to the pre-state `Σ`** — the state at which the operation's precondition is read — so `resolve_Σ(R)` consults `Σ.M(d)` *before* the displacement opens the gap. We write the result with that state pinned:
