...
- *P4a, P7a (composite-boundary Class (b) beyond P4★).* P4a (TraceWitnessing), discharged at the composite boundary `Σ_clo` — mirroring the P4★ argument, since COPY's elementary post-state `Σ'` is not in general a trace state. Every pair `(a, d)` requiring a witness lies in `R_clo` and has `a ∈ A`. If `(a, d) ∈ R_B`, it is witnessed by `B`'s pre-existing P4a (a composite-boundary property holding at `B` by the Amendment). Otherwise `(a, d) ∈ R_clo ∖ R_B`, and composite-wide J1'★ forces `a` to be content-subspace-range-new across `B →* Σ_clo` — hence range-resident at the closing boundary, `a ∈ ran_{s_C}(Σ_clo.M(d))` — so `Σ_clo`, a genuine trace state, witnesses the pair. P7a: COPY adds no content (X1) and only adds provenance, so every content address covered at `B` remains covered and no new content address needing coverage is introduced; P7a at `Σ_clo` follows.
...
- **J1'★** is discharged on its `Old`-branch via P4★ at the opening boundary. Taking `B = Σ` (X14), `x_3` is referenced by `d` at `[1,3]` in the content subspace, so `(x_3, d) ∈ Contains_C(B)` and P4★ *at `B`* gives `(x_3, d) ∈ R_B = Σ.R` — with no appeal to any mid-composite inclusion. Hence `Σ'.R = Σ.R`, `R' ∖ R = ∅`, and the J1'★ antecedent is false for every pair.
