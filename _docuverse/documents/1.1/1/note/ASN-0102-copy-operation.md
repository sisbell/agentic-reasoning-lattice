...
- **(PC3) Content subspace.** The target subspace is the content (byte) subspace: `S = s_C`. COPY places transcluded *content*: by PC1 every source span is content-subspace-resident, so its `s_C` V-positions map via S3★ to addresses in `dom(Σ.C)` (C1), and L0 (ASN-0093/0047) fixes `subspace_I(·) = s_C` on every member of `dom(Σ.C)`. This pins `subspace(v) = s_C` for the inserted positions.
