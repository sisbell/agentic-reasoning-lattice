...
This yields COPY's **step-local recording fact (SL)**: the sole `s_C`-range growth COPY's *own* step contributes is `New` — the displaced and unmoved classes retain their pre-state images (X3 restricted to `s_C`), and `Old` was already in range at `Σ` — and COPY records `(a, d)` for every `a ∈ A ⊇ New` (Definition), so every range addition of COPY's own step is written into `Σ'.R`. Both the J1★ and P4★ arguments below invoke (SL) rather than re-deriving it.
