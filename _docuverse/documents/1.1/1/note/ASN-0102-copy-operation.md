...
Merging the copied blocks among themselves thus yields `≤ k` blocks, with equality exactly when no inter-reference boundary is I-adjacent (in particular whenever consecutive references draw from distinct origins, X11). Where the copied region abuts the surrounding arrangement, the whole-arrangement maximal merge (M12 of `Σ'.M(d)`) may reduce the count further; that boundary behaviour is treated in X12. The abstract state commits only to the *arrangement* — the V→I mapping `Σ'.M(d)` — and not to any particular block count: the constructed `k`-block form and every merged form denote the *same* arrangement, differing only as representations of it. An alternative implementation is free to store either (Q8).
...
- *J1★ (ExtensionRecordsProvenance, ASN-0047), composite-wide.* J1★ binds the whole composite `B →* Σ_clo`: every address made content-subspace-range-new across it must lie in `R_clo` (the provenance at `Σ_clo`). COPY discharges its share. The only `s_C`-range growth COPY's *own* step contributes is `New` (X3 restricted to `s_C`; the displaced and unmoved classes retain their pre-state images, and `Old` was already in range at `Σ`), and COPY records `(a, d)` for every `a ∈ A ⊇ New` (Definition). By provenance permanence (P2) each such pair persists from `Σ'.R` into `R_clo`. COPY thus leaves no range addition of its own unrecorded; together with each other step discharging its own additions, composite-wide J1★ holds.
- *J1'★ (ProvenanceRequiresExtension), composite-wide.* J1'★ binds the same boundaries: every pair `(a, d) ∈ R_clo ∖ R_B` must have `a` range-new across `B →* Σ_clo` — present in `ran_{s_C}(Σ_clo.M(d))` and absent from `ran_{s_C}(B.M(d))`. COPY contributes the pairs `{(a, d) : a ∈ A}`; we show none of them offends. We split `A` *at the opening boundary `B`* — where P4★ legitimately holds — rather than at the mid-composite state `Σ`, whose Σ-local `Old`/`New` split would be unsound here: an address may be in range at `Σ` via a prior K.μ⁺ yet absent from `Σ.R`, since P4★ can fail mid-composite.
  - `a ∉ ran_{s_C}(B.M(d))`: COPY maps `a` at a content-subspace position `v + c` (P3, PC3), so `a ∈ ran_{s_C}(Σ'.M(d))` at COPY's own post-state, and `a` is absent from the range at `B`. Hence any pair `(a, d) ∈ R_clo ∖ R_B` it backs is a genuine range extension — `a` is range-new across the composite, with residence supplied by COPY's step or by a prior K.μ⁺, the step responsible. (Should a *later* step contract `a` out of range while retaining the record, that J1'★ violation is charged to that step's coupling, not COPY's.) J1'★'s consequent is met.
  - `a ∈ ran_{s_C}(B.M(d))`: then `(a, d) ∈ Contains_C(B)`, and P4★ *at the boundary `B`* gives `(a, d) ∈ R_B`. Hence `(a, d) ∉ R_clo ∖ R_B` — the antecedent is vacuous for this pair, with no appeal to any inclusion at the mid-composite state `Σ`.
  Either way COPY adds no pair in `R_clo ∖ R_B` whose address fails composite-wide range-newness.

Because COPY writes new pairs into `Σ.R`, it incurs two further provenance obligations that the foundation otherwise carries implicitly, and both are discharged at the point of the `Σ.R` extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`), discharged at the composite boundary.* P4★ is a composite-boundary property, not a per-state invariant: it need not hold at the mid-composite state `Σ`. A K.μ⁺ step grows `Contains_C` without growing `R` (its frame is `R' = R`), so between a content-subspace extension and its coupled K.ρ the inclusion fails — which is exactly why P4★ is asserted only at boundaries. We therefore do *not* argue by a per-elementary-step invariant `Contains_C ⊆ R`; K.μ⁺ refutes any such invariant, and the premise `(i)/(ii)` at `B` gives only `R_B ⊆ Σ.R`, never `Contains_C(Σ) ⊆ Σ.R`. Instead we establish P4★ at the closing boundary `Σ_clo` from P4★ at the opening boundary `B` together with composite-wide J1★. Take any `(a, d) ∈ Contains_C(Σ_clo)`, i.e. `a ∈ ran_{s_C}(Σ_clo.M(d))`. If `a ∈ ran_{s_C}(B.M(d))`, then `(a, d) ∈ Contains_C(B)` and P4★ at `B` gives `(a, d) ∈ R_B ⊆ R_clo`. Otherwise `a` is content-subspace-range-new across `B →* Σ_clo`, and J1★ — which COPY upholds by recording every address it places (`New ⊆ A`), and each other step by recording its own additions — yields `(a, d) ∈ R_clo`. Either way `(a, d) ∈ R_clo`, so `Contains_C(Σ_clo) ⊆ R_clo`. The only frame assumption COPY itself supplies to this composite argument is the step-local one already used for J1★: the sole `s_C`-range growth at COPY's own step is `New`, every member of which COPY writes into `Σ'.R`. P4★ thus holds at every composite boundary by induction on boundaries — base case `Σ₀` (`Contains_C(Σ₀) = ∅ ⊆ R₀`), step case the boundary-to-boundary argument just given — never via a per-elementary-step invariant `Contains_C ⊆ R`.
...
| X8 | RunFragmentation — copied region constructed as `k` blocks (one per resolution run, independent of `W`); within-region merge yields `≤ k`, whole-arrangement merge absorbs a boundary block only where I-adjacent (X12), generically neither | introduced |
