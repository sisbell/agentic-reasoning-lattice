...
**Amendment to `ValidComposite★`.** COPY is added to `ValidComposite★`'s atomic vocabulary (ASN-0047) as a new transition kind, *freely composable*: it may stand as a length-1 (standalone) composite or appear as one step within a longer valid composite. We do not restrict it to standalone use, because both authorities place COPY inside larger units of work. Gregory's `docopy` is never a FEBE operation in its own right — it is always an internal step of a larger dispatch: INSERT (`doinsert`) calls it after a granfilade write, CREATELINK (`docreatelink`) as step 4 of an eleven-step chain with further POOM and spanfilade writes *after* it, CREATENEWVERSION (`docopyinternal`) to lay down a forked version (Q1, evidence). Nelson likewise specifies COPY as a discrete command but provides no transaction bracket, and where he wanted compound behaviour he minted a dedicated primitive (REARRANGE) rather than forbidding composition (Q1, theory). A standalone-only COPY would contradict the very call pattern the operation was built for; with the restriction dropped, a COPY-then-DELETE (a COPY step followed by a `K.μ⁻` contraction) may itself form one valid composite.

What licenses free composition is that COPY is *self-sufficient* in exactly the sense ASN-0047 attributes to `K.μ⁻` (J2) and `K.μ~` (J3): it records its own provenance and discharges its own couplings around its own step, needing no support from neighbouring steps. ValidComposite★ evaluates the clause-2 couplings J0/J1★/J1'★ between a composite's *initial and final boundaries* and asserts the composite-boundary property P4★ (`Contains_C(Σ) ⊆ R`) only at those boundaries. We therefore read COPY's coupling discharge in two ways, both settled in X14. (i) *Standalone:* COPY is the whole composite, its endpoints `Σ → Σ'` are themselves the boundaries, P4★ holds at `Σ`, and the couplings are read between `Σ` and `Σ'`. (ii) *Embedded:* COPY carries an intermediate step `Σ_i → Σ_{i+1}` of a valid composite `Σ_0 →* Σ_n`; the couplings are read between `Σ_0` and `Σ_n`, P4★ holds at the boundary `Σ_0`, and COPY's step-local discharge lifts to that boundary reading by provenance permanence (P2) and store monotonicity. In either reading the precondition PC1–PC4 — including `resolve_Σ(R)` — is evaluated at COPY's own immediate pre-state (`Σ`, resp. `Σ_i`), as ValidComposite★ clause-1 requires of every step; the self-transclusion guarantee of X10(b)/X15 thus reads the source against COPY's own pre-state in both cases.
...
**X14 (ContainmentRecording and coupling discharge).** At completion, `d` contains each copied address: `(A j, i : 0 ≤ i < n_j : a_j + i ∈ ran(Σ'.M(d)))`, so `Contains_C(Σ') ⊇ {(a_j + i, d)}`, and COPY's effect has written the corresponding pairs into `Σ.R` (Definition: `Σ'.R = Σ.R ∪ {(a_j + i, d)}`). We must show this post-state is well-formed against the coupling invariants of ValidComposite★ (ASN-0047). Let `B` denote the composite boundary that carries P4★: in the *standalone* reading `B = Σ` (COPY's own pre-state is a boundary); in the *embedded* reading COPY carries `Σ_i → Σ_{i+1}` of a valid composite `Σ_0 →* Σ_n`, `Σ = Σ_i`, and `B = Σ_0`. By the Amendment, P4★ (`Contains_C(B) ⊆ R_B`) holds at `B`, and provenance permanence (P2) along `B →* Σ` gives `R_B ⊆ Σ.R`. ValidComposite★ reads the couplings J0/J1★/J1'★ between the boundaries (between `Σ` and `Σ'` standalone, between `Σ_0` and `Σ_n` embedded); we discharge each by a step-local argument around COPY's own step `Σ → Σ'` that lifts to the boundary reading via P2 and store monotonicity. Each clause is discharged:
...
- *J1★ (ExtensionRecordsProvenance, ASN-0047).* The content-subspace range gains exactly `New` (X3, restricted to subspace `s_C`) — a subset of `A`, possibly empty. For each `a ∈ New` the Definition's provenance effect records `(a, d) ∈ Σ'.R`, so every genuine range extension is recorded; the obligation is met by COPY's own effect, not deferred. For `a ∈ Old`, J1★'s antecedent `a ∈ ran(Σ'.M(d)) ∖ ran(Σ.M(d))` is false (the address was already in range), so J1★ imposes no obligation on it. More strongly, COPY records `(a, d)` for *every* copied address `a ∈ A`, not only those in `New`, and provenance is permanent (P2); so any content-range extension COPY makes — at its own step, or as seen between the boundaries `Σ_0`/`Σ_n` in the embedded reading — is backed in `R'` and hence in `R_n`. J1★ is therefore met by COPY's blanket recording in both readings.
- *J1'★ (ProvenanceRequiresExtension).* We must show that every pair COPY adds to `R` that lands in the boundary difference backs a *genuine* range extension across that boundary — COPY records no spurious provenance. The pairs COPY adds are `{(a, d) : a ∈ A}`. Fix such an `a` and split on its membership in `d`'s content-subspace range *at the boundary* `B`, writing `ran_{s_C}(B.M(d))` for that range. *(b) `a ∈ ran_{s_C}(B.M(d))`:* `a` is already referenced by `d` at a content-subspace position at `B` (it is a content address, so by L14 (ASN-0043) it cannot be the image of any `s_L` V-position, and by S3★ its referencing position lies in `s_C`); hence `(a, d) ∈ Contains_C(B)`, and by P4★ at `B` with `R_B ⊆ Σ.R ⊆ R'` (P2), the pair `(a, d)` is present *before* the difference — `(a, d) ∉ R' ∖ R` standalone (where `B = Σ`), and `(a, d) ∈ R_0` so `(a, d) ∉ R_n ∖ R_0` embedded. J1'★'s antecedent is false for it. *(a) `a ∉ ran_{s_C}(B.M(d))`:* `a` is new to `d`'s content-subspace range at the boundary. COPY maps `a` at a fresh copied position in subspace `s_C` (P3); in the standalone reading `B = Σ`, so this *is* a genuine extension `a ∈ ran(Σ'.M(d)) ∖ ran(Σ.M(d))` and both conjuncts of J1'★ hold. In the embedded reading `a` is new to the range at `Σ_0`; COPY supplies a range witness at `Σ_{i+1}`, and since the composite is valid by hypothesis its closing witness at `Σ_n` is guaranteed — COPY's blanket record cannot be stranded without the composite itself failing J1'★, which validity excludes. The standalone case (`Old = A ∩ ran(Σ.M(d))` at `B = Σ` recovering branch (b), `New = A ∖ ran(Σ.M(d))` recovering branch (a)) is the specialization with no intervening steps. Either way no pair COPY contributes to the boundary difference lacks a genuine extension, so J1'★ holds.

Because COPY writes new pairs into `Σ.R`, it incurs two further provenance obligations that the foundation otherwise carries implicitly, and both are discharged at the point of the `Σ.R` extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`) at the closing boundary.* P4★ is a composite-boundary property, required at `Σ'` in the standalone reading (where `Σ'` is a boundary) and at `Σ_n` in the embedded reading. Standalone: the post-state content-containment relation is `Contains_C(Σ') = Contains_C(Σ) ∪ {(a_j + i, d)}` (X14's first sentence gives the `⊇`; the displaced and unmoved classes carry their pre-state pairs forward unchanged, and no `s_C` image leaves the range, giving the `⊆`). Each pre-state pair lies in `R ⊆ R'` by P4★ at `Σ` (`B = Σ`); each new pair `(a_j + i, d)` lies in `R'` by COPY's effect. Hence `Contains_C(Σ') ⊆ R'`. Embedded: P4★ is not asked of the interior states `Σ_i, Σ_{i+1}`; at `Σ_n` the same accounting applies — COPY adds provenance for every containment it creates and removes none from `R` or from `Contains_C`, so it cannot turn a contained pair into an unrecorded one. In both readings P4★ is preserved at the boundary.
...
| X14 | ContainmentRecording and coupling discharge — `d` recorded as containing each copied address; provenance written to `Σ.R`; J0/J1★/J1'★ discharged step-locally and lifted to the composite boundary `B` (standalone `B = Σ`, embedded `B = Σ_0`) via P4★-at-`B` and provenance permanence; P7/P4★/P4a/P7a preserved at the closing boundary, link/entity Class (a) invariants (incl. ActivatedEmission) vacuous, P3 (ExtendedTransitionInvariants) discharged from frame; record permanent; against destination | introduced |
