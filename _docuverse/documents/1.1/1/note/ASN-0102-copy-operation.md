...
**Amendment to `ValidComposite★`.** COPY is added to `ValidComposite★`'s atomic vocabulary (ASN-0047) as a new transition kind, *freely composable*: it may stand as a length-1 (standalone) composite or appear as one step within a longer valid composite (both authorities place COPY inside larger units of work — Q1). It is *self-sufficient* in exactly the sense ASN-0047 attributes to `K.μ⁻` (J2) and `K.μ~` (J3): around its own step it records its own provenance and discharges the step-local content of the couplings J0/J1★/J1'★ and the boundary property P4★, needing no support from neighbouring steps. The precondition PC1–PC4 — including the resolution `resolve_Σ(R)` — is evaluated at COPY's own immediate pre-state, as `ValidComposite★` clause-1 requires of every step; the self-transclusion guarantee of X10(b)/X15 thus reads the source against COPY's own pre-state. The coupling obligations are discharged in X14.
...
**Boundary lift (invoked uniformly below).** `ValidComposite★` reads each coupling and composite-boundary property at a *composite boundary*. In the *standalone* reading COPY is a length-1 composite: the opening boundary is `B = Σ` (its own pre-state) and the closing boundary is `Σ'`. In the *embedded* reading COPY carries `Σ_i → Σ_{i+1}` of a valid composite `Σ_0 →* Σ_n` with `Σ = Σ_i`: the opening boundary is `B = Σ_0` and the closing boundary is `Σ_n`. Two facts hold in both readings: **(i)** by the Amendment, P4★ (`Contains_C(B) ⊆ R_B`) holds at the opening boundary `B`; **(ii)** provenance permanence (P2) and store monotonicity along `B →* Σ` give `R_B ⊆ Σ.R ⊆ Σ'.R` and `dom(B.C) ⊆ dom(Σ.C)`. The standalone reading is the specialization `B = Σ` in which COPY *is* the whole composite, so its step-local effect and the boundary obligation coincide. We cite "(i)/(ii) at `B`" below without re-deriving the standalone/embedded split per clause.
...
- *P4a, P7a (composite-boundary Class (b) beyond P4★).* P4a: each new pair `(a_j + i, d)` is witnessed by COPY's own post-state arrangement `Σ'.M(d)` (`a_j + i ∈ ran(Σ'.M(d))` at a content-subspace position, X14), and `Σ'` is a trace state in either reading. P7a (a composite-boundary property): COPY adds no content (X1) and only adds provenance, so every content address covered at the boundary `B` remains covered and no new content address needing coverage is introduced; P7a at the closing boundary (`Σ'` standalone, `Σ_n` embedded) follows.
...
| X14 | ContainmentRecording and coupling discharge — `d` recorded as containing each copied address; provenance written to `Σ.R`; a single *boundary lift* (standalone `B = Σ`, embedded `B = Σ_0`) supplies P4★-at-`B` and provenance permanence for every clause; J0 vacuous, J1★ met by blanket recording; J1'★ is `ValidComposite★` clause-2's obligation — COPY records only pairs backed by its own post-state placement (with the `New`/`Old` split taken at `Σ_i`, not lifted to `Σ_0`); P7/P4★/P4a/P7a preserved at the closing boundary, link/entity Class (a) invariants (incl. ActivatedEmission) vacuous, P3 (ExtendedTransitionInvariants) discharged from frame; record permanent; against destination | introduced |
