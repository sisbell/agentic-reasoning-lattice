...
- **(PC3) Content subspace.** The target subspace is the content (byte) subspace: `S = s_C`. COPY places transcluded *content*: by PC1 ∧ C1 every placed I-address lies in `dom(Σ.C)`. The target V-position subspace is then forced by S3★ applied to the *inserted* (target) mappings, not by any property of the I-addresses' element fields. S3★'s `s_L`-routing clause requires `subspace(v) = s_L ⟹ Σ'.M(d)(v) ∈ dom(Σ'.L)`; but each inserted image lies in `dom(Σ.C) = dom(Σ'.C)` (X1), and `dom(Σ'.C) ∩ dom(Σ'.L) = ∅` (store disjointness, ASN-0093 SD), so an inserted position cannot satisfy the `s_L` clause. Every post-state V-position has subspace `s_C` or `s_L` (S3★-aux), so the inserted positions must take `subspace(v) = s_C`. The reading runs from the placement obligation to the V-position subspace — a content image forces an `s_C` slot — not from the copied address's `subspace_I` (which is an image-routing fact discharged in the `wp` computation below, over distinct objects: the V-position projector `subspace(v) = v₁` ranges over `dom(M(d))`, the address projector `subspace_I` over `dom(C)`).
...
Write the copied address set `A = {a_j + i : 1 ≤ j ≤ k, 0 ≤ i < n_j}` and split it by prior membership in `d`'s range *at COPY's own pre-state* `Σ`: `New = A ∖ ran(Σ.M(d))` (addresses `d` did not already reference at `Σ`) and `Old = A ∩ ran(Σ.M(d))` (addresses `d` already referenced at `Σ`). `Old` is non-empty exactly when `d` already holds some copied content at `Σ` — in particular under self-transclusion (`d_s = d`) or when a prior step already placed the same content in `d`. Every copied address is *mapped at a fresh copied position* `v + c` regardless of class; only those in `New` enlarge `ran(Σ.M(d))` at COPY's own step. This split governs COPY's step-local contribution and is taken at `Σ`, not at the boundary `B`.

This yields COPY's **step-local recording fact (SL)**: the sole `s_C`-range growth COPY's *own* step contributes is `New` — the displaced and unmoved classes retain their pre-state images (X7 restricted to `s_C`), and `Old` was already in range at `Σ` — and COPY records `(a, d)` for every `a ∈ A ⊇ New` (Definition), so every range addition of COPY's own step is written into `Σ'.R`.

- *J1★ (ExtensionRecordsProvenance, ASN-0047), composite-wide.* J1★ binds the whole composite `B →* Σ_clo`: every address made content-subspace-range-new across it must lie in `R_clo` (the provenance at `Σ_clo`). COPY discharges its share by (SL): its own step's sole `s_C`-range growth is `New`, every member of `A ⊇ New` recorded into `Σ'.R`, and by provenance permanence (P2) each such pair persists from `Σ'.R` into `R_clo`. COPY thus leaves no range addition of its own unrecorded; together with each other step discharging its own additions, composite-wide J1★ holds.
- *J1'★ (ProvenanceRequiresExtension), composite-wide.* J1'★ binds the same boundaries: every pair `(a, d) ∈ R_clo ∖ R_B` must have `a` range-new across `B →* Σ_clo` — present in `ran_{s_C}(Σ_clo.M(d))` and absent from `ran_{s_C}(B.M(d))`. COPY contributes the pairs `{(a, d) : a ∈ A}`; we show COPY's *own recording never causes* a J1'★ violation. We split `A` *at the opening boundary `B`*, where P4★ holds.
  - `a ∉ ran_{s_C}(B.M(d))`: COPY maps `a` at a content-subspace position `v + c` (P3, PC3), so `a ∈ ran_{s_C}(Σ'.M(d))` at COPY's *own post-state* `Σ'`. This is range-newness across COPY's step `Σ → Σ'`, **not** yet across the whole composite: J1'★'s presence conjunct asks for residence at the *closing* boundary `Σ_clo`, a different state COPY's step does not by itself reach. The two states can diverge — a later K.μ⁻ (ArrangementContraction) in the same composite may remove `a` from `d`'s content-subspace range while the recorded pair `(a, d)` persists into `R_clo` by provenance permanence (P2). We therefore restrict the claim to COPY's step and trace the closing-boundary obligation to the step responsible:
    - *Residence preserved.* If no later step removes `a` from `ran_{s_C}(·.M(d))`, then `a ∈ ran_{s_C}(Σ'.M(d))` carries forward unchanged to `a ∈ ran_{s_C}(Σ_clo.M(d))`. (The sole content-subspace range-shrinking transition is K.μ⁻; every other operation preserves or extends the range — P0, M1, K.μ⁺'s monotone extension, K.μ~'s range-invariance J3, and COPY's own X7-displacement, which relabels but never drops a binding.) Together with absence at `B`, the consequent is met in full.
    - *Residence destroyed.* If some later K.μ⁻ does remove `a`, the resulting stranded pair `(a, d) ∈ R_clo ∖ R_B` with `a ∉ ran_{s_C}(Σ_clo.M(d))` is a genuine composite-wide J1'★ violation — but the *offending step is that contraction, not COPY*. At COPY's post-state the residence J1'★ demands held; only the subsequent K.μ⁻ broke it, contracting away an address whose provenance a prior step had already recorded. The obligation not to strand a provenance-recorded address by contracting it away is borne by that K.μ⁻'s position in the composite — a constraint on which composites are *valid* — leaving COPY's recording blameless. COPY writes `(a, d)` only with `a` resident at `Σ'`.
  - `a ∈ ran_{s_C}(B.M(d))`: then `(a, d) ∈ Contains_C(B)`, and P4★ *at the boundary `B`* gives `(a, d) ∈ R_B`. Hence `(a, d) ∉ R_clo ∖ R_B` — the antecedent is vacuous for this pair.
  Thus every pair COPY contributes either satisfies composite-wide J1'★ (residence preserved to `Σ_clo`, or antecedent vacuous) or fails it only through a subsequent K.μ⁻ that is itself the J1'★-offender; COPY's own step never grounds a violation, because at `Σ'` it records exactly the addresses it has just placed in range.

COPY writes new pairs into `Σ.R`, so it discharges P7 and P4★ at the point of the extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`), discharged at the composite boundary.* From P4★ at the opening boundary `B` together with composite-wide J1★: take any `(a, d) ∈ Contains_C(Σ_clo)`, i.e. `a ∈ ran_{s_C}(Σ_clo.M(d))`. If `a ∈ ran_{s_C}(B.M(d))`, then `(a, d) ∈ Contains_C(B)` and P4★ at `B` gives `(a, d) ∈ R_B ⊆ R_clo`. Otherwise `a` is content-subspace-range-new across `B →* Σ_clo`, and composite-wide J1★ — to which COPY contributes its own additions by (SL), and each other step its own — yields `(a, d) ∈ R_clo`. Either way `(a, d) ∈ R_clo`, so `Contains_C(Σ_clo) ⊆ R_clo`. The only frame assumption COPY supplies to this argument is (SL), already used for J1★.
...
- *S7a, S7b, S7, C-fin.* The content-address conjuncts S7a (DocumentScopedAllocation), S7b (ElementLevelIAddresses), S7 (StructuralAttribution), and C-fin hold because the content store is unchanged by X1 (`dom(Σ'.C) = dom(Σ.C)`, values fixed), so every claim quantifying over `dom(Σ.C)` carries forward.
- *S7d (DocumentAllocationDiscipline).* Preserved because `Σ'.E = Σ.E`: S7d constrains document tumblers and their allocation events in the entity set, not the content store, and COPY allocates no document, so the set of document-level allocation events — and S7d's distinctness discipline over them — is identical at `Σ'` and `Σ`.
...
- *P4a, P7a (composite-boundary Class (b) beyond P4★).* P4a: each new pair `(a_j + i, d)` is witnessed by COPY's own post-state arrangement `Σ'.M(d)` (`a_j + i ∈ ran(Σ'.M(d))` at a content-subspace position, X14), and `Σ'` is a trace state. P7a: COPY adds no content (X1) and only adds provenance, so every content address covered at `B` remains covered and no new content address needing coverage is introduced; P7a at `Σ_clo` follows.
...
- **J1'★** is discharged on its `Old`-branch via P4★ at the opening boundary. Reading this example's pre-state `Σ` as the composite boundary `B` (the natural framing for a standalone, self-coupling COPY), `x_3` is referenced by `d` at `[1,3]` in the content subspace, so `(x_3, d) ∈ Contains_C(B)` and P4★ *at `B`* gives `(x_3, d) ∈ R_B = Σ.R` — with no appeal to any mid-composite inclusion. Hence `Σ'.R = Σ.R`, `R' ∖ R = ∅`, and the J1'★ antecedent is false for every pair.
