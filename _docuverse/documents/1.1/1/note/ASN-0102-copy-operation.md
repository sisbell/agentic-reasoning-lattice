...
**Amendment to `ValidComposite★`.** COPY is added to `ValidComposite★`'s atomic vocabulary (ASN-0047) as a new elementary transition kind. It changes two state components — the arrangement `M` and the provenance relation `R` — so its coupling obligation is to record provenance for every copied address. It is *coupling-self-sufficient*: around its own step it records its own provenance (`Σ'.R = Σ.R ∪ {(a_j+i, d)}`) and discharges, for its own effect, the step-local content of the couplings J0/J1★/J1'★ and the boundary property P4★, needing no support from neighbouring steps. The precondition PC1–PC4 — including the resolution `resolve_Σ(R)` — is evaluated at COPY's own immediate pre-state, as `ValidComposite★` clause-1 requires of every step; the self-transclusion guarantee of X10(b)/X15 thus reads the source against COPY's own pre-state.
...
**X8 (RunFragmentation).** The copied region is *constructed* as `B_copy = {(v + c_j, a_j, n_j) : 1 ≤ j ≤ k}` — `k` blocks, one per run of the resolution *list* `resolve_Σ(R)`, laid at consecutive V-starts (`c_{j+1} = c_j + n_j`). This constructed count `k` tracks the I-space fragmentation of the source, not the width `W`: copying heavily-edited (fragmented) source costs more blocks than copying pristine source of the same width. The maximally-merged count of the copied region *taken in isolation* — merging the copied blocks among themselves only — need not equal `k`, however, and we must distinguish two notions. M12 (CanonicalUniqueness, ASN-0058) gives the unique maximal merge of the *whole* arrangement `Σ'.M(d)`; there is no "canonical decomposition of a region" in that global sense. What we count below is the strictly *local* in-isolation merge of `B_copy` among its own blocks. The whole-arrangement canonical count is a different, smaller number, reduced further by the two boundary absorptions of X12 (leading and trailing); this paragraph treats only the in-isolation count, and X12 treats the boundary merges that complete the global picture. Two cases separate:
...
Hence the in-isolation canonical count of the copied region is `≤ k`, with equality exactly when no inter-reference boundary is I-adjacent (in particular whenever consecutive references draw from distinct origins, X11). This count is local to `B_copy`: in the whole-arrangement canonical form (M12 of `Σ'.M(d)`) the copied blocks have *no* independent count, because X12's boundary absorption may coalesce the leading copied block with the unmoved predecessor and the trailing copied block with the first displaced block, leaving the copied region no longer separately delimited. The two notions are distinct — the in-isolation count `≤ k` measures fragmentation internal to the copied region; the whole-arrangement M12 count of `Σ'.M(d)` is generally smaller still. The abstract state commits only to the *arrangement* — the V→I mapping `Σ'.M(d)` — and not to any particular block count: the constructed `k`-block form and the canonical `≤ k` form denote the *same* arrangement, differing only as representations of it. An alternative implementation is free to store either (Q8).
...
**X14 (ContainmentRecording and coupling discharge).** At completion, `d` contains each copied address: `(A j, i : 0 ≤ i < n_j : a_j + i ∈ ran(Σ'.M(d)))`, so `Contains_C(Σ') ⊇ {(a_j + i, d)}`, and COPY's effect has written the corresponding pairs into `Σ.R` (Definition: `Σ'.R = Σ.R ∪ {(a_j + i, d)}`). We must show this post-state is well-formed against the coupling invariants of ValidComposite★ (ASN-0047).

**Premise (boundary `B`).** `ValidComposite★` reads each coupling and composite-boundary property at a *composite boundary* `B` with closing boundary `Σ_clo`. Standalone, COPY is a length-1 composite: `B = Σ` (its own pre-state), `Σ_clo = Σ'`. Embedded, COPY carries `Σ_i → Σ_{i+1}` of a valid composite `Σ_0 →* Σ_n` with `Σ = Σ_i`: `B = Σ_0`, `Σ_clo = Σ_n`. Two facts hold of `B` in either case: **(i)** by the Amendment, P4★ (`Contains_C(B) ⊆ R_B`) holds at `B`; **(ii)** provenance permanence (P2) and store monotonicity along `B →* Σ` give `R_B ⊆ Σ.R ⊆ Σ'.R` and `dom(B.C) ⊆ dom(Σ.C)`. Each clause below is discharged once against `B`.
...
- *J1'★ (ProvenanceRequiresExtension).* This is `ValidComposite★`'s clause-2 obligation, evaluated at the composite boundary over the whole composite; COPY's part is to contribute no provenance its own step cannot back. Every pair COPY adds is `(a, d)` with `a ∈ A`, and by the effect clause COPY maps each such `a` at a content-subspace position `v + c` (P3), so `a ∈ ran_{s_C}(Σ'.M(d))` — every recorded pair is witnessed as contained in COPY's *own* post-state. COPY removes nothing: no content leaves `dom(C)` (X1) and no `s_C` image leaves the range (X7). Hence COPY strands no pair: each pair it records is backed by its own post-state placement, and a composite that nonetheless leaves a COPY-recorded pair without a boundary-level range extension is *rejected* by `ValidComposite★` (clause-2), not admitted by it.

Because COPY writes new pairs into `Σ.R`, it incurs two further provenance obligations that the foundation otherwise carries implicitly, and both are discharged at the point of the `Σ.R` extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`) at the closing boundary.* The post-state content-containment relation is `Contains_C(Σ') = Contains_C(Σ) ∪ {(a_j + i, d)}` (X14's first sentence gives the `⊇`; the displaced and unmoved classes carry their pre-state pairs forward unchanged, and no `s_C` image leaves the range, giving the `⊆`). Each pre-state pair lies in `R_Σ ⊆ Σ'.R` by (i)/(ii) at `B`; each new pair `(a_j + i, d)` lies in `Σ'.R` by COPY's effect. Hence `Contains_C(Σ') ⊆ Σ'.R` at `Σ_clo` — COPY adds provenance for every containment it creates and removes none, so it strands no contained pair.
...
| X8 | RunFragmentation — copied region is *constructed* as `k` blocks (one per run of the resolution *list* `resolve_Σ(R)`, each per-reference maximally merged but not merged across reference boundaries, independent of `W`); in-isolation canonical count of the copied region `≤ k`, equality iff no inter-reference boundary is I-adjacent; whole-arrangement M12 count of `Σ'.M(d)` is distinct and reduced further by X12 boundary absorption | introduced |
...
| X14 | ContainmentRecording and coupling discharge — `d` recorded as containing each copied address; provenance written to `Σ.R`; a single boundary premise `B` (standalone `B = Σ`, embedded `B = Σ_0`) supplies P4★-at-`B` and provenance permanence, each clause discharged once against `B`; J0 vacuous, J1★ met by blanket recording; J1'★ is `ValidComposite★` clause-2's obligation — COPY records only pairs backed by its own post-state placement; P7/P4★/P4a/P7a preserved at the closing boundary, link/entity Class (a) invariants (incl. ActivatedEmission) vacuous, P3 (ExtendedTransitionInvariants) discharged from frame; record permanent; against destination | introduced |
