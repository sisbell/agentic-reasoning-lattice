...
- *Within a single reference*, no two blocks coalesce *by construction*: `resolve(d_s, σ)` already returns the maximally-merged decomposition (ASN-0058, C1a/M12), which by definition contains no pair satisfying M7's conjunction of V- *and* I-adjacency. Hence no within-reference pair is a merge candidate.
