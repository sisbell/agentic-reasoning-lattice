...
Now the recording (SL). The copied address set is `A = {x_3}`, and `x_3` is already referenced by `d` at `[1,3]` in the pre-state content subspace (`x_3 ∈ ran_{s_C}(Σ.M(d))`). By (SL), COPY records `(x_3, d)` and `x_3` is content-subspace-resident at `Σ'`. Because `x_3` was already referenced pre-state, COPY's sole new V-position `[1,2]` carries an address `d` already references — COPY introduces no genuine content-subspace range extension, and its provenance write `Σ'.R = Σ.R ∪ {(x_3, d)}` adds nothing not already present. To see `R' ∖ R = ∅` we take `Σ` here to be a *reachable composite boundary* — so that P4★ (`Contains_C(Σ) ⊆ R`), a composite-boundary property of ExtendedReachableStateInvariants (ASN-0047) that need not hold at an arbitrary mid-composite state, applies. Then `(x_3, d) ∈ Contains_C(Σ)` — since `Σ.M(d)([1,3]) = x_3` with `subspace([1,3]) = s_C` — yields `(x_3, d) ∈ Σ.R` by P4★, hence `R' ∖ R = ∅`. (Absent the boundary assumption, the same conclusion follows from the step-local recording fact of whatever operation placed `x_3` at `[1,3]` together with P2, but that route equally requires `Σ` to be reachable for the placing operation's record to survive unmasked.) This is the record-with-residency the composite couplings consume, with no new range entry to ground.
...
| X14 | ContainmentRecording — `d` recorded as containing each copied address, provenance written to `Σ.R`; step-local recording fact (SL); per-state invariants (incl. P7) and transition invariant P3 discharged; composite couplings J0/J1★/J1'★ follow from (SL) + X1 | introduced |
