...
**X2 (NoFreshAllocation).** *A corollary of X1.* COPY consumes no previously-unallocated address: the concrete allocation handle that K.α (ASN-0093) consults is identical before and after COPY. That handle is a function of the per-document content set `D_d = {a' ∈ dom(Σ.C) : origin(a') = d}`. Since COPY leaves `dom(Σ.C)` unchanged (X1) and alters no origin (X6), `D_d` is identical at `Σ'` and `Σ`, so K.α's allocation handle is unchanged.
