...
**X15 (Atomicity).** COPY is a *single* elementary transition (Definition), not a composite of K.μ steps, so SequentialTransitionAxiom (ASN-0047/0093) applies to it directly: the precondition is read against `Σ` and the effect committed to `Σ'` in one indivisible step, with no observable intermediate state. This atomicity is *forced*, not chosen — no multi-step decomposition of COPY is admissible. *Derivation.* COPY's effect both displaces the content-subspace tail forward by `W` (X7) and fills the freed range `[v, v+W)` with the copied region (X16 tiling). Any decomposition into two or more elementary transitions must separate these two acts — the displacement (any step that relabels `{u ∈ V_{s_C}(d) : u ≥ v}` upward by `· + W`) from the fill (a K.μ⁺ extension that binds the copied positions). Consider the state after the displacement but before the fill. The displaced positions now occupy last-component range `[p+W, n_S+W]` while the unmoved positions occupy `[1, p)`, so the content subspace `V_{s_C}(d)` is `{[s_C,1,…,1,c] : 1 ≤ c < p ∨ p+W ≤ c ≤ n_S+W}` — a set with a hole at `[p, p+W)` whenever content is displaced (`p ≤ n_S`) and `W ≥ 1` (PC1). That hole violates D-CTG★ (per-subspace contiguity) and D-SEQ★ (sequential positions from the minimum), both of which `ExtendedReachableStateInvariants` (ASN-0047) demands of *every* state reachable by elementary transitions drawn from a valid composite — not merely at composite boundaries. The reverse order (fill before displace) fares no better: binding the copied positions before the displaced tail vacates `[v, v+W)` would require two `s_C`-positions to share a last component, violating S2 (functionality) at the intermediate state. Either ordering exposes an intermediate state that fails a per-state invariant, so `ValidComposite★`'s clause (1) — each step's elementary precondition must hold at the *intermediate* state — cannot be met by any decomposition. Hence COPY is irreducibly atomic: the displacement and the fill must commit together, in the one indivisible step that carries `Σ` directly to the gap-free post-state `Σ'` (X16).
...
| X15 | Atomicity — COPY is a single elementary transition (SequentialTransitionAxiom); atomicity is *forced*, since any displace-then-fill decomposition exposes an intermediate `s_C` V-gap violating D-CTG★/D-SEQ★ (or S2 in the reverse order) at a state `ValidComposite★` binds | introduced |
