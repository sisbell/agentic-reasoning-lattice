...
**Amendment to `ValidComposite★`.** COPY is added to `ValidComposite★`'s atomic vocabulary (ASN-0047) as a new transition kind, *freely composable*: it may stand as a length-1 (standalone) composite or appear as one step within a longer valid composite (both authorities place COPY inside larger units of work — Q1). It is *self-sufficient* in exactly the sense ASN-0047 attributes to `K.μ⁻` (J2) and `K.μ~` (J3): around its own step it records its own provenance and discharges the step-local content of the couplings J0/J1★/J1'★ and the boundary property P4★, needing no support from neighbouring steps. The precondition PC1–PC4 — including the resolution `resolve_Σ(R)` — is evaluated at COPY's own immediate pre-state, as `ValidComposite★` clause-1 requires of every step; the self-transclusion guarantee of X10(b)/X15 thus reads the source against COPY's own pre-state. The coupling discharge itself — and the standalone/embedded boundary reading it turns on — is carried out once, in X14.
...
- *J1'★ (ProvenanceRequiresExtension).* We must show that every pair COPY adds to `R` that lands in the boundary difference backs a *genuine* range extension across that boundary — COPY records no spurious provenance. The pairs COPY adds are `{(a, d) : a ∈ A}`. Fix such an `a` and split on its membership in `d`'s content-subspace range *at the boundary* `B`, writing `ran_{s_C}(B.M(d))` for that range. *(b) `a ∈ ran_{s_C}(B.M(d))`:* `a` is already referenced by `d` at a content-subspace position at `B` (it is a content address, so by L14 (ASN-0043) it cannot be the image of any `s_L` V-position, and by S3★ its referencing position lies in `s_C`); hence `(a, d) ∈ Contains_C(B)`, and by P4★ at `B` with `R_B ⊆ Σ.R ⊆ R'` (P2), the pair `(a, d)` is present *before* the difference — `(a, d) ∉ R' ∖ R` standalone (where `B = Σ`), and `(a, d) ∈ R_0` so `(a, d) ∉ R_n ∖ R_0` embedded. J1'★'s antecedent is false for it. *(a) `a ∉ ran_{s_C}(B.M(d))`:* `a` is new to `d`'s content-subspace range at the boundary. COPY maps `a` at a fresh copied position in subspace `s_C` (P3); in the standalone reading `B = Σ`, so this *is* a genuine extension `a ∈ ran(Σ'.M(d)) ∖ ran(Σ.M(d))` and both conjuncts of J1'★ hold. In the embedded reading COPY's own step makes the genuine *step-local* extension `a ∈ ran(Σ_{i+1}.M(d)) ∖ ran(Σ_i.M(d))` and records `(a, d)`. Whether that extension still witnesses J1'★ *at the composite boundary* `Σ_0 →* Σ_n` is not something COPY can settle alone: a later step — e.g. a `K.μ⁻` removing exactly the copied content — could retract the range witness while the recorded pair persists by P2, leaving `(a, d) ∈ R_n ∖ R_0` with no boundary-level content-range extension. Boundary J1'★ is therefore a composite-level obligation, evaluated by `ValidComposite★` (clause-2) over the whole composite `Σ_0 →* Σ_n`; COPY discharges only its step-local range extension and contributes no spurious provenance at its own step, and a composite that strands COPY's contribution is *rejected* by `ValidComposite★`, not admitted by it. The standalone case (`Old = A ∩ ran(Σ.M(d))` at `B = Σ` recovering branch (b), `New = A ∖ ran(Σ.M(d))` recovering branch (a)) is the specialization with no intervening steps, where COPY *is* the whole composite, so its step-local extension and the boundary obligation coincide and J1'★ holds outright.
...
| X14 | ContainmentRecording and coupling discharge — `d` recorded as containing each copied address; provenance written to `Σ.R`; J0/J1★ discharged step-locally and lifted to the composite boundary `B` (standalone `B = Σ`, embedded `B = Σ_0`) via P4★-at-`B` and provenance permanence; J1'★ step-local range extension discharged, boundary obligation left to `ValidComposite★` over the whole composite; P7/P4★/P4a/P7a preserved at the closing boundary, link/entity Class (a) invariants (incl. ActivatedEmission) vacuous, P3 (ExtendedTransitionInvariants) discharged from frame; record permanent; against destination | introduced |
