...
**X2 (NoFreshAllocation).** *A corollary of X1.* COPY consumes no previously-unallocated address: the concrete allocation handle that K.α (ASN-0093) consults is identical before and after COPY. K.α selects between two cases on the per-document content set `D_d = {a' ∈ dom(Σ.C) : origin(a') = d}`. Since COPY leaves `dom(Σ.C)` unchanged (X1) and alters no origin (X6), `D_d` is identical at `Σ'` and `Σ`, so the same case fires with the same handle. When `D_d = ∅` the next address is the *first emission* `[d.0.s_C.1]`, determinate and unchanged. When `D_d ≠ ∅` the next address is the *subsequent emission* `inc(a_prev, 0)` off the per-document frontier `a_prev = max D_d`, and X1 leaves that maximum unchanged. In both K.α cases the handle from which the next address is drawn is identical at `Σ'` and `Σ`.
...
**X4 (IdentityOfInstance).** If `Σ'.M(d)(v') = a` (a copied appearance) and `Σ.M(d_s)(v_s) = a` (its source appearance), then both denote the one value `Σ.C(a) = Σ'.C(a)`. The content store is a function (a key has one value), so there is nothing for the two appearances to disagree about. There is no second copy from which to diverge — non-native bytes are never duplicated but remain at their home locations, fetched by request [LM 4/11]. Identity here is *of instance*, not of value: two independently authored occurrences of the same text hold distinct I-addresses and are not made equal by COPY; only addresses that share an origin share an I-address.
...
Write the copied address set `A = {a_j + i : 1 ≤ j ≤ k, 0 ≤ i < n_j}`. Every member of `A` is mapped at a fresh copied position `v + c` (COPY effect clause, PC3), so `A ⊆ ran_{s_C}(Σ'.M(d))` at COPY's post-state. This yields COPY's **step-local recording fact (SL)**: COPY records `(a, d)` for every `a ∈ A` (Definition), and each such `a` is content-subspace-range-resident — `a ∈ ran_{s_C}(Σ'.M(d))` — at COPY's post-state `Σ'`; by provenance permanence (P2) every recorded pair persists. At the instant COPY acts it thus both records `(a, d)` and makes `a` content-subspace-resident. COPY's only coupling-relevant contribution is (SL); J0 is vacuous by X1 (no allocation).

COPY's provenance write is *unconditional*: it records `(a, d)` for every `a ∈ A`, whether or not `a` was already content-subspace-range-resident in `d`. The couplings J1★/J1'★ are range-based and evaluated only between an embedding composite's initial boundary `Σ_0` and its final state — so for each recorded `a` the operative question is whether `a` is range-new *relative to `Σ_0`*, not relative to COPY's possibly-mid-composite pre-state `Σ`. We split on `Σ_0`-residency, naming the premises in each case.

- *`a ∉ ran_{s_C}(M_{Σ_0}(d))`.* Then `a` is range-new relative to `Σ_0`. J1★ (range-new ⟹ recorded) is met by COPY's write, and J1'★ (new provenance ⟹ range-new) holds with its consequent satisfied — regardless of whether `a` happened to be resident at COPY's pre-state. This is the *mid-composite* case: an earlier elementary step of the same composite (an earlier K.μ⁺ or COPY) may have made `a` resident after `Σ_0`, so `(a, d)` may be genuinely `R`-new at the composite level; J1'★ is nonetheless discharged because `a` is range-new relative to `Σ_0`.
- *`a ∈ ran_{s_C}(M_{Σ_0}(d))`.* Then by P4★ — a composite-*boundary* property, invoked here at the boundary `Σ_0` where it genuinely applies — `(a, d) ∈ Contains_C(Σ_0) ⊆ R_{Σ_0}`, and by P2 (provenance permanence) the pair persists through the composite. So `(a, d)` is not `R`-new relative to `Σ_0`; COPY's write contributes no composite-level `R`-new pair for such `a`, and J1'★ is vacuous for it.

COPY writes new pairs into `Σ.R`, so it discharges the per-state invariant **P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`)** at the point of the extension. Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`).
...
- *P4★ (`Contains_C(Σ') ⊆ R'`).* Take `(a, d') ∈ Contains_C(Σ')`. If `d' ≠ d`, the frame `Σ'.M(d') = Σ.M(d')` gives `(a, d') ∈ Contains_C(Σ)`, whence pre-state P4★ places it in `R` and P2 (`R ⊆ R'`) in `R'`. If `d' = d`, then `a ∈ ran_{s_C}(Σ'.M(d))`, and the post-state content-subspace range partitions as `ran_{s_C}(Σ'.M(d)) = ran_{s_C}(Σ.M(d)) ∪ A` — the unmoved and displaced classes carry exactly the pre-state images `{Σ.M(d)(u) : u ∈ V_{s_C}(d)}`, the copied class contributes `A`. A pre-state-range member reaches `R'` by pre-state P4★ + P2 as before; a member of `A` is recorded by COPY's unconditional write (SL), so `(a, d) ∈ R'` directly. Either way `(a, d') ∈ R'`.
...
- *P4a (TraceWitnessing).* Read the standalone COPY as the trace `Σ_0 →* Σ'`, whose trace states are `{Σ_0, Σ'}`. A pair already in `R_{Σ_0}` is witnessed by `Σ_0`'s own P4a — its witnessing trace state lies in the history up to `Σ_0`, a prefix of this trace. A pair in `R' ∖ R` is one COPY recorded, so `d' = d` and `a ∈ A`; by (SL) `a ∈ ran_{s_C}(Σ'.M(d))`, i.e. some content-subspace `v` has `Σ'.M(d)(v) = a`, making `Σ'` itself the witnessing trace state.

Finally, COPY is a transition, so it must also discharge the separate transition theorem **ExtendedTransitionInvariants** (ASN-0047), whose sole conjunct is **P3** (`dom(C) ⊆ dom(C') ∧ dom(L) ⊆ dom(L') ∧ E ⊆ E' ∧ R ⊆ R' ∧ (A a ∈ dom(C) :: C'(a) = C(a)) ∧ (A ℓ ∈ dom(L) :: L'(ℓ) = L(ℓ))`). It is discharged directly from COPY's frame: `Σ'.C = Σ.C` gives both `dom(C) ⊆ dom(C')` and content value-fixity; `Σ'.L = Σ.L` gives both `dom(L) ⊆ dom(L')` and link value-fixity; `Σ'.E = Σ.E` gives `E ⊆ E'`; and `Σ'.R = Σ.R ∪ {(a_j+i, d)} ⊇ Σ.R` gives `R ⊆ R'`. Every conjunct of P3 holds. Of COPY's two mutated components (Definition), `R` changes by extension only (`R ⊆ R'`), so `M` is the only component that can *contract or lose information*, which is exactly what P3 guarantees.

**X15 (Atomicity).** COPY is a *single* elementary transition (Definition), not a composite of K.μ steps, so SequentialTransitionAxiom (ASN-0047/0093) applies to it directly: the precondition is read against `Σ` and the effect committed to `Σ'` in one indivisible step, with no observable intermediate state. Nelson reaches the same conclusion from the design side: the file is left "in canonical order, which was an internal mandate of the system" [LM 1/34] — there is no acknowledged state in which canonical order is suspended.
...
The source span names V-position `[1,3]`, which satisfies `[1,3] ≥ v`. Resolution against the pre-state `Σ` reads `Σ.M(d)([1,3]) = x_3` and lays it down at the copied slot `[1,2] ∈ [v, v+W)`, while the original `x_3` shifts to `[1,4] ∈ [v+W, …)`. Had resolution instead read the *post-state* `Σ'`, position `[1,3]` would hold `x_2` — the content the displacement had just shoved up from `[1,2]` — so the copy would transclude `x_2` rather than `x_3`: a different, circular result. Pre-state resolution (X10(b)) is what makes this self-transclusion well-defined.

Now the recording (SL). The copied address set is `A = {x_3}`, and `x_3` is already referenced by `d` at `[1,3]` in the pre-state content subspace (`x_3 ∈ ran_{s_C}(Σ.M(d))`). By (SL), COPY records `(x_3, d)` and `x_3` is content-subspace-resident at `Σ'`. Taking this COPY as the whole embedding composite (pre-state `Σ = Σ_0`), `x_3` is resident at `Σ_0`, so this is X14's second case: by P4★ at `Σ_0`, `(x_3, d) ∈ R_{Σ_0}`, and COPY's write `Σ'.R = Σ.R ∪ {(x_3, d)}` adds nothing — `R' ∖ R = ∅`, so J1'★ is vacuous for `x_3`.
...
- **X14 (recording)** — `A = {a_1, a_1+1}`; the pre-state content-subspace range is empty, so both copied addresses are genuinely range-new. By (SL), COPY records `(a_1, d)` and `(a_1+1, d)` into `Σ'.R`, each resident at `Σ'` — the record-with-residency the composite's J1★/J1'★ consume for these new range entries. J0 is vacuous by X1.
...
| X14 | ContainmentRecording — `d` recorded as containing each copied address, provenance written to `Σ.R`; step-local recording fact (SL); per-state invariants (incl. P7), composite-boundary properties (P4★, P4a, P7a), and transition invariant P3 discharged | introduced |
| X15 | Atomicity — COPY is a single elementary transition (SequentialTransitionAxiom); precondition-read and effect-commit are one indivisible step, no observable intermediate | introduced |
