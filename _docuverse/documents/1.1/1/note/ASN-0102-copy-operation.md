...
- *J1★ (ExtensionRecordsProvenance, ASN-0047).* COPY records `(a, d)` for *every* copied address `a ∈ A` (Definition), and provenance is permanent (P2). The content-subspace range gains exactly `New` at COPY's step (X3, restricted to `s_C`), and each such pair is recorded by COPY's own effect. Every range-new address thus already has its pair in `Σ'.R`, which persists to `Σ_clo` by (ii); J1★ is met by COPY's blanket recording.
- *J1'★ (ProvenanceRequiresExtension).* For each pair `(a, d) ∈ Σ'.R ∖ Σ.R` introduced by COPY's step, J1'★ requires two conjuncts: (i) `a` lies in the post-state content-subspace range of `d`, and (ii) `a` is *range-new* — absent from the pre-state content-subspace range. Every pair COPY adds is `(a, d)` with `a ∈ A`; the `New`/`Old` split discharges both conjuncts. For `a ∈ New = A ∖ ran(Σ.M(d))`: COPY maps `a` at a content-subspace position `v + c` (P3), so `a ∈ ran_{s_C}(Σ'.M(d))` — conjunct (i); and `a ∉ ran(Σ.M(d))` by definition of `New`, a fortiori `a ∉ ran_{s_C}(Σ.M(d))` — conjunct (ii). For `a ∈ Old = A ∩ ran(Σ.M(d))`: `a` is a content address (resolved from `s_C`, X3) already referenced by `d`, and since S3★ routes every `s_C` image to `dom(C)` and every `s_L` image to `dom(L)` (disjoint by L14, ASN-0047), `a` can only sit at an `s_C` position — so `(a, d) ∈ Contains_C(Σ)`, and the entering inductive hypothesis `Contains_C(Σ) ⊆ Σ.R` (the inclusion COPY preserves step-locally, P4★ below) gives `(a, d) ∈ Σ.R`. Hence `(a, d) ∉ Σ'.R ∖ Σ.R` and the antecedent is vacuous for `a ∈ Old`. Either way J1'★ is met: COPY records no pair its own post-state placement (for `New`) or the entering hypothesis (for `Old`) fails to back.

Because COPY writes new pairs into `Σ.R`, it incurs two further provenance obligations that the foundation otherwise carries implicitly, and both are discharged at the point of the `Σ.R` extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`), preserved step-locally.* P4★ is a composite-boundary property, not a per-state invariant, so we do not assert it at the mid-composite state `Σ` — COPY is an elementary transition applicable at intermediate states, and the premise `(i)/(ii)` at `B` gives only `R_B ⊆ Σ.R`, not `Contains_C(Σ) ⊆ Σ.R`. Instead we show COPY *preserves the inclusion* `Contains_C ⊆ R` step-locally: assuming the inductive hypothesis `Contains_C(Σ) ⊆ Σ.R` entering COPY, we derive `Contains_C(Σ') ⊆ Σ'.R` leaving it. The post-state content-containment relation is `Contains_C(Σ') = Contains_C(Σ) ∪ {(a_j + i, d)}` (X14's first sentence gives the `⊇`; the displaced and unmoved classes carry their pre-state pairs forward unchanged, and no `s_C` image leaves the range, giving the `⊆`). Each pre-state pair lies in `Σ.R ⊆ Σ'.R` by the hypothesis and provenance permanence (P2); each new pair `(a_j + i, d)` lies in `Σ'.R` by COPY's effect. Hence `Contains_C(Σ') ⊆ Σ'.R` — COPY adds provenance for every containment it creates and removes none, so the inclusion holds leaving COPY exactly when it held entering. Since the inclusion holds at the initial boundary (`Contains_C ⊆ R` base case) and is preserved by every elementary step, P4★ holds at every composite boundary `Σ_clo`.
...
- **J1'★** is discharged on its `Old`-branch: `(x_3, d) ∈ Contains_C(Σ)` (x_3 is referenced by `d` at `[1,3]` in the pre-state, content subspace), so by the inductive hypothesis `Contains_C(Σ) ⊆ Σ.R` (the inclusion COPY preserves step-locally, X14) it is already in `Σ.R`; hence `Σ'.R = Σ.R`, `R' ∖ R = ∅`, and the antecedent is false for every pair.
