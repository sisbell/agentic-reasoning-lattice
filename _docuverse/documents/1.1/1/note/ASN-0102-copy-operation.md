...
This restriction is not arbitrary fiat. COPY writes provenance *directly* into `Σ.R` and can place *already-referenced* content, so its J1'★ discharge has an `Old`-branch (X14) that leans on P4★ (`Contains_C(Σ) ⊆ R`) holding at COPY's pre-state. But P4★ is a composite-*boundary* property: ASN-0047 guarantees it only at boundaries, not at the intermediate states interleaving a composite's atomic steps. Confining COPY to a standalone composite makes its pre-state a boundary, supplying the P4★ on which X14's discharge rests; X14 carries the full argument. The scope consequence is that a COPY whose source or target is mutated earlier in the same composite is outside this note's scope; in the integrated model such an edit must be expressed as a separate standalone COPY against the already-committed boundary state.
