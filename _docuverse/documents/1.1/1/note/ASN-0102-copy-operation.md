...
*(b) Snapshot resolution for `d_s = d`.* When the source *is* the target (self-transclusion), the source document is not unaltered — it is the target, and its content-subspace arrangement is displaced by `· + W`. The guarantee that holds here is not non-alteration but *pre-state resolution*: for `d_s = d`, the target-as-source is read at the pre-state `Σ` and is itself displaced by `· + W`. Gregory's trace exhibits the same ordering concretely (`specset2ispanset` precedes `insertpm`, Q15).
...
COPY is a *single elementary transition*. We discharge here what such a transition owes: its frame, the per-state invariants, the transition invariant P3, and the *local* recording fact below. The composite-level couplings (J0, J1★, J1'★) and the composite-boundary properties (P4★, P4a, P7a) are evaluated by `ValidComposite★` only between an embedding composite's initial and final states; they are that composite's obligation, not the elementary step's, and we do not re-prove them here with a private boundary apparatus.

Write the copied address set `A = {a_j + i : 1 ≤ j ≤ k, 0 ≤ i < n_j}`. Every member of `A` is mapped at a fresh copied position `v + c` (COPY effect clause, PC3), so `A ⊆ ran_{s_C}(Σ'.M(d))` at COPY's post-state. This yields COPY's **step-local recording fact (SL)**: COPY records `(a, d)` for every `a ∈ A` (Definition), and each such `a` is content-subspace-range-resident — `a ∈ ran_{s_C}(Σ'.M(d))` — at COPY's post-state `Σ'`; by provenance permanence (P2) every recorded pair persists. At the instant COPY acts it thus both records `(a, d)` and makes `a` content-subspace-resident — the record-with-residency pairing the composite couplings consume.

COPY's contribution to the composite couplings is therefore (SL) together with X1, requiring no boundary reconstruction. *J0 (AllocationPlacementCoupling, ASN-0047)* is vacuous: by X1 `dom(Σ'.C) = dom(Σ.C)`, so its antecedent `a ∈ dom(Σ'.C) ∖ dom(Σ.C)` never holds — COPY allocates no content. For *J1★ / J1'★ (ASN-0047)*, every address COPY makes content-subspace-range-new is one it records (SL), with that address resident at `Σ'`; composite-wide coupling validity then follows from (SL) and ValidComposite★'s own boundary evaluation across the embedding composite.
...
The composite-boundary properties **P4★, P4a, P7a** (ExtendedReachableStateInvariants Class (b)) are not in this list: they are evaluated only at composite boundaries, so an embedding composite discharges them from (SL) plus ValidComposite★'s boundary evaluation, not COPY's elementary step.
...
Now the recording (SL). The copied address set is `A = {x_3}`, and `x_3` is already referenced by `d` at `[1,3]` in the pre-state content subspace (`x_3 ∈ ran_{s_C}(Σ.M(d))`). By (SL), COPY records `(x_3, d)` and `x_3` is content-subspace-resident at `Σ'`. Because `x_3` was already referenced pre-state, COPY's sole new V-position `[1,2]` carries an address `d` already references — COPY introduces no genuine content-subspace range extension, and its provenance write `Σ'.R = Σ.R ∪ {(x_3, d)}` adds nothing not already present (`R' ∖ R = ∅` since `(x_3, d) ∈ Σ.R` by P4★ at the pre-state). This is the record-with-residency the composite couplings consume, with no new range entry to ground.
...
| X14 | ContainmentRecording — `d` recorded as containing each copied address, provenance written to `Σ.R`; step-local recording fact (SL); per-state invariants (incl. P7) and transition invariant P3 discharged; composite couplings J0/J1★/J1'★ and boundary properties P4★/P4a/P7a follow from (SL) + ValidComposite★ boundary checks, not re-proved here | introduced |
