...
- **(PC2) Target document.** `d ∈ E_doc`, equivalently `d ∈ dom(Σ.M)` (`dom(Σ.M) = E_doc`, ASN-0047). In particular the pair `(a_j + i, d)` that COPY's effect writes into `Σ.R` is well-typed precisely because `d ∈ E_doc`.
...
- *Within a single reference*, consecutive blocks never coalesce *by construction*: `resolve(d_s, σ)` already returns the maximally-merged decomposition (ASN-0058, C1a/M12), so its blocks are pairwise non-I-adjacent. The merge condition M7 requires *both* V- and I-adjacency, so non-I-adjacency alone forecloses it — no within-reference boundary is a merge candidate; the new content is the inter-reference boundary below.
...
**X16 (PostStateDensity).** The post-state content subspace `V_{s_C}(d)` in `Σ'` is exactly `{[s_C,1,…,1,c] : 1 ≤ c ≤ n_S + W}` at depth `m` — contiguous with no V-gap (D-SEQ) and with minimum `[s_C,1,…,1]` (D-MIN). *Derivation.* By PC4 the pre-state positions are `[s_C,1,…,1,c]` for `1 ≤ c ≤ n_S`, and `v = [s_C,1,…,1,p]`. The shift `· + W` on subspace `s_C` increments only the last component (it is the ordinal shift `δ(W, m)`, OrdShiftHom, leaving the subspace identifier and the intermediate `1`-components fixed). The three classes of post-state `s_C`-positions therefore occupy disjoint last-component ranges:
