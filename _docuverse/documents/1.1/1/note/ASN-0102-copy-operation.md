...
Finally, every copied position — not only the insertion anchor `v` — is well-formed in the sense of S8a. ValidInsertionPosition (P4) discharges S8a for `v = [s_C,1,…,1,p]` itself, but the interior copied positions `v + 1, …, v + (W−1)` are new entries in `dom(Σ'.M(d))` and must satisfy S8a independently. Each copied position `v + c = [s_C,1,…,1,p+c]` (`0 ≤ c < W`) does: `zeros = 0`, depth `m ≥ 2`, and all components positive (`s_C ≥ 1`, the interior components are `1`, the last component `p + c ≥ 1`). With S8a holding for every copied position, all post-state `s_C`-positions sharing depth `m` (S8-depth), and the copied, displaced, and unmoved classes occupying pairwise-disjoint V-positions (the tiling above), the post-state arrangement `Σ'.M(d)` is a well-defined partial function — S2 (functionality) is fully discharged for the post-state.
