...
**X14 (ContainmentRecording and coupling discharge).** At completion, `d` contains each copied address: `(A j, i : 0 ≤ i < n_j : a_j + i ∈ ran(Σ'.M(d)))`, so the *derived* content-containment relation records `Contains_C(Σ') ⊇ {(a_j + i, d)}`, while COPY's effect writes the corresponding pairs into the persistent provenance relation `Σ.R` (Definition): `Σ'.R = Σ.R ∪ {(a_j + i, d)}`. We must show this post-state is well-formed against the coupling invariants of ValidComposite★ (ASN-0047). As established in the Definition, a standalone COPY is a valid length-1 composite whose endpoints `Σ`, `Σ'` are composite boundaries — so the coupling clauses J0/J1★/J1'★ are evaluated between them and the composite-boundary property P4★ holds at `Σ`. Each clause is discharged:
