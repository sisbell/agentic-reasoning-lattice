...
**Amendment to `ValidComposite★`.** COPY is added to `ValidComposite★`'s atomic vocabulary (ASN-0047) as a new elementary transition kind. It is *coupling-self-sufficient*: around its own step it records its own provenance (`Σ'.R = Σ.R ∪ {(a_j+i, d)}`) and discharges, for its own effect, the step-local content of the couplings J0/J1★/J1'★ and the boundary property P4★, needing no support from neighbouring steps. This is the discipline ASN-0047's composite `J4` (Fork) follows — bundling arrangement extension with its own `K.ρ` provenance recording. It is *weaker* than the full state-isolation ASN-0047 attributes to `K.μ⁻` (J2) and `K.μ~` (J3), which assert `C' = C ∧ L' = L ∧ E' = E ∧ R' = R` and so record no provenance; COPY changes both `M` and `R`. The precondition PC1–PC4 — including the resolution `resolve_Σ(R)` — is evaluated at COPY's own immediate pre-state, as `ValidComposite★` clause-1 requires of every step; the self-transclusion guarantee of X10(b)/X15 thus reads the source against COPY's own pre-state.
