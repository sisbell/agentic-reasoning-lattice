...
COPY's effect *relabels* the content-subspace positions at or after `v` by the forward shift `· + W` (the effect clause below). Because the standing state carries five components, `Σ = (Σ.C, Σ.L, Σ.E, Σ.M, Σ.R)`, the contract must pin all five.
...
The displacement is the same forward shift that INSERT performs — Nelson treats COPY's positional effect as identical to INSERT's [LM 4/66–67]. The half of the definition that distinguishes COPY from every content-creating operation is `Σ'.C = Σ.C`.
...
**X2 (NoFreshAllocation).** *A corollary of X1.* COPY consumes no previously-unallocated address: the set of addresses available to a subsequent content-creating allocation in `d` is identical before and after COPY. Beyond X1's invariance of `dom(Σ.C)`, X2 fixes the concrete allocation handle that K.α (ASN-0093) consults: `d`'s next content address is drawn as `inc(a_prev, 0)` off the per-document frontier `a_prev = max{a' ∈ dom(Σ.C) : origin(a') = d}`. X1 leaves `dom(Σ.C)` — and hence that maximum — unchanged, so the frontier from which the next address is drawn is identical at `Σ'` and `Σ`.
...
(using `dom(Σ'.C) = dom(Σ.C)` by X1, with PC3 fixing the inserted subspace to `s_C` so these mappings fall under S3★'s `s_C` conjunct). Therefore the placed addresses must *already* lie in `dom(Σ.C)` at the pre-state, and they do, by C1 (PC1). The operation has no freedom here: having renounced content creation, the only addresses it can legally place are ones that already exist — and PC3 guarantees it places them where S3★ demands.
...
Merging the copied blocks among themselves thus yields `≤ k` blocks, with equality exactly when no inter-reference boundary is I-adjacent (in particular whenever consecutive references draw from distinct origins, X11). The abstract state commits only to the *arrangement* — the V→I mapping `Σ'.M(d)` — and not to any particular block count: the constructed `k`-block form and every merged form denote the *same* arrangement, differing only as representations of it. An alternative implementation is free to store either (Q8).
...
*(b) Snapshot resolution for `d_s = d`.* When the source *is* the target (self-transclusion), the source document is not unaltered — it is the target, and its content-subspace arrangement is displaced by `· + W`. The guarantee that holds here is not non-alteration but *pre-state resolution*, established once in the resolution section: `resolve_Σ(R)` is pinned to the pre-state `Σ`, so it reads `Σ.M(d)` before the displacement opens the gap. The atomicity of COPY (X15) is what makes that pinning a single indivisible read. Gregory's trace exhibits the same ordering concretely (`specset2ispanset` precedes `insertpm`, Q15).
...
**X14 (ContainmentRecording and coupling discharge).** At completion, `d` contains each copied address: `(A j, i : 0 ≤ i < n_j : a_j + i ∈ ran(Σ'.M(d)))`, so `Contains_C(Σ') ⊇ {(a_j + i, d)}`, and COPY's effect has written the corresponding pairs into `Σ.R` (Definition: `Σ'.R = Σ.R ∪ {(a_j + i, d)}`).

**Premise (boundary `B`).** `ValidComposite★` evaluates its coupling and composite-boundary properties at a composite boundary `B` (with closing boundary `Σ_clo`) at which P4★ already holds. Two facts of `B` are all COPY needs: **(i)** by the Amendment, P4★ (`Contains_C(B) ⊆ R_B`) holds at `B`; **(ii)** provenance permanence (P2) and store monotonicity along `B →* Σ` give `R_B ⊆ Σ.R ⊆ Σ'.R` and `dom(B.C) ⊆ dom(Σ.C)`.
...
Write the copied address set `A = {a_j + i : 1 ≤ j ≤ k, 0 ≤ i < n_j}`. Every member of `A` is mapped at a fresh copied position `v + c` (COPY effect clause, PC3), so `A ⊆ ran_{s_C}(Σ'.M(d))` at COPY's post-state. We split `A` once, at the opening boundary `B`: `Old = A ∩ ran_{s_C}(B.M(d))` (addresses `d` already references in the content subspace at `B`) and `New = A ∖ ran_{s_C}(B.M(d))` (those it does not).

This yields COPY's **step-local recording fact (SL)**: COPY records `(a, d)` for every `a ∈ A` (Definition), and each such `a` is range-resident — `a ∈ ran_{s_C}(Σ'.M(d))` — at COPY's post-state `Σ'`; by provenance permanence (P2) every recorded pair persists into `R_clo`. Hence every address COPY makes content-subspace-range-new (a subset of `New ⊆ A`) is recorded, with the recorded address resident at `Σ'`.

**Boundary dichotomy (BD).** The couplings and boundary properties below all turn on one split of a pair `(a, d)` at the opening boundary `B`, the two horns handled by `B`'s own properties and by the composite-wide couplings respectively:

- *(BD-Old) Pre-existing at `B`.* If `a ∈ ran_{s_C}(B.M(d))`, then `(a, d) ∈ Contains_C(B)`, so P4★ at `B` gives `(a, d) ∈ R_B ⊆ R_clo`, and `B`'s pre-existing P4a witnesses the pair at a trace state of `Σ₀ →* B`.
- *(BD-New) Range-new across `B →* Σ_clo`.* If instead `a ∉ ran_{s_C}(B.M(d))` but `a ∈ ran_{s_C}(Σ_clo.M(d))`, then composite-wide J1★ gives `(a, d) ∈ R_clo`, and `Σ_clo` — a genuine trace state — witnesses it. Dually, composite-wide J1'★ forces every pair of `R_clo ∖ R_B` into this horn. The horn's membership test is residency at the *closing* boundary `Σ_clo`. COPY's own additions contribute here for `a ∈ New`, but (SL) supplies only the record into `Σ'.R ⊆ R_clo` and residency at COPY's *post-state* `Σ'` — and `New` itself is defined relative to the *opening* boundary `B` with no `Σ_clo` condition. The bridge from `Σ'`-residency to the `Σ_clo`-residency this horn demands is discharged in the J1'★ paragraph below — directly when `Σ_clo = Σ'`, and conditionally on no later removal otherwise.

Each discharge below selects the horn(s) it needs.

- *J1★ (ExtensionRecordsProvenance, ASN-0047), composite-wide.* J1★ binds the whole composite `B →* Σ_clo`: every address made content-subspace-range-new across it must lie in `R_clo`. COPY's range additions are a subset of `New ⊆ A`; (BD-New) records each into `R_clo`, and together with each other step discharging its own additions, composite-wide J1★ holds.
- *J1'★ (ProvenanceRequiresExtension), composite-wide.* J1'★ binds the same boundaries: every pair `(a, d) ∈ R_clo ∖ R_B` must have `a` content-subspace-range-new across `B →* Σ_clo` — in particular `a ∈ ran_{s_C}(Σ_clo.M(d))`. Every pair COPY contributes is `(a, d)` with `a ∈ A`. If `a ∈ Old`, (BD-Old) places it in `R_B`, so it is not in `R_clo ∖ R_B` and the antecedent is vacuous. If `a ∈ New`, we must keep COPY's *local* fact distinct from the *composite-wide* obligation: (SL) establishes only that COPY records the pair with `a` resident at its post-state `Σ'`, whereas J1'★ demands residency at `Σ_clo`. Two cases close the gap. **(i) COPY closing or standalone** (`Σ_clo = Σ'`, hence `B = Σ`): `Σ'`-residency *is* `Σ_clo`-residency, so `a ∈ ran_{s_C}(Σ_clo.M(d)) ∖ ran_{s_C}(B.M(d))` and the pair is genuinely range-new across the composite — J1'★ holds for it. **(ii) COPY embedded mid-composite** (`Σ_clo ≠ Σ'`): COPY places `a` into `ran_{s_C}(Σ'.M(d))`, so the antecedent of J1'★ can hold with its consequent failing for this pair *only* if some later step of `B →* Σ_clo` removes `a` from `ran_{s_C}(·.M(d))` before `Σ_clo`; that removal — a K.μ⁻ contraction, never COPY's laydown — is the step that severs residency, and a composite that copies `a` in and contracts it out before its close is one that composite-wide J1'★ correctly rejects as invalid, with the rejection charged to the removing step. COPY's own step thus never grounds a J1'★ violation: at the instant COPY acts it both records `(a, d)` and makes `a` content-subspace-resident, exactly the pairing J1'★ requires.

COPY writes new pairs into `Σ.R`, so it discharges P7 and P4★ at the point of the extension. *P7 (ProvenanceGrounding: `(a, d) ∈ R ⟹ a ∈ dom(C)`).* Every pair COPY adds is `(a_j + i, d)` with `a_j + i ∈ A`; by C1 (via PC1, X3) every such address lies in `dom(Σ.C) = dom(Σ'.C)` (X1), so each newly recorded pair is grounded in the content store and P7 is preserved. The same pair is well-typed against `Σ.R ⊆ T_elem × E_doc` (ASN-0047) on both factors: the `E_doc` side by `d ∈ E_doc` (PC2), and the `T_elem` side because `Element(a_j + i)` holds — `a_j + i ∈ dom(Σ.C)` (just shown) and by S7b (ASN-0036) every content address is element-level (`zeros = 3`). *P4★ (`Contains_C ⊆ R`), discharged at the composite boundary.* For any `(a, d) ∈ Contains_C(Σ_clo)` — `a ∈ ran_{s_C}(Σ_clo.M(d))` — apply (BD): the (BD-Old) horn gives `(a, d) ∈ R_B ⊆ R_clo`, the (BD-New) horn gives `(a, d) ∈ R_clo` via composite-wide J1★ (to which COPY contributes by (SL), each other step its own). Either way `Contains_C(Σ_clo) ⊆ R_clo`.
...
- *Frame-trivial invariants over `C`, `L`, `E`.* COPY's frame freezes the content, link, and entity components — `Σ'.C = Σ.C` with values fixed (X1), `Σ'.L = Σ.L`, `Σ'.E = Σ.E` — and introduces no `s_L`-subspace V-position. Every invariant quantifying solely over these frozen components or their tumbler structure therefore holds at `Σ'` exactly as at `Σ`: **L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, CL-UNIQ, P8, NodeLineage, ActivatedEmission** (over `dom(Σ.L)`, `E`, and link-subspace positions); **S7a, S7b, S7, C-fin, C1b, C1c** (over `dom(Σ.C)` and its members' tumbler structure); **S7d** (document tumblers and their allocation events in `E`); **S4** (the allocation-event set and its pairwise distinctness); and **P6** (`origin(a) ∈ E_doc` for `a ∈ dom(Σ.C)`, with origins frozen by X6 and `E_doc` unchanged).
...
- *P4a, P7a (composite-boundary Class (b) beyond P4★).* P4a (TraceWitnessing) is discharged at the composite boundary `Σ_clo` by (BD), since COPY's elementary post-state `Σ'` is not in general a trace state: every pair `(a, d) ∈ R_clo` requiring a witness falls under (BD-Old) — `(a, d) ∈ R_B`, witnessed at a trace state of `Σ₀ →* B` — or under (BD-New) — `(a, d) ∈ R_clo ∖ R_B`, forced range-new by composite-wide J1'★ and witnessed at `Σ_clo`. P7a: COPY adds no content (X1) and only adds provenance, so every content address covered at `B` remains covered and no new content address needing coverage is introduced; P7a at `Σ_clo` follows.

Finally, COPY is a transition, so it must also discharge the separate transition theorem **ExtendedTransitionInvariants** (ASN-0047), whose sole conjunct is **P3** (`dom(C) ⊆ dom(C') ∧ dom(L) ⊆ dom(L') ∧ E ⊆ E' ∧ R ⊆ R' ∧ (A a ∈ dom(C) :: C'(a) = C(a)) ∧ (A ℓ ∈ dom(L) :: L'(ℓ) = L(ℓ))`). It is discharged directly from COPY's frame: `Σ'.C = Σ.C` gives both `dom(C) ⊆ dom(C')` and content value-fixity; `Σ'.L = Σ.L` gives both `dom(L) ⊆ dom(L')` and link value-fixity; `Σ'.E = Σ.E` gives `E ⊆ E'`; and `Σ'.R = Σ.R ∪ {(a_j+i, d)} ⊇ Σ.R` gives `R ⊆ R'`. Every conjunct of P3 holds. COPY changes two components — `M` and `R` — but `R` changes by extension only (`R ⊆ R'`), so `M` is the only component that can *contract or lose information*, which is exactly what P3 guarantees.
...
- **X8** — the copied region is `k = 2` blocks, *independent of* `W = 4`. The two blocks are *not* I-adjacent (`a_2 ≠ a_1 + 2`, distinct origins), so the in-isolation canonical count is also 2 — equality holds because the single inter-reference boundary is not I-adjacent.
...
This scenario is a *self-transclusion* — the target is its own source — with the source span positioned *at or after* `v`, so the copied span overlaps the region the displacement will move. It exercises the J1'★ branch for already-referenced addresses (`Old = A ∩ ran(Σ.M(d)) ≠ ∅`).
...
**Why X10(b)/X15 are load-bearing here.** The source span names V-position `[1,3]`, which satisfies `[1,3] ≥ v`. Resolution against the pre-state `Σ` reads `Σ.M(d)([1,3]) = x_3` — the correct content — and lays it down at the copied slot `[1,2] ∈ [v, v+W)`, while the original `x_3` shifts to `[1,4] ∈ [v+W, …)`. Had resolution instead read the *post-state* `Σ'`, position `[1,3]` would hold `x_2` — the content the displacement had just shoved up from `[1,2]` — so the copy would transclude `x_2` rather than `x_3`: a *different, circular* result, in which the operation feeds on its own displacement. The pre-state pinning of X10(b)/X15 is exactly what forecloses this circularity, making a self-transclusion that overlaps the displaced region well-defined.
...
- **J1'★** is discharged on its `Old`-branch by (BD-Old) (X14). For this standalone COPY the opening boundary is its own pre-state, `B = Σ`; `x_3` is referenced by `d` at `[1,3]` in the content subspace, so `(x_3, d) ∈ Contains_C(B)` and P4★ at `B` gives `(x_3, d) ∈ R_B = Σ.R` — with no appeal to any mid-composite inclusion. Hence `Σ'.R = Σ.R`, `R' ∖ R = ∅`, and the J1'★ antecedent is false for every pair.
...
This is the first insertion into an *empty* content subspace, where there is no pre-state common depth to inherit: the operation must *choose* a depth `m` and pin it (PC4, ValidFirstInsertionPosition), `New = A` with `Old = ∅`, and there are neither unmoved nor displaced positions.
...
This is the *append*: inserting at `p = n_S + 1`, past the last occupied position. No content is displaced (there is no `u ∈ V_{s_C}(d)` with `u ≥ v`), so the trailing boundary of X12 is *absent* and only the leading boundary is a merge candidate.
...
This is the discriminating case for the *merging* side of X8 and X12 — a same-origin source whose two references abut in I-space (so the inter-reference boundary *coalesces*, `canonical = k − 1`), placed against a predecessor it I-abuts (so the *leading* boundary *absorbs*).
...
| X8 | RunFragmentation — copied region constructed as `k` blocks (one per resolution run, independent of `W`); within-region merge yields `≤ k`, with equality when no inter-reference boundary is I-adjacent | introduced |
...
| X14 | ContainmentRecording and coupling discharge — `d` recorded as containing each copied address, provenance written to `Σ.R`; J0/J1★/J1'★, P4★/P7/P4a/P7a, and P3 all discharged, link/entity invariants vacuous under COPY's frame | introduced |
