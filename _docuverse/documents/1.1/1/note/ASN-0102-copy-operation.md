...
*(b) Snapshot resolution for `d_s = d`.* When the source *is* the target (self-transclusion), the source document is not unaltered — it is the target, and its content-subspace arrangement is displaced by `· + W`. The guarantee here is not non-alteration but the pre-state pinning already fixed by the source-resolution convention: the target-as-source is read at the pre-state `Σ`, which is what makes self-transclusion well-defined. Gregory's trace exhibits the same ordering concretely (`specset2ispanset` precedes `insertpm`, Q15).
...
COPY is a *single elementary transition*. We discharge here what such a transition owes: its frame, the per-state invariants, the transition invariant P3, and the *local* recording fact below.

Write the copied address set `A = {a_j + i : 1 ≤ j ≤ k, 0 ≤ i < n_j}`. Every member of `A` is mapped at a fresh copied position `v + c` (COPY effect clause, PC3), so `A ⊆ ran_{s_C}(Σ'.M(d))` at COPY's post-state. This yields COPY's **step-local recording fact (SL)**: COPY records `(a, d)` for every `a ∈ A` (Definition), and each such `a` is content-subspace-range-resident — `a ∈ ran_{s_C}(Σ'.M(d))` — at COPY's post-state `Σ'`; by provenance permanence (P2) every recorded pair persists. At the instant COPY acts it thus both records `(a, d)` and makes `a` content-subspace-resident — the record-with-residency pairing the composite couplings consume. The composite-level couplings (J0, J1★, J1'★) and the composite-boundary properties (P4★, P4a, P7a) are `ValidComposite★`'s obligation, evaluated only between an embedding composite's initial and final states, not the elementary step's; COPY supplies them this pairing and nothing more.

COPY's contribution to the composite couplings is therefore (SL) together with X1. *J0 (AllocationPlacementCoupling, ASN-0047)* is vacuous: by X1 `dom(Σ'.C) = dom(Σ.C)`, so its antecedent `a ∈ dom(Σ'.C) ∖ dom(Σ.C)` never holds — COPY allocates no content. For *J1★ / J1'★ (ASN-0047)*, every address COPY makes content-subspace-range-new is one it records (SL), with that address resident at `Σ'`.
