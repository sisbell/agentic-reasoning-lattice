...
**X5 (TransitiveIdentity).** The address placed by COPY is the content's *original* I-address, irrespective of how many copy hops separate source from origin. *Derivation.* Every address in `dom(Σ.C)` is produced by exactly one allocation event (S4, ASN-0036, via GlobalUniqueness, ASN-0034), and its `origin` is fixed once and for all by its own tumbler structure (S7) — neither is a property of how the address came to be referenced. COPY allocates nothing (X1) and rewrites no I-coordinate (X3). Resolution reads the source arrangement to extract a *stored* I-address (ASN-0058 `resolve` consults `Σ.M(d_s)`); because no COPY hop ever allocates a fresh address or alters an existing one, the tumbler resolution extracts is identically the one produced at that address's single allocation event — whether `d_s` authored the content or itself obtained it by any number of prior COPYs. Hence `a` is the same tumbler at the end of any chain `… → d_s → d`, and Gregory's trace confirms no copy hop ever rewrites the I-coordinate of a crum (Q13). Identity is invariant under arbitrarily deep copy chains.

**X6 (OriginPreservation).** For every copied address `a`, `origin(a)` is unchanged by COPY and continues to identify the document that allocated `a` (ASN-0036, S7; ASN-0058, M16a gives invariance of origin under the ordinal shift used within a run). Because the I-address itself encodes the home document, the system can, after placement, reveal that the content originated elsewhere: the recoverability of attribution is a structural consequence of X3, not a separately maintained annotation.
...
- *P4a (TraceWitnessing).* We discharge P4a by preservation, taking P4a at `Σ` as the inductive hypothesis — it holds because `Σ` is itself a composite boundary. Any valid transition trace reaching `Σ'` factors as a valid trace reaching `Σ` followed by this COPY composite, so its trace states are `{Σ_init, …, Σ, Σ'}` — the reaching prefix is genuinely part of the full trace. A pair already in `R` (the provenance relation at `Σ`) is witnessed at some trace state within that reaching prefix by the inductive hypothesis. A pair in `R' ∖ R` is one COPY recorded (X14), so `d' = d` and `a ∈ A`; RR's recorded route makes `a` content-subspace-resident at `Σ'` (some `v` with `Σ'.M(d)(v) = a`), so `Σ'` itself is its witnessing trace state.
...
**X15 (Atomicity).** COPY is a *single* elementary transition (Definition), not a composite of K.μ steps, so SequentialTransitionAxiom (ASN-0047/0093) applies to it directly: the precondition is read against `Σ` and the effect committed to `Σ'` in one indivisible step, with no observable intermediate state. Whether this atomicity is *forced* or merely *chosen* depends on the case, and we separate them honestly.

*The displacing case (`p ≤ n_S ∧ W ≥ 1`): atomicity is forced.* When the insertion position precedes some occupied position, COPY's effect both displaces the content-subspace tail forward by `W` (X7) and fills the freed range `[v, v+W)` with the copied region (X16 tiling), and these two acts cannot be sequenced. Any decomposition into two or more elementary transitions must separate the displacement (any step that relabels `{u ∈ V_{s_C}(d) : u ≥ v}` upward by `· + W`) from the fill (a K.μ⁺ extension that binds the copied positions). Consider the state after the displacement but before the fill. The displaced positions now occupy last-component range `[p+W, n_S+W]` while the unmoved positions occupy `[1, p)`, so the content subspace `V_{s_C}(d)` is `{[s_C,1,…,1,c] : 1 ≤ c < p ∨ p+W ≤ c ≤ n_S+W}` — a set with a hole at `[p, p+W)`, since `p ≤ n_S` and `W ≥ 1` make that interval non-empty. That hole violates D-CTG★ (per-subspace contiguity) and D-SEQ★ (sequential positions from the minimum), both of which `ExtendedReachableStateInvariants` (ASN-0047) demands of *every* state reachable by elementary transitions drawn from a valid composite — not merely at composite boundaries. The reverse order (fill before displace) fares no better, but the obstruction is different and worth stating precisely: in the displacing case the freed range `[v, v+W)` overlaps the still-occupied displaced-source positions `[v, n_S]` (non-empty since `p ≤ n_S`), so binding a copied position at any key `u ∈ [v, v+W) ∩ [v, n_S]` before that key's displaced binding has vacated *overwrites and loses* the displaced binding at that single key — `M(d)(u)` cannot hold both the copied image and the surviving displaced image. That is not a shared-component collision but a destruction of a pre-existing binding, which X7 (NonDestructivePlacement) forbids and which no intermediate state can repair, since the lost binding is gone. Either ordering thus exposes an intermediate state that fails a per-state invariant (D-CTG★/D-SEQ★ forward, X7's non-destruction reverse), so `ValidComposite★`'s clause (1) — each step's elementary precondition must hold at the *intermediate* state — cannot be met by any decomposition. Hence in the displacing case COPY is irreducibly atomic: the displacement and the fill must commit together, in the one indivisible step that carries `Σ` directly to the gap-free post-state `Σ'` (X16).

*The non-displacing cases (`p = n_S + 1`, append; or `n_S = 0`, empty subspace): atomicity is a choice.* When nothing is displaced — either because `v` lies past the last occupied position (`p = n_S + 1`, the freed region empty) or because the subspace is empty (`n_S = 0`, `p = 1`) — COPY coincides with a *contiguous tail extension*. There is no displaced region (`{u ∈ V_{s_C}(d) : u ≥ v} = ∅`), so the copied positions `[n_S+1, n_S+W]` (equivalently `[1, W]` when `n_S = 0`) extend `V_{s_C}(d)` past its current maximum, and the post-state subspace is `{[s_C,1,…,1,c] : 1 ≤ c ≤ n_S+W}` (X16). Such an extension *is* expressible as a valid composite: fill the new positions one at a time (`[s_C,1,…,1,n_S+1]`, then `[s_C,1,…,1,n_S+2]`, …, each a single-position K.μ⁺ extension), or in one K.μ⁺ binding all of `B_copy` at once. At every intermediate state `V_{s_C}(d)` remains contiguous from its minimum (D-CTG★/D-SEQ★/D-MIN★ hold throughout — each step appends the next consecutive last component) and S2 is never threatened, because every bound key is fresh (no position is ever double-bound). The provenance recording then follows as a K.ρ step (or steps), with J1★/J1'★ evaluated initial-to-final. So in these cases `ValidComposite★` clause (1) *is* satisfiable by a decomposition; modeling COPY as one elementary transition is therefore a *choice* — justified by uniformity with the displacing case, where atomicity is genuinely forced — not a consequence forced by the per-state invariants. We adopt the elementary-transition model uniformly across all cases, but only the displacing case compels it.
...
| X14 | ContainmentRecording — `d` recorded as containing each copied address, provenance written to `Σ.R`; step-local recording fact (SL) | introduced |
| X15 | Atomicity — COPY is a single elementary transition (SequentialTransitionAxiom). In the displacing case (`p ≤ n_S ∧ W ≥ 1`) atomicity is *forced*: any displace-then-fill decomposition exposes an intermediate `s_C` V-gap violating D-CTG★/D-SEQ★ (forward) or destroys a displaced binding violating X7 (reverse). In the append (`p = n_S+1`) and empty-subspace (`n_S = 0`) cases COPY coincides with a contiguous extension expressible as a valid composite, so the elementary-transition model is a uniform *choice*, not forced | introduced |
| X16 | PostStateDensity — post-state `V_{s_C}(d) = {[s_C,1,…,1,c] : 1 ≤ c ≤ n_S + W}`, contiguous (D-SEQ), min `[s_C,1,…,1]` (D-MIN) | introduced |
| X17 | InvariantPreservation — COPY maintains the per-state `ExtendedReachableStateInvariants` (incl. P7), the composite-boundary properties (P4★, P4a, P7a), and the transition invariant P3; provenance couplings (RR routing, J1★/J1'★) discharged via X14's unconditional write | introduced |
