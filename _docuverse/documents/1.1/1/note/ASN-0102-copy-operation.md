...
COPY touches only the arrangement; it allocates no fresh I-address and adds nothing to `dom(Σ.C)`. Nelson is explicit that placing existing content and creating new content are different acts over the same address machinery — one touches both streams, the other only the arrangement:
