...
- *P4a (TraceWitnessing).* We discharge P4a parametrically, as one operation-preservation step in the reachability induction — not by claiming anything about *all* traces to the state value `Σ'`. Fix any invariant-satisfying pre-state `Σ` and any COPY carrying it to `Σ'`, and take P4a at `Σ` as the inductive hypothesis (it holds because `Σ` is a composite boundary reached by a shorter trace). The COPY-terminated trace — a witnessing trace reaching `Σ`, extended by this COPY composite, with trace states `{Σ_init, …, Σ, Σ'}` — witnesses every pair in `R'`: a pair already in `R` is witnessed at some state of the reaching prefix by the inductive hypothesis, and a pair in `R' ∖ R` is one COPY recorded (X14), so `d' = d` and `a ∈ A`, and RR's recorded route makes `a` content-subspace-resident at `Σ'` (some `v` with `Σ'.M(d)(v) = a`), so `Σ'` itself witnesses it. We do *not* assert that every trace reaching the state value `Σ'` passes through this `Σ`; the universal-over-traces form of P4a follows from the reachability induction over all valid traces, in which each trace's final transition is discharged by its own preservation step — this COPY step being one such.
...
*The non-displacing cases (`p = n_S + 1`, append; or `n_S = 0`, empty subspace).* When nothing is displaced — either because `v` lies past the last occupied position (`p = n_S + 1`, the freed region empty) or because the subspace is empty (`n_S = 0`, `p = 1`) — COPY coincides with a *contiguous tail extension*. There is no displaced region (`{u ∈ V_{s_C}(d) : u ≥ v} = ∅`), so the copied positions `[n_S+1, n_S+W]` (equivalently `[1, W]` when `n_S = 0`) extend `V_{s_C}(d)` past its current maximum, and the post-state subspace is `{[s_C,1,…,1,c] : 1 ≤ c ≤ n_S+W}` (X16). Such an extension is expressible as a valid composite: fill the new positions one at a time (`[s_C,1,…,1,n_S+1]`, then `[s_C,1,…,1,n_S+2]`, …, each a single-position K.μ⁺ extension), or in one K.μ⁺ binding all of `B_copy` at once, with the provenance recording following as a K.ρ step. At every intermediate state `V_{s_C}(d)` remains contiguous from its minimum (D-CTG★/D-SEQ★/D-MIN★ hold throughout — each step appends the next consecutive last component) and S2 is never threatened, because every bound key is fresh. So in these cases `ValidComposite★` clause (1) is satisfiable by a decomposition; only the displacing case compels the elementary-transition model.
