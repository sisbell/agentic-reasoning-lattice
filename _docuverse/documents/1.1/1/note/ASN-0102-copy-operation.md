...
**X2 (NoFreshAllocation).** *A corollary of X1.* COPY consumes no previously-unallocated address: the set of addresses available to a subsequent content-creating allocation in `d` is identical before and after COPY. Beyond X1's invariance of `dom(Σ.C)`, X2 fixes the concrete allocation handle that K.α (ASN-0093) consults. K.α selects between two cases on the per-document content set `D_d = {a' ∈ dom(Σ.C) : origin(a') = d}`. Since COPY leaves `dom(Σ.C)` unchanged (X1) and alters no origin (X6), `D_d` is identical at `Σ'` and `Σ`, so the same case fires with the same handle. When `D_d = ∅` the next address is the *first emission* `[d.0.s_C.1]` — this is the regime of a cross-origin copy into a natively-empty document, where COPY adds only addresses of origin `≠ d` and so leaves `D_d` empty, and the determinate first emission is unchanged. When `D_d ≠ ∅` the next address is the *subsequent emission* `inc(a_prev, 0)` off the per-document frontier `a_prev = max D_d`, and X1 leaves that maximum unchanged. In both K.α cases the handle from which the next address is drawn is identical at `Σ'` and `Σ`.
...
In neither case does an `R`-new pair (relative to `Σ_0`) lack a range-new address. The split turns on `Σ_0`-residency alone: P4★ is invoked only where it applies, on genuine `Σ_0`-residency (second bullet), while pre-state residency that reflects a mid-composite write rather than `Σ_0`-residency is routed to the first bullet, where range-newness relative to `Σ_0` discharges J1'★ directly.
