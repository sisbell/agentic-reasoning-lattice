...
*(b) Snapshot resolution for `d_s = d`.* When the source *is* the target (self-transclusion), the guarantee is pre-state pinning, not non-alteration: the target-as-source is read at the pre-state `Σ` and is itself displaced by `· + W` (not unaltered). Gregory's trace exhibits the same ordering concretely (`specset2ispanset` precedes `insertpm`, Q15).
...
Elementary status is a deliberate modeling choice, not a reachability necessity — the post-state `Σ'` is also reachable by a valid composite. In the displacing case (`p ≤ n_S ∧ W ≥ 1`) a *contract-then-extend* decomposition realises it: a K.μ⁻ retaining the content-subspace prefix `n'_{s_C} = p − 1` (admissible, since `p − 1 < n_S` supplies the required strict contraction, with full link retention), followed by the amended K.μ⁺ binding `[p, p+W)` to the copied I-addresses and `[p+W, n_S+W]` to the displaced images `x_p, …, x_{n_S}` (all in `dom(Σ.C)` by content immutability). The intermediate content subspace `{[s_C,1,…,1,c] : 1 ≤ c ≤ p−1}` is a contiguous prefix from `[s_C,1,…,1]`, so D-CTG★/D-MIN★/D-SEQ★ all hold there — no gap; and X6 is not breached, since X6 constrains COPY's *net* effect, while K.μ⁻ may legitimately drop references (as DELETE does) for the following K.μ⁺ to restore. Provenance is reproducible by appended K.ρ steps under J1★/J1'★. (The naive *displace-then-fill* and *fill-then-displace* orderings, by contrast, do fail `ValidComposite★` clause (1): the former exposes an intermediate `s_C` V-gap at `[p, p+W)` violating D-CTG★/D-SEQ★, the latter overwrites a still-occupied displaced binding at a key `u ∈ [v, v+W) ∩ [v, n_S]` violating X6 (NonDestructivePlacement) — but their failure does not bear on the contract-then-extend route, which succeeds.) COPY is therefore *defined* as elementary so that the transient contraction — references momentarily dropped from `d`'s arrangement under any decomposition that exhibits one — is never observable, not because no valid composite reaches `Σ'`.
...
| X14 | Atomicity — COPY is a single elementary transition in all cases (SequentialTransitionAxiom), committed in one indivisible step with no observable intermediate state. Elementary status is a deliberate modeling choice: `Σ'` is also reachable by a valid composite (contract-then-extend), so atomicity is chosen to suppress the transient contraction, not forced by reachability | introduced |
