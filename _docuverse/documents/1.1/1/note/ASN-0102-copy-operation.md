...
- *(BD-New) Range-new across `B →* Σ_clo`.* If instead `a ∉ ran_{s_C}(B.M(d))` but `a ∈ ran_{s_C}(Σ_clo.M(d))`, then composite-wide J1★ gives `(a, d) ∈ R_clo`, and `Σ_clo` — a genuine trace state — witnesses it. Dually, composite-wide J1'★ forces every pair of `R_clo ∖ R_B` into this horn. COPY's own additions land here exactly for `a ∈ New`, where (SL) supplies both the record into `Σ'.R ⊆ R_clo` and residency at `Σ'`.
...
- *J1'★ (ProvenanceRequiresExtension), composite-wide.* J1'★ binds the same boundaries: every pair `(a, d) ∈ R_clo ∖ R_B` must have `a` range-new across `B →* Σ_clo`. Every pair COPY contributes is `(a, d)` with `a ∈ A`. If `a ∈ Old`, (BD-Old) places it in `R_B`, so it is not in `R_clo ∖ R_B` and the antecedent is vacuous; if `a ∈ New`, (BD-New) gives it range-residency at `Σ'`. Either way COPY's step never grounds a J1'★ violation.
