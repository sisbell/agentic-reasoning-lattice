...
This framework is compatible with the (Σ, 𝒮)-vocabulary fixed by ASN-0034's AllocatedSet, but differs in notation: ASN-0034 writes Σ for the transition vocabulary and s for individual states, whereas this ASN writes Op for the vocabulary and Σ for states. The two notations are merely re-letterings of the same Kripke framework; the relationship between Σ.B and ASN-0034's `allocated(s)` is articulated below at the introduction of Σ.B.
...
A tumbler t is *baptized* iff t ∈ Σ.B. Initially Σ.B contains some finite seed set B₀ ⊆ T (possibly empty) of root addresses established at system genesis, subject to the conformance requirement stated at B₀ conf. below. Thereafter it grows monotonically:
...
The bridge has two parts, each an obligation across the two ASNs rather than a theorem of either alone. The first part identifies ASN-0034's allocator-extension transitions (those that enlarge some `domₛ(A)`) with a subset of ASN-0040's baptismal operations: every transition that adds an address `a` to some `domₛ(A)` is a baptismal operation `baptize(p, d) ∈ Op` whose postcondition adds the same `a` to Σ.B. (The converse — that every baptismal operation is an allocator extension — is the reverse inclusion, conditional below.) The second part is a genesis stipulation: `allocated(Σ_init) ⊆ B₀`. This is not a consequence of B₀ conf., which stipulates only finiteness, contiguity, and T4 for the seed; nor is it a consequence of ASN-0034 in isolation, which fixes `allocated(s₀)` but does not name B₀. It is an additional cross-ASN axiom which the activation-discipline ASN (where allocator activation is specified) must arrange: every address inhabiting an activated allocator's initial domain must also be a seed element. Under both parts of the bridge, the inclusion is preserved at every transition. ASN-0034's non-extending transitions leave `allocated` fixed; allocator extensions, by the first part, are baptismal operations that add the same element to both sets; B0a forbids any non-baptismal enlargement of Σ.B, so Σ.B never grows in a way that would let `allocated` outpace it. By induction from genesis, `allocated(Σ) ⊆ Σ.B` in every reachable state. Without either part of the bridge, the inclusion is unjustified.
...
We state B0 (Irrevocability) below as a stand-alone obligation rather than as a corollary of T8 (AllocationPermanence). The corollary route would chain three inclusions: (i) `Σ.B ⊆ allocated(Σ)` — the conditional reverse inclusion above, requiring parent-prerequisite enforcement; (ii) `allocated(Σ) ⊆ allocated(Σ')` — T8 directly; and (iii) `allocated(Σ') ⊆ Σ'.B` — the forward inclusion above, requiring the cross-ASN bridge axiom. Composing the three yields `Σ.B ⊆ Σ'.B`. Two of the three links — (i) and (iii) — rest on assumptions outside ASN-0034, so the corollary derivation is not unconditional even when T8 holds. Stating B0 directly on Σ.B is moreover essential for the inductive proofs of B1 and B10 to remain legible, and keeps ASN-0040 readable independently of the algebraic derivation in ASN-0034.
...
The case analysis is exhaustive over arbitrary (p, d) ≠ (p₀, d₀). We split first on whether (p, d) satisfies B6, and within the non-B6 branch on whether S(p, d) contains any T4-valid element. This yields three sub-cases: (A) (p, d) satisfies B6; (B) (p, d) violates B6 and every element of S(p, d) violates T4 — corresponding to p with a leading zero, p with adjacent zeros, p violating B6(iii) (zero budget), or d ≥ 3, each of which the B6 necessity analysis below shows propagates a T4 defect to every stream element; and (C) (p, d) violates B6 but S(p, d) contains T4-valid elements — which the B6 necessity analysis restricts to the trailing-zero case (p T4-valid except that p_{#p} = 0, and d = 1). Sub-cases (A), (B), (C) partition the pairs (p, d) ≠ (p₀, d₀): B6 either holds (A) or fails, and when B6 fails the analysis exhibits exactly two configurations — every stream element fails T4 (B), or every stream element satisfies T4 by the trailing-zero coincidence (C); no intermediate configuration arises, because the B6 necessity argument shows that the only failure of B6 that does *not* propagate to the stream is the sole-defect trailing-zero case.
...
When (p, d) does not satisfy B6 and every element of S(p, d) violates T4 (sub-case B): since a satisfies T4, a ∉ S(p, d). Moreover, B10 for the current state ensures every element of B satisfies T4, so children(B, p, d) = ∅. Therefore children(B', p, d) = ∅, trivially a contiguous prefix. (This covers cases where p has adjacent zeros, p starts with zero, zeros(p) + (d − 1) > 3, or d ≥ 3.)

When (p, d) does not satisfy B6 but S(p, d) contains T4-valid elements (sub-case C): this occurs when p ends in zero (with no other T4 defect) and d = 1. (When d = 2 with the same trailing-zero defect, all stream elements violate T4 and the configuration belongs to sub-case B above. The defect does not preexist in p's interior, so TA5(b)-propagation is not the right description; instead, by TA5(b), c₁ preserves position #p of p, so (c₁)_{#p} = p_{#p} = 0, and by TA5(d) with d = 2, c₁ holds the field separator at position #p + 1 with value 0, creating adjacent zeros at positions #p and #p + 1 of c₁ — a T4 violation. Sibling increments cₙ₊₁ = inc(cₙ, 0) modify only sig(cₙ) = #p + 2 by TA5(c), leaving positions #p and #p + 1 invariant; by induction every cₙ carries the adjacent-zero violation. With every stream element violating T4 in this configuration, the sub-case B argument applies.) Let p' be p with its trailing zero removed, so #p' = #p − 1 and p'ᵢ = pᵢ for 1 ≤ i ≤ #p − 1, and let d' = 2. We show S(p, 1) = S(p', 2) by proving first-element equality and then applying the shared recurrence.
...
B₀ must be finite, satisfy B1 for every namespace at genesis, and have every seed element be a valid address under T4. Non-emptiness is *not* required at this layer: with B₀ = ∅ the conformance conditions hold vacuously, and Bop's preconditions (B6 alone — the parent prerequisite is deferred to the Open Questions) admit any B6-valid (p, d), so a system seeded with an empty registry can still grow by baptizing children of a T4-valid p that is not itself in Σ.B. Whether such ungrounded baptism is admissible is precisely the parent-prerequisite question: B₀ conf. does not pre-empt that question by demanding at least one ancestor seed. If the parent prerequisite is later enforced — requiring p ∈ Σ.B for any baptism — then non-emptiness becomes a derived requirement rather than a primitive one, since an empty Σ.B would block all baptisms under that enforcement. Finiteness is required because the next function's well-definedness depends on max(children(B, p, d)) existing, which requires children to be a finite set; since B starts as B₀ and grows by one element per baptism, B₀ finite implies B finite in every reachable state. Without the contiguity requirement, the seed set could contain {c₁, c₃} for some namespace — a gap that the inductive argument cannot repair, since baptism only appends the next sibling. Without the T4 requirement, a seed element could serve as a parent that violates B6(i), undermining B7's disjointness guarantee.
...
*Proof.* We must show that in every reachable state, Σ.B is finite. The argument proceeds by induction on the number of state transitions from the initial state.
...
**B9 (Unbounded Extent).** `(A p, d : B6(p, d) : (A M ∈ ℕ : (E B' : B' reachable from Σ.B by a finite sequence of baptisms : hwm(B', p, d) ≥ M)))`.
...
*Proof.* We must show that for any pair (p, d) satisfying B6 and any bound M ∈ ℕ, there exists a registry B' reachable from Σ.B by a finite sequence of baptisms such that hwm(B', p, d) ≥ M. The argument is constructive: we exhibit the required sequence of baptisms.
...
*Inductive step.* Assume Bₖ is a registry reachable from Σ.B by k baptisms in namespace (p, d), with hwm(Bₖ, p, d) = m + k < M. We perform baptize(p, d) on state Bₖ. The preconditions of Bop are satisfied: B6(p, d) holds by hypothesis; B4 (Atomic Baptism) holds because each baptism in the constructed sequence is its own Op-transition and the sequence is presented as successive transitions — no two share an edge in the transition graph.
