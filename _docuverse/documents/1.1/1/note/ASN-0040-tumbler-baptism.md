...
Conditions (ii) and (iii) are necessary and sufficient for T4 preservation of the sibling stream, given (i). Condition (i) is a requirement of a different kind: it is not what secures stream T4-validity in every case, but is independently required so that (a) a defective prefix or excess zero-count in p does not propagate into the stream, and (b) a trailing-zero parent does not alias an existing B6 namespace — keeping B7 (Namespace Disjointness) and B8 (Co-reachable Uniqueness) intact (necessity sub-case (b) below). Condition (ii) follows from the ASN-0034 lemma "TA5 preserves T4": for d ≥ 3, the appended sequence contains adjacent zeros, violating T4's non-empty-field constraint. Condition (iii) ensures no address exceeds the four-level hierarchy; it is binding only at d = 2 (at d = 1 it reduces to T4's own zero bound on p). Together:
...
*Proof.* We prove sufficiency (all three conditions imply T4 preservation) and then necessity. For (ii) and (iii), necessity is about *stream* T4-preservation: violating either produces a T4 violation in the stream. For (i), necessity is of a different kind — it secures the stream against defect propagation (sub-case (a)) and secures namespace disjointness against aliasing (sub-case (b)). These two roles are kept separate throughout.
...
**(⟹) Necessity.** Violating condition (ii) or (iii) produces a T4 violation in the stream. Condition (i) is necessary for distinct reasons, separated by sub-case below: a defective prefix or excess zero-count in p propagates into the stream (sub-case (a)), while a pure trailing zero in p — though it may leave the d = 1 stream T4-valid — aliases an existing B6 namespace via S2, breaking B7 and B8 (sub-case (b)).
...
*(b) Pure trailing zero as the sole T4 defect: p_{#p} = 0, p₁ > 0, zeros(p) ≤ 3, no adjacent zeros in p (which forces #p ≥ 2, since p₁ > 0 = p_{#p} requires the leading and trailing positions to be distinct).* Here p fails only T4's t_{#t} ≠ 0 clause. At d = 2 the stream itself violates T4 (shown next), so in effect this case behaves like sub-case (a). At d = 1, however, the stream remains fully T4-valid — by S2, a pure-trailing-zero parent p at d = 1 generates the same stream as the namespace obtained by dropping p's trailing zero and incrementing the depth, e.g. S([1, 0], 1) = S([1], 2) = {[1, 0, n] : n ≥ 1}. The d = 1 stream therefore supplies no T4 obstruction, and the necessity of (i) here is not about stream validity at all — it is about *aliasing*. Were (i) dropped, ([1, 0], 1) and ([1], 2) would be two distinct pairs both satisfying (ii) and (iii) yet producing the identical stream [1, 0, n]. That directly falsifies B7 (Namespace Disjointness), which asserts S(p, d) ∩ S(p', d') = ∅ for distinct valid pairs, and it collapses B8 Case 2, whose entire argument is "different namespaces → B7 → distinct addresses." Condition (i) breaks the tie: of the two aliasing pairs, S2's truncation ([1], 2) carries a T4-valid parent while ([1, 0], 1) does not, so (i) admits exactly one. Thus (i) is load-bearing for B7 and B8 — it is what keeps distinct B6-valid namespaces from coinciding — not a mere parent-level hygiene rule.
...
Condition (i) is therefore necessary, for two distinct reasons: a count violation (zeros(p) > 3) and T4 defects in p's preserved prefix — interior, leading, or the singleton p = [0] — propagate to every stream element via TA5(b) and B5/B5a (sub-case (a)); a pure trailing zero creates an adjacent-zero stream violation at d = 2 and, at d = 1, aliases a distinct B6-valid namespace via S2 — breaking B7 disjointness and the B8 uniqueness that rests on it — so (i) is required to keep namespaces disjoint (sub-case (b)). ∎

*Formal Contract:*
- *Preconditions:* p ∈ T, d ∈ ℕ with d ≥ 1.
- *Postconditions:* (a) Sufficiency: `(p satisfies T4 ∧ d ∈ {1, 2} ∧ zeros(p) + (d − 1) ≤ 3) ⟹ (A n ≥ 1 : cₙ ∈ S(p, d) satisfies T4)`. (b) Necessity: violating (ii) or (iii) forces a stream T4 violation. Condition (i) is independently required: a defective prefix or excess zero-count in p propagates into the stream (sub-case (a)); a pure trailing zero, though it leaves the d = 1 stream T4-valid (S2), aliases a distinct B6-valid namespace, so (i) is necessary for B7 (Namespace Disjointness) and B8 (sub-case (b)).
