...
Conditions (ii) and (iii) are necessary and sufficient for T4 preservation of the sibling stream, given (i). Condition (i) requires p to be a valid address; it is necessary because a parent violating T4 is excluded at the parent level — including the d = 1 trailing-zero case, where the resulting stream would itself be T4-valid yet the parent still fails T4 (necessity sub-case (b) below). Condition (ii) follows from the ASN-0034 lemma "TA5 preserves T4": for d ≥ 3, the appended sequence contains adjacent zeros, violating T4's non-empty-field constraint. Condition (iii) ensures no address exceeds the four-level hierarchy; it is binding only at d = 2 (at d = 1 it reduces to T4's own zero bound on p). Together:
...
*Proof.* We prove sufficiency (all three conditions imply T4 preservation) and then necessity (violating any single condition either produces a T4 violation in the stream or — for condition (i) — fails T4 at the parent address itself).
...
**(⟹) Necessity.** We show that violating condition (ii) or (iii) produces a T4 violation in the stream, and that violating condition (i) means p is not a valid parent address (a stream T4 violation also follows, except in the d = 1 trailing-zero case, where S2 makes the stream T4-valid yet the parent still fails T4).
...
*(b) Pure trailing zero as the sole T4 defect: p_{#p} = 0, p₁ > 0, zeros(p) ≤ 3, no adjacent zeros in p (which forces #p ≥ 2, since p₁ > 0 = p_{#p} requires the leading and trailing positions to be distinct).* Condition (i) requires p to satisfy T4, and a pure-trailing-zero p fails T4's t_{#t} ≠ 0 clause — it is not a valid address, so (i) excludes it at the parent level for any d, independent of what its stream looks like. At d = 2 the stream itself also violates T4 (shown next). At d = 1 the stream happens to remain fully T4-valid — by S2, a pure-trailing-zero parent p at d = 1 generates the same stream as the B6-valid namespace obtained by dropping p's trailing zero and incrementing the depth (e.g. ([1, 0], 1) and ([1], 2) share the stream [1, 0, n]). The d = 1 stream therefore supplies no T4 obstruction, but none is needed: (i) excludes p directly because p is not a valid parent address.
...
Condition (i) is therefore necessary: a count violation (zeros(p) > 3) and T4 defects in p's preserved prefix — interior, leading, or the singleton p = [0] — propagate to every stream element via TA5(b) and B5/B5a (sub-case (a)); a pure trailing zero fails T4 at the parent level for any d, additionally creating an adjacent-zero stream violation at d = 2 (sub-case (b)). ∎

*Formal Contract:*
- *Preconditions:* p ∈ T, d ∈ ℕ with d ≥ 1.
- *Postconditions:* (a) Sufficiency: `(p satisfies T4 ∧ d ∈ {1, 2} ∧ zeros(p) + (d − 1) ≤ 3) ⟹ (A n ≥ 1 : cₙ ∈ S(p, d) satisfies T4)`. (b) Necessity: violating (ii) or (iii) forces a stream T4 violation; violating (i) means p is not a valid address. The d = 1 trailing-zero case produces no stream violation (S2), but (i) still excludes p directly because it fails T4.
