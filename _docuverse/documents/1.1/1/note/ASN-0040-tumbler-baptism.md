...
We state B0 (Irrevocability) below as a stand-alone obligation rather than as a corollary of T8 (AllocationPermanence). T8 asserts `allocated(Σ) ⊆ allocated(Σ')` for every transition; under the conditional reverse inclusion `Σ.B ⊆ allocated(Σ)` this would yield `Σ.B ⊆ Σ'.B`, but since the parent prerequisite is open the derivation is not unconditional. Stating B0 directly on Σ.B is moreover essential for the inductive proofs of B1 and B10 to remain legible, and keeps ASN-0040 readable independently of the algebraic derivation in ASN-0034.
...
Equivalently, `(A Σ, Σ' : Σ → Σ' : Σ'.B = Σ.B ∨ Σ'.B = Σ.B ∪ {next(Σ.B, p, d)} for some (p, d) satisfying B6)` — every transition either leaves the registry unchanged or extends it by exactly the address that the corresponding baptismal operation would produce.
...
The frame condition is essential: baptism alters the registry and nothing else. Content storage, link structures, arrangement — all are untouched. The precondition B4 makes each baptize(p, d) a single edge in the transition relation, so next(Σ.B, p, d) is evaluated against the precondition state Σ of the same transition that produces Σ'; no other transition mediates between the evaluation of next and the registry update.
