...
*Proof.* We argue directly from the element structure of the two streams, rather than routing through the foundation's domain-disjointness theorem. That theorem (T10a.6) would require both allocators to inhabit one allocator tree with a defined common ancestor; B6 — T4-validity of the parents and d, d' ∈ {1, 2} — supplies no such common tree (two unrelated T4-valid parents, e.g. [5] and [7], have no derived ancestor), so the hypothesis it needs is precisely what B6 fails to give. The direct argument needs no tree.

Suppose, for contradiction, some `x ∈ S(p, d) ∩ S(p', d')`. By S(p, d)'s postconditions, x is simultaneously
