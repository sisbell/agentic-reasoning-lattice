...
The bridge is the identification of ASN-0034's allocator-extension transitions (those that enlarge some `domₛ(A)`) with a subset of ASN-0040's baptismal operations: every transition that adds an address `a` to some `domₛ(A)` is a baptismal operation `baptize(p, d) ∈ Op` whose postcondition adds the same `a` to Σ.B. (The converse — that every baptismal operation is an allocator extension — is the reverse inclusion, conditional below.) The bridge also covers genesis: `allocated(Σ_init) ⊆ Σ_init.B`, since every address inhabiting an activated allocator's initial domain is presumed to inhabit B₀ by stipulation of B₀ conf. Under the bridge, the inclusion is preserved at every transition. ASN-0034's non-extending transitions leave `allocated` fixed; allocator extensions, by the bridge, are baptismal operations that add the same element to both sets; B0a forbids any non-baptismal enlargement of Σ.B, so Σ.B never grows in a way that would let `allocated` outpace it. By induction from genesis, `allocated(Σ) ⊆ Σ.B` in every reachable state. The bridge — that allocator-extension transitions are baptismal operations, and that `allocated(Σ_init) ⊆ B₀` — is an obligation across the two ASNs, not a theorem of either alone; without it, the inclusion is unjustified.
...
*Formal Contract:*
- *Preconditions:* p ∈ T, d ∈ ℕ with d ≥ 1; B6(p, d) holds; Σ.B satisfies B1 and B10.
