...
**Correspondence to the allocator framework.** The sibling stream S(p, d) is exactly the domain of a foundation allocator (T10a, ASN-0034). Its base c₁ = inc(p, d) is a child-spawn output inc(p, d) — with d ∈ {1, 2} under B6(ii) this is precisely T10a's permitted spawning increment — and its successive elements are generated by repeated inc(·, 0), matching T10a's domain form dom(A) = {tₙ : n ≥ 0} with tₙ₊₁ = inc(tₙ, 0) (offset by one index: t_{n−1} = cₙ). Write A(p, d) for this allocator, so dom(A(p, d)) = S(p, d). When (p, d) satisfies B6, p is T4-valid (B6(i)) and B6's sufficiency result (§B6) makes every output of A(p, d) T4-valid, so A(p, d) conforms to T10a. We use this correspondence to *inherit* the foundation's enumeration and disjointness results — T10a.7 and T10a.6 — rather than re-deriving them. Strict ordering (S0) needs only the inc(·, 0)-enumeration form, so it holds for any p ∈ T, d ≥ 1; disjointness (B7) needs full T10a-conformance, supplied there by B6.

**S0 (StreamOrdering).** `(A i, j : 1 ≤ i < j : cᵢ < cⱼ)`.

*Proof.* S(p, d) is the inc(·, 0)-enumeration of the allocator A(p, d): base c₁ = inc(p, d), then cₙ₊₁ = inc(cₙ, 0). T10a.7 (EnumerationInjectivity, ASN-0034) establishes that such an enumeration is strictly increasing under the tumbler order T1 — its contract concludes `(A m, n : m < n : tₘ < tₙ)`, proved from TA5(a) (per-step strict increase) and T1 transitivity/irreflexivity. Transporting through the index correspondence t_{n−1} = cₙ yields `(A i, j : 1 ≤ i < j : cᵢ < cⱼ)`. The base c₁ = inc(p, d) ∈ T (TA5(d)) and each cₙ ∈ T (TA5(c)) supply the well-formed operands. ∎
...
- *Depends:* T10a.7 (EnumerationInjectivity — the inc(·, 0)-enumeration of an allocator is strictly increasing under T1), applied to A(p, d) via the allocator correspondence above; the foundation result itself rests on TA5(a) and T1.
...
*Proof.* We reduce to the foundation's domain-disjointness theorem rather than re-deriving it. By the allocator correspondence (§S(p,d)), each B6-valid pair names a T10a-conforming allocator A(p, d) with dom(A(p, d)) = S(p, d); the allocators all sit in the common allocator tree rooted at the seed. The claim is therefore `S(p, d) ∩ S(p', d') = dom(A(p, d)) ∩ dom(A(p', d')) = ∅`, which T10a.6 (DomainDisjointness, ASN-0034) delivers for any two *distinct* conforming allocators — covering both the ancestor–descendant case (length separation) and the prefix-incomparable case that the earlier hand-written argument re-walked. It remains only to show that distinct B6-valid pairs name *distinct* allocators — equivalently, distinct base addresses `inc(p, d) ≠ inc(p', d')`. This *namespace injectivity* is the genuinely new obligation; without B6(i) it fails (aliasing, below).

*Namespace injectivity.* Suppose `inc(p, d) = inc(p', d')` with both pairs B6-valid. By TA5(d), inc(p, d) has length #p + d (first #p components from p, then d − 1 zeros, then 1) and symmetrically inc(p', d') has length #p' + d', so #p + d = #p' + d'. If #p = #p', then d = d', and the first #p components of the shared base equal both p and p', so p = p' by T3 (CanonicalRepresentation), whence (p, d) = (p', d') — not distinct. Otherwise #p ≠ #p'; with d, d' ∈ {1, 2} (B6(ii)) and equal totals the lengths differ by at most 1, so WLOG #p' = #p + 1, d = 2, d' = 1. Read position #p + 1 of the shared base two ways: from the (p, 2) form it is the lone zero separator (value 0); from the (p', 1) form (no separator) positions 1 … #p' = 1 … #p + 1 are preserved from p', so it equals p'_{#p'}, the last component of p'. Since (p', d') satisfies B6, p' satisfies T4, whose field-segment constraint forbids a zero final component (TA5-SigValid: p'_{#p'} ≠ 0). The two readings, 0 and a nonzero value, are incompatible, ruling out #p ≠ #p'. Hence `inc(p, d) = inc(p', d')` forces (p, d) = (p', d'); contrapositively, distinct B6-valid pairs give distinct bases, hence distinct allocators A(p, d) ≠ A(p', d').

With distinctness established, T10a.6 gives `dom(A(p, d)) ∩ dom(A(p', d')) = S(p, d) ∩ S(p', d') = ∅`. ∎

Dropping B6(i) admits aliasing: a pure-trailing-zero parent and its truncation at the next depth produce the identical base, hence the identical stream — e.g. ([1, 0], 1) and ([1], 2) both yield base [1, 0, 1] and stream {[1, 0, n] : n ≥ 1}. B6(i) excludes the pure-trailing-zero parent ([1, 0] fails T4), which is exactly what namespace injectivity needs.

*Formal Contract:*
- *Preconditions:* (p, d) and (p', d') both satisfy B6, with (p, d) ≠ (p', d').
- *Postconditions:* `S(p, d) ∩ S(p', d') = ∅`.
- *Depends:* allocator correspondence (§S(p,d): dom(A(p, d)) = S(p, d), A(p, d) conforming to T10a under B6), T10a.6 (DomainDisjointness — distinct conforming allocators have disjoint domains, the disjointness core), B6 (T4-validity and d ∈ {1, 2} of both parents, underwriting conformance and namespace injectivity), TA5(d) (base-address length and component structure), T3 (CanonicalRepresentation — equal-length parents share the base prefix, forcing parent equality), T4 / TA5-SigValid (valid parent has a nonzero last component, excluding the unequal-length alias).
...
| S0 | `(A i, j : 1 ≤ i < j : cᵢ < cⱼ)` — stream strictly ordered | from T10a.7 (via allocator correspondence) |
...
| B7 | `(p, d) ≠ (p', d') ⟹ S(p, d) ∩ S(p', d') = ∅` — namespace disjointness | from allocator correspondence, T10a.6, B6, TA5(d), T3, T4/TA5-SigValid |
