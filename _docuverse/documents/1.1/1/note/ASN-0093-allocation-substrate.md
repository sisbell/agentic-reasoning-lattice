...
**SubAllocatorAxiom (Axiom, ContentLinkSubAllocatorChainDiscipline).** For each `d ∈ dom(M)`, the content and link sub-allocator chains under `d` are the ASN-0040 sibling streams `A_C(d) = S(b_C(d), 1)` and `A_L(d) = S(b_L(d), 1)` (SiblingStream, ASN-0040). We refer to this clause as SubAllocatorAxiom.ChainDiscipline.
...
*Proof.* The content and link cases follow parallel structure with one substitution rule: for the cross-document freshness sub-proof, the content case uses `b_C` and ChainMembershipForOrigin's content clause backed by C2; the link case uses `b_L` and ChainMembershipForOrigin's link clause backed by L1a. For the cross-subspace freshness sub-proof, the content case applies T7 against `dom(L)` (peer `E(ℓ)₁ = s_L`, new key `E(a)₁ = s_C`); the link case applies T7 against `dom(C)` (peer `E(a)₁ = s_C`, new key `E(ℓ)₁ = s_L`) — the same SC-NEQ disjunction, read in the reverse direction. In both cross-subspace sub-proofs the new key's subspace identifier is read from its FirstEmission structural form rather than from L0: the new key is not yet committed at the firing event, and invoking L0 at `Σ'` would be circular under the simultaneous induction (L0 at `Σ'` itself depends on FirstEmissionFreshness). The substitution rule carries this caveat across both cases.
