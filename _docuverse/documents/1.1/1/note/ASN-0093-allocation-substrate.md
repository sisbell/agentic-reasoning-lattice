...
*Content case, against `dom(L)`.* StoreT4Validity at `Σ` gives T4-validity of every `ℓ ∈ dom(L)`; `a` is T4-valid by ChainElementT4Validity applied to `A_C(d)` (whose first emission `[d.0.s_C.1]` is T4-valid by the FirstEmission lemma). The subspace identifiers split by source: by L0 at `Σ`, `E(ℓ)₁ = s_L` for the pre-existing peer `ℓ ∈ dom(L)`; for the new key `a` we read `E(a)₁ = s_C` from the FirstEmission lemma's structural form `a = [d.0.s_C.1]` rather than from L0, since `a` is not yet committed at the firing event and invoking L0 at `Σ'` would be circular under the simultaneous induction (L0 at `Σ'` itself depends on FirstEmissionFreshness). By SC-NEQ, `s_C ≠ s_L`; `zeros(a) = zeros(ℓ) = 3` by FirstEmission's structural form and L1. T7 (FirstElementFieldDistinction, ASN-0034) closes: `a ≠ ℓ`.
...
*Link case, against `dom(C)`.* StoreT4Validity at `Σ` gives T4-validity of every `a ∈ dom(C)`; `ℓ` is T4-valid by ChainElementT4Validity applied to `A_L(d)` (whose first emission `[d.0.s_L.1]` is T4-valid by the FirstEmission lemma). The subspace identifiers split by source: by L0 at `Σ`, `E(a)₁ = s_C` for the pre-existing peer `a ∈ dom(C)`; for the new key `ℓ` we read `E(ℓ)₁ = s_L` from the FirstEmission lemma's structural form `ℓ = [d.0.s_L.1]` rather than from L0, by the same non-circularity reasoning as the content case (`ℓ` is uncommitted at the firing event, so L0 at `Σ'` is unavailable). By SC-NEQ, `s_C ≠ s_L`; `zeros(ℓ) = zeros(a) = 3` by FirstEmission's structural form and C1. T7 (FirstElementFieldDistinction, ASN-0034) closes: `ℓ ≠ a`. ∎
...
- *Link withdrawal.* The substrate admits no withdrawal of `dom(L)` entries; Nelson's tombstone-style withdrawal (LM 4/9) is deferred to a future tombstoning ASN. The load-bearing constraint that ASN must revisit is L12's value-equality clause `L'(a) = L(a)`: any withdrawal modelled as a value transition on `L(a)` must restate L12 (its domain-persistence clause may stand), while arrangement-side or embedded-marker formulations leave L12 intact.
