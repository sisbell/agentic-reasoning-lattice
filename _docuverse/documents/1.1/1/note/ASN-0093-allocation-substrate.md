...
- **Chain disciplines (ASN-0040 citations):** ChainElementT4Validity (B6(a)), ChainUniformLength + ChainUniformZeroCount (SiblingStream postconditions / B5a), ChainEnumerationInjectivity (S0), ChainPrefixExtension (S1), DisjointSubAllocatorChains (B7) — each named for local reference and discharged by the cited ASN-0040 result, not redeveloped
...
Derived from L0 + SC-NEQ + StoreT4Validity + T7 (FirstElementFieldDistinction, ASN-0034): every content address has `E(·)₁ = s_C`, every link address has `E(·)₁ = s_L`, and `s_C ≠ s_L`, so the domains are disjoint. StoreT4Validity is required to discharge T7's T4-validity precondition for the compared pair `(a ∈ dom(C), ℓ ∈ dom(L))`; it is a transition-indexed lemma derived later in this note (from ChainMembershipForOrigin + ChainElementT4Validity), and the simultaneous-induction discipline (see *Simultaneous-induction framing* below) underwrites its availability at every reachable `Σ` where L14 is consumed.
...
**Sub-allocator chains are ASN-0040 sibling streams.** Each sub-allocator chain is an instance of ASN-0040's `SiblingStream`. Writing `S(p, k)` for the stream `c₁ = inc(p, k)`, `cₙ₊₁ = inc(cₙ, 0)` (SiblingStream, ASN-0040):

  `A_C(d) = S(b_C(d), 1)`  and  `A_L(d) = S(b_L(d), 1)`,

since each chain's first emission is `inc(anchor, 1)` — `t_1^C(d) = inc(b_C(d), 1) = [d.0.s_C.1]`, `t_1^L(d) = inc(b_L(d), 1) = [d.0.s_L.1]` — and successive elements advance by `inc(·, 0)`. The depth parameter is `1` in both cases, so the streams append no interior zero.

Both anchors satisfy ASN-0040's `B6` (ValidDepth) at depth `1`: `b_C(d)` and `b_L(d)` are T4-valid with `zeros = 3` (one separator inserted at position `#d + 1` under M0's T4-valid, `zeros = 2` document `d`), depth `1 ∈ {1, 2}`, and `zeros(b_·(d)) + (1 − 1) = 3 ≤ 3`. Consequently every ASN-0040 result whose precondition is `B6(p, d)` — `B6(a)`'s stream-T4-validity conclusion, the `SiblingStream` postconditions, `S0` (StreamOrdering), `S1` (StreamPrefix), `B5a` (SiblingZerosPreservation), `B7` (NamespaceDisjointness), `B1` (ContiguousPrefix), and `B9` (UnboundedExtent) — applies to `A_C(d)` and `A_L(d)` directly, without redevelopment from the increment primitives.

ASN-0040's `SiblingStream` is infinite — defined for every `n ≥ 1`. This is load-bearing: the substrate's per-chain disciplines conclude universally over `n ≥ 1`, and K.α/K.λ admit unbounded subsequent emissions (B9, UnboundedExtent, ASN-0040). A chain element exists at every index, including those past any finite truncation point, so the emission rule's pinning step always has a chain element to land on.

The substrate imports only ASN-0040's stream-level results, whose preconditions are `B6(b_·(d), 1)` (discharged above) and which require no allocator-tree embedding. It therefore makes no commitment about whether an implementation realises `A_C(d)`/`A_L(d)` as standalone allocators with spawning triples or as discipline-conforming streams within a flatter structure.

**SubAllocatorAxiom (Axiom, ContentLinkSubAllocatorExistence).** For each `d ∈ dom(M)`, two sub-allocator chains are simultaneously activated under `d` at the moment of `d`'s registration into `dom(M)` (by `K.σ`). The substrate commits the chains to be the ASN-0040 sibling streams `A_C(d) = S(b_C(d), 1)` and `A_L(d) = S(b_L(d), 1)` (SiblingStream, ASN-0040), without claiming that they are embedded in any global allocator tree. Three clauses, independently citable as discharge premises:

- *Existence (SubAllocatorAxiom.Exists).* For every `d ∈ dom(M)`, the content sub-allocator chain `A_C(d)` (anchored at `b_C(d)`) and the link sub-allocator chain `A_L(d)` (anchored at `b_L(d)`) are active (per the *Active sub-allocator chains* definition above). The axiom commits *activation at every `Σ` in which `d ∈ dom(M)`* — it does not directly assert "once active, always active." The permanence reading (sub-allocator chains remain active at every successor state once `d` enters `dom(M)`) is established *inductively along with M1*, not as an after-the-fact corollary that requires M1 to be in scope independently. Concretely, the simultaneous induction over transition sequences from `Σ₀` (described in the *Simultaneous-induction framing* paragraph below) carries M1 as one of its transition-indexed conjuncts; at each step `Σ → Σ'`, the inductive hypothesis at the pre-state `Σ` includes M1 at `Σ` together with the per-state activation reading of this axiom at every `d ∈ dom(M(Σ))`, and the inductive step exhibits (a) M1 at `Σ'` (via the discharge matrix below) and (b) the per-state activation reading of this axiom at every `d ∈ dom(M(Σ'))` — which, via M1 at `Σ'`, includes every `d` already in `dom(M(Σ))`. The permanence reading is therefore not derived from M1 *after* the induction completes; it is a fact whose preservation across each transition step is part of what the simultaneous induction inductively establishes. The base case at `Σ₀ = (∅, ∅, ∅)` holds vacuously since `dom(M(Σ₀)) = ∅`. The axiom itself remains a per-state existence statement; the across-state lift is what the simultaneous induction adds.
...
- *Chain discipline (SubAllocatorAxiom.ChainDiscipline).* Each chain `A_C(d)` (resp. `A_L(d)`) is the ASN-0040 sibling stream `S(b_C(d), 1)` (resp. `S(b_L(d), 1)`), rooted at FirstEmission's `t_1^C(d) = inc(b_C(d), 1)` (resp. `t_1^L(d) = inc(b_L(d), 1)`) and extended by `inc(·, 0)`. The stream's parent `(b_·(d), 1)` satisfies `B6` (verified under *Sub-allocator chains are ASN-0040 sibling streams* above), so ASN-0040's stream results apply; the substrate makes no commitment about whether an implementation realises the chains as standalone allocators or as discipline-conforming streams within a flatter structure. The per-chain disciplines below are direct ASN-0040 citations.
...
**Per-chain disciplines (ASN-0040 citations).** Each discipline below is named for the substrate's local reference and discharged by the cited ASN-0040 result applied to the sibling stream `A_C(d) = S(b_C(d), 1)` (resp. `A_L(d) = S(b_L(d), 1)`), whose parent `(b_·(d), 1)` is `B6`-valid (verified above). None is reproved from the increment primitives; the corollary forms consumed downstream are stated inline.

- **ChainElementT4Validity.** Every element of `A_C(d)` (resp. `A_L(d)`) is T4-valid. *Source: ASN-0040 B6(a) (ValidDepth sufficiency)* — for `B6`-valid `(p, d)`, every `cₙ ∈ S(p, d)` satisfies T4. Consumed at ChainUniformZeroCount, DisjointSubAllocatorChains, StoreT4Validity, FirstEmissionFreshness, and the K.α/K.λ subsequent-emit cross-subspace freshness derivations.
...
- **ChainEnumerationInjectivity.** The enumeration of `A_C(d)` (resp. `A_L(d)`) is strictly increasing under T1, `m < n ⟹ t_m < t_n`; hence `n ↦ t_n` is injective and (by T1 trichotomy) order-preserving in both directions, `m < n ⟺ t_m < t_n`. *Source: ASN-0040 S0 (StreamOrdering)* — `(A i, j : 1 ≤ i < j : cᵢ < cⱼ)`. This discharges within-chain freshness at the K.α/K.λ subsequent-emit branches.
...
- **ChainPrefixExtension.** Every element of an active sub-allocator chain extends its anchor under the prefix relation:
  ...
  *Quantifier scope.* S1 ranges over the abstract stream `S(b_·(d), 1)` at every index `n ≥ 1`, independent of which elements are committed to `dom(C)` (resp. `dom(L)`) at `Σ`. The K.α and K.λ subsequent-emit derivations exploit this by citing the prefix relation at a freshly emitted address `a` (resp. `ℓ`) — a stream element by closure under `inc(·, 0)` applied to `a_prev` (resp. `ℓ_prev`) — *before* it is committed. Consumed in (i) the FirstEmissionFreshness lemma below; (ii) the K.α/K.λ subsequent-emit cross-document freshness derivations; (iii) the ChainMembershipForOrigin lemma's contiguous-prefix postcondition below.

The substrate's transition-indexed proofs (ChainMembershipForOrigin, StoreT4Validity, FirstEmissionFreshness, and the discharge matrix entries) cite these per-chain disciplines freely, since each holds once-and-for-all on the sibling stream `S(b_·(d), 1)` the instant `d` is registered.
...
*Proof.* The content and link cases follow parallel structure with one substitution rule: for the cross-document freshness sub-proof, the content case uses `b_C` and ChainMembershipForOrigin's content clause backed by C2; the link case uses `b_L` and ChainMembershipForOrigin's link clause backed by L1a. For the cross-subspace freshness sub-proof, the content case applies T7 against `dom(L)` (peer `E(ℓ)₁ = s_L`, new key `E(a)₁ = s_C`); the link case applies T7 against `dom(C)` (peer `E(a)₁ = s_C`, new key `E(ℓ)₁ = s_L`) — the same SC-NEQ disjunction, read in the reverse direction.
...
*Content case, against `dom(L)`.* StoreT4Validity at `Σ` gives T4-validity of every `ℓ ∈ dom(L)`; `a` is T4-valid by ChainElementT4Validity applied to `A_C(d)` (whose first emission `[d.0.s_C.1]` is T4-valid by SubAllocatorAxiom.FirstEmission). The subspace identifiers split by source: by L0 at `Σ`, `E(ℓ)₁ = s_L` for the pre-existing peer `ℓ ∈ dom(L)`; by SubAllocatorAxiom.FirstEmission's structural form `a = [d.0.s_C.1]`, `E(a)₁ = s_C` by inspection. (The new key `a` is not yet committed to `dom(C)` at the K.α event firing the first-emit predicate, so L0 at `Σ` does not apply to `a`; L0 at `Σ'` would be circular since L0 at `Σ'` itself depends on FirstEmissionFreshness via the discharge matrix's K.α entry.) By SC-NEQ, `s_C ≠ s_L`; `zeros(a) = zeros(ℓ) = 3` by FirstEmission's structural form and L1. T7 (FirstElementFieldDistinction, ASN-0034) closes: `a ≠ ℓ`.
...
*Link case, against `dom(C)`.* StoreT4Validity at `Σ` gives T4-validity of every `a ∈ dom(C)`; `ℓ` is T4-valid by ChainElementT4Validity applied to `A_L(d)` (whose first emission `[d.0.s_L.1]` is T4-valid by SubAllocatorAxiom.FirstEmission). The subspace identifiers split by source: by L0 at `Σ`, `E(a)₁ = s_C` for the pre-existing peer `a ∈ dom(C)`; by SubAllocatorAxiom.FirstEmission's structural form `ℓ = [d.0.s_L.1]`, `E(ℓ)₁ = s_L` by inspection. (The new key `ℓ` is not yet committed to `dom(L)` at the K.λ event firing the first-emit predicate, so L0 at `Σ` does not apply to `ℓ`; L0 at `Σ'` would be circular since L0 at `Σ'` itself depends on FirstEmissionFreshness via the discharge matrix's K.λ entry.) By SC-NEQ, `s_C ≠ s_L`; `zeros(ℓ) = zeros(a) = 3` by FirstEmission's structural form and C1. T7 (FirstElementFieldDistinction, ASN-0034) closes: `ℓ ≠ a`. ∎
...
The chain-indexed disciplines (ChainElementT4Validity, ChainUniformLength, ChainEnumerationInjectivity, ChainUniformZeroCount, DisjointSubAllocatorChains, ChainPrefixExtension) are not state-dependent in their conclusions and require no per-transition discharge — each is an ASN-0040 citation holding once-and-for-all on the sibling stream `S(b_·(d), 1)`.
...
| Sub-allocator chains | A_C(d), A_L(d) as ASN-0040 sibling streams | DEF | `A_C(d) = S(b_C(d), 1)` and `A_L(d) = S(b_L(d), 1)` (SiblingStream, ASN-0040): `c₁ = inc(anchor, 1)` (the first emission), `cₙ₊₁ = inc(cₙ, 0)`. Each parent `(b_·(d), 1)` is `B6`-valid (T4-valid anchor, `zeros = 3`, depth `1`, `zeros + (1−1) = 3 ≤ 3`), so all `B6`-preconditioned ASN-0040 results apply. Consumed by every per-chain discipline below and by SubAllocatorAxiom.ChainDiscipline. |
| SubAllocatorAxiom | ContentLinkSubAllocatorExistence | AXIOM | Three clauses: Exists, FirstEmission (structural form only — `[d.0.s_C.1]` resp. `[d.0.s_L.1]`, both T4-valid by inspection), ChainDiscipline (sub-allocator chains are the ASN-0040 sibling streams `S(b_·(d), 1)`). The earlier-draft *Disjointness* and *FirstEmission's freshness conclusion* clauses are restated below as derived lemmas DisjointSubAllocatorChains and FirstEmissionFreshness, not retained as axiom content. |
| ChainElementT4Validity | ChainElementT4Validity | CITATION | Every element of `A_C(d)` (resp. `A_L(d)`) is T4-valid. Cites ASN-0040 B6(a) (ValidDepth sufficiency) at `B6(b_·(d), 1)`. Consumed at ChainUniformZeroCount, DisjointSubAllocatorChains, StoreT4Validity, FirstEmissionFreshness, and K.α/K.λ subsequent-emit cross-subspace freshness derivations. |
...
| ChainEnumerationInjectivity | ChainEnumerationInjectivity | CITATION | The enumeration of `A_C(d)` (resp. `A_L(d)`) is strictly increasing under T1; hence injective and (by T1 trichotomy) order-preserving in both directions. Cites ASN-0040 S0 (StreamOrdering). Consumed at ChainMembershipForOrigin's subsequent-emit step and at K.α/K.λ subsequent-emit within-document freshness derivations. |
...
| DisjointSubAllocatorChains | DisjointSubAllocatorChains | CITATION | `A_C(d) ∩ A_L(d) = ∅`, with addresses of `A_C(d)` (resp. `A_L(d)`) carrying `E(·)₁ = s_C` (resp. `s_L`). Cites ASN-0040 B7 (NamespaceDisjointness) at the distinct `B6`-valid parents `(b_C(d), 1)`, `(b_L(d), 1)` (anchors disagree at position `#d + 2`, `s_C` vs `s_L`, by SC-NEQ). Replaces the former SubAllocatorAxiom.Disjoint axiom clause. |
| ChainPrefixExtension | ChainPrefixExtension | CITATION | Every chain element extends its anchor under Prefix: `b_C(d) ≼ t` for `t ∈ A_C(d)`, `b_L(d) ≼ t` for `t ∈ A_L(d)`. Cites ASN-0040 S1 (StreamPrefix) at `p = b_·(d)`; the abstract-stream quantifier covers chain elements not yet committed. Consumed in cross-document freshness derivations (K.α/K.λ first-emit and subsequent-emit) and in the FirstEmissionFreshness lemma. |
