...
The infinite recurrence is well-formed because TA5 (HierarchicalIncrement, ASN-0034) at `k = 0` carries the postcondition `t' ∈ T` whenever `t ∈ T`, so applying `inc(·, 0)` at every step preserves carrier-set membership; together with `t_1 ∈ T` (a consequence of FirstElementValidity, since T4-validity is a predicate on the carrier `T`), the recurrence's pointwise output `t_{n+1} = inc(t_n, 0) ∈ T` is well-defined at every `n ≥ 1` by straightforward induction over the index. No further foundation premise is required to witness the existence of the sequence within `T`.
...
*Proof.* By SubAllocatorAxiom.FirstEmission, the first emission `t_1^C(d) = [d.0.s_C.1]` of `A_C(d)` has `E(·)₁ = s_C` at position `#d + 2`; the first emission `t_1^L(d) = [d.0.s_L.1]` of `A_L(d)` has `E(·)₁ = s_L` at position `#d + 2`. Both first emissions have length `#d + 3`. By ChainUniformLength, all elements of `A_C(d)` (resp. `A_L(d)`) have length `#d + 3`. The chain-wide preservation of the value at position `#d + 2` is established by induction on the chain index `n`:

*Base (n = 1).* `t_1^C(d)` has `E₁ = s_C` at position `#d + 2` by FirstEmission's structural form (and analogously `t_1^L(d)` has `E₁ = s_L` at the same position).

*Step (n + 1).* Assume `t_n^C(d)` has value `s_C` at position `#d + 2` (link case: `s_L`). By ChainElementT4Validity, `t_n^C(d)` is T4-valid; TA5-SigValid then gives `sig(t_n^C(d)) = #t_n^C(d) = #d + 3`. By SiblingRecurrence, `t_{n+1}^C(d) = inc(t_n^C(d), 0)`. TA5(b) at `k = 0` supplies positional agreement at positions `1..#t_n^C(d)` except at `sig(t_n^C(d))` — i.e., at every position `i ≠ sig(t_n^C(d))` with `1 ≤ i ≤ #t_n^C(d)`, `t_{n+1}^C(d)[i] = t_n^C(d)[i]`. Position `#d + 2` is in the preserved range (`#d + 2 ≤ #t_n^C(d) = #d + 3`) and is not the signature position (`#d + 2 ≠ #d + 3 = sig(t_n^C(d))`). Hence `t_{n+1}^C(d)[#d + 2] = t_n^C(d)[#d + 2] = s_C` by IH. The same argument with `s_L` in place of `s_C` and `A_L(d)` in place of `A_C(d)` establishes `t_{n+1}^L(d)[#d + 2] = s_L`.

Hence every element of `A_C(d)` inherits `E₁ = s_C` from `t_1^C(d)` at the chain-wide index range, and every element of `A_L(d)` inherits `E₁ = s_L` from `t_1^L(d)`. SC-NEQ then forces the two chains' images to be disjoint. ∎
...
*Step, content case (chain index `n + 1`).* Assume `b_C(d) ≼ t_n` for `t_n ∈ A_C(d)`. The next element is `t_{n + 1} = inc(t_n, 0)`. By ChainUniformLength (corollary on sub-allocator chains), `#t_n = #d + 3`. By ChainElementT4Validity (corollary on sub-allocator chains), `t_n` is T4-valid — supplied as a standalone fact established by a prior chain induction, not by a nested induction inside the present proof. TA5-SigValid (SigOnValidAddresses, ASN-0034) then pins `sig(t_n) = #t_n = #d + 3`. TA5(c) gives `#t_{n + 1} = #t_n` and confines the modification to position `sig(t_n) = #t_n` (TA5(b) and TA5(c)'s single-position-modification clause jointly preserve positions `1..#t_n − 1`). Since `#b_C(d) = #d + 2 = #t_n − 1`, the prefix `b_C(d)` lives entirely within the preserved positional range, so `t_{n + 1}` agrees with `t_n` (and thus with `b_C(d)`) at positions `1..#b_C(d)`. The length condition `#b_C(d) = #d + 2 ≤ #d + 3 = #t_{n + 1}` holds. By Prefix, `b_C(d) ≼ t_{n + 1}`.

*Step, link case (chain index `n + 1`).* Assume `b_L(d) ≼ t_n` for `t_n ∈ A_L(d)`. The next element is `t_{n + 1} = inc(t_n, 0)`. By ChainUniformLength applied to `A_L(d)` (whose first emission `t_1^L(d) = [d.0.s_L.1]` has length `#d + 3`), `#t_n = #d + 3`. By ChainElementT4Validity applied to `A_L(d)`, `t_n` is T4-valid — supplied as a standalone fact established by a prior chain induction, not by a nested induction inside the present proof. TA5-SigValid then pins `sig(t_n) = #t_n = #d + 3`. TA5(c) gives `#t_{n + 1} = #t_n` and confines the modification to position `sig(t_n) = #t_n` (TA5(b) and TA5(c)'s single-position-modification clause jointly preserve positions `1..#t_n − 1`). Since `#b_L(d) = #d + 2 = #t_n − 1`, the prefix `b_L(d)` lives entirely within the preserved positional range, so `t_{n + 1}` agrees with `t_n` (and thus with `b_L(d)`) at positions `1..#b_L(d)`. The length condition `#b_L(d) = #d + 2 ≤ #d + 3 = #t_{n + 1}` holds. By Prefix, `b_L(d) ≼ t_{n + 1}`. ∎
...
*Case B — Prefix-incomparable.* `d₁ ⋠ d₂ ∧ d₂ ⋠ d₁` at the document level. The joint conjunction does not directly yield a position divergence at `k ≤ min(#d₁, #d₂)`: in asymmetric-length subcases, one of the two `⋠` clauses is satisfied by the failure of Prefix's length conjunct alone, supplying no component-divergence witness; the position-divergence witness must be extracted from the *other* clause. We case-split on the length relationship between `d₁` and `d₂`, exhaustive by NAT-order's at-least-one trichotomy at `(#d₁, #d₂)`. Sub-cases B.i (hypothesis `#d₁ ≤ #d₂`, covering `<` and `=`) and B.ii (hypothesis `#d₂ < #d₁`, strict `>`) are exhaustive and mutually exclusive, partitioning NAT-order's trichotomy: `≤` and `>` are disjoint relations on ℕ, and their union covers all three trichotomy disjuncts (`<`, `=`, `>`). At equality `#d₁ = #d₂`, only B.i fires (B.ii's strict `#d₂ < #d₁` fails); the proof's witness extraction may equivalently proceed from `d₂ ⋠ d₁` (a mirror argument) rather than `d₁ ⋠ d₂`, since at equality both `⋠` conjuncts admit component witnesses at indices within `min(#d₁, #d₂)`. The mirror reading is an alternative proof structure that extracts the same divergence position from the other conjunct of the joint `⋠`-pair, not a separate sub-case being triggered.
...
*Verifying the Cross-document disjointness lemma at `Σ₉` for the prefix-incomparable pair `(d, d_alt)` — Case B sub-case B.i.* The document prefixes are prefix-incomparable: position 3 of `d = [1, 0, 2, 0, 5]` is `2`, position 3 of `d_alt = [1, 0, 3, 0, 7]` is `3`, both within native domains `{1, …, 5}`, so neither is a prefix of the other. Length comparison: `#d = 5 = #d_alt`, so NAT-order's trichotomy at `(#d, #d_alt)` selects sub-case **B.i** (`#d ≤ #d_alt`, with equality holding here). At equality `#d = #d_alt`, only B.i fires — B.ii's strict `#d_alt < #d` fails, so B.i and B.ii remain mutually exclusive even at equality (B.i covers the `=` disjunct, B.ii covers the strict `>` disjunct, with `<` also in B.i). The proof's witness extraction may equivalently proceed from `d_alt ⋠ d` (a mirror argument) rather than `d ⋠ d_alt`, since at equality both `⋠` conjuncts admit component witnesses at indices within `min(#d, #d_alt)`; we pursue the `d ⋠ d_alt` reading below, but the mirror argument extracts the same position-divergence witness. The length conjunct `#d ≤ #d_alt` of `d ≼ d_alt` holds, so `d ⋠ d_alt` must be witnessed by failure of the component conjunct — at position `i = 3`, `d[3] = 2 ≠ 3 = d_alt[3]`. Take `k := 3`. Then `k = 3 ≤ 5 = #d = #d_alt = min(#d, #d_alt)` ✓, and `d[k] ≠ d_alt[k]` ✓.
...
*Exercising sub-case B.i with strict `#d_1 < #d_2`.* The pairs verified above cover B.i at equality (`#d = #d_alt = 5`) and B.ii at strict (`#d_alt < #d'`); the remaining structural sub-case is B.i at strict (`#d_1 < #d_2`). Consider a hypothetical fourth document `d_alt' = [3, 0, 7, 0, 11, 13]` — `#d_alt' = 6`, with zeros at positions 2 and 4 (`zeros(d_alt') = 2`), no adjacent zeros, `d_alt'[1] = 3 ≠ 0` and `d_alt'[6] = 13 ≠ 0`, hence T4-valid. Compare with `d = [1, 0, 2, 0, 5]`: position 1 of `d` is `1`, position 1 of `d_alt'` is `3`, both within native domains `{1, …, 5}` and `{1, …, 6}` respectively, so neither is a prefix of the other. Length comparison: `#d = 5 < 6 = #d_alt'`, so NAT-order's trichotomy at `(#d, #d_alt')` selects sub-case **B.i** at strict (`#d_1 < #d_2`, with strict `<`). The witness extraction works through `d ⋠ d_alt'` alone (not symmetrically — at strict `<`, `d_alt' ⋠ d` is already satisfied by length-disjunct failure of `d_alt' ≼ d`, supplying no component witness on that side). The length conjunct `#d ≤ #d_alt'` of `d ≼ d_alt'` holds (`5 ≤ 6` ✓), so `d ⋠ d_alt'` must be witnessed by failure of the component conjunct — at position `i = 1`, `d[1] = 1 ≠ 3 = d_alt'[1]`. Take `k := 1`. Then `k = 1 ≤ 5 = #d = min(#d, #d_alt')` ✓, and `d[k] ≠ d_alt'[k]` ✓. Lift to the anchors `p₁ = b_L(d) = [1, 0, 2, 0, 5, 0, 2]` (length 7) and `p₂ = b_L(d_alt') = [3, 0, 7, 0, 11, 13, 0, 2]` (length 8). NAT-addcompat's strict successor lifts `#d_i ≤ #p_i`, so `min(#d, #d_alt') = 5 ≤ 7 = min(#p₁, #p₂)`. The anchors agree with their documents at positions `1..#d_i`, so `p₁[1] = d[1] = 1 ≠ 3 = d_alt'[1] = p₂[1]` at index `k = 1 ≤ min(#p₁, #p₂) = 7`. This witnesses `b_L(d) ⋠ b_L(d_alt') ∧ b_L(d_alt') ⋠ b_L(d)` via the position-divergence clause of Prefix; the `p₂ ⋠ p₁` direction is also independently discharged by the length-disjunct route (`#p₂ = 8 > 7 = #p₁`). T10 (PartitionIndependence, ASN-0034) then forces every link allocated under `d` to differ from every link allocated under `d_alt'`. Sub-case B.i at strict `<` thus fires correctly against a concrete document pair.
...
The extended example confirms invariants M0, M1, C0–C2, C-fin, L0–L14, L-fin at each successor state across three documents, exercises both first-emit and subsequent-emit branches of K.α (Steps 2, 4, 6) and K.λ (Steps 3, 7, 8), verifies the Cross-document disjointness lemma in both Case A (prefix-comparable, Step 5 with `d ≼ d'`) and Case B (prefix-incomparable; sub-case B.i at equality `#d = #d_alt` and at strict `#d_1 < #d_2` against the hypothetical `d_alt' = [3, 0, 7, 0, 11, 13]`, sub-case B.ii at strict `#d_alt < #d'`, all in Step 9), underwrites cross-document freshness at Steps 6, 7, and 8, and exhibits the ChainMembershipForOrigin lemma's contiguous-prefix postcondition together with the ChainPrefixExtension lemma at every emission past the first.
