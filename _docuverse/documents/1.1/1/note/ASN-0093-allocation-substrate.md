...
*Derived structural facts.* The chain-emission clause pins `a` uniquely from `(d, Σ)`, and the following are its consequences, not independent constraints — listed for readability and to mark which invariant each discharges: `zeros(a) = 3` (ChainUniformZeroCount — C1), `E(a)₁ = s_C` (DisjointSubAllocatorChains — L0, see L0 discharge note), `#E(a) ≥ 2` (FirstEmission / ChainUniformLength — C1b), `origin(a) = d` (chain rooted at `b_C(d)` via ChainPrefixExtension — C2), and `a ∉ dom(C) ∪ dom(L)` (the freshness conclusions above — L14).
...
*Derived structural facts.* As in K.α, the chain-emission clause pins `ℓ` uniquely from `(d, Σ)`, and the following are its consequences, not independent constraints: `zeros(ℓ) = 3` (ChainUniformZeroCount — L1), `E(ℓ)₁ = s_L` (DisjointSubAllocatorChains — L0, see L0 discharge note), `#E(ℓ) ≥ 2` (FirstEmission / ChainUniformLength — L1b), `origin(ℓ) = d` (chain rooted at `b_L(d)` — L1a), and `ℓ ∉ dom(L) ∪ dom(C)` (the freshness conclusions above — L14).
