...
- **Link withdrawal.** Nelson's tombstone-style withdrawal (LM 4/9) is deferred to a higher-layer retraction mechanism (see Open Questions for the invariant it must revisit).
...
**Per-chain disciplines (ASN-0040 citations).** Each discipline below is named for the substrate's local reference and discharged by the cited ASN-0040 result applied to the sibling stream `A_C(d) = S(b_C(d), 1)` (resp. `A_L(d) = S(b_L(d), 1)`), whose parent `(b_·(d), 1)` is `B6`-valid (verified above). Their conclusions are determined per-chain and not by system state, so as ASN-0040 citations they require no per-transition discharge.
...
*Link case, against `dom(C)`.* StoreT4Validity at `Σ` gives T4-validity of every `a ∈ dom(C)`; `ℓ` is T4-valid by ChainElementT4Validity applied to `A_L(d)` (whose first emission `[d.0.s_L.1]` is T4-valid by the FirstEmission lemma). The subspace identifiers split by source: by L0 at `Σ`, `E(a)₁ = s_C` for the pre-existing peer `a ∈ dom(C)`; for the new key `ℓ` we read `E(ℓ)₁ = s_L` from the FirstEmission lemma's structural form `ℓ = [d.0.s_L.1]`. By SC-NEQ, `s_C ≠ s_L`; `zeros(ℓ) = zeros(a) = 3` by FirstEmission's structural form and C1. T7 (SubspaceDisjointness, ASN-0034) closes: `ℓ ≠ a`. ∎
...
*Arrangement frame.* The K.α and K.λ frames carry both `dom(M') = dom(M)` and the pointwise `(A d' ∈ dom(M) :: M'(d') = M(d'))`; under partial-function semantics the two together force `M' = M`, so C2 and L1a at `Σ` transfer to `Σ'` directly (`origin(·) ∈ dom(M)` implies `origin(·) ∈ dom(M')`).
...
*Frame:* `L' = L; dom(M') = dom(M); (A d' ∈ dom(M) :: M'(d') = M(d'))` (forcing `M' = M` per the *Arrangement frame* preamble).
...
*Frame:* `C' = C; dom(M') = dom(M); (A d' ∈ dom(M) :: M'(d') = M(d'))` (forcing `M' = M` per the *Arrangement frame* preamble).
...
Verifying preconditions: freshness `ℓ_new ∉ dom(L₇) ∪ dom(C₇) = {ℓ, ℓ'', a, a', a''}` is supplied by K.λ's subsequent-emit freshness discharges: within-document freshness against `dom(L)` (ChainEnumerationInjectivity + ChainMembershipForOrigin, separating `ℓ_new = s₂` from `ℓ = s₁`), cross-document freshness against `dom(L)` (ChainPrefixExtension + Cross-document disjointness + T10, separating `ℓ_new` under `b_L(d)` from `ℓ''` under `b_L(d')`), and cross-subspace freshness against `dom(C)` (ChainElementT4Validity + L0 + SC-NEQ + T7).
...
| **C1b** (ContentElementFieldDepth) | Preserved: `C` in frame — `E(·)` is T4b's structural projection on the address alone (ASN-0034), depending on no state component, so the keys are unchanged and each prior key's `#E ≥ 2` transfers | Discharged at new key: first-emit branch pins `a = [d.0.s_C.1]`, whose form gives `#E(a) = 2`; subsequent-emit branch has `a = inc(a_prev, 0)`, where `#a = #a_prev` by length preservation (TA5(c)), so `#E(a) = #E(a_prev) ≥ 2` by the IH on `a_prev` | Preserved: `C` in frame |
...
| **L1b** (LinkElementFieldDepth) | Preserved: `L` in frame (`E(·)` structural, same argument as the C1b row) | Preserved: `L` in frame (same argument as the C1b row) | Discharged at new key: first-emit branch pins `ℓ = [d.0.s_L.1]`, whose form gives `#E(ℓ) = 2`; subsequent-emit branch has `ℓ = inc(ℓ_prev, 0)`, where `#ℓ = #ℓ_prev` by length preservation (TA5(c)), so `#E(ℓ) = #E(ℓ_prev) ≥ 2` by the IH on `ℓ_prev` |
...
| **L14** (StoreDisjointness) | Preserved by frame: `C`, `L` both in frame, so IH-L14 (`dom(C) ∩ dom(L) = ∅`) transfers directly | Discharged at the new key `a` by the `E(·)₁`-partition + T7 derivation: FirstEmissionFreshness (content case against `dom(L)`, first-emit) / K.α's cross-subspace freshness clause (subsequent-emit); prior keys by IH-L14 + frame on `L` | Discharged at the new key `ℓ`, symmetric: FirstEmissionFreshness (link case against `dom(C)`, first-emit) / K.λ's cross-subspace freshness clause (subsequent-emit); prior keys by IH-L14 + frame on `C` |
