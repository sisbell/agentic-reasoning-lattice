...
Inspection of the proofs of T10a.1, T10a.7, and T10a.8 in ASN-0034 confirms that they depend only on per-step structure of an `inc(·, 0)` chain together with chain-wide T4-validity preservation (so TA5-SigValid pins `sig(t_n) = #t_n` at every chain element) and the foundation-level claims T1 (LexicographicOrder), TA5(a), and NAT-* axioms. T10a.8's proof body in particular cites T10a.4 to obtain T4-validity of every sibling — a citation that propagates the tree-embedding requirement through T10a.8 transitively even though T10a.8's own proof structure does not invoke `(parent, spawnPt, spawnParam)` or the cross-allocator results. The substrate substitutes the TA5a-based per-step propagation above for that T10a.4 citation, removing the tree-embedding dependency from T10a.8 when applied to sub-allocator chains. None of T10a.1, T10a.7, or T10a.8 (under this substitution) invokes the spawning-triple machinery, the tree-embedding structure itself, or T10a's cross-allocator results (T10a.5/T10a.6). Hence the three lemmas apply to any sequence `(c₁, c₂, …)` with `c_{n+1} = inc(c_n, 0)` whose first element is T4-valid — including the chains `A_C(d)` and `A_L(d)` supplied by ChainDiscipline, whose first elements are T4-valid by FirstEmission. This justifies invoking T10a.1, T10a.7, and T10a.8 on the sub-allocator chains throughout this note.
...
**Lemma (ChainMembershipForOrigin).** At every reachable state `Σ`, the entries of `dom(C)` and `dom(L)` partition by origin into the sub-allocator chains:
