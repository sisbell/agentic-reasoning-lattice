...
| **L1** (LinkElementLevel) | Preserved: `L` in frame | Preserved: `L` in frame | Discharged at new key: first-emit branch pins `ℓ = [d.0.s_L.1]`, whose form gives `zeros(ℓ) = 3`; subsequent-emit branch has `ℓ = inc(ℓ_prev, 0)`, where `zeros(ℓ) = zeros(ℓ_prev) = 3` by B5a (SiblingZerosPreservation, the per-step `zeros(inc(·, 0)) = zeros(·)`) and the IH on `ℓ_prev` |
