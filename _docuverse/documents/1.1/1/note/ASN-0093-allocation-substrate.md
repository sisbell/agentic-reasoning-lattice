...
- **Axiom:** SubAllocatorAxiom (single clause, ChainDiscipline: the sub-allocator chains are the ASN-0040 sibling streams `S(b_·(d), 1)`); FirstEmission is a derived lemma
...
**SubAllocatorAxiom (Axiom, ContentLinkSubAllocatorChainDiscipline).** For each `d ∈ dom(M)`, the substrate commits the content and link sub-allocator chains under `d` to be the ASN-0040 sibling streams `A_C(d) = S(b_C(d), 1)` and `A_L(d) = S(b_L(d), 1)` (SiblingStream, ASN-0040), without claiming that they are embedded in any global allocator tree.

- *Chain discipline (SubAllocatorAxiom.ChainDiscipline).* Each chain `A_C(d)` (resp. `A_L(d)`) is the ASN-0040 sibling stream `S(b_C(d), 1)` (resp. `S(b_L(d), 1)`), rooted at `t_1^C(d) = inc(b_C(d), 1)` (resp. `t_1^L(d) = inc(b_L(d), 1)`) and extended by `inc(·, 0)`. The stream's parent `(b_·(d), 1)` satisfies `B6` (verified under *Sub-allocator chains are ASN-0040 sibling streams* above), so ASN-0040's stream results apply; the substrate makes no commitment about whether an implementation realises the chains as standalone allocators or as discipline-conforming streams within a flatter structure. The per-chain disciplines below are direct ASN-0040 citations.

The chains exist unconditionally for every `d ∈ dom(M)`: each `(b_·(d), 1)` is `B6`-valid (verified above) and ASN-0040's `SiblingStream` is defined for every `B6`-valid parent, so no separate existence postulate is required. A chain `A_·(d)` is available exactly when `d ∈ dom(M)`, and by M1 (ArrangementMonotonicity) that availability persists across all successor states.
...
*Proof.* By ChainDiscipline, `A_C(d) = S(b_C(d), 1)` with first element `c₁ = inc(b_C(d), 1)`. ASN-0040's SiblingStream postcondition `cₙ = [p₁, …, p_{#p}, 0…0, n]` (with `d − 1 = 0` interior zeros at depth `1`) gives `c₁ = [b_C(d)₁, …, b_C(d)_{#b_C(d)}, 1]`; since `b_C(d) = [d.0.s_C]`, this is `[d.0.s_C.1]`, whence `E(·)₁ = s_C`, `#E(·) = 2`, `origin(·) = d`, and `zeros(·) = 3`. T4-validity is TA5a's `k = 1` case, unconditional on a T4-valid input — here `b_C(d)`, T4-valid by B6(a) (verified above). The link case is symmetric. ∎
...
- **ChainPrefixExtension.** Every element of an active sub-allocator chain extends its anchor under the prefix relation:
  ...
  *Quantifier scope.* S1 ranges over the abstract stream `S(b_·(d), 1)` at every index `n ≥ 1`, independent of which elements are committed to `dom(C)` (resp. `dom(L)`) at `Σ`; in particular the prefix relation holds at a freshly emitted stream element `inc(a_prev, 0)` (resp. `inc(ℓ_prev, 0)`) before it is committed.
...
- *K.λ(d, ℓ, (e₁, …, eₙ)):* Symmetric to K.α with content↔link, using the FirstEmission lemma for the first-emit branch (placing `ℓ = s_1`, witnessing `n_d = 1`) and SubAllocatorAxiom.ChainDiscipline for the subsequent-emit branch (placing `ℓ = s_{n_d + 1}` from `ℓ_prev = s_{n_d}` by ChainEnumerationInjectivity, witnessing `n_d + 1` at `Σ'`). The content contiguous-prefix postcondition is unchanged by frame on `dom(C)`. ∎
...
- `a` is produced by `d`'s content sub-allocator `A_C(d)`:
  ...
  - *Subsequent emission* (predicate: `{a' ∈ dom(C) : origin(a') = d} ≠ ∅`): `a = inc(a_prev, 0)` (TA5(c)) where `a_prev := max{a' ∈ dom(C) : origin(a') = d}`, the next sibling on `A_C(d)`'s `inc(·, 0)` chain. The `max` is well-defined because the set is finite (C-fin restricted by `origin(·) = d`). Freshness of `a` against `dom(C) ∪ dom(L)` is discharged by three governing results, with the full multi-step derivation kept once in the *C1c chain exhibition* subsequent-emit case and the discharge matrix's L14 entry below: within-document freshness against `dom(C)` via ChainEnumerationInjectivity + ChainMembershipForOrigin (the strictly increasing chain order separates `a = t_{n_prev + 1}` from every prior `t_m`); cross-document freshness against `dom(C)` via ChainPrefixExtension + Cross-document disjointness + T10; cross-subspace freshness against `dom(L)` via the ChainElementT4Validity + L0 + SC-NEQ + T7 triad (equivalently L14 at the pre-state).
...
| C1c | ContentAllocatorConformance | INV | Substrate, content-side analog of L1c, stated in parallel form (including `k₁ = 2` and `#tᵢ > #origin(a)` clauses); established at K.α via the C1c chain exhibition (first-emit chain has two steps `(d → b_C(d) → a)` with `k₁ = 2`, `k₂ = 1` and lengths `#d → #d + 2 → #d + 3`, all strictly exceeding `#d`; subsequent-emit extends a length-monotone chain by `inc(·, 0)` and inherits `k₁ = 2`). The first-emit gap is closed by the FirstEmission lemma, subsequent-emit by SubAllocatorAxiom.ChainDiscipline (ChainEnumerationInjectivity); preserved at K.σ/K.λ by frame on `C` |
...
| L1c | LinkAllocatorConformance | INV | ASN-0043, restated verbatim including the `k₁ = 2` and `#tᵢ > #origin(ℓ)` clauses. Established at K.λ via the L1c chain exhibition (first-emit chain has three steps `(d → b_C(d) → b_L(d) → ℓ)` with `k₁ = 2`, `k₂ = 0`, `k₃ = 1` and monotonically increasing lengths `#d → #d + 2 → #d + 2 → #d + 3`, all strictly exceeding `#d`; subsequent-emit extends a length-monotone chain by `inc(·, 0)` and inherits `k₁ = 2` from the prior chain). The first-emit gap is closed by the FirstEmission lemma, subsequent-emit by SubAllocatorAxiom.ChainDiscipline (ChainEnumerationInjectivity); preserved at K.σ/K.α by frame on `L` |
...
| Sub-allocator chains | A_C(d), A_L(d) as ASN-0040 sibling streams | DEF | `A_C(d) = S(b_C(d), 1)` and `A_L(d) = S(b_L(d), 1)` (SiblingStream, ASN-0040): `c₁ = inc(anchor, 1)` (the first emission), `cₙ₊₁ = inc(cₙ, 0)`. Each parent `(b_·(d), 1)` is `B6`-valid (T4-valid anchor, `zeros = 3`, depth `1`, `zeros + (1−1) = 3 ≤ 3`), so all `B6`-preconditioned ASN-0040 results apply. |
| SubAllocatorAxiom | ContentLinkSubAllocatorChainDiscipline | AXIOM | Single clause (ChainDiscipline): the content/link sub-allocator chains are the ASN-0040 sibling streams `S(b_·(d), 1)`. Chain existence is unconditional from ASN-0040's SiblingStream at the `B6`-valid parents `(b_·(d), 1)`, not postulated. |
