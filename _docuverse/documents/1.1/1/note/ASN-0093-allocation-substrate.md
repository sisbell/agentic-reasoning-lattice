...
- *FirstEmission's freshness conclusion `a ∉ dom(C) ∪ dom(L)` follows from the first-emit predicate + L0 + Cross-document disjointness + SC-NEQ.* Under the K.α first-emit predicate `{a' ∈ dom(C) : origin(a') = d} = ∅`, every `a' ∈ dom(C)` has `origin(a') ≠ d`, so Cross-document disjointness (applied with `b_C(d)` vs `b_C(origin(a'))`) gives `a' ≠ [d.0.s_C.1] = a` since `a` extends `b_C(d)` and `a'` extends a disjoint `b_C(origin(a'))`. Against `dom(L)`: every `ℓ ∈ dom(L)` has `E(ℓ)₁ = s_L ≠ s_C = E(a)₁` by L0 + SC-NEQ, so `ℓ ≠ a`. The K.λ first-emit derivation is symmetric.
...
**Lemma (ChainMembershipForOrigin).** At every reachable state `Σ`, every entry of `dom(C)` (resp. `dom(L)`) is contained in the content (resp. link) sub-allocator chain of its origin:

- `(A d ∈ dom(M) :: dom(C) ∩ {a' ∈ T : origin(a') = d} ⊆ A_C(d))`
- `(A d ∈ dom(M) :: dom(L) ∩ {ℓ' ∈ T : origin(ℓ') = d} ⊆ A_L(d))`

The proof below establishes only these subset inclusions; the stronger *partition* claim — that the chain memberships are pairwise disjoint across distinct origins and jointly cover `dom(C)` and `dom(L)` — is recoverable as a corollary but not needed by downstream consumers. Covering follows from C2 + L1a (every store entry has `origin(·) ∈ dom(M)`); cross-document disjointness `A_C(d) ∩ A_C(d') = ∅` for `d ≠ d'` (and the link analogue) follows from the Cross-document disjointness lemma applied at the first-emission anchors `b_C(d)`, `b_L(d)`. Subsequent uses cite only the subset form.
...
- *K.σ(d_new):* `C` and `L` are in frame, so for every `d` already in `dom(M)` the intersection sets are unchanged and the inclusions transfer. For the freshly registered `d_new`, the intersection sets are empty in `Σ'`. *Content clause derivation:* By the inductive hypothesis on C2 at `Σ`, every `a ∈ dom(C(Σ))` satisfies `origin(a) ∈ dom(M(Σ))`. By K.σ's precondition, `d_new ∉ dom(M(Σ))`. Therefore `origin(a) ≠ d_new` for every `a ∈ dom(C(Σ))`. Since `C` is in frame (`C(Σ') = C(Σ)`), `dom(C(Σ')) ∩ {a' : origin(a') = d_new} = dom(C(Σ)) ∩ {a' : origin(a') = d_new} = ∅ ⊆ A_C(d_new)`. *Link clause derivation:* Symmetric, using the inductive hypothesis on L1a at `Σ` together with K.σ's precondition `d_new ∉ dom(M(Σ))` and frame on `L`.

- *K.α(d, a, v):* Only `dom(C)` grows, by one element `a` with `origin(a) = d`. For `d' ∈ dom(M)` with `d' ≠ d`, the intersection `dom(C') ∩ {a' : origin(a') = d'} = dom(C) ∩ {a' : origin(a') = d'}` is unchanged (the new `a` has `origin(a) = d ≠ d'`), so the prior inclusion transfers. For `d` itself, two sub-cases via the K.α emission rule:
  - *First emission* (`{a' ∈ dom(C) : origin(a') = d} = ∅`): by SubAllocatorAxiom.FirstEmission, `a = [d.0.s_C.1]` is the first emission of `A_C(d)`'s chain, hence `a ∈ A_C(d)`. The intersection set at `Σ'` is `{a}`, and the inclusion `{a} ⊆ A_C(d)` holds.
  - *Subsequent emission* (`{a' ∈ dom(C) : origin(a') = d} ≠ ∅`): let `a_prev = max{a' ∈ dom(C) : origin(a') = d}`. By the inductive hypothesis at `Σ`, `a_prev ∈ A_C(d)`. By SubAllocatorAxiom.ChainDiscipline, `A_C(d)` is the `inc(·, 0)`-extension chain rooted at its first emission, hence closed under `inc(·, 0)`; therefore `a = inc(a_prev, 0) ∈ A_C(d)`. The new intersection set at `Σ'` is the prior intersection together with `a`, both in `A_C(d)`.

  The link inclusion is unchanged by frame on `dom(L)`.

- *K.λ(d, ℓ, F, G, Θ):* Symmetric to K.α with content↔link, using SubAllocatorAxiom.FirstEmission for the first-emit branch and SubAllocatorAxiom.ChainDiscipline for the subsequent-emit branch. The content inclusion is unchanged by frame on `dom(C)`. ∎
...
- `a` is produced by `d`'s content sub-allocator `A_C(d)`:
  ...
  - *Subsequent emission* (predicate: `{a' ∈ dom(C) : origin(a') = d} ≠ ∅`): `a = inc(a_prev, 0)` (TA5(c)) where `a_prev := max{a' ∈ dom(C) : origin(a') = d}`, the next sibling on `A_C(d)`'s `inc(·, 0)` chain. The `max` is well-defined because the set is finite (C-fin restricted by `origin(·) = d`). *Freshness against `dom(C)`* is discharged by combining T10a.7 with the max-property of `a_prev`. Let `n_prev` be the chain index of `a_prev` within `A_C(d)`. For every `a' ∈ dom(C)` with `origin(a') = d`: (i) ChainMembershipForOrigin places `a' ∈ A_C(d)` at some chain index `m`; (ii) T10a.7 (EnumerationInjectivity, applicability justified by the *T10a chain-lemma applicability* remark) gives strict monotonicity `m < n ⟹ t_m < t_n`, whose contrapositive `t_m ≥ t_n ⟹ m ≥ n` combined with `a' ≤ a_prev` (the lex-order max-property) yields `m ≤ n_prev`; (iii) SubAllocatorAxiom.ChainDiscipline places `a = inc(a_prev, 0)` at chain index `n_prev + 1`, so T10a.7 then yields `a' = t_m ≤ t_{n_prev} = a_prev < t_{n_prev + 1} = a`, hence `a' ≠ a`. The max-property of `a_prev` alone bounds `dom(C)_d`'s chain indices from above; contiguity of `dom(C)_d` within `A_C(d)` is not required for this argument. Cross-document collisions within `dom(C)` are ruled out by the Cross-document disjointness lemma. *Freshness against `dom(L)`* is discharged by SC-NEQ + T7 (equivalently L14 at the pre-state).
...
Effect: `L₇ = L₆ ∪ {ℓ'' ↦ (F'', G'', Θ'')}`; `C₇ = C₆`; `M₇ = M₆`. Invariants at `Σ₇`: L0 (`E(ℓ'')₁ = s_L`), L1 (`zeros(ℓ'') = 3`), L1a (`origin(ℓ'') = d' ∈ dom(M₇)`), L1b (`#E(ℓ'') = 2`), L1c (chain exhibited above), L3 (triple endset with non-empty `Θ''`), L12 (existing link `ℓ ↦ (F, G, Θ)` unchanged), L14 (`dom(C₇) ∩ dom(L₇) = {a, a', a''} ∩ {ℓ, ℓ''} = ∅` by SC-NEQ), L-fin (`|L₇| = 2 < ∞`); ChainMembershipForOrigin extends: `{ℓ''} ⊆ A_L(d')` by FirstEmission.

The extended example confirms invariants M0, M1, C0–C2, C-fin, L0–L14, L-fin at each successor state across two documents, exercises both first-emit and subsequent-emit branches of K.α (Steps 2, 4, 6) and the first-emit branch of K.λ under two documents (Steps 3, 7), verifies the Cross-document disjointness lemma Case A at index `#d + 1 = 6` (Step 5) underwriting cross-document freshness at Steps 6 and 7, and exhibits the ChainMembershipForOrigin lemma across both content and link allocations.
...
| ChainMembershipForOrigin | ChainMembershipForOrigin | LEMMA | Inductive invariant: `dom(C) ∩ {a' : origin(a') = d} ⊆ A_C(d)` and `dom(L) ∩ {ℓ' : origin(ℓ') = d} ⊆ A_L(d)` at every reachable state. Proved by induction over transitions using SubAllocatorAxiom.FirstEmission (first-emit branches) and ChainDiscipline (subsequent-emit branches). Licenses application of T10a.7 to `(a_prev, a)` and `(ℓ_prev, ℓ)` in K.α/K.λ subsequent-emit cases. |
