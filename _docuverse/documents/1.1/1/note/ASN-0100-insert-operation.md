...
The subspace partition L0 requires more care, because it has two conjuncts: `(A a ∈ dom(Σ.L) :: subspace_I(a) = s_L)` and `(A a ∈ dom(Σ.C) :: subspace_I(a) = s_C)`. The first conjunct ranges over `dom(L)`, unchanged, and so is discharged trivially. The second conjunct ranges over `dom(C)`, which INSERT *extends* by the fresh content addresses `a_0, …, a_{n−1}` (INS.C) — it is therefore not a property of `L` alone. The pre-existing entries of `dom(C)` satisfy the conjunct by the pre-state invariant; each freshly allocated `a_k` satisfies it as part of the per-fresh-address content-invariant discharge in §Atomicity and Canonical Order (which establishes `subspace_I(a_k) = s_C` for every `a_k` via DisjointSubAllocatorChains; ASN-0093). Hence L0's content clause is preserved.
...
We claim INSERT is permitted at any valid insertion position — beginning, middle, end, and on the first insertion into an empty document. These edge cases require no special handling in the post-condition specification: the universal forms of the three regions handle them uniformly, with quantifier-bounded clauses vacuously satisfied when a region is empty.
