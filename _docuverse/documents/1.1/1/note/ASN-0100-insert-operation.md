...
**Re-insertion into a cleared content subspace (subsequent-emission branch).** Suppose `d` once held `V_{s_C}(d) = {[1,1], [1,2]}` mapping to `[d.0.s_C.1]`, `[d.0.s_C.2]`, then a full content-subspace clearance (K.μ⁻ with `n'_{s_C} = 0`) removed both V-positions while those I-addresses remain in `dom(Σ.C)`. So `V_{s_C}(d) = ∅` with residual frontier `a_prev = [d.0.s_C.2]`.

Invoke `INSERT(d, [1,1], ⟨v₀, v₁⟩)` with `n = 2`. Because the residual set `{a' ∈ dom(Σ.C) : origin(a') = d}` is non-empty, K.α's *subsequent-emission* branch fires (INS.alloc): `A_C(d)` resumes the chain off the persisted frontier — `a_{new0} = inc([d.0.s_C.2], 0) = [d.0.s_C.3]`, `a_{new1} = [d.0.s_C.4]` — rather than restarting at `[d.0.s_C.1]`. The post-state is `M'(d) = {[1,1] ↦ [d.0.s_C.3], [1,2] ↦ [d.0.s_C.4]}`.

This exposes the *decoupling* of the V-position index from the I-address chain index: D-MIN★ and D-SEQ★ are stated over `V_{s_C}(d')` alone — `min = [s_C, 1]`, explicit form `{[1, k] : 1 ≤ k ≤ 2}` — and so are blind to the chain frontier index. The V-positions restart at `[s_C, 1]` while the I-addresses resume at chain indices 3 and 4, and the post-state V-positions are identical to the empty-document outcome whether the images are `[d.0.s_C.1..2]` or `[d.0.s_C.3..4]`.

**Deep-subspace interior insertion (`m_C = 3`).** Let `d` have `V_{s_C}(d) = {[1,1,1], [1,1,2], [1,1,3]}` (so `m_C = 3`, `N = 3`), with arrangement:
...
For Left and Shifted-right content-subspace positions: each image is `M(d)(v')` for some pre-state position `v' ∈ dom(M(d))` with `subspace(v') = s_C`, which pre-state S3★ places in `dom(C)`, and `dom(C) ⊆ dom(C')` by P0 (ContentPermanence; ASN-0047) — so `ran(M'(d)) ⊆ dom(C')` there.
...
The post-state Σ' is *uniquely determined* by the operation contract; the substrate decomposition that realises it is not. We verify uniqueness component by component.
  ...
  *Content store.* Every admissible decomposition fires exactly `n` K.α steps in forced order, the `k`-th producing the determined chain element `t_{m_d + k + 1}` of `A_C(d)` (K.α's subsequent-emission predicate reads `dom(C)`; ASN-0093). So `dom(C') = dom(C) ∪ {a_0, …, a_{n−1}}`, each `a_k` fixed by the pre-state chain index `m_d` and the inputs, with `C'(a_k) = v_k` from INSERT's value sequence; every other step frames C, so `C'(a) = C(a)` for `a ∈ dom(C)`.

  *Arrangement of `d`.* At the boundary, `V_{s_C}(d') = Left ∪ Insertion ∪ Shifted-right` with no fourth region (INS.M-exhaustive, §Arrangement functionality), and the mapping on each region is fixed by `p`, `n`, the determined `a_k`, and the pre-state `V_{s_C}(d)` (INS.M-left, INS.M-insert, INS.M-shift). Any admissible K.μ⁻ + K.μ⁺ pairing reaches this same M'(d), independent of the K.μ⁻ retention parameter — whatever K.μ⁻ retains, K.μ⁺ must re-add exactly the missing positions to satisfy the boundary.

  *Other components.* `M'(d') = M(d')` for `d' ≠ d`, `L' = L`, `E' = E`, `dom(M') = dom(M)` by every elementary step's frame (no K.λ, no K.δ); `R' = R ∪ {(a_k, d) : 0 ≤ k < n}`, order-independent because set union is.

The decomposition admits free interleaving subject only to the per-step preconditions of ASN-0047/0093. The forced orderings are: each K.α before the next (subsequent-emission reads `dom(C)`; ASN-0093); each K.α(a_k) before the K.μ⁺ placing `a_k` and before K.ρ(a_k, d) (both require `a_k ∈ dom(C)`); and, when K.μ⁻ fires, K.μ⁻ before K.μ⁺ (K.μ⁺'s image-preserving precondition `(A v ∈ dom(M(d)) : M'(d)(v) = M(d)(v))` forbids rebinding a Right-region position still in `dom(M(d))`). Every remaining interleaving reaches the same Σ': no per-state invariant is sensitive to the relative order of the remaining steps, each intermediate is a reachable state satisfying the per-state invariants, and the couplings J0, J1★, J1'★ are obligations on INSERT's own boundary `(Σ, Σ')`. The abstract specification commits to none of the admissible interleavings.
