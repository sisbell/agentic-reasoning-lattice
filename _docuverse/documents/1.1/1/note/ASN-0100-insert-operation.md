...
**Empty-document first insertion.** Let `d` have `V_{s_C}(d) = ∅` and additionally `V_{s_L}(d) = ∅` (so the document's arrangement is entirely empty), and stipulate further that no content has ever been allocated under `d` — `{a' ∈ dom(Σ.C) : origin(a') = d} = ∅` — so K.α's first-emission branch fires. Arrangement emptiness alone would not suffice: by S0/P0, prior d-origin content persists in `dom(C)` even after removal from the arrangement, in which case K.α's subsequent-emission branch would fire and the fresh `a_0` would be `inc(a_prev, 0) ≠ [d.0.s_C.1]`. The post-state invariants hold for whichever fresh `a_k` the branch produces; only the address *values* depend on allocation history, not on arrangement emptiness. Invoke `INSERT(d, [1,1], ⟨v₀, v₁, v₂⟩)` with `n = 3` (so the depth is `m = #p = #[1,1] = 2`). The position `p = [1,1]` is the unique value admitted by `ValidFirstInsertionPosition(d, p, 2)` (ASN-0036). K.μ⁻ is omitted — the empty-content-subspace case of (INS.μ⁻-fires). The composite reduces to:
...
Cross-document independence extends to link projection: for any link `ℓ ∈ dom(L)` and any document `d' ≠ d`, the projection from `d'` is unchanged, `project(ℓ, i, d', Σ') = project(ℓ, i, d', Σ)`. Every elementary step of INSERT's substrate decomposition frames `M(d')` (`(A d'' : d'' ≠ d : M'(d'') = M(d''))`), so LP4 (ArrangementSpecificity; ASN-0098) gives equality at each step, and composing across the finite step sequence yields the result.
...
We verify the post-state V-position predicates (S8-depth, S8a, S8-fin) and the S7 invariants directly. The S7 invariants range over `dom(C)` and the document set: S7a/S7b/S7d on pre-existing addresses follow from pointwise S0/P0 preservation, and the fresh `a_k` are discharged in §Atomicity.
...
We claim INSERT is permitted at any valid insertion position — beginning, middle, end, and on the first insertion into an empty document. The precondition predicates are fixed upstream: the binary `ValidInsertionPosition(d, p)` (non-empty `V_{s_C}(d)`) and the ternary `ValidFirstInsertionPosition(d, p, m)` with `m := #p` (empty `V_{s_C}(d)`), both established in §The Operation's Inputs and recorded in the State Preconditions of §The Operation: Formal Contract. What remains to be shown here is that each admissible position yields a well-typed composite — and the determining factor is which of the three post-state regions are empty.

For the non-empty case, the index `j` of the admissible position `p = shift(min(V_{s_C}(d)), j)` determines region emptiness:

- `j = 0` (beginning): Left is empty (no `v < p`); the entire pre-state text subspace is the Shifted-right region.
- `j = N` (append): Shifted-right is empty (no `v ≥ p`, since `max(V_{s_C}(d)) = shift(min, N−1) < p`); no shift occurs, and K.μ⁻ is omitted (the Right-region-empty case of (INS.μ⁻-fires)).
- `j ∈ {1, …, N−1}` (interior): both Left and Shifted-right are non-empty.

For the empty case, both Left and Shifted-right are empty: the composite reduces to the Insertion region alone, and K.μ⁻ is omitted (the empty-content-subspace case of (INS.μ⁻-fires)).

These edge cases require no special handling in the post-condition specification: the universal forms of the three regions handle them uniformly, with quantifier-bounded clauses vacuously satisfied when a region is empty.
