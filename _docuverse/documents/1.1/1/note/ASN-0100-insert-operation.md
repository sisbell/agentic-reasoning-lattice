...
S8★ (PerSubspaceSpanDecomposition; ASN-0047) requires that each per-subspace arrangement `M'(d)|_{V_S(d')}` admit a finite block decomposition satisfying ASN-0036's S8 conditions. The post-state's standing preconditions for M2 (DecompositionExistence; ASN-0058) — S8-fin, S2, S3 (here in its extended form S3★), S8a, S8-depth — are all verified above (under §Arrangement functionality, §Referential integrity, §Sequential text-subspace structure, and §Post-state V-position well-formedness), so the existence of a block decomposition for `M'(d)|_{V_{s_C}(d')}` follows from M2 directly. The link-subspace branch S8★ requires for `M'(d)|_{V_{s_L}(d')}` is discharged by the trivial length-1 decomposition (per ASN-0047), inherited unchanged from the pre-state by the cross-subspace frame `V_{s_L}(d') = V_{s_L}(d)`.
...
This discharges S8★ conditions (a) (lockstep displacement, established by the run construction above) and (b) (label well-definedness, from each block being a well-formed mapping block over `dom(C')`), together with *existence* via M2/M11. It remains to discharge condition (c) — uniqueness of the maximal-run decomposition, which S8★ requires only on the content subspace. M12 (CanonicalUniqueness; ASN-0058) supplies it: M12 establishes that the maximally merged block decomposition of any arrangement (or restriction thereof) is unique, factoring through M12a (maximal runs partition the domain) and M12b (every block of a maximally merged decomposition is a maximal run). Its standing preconditions are exactly those already confirmed of the post-state for M2 — S8-fin, S2, S3★ restricted to the content subspace, S8a, and S8-depth — all verified above for `M'(d)|_{V_{s_C}(d')}`. M12 therefore makes the maximal-run decomposition of `M'(d)|_{V_{s_C}(d')}` unique, closing condition (c) on the content subspace. (Condition (c) is not required on the link subspace, where S8★ asks only for the trivial length-1 decomposition.)
