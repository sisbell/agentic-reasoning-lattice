...
The subspace partition L0 has two conjuncts. Its `dom(L)` conjunct `(A a ∈ dom(Σ.L) :: subspace_I(a) = s_L)` ranges over the unchanged `dom(L)` and is discharged trivially. Its content conjunct `(A a ∈ dom(Σ.C) :: subspace_I(a) = s_C)` ranges over the `dom(C)` that INSERT *extends* by the fresh `a_0, …, a_{n−1}` (INS.C) and is discharged once, for both the K.α intermediates and the boundary, in the grouped per-address paragraph of §Atomicity and Canonical Order.
