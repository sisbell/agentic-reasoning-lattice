...
The `a_k ∉ dom(Σ_k.L)` clause is discharged by subspace separation. By DisjointSubAllocatorChains (ASN-0093), `a_k` is produced by `A_C(d)` and so satisfies `subspace_I(a_k) = s_C`; every `ℓ ∈ dom(Σ_k.L)` satisfies `subspace_I(ℓ) = s_L` by L0 (SubspacePartition; ASN-0047); and `s_C ≠ s_L` by SC-NEQ (ASN-0093). Hence `a_k ∉ dom(Σ_k.L)`.
...
**Only I3's shift clause transfers.** This ASN cites I3 (and the companion frame lemmas I3-L, I3-X, I3-D for the unchanged regions) only for the positive shift clause. I3's whole-post-state frames do *not* transfer to INSERT: in particular I3-C (`Σ'.C = Σ.C`) and I3-S7 (whose ASN-0082 justification rests on I3-C) both fail here, because INSERT extends `dom(C)` with freshly allocated content that the shift-only model omits. The post-state invariants those frames would have supplied — S2, S3★, S8a, S8-depth, S8-fin, the S7 family — are re-derived directly in §Verifying the Invariants rather than imported from I3-S2/I3-S3/I3-VP/I3-VD/I3-fin/I3-S7, whose justifications depend on the failing frame.
...
**Composite atomicity.** By ValidComposite★ (ASN-0047), INSERT's elementary transitions form a *contiguous* finite sequence `Σ = Σ₀ → Σ₁ → … → Σₙ = Σ'`; transitions are totally ordered (SequentialTransitionAxiom; ASN-0093), so no foreign elementary transition interleaves between INSERT's steps. Composite-level atomicity is therefore *definitional* — a consequence of INSERT being a valid composite — not an extra property the substrate must separately supply. Because the chain index `m_d` consulted by K.α cannot advance via a foreign K.α mid-composite, and the text subspace `V_{s_C}(d)` cannot be modified mid-composite, the freshness and placement arguments above determine Σ' from the pre-state. An implementation that realizes the sequential transition model (for instance, multiple threads serialised onto the abstract total order) inherits this determinacy; the concurrency-control mechanism it uses is below this ASN's abstraction level.
...
*Non-tight alternative.* If `tight(e_1, Σ_{e_1})` (ASN-0098) does not hold — under the conditions ASN-0098 specifies for tightness failure — then LP19a is inapplicable, and a freshly allocated `a_{new0}` or `a_{new1}` may land in `coverage(e_1)`. Then `N_I ⊆ {[1,3], [1,4]}` may be non-empty, with each `[1, 3 + k]` contributing iff `a_{new k} ∈ coverage(e_1)`, so the post-state `project(ℓ, 1, d, Σ') = {[1,2], [1,5], [1,6]} ∪ N_I` could include up to two additional positions. The INS.proj general form `π(project) ∪ N_{ℓ,i}` captures both regimes; the tight case is the special simplification with `N_I = ∅`.
...
with `V_{s_C}(d') = {[1,1], [1,2], [1,3]}`; this first insertion fixes `m_C = 2` for `d` (§The Operation's Inputs). Verifying the three regions: *Left* is empty (no pre-state position with `v < p`); *Insertion* is `{[1,1] ↦ a_{new0}, [1,2] ↦ a_{new1}, [1,3] ↦ a_{new2}}` matching `shift(p, k) ↦ a_k` for `k ∈ {0, 1, 2}` (with `shift([1,1], 0) = [1,1]` by OrdinalShiftBase); *Shifted right* is empty (no pre-state position with `v ≥ p`).
...
The pairwise-disjoint, uniquely-defined regions just given establish that `M'(d)` is a function directly — we do not import ASN-0082's I3-S2, whose shift-only justification rests on the failing frame. For other subspaces and other documents, `M'` equals `M`, which is already a function by the pre-state S2.
...
These three cases exhaust `dom(M'(d))` (by INS.M-exhaustive plus the cross-subspace frame), so S3★ holds on the whole post-state arrangement. We do not import ASN-0082's I3-S3, whose shift-only justification reaches `ran(M'(d)) ⊆ dom(C')` via `dom(C') = dom(C)` (I3-C) — exactly the frame that fails here.
...
- *S8-depth:* every position in `V_{s_C}(d')` has length `m`. Pre-state `V_{s_C}(d) = ∅` imposes no depth constraint, so this first insertion fixes `m_C = m` for `d` (§The Operation's Inputs).
...
Both atomicity levels INSERT relies on are accounted for in the contract and not re-derived here: elementary-level atomicity is supplied unconditionally by SequentialTransitionAxiom (ASN-0093), and composite-level atomicity is definitional under ValidComposite★ — INSERT's elementaries form a contiguous transition sequence, so its intermediate states are non-observable to other composites and Σ' is determined by the contract above.
...
| INS.pre | INSERT preconditions: d ∈ dom(M); p valid in text subspace of d (binary predicate ValidInsertionPosition for non-empty case, ternary predicate ValidFirstInsertionPosition(d, p, m) with caller-chosen m ≥ 2 for empty case); n ≥ 1; v_k ∈ Val. INSERT's elementaries form a contiguous valid composite (ValidComposite★, ASN-0047), so composite-level atomicity is definitional rather than a separate precondition | introduced |
...
| INS.inv.refint | Referential integrity S3★ (ASN-0047) preserved: ran(M'(d)) ⊆ dom(C') ∪ dom(L') per-subspace, re-derived directly (not via I3-S3, whose shift-only justification rests on the failing dom(C') = dom(C) frame) | introduced |
...
| INS.inv.depth | S8-depth (ASN-0036) preserved: non-empty case leaves m_C unchanged; empty case fixes m_C = m on first insertion (§The Operation's Inputs). A later K.μ⁻ emptying V_{s_C}(d) makes S8-depth vacuous and permits a different depth on the next first-insertion | introduced |
