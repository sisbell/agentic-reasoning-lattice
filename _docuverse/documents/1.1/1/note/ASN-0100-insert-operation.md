...
- *Composite-boundary premise.* The pre-state Σ is a composite boundary in the sense of ValidComposite★ (ASN-0047) — it satisfies not only the per-state invariants but also the composite-boundary properties P4★, P4a, P7a of ExtendedReachableStateInvariants. These hold only at composite boundaries, not at arbitrary elementary-reachable states; carrying the premise makes pre-state P4★/P4a/P7a available to the post-state discharges of those same properties (§Provenance). INSERT realises Σ →* Σ' as one composite, so Σ' is again a composite boundary.
