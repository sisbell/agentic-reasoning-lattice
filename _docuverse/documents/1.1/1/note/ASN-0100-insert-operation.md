...
For Left and Shifted-right content-subspace positions: we re-derive referential integrity directly (re-derived, not inherited; see §Effect Three). Each image is `M(d)(v')` for some pre-state position `v' ∈ dom(M(d))` with `subspace(v') = s_C`, which pre-state S3★ places in `dom(C)`, and `dom(C) ⊆ dom(C')` by P0 (ContentPermanence; ASN-0047) — so `ran(M'(d)) ⊆ dom(C')` there.
