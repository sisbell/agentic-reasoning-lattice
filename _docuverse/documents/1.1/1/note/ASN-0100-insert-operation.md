...
2. **One K.μ⁻ on `d`** retaining the Left prefix of `V_{s_C}(d)` (with `n'_{s_C} = p_m − 1`) and retaining all of `V_{s_L}(d)` (with `n'_{s_L} = n_{s_L}`). The canonical decomposition omits this step in three cases, distinguished by whether the omission is *forced* (no admissible K.μ⁻ firing exists under the canonical retention parameters and INSERT's frame) or a *canonical-decomposition choice* (an alternative decomposition with different K.μ⁻ retention parameters could fire K.μ⁻ and reach the same Σ' via a subsequent K.μ⁺ rebuild, but the canonical decomposition omits K.μ⁻ for parsimony):
   ...
   *(i.b) — Canonical-decomposition choice (forced under canonical K.μ⁻ + K.μ⁺ sub-vocabulary, conditionally admissible under ValidComposite★'s full vocabulary).* When `V_{s_C}(d) = ∅` and `V_{s_L}(d) ≠ ∅`, K.μ⁻'s `dom(M(d)) ≠ ∅` precondition holds, but `n_{s_C} = 0` forces the content-subspace retention parameter `n'_{s_C} ∈ {0, …, 0}` to equal `n_{s_C}` (foreclosing strict shrinkage in `s_C`); the strict-shrinkage clause `(E S :: n'_S < n_S)` therefore reduces to `n'_{s_L} < n_{s_L}`, which shrinks `V_{s_L}(d)` and violates INS.frame.subspace at the composite boundary. Within the canonical K.μ⁻ + K.μ⁺ sub-vocabulary used by this decomposition (K.μ⁺ adds only `s_C` positions per its content-subspace restriction in ASN-0047), the shrunken `s_L` positions cannot be restored; every admissible K.μ⁻ firing leaves the post-state with strictly fewer `s_L` positions and violates the boundary frame. Under ValidComposite★'s full vocabulary, an alternative decomposition becomes available *only when the substrate environment retains the pre-state's link arrangement information across the K.μ⁻ step* — concretely: writing the pre-state link arrangement as `Σ.M(d)|_{V_{s_L}(d)} = {[s_L, 1, ..., 1, k] ↦ ℓ_k : 1 ≤ k ≤ n_{s_L}}` (D-SEQ★ enumeration, ASN-0047), the implementation must record the ordered sequence `⟨ℓ_1, ℓ_2, …, ℓ_{n_{s_L}}⟩` before firing K.μ⁻. The alternative decomposition then proceeds as follows. Step (a): K.μ⁻ shrinks `V_{s_L}(d)` to some `n'_{s_L} < n_{s_L}` (typical choice: `n'_{s_L} = 0`, full link-subspace shrinkage). Step (b): K.μ⁺ adds the Insertion region in subspace `s_C` (mapping `shift(p, k) ↦ a_k` for `0 ≤ k < n`). Steps (c)…(c_{n_{s_L}}): successive K.μ⁺_L firings re-add the link-subspace positions in their original order — the first firing (call it step c_1) places `ℓ_1` at `[s_L, 1, ..., 1]` per K.μ⁺_L's V-position selection rule (D-MIN★ in ASN-0047, since `V_{s_L}(d_intermediate) = ∅` at the start of c_1); the second firing (c_2) places `ℓ_2` at `shift([s_L, 1, ..., 1], 1) = [s_L, 1, ..., 1, 2]` per D-CTG★ (since `V_{s_L}(d_intermediate)` now contains `[s_L, 1, ..., 1]` only, so `max(V_{s_L}(d_intermediate)) = [s_L, 1, ..., 1]` and the next position is `shift(max, 1)`); and so on, with the k-th re-add firing placing `ℓ_k` at `[s_L, 1, ..., 1, k]`. The reconstruction succeeds because K.μ⁺_L's selection rule places each newly added link at the position immediately after the current maximum, which — combined with the implementation's retention of the original `⟨ℓ_1, …, ℓ_{n_{s_L}}⟩` sequence as a side channel — admits sequential reconstruction of `Σ.M(d)|_{V_{s_L}(d)}` exactly. *Without* this retention assumption, the alternative decomposition cannot be carried out: K.μ⁺_L's first-arrangement constraint (`ℓ ∉ ran(M(d))`, ASN-0047) does not by itself force any particular re-add order, and the implementation has no in-substrate source for the original ordering after K.μ⁻ has emptied `V_{s_L}(d)`. Case (i.b) is therefore *conditionally* symmetric in character to case (ii): the alternative decomposition is admissible only when the substrate environment retains pre-state link ordering across K.μ⁻; without that retention, case (i.b) reduces to case (i.a)'s style of forced omission for any decomposition required to preserve INS.frame.subspace. Only case (i.a) is intrinsically forced regardless of substrate-environment capability (K.μ⁻'s `dom(M(d)) ≠ ∅` precondition fails outright under any vocabulary).
...
*Projection-shift correspondence (INS.proj).* Suppose a link `ℓ ∈ dom(L)` has a slot with endset `e_1` whose coverage is exactly `{a₂, a₃, a₄}` — three I-addresses in the pre-state range. **Tightness precondition of the trace below, grounded in the example's substrate state.** We construct `Σ_{e_1}` — the state at which `e_1` was incorporated into `ℓ` via K.λ — concretely. Let `Σ_{e_1}` be the substrate state of `d` immediately after `M(d) = {[1,1] ↦ a₁, [1,2] ↦ a₂, [1,3] ↦ a₃, [1,4] ↦ a₄, [1,5] ↦ a₅}` had been established (the pre-state of our INSERT) and *before* any subsequent K.α firing of INSERT advances `A_C(d)`'s chain. At `Σ_{e_1}`, the content addresses `a₁, a₂, a₃, a₄, a₅ ∈ dom(Σ_{e_1}.C)` are all present — they were placed there by prior K.α firings constituting the chain of `A_C(d)` (per ChainMembershipForOrigin; ASN-0093), with structural form `aᵢ = [d.0.s_C.i]` for `1 ≤ i ≤ 5`. The endset `e_1`'s coverage `{a₂, a₃, a₄}` is delivered by the canonical span `(a_2, δ(3, #a_2))`: this span has start `a_2 ∈ dom(Σ_{e_1}.C)`, width `δ(3, #a_2)` of length `#a_2 = #d + 3` (so the action point equals the length — canonical), and reach `a_2 ⊕ δ(3, #a_2)`. By LP-Fin Corollary (CanonicalIntervalCharacterisation; ASN-0098), the F-candidates in the half-open interval `[a_2, a_2 ⊕ δ(3, #a_2))` are exactly `{[d.0.s_C.2], [d.0.s_C.3], [d.0.s_C.4]} = {a_2, a_3, a_4}`. All three are members of `dom(Σ_{e_1}.C)` by construction. Therefore the tightness conditions of ASN-0098 hold: the span is canonical (width is `δ(3, #a_2)`), the start lies in `dom(Σ_{e_1}.C) ∪ dom(Σ_{e_1}.L)`, and every F-candidate in the interval lies in `dom(Σ_{e_1}.C) ∪ dom(Σ_{e_1}.L)`. We conclude `tight(e_1, Σ_{e_1})` (ASN-0098). This is the load-bearing assumption that makes `N_I = ∅` concrete via LP19a (TightFreshness; ASN-0098); we trace the non-tight alternative at the end of this example. We trace the projection through the composite.

Pre-state `project(ℓ, 1, d, Σ) = {v ∈ dom(M(d)) : M(d)(v) ∈ {a₂, a₃, a₄}} = {[1,2], [1,3], [1,4]}`. Partition into regions relative to `p = [1,3]`: `P_0^L = {[1,2]}` (the single position with `v < p`); `P_0^R = {[1,3], [1,4]}` (positions with `v ≥ p`); `P_0^{s_L} = ∅` (the link subspace is empty for this example).
...
Step 3 (K.μ⁺): adds Insertion positions `{[1,3] ↦ a_{new0}, [1,4] ↦ a_{new1}}` and Shifted-right positions `{[1,5] ↦ a₃, [1,6] ↦ a₄, [1,7] ↦ a₅}`. By LP9, the new projection contributions are exactly those new V-positions whose image lies in `coverage(e_1) = {a₂, a₃, a₄}`. Among Insertion: by LP19a (TightFreshness; ASN-0098) applied to the tightness assumption `tight(e_1, Σ_{e_1})`, the freshly allocated `a_{new0}, a_{new1}` cannot lie in `coverage(e_1)` — so `a_{new0}, a_{new1} ∉ coverage(e_1)` and `N_I = ∅`. Among Shifted-right: `a₃, a₄ ∈ coverage(e_1)` but `a₅ ∉ coverage(e_1)`, so `N_S = {[1,5], [1,6]}`.
