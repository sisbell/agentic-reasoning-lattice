...
**Composite atomicity.** The post-state Σ' is uniquely determined by this contract, though the substrate decomposition that realises it is not.
...
The one genuinely novel intermediate is **post-K.μ⁻**; the other M-modifying step, K.μ⁺, lands on `Σ'` itself (below), so only the contracted state needs a self-contained argument.
...
- *After step 3's K.μ⁺ — this intermediate is `Σ'`.* K.μ⁺ is the last step that modifies `M`, and the subsequent K.ρ firings frame `M`, so the post-K.μ⁺ arrangement equals the composite's final arrangement `M'(d)`. The arrangement invariants here — S2, S3★, S8a, S8-depth, S8-fin, S8★, D-CTG★, D-MIN★, D-SEQ★ on `V_{s_C}(d') = {[s_C, 1, …, 1, k] : 1 ≤ k ≤ N + n}` — are therefore not a fresh obligation: they are the very post-state assertions established in §Verifying the Invariants, which verified `Σ'` directly. No separate per-intermediate derivation exists to give. J0 (composite-boundary) is first satisfied at this step: each `a_k ∈ dom(C')` has its placement at `shift(p, k)`.
