...
- *S8-depth (FixedDepthVPositions, ASN-0036).* Insertion positions have depth `m_C` by claim **INS.inv.depth**, established at §Effect Two: Placement. Left and Shifted-right positions have depth `m_C` by §Effect Three's shift clause and the pre-state S8-depth. S8-depth holds across all subspaces of the post-state.

- *S8a (VPositionWellFormedness, ASN-0036).* Insertion positions satisfy S8a by claim **S8a**, established at §Effect Two: Placement. Left positions inherit S8a from the pre-state (unchanged), and Shifted-right positions satisfy S8a because `shift(v, n)` preserves zero-freedom, depth, and positivity (§Atomicity, step 3). So S8a holds across the post-state.
