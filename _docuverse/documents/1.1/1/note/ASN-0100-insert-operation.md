...
This exposes the *decoupling* of the V-position index from the I-address chain index: the V-positions restart at `[s_C, 1]` while the I-addresses resume at chain indices 3 and 4. D-MIN★ and D-SEQ★ are stated over `V_{s_C}(d')` alone — `min = [s_C, 1]`, explicit form `{[1, k] : 1 ≤ k ≤ 2}` — and so are blind to the chain frontier index: the post-state V-positions are identical to the empty-document outcome whether the images are `[d.0.s_C.1..2]` or `[d.0.s_C.3..4]`.
