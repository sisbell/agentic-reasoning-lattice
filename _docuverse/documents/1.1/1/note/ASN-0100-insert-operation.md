...
- *I3-V (PostInsertionVacating).* I3-V asserts that positions `v ≥ p` in the source subspace which are *not* the image of any pre-state shift are *not* in `dom(M'(d))`. Applied to INSERT, this would exclude the Insertion positions `shift(p, k)` from `dom(M'(d))` — directly contradicting INS.M-insert.
...
2. **One K.μ⁻ on `d`** retaining the Left prefix of `V_{s_C}(d)` (with `n'_{s_C} = p_m − 1`) and retaining all of `V_{s_L}(d)` (with `n'_{s_L} = n_{s_L}`). The canonical decomposition omits this step in three cases, distinguished by whether the omission is *forced* (no admissible K.μ⁻ firing exists under the canonical retention parameters and INSERT's frame) or a *canonical-decomposition choice* (an alternative decomposition with different K.μ⁻ retention parameters could fire K.μ⁻ and reach the same Σ' via a subsequent K.μ⁺ rebuild, but the canonical decomposition omits K.μ⁻ for parsimony):
   ...
   *(i.b) — Forced under INS.frame.subspace.* When `V_{s_C}(d) = ∅` and `V_{s_L}(d) ≠ ∅`, K.μ⁻'s `dom(M(d)) ≠ ∅` precondition holds, but `n_{s_C} = 0` forces the content-subspace retention parameter `n'_{s_C} ∈ {0, …, 0}` to equal `n_{s_C}` (foreclosing strict shrinkage in `s_C`); the strict-shrinkage clause `(E S :: n'_S < n_S)` therefore reduces to `n'_{s_L} < n_{s_L}`, which shrinks `V_{s_L}(d)` and violates INS.frame.subspace at the composite boundary. Within the K.μ⁻ + K.μ⁺ vocabulary (K.μ⁺ adds only `s_C` positions per its content-subspace restriction in ASN-0047), the shrunken `s_L` positions cannot be restored; every admissible K.μ⁻ firing leaves the post-state with strictly fewer `s_L` positions and violates the boundary frame.

   *(ii) — Canonical-decomposition choice when `V_{s_L}(d) ≠ ∅`; forced when `V_{s_L}(d) = ∅`.* When `p_m = N + 1` (append case — Left = entire pre-state `V_{s_C}(d)`), the canonical retention `n'_{s_C} = N = n_{s_C}` forecloses strict shrinkage in `s_C`, so the strict-shrinkage clause `(E S :: n'_S < n_S)` reduces to `n'_{s_L} < n_{s_L}`. If `V_{s_L}(d) ≠ ∅`, strict shrinkage via `s_L` is admissible by K.μ⁻'s precondition but violates INS.frame.subspace at the boundary; the omission of K.μ⁻ in the canonical decomposition is then a parsimony choice rather than a forced consequence — an alternative decomposition could fire K.μ⁻ with `n'_{s_C} < N` (admissible since `s_C` is non-empty here) and follow with a K.μ⁺ that re-adds the discarded `s_C` positions before adding Insertion, reaching the same Σ'. If `V_{s_L}(d) = ∅`, strict shrinkage via `s_L` is impossible (`n'_{s_L} < n_{s_L} = 0` has no solution in ℕ); combined with the foreclosed `s_C` shrinkage under the canonical retention parameters, no K.μ⁻ firing is admissible consistent with the canonical retention and INSERT's frame.
...
The exhaustiveness clause follows from the substrate decomposition: step 3's K.μ⁺ adds *precisely* the Insertion and Shifted-right positions (per the K.μ⁺ amendment, only `s_C` positions are added); step 2's K.μ⁻ (when fired) retains *only* the Left positions in `s_C`; and no other elementary step modifies `s_C` positions. The S2 functionality argument in §Arrangement functionality below depends on this exhaustiveness — pairwise disjointness of Left, Insertion, and Shifted-right closes only because no fourth region of `s_C` positions exists in the post-state.
