...
I3-S2 (PostInsertionFunctionality; ASN-0082) discharges functionality on the Left + Shifted-right + cross-subspace portion of the post-state; the Insertion region's contribution to functionality is the explicit pairwise-disjointness argument just given. For other subspaces and other documents, `M'` equals `M`, which is already a function by the pre-state S2.
...
I3-S3 (PostInsertionReferentialIntegrity; ASN-0082) discharges referential integrity over the Left + Shifted-right + cross-subspace portion of the post-state; the Insertion region's contribution is the freshness of each `a_k ∈ dom(C')` shown above.
