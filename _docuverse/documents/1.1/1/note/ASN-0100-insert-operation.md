...
This yields `N + 1` admissible positions: `j = 0` inserts before the first character, `j = N` after the last, and `j ∈ {1, …, N−1}` in the interior. The *append* operation Nelson lists as a separate convenience (APPEND) is the `j = N` case of INSERT — distinct in name only (a caller need not know `N` if a separate API offers append directly), identical in semantic effect.
...
The subspace partition L0 requires more care, because it has two conjuncts: `(A a ∈ dom(Σ.L) :: subspace_I(a) = s_L)` and `(A a ∈ dom(Σ.C) :: subspace_I(a) = s_C)`. The first conjunct ranges over `dom(L)`, unchanged, and so is discharged trivially. The second conjunct ranges over `dom(C)`, which INSERT *extends* by the fresh content addresses `a_0, …, a_{n−1}` (INS.C) — it is therefore not a property of `L` alone. The pre-existing entries of `dom(C)` satisfy the conjunct by the pre-state invariant; each freshly allocated `a_k` satisfies it directly — `subspace_I(a_k) = s_C` because `a_k` is an emission of `A_C(d)` (DisjointSubAllocatorChains; ASN-0093). Hence L0's content clause is preserved.
