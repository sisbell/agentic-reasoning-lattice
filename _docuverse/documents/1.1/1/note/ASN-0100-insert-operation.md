...
Because `M'(d)` coincides pointwise with the I3 arrangement on Left ∪ Shifted-right, every property I3 establishes of that arrangement transfers verbatim to `M'(d)` restricted to those two regions. We record the consequence once, here, and cite it directly in the verification sections below without rebuilding the premise: on Left ∪ Shifted-right, `M'(d)` inherits I3-S2 (PostInsertionFunctionality), I3-S3 (PostInsertionReferentialIntegrity), I3-VP (PostInsertionWellFormedness), I3-VD (PostInsertionDepthUniformity), and I3-fin (PostInsertionFiniteness), all from ASN-0082. The Insertion region lies outside I3's scope and is discharged separately wherever it bears.
...
For Left and Shifted-right content-subspace positions: referential integrity on these regions is the inherited I3-S3 (§Effect Three) — `ran(M'(d)) ⊆ dom(C')` there. Concretely, each image is `M(d)(v')` for some pre-state position `v' ∈ dom(M(d))` with `subspace(v') = s_C`, which pre-state S3★ places in `dom(C)`, and `dom(C) ⊆ dom(C')` by P0 (ContentPermanence; ASN-0047).
...
| INS.proj | Projection-shift correspondence: project(ℓ, i, d', Σ') = π(project(ℓ, i, d', Σ)) ∪ N_{ℓ,i}, with π the region-aware shift map and N_{ℓ,i} the fresh-Insertion captures (= ∅ for tight endsets); π, N_{ℓ,i} defined and derived in §Coverage and link discoverability | introduced |
| INS.atomicity | INSERT's substrate composite preserves per-state invariants at every intermediate state, with composite-boundary properties (P4★, P4a, P7a) and couplings (J0, J1★, J1'★) holding at the boundary Σ →* Σ' (established in §Atomicity and Canonical Order) | introduced |
