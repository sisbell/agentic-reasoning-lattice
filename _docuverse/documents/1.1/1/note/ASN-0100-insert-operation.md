...
**Chain emissions in ordinal-shift form (INS.chain-shift).** The allocation recurrence `a_{k+1} = inc(a_k, 0)` admits an equivalent ordinal-shift reading that is load-bearing wherever a contiguous run of `A_C(d)`'s emissions is treated as a single mapping block. We claim that for any contiguous chain segment of `A_C(d)`, successive emissions are ordinal shifts: if `a_i` and `a_{i+j}` are chain elements with `a_{i+1} = inc(a_i, 0)` at each step, then `a_{i+j} = shift(a_i, j)`. In particular `a_k = shift(a_0, k)` for the Insertion chain (`0 ≤ k < n`).
...
The identity iterates because `inc(·, 0)` preserves T4 (TA5a; ASN-0034) and preserves length (TA5(c); ASN-0034): every successor `a_{i+1}` is again a T4-valid same-length address, so `sig = #` holds at every index and the inc/shift equivalence applies at each step. Composing by TS3 (ShiftComposition; ASN-0034) — `shift(shift(a_i, j), 1) = shift(a_i, j + 1)` — and unfolding from the base `a_i = shift(a_i, 0)` (OrdinalShiftBase; ASN-0058) yields `a_{i+j} = shift(a_i, j)` by induction on `j`. Specialising to `i = 0` gives `a_k = shift(a_0, k)`.
...
**Scope of ASN-0082's I3 against INSERT's post-state.** This ASN cites I3's positive shift clause and the companion lemmas I3-L, I3-X, I3-D, I3-VD, I3-VP, I3-fin, I3-S2, I3-S3, which govern the regions ASN-0082's shift-only model covers; it does not cite I3-V, I3-CS, I3-CX, I3-C, or I3-S7, which characterise that shift-only post-state and do not hold of INSERT's `M'(d)`, since INSERT adds an Insertion region and allocates fresh content that the shift-only model omits. We state the partition once here and apply it throughout the invariant verification below: each cited I3 lemma discharges its predicate on the Left + Shifted-right + cross-subspace regions, and the Insertion region's contribution is verified separately within that subsection. The per-invariant subsections do not re-explain this partition.
...
3. **One K.μ⁺ on `d`** adding the Insertion V-positions (mapping `shift(p, k) ↦ a_k` for `0 ≤ k < n`) and the Shifted-right V-positions (mapping `shift(v, n) ↦ M(d)(v)` for each `v ∈ V_{s_C}(d)` with `v ≥ p`). In the canonical decomposition, where step 2's K.μ⁻ retains exactly the Left prefix, these are the only `s_C` positions K.μ⁺ adds. This "adds exactly Insertion ∪ Shifted-right" is a feature of the *canonical* firing, not a constraint binding on every admissible decomposition: K.μ⁺'s vocabulary precondition (ASN-0047) is permissive — it admits any new `s_C` positions with images in `dom(C)` provided the resulting `M'(d)` satisfies D-CTG★ and D-MIN★ — so an alternative decomposition may discard more of `V_{s_C}(d)` under K.μ⁻ and re-add it under K.μ⁺ (see §Atomicity). Exhaustiveness (INS.M-exhaustive) is a property of the post-state `V_{s_C}(d')`, not of the canonical K.μ⁺ firing; it is established decomposition-independently in the §Effect — Arrangement clause below. All additions in step 3 lie in subspace `s_C`, as required by K.μ⁺'s content-subspace restriction (ASN-0047).
...
The exhaustiveness clause is a property of the uniquely-determined post-state `V_{s_C}(d')`, established once and decomposition-independently. The canonical decomposition produces exactly Left ∪ Insertion ∪ Shifted-right: when K.μ⁻ fires it retains exactly the Left prefix and removes the Right region, and step 3's K.μ⁺ adds exactly the Insertion and Shifted-right positions (in subspace `s_C`, per the K.μ⁺ amendment); when K.μ⁻ is omitted the Right region of the pre-state `V_{s_C}(d)` — the set `{v ∈ V_{s_C}(d) : v ≥ p}` — is empty (the empty-content-subspace cases have `V_{s_C}(d) = ∅`, so no V-position satisfies `v ≥ p`; the append case has `p_m = N + 1`, so no `v ∈ V_{s_C}(d)` with last components in `{1, …, N}` satisfies `v ≥ p`), so the preserved pre-state positions are exactly Left, the Shifted-right region is empty, and K.μ⁺ adds exactly Insertion. In both routings the canonical decomposition yields `V_{s_C}(d') =` Left ∪ Insertion ∪ Shifted-right with no fourth region — no other elementary step can introduce an `s_C` position, since K.α and K.ρ frame `M` (`M' = M`, ASN-0047) and K.μ⁻ only removes. This direct step-tracking establishes exhaustiveness for the canonical decomposition without appeal to any uniqueness claim. To transfer it to every admissible decomposition we then invoke the uniqueness of Σ' (§Atomicity): because that uniqueness argument takes the canonical exhaustiveness as input (it is the ⊆ direction of the boundary `V_{s_C}(d')` equality there), the transfer cites the direct result plus uniqueness and so closes no circle — an alternative decomposition that discards and re-adds more of `V_{s_C}(d)` under K.μ⁻ + K.μ⁺ reaches the identical post-state content subspace. The S2 functionality argument in §Arrangement functionality below depends on this exhaustiveness — pairwise disjointness of Left, Insertion, and Shifted-right closes only because no fourth region of `s_C` positions exists in the post-state.
...
I3-S2 (PostInsertionFunctionality; ASN-0082) discharges functionality on the regions it covers; the Insertion region's contribution is the explicit pairwise-disjointness argument just given. For other subspaces and other documents, `M'` equals `M`, which is already a function by the pre-state S2.
...
I3-S3 (PostInsertionReferentialIntegrity; ASN-0082) discharges referential integrity on the regions it covers; the Insertion region's contribution is the freshness of each `a_k ∈ dom(C')` shown above.
...
For D-CTG★ we must discharge the full closed-interval form: for every depth-`m_C`, subspace-`s_C`, all-positive tuple `z` with `min ≤ z ≤ max` under T1 — where `min = [s_C, 1, …, 1]` and `max = [s_C, 1, …, 1, N + n]` are the union's extremes — we must show `z ∈ V_{s_C}(d')`. The slice ranges over *every* such tuple, not only those sharing the prefix `[s_C, 1, …, 1]`; for `m_C ≥ 3` it contains off-prefix tuples such as `[s_C, 2, 1, …, 1]` that must be shown to fall outside `[min, max]`. We invoke D-CTG-depth (SharedPrefixReduction; ASN-0036), which for depth `m_C ≥ 3` reduces contiguity of a non-empty `V_{s_C}` to contiguity of the last component alone, all positions sharing components `2` through `m_C − 1`. Concretely: write `z = [s_C, z_2, …, z_{m_C}]` with every `z_j ≥ 1`. Suppose some `z_j > 1` for a least `j` with `2 ≤ j ≤ m_C − 1`. Then `z` agrees with `max` on components `1, …, j − 1` (all equal to `1` at those positions for `2 ≤ i < j`, and `s_C` at position 1) and has `z_j > 1 = max_j` at the first divergence, so `z > max` by T1 case (i) — contradicting `z ≤ max`. Hence `z_j = 1` for all `2 ≤ j ≤ m_C − 1`, so `z = [s_C, 1, …, 1, z_{m_C}]`; with `1 ≤ z_{m_C} ≤ N + n` forced by `min ≤ z ≤ max` at the last component, `z ∈ V_{s_C}(d')`. (For `m_C = 2` there are no interior positions to constrain and the reduction is immediate.) The last-component range `{1, …, N + n}` being contiguous then closes D-CTG★.
...
- *D-CTG★:* we discharge the full closed-interval form over the entire depth-`m`, subspace-`s_C` slice, not only the prefix family `{[s_C, 1, …, 1, k]}`. The extremes are `min = [s_C, 1, …, 1]` and `max = [s_C, 1, …, 1, n]`. By D-CTG-depth (SharedPrefixReduction; ASN-0036), for `m ≥ 3` contiguity reduces to the last component once all positions are shown to share components `2` through `m − 1`. For any all-positive depth-`m` tuple `z = [s_C, z_2, …, z_m]` with `min ≤ z ≤ max`: if some `z_j > 1` for a least `j` with `2 ≤ j ≤ m − 1`, then `z` agrees with `max` on positions `1, …, j − 1` and diverges with `z_j > 1 = max_j`, giving `z > max` by T1 case (i) — contradicting `z ≤ max`. So `z_j = 1` for `2 ≤ j ≤ m − 1`, and `1 ≤ z_m ≤ n` forces `z = [s_C, 1, …, 1, z_m] ∈ V_{s_C}(d')`. (For `m = 2` there are no interior components and the reduction is immediate.) The last-component range `{1, 2, …, n}` being contiguous closes D-CTG★.
...
For V-adjacency, M7 requires `shift(p, k+1) = shift(p, k) + 1`, reading the mapping-block `+ 1` as `shift(·, 1)` (OrdinalShiftBase; ASN-0058): this is `shift(shift(p, k), 1) = shift(p, k+1)`, an instance of TS3 (ShiftComposition; ASN-0034).

For I-adjacency, M7 requires `a_{k+1} = a_k + 1`, again reading `+ 1` as `shift(·, 1)`. By INS.chain-shift (Effect One above), `a_{k+1} = shift(a_k, 1)` exactly — established there from the T4-validity of each chain element (ChainElementT4Validity; ASN-0093) via TA5-SigValid (so `sig = #`), TA5 (so `inc(·, 0)` bumps only the last component, matching `shift(·, 1)`), TA5a (so the identification iterates under T4 preservation), and TS3 (composing the per-step shifts). This is the M-adjacency M7 demands, not an assertion.

Both adjacencies holding, the `n` Insertion blocks merge into a single length-`n` block `(p, a_0, n)`, whose denotation `{(shift(p, k), shift(a_0, k)) : 0 ≤ k < n}` equals `{(shift(p, k), a_k) : 0 ≤ k < n}` by INS.chain-shift's `a_k = shift(a_0, k)`. The Left and Shifted-right portions are derived from the pre-state decomposition as follows. A pre-state block `(v', a', m')` whose V-extent straddles `p` — i.e., `v' < p` and `p ≤ shift(v', m' − 1)` — is first split at the interior offset `c := p_m − v'_m ∈ {1, …, m' − 1}` via M4 (ASN-0058), yielding a Left piece `(v', a', c)` and a Right piece `(shift(v', c), a' + c, m' − c)`. After all straddling pre-state blocks are split, every remaining pre-state block lies entirely below `p` (a Left block) or entirely at or above `p` (a Right block). The Left blocks transfer unchanged to the post-state; each Right block `(v', a', m')` becomes a Shifted-right block `(shift(v', n), a', m')` — V-start advanced by `n`, with I-start and width unchanged (shift acts only on V-positions, not on I-addresses; the width is the count of mapped positions, which is the same as in the source block). The post-state content-subspace decomposition is finite and well-defined; its existence is guaranteed by C1a applied to the restriction `M'(d)|_{V_{s_C}(d')}` (preconditions discharged below). We do not appeal to M2 on the whole post-state arrangement `M'(d)`: M2 carries the precondition S3 (ReferentialIntegrity, ASN-0036), `ran(M(d)) ⊆ dom(C)`, which fails for the whole extended-state arrangement whenever `V_{s_L}(d') ≠ ∅` — there link-subspace positions map into `dom(L)`, so the whole arrangement satisfies only S3★ (GeneralizedReferentialIntegrity, ASN-0047), not S3. C1a sidesteps this by ranging over the single content subspace, whose range is `⊆ dom(C)`; the link subspace is handled separately by S8★'s trivial length-1 decomposition.

This discharges S8★ conditions (a) (lockstep displacement, established by the run construction above) and (b) (label well-definedness, from each block being a well-formed mapping block over `dom(C')`), together with *existence* via C1a (which lifts M11 to the restriction, as discharged above). It remains to discharge condition (c) — uniqueness of the maximal-run decomposition, which S8★ requires only on the content subspace. Because `M'(d)|_{V_{s_C}(d')}` is a restriction, uniqueness is again supplied not by M12 (CanonicalUniqueness; ASN-0058) directly — which is stated for whole arrangements — but by C1a (RestrictionDecomposition; ASN-0058), which lifts M12 to restrictions whose induced domain lies within a single subspace and asserts that such a restriction "admits a *unique* maximally merged block decomposition." C1a's three preconditions for `f = M'(d)|_{V_{s_C}(d')}` — (i) functionality from S2, (ii) finite `dom(f)` from S8-fin, (iii) single-subspace induced domain with common depth `m_C ≥ 2` from S8-depth — are the same preconditions discharged for existence above, and the C1a-lifted M12 factors through M12a (maximal runs partition the domain) and M12b (every block of a maximally merged decomposition is a maximal run). C1a therefore makes the maximal-run decomposition of `M'(d)|_{V_{s_C}(d')}` unique, closing condition (c) on the content subspace. (Condition (c) is not required on the link subspace, where S8★ asks only for the trivial length-1 decomposition.)
...
where:
- *For `d' ≠ d`:* `π` is the identity and `N_{ℓ,i} = ∅`, so `project(ℓ, i, d', Σ') = project(ℓ, i, d', Σ)`. LP4 (ArrangementSpecificity; ASN-0098) and LP5 (CrossDocumentIndependence; ASN-0098) are single-step lemmas ("for every transition `Σ → Σ'`"), and INSERT is a composite of `2n+1`/`2n+2` elementary steps, so we must chain them rather than cite them once. Each elementary step of the substrate decomposition (every K.α, the optional K.μ⁻, K.μ⁺, every K.ρ) carries the explicit cross-document frame `(A d'' : d'' ≠ d : M'(d'') = M(d''))`, so `M(d')` is unmodified at every step. LP4 applied at each step (its hypothesis `M_{j+1}(d') = M_j(d')` is met by that step's frame) gives `project(ℓ, i, d', Σ_{j+1}) = project(ℓ, i, d', Σ_j)`; composing across the finite step sequence `Σ = Σ_0 → … → Σ_m = Σ'` yields `project(ℓ, i, d', Σ') = project(ℓ, i, d', Σ)`. Equivalently and directly: `project(ℓ, i, d', ·)` depends only on `M(d')` (composite-invariant, just shown) and on `coverage(e_i)` (composite-invariant by LP3★), so it is unchanged across the composite. This matches the step-by-step rigor of the `d' = d` case below; no unproved "LP4★/LP5★" closure is invoked.
...
- *After each of the `n` K.ρ firings of step 4.* R extends by one `(a_k, d)` pair. The composite-boundary coupling J1★ requires every newly-arranged content-subspace I-address to be in R'; after all `n` firings, J1★ holds. P4★ (`Contains_C(Σ') ⊆ R'`) holds because every content-subspace range entry of M'(d) is either a pre-state entry (already in R via pre-state P4★) or an Insertion-region freshly allocated `a_k` (now in R via step 4). P4a (TraceWitnessing) holds because each `(a_k, d) ∈ R'` admits the composite boundary `Σ'` as its witness state: K.ρ frames M, so the Insertion placement of step 3's K.μ⁺ keeps `a_k ∈ ran(M'(d))` at `Σ'` (a member of the boundary trace, unlike the composite-interior end-of-step-3 state). P7 (ProvenanceGrounding; ASN-0047) holds at this intermediate because the new R-entry `(a_k, d)` satisfies `a_k ∈ dom(C)` — by the forced ordering of step 1's K.α(a_k) firing before step 4's K.ρ(a_k, d) firing — and pre-state R entries inherit by P0; the per-state predicate P7 is therefore preserved across each K.ρ commit. S8★ (PerSubspaceSpanDecomposition; ASN-0047) holds at each K.ρ intermediate because K.ρ frames `M` (`M' = M`): the arrangement and hence its per-subspace decompositions are identical to the end-of-step-3 (= post-state) arrangement, where S8★ was already discharged.
...
- *K.ρ commutes with K.μ⁺ at the per-state level; cross-composite coupling consistency is secured by atomicity, not by elementary order.* At the level of per-state invariants, K.ρ neither constrains nor is constrained by K.μ⁻/K.μ⁺: K.ρ's precondition depends only on C and the entity set and its effect modifies only R; K.μ⁻ and K.μ⁺ modify only M and depend (besides M itself) on C only for K.μ⁺'s `a ∈ dom(C)` clause; K.ρ does not consult M, and K.μ⁻/K.μ⁺ do not consult R. So no per-state invariant is sensitive to the order of K.ρ relative to K.μ⁻ or K.μ⁺. The only forced ordering on K.ρ(a_k, d) is that it follow its own K.α(a_k), which supplies `a_k ∈ dom(C)` (the third K.α-induced ordering above); relative to K.μ⁺ it commutes — it may fire before or after the placement of `a_k`.

  Each composite's coupling constraints J0, J1★, J1'★ are obligations on *that composite's own* boundary `(Σ, Σ')` — per ValidComposite★ (ASN-0047) they hold for the composite as a whole, evaluated only between its initial state Σ and final state Σ'. INSERT discharges all three at its own boundary, where every `a_k` is both placed by K.μ⁺ and recorded by K.ρ regardless of the internal order among those steps. Composite-level atomicity (INS.pre) makes INSERT's intermediate states non-observable to other composites, so no in-flight intermediate is ever evaluated as a foreign composite's boundary. Consequently the K.ρ/K.μ⁺ order is free.
...
The canonical decomposition (steps 1–4 above) places K.ρ firings at the end, but — by the argument just given — this placement is expository, not mandatory: each K.ρ(a_k, d) is forced only to follow its own K.α(a_k) (so that `a_k ∈ dom(C)`), and is otherwise free to precede or follow the K.μ⁺ placement of `a_k`. The ordering freedom that remains is in the order of the K.ρ firings among themselves (they commute, by the next bullet) and in interleaving them with the K.μ⁻/K.μ⁺ steps and later K.α steps, provided each K.ρ(a_k, d) still follows its own K.α(a_k).
...
The two atomicity levels INSERT relies on are stated in the contract's Environmental Assumptions (INS.pre) and not re-derived here: elementary-level atomicity is supplied unconditionally by SequentialTransitionAxiom (ASN-0093), while composite-level atomicity is the stronger environmental precondition the substrate must provide. When it holds, INSERT's intermediate states are non-observable to other composites and Σ' is determined by the contract above; when it is violated, Σ' is a joint product of the interleaved composites rather than the post-state specified here.
...
| INS.M-exhaustive | (A v : v ∈ dom(M'(d)) ∧ subspace(v) = s_C :: v ∈ Left ∪ Insertion ∪ Shifted-right); the post-state's text-subspace domain contains no s_C positions beyond the three regions — a property of the uniquely-determined Σ' (§Atomicity), decomposition-independent: the canonical K.μ⁻ + K.μ⁺ produce exactly those positions and uniqueness transfers it to all admissible decompositions | introduced |
