...
- *K.ρ's per-state independence, and why its placement after K.μ⁺ is nonetheless fixed.* At the level of per-state invariants, K.ρ neither constrains nor is constrained by K.μ⁻/K.μ⁺: K.ρ's precondition depends only on C and the entity set and its effect modifies only R; K.μ⁻ and K.μ⁺ modify only M and depend (besides M itself) on C only for K.μ⁺'s `a ∈ dom(C)` clause; K.ρ does not consult M, and K.μ⁻/K.μ⁺ do not consult R. So no per-state invariant is sensitive to the order of K.ρ relative to K.μ⁻ or K.μ⁺. The placement of K.ρ is nevertheless fixed — K.ρ(a_k, d) must fire *after* K.μ⁺ has placed `a_k` at `shift(p, k)` — by a boundary consideration, and fixing it is exactly what keeps INS.pre's atomicity scope *precisely two-fold*. The relevant coupling is J1'★ (ProvenanceRequiresExtensionContentSubspace; ASN-0047), which quantifies over the set difference `R' \ R` of whatever composite is evaluated against it — not over the pairs a composite "intends" to record. Were INSERT to fire K.ρ(a_k, d) *before* K.μ⁺ places `a_k`, the resulting intermediate would carry `(a_k, d) ∈ R` with `a_k ∉ ran(M(d))`. Composite atomicity over INS.pre's two named resources does not shield such an intermediate: items (i) and (ii) name the `A_C(d)` chain emission state and `M(d)`'s text subspace, *not* R, so another composite's elementaries may legally interleave at that point, and that composite's own `R' \ R` would then pick up the unmatched `(a_k, d)` by set difference and fail J1'★. There are two ways to make the spec coherent — enlarge the atomicity scope to cover R as a third protected resource, or fix the elementary order so the offending intermediate never arises — and INSERT takes the second. By firing every K.ρ(a_k, d) only after the K.μ⁺ placement of `a_k` (canonical step 4 after step 3), each provenance pair is matched by a placed content address at *every* intermediate, not merely at INSERT's own composite boundary; J1'★ then holds against any interleaving composite, and R needs no atomicity protection. This is why the required scope is exactly the two resources of INS.pre and no more: protecting the chain emission state and the text subspace suffices precisely because the elementary order keeps R self-consistent at every step.
...
The canonical decomposition (steps 1–4 above) places K.ρ firings at the end, and — by the argument just given — this placement is mandatory rather than merely expository: each K.ρ(a_k, d) must follow the K.μ⁺ placement of `a_k` so that no intermediate carries an unmatched provenance pair against J1'★. The ordering freedom that remains is in the order of the K.ρ firings among themselves (they commute, by the next bullet) and in interleaving them with later K.α/K.μ⁻ steps, provided each K.ρ(a_k, d) still follows the placement of its own `a_k`.
