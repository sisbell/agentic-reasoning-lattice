...
This discharges S8★ conditions (a) (lockstep displacement, established by the run construction above) and (b) (label well-definedness, from each block being a well-formed mapping block over `dom(C')`), together with *existence* via C1a (which lifts M11 to the restriction, as discharged above). It remains to discharge condition (c) — uniqueness of the maximal-run decomposition, which S8★ requires only on the content subspace. Because `M'(d)|_{V_{s_C}(d')}` is a restriction, uniqueness is again supplied not by M12 (CanonicalUniqueness; ASN-0058) directly — which is stated for whole arrangements — but by C1a (RestrictionDecomposition; ASN-0058), which lifts M12 to restrictions whose induced domain lies within a single subspace and asserts that such a restriction "admits a *unique* maximally merged block decomposition." C1a, whose preconditions were discharged above, lifts M12 to the restriction, factoring through M12a (maximal runs partition the domain) and M12b (every block of a maximally merged decomposition is a maximal run). C1a therefore makes the maximal-run decomposition of `M'(d)|_{V_{s_C}(d')}` unique, closing condition (c) on the content subspace. (Condition (c) is not required on the link subspace, where S8★ asks only for the trivial length-1 decomposition.)
