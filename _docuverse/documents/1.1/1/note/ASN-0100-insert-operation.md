...
Which invariants hold at each intermediate versus only at the boundary — the Class (a) per-state / Class (b) composite-boundary split — is stated and proved in §Atomicity and Canonical Order.
...
INS.proj's `d' = d` text-subspace clause gives `project(ℓ, 1, d, Σ') = π(project(ℓ, 1, d, Σ)) ∪ N_{ℓ,1}`. Instantiating numerically: π is the identity on `P_0^L = {[1,2]}` and `shift(·, 2)` on `P_0^R = {[1,3], [1,4]}`, so `π(project(ℓ, 1, d, Σ)) = {[1,2]} ∪ {[1,5], [1,6]} = {[1,2], [1,5], [1,6]}`. The new term `N_{ℓ,1} = N_I` is empty: `e_1` is tight at `Σ_{e_1}`, so by LP19a (TightFreshness; ASN-0098) the fresh `a_{new0}, a_{new1} ∉ coverage(e_1)`. Hence `project(ℓ, 1, d, Σ') = {[1,2], [1,5], [1,6]}` — the projection has shifted with the content, and the link has tracked it.
