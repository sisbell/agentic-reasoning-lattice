...
Consequently the Insertion region `{(shift(p, k), a_k) : 0 ≤ k < n}` coincides with `{(shift(p, k), shift(a_0, k)) : 0 ≤ k < n}`, which is exactly the denotation `⟦(p, a_0, n)⟧` of the mapping block `(p, a_0, n)` under OrdinalShiftBase (ASN-0058) — there the run's I-address `a_0 + k` reads as `shift(a_0, k)`. This is the lemma we invoke when the Insertion region is collapsed to one block (below, §S8★) or one run.
...
This ASN cites ASN-0082's I3 only for its positive shift clause and the affirmative companion lemmas (I3-L, I3-X, I3-D, I3-VD, I3-VP, I3-fin, I3-S2, I3-S3) that govern the regions ASN-0082's model does cover. ASN-0082's I3-S7 is *not* cited: it ranges over `dom(C)` and the document set rather than over V-position regions, and its own justification rests on the I3-C content-store frame (`Σ'.C = Σ.C`) that INSERT breaks; the S7-family invariants are instead discharged independently in §Verifying the Invariants below. I3-V, I3-CS, the redundant I3-CX, and I3-C are *disclaimed*: the first three describe a hypothetical shift-only operation whose post-state is properly contained in INSERT's, and they do not hold of INSERT's `M'(d)`; I3-C asserts the global content-store equality `Σ'.C = Σ.C`, which INSERT breaks by allocating `n` fresh addresses (Effect One). I3-C has no partial reading — there is no region of INSERT's post-state where `Σ'.C = Σ.C` holds. The content-store guarantee INSERT does affirm is the *pointwise* frame `(A a ∈ dom(C) :: C'(a) = C(a))` (S0, ContentImmutability, ASN-0036; P0, ContentPermanence, ASN-0047), not the exact-equality frame I3-C. The Insertion region's contribution to each invariant is verified independently below, alongside the shift-only contributions inherited from I3.
...
- *K.ρ commutes with K.μ⁺ at the per-state level; cross-composite coupling consistency is secured by atomicity, not by elementary order.* At the level of per-state invariants, K.ρ neither constrains nor is constrained by K.μ⁻/K.μ⁺: K.ρ's precondition depends only on C and the entity set and its effect modifies only R; K.μ⁻ and K.μ⁺ modify only M and depend (besides M itself) on C only for K.μ⁺'s `a ∈ dom(C)` clause; K.ρ does not consult M, and K.μ⁻/K.μ⁺ do not consult R. So no per-state invariant is sensitive to the order of K.ρ relative to K.μ⁻ or K.μ⁺. The only forced ordering on K.ρ(a_k, d) is that it follow its own K.α(a_k), which supplies `a_k ∈ dom(C)` (the third K.α-induced ordering above); relative to K.μ⁺ it commutes — it may fire before or after the placement of `a_k`.
  ...
  One might attempt a *cross-composite* argument that K.ρ(a_k, d) must nonetheless follow K.μ⁺: J1'★ (ProvenanceRequiresExtensionContentSubspace; ASN-0047) quantifies over the set difference `R' \ R` of whatever composite is evaluated against it, so were INSERT to fire K.ρ(a_k, d) *before* K.μ⁺ places `a_k`, the intermediate would carry `(a_k, d) ∈ R` with `a_k ∉ ran(M(d))`, and a foreign composite interleaving at that point would pick up the unmatched pair by set difference and fail J1'★. We reject this argument, because the identical set-difference reasoning applied to `dom(C)` shows that reordering cannot be the mechanism that secures cross-composite consistency. Step 1 fires all `n` K.α firings before step 3's K.μ⁺; between them each freshly allocated `a_k ∈ dom(C)` is *unplaced*. By the same reading, a foreign composite interleaving in the K.α→K.μ⁺ window would have `a_k ∈ dom(C_B') \ dom(C_B)` with `a_k` placed in no arrangement, failing its J0 (AllocationRequiresPlacement) and — before any K.ρ — its J1★/P7a. Unlike the R case, this exposure *cannot* be removed by reordering: K.μ⁺'s precondition requires `a_k ∈ dom(C)`, so allocation necessarily precedes placement and the unplaced window is structurally unavoidable. The two cases are therefore symmetric under the literal reading, and the C side is irreparable by elementary order; so if that reading were INSERT's concern, no decomposition could discharge it, and if it is not INSERT's concern, the R reordering buys nothing. Either way the apparent asymmetry between R and C collapses, and the "precisely two-fold by reordering R away" justification is unsound.

  The coherent reading is that each composite's coupling constraints J0, J1★, J1'★ are obligations on *that composite's own* boundary `(Σ, Σ')` — per ValidComposite★ (ASN-0047) they "hold for the composite as a whole, evaluated only between the initial state Σ and the final state Σ'" of the composite in question — accounting for its own allocations, placements, and provenance recordings, not for another composite's in-flight intermediates. INSERT discharges all three at *its own* boundary, where every `a_k` is both placed by K.μ⁺ and recorded by K.ρ regardless of the internal order among those steps. What keeps this accounting well-posed under concurrency is composite-level atomicity (INS.pre): it makes INSERT's intermediate states non-observable to other composites, so neither the unplaced-allocation window for C nor any unmatched-provenance window for R is ever evaluated as a foreign composite's `(Σ_B, Σ_B')` boundary. C and R are shielded by the same mechanism; neither is privileged. Consequently the K.ρ/K.μ⁺ order is free, and the protected resources named in INS.pre — the affected document's arrangement and its content sub-allocator chain — are required because the unplaced-allocation window lives there and INSERT's own post-state determinism depends on them, not because reordering renders R exempt.
