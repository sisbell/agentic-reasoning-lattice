...
realised by step 4's `n` K.ρ firings. The composite-boundary couplings J0, J1★, J1'★ (ASN-0047) hold at the boundary `Σ →* Σ'`; the discharge is given in §Provenance.
...
with `V_{s_C}(d') = {[1,1], [1,2], [1,3]}`; this first insertion fixes `m_C = 2` for `d`. Verifying the three regions: *Left* is empty (no pre-state position with `v < p`); *Insertion* is `{[1,1] ↦ a_{new0}, [1,2] ↦ a_{new1}, [1,3] ↦ a_{new2}}` matching `shift(p, k) ↦ a_k` for `k ∈ {0, 1, 2}` (with `shift([1,1], 0) = [1,1]` by OrdinalShiftBase); *Shifted right* is empty (no pre-state position with `v ≥ p`).
