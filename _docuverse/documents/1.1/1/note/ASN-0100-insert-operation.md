...
**Scope of ASN-0082's I3 against INSERT's post-state.** This ASN cites I3 (and its companion region lemmas I3-L, I3-X, I3-D) only for the positive shift clause and the regions those lemmas cover. I3's whole-post-state frames do *not* transfer to INSERT: in particular I3-C (`Σ'.C = Σ.C`) and I3-S7 (whose own ASN-0082 justification rests on I3-C) both fail here, because INSERT extends `dom(C)` with freshly allocated content that the shift-only model omits.
...
Pre-existing content store entries are preserved pointwise — every `a ∈ dom(C)` has `a ∈ dom(C')` with `C'(a) = C(a)` (S0, ContentImmutability; ASN-0036, and P0, ContentPermanence; ASN-0047) — discharged by INS.C's third clause. INSERT extends `dom(C)` by the freshly allocated addresses (Effect One), so the store itself is *not* unchanged (cf. the I3 scope note above).
