...
The branch selection keys on the *content store*, not the *arrangement*: K.α's first-emission predicate is `{a' ∈ dom(Σ.C) : origin(a') = d} = ∅`, a condition on `dom(C)`. The nuance this exposes is that an empty content subspace `V_{s_C}(d) = ∅` does not entail an empty content store — full content-subspace clearance via K.μ⁻ with `n'_{s_C} = 0` (PerSubspaceContractionScope, ASN-0047) leaves `d`'s prior content addresses in `dom(C)` by S0/P0. So when residual content persists (`{a' ∈ dom(Σ.C) : origin(a') = d} ≠ ∅`) under an empty arrangement, the *subsequent*-emission branch selected above off the persisted frontier `a_prev` is the one that fires — continuing `A_C(d)`'s chain rather than restarting it — even though `V_{s_C}(d)` is empty.
...
The subspace partition L0 has two conjuncts. Its `dom(L)` conjunct `(A a ∈ dom(Σ.L) :: subspace_I(a) = s_L)` ranges over the unchanged `dom(L)` and is discharged trivially. Its content conjunct `(A a ∈ dom(Σ.C) :: subspace_I(a) = s_C)` ranges over the `dom(C)` that INSERT *extends* by the fresh `a_0, …, a_{n−1}` (INS.C) and is discharged once, for both the K.α intermediates and the boundary, in the per-address paragraph of §Post-state V-position well-formedness (S7 bullet).
