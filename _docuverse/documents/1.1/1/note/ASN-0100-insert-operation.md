...
The post-state `V_{s_C}(d') = {[s_C, 1], [s_C, 2]}` at the freshly-pinned depth `m_C = 2`, with `M'(d') = {[s_C, 1] ↦ a_0, [s_C, 2] ↦ a_1}`. The invariant discharges go through exactly as in the first-insertion example, because the V-side structure depends only on the empty pre-state and the re-pinned depth, not on whether the I-side chain was fresh or continued: D-MIN★ holds (`min(V_{s_C}(d')) = [s_C, 1]`); D-SEQ★ holds with `n_{s_C} = 2` and `m_{s_C} = m_C = 2`; D-CTG★ holds via the closed-interval reduction at `K = n`; S8-depth holds at the re-pinned `m_C` (no prior depth to conflict); S8a holds (each `[s_C, k]` zero-free, depth `2`, positive); and S8★'s content-subspace run decomposition holds because `a_0, a_1` are a contiguous chain segment (INS.chain-shift), so `{([s_C, 1], a_0, 2)}` is the single lockstep run. The only difference from the pristine empty case is the K.α branch and the continuation of `A_C(d)` past `a_prev`; every V-side invariant is insensitive to that difference.
