...
Branch selection keys on `dom(C)`, not the arrangement: when residual `origin = d` content persists in `dom(C)` (`{a' ∈ dom(Σ.C) : origin(a') = d} ≠ ∅`), the *subsequent*-emission branch fires off the persisted frontier `a_prev` — continuing `A_C(d)`'s chain rather than restarting it — even when `V_{s_C}(d) = ∅`. (The worked example "Re-insertion into a cleared content subspace" exercises this.)
...
Because `M'(d)` coincides pointwise with the I3 arrangement on Left ∪ Shifted-right, the *content-frame-independent* arrangement lemmas I3 establishes of that arrangement transfer to `M'(d)` restricted to those two regions — specifically I3-S2 (functionality), I3-VP (S8a well-formedness), I3-VD (fixed depth), and I3-fin (finiteness), each a property of the arrangement alone. The transfer does *not* extend to I3's content-frame-dependent lemmas: ASN-0082's I3 assumes the content frame I3-C (`dom(C') = dom(C)`), which INSERT violates by growing `dom(C)` (INS.C). The lemmas resting on I3-C — I3-S7 (PostInsertionAllocationInvariants, which ASN-0082 derives "trivially by I3-C") and the inherited proof premise of I3-S3 — therefore do not transfer; INSERT re-derives S3 (§Referential integrity) and S7 (§Post-state V-position well-formedness) independently against the grown content store.
...
For Left and Shifted-right content-subspace positions: we re-derive referential integrity directly (not by inheriting I3-S3, whose proof premise rests on the content frame I3-C that INSERT violates; §Effect Three). Each image is `M(d)(v')` for some pre-state position `v' ∈ dom(M(d))` with `subspace(v') = s_C`, which pre-state S3★ places in `dom(C)`, and `dom(C) ⊆ dom(C')` by P0 (ContentPermanence; ASN-0047) — so `ran(M'(d)) ⊆ dom(C')` there.
