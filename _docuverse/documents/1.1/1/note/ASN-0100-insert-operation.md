...
2. **One K.μ⁻ on `d`** retaining the Left prefix of `V_{s_C}(d)` (with `n'_{s_C} = p_m − 1`) and retaining all of `V_{s_L}(d)` (with `n'_{s_L} = n_{s_L}`). The canonical decomposition omits this step in three cases, distinguished by whether the omission is *forced* (no admissible K.μ⁻ firing exists under the canonical retention parameters and INSERT's frame) or a *canonical-decomposition choice* (an alternative decomposition with different K.μ⁻ retention parameters could fire K.μ⁻ and reach the same Σ' via a subsequent K.μ⁺ rebuild, but the canonical decomposition omits K.μ⁻ for parsimony):
   ...
   *(i.b) — Canonical-decomposition choice (forced under canonical K.μ⁻ + K.μ⁺ sub-vocabulary, not under ValidComposite★'s full vocabulary).* When `V_{s_C}(d) = ∅` and `V_{s_L}(d) ≠ ∅`, K.μ⁻'s `dom(M(d)) ≠ ∅` precondition holds, but `n_{s_C} = 0` forces the content-subspace retention parameter `n'_{s_C} ∈ {0, …, 0}` to equal `n_{s_C}` (foreclosing strict shrinkage in `s_C`); the strict-shrinkage clause `(E S :: n'_S < n_S)` therefore reduces to `n'_{s_L} < n_{s_L}`, which shrinks `V_{s_L}(d)` and violates INS.frame.subspace at the composite boundary. Within the canonical K.μ⁻ + K.μ⁺ sub-vocabulary used by this decomposition (K.μ⁺ adds only `s_C` positions per its content-subspace restriction in ASN-0047), the shrunken `s_L` positions cannot be restored; every admissible K.μ⁻ firing leaves the post-state with strictly fewer `s_L` positions and violates the boundary frame. Under ValidComposite★'s full vocabulary, however, an alternative decomposition is admissible: K.μ⁻ shrinks `V_{s_L}(d)` to some `n'_{s_L} < n_{s_L}`, K.μ⁺ adds the Insertion region (subspace `s_C`), and successive K.μ⁺_L firings then restore the discarded `s_L` positions — each placing one link at `shift(max(V_{s_L}(d)), 1)` per K.μ⁺_L's V-position selection rule (D-CTG★ in ASN-0047), which admits sequential reconstruction of the original `V_{s_L}(d)` arrangement. Case (i.b) is therefore symmetric in character to case (ii) — both are canonical-decomposition parsimony choices rather than intrinsic-structural forcings; only case (i.a) is intrinsically forced (K.μ⁻'s `dom(M(d)) ≠ ∅` precondition fails outright under any vocabulary).
...
*Projection-shift correspondence (INS.proj).* Suppose a link `ℓ ∈ dom(L)` has a slot with endset `e_1` whose coverage is exactly `{a₂, a₃, a₄}` — three I-addresses in the pre-state range. **Tightness precondition of the trace below.** We assume `tight(e_1, Σ_{e_1})` (ASN-0098) — `e_1`'s coverage was tight at the state `Σ_{e_1}` of its incorporation into `ℓ`. This is the load-bearing assumption that makes `N_I = ∅` concrete; we trace the non-tight alternative at the end of this example. We trace the projection through the composite.
...
- *P6 (ExistentialCoherence, ASN-0047).* `(A a ∈ dom(C') :: origin(a) ∈ E')` (with `E_doc` the document subset of `E`; under ValidComposite★, `dom(M') = E'_doc`). Every pre-state `a ∈ dom(C)` inherits P6 from the pre-state because `dom(C) ⊆ dom(C')` and `origin(a)` is a property of the address `a` itself (an invariant of the addressing scheme, by S7 / StructuralAttribution), unchanged across the composite; meanwhile `E' = E` by INS.frame.E (no K.δ fires), so `origin(a) ∈ E_doc` lifts to `origin(a) ∈ E'_doc`. For each freshly allocated `a_k ∈ dom(C') ∖ dom(C)`: `origin(a_k) = d` by K.α's emission discipline (ASN-0093), and `d ∈ dom(M) = dom(M')` by INSERT's precondition combined with INS.frame.dom; hence `origin(a_k) ∈ E'_doc`. P6 is preserved across the composite.
