...
Because `M'(d)` coincides pointwise with the I3 arrangement on Left ∪ Shifted-right, every per-state property I3 establishes of that arrangement transfers verbatim to `M'(d)` restricted to those two regions. The Insertion region is discharged separately wherever it bears.
