...
INSERT grows `dom(C)`, so the post-state invariants are re-derived directly in §Verifying the Invariants.
...
The exhaustiveness clause is a property of the post-state `V_{s_C}(d')`. It is established in §Atomicity (the uniqueness argument for the arrangement of `d`), where step-tracking the canonical K.μ⁻ + K.μ⁺ decomposition shows the post-state text-subspace domain contains no `s_C` position beyond the three regions. The S2 functionality argument in §Arrangement functionality below depends on this exhaustiveness.
