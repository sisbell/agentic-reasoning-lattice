...
*Provenance discharge (J0, J1★, J1'★).* Step 4's two K.ρ firings add exactly `(a_{new0}, d)` and `(a_{new1}, d)` to R, giving `R' = R ∪ {(a_{new0}, d), (a_{new1}, d)}`. These are the two freshly allocated Insertion images, placed at `[1,3]` and `[1,4]`. The Shifted-right images `a₃, a₄, a₅` (placed at `[1,5], [1,6], [1,7]`) were already arranged pre-state at `[1,3], [1,4], [1,5]`, so they are already in R by pre-state P4★ and impose no new obligation. The coupling logic — J0 pairing each fresh `dom(C') \ dom(C)` address with its K.μ⁺ placement, J1★ recording each newly-arranged content-subspace image, J1'★ matching each new R'-entry back to a placement — is discharged generally in §Provenance; here it instantiates to the two pairs above, satisfied when step 4's K.ρ firings commit.
...
- *After each of the `n` K.ρ firings of step 4.* R extends by one `(a_k, d)` pair; the final K.ρ intermediate *is* the composite boundary `Σ'`, the state at which the provenance obligations come due. The composite-boundary couplings and properties J1★, P4★, P4a, P7a — together with the per-state P7 across each K.ρ commit — are discharged in full in §Provenance (R, P4★, P4a, P7a), evaluated at exactly this boundary state; we do not repeat that argument here. S8★ (PerSubspaceSpanDecomposition; ASN-0047) holds at each K.ρ intermediate because K.ρ frames `M` (`M' = M`): the arrangement and hence its per-subspace decompositions are identical to the end-of-step-3 (= post-state) arrangement, where S8★ was already discharged.
