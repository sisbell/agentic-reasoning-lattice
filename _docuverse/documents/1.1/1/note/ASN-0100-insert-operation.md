...
- *Left ∩ Insertion = ∅.* Left positions have last component `< p_m`; Insertion positions have last component in `{p_m, p_m + 1, …, p_m + n − 1}`. The component arithmetic splits on `k`, since `δ(k, m_C)` is defined only for `k ≥ 1` (OrdinalDisplacement, ASN-0034). For `k = 0`, OrdinalShiftBase (ASN-0034) gives `shift(p, 0) = p`, so the final component is `(shift(p, 0))_{m_C} = p_m`. For `1 ≤ k < n`, the OrdinalShift definition `shift(p, k) = p ⊕ δ(k, m_C)` (ASN-0034) and TumblerAdd's piecewise rule (ASN-0034) at action point `m_C` apply: positions `1, …, m_C − 1` are inherited from `p`, and the final component is `(shift(p, k))_{m_C} = p_{m_C} + δ(k, m_C)_{m_C} = p_m + k`. Across both cases the last component ranges over `{p_m, p_m + 1, …, p_m + n − 1}` for `0 ≤ k < n`.
