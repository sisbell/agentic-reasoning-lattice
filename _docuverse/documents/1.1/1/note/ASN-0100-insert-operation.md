...
**Notational convention** (used throughout this ASN). We adopt `shift(t, 0) := t`, extending OrdinalShift (ASN-0034), which is defined only for `n ≥ 1`. With this convention, the `j = 0` admissible position above resolves to `shift(min, 0) = min(V_{s_C}(d))` — insertion at the very beginning.
...
2. **One K.μ⁻ on `d`** — fired iff the pre-state content-subspace Right region `Right := {v ∈ V_{s_C}(d) : v ≥ p}` is non-empty — retaining the Left prefix of `V_{s_C}(d)` (with `n'_{s_C} = p_m − 1`) and retaining all of `V_{s_L}(d)` (with `n'_{s_L} = n_{s_L}`). The operative condition is single: K.μ⁻ appears in the canonical decomposition exactly when strict `s_C` contraction is both needed (the Right region must be removed before K.μ⁺ can reintroduce it shifted) and admissible while preserving `s_L`. When the Right region is empty, K.μ⁻ is omitted, and step 3's K.μ⁺ alone adds the Insertion region, leaving `V_{s_L}(d)` untouched.
...
We compute the post-state projection *directly* from the exhibited `M'(d)`, then check it against INS.proj. The coverage targets persist by S0, so `coverage(e_1) ∩ ran(M'(d)) = {a₂, a₃, a₄}`, and the V-positions of `M'(d)` carrying those three I-addresses are read off the arrangement: `[1,2] ↦ a₂`, `[1,5] ↦ a₃`, `[1,6] ↦ a₄`. Hence `project(ℓ, 1, d, Σ') = {v ∈ dom(M'(d)) : M'(d)(v) ∈ coverage(e_1)} = {[1,2], [1,5], [1,6]}`. This corroborates INS.proj's `d' = d` text-subspace clause `project(ℓ, 1, d, Σ') = π(project(ℓ, 1, d, Σ)) ∪ N_{ℓ,1}`: π is the identity on `P_0^L = {[1,2]}` and `shift(·, 2)` on `P_0^R = {[1,3], [1,4]}`, so `π(project(ℓ, 1, d, Σ)) = {[1,2]} ∪ {[1,5], [1,6]} = {[1,2], [1,5], [1,6]}`, and the new term `N_{ℓ,1} = ∅` (justified below). The directly-computed value and the INS.proj form agree — the projection has shifted with the content, and the link has tracked it.

*Why `N_I = ∅` here, and when it is not.* For `e_1`, both fresh Insertion addresses lie strictly above `coverage(e_1)`'s ceiling: K.α's subsequent-emission rule advances the chain frontier, so `a_{new0} = inc([d.0.s_C.5], 0) = [d.0.s_C.6]` and `a_{new1} = [d.0.s_C.7]` exceed `a_5 = [d.0.s_C.5]` under T1 (last components 6, 7 > 5), so neither lies in `coverage(e_1) = [a_2, a_5)` and `N_I = ∅`. (The endset `e_1` is in fact tight at its incorporation state — its span is canonical and every F-candidate `{a_2, a_3, a_4}` in `[a_2, a_5)` already lies in `dom(C)` — so LP19a (TightFreshness; ASN-0098) yields `N_I = ∅` independently; here the chain-frontier ordering already settles it, so tightness is not load-bearing for `e_1`.) Tightness becomes decisive precisely when an endset's declared coverage reaches *past* the chain frontier. Take `e_1'` delivered by the canonical span `(a_2, δ(10, #a_2))`, with `coverage(e_1') = [a_2, [d.0.s_C.12])` (since `a_2 ⊕ δ(10, #a_2) = shift(a_2, 10) = [d.0.s_C.12]`). This endset is non-tight at the INSERT pre-state: the F-candidates `[d.0.s_C.6], …, [d.0.s_C.11]` lie in the interval but are absent from `dom(C) ∪ dom(L)` — the chain frontier is only at `[d.0.s_C.5]` — so the tightness condition of ASN-0098 fails and LP19a is inapplicable. Now both fresh addresses fall inside coverage: `a_{new0} = [d.0.s_C.6]` and `a_{new1} = [d.0.s_C.7]` satisfy `a_2 ≤ a_{new k} < [d.0.s_C.12]`, so the Insertion positions `[1,3]` and `[1,4]` both contribute and `N_I = {[1,3], [1,4]}`. The INS.proj general form `π(project) ∪ N_{ℓ,i}` captures both regimes; the tight case is the special simplification with `N_I = ∅`.
...
The one genuinely novel intermediate is **post-K.μ⁻**; the other M-modifying step, K.μ⁺, produces the composite's final arrangement `M'(d)` (below), so only the contracted state needs a self-contained argument.
...
- *After step 3's K.μ⁺ — the arrangement at this intermediate equals the final `M'(d)`.* K.μ⁺ is the last step that modifies `M`, and the subsequent K.ρ firings frame `M`, so the post-K.μ⁺ arrangement equals the composite's final arrangement `M'(d)` even though the state itself is not yet `Σ'` (its provenance component `R` is extended only by the step-4 K.ρ firings). The arrangement invariants here — S2, S3★, S8a, S8-depth, S8-fin, S8★, D-CTG★, D-MIN★, D-SEQ★ on `V_{s_C}(d') = {[s_C, 1, …, 1, k] : 1 ≤ k ≤ N + n}` — are therefore not a fresh obligation: they are the very post-state assertions established in §Verifying the Invariants, which verified `Σ'` directly. No separate per-intermediate derivation exists to give. J0 (composite-boundary) is first satisfied at this step: each `a_k ∈ dom(C')` has its placement at `shift(p, k)`.
