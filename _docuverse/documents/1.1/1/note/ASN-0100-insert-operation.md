...
- *Entity-set invariants* — P8 (EntityHierarchy), NodeLineage (NodeDescentFromBootstrap), ActivatedEmission (every non-node entity is an emission of an activated entity-level sub-allocator), M0 (DocumentTumblerWellFormed; ASN-0093). The entity set E (equivalently `dom(M)` for documents under ValidComposite★) is unchanged at every intermediate by the frame `E' = E` (INS.frame.E): no K.δ fires. Each invariant is a predicate over E (or `dom(M)`) and so holds at every intermediate by inheritance from the pre-state — for ActivatedEmission, the existential witness for each `e ∈ E` is an activated entity-level sub-allocator `A` with `e ∈ dom(A)`, which by ActivatedEmission's own preservation argument (ASN-0047) is itself unchanged when no K.δ fires.
...
Fresh allocation is already fixed by INS.C and INS.alloc (§Effect One): the content-store effect appends `a_0, …, a_{n−1}`, each a fresh emission of `A_C(d)` with `origin(a_k) = d`. The consequence for attribution follows directly — attribution accrues to `d`'s owner, royalties (if priced) flow to `d`'s account.
...
The defining structural difference from COPY is captured by INS.identity: INSERT's allocation is fresh (INS.C, INS.alloc), whereas COPY introduces V→I references without allocating.
