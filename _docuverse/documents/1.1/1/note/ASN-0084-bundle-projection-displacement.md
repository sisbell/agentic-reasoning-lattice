...
*Parameterization convention.* The subscript K on REARRANGE_K denotes *static* parameterization by a cut sequence: the cut sequence is fixed at the point where the operation is invoked, not threaded through the runtime call's argument list. The runtime arguments are (Σ, d) — the global state and the target document — while K is a compile-time selector that indexes the family of operations {REARRANGE_K : K is an admissible cut sequence}. Each member of this family is a distinct operation, with its own postcondition (PivotPostcondition or SwapPostcondition specialized to K) and its own precondition R-PRE(K). Equivalently, one may read REARRANGE_K(Σ, d) as a three-argument operation REARRANGE(Σ, d, K); the subscripted form is a notational convenience that makes the dependence on K explicit at the operation name and keeps the runtime signature uniform across cut sequences. The letter K is chosen to disambiguate from the content store Σ.C: the subscript indexes *cut* sequences, not content. We retain the subscripted form REARRANGE_K throughout this ASN.

With this convention, REARRANGE_K has precondition R-PRE(K) and runtime signature (Σ, d) ↦ Σ'. The intra-document arrangement Σ.M(d) is the only mutated component: Σ.C, all other documents' arrangements Σ.M(d') for d' ≠ d, and the within-d non-S subspace portion of Σ.M(d) are preserved by the frame conditions; dom(M'(d)) = dom(M(d)) is asserted by the postconditions. This is the operation referent invoked by `wp(REARRANGE_K, Q)` in R-SP below.
...
The identity convention extends each step to the j = 0, w_μ = 0, or w_β = 0 corner cases (although Width positivity, which derives w_α, w_β ≥ 1 from R-PRE(iii)–(iv) for both n = 3 and n = 4, and additionally w_μ ≥ 1 for n = 4, excludes the latter two from this ASN's scope). Every appearance of "by associativity" or "associativity of ordinal addition" below this point cites the Extended Associativity reductions of the present paragraph; we do not repeat the case analysis at each consumer.
...
REARRANGE_K affects only the subspace-S portion of M(d); positions in any other subspace pass through unchanged. We collect the consequences of this single structural fact into one lemma so that subsequent sections (R-PPERM, R-SPERM, R-COMM, R-BLK, R-SP) can cite it once rather than re-deriving the non-S case in each invariant argument.
...
*Dependencies and direction.* R-NS depends on (i) the frame condition R-FRAME-P(a) for n = 3 or R-FRAME-S(a) for n = 4, which pins M'(d) = M(d) on positions with subspace(v) ≠ S; and (ii) the non-S branch of the bijection definition supplied by R-PPERM (3-cut) and R-SPERM (4-cut) — specifically the first clause of each piecewise definition, which sets π(v) = v whenever subspace(v) ≠ S. Clause (ii) is a *definitional* component of π (a stipulation, not a derived equation), pinned in place by the frame condition (i); it is independent of the subspace-S branches that R-PPERM and R-SPERM subsequently verify. R-NS is therefore upstream of the bijection lemmas: R-PPERM and R-SPERM cite R-NS(NS-π) at their non-S cases (rather than re-proving them), and R-NS does *not* depend on the bijectivity, injectivity, surjectivity, or subspace-S verification arguments of R-PPERM/R-SPERM.

*Forward-reference note (on presentation order only).* This lemma is stated here, immediately after the operation definition, so that subsequent sections (R-PPERM, R-SPERM, R-COMM, R-BLK, R-SP) cite a single non-S dispatch point rather than re-deriving the non-S case in each invariant argument. The non-S clause of π's piecewise definition is shown below as it appears in R-PPERM and R-SPERM, but the proof of R-NS below does not depend on any other clause of those lemmas — only on the frame condition and the first-clause stipulation, both of which are determined by the operation's contract and the definition of π. The reader who prefers a strictly bottom-up presentation may read the first lines of R-PPERM's and R-SPERM's piecewise definitions first to confirm the non-S clause, then return to R-NS; the substantive subspace-S content of R-PPERM and R-SPERM is downstream and is not invoked.
...
*Well-typedness argument (R-PRE(iii) — CS3).* CS3 — the same-subspace clause of R-PRE(iii) — does not admit a Q-falsifying counterexample of the same character as R-PRE(iv)'s, because CS3 is a *well-typedness guard* on the postcondition rather than a semantic precondition whose negation produces a contradiction at a well-typed witness. We sketch the argument by exhibiting a configuration that fails CS3 alone and showing that Q is *ill-typed* (no well-formed instance of Q exists to evaluate), in contrast to the R-PRE(iv) sketch above, where Q was well-formed but unsatisfiable. *Pre-state.* Let V_S(d) = {[1, 1], ..., [1, 5]} as before; additionally suppose subspace 2 contains at least one V-position at depth 2 — say [2, 1] ∈ dom(M(d)) (S5 admits multi-subspace domains; the pre-state ASN-0036 invariants hold across both subspaces). Take the 3-cut sequence K = ([1, 2], [1, 5], [2, 1]). *R-PRE(i), (ii), (iv) hold or are vacuous; (iii) fails at CS3.* Clauses (i) and (ii) are immediate. CS1 (n = 3): satisfied. CS2 (strict tumbler ordering [1, 2] < [1, 5] < [2, 1]): satisfied under T1's lexicographic strict ordering on tumblers — [1, 2] < [1, 5] from agreement on the subspace coordinate (1 = 1) together with 2 < 5 on the depth-2 ordinal, and [1, 5] < [2, 1] because the subspace coordinate 1 < 2 dominates regardless of the depth-2 ordinals. CS4 (all depth 2): satisfied (each cut is at depth 2 by construction). CS3 (all share one subspace): *violated* — c₂ = [2, 1] is in subspace 2 while c₀, c₁ are in subspace 1. R-PRE(iv) is now ill-defined as stated (it quantifies over positions in *the* subspace S of the cuts, presupposing CS3); under the most natural reading where R-PRE(iv) is vacuous in the absence of CS3 (no single "subspace S of the cuts" exists), (iv) does not refute the configuration. (The Width positivity consequence is likewise vacuous: it derives w_α, w_β ≥ 1 from R-PRE(iii) ∧ R-PRE(iv), and without CS3 the region widths themselves are not well-typed — NAT-sub at depth 2 across subspaces does not yield a region width within either subspace.) The unique load-bearing failure is CS3. *Q is ill-typed.* The postcondition clause R-P1 references M(d)(c₁ + j) for 0 ≤ j < w_β with c₁ = [1, 5] and the iteration bound w_β ill-typed because c₂ = [2, 1] sits in a different subspace from c₁; the iteration range itself is undefined, so the rearrangement equation M'(d)(c₀ + j) = M(d)(c₁ + j) has no well-typed instances to evaluate. Equivalently, region β — denotationally the V-positions at depth 2 in subspace S with ord in [ord(c₁), ord(c₂)) — has no well-typed extent when its bounding cuts straddle subspaces (no single subspace S of the cuts exists, and the cross-subspace half-open interval [c₁, c₂) under T1 lex order does not correspond to a region width in either subspace). The frame-condition collision is also a type-level mismatch: R-FRAME-P(a) demands V-positions outside V_S(d) (where S = 1 is the subspace shared by c₀ and c₁) be pointwise preserved, so M'(d)([2, 1]) = M(d)([2, 1]); but c₂ = [2, 1] is simultaneously named as a cut bounding region β within the postcondition machinery — a structural use of [2, 1] as an arithmetic endpoint incompatible with treating subspace 2 as an inert frame. Neither machinery can be evaluated at the witness, because each appeals to objects (a region width, a frame partition) that the type system of the postcondition does not assign meaning to. *This is not a Q-failing counterexample in the R-PRE(iv) sense.* Where the R-PRE(iv) sketch exhibited a well-typed instance of Q (specifically the j = 2 equation M'(d)([1, 4]) = M(d)([1, 6])) that no candidate M'(d) can satisfy, no such well-typed instance exists here. CS3 is therefore a *well-typedness guard*, not a semantic precondition: it ensures that the operational machinery R-P1/R-P2/R-S1/R-S2/R-S3 and the partition into regions α, μ, β have well-defined extents to begin with. *Why CS3 is load-bearing.* No ASN-0036 invariant is violated by the pre-state (the multi-subspace V-domain is admitted by S2 over each subspace and by S8 with per-subspace runs); CS3 alone is what isolates the cut sequence to a single subspace so that ordinal arithmetic at depth 2 is well-defined, the region widths w_α, w_β are well-typed, and R-FRAME-P/S(a) is consistent with the cut-induced region partition. Dropping CS3 therefore makes the postcondition unevaluable rather than unsatisfiable — a distinct, but equally load-bearing, mode of indispensability.
...
The explicit `v ∈ V_S(d)` guard on the (+) and (−) branches is the domain condition that keeps NAT-sub well-typed: on V_S(d), depth-2 ordinals are singleton tumblers (identified with positive naturals under the singleton-tumbler identification of ASN-0036), so the strict T1 ordering `ord(π(v)) > ord(v)` or `ord(π(v)) < ord(v)` reduces to strict ordering on positive naturals, and the corresponding `ord(π(v)) − ord(v)` and `ord(v) − ord(π(v))` are NAT-sub applications on naturals (its defined domain). The non-S convention `Δ(v) = (0, 0)` on `subspace(v) ≠ S` is consistent with the (S-exterior / fixed) clause because R-PPERM and R-SPERM both fix every non-S position (R-NS(NS-π)); the non-S clause is not needed for soundness but is stated separately to make explicit that the (+) and (−) branches do not even need to be considered when subspace(v) ≠ S — sidestepping any question of NAT-sub typing on multi-component ordinals. We write `+n` for `(+, n)`, `−n` for `(−, n)`, and `0` for `(0, 0)` when no ambiguity arises.

*Canonical form.* The only values produced by the case analysis are (0, 0), (+, n) with n ≥ 1, and (−, n) with n ≥ 1. The (+) branch fires only when ord(π(v)) > ord(v), so n = ord(π(v)) − ord(v) ≥ 1; the (−) branch fires only when ord(v) > ord(π(v)), so n = ord(v) − ord(π(v)) ≥ 1; the (0, 0) branch fires when subspace(v) ≠ S or when π(v) = v. The spurious carrier values (+, 0), (−, 0), and (0, n) with n ≥ 1 are never produced. We say Δ(v₁) = Δ(v₂) when their canonical forms are componentwise equal (same σ and same n); the canonical-form restriction makes equality unambiguous (no two distinct case-analysis outputs share a carrier representation).
...
By (b) — equivalently, by S8's uniqueness of the maximal-run decomposition (ASN-0036) — the partition into maximal runs is unique, so every termination state is the same partition regardless of merge order.
...
*S8(b) verification on reassembled runs:* ([1,3], E, 2): M'(d)([1,3]) = E, M'(d)([1,4]) = F = E + 1 ✓. ([1,5], B, 3): M'(d)([1,5]) = B, M'(d)([1,6]) = C = B + 1, M'(d)([1,7]) = D = B + 2 ✓. The width-1 runs satisfy S8(b) trivially.
...
Sorted by V-start: {([1,1], B, 1), ([1,2], A, 1)}. *S8(b) verification:* both runs are width 1, so S8(b) holds trivially at the lone offset k = 0 (M'(d)([1,1]) = B = B + 0; M'(d)([1,2]) = A = A + 0).
...
| PermutationDisplacement | DEF | Signed magnitude Δ(v) ∈ {+, −, 0} × ℕ recording the ordinal shift from v to π(v); (+)/(−) branches restricted to v ∈ V_S(d) so NAT-sub operates on single-component depth-2 ordinals, non-S positions fixed to (0, 0) by convention | introduced |
