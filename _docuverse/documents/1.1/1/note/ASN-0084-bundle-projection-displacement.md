...
*(NS-inv).* Two structural facts feed the transport: *(a) dom(M'(d)) = dom(M(d))*, asserted by the rearrangement postconditions, and *(b)* M'(d) and M(d) agree pointwise on dom(M(d)) \ V_S(d), by (NS-π). Any ASN-0036 invariant evaluated only on non-S positions is therefore preserved, since dom is unchanged by (a) and M'(d) = M(d) pointwise on non-S positions by (b). R-SP performs the per-clause invariant audit. ∎
...
*S0, S1, S4, S7 (StructuralAttribution) with its sub-clauses S7a, S7b, S7d (content store invariants).* Each is stated on Σ.C alone and transports by identity via C-transport (Invariant preservation). S7 is a theorem of ASN-0036 whose postconditions concern origin(a) = N(a).0.U(a).0.D(a); since origin(a) depends only on Σ.C and C' = C, origin is recomputed identically on M'(d), so S7's postconditions (well-definedness, distinct origins for distinct documents, invariance) are preserved.
