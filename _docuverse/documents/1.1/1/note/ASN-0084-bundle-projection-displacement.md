...
**Reduction of compound shifts (R-P2, R-S2, R-S3).** By Extended Associativity, the destination expressions `c₀ + w_β + j` (R-P2, R-S2) and `c₀ + w_β + w_μ + j` (R-S3) reduce to single-step shifts `(c₀ + w_β) + j` and `((c₀ + w_β) + w_μ) + j`. Each intermediate position retains subspace S by OrdShiftHom (a), so every shift is a valid OrdinalShift application on a subspace-S V-position.
