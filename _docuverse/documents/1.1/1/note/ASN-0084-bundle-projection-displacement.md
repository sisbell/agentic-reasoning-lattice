...
*Existence of a maximum (helper lemma).* For any non-empty S ⊆ {0, 1, ..., B} ⊆ ℕ with B ∈ ℕ, max(S) exists. *Proof.* The set T = {B − s : s ∈ S} is a non-empty subset of {0, 1, ..., B} ⊆ ℕ (each B − s is a natural number because s ≤ B, discharging NAT-sub's domain condition). By NAT-wellorder, T has a least element m ∈ ℕ. Then B − m ∈ S (its preimage under s ↦ B − s, which is involutive on {0, ..., B}); for every s ∈ S, m ≤ B − s gives s ≤ B − m, so B − m is an upper bound for S. Hence max(S) = B − m exists. We invoke this lemma once each below for f(v) and r(v); both invocations supply the bound B explicitly.
