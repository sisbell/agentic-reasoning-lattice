...
*A second counterexample (R-PRE(iii) — CS3 violation).* To exhibit that R-PRE(iv) is not the only non-redundant conjunct, we sketch a parallel construction in which R-PRE(iii) — specifically CS3, the same-subspace clause — is the unique R-PRE failure. *Pre-state.* Let V_S(d) = {[1, 1], ..., [1, 5]} as before; additionally suppose subspace 2 contains at least one V-position at depth 2 — say [2, 1] ∈ dom(M(d)) (S5 admits multi-subspace domains; the pre-state ASN-0036 invariants hold across both subspaces). Take the 3-cut sequence C = ([1, 2], [2, 1], [1, 5]). *R-PRE(i), (ii), (iv), (v) hold; (iii) fails at CS3.* Clauses (i) and (ii) are immediate. CS1 (n = 3): satisfied. CS2 (strict tumbler ordering [1, 2] < [2, 1] < [1, 5]): satisfied under T1's lexicographic strict ordering on tumblers, since the subspace coordinate dominates. CS4 (all depth 2): satisfied (each cut is at depth 2 by construction). CS3 (all share one subspace): *violated* — c₁ = [2, 1] is in subspace 2 while c₀, c₂ are in subspace 1. R-PRE(iv) is now ill-defined as stated (it quantifies over positions in *the* subspace S of the cuts, presupposing CS3); under the most natural reading where R-PRE(iv) is vacuous in the absence of CS3 (no single "subspace S of the cuts" exists), (iv) does not refute the configuration. R-PRE(v): the widths w_α = ord(c₁) − ord(c₀) and w_β = ord(c₂) − ord(c₁) are themselves ill-typed when c₀, c₁, c₂ span subspaces (NAT-sub at depth 2 requires the operands to share a depth-2 ordinal coordinate within a fixed subspace; ord is well-defined per cut but the differences across subspaces do not yield a region width within either subspace), so (v) cannot be evaluated; again, under a vacuous-by-presupposition reading, (v) does not refute the configuration. The unique load-bearing failure is CS3. *Q fails.* Region α would consist of "V-positions with ord in [ord(c₀), ord(c₁))" — but ord(c₀) and ord(c₁) lie in different subspaces, and the postcondition clause R-P1 references M(d)(c₁ + j) for 0 ≤ j < w_β with c₁ in subspace 2 and the destination position c₀ + j in subspace 1; the rearrangement equation M'(d)(c₀ + j) = M(d)(c₁ + j) then places subspace-2 content at a subspace-1 V-position, which on its face is consistent (M(d) ranges over I-addresses, not V-positions) but violates the frame condition R-FRAME-P(a) (V-positions outside V_S(d) — including those in subspace 2 — must be pointwise preserved). Specifically, R-FRAME-P(a) demands M'(d)([2, 1]) = M(d)([2, 1]), whereas the postcondition with C as given would treat [2, 1] = c₁ as a *source* position whose content moves to [1, 2] = c₀ (via R-P1 with j = 0), so M(d)([2, 1]) — the content at [2, 1] — appears as the new value at [1, 2] without [2, 1] itself being demoted to undefined: the dual postcondition assignments (one frame, one rearrangement) impose M'(d)([2, 1]) = M(d)([2, 1]) and simultaneously M'(d)([1, 2]) = M(d)([2, 1]), which is consistent for the source side but the destination side (R-P2 / R-P1) is contradictory once one traces through the symmetric clause for c₀: R-P1's symmetric application would also require [1, 2]-content to move to a position not in subspace 1, conflicting with R-FRAME-P(a) applied at [1, 2] (which is in V_S(d) for S = 1 and so *not* protected by R-FRAME-P(a), but the rearrangement equation degenerates because the source ordinal arithmetic c₁ + j has no consistent interpretation across subspaces). Q's S2 (functionality) clause — every position has a single I-address — is consequently violated at [1, 2] (two candidate values for M'(d)([1, 2]): the pre-state M(d)([1, 2]) by an extended R-FRAME interpretation, and M(d)([2, 1]) by R-P1) whenever the same-subspace presupposition CS3 fails. *Why CS3 is structural.* No ASN-0036 invariant is violated by the pre-state (the multi-subspace V-domain is admitted by S2 over each subspace and by S8 with per-subspace runs); the failure is purely CS3. CS3 isolates the cut sequence to a single subspace so that ordinal arithmetic at depth 2 is well-defined within that subspace and the rearrangement does not collide with R-FRAME-P(a)/R-FRAME-S(a) on the other subspaces.
