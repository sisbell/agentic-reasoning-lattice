...
**R-SP — RearrangeSufficientPrecondition (LEMMA).** This lemma establishes sufficiency only (the ⇐ direction): the conjunction below suffices for the post-state predicate Q. It does not characterize the weakest precondition; a full necessity characterization is left as an open question. The proof uses R-BLK (RunDecompositionTransformation), whose output supplies the S8 witness B', and R-COMM (PermutationShiftCommutativity), the within-region shift-commutativity of π.
