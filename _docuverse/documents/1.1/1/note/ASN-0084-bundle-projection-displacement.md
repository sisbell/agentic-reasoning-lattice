...
*Necessity sketch (R-PRE(iv) coverage).* Although a full weakest-precondition derivation is outside this ASN's scope, we exhibit one concrete pre-state demonstrating that R-PRE(iv) is not gratuitous: dropping the coverage conjunct produces a configuration on which Q fails. *Pre-state.* Let V_S(d) = {[1, 1], [1, 2], [1, 4], [1, 5]} (the position [1, 3] is *absent* from V_S(d) — D-CTG and D-SEQ of ASN-0036 forbid such a gap in practice; this counterexample is constructed precisely by dropping the coverage hypothesis that R-PRE(iv) supplies). Take the arrangement M(d)([1, 1]) = A, M(d)([1, 2]) = B, M(d)([1, 4]) = D, M(d)([1, 5]) = E (each I-address distinct, so no merge interactions confuse the analysis), and the 3-cut sequence C = ([1, 2], [1, 4], [1, 5]) — satisfying CS1–CS4 (n = 3; strictly ordered; subspace 1; depth 2). *R-PRE(i)–(iii), (v) hold; (iv) fails.* The affected range [c₀, c₂) at depth 2 in subspace 1 has ordinals {2, 3, 4}, but [1, 3] ∉ V_S(d); so the universal "(A v : subspace(v) = 1 ∧ #v = 2 ∧ c₀ ≤ v < c₂ : v ∈ V_S(d))" of R-PRE(iv) is falsified at v = [1, 3]. With w_α = ord(c₁) − ord(c₀) = 2 and w_β = ord(c₂) − ord(c₁) = 1, the postcondition clauses R-P1 and R-P2 still type-check syntactically. *Q fails.* R-P2 with j = 1 demands M'(d)(c₀ + w_β + 1) = M(d)(c₀ + 1), i.e., M'(d)([1, 3]) = M(d)([1, 3]); but [1, 3] ∉ dom(M(d)), so the right-hand side is undefined and M'(d)([1, 3]) cannot be assigned. Equivalently, the well-definedness step (b) of R-PIV fails: R-P2's reference to M(d)(c₀ + j) for j < w_α requires the source position to lie in dom(M(d)), and at j = 1 the source is [1, 3] ∉ dom(M(d)). This breaks S2 (arrangement functionality on the post-state) at u = [1, 3] — which lies in dom(M'(d)) = dom(M(d)) only if [1, 3] ∈ dom(M(d)) to begin with, but the analogous failure surfaces under any model that attempts to honour both R-P2 and dom(M'(d)) = dom(M(d)). Phase 1 of R-BLK exhibits the same failure mode: the cut c₁ = [1, 4] processes a partition in which the run ([1, 1], A, 2) and the run ([1, 4], D, 2) are separated by the gap at [1, 3], so the [c₀, c_{n−1}) interval is not covered by any union of runs, and the Phase-1 "Outside ⋃_k V(b_k)" clause would fire at c₁ — but c₁ is not the last cut, so the clause's justification (which appeals to c_{n−1} being the only admissible outside-cut, by CS2 and R-PRE(iv)) does not apply, and the Phase becomes ill-defined. *Conclusion.* R-PRE(iv) is load-bearing: dropping it admits pre-states on which R-P2 (and hence Q) cannot be satisfied. We do not enumerate the corresponding counterexamples for R-PRE(i), (ii), (iii), or (v); each conjunct guards a distinct construction step (M(d) well-definedness for the source references, V_S(d) ≠ ∅ for region non-emptiness, CS1–CS4 for cut-sequence well-formedness, w_α/w_β ≥ 1 for non-degenerate regions), and a similar pre-state can be constructed for each. The sketch above establishes the qualitative claim that R-PRE is non-redundant; a full necessity proof is left as the open question recorded above.
...
*Contiguity of reassembled runs.* Within each region, π applies a uniform ordinal displacement. After Phase 1, every run lies entirely in a single region, so for each run (vⱼ, aⱼ, nⱼ) and 0 ≤ k < nⱼ, positions vⱼ and vⱼ + k are in the same region: for subspace-S runs this is by Phase 1's split-at-cuts construction; for non-S runs this is by OrdShiftHom (b) of ASN-0036, which gives subspace(vⱼ + k) = subspace(vⱼ) ≠ S for all 1 ≤ k < nⱼ (and the identity convention gives subspace(vⱼ + 0) = subspace(vⱼ)). By R-COMM applied with the same-region precondition discharged (π(vⱼ + k) = π(vⱼ) + k), consecutive V-positions in the original run map to consecutive V-positions, so each reassembled run (π(vⱼ), aⱼ, nⱼ) occupies a contiguous V-position range and is therefore a valid run. For non-S runs π is the identity, so the reassembled run is identical to the original — contiguity is immediate.

The resulting runs satisfy S8(b) (consistency under M'(d)). *Subspace-S runs:* for each reassembled run (π(vⱼ), aⱼ, nⱼ) and 0 ≤ k < nⱼ: M'(d)(π(vⱼ) + k) = M'(d)(π(vⱼ + k)) = M(d)(vⱼ + k) = aⱼ + k. The second equality uses the permutation defining property M'(d)(π(v)) = M(d)(v); the first uses R-COMM. *Non-S runs:* the run (vⱼ, aⱼ, nⱼ) passes through unchanged, and for 0 ≤ k < nⱼ: M'(d)(vⱼ + k) = M(d)(vⱼ + k) = aⱼ + k. The first equality uses R-FRAME-P(a)/R-FRAME-S(a) at vⱼ + k (whose subspace is ≠ S by OrdShiftHom (b) of ASN-0036 as cited above); the second is the original S8(b).
