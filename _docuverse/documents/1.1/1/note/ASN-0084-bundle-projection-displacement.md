...
**Consequences of R-PRE.** *Subspace confinement.* All cuts lie in subspace S by CS3, and every V-position in the affected range [c₀, c_{n−1}) ∩ V_S(d) has subspace S by membership in V_S(d). Any cut-relative shift `c_i + j` retains subspace S: by CS3, subspace(c_i) = S, and OrdShiftHom (b) of ASN-0036 — extended to j = 0 by the identity convention, as recorded under Extended Associativity — gives subspace(c_i + j) = subspace(c_i) = S. The rearrangement constructions in this ASN (PivotPostcondition, SwapPostcondition) only assign new I-addresses to V-positions in V_S(d) and leave all other positions fixed (R-FRAME-P, R-FRAME-S), so no position outside subspace S is ever produced. This is a derived consequence of CS3 and OrdShiftHom (b) (ASN-0036), not a separate verification obligation.
...
The two forms are distinct primitives: the 3-cut pivot transposes two *adjacent* regions, while the 4-cut swap transposes two regions separated by at least one middle position (CS2 forces w_μ ≥ 1).


## Sufficient Precondition

**R-SP — RearrangeSufficientPrecondition (LEMMA).** This lemma establishes sufficiency only (the ⇐ direction): the conjunction below suffices for the post-state predicate Q. It does not characterize the weakest precondition; a full necessity characterization is left as an open question. The proof uses R-BLK (RunDecompositionTransformation), which describes how the pre-state runs transform under the rearrangement and exhibits *a* valid (non-maximal) run partition B', and R-COMM (PermutationShiftCommutativity), the within-region shift-commutativity of π. Post-state S8 itself is not discharged by B' — it follows from foundation S8, as shown below.

Let Q be the post-condition

> *M'(d) satisfies every ASN-0036 invariant carried by an arrangement transition — S0, S1, S2, S3, S4, S5, S7 (StructuralAttribution, ≡ S7a ∧ S7b ∧ S7d), S7a, S7b, S7d, D-CTG, D-CTG-depth, D-MIN, D-SEQ, S8a (VPositionWellFormedness), S8-fin, S8-depth, and S8 (CorrespondenceRunPartition) — the existence and uniqueness of the maximal correspondence-run decomposition of dom(M'(d)) under M'(d).*

Post-state S8 is met by foundation S8 (ASN-0036) directly: S8 of the foundation states that the *maximal* runs partition dom(Σ.M(d)) and that this maximal decomposition is *unique*, and the ASN preserves every precondition S8 requires of M'(d) — dom(M'(d)) = dom(M(d)) (so S8-fin, S8a, S8-depth carry over), S2 via bijectivity of π, and S3 via R-RI. Once those hold, foundation S8 applies to M'(d) and supplies the unique maximal partition for free; no construction is needed to witness it. R-BLK's B' is explicitly *not* maximal, so it is not the object S8 names — it serves only to make the run-level effect of the rearrangement explicit. The "S8" clause of Q is therefore the foundation guarantee, and the per-run verification below confirms that B' is *a* valid (non-maximal) correspondence-run partition consistent with M'(d), which the worked examples then exhibit concretely.

Then

`wp(REARRANGE_K, Q) ⇐ R-PRE(K) ∧ ASN-0036-invariants(Σ, d) ∧ (B is a correspondence-run partition of dom(M(d)) under M(d))`

i.e., R-PRE on the cut sequence K, the full ASN-0036 invariant suite on the pre-state, and a designated pre-state partition B together suffice to establish the invariant suite on the post-state. The pre-state partition B and the constructed B' are not needed for S8 (which the foundation supplies); they document how runs transform.
...
*Remark.* Post-state S8 is supplied by the foundation, not by B': once the rearrangement preserves dom(M(d)), S2, and S3, foundation S8 gives the unique maximal partition of M'(d) directly. R-BLK's role is descriptive — R-PRE supplies cut-coverage (so Phase 1 is well-defined) and non-empty regions (so Phases 2–3 do not degenerate); pre-state S8 supplies the initial partition B that Phases 1 and 3 transform; pre-state ASN-0036 invariants supply the foundation S0–S3, S7, D-CTG, D-MIN, S8-fin, S8-depth that the post-state inherits via the transport arguments above. The B' that R-BLK exhibits is in general *not* the canonical (maximal) partition of M'(d); recovering the maximal partition from B' by exhaustive merging is the source of post-rearrangement merges (R-BLK).
...
The partition B' is valid but not necessarily maximal: B' may contain V-adjacent, I-adjacent pairs of runs that satisfy the merge condition. The S8-unique maximal partition of M'(d) is guaranteed to exist by the foundation (S8, ASN-0036); recovering it from B' by exhaustive merging — and characterizing *which* pre-state run pairs produce post-state mergeability — is deferred to a future ASN, as noted under Canonical decomposition above. R-BLK claims only that B' is a valid correspondence-run partition of M'(d), not that it is the maximal one S8 names. The 4-cut worked example below exhibits a concrete instance of post-state mergeability (B and H merging into a width-3 run).
...
| R-SP | LEMMA | R-PRE(K) ∧ pre-state ASN-0036 invariants ∧ pre-state run partition B is sufficient for REARRANGE_K to establish ASN-0036 invariants on M'(d); post-state S8 follows from foundation S8 (preconditions preserved), with B' = R-BLK(B) exhibiting a valid non-maximal partition (sufficiency only; necessity not claimed) | introduced |
