...
**Reduction of compound shifts.** By Extended Associativity, the compound destinations `c₀ + w_β + j` and `c₀ + w_β + w_μ + j` are read left-associatively and equal the single-step shifts `(c₀ + w_β) + j` and `((c₀ + w_β) + w_μ) + j`.
...
This completes R-BLK: B' is a valid run partition of M'(d). The existence and uniqueness of the post-state maximal (S8-unique canonical) correspondence-run decomposition is the post-state S8 discharge recorded in the Invariant-preservation audit above. B' need not itself coincide with that maximal partition — the 4-cut worked example below exhibits a B' containing a mergeable pair (B and H, merging into a width-3 run).
