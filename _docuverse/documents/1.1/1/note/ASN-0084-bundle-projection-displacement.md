...
**OrdinalShift consumers under the identity extension.** Each ASN-0034 consumer of OrdinalShift used below has a stated domain restriction. We record here, once, which extend uniformly to n = 0 via the identity convention and which do not. Subsequent invocations cite these consumers without re-deriving the n = 0 branch.
...
(v) Every region is non-empty: w_α ≥ 1 and w_β ≥ 1 in both forms, and additionally w_μ ≥ 1 when n = 4. *Derivation note.* Clause (v) restates a derived consequence: CS2 (strict cut ordering) gives ord(c_{i+1}) > ord(c_i), so under the singleton-tumbler identification with ℕ⁺ each width ord(c_{i+1}) − ord(c_i) is a positive natural; combined with CS3, CS4, and R-PRE(iv) (placing the inter-cut positions in V_S(d)), this yields w_α ≥ 1, w_β ≥ 1, and (for n = 4) w_μ ≥ 1. The w_μ ≥ 1 derivation specifically — exhibiting c₁ ∈ [c₁, c₂) ∩ V_S(d) under R-PRE(iv), CS2, CS3, and CS4 — is recorded in full as the "Middle region non-empty (n = 4 only)" paragraph of *Consequences of R-PRE* immediately below; consumers of the w_μ ≥ 1 bound (R-SWP, R-DISP, R-BLK) cite either R-PRE(v) directly or "Consequences of R-PRE, n = 4 case" interchangeably. Clause (v) is retained as an explicit precondition — listing all three width bounds at the checklist — to keep the R-PRE checklist self-contained for invocations of REARRANGE.

Clause (iv) ensures that the affected range is covered: no gap exists within [c₀, c_{n−1}). Combined with D-CTG, this says the entire inter-cut range consists of valid V-positions in V_S(d). Clause (v) excludes degenerate cases where any region is empty — both transposed regions for n = 3, and additionally the middle region μ for n = 4.
...
*Middle region non-empty (n = 4 only).* For n = 4, we exhibit c₁ itself as a witness for [c₁, c₂) ∩ V_S(d). By CS3, subspace(c₁) = S; by CS4, #c₁ = 2 (depth-2 in subspace S, i.e., a V-position-shaped address). CS2 gives c₁ < c₂ < c₃, so identifying c₃ = c_{n−1} for n = 4 yields c₀ < c₁ < c_{n−1}, placing c₁ in the half-open range [c₀, c_{n−1}). By R-PRE(iv) (every position in [c₀, c_{n−1}) at depth 2 in subspace S lies in V_S(d)), c₁ ∈ V_S(d). Trivially c₁ ∈ [c₁, c₂) (CS2 gives c₁ < c₂). Hence c₁ ∈ [c₁, c₂) ∩ V_S(d), so [c₁, c₂) ∩ V_S(d) is non-empty and w_μ ≥ 1. This consequence is used uniformly by R-SWP, R-DISP, and R-BLK without re-derivation.
...
**Operation — REARRANGE_C.** REARRANGE_C(Σ, d) is the state transition Σ → Σ' that produces Σ' satisfying PivotPostcondition (when n = 3) or SwapPostcondition (when n = 4) together with the corresponding frame conditions R-FRAME-P (n = 3) or R-FRAME-S (n = 4). The two cases of n are mutually exclusive (CS1) and exhaustive over admissible cut sequences, so REARRANGE_C is a single operation whose postcondition specializes by the cut count. *Partiality.* REARRANGE_C is a partial operation on the state space: it is defined exactly on those (Σ, d) for which R-PRE(C) holds against Σ.M(d); on inputs that violate R-PRE(C), REARRANGE_C is undefined, and this ASN makes no commitment about the resulting state — neither that the operation aborts, nor that it produces some particular Σ', nor that any postcondition is satisfied. Out-of-contract behaviour is the caller's obligation to avoid via R-PRE(C), not the specification's to characterize.

*Parameterization convention.* The subscript C on REARRANGE_C denotes *static* parameterization by a cut sequence: the cut sequence is fixed at the point where the operation is invoked, not threaded through the runtime call's argument list. The runtime arguments are (Σ, d) — the global state and the target document — while C is a compile-time selector that indexes the family of operations {REARRANGE_C : C is an admissible cut sequence}. Each member of this family is a distinct operation, with its own postcondition (PivotPostcondition or SwapPostcondition specialized to C) and its own precondition R-PRE(C). Equivalently, one may read REARRANGE_C(Σ, d) as a three-argument operation REARRANGE(Σ, d, C); the subscripted form is a notational convenience that makes the dependence on C explicit at the operation name and keeps the runtime signature uniform across cut sequences. We retain the subscripted form REARRANGE_C throughout this ASN.

With this convention, REARRANGE_C has precondition R-PRE(C) and runtime signature (Σ, d) ↦ Σ'. The intra-document arrangement Σ.M(d) is the only mutated component: Σ.C, all other documents' arrangements Σ.M(d') for d' ≠ d, and the within-d non-S subspace portion of Σ.M(d) are preserved by the frame conditions; dom(M'(d)) = dom(M(d)) is asserted by the postconditions. This is the operation referent invoked by `wp(REARRANGE_C, Q)` in R-WP below.

**Reduction of compound shifts (R-P2, R-S2, R-S3).** The destination expressions `c₀ + w_β + j` (R-P2 and R-S2) and `c₀ + w_β + w_μ + j` (R-S3) are read as iterated ordinal shifts that reduce to single-step shifts via Extended Associativity (recorded under "OrdinalShift consumers under the identity extension" above). Explicitly, for any j ∈ ℕ:

- `c₀ + w_β + j = (c₀ + w_β) + j` (R-P2 and R-S2), by Extended Associativity with the outer pair (w_β, j) ∈ ℕ × ℕ. The intermediate position `c₀ + w_β` has subspace S by OrdShiftHom (b) (and the identity convention when w_β = 0, although w_β ≥ 1 here by R-PRE(v)), so the subsequent shift by j is again a valid OrdinalShift application on a subspace-S V-position.
...
The identity convention extends each step to the j = 0, w_μ = 0, or w_β = 0 corner cases (although R-PRE(v) and the n = 4 derivation of w_μ ≥ 1 exclude the latter two from this ASN's scope). Every appearance of "by associativity" or "associativity of ordinal addition" below this point cites the Extended Associativity reductions of the present paragraph; we do not repeat the case analysis at each consumer.
...
REARRANGE_C affects only the subspace-S portion of M(d); positions in any other subspace pass through unchanged. We collect the consequences of this single structural fact into one lemma so that subsequent sections (R-PPERM, R-SPERM, R-COMM, R-BLK, R-WP) can cite it once rather than re-deriving the non-S case in each invariant argument.
...
*(NS-run) Non-S runs carry verbatim into B'.* For every run b = (v_b, a_b, n_b) ∈ B with subspace(v_b) = S' ≠ S, the same triple (v_b, a_b, n_b) appears unchanged in B' = R-BLK(B), with the post-state S8(b) consistency M'(d)(v_b + k) = a_b + k for 0 ≤ k < n_b inherited verbatim from the pre-state S8(b) consistency of b under M(d).
...
*Forward-reference note.* This lemma is stated here, immediately after the operation definition and before the bijection lemmas R-PPERM (3-cut) and R-SPERM (4-cut), so that subsequent sections (R-PPERM, R-SPERM, R-COMM, R-BLK, R-WP) cite a single non-S dispatch point rather than re-deriving the non-S case in each invariant argument. The proof below invokes two results whose formal statement appears later in the ASN: R-PPERM (PivotPermutation) and R-SPERM (SwapPermutation), specifically the non-S branches of their piecewise definitions, in which π is set to the identity on positions with subspace(v) ≠ S. The non-S branch of each piecewise definition is fixed by the operation's frame condition — R-FRAME-P(a) for n = 3 and R-FRAME-S(a) for n = 4 — and is independent of the subspace-S branches that R-PPERM and R-SPERM verify. The reader who prefers a strictly bottom-up presentation may read R-PPERM (The 3-Cut Pivot Permutation, below) and R-SPERM (The 4-Cut Swap Permutation, below) first, then return to R-NS; the proof of R-NS below proceeds as if those non-S branches were in hand.

*Proof.*

*(NS-π).* The non-S branch of the R-PPERM piecewise definition (and of R-SPERM) sets π(v) = v whenever subspace(v) ≠ S. The defining equation M'(d)(π(v)) = M(d)(v) under this branch reduces to M'(d)(v) = M(d)(v), which is exactly R-FRAME-P(a) for n = 3 or R-FRAME-S(a) for n = 4. Hence π fixes every non-S V-position pointwise, and M'(d) agrees with M(d) on every such position.

*(NS-run).* Let b = (v_b, a_b, n_b) ∈ B with subspace(v_b) = S' ≠ S. *V-extent confinement.* By OrdShiftHom (b) of ASN-0036, subspace(shift(v_b, k)) = subspace(v_b) = S' for every 1 ≤ k < n_b; the identity convention gives subspace(v_b + 0) = subspace(v_b) = S'. Hence every V-position v_b + k of b satisfies subspace(v_b + k) = S' ≠ S, so V(b) ⊆ dom(M(d)) \ V_S(d). *Cut separation.* By CS3, every cut position cᵢ has subspace(cᵢ) = S; since S' ≠ S, cᵢ ∉ V(b). Hence Phase 1 of R-BLK never splits b — no cut position falls in V(b), so neither the interior-of-a-run nor the boundary-of-a-run sub-case fires for b at any step. *Phases 2 and 3.* Phase 2 classifies b into the dedicated non-S region (V-extent in a subspace other than S). Phase 3 applies displacement zero to non-S runs (the non-S clause of Phase 3 explicitly preserves V-start, I-start, and width — equivalently, π acts as the identity on V(b) by (NS-π)). The resulting B' contains the triple (v_b, a_b, n_b) unchanged. *Post-state S8(b) consistency.* For 0 ≤ k < n_b: M'(d)(v_b + k) = M(d)(v_b + k) (by (NS-π) applied at v_b + k, which has subspace ≠ S as established above) = a_b + k (by S8(b) of b under M(d), supplied by B). Hence (v_b, a_b, n_b) ∈ B' satisfies S8(b) under M'(d).

*(NS-inv).* Two structural facts feed the transport. *(a) dom(M'(d)) = dom(M(d))*, asserted by the rearrangement postconditions (R-EXT plus the R-P1/R-P2 or R-S1/R-S2/R-S3 image equalities cover V_S(d); R-FRAME-P(a)/R-FRAME-S(a) covers the rest). *(b)* M'(d) and M(d) agree pointwise on dom(M(d)) \ V_S(d), by (NS-π). Any ASN-0036 invariant whose evaluation at a non-S V-position depends only on (i) which non-S V-positions are in dom and (ii) the values of M restricted to non-S V-positions is therefore preserved unchanged. *Catalogue (non-S-applicable invariants).* S8a (VPositionWellFormedness) — applicable at any V-position regardless of subspace, requiring positive ordinal — depends only on (i) and is preserved by (a) at every non-S V-position. The per-subspace clauses of S8-fin and S8-depth applicable to non-S subspaces S' ≠ S (each constraining only dom restricted to a single subspace) likewise depend only on (i) and are preserved by (a). S2 (arrangement functionality) at non-S positions depends on (i) and the agreement of M and M', preserved by (a) and (b). *Out of scope for (NS-inv).* D-CTG, D-CTG-depth, D-MIN, D-SEQ are stated about V_S(d) specifically (subspace S = 1: contiguous depth structure of V_S(d), the minimum-depth text subspace, sequential ordinal labelling V_S(d) = {[S, 1], ..., [S, N]}), so they have no non-S evaluation and are not (NS-inv) consumers; their preservation under REARRANGE_C is discharged at the global dom-preservation step in R-WP (dom(M'(d)) = dom(M(d))), not via the non-S agreement above. S3 contributions from non-S V-positions reduce to {M'(d)(v) : subspace(v) ≠ S} = {M(d)(v) : subspace(v) ≠ S} by (b), so the ran(M'(d)) ⊆ dom(C') containment on the non-S portion follows from the pre-state's analogous containment on the non-S portion (with C' = C by R-FRAME-P(c)/R-FRAME-S(c)). S8(a) on non-S runs reduces to the pre-state's uniqueness-of-containing-run on non-S V-positions via (NS-run): the carried-over runs are pairwise disjoint and cover dom(M(d)) \ V_S(d) by the pre-state partition property, and (NS-run) preserves this verbatim into B'. S8(b) on non-S runs is the consistency clause established in (NS-run). The S8 corollary (preservation of subspace_I, zeros, and #E across runs in B') on non-S runs follows because (NS-run) preserves I-starts and ShiftPreservation (ASN-0036) applies. The content-store invariants (S0, S1, S4, S5, S7a, S7b, S7c, S7d, S9) are independent of M and so independent of the non-S/subspace-S distinction; they appear here only for completeness — REARRANGE_C preserves them by C' = C. ∎

*Citation convention.* Throughout the remainder of this ASN, when an invariant proof, a postcondition discharge, or a permutation verification splits into "subspace-S" and "non-S" cases, the non-S case is dispatched by citing R-NS (with the relevant clause (NS-π), (NS-run), or (NS-inv)) without re-derivation. The non-S branches in the R-PPERM and R-SPERM piecewise definitions, the R-FRAME-P(a)/R-FRAME-S(a) frame clauses, the non-S branch in Phase 3 of R-BLK, and the scope-restriction conventions throughout the canonical-decomposition argument are all consequences of R-NS and may be read as instances of it. The relevant clauses below cite R-NS explicitly at each consumer.
...
The R-P1 ordinal range is [p, p + w_β). The R-P2 ordinal range is [p + w_β, p + w_β + w_α). Both ranges are non-empty (since w_β ≥ 1 and w_α ≥ 1, R-PRE(v)), and they are disjoint because [p, p + w_β) ∩ [p + w_β, p + w_β + w_α) = ∅ — the half-open intervals meet at the shared endpoint p + w_β, which is included only in the second range. Their union is [p, p + w_β + w_α) = [p, p + w_α + w_β). And p + w_α + w_β is the ordinal of c₂ (since |[c₀, c₂)| = w_α + w_β, and by R-PRE(iv) the ordinals in [c₀, c₂) lie consecutively in V_S(d)). So the union of R-P1 and R-P2 covers exactly [c₀, c₂) ∩ V_S(d). Together with R-EXT (covering V_S(d) \ [c₀, c₂)), every position is covered exactly once.
...
Pairwise disjointness: the four ordinal ranges are [p, p + w_β), [p + w_β, p + w_β + w_μ), [p + w_β + w_μ, p + w_β + w_μ + w_α), and the exterior. Since w_β ≥ 1 and w_α ≥ 1 (R-PRE(v)) and w_μ ≥ 1 (Consequences of R-PRE, n = 4 case), the half-open intervals are non-empty and their left endpoints are strictly increasing: p < p + w_β < p + w_β + w_μ < p + w_β + w_μ + w_α. Hence no two intervals overlap, and none overlaps with the exterior.
...
Injectivity: within each case, the mapping is injective (the exterior is the identity; the α case maps distinct j to distinct c₀ + w_β + j; the β case maps distinct j to distinct c₀ + j). Across cases: the four image sets — {v ∈ dom(M(d)) : subspace(v) ≠ S}, V_S(d) \ [c₀, c₂), {c₀ + w_β + j : 0 ≤ j < w_α}, {c₀ + j : 0 ≤ j < w_β} — are pairwise disjoint (the first is disjoint from the rest by subspace separation; the remaining three are pairwise disjoint as shown in R-PIV). Surjectivity: π is an injection from dom(M(d)) into itself, and dom(M(d)) is finite (S8-fin of ASN-0036, combined with the finiteness of the non-S portion which inherits from dom(M(d))'s overall finiteness under S8-fin applied across subspaces); on a finite set, every self-injection is a bijection, so π is surjective. Equivalently — and exhibited explicitly for the reader — the three subspace-S image sets {c₀ + j : 0 ≤ j < w_β}, {c₀ + w_β + j : 0 ≤ j < w_α}, and V_S(d) \ [c₀, c₂) have ordinal extents [ord(c₀), ord(c₀) + w_β), [ord(c₀) + w_β, ord(c₀) + w_β + w_α), and V_S(d) \ [ord(c₀), ord(c₂)); their union is V_S(d) because ord(c₀) + w_β + w_α = ord(c₂) (by R-PRE(iv) and D-SEQ, the ordinals in [c₀, c₂) lie consecutively in V_S(d)). Together with the identity on non-S positions, this covers dom(M(d)). ∎
...
Injectivity: within each case, the mapping is injective (the exterior is the identity; the α case maps distinct j to distinct c₀ + w_β + w_μ + j; the μ case maps distinct j to distinct c₀ + w_β + j; the β case maps distinct j to distinct c₀ + j). Across cases: the five image sets — {v ∈ dom(M(d)) : subspace(v) ≠ S}, V_S(d) \ [c₀, c₃), {c₀ + w_β + w_μ + j : 0 ≤ j < w_α}, {c₀ + w_β + j : 0 ≤ j < w_μ}, {c₀ + j : 0 ≤ j < w_β} — are pairwise disjoint (the first is disjoint from the rest by subspace separation; the remaining four are pairwise disjoint as shown in R-SWP). Surjectivity: π is an injection from dom(M(d)) into itself, and dom(M(d)) is finite (S8-fin of ASN-0036); on a finite set, every self-injection is a bijection, so π is surjective. Equivalently — and exhibited explicitly for the reader — the four subspace-S image sets {c₀ + j : 0 ≤ j < w_β}, {c₀ + w_β + j : 0 ≤ j < w_μ}, {c₀ + w_β + w_μ + j : 0 ≤ j < w_α}, and V_S(d) \ [c₀, c₃) have ordinal extents [ord(c₀), ord(c₀) + w_β), [ord(c₀) + w_β, ord(c₀) + w_β + w_μ), [ord(c₀) + w_β + w_μ, ord(c₀) + w_β + w_μ + w_α), and V_S(d) \ [ord(c₀), ord(c₃)); their union is V_S(d) because ord(c₀) + w_β + w_μ + w_α = ord(c₃) (by R-PRE(iv) and D-SEQ, the ordinals in [c₀, c₃) lie consecutively in V_S(d)). Together with the identity on non-S positions, this covers dom(M(d)). ∎
...
**R-WP — RearrangeSufficientPrecondition (LEMMA).** *Label convention.* The label R-WP is retained as a stable identifier. The lemma establishes sufficiency only (one direction, ⇐): the conjunction below is sufficient for the post-state predicate, not necessarily equivalent to the weakest precondition. In R-WP and its proof, *S8a* (without parenthesized clause) denotes ASN-0036's S8a (VPositionWellFormedness); *S8(a)* and *S8(b)* (with parenthesized clauses) denote clauses (a) and (b) of ASN-0036's S8 (SpanDecomposition).

*Forward-reference note.* This lemma is stated here, immediately after the postcondition definitions, so that the reader sees the discharge obligation for the rearrangement contract before the supporting machinery is introduced. The proof below invokes two results that appear later in the ASN: R-BLK (RunDecompositionTransformation), the constructive transformation of correspondence-run partitions whose output supplies the S8 witness B'; and R-COMM (PermutationShiftCommutativity), the within-region shift-commutativity property of π. Both are proved independently of R-WP — R-COMM appeals only to the explicit R-PPERM/R-SPERM formulas together with associativity, and R-BLK assembles its three Phases from Split, Merge, and the maximal-run argument — and neither depends on Q. The reader who prefers a strictly bottom-up presentation may read R-COMM (Permutation Shift Commutativity, below), the canonical-decomposition steps (a)–(d), and R-BLK (Run Decomposition Transformation) first, and return to R-WP afterwards; the proof of R-WP below proceeds as if those results were in hand.

Let Q be the post-condition

> *M'(d) satisfies every ASN-0036 invariant carried by an arrangement transition — S0, S1, S2, S3, S4, S5, S7 (StructuralAttribution, ≡ S7a ∧ S7b ∧ S7c ∧ S7d, with sub-clauses listed explicitly below), S7a, S7b, S7c, S7d, S9, D-CTG, D-CTG-depth, D-MIN, D-SEQ, S8a (VPositionWellFormedness), S8-fin, S8-depth, and S8 (SpanDecomposition) with its clauses S8(a) (uniqueness of containing run) and S8(b) (consistency under M'(d)) — with the constructive witness B' = R-BLK(B) discharging the S8 existence clause: a correspondence-run partition of dom(M'(d)) under M'(d) obtained from the pre-state partition B of dom(M(d)) via Phases 1–3 on the V_S(d) portion (the runs in dom(M(d)) \ V_S(d), preserved pointwise by R-FRAME-P(a)/R-FRAME-S(a), carry over unchanged into B').*

Then

`wp(REARRANGE_C, Q) ⇐ R-PRE(C) ∧ ASN-0036-invariants(Σ, d) ∧ (B is a correspondence-run partition of dom(M(d)) under M(d))`
...
*S0, S1, S4, S7 (StructuralAttribution) with its sub-clauses S7a, S7b, S7c, S7d, S9 (content store invariants).* C' = C from the rearrangement definition transports each invariant verbatim: each is stated on Σ.C alone and is independent of M (S9 is preserved because the only mutation is to Σ.M(d), leaving Σ.C unchanged). S7 reduces to the conjunction S7a ∧ S7b ∧ S7c ∧ S7d on Σ.C, so its preservation follows from the preservation of its sub-clauses.
...
*Necessity sketch (R-PRE(iv) coverage).* Although a full weakest-precondition derivation is outside this ASN's scope, we exhibit one concrete pre-state demonstrating that R-PRE(iv) is not gratuitous: dropping the coverage conjunct admits a configuration on which Q fails *even when the pre-state satisfies every ASN-0036 invariant*. The counterexample isolates R-PRE(iv) as a constraint on the cut sequence relative to V_S(d), distinct from the structural invariants D-CTG and D-SEQ that the foundation already supplies. *Pre-state.* Let V_S(d) = {[1, 1], [1, 2], [1, 3], [1, 4], [1, 5]} — a sequential V-position set satisfying D-CTG (no gaps), D-SEQ (contiguous ordinals starting at 1), and S8a (positive ordinals at depth 2) of ASN-0036. Take any arrangement M(d) defined on these five positions (the specific I-addresses are irrelevant to the analysis below); the pre-state run partition B exists and is valid by S8. Take the 3-cut sequence C = ([1, 2], [1, 4], [1, 100]) — satisfying CS1 (n = 3), CS2 ([1, 2] < [1, 4] < [1, 100] under T1's strict ordering on tumblers), CS3 (all subspace 1), and CS4 (all depth 2). *R-PRE(i)–(iii), (v) hold; (iv) fails.* Clauses (i) and (ii) are immediate. Clause (iii) is just CS1–CS4, verified above. With w_α = ord(c₁) − ord(c₀) = 4 − 2 = 2 and w_β = ord(c₂) − ord(c₁) = 100 − 4 = 96 (each width derived from the cut ordinals alone, independent of V_S(d) membership), clause (v) holds: w_α ≥ 1 and w_β ≥ 1. But the affected range [c₀, c_{n−1}) at depth 2 in subspace 1 covers ordinals {2, 3, ..., 99}, and positions [1, 6], [1, 7], ..., [1, 99] are absent from V_S(d) (which extends only to [1, 5]); the universal "(A v : subspace(v) = 1 ∧ #v = 2 ∧ c₀ ≤ v < c₂ : v ∈ V_S(d))" of R-PRE(iv) is therefore falsified at any such witness — e.g., at v = [1, 6]. *Q fails.* R-P1 with j = 2 demands M'(d)(c₀ + 2) = M(d)(c₁ + 2), i.e., M'(d)([1, 4]) = M(d)([1, 6]). The left-hand side is well-typed ([1, 4] ∈ V_S(d) ⊆ dom(M(d)) = dom(M'(d))), but [1, 6] ∉ dom(M(d)), so the right-hand side is undefined and the postcondition equation cannot be satisfied for any candidate M'(d). Equivalently, the well-definedness step (b) of R-PIV fails: R-P1's reference to M(d)(c₁ + j) for 0 ≤ j < w_β requires the source position to lie in dom(M(d)), and at j = 2 the source is [1, 6] ∉ dom(M(d)). The failure compounds at j = 3, 4, ..., 95 — both as missing source references (M(d)([1, 7]), ..., M(d)([1, 99])) and, beyond j = 3, as missing destination V-positions (M'(d)([1, 6]), ..., M'(d)([1, 97]) lie outside dom(M'(d)) = dom(M(d))) — but a single witness at j = 2 already refutes Q. The R-BLK partition witness clause of Q inherits the failure: Phases 1–2 still complete (c₀ and c₁ classify against B by the interior/boundary dispatch on V_S(d); c₂ = [1, 100] falls outside ⋃_k V(b_k) and is admitted by the "Outside ⋃_k V(b_k)" clause as the last cut), but Phase 3 has no valid M'(d) to reassemble against, since M'(d) is the very object whose well-definedness R-P1 just disqualified. *Why R-PRE(iv) is structural.* No ASN-0036 invariant is violated by the pre-state above; the only failure is R-PRE(iv). This isolates R-PRE(iv) as a constraint *on the cut sequence relative to V_S(d)* — specifically bounding c_{n−1} ≤ [S, N + 1] where N = max{ord(v) : v ∈ V_S(d)} (under the D-SEQ characterization V_S(d) = {[S, 1], ..., [S, N]}), and analogously bounding each non-last cut into V_S(d) by CS2 — rather than as a redundant restatement of D-CTG or D-SEQ. *Conclusion.* R-PRE(iv) is load-bearing: even with a well-formed pre-state arrangement, dropping it admits cut sequences on which R-P1 (and hence Q) cannot be satisfied.

*A second counterexample (R-PRE(iii) — CS3 violation).* To exhibit that R-PRE(iv) is not the only non-redundant conjunct, we sketch a parallel construction in which R-PRE(iii) — specifically CS3, the same-subspace clause — is the unique R-PRE failure. The construction differs from the R-PRE(iv) sketch in character: CS3 is structurally upstream of R-PRE(iv) and R-PRE(v) (both presuppose a single "subspace S of the cuts"), so the witness exhibits Q's failure indirectly through ill-typing of the operational machinery rather than through a postcondition equation that evaluates to a contradiction on well-typed witnesses. *Pre-state.* Let V_S(d) = {[1, 1], ..., [1, 5]} as before; additionally suppose subspace 2 contains at least one V-position at depth 2 — say [2, 1] ∈ dom(M(d)) (S5 admits multi-subspace domains; the pre-state ASN-0036 invariants hold across both subspaces). Take the 3-cut sequence C = ([1, 2], [1, 5], [2, 1]). *R-PRE(i), (ii), (iv), (v) hold or are vacuous; (iii) fails at CS3.* Clauses (i) and (ii) are immediate. CS1 (n = 3): satisfied. CS2 (strict tumbler ordering [1, 2] < [1, 5] < [2, 1]): satisfied under T1's lexicographic strict ordering on tumblers — [1, 2] < [1, 5] from agreement on the subspace coordinate (1 = 1) together with 2 < 5 on the depth-2 ordinal, and [1, 5] < [2, 1] because the subspace coordinate 1 < 2 dominates regardless of the depth-2 ordinals. CS4 (all depth 2): satisfied (each cut is at depth 2 by construction). CS3 (all share one subspace): *violated* — c₂ = [2, 1] is in subspace 2 while c₀, c₁ are in subspace 1. R-PRE(iv) is now ill-defined as stated (it quantifies over positions in *the* subspace S of the cuts, presupposing CS3); under the most natural reading where R-PRE(iv) is vacuous in the absence of CS3 (no single "subspace S of the cuts" exists), (iv) does not refute the configuration. R-PRE(v): the width w_α = ord(c₁) − ord(c₀) = 5 − 2 = 3 is well-typed within subspace 1 and satisfies w_α ≥ 1, but w_β = ord(c₂) − ord(c₁) = ord([2, 1]) − ord([1, 5]) is ill-typed when c₁, c₂ span subspaces (NAT-sub at depth 2 requires the operands to share a depth-2 ordinal coordinate within a fixed subspace; ord is well-defined per cut but the difference across subspaces does not yield a region width within either subspace), so (v) cannot be evaluated as a whole; under a vacuous-by-presupposition reading, (v) does not refute the configuration. The unique load-bearing failure is CS3. *Q fails.* The postcondition clause R-P1 references M(d)(c₁ + j) for 0 ≤ j < w_β with c₁ = [1, 5] and the iteration bound w_β ill-typed because c₂ = [2, 1] sits in a different subspace from c₁; the iteration range itself is undefined, leaving the rearrangement equation M'(d)(c₀ + j) = M(d)(c₁ + j) without any well-typed instance to evaluate. Equivalently, region β — denotationally the V-positions at depth 2 in subspace S with ord in [ord(c₁), ord(c₂)) — has no well-typed extent when its bounding cuts straddle subspaces (no single subspace S of the cuts exists, and the cross-subspace half-open interval [c₁, c₂) under T1 lex order does not correspond to a region width in either subspace). The frame-condition collision compounds the failure: R-FRAME-P(a) demands V-positions outside V_S(d) (where S = 1 is the subspace shared by c₀ and c₁) be pointwise preserved, so M'(d)([2, 1]) = M(d)([2, 1]); but c₂ = [2, 1] is simultaneously named as a cut bounding region β within the postcondition machinery — a structural use of [2, 1] as an arithmetic endpoint that is incompatible with treating subspace 2 as an inert frame. Q is therefore unsatisfiable because no candidate M'(d) is admissible: the postcondition contract cannot be evaluated, so no value of M'(d) can witness it. *Why CS3 is structural.* No ASN-0036 invariant is violated by the pre-state (the multi-subspace V-domain is admitted by S2 over each subspace and by S8 with per-subspace runs); the failure is purely CS3. CS3 isolates the cut sequence to a single subspace so that ordinal arithmetic at depth 2 is well-defined within that subspace, the region widths w_α and w_β are well-typed, and the rearrangement source-extent does not collide with R-FRAME-P(a)/R-FRAME-S(a) on positions in other subspaces.

*R-PRE(v) is non-independent.* R-PRE(v) (region widths w_α, w_β, w_μ ≥ 1) is logically implied by R-PRE(iii): under CS3 and CS4 (cuts share subspace and depth 2), the singleton-tumbler identification of depth-2 ordinals reduces strict tumbler ordering to strict ordinal ordering, so CS2's strict ordering c_i < c_{i+1} forces ord(c_{i+1}) − ord(c_i) ≥ 1 — i.e., each region width is at least 1. Dropping R-PRE(v) while preserving R-PRE(iii) therefore leaves Q unaffected; R-PRE(v) is stated separately for emphasis (foregrounding region non-degeneracy as the salient structural condition Phase 2 of R-BLK exploits) rather than as an independent precondition conjunct. We do not construct a separate counterexample for R-PRE(v); any such construction would require violating R-PRE(iii) (specifically CS2), and the CS3-violation counterexample above already exhibits an R-PRE(iii) failure.
...
The sketches above establish the qualitative claim that R-PRE is non-redundant at its three load-bearing conjuncts (R-PRE(i), R-PRE(iii), R-PRE(iv)); a full necessity proof — characterizing the weakest precondition exhaustively — is left as the open question recorded above.
...
*Informal commentary (not used by downstream lemmas).* The 3-cut case exhibits a symmetry: the forward displacement of α equals the width of β and vice versa, with w_α positions shifting forward by w_β and w_β positions shifting backward by w_α. The 4-cut case is more subtle: forward α-displacement is w_β + w_μ, backward β-displacement is w_α + w_μ, equal only when w_α = w_β, with the middle absorbing the imbalance — w_μ middle positions shift by `|w_β − w_α|` in the direction of the wider region. One expects forward and backward magnitudes to balance because π is a bijection on a contiguous range; making this precise would require defining sums and products on the signed-magnitude carrier, which we deliberately do not do in this ASN. The downstream lemmas (R-DISP, R-PPERM, R-SPERM, R-BLK) depend only on per-region uniformity of Δ, not on any totalled-displacement identity.
...
*Non-S domain (both forms):* For v ∈ dom(M(d)) with subspace(v) ≠ S, R-NS(NS-π) gives π(v) = v, so ord(π(v)) = ord(v) and Δ(v) = 0 by the (0, 0) branch.
...
*Existence of a maximum (helper lemma).* For any non-empty S ⊆ {0, 1, ..., B} ⊆ ℕ with B ∈ ℕ, max(S) exists. *Proof.* The set T = {B − s : s ∈ S} is a non-empty subset of {0, 1, ..., B} ⊆ ℕ (each B − s is a natural number because s ≤ B, discharging NAT-sub's domain condition). By NAT-wellorder, T has a least element m ∈ ℕ. We first establish that s ↦ B − s is involutive on {0, ..., B}, i.e., B − (B − s) = s for every s ∈ {0, ..., B}. NAT-sub's right-inverse property (ASN-0034) gives (B − s) + s = B (the domain condition B ≥ s holds since s ∈ {0, ..., B}) and (B − (B − s)) + (B − s) = B (the domain condition B ≥ B − s holds because B − s ≤ B). NAT-sub's left-inverse property gives s + (B − s) = B. Equating these two expressions for B, (B − (B − s)) + (B − s) = B = s + (B − s); NAT-cancel's right-cancellation property (ASN-0034) then yields B − (B − s) = s. Then B − m ∈ S: since m ∈ T, m = B − s* for some s* ∈ S, and the involution gives B − m = B − (B − s*) = s*, so B − m = s* ∈ S. For every s ∈ S, we derive s ≤ B − m from m ≤ B − s in two NAT-sub steps. *Add s to both sides.* From m ≤ B − s, adding s preserves ≤, giving m + s ≤ (B − s) + s; NAT-sub's right-inverse property (ASN-0034) gives (B − s) + s = B (the domain condition B ≥ s holds since s ∈ {0, ..., B}), so m + s ≤ B. *Subtract m from both sides, by case split.* NAT-sub's left-inverse property gives m + (B − m) = B (the domain condition B ≥ m holds because m ∈ T ⊆ {0, ..., B}). Case A: m + s = B. Then m + s = B = m + (B − m), and NAT-cancel's left-cancellation property (ASN-0034) yields s = B − m. Case B: m + s < B. Then m + s < B = m + (B − m), and NAT-addcompat's left order-compatibility's contrapositive (strict-inequality form) yields s < B − m. In both cases s ≤ B − m, so B − m is an upper bound for S. Hence max(S) = B − m exists. We invoke this lemma once each below for f(v) and r(v); both invocations supply the bound B explicitly.
...
- *Exactly one of k₁, k₂ is zero (WLOG k₁ = 0, k₂ ≥ 1; the other ordering is symmetric).* By the identity convention, v₁ + 0 = v₁; by TS4 (StrictIncrease, ASN-0034), v₁ + k₂ = shift(v₁, k₂) > v₁ under T1's strict ordering on tumblers (ASN-0034). Substituting v₁ + k₂ = v₁ on the left-hand side of v₁ + k₂ > v₁ then yields v₁ > v₁, contradicting T1 irreflexivity (the strict ordering excludes equality, by the trichotomy/asymmetry built into a strict total order). So this sub-case cannot arise given w = v₁ + k₁ = v₁ + k₂.
- *Both k₁ = 0 and k₂ = 0.* Immediate equality k₁ = k₂ = 0.

In every admissible sub-case, k₁ = k₂. Hence a₁ + k₁ = M(d)(w) = a₂ + k₁. When k₁ = 0: a₁ = a₂ directly. When k₁ ≥ 1: shift(a₁, k₁) = shift(a₂, k₁); since shift(a₁, k₁) = shift(a₂, k₁) is one tumbler with one length, and OrdinalShift's `#shift(v, n) = #v` (ASN-0034) gives #shift(aᵢ, k₁) = #aᵢ, we have #a₁ = #a₂, so TS2 gives a₁ = a₂.

*n₁ = n₂:* Suppose n₂ > n₁. Then v₁ + n₁ ∈ V(b₂) (since n₁ < n₂ and v₁ = v₂), and S8(b) of b₂ gives M(d)(v₁ + n₁) = a₂ + n₁ = a₁ + n₁. The singleton run b₃ = (v₁ + n₁, a₁ + n₁, 1) — valid as a length-1 correspondence run since S8(b) holds trivially at offset 0 — is V-adjacent to b₁ (its V-start equals v₁ + n₁) and I-adjacent to b₁ (its I-start equals a₁ + n₁). Applying Merge to b₁ and b₃ yields the run (v₁, a₁, n₁ + 1), whose V-extent strictly contains V(b₁), contradicting b₁'s maximality. The case n₁ > n₂ follows by re-applying the argument just given with the labels b₁ and b₂ exchanged: the hypothesis "b₁ and b₂ are maximal runs sharing some w ∈ V(b₁) ∩ V(b₂), with v₁ = v₂ and a₁ = a₂ already established" is symmetric in the two labels (none of v₁ = v₂, a₁ = a₂, or w ∈ V(b₁) ∩ V(b₂) distinguishes b₁ from b₂), so the construction of a singleton extending the now-shorter run and the resulting maximality contradiction transfer verbatim — yielding a contradiction with b₂'s maximality. Therefore n₁ = n₂, and b₁ = b₂.
...
By (b), the partition into maximal runs is unique, so every termination state is the same partition regardless of merge order.
...
**R-BLK — RunDecompositionTransformation (LEMMA).** R-BLK names both the lemma below and the constructive transformation (B, C, M(d), M'(d)) ↦ B' it specifies; R-WP invokes the latter via R-BLK(B). Let B = {b₁, ..., bₘ} be a run partition of M(d) (per S8) — including runs whose V-extents lie in V_S(d) and runs whose V-extents lie in subspaces other than S. Let the cut sequence C have cut positions c₀, ..., c_{n−1}. The rearranged arrangement M'(d) admits a run partition B' obtained by:

*Scope note on non-S runs.* The non-S behaviour through all three Phases is supplied by R-NS(NS-run): for any run b ∈ B with V-extent in some subspace S' ≠ S, Phase 1 performs no split (cuts have subspace S by CS3, disjoint from S'), Phase 2 classifies b into the dedicated non-S region, and Phase 3 carries the triple (v_b, a_b, n_b) through unchanged into B'. The Phase clauses below state explicitly where this dispatch fires, but the substantive verification — V-extent confinement to S' by OrdShiftHom (b) of ASN-0036, post-state S8(b) consistency under M'(d) via R-NS(NS-π), and pairwise disjointness from subspace-S runs by T10 of ASN-0034 — is recorded once in R-NS and not repeated.
...
- Non-S runs: unchanged, by R-NS(NS-run).
...
The resulting runs satisfy S8(b) (consistency under M'(d)). *Subspace-S runs:* for each reassembled run (π(vⱼ), aⱼ, nⱼ) and 0 ≤ k < nⱼ: M'(d)(π(vⱼ) + k) = M'(d)(π(vⱼ + k)) = M(d)(vⱼ + k) = aⱼ + k. The second equality uses the permutation defining property M'(d)(π(v)) = M(d)(v); the first uses R-COMM. *Non-S runs:* discharged by R-NS(NS-run), whose post-state S8(b) clause supplies M'(d)(vⱼ + k) = aⱼ + k for 0 ≤ k < nⱼ directly.

Uniqueness of the containing run (S8(a)) for M'(d): π is a bijection on dom(M(d)) = dom(M'(d)), and π restricts to the identity on the non-S part of dom(M(d)) (R-NS(NS-π)) and to a bijection on V_S(d) (R-PPERM/R-SPERM). The V-extents of the reassembled subspace-S runs are pairwise disjoint and cover V_S(d) (from the partition property of the pre-reassembly subspace-S partition and bijectivity of π|_{V_S(d)}); the V-extents of the carried-over non-S runs are pairwise disjoint and cover dom(M(d)) \ V_S(d) (inherited from the pre-state partition, via R-NS(NS-run)). Pairwise disjointness across the two groups holds because subspace-S and non-S V-extents lie in distinct subspaces (T10 of ASN-0034: non-nesting prefixes generate disjoint subtrees). Together these yield the E! quantification of S8(a) on dom(M'(d)).
...
Sorted by V-start: {([1,1], A, 1), ([1,2], D, 1), ([1,3], B, 2), ([1,5], E, 1)}. *S8(b) verification on reassembled runs:* ([1,3], B, 2): M'(d)([1,3]) = B, M'(d)([1,4]) = C = B + 1 ✓. The width-1 runs ([1,1], A, 1), ([1,2], D, 1), ([1,5], E, 1) satisfy S8(b) trivially at their lone offset k = 0.
...
| REARRANGE_C | OPERATION | State transition Σ → Σ' parameterized by cut sequence C and document d; precondition R-PRE(C); postcondition PivotPostcondition (n=3) or SwapPostcondition (n=4) plus frame conditions R-FRAME-P or R-FRAME-S | introduced |
...
| R-NS | LEMMA | REARRANGE_C is the identity on dom(M(d)) \ V_S(d): π fixes non-S positions pointwise (NS-π), non-S runs carry verbatim into B' (NS-run), and ASN-0036 invariants on non-S positions transport trivially (NS-inv) | introduced |
