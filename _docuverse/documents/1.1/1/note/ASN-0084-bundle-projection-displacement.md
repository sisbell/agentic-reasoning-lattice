...
i.e., R-PRE on the cut sequence K and the full ASN-0036 invariant suite on the pre-state together suffice to establish the invariant suite on the post-state. No designated pre-state partition is required; post-state S8 follows from foundation S8.
...
*S8 (CorrespondenceRunPartition).* Discharged by foundation S8, whose preconditions (S8-fin, S2, S3, S8a, S8-depth) are preserved above — dom(M'(d)) = dom(M(d)) for S8-fin/S8a/S8-depth, S2 by bijectivity of π, S3 by R-RI. How the rearrangement acts on the runs of the decomposition is characterized separately by R-BLK; that characterization discharges nothing in Q, which asks only about the (maximal) decomposition of M'(d).
