...
**Extended Associativity.** For all j, k ∈ ℕ, `(c + j) + k = c + (j + k)`: the j, k ≥ 1 case is TS3 (ShiftComposition, ASN-0034), `shift(shift(v, n₁), n₂) = shift(v, n₁ + n₂)`, and the cases with j = 0 or k = 0 hold by the identity convention. We cite this identity below as *Extended Associativity*. The same identity convention extends TS2 (ShiftInjectivity) and OrdShiftHom (a) — `subspace(shift(v, n)) = subspace(v)` (ASN-0036) — to n = 0, since shift(v, 0) = v; TS5 (ShiftAmountMonotonicity) extends to a zero amount via TS4 (StrictIncrease), since shift(v, 0) = v differs from shift(v, n) > v for n ≥ 1. TS4 itself requires n ≥ 1: `shift(v, n) > v` fails at n = 0 under T1 irreflexivity.
...
i.e., R-PRE on the cut sequence K, the full ASN-0036 invariant suite on the pre-state, and a designated pre-state partition B together suffice to establish the invariant suite on the post-state.
