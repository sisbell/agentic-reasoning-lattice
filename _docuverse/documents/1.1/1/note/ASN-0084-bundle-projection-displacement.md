...
**Merge.** Two runs (v₁, a₁, n₁) and (v₂, a₂, n₂) under arrangement A are *mergeable* when v₂ = v₁ + n₁ (V-adjacent) and a₂ = a₁ + n₁ (I-adjacent). The merged run is (v₁, a₁, n₁ + n₂).

**Canonical decomposition.** The *canonical run decomposition* of an arrangement is S8's (CorrespondenceRunPartition, ASN-0036) unique maximal-run partition. We note that S8's maximality criterion and merge-non-extendability coincide: a forward lockstep extension of (v, a, n) to (v, a, n+1) requires v + n ∈ dom(M(d)) with M(d)(v + n) = a + n, which is precisely a V-adjacent, I-adjacent neighbor run (v + n, a + n, 1) available for merging, and symmetrically a backward lockstep predecessor u with shift(u, 1) = v, shift(M(d)(u), 1) = a is a V-adjacent, I-adjacent neighbor (u, M(d)(u), 1). Hence a run is non-extendable in S8's sense iff it admits no merge, and the two notions of *maximal* name the same partition. Its existence and uniqueness for M'(d) are discharged in the Invariant-preservation audit above.
...
**R-BLK — RunDecompositionTransformation (LEMMA).** Let B = {b₁, ..., bₘ} be a run partition of M(d) (per S8) — including runs whose V-extents lie in V_S(d) and runs whose V-extents lie in subspaces other than S. Let the cut sequence K have cut positions c₀, ..., c_{n−1}. The rearranged arrangement M'(d) admits a run partition B' obtained by:
...
- Non-S runs: carried verbatim, as established in Phase 1 (*Non-S runs are carried verbatim*).
...
The resulting runs satisfy S8-cons (consistency under M'(d)). *Subspace-S runs:* for each reassembled run (π(vⱼ), aⱼ, nⱼ) and 0 ≤ k < nⱼ: M'(d)(π(vⱼ) + k) = M'(d)(π(vⱼ + k)) = M(d)(vⱼ + k) = aⱼ + k. The second equality uses the permutation defining property M'(d)(π(v)) = M(d)(v); the first uses R-COMM. *Non-S runs:* S8-cons holds by Phase 1 (*Non-S runs are carried verbatim*).
