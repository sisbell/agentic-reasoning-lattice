...
*Middle region non-empty (n = 4 only).* For n = 4, CS2 forces c₁ < c₂, so [c₁, c₂) is non-empty as a set of ordinals; combined with R-PRE(iv) (every position in [c₀, c₃) ∩ V_S(d) is well-defined), [c₁, c₂) ∩ V_S(d) is non-empty, i.e., w_μ ≥ 1. This consequence is used uniformly by R-SWP, R-DISP, and R-BLK without re-derivation.
