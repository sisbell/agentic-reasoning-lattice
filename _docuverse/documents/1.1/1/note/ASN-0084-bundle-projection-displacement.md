...
*Empty-exterior boundary cases (EXT-VAC).* Two emptiness facts hold at the boundary of the cut sequence. (a) When ord(c₀) = 1, no V-position satisfies v < c₀ (V-position ordinals are ≥ 1 by S8a, ASN-0036), so the left-exterior set {v ∈ V_S(d) : v < c₀} is empty. (b) When c_{n−1} ∉ V_S(d), c_{n−1} ∉ dom(M(d)): by D-SEQ (ASN-0036), V_S(d) = {[S, 1], ..., [S, N]}, and any depth-2 subspace-S cut [S, q] with 1 ≤ q ≤ N would lie in V_S(d), so c_{n−1} ∉ V_S(d) lies strictly above the maximum [S, N]; the right-exterior set {v ∈ V_S(d) : v ≥ c_{n−1}} is therefore empty, and since every subspace-S depth-2 element of dom(M(d)) belongs to V_S(d), c_{n−1} ∉ dom(M(d)).
