...
*Remark (uniqueness scope).* As in R-PPERM, π is the unique bijection satisfying M'(d)(π(v)) = M(d)(v) on V_S(d) when M(d) is injective on V_S(d); under S5 sharing it is the canonical fibre-permutation representative, determined purely by the cut sequence and the regions α, μ, β.
...
*Proof.* REARRANGE_K is an arrangement rearrangement (established at the close of R-PPERM and R-SPERM: it preserves dom, preserves C, and admits the bijection π satisfying M'(d)(π(v)) = M(d)(v)). Every clause of Q except S8 is discharged generically by the *Invariant preservation* paragraph above. It remains to discharge S8.

*S8 (CorrespondenceRunPartition).* Discharged by the *Foundation-S8 transport* paragraph above. How the rearrangement acts on the runs of the decomposition is characterized separately by R-BLK; that characterization discharges nothing in Q, which asks only about the (maximal) decomposition of M'(d).
