...
*Proof.* The satisfaction predicate (F1) references only Σ.L(a), home(a), and Q. The function home(a) = origin(a) depends on the link address a alone (L2, ASN-0043) — not on any arrangement. The endset coverages coverage(Σ.L(a).eᵢ) depend on Σ.L alone — coverage is defined (ASN-0043) as a function of the span set in the endset, which is read from Σ.L(a).eᵢ; no other state component is consulted. The search constraints Sᵢ are provided as input. No component of the predicate references M(d) for any d. ∎
