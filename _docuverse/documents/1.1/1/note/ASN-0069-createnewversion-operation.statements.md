...
`V_{s_C}(d) = {v ∈ dom(M(d)) : subspace(v) = s_C}` — the content-subspace V-positions of `d`

---

## V0 — ForkOperation (DEF, operation)
...
*Effects.* When `V_{s_C}(d_op) ≠ ∅` (composite is K.δ + K.μ⁺ + K.ρ × n, where `n = |ran(M'(d_new))|`):
...
M'(d_new)(v) = M(d_op)(v)   for v ∈ V_{s_C}(d_op)
M'(d_new)(v) undefined       for v ∉ V_{s_C}(d_op)
(A d' : d' ≠ d_new : M'(d') = M(d'))
R' = R ∪ {(a, d_new) : a ∈ ran(M'(d_new))}
```

When `V_{s_C}(d_op) = ∅` (composite is K.δ alone, per V7): `C' = C`, `L' = L`, `E' = E ∪ {d_new}`, `M'(d_new) = ∅`, `M'(d') = M(d')` for `d' ≠ d_new`, `R' = R`.

---

## V1 — NewVersionIdentity (POST, ensures)

A fork of `d_src` produces a new entity `d_new` allocated as `A_v(d_src) = S(d_src, 1)`'s next emission `next(s.B, d_src, 1)`:
...
## V3a — AllocationInvariance (INV, invariant)
...
## V4b — DomainEquality (LEMMA, corollary)
...
V5 is V5a at `d* = d_src`: `d_src ∈ Σ.E_doc` by V0's precondition, and the fork composite has no step M-targeted at `d_src` (its only K.μ⁺ step targets `d_new ≠ d_src`).
...
*Derivation.* K.δ's effect on the newly created document is `M'(d_new) = ∅`. K.μ⁺ in J4's clause (ii) extends `M'(d_new)` only with positions drawn from `V_{s_C}(d_op)`, all of which have `subspace(v) = s_C`. No link-subspace V-position is added. K.ρ does not modify arrangements.

---

## V6a — LinkStoreAndSharedAddressesPersist (LEMMA, lemma)

For every link `a ∈ dom(Σ.L)`, after the fork composite `Σ →* Σ'`, `Σ'.L(a) = Σ.L(a)` — the link store is unchanged.

The fork composite decomposes into K.δ + K.μ⁺ + K.ρ × n (or K.δ alone in the empty-source case per V7), and each elementary transition frames `L' = L`. Composing across the constituent steps: `Σ'.L = Σ.L`.

---

## V7 — EmptySourceBehavior (DEF, case)
...
## V8 — PositionalCorrespondence (COROLLARY, lemma)

For every `v ∈ V_{s_C}(d_op)`: `v ∈ dom(M'(d_new))` and `M'(d_op)(v) = M'(d_new)(v)`.

The per-document frame gives `M'(d_op) = M(d_op)` (the K.μ⁺ phase targets only `d_new`, and `d_op ≠ d_new`), and V4 gives `v ∈ dom(M'(d_new))` with `M'(d_new)(v) = M(d_op)(v)`, so `M'(d_op)(v) = M(d_op)(v) = M'(d_new)(v)`.

---

## V8c — CorrespondenceSymmetricUntyped (COROLLARY, lemma)
...
## V8d — PerpetualCorrespondence (LEMMA, lemma)
...
The fork composite's K.ρ × n phase records one pair `(aⱼ, d_new)` per `aⱼ ∈ ran(M'(d_new))`, with cumulative effect `R' = R ∪ {(a, d_new) : a ∈ ran(M'(d_new))}`.

---

## V9a — ProvenanceRecordsContainment (OBS, note)
...
## V9b — FreshForksInheritExternalAddresses (LEMMA, lemma)
...
(a) *Distinct identities.* `d_new¹ ≠ d_new²`. By B8 (Uniqueness, ASN-0040), `d_new¹` is already in `E` at `Σ_g`, so the K.δ event placing `d_new²` is a strictly later baptism in the same namespace `S(d_src, 1)`, giving `d_new¹ ≠ d_new²` directly.

(b) *Independent content shares.* Each sibling fork inherits content from its own content source operand `d_op` at the moment of that fork. Their inherited V→I mappings live in separate arrangements `M¹(d_new¹)` and `M²(d_new²)`. Modifications M-targeted at `d_new¹` preserve `M(d_new²)`, and vice versa — V5a instantiated at `d* = d_new²` and at `d* = d_new¹` respectively.
...
## V10a — TimeSensitivityOfDerivation (OBS, note)
...
Let `Σ` denote the pre-state of the first fork. For every chain length `k ≥ 1` and every chain of forks `d_src → d¹_new → d²_new → ... → d^k_new` starting from `Σ` (with `d⁰_new := d_src`) satisfying two premises —
...
## V11a — AncestryComposition (COROLLARY, corollary)

The prefix relation chains: `d_src ≼ d¹_new ≼ d²_new ≼ ... ≼ d^k_new`. Each step `dⁱ⁻¹_new → dⁱ_new` is the *first* fork of its immediate source (V11's premise), so by V1 it is `inc(dⁱ⁻¹_new, 1)` — K.δ case (ii) at `k = 1` — extending the tumbler by exactly one component at position `#dⁱ⁻¹_new + 1` whose value is `1`. The full chain is recoverable from `d^k_new`'s tumbler alone by reading prefixes of strictly increasing length: the prefix of `d^k_new` of length `#d_src + i` is exactly `dⁱ_new` for every `0 ≤ i ≤ k` (with `d⁰_new := d_src`).
