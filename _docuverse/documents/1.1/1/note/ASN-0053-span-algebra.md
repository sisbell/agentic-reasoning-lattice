...
*Proof.* First we verify T12 for both constructed spans. For λ = (s, d) where d = p ⊖ s: since s < p and #s = #p, WF gives a well-formed level-uniform span with reach(λ) = p. For ρ = (p, d') where d' = reach(σ) ⊖ p: WF's start endpoint p is the given interior point, already in T, but its reach endpoint reach(σ) = start(σ) ⊕ width(σ) is a sum whose carrier-membership is not immediate. We discharge it as S11 does: σ is well-formed, so start(σ) ∈ T, width(σ) ∈ T, Pos(width(σ)), and actionPoint(width(σ)) ≤ #start(σ) hold — exactly TumblerAdd's preconditions at (a, w) = (start(σ), width(σ)) — whence TumblerAdd's carrier postcondition a ⊕ w ∈ T gives reach(σ) ∈ T. With p, reach(σ) ∈ T, p < reach(σ), and #p = #reach(σ) (level-uniformity gives #reach = #s = #p), WF gives a well-formed level-uniform span with reach(ρ) = reach(σ).
...
*Construction.* If n = 0, the result is the empty span-set ⟨⟩, which vacuously satisfies N1 and N2. For n ≥ 1, proceed as follows. Sort the component spans into non-decreasing order of start position; T1 totally orders tumblers, but distinct spans may share a start (SC cases (iv) and (v)), so any ties are broken arbitrarily; any resulting normalized equivalent satisfies N1/N2 with ⟦Σ̂⟧ = ⟦Σ⟧, which is all S8 requires. Seed the current interval [s, r) = [start(σ₁), reach(σ₁)) from the first span in sorted order, with the emitted set E = ∅. Then scan σ₂, ..., σₙ left to right. For each span σᵢ with i ≥ 2:
  ...
  — If start(σᵢ) > r (separated): emit the current interval as a span (s, r ⊖ s). WF's endpoints lie in the carrier T: the start s is the start of a component span, hence in T directly, while the reach r is the running maximum of the component reaches reach(σ₁), …, reach(σₙ), hence equal to one of them — and each component σ is well-formed, so TumblerAdd's preconditions at (a, w) = (start(σ), width(σ)) hold and its carrier postcondition a ⊕ w ∈ T gives reach(σ) = start(σ) ⊕ width(σ) ∈ T, placing r ∈ T. Level-uniformity and S6 ensure #s = #r, and s < r (the current interval was initialized from a non-empty span and is only extended by the merge step), so by WF the emitted pair (s, r ⊖ s) is a well-formed level-uniform span with reach r. Then start a new current interval at [start(σᵢ), reach(σᵢ)).

After processing all spans, emit the final interval — WF applies identically (its endpoints are placed in T by the same TumblerAdd discharge, and the final interval is non-empty because it was initialized from a non-empty span).
...
*Proof.* Before characterizing containment we must locate the reach tumblers in the carrier T, for the boundary argument tests reach(α) for membership in a subset of T and that test is well-defined only once reach(α) ∈ T is in hand. Each span σ ∈ {α, β} is well-formed, so start(σ) ∈ T, width(σ) ∈ T, Pos(width(σ)), and actionPoint(width(σ)) ≤ #start(σ) hold — these are exactly TumblerAdd's preconditions at (a, w) = (start(σ), width(σ)) — and TumblerAdd's carrier postcondition a ⊕ w ∈ T then yields reach(σ) = start(σ) ⊕ width(σ) ∈ T. Applied once at α and once at β, this places reach(α) ∈ T and reach(β) ∈ T, facts we draw on both in the boundary derivation that follows and in the ρ-construction below.

With these in hand we derive the boundary characterization of containment: ⟦β⟧ ⊆ ⟦α⟧ implies start(α) ≤ start(β) and reach(β) ≤ reach(α). For the start: start(β) ∈ ⟦β⟧ ⊆ ⟦α⟧ gives start(α) ≤ start(β) (and, since start(β) ∈ ⟦α⟧, also start(β) < reach(α)). For the reach: suppose for contradiction reach(β) > reach(α). Then start(β) < reach(α) < reach(β); since reach(α) ∈ T is in hand, all three conjuncts of ⟦β⟧ = {t ∈ T : start(β) ≤ t < reach(β)} are met at t = reach(α), so reach(α) ∈ ⟦β⟧ ⊆ ⟦α⟧, whence reach(α) < reach(α) — contradiction. Hence reach(β) ≤ reach(α). We now derive the decomposition by element-chasing. Let t ∈ ⟦α⟧, i.e., start(α) ≤ t < reach(α). The total order T1 splits this range into three sub-ranges relative to β's endpoints:
...
For the right interval, when reach(β) < reach(α), define ρ = (reach(β), reach(α) ⊖ reach(β)). Unlike λ's, ρ's endpoints reach(β) and reach(α) are not primitive start tumblers but the sums reach(σ) = start(σ) ⊕ width(σ), so WF's carrier preconditions reach(β) ∈ T and reach(α) ∈ T are not immediate; they are, however, exactly the carrier-membership facts placed at the outset of the proof, so reach(β) ∈ T and reach(α) ∈ T are already in hand. With #reach(β) = #reach(α) (level-uniformity and level-compatibility ensure all boundary tumblers share the same length) and reach(β) < reach(α), WF gives a well-formed level-uniform span with reach(ρ) = reach(α).
