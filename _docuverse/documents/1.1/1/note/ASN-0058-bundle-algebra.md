...
This is S8 (SpanDecomposition, ASN-0036) restated in our vocabulary, with explicit conjunct correspondence. S8(a) is the unique-existential `(A v ∈ dom(M(d)) :: (E! j :: v ∈ V(βⱼ)))`: its *existence* half — every V-position lies in *some* block's V-extent — is B1 (Coverage), and its *uniqueness* half — no V-position lies in two blocks' V-extents — is B2 (Disjointness). S8(b), asserting that each block's predicted I-addresses agree with `M(d)` pointwise, is B3 (Consistency). S8(a)'s range coincides with `⋃ⱼ V(βⱼ)` exactly when restricted to `dom(M(d))`: by S8a (VPositionWellFormedness, ASN-0036) every V-position satisfies `v₁ ≥ 1` unconditionally, and by S8-depth (ASN-0036) every V-position in a given subspace shares a common depth `m`, so the V-positions S8(a) ranges over are precisely the depth-`m` tumblers in each occupied subspace — exactly what `V(βⱼ)` enumerates when `βⱼ` covers part of that subspace. M2 inherits S8's preconditions verbatim; downstream claims that lean on M2 (notably C1a and C2) carry the same dependency. The question that S8 leaves open is: given that at least one decomposition exists, how many are there, and what relates them?
...
*No right-extension.* Suppose condition 3 fails: `v + n ∈ dom(f)` and `f(v + n) = a + n`. Some block `β' = (v', a', n') ∈ B` covers `v + n`, so `v + n = v' + j` for some `0 ≤ j < n'`. We show `j = 0`. Suppose for contradiction `j ≥ 1`. Since `v, v' ∈ dom(M(d))` (both are V-starts of blocks in `B`), S8a (ASN-0036) gives `#v ≥ 2` and `#v' ≥ 2`, discharging the depth preconditions of OrdShiftHom (ASN-0036). With `j ≥ 1` and `n ≥ 1` supplying the positive-shift preconditions, OrdShiftHom gives `subspace(v' + j) = subspace(v')` and `subspace(v + n) = subspace(v)`; since `v' + j = v + n`, `subspace(v') = subspace(v)`. By S8-depth (ASN-0036) applied to that subspace, `#v' = #v = m ≥ 2`. For depth of `v + (n − 1)`: if `n = 1`, then `v + (n − 1) = v + 0 = v` by OrdinalShiftBase, which has depth `m`; if `n ≥ 2`, then `n − 1 ≥ 1` and OrdShiftHom gives `v + (n − 1)` depth `m`. The same case split for `v' + (j − 1)` gives depth `m`. By M-aux, `v + n = (v + (n − 1)) + 1` and `v + n = v' + j = (v' + (j − 1)) + 1`. The unit shift `δ(1, m)` has action point `m`: applied to a depth-`m` tumbler `x`, TumblerAdd (ASN-0034) produces `[x₁, ..., x_{m−1}, x_m + 1]`. Equal outputs force equal inputs componentwise — agreement below `m`, plus `x_m + 1 = y_m + 1` forcing `x_m = y_m`. So `v + (n − 1) = v' + (j − 1)`. Since `v + (n − 1) ∈ V(β)` and `v' + (j − 1) ∈ V(β')`, `V(β) ∩ V(β') ≠ ∅`, contradicting B2. Hence `j = 0` and `v' = v + n`. Then `a' = f(v + n) = a + n`, so `β` and `β'` are V-adjacent (`v' = v + n`) and I-adjacent (`a' = a + n`) — contradicting `B` being maximally merged.

*No left-extension.* Suppose condition 2 fails: there exists `v'` with `v' + 1 = v`, `v' ∈ dom(f)`, and `f(v') + 1 = a`. Some block `β'' = (v'', a'', n'') ∈ B` covers `v'`, so `v' = v'' + k` for some `0 ≤ k < n''`. By M-aux, `v = v' + 1 = (v'' + k) + 1 = v'' + (k + 1)`. If `k + 1 < n''`, then `v'' + (k + 1) ∈ V(β'')`, so `v ∈ V(β'') ∩ V(β)`, contradicting B2. Hence `k + 1 = n''` (combined with `k < n''`), so `v' = v'' + (n'' − 1)`. By M-aux again, `v'' + n'' = (v'' + (n'' − 1)) + 1 = v' + 1 = v` (V-adjacent). And `a'' + n'' = (a'' + (n'' − 1)) + 1 = f(v') + 1 = a` (I-adjacent, since `f(v') = a'' + (n'' − 1)` by B3). So `β''` and `β` satisfy the merge condition — contradicting `B` being maximally merged. ∎

*Proof of M12.* (⟹) Let `B` be maximally merged. By M12b, every `β ∈ B` satisfies conditions 2 and 3 of being a maximal run; condition 1 is B3 applied to `β`. So every block of `B` is a maximal run. Since `B` covers `dom(f)` (B1) with disjoint blocks (B2), and the maximal runs partition `dom(f)` (M12a + the partition corollary), the two partitions coincide — so `B` is exactly the set of maximal runs.
...
| M12a | RunDisjointness: distinct maximal runs of `f = M(d)` have disjoint V-extents; with the trivial partition corollary, the set of maximal runs partitions `dom(f)` and is uniquely determined by `f` | introduced |
