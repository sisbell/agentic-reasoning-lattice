...
*Verification of (b).* `V(β_L) = {v + k : 0 ≤ k < c}` and `V(β_R) = {v + k : c ≤ k < n}`. The ranges `[0, c)` and `[c, n)` are disjoint, so `V(β_L) ∩ V(β_R) = ∅`. A pair in `⟦β_L⟧ ∩ ⟦β_R⟧` would have its first component in `V(β_L) ∩ V(β_R)`, which is empty. ∎
...
Both conditions are necessary. V-adjacency alone is insufficient: if the I-extents are not contiguous, the merged block `(v₁, a₁, n₁ + n₂)` would predict `M(d)(v₁ + n₁) = a₁ + n₁`, but the arrangement maps that position to `a₂ ≠ a₁ + n₁`, violating B3. I-adjacency alone is insufficient: if `v₂ > v₁ + n₁` (the V-extents leave a gap), the merged block claims a mapping at `v₁ + n₁ ∉ V(β₁) ∪ V(β₂)`, so either (a) `v₁ + n₁ ∉ dom(M(d))`, violating B3 for the merged block (which asserts the position is in its domain); or (b) some other block `β'' ∈ B` covers `v₁ + n₁`, in which case `V(β₁ ⊞ β₂) ∩ V(β'') ⊇ {v₁ + n₁}`, violating B2 between the merged block and `β''`. The case `v₂ < v₁ + n₁` (overlap) cannot occur when `β₁, β₂ ∈ B`. By OrdShiftHom (ASN-0036), `v₁ + n₁` shares subspace `V_{v₁}(d)` with `v₁`. For `v₂ < v₁ + n₁` to hold as a tumbler comparison, T1 at position 1 forces `(v₂)₁ = (v₁)₁` (a differing first component would place `v₂` outside the interval anchored at `v₁`); hence `v₂ ∈ V_{v₁}(d)`, and by S8-depth (ASN-0036) applied to that subspace, `v₁`, `v₂`, and `v₁ + n₁` are all at depth `m = depth(v₁)`. M-aux gives `v₁ + n₁` the same first `m − 1` components as `v₁`, and T1 then reduces the comparison `v₁ ≤ v₂ < v₁ + n₁` to component `m`: `(v₁)_m ≤ (v₂)_m < (v₁)_m + n₁`. So `v₂ = v₁ + k` for `k = (v₂)_m − (v₁)_m ∈ [0, n₁)`, placing `v₂ ∈ V(β₁)`. Combined with `v₂ ∈ V(β₂)`, this gives `v₂ ∈ V(β₁) ∩ V(β₂)`, violating B2 of the original decomposition.
...
(⟹) Let `B` be maximally merged. Take `β = (v, a, n) ∈ B` and suppose `β` is not a maximal run — say condition 3 fails: `v + n ∈ dom(f)` and `f(v + n) = a + n`. Some block `β' ∈ B` covers `v + n`. We claim `β'` starts at `v + n`. If `β'` starts at `v' < v + n`: since `v + n ∈ V(β')`, `v + n = v' + j` for some `0 ≤ j < n'`, with `j ≥ 1` (else `v' = v + n`, contradicting `v' < v + n`). By M-aux, `v + n = (v + (n − 1)) + 1` and `v + n = v' + j = (v' + (j − 1)) + 1`; the depth-`m` unit shift is injective (it increments only the `m`-th component), so `v + (n − 1) = v' + (j − 1) ∈ V(β')`. And `v + (n − 1) ∈ V(β)` (using `n ≥ 1`), so `v + (n − 1) ∈ V(β) ∩ V(β')`, contradicting B2 (disjointness). So `v' > v + n − 1`, hence `v' ≥ v + n` (TS4/TS5: `v + n` is the immediate successor of `v + n − 1` in the linear order, so there is no tumbler strictly between them). Since `v + n ∈ V(β')`, write `v + n = v' + j` for some `0 ≤ j < n'`; combined with `v' ≥ v + n`, this forces `j = 0` and `v' = v + n`. Then `β' = (v + n, a', n')` with `a' + 0 = f(v + n) = a + n`, so `a' = a + n`. Now `β` and `β'` are V-adjacent (`v + n = v + n`) and I-adjacent (`a + n = a + n`) — contradicting `B` being maximally merged.
