...
Both conditions are necessary. V-adjacency alone is insufficient: if the I-extents are not contiguous, the merged block `(v₁, a₁, n₁ + n₂)` would predict `M(d)(v₁ + n₁) = a₁ + n₁`, but the arrangement maps that position to `a₂ ≠ a₁ + n₁`, violating B3. I-adjacency alone is insufficient: if `v₂ > v₁ + n₁` (the V-extents leave a gap), the merged block claims a mapping at `v₁ + n₁ ∉ V(β₁) ∪ V(β₂)`, so either (a) `v₁ + n₁ ∉ dom(M(d))`, violating B3 for the merged block (which asserts the position is in its domain); or (b) some other block `β'' ∈ B` covers `v₁ + n₁`, in which case `V(β₁ ⊞ β₂) ∩ V(β'') ⊇ {v₁ + n₁}`, violating B2 between the merged block and `β''`. The case `v₂ < v₁ + n₁` (overlap) cannot occur when `β₁, β₂ ∈ B`. By OrdShiftHom (ASN-0036), `subspace(v₁ + n₁) = subspace(v₁)`. Let `m = depth(v₁)`; by S8-depth (ASN-0036) applied to `subspace(v₁)`, every V-position in that subspace has depth `m`, and M-aux gives `(v₁ + n₁)_j = (v₁)_j` for all `j < m`. We show first that `subspace(v₂) = subspace(v₁)`, by case-elimination on the divergence point of `v₂` from `v₁`. Suppose `J = {j : 1 ≤ j < m ∧ (v₂)_j ≠ (v₁)_j}` is non-empty, and let `j₀ = min(J)`. Then `(v₂)_i = (v₁)_i = (v₁ + n₁)_i` for all `i < j₀`. Since `v₁ ≤ v₂`, T1(i) (ASN-0034) gives `(v₂)_{j₀} > (v₁)_{j₀}`. Since `(v₁ + n₁)_{j₀} = (v₁)_{j₀} < (v₂)_{j₀}` and they agree below `j₀`, T1(i) gives `v₁ + n₁ < v₂`, contradicting `v₂ < v₁ + n₁`. Hence `J = ∅`, so `(v₂)_j = (v₁)_j` for all `1 ≤ j < m`; in particular `(v₂)_1 = (v₁)_1`, so `subspace(v₂) = subspace(v₁)` and by S8-depth `v₂` has depth `m`. T1 now reduces the comparison `v₁ ≤ v₂ < v₁ + n₁` to component `m`: `(v₁)_m ≤ (v₂)_m < (v₁)_m + n₁`. So `v₂ = v₁ + k` for `k = (v₂)_m − (v₁)_m ∈ [0, n₁)`, placing `v₂ ∈ V(β₁)`. Combined with `v₂ ∈ V(β₂)`, this gives `v₂ ∈ V(β₁) ∩ V(β₂)`, violating B2 of the original decomposition.
...
(⟹) Let `B` be maximally merged. Take `β = (v, a, n) ∈ B` and suppose `β` is not a maximal run — say condition 3 fails: `v + n ∈ dom(f)` and `f(v + n) = a + n`. Some block `β' = (v', a', n') ∈ B` covers `v + n`, so `v + n = v' + j` for some `0 ≤ j < n'`. We show `j = 0` (hence `v' = v + n`). Suppose for contradiction `j ≥ 1`. By OrdShiftHom (ASN-0036), `subspace(v' + j) = subspace(v')`, and separately `subspace(v + n) = subspace(v)`; since `v' + j = v + n`, this gives `subspace(v') = subspace(v)`. By S8-depth (ASN-0036) applied to that subspace, `depth(v') = depth(v) = m`, so `#v' = m`. By OrdShiftHom, `v + (n − 1)` and `v' + (j − 1)` are both depth-`m` tumblers (using `n ≥ 1` and `j ≥ 1`). By M-aux, `v + n = (v + (n − 1)) + 1` and `v + n = v' + j = (v' + (j − 1)) + 1`. The unit shift `δ(1, m)` has action point `m`: applied to a depth-`m` tumbler `x`, it produces `[x₁, ..., x_{m−1}, x_m + 1]` (TA0 preserves components below the action point, ASN-0034). Equal outputs force equal inputs componentwise — agreeing below `m` and with `x_m + 1 = y_m + 1` forcing `x_m = y_m`. So `v + (n − 1) = v' + (j − 1)`. Since `v + (n − 1) ∈ V(β)` and `v' + (j − 1) ∈ V(β')`, we have `V(β) ∩ V(β') ≠ ∅`, contradicting B2 (disjointness). Hence `j = 0` and `v' = v + n`. Then `β' = (v + n, a', n')` with `a' = f(v + n) = a + n`. Now `β` and `β'` are V-adjacent (`v + n = v + n`) and I-adjacent (`a + n = a + n`) — contradicting `B` being maximally merged.
