...
*Proof.* This is exactly the strict monotonicity established in M0's verification (the *M0 argument*): for `0 ≤ j < k < n`, `v + j < v + k` by TS4 (ShiftStrictIncrease, ASN-0034) at `j = 0` and TS5 (ShiftAmountMonotonicity, ASN-0034) for `j ≥ 1`. The identical argument with `a` in place of `v` gives `a + j < a + k`. ∎
...
*Partition claim ⟺ B1 ∧ B2 via V-extent translation.* S8's partition claim states that `dom(M(d))` is the disjoint union of the run V-extents `V(βⱼ) = {vⱼ + k : 0 ≤ k < nⱼ}`. To run the M-int reverse inclusion below over intervals, we first establish the *V-extent translation*: for each `j`, the interval `[vⱼ, shift(vⱼ, nⱼ))` intersected with `dom(M(d))` coincides with `V(βⱼ)`.

The forward inclusion `V(βⱼ) ⊆ [vⱼ, shift(vⱼ, nⱼ))` is direct. At `k = 0`, `vⱼ + 0 = vⱼ` by OrdinalShiftBase, and S8(a) at `k = 0` gives `M(d)(vⱼ) = aⱼ`, hence `vⱼ ∈ dom(M(d))`. For `1 ≤ k < nⱼ`, set `m = #vⱼ`; with `m ≥ 2` (by S8a) the action point of `δ(k, m)` is at index `m`, so by TumblerAdd's prefix-copy clause `vⱼ + k` agrees with `vⱼ` on components 1..m−1 and has `(vⱼ + k)_m = (vⱼ)_m + k`. T1(i) (ASN-0034) gives `vⱼ < vⱼ + k < shift(vⱼ, nⱼ)`. Membership of `vⱼ + k` in `dom(M(d))` for `k ≥ 1` follows from S8(a) at index `k`, which asserts `M(d)(vⱼ + k) = aⱼ + k`.
