...
*Vacuity of both report halves.* Let `Q0` abbreviate `DeletedFromAWithB(d_A, d_B) = ∅ ∧ DeletedFromBWithA(d_A, d_B) = ∅`. The wp derivation below invokes P4★, which ASN-0047 establishes as a composite-boundary property rather than a per-state invariant — so the derivation requires the pre-state `Σ` at which wp is evaluated to be reachable from `Σ_0` by a finite sequence of valid composite transitions (equivalently, a composite boundary). This reachability hypothesis is consistent with the operational scope of SHOWDELETIONS: by D-OBS, the operation is observational and modifies no state; by D-RECONS, it is a function of state alone; meaningful invocation occurs at the reachable, composite-boundary states the system actually inhabits. Under this hypothesis:
...
The split is exhaustive on the cross-product of `origin(t) ∈ {d, ≠ d}` and `#t` against `L_d + 3` (less than, equal, greater than): every `t ∈ dom(C)` falls into exactly one of the four cases enumerated below, and the cross-product is jointly exhaustive on these two axes.
