...
- *Different origin* (`origin(t) = d' ≠ d`). We split on the prefix relation between `d` and `d'`. In each sub-case we show that `t` (an emission of `A_C(d')`) cannot lie strictly between `a` and `a'` (consecutive emissions of `A_C(d)`).
