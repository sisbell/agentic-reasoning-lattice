...
> **R1 (MinimalDecrementNoStoreRetraction).** The section opener already settled the store side: no link-removal transition exists, so the existence count never falls and only the discovery count moves, through arrangement contraction. Consider the *minimal contraction*: a `K.μ⁻` step removing a single consulted entry `v ↦ a`, under three preconditions:
>
...
| R1 | No store-level link retraction exists; under (P-last)/(P-slot)/(P-sole), minimal contraction gives `Δnum_disc ∈ {−1,0}` | introduced |
