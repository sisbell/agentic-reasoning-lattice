...
*Derivation. K.μ~-FIX (ASN-0047) gives `dom(Σ'.M(d)) = dom(Σ.M(d))`, so the witness `π : dom(Σ.M(d)) → dom(Σ'.M(d))` is a bijection of `dom(Σ.M(d))` onto itself, satisfying the bijection equation `Σ'.M(d)(π(u)) = Σ.M(d)(u)` for every `u ∈ dom(Σ.M(d))`. Unfolding F-IMG at `Σ'` and reindexing each `v = π(u)`: since `π` ranges over all of `dom(Σ'.M(d))`, `v ∈ W ⟺ u ∈ π⁻¹(W)`, and `Σ'.M(d)(v) = Σ'.M(d)(π(u)) = Σ.M(d)(u)`, whence `image(W, d, Σ') = {Σ'.M(d)(v) : v ∈ W ∩ dom(Σ'.M(d))} = {Σ.M(d)(u) : u ∈ π⁻¹(W) ∩ dom(Σ.M(d))}`. That `π` need not fix `W` setwise is why the image membership can change. The cardinality, however, is not free to move under an arbitrary reorder: `π` is a bijection on `dom(Σ.M(d))`, so `|π⁻¹(W) ∩ dom(Σ.M(d))| = |W ∩ dom(Σ.M(d))|` always. When `Σ.M(d)` is injective, these equal-size index sets carry to equal-size images — the image can only change membership, never gain or lose. *Witness admissibility.* Each reorder witness below — injective, gain, loss, and four-position — is an admissible K.μ~ instance (ASN-0047): pin `vₖ = [1, k]` (`k = 1, …, 4`; each witness's domain an initial segment of these, the canonical shape available under D-SEQ★, ASN-0047), whereupon any permutation of these same-depth content-subspace positions is length-preserving (iii) and subspace-preserving (iv) automatically and link-subspace-fixing (v) vacuously (no `s_L`-position in the domain); K.μ~-FIX keeps the V-position domain fixed, so the shape invariants (i) — S8a, S8-depth, D-CTG★, D-MIN★, all properties of that unchanged domain — persist in the post-state; each witness's value assignment takes at least two distinct values (K.μ~'s precondition); and each witness's `π` moves some position's image — the non-trivial net effect (ii). *Injective witness:* with `Σ.M(d) : v₁ ↦ a, v₂ ↦ b` (injective, `a ≠ b`) and `W = {v₁}`, `image(W, d, Σ) = {a}`; the transposition reorder `π = (v₁ v₂)` yields `Σ'.M(d) : v₁ ↦ b, v₂ ↦ a` and `π⁻¹(W) = {v₂}`, so `image(W, d, Σ') = {b}` — the same cardinality, membership moved. A genuine cardinality change therefore requires `Σ.M(d)` non-injective, i.e. content sharing (M13/M14, ASN-0058). *Non-injective gain witness:* with `Σ.M(d) : v₁ ↦ a, v₂ ↦ a, v₃ ↦ b` (so `a` is shared) and `W = {v₁, v₂}`, `image(W, d, Σ) = {a}`; the reorder `π` given by `π(v₁) = v₁, π(v₂) = v₃, π(v₃) = v₂` yields `Σ'.M(d) : v₁ ↦ a, v₂ ↦ b, v₃ ↦ a`, and `π⁻¹(W) = {v₁, v₃}` gives `image(W, d, Σ') = {a, b}` — a gain from one member to two, with `ran(Σ'.M(d)) = ran(Σ.M(d)) = {a, b}` preserved throughout. *Non-injective loss witness:* with `Σ.M(d) : v₁ ↦ a, v₂ ↦ b, v₃ ↦ b` (so `b` is shared) and `W = {v₁, v₂}`, `image(W, d, Σ) = {a, b}`; the reorder `π` given by `π(v₁) = v₃, π(v₂) = v₁, π(v₃) = v₂` yields `Σ'.M(d) : v₁ ↦ b, v₂ ↦ b, v₃ ↦ a`, and `π⁻¹(W) = {v₂, v₃}` gives `image(W, d, Σ') = {b}` — a loss from two members to one (`{b} ⊊ {a, b}`), again with the total range preserved. *Availability taxonomy.* A moved image stands to its predecessor in one of two shapes: *containment* motion (`⊆`-comparable — strict, since equality is no move — so the cardinality changes) or *incomparable* motion (neither `⊆` nor `⊇`). Injectivity decides which shapes are available. Under injective `Σ.M(d)` only incomparable motion is realizable: the pinned cardinality makes a moved image take two distinct equal-size values, and distinct equal-size finite sets cannot nest (the image is finite: `image(W, d, Σ) ⊆ ran(Σ.M(d))` and `dom(Σ.M(d))` is finite by S8-fin, ASN-0036). Non-injectivity makes containment motion available — the gain and loss witnesses above — but does not force it; incomparable motion remains realizable as well: with `Σ.M(d) : v₁ ↦ a, v₂ ↦ b, v₃ ↦ c, v₄ ↦ a` (so `a` is shared by `v₁` and `v₄`) and `W = {v₁, v₂}`, `image(W, d, Σ) = {a, b}`, and the transposition `π = (v₂ v₃)` yields `Σ'.M(d) : v₁ ↦ a, v₂ ↦ c, v₃ ↦ b, v₄ ↦ a` with `image(W, d, Σ') = {a, c}` — `⊆`-incomparable with `{a, b}` despite the non-injective arrangement. Non-injectivity is thus necessary for containment motion but not sufficient to force one.*
...
A link matches the I-address set when *some* slot's coverage meets it. The existential over slots is essential: a multi-slot link that meets `I` in any one slot is matched.
...
The factoring is what makes the stability analysis tractable: each phase consults only one of `Σ.M(d)` and `Σ.L`, so the composite's stability decomposes accordingly.
...
A weaker per-link form is the residual that F-LAMBDA applies at each prior key:
...
- *K.μ~ on `d_q`*: the reorder holds `Σ.L` fixed (F-PRES/F-INERT), so every motion of the discovery set comes through the image, and F-IMG-SWING moves the image only when `W` is *not* fixed setwise by `π` — when `π⁻¹(W) ∩ dom(Σ.M(d_q)) = W ∩ dom(Σ.M(d_q))`, image and discovery set are both invariant. When the image *does* move, whether `findlinks_disc` inherits a monotone motion is decided not by the injectivity of `Σ.M(d_q)` but by which of F-IMG-SWING's two moved-image shapes obtains; injectivity governs only which shapes are *available* (F-IMG-SWING's availability taxonomy and its gain, loss, and four-position witnesses), not the monotonicity conclusion directly. *Containment image-motion* (`image(W, d_q, Σ) ⊆ image(W, d_q, Σ')` or the reverse; realizable only under content sharing, per the taxonomy): F-IMONO applies in that step — bridged through F-INERT, since K.μ~ preserves `Σ.L` — and `findlinks_disc` moves monotonically, exactly as in the K.μ⁺ and K.μ⁻ clauses. *Incomparable image-motion* (the only realizable move under injective `Σ.M(d_q)`, and realizable under non-injective arrangements as well, per the taxonomy): F-IMONO is unavailable — `findlinks` is monotone (F-IMONO) but not order-reflecting, so the discovery set's behavior is not inherited from the image and must be read off directly; non-injectivity alone therefore does not license the monotone conclusion. (Image-motion is in any case *necessary but not sufficient* for the discovery set to move: by F-MATCH's per-slot existential, a displaced in-region I-address relocates a link only when it was that link's *sole* in-region witness and the swapped-in address does not re-witness the same links.) Non-monotonicity is therefore established directly, by an incomparable discovery swing: the Worked illustration's reorder clause supplies an injective lateral swing at fixed cardinality — neither `⊆` nor `⊇`, with no link created or retracted — which alone refutes monotonicity, alongside a cardinality-changing swing confirming that discovery-set cardinality is not pinned even when image cardinality is.
