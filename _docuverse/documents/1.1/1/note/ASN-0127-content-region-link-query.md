...
*Derivation. Fix `a ∈ dom(Σ.L)` and unfold the match predicate at `I₁ ∪ I₂`: `matches(a, I₁ ∪ I₂, Σ) ≡ (E i : 1 ≤ i ≤ |Σ.L(a)| : coverage(Σ.L(a).eᵢ) ∩ (I₁ ∪ I₂) ≠ ∅)`. Intersection distributes over union — `coverage(Σ.L(a).eᵢ) ∩ (I₁ ∪ I₂) = (coverage(Σ.L(a).eᵢ) ∩ I₁) ∪ (coverage(Σ.L(a).eᵢ) ∩ I₂)` — and a union is non-empty iff one of its parts is, so the slot test becomes `coverage(Σ.L(a).eᵢ) ∩ I₁ ≠ ∅ ∨ coverage(Σ.L(a).eᵢ) ∩ I₂ ≠ ∅`. The existential distributes over this disjunction, giving `matches(a, I₁, Σ) ∨ matches(a, I₂, Σ)`. None of these steps consults `I₁ ∩ I₂`, so the law holds for arbitrary `I₁, I₂` — this is union-distribution of a set-valued operation, not a measure-style additive law over disjoint pieces. Set-builder over the disjunction splits the comprehension into `findlinks(I₁, Σ) ∪ findlinks(I₂, Σ)`.*
...
The meta-lemma below is the keystone of the *store-fixed* lane: its hypothesis is literal store equality `Σ.L = Σ'.L` — supplied by the `Σ.L`-preserving transitions (F-PRES) but failing on any path that admits K.λ, where the existence-anchoring claims rest instead on LP13's per-link value persistence (E-INV).
...
*F-PRES gives `Σ.L = Σ'.L`; F-CIL forces the equality. The transitive closure over `→*` whose every atomic step is in `V_atomic ∖ {K.λ}` follows by chaining the per-step equality — the single-step-to-closure lift that ASN-0098's Closure schema (★) names, here instantiated over the K.λ-free restriction of the step relation.*
