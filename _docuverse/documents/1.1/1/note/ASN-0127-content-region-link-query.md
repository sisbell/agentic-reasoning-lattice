...
- *K.μ~ on `d_q`*: the reorder holds `Σ.L` fixed (F-PRES/F-INERT), so every motion of the discovery set comes through the image (D-ABSORB), and F-IMG-SWING moves the image only when `W` is *not* fixed setwise by `π` — when `π⁻¹(W) ∩ dom(Σ.M(d_q)) = W ∩ dom(Σ.M(d_q))`, image and discovery set are both invariant. When the image *does* move, whether `findlinks_V` inherits a monotone motion is decided not by the injectivity of `Σ.M(d_q)` but by which of F-IMG-TAX's two moved-image shapes obtains; injectivity governs only which shapes are *available* (F-IMG-TAX), not the monotonicity conclusion directly. *Containment image-motion* (`image(W, d_q, Σ) ⊆ image(W, d_q, Σ')` or the reverse; realizable only under content sharing, F-IMG-TAX): F-IMONO applies in that step — bridged through F-INERT, since K.μ~ preserves `Σ.L` — and `findlinks_V` moves monotonically, exactly as in the K.μ⁺ and K.μ⁻ clauses. *Incomparable image-motion* (the only realizable move under injective `Σ.M(d_q)`, and realizable under non-injective arrangements as well, F-IMG-TAX): F-IMONO is unavailable — `findlinks` is monotone (F-IMONO) but not order-reflecting, so the discovery set's behavior is not inherited from the image and must be read off directly (and image-motion is in any case necessary but not sufficient for the discovery set to move, D-ABSORB). Non-monotonicity is therefore established directly, by an incomparable discovery swing: the Worked illustration's reorder clause supplies an injective lateral swing at fixed cardinality — neither `⊆` nor `⊇`, with no link created or retracted — which alone refutes monotonicity, alongside a cardinality-changing swing confirming that discovery-set cardinality is not pinned even when image cardinality is.
