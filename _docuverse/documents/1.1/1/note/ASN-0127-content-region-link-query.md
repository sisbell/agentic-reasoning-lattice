...
A weaker per-link form survives where F-CIL's global hypothesis fails — under K.λ, `dom(Σ'.L) = dom(Σ.L) ∪ {ℓ_new} ≠ dom(Σ.L)`, while each prior key `a ∈ dom(Σ.L)` keeps its value — and is the residual that F-LAMBDA applies at those prior keys:
