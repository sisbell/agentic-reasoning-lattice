...
- *K.μ~ on `d_q`*: the reorder holds `Σ.L` fixed (F-PRES/F-INERT), so every motion of the discovery set comes through the image, and F-IMG-SWING moves the image only when `W` is *not* fixed setwise by `π` — when `π⁻¹(W) ∩ dom(Σ.M(d_q)) = W ∩ dom(Σ.M(d_q))`, image and discovery set are both invariant. When the image *does* move, whether `findlinks_disc` inherits a monotone motion is decided not by the injectivity of `Σ.M(d_q)` but by whether the moved image is *⊆-comparable* (a containment) or *⊆-incomparable* (a lateral swing); injectivity governs only *which* of these is available, not the monotonicity conclusion directly. *Containment image-motion* (`image(W, d_q, Σ) ⊆ image(W, d_q, Σ')` or the reverse): F-IMONO applies in that step — bridged through F-INERT, since K.μ~ preserves `Σ.L` — and `findlinks_disc` moves monotonically, exactly as in the K.μ⁺ and K.μ⁻ clauses. A *strict* containment changes the image cardinality, so it requires `Σ.M(d_q)` non-injective (content sharing, M13/M14, ASN-0058): F-IMG-SWING's own non-injective witness grows the image (`{a} ⊊ {a, b}`), while `Σ.M(d_q) : v₁ ↦ a, v₂ ↦ b, v₃ ↦ b` with `W = {v₁, v₂}` under `π(v₁) = v₃, π(v₂) = v₁, π(v₃) = v₂` shrinks it (`{b} ⊊ {a, b}`); under injective `Σ.M(d_q)` this sub-case is vacuous, since F-IMG-SWING pins the cardinality and no strict containment is realizable. *Incomparable image-motion* (the moved image is neither `⊆` nor `⊇` its predecessor): F-IMONO is unavailable — `findlinks` is monotone (F-IMONO) but not order-reflecting, so the discovery set's behavior is not inherited from the image and must be read off directly. This sub-case is *not* confined to injective arrangements. Under injective `Σ.M(d_q)` it is the *only* realizable move: F-IMG-SWING pins the cardinality, so a moved image takes two *distinct equal-size* values, necessarily `⊆`-incomparable (distinct equal-size finite sets cannot nest). But non-injectivity *permits it as well* — with `Σ.M(d_q) : v₁ ↦ a, v₂ ↦ b, v₃ ↦ c, v₄ ↦ a` (so `a` is shared by `v₁` and `v₄`) and `W = {v₁, v₂}`, `image(W, d_q, Σ) = {a, b}`, and the transposition `π = (v₂ v₃)` yields `Σ'.M(d_q) : v₁ ↦ a, v₂ ↦ c, v₃ ↦ b, v₄ ↦ a` with `image(W, d_q, Σ') = {a, c}`, `⊆`-incomparable with `{a, b}` despite the non-injective arrangement. So non-injectivity is *necessary* for a containment move (a cardinality change demands it) but not *sufficient* to force one; it alone does not license the monotone conclusion, and an incomparable swing under a non-injective arrangement must be read off directly, just as under injectivity. (Image-motion is in any case *necessary but not sufficient* for the discovery set to move: by F-MATCH's per-slot existential, a displaced in-region I-address relocates a link only when it was that link's *sole* in-region witness and the swapped-in address does not re-witness the same links.) Non-monotonicity is therefore established directly, by an incomparable discovery swing: the Worked illustration's reorder clause supplies an injective lateral swing at fixed cardinality — neither `⊆` nor `⊇`, with no link created or retracted — which alone refutes monotonicity, alongside a cardinality-changing swing confirming that discovery-set cardinality is not pinned even when image cardinality is.
...
*Coverage of the endset shorthand.* Each singleton `{x}` here abbreviates the canonical unit-depth endset `{(x, δ(1, #x))}`, whose coverage is the entire subtree `coverage({x}) = {t ∈ T : x ≼ t} = subtree(x)` (PrefixSpanCoverage, ASN-0043) — never the bare singleton `{x}`, which no endset can denote. The slot reductions below rest on one structural premise: the generating addresses are pairwise prefix-incomparable. This holds because `a_1, a_2, a_3` are distinct content addresses of the same document `d`, hence siblings on `d`'s content chain `A_C(d)` (ChainMembershipForOrigin, ASN-0093) and pairwise prefix-incomparable (T10a.2, ASN-0034); and `a_θ` is a type address in a distinct subspace (`s_L`), so it diverges from every content address at or before the subspace component and is prefix-incomparable with each — a fortiori `a_θ ∉ {a_1, a_2, a_3}`. Under this premise each subtree meets the query I-set only at its own generating address: for any `I ⊆ {a_1, a_2, a_3}`, `coverage({a_i}) ∩ I = subtree(a_i) ∩ I = {a_i} ∩ I` (no other listed address lies under `a_i`), and `subtree(a_θ) ∩ {a_1, a_2, a_3} = ∅`.
