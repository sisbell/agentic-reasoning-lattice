...
*Non-weakestness.* PC is not necessary for the postcondition, so `P0 ∧ P1 ∧ PC` strictly over-constrains. Witness: a state Σ carrying a single nested link pair `(c, c'')`, `c ≼ c''`, both homed at a document `d_c ≠ home(a)`, with `a`'s prefix-subtree containing no link address other than `a`. The pair breaks R0a's antichain, so `¬PC`; yet `P0 ∧ P1` holds, and — choosing `d_retr` so that `a_emit(Σ, d_retr) ∉ {t : a ≼ t}` — the postcondition `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}` holds at Σ', because the nesting lies entirely outside `a`'s subtree. Hence the postcondition is reachable from a `¬PC` state, and the true weakest precondition is strictly weaker than `P0 ∧ P1 ∧ PC`. Concretely, the global PC can be replaced by the strictly weaker *local* pair `L(Σ) ≡ {t : a ≼ t} ∩ dom(Σ.L) = {a}` (a local antichain on `a`'s subtree, which already entails P1) together with the fresh-emitter exclusion `a_emit(Σ, d_retr) ∉ {t : a ≼ t}`. *Sufficiency of the local pair (given P0).* P0 makes Σ' well-defined with `dom(Σ'.L) = dom(Σ.L) ∪ {a_emit(Σ, d_retr)}` (L12a — the only new key is the single fresh emitter), so `{t : a ≼ t} ∩ A_rel^{Σ'} = ({t : a ≼ t} ∩ dom(Σ.L)) ∪ ({t : a ≼ t} ∩ {a_emit(Σ, d_retr)}) = {a} ∪ ∅ = {a}` — the first term by `L(Σ)`, the second by the fresh-emitter exclusion. *Relation to PC.* `L(Σ)` is implied by PC (PC's R0a antichain restricts to the local antichain on `a`'s subtree) but does not imply PC (the witness above satisfies `L(Σ)` while violating global R0a), confirming PC's non-weakestness. We do *not* claim the local pair is itself the weakest precondition: it demands `a ∈ dom(Σ.L)`, yet the self-emission boundary case `a = a_emit(Σ, d_retr)` (where `a ∉ dom(Σ.L)` but the postcondition can still hold) lies outside it, so a genuinely weakest precondition is broader still. The local pair is offered as a *strictly-weaker sufficient* precondition witnessing PC's non-weakestness; the exact weakest precondition for Case 1's postcondition is left open.
