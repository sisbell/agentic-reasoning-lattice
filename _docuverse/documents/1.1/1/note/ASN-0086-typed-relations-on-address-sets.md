...
*Value-shape postcondition (downstream hook).* R0 imposes exactly one content-dependent requirement on the caller-supplied value: L3-conformance of the triple `(F, G, K)` — arity 3, both content slots in `Endset`, non-empty type slot. Every other L-invariant at the fresh emitter key is discharged structurally by the single K.λ-step, independent of the value: the L/S/M/C catalog by RT-closure, and L14/L14a by FreshLinkKeyDisjointness.
...
*(Step 3 — Self-targeting emission via R0.)* Pick any `d ∈ dom(Σ.M)` and any `K ∈ T_admissible`. The triple `(∅, G_self, K)` is L3-conforming: arity 3, with `∅ ∈ Endset` (the empty endset) and `G_self ∈ Endset` (the singleton built in Step 2), and `K ∈ T_admissible` non-empty by assumption. Apply R0 at this L3-conforming triple and home `d`. By R0's value-shape postcondition, L3-conformance is R0's sole content-dependent requirement — met here by the conformance just verified — and every other L-invariant at the emitter key is discharged structurally. R0 therefore produces a fresh emitter `a' ∉ dom(Σ.L)` and `→*`-reachable post-state Σ' with `Σ'.L(a') = (∅, G_self, K)`. The self-reference is recorded at the substrate level: `a ∈ coverage(Σ'.L(a').e₂)` — the to-set case.
