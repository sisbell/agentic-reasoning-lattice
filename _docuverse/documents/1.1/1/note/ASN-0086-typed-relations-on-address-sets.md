...
**Definition — relational layer.** The relational layer's link-store operations are `{Emit_K, Observe_K, Nullify}`. Its one *discipline commitment* constrains only the type-`R` growth of `Σ.L`: every `L_R`-growing step is a `Nullify` — the layer never invokes `Emit_K` at a type index `K ~ R` except through the `Nullify` alias.
