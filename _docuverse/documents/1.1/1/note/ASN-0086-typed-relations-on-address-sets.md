...
*`A_rel^Σ` names the whole link store, not only the tuples.* `A_rel^Σ = dom(Σ.L)` collects *every* link address, whereas the standard-triple tuples of this note's relational vocabulary are exactly those with `|Σ.L(a)| = 3` (see Definition — TypedRelation, below).
