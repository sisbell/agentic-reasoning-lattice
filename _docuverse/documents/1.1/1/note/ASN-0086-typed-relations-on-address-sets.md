...
*Domain of quantification.* This analysis ranges over the state-local-conforming sub-space (Definition — Emit_K). PC is supplied at every layer-issued call by the relational layer's usage discipline — not by the operation's executable domain — as established at the *Single-tuple scope under R0a* paragraph of the Definition of Nullify.
...
*Result.* For this note's operation set `{Emit_K, Observe_K, Nullify}`, and over the substrate-conforming sub-space of pre-states Σ (the domain restriction stated below), the weakest precondition is

`wp(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) ≡ d ∈ dom(Σ.M) ∧ K ∈ T_admissible`   (over substrate-conforming Σ)

*Domain restriction.* This wp is asserted only over substrate-conforming pre-states Σ, matching Case 1's carrying of PC. The restriction is load-bearing, not cosmetic: the derivation's `a ∉ nullified(Σ')` step invokes R0a's antichain, which holds only at substrate-conforming states. Over the broader state-local-conforming domain of Emit_K (Definition — Emit_K, which admits antichain-violating non-conforming states), the two operation-preconditions do *not* suffice. Witness a state-local-conforming but non-substrate-conforming Σ of the kind the Definition — state-local-conforming state constructs — a nested link pair `b' ≼ ℓ_prev` at home `d`, with `b'` the target of a pre-existing unit-depth retraction. The subsequent-branch emission `a = a_emit(Σ, d) = inc(ℓ_prev, 0)` preserves positions `1..#ℓ_prev − 1`, so `b' ≼ a`, whence `a ∈ coverage({(b', δ(1, #b'))})` and `a ∈ nullified(Σ')`; then `(a, F, G) ∉ A_K^{Σ'}` though `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` both hold. The restriction excludes exactly these states.

*Derivation.* `d ∈ dom(Σ.M)` and `K ∈ T_admissible` are Emit_K's own preconditions, each load-bearing: dropping the first leaves K.λ's home-precondition undischarged, so no post-state Σ' is produced; dropping the second admits `K = ∅` (the sole non-admissible endset), violating K.λ's `e₃ ≠ ∅` precondition (L3), so again no Σ' exists. It remains to show `a ∉ nullified(Σ')`. The domain restriction supplies substrate-conformance of Σ, so R0a's antichain holds at Σ; the conformance-preserving K.λ `→`-step carries it to Σ' (Definition — substrate-conforming layer), so R0a's antichain on `dom(Σ'.L)` is available. Nullify emits only unit-depth to-spans `{(b, δ(1, #b))}` (Definition — relational layer), so every `L_R^Σ` tuple has coverage `{t : b ≼ t}` for some `b ∈ A_rel^Σ`; the fresh `a = a_emit(Σ, d)` is prefix-incomparable with every such `b` by K.λ's emission rule together with R0a, so `a ∉ coverage(G')` for any pre-existing retraction `(_, _, G') ∈ L_R^Σ`, and no pre-state retraction nullifies `a`. The Nullify-as-sole-`R`-producer rule keeps every direct `Emit_K` at `K ≁ R`, so the fresh tuple does not enter `L_R^{Σ'}` and cannot self-nullify. Hence no `L_R^{Σ'}` tuple — pre-existing or just-emitted — has to-coverage containing `a`, so `a ∉ nullified(Σ')`; with `(a, F, G) ∈ L_K^{Σ'}` unconditional, `(a, F, G) ∈ A_K^{Σ'}`.

*Scope of the result.* This wp is computed relative to this note's operation set `{Emit_K, Observe_K, Nullify}`; the collapse to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` relies on the unit-depth retraction discipline (every `L_R` to-span has the shape `{(b, δ(1, #b))}`) and the Nullify-as-sole-`R`-producer rule (`K ≁ R` at every direct `Emit_K`). A direct K.λ caller voids both disciplines, so the result does not extend to such callers.
...
| R6b | DEF-Consequence | SingleDepthRetraction — within-state claim: `a ∈ nullified(Σ)` holds even when its witnessing retractor `b ∈ nullified(Σ)` (see R6b statement and proof for the audit-vs-active mechanism) |
