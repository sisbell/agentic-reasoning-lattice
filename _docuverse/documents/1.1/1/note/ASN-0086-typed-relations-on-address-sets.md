...
- (c) **frontier-landing** — if a step adds a fresh key at home `d` whose homed-set occupied chain indices `0..J` before the step, that key occupies exactly chain index `J+1` of `A_L(d)` — no gap, no index skipped.

Clauses (b) and (c) together are the ASN-0093 chain discipline: (b) makes "the fresh key at `d`" singular, so (c)'s "occupies exactly chain index `J+1`" is unambiguous rather than open between several keys seeing the same pre-step frontier `J`. The nested emission `a'' = inc(a, 1)` of Remark — NestedLinkWitness satisfies (b) yet lands off the frontier, so it violates (c). Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a)–(c) by their ASN-0093 contracts (each K.λ adds at most one key per home, at the frontier).
...
*Proof.* The contiguous-prefix form follows from conformance clauses (b) and (c) (Definition — substrate-conforming state), by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)–(c)). The two clauses are jointly load-bearing: clause (c)'s frontier-landing requirement is framed per single fresh key (a key occupying chain index `J+1` past a pre-step frontier `J`), and clause (b) guarantees a single step deposits at most one fresh key per home — making "extends the prefix by one chain index" coherent rather than ambiguous between two keys both seeing the same pre-step frontier `J`. Write `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` for the homed-set. *Base:* By the EmptyInitialLinkStore assumption (Foundation, above), `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J_d^{Σ_init} = -1`. *Step:* assume `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` is a contiguous initial segment for every `d` (the IH), with `J = J_d^{Σ_k}`. By clause (b) (at-most-one-key-per-home, Definition — substrate-conforming state), fix a home `d`. If the step adds no fresh key homed at `d`, then `H_d^{Σ_{k+1}} = H_d^{Σ_k}` and contiguity is preserved with `J_d^{Σ_{k+1}} = J`. Otherwise the step adds exactly one fresh key homed at `d`; the frontier-landing requirement (clause (c), Definition — substrate-conforming state) extends the prefix by one chain index, so `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + 1}` with `J_d^{Σ_{k+1}} = J + 1`, again a contiguous initial segment. Since `d` was arbitrary, contiguity is preserved at every home. The set equality
