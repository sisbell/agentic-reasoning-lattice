...
**Definition — Nullify.** Nullify is the composition stated below, with three preconditions: **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition); **P1**: `a ∈ A_rel^Σ`; **PC**: Σ →*-reachable. P0 governs execution; P1 and PC condition the single-tuple-scope postcondition R-Scope. (Under PC, the internal `Emit_R`'s emission lands on a genuine chain sibling: `d_retr`'s homed link-set is a contiguous chain prefix of `A_L(d_retr)` at every allocated home by L-ContiguousPrefix.)
