...
*Domain caveat.* This formula is the weakest precondition **relative to the layer-reachable domain only** (Definition — layer-reachable). It does *not* characterize `wp` over the strictly larger `→*`-reachable domain on which `Emit_K` is otherwise defined. The derivation's "no pre-existing retraction covers the fresh `a`" step (below) leans on the unit-depth retraction discipline, which holds only at layer-reachable states; at a `→*`-reachable-but-undisciplined state a pre-existing wide retraction to-span can already cover the fresh `a`, adding a further necessary disjunct that the formula above omits. The stated formula is therefore asserted as `wp` only where disciplinedness is available.
