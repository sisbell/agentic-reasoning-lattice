...
Where Σ ranges over the state-local-conforming sub-space (Definition — state-local-conforming state). `Emit_K` is *partial* over this sub-space and *total* over the substrate-conforming sub-domain: R0's substrate-conformance hypothesis is exactly what makes K.λ's emission admissible (R0; Remark — NestedLinkWitness exhibits a state-local-conforming Σ at which the subsequent-emission `inc(ℓ_prev, 0)` is off-chain, so no legitimate K.λ-edge exists and `Emit_K(Σ, d, F, G)` is undefined). Where it is defined, it is a function (Lemma — Emit_K function-ness, below).
...
| Nullify | OP | `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})`; conditions P0/P1/P2 and their gating/postcondition/scope roles derived in Definition — Nullify |
