...
| R6b | DEF-Consequence | SingleDepthRetraction — deciding `a ∈ nullified` is a single flat existential pass over `L_R^Σ` (the audit slice, not the active subset `A_R^Σ`), checking only direct targeting; the witness retractor's own status does not enter the test | introduced |
...
| Relational layer | COMMITMENT | Operation set `{Emit_K, Observe_K, Nullify}` + reduction corollary; see Definition — relational layer | introduced |
