...
*Value-shape consequence (L3-conformance check).* Every L-invariant at the fresh emitter key is value-independent given `Endset`-typed inputs: the L/S/M/C catalog is discharged structurally by the single K.λ-step via RT-closure, not consulting the triple's content. **L3-conformance** of a triple `(F, G, K)` holds by the typed signature itself — arity is 3, both content slots `F, G ∈ Endset`, and `K ∈ T_admissible` forces a non-empty type slot — so the caller discharges no separate value requirement.

*Proof.* R0 is a near-direct consequence of ASN-0093's K.λ contract. Fix the caller-supplied `d ∈ dom(Σ.M)` bound by the universal. We invoke K.λ at home `d` with value `(F, G, K)` ∈ Endset × Endset × T_admissible, which satisfies K.λ's L3-discharge precondition by the L3-conformance check (Value-shape consequence above).
...
*Proof.* Fix any `a ∈ A_rel^Σ` at any →*-reachable state Σ. By L1a (LinkScopedAllocation, ASN-0043) applied at `a`, `home(a) ∈ dom(Σ.M)`, so `dom(Σ.M) ≠ ∅` — R0's home precondition can be discharged at some allocated document; Σ's `→*`-reachability discharges R0's reachability precondition. (Equivalently, "may appear in the from-set or to-set of an emitted tuple" presupposes a state with at least one document allocated, which `a ∈ A_rel^Σ` itself supplies.) The self-targeting emission may be homed at *any* allocated document, not only at `home(a)`.
...
`A_K^Σ` is computable from `Σ.L` alone. The slice `L_K^Σ` is selected from the finite store `Σ.L` (L-fin) by the per-address test `coverage(Σ.L(a).e₃) = coverage(K)`, decidable by Lemma CoverageEqualityDecidable; and `nullified(Σ)` is selected from the same finite domain `A_rel^Σ = dom(Σ.L)` (L-fin) by the test `(E (b, F', G') ∈ L_R^Σ :: a ∈ coverage(G'))`. This test has two decidable layers: selecting the slice member `(b, F', G') ∈ L_R^Σ` requires deciding `coverage(Σ.L(b).e₃) = coverage(R)` for each `b ∈ dom(Σ.L)`, decidable by Lemma CoverageEqualityDecidable exactly as for the `L_K^Σ` selection above; and, for a selected `b`, the per-span membership `s ≤ a < s ⊕ ℓ` is decidable by T2 (IntrinsicComparison, ASN-0034) even though a single span's `coverage(G')` may be infinite (a prefix span covers an entire subtree). `A_K^Σ` is then the finite slice `L_K^Σ` with `nullified(Σ)` excluded.
...
**Definition — Nullify.** *Precondition* **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition). Write **P1** for `a ∈ A_rel^Σ`. *Precondition* **P-tgt**: `P1 ∨ (a = a_emit(Σ, d_retr))` — the retraction target must be an *already-allocated link address* (`a ∈ dom(Σ.L)`) or the call's own *self-emit address* `a_emit(Σ, d_retr)`. Targets outside this set — ghost (never-emitted) addresses, and above all an allocator anchor whose prefix-subtree `{t : a ≼ t}` embraces an entire future sibling stream — are *forbidden*. The restriction is not an artifact of the proof but a design requirement: retraction is a destructive withdrawal, and Nelson scopes withdrawal to existing, owned material — *"Only the owner has a right to withdraw a document or change it"* — never to addresses no one has yet baptized; udanax-green's link lookup enforces the same boundary, rejecting any never-allocated tumbler at the granfilade exact-match guard (`tumblereq`, `granf2.c:37`). Forward-embrace of an anchor's subtree is a capability the system grants to *links and search* (additive, gathering), not to retraction (which would weaponize it to destroy unwritten future content). With P-tgt in force, the admissible targets split *exhaustively* into the two branches P1 and self-emit, on which the discipline induction and R-Scope below rest.
...
Both disjuncts fall under R-Scope (SingleTupleScope), whose domain is now precisely `P1 ∨ self-emit` — co-extensive with Nullify's target precondition P-tgt. The weakest precondition `P0 ∧ (P1 ∨ a = a_emit)` thus *coincides* with the operation's own precondition `P0 ∧ P-tgt`: every legal Nullify call attains single-tuple scope, and no legal call fails to. The branch-by-branch realizability below records why neither disjunct may be dropped.

— *P1 branch* (`a ∈ A_rel^Σ`, the standard case): the target already exists, `a ≠ e`, and `A_rel^{Σ'} = A_rel^Σ ∪ {e}` contains `a`; the scope conclusion is R-Scope's P1 branch. Dropping P1 *while also forbidding the self-emit branch* admits a counterexample: choose `a ∉ A_rel^Σ` with `a ≠ e` — such an `a` exists because `A_rel^Σ ∪ {e}` is finite while `T` is infinite (T0(b), ASN-0034) — and then `a ∉ A_rel^{Σ'} = A_rel^Σ ∪ {e}`, so `a ∉ {t : a ≼ t} ∩ A_rel^{Σ'}` and the intersection cannot equal `{a}`. This is exactly the ghost-target case P-tgt forbids: the precondition does not merely happen to exclude it but must, since the scope postcondition fails there. So `P1` cannot simply be dropped; the weakest precondition *widens* it by the self-emit disjunct rather than admitting off-`A_rel` targets.

— *Self-emit branch* (`a = e`, hence `a ∉ A_rel^Σ`): this is R-Scope's self-emit branch. The internal `Emit_R` deposits `(∅, {(a, δ(1, #a))}, R)` at `e = a`, so `a ∈ A_rel^{Σ'}`; R0a's antichain at Σ' then gives `{a' ∈ A_rel^{Σ'} : a ≼ a'} = {a}`, i.e. the scope postcondition — even though `a ∉ A_rel^Σ` and P1 is false. Omitting this disjunct would wrongly reject a pre-state under which the postcondition holds.
...
*The disjunct is non-redundant.* The second conjunct's escape branch is required for weakestness, not merely sufficient: an `Emit_K` call with `K ~ R` but `a_emit(Σ, d) ∉ coverage(G)` (witness `G = ∅`, whose coverage is empty) deposits a retraction not covering `a`, so `a ∉ nullified(Σ')` and the postcondition holds where a bare `K ≁ R` would wrongly reject it. The Nullify-as-sole-`R`-producer rule discharges `K ≁ R` for the relational layer's own callers; a direct K.λ caller at `K ~ R` must establish `a_emit(Σ, d) ∉ coverage(G)` explicitly. The derivation below establishes the biconditional.

*Derivation (both directions).* `d ∈ dom(Σ.M)` is Emit_K's home-precondition, and it is load-bearing: dropping it leaves K.λ's home-precondition undischarged, so no post-state Σ' is produced. With the home-precondition established and the index admissible (per the Result statement above), Σ' exists and `(a, F, G) ∈ L_K^{Σ'}` holds unconditionally (Definition of Emit_K), so the postcondition `(a, F, G) ∈ A_K^{Σ'}` is equivalent to `a ∉ nullified(Σ')`. It therefore suffices to show, over the layer-reachable domain, that `a ∉ nullified(Σ')` is equivalent to the second conjunct `K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G)`.

Layer-reachability of Σ gives `→*`-reachability, so R0a's antichain holds at Σ; the K.λ `→`-step carries it to Σ' (RT-closure), so R0a's antichain on `dom(Σ'.L)` is available. Disciplinedness — derived for layer-reachable states by the Definition — relational layer discharge — gives that no pre-existing retraction covers the fresh `a`: every *pre-existing* `L_R^Σ` tuple has a unit-depth to-span `{(b, δ(1, #b))}` with coverage `{t : b ≼ t}` for some `b ∈ A_rel^Σ`; the fresh `a = a_emit(Σ, d)` is prefix-incomparable with every such `b` by K.λ's emission rule together with R0a, so `a ∉ coverage(G')` for any pre-existing retraction `(_, _, G') ∈ L_R^Σ`. Thus no pre-existing retraction nullifies `a`, and whether `a ∈ nullified(Σ')` is decided entirely by the fresh emission. Now `L_R^{Σ'} = L_R^Σ ∪ {(a, F, G)}` when `K ~ R`, and `L_R^{Σ'} = L_R^Σ` when `K ≁ R`. The fresh tuple lies in `L_R^{Σ'}` iff `K ~ R`, and — when it does — its to-coverage contains `a` iff `a = a_emit(Σ, d) ∈ coverage(G)`. Hence `a ∈ nullified(Σ') ⟺ (K ~ R ∧ a_emit(Σ, d) ∈ coverage(G))`. Negating both sides, `a ∉ nullified(Σ') ⟺ (K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))` — exactly the second conjunct. Both the necessary and the sufficient direction are thereby established, and with `(a, F, G) ∈ L_K^{Σ'}` unconditional, the stated formula is the weakest precondition over the layer-reachable domain.
...
| Nullify | OP | `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})`; precondition P0 ∧ P-tgt (target `a ∈ A_rel^Σ` or self-emit); per Definition — Nullify |
