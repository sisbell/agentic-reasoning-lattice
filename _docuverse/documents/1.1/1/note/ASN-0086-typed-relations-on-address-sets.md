...
Single-tuple scope is therefore *arity-independent*: nullifying an arity-4 address would establish `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}` identically, since the argument consults only `a`'s tumbler prefix and the antichain, never `|Σ.L(a)|`. The wp lists P0 and P1 for distinct reasons (Definition of Nullify): P0 because its violation aborts the underlying Emit_R, so no Σ' is produced and the postcondition is unreachable; P1 because — although its violation does *not* abort emission — it is load-bearing for the postcondition, since without `a ∈ A_rel^Σ` the only new key at Σ' is the fresh emitter `b ≠ a`, leaving `a ∉ A_rel^{Σ'}` and the intersection unequal to `{a}`. The scope condition P2 (`|Σ.L(a)| = 3`) is neither: it neither gates emission nor bears on single-tuple scope (arity-independent), so it does not appear.
