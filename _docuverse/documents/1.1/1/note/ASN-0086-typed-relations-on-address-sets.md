...
**Working domain — `→*`-reachable states.** All results below are stated over states `→*`-reachable from `Σ_init`. This class is closed under `→` by construction — the reflexive-transitive closure absorbs each further `→`-step — so every emission from a `→*`-reachable state lands again in the class. Each `→`-step is a single K.σ/K.α/K.λ primitive, which preserves the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and, for K.λ, lands its single fresh link key at its home's sibling frontier (`[d.0.s_L.1]` for an empty homed-set, else `inc(ℓ_prev, 0)` at the prior T1-maximum). Two structural facts follow at every `→*`-reachable state, both *derived* below from this K.λ frontier-emission discipline rather than assumed: the homed link-set at each document is a contiguous chain prefix of `A_L(d)` (L-ContiguousPrefix), and `dom(Σ.L)` is a tumbler-prefix antichain (R0a).

We do not quantify over hypothetical higher-layer operations. The substrate of this note admits no transition other than K.σ/K.α/K.λ, so there is no operation against which a separate "categorical" reachability or a layered conformance taxonomy would range. The question of which nullification guarantees survive operations beyond K.σ/K.α/K.λ is deferred to the ASN that actually introduces such operations.
...
*`A_rel^Σ` names the whole link store, not only the tuples.* `A_rel^Σ = dom(Σ.L)` collects *every* link address, whereas the standard-triple links of this note's relational vocabulary are exactly those with `|Σ.L(a)| = 3`. By L3 (NEndsetStructure, ASN-0043) the store admits higher-arity links (`|Σ.L(a)| > 3`); such addresses inhabit `A_rel^Σ` but correspond to no tuple of any `L_K` (Definition — TypedRelation, below). We keep the broader `A_rel^Σ = dom(Σ.L)` because the substrate-level guarantees stated over it — SD-disjointness here, R0a's antichain, R-Scope's single-address scope — are genuinely arity-independent; where a result concerns tuples specifically we say so and restrict to `|Σ.L(a)| = 3`.
...
**Lemma — CoverageEqualityDecidable.** For any two endsets `e, e' ∈ Endset`, the predicate `coverage(e) = coverage(e')` is decidable, using only T2 comparisons, TumblerAdd, and zero-extension on T0's carrier (appending a single `0` component to a tumbler sequence). *Proof.* By `Endset = 𝒫_fin(Span)` (ASN-0043), `e` and `e'` are finite, so `coverage(e)` and `coverage(e')` are finite unions of half-open T1-intervals `[s, s ⊕ ℓ) = {t : s ≤ t < s ⊕ ℓ}` (T12, SpanWellDefinedness, ASN-0034), each upper endpoint `s ⊕ ℓ` computed by TumblerAdd (ASN-0034). Let `P = {s : (s, ℓ) ∈ e ∪ e'} ∪ {s ⊕ ℓ : (s, ℓ) ∈ e ∪ e'}`, a finite endpoint set; sort it under T1 via T2 (IntrinsicComparison, ASN-0034) into the distinct values `c₁ < … < c_m`. *Base case `m = 0`:* this occurs exactly when `e = e' = ∅` (any non-empty span `(s, ℓ)` has `ℓ > 0`, so `s < s ⊕ ℓ` by TA-strict (ASN-0034), contributing two distinct endpoints and forcing `m ≥ 2`); then `coverage(e) = coverage(e') = ∅`, so the predicate holds and is decided affirmatively. Assume henceforth `m ≥ 2`. Every interval's endpoints lie in `P`, so each interval is a union of consecutive *cells* of the partition `{c₁}, (c₁, c₂), {c₂}, …, {c_m}` (points and open gaps); no endpoint falls strictly inside an open gap, so each coverage is constant — wholly in or wholly out — on every cell. An interval `[s, s ⊕ ℓ)` covers the point `c_k` iff `s ≤ c_k ∧ c_k < s ⊕ ℓ`, and covers the gap `(c_k, c_{k+1})` iff `s ≤ c_k ∧ c_{k+1} ≤ s ⊕ ℓ` — each a finite conjunction of T2 comparisons evaluated without exhibiting any interior tumbler.

A gap-indicator faithfully reflects coverage only on a *non-empty* gap; an empty gap is `∅` in both coverages, so it must be excluded from the comparison, since its endpoint-arithmetic indicator constrains endpoints rather than membership of a vacuous interval and could spuriously disagree. A gap `(c_k, c_{k+1})` is **empty iff `c_{k+1} = c_k.0`**, where `c_k.0` is the immediate T1-successor of `c_k` (ASN-0034, TA5 note): `c_k < c_k.0` by T1 case (ii) (`c_k` is a proper prefix of `c_k.0`), and no tumbler lies strictly between them — any `x` with `c_k ≼ x` and `#x > #c_k` either equals `c_k.0`, carries a positive component at position `#c_k + 1` (so `c_k.0 < x` by case (i)), or has a zero there with `#x > #c_k + 1` (so `c_k.0 ≼ x`); any `x` diverging from `c_k` at some `j ≤ #c_k` with `x_j > c_{k,j}` satisfies `c_k.0 < x` by case (i). Emptiness is thus decided by the single T2 comparison `c_{k+1} = c_k.0`; on a non-empty gap, `c_k.0 ∈ (c_k, c_{k+1})` witnesses non-emptiness.

Over the non-empty cells — every point `{c_k}`, and every gap with `c_{k+1} ≠ c_k.0` — gap-indicator equality reflects coverage-set equality: a coverage agreement with an indicator disagreement would place the cell's witness (`c_k.0` for a gap, `c_k` for a point) in one coverage and not the other. Both coverages lie within `[c₁, c_m)` (every interval's endpoints are in `P`), so they agree trivially on the exterior cells `(−∞, c₁)` and `[c_m, ∞)`. Computing each coverage's indicator over the finitely many non-empty cells (a finite disjunction over `e`, resp. `e'`) and comparing the two indicator vectors decides `coverage(e) = coverage(e')`. ∎
...
*Proof.* This is ChainMembershipForOrigin (ASN-0093, link half) restated at `→*`-reachable states in the `home`/`J_d^Σ` notation; we cite the foundation rather than re-derive it. ChainMembershipForOrigin gives, at every reachable state, `dom(Σ.L) ∩ {ℓ' : origin(ℓ') = d} = {s_1, …, s_{n_d}}`, a contiguous initial segment of the link sub-allocator chain `A_L(d)` (anchor `[d.0.s_L.1]` by FirstEmission, sibling recurrence `inc(·, 0)`). The translation to the stated form uses two identifications. First, `origin` and `home` coincide on link addresses: under ASN-0036, both are the NUDE-prefix projection `N(·).0.U(·).0.D(·)`, so `{a ∈ dom(Σ.L) : home(a) = d} = dom(Σ.L) ∩ {ℓ' : origin(ℓ') = d}`. Second, re-index the chain by `j = (chain position) − 1` so that `s_{j+1} = incʲ(d.0.s_L.1, 0)`; then the segment `{s_1, …, s_{n_d}}` is `{incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J_d^Σ}` with `J_d^Σ = n_d − 1 ∈ ℤ_{≥-1}` (the empty case `n_d = 0` giving `J_d^Σ = -1`).
...
**L-ContiguousPrefix-Cor1 — DepthTwoLinkAddresses.** (Corollary of L-ContiguousPrefix.) At every →*-reachable state Σ, every link address in `dom(Σ.L)` has an element field (T4b's `E` projection) of length exactly 2:

`(A Σ : Σ →*-reachable :: (A a ∈ dom(Σ.L) :: #E(a) = 2))`
...
**R0 — TupleAddressFreshness.** For any →*-reachable state Σ with `dom(Σ.M) ≠ ∅` and any `(F, G, K) ∈ Endset × Endset × T_admissible`, there exists a state Σ' with Σ → Σ' that emits a tuple with content (F, G) of type K at a fresh address, and the resulting post-state Σ' is itself →*-reachable:
...
*L-invariant preservation across the K.λ-step.* The emission above is a valid K.λ `→`-step — its on-chain gating and freshness preconditions discharged in the freshness bullets — taken from the `→*`-reachable pre-state Σ. Since `→*`-reachability is closed under `→`, Σ' is `→*`-reachable. Every catalog invariant (the full L/S/M/C catalog of ASN-0036, ASN-0043, ASN-0093) holds at the fresh key `a` by K.λ's own ASN-0093 invariant-preservation contract. The one obligation not closed structurally by that contract is the match between the emitted value and K.λ's value-shape precondition: `Emit_K` supplies the triple `(F, G, K)`, a `Link`-typed value of arity 3 with non-empty type slot `K`, which satisfies K.λ's `N ≥ 3 ∧ e₃ ≠ ∅` at `N = 3`, `e₃ = K` — so the emission is a well-formed K.λ value and L3 is met. This completes the final conjunct of R0's conclusion: Σ' is `→*`-reachable. ∎
...
*Proof.* Fix any `a ∈ A_rel^Σ` at any →*-reachable state Σ. By L1a (LinkScopedAllocation, ASN-0043) applied at `a`, `home(a) ∈ dom(Σ.M)`, so `dom(Σ.M) ≠ ∅` — discharging R0's `dom(Σ.M) ≠ ∅` precondition for the home `d` chosen below; Σ's `→*`-reachability discharges R0's reachability precondition. (Equivalently, "may appear in the from-set or to-set of an emitted tuple" presupposes a state with at least one document allocated, which `a ∈ A_rel^Σ` itself supplies.)
...
*(Step 3 — Self-targeting emission via R0.)* Pick any `d ∈ dom(Σ.M)` and any `K ∈ T_admissible`. The triple `(∅, G_self, K)` is L3-conforming: arity 3, with `∅ ∈ Endset` (the empty endset) and `G_self ∈ Endset` (the singleton built in Step 2), and `K ∈ T_admissible` non-empty by assumption. Apply R0 at this L3-conforming triple and home `d`. R0's emission discharges every L-invariant except L3 on the emitter address alone — K.λ's Frame fixes `(Σ.C, Σ.M)` pointwise, and the L14/L14a fresh-key obligation is the FreshLinkKeyDisjointness sub-lemma — the only content-dependent check, L3, is met here by the conformance just verified. R0 therefore produces a fresh emitter `a' ∉ dom(Σ.L)` and `→*`-reachable post-state Σ' with `Σ'.L(a') = (∅, G_self, K)`. The self-reference is recorded at the substrate level: `a ∈ coverage(Σ'.L(a').e₂)` — the to-set case.
...
The existential checks `coverage(G')` only — the to-set's coverage — and does not inspect `coverage(F')`, per Convention RetractionDirectionality; an `Emit_R` call whose to-span coverage misses `a` does not nullify `a`, regardless of what its from-set covers. The set-builder restriction `a ∈ A_rel^Σ` confines `nullified(Σ)` to link-store addresses: ghost, content, and document addresses in `coverage(G')` — which by R5/L9 may include link, content, document, or ghost addresses — are not collected. Because `A_rel^Σ = dom(Σ.L)` holds higher-arity link addresses as well as arity-3 tuple addresses, `nullified(Σ)` may collect a higher-arity link address; such an address indexes no `L_K`-tuple (Definition — TypedRelation), so the active-subset exclusion `a ∉ nullified(Σ)` in `A_K^Σ` simply never references it — the collection is harmless.
...
*Consequence — `A_K` is not monotone, though `L_K` is.* R3, lifted along `→*` by the same induction as R6c, makes the audit slice monotone (`Σ →* Σ' ⟹ L_K^Σ ⊆ L_K^{Σ'}`); the active subset is not. A retraction removes a tuple from `A_K` while leaving it in `L_K` — so `A_K` shrinks across that transition, falsifying `⊆`. A later re-emission (R0) admits a fresh tuple at a fresh address into `A_K` — so `A_K` grows across that transition, falsifying `⊇`. Neither inclusion holds in general between `A_K^Σ` and `A_K^{Σ'}`. Predicates and observation views over `A_K` must therefore treat its evolution as non-monotone, not inherit monotonicity from `L_K`. (The Worked Sketch below exhibits both failures concretely.)


## Three Operations

The six properties yield three operations that span every `Σ.L` change the relational layer effects. (Document allocation K.σ and content emission K.α are also visible substrate changes, but lie outside this note's `Σ.L` scope and outside the three operations.)
...
Where Σ ranges over the `→*`-reachable states. `Emit_K` is total over `→*`-reachable Σ (R0): L-ContiguousPrefix guarantees `d`'s homed-set `{ℓ ∈ dom(Σ.L) : home(ℓ) = d}` is a contiguous chain prefix of `A_L(d)`, so `a_emit(Σ, d) = inc(ℓ_prev, 0)` lands on a genuine chain sibling (or `[d.0.s_L.1]` for an empty homed-set) and the emission is admissible. `Emit_K` is a function: the address is `a = a_emit(Σ, d)` (a function of `(Σ, d)` by Definition — `a_emit`), the value `Σ'.L(a) = (F, G, K)` is fixed by the caller-supplied arguments, and K.λ's Frame fixes the rest of Σ'.
...
**Definition — Nullify.** Nullify is the composition stated below, with four preconditions: **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition); **P0f**: `d_retr`'s homed link-set `{ℓ ∈ dom(Σ.L) : home(ℓ) = d_retr}` is a contiguous chain prefix of `A_L(d_retr)` (a well-formed frontier), so the internal `Emit_R`'s emission lands on a genuine chain sibling — without it the subsequent-emission `inc(ℓ_prev, 0)` may be off-chain and `Emit_R` undefined (Definition — Emit_K); over →*-reachable Σ, P0f holds automatically at every allocated home (L-ContiguousPrefix); **P1**: `a ∈ A_rel^Σ`; **PC**: Σ →*-reachable. P0 ∧ P0f govern execution; P1 and PC condition the single-tuple-scope postcondition R-Scope.
...
*Proof.* The reduction follows directly from the layer's *Definition*: its only state-affecting operations are `Emit_K` and its alias `Nullify`, each *already* a single K.λ `→`-step specialized to a standard-triple value, while `Observe_K` takes no transition. So every `Σ.L`-affecting step the layer takes simply *is* an `Emit_K` call, with nothing to decompose. ∎
...
*Result.* For this note's operation set `{Emit_K, Observe_K, Nullify}`, and over the sub-space of pre-states Σ that are both →*-reachable *and* unit-depth-disciplined (the domain restriction stated below), the weakest precondition is

`wp(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) ≡ d ∈ dom(Σ.M) ∧ (K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))`   (over →*-reachable, unit-depth-disciplined Σ; `K` is an index, not a free wp variable)

The second conjunct is a *disjunction*, and it constrains the *call* — its type index `K` and to-set `G` — not solely the pre-state. It captures exactly the self-nullification boundary: the fresh emission self-nullifies precisely when it lands in the retraction slice (`K ~ R`) *and* its to-set covers its own address (`a_emit(Σ, d) ∈ coverage(G)`), and the disjunction is the negation of that conjunction. The derivation below establishes the biconditional.
...
*Domain restriction.* This wp is asserted over pre-states Σ that are (i) `→*`-reachable and (ii) unit-depth-disciplined (Definition — Unit-depth retraction discipline) — in particular, every Σ reached using only the relational layer's operations qualifies (those operations keep states `→*`-reachable and commit to the discipline). Restriction (i) is the ambient substrate domain, supplying R0a's antichain. Restriction (ii) is the genuine added assumption: it is a *layer* commitment, not a substrate guarantee, because K.λ fixes emission *address* but not endset *shape*. A direct K.λ caller — one not bound by the layer discipline — can craft a non-unit-depth retraction span while staying `→*`-reachable, so the result does not extend to such callers, as the next paragraph shows.

*The unit-depth discipline is load-bearing.* The discipline is a layer commitment, not a substrate guarantee: because K.λ fixes emission *address* but not endset *shape* (the address-vs-shape gap), a direct K.λ caller can emit a crafted-span retraction that is L-invariant-conforming and leaves the state `→*`-reachable, yet violates the unit-depth retraction discipline. Hence a `→*`-reachable Σ may carry a crafted non-unit-depth retraction span emitted by such a caller. A crafted span `(s, ℓ)` has coverage the lexicographic interval `{t : s ≤ t < s ⊕ ℓ}` (T12, SpanWellDefinedness, ASN-0034), *not* a prefix-subtree, so prefix-incomparability of the fresh `a` against `s` does not exclude `a`: a wide span rooted near `d`'s link chain can satisfy `s ≤ a < s ⊕ ℓ`, putting `a ∈ coverage(G')` and hence `a ∈ nullified(Σ')`, while `d ∈ dom(Σ.M)` holds. This is exactly the case the unit-depth discipline excludes, and why restriction (ii) cannot be dropped.
...
| R0 | LEMMA | TupleAddressFreshness — over →*-reachable Σ with `dom(Σ.M) ≠ ∅`, every emission allocates an address fresh against `dom(Σ.L)`, on-chain in `A_L(d)` (K.λ's gating precondition), in both the first- and subsequent-emission branches, and yields a →*-reachable post-state Σ' |
...
| R-Scope | LEMMA | SingleTupleScope — at a →*-reachable Σ, `Nullify(Σ, d_retr, a)`'s `→`-step gives `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}`; arity-independent (= R0a antichain + K.λ freshness + P1 + L12a) |
