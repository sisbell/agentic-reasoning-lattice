...
**Definition — Nullify.** *[Discipline-conditional for single-tuple scope.]* Nullify has three preconditions: (P1) `a ∈ A_rel^Σ`, (P2) `|Σ.L(a)| = 3` (i.e., `a` is the address of a standard-triple link), and (P3) `a` has no strict prefix-extension in `dom(Σ.L)`:
...
`Nullify(Σ, a) ≡ Emit_R(Σ, home(a), ∅, {(a, δ(1, #a))})`

That is, emit a tuple into the retraction relation with empty from-set and a unit-depth to-span targeting `a`, with the retraction itself homed at `a`'s home document `home(a) ∈ dom(Σ.M)` (well-defined by L1a + L12b: `home(a)` is determined by `a`'s tumbler structure and persists in `dom(Σ.M)`). By PrefixSpanCoverage (ASN-0043), `coverage({(a, δ(1, #a))}) = {t : a ≼ t}`, which contains `a`. Let `(Σ', _) = Nullify(Σ, a)`. By Definition of `nullified`, `a ∈ nullified(Σ')`. By R6a, `a` remains nullified thereafter.
...
*Step 2 (concrete).* Re-emit `Emit_K(d, F₁, G₁)`. R0 Step 2 Case B (`b = a₁` again works; `b = b₁` would also work) seeks the least `i ≥ 1` with `incⁱ(a₁, 0) ∉ dom(Σ_1.L) = {a₁, b₁}`. `inc¹(a₁, 0) = b₁ ∈ dom(Σ_1.L)`; `inc²(a₁, 0) = 1.0.1.0.1.0.2.3 ∉ dom(Σ_1.L)`. Set `a₂ = 1.0.1.0.1.0.2.3` — the third emission of the depth-2 allocator `A_{a₁}`. Then `Σ_2.L = Σ_1.L ∪ {a₂ ↦ (F₁, G₁, K)}` and:
...
| Nullify | OP | `Nullify(Σ, a) ≡ Emit_R(Σ, home(a), ∅, {(a, δ(1, #a))})` for `a ∈ A_rel^Σ` with `|Σ.L(a)| = 3` and `(A a' ∈ dom(Σ.L) : a ≼ a' :: a' = a)` (P3 — no strict prefix-extension; automatic under R0a's reachable-state antichain); *single-tuple scope is discipline-conditional* — Nullify's guarantee that exactly `a` (and no subtree of `A_rel`) is contributed to `nullified(Σ')` requires the sibling-frontier emission discipline of R0a in addition to P3 | introduced |
