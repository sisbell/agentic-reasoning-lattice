...
**Definition — substrate-conforming state.** A state Σ is *substrate-conforming* iff it is reachable from `Σ_init` by transitions that each (a) preserve the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and (b) emit every fresh link key at its home document's sibling frontier (the ASN-0093 chain discipline). Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a) and (b) by their ASN-0093 contracts.
...
*Proof.* The conclusion follows from conformance clause (b) alone, by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)/(b)); we do not appeal to ASN-0093's ChainMembershipForOrigin, whose stated scope is `→`-reachability. *Base:* `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J = -1`. *Step:* assume `H_d^{Σ_k}` is a contiguous initial segment for every `d`. A conforming transition `Σ_k ↝ Σ_{k+1}` extends `dom(·.L)` only by fresh keys, and clause (b) emits each fresh key at its home's sibling frontier — at first-emission `[d.0.s_L.1] = inc⁰(d.0.s_L.1, 0)` when `H_d^{Σ_k} = ∅`, or at `inc(max H_d^{Σ_k}, 0)` (the chain element immediately past the current contiguous prefix) when `H_d^{Σ_k} ≠ ∅`. In the first case `H_d` grows from `∅` to `{inc⁰(d.0.s_L.1, 0)}`; in the second, from `{incʲ : 0 ≤ j ≤ J}` to `{incʲ : 0 ≤ j ≤ J + 1}`. Either way `H_d` remains a contiguous initial segment, and a key homed at `d' ≠ d` leaves `H_d` unchanged. Contiguity is therefore preserved at every `d`. ∎
...
*Case 2 — Same-home (`home(a) = home(a') = d`).* By the ConformingHomedContiguity sub-lemma — which derives contiguity from conformance clause (b) directly, so it holds whether Σ is `→*`-reachable or an `↝`-reachable conforming-layer post-state — the set `{a'' ∈ dom(Σ.L) : origin(a'') = d}` is a contiguous initial segment of `A_L(d)`'s chain enumeration `(t_1, t_2, t_3, …)` with `t_1 = [d.0.s_L.1]` and `t_{n+1} = inc(t_n, 0)`. Hence both `a` and `a'` are chain elements: `a = t_i` and `a' = t_j` for some `i, j ≥ 1`. By (UL), `#a = #t_i = #t_1 = #a'` — all chain elements have equal length. If `a ≼ a'`, then by the prefix definition (positions `1..#a` of `a'` agree with `a`) combined with `#a = #a'`, `a` and `a'` coincide pointwise, so `a = a'` by T3 (CanonicalRepresentation, ASN-0034). (Equivalently, by T10a.2 applied to the distinct siblings `a`, `a'` of `A_L(d)`.)
...
*Proof.* The contiguous-prefix form is the ConformingHomedContiguity sub-lemma applied to the link store, which derives the contiguity from conformance clause (b) at every substrate-conforming Σ, `→*`-reachable or `↝`-reachable alike (rather than from ASN-0093's ChainMembershipForOrigin, whose stated scope is `→`-reachability). The sub-lemma gives `dom(Σ.L) ∩ {a' ∈ T : origin(a') = d}` as a contiguous initial segment `{s_1, …, s_{n_d}}` of `A_L(d)`'s chain enumeration. Setting `J_d^Σ := n_d − 1`, we have `s_k = inc^{k−1}([d.0.s_L.1], 0)` by FirstEmission (ASN-0093) (`s_1 = t_1^L(d) = [d.0.s_L.1]`) and the sibling recurrence `s_{k+1} = inc(s_k, 0)`. Hence
...
| R0a | LEMMA | FlatLinkDomain — `dom(Σ.L)` is an antichain in `≼` at every substrate-conforming state (clause (b) frontier emission is the consumed hypothesis; covers `→*`-reachable and conforming-layer `↝`-reachable states alike). Case 1 cross-home via L1 + L1a; Case 2 same-home via ConformingHomedContiguity sub-lemma + (UL) + T3, equivalently via T10a.2 |
| R0a-Cor1 | LEMMA | ContiguousPrefix — `{a ∈ dom(Σ.L) : home(a) = d} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J_d^Σ}` for some `J_d^Σ ∈ ℤ_{≥-1}`; direct re-expression of the ConformingHomedContiguity sub-lemma, which derives contiguity from conformance clause (b) (not ASN-0093's `→`-scoped ChainMembershipForOrigin) |
