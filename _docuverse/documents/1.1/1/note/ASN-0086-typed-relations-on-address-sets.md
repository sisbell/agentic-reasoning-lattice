...
**Definition — substrate-conforming state.** A state Σ is *substrate-conforming* iff it is reachable from `Σ_init` by transitions that each (a) preserve the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and (b) emit the fresh link keys homed at each document as the next *contiguous* chain segment past that home's current sibling frontier (the ASN-0093 chain discipline). Concretely: if a single step adds `r ≥ 1` fresh keys at home `d` whose homed-set occupied chain indices `0..J` before the step, those keys occupy exactly chain indices `J+1, …, J+r` of `A_L(d)` — no gaps, no indices skipped. (For a single K.λ primitive `r = 1`; the contiguous-block form covers composite `↝`-steps that deposit several keys at one home at once.) Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a) and (b) by their ASN-0093 contracts (each K.λ adds one key at the frontier, the `r = 1` case).
...
**Sub-lemma — ConformingHomedContiguity.** At every substrate-conforming state Σ and every `d`, the homed-set `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` is a contiguous initial segment of `A_L(d)`'s chain enumeration: `H_d^Σ = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` for some `J ∈ ℤ_{≥-1}` (with `J = -1` denoting `H_d^Σ = ∅`).

*Proof.* The conclusion follows from conformance clause (b) alone, by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)/(b)). *Base:* `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J = -1`. *Step:* assume `H_d^{Σ_k}` is a contiguous initial segment for every `d`. A conforming transition `Σ_k ↝ Σ_{k+1}` extends `dom(·.L)` only by fresh keys. We do not assume the step adds a single key: a composite `↝`-step may deposit a finite set of fresh keys, several of which share a home. Fix a home `d`, and let `r ≥ 0` be the number of fresh keys homed at `d` in this step (if `r = 0`, `H_d` is unchanged and contiguity is trivially preserved). Suppose `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` for some `J ∈ ℤ_{≥-1}` (the IH). Clause (b) requires the `r` fresh keys homed at `d` to occupy exactly chain indices `J+1, …, J+r` of `A_L(d)` — the next contiguous segment past the current frontier, with no index skipped. Consequently `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + r}`, again a contiguous initial segment. (The single-key cases are the specializations `r = 1`: first-emission `[d.0.s_L.1] = inc⁰(d.0.s_L.1, 0)` when `J = -1`, or `inc(max H_d^{Σ_k}, 0)` when `J ≥ 0`.) Since `d` was arbitrary, contiguity is preserved at every home, whether the step is a single K.λ primitive or a multi-key composite. (Because the argument rests on clause (b) alone and never on ASN-0093's `→`-scoped ChainMembershipForOrigin, the sub-lemma holds at every substrate-conforming Σ — `→*`-reachable or `↝`-reachable conforming-layer post-state alike.) ∎
...
*Case 2 — Same-home (`home(a) = home(a') = d`).* By the ConformingHomedContiguity sub-lemma, the set `{a'' ∈ dom(Σ.L) : origin(a'') = d}` is a contiguous initial segment of `A_L(d)`'s chain enumeration `(t_1, t_2, t_3, …)` with `t_1 = [d.0.s_L.1]` and `t_{n+1} = inc(t_n, 0)`. Hence both `a` and `a'` are chain elements: `a = t_i` and `a' = t_j` for some `i, j ≥ 1`. By (UL), `#a = #t_i = #t_1 = #a'` — all chain elements have equal length. If `a ≼ a'`, then by the prefix definition (positions `1..#a` of `a'` agree with `a`) combined with `#a = #a'`, `a` and `a'` coincide pointwise, so `a = a'` by T3 (CanonicalRepresentation, ASN-0034). (Equivalently, by T10a.2 applied to the distinct siblings `a`, `a'` of `A_L(d)`.)
...
*Proof.* The contiguous-prefix form is the ConformingHomedContiguity sub-lemma applied to the link store at every substrate-conforming Σ. The sub-lemma gives `dom(Σ.L) ∩ {a' ∈ T : origin(a') = d}` as a contiguous initial segment `{s_1, …, s_{n_d}}` of `A_L(d)`'s chain enumeration. Setting `J_d^Σ := n_d − 1`, we have `s_k = inc^{k−1}([d.0.s_L.1], 0)` by FirstEmission (ASN-0093) (`s_1 = t_1^L(d) = [d.0.s_L.1]`) and the sibling recurrence `s_{k+1} = inc(s_k, 0)`. Hence

`{a ∈ dom(Σ.L) : home(a) = d} = {s_1, …, s_{n_d}} = {incʲ([d.0.s_L.1], 0) : 0 ≤ j ≤ J_d^Σ}`

with `J_d^Σ = -1` corresponding to `n_d = 0` (empty homed set). Under ASN-0036, `origin(a)` and `home(a)` coincide on every `a ∈ dom(Σ.L)` because L1 + L1a's NUDE-prefix projection is exactly the `origin(·) = N(·).0.U(·).0.D(·)` projection. The unique T1-maximum is derived above. ∎
...
*Scope of the conclusion — contingent on conforming-layer clause (b).* The claim `Σ_m.L = Σ'.L` asserts not merely that the replay reconstructs *some* fresh conforming addresses, but that it reconstructs *exactly the addresses `Σ'` holds*. This stronger address-reconstruction rests entirely on clause (b) of the *Definition — substrate-conforming layer* (frontier-contiguous deposition), which the discharge (4)(iii) below consumes. Clause (b) is part of what *defines* a conforming layer — it is not forced by the substrate's K-op vocabulary for composite steps: a single K.λ primitive (`r = 1`) does deposit at the frontier by its ASN-0093 contract, but a composite `↝`-step depositing several keys at one home could in principle scatter them across non-contiguous chain indices, and only clause (b) excludes that. R7a is therefore a *conditional* lemma — strong for layers that satisfy clause (b), and silent about deposition order for layers that do not. Downstream consumers must not read `Σ_m.L = Σ'.L` as an unconditional substrate guarantee.
...
| R0 | LEMMA | TupleAddressFreshness — under precondition `dom(Σ.M) ≠ ∅`, every emission allocates a fresh address. Discharged via ASN-0093 K.λ's first/subsequent emission rule, plus FirstEmissionFreshness (first-emission branch) and a conformance-free freshness derivation (subsequent-emission branch: `ℓ_prev` T4-valid via L1c→T10a.4, then within-home by TA5(a)+T1, cross-home by T10, cross-subspace by T7+SC-NEQ — valid at every state, not only `→*`-reachable ones), L-invariant preservation under K.λ's frame, and ASN-0093 L0 + SC-NEQ for L14/L14a |
| R0a | LEMMA | FlatLinkDomain — `dom(Σ.L)` is an antichain in `≼` at every substrate-conforming state. Case 1 cross-home via L1 + L1a; Case 2 same-home via ConformingHomedContiguity sub-lemma + (UL) + T3, equivalently via T10a.2 |
| R0a-Cor1 | LEMMA | ContiguousPrefix — `{a ∈ dom(Σ.L) : home(a) = d} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J_d^Σ}` for some `J_d^Σ ∈ ℤ_{≥-1}`; direct re-expression of the ConformingHomedContiguity sub-lemma |
