...
The witness of Remark — NestedLinkWitness satisfies (b) yet is not the frontier successor `inc(ℓ_prev, 0)`, so it violates (c). Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a)–(c) by their ASN-0093 contracts (each K.λ adds at most one key per home, at the frontier).
