...
Both coverages lie within `[c₁, c_m)`, agreeing trivially (both `∅`) on the exterior cells `(−∞, c₁)` and `[c_m, ∞)`. Computing each coverage's indicator over all finitely many cells — the `m` points `{c_k}`, the `m−1` open gaps `(c_k, c_{k+1})`, and the two exterior cells — and comparing the two indicator vectors decides `coverage(e) = coverage(e')`. Empty gaps need no special handling: an empty gap is `∅` in both coverages, so it contributes matching indicator entries automatically and never forces a spurious difference or a spurious match. ∎
...
The two address conditions `a ∉ dom(Σ.L)` (*freshness*) and `a ∈ A_L(d)` (*on-chain admissibility*) are explicit postconditions of the lemma, established below for the caller-chosen `d` in both the first- and subsequent-emission branches.
...
*Proof.* R0 is a near-direct consequence of ASN-0093's K.λ contract. Fix the caller-supplied `d ∈ dom(Σ.M)` bound by the universal. We invoke K.λ at home `d` with value `(F, G, K)` ∈ Endset × Endset × T_admissible (which satisfies K.λ's L3-discharge precondition by L3-conformance of the triple: `|·| = 3`, `F, G ∈ Endset`, `K ∈ T_admissible` non-empty).
...
- *First emission* (`a_emit`'s first-emission branch fires): `a = a_emit(Σ, d) = [d.0.s_L.1]`. By FirstEmission (ASN-0093), this address has `E(a)₁ = s_L`, `origin(a) = d` (hence `home(a) = d`), `#E(a) = 2`, `zeros(a) = 3`, and is T4-valid by direct inspection — discharging the shared `dom(Σ.C)` exclusion above. By ChainDiscipline + FirstEmission (ASN-0093), the link sub-allocator chain `A_L(d)` is active at every state with `d ∈ dom(Σ.M)`, and `a = [d.0.s_L.1]` is its first emission `t₁^L(d) = inc(b_L(d), 1)` — so `a ∈ A_L(d)`, discharging the *on-chain admissibility* postcondition in this branch. *Freshness against `dom(Σ.L)`.* Suppose `ℓ' ∈ dom(Σ.L)`. If `home(ℓ') = d`, then `ℓ'` is a link homed at `d`, contradicting the first-emission predicate `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} = ∅` (by HomeOriginCoincidence). If `home(ℓ') = d' ≠ d`, then since `home(a) = d` (established above), Cross-home distinctness gives `a ≠ ℓ'`. Either way `a ≠ ℓ'`, so `a ∉ dom(Σ.L)`. Together with the shared `dom(Σ.C)` exclusion, this discharges K.λ's freshness precondition `a ∉ dom(Σ.L) ∪ dom(Σ.C)` at the K.λ-event committing `a`.
- *Subsequent emission* (`a_emit`'s subsequent-emission branch fires): `a = a_emit(Σ, d) = inc(ℓ_prev, 0)` with `ℓ_prev := max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}`. By Definition — `a_emit` (Allocator Structure), this max is well-defined. We discharge K.λ's gating precondition and freshness against `dom(Σ.L)` as follows:
  ...
  - *Cross-home freshness.* For `ℓ' ∈ dom(Σ.L)` with `origin(ℓ') = d' ≠ d`: the *well-formedness* bullet established `origin(a) = d`, hence `home(a) = d` by HomeOriginCoincidence; and `home(ℓ') = d'` from `origin(ℓ') = d'` by the same fact. Since `d ≠ d'`, Cross-home distinctness gives `a ≠ ℓ'` directly — no prefix relationship between `d` and `d'` is assumed.
...
*Proof.* Fix any `a ∈ A_rel^Σ` at any →*-reachable state Σ. By L1a (LinkScopedAllocation, ASN-0043) applied at `a`, `home(a) ∈ dom(Σ.M)` — so taking `d = home(a)` discharges R0's home precondition `d ∈ dom(Σ.M)`; Σ's `→*`-reachability discharges R0's reachability precondition. (Equivalently, "may appear in the from-set or to-set of an emitted tuple" presupposes a state with at least one document allocated, which `a ∈ A_rel^Σ` itself supplies.)
...
*Scope — retractors are standard-triple links only.* The witnessing tuple `(b, F', G')` ranges over `L_R^Σ`, which carries the `|Σ.L(a)| = 3` conjunct of *Definition — TypedRelation*. A retractor must therefore be a standard-triple link; a higher-arity link (`|Σ.L(b)| > 3`) whose slot-3 coverage equals `coverage(R)` and whose slot-2 covers a target address has *no* nullifying effect. The to-set/from-set reading on which `nullified` depends is the convention-fixed slot assignment of the standard triple (StandardTriple, ASN-0043; Convention RetractionDirectionality), and the relational layer `L^Σ` of this note is by construction the standard-triple slice (`L^Σ = ⨆_{[K]} L_K^Σ`, each `L_K` triple-restricted). For `N > 3` links the model fixes no canonical to-slot, so `coverage(G')` is undefined for them and they cannot serve as retractors within this layer.
...
**Definition — Nullify.** Nullify is the composition stated below, with three preconditions: **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition); **P1**: `a ∈ A_rel^Σ`; **PC**: Σ →*-reachable. P0 governs execution; P1 and PC condition the single-tuple-scope postcondition R-Scope. (Under PC and P0, R0 — applied at the caller-supplied home `d_retr` — guarantees the internal `Emit_R`'s emission lands on a genuine chain sibling of `A_L(d_retr)` via its *on-chain admissibility* postcondition.)
...
**Definition — relational layer.** The relational layer's operations are `{Emit_K, Observe_K, Nullify}`. Its one discipline commitment: the layer never invokes `Emit_K` at a type index `K ~ R` except through the `Nullify` alias — at either of `Nullify`'s two branches, a P1 target (`a ∈ A_rel^Σ`) or the self-emit branch (`a = a_emit(Σ, d)`, hence `a ∉ A_rel^Σ`). Both branches qualify because `Nullify` emits only unit-depth retraction to-spans: the P1 branch targets a pre-existing `a ∈ A_rel^Σ`, and the self-emit branch deposits its retractor at `a`, so `a ∈ A_rel^{Σ'}` and the resulting tuple is unit-depth-disciplined when its target is evaluated at the post-state (the Membership clause of *Definition — Unit-depth retraction discipline* evaluates `t ∈ A_rel^Σ` at the state in question, not at the producing call's pre-state). The "at a P1 target" qualifier alone would be stronger than the discipline requires. With both branches admitted, the layer satisfies the *unit-depth retraction discipline* (Definition — Unit-depth retraction discipline).
...
*Step 4 — a self-nullifying emission verifies the wp Case 2 false branch.* The wp of Case 2 (`wp(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'})`) collapses to false exactly when an `Emit_K` call at a type `K ~ R` lands its own deterministic emission address in its to-coverage; the Result's load-bearingness argument asserts this abstractly, and we now instantiate it. The fresh emission address at `d` in Σ_3 is `a_emit(Σ_3, d)`: the subsequent-emission branch fires (`{ℓ' ∈ dom(Σ_3.L) : origin(ℓ') = d} = {a₁, b₁, a₂, b₂} ≠ ∅`), `ℓ_prev := max{a₁, b₁, a₂, b₂} = b₂` (shared prefix `1.0.1.0.1.0.2`, last components `1 < 2 < 3 < 4`), so `a_emit(Σ_3, d) = inc(b₂, 0) = 1.0.1.0.1.0.2.5`. Set `a₃ = 1.0.1.0.1.0.2.5` — `A_L(d)`'s fifth chain element, with `#a₃ = 8`. Because `a₃` is caller-computable from `(Σ_3, d)` *before* the emission commits, a caller can target it: `Σ_3 → Σ_4` via `Emit_R(Σ_3, d, ∅, {(a₃, δ(1, 8))})` — i.e. `Emit_K` at `K := R`, `F := ∅`, `G := {(a₃, δ(1, #a₃))}`, the to-set rooted at the call's own emission address. (This is the *self-emit branch* of `Nullify`: with `#a₃ = 8`, `Emit_R(Σ_3, d, ∅, {(a₃, δ(1, 8))}) = Nullify(Σ_3, d, a₃)` by Definition — Nullify. The step is therefore a layer operation, not a direct substrate caller: it is exactly the self-emit branch the relational-layer commitment admits (Definition — relational layer), and it maintains the unit-depth retraction discipline because the deposited retractor at `a₃` lands in `A_rel^{Σ_4}`. Only P0 (`d ∈ dom(Σ_3.M)`) governs execution; P1 (`a ∈ A_rel^Σ`) conditions the R-Scope postcondition, not admissibility. Here P0 holds while P1 is false (`a₃ ∉ dom(Σ_3.L)`), so the call executes as the self-emit instance of wp Case 1, with `a₃ = a_emit(Σ_3, d)`. Σ_3 lies within the wp's domain by its own pre-state properties: Σ_3 is `→*`-reachable (built by a chain of `→`-steps), and its pre-existing retraction tuples `L_R^{Σ_3}` target live links `a₁, b₁` via unit-depth to-spans (hence unit-depth-disciplined). The new call's to-span shape is irrelevant to pre-state domain membership.)
...
| R0 | LEMMA | TupleAddressFreshness — over →*-reachable Σ, for every caller-supplied home `d ∈ dom(Σ.M)`, emission allocates an address with two explicit postconditions — *fresh* against `dom(Σ.L)` and *on-chain* in `A_L(d)`, homed at `d` — in both the first- and subsequent-emission branches, yielding a →*-reachable post-state Σ' |
