...
`(A Σ : dom(Σ.M) ≠ ∅ :: (A d ∈ dom(Σ.M), F, G ∈ Endset, K ∈ T_admissible :: (E Σ' reached by one →-step from Σ, a : a ∉ dom(Σ.L) :: a ∈ dom(Σ'.L) ∧ Σ'.L(a) = (F, G, K))))`
...
- *(4) First/subsequent emission rule selects `a_k`*: ASN-0093's K.λ contract deterministically selects the next link address for home `d_k` based on the predicate `{ℓ' ∈ dom(Σ_{prev}'.L) : origin(ℓ') = d_k} = ∅`. We must show that the rule, applied at `Σ_{prev}'`, produces precisely `a_k`. The argument has three parts:
  ...
  - *(iii) Iteration in chain-order at each home selects `a_k`.* Re-order the Δ-enumeration so that fresh addresses homed at the same `d_k` appear in chain-order from least to greatest chain index. The re-enumeration is permissible by (ii) since Δ is finite, and within-home chain-order is well-defined as a total order on each home's fresh addresses: by R0a-Cor1 at Σ' (a consequence of (i)) each homed-set is a contiguous initial segment of `A_L(d_k)`'s chain enumeration, and by ChainEnumerationInjectivity (ASN-0093) each address corresponds to a *unique* chain index — the chain-index-to-address map is injective, so distinct addresses at the same home occupy distinct chain indices and within-home ordering by chain index is total. Under this ordering, each `a_k` is the chain element at the next available chain index past the prior realized prefix at the same home; K.λ's first/subsequent rule, evaluated against the origin-scoped homed-set at `Σ_{prev}'`, produces exactly this chain element. This is the load-bearing step relying on catalog (b) being strictly stronger than catalog (a). (Remark: catalog (a) alone is insufficient here — the tumbler `a* = [d.0.s_L.1.1]` is T10a-conforming and L-invariant-admissible (reachable by an L1c-admissible chain `d → d.0.1 → d.0.2 → d.0.2.1 → d.0.2.1.1`, satisfying L0/L1/L1a/L1b/L1c) yet is not a K.λ chain emission — it lies off `A_L(d)`'s sibling-frontier chain, whose outputs have `#E = 2`, not `a*`'s `#E = 3` (R0a-Cor2) — so without catalog (b) it would have no K.λ-replay; catalog (b) excludes it.) The discharge of "first occurrence at `d_k`" splits by whether `d_k` was introduced by a K.σ-prefix at this iteration or was already in `dom(Σ.M)`; "subsequent occurrence at `d_k`" reduces to a single chain-extension step common to both cases.
...
*Auxiliary pre-step.* The K.σ-then-K.λ first-emission setup (a fresh document allocated by K.σ, then its first link emitted by K.λ, with R0a-Cor1's `J_d^Σ` transitioning from `-1` to `0`) is exactly R7a's Worked Example 1 (CreateDocAndLink); see there. Steps 0–3 below operate at the pre-existing `d` (allocated at or before `Σ_{-1}` per the Setup), exercising subsequent-emission cases only.
