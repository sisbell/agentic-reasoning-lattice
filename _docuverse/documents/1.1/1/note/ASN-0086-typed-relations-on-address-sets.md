...
*Proof.* We record the `→*`-reachable case as a foundation fact, then extend it; the extension is L-ContiguousPrefix's sole added content over the foundation.
...
*Proof.* The pre-state Σ is substrate-conforming by hypothesis. By the *Definition — substrate-conforming layer*, every operation the layer publishes carries substrate-conforming states to substrate-conforming states; applying this preservation property to the `Σ ↝ Σ'` step in scope, the post-state Σ' is itself substrate-conforming. By substrate-conformance, every `Σ ↝ Σ'` in scope preserves L12, L12a, L-fin, S0, and S1 at each step — the facts the decomposition consumes. The proof has two structural phases: a monotonicity argument establishing the Δ-enumeration of fresh link addresses, and a K.σ/K.λ replay reconstructing them as `→`-steps.
