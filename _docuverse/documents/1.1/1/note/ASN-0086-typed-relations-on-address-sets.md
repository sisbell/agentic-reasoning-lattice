...
*Proof.* Let `↝_c` denote the *conformance-preserving sub-relation* of `↝` — the set of `↝`-steps satisfying clauses (a)–(c) of *Definition — substrate-conforming state* — and `↝_c*` its reflexive-transitive closure. By that definition, Σ is substrate-conforming iff `Σ_init ↝_c* Σ`; this is the witnessing closure, distinct from `→*` (which is generated by K-ops only and yields a strictly smaller set, per *Definition — state-local-conforming state*). Substrate-conformance of Σ is thus witnessed by a trajectory `Σ_init ↝_c* Σ`; appending the conformance-preserving step Σ → Σ' (resp. Σ ↝ Σ'), which is by hypothesis an `↝_c`-step, extends that trajectory to `Σ_init ↝_c* Σ'`, so Σ' too is substrate-conforming. The K-op claim holds by the lemma statement's cited ASN-0093 contracts. ∎
...
**Definition — Nullify.** Nullify has a single *execution precondition* — **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition). Whenever P0 holds, Nullify executes and produces a post-state Σ'. The roles of the two further conditions — **P1**: `a ∈ A_rel^Σ` and **PC**: Σ substrate-conforming — are not execution gates but conditions on the single-tuple-scope postcondition R-Scope; the precise P0-gate / (P1,PC)-scope distinction, and the load-bearingness of each, are derived once at the wp Case 1 analysis (Weakest-Precondition Analysis — *Case 1*).
