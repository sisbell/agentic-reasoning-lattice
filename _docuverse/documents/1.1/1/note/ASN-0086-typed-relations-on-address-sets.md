...
*Proof.* Appending one conformance-preserving step to `{Σ_init}`'s closure under such steps stays within the closure (Definition — substrate-conforming state), and induction lifts this to any conformance-preserving trajectory. The K-op claim we discharge by clause across all three K-ops. Clause (a) holds for every K.σ/K.α/K.λ `→`-step by its ASN-0093 invariant-preservation contract. Clauses (b) and (c) constrain *fresh link-key* emission, so they split by whether the op touches `dom(Σ.L)`. K.σ (document registration) and K.α (content allocation) add no link key — by their Frames `Σ'.L = Σ.L` — so the "at most one fresh link key per home" obligation (b) and the "frontier-landing" obligation (c) are met *vacuously*: there is no fresh link key to count or to place. For K.λ, (b) holds because it emits a single key at one home; and (c) holds because K.λ's first/subsequent emission rule deposits exactly `[d.0.s_L.1]` (empty homed-set) or `inc(ℓ_prev, 0)` (the prior T1-maximum), which is precisely the frontier-landing key. Since `Σ_init` is substrate-conforming, every `→*`-reachable state is substrate-conforming; the converse fails (Remark — NestedLinkWitness). ∎

**Remark — NestedLinkWitness.** A higher layer may emit `a'' = inc(a, 1)` at the same home as an existing link address `a`: the `k = 1` step appends `[1]`, preserving `zeros = 3` (L1) and giving `#E(a'') = #E(a) + 1 ≥ 2` (L1b), so the nested pair `a ≼ a''` (with `a'' ≠ a`) has both addresses L0/L1/L1a/L1b/L1c-conforming, with no S-invariant disturbed. The resulting state therefore preserves the entire state-local L/S-invariant catalog (catalogued in the next definition) yet contains a nested, non-frontier link pair: it violates the tumbler-prefix antichain that R0a establishes for substrate-conforming states (clause (c)'s frontier-landing requirement fails — its `a''` is not the frontier successor `inc(ℓ_prev, 0)`). The state-local-conforming sub-space thus strictly contains the substrate-conforming one.
...
The max is the unique T1-extremum of a finite (L-fin, ASN-0043) non-empty set, by T1 (LexicographicOrder, ASN-0034) trichotomy. Both branches evaluate from `(Σ, d)` alone, so `a_emit` is a *total* function of `(Σ, d ∈ dom(Σ.M))`. It is a formula, not a commitment: `a_emit` carries no claim that K.λ deposits its value. K.λ commits `a_emit(Σ, d)` only when the chain frontier at `d` is well-formed (Definition — Emit_K); at a non-frontier state (Remark — NestedLinkWitness) the formula still yields a value but no legitimate K.λ-edge exists there, so the two are co-extensive precisely on the frontier-well-formed states.
...
*Proof.* The statement holds at every substrate-conforming state. Every `→*`-reachable state is reached by K-op `→`-steps, each of which is conformance-preserving (K-Step Conformance Preservation), so the `→*`-reachable states are a subset of the substrate-conforming states; a single induction over conformance-witnessing transitions therefore covers them and the `↝*`-reachable-but-not-`→*`-reachable states (reached by conformance-preserving `↝`-steps that are not K-op `→`-steps, per Definition — substrate-conforming state) uniformly. The one foundational fact the induction imports is the origin/home coincidence: under ASN-0036, `origin(a)` and `home(a)` agree on every `a ∈ dom(Σ.L)`, because L1 + L1a's NUDE-prefix projection is exactly `origin(·) = N(·).0.U(·).0.D(·)`. This identifies the homed-set `{a ∈ dom(Σ.L) : home(a) = d}` with the origin-indexed chain `A_L(d)` (anchor `[d.0.s_L.1]` by FirstEmission, ASN-0093; sibling recurrence `inc(·, 0)`), so the chain index used below is well-defined.

Write `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` and induct on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ`. *Base:* By the EmptyInitialLinkStore assumption (Foundation, above), `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J_d^{Σ_init} = -1`. *Step:* assume `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` is a contiguous initial segment for every `d` (the IH), with `J = J_d^{Σ_k}`. Fix a home `d`. By clause (b) (at-most-one-key-per-home), the step adds at most one fresh key homed at `d`. If it adds none, `H_d^{Σ_{k+1}} = H_d^{Σ_k}` and contiguity is preserved with `J_d^{Σ_{k+1}} = J`. Otherwise clause (c) (frontier-landing) places the single fresh key at chain index `J + 1` — empty homed-set → anchor `[d.0.s_L.1]`, non-empty → `inc(ℓ_prev, 0)`, which are exactly `A_L(d)`'s anchor and recurrence — so `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + 1}` with `J_d^{Σ_{k+1}} = J + 1`, again a contiguous initial segment. Since `d` was arbitrary, contiguity is preserved at every home, hence at the terminal Σ. The unique T1-maximum is derived above. ∎
...
*L-invariant preservation across the K.λ-step.* The emission above is a valid K.λ `→`-step — its on-chain gating and freshness preconditions discharged in the freshness bullets — taken from the substrate-conforming pre-state Σ. By K-Step Conformance Preservation, Σ' is therefore substrate-conforming; since substrate-conformance clause (a) *is* preservation of the full state-local L/S/M/C invariant catalog, every catalog invariant holds at the fresh key `a` by that single lemma invocation. The one obligation not closed structurally by the lemma is the match between the emitted value and K.λ's value-shape precondition: `Emit_K` supplies the triple `(F, G, K)`, a `Link`-typed value of arity 3 with non-empty type slot `K`, which satisfies K.λ's `N ≥ 3 ∧ e₃ ≠ ∅` at `N = 3`, `e₃ = K` — so the emission is a well-formed K.λ value and L3 is met. This completes the final conjunct of R0's conclusion: Σ' is substrate-conforming. ∎
...
Where Σ ranges over the state-local-conforming sub-space (Definition — state-local-conforming state). `Emit_K` is total over substrate-conforming Σ (R0) and partial over the broader state-local-conforming sub-space: it is undefined exactly where the chain frontier is ill-formed. At a state carrying a non-frontier nested link pair `a ≼ a''` at home `d` (the structural witness of Remark — NestedLinkWitness), the apparent `ℓ_prev` at `d` is the nested key, so the subsequent-emission `inc(ℓ_prev, 0)` would be off-chain — a child of the nested key rather than a chain sibling — and no legitimate K.λ-edge exists at `d`. Where defined, `Emit_K` is a function: the address is `a = a_emit(Σ, d)` (a function of `(Σ, d)` by Definition — `a_emit`), the value `Σ'.L(a) = (F, G, K)` is fixed by the caller-supplied arguments, and K.λ's Frame fixes the rest of Σ'.
...
**Definition — Nullify.** Nullify is the composition stated below, with four preconditions: **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition); **P0f**: `d_retr`'s homed link-set `{ℓ ∈ dom(Σ.L) : home(ℓ) = d_retr}` is a contiguous chain prefix of `A_L(d_retr)` (a well-formed frontier), so the internal `Emit_R`'s emission lands on a genuine chain sibling — without it the subsequent-emission `inc(ℓ_prev, 0)` may be off-chain and `Emit_R` undefined (Definition — Emit_K); over substrate-conforming Σ, P0f holds automatically at every allocated home (L-ContiguousPrefix); **P1**: `a ∈ A_rel^Σ`; **PC**: Σ substrate-conforming. P0 ∧ P0f govern execution; P1 and PC condition the single-tuple-scope postcondition R-Scope.
...
**R6d — RetractionStabilityUnderConformingLayer.** R6a and R6c extend from `→`/`→*` to the `↝`-steps of a substrate-conforming layer (Definition — substrate-conforming layer, above): for any state-affecting `Σ ↝ Σ'` issued by a substrate-conforming layer from a substrate-conforming pre-state Σ,
