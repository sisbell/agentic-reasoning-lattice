...
A gap `(c_k, c_{k+1})` is empty exactly when `c_{k+1}` is the immediate T1-successor `c_k.0` of `c_k` (the zero-extension; ASN-0034, TA5 note) — we cite that characterization rather than re-derive it — decided by the single T2 comparison `c_{k+1} = c_k.0`. Empty gaps are `∅` in both coverages and are excluded from the comparison. Both coverages lie within `[c₁, c_m)`, agreeing trivially on the exterior cells `(−∞, c₁)` and `[c_m, ∞)`. Computing each coverage's indicator over the finitely many non-empty cells and comparing the two indicator vectors decides `coverage(e) = coverage(e')`. ∎
...
**Fact — HomeOriginCoincidence.** On link addresses, `home` and `origin` coincide: both are the NUDE-prefix projection `N(·).0.U(·).0.D(·)` (ASN-0043's `Home`, ASN-0036's `origin`, ASN-0034's T4b field extraction). Consequently `{a ∈ dom(Σ.L) : home(a) = d} = dom(Σ.L) ∩ {ℓ' : origin(ℓ') = d}` for any document `d`. We cite this fact at each site below rather than re-derive it.
