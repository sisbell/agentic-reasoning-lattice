...
Both sides are evaluated at the single state Σ. The restriction `a ∈ A_rel^Σ` is carried for the reason given at Definition — Nullified.
...
*Two instances of the general construction.* When the `↝`-step is itself a single-fresh-key primitive (Frame conditions `Σ'.C = Σ.C`, `Σ'.M = Σ.M`) whose home `home(a_1)` already lies in `dom(Σ.M)`, then `Δ = {a_1}`, no K.σ-prefix fires, and the replay collapses to the length-1 sequence consisting of that single K.λ-step. When instead the `↝`-step adds one fresh document together with one link homed at it (the *single-fresh-home* case), `d_1 = home(a_1) ∉ dom(Σ.M)` triggers one K.σ-prefix discharging L1a, followed by one first-emission K.λ depositing `a_1` — the length-2 sequence. ∎
...
`wp(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) ≡ d ∈ dom(Σ.M) ∧ K ∈ T_admissible ∧ K ≁ R`   (over substrate-conforming Σ satisfying the unit-depth retraction discipline)

The conjunct `K ≁ R` is load-bearing and not a pre-state property: it constrains the *call's* type index. Without it the formula is false. Since `a = a_emit(Σ, d)` is a deterministic, caller-computable function of `(Σ, d)`, a caller invoking `Emit_K` at a type `K ~ R` with `G = {(a_emit(Σ, d), δ(1, #a_emit))}` deposits a tuple that enters `L_R^{Σ'}` with `a ∈ coverage(G)`, so `a ∈ nullified(Σ')` and `(a, F, G) ∉ A_K^{Σ'}` — while `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` both hold. This self-nullification persists even under the unit-depth discipline: a unit-depth self-targeting to-span is discipline-conforming yet still nullifies `a`. Only `K ≁ R` excludes it. The Nullify-as-sole-`R`-producer rule discharges this conjunct for the relational layer's own callers (every direct `Emit_K` has `K ≁ R`), but a direct K.λ caller must establish it explicitly.

*Domain restriction.* This wp is asserted only over pre-states Σ that are both (i) substrate-conforming and (ii) satisfy the unit-depth retraction discipline (Definition — Unit-depth retraction discipline) — equivalently, Σ reached using only the relational layer's operations. Both restrictions are load-bearing, and the `a ∉ nullified(Σ')` step of the derivation consumes both: (i) R0a's antichain — which holds only at substrate-conforming states — supplies prefix-incomparability of the fresh `a = a_emit(Σ, d)` against every link address; (ii) the discipline guarantees every pre-existing `L_R^Σ` tuple has a unit-depth to-span `{(b, δ(1, #b))}` whose coverage is the prefix-subtree `{t : b ≼ t}`, which that prefix-incomparability then excludes. Neither restriction alone suffices, and the two failure modes are distinct.

*Substrate-conformance alone is insufficient (discipline necessary).* K.λ constrains every emission *address* to the sibling frontier but leaves a link's endset *shape* unconstrained, so a substrate-conforming Σ may still carry a crafted (non-unit-depth) retraction span emitted by a direct K.λ caller (Definition — Unit-depth retraction discipline: such a retraction "is L-invariant-conforming yet violates it"). A crafted span `(s, ℓ)` has coverage the lexicographic interval `{t : s ≤ t < s ⊕ ℓ}` (T12, SpanWellDefinedness, ASN-0034), *not* a prefix-subtree, so prefix-incomparability of the fresh `a` against `s` does not exclude `a`: a wide span rooted near `d`'s link chain can satisfy `s ≤ a < s ⊕ ℓ`, putting `a ∈ coverage(G')` and hence `a ∈ nullified(Σ')`, while `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` both hold. This is exactly the case the unit-depth discipline excludes.
...
*Derivation.* `d ∈ dom(Σ.M)` and `K ∈ T_admissible` are Emit_K's own preconditions, each load-bearing: dropping the first leaves K.λ's home-precondition undischarged, so no post-state Σ' is produced; dropping the second admits `K = ∅` (the sole non-admissible endset), violating K.λ's `e₃ ≠ ∅` precondition (L3), so again no Σ' exists. It remains to show `a ∉ nullified(Σ')`. The domain restriction supplies, via (i), substrate-conformance of Σ, so R0a's antichain holds at Σ; the conformance-preserving K.λ `→`-step carries it to Σ' (Definition — substrate-conforming layer), so R0a's antichain on `dom(Σ'.L)` is available. By the unit-depth retraction discipline (domain precondition (ii)), every `L_R^Σ` tuple has a unit-depth to-span `{(b, δ(1, #b))}` with coverage `{t : b ≼ t}` for some `b ∈ A_rel^Σ`; the fresh `a = a_emit(Σ, d)` is prefix-incomparable with every such `b` by K.λ's emission rule together with R0a, so `a ∉ coverage(G')` for any pre-existing retraction `(_, _, G') ∈ L_R^Σ`, and no pre-state retraction nullifies `a`. The conjunct `K ≁ R` (the third clause of the stated precondition) gives that the fresh tuple does not enter `L_R^{Σ'}` and cannot self-nullify. Hence no `L_R^{Σ'}` tuple — pre-existing or just-emitted — has to-coverage containing `a`, so `a ∉ nullified(Σ')`; with `(a, F, G) ∈ L_K^{Σ'}` unconditional, `(a, F, G) ∈ A_K^{Σ'}`.
