...
**Definition — Unit-depth retraction discipline.** A state Σ is *unit-depth-disciplined* iff every `(b, F', G') ∈ L_R^Σ` has to-endset `G' = {(t, δ(1, #t))}` for some target `t ∈ A_rel^Σ`. Production by a *P1-satisfying* `Nullify(Σ_pre, d_retr, t)` call (one whose target satisfies P1 *against its producing pre-state*, `t ∈ A_rel^{Σ_pre}`) is *sufficient* for this per-state predicate but not equivalent to it: a P1 target stays in `A_rel` by L12a and Nullify builds a unit-depth span, so any such tuple meets the per-state shape; but the converse fails — a direct `Emit_R` whose target `t ∉ A_rel` at its *emission* pre-state (e.g. Worked Sketch Step 4, where `a₃ = a_emit(Σ_3, d) ∉ A_rel^{Σ_3}`) is not P1-satisfying, yet once `t` enters `A_rel` at the post-state the tuple satisfies the unit-depth predicate. Membership `t ∈ A_rel^Σ` in the per-state predicate is therefore evaluated at the state Σ in question, not at the producing call's pre-state. A *layer* satisfies the *unit-depth retraction discipline* iff every state it reaches is unit-depth-disciplined. A layer can guarantee the discipline by confining its `R`-typed emissions to P1-satisfying Nullify calls (the relational layer does so — Definition — relational layer); this is one sufficient strategy, not the only configuration the per-state predicate admits.
