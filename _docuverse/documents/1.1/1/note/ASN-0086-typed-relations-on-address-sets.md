...
We will show (R1) that this disjoint union is well-defined: each tuple address belongs to exactly one coverage-class slice. Where ambient state is clear we drop the superscript and write `L_K`, `L`. Coverage-equivalence at the type slot aligns `L_K` with L8's same-type relation, which also projects through coverage.
...
**Definition — Nullify.** Nullify has a single *execution precondition* — **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition). Whenever P0 holds, Nullify executes and produces a post-state Σ'; the `Emit_R` alias emits unconditionally given P0. The other two conditions are *not* execution gates — they neither validate nor abort the operation — but conditions on the *single-tuple-scope postcondition* R-Scope: **P1**: `a ∈ A_rel^Σ` (the target tuple's address is a live link, which L12a then carries to `a ∈ A_rel^{Σ'}`) and **PC**: Σ substrate-conforming (under which the internal `Emit_R` is admissible and R0a's antichain holds). Nullify still executes when P1 or PC fails; what fails is only the guarantee that the emission's nullified-scope is exactly `{a}` (R-Scope), not the operation itself. Arity `|Σ.L(a)|` likewise does not condition R-Scope: single-tuple scope is arity-independent (R-Scope).
...
| Nullify | OP | `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})`; single execution precondition P0, with P1 and substrate-conformance PC conditioning the single-tuple-scope postcondition (R-Scope), not execution; arity non-conditioning, per Definition — Nullify |
