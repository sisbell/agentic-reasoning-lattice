...
A gap `(c_k, c_{k+1})` is empty exactly when `c_{k+1}` is the zero-extension `c_k.0` of `c_k` (the tumbler of length `#c_k + 1` agreeing with `c_k` on positions `1..#c_k` and with a `0` appended), decided by the single T2 comparison `c_{k+1} = c_k.0`. We derive inline from T1 (LexicographicOrder, ASN-0034) that no tumbler lies strictly between `c_k` and `c_k.0`, so `c_k.0` is the immediate T1-successor of `c_k`. Write `t = c_k`. First, `t < t.0`: the two agree on positions `1..#t` and `#t = #(t.0) − 1 < #(t.0)`, so `t` is a proper prefix of `t.0` and T1 case (ii) gives `t < t.0`. Now take any `s` with `t < s`; we show `s ≥ t.0` by two cases on whether `s` extends `t`. If `t ≼ s` (so `s` agrees with `t` on positions `1..#t`): at position `#t + 1`, either `s` has length `#t` — impossible, as `t ≼ s` with `t < s` forces `#t < #s` — or `s_{#t+1} ≥ 0 = (t.0)_{#t+1}`; if `s_{#t+1} > 0` then `s` and `t.0` first diverge at `#t + 1` with `s_{#t+1} > (t.0)_{#t+1}`, giving `s > t.0` by T1 case (i), and if `s_{#t+1} = 0` then `t.0 ≼ s` so `s ≥ t.0` (equality when `#s = #t + 1`, else case (ii)). If `t ⋠ s`, then since `t < s` the pair first diverges at some `k ≤ #t` with `s_k > t_k`; because `(t.0)_k = t_k` for `k ≤ #t` and `s` agrees with both `t` and `t.0` on positions `1..k−1`, the pair `(t.0, s)` first diverges at the same `k ≤ #t < #(t.0)` with `s_k > (t.0)_k`, giving `s > t.0` by T1 case (i). In every case `t < s ⟹ s ≥ t.0`, so the open interval `(t, t.0)` is empty; conversely if `c_{k+1} ≠ c_k.0`, then `c_k.0` itself witnesses a non-empty gap: `c_k < c_k.0` was shown above, and applying the result `t < s ⟹ s ≥ t.0` with `s := c_{k+1} > c_k` gives `c_{k+1} ≥ c_k.0`, which with `c_{k+1} ≠ c_k.0` yields `c_k.0 < c_{k+1}`, so `c_k < c_k.0 < c_{k+1}`. Empty gaps are `∅` in both coverages and are excluded from the comparison. Both coverages lie within `[c₁, c_m)`, agreeing trivially on the exterior cells `(−∞, c₁)` and `[c_m, ∞)`. Computing each coverage's indicator over the finitely many non-empty cells and comparing the two indicator vectors decides `coverage(e) = coverage(e')`. ∎
...
The two address conditions `a ∉ dom(Σ.L)` (*freshness*) and `a ∈ A_L(d)` (*on-chain admissibility*) are explicit postconditions of the lemma, established below for the caller-chosen `d` in both the first- and subsequent-emission branches. The earlier outer condition `dom(Σ.M) ≠ ∅` is subsumed: when `dom(Σ.M) = ∅` the universal over `d` is vacuous.
...
*Precondition.* `K ∈ T_admissible`. R0's home precondition `d ∈ dom(Σ.M)` is enforced by parameter typing: the `d ∈ dom(Σ.M)` argument is exactly the home R0 quantifies over, and cannot be supplied unless the document-allocation domain is non-empty.
