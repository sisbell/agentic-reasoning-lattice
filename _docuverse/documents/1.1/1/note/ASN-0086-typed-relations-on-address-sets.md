...
**Definition — Nullify.** Nullify has two *execution preconditions* — **P0**: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition), and **P0f**: `d_retr`'s homed link-set `{ℓ ∈ dom(Σ.L) : home(ℓ) = d_retr}` is a contiguous chain prefix of `A_L(d_retr)` (a well-formed frontier), so the internal `Emit_R`'s emission lands on a genuine chain sibling and the K.λ-edge at `d_retr` exists. P0 alone does *not* guarantee execution over the state-local-conforming sub-space: by Definition — Emit_K, an off-chain nested key at `d_retr` can leave `inc(ℓ_prev, 0)` undefined even when `d_retr ∈ dom(Σ.M)`, so P0f is an independent gate. Over substrate-conforming Σ, P0f holds automatically at every allocated home (L-ContiguousPrefix), so P0 suffices there. Whenever P0 ∧ P0f holds, Nullify executes and produces a post-state Σ'. The two further conditions — **P1**: `a ∈ A_rel^Σ` and **PC**: Σ substrate-conforming — are *not* execution gates; they condition the single-tuple-scope postcondition R-Scope: P1 places the target in the active relation so the retraction lands on a real tuple, and PC supplies the antichain R-Scope reads off. Execution is governed by P0 ∧ P0f.
