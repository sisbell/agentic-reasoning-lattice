...
Where Σ ranges over the state-local-conforming sub-space (Definition — state-local-conforming state). `Emit_K` is total over substrate-conforming Σ (R0) and partial over the broader state-local-conforming sub-space. The partiality is located by K.λ's "produced by `A_L(d)`" gate, not by `a_emit`, which is itself total (Definition — `a_emit`): K.λ accepts `a_emit(Σ, d)` only when that output is the genuine frontier successor of `d`'s homed-set — i.e. when `inc(ℓ_prev, 0)` lands on a chain sibling of `A_L(d)`. This on-chain gate succeeds iff `d`'s homed-set `{ℓ ∈ dom(Σ.L) : home(ℓ) = d}` is a contiguous chain prefix of `A_L(d)` — the P0f condition (Definition — Nullify). *(Defined direction.)* When the homed-set is a contiguous chain prefix, `ℓ_prev = max{ℓ : home(ℓ) = d}` is the prefix's frontier element, `a_emit(Σ, d) = inc(ℓ_prev, 0)` is the next element of `A_L(d)`, K.λ's gate accepts, and `Emit_K` is defined. *(Undefined direction.)* When the homed-set is not a contiguous chain prefix — when it carries a nested, non-frontier address, as in the NestedLinkWitness construction (Remark — NestedLinkWitness) — `ℓ_prev` is that off-frontier T1-maximum, `inc(ℓ_prev, 0)` is not an element of `A_L(d)`, the gate rejects, and `Emit_K` is undefined. The two directions together give: `Emit_K` is undefined *exactly* where the chain frontier is ill-formed. Where defined, `Emit_K` is a function: the address is `a = a_emit(Σ, d)` (a function of `(Σ, d)` by Definition — `a_emit`), the value `Σ'.L(a) = (F, G, K)` is fixed by the caller-supplied arguments, and K.λ's Frame fixes the rest of Σ'.
