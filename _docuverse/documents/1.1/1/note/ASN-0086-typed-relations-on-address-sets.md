...
**Definition — substrate-conforming state.** A state Σ is *substrate-conforming* iff it is reachable from `Σ_init` by transitions that each (a) preserve the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and (b) emit every fresh link key at its home document's sibling frontier (the ASN-0093 chain discipline). Two state-families qualify, and the antichain/contiguity lemmas below are quantified over their union rather than over `→*`-reachability alone. First, every `→*`-reachable state is substrate-conforming: the K-op primitives K.σ/K.α/K.λ satisfy (a) and (b) by their ASN-0093 contracts. Second, every `↝`-reachable state produced by a substrate-conforming *layer* (Definition — substrate-conforming layer, below) is substrate-conforming, because that Definition's clauses (a)/(b) are exactly the two conditions named here. R0a, R0a-Cor1, and R0a-Cor2 are stated over substrate-conforming states; this brings their `↝`-reachable consumers — Nullify's single-tuple scope, R7a's replay discharge, and the weakest-precondition analysis, all of which evaluate these lemmas at layer-operation post-states — into scope uniformly, with clause (b) (not `→*`-reachability) as the stated hypothesis the proofs consume.
...
**R6b — SingleDepthRetraction (Consequence of Definition `nullified`; cross-state persistence via R3).** Retraction-of-retraction is not a fixpoint operation: an `Emit_R` call whose to-coverage targets a retractor `b` does not "undo" `b`'s nullifying effect on its prior targets. The claim has two halves. (i) *Within-state flatness* (definitional): the decision procedure for `a ∈ nullified(Σ)` is *flat* — a single set-membership test independent of any retraction-chain depth in `L_R^Σ`, and unaffected by whether any witnessing retractor `b` is itself nullified. (ii) *Cross-state persistence of effect* (via R3): emitting `Nullify(b)` across `Σ → Σ'` does not remove `b`'s nullifying effect on its prior targets.
...
where P0: `d_retr ∈ dom(Σ.M)` and P1: `a ∈ A_rel^Σ`. *Sufficiency:* P1 combined with L12a discharges `a ∈ A_rel^{Σ'}`, and the only other requirement — that no other link address fall within `{t : a ≼ t}` at Σ' — is `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}`, a purely *address-prefix* property of `a` against `dom(Σ'.L)`. It is discharged by R0a's antichain — applicable because the Nullify call's pre-state Σ and post-state Σ' are both substrate-conforming states — on `dom(Σ.L)` together with `a ∈ A_rel^{Σ'}`: the internal Emit_R's fresh emitter `b` is prefix-incomparable with `a` at Σ' (`b ∉ {t : a ≼ t}`) by R0a at Σ' applied to `dom(Σ'.L)`.
...
Single-tuple scope is therefore *arity-independent*: nullifying an arity-4 address would establish `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}` identically, since the argument consults only `a`'s tumbler prefix and the antichain, never `|Σ.L(a)|`. The wp lists only P0 and P1 — Nullify's two *executing* preconditions (Definition of Nullify) — because it ranges over the conditions whose violation aborts the underlying Emit_R and leaves the postcondition unreachable; the scope condition P2 (`|Σ.L(a)| = 3`) does not gate emission and so does not appear.
