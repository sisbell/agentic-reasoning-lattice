...
- *Subsequent emission* (predicate `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} ≠ ∅` fires): `a = inc(ℓ_prev, 0)` where `ℓ_prev := max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}`. By ChainEnumerationInjectivity (ASN-0093) and ChainMembershipForOrigin (ASN-0093), `ℓ_prev` is the maximum index of the contiguous prefix of `A_L(d)`'s realized chain, and `inc(ℓ_prev, 0)` is the next chain element of `A_L(d)`. By ChainUniformLength and ChainUniformZeroCount, the result has the same length and zero count as `ℓ_prev` (which carries `zeros = 3` directly by L1 (LinkElementLevel, ASN-0043) as a substrate-level invariant at the reachable state Σ — R0 is a per-step argument at any reachable conforming Σ, not an inductive proof, so L1 supplies `zeros(ℓ_prev) = 3` directly rather than through an inductive hypothesis); by ChainPrefixExtension, `b_L(d) ≼ a`, so `origin(a) = d`. Freshness of `a` against `dom(Σ.L) ∪ dom(Σ.C)` decomposes into three sub-claims:
...
*(Step 1 — Σ.L pointwise-constancy under arrangement-modifying steps.)* By the Definitions of TypedRelation (`L_K^Σ` depends only on `Σ.L`), Nullified (`nullified(Σ)` is determined by `L_R^Σ`, itself a slice of `Σ.L`), and ActiveSubset (`A_K^Σ = L_K^Σ \ {(a, F, G) : a ∈ nullified(Σ)}`), the four quantities `L_K^Σ`, `L_R^Σ`, `nullified(Σ)`, `A_K^Σ` depend on `Σ` only through `Σ.L`. By the arrangement-modification frame on `↦`-steps in `↦ \ →` established in the Definition of `↦` (above) — combining (i) ASN-0043's L12 (LinkImmutability) value-preservation, (ii) ASN-0043's L12a (LinkStoreMonotonicity) no-removal, and (iii) the partition's restriction of link-store extension to K.λ-steps — every arrangement-modifying step `Σ_k ↦ Σ_{k+1}` in `↦ \ →` holds `Σ_{k+1}.L = Σ_k.L` pointwise; consequently `A_K^{Σ_{k+1}} = A_K^{Σ_k}`, and the same equality holds for `L_K`, `L_R`, and `nullified`.
...
*Per-step substrate-invariant discharge.* Discharges (1)–(4) above cover each replay step's *preconditions*. We now enumerate, per replay step type, how the full substrate-invariant catalog is *preserved* at the post-state, grouped by mechanism. The four groups jointly exhaust the catalog of ASN-0036 S-invariants, ASN-0043 L-invariants, and ASN-0093 M-/C-invariants enumerated in the Definition of *substrate-conforming layer*.

*(α) K.σ-prefix step `Σ_{prev} → Σ_{prev}'` (extends `dom(M)` with `d_k`; Frame fixes `Σ_{prev}'.L = Σ_{prev}.L` and `Σ_{prev}'.C = Σ_{prev}.C`).*

  - *Discharged at the new key `d_k` by structural commitments:* M0 (DocumentTumblerWellFormed, ASN-0093) and S7d (DocumentAllocationDiscipline, ASN-0036) — both follow from the K.σ-prefix's structural-precondition discharge `T4-valid(d_k) ∧ zeros(d_k) = 2`, inherited from L1a at `a_k` evaluated at Σ' (Σ' is a reachable conforming state, so `d_k = home(a_k) ∈ dom(Σ'.M)` satisfies M0 and S7d at Σ', and these are state-independent structural predicates over `d_k`).
  - *Established by extension and vacuity:* M1 (ArrangementMonotonicity, ASN-0093) by `dom(Σ_{prev}'.M) ⊋ dom(Σ_{prev}.M)`; S2 (ArrangementFunctionality, ASN-0036), S3 (ReferentialIntegrity, ASN-0036), S8-fin, S8a, S8-depth, D-CTG, D-MIN, D-SEQ (ASN-0036 arrangement invariants) — all hold at the new key `d_k` vacuously since `M(d_k) = ∅` (empty arrangement satisfies every positional / functional / cardinality predicate vacuously); preserved at existing keys via the next bullet.
  - *Preserved at existing keys by Frame on `(Σ.C, Σ.L)`:* S0 (ContentImmutability), S1 (StoreMonotonicity), S7a–c (content-attribution invariants), S2/S3/S8-fin/S8a/S8-depth/D-CTG/D-MIN/D-SEQ at existing `d ∈ dom(Σ_{prev}.M)` (ASN-0036); C0, C1, C1b, C1c, C-fin (ASN-0093 content-store invariants); L0, L1, L1a, L1b, L1c, L2, L3, L5, L6, L8, L11a, L12, L12a, L12b, L13, L-fin (ASN-0043 link-store invariants). K.σ's Frame fixes both `Σ.C` and `Σ.L` pointwise, so every invariant over these two stores transfers without change.
  - *Preserved by structural property of the K.σ step:* L14 (StoreDisjointness, ASN-0043; equivalently ASN-0093 L14 / DualPrimitive) — K.σ extends only `dom(M)`, leaving `dom(C) ∩ dom(L) = ∅` from the prior state untouched. L14a (NonTranscludability, ASN-0043) — the new arrangement `M(d_k) = ∅` contributes nothing to `ran(M)`, so `ran(Σ_{prev}'.M) = ran(Σ_{prev}.M)` and `ran(M) ∩ dom(L) = ∅` transfers.

*(β) K.λ-emission step `Σ_{prev}' → Σ_k` (extends `dom(L)` with `a_k`; Frame fixes `Σ_k.M = Σ_{prev}'.M` and `Σ_k.C = Σ_{prev}'.C`).*

  - *Discharged at the new key `a_k`:* L0, L1, L1b — state-independent structural predicates over `a_k` (`E(a_k)₁ = s_L`, `zeros(a_k) = 3`, `#E(a_k) ≥ 2`), inherited from Σ' per discharge (2) above. L1a — `home(a_k) = d_k ∈ dom(Σ_{prev}'.M)` per discharge (2/3) above. L1c — chain admissibility witnessed by SubAllocatorAxiom.ChainDiscipline + ChainElementT4Validity (ASN-0093) at `A_L(d_k)`'s chain, whose existence is axiomatized by ASN-0093 and not re-established at the emission step. L3 — value well-formedness `(F_k, G_k, K_k)` is L3-conforming, carried by R2's value-preservation argument from Σ' (where Σ'.L(a_k) was L3-conforming as a reachable substrate state).
  - *Established by extension at `a_k`:* L12a (LinkStoreMonotonicity) by `dom(Σ_k.L) ⊋ dom(Σ_{prev}'.L)`; L11a (LinkUniqueness) at the new pair — existing pairwise uniqueness preserved by L12 below, freshness of `a_k` against `dom(Σ_{prev}'.L)` (discharge (1) above) supplies uniqueness at the new key; L12b (HomeDocumentPersistence) — `home(a_k) = d_k ∈ dom(Σ_{prev}'.M) = dom(Σ_k.M)` by Frame; L-fin (LinkStoreFiniteness) — finite + single new element = finite.
  - *Preserved at existing keys by Frame on `(Σ.C, Σ.M)` and L12 on `Σ.L`:* S0, S1, S2, S3, S7a–d, S8-fin, S8a, S8-depth, D-CTG, D-MIN, D-SEQ (ASN-0036); M0, M1, C0, C1, C1b, C1c, C-fin (ASN-0093) — K.λ's Frame fixes both `Σ.C` and `Σ.M` pointwise, so every C-/M-invariant transfers. L12 itself fixes `Σ_k.L|_{dom(Σ_{prev}'.L)} = Σ_{prev}'.L` pointwise, so every L-invariant over existing keys (L0, L1, L1a, L1b, L1c, L2, L3, L5, L6, L8, L11a, L12b, L13) transfers.
  - *Preserved by SC-NEQ at the new key:* L14 (StoreDisjointness) — `a_k` has `E(a_k)₁ = s_L` by discharge (2); every content address has `E(·)₁ = s_C` by ASN-0093 L0; `s_L ≠ s_C` by SC-NEQ; therefore `{a_k} ∩ dom(C) = ∅` and L14 is preserved at the new key. L14a (NonTranscludability) — by S3 (ReferentialIntegrity), `ran(M) ⊆ dom(C)`; every element of `ran(M)` has `E(·)₁ = s_C`; by SC-NEQ, `a_k ∉ ran(M)`, so the new key does not enter `ran(M) ∩ dom(L)` and L14a is preserved.

*L-permissions (not invariants requiring preservation).* L4(c), L7, L9, L10, L11b are permissions (per ASN-0043), not invariants — they license certain endset-content or non-injectivity patterns but do not constrain state-bound values. No preservation obligation arises across either step type; they are listed here for completeness against the substrate-conforming catalog.
...
*Step 3 — Retracting the retractor exhibits R6b's non-fixpoint semantics.* `Σ_2 → Σ_3` via `Nullify(Σ_2, d, b₁) = Emit_R(Σ_2, d, ∅, {(b₁, δ(1, 8))})` — a retraction whose to-set targets the retractor `b₁` itself. Emit_R invokes K.λ at home `d`. The first/subsequent emission predicate fires *subsequent* (since `{ℓ' ∈ dom(Σ_2.L) : origin(ℓ') = d} = {a₁, b₁, a₂} ≠ ∅`); `ℓ_prev := max{a₁, b₁, a₂} = a₂` (by T1 lex order on the shared prefix `1.0.1.0.1.0.2` with last components `1 < 2 < 3`); K.λ deposits at `inc(a₂, 0) = 1.0.1.0.1.0.2.4`. Set `c₁ = 1.0.1.0.1.0.2.4` — `A_L(d)`'s fourth chain element, fresh against `dom(Σ_2.L)` by ChainEnumerationInjectivity. L-invariants at `c₁` discharge by R0's generic argument with the concrete `c₁` substituted (L0: `E(c₁)₁ = 2 = s_L`; L1: `zeros(c₁) = 3` by ChainUniformZeroCount; L1a: `home(c₁) = d`; L1b: `#E(c₁) = 2` by ChainUniformLength; L1c: the structural chain extends one step further by SubAllocatorAxiom).

Then `Σ_3.L = Σ_2.L ∪ {c₁ ↦ (∅, {(b₁, δ(1, 8))}, R)}` and:

- `L_K^{Σ_3} = {(a₁, F₁, G₁), (a₂, F₁, G₁)}` — unchanged from Σ_2; the new emission targets `L_R`, not `L_K`. Witnesses *R3*. ✓
- `L_R^{Σ_3} = {(b₁, ∅, {(a₁, δ(1, 8))}), (c₁, ∅, {(b₁, δ(1, 8))})}` — both retraction tuples persist (*R3* applied to the `R` coverage class; *R2* preserves the original retraction tuple at `b₁`). ✓
...
- *Deciding `b₁ ∈ nullified(Σ_3)`.* The new tuple `(c₁, ∅, {(b₁, δ(1, 8))})` has `coverage(G') = {t : b₁ ≼ t}`; `b₁ ∈ coverage(G')` by reflexivity. Witness found. `b₁ ∈ nullified(Σ_3)`. ✓
- Computing `nullified(Σ_3) = {a ∈ A_rel^{Σ_3} : (E witness)}`: by inspection on each member of `A_rel^{Σ_3} = {a₁, b₁, a₂, c₁}`:
  - `a₁`: witness `(b₁, …)` above ⟹ `a₁ ∈ nullified(Σ_3)`.
  - `b₁`: witness `(c₁, …)` above ⟹ `b₁ ∈ nullified(Σ_3)`.
  - `a₂`: neither retraction tuple's to-coverage contains `a₂` (a₂ ⊀ b₁ via R0a; a₂ ⊀ a₁ via R0a) ⟹ `a₂ ∉ nullified(Σ_3)`.
  - `c₁`: neither retraction tuple's to-coverage contains `c₁` (c₁ ⊀ b₁ via R0a; c₁ ⊀ a₁ via R0a) ⟹ `c₁ ∉ nullified(Σ_3)`.
...
*R0a-Cor1/Cor2 verification at Σ_2 and Σ_3.* The set of link addresses homed at `d` is `{a₁, b₁, a₂} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ 2}` at Σ_2 — a contiguous prefix of `A_L(d)`'s chain enumeration (by ASN-0093 ChainMembershipForOrigin) — so R0a-Cor1 holds at Σ_2 with `J_d^{Σ_2} = 2`. At Σ_3, the homed set extends contiguously to `{a₁, b₁, a₂, c₁} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ 3}`, so R0a-Cor1 holds at Σ_3 with `J_d^{Σ_3} = 3`. ✓ Each of `a₁ = 1.0.1.0.1.0.2.1`, `b₁ = 1.0.1.0.1.0.2.2`, `a₂ = 1.0.1.0.1.0.2.3`, `c₁ = 1.0.1.0.1.0.2.4` has element-field projection of length 2 (E = `[2, 1]`, `[2, 2]`, `[2, 3]`, `[2, 4]` respectively) by ChainUniformLength, so R0a-Cor2 holds at both Σ_2 and Σ_3. ✓
...
| R0a-Cor2 | LEMMA | DepthTwoLinkAddresses — `#E(a) = 2` strictly for every `a ∈ dom(Σ.L)`; tightens L1b's `#E ≥ 2` admission to depth-2 strictly (= R0a-Cor1 + ChainUniformLength + ChainUniformZeroCount + zero-position stability via TA5(c) + TA5-SigValid + ChainElementT4Validity (ASN-0093) — routing through T10a.4 (ASN-0034) as its underlying hook — [equivalently ChainPrefixExtension]) | introduced |
...
| R7a | LEMMA | NoExtraClassAffectsL — for any state-affecting `Σ ↝ Σ'` issued by a substrate-conforming layer with `Σ.L ≠ Σ'.L`, the `Σ.L`-affecting effect decomposes into K.λ-steps interleaved with K.σ-setup steps for L1a's home-precondition: `Σ = Σ_0 → Σ_1 → … → Σ_m` (`m ≥ 1`) with `Σ_m.L = Σ'.L`, `dom(Σ_m.M) ⊆ dom(Σ'.M)`, `dom(Σ_m.C) = dom(Σ.C) ⊆ dom(Σ'.C)`. Substrate-conformance (L12 + L12a on the link store; S0 + S1 on the content store) is lifted into R7a's claim statement. The proof's *Per-step substrate-invariant discharge* block enumerates per replay step type (K.σ-prefix or K.λ-emission) and per mechanism (precondition-discharged at the new key, established by extension, preserved at existing keys by Frame, preserved by SC-NEQ at the new key) how every entry in the substrate-conforming catalog is preserved at each step (= L12 + L12a + L-fin + L1a + S7d + ASN-0093 K-op frame conditions + ASN-0093 ChainMembershipForOrigin for replay determinism + per-step discharge of the full ASN-0036 / ASN-0043 / ASN-0093 invariant catalog) | introduced |
