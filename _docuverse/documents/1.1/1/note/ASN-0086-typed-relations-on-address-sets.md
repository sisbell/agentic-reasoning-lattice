...
**Definition — state-local-conforming state.** A state Σ is *state-local-conforming* iff it is `↝*`-reachable and preserves ASN-0043's state-local L- and S-invariant catalog (its `StateLocalInvariants` — in particular L0, L1, L1a, L1b, L1c, L3, L-fin and ASN-0036's S0–S3), but need *not* satisfy the ASN-0093 chain discipline (substrate-conformance clause (b)) or R0a's antichain. This is strictly weaker than substrate-conforming and strictly stronger than bare `↝*`-reachability. The separation is witnessed by states that preserve every state-local invariant yet violate R0a's antichain (Remark — NestedLinkWitness, below). The four-way containment is `{→*-reachable} ⊆ {substrate-conforming} ⊆ {state-local-conforming} ⊆ {↝*-reachable} = full state space`.
...
**Definition — substrate-conforming state.** A state Σ is *substrate-conforming* iff it is reachable from `Σ_init` by transitions that each (a) preserve the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and (b) emit the fresh link keys homed at each document as the next *contiguous* chain segment past that home's current sibling frontier (the ASN-0093 chain discipline). Concretely, clause (b) rests on the **at-most-one-key-per-home discipline**: every transition in this note's vocabulary deposits at most one fresh link key per home per step — each K.λ primitive emits a single key at the sibling frontier, and a composite `↝`-step may touch several homes but contributes at most one fresh key to any single home. Its **frontier-landing consequence** is the index-contiguity fact: if a step adds a fresh key at home `d` whose homed-set occupied chain indices `0..J` before the step, that key occupies exactly chain index `J+1` of `A_L(d)` — no gap, no index skipped. This discipline and its frontier-landing consequence are stated authoritatively here; downstream proofs (R0a-Cor1, R7a) invoke them by name without re-deriving the `J+1` mechanics. Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a) and (b) by their ASN-0093 contracts (each K.λ adds one key at the frontier).

**Remark — NestedLinkWitness.** A higher layer may emit `a'' = inc(a, 1)` at the same home as an existing link address `a`: the `k = 1` step appends `[1]`, preserving `zeros = 3` (L1) and giving `#E(a'') = #E(a) + 1 ≥ 2` (L1b), so the nested pair `a ≼ a''` has both addresses L0/L1/L1a/L1b/L1c-conforming. The resulting state is state-local-conforming yet violates R0a's antichain — a separating witness between the two classes, reused below wherever an antichain-violating pre-state is needed.
...
*Proof.* The contiguous-prefix form follows from conformance clause (b) alone, by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)/(b)). Write `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` for the homed-set. *Base:* By the EmptyInitialLinkStore assumption (Foundation, above), `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J_d^{Σ_init} = -1`. *Step:* assume `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` is a contiguous initial segment for every `d` (the IH), with `J = J_d^{Σ_k}`. By the at-most-one-key-per-home discipline (clause (b), Definition — substrate-conforming state), fix a home `d`. If the step adds no fresh key homed at `d`, then `H_d^{Σ_{k+1}} = H_d^{Σ_k}` and contiguity is preserved with `J_d^{Σ_{k+1}} = J`. Otherwise the step adds exactly one fresh key homed at `d`; the frontier-landing consequence (Definition — substrate-conforming state) extends the prefix by one chain index, so `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + 1}` with `J_d^{Σ_{k+1}} = J + 1`, again a contiguous initial segment. Since `d` was arbitrary, contiguity is preserved at every home. The set equality
