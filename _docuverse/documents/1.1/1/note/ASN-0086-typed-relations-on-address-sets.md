...
*Proof.* The contiguous-prefix form follows from conformance clause (b) together with the at-most-one-key-per-home auxiliary commitment (both from the Definition — substrate-conforming state), by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)/(b)). The two are jointly load-bearing: clause (b)'s frontier-landing requirement is framed per single fresh key (a key occupying chain index `J+1` past a pre-step frontier `J`), and the at-most-one commitment is precisely what guarantees a single step deposits at most one fresh key per home — making "extends the prefix by one chain index" coherent rather than ambiguous between two keys both seeing the same pre-step frontier `J`. Write `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` for the homed-set. *Base:* By the EmptyInitialLinkStore assumption (Foundation, above), `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J_d^{Σ_init} = -1`. *Step:* assume `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` is a contiguous initial segment for every `d` (the IH), with `J = J_d^{Σ_k}`. By the at-most-one-key-per-home discipline (auxiliary commitment, Definition — substrate-conforming state), fix a home `d`. If the step adds no fresh key homed at `d`, then `H_d^{Σ_{k+1}} = H_d^{Σ_k}` and contiguity is preserved with `J_d^{Σ_{k+1}} = J`. Otherwise the step adds exactly one fresh key homed at `d`; the frontier-landing requirement (clause (b), Definition — substrate-conforming state) extends the prefix by one chain index, so `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + 1}` with `J_d^{Σ_{k+1}} = J + 1`, again a contiguous initial segment. Since `d` was arbitrary, contiguity is preserved at every home. The set equality
