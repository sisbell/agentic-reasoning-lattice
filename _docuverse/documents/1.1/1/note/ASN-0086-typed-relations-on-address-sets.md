...
- *L1c (LinkAllocatorConformance):* `b₁ = inc(a₁, 0)` extends the same depth-1 allocator `A_d` (rooted at `1.0.1.0.1.0.1`) that produced `a₁`; the witness chain `(d, 2)` ⟶ sibling sweep ⟶ child-spawn `(d.0.2, 1)` from `a₁`'s construction is reused with one additional `inc(·, 0)` step. ✓
