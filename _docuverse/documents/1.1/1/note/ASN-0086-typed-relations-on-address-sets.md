...
Where Σ ranges over the substrate's full state space — every state `↝*`-reachable from `Σ_init` (Definition — Categorical reachability), which subsumes the `→*`-reachable conforming states and admits non-conforming states besides; `Emit_K` is a function over this full state space (Lemma — Emit_K function-ness, below).
...
**Definition — Nullify.** Nullify has a single *executing precondition* that gates the underlying Emit_R — (P0) `d_retr ∈ dom(Σ.M)`, the caller-supplied home document for the retraction tuple itself, which discharges K.λ's home precondition. It has two further *postcondition-establishing conditions* that do **not** gate emission: (P1) `a ∈ A_rel^Σ` — the target tuple's address — and one *scope condition* (P2) `|Σ.L(a)| = 3` — `a` is the address of a standard-triple link. Neither P1 nor P2 gates emission: the to-span `(a, δ(1, #a))` is T12-well-formed for *any* tumbler `a` (since `#a ≥ 1` by T0 and `actionPoint(δ(1, #a)) = #a ≤ #a`), independently of whether `a ∈ dom(Σ.L)` or `|Σ.L(a)| = 3`, so the underlying Emit_R executes and produces a Σ' even when `a ∉ A_rel^Σ` or `|Σ.L(a)| ≠ 3`. P1 is required for the *postcondition* `a ∈ nullified(Σ')` — specifically for `a ∈ A_rel^{Σ'}` — though not for emission to execute. P2 restricts the operation to the standard-triple addresses on which Nullify's downstream active-subset effect is meaningful (only arity-3 addresses populate some `A_K`), placing higher-arity targets outside Nullify's intended scope rather than making the operation undefined.

Under P0 (the sole executing precondition); P1 additionally establishes the `a ∈ nullified(Σ')` postcondition. Nullify is the composition
...
*Worked example — composite emitting two links at one existing home (subsequent-emission replay).* The distinct-home example above exercises only the first-emission branch (each home is fresh, so its homed-set is empty at emission). The non-trivial machinery of discharge (4)(iii) — chain-order re-enumeration with per-home `ℓ_prev` tracking — fires only when a single `↝`-step deposits *several* links at the *same* home, forcing the subsequent-emission branch (Cases B2 / "subsequent occurrence"). We trace that path here. Let `Σ` have `d ∈ dom(Σ.M)` and a homed-set at `d` already occupying chain indices `0..J` of `A_L(d)` — concretely take `J = 0`, so `dom(Σ.L) = {t_0}` with `t_0 = inc⁰(d.0.s_L.1, 0) = [d.0.s_L.1]` and `home(t_0) = d` (R0a-Cor1 at Σ gives `J_d^Σ = 0`). A higher-layer operation `EmitTwoAtSameHome` deposits, in one atomic `↝`-step, two fresh links `a_1, a_2` both homed at `d`, with `Σ'.L = Σ.L ∪ {a_1 ↦ (F_1, G_1, K_1), a_2 ↦ (F_2, G_2, K_2)}`. By clause (b), the two fresh keys homed at `d` occupy exactly chain indices `J+1, J+2 = 1, 2`: `a_1 = inc¹(d.0.s_L.1, 0) = inc(t_0, 0)` and `a_2 = inc²(d.0.s_L.1, 0) = inc(a_1, 0)`. Clause (b)'s no-gap requirement is what licenses the chain-order replay below.

R7a decomposes `Σ ↝ Σ'` into a length-2 `→`-sequence `Σ → Σ_1 → Σ_2 = Σ_m` with no K.σ-prefix (the home `d` is already allocated, so the home-precondition discharge needs no class-(i) step):

- *Step 1 (K.λ at a_1 under d):* `Σ → Σ_1` emitting `(F_1, G_1, K_1)` at `a_1`. K.λ's first/subsequent predicate at `d` evaluates `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} = {t_0} ≠ ∅`, so the *subsequent-emission* branch fires (Case B2, `J_d^Σ = 0 ≥ 0`) with `ℓ_prev = max{t_0} = t_0` (the T1-max of the pre-existing prefix by ChainEnumerationInjectivity). K.λ deposits at `inc(ℓ_prev, 0) = inc(t_0, 0) = inc¹(d.0.s_L.1, 0)` — chain index `J_d^Σ + 1 = 1`, which R0a-Cor1 at Σ' assigns to `a_1`. Hence the chain element selected equals the intended `a_1`. After Step 1: `dom(Σ_1.L) = {t_0, a_1}`.
- *Step 2 (K.λ at a_2 under d):* `Σ_1 → Σ_2` emitting `(F_2, G_2, K_2)` at `a_2`. This is the "subsequent occurrence" replay step: the prior iteration just emitted `a_1`, the immediately preceding chain element. K.λ's predicate at `d` now evaluates `{ℓ' ∈ dom(Σ_1.L) : origin(ℓ') = d} = {t_0, a_1} ≠ ∅`, the subsequent branch fires with `ℓ_prev = max{t_0, a_1} = a_1` (by T1, `a_1 > t_0` since they share prefix and `a_1` has the larger terminal component), and `inc(ℓ_prev, 0) = inc(a_1, 0) = inc²(d.0.s_L.1, 0)` — chain index 2, which R0a-Cor1 at Σ' assigns to `a_2`. The chain element selected at this replay step equals the intended `a_2`. After Step 2: `dom(Σ_2.L) = {t_0, a_1, a_2} = dom(Σ'.L)`.

The Δ-enumeration `a_1, a_2` is forced into chain-order by discharge (4)(iii): both keys home at the same `d`, so the cross-home immateriality of (4)(ii) does not apply, and the re-enumeration orders them by chain index `1 < 2`. At each replay step the subsequent-emission branch's `ℓ_prev` is the prior chain element, and `inc(ℓ_prev, 0)` lands at the next index — reconstructing `a_1` then `a_2` exactly. This is the multi-key-same-home path that the contiguous-block clause (b) (and the matching multi-key step of the ConformingHomedContiguity sub-lemma) underwrites.
...
*Domain of quantification.* This wp is computed over the *full state space* — every state at which Nullify executes. The underlying `Emit_R` runs whenever its K.λ home-precondition P0 holds; it inspects neither conformance nor `a`'s store membership, so the bare operation can execute from a non-conforming pre-state. P2c is therefore a genuine, dischargeable conjunct, not a vacuous standing invariant. The "both pre- and post-state are substrate-conforming" claim in the Definition of Nullify describes the *relational layer's usage discipline*, which guarantees P2c at every layer-issued call — not the operation's executable domain. For layer-initiated calls P2c holds for free and the effective precondition collapses to `P0 ∧ P1`; the honest wp over the full state space retains P2c, just as the Emit_K function-ness Lemma is stated over the full state space.
