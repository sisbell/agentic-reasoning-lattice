...
**Definition — relational layer.** The relational layer's operations are `{Emit_K, Observe_K, Nullify}`. Its one discipline commitment: the layer never invokes `Emit_K` at a type index `K ~ R` except through the `Nullify` alias at a P1 target (`a ∈ A_rel^Σ`). Since `Nullify` emits only unit-depth retraction to-spans, the layer satisfies the *unit-depth retraction discipline* (Definition — Unit-depth retraction discipline).
