...
**Definition — Categorical reachability.** `Σ' is ↝-reachable from Σ`, written `Σ ↝* Σ'`, is the reflexive-transitive closure of the categorical relation `↝`. The set of states `↝*`-reachable from `Σ_init` is the substrate's *full state space*: it includes every `→*`-reachable state but also states produced by higher-layer operations that need not preserve the L/S/M/C invariant catalog or the ASN-0093 chain discipline. Consequently `↝*`-reachable states may be *non-conforming*: a higher layer may, for instance, emit `a'' = inc(a, 1)` at the same home as an existing link address `a` (the `k = 1` step appends `[1]`, preserving `zeros = 3` per L1 and giving `#E(a'') = #E(a) + 1 ≥ 2` per L1b), yielding a nested pair `a ≼ a''` with both addresses L0/L1/L1a/L1b/L1c-conforming yet jointly violating R0a's antichain. The three-way containment is `{→*-reachable} ⊆ {substrate-conforming} ⊆ {↝*-reachable} = full state space`. **The bare operations `Emit_K`/`Nullify` and the weakest-precondition computations below range over the full (`↝*`-reachable) state space**, not merely the conforming sub-space.
...
**Definition — substrate-conforming state.** A state Σ is *substrate-conforming* iff it is reachable from `Σ_init` by transitions that each (a) preserve the full L/S/M/C invariant catalog (ASN-0036, ASN-0043, ASN-0093) and (b) emit the fresh link keys homed at each document as the next *contiguous* chain segment past that home's current sibling frontier (the ASN-0093 chain discipline). Concretely: this note's operations deposit at most one fresh link key per home per step, so if a step adds a fresh key at home `d` whose homed-set occupied chain indices `0..J` before the step, that key occupies exactly chain index `J+1` of `A_L(d)` — no gap, no index skipped. (A composite `↝`-step may touch several homes, but contributes at most one fresh key to any single home.) Every `→*`-reachable state is substrate-conforming, since the K-op primitives K.σ/K.α/K.λ satisfy (a) and (b) by their ASN-0093 contracts (each K.λ adds one key at the frontier).
...
- *First emission* (predicate `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} = ∅` fires): `a = [d.0.s_L.1]`. By FirstEmission (ASN-0093), this address has `E(a)₁ = s_L`, `origin(a) = d` (hence `home(a) = d`), `#E(a) = 2`, `zeros(a) = 3`, and is T4-valid by direct inspection. By ChainDiscipline + FirstEmission (ASN-0093), the link sub-allocator chain `A_L(d)` is active at every state with `d ∈ dom(Σ.M)`. We discharge freshness `a ∉ dom(Σ.L) ∪ dom(Σ.C)` directly, *without* ASN-0093's FirstEmissionFreshness — which is established only at `→*`-reachable (conforming) states — so that the argument carries over to the non-conforming `↝*`-reachable states the wp computations range over:
  ...
  These two discharge K.λ's freshness precondition `a ∉ dom(Σ.L) ∪ dom(Σ.C)` at the K.λ-event committing `a`, at every state — not only at the `→*`-reachable ones.
- *Subsequent emission* (predicate `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} ≠ ∅` fires): `a = inc(ℓ_prev, 0)` where `ℓ_prev := max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}`. The max is well-defined at *every* state in the full state space — `→*`-reachable or not — because the homed set is finite (L-fin restricted to `origin(·) = d`) and non-empty (the predicate), and every finite non-empty subset of a T1-totally-ordered set has a unique maximum by T1 (LexicographicOrder, ASN-0034) trichotomy alone; no contiguity or `→*`-reachability hypothesis is consumed. We discharge freshness `a ∉ dom(Σ.L) ∪ dom(Σ.C)` directly, *without* ASN-0093's ChainMembershipForOrigin or SubsequentEmissionFreshness — both of which hold only at `→*`-reachable states, where the realized chain is contiguous — so that the argument carries over to the non-conforming `↝*`-reachable states the wp computations range over:
  - *Well-formedness of `a`.* `ℓ_prev ∈ dom(Σ.L)` is T4-valid: the state-local invariant L1c (LinkAllocatorConformance, ASN-0043) holds at every state the substrate admits, the non-conforming `↝*`-reachable states included (a non-conforming state violates only R0a's antichain, never L1c), so `ℓ_prev` is the terminus of a T10a-conforming allocation chain and T10a.4 (ASN-0034) gives `T4-valid(ℓ_prev)`. By TA5-SigValid (ASN-0034), `sig(ℓ_prev) = #ℓ_prev`, so `inc(ℓ_prev, 0)` (TA5(c), ASN-0034) advances only the terminal component: `#a = #ℓ_prev` and `zeros(a) = zeros(ℓ_prev) = 3`, with `a` agreeing with `ℓ_prev` on positions `1..#ℓ_prev − 1`. Hence `origin(a) = origin(ℓ_prev) = d`, `E(a)₁ = E(ℓ_prev)₁ = s_L`, and `a` is itself T4-valid (TA5a at `k = 0`, ASN-0034).
  ...
  Together these four discharge K.λ's freshness precondition `a ∉ dom(Σ.L) ∪ dom(Σ.C)` at the K.λ-event committing `a`, at every state — not only at the `→*`-reachable ones.
...
*Case 2 — Same-home (`home(a) = home(a') = d`).* By R0a-Cor1 (ContiguousPrefix, stated and proved below — its argument rests on conformance clause (b) and the chain lemmas alone, not on R0a, so the forward reference is non-circular), the set `{a'' ∈ dom(Σ.L) : origin(a'') = d}` is a contiguous initial segment of `A_L(d)`'s chain enumeration `(t_1, t_2, t_3, …)` with `t_1 = [d.0.s_L.1]` and `t_{n+1} = inc(t_n, 0)`. Hence both `a` and `a'` are chain elements: `a = t_i` and `a' = t_j` for some `i, j ≥ 1`. By (UL), `#a = #t_i = #t_1 = #a'` — all chain elements have equal length. If `a ≼ a'`, then by the prefix definition (positions `1..#a` of `a'` agree with `a`) combined with `#a = #a'`, `a` and `a'` coincide pointwise, so `a = a'` by T3 (CanonicalRepresentation, ASN-0034). (Equivalently, by T10a.2 applied to the distinct siblings `a`, `a'` of `A_L(d)`.)
...
*Proof.* The contiguous-prefix form follows from conformance clause (b) alone, by induction on the conformance-witnessing transition sequence `Σ_init = Σ_0, Σ_1, …, Σ_N = Σ` (each step a transition preserving conformance clauses (a)/(b)); the argument never invokes R0a, so the forward reference in R0a Case 2 is non-circular. Write `H_d^Σ := {a ∈ dom(Σ.L) : home(a) = d}` for the homed-set. *Base:* `dom(Σ_init.L) = ∅`, so every `H_d^{Σ_init} = ∅` and contiguity holds with `J_d^{Σ_init} = -1`. *Step:* assume `H_d^{Σ_k} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J}` is a contiguous initial segment for every `d` (the IH), with `J = J_d^{Σ_k}`. Every operation in this note's transition vocabulary deposits at most one fresh link key per home per step — each K.λ primitive emits a single key at the sibling frontier, the relational layer admits no composites, and the R7a replay issues one K.λ per iteration; a composite `↝`-step may touch several homes but contributes at most one fresh key to any single `H_d`. Fix a home `d`. If the step adds no fresh key homed at `d`, then `H_d^{Σ_{k+1}} = H_d^{Σ_k}` and contiguity is preserved with `J_d^{Σ_{k+1}} = J`. Otherwise the step adds exactly one fresh key homed at `d`; by clause (b) that key occupies chain index `J+1` of `A_L(d)` — the next index past the current frontier — namely `[d.0.s_L.1] = inc⁰(d.0.s_L.1, 0)` when `J = -1`, or `inc(max H_d^{Σ_k}, 0)` when `J ≥ 0`. Hence `H_d^{Σ_{k+1}} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J + 1}`, again a contiguous initial segment, with `J_d^{Σ_{k+1}} = J + 1`. Since `d` was arbitrary, contiguity is preserved at every home. (Because the argument rests on clause (b) alone and never on ASN-0093's `→`-scoped ChainMembershipForOrigin, the conclusion holds at every substrate-conforming Σ — `→*`-reachable or `↝`-reachable conforming-layer post-state alike.) The set equality
...
**R6b — SingleDepthRetraction.** Retraction-of-retraction is not a fixpoint operation: an `Emit_R` call whose to-coverage targets a retractor `b` does not "undo" `b`'s nullifying effect on its prior targets. Deciding `a ∈ nullified(Σ)` quantifies over the audit slice `L_R^Σ`, not the active subset `A_R^Σ`, and the nullifying effect survives `→`:

`(A Σ → Σ', a, b : a ∈ nullified(Σ) witnessed by a tuple at b : a ∈ nullified(Σ') even if b ∈ nullified(Σ'))`

*Proof.* The Definition of `nullified` quantifies its existential over the audit slice `L_R^Σ`, not the active subset `A_R^Σ`: `a ∈ nullified(Σ) ⟺ (E (b, F', G') ∈ L_R^Σ :: a ∈ coverage(G'))`. Deciding `a ∈ nullified(Σ)` is therefore a flat single-pass test — enumerate `L_R^Σ` and test `a ∈ coverage(G')` for each tuple — that never consults whether the witness `b` is itself nullified. Consequently, emitting `Nullify(b)` for a retractor `b` does not un-nullify `b`'s prior targets across the transition: `b` may itself become nullified, but the original tuple `(b, F', G')` persists in `L_R^Σ ⊆ L_R^{Σ'}` by R3, and `a ∈ coverage(G')` still witnesses `a ∈ nullified(Σ')`. Restoration must proceed by fresh emission at a distinct address. ∎
...
Where Σ ranges over the substrate's full state space — every state `↝*`-reachable from `Σ_init` (Definition — Categorical reachability), which subsumes the `→*`-reachable conforming states and admits non-conforming states besides. `Emit_K` is a function over this domain (Lemma — Emit_K function-ness, below).
...
*Proof.* K.λ's first/subsequent emission rule is deterministic in `(Σ, d)`: the first/subsequent predicate is itself a function of `Σ` and `d` (it checks whether `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}` is empty), and each branch produces a unique `a`. In the first-emission branch, `a = [d.0.s_L.1]` is a deterministic projection of `d`. In the subsequent-emission branch, `ℓ_prev` is `max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}` — a unique extremum because the homed set `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}` is finite (L-fin restricted to `origin(·) = d`) and non-empty (the subsequent branch's negated predicate), and every finite non-empty subset of a strictly totally ordered set admits a unique maximum by T1 (LexicographicOrder, ASN-0034) trichotomy alone. No conformance hypothesis is consumed: the max is well-defined at every state in Σ, contiguity of the homed-set notwithstanding, so this Lemma holds over the full state space (Definition — Emit_K). The value `Σ'.L(a) = (F, G, K)` is fixed by the caller-supplied arguments, and K.λ's Frame fixes the rest of Σ'. ∎
...
**R7a — NoExtraClassAffectsL.** For any state-affecting transition `Σ ↝ Σ'` issued by a substrate-conforming layer (per the Definition above) with `Σ.L ≠ Σ'.L`, there exists a finite sequence `Σ = Σ_0 → Σ_1 → … → Σ_m` (`m ≥ 1`) of `→`-steps, each of class (i) (K.σ) or class (iii) (K.λ), such that — *under conforming-layer clause (b) (frontier-contiguous deposition), which the discharge (4)(iii) below consumes* — `Σ_m.L = Σ'.L`, with `dom(Σ_m.M) ⊆ dom(Σ'.M)` and `dom(Σ_m.C) = dom(Σ.C) ⊆ dom(Σ'.C)`; absent clause (b), only the weaker `dom(Σ_m.L) = dom(Σ'.L)` (the same fresh addresses, up to deposition order) is guaranteed. The `Σ.L`-affecting effect decomposes into class-(iii) K.λ-steps, each prefixed if needed by the class-(i) K.σ-step its L1a home-precondition requires; no class-(ii) content-emission step is introduced (L1a constrains only `home(a_k) ∈ dom(·.M)`, not any content address).
...
`wp(Nullify(Σ, d_retr, a), single-tuple scope at Σ') ≡ P0(Σ, d_retr) ∧ P1(Σ, a) ∧ P2c(Σ)`

where P0: `d_retr ∈ dom(Σ.M)`, P1: `a ∈ A_rel^Σ`, and P2c: `Σ substrate-conforming`.

*Domain of quantification.* This wp is computed over the full state space (Definition — Emit_K). The "both pre- and post-state are substrate-conforming" claim in the Definition of Nullify describes the *relational layer's usage discipline*, which guarantees P2c at every layer-issued call — not the operation's executable domain. For layer-initiated calls P2c holds for free and the effective precondition collapses to `P0 ∧ P1`; the honest wp over the full state space retains P2c.

*Sufficiency:* P1 combined with L12a discharges `a ∈ A_rel^{Σ'}`. The only other requirement — that no other link address fall within `{t : a ≼ t}` at Σ' — is exactly the result proved under R0a in the Definition of Nullify (paragraph *Single-tuple scope under R0a*): `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}`. That derivation invokes R0a's antichain at both Σ and Σ', and R0a holds only at substrate-conforming states; P2c supplies the hypothesis at Σ, and the conformance-preserving K.λ `→`-step carries it to Σ' (Definition — substrate-conforming layer). We cite that derivation here rather than repeat the antichain argument.

*Necessity (each conjunct is load-bearing):* necessity needs only one counterexample. For P1, choose `a ∉ A_rel^Σ` distinct from the fresh emitter `a_emit(Σ, d_retr)` — such an `a` exists because `A_rel^Σ ∪ {a_emit(Σ, d_retr)}` is finite while `T` is infinite (T0(b), ASN-0034). The only new key at Σ' is `a_emit(Σ, d_retr)`, so by L12a's pointwise agreement `dom(Σ'.L) = dom(Σ.L) ∪ {a_emit(Σ, d_retr)}`; with `a ∉ dom(Σ.L) = A_rel^Σ` and `a ≠ a_emit(Σ, d_retr)`, we get `a ∉ dom(Σ'.L) = A_rel^{Σ'}`, whence `a ∉ {t : a ≼ t} ∩ A_rel^{Σ'}` and the intersection cannot equal `{a}`. (We do not assert `b ≠ a` as a generic fact: when P1 is dropped `a` is arbitrary, and nothing prevents `a = a_emit(Σ, d_retr)`; the counterexample simply selects an `a` for which they differ.) Dropping P0 admits `d_retr ∉ dom(Σ.M)`, leaving the internal `Emit_R`'s K.λ home-precondition undischarged: Nullify does not execute, no post-state Σ' is produced, and the postcondition is unreachable. Dropping P2c admits the non-conforming nested link pair `a ≼ a''` (`a'' ≠ a`, both in `dom(Σ.L)`) exhibited in the Definition — Categorical reachability: R0a's antichain excludes it at conforming states, but it is present in the full state space. Then P0 ∧ P1 holds, yet after Nullify `a''` persists by L12a and `{t : a ≼ t} ∩ A_rel^{Σ'} ⊇ {a, a''} ≠ {a}`, so the postcondition fails. Hence `P0 ∧ P1 ∧ P2c` is weakest, not merely sufficient.
...
| R0 | LEMMA | TupleAddressFreshness — under precondition `dom(Σ.M) ≠ ∅`, every emission allocates a fresh address. Discharged via ASN-0093 K.λ's first/subsequent emission rule, with conformance-free freshness derivations in both branches (first-emission: empty homed-set excludes same-home collision, home-projection distinctness excludes cross-home, T7+SC-NEQ excludes cross-subspace; subsequent-emission: `ℓ_prev` T4-valid via L1c→T10a.4, then within-home by TA5(a)+T1, cross-home by home-projection (T4b field-extraction) distinctness, cross-subspace by T7+SC-NEQ — both valid at every state, not only `→*`-reachable ones), L-invariant preservation under K.λ's frame, and ASN-0093 L0 + SC-NEQ for L14/L14a |
...
| R0a-Cor1 | LEMMA | ContiguousPrefix — `{a ∈ dom(Σ.L) : home(a) = d} = {incʲ(d.0.s_L.1, 0) : 0 ≤ j ≤ J_d^Σ}` for some `J_d^Σ ∈ ℤ_{≥-1}`, with unique T1-maximum at chain index `J_d^Σ` when non-empty. Single-key contiguity induction on conformance clause (b) (this note's operations deposit at most one fresh key per home per step); supersedes the former ConformingHomedContiguity sub-lemma, which is now folded in here |
...
| R6b | DEF-Consequence + R3 | SingleDepthRetraction — (i) deciding `a ∈ nullified` is a single flat existential pass over `L_R^Σ` (the audit slice, not the active subset `A_R^Σ`), checking only direct targeting (definitional); (ii) the nullifying effect persists across `→` because the witnessing tuple survives in `L_R` (= R3) |
...
| R7a | LEMMA (contingent on conforming-layer clause (b)) | NoExtraClassAffectsL — for any state-affecting `Σ ↝ Σ'` issued by a substrate-conforming layer with `Σ.L ≠ Σ'.L`, the `Σ.L`-affecting effect decomposes into K.λ-steps interleaved with K.σ-setup steps for L1a's home-precondition: `Σ = Σ_0 → Σ_1 → … → Σ_m` (`m ≥ 1`) with `Σ_m.L = Σ'.L`, `dom(Σ_m.M) ⊆ dom(Σ'.M)`, `dom(Σ_m.C) = dom(Σ.C) ⊆ dom(Σ'.C)` (the address-reconstruction half `Σ_m.L = Σ'.L` holds under clause (b); absent it, only `dom(Σ_m.L) = dom(Σ'.L)`, per the lemma statement's contingency clause). (= L12 + L12a + L-fin + L1a + S7d + ASN-0093 K-op frame conditions + ChainDiscipline + FirstEmission + ChainMembershipForOrigin + ChainEnumerationInjectivity for replay determinism + conforming-layer clause (b) for frontier-contiguity + per-step discharge of the full ASN-0036 / ASN-0043 / ASN-0093 invariant-and-chain-discipline catalog) |
