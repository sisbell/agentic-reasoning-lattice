...
| Nullify | OP | `Nullify(Σ, d_retr, a) ≡ Emit_R(Σ, d_retr, ∅, {(a, δ(1, #a))})` for caller-supplied `d_retr ∈ dom(Σ.M)` and `a ∈ A_rel^Σ` with `|Σ.L(a)| = 3`. Single-tuple scope and its sufficient-not-weakest precondition are established at WP Case 1 (= R5 + R0 + R0a + R6a + L12) |
