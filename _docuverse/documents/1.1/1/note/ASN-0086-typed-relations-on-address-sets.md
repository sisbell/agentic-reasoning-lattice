...
Clauses (a) and (b) here are exactly the two conditions of the Definition — substrate-conforming state, so every `↝`-reachable post-state of a substrate-conforming layer is itself a substrate-conforming state.
...
where P0: `d_retr ∈ dom(Σ.M)` and P1: `a ∈ A_rel^Σ`. *Sufficiency:* P1 combined with L12a discharges `a ∈ A_rel^{Σ'}`. The only other requirement — that no other link address fall within `{t : a ≼ t}` at Σ' — is exactly the result proved absolute under R0a in the Definition of Nullify (paragraph *Single-tuple scope, absolute under R0a*): `{t : a ≼ t} ∩ A_rel^{Σ'} = {a}`. We cite that derivation here rather than repeat the antichain argument.
