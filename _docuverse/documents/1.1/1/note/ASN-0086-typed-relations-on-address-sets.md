...
The arity-3 restriction P2 matches this note's scope. `A_K^Σ` is defined only over standard-triple links (Definition of `L_K^Σ`), so the active-subset effect of Nullify is meaningful only on arity-3 addresses. P2 is therefore a *scope label*, not an executable gate: nullifying a higher-arity address (`|Σ.L(a)| > 3`) is a well-formed Emit_R that deposits `a` into `nullified(Σ')`, but no `A_K^{Σ'}` would feel the effect under the present definitions. The single-tuple scope result itself is consequently *arity-independent* — the `→` step contributes exactly `a` to `nullified(Σ')` regardless of `|Σ.L(a)|`, since the argument above consults only `a`'s tumbler prefix and R0a's antichain, never the arity.
...
The scope condition P2 (`|Σ.L(a)| = 3`) is consequently absent from the wp: single-tuple scope is arity-independent, as established in the Definition of Nullify (paragraph on the arity-3 restriction).
