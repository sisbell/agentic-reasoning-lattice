...
*Proof.* The induction does not go through directly on the antichain property: in Case 1, sub-case B (below), the existing `a' ∈ dom(Σ.L)` with `home(a') = d` must be placed in the same sibling stream as the freshly emitted `a` to apply T10a.2, and that placement is information strictly stronger than antichain (antichain alone does not pin `a'` to `incʲ(d.0.s_L.1, 0)` for any `j`). We therefore strengthen the induction invariant to the *sibling-stream invariant* below, prove it by induction, and derive the stated antichain conclusion as a corollary.
