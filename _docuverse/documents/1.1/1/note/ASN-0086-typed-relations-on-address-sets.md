...
K.λ's contract supplies the fresh address `a` via its first/subsequent emission rule, in the branch selected by `a_emit` (Allocator Structure). R0 contributes only the structural postconditions — `home(a) = d`, on-chain membership `a ∈ A_L(d)`, and the element-level shape — and cites ASN-0093's freshness lemmas for the `dom(Σ.L) ∪ dom(Σ.C)` exclusion, whose own proofs run the within-document / cross-document (T10) / cross-subspace (T7) case split this fact requires.
...
`Emit_K` is operationally `K.λ` of ASN-0093, restricted to the standard-triple link value `(F, G, K)`. K.λ accepts a value `(e₁, …, e_N)` with `N ≥ 3` and `e₃ ≠ ∅`; `Emit_K` specializes to `N = 3` and `e₃ = K`, which discharges K.λ's L3 precondition (R0's *Value-shape consequence*) — so K.λ's contract carries over with no separate value requirement on the caller.
...
*Rationale.* Retraction is a destructive withdrawal, which Nelson scopes by *authority* — *"Only the owner has a right to withdraw a document or change it"* — where the right attaches to ownership at the moment of the act, not to how long the address has existed. The two P-tgt branches are both owner-authorized under this reading, but evaluated at different states. In the **P1** branch the target `a ∈ A_rel^Σ` is already-baptized owned material, and ownership-at-Σ licenses the withdrawal; udanax-green's link lookup enforces this pre-existence boundary at the granfilade exact-match guard (`tumblereq`, `granf2.c:37`), which passes only for an address already inserted into the granfilade. The **self-emit** branch (`a = a_emit(Σ, d_retr)`, reached precisely when `¬P1`) is a baptize-and-retract-in-one-atomic-step: `a` is fresh against `dom(Σ.L)` at Σ but is owned-at-commit — the same `→`-step that deposits the retractor at `a` baptizes it under the caller's home `d_retr ∈ dom(Σ.M)`. Read at Σ' rather than Σ, the target is then existing owned material, so the authority requirement is met; and because the freshly-emitted address was never published and can have accrued no dependents within the atomic step, Nelson's *protective* gloss on withdrawal (*"a thing once published stay published … its author may not withdraw it except by lengthy due process"*, scoped to published material with dependents) does not engage. In udanax-green this mirrors `docreatelink`'s own allocate-then-lookup sequence (`createorglingranf`'s `insertseq` runs before the `findorgl` exact-match guard, so the just-allocated address satisfies `tumblereq` immediately).
