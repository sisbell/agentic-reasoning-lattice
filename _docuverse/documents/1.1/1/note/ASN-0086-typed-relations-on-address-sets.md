...
**R6b — SingleDepthRetraction (Consequence of Definition `nullified`; cross-state persistence via R3).** Retraction-of-retraction is not a fixpoint operation: an `Emit_R` call whose to-coverage targets a retractor `b` does not "undo" `b`'s nullifying effect on its prior targets.

*Justification.* For (i), the Definition of `nullified` quantifies its existential over the audit slice `L_R^Σ`, not the active subset `A_R^Σ`: `a ∈ nullified(Σ) ⟺ (E (b, F', G') ∈ L_R^Σ :: a ∈ coverage(G'))`. Because membership ranges over `L_R^Σ` rather than `A_R^Σ`, it is a flat single-pass test — enumerate `L_R^Σ` and test `a ∈ coverage(G')` for each tuple — that is witness-status-independent: it never consults whether the witness `b` is itself nullified. This half rests on the Definition alone. For (ii), "un-nullifying" `a` by emitting `Nullify(b)` for the retractor `b` has no effect *across the transition*: `b` may itself become nullified, but the original tuple `(b, F', G')` persists in `L_R^Σ ⊆ L_R^{Σ'}` by R3 (slice monotonicity), and `a ∈ coverage(G')` still witnesses `a ∈ nullified(Σ')`. This cross-state half is not definitional — it depends on R3. Restoration must proceed by fresh emission at a distinct address. ∎
