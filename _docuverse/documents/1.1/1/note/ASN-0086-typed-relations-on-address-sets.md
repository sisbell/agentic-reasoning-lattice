...
*L-invariant preservation across the K.λ-step.* The emission above is a valid K.λ `→`-step — its on-chain gating and freshness preconditions discharged in the freshness bullets — taken from the `→*`-reachable pre-state Σ. Since `→*`-reachability is closed under `→`, Σ' is `→*`-reachable. The invariants published in ASN-0093's K.λ contract (M*, C*, L0–L3, L12, SD, L-fin, C-fin) hold at the fresh key `a` by that contract directly. The one obligation among these not closed structurally is the match between the emitted value and K.λ's value-shape precondition: `Emit_K` supplies the triple `(F, G, K)`, a `Link`-typed value of arity 3 with non-empty type slot `K`, which satisfies K.λ's `N ≥ 3 ∧ e₃ ≠ ∅` at `N = 3`, `e₃ = K` — so the emission is a well-formed K.λ value and L3 is met. The remaining ASN-0043 invariants L14 (DualPrimitive) and L14a (NonTranscludability), which ASN-0093's K.λ contract does not publish, are discharged at the fresh key by the FreshLinkKeyDisjointness sub-lemma: the K.λ-step has the value-preserving single-key form (`Σ'.C = Σ.C`, `Σ'.M = Σ.M`) with `E(a)₁ = s_L`, so L14/L14a carry from Σ to Σ'. This completes the final conjunct of R0's conclusion: Σ' is `→*`-reachable. ∎
