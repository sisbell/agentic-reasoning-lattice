...
**Definition — Nullify.** Nullify gates on two conditions — P0: `d_retr ∈ dom(Σ.M)` (the home document must be allocated, discharging the internal `Emit_R`'s K.λ home-precondition) and P1: `a ∈ A_rel^Σ` (the target tuple's address must be a live link, which L12a then carries to `a ∈ A_rel^{Σ'}`) — together with the precondition PC that Σ be substrate-conforming, under which the internal `Emit_R` is admissible and single-tuple scope (R-Scope) holds. Arity `|Σ.L(a)|` is not gating: single-tuple scope is arity-independent (R-Scope).
