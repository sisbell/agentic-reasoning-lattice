...
We analyze the preconditions of two postconditions: Nullify's single-tuple scope at Σ' (Case 1) and membership of Emit_K's fresh tuple in the active subset at Σ' (Case 2), the latter written in the standard wp notation `wp(S, R)` — the weakest predicate over the prior state Σ that guarantees the post-state Σ' satisfies R after S executes.
