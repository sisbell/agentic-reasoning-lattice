...
*Source: ASN-0077-showorigin-operation.md (revised 2026-05-25) — Extracted: 2026-05-26*

## Definition — OriginsISpan

`origins_I(Σ, σ) = { origin(a) : a ∈ ⟦σ⟧ ∩ dom(Σ.C) }`

Where σ is an I-span with start `s` and width `ℓ`, denoting `⟦σ⟧ = { t ∈ T : s ≤ t < s ⊕ ℓ }`.

## Definition — OriginsVSpan

Three equivalent forms, (F1) adopted as definition:
...
Where `{β₁, ..., βₖ} = {(v₁, a₁, n₁), ..., (vₖ, aₖ, nₖ)}` is the unique maximally merged block decomposition of `f = M(d) ↾ ⟦σ⟧` via C1a.

---

## O0 — OriginExtendedToLinks (LEMMA, claim)
...
## O1 — OriginPartitionsContent (LEMMA, claim)
...
## O1.1 — SingleOriginSufficiency (LEMMA, corollary)
...
## O1.2 — MultiOriginDiagnostic (LEMMA, corollary)
...
## O4 — ParallelWitnesses (LEMMA, claim)
...
## O10 — ReadOnlyFrameIdempotence (LEMMA, claim)
...
## O11 — VSpanPreservationKMuPlus (LEMMA, claim)
...
## O11' — VSpanPreservationKMuPlusL (LEMMA, claim)
...
## O12 — VSpanContainmentMonotonicity (LEMMA, claim)

*For V-spans `σ₁, σ₂` over the same document `d` with `⟦σ₁⟧ ⊆ ⟦σ₂⟧`: `origins_V(Σ, d, σ₁) ⊆ origins_V(Σ, d, σ₂)`.*

---

## F1 ≡ F2 ≡ F3 — OriginsVEquivalenceChain (LEMMA, claim)

Equivalence chain for `origins_V`: reader-form (F1) ≡ decomposition-form (F2) ≡ block-collapsed-form (F3):

> *(F1)* `{ origin(M(d)(v)) : v ∈ ⟦σ⟧ ∩ dom(M(d)) }`
> *(F2)* `⋃_{j=1}^{k} { origin(aⱼ + i) : 0 ≤ i < nⱼ }`
> *(F3)* `{ origin(aⱼ) : 1 ≤ j ≤ k }`

Where `{(v₁, a₁, n₁), ..., (vₖ, aₖ, nₖ)}` is the C1a block decomposition of `M(d) ↾ ⟦σ⟧`.

---

## wp(SHOWORIGIN_I, |result| = 1) — WpShoworiginISingleOrigin (WP, claim)

> `wp(SHOWORIGIN_I(σ), |result| = 1) = (⟦σ⟧ ∩ dom(C) ≠ ∅) ∧ (A a, b : a, b ∈ ⟦σ⟧ ∩ dom(C) : origin(a) = origin(b))`

---

## wp(SHOWORIGIN_V, d_q ∈ result) — WpShoworiginVDocumentPresence (WP, claim)

> `wp(SHOWORIGIN_V(d, σ), d_q ∈ result) = (E v : v ∈ ⟦σ⟧ ∩ dom(M(d)) : origin(M(d)(v)) = d_q)`

---

## SHOWORIGIN (I-span) — ShoworiginISpan (OP, operation)
...
## SHOWORIGIN (V-span) — ShoworiginVSpan (OP, operation)

- *Preconditions*: `(d, σ)` is a well-formed content reference — explicitly, the conjuncts from the ContentReference definition of ASN-0058: (i) `d ∈ Σ.E_doc`; (ii) `σ = (u, ℓ)` is a level-uniform V-span, i.e. `#u = #ℓ` (S6 of ASN-0053); (iii) `V_{u₁}(d) ≠ ∅`; (iv) T12 holds for `(u, ℓ)` — `Pos(ℓ)` and `actionPoint(ℓ) ≤ #u`; (v) `#ℓ = #u = m`, where `m` is the common V-position depth in subspace `u₁` of `d` (S8-depth, ASN-0036); (vi) `{v ∈ T : u ≤ v < reach(σ) ∧ #v = m} ⊆ dom(M(d))`.
