...
(b) For `x ∈ dom(C)`, S7 of ASN-0036 supplies the correspondence: `origin(x)` is the document that performed the allocation event placing `x` into `dom(C)`. For `x ∈ dom(L)`, L1c (LinkAllocatorConformance, ASN-0047) supplies the analogous correspondence directly: every `ℓ ∈ dom(L)` is reachable from a T4-valid document-level seed via a finite increment sequence `(t₀, …, tₙ)` with `t₀ = origin(ℓ)` and `tₙ = ℓ`. The seed `t₀` is document-level (`zeros(t₀) = 2`) and equals `origin(ℓ)` by L1c's stated identification, so `origin(ℓ)` is precisely the document-level tumbler at the root of `ℓ`'s allocation chain — naming exactly the document under which `ℓ` was allocated. K.λ (LinkAllocation, ASN-0047) corroborates this at the elementary-transition level: its precondition `origin(ℓ) = d` with `d ∈ E_doc` records the same identification at the point of emission, so any fresh `ℓ` placed into `dom(L)` by a K.λ event carries the allocating document's tumbler as its origin. Hence `origin(ℓ)` names the document that allocated `ℓ`.
