...
The membership preservation for `dom(L)` that the V-span and permanence claims (O5/O5★, O7) consume is supplied directly by P3 (ArrangementMutabilityOnly, ASN-0047) — `(A Σ → Σ' :: dom(L) ⊆ dom(L'))` and `(A ℓ ∈ dom(L) :: L'(ℓ) = L(ℓ))` — and by LP13 (UnconditionalLinkPersistence, ASN-0098), which closes link permanence to multi-step `Σ →* Σ'`.
...
The single-step claims O11 and O11' lift to a multi-step version by induction on chain length, paralleling the O5/O5★ and O6/O6★ pattern. The per-step case analysis does not depend on enumerating the transition vocabulary: every transition partitions into two mutually exclusive and jointly exhaustive classes — those that modify `M(d)` and those that leave `M(d)` unchanged — and exhaustiveness rests on this binary distinction alone. The first class is the arrangement extensions on `d` (K.μ⁺, K.μ⁺_L) handled by O11/O11'; every transition outside this class leaves `M(d)` unchanged and is absorbed by O7 (V-span stability under fixed arrangement). Corollary O11.1 supplies per-step well-formedness preservation at each arrangement-extension sub-case of the induction. We prove the general mixed-chain lemma directly and obtain the pure-K.μ⁺ and pure-K.μ⁺_L chains as one-line specializations.
...
**Claim O11★ (Multi-step V-span preservation under K.μ⁺ chain).** *For any reachable state sequence `Σ →* Σ'` in which every `M(d)`-modifying step is K.μ⁺ on `d`, and any V-span `σ` over `d` satisfying the SHOWORIGIN_V well-formedness preconditions at Σ: `origins_V(Σ, d, σ) = origins_V(Σ', d, σ)`.*

*Derivation.* Such a chain is the special case of O11★★ in which sub-case (ii) never fires; O11★★ applies directly. ∎

**Claim O11'★ (Multi-step V-span preservation under K.μ⁺_L chain).** *For any reachable state sequence `Σ →* Σ'` in which every `M(d)`-modifying step is K.μ⁺_L on `d`, and any V-span `σ` over `d` satisfying the SHOWORIGIN_V well-formedness preconditions at Σ: `origins_V(Σ, d, σ) = origins_V(Σ', d, σ)`.*

*Derivation.* Such a chain is the special case of O11★★ in which sub-case (i) never fires; O11★★ applies directly. ∎
