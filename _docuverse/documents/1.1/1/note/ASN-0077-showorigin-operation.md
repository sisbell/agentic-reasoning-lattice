...
*Derivation.* (a) For `x ∈ dom(C)`, S7b (ASN-0036) gives `zeros(x) = 3`. For `x ∈ dom(L)`, SubAllocatorAxiom (c) (ASN-0047) gives `zeros(x) = 3` — every output of a document's sub-allocators is T4-valid with `zeros(·) = 3` — while L1b (ASN-0047) supplies the complementary element-field depth `#E(x) ≥ 2`. In both cases T4b (UniqueParse, ASN-0034) is applicable, so the projections `N(x), U(x), D(x)` are well-defined; the constructed tumbler `N(x).0.U(x).0.D(x)` is document-level by direct count of separators (`zeros = 2`).

(b) For `x ∈ dom(C)`, S7 of ASN-0036 supplies the correspondence: `origin(x)` is the document that performed the allocation event placing `x` into `dom(C)`. For `x ∈ dom(L)`, three foundation pieces compose, and *all three* are load-bearing. First, L1c (LinkAllocatorConformance, ASN-0047) supplies the *structural-projection-equals-chain-seed* identity: every `ℓ ∈ dom(L)` is reachable from a T4-valid document-level seed via a finite increment sequence `(t₀, …, tₙ)` with `t₀ = origin(ℓ)` and `tₙ = ℓ`, so `origin(ℓ)` is precisely the document-level tumbler at the root of `ℓ`'s allocation chain (with `zeros(t₀) = 2`). L1c names the chain seed but does not by itself bind that seed to the *allocating* document. Second, K.λ (LinkAllocation, ASN-0047) supplies the *chain-seed-equals-allocating-document* identification: K.λ's precondition `origin(ℓ) = d` with `d ∈ E_doc` is the elementary-transition condition that any K.λ event placing `ℓ` into `dom(L)` requires the allocating document `d` to coincide with the structural projection `origin(ℓ)`. K.λ binds the seed to an allocating document, but only for `ℓ`s actually placed by K.λ events — without further argument, L1c's seed and K.λ's precondition could speak to disjoint populations. Third — the closure step that links them — we fix the working frame's complete elementary transition vocabulary once and assert its exhaustiveness. The vocabulary is `{K.α, K.δ, K.μ⁺, K.μ⁻, K.μ~, K.λ, K.μ⁺_L, K.ρ} ∪ {K.σ}` — ASN-0047's eight transitions together with the spanfilade-write transition K.σ. **Frame-vocabulary exhaustiveness:** for the system state Σ at the content layer addressed by this ASN, no transition outside these nine modifies Σ; every reachable transition sequence decomposes into a finite chain of atomic transitions each of one of these nine kinds. (K.σ is enumerated explicitly here so that the closure arguments below — O0a, O0(c) totality, and O11★★'s absorption case — range over the complete vocabulary rather than over ASN-0047's eight alone.) Our derivation rests, in addition, on the **frame-exhaustiveness assumption** inherited from ASN-0047: each transition's Effect and Frame clauses are jointly exhaustive, so components of Σ named in neither Effect nor Frame are unchanged across the transition. This reading is not discharged by any single foundation axiom — it is the semantic convention under which ASN-0047's elementary transitions are read, and it is what foundation ASN-0098 inherits when it characterises K.α as *modifying only* `Σ.C` (LP6, ContentAllocationInvariance), K.λ as *modifying only* `Σ.L` (LP7, LinkAllocationInvariance), and K.ρ as having frame `(A d :: M'(d) = M(d))` and *"leaving every document's arrangement intact"* (LP14, ProvenanceRecordingInvariance). LP6, LP7, and LP14 record consequences of the assumption; they do not establish it. Any future transition added to the working frame whose Effect or Frame modifies `L` (or any other component) without naming it would invalidate the L-closure argument below and the downstream claims O5, O5★, O11 that depend on it. P3 (ArrangementMutabilityOnly, ASN-0047) discharges monotonicity directly: `(A Σ → Σ' :: dom(L) ⊆ dom(L'))` is asserted per-transition over every reachable transition without dependence on enumeration completeness. To identify K.λ as the unique source of that growth, we inspect each transition's effect and frame clauses directly from their defining ASN. K.λ's effect `L' = L ∪ {ℓ ↦ (e₁, …, eₙ)}` is the only effect clause that names `L` by extending it; LP7's modifies-only characterisation records the dual statement that no other component is touched. Every other transition in the working frame either declares an explicit `L' = L` frame clause (K.α, K.δ in all sub-cases — IsNode, IsAccount, and IsDocument — K.μ~, K.μ⁺_L) or has both effect and frame that name only other components — K.μ⁺ and K.μ⁻ name `M(d)`, K.ρ names `R`. By the frame-exhaustiveness assumption, `L' = L` for K.μ⁺, K.μ⁻, and K.ρ as well. (K.σ — which is the ninth member of the complete working-frame vocabulary fixed above, lying outside ASN-0047's eight and so not addressed by the per-transition inspection of those eight — has its M-effect characterised by LP8 (DocumentRegistrationInvariance, ASN-0098): K.σ registers a fresh document `d_new` with `dom(M') = dom(M) ∪ {d_new}` and `M'(d_new) = ∅`, preserving prior M-entries. K.σ's effect therefore names only `M`; by the frame-exhaustiveness assumption — the same assumption used above to discharge K.μ⁺, K.μ⁻, and K.ρ on `L` — `L' = L` across every K.σ transition. K.δ-IsDocument is already inside this working frame with its own explicit `L' = L` frame as a K.δ sub-case; LP8's incidental coverage of K.δ-IsDocument's M-effect is not consumed here.) The only source of growth in `dom(L)` is therefore a K.λ event. P3 additionally records the complementary value-preservation guarantee at the invariant level — `(A ℓ ∈ dom(L) :: L'(ℓ) = L(ℓ))` — across every reachable transition, fixing that no transition rewrites the endsets of an already-allocated link. Combined with `L₀ = ∅` in the initial state Σ₀, induction over reachable transition sequences gives that every `ℓ ∈ dom(L)` at any reachable state was placed by some K.λ event. Composing all three: every `ℓ ∈ dom(L)` arose through a K.λ event (closure) whose precondition (K.λ) pins L1c's chain seed to `d ∈ E_doc`, the allocating document. Hence `origin(ℓ)` names the document that allocated `ℓ`.

(c) Totality on `dom(C) ∪ dom(L)` requires both well-formedness of the structural projection at every `x` in the domain *and* membership of the result in the stated codomain `E_doc`. (a) discharges the first conjunct — for every `x ∈ dom(C) ∪ dom(L)`, T4b's projections `N(x), U(x), D(x)` are defined and `origin(x) = N(x).0.U(x).0.D(x)` is a syntactically well-formed document-level tumbler. (b) discharges the second — `origin(x)` is the tumbler of the document that allocated `x`, and that allocating document inhabits `E_doc`. For `x ∈ dom(C)`: closure that every `x ∈ dom(C)` arose through some K.α event follows by direct enumeration parallel to the closure step in (b) for `dom(L)`. K.α's effect clause `C' = C ∪ {a ↦ v}` is the only ASN-0047 transition effect clause that names `C` — a fact foundation ASN-0098's LP6 (ContentAllocationInvariance) records as the modifies-only characterisation of K.α. Every other ASN-0047 transition declares an explicit `C' = C` frame clause (K.δ, K.μ⁺, K.μ⁻, K.μ~, K.λ, K.μ⁺_L, K.ρ — K.ρ's full frame is `C' = C; E' = E; (A d :: M'(d) = M(d))`, supplying the explicit content-preservation clause directly without recourse to the frame-exhaustiveness assumption; the asymmetry with (b) reflects that K.ρ's actual frame names `C` with a preservation clause but does not mention `L`, so the L-closure of (b) needed the frame-exhaustiveness assumption for K.ρ while the C-closure here does not). (K.σ — the ninth member of the complete working-frame vocabulary fixed above, lying outside ASN-0047's eight and so not addressed by the per-transition inspection of those eight — similarly does not modify `C`. By the same K.σ M-effect characterisation invoked in (b) (LP8, ASN-0098: K.σ's effect names only `M`, registering a fresh `d_new` with `dom(M') = dom(M) ∪ {d_new}` and `M'(d_new) = ∅`), the frame-exhaustiveness assumption yields `C' = C` across every K.σ transition. The K.σ arguments in (b) and (c) are now precisely parallel — both source K.σ's M-effect from LP8 and discharge the L/C-closure via frame-exhaustiveness.) Combined with `C₀ = ∅` in the initial state Σ₀, induction over reachable transition sequences gives that every `x ∈ dom(C)` at any reachable state was placed by some K.α event. K.α's precondition `d ∈ E_doc` (ASN-0047) at that allocation event together with P1 (EntityPermanence, ASN-0047), which keeps `d` in `E_doc` at every subsequent reachable state, then delivers `origin(x) ∈ E_doc`. For `x ∈ dom(L)`: by K.λ's precondition `d ∈ E_doc` together with P1 (EntityPermanence, ASN-0047), which keeps `d` in `E_doc` at every subsequent reachable state. Composing the two cases, `origin(x) ∈ E_doc` for every `x ∈ dom(C) ∪ dom(L)`. Single-valuedness is T4b's functional definition of projections. ∎

We extract a standalone closure conclusion from O0(b) so that downstream uses can cite it directly without re-walking the per-transition inspection of `dom(L)`'s growth.

**Claim O0a (K.λ-event closure for `dom(L)`).** *Every `ℓ ∈ dom(Σ.L)` at any reachable state `Σ` arose through some K.λ event in the transition chain `Σ₀ →* Σ`. Equivalently, for every reachable `Σ` and every `ℓ ∈ dom(Σ.L)`, there exists a transition `Σ_k → Σ_{k+1}` on the chain `Σ₀ →* Σ_k → Σ_{k+1} →* Σ` whose elementary kind is K.λ and whose effect placed `ℓ` into `dom(L)`.*

*Derivation.* This is exactly the closure step established within O0(b)'s `dom(L)` derivation above; we re-extract it here as a separately labeled lemma. By per-transition inspection across the complete working-frame vocabulary `{K.α, K.δ, K.μ⁺, K.μ⁻, K.μ~, K.λ, K.μ⁺_L, K.ρ} ∪ {K.σ}` fixed and asserted exhaustive in O0(b), together with the frame-exhaustiveness assumption (inherited from ASN-0047 and recorded as a load-bearing premise in O0(b)): K.λ's effect clause `L' = L ∪ {ℓ ↦ (e₁, …, eₙ)}` is the only effect in the working frame that names `L` by extension; every other transition (K.α, K.δ in all sub-cases — IsNode, IsAccount, IsDocument — K.μ⁺, K.μ⁻, K.μ~, K.μ⁺_L, K.ρ, K.σ) either declares an explicit `L' = L` frame clause (K.α, K.δ in all sub-cases, K.μ~, K.μ⁺_L) or has both effect and frame naming only components other than `L` (K.μ⁺, K.μ⁻, K.ρ, K.σ), which the frame-exhaustiveness assumption closes to `L' = L`. (K.σ's M-effect characterisation via LP8 of ASN-0098 names only `M`; the same frame-exhaustiveness assumption yields `L' = L` across every K.σ transition.) P3 (ArrangementMutabilityOnly, ASN-0047) records `dom(L) ⊆ dom(L')` per-transition independently of enumeration completeness, but does not by itself identify K.λ as the unique source of growth; the per-transition inspection above supplies that uniqueness. Combined with `L₀ = ∅` in the initial state Σ₀, induction over reachable transition sequences gives the closure conclusion. ∎

Downstream ASNs needing the "every `ℓ ∈ dom(L)` arose through some K.λ event with K.λ's precondition `origin(ℓ) = d ∈ E_doc`" identification can cite O0a directly, composed with K.λ's precondition. Within this ASN, O5/O5★ membership preservation for `dom(L)` is independently supplied by P3 and LP13, and so does not require O0a.
...
*Derivation.* Fix `0 ≤ i < nⱼ`. B3 (Consistency, ASN-0058) gives `f(vⱼ + i) = aⱼ + i`; since `f` is a restriction of `M(d)`, also `M(d)(vⱼ + i) = aⱼ + i`. B1 (Coverage, ASN-0058) gives `vⱼ + i ∈ V(βⱼ) ⊆ dom(f) ⊆ dom(M(d))`. In either subspace case we first discharge the subspace identification at `vⱼ + i`, since S3★ (ASN-0047) — which we will invoke to place `aⱼ + i` in the appropriate I-domain — requires the antecedent `subspace(vⱼ + i) = s_C` (resp. `s_L`). The bridge is uniform: S8a (ASN-0036) at `vⱼ ∈ dom(M(d))` gives `#vⱼ ≥ 2`, which discharges M-sub(a)'s precondition on the block `(vⱼ, aⱼ, nⱼ)`; M-sub(a) (SubspaceConfinement, ASN-0058) then yields `subspace(vⱼ + i) = subspace(vⱼ)` for every `0 ≤ i < nⱼ`. Two cases by subspace of `vⱼ`, exhaustive by S3★-aux (SubspaceExhaustiveness, ASN-0047) applied to `vⱼ ∈ dom(M(d))`: `subspace(vⱼ) ∈ {s_C, s_L}`. *Content block* (`subspace(vⱼ) = s_C`): M-sub(a) gives `subspace(vⱼ + i) = s_C`; with this antecedent discharged, S3★ (ASN-0047) at `vⱼ + i ∈ dom(M(d))` gives `aⱼ + i ∈ dom(C)`. M16a (OriginInvarianceUnderShift, ASN-0058) requires *both* conjuncts `aⱼ ∈ dom(C)` and `aⱼ + i ∈ dom(C)`: the latter is the instance just derived, and the former is the same derivation at `i = 0` (which gives `aⱼ + 0 = aⱼ ∈ dom(C)`). With both conjuncts of M16a's precondition discharged at `(aⱼ, i)`, M16a delivers `origin(aⱼ + i) = origin(aⱼ)`. *Link block* (`subspace(vⱼ) = s_L`): M-sub(a) gives `subspace(vⱼ + i) = s_L`; with this antecedent discharged, S3★ at `vⱼ + i ∈ dom(M(d))` gives `aⱼ + i ∈ dom(L)`. With both CL-OWN preconditions — `vⱼ + i ∈ dom(M(d))` and `subspace(vⱼ + i) = s_L` — discharged at `vⱼ + i` and (by `i = 0`) at `vⱼ`, CL-OWN (ASN-0047) gives `origin(M(d)(vⱼ)) = d` (so `origin(aⱼ) = d`) and `origin(M(d)(vⱼ + i)) = d` (so `origin(aⱼ + i) = d`). Hence `origin(aⱼ + i) = d = origin(aⱼ)`. In both cases `origin(aⱼ + i) = origin(aⱼ)`. ∎
...
*Sub-case (a): `subspace(u) = s_C`.* The cross-state depth identification proceeds in three steps. We write `V_{s_C}(d)|_Σ` and `V_{s_C}(d)|_Σ'` to disambiguate the state-relative readings of the same symbol. (1) At Σ, precondition (iii) gives `V_{s_C}(d)|_Σ ≠ ∅` and precondition (v) names `m` as `d`'s common content-subspace depth via S8-depth at Σ — so every position in `V_{s_C}(d)|_Σ` has length `m`. (2) K.μ⁺ extends `dom(M(d)) ⊆ dom(M'(d))` while preserving prior mappings: `(A v : v ∈ dom(M(d)) : M'(d)(v) = M(d)(v))`. The bridge from this to the inclusion `V_{s_C}(d)|_Σ ⊆ V_{s_C}(d)|_Σ'` rests on a structural observation: `subspace(v) = v_1` (Subspace, ASN-0036) and `#v` are projections that read only the component sequence of the tumbler `v` itself, independent of state. The same tumbler `v` therefore has the same first component and the same length at Σ and Σ'. Pre-state positions in `V_{s_C}(d)|_Σ` — being already in `dom(M(d))` and (by K.μ⁺'s extension clause) still in `dom(M'(d))` — therefore inhabit `V_{s_C}(d)|_Σ'` as well, with their subspace identifiers and depths preserved. (3) S8-depth at Σ' obligates a single common content-subspace depth — say `m'` — across all of `V_{s_C}(d)|_Σ'`. Since the pre-state positions (all of depth `m`) lie in this set and S8-depth forces a single value, `m' = m`. Hence `v ∈ V_{s_C}(d)|_Σ' ⊆ dom(M'(d))` has `#v = m' = m` (the post-state reading is the one we need here, since `v ∈ dom(M'(d))` but `v ∉ dom(M(d))`). Combined with `v ∈ ⟦σ⟧` (which unfolds to `u ≤ v < reach(σ)`), precondition (vi) at Σ gives `v ∈ dom(M(d))`, contradicting `v ∉ dom(M(d))`.
...
Both O11 and O11' assume σ well-formed at the pre-state Σ. To chain these single-step claims into multi-step lemmas — and to make available a citable handle for callers chaining queries across post-states — we extract the post-state preservation of well-formedness as a stand-alone corollary.
...
- *Precondition (vi) (range condition):* the set `{v ∈ T : u ≤ v < reach(σ) ∧ #v = m}` is determined by σ and the integer `m` alone — state-independent once `m` is fixed. We show that the common depth `m` in subspace `u₁` at Σ' coincides with the common depth at Σ:
  ...
  - *`u₁ = s_L`, Σ → Σ' a K.μ⁺ step:* K.μ⁺ adds only content-subspace V-positions (KMuPlusContentSubspaceRestriction, ASN-0047). `V_{s_L}(d)` is unchanged across Σ → Σ', so the common link-subspace depth is unchanged.
  - *`u₁ = s_L`, Σ → Σ' a K.μ⁺_L step:* LinkVPositionDepthAxiom (ASN-0047) — `(A d ∈ E_doc :: m_L = 2)` — fixes the link-subspace depth uniformly at every state, so `m' = m_L = m = 2`.
...
The single-step claims O11 and O11' lift to multi-step versions by induction on chain length, paralleling the O5/O5★ and O6/O6★ pattern. Both multi-step claims admit interleaved non-M(d)-modifying transitions (K.α, K.λ, K.δ, K.ρ, K.σ, and K.μ⁺/K.μ⁻/K.μ~/K.μ⁺_L on documents `d' ≠ d`) — these leave `M(d)` unchanged and are absorbed by O7 (V-span stability under fixed arrangement). Corollary O11.1 supplies per-step well-formedness preservation at each arrangement-extension sub-case of the induction.
...
*Derivation.* By induction on the length `n ≥ 0` of the chain `Σ = Σ_0 → Σ_1 → ⋯ → Σ_n = Σ'`. *Base* (`n = 0`): `Σ' = Σ`, so both sides coincide. *Step* (`n ≥ 1`): the inductive hypothesis applied to `Σ →* Σ_{n-1}` gives `origins_V(Σ, d, σ) = origins_V(Σ_{n-1}, d, σ)`. Well-formedness of σ at `Σ_{n-1}` is supplied by Corollary O11.1 applied stepwise along the chain: each arrangement-extension step (K.μ⁺ or K.μ⁺_L on `d`) preserves all SHOWORIGIN_V well-formedness preconditions — including the common-depth condition for *both* `u₁ = s_C` and `u₁ = s_L` — and each non-M(d)-modifying step leaves them untouched, so σ is well-formed at every `Σ_k`. Two sub-cases for the final step `Σ_{n-1} → Σ_n`:

(i) The step is K.μ⁺ on `d`. By O11 applied at `Σ_{n-1} → Σ_n` (with σ well-formed at `Σ_{n-1}`), `origins_V(Σ_{n-1}, d, σ) = origins_V(Σ_n, d, σ)`.

(ii) The step does not modify `M(d)`. Then `M(d)|_{Σ_n} = M(d)|_{Σ_{n-1}}`, so the restriction `M(d) ↾ ⟦σ⟧` is unchanged; O7 (V-span stability under fixed arrangement) at `Σ_{n-1} → Σ_n` yields `origins_V(Σ_{n-1}, d, σ) = origins_V(Σ_n, d, σ)`.
...
*Derivation.* Parallel to O11★ by induction on chain length, using O11' at each K.μ⁺_L sub-case (in place of O11 for K.μ⁺) and O7 at each non-M(d)-modifying sub-case. Well-formedness preservation along the chain is supplied by Corollary O11.1 applied stepwise, exactly as in O11★ — O11.1 covers both subspace cases (`u₁ = s_C` and `u₁ = s_L`), so the link-subspace depth condition is discharged by citation rather than re-derived. ∎

O11★ and O11'★ together close the multi-step gap for pure-K.μ⁺ and pure-K.μ⁺_L extension chains. A combined chain — mixing K.μ⁺ and K.μ⁺_L steps on `d` — is *not* covered by either O11★ (which explicitly excludes K.μ⁺_L on `d`) or O11'★ (which explicitly excludes K.μ⁺ on `d`). Downstream ASNs that reason about origin preservation across mixed extension activity therefore need a labeled lemma for this combined case. We supply it here.
...
(iii) The step does not modify `M(d)` — that is, an `M(d')`-modifying step for `d' ≠ d`, or a non-arrangement-modifying transition (K.α, K.λ, K.δ, K.ρ, K.σ). Then `M(d)|_{Σ_n} = M(d)|_{Σ_{n-1}}`, so the restriction `M(d) ↾ ⟦σ⟧` is unchanged; O7 (V-span stability under fixed arrangement) at `Σ_{n-1} → Σ_n` gives `origins_V(Σ_{n-1}, d, σ) = origins_V(Σ_n, d, σ)`.

Composing with the inductive hypothesis by transitivity of equality, `origins_V(Σ, d, σ) = origins_V(Σ_n, d, σ)`. ∎

The worked example below exercises the K.μ⁺ branch (via O11★) and the K.μ⁺_L branch (via O11'★) on disjoint chain segments; a mixed-chain witness combining both kinds of `M(d)`-modifying steps would route through O11★★ rather than either single-kind lemma.

O11, O11', and O11★★ (with their pure-kind multi-step companions O11★ and O11'★) together cover every arrangement-extending transition. The non-extension transitions K.μ⁻ and K.μ~ behave differently. We record the failure modes as labeled negative claims so that future ASNs do not mistakenly assume monotonicity-under-arrangement-modification beyond what the extension lemmas guarantee.
...
*Witness (exhibited concretely in the worked example).* At Σ₁ the V-span σ_{1..7} is well-formed with `n_{s_C} = 7`; the K.μ⁻ contraction to `n'_{s_C} = 5` removes V-positions `[1,1,6]` and `[1,1,7]` from `dom(M(d₃))`, both inside `⟦σ_{1..7}⟧`, so precondition (vi) fails for σ_{1..7} at Σ₂.

*Consequence.* SHOWORIGIN_V's domain of admissibility shrinks as `dom(M(d))` shrinks. A reader at Σ' must pose a smaller still-admissible query. The projection-level counterpart is LP10 (ContractionMonotonicity, ASN-0098) — `project(e, d, Σ') ⊆ project(e, d, Σ)` — which records the same V-position loss as set shrinkage rather than as inadmissibility, because LP10's `project` is defined whenever `d ∈ dom(Σ.M) ∩ dom(Σ'.M)` without σ-style well-formedness preconditions. The asymmetry between LP10 and O13 reflects different total/partial conventions for the operation, not different facts about K.μ⁻.
...
*Witness (exhibited concretely in the worked example).* At Σ = Σ₁, the K.μ~ alternative transition to Σ' = Σ₁' reorders `d₃`'s arrangement via the bijection π swapping `[1,1,3]` and `[1,1,7]`. K.μ~-FIX (ASN-0047) gives `dom(M'(d₃)) = dom(M(d₃))`, so the singleton V-span σ_{3} = ([1,1,3], [0,0,1]) is well-formed at both Σ₁ and Σ₁' (precondition (vi)'s range is the singleton `{[1,1,3]} ⊆ dom(M(d₃))` at both states). On σ_{3}:
- `origins_V(Σ₁, d₃, σ_{3}) = {origin([d₁.0.1.3])} = {d₁}` (pre-state arrangement at `[1,1,3]`).
- `origins_V(Σ₁', d₃, σ_{3}) = {origin([d₃.0.1.2])} = {d₃}` (post-state arrangement at `[1,1,3]` after swap).

Neither `{d₁} ⊆ {d₃}` nor `{d₃} ⊆ {d₁}` holds.

*Failure mechanism.* K.μ~ realises the bijection equation `M'(d)(π(v)) = M(d)(v)` with `π ≠ id`; equivalently `M'(d)(v) = M(d)(π⁻¹(v))`, which reassigns function values at individual V-positions. The argument used in O11's derivation — invoking K.μ⁺'s mapping-preservation clause `M'(d)(v) = M(d)(v)` for `v ∈ dom(M(d))` — has no K.μ~ analogue, because K.μ~ permits exactly the opposite relation. (The projection-level counterpart is LP11 (ReorderingRebinding, ASN-0098): `project(e, d, Σ') = π(project(e, d, Σ))` records the same π-permutation of V-positions that drives origin reassignment here.)
...
*Verifying O5★ and O6★ (multi-step permanence and growth).* Consider the I-address `[d₁.0.1.1]` itself, viewed across the chain Σ₀ →* Σ₁ (three atomic transitions). At Σ₀, `origin([d₁.0.1.1]) = d₁`; at Σ₁, the same projection on the same address still yields `d₁`. O5★ holds pointwise across the chain (O5 at each atomic step composes by induction). For the I-span lift, consider an I-span σ_{cover} containing all of `d₁`'s and `d₃`'s allocated content addresses:
- At Σ₀: `origins_I(Σ₀, σ_{cover}) = {d₁}` (only `d₁`'s addresses are allocated; `d₃` has none in this span yet).
- At Σ₀' (after the first K.α): `origins_I(Σ₀', σ_{cover}) = {d₁, d₃}` (the new address `[d₃.0.1.1]` lies in σ_{cover} and has origin `d₃`).
- At Σ₀'' (after the second K.α): `origins_I(Σ₀'', σ_{cover}) = {d₁, d₃}` (the second new address `[d₃.0.1.2]` shares origin with the first; the set does not grow further).
- At Σ₁ (after the K.μ⁺): `origins_I(Σ₁, σ_{cover}) = {d₁, d₃}` (K.μ⁺ does not modify `dom(C)`, so origins_I is unchanged from Σ₀'').

The chain inclusion `origins_I(Σ₀, σ_{cover}) ⊆ origins_I(Σ₁, σ_{cover})` holds (O6★ verified): `{d₁} ⊆ {d₁, d₃}`. Allocation can only enlarge the set, never reduce or rewrite it. Each atomic step contributes an O6-instance; the K.α steps add `d₃` to the set, and the K.μ⁺ step contributes a trivial inclusion (no new content).

*Alternative transition Σ₁ → Σ₁' (arrangement reordering in `d₃`, exhibiting K.μ~).* Consider an alternative path from Σ₁ in which `d₃` reorders rather than contracts. A K.μ~ transition (ASN-0047) realises the bijection equation: choose `π : dom(M(d₃)) → dom(M(d₃))` that swaps `[1,1,3]` and `[1,1,7]` and fixes every other V-position. The post-state arrangement is `M'(d₃)([1,1,3]) = M(d₃)([1,1,7]) = [d₃.0.1.2]` (formerly `[d₁.0.1.3]`) and `M'(d₃)([1,1,7]) = M(d₃)([1,1,3]) = [d₁.0.1.3]` (formerly `[d₃.0.1.2]`), with all other entries unchanged. K.μ~'s admissibility imposes three obligations on the (Σ, π) pair: (a) the precondition `|dom_C(M(d₃))| ≥ 2`, (b) the bijection is non-identity `π ≠ id`, and (c) the induced post-state satisfies the listed invariants (S8a, S8-depth, D-CTG★, D-MIN★, S3★, S3★-aux). We discharge each obligation separately.

(a) *Precondition `|dom_C(M(d₃))| ≥ 2`.* `dom_C(M(d₃))` denotes the content-subspace projection `V_{s_C}(d₃)` at Σ₁. By the worked example's setup, V-positions `[1,1,1]` through `[1,1,7]` all lie in `dom(M(d₃))` with `subspace(·) = s_C` (depth-3 content positions). Hence `|dom_C(M(d₃))| = 7 ≥ 2`.

(b) *Non-identity `π ≠ id`.* The swap exchanges `[1,1,3]` and `[1,1,7]`, both fixed points of identity but not of π; so π ≠ id.

(c) *Post-state invariants.* K.μ~-FIX (ASN-0047) gives `dom(M'(d₃)) = dom(M(d₃))`: the indexing set of V-positions is unchanged across the transition (the bijection π relabels mappings, not addresses; a post-state stored at `π(v)` is the pre-state stored at `v`). We verify each invariant clause:
- *S8a (V-position well-formedness — `zeros(v) = 0`, `#v ≥ 2`, componentwise positive).* For every `v ∈ dom(M'(d₃)) = dom(M(d₃))`, the conjuncts hold at Σ₁ by hypothesis; since the V-positions themselves are unchanged and `zeros(·)`, `#·`, and componentwise positivity are structural projections of `v` alone (state-independent), the conjuncts carry to Σ₁'.
- *S8-depth (per-subspace common depth).* Every `v ∈ dom(M'(d₃))` with `subspace(v) = s_C` has `#v = 3`; the common content-subspace depth is `m_C = 3`. No `v ∈ dom(M'(d₃))` has `subspace(v) = s_L` in this scenario (no link-subspace V-positions present), so the link case is vacuous.
- *D-CTG★ (per-subspace contiguity).* `V_{s_C}(d₃)|_{Σ₁'} = {[1,1,k] : 1 ≤ k ≤ 7}` is a contiguous prefix at depth 3 under the V-ordering on subspace s_C; the indexing set is unchanged from Σ₁, so contiguity carries through. `V_{s_L}(d₃)|_{Σ₁'} = ∅` trivially satisfies D-CTG★.
- *D-MIN★ (per-subspace minimum position).* `min(V_{s_C}(d₃)|_{Σ₁'}) = [1,1,1]` of depth 3 (matching the form `[s_C, 1, ..., 1]`); unchanged from Σ₁. `V_{s_L}(d₃)|_{Σ₁'} = ∅` is exempt.
- *S3★ (per-subspace referential integrity).* For every `v ∈ dom(M'(d₃))`, the per-subspace clauses of S3★ require `subspace(v) = s_C ⟹ M'(d₃)(v) ∈ dom(C)` (and the symmetric clause for `s_L`, which is vacuous here). At the swapped positions: `M'(d₃)([1,1,3]) = M(d₃)([1,1,7]) = [d₃.0.1.2] ∈ dom(C)` (the pre-state value, which lay in `dom(C)` at Σ₁) and `M'(d₃)([1,1,7]) = M(d₃)([1,1,3]) = [d₁.0.1.3] ∈ dom(C)` (similarly). At every fixed point, `M'(d₃)(v) = M(d₃)(v) ∈ dom(C)`. Hence the content-subspace clause holds for every `v ∈ V_{s_C}(d₃)|_{Σ₁'}`. The subspace agreement (content positions mapped to content addresses) that S3★ asserts is preserved by the swap because both swapped values were already in `dom(C)` at Σ₁.
- *S3★-aux (subspace exhaustiveness).* Every `v ∈ dom(M'(d₃))` has `subspace(v) ∈ {s_C, s_L}`. The indexing set is unchanged from Σ₁ where the conjunct held; it carries through to Σ₁'.

All three obligations (a), (b), (c) are discharged; K.μ~ is admissible at this scenario.
...
*Verifying O11 and O11★ (V-span preservation under K.μ⁺ extension).* Consider the V-span σ_{1..5} over `d₃`'s first five V-positions, across the chain Σ₀ →* Σ₁. We first verify the single-step O11 at the K.μ⁺ atomic step `Σ₀'' → Σ₁`, then lift to the full chain via O11★. At Σ₀'', `dom(M(d₃))` already holds the five pre-existing entries for V-positions `[1,1,1]` through `[1,1,5]` (the two K.α steps did not modify `M(d₃)`, so the pre-existing arrangement carries unchanged from Σ₀ through Σ₀'). The K.μ⁺ step `Σ₀'' → Σ₁` adds V-positions `[1,1,6]` and `[1,1,7]` to `dom(M(d₃))`; both lie *outside* `⟦σ_{1..5}⟧`, so the pre-existing entries inside the span are preserved by K.μ⁺'s mapping-preservation clause. Concretely: at Σ₀'', `M(d₃) ↾ ⟦σ_{1..5}⟧ = {[1,1,1] ↦ [d₁.0.1.1], ..., [1,1,5] ↦ [d₁.0.1.5]}`; at Σ₁, the same five entries hold, and no newly added V-position lies inside `⟦σ_{1..5}⟧`. Evaluating the V-span lift at the endpoints of the K.μ⁺ atomic step: `origins_V(Σ₀'', d₃, σ_{1..5}) = {d₁}` and `origins_V(Σ₁, d₃, σ_{1..5}) = {d₁}` — O11 verified at the K.μ⁺ atomic step. Across the K.α atomic steps Σ₀ → Σ₀' and Σ₀' → Σ₀'', neither modifies `M(d₃)`, so O7 (V-span stability under fixed arrangement) gives `origins_V(Σ₀, d₃, σ_{1..5}) = origins_V(Σ₀', d₃, σ_{1..5}) = origins_V(Σ₀'', d₃, σ_{1..5}) = {d₁}`. By O11★ across the full chain Σ₀ →* Σ₁ (which inducts O11 at the K.μ⁺ step and O7 at the two K.α steps), `origins_V(Σ₀, d₃, σ_{1..5}) = origins_V(Σ₁, d₃, σ_{1..5}) = {d₁}` — equal, not merely related by inclusion. This is the strengthening O11/O11★ record over a generic monotonicity claim, and it is the assertion that lets a reader of `d₃` who queried origins of the transcluded passage before the native suffix was added rely on the same answer afterward, independent of how many intervening allocation and arrangement-extension steps occurred.

*Verifying O11' and O11'★ (V-span preservation under K.μ⁺_L extension).* Consider an additional chain Σ₁ → Σ₁_a → Σ₁'' (parallel to the Σ₁ → Σ₂ contraction examined separately) decomposing into two atomic transitions: `Σ₁ → Σ₁_a` is a K.λ (LinkAllocation, ASN-0047) emitting a fresh link `ℓ_a = [d₃.0.2.1]` into `dom(L)`; `Σ₁_a → Σ₁''` is a K.μ⁺_L (LinkSubspaceExtension, ASN-0047) arranging `ℓ_a` at link-subspace V-position `v_{ℓ_a} = [2, 1]` in `M(d₃)`. The K.μ⁺_L step satisfies its preconditions: `ℓ_a ∈ dom(L)` (from K.λ at Σ₁_a), `origin(ℓ_a) = d₃` (from `ℓ_a`'s structural projection), `ℓ_a ∉ ran(M(d₃))` (fresh allocation), and `v_{ℓ_a}` is the minimum link-subspace position `[2, 1]` of depth `m_L = 2` (LinkVPositionDepthAxiom, ASN-0047, with `V_{s_L}(d₃) = ∅` at Σ₁_a). Now evaluate the V-span lift on the content-subspace span σ_{1..5} across the K.μ⁺_L atomic step Σ₁_a → Σ₁''. The newly added V-position `v_{ℓ_a} = [2, 1]` has `subspace(v_{ℓ_a}) = s_L = 2`, while every position in `⟦σ_{1..5}⟧` has subspace `s_C = 1` (by C0a applied to level-uniform σ_{1..5}); hence `v_{ℓ_a} ∉ ⟦σ_{1..5}⟧`, so the K.μ⁺_L step adds no entry within the span. The restriction `M(d₃) ↾ ⟦σ_{1..5}⟧` is unchanged across Σ₁_a → Σ₁'': `origins_V(Σ₁_a, d₃, σ_{1..5}) = origins_V(Σ₁'', d₃, σ_{1..5}) = {d₁}` — O11' verified at the K.μ⁺_L atomic step. The K.λ atomic step Σ₁ → Σ₁_a does not modify `M(d₃)` (its effect names only `L`), so O7 gives `origins_V(Σ₁, d₃, σ_{1..5}) = origins_V(Σ₁_a, d₃, σ_{1..5}) = {d₁}`. By O11'★ across the full chain Σ₁ →* Σ₁'' (one K.μ⁺_L step plus one non-M(d₃)-modifying K.λ step), `origins_V(Σ₁, d₃, σ_{1..5}) = origins_V(Σ₁'', d₃, σ_{1..5}) = {d₁}` — equality, not merely inclusion. Link-allocation activity in `d₃` is invisible to a content-subspace SHOWORIGIN query.
...
*Verifying the wp characterisations at concrete states.* The two weakest-precondition formulas derived earlier — `wp(SHOWORIGIN_I(σ), |result| = 1)` and `wp(SHOWORIGIN_V(d, σ), d_q ∈ result)` — are the operational instruments by which a reader treats SHOWORIGIN as a discovery probe. We exhibit each evaluated at a satisfying configuration and a falsifying configuration drawn from the scenario above.

For `wp(SHOWORIGIN_I(σ), |result| = 1) = (⟦σ⟧ ∩ dom(C) ≠ ∅) ∧ (A a, b : a, b ∈ ⟦σ⟧ ∩ dom(C) : origin(a) = origin(b))`:

- *Falsifying configuration at Σ₁ with σ_{cover}.* The intersection `⟦σ_{cover}⟧ ∩ dom(C)` contains all of `d₁`'s allocated content addresses `[d₁.0.1.1]` through `[d₁.0.1.5]` and `d₃`'s allocated content addresses `[d₃.0.1.1]` and `[d₃.0.1.2]` — non-empty (first conjunct holds). The pair-equality conjunct fails: `origin([d₁.0.1.1]) = d₁ ≠ d₃ = origin([d₃.0.1.1])`. Hence the wp evaluates to *false* at this state, and indeed `origins_I(Σ₁, σ_{cover}) = {d₁, d₃}` has cardinality 2, not 1.
- *Satisfying configuration at Σ₁ with σ_{d₁only}.* The smaller I-span σ_{d₁only} (spanning `[d₁.0.1.1]` through `[d₁.0.1.5]`) has intersection `{[d₁.0.1.k] : 1 ≤ k ≤ 5}` — non-empty (first conjunct holds). Every pair in the intersection shares origin `d₁` (S7a applied to addresses with the same document-level prefix `d₁`), so the pair-equality conjunct holds. Hence the wp evaluates to *true*, and indeed `origins_I(Σ₁, σ_{d₁only}) = {d₁}` has cardinality 1.

For `wp(SHOWORIGIN_V(d, σ), d_q ∈ result) = (E v : v ∈ ⟦σ⟧ ∩ dom(M(d)) : origin(M(d)(v)) = d_q)` evaluated at Σ₁ with d = `d₃` and the V-span σ_{1..7}:
...
(1) The pointwise projection `origin : dom(C) ∪ dom(L) → E_doc` (established in S7 of foundation ASN-0036 for `dom(C)` and extended to `dom(L)` by O0, grounded structurally in SubAllocatorAxiom (c) of ASN-0047 and semantically in K.λ's allocation precondition `origin(ℓ) = d`), which is structural, total, and permanent.
...
Any implementation of Xanadu that claims to support SHOWORIGIN must satisfy O0–O14 (with the O0 companion O0a, the O1 corollaries O1.1, O1.2, the well-formedness corollary O11.1, and the multi-step companions O5★, O6★, O11★, O11'★, O11★★). O0 is load-bearing: without the extension of `origin` to `dom(L)`, the link-subspace V-span case (which the V-span operation admits uniformly) has no defined answer. O13 and O14 are load-bearing in the opposite direction — they bound the preservation claims by recording the non-extension transitions (K.μ⁻ admissibility loss, K.μ~ non-preservation) under which no monotonicity claim holds, so that downstream ASNs do not over-generalize O11/O11'/O11★★ beyond the arrangement-extension regime. The operation may be realised through different mechanisms — direct tumbler-prefix decomposition, spanfilade lookup, granfilade traversal, or per-block `homedoc` records [Q12, Q13] — and these mechanisms may have different operational characteristics. But the abstract guarantees they deliver must coincide: every byte names its home, every span reveals its sources, and the answer never changes once given.

## Claims Introduced

| Label | Statement | Status |
|-------|-----------|--------|
| O0 | Origin extended to `dom(L)`: `origin : dom(C) ∪ dom(L) → E_doc` defined by uniform structural projection, with (a) structural well-definedness via S7b (dom(C)) / SubAllocatorAxiom (c) + L1b (dom(L)), (b) semantic correspondence via S7 (for dom(C)) and L1c + K.λ-precondition + closure of `dom(L)` under K.λ (for dom(L)), (c) totality and single-valuedness | introduced |
| O0a | K.λ-event closure for `dom(L)`: every `ℓ ∈ dom(Σ.L)` at any reachable state arose through some K.λ event in the chain `Σ₀ →* Σ` — standalone extraction of O0(b)'s closure step for downstream citation | introduced |
...
| O11.1 | Well-formedness preservation under arrangement extension: σ well-formed at Σ + K.μ⁺ / K.μ⁺_L on `d` ⇒ σ well-formed at Σ' (corollary discharging the post-state admissibility that O11/O11'/O11★/O11'★/O11★★ rely on) | introduced |
| O11★ | Multi-step V-span preservation under K.μ⁺ chain: for σ well-formed at Σ, any reachable chain whose `M(d)`-modifying steps are all K.μ⁺ on `d` preserves `origins_V` exactly; proved by induction on chain length combining O11 (K.μ⁺ steps) with O7 (non-M(d)-modifying interleaved steps) | introduced |
| O11'★ | Multi-step V-span preservation under K.μ⁺_L chain: parallel multi-step version for K.μ⁺_L on `d`; same induction technique, using O11' and O7 | introduced |
| O11★★ | Multi-step V-span preservation under mixed K.μ⁺/K.μ⁺_L chain: for σ well-formed at Σ, any reachable chain whose `M(d)`-modifying steps are each K.μ⁺ or K.μ⁺_L on `d` preserves `origins_V` exactly; proved by induction using O11, O11', and O7 with well-formedness lifted by Corollary O11.1 | introduced |
