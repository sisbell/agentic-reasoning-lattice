...
**Claim O5 (Origin permanence).** *For any `a ∈ dom(Σ.C) ∪ dom(Σ.L)` and any reachable transition `Σ → Σ'`, if `a ∈ dom(Σ'.C) ∪ dom(Σ'.L)`, then `origin'(a) = origin(a)`.*

*Derivation.* By O3 (sub-claim 1), `origin` is a pure projection of the component sequence of its argument — it consults no state beyond the address itself. Evaluating the same pure function on the same value `a` yields the same result in any state; hence `origin'(a) = origin(a)`. P3 (ArrangementMutabilityOnly, ASN-0047) — which includes `dom(C) ⊆ dom(C')` and `dom(L) ⊆ dom(L')` — is invoked only to justify that the hypothesis `a ∈ dom(Σ'.C) ∪ dom(Σ'.L)` is consistent with `a ∈ dom(Σ.C) ∪ dom(Σ.L)` under transitions, so the projection is defined in both states. ∎
...
| O5 | Origin permanence: `origin(a)` is identical in every reachable state containing `a` (for any `a ∈ dom(C) ∪ dom(L)`) | introduced |
