...
*Derivation.* We do not re-run the per-step induction: ASN-0098 already abstracts it. The disjunctive hypothesis `a ∈ dom(Σ.C) ∪ dom(Σ.L)` is not itself in the schema's clause grammar — the Closure schema (★) (ClosureSchema, ASN-0098) admits membership-persistence clauses only in the per-store form `x ∈ dom(Σ.X) ⟹ x ∈ dom(Σ'.X)`. The value-preservation half is likewise constrained: the schema admits a clause `f(Σ') = f(Σ)` only when its accessor `f` is well-defined once an *accompanying* membership clause holds, and `origin` is well-defined precisely on `dom(C) ∪ dom(L)` — a disjunction of two memberships, not a single one. An unconditional clause `[origin'(a) = origin(a)]` therefore lies outside the schema's per-clause grammar. We split it into two store-conditioned clauses, each pairing the value-preservation with the accompanying membership clause that makes its accessor well-defined: `c₃_C ≡ [a ∈ dom(Σ.C) ⟹ origin'(a) = origin(a)]` and, symmetrically, `c₃_L ≡ [a ∈ dom(Σ.L) ⟹ origin'(a) = origin(a)]`. We therefore take as the single-step guarantee the finite conjunction of four per-store clauses: the membership-persistence clauses `c₁ ≡ [a ∈ dom(Σ.C) ⟹ a ∈ dom(Σ'.C)]` and `c₂ ≡ [a ∈ dom(Σ.L) ⟹ a ∈ dom(Σ'.L)]`, and the conditioned value-preservation clauses `c₃_C` and `c₃_L`. O5 establishes each single-step: under `a ∈ dom(Σ.C)`, `c₁` gives `a ∈ dom(Σ'.C)` (so `origin'` is well-defined at `a` and `c₃_C` follows), and symmetrically `c₂`, `c₃_L` under `a ∈ dom(Σ.L)`. The Closure schema (★) lifts the conjunction `c₁ ∧ c₂ ∧ c₃_C ∧ c₃_L` to its transitive closure `Σ →* Σ'`. The disjunctive conclusion then follows by case split on the disjunctive hypothesis: if `a ∈ dom(Σ.C)`, the closed `c₁` gives `a ∈ dom(Σ'.C)` and the closed `c₃_C` gives `origin'(a) = origin(a)`; if `a ∈ dom(Σ.L)`, the closed `c₂` and `c₃_L` give the same pair symmetrically. Either way both conjuncts of the claim hold. ∎
...
*Sub-case (b): `subspace(u) = s_L`.* The depth coincidence the contradiction needs is `#v_ℓ = m`, derived from two readings of the same link-subspace depth at the pre-state Σ, supported by the state-independence of the structural projection `#v`. (1) Since `subspace(u) = s_L`, precondition (iii) gives `V_{s_L}(d)|_Σ ≠ ∅`, and precondition (v) names `m` as `d`'s common link-subspace depth via S8-depth at Σ — so `m = m_L(d)` (LinkSubspaceDepth, ASN-0047), the depth S8-depth fixes on the non-empty `V_{s_L}(d)|_Σ`. (2) Because `V_{s_L}(d)|_Σ ≠ ∅`, K.μ⁺_L's non-empty-subspace branch applies, so its precondition gives `#v_ℓ = m_L(d) = m`. (3) Since `#v_ℓ` is a structural projection of the tumbler `v_ℓ` (the length of its component sequence), it reads only `v_ℓ` and is independent of state, so `#v_ℓ = m` holds at Σ as well. With `v_ℓ ∈ ⟦σ⟧`, `#v_ℓ = m`, and `u ≤ v_ℓ < reach(σ)`, precondition (vi) at Σ gives `v_ℓ ∈ dom(M(d))`, contradicting `v_ℓ ∉ dom(M(d))`.
...
The pure-K.μ⁺ and pure-K.μ⁺_L chains are the obvious specializations of O11★★ (sub-case (ii), respectively sub-case (i), never fires).
...
*Alternative transition Σ₁ → Σ₁' (arrangement reordering in `d₃`, exhibiting K.μ~).* Consider an alternative path from Σ₁ in which `d₃` reorders rather than contracts. A K.μ~ transition (ASN-0047) realises the bijection equation: choose `π : dom(M(d₃)) → dom(M(d₃))` that swaps `[1,1,3]` and `[1,1,7]` and fixes every other V-position. The post-state arrangement is `M'(d₃)([1,1,3]) = M(d₃)([1,1,7]) = [d₃.0.1.2]` (formerly `[d₁.0.1.3]`) and `M'(d₃)([1,1,7]) = M(d₃)([1,1,3]) = [d₁.0.1.3]` (formerly `[d₃.0.1.2]`), with all other entries unchanged. K.μ~'s admissibility (ASN-0047) imposes a named precondition plus five clauses on the (Σ, π) pair: the precondition that `M(d₃)|_{dom_C}` takes at least two distinct values; (i) the induced post-state satisfies the arrangement-shape invariant package (S8a, S8-depth, D-CTG★, D-MIN★, together with S3★ and S3★-aux); (ii) the net effect is non-trivial, `M'(d₃) ≠ M(d₃)`; (iii) π is length-preserving, `#π(v) = #v`; (iv) π is subspace-preserving, `subspace(π(v)) = subspace(v)`; (v) π is link-subspace fixing, `π(v) = v` for every `v ∈ dom_L(M(d₃))`. We discharge each below.

*Precondition (`M(d₃)|_{dom_C}` takes at least two distinct values).* By the worked example's setup, V-positions `[1,1,1]` through `[1,1,7]` all lie in `dom(M(d₃))` with `subspace(·) = s_C` (depth-3 content positions), and they map to seven distinct I-addresses (`[d₁.0.1.1]` through `[d₁.0.1.5]`, then `[d₃.0.1.1]`, `[d₃.0.1.2]`). Hence `M(d₃)|_{dom_C}` takes seven distinct values, well above two.

(i) *Post-state invariants.* K.μ~-FIX (ASN-0047) gives `dom(M'(d₃)) = dom(M(d₃))`: the indexing set of V-positions is unchanged across the transition (the bijection π relabels mappings, not addresses; a post-state stored at `π(v)` is the pre-state stored at `v`). We verify each invariant clause:
...
(ii) *Net effect `M'(d₃) ≠ M(d₃)`.* The swapped positions carry distinct I-values: `M(d₃)([1,1,3]) = [d₁.0.1.3]` while `M(d₃)([1,1,7]) = [d₃.0.1.2]`. After the swap, `M'(d₃)([1,1,3]) = [d₃.0.1.2] ≠ [d₁.0.1.3] = M(d₃)([1,1,3])`, so `M'(d₃) ≠ M(d₃)`. (Non-identity of π alone does not suffice — a swap of two positions carrying equal I-values would leave `M'(d₃) = M(d₃)`; here the values differ, so the net effect is genuine.)

(iii) *Length preservation (`#π(v) = #v`).* π swaps `[1,1,3]` and `[1,1,7]`, both depth-3 tumblers, and fixes every other position; so `#π(v) = #v` for all `v ∈ dom(M(d₃))`.

(iv) *Subspace preservation (`subspace(π(v)) = subspace(v)`).* Both swapped positions have first component 1, i.e. `subspace = s_C`; the swap carries an s_C position to an s_C position, and fixed points trivially preserve subspace. So `subspace(π(v)) = subspace(v)` for all `v`.

(v) *Link-subspace fixing (`π(v) = v` for `v ∈ dom_L(M(d₃))`).* `dom_L(M(d₃)) = V_{s_L}(d₃) = ∅` in this scenario (no link-subspace V-positions present), so the clause holds vacuously; in particular both swapped positions lie in the content subspace.

The precondition and all five clauses (i)–(v) are discharged; K.μ~ is admissible at this scenario.
