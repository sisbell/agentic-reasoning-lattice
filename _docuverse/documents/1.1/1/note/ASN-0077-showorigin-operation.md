...
*Derivation.* By induction on the length `n ≥ 0` of the chain `Σ = Σ_0 → Σ_1 → ⋯ → Σ_n = Σ'`. *Base* (`n = 0`): `Σ' = Σ`, so both sides coincide. *Step* (`n ≥ 1`): the inductive hypothesis applied to `Σ →* Σ_{n-1}` gives `origins_V(Σ, d, σ) = origins_V(Σ_{n-1}, d, σ)`. Well-formedness of σ at `Σ_{n-1}` is preserved: precondition (iii) (`V_{u₁}(d) ≠ ∅`) and precondition (vi) (the range condition `{v ∈ T : u ≤ v < reach(σ) ∧ #v = m} ⊆ dom(M(d))`) both preserve because `dom(M(d))` is non-decreasing along the chain (no K.μ⁻ or K.μ~ on `d`); the common depth `m` in subspace `u₁` is fixed by S8-depth across the chain (K.μ⁺ additions enter `s_C` at the prevailing content depth, parallel to the cross-state depth argument in O11's sub-case (a)). Two sub-cases for the final step `Σ_{n-1} → Σ_n`:
...
*Derivation.* Parallel to O11★ by induction on chain length, using O11' at each K.μ⁺_L sub-case (in place of O11 for K.μ⁺) and O7 at each non-M(d)-modifying sub-case. Well-formedness preservation along the chain mirrors O11★'s argument: `dom(M(d))` is non-decreasing (no K.μ⁻ or K.μ~ on `d`), and the link-subspace depth `m_L = 2` is fixed unconditionally by LinkVPositionDepthAxiom (ASN-0047), so the cross-state depth identification used in O11''s sub-case (b) extends to every step. ∎
