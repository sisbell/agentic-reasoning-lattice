...
**D8 — Arrangement well-formedness preservation under DELETE.** For every transition `Σ → Σ'` arising from DEL[d, σ], the post-state satisfies every foundation *per-state* invariant that the pre-state was required to satisfy. The per-state invariants fall into three groups: Group (i), the arrangement invariants (S2, S8-fin, S8a, S8-depth, S3★, D-CTG★, D-MIN★, D-SEQ★, S8★, S3★-aux, CL-OWN, CL-UNIQ); and Groups (ii)–(iii), the allocation, store, transition, and remaining per-state invariants. The composite-boundary properties of ASN-0047 (P4★, P4a, P7a) are not per-state invariants and lie outside D8's scope.
...
*Composite-boundary obligations.* A valid composite ending in DEL has `Σ'` as a composite boundary, so beyond the coupling constraints `Σ'` must satisfy ASN-0047's composite-boundary properties P4★, P4a, and P7a. ASN-0047's ExtendedReachableStateInvariants theorem guarantees these at composite boundaries only for the pre-DEL vocabulary; admitting DEL obliges us to re-establish them for a DEL-terminated composite.

*Neutrality.* The composite-boundary properties P4★, P4a, P7a are not per-state invariants. In ASN-0047's ExtendedReachableStateInvariants theorem, P4★ (provenance bounds), P4a (trace-witnessing / historical fidelity), and P7a (provenance coverage) are listed *separately* from the per-state invariants — they are guaranteed only at *composite boundaries*, not at every reachable state. DEL is an elementary transition that may occur *inside* a composite, at an intermediate pre-state `Σ` that is not a composite boundary; at such a pre-state P4★ need not hold (for instance, a preceding K.μ⁺ may have placed content whose provenance a later K.ρ has not yet recorded).
...
*Boundary derivation.* Once DEL is admitted, a boundary state may be reached by a composite that contains DEL, and ASN-0047's ExtendedReachableStateInvariants theorem — proved over the pre-DEL vocabulary — does not by itself establish P4★/P7a at such a boundary. We therefore prove the boundary obligations by induction over the sequence of composite boundaries in a *DEL-extended trace* `Σ₀ →* B₁ →* B₂ →* ... →* B_N`, where each `B_j →* B_{j+1}` is a valid composite drawn from the DEL-extended vocabulary. The induction hypothesis is that every boundary `B_j` satisfies P4★ and P7a.
...
*Inductive step.* Fix a boundary `B_j` satisfying P4★ and P7a, and let `B_j →* B_{j+1}` be a valid composite. If its final step is *not* DEL, the entire composite lies in the pre-DEL vocabulary, and ASN-0047's ExtendedReachableStateInvariants theorem establishes P4★ and P7a at `B_{j+1}` directly. If its final step *is* DEL, we discharge `B_{j+1}` by the DEL-terminated derivation that follows. Together these two cases close the induction, so every boundary in a DEL-extended trace — in particular every `Σ'` reached by a DEL-terminated valid composite — satisfies P4★ and P7a.

It remains to give the DEL-terminated case. Let `Σ →* Σ'` be a valid composite whose final step is DEL, with `Σ = B_j` the immediately preceding boundary. We use four premises: P4★ at `Σ` and P7a at `Σ` (the induction hypothesis at the previous boundary `B_j` — not a fresh appeal to ASN-0047's theorem, which does not cover the DEL-containing composite that may have produced `Σ`); J0 and J1★ between `Σ` and `Σ'` (validity clause (2)); and `R ⊆ R'` (P2, provenance monotonicity).
...
The final DEL step enters this derivation only through its neutrality: it adds nothing to `dom(C)` (D2), removes nothing from `R` (`R' = R`, D0), and only shrinks `Contains_C` (P4★ neutrality bullet above) — so it can neither raise a P4★/P7a obligation nor destroy a record the coupling constraints rely on. P4★ and P7a at `Σ'` are therefore the joint product of the induction hypothesis at the previous boundary, the coupling constraints (J0, J1★) on the allocation-and-placement steps, and DEL's neutrality on the final step. This discharges the DEL-terminated case of the inductive step; with the base case `Σ₀` and the non-DEL case above, the induction closes and every composite boundary in a DEL-extended trace satisfies P4★ and P7a. P4a at `Σ'` follows directly from the neutrality bullet: `R' = R`, so DEL records no new pair, and every `(a, d) ∈ R'` keeps its pre-DEL witnessing trace state.
...
| D8 | Arrangement well-formedness preservation: every foundation per-state invariant holding at the pre-state holds at the post-state. (Body: three-group justification.) | introduced |
