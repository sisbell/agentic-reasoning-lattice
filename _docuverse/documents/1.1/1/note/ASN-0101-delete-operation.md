...
First, when the composite is well-formed, it produces a strictly longer history sequence than DEL does. On a *content-subspace* (`S = s_C`) interior span with `|V_{s_C}(d)| ≥ 2`, one could in principle apply `K.μ~` with an admissible permutation that moves the to-be-deleted I-addresses to the suffix of `V_{s_C}(d)` (the permutation is admissible because `K.μ~-FIX` preserves `dom(M(d))`, and rebinding I-addresses across content-subspace V-positions preserves D-CTG★, D-MIN★, D-SEQ★ and S3★), then apply `K.μ⁻` to truncate that suffix. The strategy is confined to `S = s_C`: K.μ~'s admissibility clause (v) — link-subspace fixing, `(A v ∈ dom_L(M(d)) :: π(v) = v)` (ASN-0047) — forbids any admissible π from relocating a link-subspace V-position, so the "move to the suffix" manoeuvre is structurally unavailable for `S = s_L` regardless of arrangement state (the obstacle-2 analysis below records this as an unconditional unavailability). The post-`K.μ~` arrangement is a state in which the to-be-deleted I-addresses still inhabit `dom(M(d))` but at different V-positions than in either the pre- or post-state of the composite. Make this precise via sequence length: SequentialAtomicTransitions (ASN-0093) treats every reachable history as a totally ordered sequence `Σ_0 → Σ_1 → ... → Σ_n` of atomic states, each obtained from its predecessor by a single elementary transition. Applied to a starting history of length `n + 1` (indices `0, ..., n`, terminating at `Σ_pre = Σ_n`), the composite `K.μ~`-then-`K.μ⁻` — counted at the elementary granularity that SequentialAtomicTransitions enforces, under which K.μ~ is not itself atomic but stands as shorthand for its named-composite decomposition `K.μ⁻ + K.μ⁺` (ASN-0047), so the full run is the three elementary transitions `K.μ⁻, K.μ⁺, K.μ⁻` — yields a post-composite history of length `n + 4` (indices `0, ..., n + 3`: `Σ_{n+1}` after K.μ~'s internal K.μ⁻, `Σ_{n+2} = Σ_mid` after K.μ~'s K.μ⁺ completes the reordering, and `Σ_{n+3} = Σ_post` after the trailing K.μ⁻ truncates). DEL as a single elementary transition yields a post-DEL history of length `n + 2` (indices `0, ..., n + 1`, with `Σ_{n+1} = Σ_post`). The post-transition history lengths differ by exactly two between the two scenarios. This length difference — not a membership predicate over states — is the load-bearing formal property, derived entirely from the foundation's state-space semantics under SequentialAtomicTransitions and independent of any implementation features such as external monitors, logs, or queries.

The length distinction is unconditional in a way that a state-membership predicate is not. Even when `Σ_mid` happens to be observationally equal to `Σ_pre` or `Σ_post` as a state tuple — which can occur, for instance, when K.μ~ permutes two V-positions both mapping to the same I-address (CL-UNIQ does not enforce uniqueness in the content subspace) and the resulting `M_mid(d)` is observably equal to `M(d)` — the composite's history still has exactly two more entries than the DEL history. Sequence length distinguishes the two scenarios in every case; a state-membership predicate `(E i : Σ_i = Σ_mid)` would collapse to a trivial truth in exactly those configurations where the intermediate state coincides with a boundary state. The architectural point: DEL closes the gap atomically, leaving the post-transition history two entries shorter than the corresponding composite would.

Second, the composite is not always *available*. K.μ~ (ASN-0047) requires, as a formal precondition, that `M(d)|_{dom_C}` — the restriction of `d`'s arrangement to the content subspace — take *at least two distinct image values*. This is a constraint on content-subspace *values*, not on content-subspace cardinality: by S5 (UnrestrictedSharing, ASN-0036) and M13 (SharedContent, ASN-0058), a document may have `|V_{s_C}(d)| ≥ 2` while every content V-position maps to a single I-address, in which case `M(d)|_{dom_C}` takes only one distinct value and K.μ~ is unavailable despite the large content-subspace cardinality. The precondition references only content-subspace values, even though K.μ~'s *effect* is a bijection on the full domain `dom(M(d))` covering both subspaces; the foundation supplies no analogue precondition conditioned on the link subspace. Consequently the K.μ~-then-K.μ⁻ composite is structurally unavailable whenever `M(d)|_{dom_C}` takes fewer than two distinct values. We catalogue when this blocks DEL. Suffix truncations need no composite — K.μ⁻ handles them alone — so only *interior* deletions are at issue. (i) A content-subspace interior deletion requires `|V_{s_C}(d)| ≥ 2`; but when all those positions map to a single I-address (an S5/M13 shared-single-value configuration), K.μ~'s admissibility clause (ii) — non-trivial net effect `M'(d) ≠ M(d)` — cannot be met by any content-subspace permutation, since transposing two positions with equal image leaves `M(d)` fixed, so no non-identity admissible permutation exists and the composite is unavailable. (When the content subspace does carry `≥ 2` distinct values, such a transposition *is* admissible — it changes `M(d)`, preserves the range in `dom(C)` so S3★ holds, and is subspace-fixing so S8a, S8-depth, CL-OWN, CL-UNIQ hold — so the composite is available for content-subspace interior deletion in that case.) (ii) A link-subspace interior deletion is *unconditionally* unavailable via the K.μ~-then-K.μ⁻ composite, independent of the content-subspace value count. The composite-substitute strategy requires K.μ~ to relocate the to-be-deleted I-addresses to the suffix of `V_{s_L}(d)`, but K.μ~'s admissibility clause (v) — link-subspace fixing, `(A v ∈ dom_L(M(d)) :: π(v) = v)` (ASN-0047) — pins every link-subspace V-position, so no admissible π can move any of them. The content-subspace precondition (`M(d)|_{dom_C}` with `≥ 2` distinct values) is irrelevant here: even a content subspace rich in distinct values cannot license a link-position relocation, because clause (v) gates the link subspace separately from the content-value precondition. Thus link-subspace interior deletion has *no* composite substitute in any configuration.

The genuine killer cases for the K.μ~-then-K.μ⁻ composite-substitute strategy are therefore twofold: (a) a *content-subspace* interior deletion on a document whose content-subspace arrangement `M(d)|_{dom_C}` takes fewer than two distinct image values (case (i), value-count gated); and (b) *every* link-subspace interior deletion (case (ii), unconditionally blocked by clause (v)). A link-subspace interior deletion with content `≥ 2` distinct values is itself a killer case — the content-value count never rescues it.
...
*Verification of D11 cross-document wps.* The pre-state on `d'` gives `discoverable_from(ℓ_0, d', Σ) = true` and `|project(L(ℓ_0).e_1, d', Σ)| = 2`. DEL acts on `d`, and is enabled at this pre-state, so `enabled(DEL[d, σ]) = true` discharges the enabledness conjunct in each cross-document wp; the residual equivalences below are the pullback factor only. We evaluate D11's cross-document bullets:

- *Cross-document discoverability wp (D11 bullet 2).* `wp(DEL[d, σ], discoverable_from(ℓ_0, d', ·)) ≡ enabled(DEL[d, σ]) ∧ discoverable_from(ℓ_0, d', Σ)`; with `enabled(DEL[d, σ]) = true` the pullback factor `discoverable_from(ℓ_0, d', Σ) = true` coincides with the pre-state predicate. Cross-check against the post-state: `project(L'(ℓ_0).e_1, d', Σ') = {[1, 1, 1], [1, 1, 2]} ≠ ∅`, so `discoverable_from(ℓ_0, d', Σ') = true`. ✓

- *Cross-document cardinality wp (D11 bullet 4).* `wp(DEL[d, σ], |project(L(ℓ_0).e_1, d', ·)| = 2) ≡ enabled(DEL[d, σ]) ∧ |project(L(ℓ_0).e_1, d', Σ)| = 2`; with `enabled(DEL[d, σ]) = true` the pullback factor `|project(L(ℓ_0).e_1, d', Σ)| = 2 = true` coincides with the pre-state cardinality predicate. Cross-check against the post-state: `|project(L'(ℓ_0).e_1, d', Σ')| = |{[1, 1, 1], [1, 1, 2]}| = 2`. ✓

The two cross-document wps confirm that DELETE on `d` is transparent to `d'`'s projection state — both predicates wp-back to themselves, and both post-state evaluations match. This is the wp form of D5's bytewise-isolation guarantee: the post-state predicate on `d'` reduces to the pre-state predicate on `d'`, with no contribution from `d`'s deletion parameters.
...
*Group (ii): Allocation and store invariants.* All of M0, S4, S7a, S7b, S7d, C1, C1b, C1c, C2, L0, L1, L1a, L1b, L1c, L3, L12, SD, L-fin, C-fin, NodeLineage, ActivatedEmission (ASN-0036, ASN-0047, ASN-0093) hold trivially at the post-state because `C' = C`, `L' = L`, `E' = E`, and `dom(M') = dom(M)` by D0's frame — every clause of every invariant in this group is a predicate over one or more of these components, and equality of each component pointwise propagates the predicate from `Σ` to `Σ'`. We catalogue each invariant by the component(s) it predicates over and the frame fact that discharges it:

- *M0 (DocumentTumblerWellFormed, ASN-0093)* predicates over `dom(M)` (T4-validity and `zeros = 2` of each document tumbler), preserved by `dom(M') = dom(M)`; each post-state `d' ∈ dom(M')` inherits its pre-state T4-validity and `zeros = 2` directly.
- *C1, C1b, C1c, C2 (ASN-0093)* range over `dom(C)` (element-level structure, element-field depth, structural inc-chain to the home document, document-scoping), with C1c additionally referencing structural inc-step admissibility and C2 additionally referencing `dom(M)`; all preserved by `dom(C') = dom(C)` and `dom(M') = dom(M)`, with structural inc-chains determined by tumbler arithmetic alone.
- *S7a, S7b (ASN-0036)* predicate over `dom(C)` (document-scoped allocation, element-level I-addresses), preserved by `dom(C') = dom(C)`; S7a additionally references `dom(M)` for the home-document membership clause, preserved by `dom(M') = dom(M)`. (The content element-field depth `≥ 2` property is C1b of ASN-0093, catalogued above, not a separate ASN-0036 invariant.)
- *S7d (ASN-0036)* predicates over document tumblers in `dom(M)` (allocation discipline producing the document-level addresses), preserved by `dom(M') = dom(M)` and `E' = E`.
- *S4 (OriginBasedIdentity, ASN-0036)* predicates over allocation events under T10a: distinct allocation events produce distinct addresses. DEL introduces no new allocation events, and `dom(C') = dom(C)` and `dom(L') = dom(L)` by D2 and D3, so the set of allocation-event addresses is unchanged; S4 carries to `Σ'` pointwise.
- *L0 (SubspacePartition, ASN-0093)* predicates over `dom(L)` (subspace `s_L` identifier) and `dom(C)` (subspace `s_C` identifier), preserved by `dom(L') = dom(L)` and `dom(C') = dom(C)`.
- *L1 (LinkElementLevel, ASN-0093)* predicates over `dom(L)` (`zeros(a) = 3`), preserved by `dom(L') = dom(L)`.
- *L1a (LinkScopedAllocation, ASN-0093)* predicates over `dom(L)` (each link's home document) and `dom(M)` (home-document membership), preserved by `dom(L') = dom(L)` and `dom(M') = dom(M)`.
- *L1b (LinkElementFieldDepth, ASN-0093)* predicates over `dom(L)` (`#E(a) ≥ 2`), preserved by `dom(L') = dom(L)`.
- *L1c (LinkAllocatorConformance, ASN-0093)* predicates over `dom(L)` and structural inc-chains from each link's home document; preserved by `dom(L') = dom(L)`, with structural inc-chains determined by tumbler arithmetic alone (the chain existence claim depends on tumbler-level structure, not on any state component beyond `dom(L)` and the home documents in `dom(M)`).
- *L3 (NEndsetStructure, ASN-0093)* predicates over `dom(L)` and per-link values `L(a)` (arity, endset membership, non-empty type endset at slot 3), preserved by `dom(L') = dom(L)` and `(A a ∈ dom(L) :: L'(a) = L(a))` from D3.
- *L12 (LinkImmutability, ASN-0093)* is a transition predicate `(A a ∈ dom(L) :: a ∈ dom(L') ∧ L'(a) = L(a))`, discharged by `dom(L') = dom(L)` and the value-equality conjunct of D3.
- *SD (StoreDisjointness, ASN-0093)* predicates over `dom(C)` and `dom(L)` jointly (`dom(C) ∩ dom(L) = ∅`), preserved by `dom(C') = dom(C)` and `dom(L') = dom(L)`.
- *L-fin (LinkStoreFiniteness, ASN-0093)* predicates over `|dom(L)|`, preserved by `dom(L') = dom(L)`.
- *C-fin (ContentStoreFiniteness, ASN-0093)* predicates over `|dom(C)|`, preserved by `dom(C') = dom(C)`.
- *NodeLineage (NodeDescentFromBootstrap, ASN-0047)* predicates over `E_node ⊆ E` (every node descends from the bootstrap node `n₀` under the prefix order), preserved by `E' = E`.
- *ActivatedEmission (ASN-0047)* predicates over `Σ.E` and the activation state of entity-level sub-allocators (`(A e ∈ Σ.E : ¬Node(e) : (E A : Activated(A) ∧ EntityLevel(A) : e ∈ dom(A))`); DEL leaves `E' = E` and performs no entity-allocation event, so both the entity set and the activated-sub-allocator set are unchanged, and the predicate transfers pointwise.

Likewise the substrate-level chain-discipline lemmas — ChainElementT4Validity, ChainEnumerationInjectivity, DisjointSubAllocatorChains, ChainPrefixExtension, ChainMembershipForOrigin, StoreT4Validity, FirstEmissionFreshness, and CrossDocumentDisjointness (ASN-0093) — hold trivially at `Σ'`: each is a structural property of the allocator chains `A_C(d_i)` and `A_L(d_i)` indexed by `d_i ∈ dom(M)`, with the realised content and link chain elements determined by `dom(C)` and `dom(L)` respectively. Since `dom(C') = dom(C)`, `dom(L') = dom(L)`, and `dom(M') = dom(M)` under D0's frame, every chain's realised element set is unchanged, and each chain-discipline predicate transfers from `Σ` to `Σ'` pointwise.

*Group (iii): Transition and per-state invariants discharged by frame.* The group mixes transition predicates (which compare `Σ` and `Σ'`) with per-state predicates (which range over a single state); both kinds reduce to triviality under D0's frame because the components they predicate over — `(C, L, E, R, dom(M))` — are pointwise unchanged. Members: M1 (arrangement monotonicity, ASN-0093), C0 (content immutability, ASN-0093), P0, P1, P2, P3 (permanence and arrangement-mutability-only), P6, P7 (existence and grounding), P8 (entity hierarchy), and L12a, L12b (link-store monotonicity and home persistence). Each is preserved trivially: M1 by `dom(M') = dom(M)` (which entails `dom(M) ⊆ dom(M')` vacuously); C0 by `dom(C') = dom(C)` together with the value-preservation conjunct of D2; P0 by `dom(C') = dom(C)`; P1 by `E' = E`; P2 by `R' = R`; P3 as the conjunction of P0, P1, P2 plus L12; P6, P7, P8 by `dom(C') = dom(C)`, `dom(L') = dom(L)`, `E' = E`, `R' = R`, and the structural definition of `origin`; L12a, L12b similarly.

*The composite-boundary properties P4★, P4a, P7a are not per-state invariants.* In ASN-0047's ExtendedReachableStateInvariants theorem, P4★ (provenance bounds), P4a (trace-witnessing / historical fidelity), and P7a (provenance coverage) are listed *separately* from the per-state invariants — they are guaranteed only at *composite boundaries*, not at every reachable state. By D10, DEL is an elementary transition that may occur *inside* a composite, at an intermediate pre-state `Σ` that is not a composite boundary; at such a pre-state P4★ need not hold (for instance, a preceding K.μ⁺ may have placed content whose provenance a later K.ρ has not yet recorded). We therefore do not claim DEL *preserves* these three as per-state invariants, nor do we assume them at the DEL pre-state. What we establish is the strictly weaker and licensed fact that DEL *cannot break* them — it is neutral-to-helpful for each — so it never converts a boundary-satisfying state into a violating one; the boundary obligation itself is discharged at the composite level (D10), not by any per-step assumption.

- *P4★ (`Contains_C(Σ) ⊆ R`).* DEL is content-subspace-monotone-shrinking: `Contains_C(Σ') ⊆ Contains_C(Σ)`. To see this, let `(a, d'') ∈ Contains_C(Σ')`, so there is `v ∈ dom(M'(d''))` with `subspace(v) = s_C` and `M'(d'')(v) = a`; we lift `v` to a pre-state witness. When `d'' ≠ d`, D5 gives `M'(d'') = M(d'')`, so `v ∈ dom(M(d''))` with `M(d'')(v) = a` and `(a, d'') ∈ Contains_C(Σ)`. When `d'' = d`, D0's effect partitions `v ∈ dom(M'(d))` into `Λ ⊎ Q ⊎ V_{S'}(d)`, where `S' = {s_C, s_L} \ {S}` is the unaffected subspace (the deletion is in subspace `S`, which may itself be either `s_C` or `s_L`); restricting to `subspace(v) = s_C` and tracing the source correspondence, either `v ∈ Λ` (so `M'(d)(v) = M(d)(v) = a` and `v` itself witnesses the pre-state pair) or `v ∈ Q` (so `v = σ_d(u)` for the unique `u ∈ Π ⊆ V_S(d)` with `M(d)(u) = a` and `u` witnesses the pre-state pair) or `v ∈ V_{S'}(d)` with `S' = s_C` and `S = s_L` (so `v ∈ V_{s_C}(d)` is unchanged with `M'(d)(v) = M(d)(v) = a`). In every case a pre-state witness exists, so `(a, d'') ∈ Contains_C(Σ)`, establishing `Contains_C(Σ') ⊆ Contains_C(Σ)`. Combined with `R' = R` (D0's frame on the provenance relation): if the pre-state inclusion `Contains_C(Σ) ⊆ R` happens to hold, then `Contains_C(Σ') ⊆ Contains_C(Σ) ⊆ R = R'` carries it to the post-state; and if it does not hold, DEL adds nothing to `Contains_C`, so it introduces no new violation. Either way DEL cannot break P4★ — it is the monotone-shrinking-and-`R`-fixing direction that matters, not a (false) per-state assumption at the DEL pre-state.

- *P4a (trace-witnessing).* `R' = R`, and any pair `(a, d) ∈ R` witnessed at some `Σ_k` in the pre-state history remains witnessed at that same `Σ_k` in the (longer) post-state history; DEL records no new provenance pair, so it raises no new witnessing obligation. DEL cannot break P4a.

- *P7a (provenance coverage: every `a ∈ dom(C)` has some `(a, d) ∈ R`).* `dom(C') = dom(C)` (D2) and `R' = R` (D0's frame), so both the content-address set and the provenance relation are pointwise unchanged; DEL therefore neither creates an unrecorded content address nor removes a provenance record, and its truth value for P7a is identical at `Σ` and `Σ'`. DEL cannot break P7a.

The composite-level discharge of P4★, P4a, P7a for DEL-terminated composites — establishing them affirmatively at the boundary `Σ'` — is supplied in D10.
...
S3★ holds via source correspondence: pre-state S3★ on `u` gives `M(d)(u) ∈ dom(C)` when `subspace(u) = s_C` and `M(d)(u) ∈ dom(L)` when `subspace(u) = s_L`; since `subspace(v) = subspace(u)` (established above for each of `Λ`, `Q`, and the unaffected-subspace cases), S3★'s biconditional at `u` on the pre-state transfers to `v` on the post-state under the same subspace classification — `subspace(v) = s_C` selects the `dom(C)`-branch and `subspace(v) = s_L` selects the `dom(L)`-branch, with the same image `M(d)(u)` discharging each; combined with D2's `dom(C') = dom(C)` and D3's `dom(L') = dom(L)`, the post-state store membership `M'(d)(v) ∈ dom(C')` or `M'(d)(v) ∈ dom(L')` follows in each case. S3★-aux holds because `subspace(v) = subspace(u) ∈ {s_C, s_L}` by pre-state S3★-aux on `u`. S8★ splits into three conditions: (a) lockstep displacement and (b) label well-definedness, which ASN-0047's S8★ requires on both subspaces, and (c) uniqueness of the maximal-run decomposition, which ASN-0047's S8★ retains *only* on the content subspace. Conditions (a) and (b) hold at the post-state by the trivial singleton decomposition `{(v, M'(d)(v), 1) : v ∈ V_S(M'(d))}` for the affected subspace — S8-fin establishes finiteness, S8's condition (a) holds by construction (each post-state V-position is its own length-1 run), and condition (b) holds trivially at length 1: the quantifier ranges over `k ∈ {0}` (non-empty), and at `k = 0` the equation `M'(d)(shift(v_j, 0)) = shift(a_j, 0)` reduces by OrdinalShiftBase (`shift(t, 0) = t`) to `M'(d)(v_j) = a_j`, the run's defining equation — and by D6 (inheritance from the unchanged pre-state arrangement) for the other subspace. Condition (c) demands a separate argument, because the singleton decomposition is in general *not* the maximal-run decomposition and so cannot witness uniqueness of it. When the affected subspace is `S = s_C`, we discharge (c) by ASN-0058's M12 (CanonicalUniqueness), whose uniqueness conclusion rests on the existence machinery of M11/M2 (and, via the single-subspace restriction route C1a, on the same standing set). M12 does *not* depend on S2 and S8-fin alone; the full precondition set it inherits through M2/C1a is the standing arrangement-shape package: S2 (functionality), S8-fin (finite domain), S3 (referential integrity), S8a (V-position well-formedness), and S8-depth (per-subspace common depth). Each of these is established at the post-state earlier in this Group (i) justification: S2 above; S8-fin above; S8a above; S8-depth above; and S3 on the content subspace as the `subspace(v) = s_C ⟹ M'(d)(v) ∈ dom(C')` branch of post-state S3★ (established just above). With the complete standing set discharged at the post-state, M12 yields a *unique* maximal-run decomposition of the post-state content arrangement — discharging (c) directly, independent of the singleton-decomposition argument that handles (a) and (b). When `S = s_L`, the content subspace is unchanged by D6 and inherits (c) from the pre-state. So (c) holds on the content subspace in both cases.

The (a)-and-(b) obligation is *existential*: it requires the existence of some finite decomposition satisfying S8 (a) and (b), and the singleton decomposition supplied here discharges that obligation at its weakest form. The earlier observation (the "no reconciliation across the gap" remark) noted that two runs separated by the deleted region become V-adjacent in the post-state, and that whether their I-extents are now I-adjacent — and the runs therefore mergeable under ASN-0058's bundle-algebra rules — is in general indeterminate. The singleton decomposition is unaffected by that indeterminacy: it satisfies (a) and (b) regardless of which I-adjacencies hold. The uniqueness condition (c) is equally independent of the indeterminacy: M12 fixes the maximal-run decomposition uniquely from the post-state arrangement function whatever the merge pattern turns out to be — the merge pattern determines *what* the unique maximal-run decomposition is, not *whether* it is unique. ASN-0058's M11 (CanonicalExistence) supplies that maximally merged decomposition and M12 (CanonicalUniqueness) certifies it is the only one, discharging (c); the singleton decomposition remains the available witness for (a) and (b). Either way, the post-state's full S8★ obligation — (a) and (b) on the affected subspace, and (c) on the content subspace — is met.
...
**D11 — Weakest preconditions for projection postconditions.** Let `Σ → Σ'` be a DEL[d, σ] transition with σ = (s, ℓ_σ) of subspace `S`, and let `Λ`, `X`, `Π ⊆ V_S(d)` be the regions of D0. Write `Q_disc(ℓ, d)` for the post-state predicate `discoverable_from(ℓ, d, ·)`, and `Q_card(ℓ, i, d, k)` for the post-state predicate `|project(L(ℓ).eᵢ, d, ·)| = k`. Write `enabled(DEL[d, σ])` for DEL's applicability predicate — the conjunction of D0's preconditions (document membership `d ∈ dom(M)`, span well-formedness, and containment) — which is exactly `wp(DEL[d, σ], true)`. Because DEL is partial, each weakest precondition for total correctness conjoins `enabled(DEL[d, σ])` with the postcondition's pullback, exactly as LP12a (ASN-0098) conjoins `enabled(K.μ⁻[d, R])`; without the guard, a pre-state at which DEL is inapplicable could spuriously satisfy the pullback clause and the predicate would not be the weakest precondition for total correctness.

*DEL is deterministic.* Each component of `Σ'` is uniquely determined by `Σ` and the parameters `(d, σ)`: by D0's frame `C' = C`, `L' = L`, `E' = E`, `R' = R`, and `M'(d') = M(d')` for every `d' ≠ d`; the affected component `M'(d)` is constructed pointwise from `M(d)` and `σ_d` (itself a function of `σ`), with each value `M'(d)(v) = M(d)(u)` for the unique source `u` determined by `v`'s region. When `v ∈ Λ ∪ V_{S'}(d)` (the unaffected positions, including the unaffected subspace `S' ≠ S`), `u = v` by D0's effect, and uniqueness is immediate. When `v ∈ Q`, `u = σ_d^{-1}(v) ∈ Π`, with the inverse `σ_d^{-1}` well-defined as a function because D1 establishes that `σ_d : Π → Q` is a bijection (uniqueness of the source supplied by TS2 ShiftInjectivity at length `m_S`, surjectivity by D1's construction). Hence the wp transformer satisfies `wp(DEL[d, σ], ¬Q) ≡ enabled(DEL[d, σ]) ∧ ¬wp(DEL[d, σ], Q)` for every postcondition `Q` — the enabledness guard is retained on negation because DEL is partial (for a partial deterministic command `wp(S, ¬Q) ≡ wp(S, true) ∧ ¬wp(S, Q)`, and here `wp(S, true) = enabled(DEL[d, σ])`) — licensing the negation equivalences below.
...
- *wp for post-DELETE discoverability from a different document `d'' ≠ d` (with `d'' ∈ dom(M)`):*
  ...
  ```
  Discoverability from any other document is invariant under DEL[d, σ], so the pullback clause coincides with the pre-state predicate; the enabledness guard remains, since the postcondition can be guaranteed only when DEL actually runs.
...
- *wp for post-DELETE projection cardinality from `d`:*
  ...
  ```
  In particular, *the projection's cardinality is preserved* iff DEL is enabled and no pre-state projection element lies in the deleted region:
  ```
...
- *wp for post-DELETE projection cardinality from a different document `d'' ≠ d` (with `d'' ∈ dom(M)`):*
  ...
  ```
  Projection cardinality from any other document is invariant under DEL[d, σ] (the projection itself is bytewise-invariant by D9's first clause), so the pullback clause coincides with the pre-state cardinality predicate, conjoined with the enabledness guard. The bullet is the cardinality analogue of the cross-document discoverability bullet above and completes the symmetry between the four projection-postcondition cases (discoverability and cardinality, from `d` and from `d'' ≠ d`).

*Justification.* Each wp below is the conjunction of `enabled(DEL[d, σ]) = wp(DEL[d, σ], true)` — DEL's applicability predicate, which must hold for the transition to occur and for total correctness to be meaningful — with the postcondition's pullback through DEL's deterministic effect. The arguments that follow compute the pullback clause; the enabledness conjunct is carried uniformly and is omitted from the intermediate steps for brevity. For discoverability from `d`: by definition, `discoverable_from(ℓ, d, Σ') ≡ (E i : project(L'(ℓ).eᵢ, d, Σ') ≠ ∅)`. By D3, `L'(ℓ).eᵢ = L(ℓ).eᵢ`, so `coverage(L'(ℓ).eᵢ) = coverage(L(ℓ).eᵢ)`. By D6, the projection restricted to subspace `S' ≠ S` is preserved bytewise: `project(L'(ℓ).eᵢ, d, Σ') ∩ V_{S'}(d) = project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d)`. By D9's third clause, the projection restricted to subspace `S` is `(project(L(ℓ).eᵢ, d, Σ) ∩ Λ) ∪ {σ_d(v) : v ∈ project(L(ℓ).eᵢ, d, Σ) ∩ Π}`. The full post-state projection is the union of these two restrictions; it is non-empty iff at least one summand is non-empty. The `Λ` summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ Λ ≠ ∅`; the shifted Π summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ Π ≠ ∅` (because σ_d is a bijection on Π — D1); the cross-subspace summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d) ≠ ∅`. The post-state projection is non-empty iff at least one of its three components is non-empty, equivalently iff `project(L(ℓ).eᵢ, d, Σ) ∩ (Λ ∪ Π ∪ V_{S'}(d)) ≠ ∅`. Computing the union: `Λ ∪ Π = V_S(d) \ X` (the disjoint partition `V_S(d) = Λ ⊎ X ⊎ Π` from D0's region definitions excludes `X`), so `Λ ∪ Π ∪ V_{S'}(d) = (V_S(d) \ X) ∪ V_{S'}(d) = (V_S(d) ∪ V_{S'}(d)) \ X = dom(M(d)) \ X` (using `X ∩ V_{S'}(d) = ∅`, which holds because `X ⊆ V_S(d)` and `V_S(d) ∩ V_{S'}(d) = ∅` by subspace disjointness). Since `project(L(ℓ).eᵢ, d, Σ) ⊆ dom(M(d))` by definition, intersecting with `dom(M(d)) \ X` is equivalent to subtracting `X`, which is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ⊄ X`. Existential quantification over slots completes the wp. For discoverability from `d'' ≠ d`: by D9's first clause, `project(L'(ℓ).eᵢ, d'', Σ') = project(L(ℓ).eᵢ, d'', Σ)` for every slot `i`, so the predicate `(E i : project(L'(ℓ).eᵢ, d'', Σ') ≠ ∅)` is equivalent to `(E i : project(L(ℓ).eᵢ, d'', Σ) ≠ ∅) = discoverable_from(ℓ, d'', Σ)`. For projection cardinality: by D9's third clause and the bijectivity of σ_d (D1), the post-state projection on subspace `S` has cardinality `|project(L(ℓ).eᵢ, d, Σ) ∩ Λ| + |project(L(ℓ).eᵢ, d, Σ) ∩ Π|` — the first summand from the unshifted `Λ` contribution, the second from the bijection-preserved Π contribution `{σ_d(v) : v ∈ project ∩ Π}` (bijection preserves cardinality on Π). Two partition facts collapse this to a single intersection. (i) `Λ ∩ Π = ∅` by last-component ranges (Λ-positions have last component `≤ p − 1`, Π-positions `≥ p + n > p − 1`, established in D8's S2 justification). (ii) `V_S(d) = Λ ⊎ X ⊎ Π` as a disjoint union by D0's region definitions (each `v ∈ V_S(d)` falls into exactly one of `v < s` (Λ), `s ≤ v < r` (X), or `v ≥ r` (Π), exhaustively by trichotomy). Adding cardinalities via (i): `|project ∩ Λ| + |project ∩ Π| = |project ∩ (Λ ∪ Π)|`. Applying (ii) by inclusion-exclusion on the partition: `|project ∩ V_S(d)| = |project ∩ Λ| + |project ∩ X| + |project ∩ Π|`, so `|project ∩ (Λ ∪ Π)| = |project ∩ V_S(d)| − |project ∩ X|`. Thus the post-state projection cardinality on subspace `S` equals `|project(L(ℓ).eᵢ, d, Σ) ∩ V_S(d)| − |project(L(ℓ).eᵢ, d, Σ) ∩ X|`; combined with the cross-subspace contribution preserved bytewise by D6 (`|project(L(ℓ).eᵢ, d, Σ') ∩ V_{S'}(d)| = |project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d)|`), the post-state total cardinality is `|project(L(ℓ).eᵢ, d, Σ)| − |project(L(ℓ).eᵢ, d, Σ) ∩ X|`. Setting this equal to `k` yields the wp. The specialisation to "cardinality preserved" sets `k = |project(L(ℓ).eᵢ, d, Σ)|`, reducing to `|project(L(ℓ).eᵢ, d, Σ) ∩ X| = 0`, i.e., `project(L(ℓ).eᵢ, d, Σ) ∩ X = ∅`. For cross-document projection cardinality (`d'' ≠ d`): by D9's first clause, `project(L'(ℓ).eᵢ, d'', Σ') = project(L(ℓ).eᵢ, d'', Σ)` as sets, so their cardinalities coincide; the post-state predicate `|project(L(ℓ).eᵢ, d'', ·)| = k` therefore evaluates at `Σ'` to the pre-state predicate `|project(L(ℓ).eᵢ, d'', Σ)| = k`, which is the wp. ∎
...
(2) *Coupling constraints and composite-boundary obligations.* J0, J1★, and J1'★ (ASN-0047) hold between the composite's initial state `Σ` and final state `Σ'`; and, because `Σ'` is a composite boundary, `Σ'` additionally satisfies ASN-0047's composite-boundary properties P4★, P4a, and P7a. The coupling constraints are treated immediately below; the boundary obligations are discharged for DEL-containing composites in the *Composite-boundary obligations* paragraph that follows.
...
*Composite-boundary obligations.* A valid composite that ends in a DEL step has `Σ'` as a composite boundary, so beyond the coupling constraints `Σ'` must satisfy ASN-0047's composite-boundary properties P4★, P4a, and P7a. These are not discharged by the per-step vacuity arguments above — like J0/J1★/J1'★, the boundary properties are evaluated on the composite as a whole — but DEL is *neutral-to-helpful* for all three, by exactly the facts D8 establishes: DEL is content-subspace-monotone-shrinking (`Contains_C(Σ') ⊆ Contains_C(Σ)`) and store/provenance-fixing (`dom(C') = dom(C)`, `R' = R`). Consequently a DEL step — terminal or interior — cannot turn a state satisfying a boundary property into one that violates it: P4★ can only lose pairs from `Contains_C` while `R` is held fixed, so the inclusion `Contains_C ⊆ R` is never newly broken; P4a's witnessing history only lengthens and DEL records no new provenance pair, so every `(a, d) ∈ R = R'` keeps its historical witness; P7a's content-address set `dom(C)` and provenance relation `R` are both pointwise unchanged, so coverage is neither created nor destroyed. The affirmative establishment of P4★, P4a, P7a at `Σ'` is therefore the work of the composite's non-DEL steps — the same K.α/K.μ⁺/K.ρ machinery by which ASN-0047 discharges these boundary properties for DEL-free composites — and every DEL step in the composite preserves what those steps establish. This parallels the J0/J1★/J1'★ treatment: DEL produces no boundary-property obligation of its own and cannot disrupt P4★/P4a/P7a. (The one disruption DEL *can* cause — removing a content-subspace witness — bears on the J0/J1★ coupling constraints, addressed next, not on these three monotone-shrinking-compatible properties.)
...
*LP-family extension under DELETE.* ASN-0098's LP-Comp note enumerates a covering case-analysis of the per-step projection lemmas LP2 through LP14 over the ASN-0047 + ASN-0093 vocabulary, which does not include DEL; beyond that note ASN-0098 also establishes the discoverability and tightness lemmas LP16 through LP21 and the substrate-structural lemmas LP-Sub (SubstrateContainment), LP-Fin (IntervalFinitude), and LP-Fin Corollary (CanonicalIntervalCharacterisation), all of which are likewise stated over a vocabulary that does not include DEL. Admitting DEL into the vocabulary (and into reachable state sequences `Σ →* Σ'`) requires that the entire LP-family — LP2 through LP14, LP16 through LP21, and the three substrate-structural lemmas — extend to DEL-containing sequences. The extension is supplied directly by D2, D3, D5, D6, D9, and D11, without requiring new LP-family lemmas. We catalogue the dispatch:
- *LP2 (SlotInvariance) under the Closure schema (★), LP3★ (MultiStepCoverageInvariance), LP13 (UnconditionalLinkPersistence)* extend to DEL trivially via D3: since `L' = L` exactly under DEL, every link's slot endsets and their coverage are pointwise unchanged across every DEL step, so the multi-step transit through DEL preserves slot identity and coverage in the same way as it does through any LP-covered step. The lemmas' "every reachable state sequence" universals therefore close under the extended vocabulary by the same induction on sequence length, with DEL contributing the identity step on `Σ.L`.
...
- *LP16 (TransclusionDiscoverability), LP17 (GhostProjection), LP20 (RangeConfinement), LP21 (RepresentationInvariance)* are state-relative in the same sense as LP12 — each is a property of a single state `Σ`, quantifying over `coverage`, `ran(M(d))`, `dom(C)`, and `dom(L)` at that state, with no appeal to how `Σ` was reached. They therefore apply to the post-state `Σ'` of any DEL transition by direct evaluation at `Σ'`, requiring no DEL-specific extension. (LP17 and LP20 read off `dom(C') = dom(C)` and `dom(L') = dom(L)` — D2, D3 — so the store-confinement and ghost characterisations name the same I-address sets pre- and post-DEL; LP21 depends only on `coverage` equality, preserved by D3.)
- *LP18 (Resurrection)* is the one discoverability lemma that ranges over a multi-step sequence `Σ →* Σ'`, and it is the lemma the orphan-reversibility prose above invokes with DEL as the orphaning step. Its dispatch under DEL rests on the two multi-step mechanisms ASN-0098 already establishes, both of which hold across DEL: *Store Monotonicity★* lifts to DEL-containing sequences because `dom(C') = dom(C)` (D2) and `dom(L') = dom(L)` (D3) — neither store shrinks under DEL, so the inclusion `dom(Σ.L) ⊆ dom(Σ'.L)` that LP18 needs to carry the orphaned link forward survives every DEL step — and *LP3★ (MultiStepCoverageInvariance)* extends through DEL via D3 (catalogued in the LP3★ bullet above), so `coverage(Σ'.L(a).eᵢ) = coverage(Σ.L(a).eᵢ)` across any sequence threading DEL. With these two carried through, LP18's four-step chain (link persistence, coverage invariance, the resurrecting arrangement entry, projection non-emptiness) closes over the DEL-extended vocabulary unchanged; DEL appears in the chain only as a step that fixes both stores and at most relocates or removes arrangement entries on a single document.
- *LP19 (TightEndsetBoundaryExclusion), LP19a (TightFreshness)* concern the freshly allocated addresses of K.α/K.λ steps and the boundary V-positions of K.μ⁺/K.μ⁺_L extension steps. DEL allocates no address (`dom(C') = dom(C)`, `dom(L') = dom(L)` by D2, D3) and extends no arrangement (DEL only contracts-and-shifts within a single document by D0), so DEL is never the allocation or extension step these lemmas quantify over. Where LP19a's argument runs over a prefix `Σ_e →* Σ` to establish freshness against an earlier tight state, any DEL steps on that prefix are absorbed by the same Store Monotonicity★ appeal LP19a already makes — DEL preserves both stores, so the freshness witness `a_new ∉ dom(Σ_e.C) ∪ dom(Σ_e.L)` propagates through DEL steps verbatim. Neither lemma requires a DEL-specific clause; their original universals stand, with DEL admitted to the prefix vocabulary harmlessly.
- *LP-Sub (SubstrateContainment), LP-Fin (IntervalFinitude), LP-Fin Corollary (CanonicalIntervalCharacterisation)* are the substrate-structural lemmas underwriting the tightness lemmas above (LP12b, LP19, LP19a). Each holds at any post-DEL state without DEL-specific extension. *LP-Sub* (`dom(Σ.C) ∪ dom(Σ.L) ⊆ F`) is state-relative and reads its two stores off the post-state; under DEL `dom(C') = dom(C)` (D2) and `dom(L') = dom(L)` (D3), so the post-state stores are the same address sets that satisfied the inclusion pre-DEL, and the inclusion holds at `Σ'` by direct evaluation. *LP-Fin* and *LP-Fin Corollary* are purely tumbler-structural: each quantifies over canonical spans `(s, ℓ)` with `s ∈ F` and computes `F ∩ [s, s ⊕ ℓ)` from the structural form of `F` alone, with no appeal to any state component or transition kind. They therefore hold independently of how a state was reached and require no extension under the DEL-augmented vocabulary; DEL neither alters `F` (a fixed structural set of emittable addresses) nor any canonical span's coverage interval.

The combined effect: ASN-0098's projection apparatus — LP-Comp's per-step lemmas LP2 through LP14, the discoverability/tightness lemmas LP16 through LP21, and the substrate-structural lemmas LP-Sub, LP-Fin, and LP-Fin Corollary — carries to the DEL-extended vocabulary intact. D9 and D11 close the only two genuinely DEL-specific extension points (the affected document's per-step projection behaviour and the DEL-specific wp); every remaining lemma is either state-relative and applies to the post-state directly (LP12, LP16, LP17, LP20, LP21, LP-Sub), purely tumbler-structural and state-independent (LP-Fin, LP-Fin Corollary), threads through the prefix via Store Monotonicity★ and LP3★ (LP18, LP19, LP19a), or concerns a non-DEL operation and needs no extension (LP6–LP8, LP14). LP-Comp's case-analysis, together with this catalogue of the discoverability, tightness, and substrate-structural lemmas, remains exhaustive over the extended vocabulary once the present cataloguing is taken as the DEL row.
...
| D8 | Arrangement well-formedness preservation: all foundation *per-state* invariants — Group (i) arrangement invariants (S2, S8-fin, S8a, S8-depth, S3★, S3★-aux, D-CTG★, D-MIN★, D-SEQ★, S8★, CL-OWN, CL-UNIQ), Group (ii) allocation/store invariants (M0, S4, S7a, S7b, S7d, C1, C1b, C1c, C2, L0, L1, L1a–c, L3, L12, SD, L-fin, C-fin, NodeLineage, ActivatedEmission, plus substrate chain-discipline lemmas ChainElementT4Validity, ChainEnumerationInjectivity, DisjointSubAllocatorChains, ChainPrefixExtension, ChainMembershipForOrigin, StoreT4Validity, FirstEmissionFreshness, CrossDocumentDisjointness), Group (iii) transition/per-state invariants discharged by frame (M1, C0, P0, P1, P2, P3, P6, P7, P8, L12a, L12b) — all hold at the post-state. The ASN-0047 *composite-boundary* properties P4★, P4a, P7a are not per-state invariants; DEL cannot break them (content-subspace-monotone-shrinking `Contains_C(Σ') ⊆ Contains_C(Σ)`, with `dom(C') = dom(C)` and `R' = R`), and their affirmative discharge at a boundary is a composite-level obligation (D10). | introduced |
...
| D10 | ValidComposite★ extension under DELETE: ASN-0047's ValidComposite★ admits DEL as an elementary transition; J0, J1★, J1'★ hold vacuously on a one-step DEL composite, but composite-level validity for multi-step composites containing DEL is not automatic — DEL can remove a V-position witness required to discharge an earlier step's allocation obligation. The composite-boundary properties P4★, P4a, P7a (ASN-0047) are discharged for DEL-terminated composites: DEL is neutral-to-helpful for all three (content-subspace-monotone-shrinking, `dom(C') = dom(C)`, `R' = R`), so the boundary properties are established by the composite's non-DEL steps and preserved by every DEL step. Includes the LP-family extension catalogue (ASN-0098): LP2★, LP3★, LP13 extend to DEL via D3; LP4, LP5 via D5; LP6, LP7, LP8, LP14, LP9, LP10, LP11 are vocabulary-disjoint from DEL with D9 supplying the DEL-specific projection characterisation; LP12 applies state-relatively to the post-state; LP12a, LP12b are supplanted for DEL by D11; LP-Sub applies state-relatively via D2/D3, and LP-Fin and LP-Fin Corollary are purely tumbler-structural and hold without extension. | introduced |
