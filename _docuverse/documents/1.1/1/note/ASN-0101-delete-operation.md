...
*Group (ii): Allocation and store invariants.* All of M0, S4, S7a, S7b, S7c, S7d, C1, C1b, C1c, C2, L0, L1, L1a, L1b, L1c, L3, L12, L14, L-fin, C-fin, NodeLineage (ASN-0036, ASN-0043, ASN-0093) hold trivially at the post-state because `C' = C`, `L' = L`, `E' = E`, and `dom(M') = dom(M)` by D0's frame — every clause of every invariant in this group is a predicate over one or more of these components, and equality of each component pointwise propagates the predicate from `Σ` to `Σ'`. We catalogue each invariant by the component(s) it predicates over and the frame fact that discharges it:
...
- *S7a, S7b, S7c (ASN-0036)* predicate over `dom(C)` (document-scoped allocation, element-level I-addresses, element-field depth `≥ 2`), preserved by `dom(C') = dom(C)`; S7a additionally references `dom(M)` for the home-document membership clause, preserved by `dom(M') = dom(M)`.
...
- *L14 (StoreDisjointness, ASN-0093)* predicates over `dom(C)` and `dom(L)` jointly (`dom(C) ∩ dom(L) = ∅`), preserved by `dom(C') = dom(C)` and `dom(L') = dom(L)`.
