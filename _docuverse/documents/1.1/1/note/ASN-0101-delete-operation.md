...
*Composite-boundary obligations.* A valid composite ending in DEL has `Σ'` as a composite boundary, so beyond the coupling constraints `Σ'` must satisfy ASN-0047's composite-boundary properties P4★, P4a, and P7a. ASN-0047's ExtendedReachableStateInvariants theorem guarantees these at composite boundaries only for the pre-DEL vocabulary; admitting DEL obliges us to re-establish them for a DEL-terminated composite. We argue in two parts: DEL's neutrality, then the boundary derivation.

*Neutrality (D8).* A DEL step preserves each composite-boundary property it touches: P4★ because `R' = R` (D0's frame) and DEL only content-subspace-monotone-shrinks `Contains_C`; P7a because `dom(C') = dom(C)` (D2) and `R' = R`; P4a because `R' = R` records no new provenance pair, so every `(a, d) ∈ R'` retains the witnessing trace state it had in the pre-DEL history. Neutrality says only that DEL cannot *break* these properties — not that they *hold* at `Σ'`.

*Boundary derivation.* Let `Σ →* Σ'` be a valid composite whose final step is DEL. We use four premises: P4★ at `Σ` and P7a at `Σ` (both hold because the composite's initial state `Σ` is itself a composite boundary); J0 and J1★ between `Σ` and `Σ'` (validity clause (2)); and `R ⊆ R'` (P2, provenance monotonicity).
...
The final DEL step enters this derivation only through its neutrality: it adds nothing to `dom(C)` (D2), removes nothing from `R` (`R' = R`, D0), and only shrinks `Contains_C` (D8) — so it can neither raise a P4★/P7a obligation nor destroy a record the coupling constraints rely on. P4★ and P7a at `Σ'` are therefore the joint product of the coupling constraints (J0, J1★) on the allocation-and-placement steps and DEL's neutrality on the final step. P4a at `Σ'` follows directly from the neutrality bullet: `R' = R`, so DEL records no new pair, and every `(a, d) ∈ R'` keeps its pre-DEL witnessing trace state.
