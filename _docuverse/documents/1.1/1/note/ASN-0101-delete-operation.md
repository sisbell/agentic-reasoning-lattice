...
*Verification of D11 cross-document wps.* The pre-state on `d'` gives `discoverable_from(ℓ_0, d', Σ) = true` and `|project(L(ℓ_0).e_1, d', Σ)| = 2`. We evaluate D11's cross-document bullets:

- *Cross-document discoverability wp (D11 bullet 2).* `wp(DEL[d, σ], discoverable_from(ℓ_0, d', ·)) ≡ discoverable_from(ℓ_0, d', Σ) = true`. The wp coincides with the pre-state predicate. Cross-check against the post-state: `project(L'(ℓ_0).e_1, d', Σ') = {[1, 1, 1], [1, 1, 2]} ≠ ∅`, so `discoverable_from(ℓ_0, d', Σ') = true`. ✓

- *Cross-document cardinality wp (D11 bullet 4).* `wp(DEL[d, σ], |project(L(ℓ_0).e_1, d', ·)| = 2) ≡ |project(L(ℓ_0).e_1, d', Σ)| = 2 = true`. The wp coincides with the pre-state cardinality predicate. Cross-check against the post-state: `|project(L'(ℓ_0).e_1, d', Σ')| = |{[1, 1, 1], [1, 1, 2]}| = 2`. ✓
