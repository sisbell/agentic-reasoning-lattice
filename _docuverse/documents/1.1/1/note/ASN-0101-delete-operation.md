...
First, when the composite is well-formed, it exposes a *distinguishable intermediate state*. On an interior span with `|V_S(d)| ≥ 2`, one could in principle apply `K.μ~` with an admissible permutation that moves the to-be-deleted I-addresses to the suffix of `V_S(d)` (the permutation is admissible because `K.μ~-FIX` preserves `dom(M(d))`, and rebinding I-addresses across V-positions preserves D-CTG★, D-MIN★, D-SEQ★ and S3★), then apply `K.μ⁻` to truncate that suffix. The post-`K.μ~` arrangement is a state in which the to-be-deleted I-addresses still inhabit `dom(M(d))` but at different V-positions than in either the pre- or post-state of the composite. Make this precise: SequentialAtomicTransitions (ASN-0093) treats every reachable history as a totally ordered sequence `Σ_0 → Σ_1 → ... → Σ_n` of atomic states, each obtained from its predecessor by a single elementary transition. The two-step composite `K.μ~` then `K.μ⁻` produces such a sequence `Σ_pre → Σ_mid → Σ_post` of length two, and the state `Σ_mid` is itself a member of the system's history. The predicate `(E v : v ∈ dom(M(d)) : M(d)(v) = a)` for any to-be-deleted I-address `a` evaluates to `true` at `Σ_pre`, to `true` at `Σ_mid` (at the post-`K.μ~` V-position), and to `false` at `Σ_post`; the predicate `M(d)(v_1) = a_1` for a pre-state pairing `v_1 ∈ Λ` evaluates differently at `Σ_mid` than at `Σ_pre` whenever the permutation moves elements of `Λ`. Either predicate distinguishes the intermediate from at least one endpoint, witnessing `Σ_mid` as a state distinct from both. DEL as a single elementary transition produces a sequence `Σ_pre → Σ_post` of length one, with no intermediate state to be distinguished.

Second, the composite is not always *available*. K.μ~ (ASN-0047) requires `|dom_C(M(d))| ≥ 2` as a formal precondition. When DEL operates on a content subspace with `n_S = 1` (a single-position content subspace deleted in full, i.e., `p = 1` and `n = 1`), no admissible permutation exists and the composite cannot be constructed at all. Even cases with `|dom_C(M(d))| ≥ 2` where the *only* admissible re-arrangement is the identity (forbidden by K.μ~'s `π ≠ id` clause) fall outside the composite's domain. Moreover, K.μ~'s admissibility *precondition* references only content-subspace cardinality — `|dom_C(M(d))| ≥ 2` ranges over `V_{s_C}(d)`, not over `V_S(d)` for arbitrary `S` — even though K.μ~'s *effect* is a bijection on the full domain `dom(M(d))` covering both subspaces. The foundation supplies no analogue precondition conditioned on link-subspace cardinality. Consequently, for any link-subspace interior deletion in a document with `|V_{s_C}(d)| < 2`, K.μ~ cannot be invoked at all, so the K.μ⁻ + K.μ~ composite is structurally unavailable. The composite-substitute strategy is therefore unavailable on a non-trivial sub-class of DEL instances: every link-subspace interior deletion in a document with `|V_{s_C}(d)| < 2`, every content-subspace interior deletion with `n_S = 1`, and every content-subspace case in which all admissible permutations equal the identity.
...
*Justification of the reduction.* The form `s ∈ V_S(d)` and D-SEQ★ together force `s = [S, 1, ..., 1, p]` of length `m_S` for some `p ∈ {1, ..., n_S}`. By TumblerAdd's componentwise rule applied to `ℓ = δ(n, m_S) = [0, ..., 0, n]`, we obtain `r = s ⊕ ℓ = [S, 1, ..., 1, p + n]`. Now let `v = [S, v_2, ..., v_{m_S}]` be any depth-`m_S` position with `subspace(v) = S` and `s ≤ v < r`; the foundation premise S8a forces every component of `v` strictly positive (`v_j ≥ 1`), in particular ruling out a zero at any middle position. At position 1, all three tumblers agree (`v_1 = s_1 = r_1 = S`). We claim `v_j = 1` for every `j` with `2 ≤ j ≤ m_S − 1`. At `m_S = 2` (the depth fixed for the link subspace by LinkVPositionDepthAxiom, and the depth that suffices for content as well) the range `2 ≤ j ≤ 1` is empty, so the claim holds vacuously; the argument then proceeds directly to the position-`m_S` case below. For `m_S ≥ 3`: suppose otherwise, and let `j₀` be the least such `j` with `v_{j₀} ≠ 1`; then `v_{j₀} ≥ 2` by positivity, and `v_i = 1 = r_i` for every `i < j₀`. Position `j₀` is therefore the first divergence between `v` and `r`, and by T1 case (i) at `j₀`, `v_{j₀} ≥ 2 > 1 = r_{j₀}` gives `v > r`, contradicting `v < r`. So `v_j = 1` for `2 ≤ j ≤ m_S − 1`, and `v = [S, 1, ..., 1, v_{m_S}]` differs from `s` and `r` only at position `m_S`. The order constraint then degrades cleanly: `s ≤ v < r` forces `p ≤ v_{m_S} < p + n`. The universal "every depth-`m_S`, subspace-`S` position in `[s, r)` lies in `V_S(d)`" therefore reduces, given D-SEQ★'s catalogue `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`, to the requirement that the integer range `{p, p + 1, ..., p + n − 1}` is contained in `{1, ..., n_S}`, equivalently `p + n − 1 ≤ n_S`.
