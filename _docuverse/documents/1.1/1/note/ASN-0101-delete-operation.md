...
The final DEL step raises no obligation of its own: by N1 and N3 it neither enlarges `Contains_C` beyond `R` nor removes a content address or provenance record, so the P4★ and P7a steps above close using only the induction hypothesis and the coupling constraints (J0, J1★). This discharges the DEL-terminated case of the inductive step; with the base case `Σ₀` and the non-DEL case above, the induction closes and every composite boundary in a DEL-extended trace satisfies P4★ and P7a. P4a at `Σ'` holds by N2.
