...
First, when the composite is well-formed, it produces a longer history sequence than DEL does, with the additional state occupying its own position in that sequence. On an interior span with `|V_S(d)| ≥ 2`, one could in principle apply `K.μ~` with an admissible permutation that moves the to-be-deleted I-addresses to the suffix of `V_S(d)` (the permutation is admissible because `K.μ~-FIX` preserves `dom(M(d))`, and rebinding I-addresses across V-positions preserves D-CTG★, D-MIN★, D-SEQ★ and S3★), then apply `K.μ⁻` to truncate that suffix. The post-`K.μ~` arrangement is a state in which the to-be-deleted I-addresses still inhabit `dom(M(d))` but at different V-positions than in either the pre- or post-state of the composite. Make this precise: SequentialAtomicTransitions (ASN-0093) treats every reachable history as a totally ordered sequence `Σ_0 → Σ_1 → ... → Σ_n` of atomic states, each obtained from its predecessor by a single elementary transition. The two-step composite `K.μ~` then `K.μ⁻` extends this sequence by exactly two atomic states, producing the segment `Σ_pre → Σ_mid → Σ_post`; equivalently, the predicate `(E i : 0 ≤ i ≤ n ∧ Σ_i = Σ_mid)` over the post-composite history sequence holds. DEL as a single elementary transition extends the sequence by exactly one atomic state, producing `Σ_pre → Σ_post`; the corresponding membership predicate `(E i : 0 ≤ i ≤ n ∧ Σ_i = Σ_mid)` does *not* hold (the post-DEL sequence contains no state equal to the composite's `Σ_mid`). This is a formal property of the history sequence under SequentialAtomicTransitions — a difference in the predicate `(E i : Σ_i = X)` evaluated over the post-transition history — derived entirely from the foundation's state-space semantics, with no appeal to implementation features such as external monitors, logs, or queries.

(The formal sequence-length distinction is the load-bearing argument. It is corroborated, in typical configurations, by a separate observational distinctness: `Σ_mid` is also observably distinct from `Σ_pre` — automatic in the link subspace by CL-UNIQ's partial injection, generic in the content subspace and witnessed by the predicate `(E v : v ∈ dom(M(d)) : M(d)(v) = a)` for any to-be-deleted I-address `a` — but this observational distinctness is corroborative only. The formal sequence-length argument closes the case on its own, even if `Σ_mid` happened to be observationally equal to `Σ_pre` or `Σ_post`.) The architectural point: DEL closes the gap atomically, leaving the history sequence one state shorter than the corresponding composite would.
