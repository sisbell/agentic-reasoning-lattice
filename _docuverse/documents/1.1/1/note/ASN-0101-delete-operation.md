...
In typical configurations `Σ_mid` is in fact observably distinct from `Σ_pre`. K.μ~ requires `π ≠ id`, so some `v ∈ dom(M(d))` has `π(v) ≠ v`; the K.μ~ bijection equation `M_mid(d)(π(v)) = M_pre(d)(v)` then yields `M_mid(d)(π(v)) = M_pre(d)(v)`, which differs from `M_pre(d)(π(v))` exactly when the pre-state values at `v` and `π(v)` are unequal. In the link subspace, distinctness is automatic — CL-UNIQ forces `M(d)|_{V_{s_L}(d)}` to be a partial injection, so distinct keys have distinct values. In the content subspace, distinctness holds generically but can fail under S5's unrestricted sharing (when two V-positions map to the same I-address); the sequence-length argument is what closes the case independently of observational distinctness. The predicate `(E v : v ∈ dom(M(d)) : M(d)(v) = a)` for any to-be-deleted I-address `a` evaluates to `true` at both `Σ_pre` and `Σ_mid` (at the post-K.μ~ V-position) but to `false` at `Σ_post`, witnessing `Σ_mid` as distinct from `Σ_post`. The architectural point holds in both forms: DEL closes the gap atomically without exposing an extra state in the history.
...
- *wp for post-DELETE discoverability from `d`:*
  ...
  ```
  A link becomes undiscoverable from `d` after DELETE iff *every* slot's pre-state projection from `d` is contained entirely within the deleted region.
...
| D8 | Arrangement well-formedness preservation: all foundation invariants — Group (i) arrangement invariants (S2, S8-fin, S8a, S8-depth, S3★, S3★-aux, D-CTG★, D-MIN★, D-SEQ★, S8★, CL-OWN, CL-UNIQ), Group (ii) allocation/store invariants (M0, S4, S7a–d, C1, C1b, C1c, C2, L0, L1, L1a–c, L3, L12, L14, L-fin, C-fin, NodeLineage, plus substrate chain-discipline lemmas ChainEnumerationInjectivity, ChainUniformLength, ChainUniformZeroCount, FirstEmissionFreshness), Group (iii) transition and per-state invariants discharged by frame (M1, C0, P0, P1, P2, P3, P4★, P4a, P6, P7, P7a, P8, L12a, L12b, S9) — all hold at the post-state | introduced |
