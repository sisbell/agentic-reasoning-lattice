...
Nelson's FEBE protocol treats DELETE as a single primitive — `DELETEVSPAN` (LM 4/66) — over arbitrary spans, with no syntactic distinction between deletions at the end of a document (suffix truncation) and deletions in the interior. We adopt this stance: `DEL[d, σ]` is a *new atomic transition kind* extending the foundation's transition vocabulary `{K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.μ~, K.ρ, K.σ}` (ASN-0047, ASN-0093). It is *not* a derived composite of `K.μ⁻` (suffix truncation, ASN-0047) and `K.μ~` (reordering, ASN-0047), because link-subspace interior deletion has no composite substitute at all: the substitute would require K.μ~ to relocate the to-be-deleted I-addresses to the suffix, but K.μ~'s admissibility clause (v) — link-subspace fixing, `(A v ∈ dom_L(M(d)) :: π(v) = v)` (ASN-0047) — pins every link-subspace V-position, so no admissible π can move them in any configuration. Gregory's `bed.c` likewise realises the deletion as a single run-to-completion procedure. DEL's admission to ASN-0047's ValidComposite★, and the precise sense in which it interacts with that protocol's coupling constraints, is recorded in D10.
...
Without D5, a deletion in `d` would have to propagate to every document sharing the deleted content, violating the autonomy of `d'`'s owner. D5 instead keeps the deletion local, and the structural separation between Istream (shared, immutable) and Vstream (per-document, mutable) is what makes it coherent. `d` modifies its own Vstream; `d'`'s Vstream is unaffected; the shared Istream is unchanged. Each document gets a different "view" of the same persistent content; deletion changes the view, not the content.
...
The implementation evidence reinforces this. Gregory's `tumblersub` uses an exponent-guarded subtraction: when the width's exponent is finer than a crum's V-displacement exponent, the subtraction is a no-op. Because text and link addresses sit at different exponents in the tree representation, the exponent guard protects the unrelated subspace from being shifted by a deletion in the other — the tree implementation delivers subspace isolation through ordinary arithmetic rather than through a special-case check.
...
*Group (ii): Allocation and store invariants.* Every invariant in this group is a predicate (or transition predicate) over one or more of the components `C`, `L`, `E`, `R`, `dom(M)`, each of which D0's frame leaves pointwise unchanged (`C' = C`, `L' = L`, `E' = E`, `R' = R`, `dom(M') = dom(M)`), with per-link and per-content values held fixed by D2 and D3. Equality of each component propagates every such predicate from `Σ` to `Σ'` trivially; no member requires an individualized argument. The members:

- *Store and allocation invariants (ASN-0036, ASN-0093):* M0, S4, S7a, S7b, S7d, C1, C1b, C1c, C2, L0, L1, L1a, L1b, L1c, L3, L12, SD, L-fin, C-fin.
- *Entity-set invariants (ASN-0047):* NodeLineage, ActivatedEmission.
...
*Vacuity does not extend to multi-step composites.* The vacuity arguments above evaluate J0, J1★, and J1'★ on a one-step composite consisting of a single DEL transition: DEL viewed in isolation contributes nothing to `dom(C') \ dom(C)`, to the J1★ antecedent, or to `R' \ R`, so it introduces no coupling obligations of its own. They do *not*, however, establish that a multi-step composite containing DEL automatically satisfies the composite-level J0, J1★, J1'★ obligations: ValidComposite★ (ASN-0047) evaluates these constraints between the composite's initial state Σ and final state Σ', not step-by-step, and a DEL step inside a composite can break composite-level J0 by removing a witness that an earlier step's allocation requires. Concretely, the three-step composite `Σ → Σ_1 → Σ_2 → Σ_3` consisting of (i) K.α emitting `a` into `dom(C)`, (ii) K.μ⁺ placing the mapping `v ↦ a` into `M(d)`, and (iii) DEL[d, σ] with `v ∈ X`, satisfies each step's elementary precondition but fails composite-level J0: `a ∈ dom(C_3) \ dom(C_0)` (K.α added it; D2 preserves it), yet no `v' ∈ dom(M_3(d))` with `subspace(v') = s_C` satisfies `M_3(d)(v') = a` (DEL removed the only witness, and no other step introduced an alternative). The failure is at the composite endpoint, not at any individual step. Composites that combine DEL with allocation-and-placement steps must therefore be checked at their endpoints: coupling-obligation *production* is confined to the K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, and K.ρ steps that introduce new content, entities, links, V-positions, or provenance pairs, while coupling-obligation *disruption* can be introduced by any K.μ⁻, K.μ~, or DEL step that modifies or relocates the V-position witness set; composite validity is the joint responsibility of both classes of step.
...
- **Prior arrangements.** The pre-state arrangement `M(d)` is whatever the system of *versions* retains. A version forked from `d` before the DELETE is a separately addressed document `d_v` whose arrangement `M(d_v)` holds the pre-DELETE layout of `d`'s content subspace, including the V→I mappings that DELETE later removes from `M(d)`. By D5, DELETE on `d` leaves `M(d_v)` untouched. How versions are created is out of scope here; DELETE's contribution is only that it does not disturb any such prior arrangement.
...
| D8 | Arrangement well-formedness preservation: every foundation per-state invariant holding at the pre-state holds at the post-state, and DEL cannot break the composite-boundary properties P4★, P4a, P7a. (Body: three-group justification.) | introduced |
...
| D10 | ValidComposite★ extension under DELETE: ASN-0047's ValidComposite★ admits DEL as an elementary transition, with composite-boundary properties P4★, P4a, P7a discharged for DEL-terminated composites and the ASN-0098 LP-family extended to DEL. (Body: vacuity argument and LP-family extension catalogue.) | introduced |
