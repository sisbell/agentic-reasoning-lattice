...
The final DEL step enters this derivation only through its neutrality: it adds nothing to `dom(C)` (D2), removes nothing from `R` (`R' = R`, D0), and only shrinks `Contains_C` (P4★ neutrality bullet above) — so it can neither raise a P4★/P7a obligation nor destroy a record the coupling constraints rely on. P4★ and P7a at `Σ'` are therefore the joint product of the coupling constraints (J0, J1★) on the allocation-and-placement steps and DEL's neutrality on the final step. P4a at `Σ'` follows directly from the neutrality bullet: `R' = R`, so DEL records no new pair, and every `(a, d) ∈ R'` keeps its pre-DEL witnessing trace state.
