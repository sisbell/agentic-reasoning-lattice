...
*Base case (`Σ₀`).* The initial state satisfies P4★ — `Contains_C(Σ₀) = ∅ ⊆ R₀` because `M₀(d) = ∅` for every `d` (ASN-0047's `Σ₀`) — and P7a vacuously, since `dom(C₀) = ∅`. Both hold by initialisation.

*Inductive step.* Fix a boundary `B_j` satisfying P4★ and P7a (the induction hypothesis), and let `B_j →* B_{j+1}` be a valid composite drawn from the DEL-extended vocabulary. Write `Σ := B_j` and `Σ' := B_{j+1}`. We establish P4★ and P7a at `Σ'` from four premises, *none specific to the composite's final step*: P4★ at `Σ` and P7a at `Σ` (the induction hypothesis); J0 and J1★ between `Σ` and `Σ'` (validity clause (2)); and `R ⊆ R'` (P2, provenance monotonicity). Because the derivation invokes only these — not the identity of any step in the composite — it discharges `B_{j+1}` whether or not the composite's final step is DEL, so a single argument closes the induction.
...
With the base case `Σ₀` and this single inductive step, the induction closes: every composite boundary in a DEL-extended trace satisfies P4★ and P7a. P4a at `Σ'` holds by N2.
