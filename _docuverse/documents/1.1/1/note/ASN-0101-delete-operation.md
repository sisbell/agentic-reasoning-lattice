...
First, when the composite is well-formed, it exposes an additional intermediate state — a state that occupies a separate position in the system's history. On an interior span with `|V_S(d)| ≥ 2`, one could in principle apply `K.μ~` with an admissible permutation that moves the to-be-deleted I-addresses to the suffix of `V_S(d)` (the permutation is admissible because `K.μ~-FIX` preserves `dom(M(d))`, and rebinding I-addresses across V-positions preserves D-CTG★, D-MIN★, D-SEQ★ and S3★), then apply `K.μ⁻` to truncate that suffix. The post-`K.μ~` arrangement is a state in which the to-be-deleted I-addresses still inhabit `dom(M(d))` but at different V-positions than in either the pre- or post-state of the composite. Make this precise: SequentialAtomicTransitions (ASN-0093) treats every reachable history as a totally ordered sequence `Σ_0 → Σ_1 → ... → Σ_n` of atomic states, each obtained from its predecessor by a single elementary transition. The two-step composite `K.μ~` then `K.μ⁻` produces such a sequence `Σ_pre → Σ_mid → Σ_post` of length two, in which `Σ_mid` is itself a member of the system's history — it can be queried, logged, or witnessed by any external monitor that reads the per-state history. DEL as a single elementary transition produces a sequence `Σ_pre → Σ_post` of length one, with no intermediate state to expose. The sequence-length distinction is independent of whether `Σ_mid` equals either endpoint observationally.

(In typical configurations `Σ_mid` is also observably distinct from `Σ_pre` — automatic in the link subspace by CL-UNIQ's partial injection, generic in the content subspace and corroborated by the witness predicate `(E v : v ∈ dom(M(d)) : M(d)(v) = a)` for any to-be-deleted I-address `a` — but observational distinctness is corroborative, not load-bearing: the sequence-length argument closes the case on its own.) The architectural point: DEL closes the gap atomically without exposing an extra state in the history.

Second, the composite is not always *available*. K.μ~ (ASN-0047) requires `|dom_C(M(d))| ≥ 2` as a formal precondition — referencing only content-subspace cardinality, even though K.μ~'s *effect* is a bijection on the full domain `dom(M(d))` covering both subspaces. The foundation supplies no analogue precondition conditioned on link-subspace cardinality. Consequently, when DEL operates on a *link*-subspace interior span in a document with `|V_{s_C}(d)| < 2`, K.μ~ cannot be invoked at all and the K.μ⁻ + K.μ~ composite is structurally unavailable. (Content-subspace interior deletions with `|V_{s_C}(d)| < 2` are not a separate killer case: `|V_{s_C}(d)| ≤ 1` rules out an *interior* content-subspace deletion, since a single-position subspace admits only the deletion `p = 1`, `n = 1`, which is suffix truncation handled by K.μ⁻ alone. Likewise the "all admissible permutations are the identity" case does not enlarge the unavailable sub-class: whenever `|V_{s_C}(d)| ≥ 2`, the transposition of any two content-subspace positions is an admissible permutation under K.μ~ — S3★ is preserved because the swap leaves the range in `dom(C)` invariant, and S8a, S8-depth, CL-OWN, and CL-UNIQ are preserved because the permutation is subspace-fixing — so a non-identity admissible permutation always exists.) The genuine killer case for the composite-substitute strategy is therefore exactly: link-subspace interior deletion in a document with `|V_{s_C}(d)| < 2`.
...
The ASN-0034 algebra fixes the shift mechanism precisely. Let `v ∈ Ρ` with `v = [S, 1, ..., 1, k]` and `k ≥ p + n`. The shift function `σ_d(v)` is the unique tumbler `u` with `shift(u, n) = v`. Because OrdinalShift advances only the last component of its argument by `n` while leaving earlier components fixed (shift(v, n)ᵢ = vᵢ for i < m; shift(v, n)ₘ = vₘ + n), the inverse simply decrements the last component: `u = [S, 1, ..., 1, k − n]`. Verification: `shift([S, 1, ..., 1, k − n], n) = [S, 1, ..., 1, k − n] ⊕ δ(n, m_S) = [S, 1, ..., 1, (k − n) + n] = [S, 1, ..., 1, k] = v` ✓. When `Ρ ≠ ∅` (equivalently, `p + n ≤ n_S`), its smallest element is `r = [S, 1, ..., 1, p + n]`, and `σ_d(r) = [S, 1, ..., 1, p] = s` — the first shifted position lands exactly where the deletion began, closing the gap precisely. When `Ρ = ∅` (deletion at the end, `p + n = n_S + 1`), no shift occurs and `r ∉ V_S(d)`, so `σ_d(r)` is not invoked.
...
- `V_1(M'(d)) = Λ ∪ Q = {[1, 1, 1]}`.
- `M'(d)([1, 1, 1]) = M(d)([1, 1, 2]) = a_2` (shifted Ρ-witness).
...
*Empty post-state (`n = n_S`, `p = 1`).* The entire affected subspace is deleted: `Λ = ∅`, `X = V_S(d)`, `Ρ = ∅`, `Q = ∅`. The post-state `V_S(M'(d)) = Λ ∪ Q = ∅`, and the shift function `σ_d` has empty domain. Tracing D8's Group (i) clauses on the affected subspace:
...
*Deletion at the start (`p = 1`, `n < n_S`).* `Λ = ∅`, `X = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n}`, `Ρ = {[S, 1, ..., 1, k] : n + 1 ≤ k ≤ n_S}`. The first position of `Ρ` is `r = [S, 1, ..., 1, n + 1]`. Computing `σ_d(r)`: `σ_d(r)` is the unique `u` with `shift(u, n) = r`. Setting `u = [S, 1, ..., 1, 1]`: `shift(u, n) = u ⊕ δ(n, m_S) = [S, 1, ..., 1, 1 + n] = r` ✓. So `σ_d(r) = [S, 1, ..., 1, 1] = [S, 1, ..., 1]` of depth `m_S`. This case is the only boundary configuration in which D-MIN★ receives a *non-vacuous* `σ_d`-witness rather than vacuous discharge:

- *D-MIN★ via `σ_d`-witness.* `V_S(M'(d)) = Q ≠ ∅`, so D-MIN★'s conditional fires. The post-state minimum `min(V_S(M'(d))) = min(Q) = σ_d(r) = [S, 1, ..., 1]` of depth `m_S` — exactly the canonical form D-MIN★ requires. The minimality follows from `σ_d` being an order-preserving bijection from `Ρ` to `Q` (D1) and `r = min(Ρ)`. The witness is supplied by the shift function, not by inheritance from the pre-state.
...
- *S8-depth via length preservation.* Each `v ∈ Q` satisfies `#v = #σ_d(v') = #v' = m_S` for its source `v' ∈ Ρ`, by OrdinalShift's length-preservation postcondition.
- *S3★, S3★-aux, CL-OWN, CL-UNIQ via source correspondence.* Each `v ∈ Q` has `M'(d)(v) = M(d)(σ_d^{-1}(v))` for a unique `σ_d^{-1}(v) ∈ Ρ ⊆ V_S(d)`; pre-state S3★, S3★-aux, CL-OWN, and CL-UNIQ on `Ρ` (subset of `V_S(d)`) transfer to the post-state under the source-correspondence argument from D8's Group (i) justification, together with `dom(C') = dom(C)` and `dom(L') = dom(L)`.
...
- If `d'' = d`, restricted to subspace `S`: `project(L'(ℓ).eᵢ, d, Σ') ∩ V_S(M'(d)) = (project(L(ℓ).eᵢ, d, Σ) ∩ Λ) ∪ {σ_d(v) : v ∈ project(L(ℓ).eᵢ, d, Σ) ∩ Ρ}`. Here `Λ`, `X`, `Ρ ⊆ V_S(d)` are the subspace-`S` regions defined in D0; the intersection of the pre-state projection with each region is therefore a subset of `V_S(d)`, well-formed by construction. The LHS uses `V_S(M'(d))` because the post-state V_S region differs from `V_S(d)` (the deletion modifies subspace `S` precisely).

The full post-state projection at `d'' = d` is then recovered as the union of the second-bullet contribution (subspace `S' ≠ S`, unchanged from the pre-state) and the third-bullet contribution (subspace `S`, the unshifted-Λ-plus-shifted-Ρ form):
```
project(L'(ℓ).eᵢ, d, Σ')
  = (project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d))                                            [from S' ≠ S]
  ∪ (project(L(ℓ).eᵢ, d, Σ) ∩ Λ) ∪ {σ_d(v) : v ∈ project(L(ℓ).eᵢ, d, Σ) ∩ Ρ}        [from S]
```
...
**D11 — Weakest preconditions for projection postconditions.** Let `Σ → Σ'` be a DEL[d, σ] transition with σ = (s, ℓ_σ) of subspace `S`, and let `Λ`, `X`, `Ρ ⊆ V_S(d)` be the regions of D0. Write `Q_disc(ℓ, d)` for the post-state predicate `discoverable_from(ℓ, d, ·)`, and `Q_card(ℓ, i, d, k)` for the post-state predicate `|project(L(ℓ).eᵢ, d, ·)| = k`.

*DEL is deterministic.* Each component of `Σ'` is uniquely determined by `Σ` and the parameters `(d, σ)`: by D0's frame `C' = C`, `L' = L`, `E' = E`, `R' = R`, and `M'(d') = M(d')` for every `d' ≠ d`; the affected component `M'(d)` is constructed pointwise from `M(d)` and `σ_d` (itself a function of `σ`), with each value `M'(d)(v) = M(d)(u)` for the unique source `u` determined by `v`'s region (Λ, Q, or the unaffected subspace). Hence the wp transformer satisfies `wp(DEL[d, σ], ¬Q) ≡ ¬wp(DEL[d, σ], Q)` for every postcondition `Q`, licensing the negation equivalences below.
...
- *wp for post-DELETE projection cardinality:*
  ```
...
*Justification.* For discoverability from `d`: by definition, `discoverable_from(ℓ, d, Σ') ≡ (E i : project(L'(ℓ).eᵢ, d, Σ') ≠ ∅)`. By D3, `L'(ℓ).eᵢ = L(ℓ).eᵢ`, so `coverage(L'(ℓ).eᵢ) = coverage(L(ℓ).eᵢ)`. By D6, the projection restricted to subspace `S' ≠ S` is preserved bytewise: `project(L'(ℓ).eᵢ, d, Σ') ∩ V_{S'}(d) = project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d)`. By D9's third clause, the projection restricted to subspace `S` is `(project(L(ℓ).eᵢ, d, Σ) ∩ Λ) ∪ {σ_d(v) : v ∈ project(L(ℓ).eᵢ, d, Σ) ∩ Ρ}`. The full post-state projection is the union of these two restrictions; it is non-empty iff at least one summand is non-empty. The `Λ` summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ Λ ≠ ∅`; the shifted Ρ summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ Ρ ≠ ∅` (because σ_d is a bijection on Ρ — D1); the cross-subspace summand is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d) ≠ ∅`. Their union is non-empty iff their union is — which is the pre-state projection intersected with `Λ ∪ Ρ ∪ V_{S'}(d) = V_S(d) \ X ∪ V_{S'}(d) = dom(M(d)) \ X`. Since `project(L(ℓ).eᵢ, d, Σ) ⊆ dom(M(d))` by definition, intersecting with `dom(M(d)) \ X` is equivalent to subtracting `X`, which is non-empty iff `project(L(ℓ).eᵢ, d, Σ) ⊄ X`. Existential quantification over slots completes the wp. For discoverability from `d'' ≠ d`: by D9's first clause, `project(L'(ℓ).eᵢ, d'', Σ') = project(L(ℓ).eᵢ, d'', Σ)` for every slot `i`, so the predicate `(E i : project(L'(ℓ).eᵢ, d'', Σ') ≠ ∅)` is equivalent to `(E i : project(L(ℓ).eᵢ, d'', Σ) ≠ ∅) = discoverable_from(ℓ, d'', Σ)`. For projection cardinality: by D9's third clause and the bijectivity of σ_d, the post-state projection cardinality on subspace `S` equals `|project(L(ℓ).eᵢ, d, Σ) ∩ (Λ ∪ Ρ)| = |project(L(ℓ).eᵢ, d, Σ) ∩ V_S(d)| − |project(L(ℓ).eᵢ, d, Σ) ∩ X|`; combined with the cross-subspace contribution preserved bytewise by D6 (`|project(L(ℓ).eᵢ, d, Σ') ∩ V_{S'}(d)| = |project(L(ℓ).eᵢ, d, Σ) ∩ V_{S'}(d)|`), the post-state total cardinality is `|project(L(ℓ).eᵢ, d, Σ)| − |project(L(ℓ).eᵢ, d, Σ) ∩ X|`. Setting this equal to `k` yields the wp. The specialisation to "cardinality preserved" sets `k = |project(L(ℓ).eᵢ, d, Σ)|`, reducing to `|project(L(ℓ).eᵢ, d, Σ) ∩ X| = 0`, i.e., `project(L(ℓ).eᵢ, d, Σ) ∩ X = ∅`. ∎
...
*Vocabulary note.* ASN-0047's ValidComposite★ originally enumerates eight transitions, `{K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~, K.ρ}`, with K.σ absent because the substrate-level DocumentRegistration operation was introduced by ASN-0093 after ASN-0047 was fixed. The extended vocabulary above admits two additions: DEL (the present operation) and K.σ. K.σ (DocumentRegistration, ASN-0093) and K.δ-IsDocument (EntityCreation, document case, ASN-0047) are alternative atomic document-registration operations with overlapping effects — both place a fresh document into `dom(M)` with an empty arrangement under disjoint preconditions. We promote K.σ alongside DEL because downstream specifications that invoke "ValidComposite★ chains" should range over the full substrate vocabulary; K.σ's promotion carries no independent burden of proof, since K.σ's frame `C' = C ∧ L' = L ∧ E' = E ∧ R' = R` together with its effect `dom(M') = dom(M) ∪ {d}` and `M'(d) = ∅` makes J0, J1★, and J1'★ vacuous on a one-step K.σ composite by the same argument given below for DEL.
...
- *J1★ (ExtensionRecordsProvenanceContentSubspace)* quantifies over pairs `(a, d)` for which some `v ∈ dom(M'(d))` has `subspace(v) = s_C` and `M'(d)(v) = a`, while no such `v` existed in `dom(M(d))`. By D0's effect, every post-state content-subspace V-position `v` either lies in `Λ` (so `M'(d)(v) = M(d)(v)`, and the pre-state V-position `v` itself witnesses the same I-address) or has the form `σ_d(u)` for some `u ∈ Ρ` (so `M'(d)(σ_d(u)) = M(d)(u)`, and the pre-state V-position `u` witnesses the same I-address). The antecedent — "no `v ∈ dom(M(d))` with subspace `s_C` mapped to `a`" — is therefore false for every `(a, d)` pair. The implication is vacuous.
...
| D10 | ValidComposite★ extension under DELETE: ASN-0047's ValidComposite★ admits DEL as an elementary transition; J0, J1★, J1'★ hold vacuously on a one-step DEL composite, but composite-level validity for multi-step composites containing DEL is not automatic — DEL can remove a V-position witness required to discharge an earlier step's allocation obligation | introduced |
| D11 | Weakest preconditions for projection postconditions: `wp(DEL[d, σ], discoverable_from(ℓ, d, ·)) ≡ (E i : project(L(ℓ).eᵢ, d, Σ) ⊄ X)`; cross-document discoverability wp coincides with the pre-state predicate; `wp(DEL[d, σ], |project(·)| = k) ≡ |project(L(ℓ).eᵢ, d, Σ)| − |project(L(ℓ).eᵢ, d, Σ) ∩ X| = k`; cardinality is preserved iff `project(L(ℓ).eᵢ, d, Σ) ∩ X = ∅` | introduced |
