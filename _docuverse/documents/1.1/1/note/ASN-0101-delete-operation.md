...
*Composite-boundary obligations.* A valid composite that ends in a DEL step has `Σ'` as a composite boundary, so beyond the coupling constraints `Σ'` must satisfy ASN-0047's composite-boundary properties P4★, P4a, and P7a; these hold at `Σ'` by D8. (The one disruption DEL *can* cause — removing a content-subspace witness — bears on the J0/J1★ coupling constraints, addressed next.)
