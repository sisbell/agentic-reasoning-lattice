...
The combined effect: ASN-0098's projection apparatus — both LP-Comp's per-step lemmas LP2 through LP14 and the discoverability/tightness lemmas LP16 through LP21 — carries to the DEL-extended vocabulary intact. D9 and D11 close the only two genuinely DEL-specific extension points (the affected document's per-step projection behaviour and the DEL-specific wp); every remaining lemma is either state-relative and applies to the post-state directly (LP12, LP16, LP17, LP20, LP21), threads through the prefix via Store Monotonicity★ and LP3★ (LP18, LP19, LP19a), or concerns a non-DEL operation and needs no extension (LP6–LP8, LP14). LP-Comp's case-analysis, together with this catalogue of the discoverability and tightness lemmas, remains exhaustive over the extended vocabulary once the present cataloguing is taken as the DEL row.
