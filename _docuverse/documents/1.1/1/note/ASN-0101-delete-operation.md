...
*Parameters.* A document `d` and a level-uniform V-span `σ = (s, ℓ_σ)` of ordinal type. *Notational convention.* From this point onward we write `ℓ_σ` for the span width of `σ` and reserve `ℓ` for link addresses (members of `dom(L)`); the subscript prevents confusion with the link-address variable `ℓ`.
...
- *Cross-document discoverability wp (D11 bullet 2).* `wp(DEL[d, σ], discoverable_from(ℓ_0, d', ·)) ≡ enabled(DEL[d, σ]) ∧ discoverable_from(ℓ_0, d', Σ)`; the pullback factor `discoverable_from(ℓ_0, d', Σ) = true` coincides with the pre-state predicate. Cross-check against the post-state: `project(L'(ℓ_0).e_1, d', Σ') = {[1, 1, 1], [1, 1, 2]} ≠ ∅`, so `discoverable_from(ℓ_0, d', Σ') = true`. ✓

- *Cross-document cardinality wp (D11 bullet 4).* `wp(DEL[d, σ], |project(L(ℓ_0).e_1, d', ·)| = 2) ≡ enabled(DEL[d, σ]) ∧ |project(L(ℓ_0).e_1, d', Σ)| = 2`; the pullback factor `|project(L(ℓ_0).e_1, d', Σ)| = 2 = true` coincides with the pre-state cardinality predicate. Cross-check against the post-state: `|project(L'(ℓ_0).e_1, d', Σ')| = |{[1, 1, 1], [1, 1, 2]}| = 2`. ✓
...
- *wp for post-DELETE discoverability from a different document `d'' ≠ d` (with `d'' ∈ dom(M)`):*
  ...
  ```
  Discoverability from any other document is invariant under DEL[d, σ], so the pullback clause coincides with the pre-state predicate.
...
| D10 | ValidComposite★ extension under DELETE: ASN-0047's ValidComposite★ admits DEL as an elementary transition, with composite-boundary properties P4★, P4a, P7a discharged for DEL-terminated composites and the ASN-0098 LP-family extended to DEL. (Body: vacuity argument and LP-family extension.) | introduced |
