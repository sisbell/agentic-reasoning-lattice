...
The abstract specification is silent on the realising data structure but does require *some* such mechanism — the operation must be able, in finite work proportional to the affected region, to produce a post-state arrangement satisfying D0's domain and value conditions. (Gregory realises this through a two-phase knife-and-shift walk over the tree of POOM crums materialising `M(d)`; the tree structure is an implementation choice, not an abstract commitment.) One observation sharpens the abstract picture:
...
**D8 — Arrangement well-formedness preservation under DELETE.** For every transition `Σ → Σ'` arising from DEL[d, σ], the post-state satisfies every foundation per-state invariant the pre-state was required to satisfy, and the transition satisfies every foundation transition invariant the pre-state transition discipline imposes. These invariants fall into three groups: Group (i), the arrangement per-state invariants (S2, S8-fin, S8a, S8-depth, S3★, D-CTG★, D-MIN★, D-SEQ★, S8★, S3★-aux, CL-OWN, CL-UNIQ); Group (ii), the allocation and store per-state invariants; and Group (iii), the transition invariants together with the remaining per-state invariants. The composite-boundary properties of ASN-0047 (P4★, P4a, P7a) are neither per-state nor transition invariants and lie outside D8's scope.

*Group (i): Arrangement invariants.* Each is universally quantified over every `d ∈ dom(M)`. For every unmodified document `d' ≠ d`, D5 gives `M'(d') = M(d')`, so the post-state arrangement of each such `d'` is identical to its pre-state arrangement and satisfies the same invariants verbatim; it remains to treat the modified document `d`, where a source-correspondence argument (below) supplies preservation. The post-state arrangement `M'(d)` satisfies:
...
With D0, D1, and D8 in hand, we instantiate them at the edge configurations to confirm the general arguments specialise cleanly. Each case fixes `(p, n, n_S)`; D8's source-correspondence justification supplies invariant preservation uniformly, so we record only what is distinctive at each edge rather than re-discharging the clause set.

*Empty post-state (`p = 1`, `n = n_S`).* The entire affected subspace is deleted — `Λ = ∅`, `X = V_S(d)`, `Π = ∅`, `Q = ∅`, so `V_S(M'(d)) = ∅`. This is D8 at `n_S − n = 0`: every Group (i) clause on the affected subspace discharges vacuously (an empty quantifier, or a false conditional antecedent for D-CTG★/D-MIN★/D-SEQ★), and the unaffected subspace `S' ≠ S` inherits its Group (i) clauses from the pre-state via D6.

*Deletion at the start (`p = 1`, `n < n_S`).* `Λ = ∅`, `Π = {[S, 1, ..., 1, k] : n + 1 ≤ k ≤ n_S}`, with first element `r = [S, 1, ..., 1, n + 1]` and `σ_d(r) = [S, 1, ..., 1]` of depth `m_S`. This is the one edge at which D-MIN★ receives a *non-vacuous* `σ_d`-witness: `min(V_S(M'(d))) = min(Q) = σ_d(r) = [S, 1, ..., 1]`, the canonical form D-MIN★ requires, with minimality from D1's order-preserving bijection and `r = min(Π)`. D8 discharges the remaining Group (i) clauses with `Λ = ∅`.

*Deletion at the end (`p + n = n_S + 1`).* `Π = ∅`, so no shift occurs and `V_S(M'(d)) = Λ = {[S, 1, ..., 1, k] : 1 ≤ k ≤ p − 1}`. This is the configuration ASN-0047's `K.μ⁻` (suffix truncation) covers as a special case; DEL generalises by admitting arbitrary `s`. D1's post-state characterisation reduces correctly with `n_S' = n_S − n = p − 1`.

*Singleton subspace deletion (`n_S = 1`, `n = 1`, `p = 1`).* Both `Λ = ∅` and `Π = ∅` — a specialisation of the empty post-state case.

*Singleton interior deletion (`n = 1`, `1 < p < n_S`).* `Λ` and `Π` are both non-empty; each position of `Π` shifts by one, `σ_d([S, 1, ..., 1, k]) = [S, 1, ..., 1, k − 1]`, and the post-state is the contiguous prefix `{[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S − 1}`. D8 discharges both regions — inheritance for `Λ`, `σ_d`-image for `Q`.

*Non-singleton interior deletion (`n ≥ 2`, `1 < p`, `p + n − 1 < n_S`).* The general case in which both `Λ` and `Q` are non-empty and `σ_d` has non-trivial domain. `Λ` (last component `≤ p − 1`) and `Q` (last component `≥ p`) are disjoint by last-component range, so D8's two-summand discharge — inheritance for `Λ`, `σ_d`-image construction for `Q` — combines without conflict, applied pointwise on each region (e.g. `n_S = 6, n = 2, p = 3` gives `Λ = {[1,1], [1,2]}`, `Q = {[1,3], [1,4]}`).

*Cross-subspace independence.* In every case above, D6 preserves the other subspace's arrangement bytewise; when DEL affects only the content subspace, CL-OWN and CL-UNIQ stay trivially intact for the untouched link subspace.
...
*Verification of cross-document discoverability (D9's first bullet, D10's cross-document remark).*
...
By D10's cross-document remark, both wps from `d'` reduce to their pre-state predicates conjoined with `enabled(DEL[d, σ])` — the wp form of D5's bytewise-isolation guarantee — and the post-state evaluations above (`project(L'(ℓ_0).e_1, d', Σ') = {[1, 1, 1], [1, 1, 2]}`, cardinality 2) confirm it.

## ValidComposite★ extension under DELETE

**D11 — ValidComposite★ extension under DELETE.** ASN-0047's ValidComposite★ is extended to admit DEL as an elementary transition. A composite transition `Σ →* Σ'` is *valid* iff it is a finite sequence of atomic transitions
...
(2) *Coupling constraints and composite-boundary obligations.* J0, J1★, and J1'★ (ASN-0047) hold between the composite's initial state `Σ` and final state `Σ'`; and, because `Σ'` is a composite boundary, `Σ'` additionally satisfies ASN-0047's composite-boundary properties P4★, P4a, and P7a.
...
*Boundary derivation.* Admitting DEL places DEL-terminated boundaries outside the scope of ASN-0047's ExtendedReachableStateInvariants theorem — which guarantees the composite-boundary properties P4★, P4a, and P7a only over the pre-DEL vocabulary — so we re-prove them by induction over the sequence of composite boundaries in a *DEL-extended trace* `Σ₀ →* B₁ →* B₂ →* ... →* B_N`, where each `B_j →* B_{j+1}` is a valid composite drawn from the DEL-extended vocabulary. The induction hypothesis is that every boundary `B_j` satisfies P4★, P4a, and P7a.

*Base case (`Σ₀`).* The initial state satisfies P4★ — `Contains_C(Σ₀) = ∅ ⊆ R₀` because `M₀(d) = ∅` for every `d` (ASN-0047's `Σ₀`) — and P4a and P7a vacuously, since `R₀ = ∅` and `dom(C₀) = ∅`. All hold by initialisation.

*Inductive step.* Fix a boundary `B_j` satisfying P4★, P4a, and P7a (the induction hypothesis), and let `B_j →* B_{j+1}` be a valid composite drawn from the DEL-extended vocabulary. Write `Σ := B_j` and `Σ' := B_{j+1}`. We establish P4★, P4a, and P7a at `Σ'` from premises *none specific to the composite's final step*: P4★, P4a, and P7a at `Σ` (the induction hypothesis); J0, J1★, and J1'★ between `Σ` and `Σ'` (validity clause (2)); and `R ⊆ R'` (P2, provenance monotonicity). Because the derivation invokes only these — not the identity of any step in the composite — it discharges `B_{j+1}` whether or not the composite's final step is DEL, so a single argument closes the induction.

*P4★ at `Σ'`* — `Contains_C(Σ') ⊆ R'`. Take `(a, d) ∈ Contains_C(Σ')`: some content-subspace `v ∈ dom(M'(d))` has `M'(d)(v) = a`. Split on whether `a` is range-new to the content-subspace range of `M'(d)` relative to `M(d)`:
  - *Range-new* (no content-subspace `v' ∈ dom(M(d))` maps to `a`): J1★ between `Σ` and `Σ'` gives `(a, d) ∈ R'`.
  - *Not range-new* (`(a, d) ∈ Contains_C(Σ)`): P4★ at `Σ` gives `(a, d) ∈ R`, and `R ⊆ R'` gives `(a, d) ∈ R'`.
  Either way `(a, d) ∈ R'`, discharging the inclusion.

*P7a at `Σ'`* — every `a ∈ dom(C')` carries a provenance record. Take `a ∈ dom(C')`. Split on whether `a` is allocation-new:
  - `a ∈ dom(C)`: P7a at `Σ` gives `(a, d) ∈ R` for some `d`, and `R ⊆ R'` gives `(a, d) ∈ R'`.
...
*P4a at `Σ'`* — every `(a, d) ∈ R'` is witnessed at some boundary in the post-state history. The single-DEL N2 argument above does not extend to a composite boundary: across `B_j →* B_{j+1}` the relation `R` can grow, because the composite may contain K.ρ steps, so the `R' = R` premise on which N2 rests fails here. We split on whether the pair is new this composite:
  - *Not new* (`(a, d) ∈ R`): P4a at `Σ` (the induction hypothesis) witnesses `(a, d)` at some boundary `Σ_k ∈ {Σ₀, ..., B_j}`. That boundary is also a boundary of the longer post-state history, and the witnessing arrangement at `Σ_k` is unchanged, so `(a, d)` remains witnessed at `Σ_k`.
  - *New* (`(a, d) ∈ R' \ R`): composite validity supplies J1'★ between `Σ` and `Σ'`, whose first conjunct gives a surviving content-subspace witness at the endpoint — some `v ∈ dom(M'(d))` with `subspace(v) = s_C` and `M'(d)(v) = a`. The endpoint `Σ' = B_{j+1}` is itself a composite boundary, so `(a, d)` is witnessed at `B_{j+1}`. (A provenance pair added by an intermediate K.ρ whose content placement is then removed by a DEL step *within the same composite* would have no surviving witness at `Σ'`; such a composite violates J1'★ and is therefore not valid, so it never reaches this case.)
  Either way `(a, d)` is witnessed at a boundary in the post-state history.

With the base case `Σ₀` and this single inductive step, the induction closes: every composite boundary in a DEL-extended trace satisfies P4★, P4a, and P7a.
...
| D8 | Arrangement well-formedness preservation: every foundation per-state invariant holding at the pre-state holds at the post-state, and every foundation transition invariant the pre-state discipline imposes holds across the transition. (Body: three-group justification.) | introduced |
