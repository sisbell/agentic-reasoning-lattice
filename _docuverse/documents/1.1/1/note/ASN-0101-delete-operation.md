...
*Inductive step.* Fix a boundary `B_j` satisfying P4★ and P7a (the induction hypothesis), and let `B_j →* B_{j+1}` be a valid composite drawn from the DEL-extended vocabulary. Write `Σ := B_j` and `Σ' := B_{j+1}`. We establish P4★ and P7a at `Σ'` from four premises, *none specific to the composite's final step*: P4★ at `Σ` and P7a at `Σ` (the induction hypothesis); J0 and J1★ between `Σ` and `Σ'` (validity clause (2)); and `R ⊆ R'` (P2, provenance monotonicity). Because the derivation invokes only these — not the identity of any step in the composite — it discharges `B_{j+1}` whether or not the composite's final step is DEL, so a single argument closes the induction. We cannot instead cite ASN-0047's ExtendedReachableStateInvariants at `B_{j+1}` for the non-DEL case: `B_j` may itself have been reached by an earlier DEL-containing composite, placing `B_{j+1}` outside that theorem's reachability scope. For a composite drawn entirely from the pre-DEL vocabulary, the derivation below reproves inductively, from coupling alone, what that theorem establishes for pre-DEL reachable states.
