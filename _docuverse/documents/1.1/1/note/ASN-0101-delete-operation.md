...
**D8 — Arrangement well-formedness preservation under DELETE.** For every transition `Σ → Σ'` arising from DEL[d, σ], the post-state satisfies every foundation *per-state* invariant that the pre-state was required to satisfy, and DEL cannot break any *composite-boundary* property. The per-state invariants fall into three groups: Group (i), the arrangement invariants on the modified document `d`, preserved by a source-correspondence argument; and Groups (ii)–(iii), the allocation, store, transition, and remaining per-state invariants, all preserved because they predicate only over components D0's frame leaves pointwise fixed. The three composite-boundary properties of ASN-0047 (P4★, P4a, P7a) are not per-state invariants and are treated separately after the group membership.
...
- *Group (ii) — allocation and store invariants.* Store and allocation invariants (ASN-0036, ASN-0093): M0, S4, S7a, S7b, S7d, C1, C1b, C1c, C2, L0, L1, L1a, L1b, L1c, L3, L12, SD (store disjointness — ASN-0093's name for the invariant that ASN-0047's ExtendedReachableStateInvariants theorem lists as L14), L-fin, C-fin. Entity-set invariants (ASN-0047): NodeLineage, ActivatedEmission.
...
The opposite direction is also informative. Suppose we wish to ensure that DEL[d, σ] does *not* affect `ℓ`'s discoverability from `d`. By the contrapositive of D11's cardinality-preservation bullet, we require `project(L(ℓ).eᵢ, d, Σ) ∩ X = ∅` for every slot `i`. The slot-wise emptiness check is a per-slot, per-V-position test against the deleted region — local in both the link and the deletion.
...
*Composite-boundary obligations.* A valid composite that ends in a DEL step has `Σ'` as a composite boundary, so beyond the coupling constraints `Σ'` must satisfy ASN-0047's composite-boundary properties P4★, P4a, and P7a. D8 establishes only DEL's *neutrality* toward these properties — that a DEL step cannot itself break them — not that they hold at `Σ'`. What secures them at the boundary depends on the composite's length.

*Single-step composite (`Σ → Σ'`, one DEL).* Here the initial state `Σ` is itself a composite boundary, so P4★, P4a, and P7a already hold at `Σ`. D8's neutrality then carries each to `Σ'`: P4★ by content-subspace-monotone-shrinking together with `R' = R` (D8's P4★ bullet), P7a by `dom(C') = dom(C)` (D2) and `R' = R` (D8's P7a bullet), and P4a because `R' = R` leaves every pre-state witness intact in the extended history (D8's P4a bullet). For this case the attribution "by D8" is exact.

*Multi-step composite ending in DEL.* The DEL pre-state `Σ_{n-1}` is then an *interior* state of the composite, not a composite boundary, so P4★ and P7a need not hold there; D8's neutrality alone would only carry a pre-existing interior violation forward to `Σ'`. What actually secures P4★ and P7a at the endpoint is the composite's coupling constraints, evaluated between `Σ` and `Σ'`: J0 and J1★ force every content address allocated by an earlier step to be matched by an arrangement placement and a provenance record, which is precisely what P4★ (`Contains_C(Σ') ⊆ R'`) and P7a (every `a ∈ dom(C')` carries a provenance record) demand at `Σ'`, given that P4★ and P7a already hold at the prior boundary `Σ`. DEL's neutrality (D8) then guarantees the final DEL step does not disturb what the coupling constraints establish: DEL removes no content address (D2), removes no provenance pair (`R' = R`, D0's frame), and only shrinks `Contains_C` (D8's P4★ bullet). Thus P4★ and P7a at `Σ'` are the joint product of the coupling constraints J0/J1★ and DEL's neutrality (D8), not of D8 alone — consistent with the *Vacuity does not extend to multi-step composites* paragraph below. P4a is discharged uniformly in both cases: `R' = R` (D0) means DEL records no new provenance pair, so every `(a, d) ∈ R'` was already present and already witnessed at some trace state in the pre-DEL history, and that witnessing state persists in the longer post-DEL trace (D8's P4a bullet); DEL raises no new witnessing obligation. (The one disruption DEL *can* cause — removing a content-subspace witness — bears on the J0/J1★ coupling constraints, addressed next.)
