...
The inquiry declares `depends: [34, 36, 40, 47]`. ASN-0034 (Tumbler Algebra), ASN-0036 (Strand Model), and ASN-0047 (Transition Model) are each consumed. The one local re-derivation — `d_src ≼ d_new` in the structural-ancestry argument, which J4 (ASN-0047) also supplies directly — is retained to establish V2 as a named, self-contained structural claim of this ASN (the ancestry relation exposed for downstream use), rather than leaving it as a citation of J4's derived consequence. Its nested length-induction by-product (`#d_new = #d_src + 1` for the subsequent-fork `k = 0` case) is local to that derivation and is *not* consumed elsewhere; in particular V11a re-derives its own length identity (`#dⁱ_new = #d_src + i` for first-fork chains, `k = 1`) independently via TA5(d).
