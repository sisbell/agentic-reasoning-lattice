...
*Remark on premise scope.* The premise constrains only each step's *immediate* source `d^{i-1}_new` across its own gap; arrangement edits M-targeted elsewhere (third documents, non-immediate chain members) are discharged operationally by V5a Corollary 2, and edits to `d_src` after step 1 are immaterial because the conclusion anchors `V_{s_C}(d_src)` and `M(d_src)(v)` at the immutable initial state `Σ`.
