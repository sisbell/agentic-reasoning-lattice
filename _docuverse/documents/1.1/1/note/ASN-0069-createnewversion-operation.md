...
*Remark on V11's premise.* The premise as stated — no transition between consecutive fork composites modifies any source's arrangement — is stronger than what V11's substance requires. The derivation reads only `V_{s_C}(d^{i-1}_new)` and the mapping values `M^{i-1}(d^{i-1}_new)|_{V_{s_C}(d^{i-1}_new)}` at the pre-state of each step `i`. Arrangement modifications confined to the link subspace (e.g., K.μ⁺_L on a chain source, which extends only `V_{s_L}(d^{i-1}_new)` and leaves `V_{s_C}(d^{i-1}_new)` and its image untouched), or modifications targeting documents outside the chain, would not disturb the inheritance chain. A tightened premise — no transition between consecutive fork composites modifies any chain source's content-subspace arrangement, i.e., the pre-state of step `i` agrees with the post-state of step `i − 1` on `M(d^{i-1}_new)|_{V_{s_C}(d^{i-1}_new)}` for every `1 ≤ i ≤ k` — would yield the same conclusion. The conservative premise is retained for proof simplicity; the tightening is recorded so downstream users of V11 can take advantage of the looser sufficient condition when needed.
