...
The version sub-allocator `A_v(d_src) = S(d_src, 1)` (ASN-0040) of `d_src` activates upon `d_src`'s creation and produces version outputs by repeated sibling generation. The first fork's `d_new = inc(d_src, 1)`; a second fork — under the chain-advancement convention `inc(prev_version, 0)` — produces a distinct sibling. B8 (Uniqueness, ASN-0040), same-namespace clause — its precondition package discharged for `A_v(d_src)` by the B-Seq bridge of §"Identity by Sub-Allocation" — gives that no two forks of the same source share a tumbler, and B9 (UnboundedExtent, ASN-0040) gives that the stream admits arbitrarily many forks — for every bound `M` a reachable state with at least `M` emissions of `S(d_src, 1)` exists.
