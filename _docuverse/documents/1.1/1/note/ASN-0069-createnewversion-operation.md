...
V4a and V4b together are the structural basis of correspondence.
...
>
> *Non-monotonicity.* `Π_g` is not monotone — later arrangement edits (K.μ⁻, K.μ⁺, or K.μ~) M-targeted at `d_op` or `d_new` may remove or restore witnesses; by V5a(a), every step not M-targeted at `d_op` or `d_new` frames `M` at both, fixing `Corr_g` and hence `Π_g`.
>
