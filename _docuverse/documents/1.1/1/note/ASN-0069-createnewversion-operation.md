...
> **V8a** (*correspondence persistence under K.α*): Subsequent K.α allocations (extending `C`) leave every arrangement unchanged — K.α's frame condition `(A d :: M'(d) = M(d))` (ASN-0047) preserves `M(d_src)` and `M(d_new)` across each K.α step. Since V8's correspondence is an equality of arrangement values `M(d_src)(v) = M(d_new)(v)`, and neither side of the equality is modified by K.α, V8's correspondence between `d_src` and `d_new` over the V-positions present at fork time is preserved across every K.α step unconditionally — K.α cannot remove or alter V-positions from either arrangement, so the witness set inherited at fork-time is intact across any K.α-only sequence. (V8a is scoped specifically to K.α to match the consumption pattern of its downstream use sites — the post-fork content-allocation argument; the analogous arrangement-preservation under K.λ and K.ρ is subsumed by V8b's per-transition-kind enumeration, which establishes invariance of `Π_g` under each non-arrangement-modifying transition. Cross-operation evolution of the witness set under arrangement-modifying transitions, including possible removal by K.μ⁻ or remapping by K.μ~, is also the subject of V8b, not V8a.) (P0/S0 govern content-store growth — `dom(C) ⊆ dom(C')` and value preservation — which is orthogonal to arrangement preservation; V8a's persistence claim rests on K.α's arrangement-preservation frame, not on P0/S0.)
...
| V8a | Correspondence persists under K.α (arrangement-preservation frame); K.λ and K.ρ subsumed by V8b's per-transition-kind enumeration | introduced |
