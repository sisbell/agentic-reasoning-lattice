...
The two-step composition just sketched does not generalise by itself: for versions past the second, the prior version's content operand is the version *before* it, not `d_src`, so iterated pairwise composition is an induction along the emission sequence of `A_v(d_src)`. We supply that induction. Write the version stream of `d_src` as `w⁰ := d_src`, `w¹ = inc(d_src, 1)` (the first version), and `wⁱ = inc(wⁱ⁻¹, 0)` for `i ≥ 2` (each subsequent version). By V1's dispatch, version `wⁱ` (for `i ≥ 1`) is a fork of `d_src` whose J4 content source operand is the *immediate predecessor in the stream*: `d_op = w⁰ = d_src` at `i = 1` (first-fork sub-case) and `d_op = wⁱ⁻¹ = max(dom(A_v(d_src)))` at `i ≥ 2` (subsequent-fork sub-case). The content-operand chain is therefore `w⁰ → w¹ → ... → wᵏ`, with each step's content operand equal to its immediate predecessor — structurally the same operand chain as V11's fork-of-a-fork chain. The only difference is the *allocation* step, `inc(·, 0)` for `i ≥ 2` versus `inc(·, 1)` throughout V11's chain; but allocation fixes the *identity* of `wⁱ`, while content inheritance is governed entirely by the operand. V11's induction (derived in §"Composability") consumes only V4 at each step (which reads `M` of the immediate-predecessor operand), the per-step unedited premise on that operand, and V5 at the base step — none of which distinguishes `inc(·, 0)` from `inc(·, 1)`. The induction therefore transfers verbatim with `wⁱ⁻¹` in place of `dⁱ⁻¹_new`.

> **V8a** (*transitive correspondence along the unedited version stream*): For every version-stream length `k ≥ 1` and every version sequence `w⁰ = d_src, w¹, ..., wᵏ` produced by `A_v(d_src)` (each `wⁱ` a fork of `d_src` with content operand `wⁱ⁻¹`, per V1's dispatch), where each step `i`'s content operand `wⁱ⁻¹` has its content-subspace arrangement (set and pointwise values) unchanged between step `i − 1`'s post-state and step `i`'s pre-state, the I-addresses inherited by `wᵏ` are the same I-addresses `d_src` held at `Σ`: for every `v ∈ V_{s_C}(d_src)` evaluated at `Σ`, `v ∈ dom(M^k(wᵏ))` at the post-state of step `k`, and `M^k(wᵏ)(v)` there equals `M(d_src)(v)` at `Σ`. *Derivation.* The content-operand chain `w⁰ → ... → wᵏ` matches the operand structure of V11's chain (each step's operand is the immediate predecessor), and V11's induction depends only on operand-based facts (V4 at each step, the per-step unedited premise, V5 at the base) that are insensitive to whether allocation is `inc(·, 0)` or `inc(·, 1)`. Substituting `wⁱ⁻¹` for `dⁱ⁻¹_new` throughout V11's derivation establishes the conclusion. ∎
