...
> **V8b** (*fork-time correspondence is the maximal witness set*): Writing `F := V_{s_C}(d_op)|_{Σ'}` for the fork-time content-subspace witness set and `Corr_g` for the V-positions at which `d_op` and `d_new` still correspond at any later `Σ_g`, V8 gives `F ⊆ Corr_{Σ'}`: at fork-time every position of `F` witnesses correspondence. Any subsequent loss of correspondence at a position of `F` requires an edit or deletion to one side — both out of scope for this ASN — so within the fork transition vocabulary `F` remains the full witness set; the V8 correspondence is total over `F` and is bounded above by `F` for definitional reasons (`Corr_g` restricted to fork-time positions cannot exceed the fork-time set).
...
| V8b | Fork-time correspondence is total over the fork-time witness set `F := V_{s_C}(d_op)|_{Σ'}` and bounded above by it; shrinkage requires out-of-scope edits | introduced |
