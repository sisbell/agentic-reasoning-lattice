...
*Foundation gap (V11a).* V11a's prefix-chain derivation requires transitivity of the foundation prefix relation `≼`, but ASN-0034's Prefix contract exposes only the definition and the derived `p ≺ q ⟹ #p < #q` postcondition — transitivity is not a published postcondition. ASN-0034 should expose a `≼`-transitivity lemma so that downstream operation ASNs cite it rather than re-deriving generic order algebra inline; until then V11a depends on an unpublished foundation property.
