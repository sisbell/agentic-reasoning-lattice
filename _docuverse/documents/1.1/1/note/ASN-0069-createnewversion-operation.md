...
>
> In either case `d_new ∈ E'_doc`, `d_new ∉ E_doc` (pre-fork), `Document(d_new)`, and `parent(d_new) = parent(d_src)` — both by the inductions above. The new entity inherits the source's account-level prefix while extending into a fresh sub-tumbler.
...
*K.δ at the pre-fork state Σ.* The K.δ sub-case is determined by `A_v(d_src)`'s state. Four preconditions are discharged identically in both sub-cases, because each rests only on V1's identity postcondition (which holds in both) or on T10a-conformance of `A_v(d_src)` (which covers every output); we state them once. `Document(d_new)` (hence `zeros(d_new) = 2`) is V1's identity postcondition; we cite it rather than re-establish it from K.δ-ID.zeros-0/1. Outer-precondition `T4-valid(d_new)` is the T4-validity already established in §"Identity by Sub-Allocation" — every emission `cₙ ∈ S(d_src, 1)` satisfies T4 by B6(a)'s sufficiency clause (ASN-0040), and `d_new` is one such emission — so we cite that result rather than re-derive it here. Outer-precondition `¬Element(d_new)` follows from `Document(d_new)`: `zeros(d_new) = 2`, while `Element(d_new)` would require `zeros(d_new) = 3`; the two are exclusive. Uniform-precondition `parent(d_new) ∈ E` is discharged in two steps: V1 gives `parent(d_new) = parent(d_src)` (its identity-postcondition consequence, not re-derived here), and P8 (EntityHierarchy, ASN-0047), applied to `d_src ∈ E` with `¬Node(d_src)` (since `Document(d_src)` from `d_src ∈ E_doc` forces `zeros(d_src) = 2 ≠ 0`), yields `parent(d_src) ∈ E`; composing, `parent(d_new) = parent(d_src) ∈ E`. Only the freshness precondition `e ∉ E` and the per-sub-case operand preconditions remain sub-case-specific.
