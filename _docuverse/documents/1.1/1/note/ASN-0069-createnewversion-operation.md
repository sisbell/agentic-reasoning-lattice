...
>
> Membership of `v ∈ Π_g` is determined by the current arrangements alone: by `v ∈ F` (a fixed condition) and by `M_g(d_src)(v) = M_g(d_new)(v)` (evaluated at `Σ_g`). The membership status of any particular `v ∈ F` may shift across the transition sequence — K.μ⁻ on either side may remove `v`, K.μ~ on either side may remap its image, and a subsequent K.μ⁺ may re-install a previously-removed `v ↦ a` binding (admissible whenever `a ∈ dom(C)`, which P0 of ASN-0047 ensures permanently for every `a ∈ ran(M'(d_new))`). The set `Π_g` therefore need not decay monotonically: a position lost from `Π_g` via one transition may re-enter `Π_h` at a later state via restoration on both sides.
>
