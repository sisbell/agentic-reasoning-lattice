...
*Address uniqueness.* The producing allocator of `inc(d_src, 1)` is the *version sub-allocator* `A_v(d_src) = S(d_src, 1)` — not the document sub-allocator of any account. B8 (Uniqueness, ASN-0040) supplies the address-distinctness guarantee directly: cross-namespace baptisms produce distinct addresses unconditionally, and same-namespace baptisms produce distinct addresses under a single authority — so no two forks, of `d_src` or any other document, share a tumbler. (At the tumbler-algebra level this is underwritten by T10a.6 (DomainDisjointness, ASN-0034), which makes `A_v(d_src)`'s domain disjoint from every other allocator's domain.) The K.δ precondition `e ∉ E` (uniformly required for all sub-cases) forces `d_new` to be a fresh tumbler. Combined with T8 (AllocationPermanence, ASN-0034), once `d_new` enters `E`, it remains in `E` for all subsequent reachable states. The identity is permanent.
...
>
> *Corollary 2 — pairwise independence.* For any two distinct documents `d¹, d² ∈ E_doc` and any subsequent sequence `Σ' →* Σ''` in which no step is M-targeted at `d¹`: `M''(d¹) = M'(d¹)`. This is V5a instantiated at `d* = d¹`.
...
The behavior of each property over the empty case is discharged concretely on `d_src°` in the worked example's "Empty source (V7)" paragraph below; we do not restate that case-by-case here. The single organising principle is quantifier domain: the *structural* properties — V1 (identity), V2 (prefix-encoded ancestry), V3 (no content allocated), and V12(a) (joint entity permanence of `d_src` and `d_new`) — hold substantively, since they assert facts about the entities and the content store rather than about content-subspace V-positions, while *every* property whose universal quantifier ranges over `V_{s_C}(d_op)` or `V_{s_C}(d_src)` holds vacuously, because that domain is empty (on a first fork `d_op = d_src` and `V_{s_C}(d_src) = ∅`; on a subsequent fork `d_op = d_prev` with `V_{s_C}(d_prev) = ∅`). V6 reduces to `V_{s_L}(d_new) = ∅`, which K.δ establishes directly by initialising `M'(d_new) = ∅`. A fork of an empty fork produces a third empty entity, each with prefix-encoded ancestry via V2 but no shared I-addresses (because there were none to share); the fork chain remains structurally coherent.
...
V8 says: immediately after forking, every content-subspace V-position of the content source `d_op` corresponds to the same V-position in `d_new`, with the same I-address. On the first fork `d_op = d_src`, so this is full correspondence between the named source and the fork; on a subsequent fork it is correspondence between the prior version `d_op` and the fork. The general transitive correspondence across the full version stream — `d_src ↔ d^k_version` for any `k` — is V8a below. The correspondence is *exact, structural, and computable from the I-address equality alone*. No history is consulted; no derivation lineage is traversed.

We establish this transitive correspondence by induction along the emission sequence of `A_v(d_src)`. Write the version stream of `d_src` as `w⁰ := d_src`, `w¹ = inc(d_src, 1)` (the first version), and `wⁱ = inc(wⁱ⁻¹, 0)` for `i ≥ 2` (each subsequent version). These `wⁱ` are exactly the *sibling forks* of `d_src` — the emissions of `A_v(d_src)`, written `d_new^i` (with `w⁰ = d_src` and `wⁱ = d_new^i` for `i ≥ 1`); each has length `#d_src + 1`. V8a's version stream is therefore the *sibling* configuration; V11's chain `d^i_new` (length `#d_src + i`) is the distinct *nesting* configuration. By V1's dispatch, version `wⁱ` (for `i ≥ 1`) is a fork of `d_src` whose J4 content source operand is the *immediate predecessor in the stream*: `d_op = w⁰ = d_src` at `i = 1` (first-fork sub-case) and `d_op = wⁱ⁻¹ = max(dom(A_v(d_src)))` at `i ≥ 2` (subsequent-fork sub-case). The content-operand chain is therefore `w⁰ → w¹ → ... → wᵏ`, each step's operand equal to its immediate predecessor. Content inheritance is governed entirely by the operand — V4 reads `M` of the operand and is silent on whether the *allocation* step was `inc(·, 0)` or `inc(·, 1)` — so the induction below is carried by V4 and V5 alone.

> **V8a** (*transitive correspondence along the unedited version stream*): For every version-stream length `k ≥ 1` and every version sequence `w⁰ = d_src, w¹, ..., wᵏ` produced by `A_v(d_src)` (each `wⁱ` a fork of `d_src` with content operand `wⁱ⁻¹`, per V1's dispatch), where each step `i`'s content operand `wⁱ⁻¹` has its content-subspace arrangement (set and pointwise values) unchanged between step `i − 1`'s post-state and step `i`'s pre-state, the I-addresses inherited by `wᵏ` are the same I-addresses `d_src` held at `Σ`: for every `v ∈ V_{s_C}(d_src)` evaluated at `Σ`, `v ∈ dom(M^k(wᵏ))` at the post-state of step `k`, and `M^k(wᵏ)(v)` there equals `M(d_src)(v)` at `Σ`.
>
> *Derivation by induction on stream length `k`.*
>
> *Base case (`k = 1`).* `w¹ = inc(d_src, 1)` is the first fork of `d_src` with content operand `w⁰ = d_src`. V4 at this fork gives `M¹(w¹)(v) = M(d_src)(v)` and `v ∈ dom(M¹(w¹))` for every `v ∈ V_{s_C}(d_src)`, both sides at the post-state of step 1. V5 at step 1 gives `M¹(d_src) = M(d_src)`, so `V_{s_C}(d_src)` is the same set, with the same pointwise values, at `Σ` (the pre-state of step 1) and at post-step-1. Hence for every `v ∈ V_{s_C}(d_src)` at `Σ`, `v ∈ dom(M¹(w¹))` and `M¹(w¹)(v) = M(d_src)(v)` at `Σ`.
>
> *Inductive step (`k ≥ 2`).* The induction hypothesis is that for every `v ∈ V_{s_C}(d_src)` at `Σ`, `v ∈ dom(M^{k-1}(wᵏ⁻¹))` at post-step-(k−1) and `M^{k-1}(wᵏ⁻¹)(v)` there equals `M(d_src)(v)` at `Σ`. The IH membership plus `subspace(v) = s_C` gives `v ∈ V_{s_C}(wᵏ⁻¹)` at post-step-(k−1). The unedited premise at step `k` carries this membership and its value unchanged into step `k`'s pre-state. `wᵏ = inc(wᵏ⁻¹, 0)` is a subsequent fork of `d_src` with content operand `wᵏ⁻¹`, so V4 at step `k` applies for every such `v`: `v ∈ dom(M^k(wᵏ))` at post-step-k with `M^k(wᵏ)(v) = M^{k-1}(wᵏ⁻¹)(v)` at pre-step-k. Composing with the premise and the IH: `M^k(wᵏ)(v)` at post-step-k equals `M(d_src)(v)` at `Σ`. The induction closes. ∎
...
The version sub-allocator `A_v(d_src) = S(d_src, 1)` (ASN-0040) of `d_src` activates upon `d_src`'s creation and produces version outputs by repeated sibling generation. The first fork's `d_new = inc(d_src, 1)`; a second fork — under the chain-advancement convention `inc(prev_version, 0)` — produces a distinct sibling. B8 (Uniqueness, ASN-0040) gives that no two forks of the same source share a tumbler (the same-namespace clause under a single authority), and B9 (UnboundedExtent, ASN-0040) gives that the stream admits arbitrarily many forks — for every bound `M` a reachable state with at least `M` emissions of `S(d_src, 1)` exists. (At the tumbler-algebra level B8's same-namespace distinctness is underwritten by T10a.7 (EnumerationInjectivity), which makes the indexing map of the sub-allocator's outputs injective.)
...
>
> (a) *Distinct identities.* `d_new¹ ≠ d_new²`. By V1, both `d_new¹` and `d_new²` are emissions of `A_v(d_src)` — both lie within a single allocator's domain, so no cross-allocator collision is possible to consider. By SequentialTransitionAxiom (ASN-0047) and P1 (entity permanence, ASN-0047), `d_new¹` is already in `E` at `Σ_g`, so the K.δ event placing `d_new²` is a strictly later baptism in the same namespace `S(d_src, 1)` under a single authority. B8 (Uniqueness, ASN-0040), same-namespace clause, gives `d_new¹ ≠ d_new²` directly. (At the tumbler-algebra level the two emissions lie at distinct enumeration indices and T10a.7 (EnumerationInjectivity, ASN-0034) gives distinct addresses at distinct indices.)
>
...
Uniform-precondition `parent(d_new) ∈ E` is discharged in two steps. V1 gives `parent(d_new) = parent(d_src)` (its identity-postcondition consequence, not re-derived here). P8 (EntityHierarchy, ASN-0047), applied to `d_src ∈ E` with `¬Node(d_src)` (since `Document(d_src)` from `d_src ∈ E_doc` forces `zeros(d_src) = 2 ≠ 0`), yields `parent(d_src) ∈ E`. Composing: `parent(d_new) = parent(d_src) ∈ E`.

`Document(d_new)` (hence `zeros(d_new) = 2`) is V1's identity postcondition; we cite it rather than re-establish it from K.δ-ID.zeros-0/1.
...
Uniform-precondition `parent(d_new) ∈ E` is discharged in two steps. V1 gives `parent(d_new) = parent(d_src)` (its identity-postcondition consequence, holding in both sub-cases; not re-derived here). P8 (EntityHierarchy, ASN-0047), applied to `d_src ∈ E` with `¬Node(d_src)` (`Document(d_src)` forces `zeros(d_src) = 2 ≠ 0`), yields `parent(d_src) ∈ E`. Composing: `parent(d_new) = parent(d_src) ∈ E`.
...
| V5a | Per-document arrangement independence: a sequence with no step M-targeted at `d*` preserves `M(d*)`, by composition of ASN-0047's per-transition frame conditions (source-isolation / pairwise-independence corollaries) | introduced |
...
| V8a | Transitive correspondence along the unedited version stream: for a version sequence `d_src, w¹, ..., wᵏ` of `A_v(d_src)` with each step's prior-version operand unedited across the gap, the I-addresses inherited by `wᵏ` equal those `d_src` held at `Σ` | introduced |
...
ASN-0040 (Tumbler Baptism) grounds version identity (§"Identity by Sub-Allocation"): the version sub-allocator `A_v(d_src)` is the baptism sibling stream `S(d_src, 1)`; the next fork address is `next(s.B, d_src, 1)` (NextAddress); the depth-1 baptism from a document is valid by B6 (ValidDepth, `zeros(d_src) + (1 − 1) = 2 ≤ 3`); every emission is T4-valid by B6(a)'s sufficiency clause; distinct forks are distinct addresses by B8 (Uniqueness); and the version stream extends without bound by B9 (UnboundedExtent). ASN-0040 is correctly retained.
