...
The version sub-allocator `A_v(d_src)` is exactly ASN-0040's *sibling stream* `S(d_src, 1)`. By definition (ASN-0040), `S(p, d) = c₁, c₂, ...` with `c₁ = inc(p, d)` and `cₙ₊₁ = inc(cₙ, 0)`; instantiating `p = d_src`, `d = 1` gives `c₁ = inc(d_src, 1)` and `cₙ₊₁ = inc(cₙ, 0)` — precisely `A_v(d_src)`'s first-and-subsequent emission rule (ASN-0047). The two are the same stream. The baptism depth is valid: B6(`d_src`, 1) (ValidDepth, ASN-0040) requires `d_src` satisfy T4 (it does, as a document entity), `d ∈ {1, 2}` (here `d = 1`), and `zeros(d_src) + (d − 1) ≤ 3` (here `2 + 0 = 2 ≤ 3`). Three foundation results then transfer directly: (i) the next address a fork emits is `next(s.B, d_src, 1)` (NextAddress, ASN-0040); (ii) every emission `cₙ ∈ S(d_src, 1)` satisfies T4 by B6(a)'s sufficiency clause; (iii) distinct emissions of one namespace are distinct addresses by B8 (Uniqueness), and the stream extends without bound by B9 (UnboundedExtent). The forking event *is* a baptism at depth 1 from `d_src`.

B8's same-namespace clause, in force for two forks of `S(d_src, 1)`, requires B-Seq, B0a, B1, B2, and B4 for that namespace; we establish each. **B-Seq.** ASN-0040's B-Seq requires that the states realized under a single serialized commit path be totally ordered by `→*`. ASN-0047's SequentialTransitionAxiom states exactly that atomic transitions `Σ → Σ'` are totally ordered and `→*` is their finite composition; the docuverse has one commit path, so its reachable states are totally ordered by `→*`, discharging B-Seq. **B0a, B1, B2, B4.** These are registry-discipline properties of the sibling stream `S(d_src, 1)`, inherited unchanged through the identification `A_v(d_src) = S(d_src, 1)`: B0a (BaptismalClosure) holds because the only ASN-0047 transition that emits into `A_v(d_src)` is K.δ case (ii) on `d_src`'s version chain, every other transition framing the registry; B1 (ContiguousPrefix) holds because `A_v(d_src)` advances solely by the `inc(·, 0)` chain from base `inc(d_src, 1)`, so its emitted children are a contiguous prefix of `S(d_src, 1)`; B2 (HighWaterMarkSufficiency) is B1's corollary; and B4 (AtomicBaptism) holds because each K.δ emission is a single atomic transition by SequentialTransitionAxiom. With B-Seq, B0a, B1, B2, B4 in force, B8's same-namespace clause applies to any two forks of `d_src`.

The *level* and *parent* of each emission — that every `d_new` satisfies `Document(d_new)` and `parent(d_new) = parent(d_src)` — follow from K.δ's structural identities, not from baptism. The producing allocator is `A_v(d_src)` (ASN-0047's Allocator hierarchy), which emits its first output via K.δ case (ii) at `k = 1` — `inc(d_src, 1)`, by TA5(d) appending a single non-zero component — and emits each subsequent output via K.δ case (ii) at `k = 0` — `inc(d_prev, 0)`, by TA5(c) preserving length while advancing the trailing component.

*Level.* `A_v(d_src)`'s outputs inhabit `E_doc` (ASN-0047's Allocator hierarchy), so every emission — `d_new` among them — satisfies `Document(d_new)`, hence `zeros(d_new) = 2` by T4c.

*Parent.* We establish `parent(d_new) = parent(d_src)` by induction on `A_v(d_src)`'s emission count. K.δ-ID.parent-0/1 (ASN-0047) supplies the per-step relation `parent(inc(t, k)) = parent(t)` for `k ∈ {0, 1}`.
...
> **V1** (*new-version identity*): A fork of `d_src` produces a new entity `d_new` allocated as `A_v(d_src) = S(d_src, 1)`'s next emission `next(s.B, d_src, 1)` (NextAddress, ASN-0040; equivalently the Allocator-hierarchy frontier of ASN-0047):
>
...
>
> In either case `d_new ∈ E'_doc`, `d_new ∉ E_doc` (pre-fork), `Document(d_new)`, and `parent(d_new) = parent(d_src)` — both established above. The new entity inherits the source's account-level prefix while extending into a fresh sub-tumbler.

Two consequences follow without further machinery.

*Structural ancestry.* J4 (ASN-0047) supplies `d_src ≼ d_new` directly as a derived consequence in both sub-cases (`d_src ≼ inc(d_src, 1)` on a first fork, `d_src ≼ inc(d_prev, 0)` on a subsequent fork).
...
We now characterize `M'(d_new)`. The fork populates it from `d_op`'s content-subspace arrangement.
...
>
> *Derivation.* Each arrangement-modifying transition carries the frame condition `(A d' : d' ≠ d_target : M'(d') = M(d'))` for its single named target (ASN-0047); the non-arrangement-modifying transitions K.α, K.λ, K.ρ preserve every document's arrangement, and K.δ preserves every document other than its fresh `d_new`. That `d_new ≠ d*` at each K.δ pre-step holds because `d* ∈ Σ.E_doc` by hypothesis and P1 (EntityPermanence, ASN-0047) carries the standing membership `d* ∈ E` to every intermediate pre-step, while the K.δ precondition `e ∉ E` places `d_new ∉ E` pre-step. Composing these per-transition frame conditions along the sequence — each step preserving `M(d*)` because none targets `d*` — gives `M'(d*) = M(d*)`. ∎
...
> **V8** (*positional correspondence — corollary of V4 + content-source operand frame*): For every `v ∈ V_{s_C}(d_op)`: `v ∈ dom(M'(d_new))` and `M'(d_op)(v) = M'(d_new)(v)`. The per-document frame gives `M'(d_op) = M(d_op)` (the K.μ⁺ phase targets only `d_new`, and `d_op ≠ d_new`), and V4 gives `v ∈ dom(M'(d_new))` with `M'(d_new)(v) = M(d_op)(v)`, so `M'(d_op)(v) = M(d_op)(v) = M'(d_new)(v)`.

V8 says: immediately after forking, every content-subspace V-position of the content source `d_op` corresponds to the same V-position in `d_new`, with the same I-address. On the first fork `d_op = d_src`, so this is full correspondence between the named source and the fork; on a subsequent fork it is correspondence between the prior version `d_op` and the fork. The correspondence is *exact, structural, and computable from the I-address equality alone*. No history is consulted; no derivation lineage is traversed.
...
>
> *Derivation.* The fork composite's K.ρ × n phase records one pair `(aⱼ, d_new)` per `aⱼ ∈ ran(M'(d_new))`, with cumulative effect `R' = R ∪ {(a, d_new) : a ∈ ran(M'(d_new))}`. Hence every `a ∈ ran(M'(d_new))` satisfies `(a, d_new) ∈ R'`. ∎
...
> **V9a** (*provenance records containment, not derivation edges*): A fork's K.ρ phase adds pairs `(a, d_new)` recording that `d_new` came to contain `a`, but no edge connecting that acquisition to `d_src`, to a prior version, or to any third document that also contained `a`. The provenance relation is therefore silent on *whether* `d_new` obtained `a` by fork or by transclusion.
...
The version sub-allocator `A_v(d_src) = S(d_src, 1)` (ASN-0040) of `d_src` is activated by its first emission — the first fork, which spawns its base address `inc(d_src, 1)` — and thereafter produces version outputs by repeated sibling generation. The first fork's `d_new = inc(d_src, 1)`; a second fork — under the chain-advancement convention `inc(prev_version, 0)` — produces a distinct sibling. B9 (UnboundedExtent, ASN-0040) gives that the stream admits arbitrarily many forks — for every bound `M` a reachable state with at least `M` emissions of `S(d_src, 1)` exists.
...
>
> Superscript position is the disambiguator: `d_new²` is the second sibling fork of `d_src`; `d²_new` is the second link in a fork chain from `d_src`. The two are structurally distinct tumblers — `d_new²` has length `#d_src + 1` (TA5(c) at sibling step) while `d²_new` has length `#d_src + 2` (TA5(d) at chain step).
...
>
> (a) *Distinct identities.* `d_new¹ ≠ d_new²`. By V1, both `d_new¹` and `d_new²` are emissions of `A_v(d_src)` — both lie within a single allocator's domain, so no cross-allocator collision is possible to consider. By SequentialTransitionAxiom (ASN-0047) and P1 (entity permanence, ASN-0047), `d_new¹` is already in `E` at `Σ_g`, so the K.δ event placing `d_new²` is a strictly later baptism in the same namespace `S(d_src, 1)`. The same-namespace clause of B8 (Uniqueness, ASN-0040) applies — its precondition package B-Seq, B0a, B1, B2, B4 is discharged for `A_v(d_src)` by the B-Seq bridge of §"Identity by Sub-Allocation" — giving `d_new¹ ≠ d_new²` directly.
>
> (b) *Independent content shares.* Each sibling fork inherits content from its own content source operand `d_op` (J4) *at the moment of that fork*. `d_new¹`'s operand `d_op¹` is fixed by V1's fork-dispatch at `Σ`: `d_op¹ = d_src` when `d_new¹` is the first fork of `d_src`, and `d_op¹ = max(dom(A_v(d_src)))` (a prior version) otherwise; either way it reads `M(d_op¹)` at `Σ`. `d_new²`'s operand `d_op²` is fixed by V1's dispatch at `Σ_g`: since `d_new¹ ∈ E` at `Σ_g` by (a)'s argument, `A_v(d_src)` already has a frontier, so `d_new²` is necessarily a subsequent fork and `d_op² = max(dom(A_v(d_src)))` — the most recent prior version emitted by `A_v(d_src)` at `Σ_g` (e.g. `d_new¹` when no intervening sibling fork occurred) — and it reads `M(d_op²)` at `Σ_g`. The two reads may or may not agree, depending on the operands `d_op¹, d_op²` and on whether any intervening transition modified them between `Σ` and `Σ_g`. Their inherited V→I mappings live in separate arrangements `M¹(d_new¹)` and `M²(d_new²)`. Both preservation directions hold for any sequence beginning at `Σ²` (or any later state), where both siblings are already in `E_doc`: `d_new¹ ∈ E_doc` at `Σ²` by (a)'s argument and `d_new² ∈ E_doc` by its own K.δ event, both persisting forward by P1 (entity permanence, ASN-0047). Restricted to such a sequence, modifications M-targeted at `d_new¹` preserve `M(d_new²)`, and vice versa — V5a instantiated at `d* = d_new²` and at `d* = d_new¹` respectively, with V5a's `d* ∈ E_doc` hypothesis discharged at the sequence's initial state as just shown, and its no-step-M-targets-`d*` hypothesis discharged because a step targeting one sibling does not target the other (`d_new¹ ≠ d_new²` by V10(a)).
>
...
V10's part (a) follows from V1 and B8 as derived above. Part (b) follows from V5a instantiated at each sibling. Part (c) follows from V9 applied at each fork independently.
...
>
> *Derivation by induction on chain length `k`.* The first-fork premise fixes step `i`'s J4 content source operand `d_op = d^{i-1}_new`, so V4 at step `i` reads `M(d^{i-1}_new)`. The per-step unedited premise is read at `i = 1` under the convention that "step 0's post-state" denotes `Σ` itself — equivalently the pre-state of step 1 — so the premise at `i = 1` holds trivially by reflexivity of set and pointwise equality.
>
...
This compounds with V8: intercomparison of any two documents in the fork tree reduces to I-address equality and is relationship-independent (V8c), surfacing the V-positions where their inherited mappings still agree.
...
>
> (d) `(A a ∈ ran(M'(d_new)) :: (a, d_op) ∈ R'')` — provenance records for the *content source operand* `d_op` are also permanent. *Derivation.* V4 + V4b give range equality `ran(M'(d_new)) = ran(M(d_op)|_{V_{s_C}(d_op)})`, so every inherited `a` is content-subspace-referenced in `M(d_op)` at the pre-fork state `Σ` with `d_op ∈ E_doc`. P4★ (`Contains_C(Σ) ⊆ R`) is not a per-state invariant — ASN-0047 lists it only among the composite-boundary properties — so we must first establish that `Σ` is a composite boundary. The fork is itself a composite (V0); hence along any valid trace its invocation point `Σ` is one of the composite boundaries `Σ₀ →* Σ₁ →* ... ` of P4a, and P4★'s precondition is discharged at `Σ`. With `Σ` a composite boundary, `(a, d_op) ∈ Contains_C(Σ) ⊆ R` by P4★ at `Σ`, and P2 carries the pair into every subsequent `Σ''`.

The consequence for source-fork pairs specifically: neither owner can remove the shared content from the storage substrate. The source owner can delete content from their own *arrangement* (K.μ⁻ on `d_src`), and similarly the fork owner can delete from their own arrangement. Neither action affects the other's arrangement (V5a, instantiated at `d* = d_new` and at `d* = d_src` respectively) and neither affects `dom(C)` (V3 holds for K.μ⁻ as well; its frame condition is `C' = C`). The I-addresses persist; the arrangements evolve independently.
...
*K.δ at the pre-fork state Σ.* The K.δ sub-case is determined by `A_v(d_src)`'s state. Four preconditions are discharged identically in both sub-cases, because each rests only on V1's identity postcondition (which holds in both) or on T10a-conformance of `A_v(d_src)` (which covers every output); we state them once. `Document(d_new)` (hence `zeros(d_new) = 2`) by V1. Outer-precondition `T4-valid(d_new)` holds because every emission `cₙ ∈ S(d_src, 1)` satisfies T4 by B6(a)'s sufficiency clause (ASN-0040), and `d_new` is one such emission. Outer-precondition `¬Element(d_new)` follows from `Document(d_new)`: `zeros(d_new) = 2`, while `Element(d_new)` would require `zeros(d_new) = 3`; the two are exclusive. Uniform-precondition `parent(d_new) ∈ E` is discharged in two steps: `parent(d_new) = parent(d_src)` by V1, and P8 (EntityHierarchy, ASN-0047), applied to `d_src ∈ E` with `¬Node(d_src)` (since `Document(d_src)` from `d_src ∈ E_doc` forces `zeros(d_src) = 2 ≠ 0`), yields `parent(d_src) ∈ E`; composing, `parent(d_new) = parent(d_src) ∈ E`. Only the freshness precondition `e ∉ E` and the per-sub-case operand preconditions remain sub-case-specific.

*K.δ sub-case A — first fork.* `A_v(d_src)` has emitted no prior version. K.δ case (ii) with `k = 1`, `t = d_src`. The four shared preconditions are discharged by the intro above; the per-sub-case precondition at `k = 1` is `t ∈ E_doc`.

Per-sub-case: `d_src ∈ E_doc` is V0's precondition.

Outer-precondition `e ∉ E` (freshness of `d_new = inc(d_src, 1)`) is discharged by ChildSpawnFreshness (ASN-0047) at `t = d_src`, `k' = 1`, which gives `inc(d_src, 1) ∉ Σ.E ⟺ the (d_src, 1) child-spawn has not yet been performed`. The lemma's operand admissibility holds: `k' = 1` requires `Document(d_src)`, which follows from `d_src ∈ E_doc`. Sub-case A's governing predicate — `A_v(d_src)` has emitted no prior version — *is* the statement that the `(d_src, 1)` child-spawn (the K.δ event that activates `A_v(d_src)` and places its base address `inc(d_src, 1)` into `E`) has not yet fired. The biconditional then yields `d_new ∉ E` directly. The remaining K.δ preconditions are the four shared discharges above.

*K.δ sub-case B — subsequent fork.* `A_v(d_src)` has prior emissions with most recent `d_prev ∈ E_doc`. K.δ case (ii) with `k = 0`, `t = d_prev`. The four shared preconditions are discharged by the intro above; the per-sub-case precondition at `k = 0` is `t ∈ E ∧ ¬Node(t) ∧ inc(t, 0) ∉ E`.
...
| V4 | `(A v ∈ V_{s_C}(d_op) :: v ∈ dom(M'(d_new)) ∧ M'(d_new)(v) = M(d_op)(v))` — literal arrangement inheritance from the content source operand `d_op` | introduced |
...
| V8d | Perpetuity of correspondence: while neither `d_op` nor `d_new` is M-targeted, V8's equality `M''(d_op)(v) = M''(d_new)(v)` persists (V5a) | introduced |
...
The inquiry declares `depends: [34, 36, 40, 47]`. All four are consumed. ASN-0034 (Tumbler Algebra), ASN-0036 (Strand Model), and ASN-0047 (Transition Model) supply the tumbler order, the strand/content model, and the transition vocabulary respectively.

ASN-0040 (Tumbler Baptism) supplies NextAddress, B6 (ValidDepth), B8 (Uniqueness), and B9 (UnboundedExtent), consumed in §"Identity by Sub-Allocation".
