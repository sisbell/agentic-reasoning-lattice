...
>
> *(a) Per-elementary-transition frame.* For any single elementary transition `Σ → Σ'` and any document `d* ∈ E_doc`: if the transition is M-targeted at some `d_target ≠ d*`, or is any non-arrangement-modifying elementary transition (K.α, K.λ, K.δ, K.ρ), then `M'(d*) = M(d*)`.
>
