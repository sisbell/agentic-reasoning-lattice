...
We pause to record what V1 and V2 do *not* yet claim. They establish identity and ancestry only; content inheritance, source–fork correspondence, and source isolation follow. The identity argument is structurally independent of all of these. K.δ creates an empty-arrangement document; the fork's arrangement starts empty (the `Document(e)` effect clause of K.δ sets `M'(d_new) = ∅`).
