...
*Sub-case (a) — first emission.* When `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d_new} = ∅`, the determinate first emission of `A_L(d_new)` fixes `ℓ_new = [d_new.0.s_L.1]` (AllocatorHierarchy, ASN-0047). SubAllocatorBundle (ASN-0047) certifies that this first emission satisfies `ℓ_new ∉ dom(Σ.C) ∪ dom(Σ.L)` at the state of allocation (freshness), with `origin(ℓ_new) = d_new` and `#E(ℓ_new) = 2`.
