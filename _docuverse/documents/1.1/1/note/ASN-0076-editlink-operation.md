...
These are the foundation items most central to the construction. Additional items — from ASN-0034 (T0, T1, T4, T4b, T10a.7, T12, TA5, TA5-SigValid, OrdinalDisplacement), ASN-0043 (L0, L1b, L14, L-fin), ASN-0047 (SC-NEQ, SubAllocatorBundle, AllocatorHierarchy, SubspaceConventionAxiom, SequentialTransitionAxiom, ExtendedReachableStateInvariants, ExtendedTransitionInvariants, ValidComposite★, the couplings J0/J1★/J1'★, the persistence invariants P0–P3 with P4★/P4a/P7a), and ASN-0098 (LP13 for multi-step link persistence across reachable sequences; LP12, LP17, LP18 for the projection/discoverability framework reconciled in E7) — are cited at the points where they bear on a specific discharge.
...
By L13 and PrefixSpanCoverage, `coverage(E_from) = {t ∈ T : ℓ_old ≼ t}` — the full prefix-closure of `ℓ_old` in `T`, an infinite set of tumblers fixed combinatorially and independent of what is allocated (coverage consults no state component; Definition — Coverage, ASN-0098). In particular `ℓ_old ∈ coverage(E_from)`, which is all the downstream proofs (E4, E7) require. Similarly for `E_to` and `E_type`. The supersession link has arity 3, satisfies L3 (`e_3 ≠ ∅`), and references the two link entities via canonical unit-depth spans.
...
**EDITLINK as a valid composite.** We verify against ValidComposite★ (ASN-0047). The composite applies K.λ twice. (i) Elementary preconditions of K.λ are satisfied at each intermediate state, as discharged in E0 below. (ii) J0 (AllocationPlacementCoupling) is vacuously satisfied: K.λ's frame preserves `C`, so `dom(Σ'.C) = dom(Σ.C)` across the composite and the antecedent `a ∈ dom(C') \ dom(C)` is empty. (iii) J1★ (ExtensionRecordsProvenance) and J1'★ (ProvenanceRequiresExtension) are vacuously satisfied: K.λ's frame preserves all arrangements `M(d)`, so the range differences `ran(M'(d)) \ ran(M(d))` are empty, and the antecedent of each coupling is empty. Therefore EDITLINK satisfies ValidComposite★.
...
*Sub-case (a) — first emission.* When `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d_new} = ∅`, the determinate first emission of `A_L(d_new)` fixes `ℓ_new = [d_new.0.s_L.1]` (AllocatorHierarchy, ASN-0047), with `origin(ℓ_new) = d_new` and `#E(ℓ_new) = 2` (SubAllocatorBundle, ASN-0047). We derive freshness *at `Σ`* — the state at which the successor step fires — rather than appealing to first-emission freshness at the earlier entity-allocation event. For `ℓ_new ∉ dom(Σ.L)`: `ℓ_new` is an emission of `A_L(d_new)`, so `origin(ℓ_new) = d_new` (AllocatorHierarchy); were `ℓ_new ∈ dom(Σ.L)`, it would belong to `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d_new}`, which the sub-case condition declares empty — contradiction. For `ℓ_new ∉ dom(Σ.C)`, we use the same subspace argument as sub-case (b): by AllocatorHierarchy `subspace_I(ℓ_new) = s_L`, by L0 (ASN-0047) every `a ∈ dom(Σ.C)` satisfies `subspace_I(a) = s_C`, and by SC-NEQ (ASN-0047) `s_C ≠ s_L`; hence `ℓ_new ∉ dom(Σ.C)`. Together, `ℓ_new ∉ dom(Σ.C) ∪ dom(Σ.L)` at `Σ`.

*Sub-case (b) — subsequent emission.* When `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d_new} ≠ ∅`, the T10a-conforming `inc(·, 0)` sibling-advance discipline of `A_L(d_new)` (SubAllocatorBundle, ASN-0047) extends the allocator's enumeration, fixing `ℓ_new = inc(max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d_new}, 0)`. Since `A_L(d_new)` is T10a-conforming (SubAllocatorBundle), by L11a (LinkUniqueness, ASN-0043) the output is distinct from every prior link allocation, so `ℓ_new ∉ dom(Σ.L)`. By AllocatorHierarchy (ASN-0047), every output of `A_L(d_new)` satisfies `subspace_I(ℓ_new) = s_L`; by L0 (ASN-0047), every `a ∈ dom(Σ.C)` satisfies `subspace_I(a) = s_C`; by SC-NEQ (ASN-0047), `s_C ≠ s_L`; hence `ℓ_new ∉ dom(Σ.C)`.

In both sub-cases, SubAllocatorBundle (ASN-0047) certifies that `ℓ_new` is T4-valid with `zeros(ℓ_new) = 3`; AllocatorHierarchy (ASN-0047) gives `E(ℓ_new)_1 = s_L`. The origin equality `origin(ℓ_new) = d_new` follows from the allocator rule (`ℓ_new` is emitted by `A_L(d_new)`, whose outputs all carry origin `d_new`, by AllocatorHierarchy). The depth bound `#E(ℓ_new) ≥ 2` holds for every emission of `A_L(d_new)` by induction on the position of `ℓ_new` in the allocator's enumeration. We do not re-derive the structural facts the foundation already fixes for `A_L(d_new)` outputs: every emission is T4-valid (SubAllocatorBundle) with `zeros = 3` (AllocatorHierarchy, ASN-0047), so for any prior emission `t` and its successor `t' = inc(t, 0)`, `zeros(t') = zeros(t) = 3` is a foundation consequence — not something this proof must establish — and we cite it rather than reconstructing it from T0/T4 field-segment primitives. The base case — first emission — has `#E = 2` by SubAllocatorBundle. For the step case it remains only to confirm that `inc(·, 0)` leaves the field-boundary structure, and hence the element-field length, unchanged: TA5(c) gives `#t' = #t` and modifies only the `sig(t)` component (`t'_{sig(t)} = t_{sig(t)} + 1`); TA5(b) confines all other positions (`t'_i = t_i` for `i ≠ sig(t)`); and since `t` is T4-valid, TA5-SigValid (ASN-0034) places `sig(t) = #t` at the terminal position. With total length preserved (TA5(c)) and the zero positions identical between `t` and `t'` (both carry `zeros = 3` by AllocatorHierarchy, and the sole modified position is the non-zero terminal one), the four field boundaries T4b derives are identical, so T4b's decomposition gives `#E(t') = #E(t)`. Hence every emission of `A_L(d_new)`, in either sub-case, satisfies `#E ≥ 2`. The endset sequence `(e'_1, ..., e'_N)` satisfies L3 by the precondition of the composite. Clauses (i)–(iv) of K.λ are therefore discharged at the successor step.
...
*Proof.* We discharge the three memberships in turn. Let `Σ_1` be the intermediate state after the successor step and `Σ_2` the state after the supersession step. By K.λ's effect clause applied to the supersession step, `Σ_2.L = Σ_1.L ∪ {ℓ_sup ↦ (E_from, E_to, E_type)}`, so `Σ_2.L(ℓ_sup) = (E_from, E_to, E_type)`. EDITLINK's composite consists of exactly two K.λ steps with no further atomic transitions intervening: this is the composite definition (which lists exactly Step 1 and Step 2) together with ValidComposite★ (ASN-0047), which fixes a composite as the literal finite sequence of atomic transitions `Σ = Σ_0 → Σ_1 → … → Σ_n = Σ'`. Instantiated at `n = 2`, the sequence is `Σ → Σ_1 → Σ_2 = Σ'`, so `Σ' = Σ_2` and hence `Σ'.L(ℓ_sup) = (E_from, E_to, E_type)`. By L6 (SlotDistinction, ASN-0043), the slot accessor `Σ'.L(ℓ_sup).e_i` returns the `i`-th component of the tuple stored at `ℓ_sup`, so `Σ'.L(ℓ_sup).e_1 = E_from = {(ℓ_old, δ(1, #ℓ_old))}`. Since `(ℓ_old, δ(1, #ℓ_old)) ∈ E_from` by the construction of the singleton, `(ℓ_old, δ(1, #ℓ_old)) ∈ Σ'.L(ℓ_sup).e_1`. The same chain — K.λ effect → composite equality `Σ' = Σ_2` → L6 slot accessor → singleton membership — discharges `(ℓ_new, δ(1, #ℓ_new)) ∈ Σ'.L(ℓ_sup).e_2` via `E_to` and `(τ_sup, δ(1, #τ_sup)) ∈ Σ'.L(ℓ_sup).e_3` via `E_type`.
...
*Proof.* By induction on `k`, with the induction-indexed states `Σ_0, Σ_1, …, Σ_k` denoting the states after `0, 1, …, k` consecutive EDITLINK composites starting from `Σ` (so `Σ_0 := Σ` by convention). The base case `k = 0` is vacuous: `Σ_0 = Σ` realizes the required structure with the empty set of supersession links, and the universal conjunction over an empty index set holds trivially. By the outer hypothesis, `Σ` is reachable, so by ExtendedReachableStateInvariants (ASN-0047) it satisfies all per-state invariants of the extended reachable state.
...
**E7 (LineageWitness).** The supersession link's endsets structurally contain `ℓ_old` and `ℓ_new` as covering witnesses. This is a structural-witness claim about the contents of `Σ'.L`, *not* a claim of discoverability in the sense of ASN-0098 (which is arrangement-conditional; see the reconciliation below). Formally, define for each `a ∈ T` the *covering set*
...
The claim is structural: it concerns the relationship between `Σ'.L(ℓ_sup)`'s endsets and the addresses `ℓ_old`, `ℓ_new`, evaluated in the post-state link store. Any lookup operation that returns `covers(Σ, ·)` — or any superset of it that respects endset coverage — will surface `ℓ_sup` when queried with `ℓ_old` or `ℓ_new`. The formalization of such operations is the proper subject of the link-search specification; E7 establishes only that the structural witness is present.

**Reconciliation with ASN-0098's discoverability.** The `covers` predicate above must not be confused with ASN-0098's `discoverable_from`. The two run in opposite directions and rest on different state components. ASN-0098 fixes `project(e, d, Σ) = {v ∈ dom(Σ.M(d)) : Σ.M(d)(v) ∈ coverage(e)}` and `discoverable_from(a, d, Σ) ≡ (E i : project(a, i, d, Σ) ≠ ∅)`, equivalently (LP12) `coverage(Σ.L(a).eᵢ) ∩ ran(Σ.M(d)) ≠ ∅`. That notion is *arrangement-conditional*: a link is discoverable from `d` only when some arrangement entry of `d` lands on an I-address its coverage meets. Our `covers(Σ, a)` is instead the *inverse* lookup — it ranges over the link store `dom(Σ.L)` and collects links whose endset spans cover the queried address `a`, consulting no arrangement. It is therefore a property of `Σ.L` alone, not of any `Σ.M(d)`.

The distinction is load-bearing because EDITLINK never arranges its outputs: E10 establishes that the composite performs no K.μ⁺_L step, so neither `ℓ_sup`, `ℓ_new`, nor `ℓ_old` is placed into any document's arrangement by EDITLINK. Unless the referents `ℓ_old`, `ℓ_new` are independently arranged in some document — i.e. unless `coverage(Σ'.L(ℓ_sup).eᵢ) ∩ ran(Σ'.M(d)) ≠ ∅` for some `d` — `ℓ_sup` is *orphaned* in the exact sense of LP17 (GhostProjection, ASN-0098): `project(ℓ_sup, i, d, Σ') = ∅` for every `d, i`, and `ℓ_sup` is not `discoverable_from` any document. LP17 also tells us this is not a defect: `ℓ_sup` remains permanently in `dom(Σ'.L)` with its value fixed (L12), and LP18 (Resurrection) guarantees that once any subsequent transition arranges an I-address in its coverage, `ℓ_sup` becomes `discoverable_from` that document. E7 asserts the structural witness — the spans are present in `Σ'.L(ℓ_sup)` — which is precisely the precondition LP18 needs; it does *not* assert ASN-0098 discoverability, which awaits arrangement of the referents.
...
*Step 1.* `ℓ_new ← K.λ(d_bob, (e'_1, e'_2, e'_3))` fires from `Σ`. The first-emission case holds (no prior outputs of `A_L(d_bob)`), so the determinate first emission (AllocatorHierarchy, ASN-0047) produces `ℓ_new = [d_bob.0.s_L.1] = [4.0.2.0.3.0.2.1]`. The preconditions are discharged: `d_bob ∈ E_doc`; `ℓ_new ∉ dom(Σ.L) ∪ dom(Σ.C)` by SubAllocatorBundle; `zeros(ℓ_new) = 3 ∧ E(ℓ_new)_1 = s_L`; `#E(ℓ_new) = 2`; `origin(ℓ_new) = d_bob`; the endset sequence has arity 3 with non-empty third slot. The effect: `dom(L_1) = dom(Σ.L) ∪ {ℓ_new}`, with `L_1(ℓ_new) = (e'_1, e'_2, e'_3)`.

*Step 2.* `ℓ_sup ← K.λ(d_bob, (E_from, E_to, E_type))` fires from the intermediate state. Now `{ℓ' ∈ dom(L_1) : origin(ℓ') = d_bob} = {ℓ_new}`, so the subsequent-emission rule applies and produces `ℓ_sup = inc(ℓ_new, 0)`. Since `ℓ_new = [4.0.2.0.3.0.2.1]` is T4-valid, TA5-SigValid gives `sig(ℓ_new) = #ℓ_new = 8`, and TA5 specifies that `inc(·, 0)` increments the component at position `sig`, so `ℓ_sup = [4.0.2.0.3.0.2.2]`. The element-field depth bound `#E(ℓ_sup) = 2` follows directly: TA5(b) confines the modification to position 8 only (every other position satisfies `ℓ_sup[i] = ℓ_new[i]`), and the modified position raises `ℓ_new[8] = 1` to `ℓ_sup[8] = 2` — non-zero before, non-zero after, no other position touched, hence `zeros(ℓ_sup) = zeros(ℓ_new) = 3` and the three zero positions of `ℓ_sup` are 2, 4, 6 (identical to `ℓ_new`'s). T4b's field decomposition therefore agrees between `ℓ_new` and `ℓ_sup`, with the element field beginning at position 7; together with `#ℓ_sup = #ℓ_new = 8` (TA5(c)), this gives `#E(ℓ_sup) = 8 - 6 = 2`. The endsets resolve to:
...
**E0.** K.λ's preconditions were discharged for both steps in the trace above. Step 1 invoked the first-emission case (no prior outputs of `A_L(d_bob)`); SubAllocatorBundle furnished `ℓ_new ∉ dom(Σ.C) ∪ dom(Σ.L)` and `#E(ℓ_new) = 2` and `zeros(ℓ_new) = 3`, AllocatorHierarchy furnished `E(ℓ_new)_1 = 2 = s_L`; the allocator rule fixed `origin(ℓ_new) = d_bob`; the supplied `(e'_1, e'_2, e'_3)` satisfied L3. Step 2 invoked the subsequent-emission (`inc(·, 0)` sibling-advance) case with `ℓ_new` as the maximum prior output of `A_L(d_bob)`; SubAllocatorBundle's T10a-conformance + L11a gave `ℓ_sup ∉ dom(L_1)`, and AllocatorHierarchy's `subspace_I = s_L` + L0 + SC-NEQ gave `ℓ_sup ∉ dom(C_1)`; SubAllocatorBundle (`zeros = 3`) + AllocatorHierarchy (`E()_1 = s_L`) + TA5(c) + TA5(b) (inheriting `#E ≥ 2` from `ℓ_new`) discharged the namespace clauses. T12 held for the three supersession spans at `actionPoint(δ(1, 8)) = 8 ≤ 8 = #x`, with `Pos(δ(1, 8))` by OrdinalDisplacement (n = 1, m = 8); L3 thus held for `(E_from, E_to, E_type)`. ✓
...
| E7 | The supersession link's endsets structurally contain `ℓ_old` and `ℓ_new` as covering witnesses (an inverse-lookup `covers` property of `Σ.L`, distinct from and weaker than ASN-0098's arrangement-conditional `discoverable_from`; absent arrangement of the referents, `ℓ_sup` is orphaned per LP17) | introduced |
