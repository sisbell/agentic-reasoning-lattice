...
S8★ has no consumer within this ASN: no property here takes it as a premise — in particular D-SEQ★ derives from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a, not from S8★ — and the dropped uniqueness condition (c) is consequently never needed. S8★ is carried as a per-state invariant solely to *provision the downstream INSERT/DELETE operation ASNs*, whose run-mechanics — lockstep V-position/I-address advance, per-subspace run splitting on insert, suffix-shifting on delete — take the per-subspace correspondence-run decomposition as their working substrate. Within this ASN, S8★ discharges no obligation beyond its own preservation across each transition; its presence in the verification matrix records that the substrate survives every elementary transition intact, ready for those operation ASNs to consume.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | admissibility filter stipulates S3★(Σ'); π_swap witnesses non-vacuity |  frame |
