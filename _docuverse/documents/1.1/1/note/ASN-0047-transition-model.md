...
A composite transition `Σ →* Σ'` is *valid* (ValidComposite★) iff (1) each elementary step satisfies its own precondition at the intermediate state it acts on, and (2) the couplings J0, J1★, J1'★ hold for the composite as a whole, evaluated only between the initial Σ and final Σ'. The full statement, with the elementary transition set it ranges over, is in *Scoped coupling constraints* below.
