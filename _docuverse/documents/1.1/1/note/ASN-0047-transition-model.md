...
*Subsumption of ASN-0093's K.σ.* When `IsDocument(e)`, K.δ subsumes ASN-0093's K.σ (DocumentRegistration). ASN-0093's K.σ has effect `dom(M') = dom(M) ∪ {d}` with `M'(d) = ∅`; by the typing note's identity `d ∈ dom(M) ⟺ d ∈ E_doc`, this is the entry of `d` into `E_doc` (with `M'(d) = ∅` matching the totality convention for previously-empty documents). Every K.δ event with `IsDocument(e)` is such a route into `E_doc`, and S7d below enumerates three: the case (ii) k = 2 event with `parent(e) ∈ E_account` (creating a document under an account); the case (ii) k = 1 event with `t ∈ E_doc` (creating a version); and the case (ii) k = 0 event with `t ∈ E_doc` (a sibling document, `zeros = 2` preserved by K.δ-ID.zeros-0/1). Each is a single atomic event that, under ASN-0093's vocabulary, would be reported as K.σ. Per ASN-0093's SubAllocatorAxiom, the same K.δ event that places `d` into `E_doc` is the *joint child-spawn step* activating both `A_C(d)` and `A_L(d)` — captured under SubAllocatorAxiom.T10aConformance and elaborated in the *Allocator hierarchy under documents* section below. ASN-0047 therefore has no separate K.σ primitive; K.δ for `IsDocument(e)` is the consolidated entity-allocation, registry, and sub-allocator-activation event.
