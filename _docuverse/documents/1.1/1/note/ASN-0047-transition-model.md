...
*Proof of Step (B) — the K.μ⁻ + K.μ⁺ full-clearance decomposition realises any admissible π.* This step discharges the non-vacuity obligation: `S3★(Σ')` is the filter's hypothesis, and what remains is to show the K.μ⁻ + K.μ⁺ (full-clearance) decomposition actually realises an admissible π and produces a post-state `Σ' = (C, L, E, M', R)` that genuinely satisfies that `S3★(Σ')`, consuming Step (A)'s subspace preservation as the input that lets K.μ⁺'s amendment fire on the full-clearance write set. Let `Σ_int = (C, L, E, M_int, R)` denote the intermediate state between the K.μ⁻ and K.μ⁺ atomic steps of the decomposition. Three sub-claims, dispatched in order:
