...
**NodeUniqueAllocation (Axiom, FreshNodeAddress).** Every K.δ node-allocation event — every elementary transition of K.δ whose effect places an entity `e` with `IsNode(e)` into E — produces an address satisfying two conditions: (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation; (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order. A third clause anchors the registry's initial state: (c) *Bootstrap registry seeding:* `n₀` inhabits the node-allocation registry's tracked domain at `Σ₀`, so the T2-spawn discharge in K.δ case (ii) k = 2 with operand `t = n₀` (which creates the first account under `n₀`) is grounded by initial-state commitment rather than by appeal to a non-existent prior K.δ event for `n₀` itself. Clause (c) closes the bootstrap gap: every other entity `t ∈ E` reaches the registry's tracked domain via the K.δ event that minted it (clause (a) at that prior event), but `n₀` enters at `Σ₀` with no prior K.δ, and clause (c) supplies the registry membership directly. These are the conditions the node-allocation registry must satisfy; details of the registry mechanism (issuing protocol, persistence model, concurrency discipline) lie outside this ASN's discharge layer.
...
The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` in K.μ⁻'s effect forces `dom(M(d)) ≠ ∅` and also forces at least one V-position to be removed; the non-emptiness obligation does not need a separate precondition, and the strict-contraction conjunct stated below at clause (2) of the K.μ⁻ amendment is similarly a *consequence* of this effect clause, not an additional precondition the operation must verify before firing. Clause (2) is recorded for clarity at the per-subspace level — distinguishing which subspace(s) contract — rather than as a load-bearing precondition; clause (1)'s per-subspace patterns supply the *shape* constraint, and the effect clause `dom(M'(d)) ⊂ dom(M(d))` supplies the *non-trivial contraction* requirement.
...
**K.μ⁻ amendment (PerSubspaceScope).** In the extended state, K.μ⁻'s D-CTG / D-MIN postconditions read as D-CTG★ / D-MIN★ (per-subspace forms introduced immediately below) and the admissible removal pattern applies *per-subspace* under the D-SEQ★ enumeration `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`. The full per-subspace precondition is:
  ...
  (2) *Strict contraction (consequence of the whole-arrangement effect clause `dom(M'(d)) ⊂ dom(M(d))`, not an additional precondition).* `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` — at least one subspace shrinks strictly. The per-subspace patterns of (1) supply `V_S(d') ⊆ V_S(d)` for each S; together with the effect clause `dom(M'(d)) ⊂ dom(M(d))`, at least one of those per-subspace inclusions must be strict (`V_S(d') ⊊ V_S(d)`) since `dom(M'(d)) = V_{s_C}(d') ∪ V_{s_L}(d')` proper-subset of `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)` rules out simultaneous equality on both subspaces. The reading of clause (2) is therefore *informational* (locating which subspace(s) carry the contraction) rather than a separate precondition to verify; the operation discharges (2) by construction whenever its effect clause is satisfied.

**K.μ⁻ admissible contraction shape (per-subspace, derivation of clause (1)).** Fix a subspace S with D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`. Under D-CTG★, D-MIN★, and D-SEQ★ at the post-state, the post-state takes the suffix-removal form `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `0 ≤ n'_S ≤ n_S`.

*Proof.* If `V_S(d') = ∅`, the conclusion holds with `n'_S = 0`. Otherwise D-SEQ★ applied at the post-state gives `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `n'_S ≥ 1` directly (D-SEQ★ is the post-state invariant K.μ⁻ must discharge per clause (1) of its precondition); `V_S(d') ⊆ V_S(d)` (from K.μ⁻'s contraction effect) forces `n'_S ≤ n_S`. ∎
...
**SubAllocatorAxiom (Axiom, ContentLinkSubAllocatorExistence).** For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]`. The axiom comprises four sub-clauses:
...
- *Full content-subspace clearance and rebuild* at `n'_{s_C} = 0`: K.μ⁻ removes V_{s_C}(d) entirely (maximal-suffix removal, with link-subspace retained) and K.μ⁺ then adds `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` in one step. This expansion works for *every* admissible π — it is the universal default and the form referenced in the K.μ~ verification arguments below. The admissibility condition on the partial-suffix form is vacuous at `k₀ = 1` (no positions lie strictly below `[s_C, 1, ..., 1, 1]` under D-MIN★), so the full-clearance form is the `k₀ = 1` specialisation of the partial-suffix family and inherits its admissibility unconditionally.

We refer to the full-clearance form as the *canonical* expansion in the sense of universal applicability, not exclusivity: every partial-suffix expansion *whose admissibility condition holds* is equally valid, and a non-identity permutation that fixes every position strictly below some `[s_C, 1, ..., 1, k₀]` admits the partial-suffix form at `n'_{s_C} = k₀ − 1`. A π that disturbs any position below the smallest candidate cut admits only the full-clearance form (or partial-suffix forms with smaller `k₀` whose admissibility condition the disturbance still respects). K.μ⁻ must retain link-subspace mappings under every expansion — K.μ⁺ (amended) is content-only and K.μ⁺_L only places at the contiguous min or max, so any removed link-subspace position could not be restored.
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ by a finite sequence of valid composite transitions satisfies the conjunction of two clauses, partitioned by *temporal scope* (i.e., which states the invariant holds at along a composite). *Elementary per-state* invariants hold at **every** state reachable from Σ₀ — including every intermediate state within a composite — because each elementary transition preserves them individually. *Composite-boundary* invariants hold at every state reachable from Σ₀ when measured at *composite boundaries* (the initial Σ and final Σ' of any valid composite), but may transiently fail at intermediate states *within* a composite (e.g., after K.α before its companion K.μ⁺/K.ρ); they are restored by the close of each valid composite via the J0/J1★/J1'★ couplings.
  ...
  *Composite-boundary* (Class (b) of the proof below — discharged at boundaries by J0/J1★/J1'★):
  ...
  The Class (b) invariants may transiently fail at intermediate states; in particular, a composite that allocates fresh content (K.α) violates P7a at the post-K.α intermediate state (the new I-address is in dom(C') but no (·, d) entry yet sits in R'), with restoration by the composite-trailing K.ρ.
...
We walk Class (a) per invariant, citing the establishing and preserving transitions for each.
