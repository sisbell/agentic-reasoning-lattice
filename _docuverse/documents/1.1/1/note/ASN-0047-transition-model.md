...
**Link-subspace fixity and realisation.** Clause (v) records pointwise fixity `π(v) = v` for every `v ∈ dom_L(M(d))` on the admissible class. The functional and domain identities the full-clearance decomposition contributes — `dom_L(M'(d)) = dom_L(M(d))` and `M'(d)|_{dom_L} = M(d)|_{dom_L}` — are exactly **LRP** (stated and proved once above), and the sub-step below shows this functional identity carries CL-UNIQ to the post-state Σ' and confirms the realised π agrees with clause (v) pointwise (premise: CL-UNIQ at the pre-state Σ). Proof:
...
The *Composite-boundary verification matrix* below records, for each Class (b) property, where it transiently fails and the coupling that restores it (per the preamble's temporal-scope distinction). P4★ and P7a are per-state properties holding at boundaries; P4a is grouped with them because it too is established by the boundary couplings rather than preserved step-by-step (see its definition box).
...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | By the induction-along-the-witnessing-trace mechanism of the P4a definition box. | Not evaluated at intermediate states (see P4a definition box); discharged at the boundary Σ', which carries the witness placed by K.μ⁺. |
