...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that differs by k in its operand `t` and in the applicable allocator-discipline clause (freshness is discharged by the per-k mechanism of the K.δ definition, with GlobalUniqueness (ASN-0034) preserving distinctness). The three regimes:
...
The provenance couplings that pair K.μ⁺ with K.ρ — J1★ and its converse J1'★ — are stated and derived in *Scoped coupling constraints* below; here J0 is the only coupling constraining content allocation K.α.
