...
The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` in K.μ⁻'s effect forces `dom(M(d)) ≠ ∅` and also forces at least one V-position to be removed; the non-emptiness obligation does not need a separate precondition. The per-subspace suffix-removal *shape* on each subspace is forced by D-CTG★ + D-MIN★ + D-SEQ★ at the post-state (derived in the *K.μ⁻ admissible contraction shape* paragraph below); the effect clause `dom(M'(d)) ⊂ dom(M(d))` supplies the *non-trivial contraction* requirement at the whole-arrangement level and, combined with the derived per-subspace shape, forces strict contraction on at least one subspace as a derived consequence.
