...
where C : T ⇀ Val is the content store (per ASN-0036), L : T ⇀ Link is the link store (introduced in the *Link store and extended system state* section below), and M : T → (T ⇀ T) is total, satisfying M(d) = ∅ for d ∉ E_doc. Sections preceding the link store's introduction read Σ with `L = ∅`, under which L-related conjuncts are vacuous.
...
**LinkVPositionDepthAxiom (Axiom, FixedLinkVPositionDepth).** Each document `d ∈ E_doc` has a fixed link-subspace V-position depth `m_L(d) ≥ 2`: every V-position `v ∈ dom(M(d))` with `subspace(v) = s_L` satisfies `#v = m_L(d)`, and `m_L(d)` is determined at the first link-subspace insertion into `d` and unchanged thereafter. Formally `(A d ∈ E_doc, v : v ∈ dom(M(d)) ∧ subspace(v) = s_L : #v = m_L(d))`. The lower bound `m_L(d) ≥ 2` is inherited from S8a (ASN-0036, carried as a Class (a) per-state invariant here), which requires `#v ≥ 2` for every `v ∈ dom(M(d))`. The axiom resolves the genuine underdetermination S8-depth leaves open: S8-depth forces a common depth among *existing* link-subspace positions but is vacuous when `V_{s_L}(d) = ∅`, so the depth of the *first* link-subspace position is otherwise unconstrained. The axiom does not fix `m_L(d)` to any particular value — an implementation is free to choose any `m_L(d) ≥ 2` at first insertion (Nelson reserves the right to subdivide the link subspace "by further digits", LM 4/31); it requires only that the chosen depth be fixed per document.
...
- V-position v_ℓ satisfies:
  - subspace(v_ℓ) = s_L
  - depth m_L(d), supplied by **LinkVPositionDepthAxiom** (FixedLinkVPositionDepth, stated above at the head of this section). When V_{s_L}(d) ≠ ∅, m_L(d) is consistent with S8-depth on existing link-subspace positions; when V_{s_L}(d) = ∅, S8-depth is vacuous and the axiom supplies the depth directly.
...
| LinkVPositionDepthAxiom | Axiom (FixedLinkVPositionDepth): each document `d` has a fixed per-document link-subspace V-position depth `m_L(d) ≥ 2`, determined at first link-subspace insertion and unchanged thereafter. Resolves the underdetermination S8-depth leaves open on an empty link subspace, without pinning `m_L(d)` to any particular value |
