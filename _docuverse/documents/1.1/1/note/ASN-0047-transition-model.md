...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases, but the freshness conjunct `e ∉ E` is discharged by two distinct disciplines according to `k`. At **k = 0** (sibling advance) freshness is a *dynamic frontier guard*: the check `inc(t, 0) ∉ E` is against the present state Σ, and FrontierEquivalence characterizes it as the condition that `t` is the frontier of its chain — there is no structural fact that forces it, since a sibling of an already-allocated `t` could collide with a previously advanced sibling. At **k ∈ {1, 2}** (child-spawn) freshness is *structurally guaranteed*: T10a's at-most-once-per-`(t, k')` constraint together with GlobalUniqueness (ASN-0034) forces `inc(t, k)` to be a fresh address whenever the spawn `(t, k')` has not already been performed, so `e ∉ E` follows from the structural allocator discipline rather than from a live-state scan (this is the discharge the worked examples and S7d invoke). Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
