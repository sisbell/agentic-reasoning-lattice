...
For each `d ∈ E_doc` and each subspace `S ∈ {s_C, s_L}`, the per-subspace arrangement `M(d)|_{V_S(d)}` decomposes into a finite set of correspondence runs `{(v_j, a_j, n_j)}`: every `v ∈ V_S(d)` lies in exactly one run, and within each run the V-positions and I-addresses advance by shift in lockstep. S8★ retains the finite-run *partition* (run-cover: every V-position lies in exactly one run) — the theorem-level statement of ASN-0036's S8 — together with ASN-0036's S8 condition (a) (lockstep displacement: `shift(vⱼ, k) ∈ dom(M(d))`, `M(d)(shift(vⱼ, k)) = shift(aⱼ, k)`, and `shift(aⱼ, k) ∈ dom(Σ.C)`) and condition (b) (label well-definedness: `aⱼ = M(d)(vⱼ)` well-defined by S2, with `aⱼ ∈ dom(Σ.C)`). It intentionally does *not* carry ASN-0036's S8 condition (c) — uniqueness of the maximal-run decomposition. For the content subspace, the partition together with conditions (a) and (b) is exactly ASN-0036's S8. For the link subspace, the membership conjuncts of *both* (a) and (b) carry `dom(Σ.C)` — `shift(aⱼ, k) ∈ dom(Σ.C)` in (a) and `aⱼ ∈ dom(Σ.C)` in (b) — which is false for link-subspace labels: they reside in `dom(L)`, disjoint from `dom(C)` by L14. S8★(s_L) therefore satisfies the partition and conditions (a) and (b) under the substitution `dom(C) → dom(L)` in the membership conjuncts of both: lockstep displacement with `shift(aⱼ, k) ∈ dom(L)`, and label well-definedness with `aⱼ ∈ dom(L)`. The decomposition is established by two distinct routes, one per subspace:
