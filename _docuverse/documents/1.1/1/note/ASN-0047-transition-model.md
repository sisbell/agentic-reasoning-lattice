...
*Necessity and sufficiency of the precondition.* The precondition is stated against the pre-state `Σ` of a K.μ~ event. Necessity assumes π admissible, so subspace-preservation (iv) and link-fixity (v) both enter as hypotheses — (v) is derived in Step (A), Case `s_L`, from (iv), CL-UNIQ, and LRP; it does not follow from (i)+(iv)+CL-UNIQ for an arbitrary π, as the link-swap transposition shows. CL-UNIQ at `Σ` holds by the ExtendedReachableStateInvariants inductive hypothesis on the per-state invariants at the reachable pre-state.
