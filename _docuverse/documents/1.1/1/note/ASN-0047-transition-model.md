...
- *ASN-0093 K.σ effect `dom(M') = dom(M) ∪ {d}`.* Read as the entry of `d` into E_doc; ASN-0047 subsumes K.σ into K.δ for `IsDocument(e)`, stated and justified at *Subsumption of ASN-0093's K.σ* (K.δ definition).
...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ ValidAddress(e) ∧ ¬IsElement(e)` apply uniformly to all three sub-cases. The freshness conjunct is a *caller-checked guard*, observed before the event fires — it is a precondition, not a conclusion: at k = 0 the guard is `inc(t, 0) ∉ E`, at k ∈ {1, 2} it is `e ∉ E` directly. §*K.δ case (ii) discharge and parent-allocator activation* below records, in one place, the parent entity-level sub-allocator on which each step acts and the allocator-discipline properties that always applying the guard maintains; that analysis is not re-derived here. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
...
*Subsumption of ASN-0093's K.σ.* ASN-0047 has no separate K.σ primitive: when `IsDocument(e)`, K.δ carries document registration through the entry of `e` into `E_doc` (with `M'(e) = ∅` by the totality convention), subsuming ASN-0093's K.σ (DocumentRegistration). The routes by which a K.δ event reaches `E_doc` are enumerated under S7d below. Per ASN-0093's SubAllocatorAxiom, the same K.δ event that places `d` into `E_doc` is the *joint child-spawn step* activating both `A_C(d)` and `A_L(d)` — captured under SubAllocatorAxiom.T10aConformance and elaborated in the *Allocator hierarchy under documents* section below. K.δ for `IsDocument(e)` is therefore the consolidated entity-allocation, registry, and sub-allocator-activation event.
...
- **SubAllocatorAxiom.Namespace.** Every output of d's sub-allocators is T4-valid with `zeros(·) = 3` (per ASN-0093).
- **SubAllocatorAxiom.T10aConformance.** `A_C(d)` and `A_L(d)` are T10a-conforming allocators within d's allocator subtree, each activated by the entity-allocation event placing d into E_doc (per ASN-0093).
- **SubAllocatorAxiom.Disjointness (local lemma).** `dom(A_C(d)) ∩ dom(A_L(d)) = ∅`, and for any d ≠ d', `dom(A_C(d)) ∩ dom(A_C(d')) = ∅`, `dom(A_L(d)) ∩ dom(A_L(d')) = ∅`, `dom(A_C(d)) ∩ dom(A_L(d')) = ∅`. *Within-document discharge:* every output of `A_C(d)` has `E(·)₁ = s_C` by SubAllocatorAxiom.Subspace, and every output of `A_L(d)` has `E(·)₁ = s_L` by SubAllocatorAxiom.Subspace; SC-NEQ (SubspaceConventionAxiom) gives `s_C ≠ s_L`; T7 (SubspaceDisjointness, ASN-0034) — whose element-level hypothesis `zeros(·) = 3` is supplied at the output level by SubAllocatorAxiom.Namespace — then makes any `A_C(d)` output distinct from any `A_L(d)` output. *Cross-document discharge:* all three clauses follow from the Cross-document disjointness chain lemma (below) instantiated at the relevant anchor pairs; the lemma's statement admits possibly-distinct subspace components, so the fourth clause `dom(A_C(d)) ∩ dom(A_L(d')) = ∅` (anchors `[d.0.s_C]` and `[d'.0.s_L]`) is dispatched by the same lemma at `(s₁, s₂) = (s_C, s_L)`.
...
- *Frontier advance* (subsequent emission, predicate non-empty): `a = inc(max{a' ∈ dom(x-store) : origin(a') = d}, 0)` (TA5(c)) is the next sibling on `A_x(d)`'s inc chain; freshness against the same-subspace store is GlobalUniqueness (ASN-0034) on `A_x(d)`'s tracked chain (SubAllocatorAxiom.T10aConformance).
...
The content-subspace qualification is essential: J1'★ scopes provenance recording to content-subspace range changes (link-subspace mappings target `dom(L)`, which is disjoint from `dom(C)` by L14, so no link-subspace V-position can witness provenance under P7's `a ∈ dom(C)` requirement). P4a in the extended state therefore reads as "every provenance entry corresponds to a past content-subspace arrangement," consistent with both P7's grounding in `dom(C)` and J1'★'s content-scoped coupling. Its derivation by induction with J1'★ as the coupling is given under Class (b) in the *Extended reachable-state invariants* section.

**J2 (Contraction isolation).** The elementary transition K.μ⁻ requires no coupling — it is self-sufficient with respect to P0–P2, L12, and the operative provenance bound P4★ (`Contains_C(Σ) ⊆ R`, *Content-scoped containment and provenance*). As an elementary transition, K.μ⁻ satisfies:

`C' = C ∧ L' = L ∧ E' = E ∧ R' = R`

The wp analysis confirms this. For P0: K.μ⁻ does not touch C. For P1: does not touch E. For P2: does not touch R. For L12: does not touch L. For the operative provenance bound P4★ (`Contains_C(Σ) ⊆ R`): contraction can only remove pairs from Contains_C, so Contains_C(Σ') ⊆ Contains_C(Σ) ⊆ R = R'. (The unscoped `Contains(Σ) ⊆ R` does not hold once any link is arranged — for an arranged link ℓ, `(ℓ, d) ∈ Contains(Σ)` but `(ℓ, d) ∉ R` by P7 + L14 — so the bound must be stated against the content-scoped relation that P4★ supersedes P4 with.) No co-occurring transition is needed to maintain any system invariant.
...
An immediate consequence of J1★ and J2 is that the provenance relation diverges from current containment over time. The operative provenance bound is P4★ (`Contains_C(Σ) ⊆ R`, *Content-scoped containment and provenance*); over the link-free fragment (`dom(L) = ∅`) every V-position is content-subspace, `Contains(Σ) = Contains_C(Σ)`, and P4★ reads as the unscoped `Contains(Σ) ⊆ R`.
...
P4★ supersedes P4 for the extended state. In pre-extension states (no link-subspace mappings), `Contains_C(Σ) = Contains(Σ)`, so P4★ reduces to P4. P4★ is a Class (b) composite-boundary property: K.μ⁺ alone may transiently violate it (it adds to Contains_C but does not extend R), with restoration at the composite boundary governed by J1★. Its full discharge — per-transition preservation by K.α/K.δ/K.ρ (M in frame), K.μ⁻ (contraction shrinks Contains_C), K.μ~ (link-subspace fixity), K.μ⁺_L (link-subspace only, excluded from Contains_C), and the boundary restoration under J1★ — is given once under Class (b) in the *Extended reachable-state invariants* section.
...
**Step 3: K.δ case (ii) k = 2 — allocate document `1.2.0.1.0.1 = inc(1.2.0.1, 2)`.** TA5(d) gives `zeros = 2`, `parent = 1.2.0.1`. Preconditions analogous to Step 2; GlobalUniqueness (ASN-0034) at the document sub-allocator `A_doc(1.2.0.1)` discharges `d ∉ E₂`. Effect: `E₃ = E₂ ∪ {1.2.0.1.0.1}`, with `M₃(1.2.0.1.0.1) = ∅` and SubAllocatorAxiom activating the content and link sub-allocators (anchors `b_C(d) = [d.0.1]`, `b_L(d) = [d.0.2]`).
...
**P7a (Provenance coverage).** Every I-address in the content store has at least one provenance record, `(A a ∈ dom(C) :: (E d :: (a, d) ∈ R))`. Stated and proved under Class (b) in the *Extended reachable-state invariants* section.
...
*TrackedEmission carried separately.* The per-state invariant TrackedEmission (EntityEmissionTracking) is *not* listed in the Class (a) conjunction above and has no row in the Class (a) verification matrix; it is established by the self-contained induction given in its own definition box (*K.δ and entity allocation*, the *Preservation* paragraph under TrackedEmission): vacuous at Σ₀, preserved by K.δ via the §*K.δ case (ii) discharge* (each non-node entity enters E only by a T10a inc-step on a tracked sub-allocator, joining that allocator's tracked domain at the placing event), and held in frame by every non-K.δ transition. That induction reaches every reachable state, so TrackedEmission holds wherever FrontierEquivalence consumes it (FrontierEquivalence's existence clause cites TrackedEmission directly).
...
**Derived distinctness corollaries.** The following two distinctness properties are *not* per-state invariants of ExtendedReachableStateInvariants and are absent from the Class (a) conjunction and the verification matrix above. They are corollaries of allocator discipline (GlobalUniqueness (ASN-0034) / NodeUniqueAllocation) at the single transition that touches the relevant store, recorded here because the S4 paragraph defers to them.
...
| SubAllocatorAxiom | Axiom (ContentLinkSubAllocatorExistence): for each d ∈ E_doc, the entity-allocation event placing d into E_doc simultaneously establishes two disjoint sub-allocators under d — a content sub-allocator with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator with anchor `b_L(d) = [d.0.s_L]` | ASN-0093 (ContentLinkSubAllocatorExistence) |
