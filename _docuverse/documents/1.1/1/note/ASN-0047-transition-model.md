...
By **K.μ~-RANGE** (range-invariance), Contains(Σ') = Contains(Σ). The "no coupling" claim holds at the *elementary* granularity the Class (a) induction uses, not merely at the composite boundary: K.μ~ decomposes into K.μ⁻ + K.μ⁺, and at the intermediate state (post-K.μ⁻, pre-K.μ⁺) the content-subspace range has contracted, but the subsequent K.μ⁺ step re-adds only V-positions whose content I-addresses already lay in `ran(M(d)|_{s_C})` — K.μ~-RANGE establishes `ran(M'(d)) = ran(M(d))` across the pair, so the K.μ⁺ step introduces no range-new content I-address. Hence `ran(M'(d)|_{s_C}) \ ran(M(d)|_{s_C})` is empty at that step, J1★ is vacuous there, and no co-occurring K.ρ is required. All invariants are trivially maintained.
