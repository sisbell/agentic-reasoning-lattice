...
**NodeUniqueAllocation (Axiom, FreshNodeAddress).** Every K.δ node-allocation event — every elementary transition of K.δ whose effect places an entity `e` with `IsNode(e)` into E — produces an address satisfying two conditions: (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation; (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order. These are the conditions the node-allocation registry must satisfy at every K.δ node-allocation event; details of the registry mechanism (issuing protocol, persistence model, concurrency discipline) lie outside this ASN's discharge layer.
...
**K.α amendment (shorthand, no local addition).** ASN-0093's K.α already encodes the content-subspace restriction `E(a)₁ = s_C` (equivalently `subspace_I(a) = s_C`) in its precondition, and its frame already includes `L' = L`. No locally-introduced amendment of K.α is required in the extended state. For brevity, the phrase **"K.α amendment"** appearing throughout this ASN (in the verification matrix and discharge prose below, in the *Local extensions and strengthenings of foundation properties* table at the head of *Properties Introduced*, and in cross-references such as "K.α's amendment fixes `subspace_I(a) = s_C`") is a documented shorthand for ASN-0093's inherited content-subspace precondition `E(a)₁ = s_C` (equivalently `subspace_I(a) = s_C`). The shorthand names the inherited foundation precondition for citation convenience — not a local addition. Every occurrence below should be read uniformly under this convention.
...
| S7b | K.α amendment + content sub-allocator chain ⟹ zeros(a)=3 | frame | frame (link addresses not in dom(C)) | frame | frame | frame | frame | frame |
...
| S8★ | frame | frame | frame | per-subspace projection via ASN-0036's S8 on M(d')\|_{V_{s_C}(d')} | per-subspace projection via ASN-0036's S8 on M(d')\|_{V_{s_L}(d')} | restriction of decomposition | inherits via decomposition | frame |
...
| L0 (C-clause) | K.α amendment: subspace_I(a)=s_C; preserved by P0 | frame | frame | frame | frame | frame | frame | frame |
...
| L14 | K.α amendment ⟹ subspace_I(a)=s_C ≠ s_L; preserves disjointness | frame | precondition ℓ∉dom(C); subspace_I(ℓ)=s_L preserves disjointness | frame | frame | frame | frame | frame |
...
The matrix is a navigational index; each cell summarises the load-bearing argument. The detailed per-invariant prose below substantiates each matrix entry, in particular for invariants whose discharge mechanism is non-trivial.

*Frame entries against link-store invariants (L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, CL-UNIQ) for the transitions K.α, K.μ⁺, K.μ⁻ refer to the amended forms* introduced in the *Amendments to existing transitions* section above (the K.α amendment and K.μ⁺ amendment in the *Amendments to existing transitions* section, and the K.μ⁻ amendment in the same section). These amendments extend each original four-component-state frame with the explicit `L' = L` conjunct; the L-related rows discharge to "frame" on the strength of that amendment clause, not the original transition's frame (which predated the link store and made no commitment about L). The matrix labels these as "frame" without footnote markers; readers cross-referencing against the original (pre-amendment) definitions should consult the amended forms instead.
...
*S7b (Element-level I-addresses).* `zeros(a) = 3`: K.α's amendment fixes `subspace_I(a) = s_C` and the inc chain under a document-level prefix gives `zeros(a) = 3`; preserved by P0.
...
*S8★ (Per-subspace span decomposition).* Established per-subspace by direct application of S8★(s_C) and S8★(s_L), each defined in the *Amendments to existing transitions* section. *Content subspace:* S8★(s_C) follows from ASN-0036's S8 applied to the projection `M(d')|_{V_{s_C}(d')} : V_{s_C}(d') → dom(C')` (S3★'s content clause is exactly S3 restricted to V_{s_C}(d'); S2/S7b/S7c/S8a/S8-depth/S8-fin are elementary-preserved as above). *Link subspace:* S8★(s_L) is established by direct application of ASN-0036's S8 to `M(d')|_{V_{s_L}(d')}`, using the same half-open range `0 ≤ k < n_j` convention. The decomposition is depth-2-specific by LinkVPositionDepthAxiom (`m_L = 2`): every `v ∈ V_{s_L}(d')` has the form `[s_L, k]`, and a sequence `{[s_L, k], [s_L, k+1], …, [s_L, k+n-1]}` mapping shift-aligned to `{M(d')([s_L, k+i]) : 0 ≤ i < n}` constitutes an n-element correspondence run. ASN-0036's S8 then delivers a finite decomposition (S8-fin gives finiteness of dom(M(d'))). For K.μ⁺_L, the trivial length-1 decomposition over the extended `V_{s_L}(d')` suffices; the content subspace is frame-preserved. Each arrangement-extending or contracting transition discharges S8★ at its post-state by these projections; others hold M in frame.
...
*L0 (SubspacePartition).* L-clause from K.λ's precondition `subspace_I(ℓ) = s_L`; preserved by L12. C-clause from K.α's amendment `subspace_I(a) = s_C`; preserved by P0.
...
| NodeUniqueAllocation | Axiom: every K.δ node-allocation event produces (a) e ∉ E (freshness, closing the GlobalUniqueness chain for nodes where T10a does not apply) and (b) n₀ ≼ e (bootstrap lineage, supplying K.δ case (i)'s lineage precondition and inductively preserving NodeLineage). The two conditions the node-allocation registry must satisfy at every node-allocation event |
...
| K.α amendment | Documented shorthand (not a local strengthening) for ASN-0093's K.α content-subspace precondition `E(a)₁ = s_C` (equivalently `subspace_I(a) = s_C`); preserves L0's C-clause and L14 in the extended state under that inherited precondition. Defined under the *K.α amendment (shorthand, no local addition)* paragraph at the head of *Amendments to existing transitions*; every downstream citation reads uniformly under this convention | Inherited from ASN-0093's K.α (ContentAllocation) precondition; this ASN names the shorthand for downstream citation convenience and supplies no local strengthening |
