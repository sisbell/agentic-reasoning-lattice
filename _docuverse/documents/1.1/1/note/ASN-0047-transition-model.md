...
**Typing note (M total — overrides foundation).** ASN-0036 and ASN-0093 type the arrangement family as *partial*, `M(d) : T ⇀ T`, with `dom(M)` serving as the set of allocated documents. This ASN deliberately *overrides* that typing: here M is *total* on T, with `M(d) = ∅` (the empty partial function) for every `d ∉ E_doc`. The override is not a reinterpretation but a replacement — the document-set role that `dom(M)` played in the foundations is now carried by E_doc, under the identity `d ∈ dom(M) ⟺ d ∈ E_doc`. This single identity translates every inherited foundation precondition and effect: wherever ASN-0093 writes `d ∈ dom(M)` (in K.α, K.λ, K.σ), read `d ∈ E_doc`; ASN-0093's K.σ effect `dom(M') = dom(M) ∪ {d}` becomes the entry of `d` into E_doc. We state the translation once here; the inherited operations below use `d ∈ E_doc` directly, without restating it.
...
**FrontierEquivalence (Lemma).** For every reachable state `Σ` and every operand `t ∈ Σ.E` with `¬IsNode(t)`, let `A` denote the allocator whose tracked chain contains `t` — `t` is one of `A`'s emissions, and `A` is unique by T10a.6 (DomainDisjointness, ASN-0034). Then:
...
*Effect on M.* K.δ leaves M unchanged: `M' = M`, uniformly across all three cases. Since `e ∉ E` (precondition), `e ∉ E_doc`, so `M(e) = ∅` by the totality convention already *before* the transition; the entity's entry into `E_doc` (when `IsDocument(e)`) is carried solely by `E' = E ∪ {e}`, with `M'(e) = M(e) = ∅` and no change to M as a function. Document registration is effected entirely through the entity set, not through M — which is precisely the totality convention's point: existence is tracked by E, not by `dom(M)`.
...
*Frame:* C' = C; L' = L; R' = R; M' = M (uniform across the IsNode, IsAccount, and IsDocument cases). The sole effect of K.δ is `E' = E ∪ {e}`; a document's entry into `E_doc` is registration through the entity set alone, with M untouched.
