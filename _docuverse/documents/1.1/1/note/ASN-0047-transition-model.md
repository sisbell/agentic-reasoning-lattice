...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
  - *k = 0 (sibling):* `t ∈ E ∧ ¬IsNode(t) ∧ parent(t) = parent(e) ∧ zeros(t) = zeros(e) ∧ t = max{t' ∈ E : parent(t') = parent(e) ∧ zeros(t') = zeros(t) ∧ #t' = #t}`. The `¬IsNode(t)` conjunct is implied by the structural identity `zeros(t) = zeros(e)` together with the case-level `¬IsNode(e)` (which gives `zeros(e) ≥ 1`), and is required for `parent(t)` to be defined (T4b's parent projection is partial on T and undefined when `IsNode(t)`); it is stated explicitly so the operand admissibility is visible at the definition site rather than left as a consequence of partial-function evaluation. The `#t' = #t` conjunct of the maximality candidate set is what isolates t's own allocator sub-stream from neighbouring sub-streams whose siblings happen to share `parent(·)` and `zeros(·)` under T4b. T4b's parent projection groups entities by entity-hierarchy spine, not by allocator-tree provenance: under a single account `A`, the document sub-allocator `A_doc(A)` emits sibling documents at length `#A + 2` (k=2 from A, then k=0 siblings), while each existing document `d` under A spawns a version sub-allocator `A_v(d)` emitting versions at length `#d + 1 = #A + 3` (k=1 from d, then k=0 siblings). All these emissions are documents (`zeros = 2`) with `parent(·) = A` under T4b, so they coexist in the T4b-children-of-A set, but they belong to distinct T10a allocator sub-streams whose siblings cannot interleave. By T10a.1 (UniformSiblingLength, ASN-0034) every T1 sibling-increment on a single allocator preserves length, so length partitions T4b-children-of-`parent(e)` into per-allocator strata; the `#t' = #t` conjunct restricts the maximality candidates to t's own stratum. Within that stratum the maximality conjunct identifies `t` as the *frontier* — the largest sibling already in `E` under the unique allocator emitting siblings of length `#t` with shared T4b-parent. Frontier-ness is what makes `inc(t, 0)` fresh against `E`: a non-frontier `t` would have some `t'' ∈ E` in the same stratum with `t'' > t`, so `t'' = inc(t, 0)` would already be in `E` and the K.δ event would violate freshness. By contrast, when `t` is the frontier, T10a's per-`(t, 0)` uniqueness guarantees `inc(t, 0) ∉ E` because no prior K.δ event has fired at this `(t, 0)` pair, and T10a's GlobalUniqueness on the sibling-increment chain closes the freshness obligation. Without the `#t' = #t` conjunct, a sibling document `d₂` newly minted under account A would supplant t's claim to frontier-ness for `A_v(d₁)`'s sub-stream — once `d₂` enters E, the unrestricted maximum over T4b-children-of-A would point at `d₂`, blocking K.δ k = 0 from extending d₁'s versions even though `A_v(d₁)`'s frontier remains uncontested.
...
*Per-subspace consequence of the effect clause (not a precondition).* `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` — at least one subspace shrinks strictly. This is a consequence of the whole-arrangement effect clause `dom(M'(d)) ⊂ dom(M(d))` combined with the per-subspace suffix pattern: the suffix patterns supply `V_S(d') ⊆ V_S(d)` for each S, and the effect clause's proper-subset requirement on `dom(M'(d)) = V_{s_C}(d') ∪ V_{s_L}(d')` against `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)` rules out simultaneous equality on both subspaces, forcing at least one per-subspace inclusion to be strict. The operation discharges this consequence by construction whenever its effect clause is satisfied; it is recorded here only to locate, at the per-subspace level, which subspace(s) carry the contraction.
