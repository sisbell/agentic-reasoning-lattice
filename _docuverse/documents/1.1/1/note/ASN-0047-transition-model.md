...
| FrontierEquivalence | Derived lemma: `inc(t, 0) ∉ Σ.E ⟺ t is the frontier of its sub-allocator's (t, 0)-branch`, for every reachable Σ and every `t ∈ Σ.E` with `¬IsNode(t)`. Three load-bearing premises: T10a per-`(t, 0)` uniqueness, P1 E-monotonicity, T10a GlobalUniqueness (via T10a.6). Counterexample to T4b-based identification: T4b's `parent`/`zeros`/length stratification does not in general identify t as the frontier of its own sub-allocator. Cited by the K.δ case (ii) k = 0 rationale and freshness discharge, and by the S4 row of the Class (a) verification matrix |
