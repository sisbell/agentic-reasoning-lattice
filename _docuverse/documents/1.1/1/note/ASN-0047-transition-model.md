...
Existing transitions preserve S3★: K.α, K.δ, K.ρ hold M in frame; K.μ⁺ creates only content-subspace V-positions (by its amended precondition `subspace(v) = s_C`), so new mappings target dom(C) and the link-subspace clause is unaffected; K.μ⁻ contracts dom(M(d)), preserving both clauses. K.μ~'s preservation of S3★ — and the stronger property that its link-subspace mappings are pointwise fixed — is established in *Decomposition of K.μ~* below.
...
S3★(Σ') for a K.μ~ event holds by the admissibility filter — clause (i) admits only those π whose induced post-state satisfies S3★(Σ'), so the verification-matrix cell is true by construction; Step (B) below shows the K.μ⁻ + K.μ⁺ decomposition realises an admissible π, making K.μ~ non-vacuous.
