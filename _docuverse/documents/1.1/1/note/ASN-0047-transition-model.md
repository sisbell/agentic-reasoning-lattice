...
is then exactly the `dom(L) = ∅` specialisation of P4★ (`Contains_C(Σ) ⊆ R`), proved in *Content-scoped containment and provenance* below. We do not prove P4 separately. Once link-subspace mappings exist, `Contains(Σ)` acquires pairs `(ℓ, d)` with `ℓ ∈ dom(L)` that no provenance entry can record (P7 grounds R in `dom(C)`, and `dom(L) ∩ dom(C) = ∅` by L14), so P4 is unsatisfiable for the unscoped relation and is superseded by P4★.
