...
**L3 (TripleEndsetStructure, amendment of ASN-0043's L3).**

  `(A a ∈ dom(Σ.L) :: Σ.L(a) = (F, G, Θ) where F, G, Θ ∈ Endset)`

Every link in the link store has exactly three endsets. This amends ASN-0043's L3 (NEndsetStructure: `|Σ.L(a)| ≥ 3`) in two respects. First, arity is fixed at three rather than admitting N > 3: the udanax-green realisation places exactly three endsets per link — from, to, and type — at three fixed positions, and Nelson's design uses the same three uniformly ("a package of connecting or marking information" with from/to/type — LM 4/12). Second, ASN-0043's non-empty type-endset clause (`Σ.L(a).e₃ ≠ ∅`) is dropped: the udanax-green implementation silently accepts empty Θ, and the Endset definition above (`𝒫_fin(Span)`) admits `∅` as a well-formed endset. The N ≥ 3 generality of ASN-0043 is preserved in principle for future extensions; this ASN's transition model is closed under the realised arity. Consequently, K.λ's precondition `(F, G, Θ) ∈ Link` permits empty Θ.
...
*Precondition:* `IsElement(a)` (S7b, ASN-0036) ∧ `origin(a) ∈ E_doc` ∧ `a` is produced by a T10a-conforming allocation event (TA5, ASN-0034) under origin(a)'s content allocator. The address is allocated under the creating document's prefix (S7a), and the allocation mechanism inc(·, k) (TA5, ASN-0034) operates within an ownership domain, requiring the document entity to exist before content can be allocated under its prefix. The inc-conformance precondition is explicit — parallel to K.λ's analogous wording for link addresses — because T10a's GlobalUniqueness guarantee, which underwrites `a ∉ dom(C)`, applies only to inc-produced addresses; without this precondition, the structural uniqueness chain would not close, and weaker phrasings ("a is allocated under origin(a)'s prefix") would admit non-conforming allocations that break the chain. By GlobalUniqueness (ASN-0034), a is distinct from every previously allocated address.

*Frame:* E' = E; (A d :: M'(d) = M(d)); R' = R.

**NodeUniqueAllocation (Axiom, FreshNodeAddress).** Every K.δ node-allocation event produces an address fresh to the entity set: for any IsNode(e) emitted by K.δ at state Σ, `e ∉ Σ.E`.

This is an axiom of this ASN. T10a's GlobalUniqueness (ASN-0034) governs inc-produced addresses within a single ownership domain; node creation, however, is *not* constrained to a single inc(·, k) operation under a single owner — Nelson's hierarchical baptism (LM 4/19–4/20), under which a parent node ceremonially issues a child node's identifier as a top-level sibling under its own root, and Gregory's global granfilade with query-and-increment dispatch (granf2.c:209) are two equally admissible protocol realisations, neither necessarily reducible to a single owner's inc operation. The abstract specification leaves the protocol mechanism unspecified — any allocator satisfying the namespace property `e ∉ E` suffices — and treats the resulting `e ∉ E` as the operative premise that closes the GlobalUniqueness chain for K.δ's node sub-case. Without NodeUniqueAllocation, the K.δ precondition `e ∉ E` for node allocations would have no underwriter — T10a alone cannot supply it, since the inc operator is defined to extend an existing tumbler within an ownership domain rather than to mint a top-level identifier across independent node-allocating actors — and the inductive entity-set invariants (P1, P8) would lose their base step at every K.δ node event. NodeUniqueAllocation stands alongside SC-NEQ (this ASN), NoDeallocation (ASN-0034), and S0 (ASN-0036) as a load-bearing axiomatic premise.

**K.δ (Entity creation).** A fresh entity address enters E with initial state:

`E' = E ∪ {e}` where `e ∉ E ∧ ValidAddress(e) ∧ ¬IsElement(e)`

*Precondition:* when ¬IsNode(e), parent(e) ∈ E — the parent entity must already exist. For nodes (IsNode(e)), no parent is required: node addresses are not restricted to single-component tumblers, and the consultation evidence confirms multi-component node forms in both the design (Nelson, LM 4/19–4/20, where 1.1, 1.2, 1.3 are nodes "forked" from a root node 1) and the implementation (Gregory, granf2.c:209, which allocates NODE→NODE with depth = 1, producing forms such as 1.1.0.1.1). The only structural constraints are ValidAddress(e) ∧ IsNode(e) ∧ e ∉ E, with zeros(e) = 0 (per the IsNode predicate of ASN-0045) — the tumbler may have any number of strictly-positive components. The Xanadu design admits node creation beyond the bootstrap n₀; Nelson's hierarchical baptism (a parent node ceremonially issues a child node's identifier as a top-level sibling under its own root) and Gregory's single global granfilade with query-and-increment dispatch are two realisations of the same uniqueness guarantee. Node uniqueness is therefore a *protocol-established* property — every new node descends from the single bootstrap root by a chain of baptism events under that root's ownership, and ownership-derived uniqueness yields the namespace property `e ∉ E` for each fresh node — rather than a T10a consequence: the inc(·, k) operator is defined to extend an existing tumbler within an ownership domain rather than to mint a top-level identifier, so it does not by itself underwrite uniqueness across independent node-allocating actors. The abstract specification leaves the protocol mechanism unspecified — any allocator satisfying the namespace property `e ∉ E` suffices — and treats the resulting `e ∉ E` as the operative axiom NodeUniqueAllocation (introduced above), closing the GlobalUniqueness chain for nodes through this protocol guarantee rather than through T10a. For non-root entities, the address is produced by a T10a-conforming allocation event:
...
In both cases the resulting e lies within parent(e)'s ownership domain (its tumbler-prefix subtree), and `zeros(e) ≤ 2` (since e ∈ E is non-element). The split makes explicit that "parent(t) = parent(e)" carries different content for k = 0 (t is itself in parent(e)'s child-level) versus k > 0 (t is at parent(e)'s level, descent then enters the child-level). By T10a's GlobalUniqueness (ASN-0034) — the same result that governs K.α — every inc-produced address is distinct from every previously allocated address, so e ∉ E. The structural requirement closes the GlobalUniqueness chain: only T10a-conforming allocations carry the uniqueness guarantee, and weaker phrasings ("typically allocated") would admit non-conforming allocations that break the chain.
...
*Note on k = 1 (version semantics).* The k = 1 sub-case admits a structurally distinguished use: when t is a document at form `[N, 0, U, 0, D]` (zeros = 2), `e = inc(t, 1) = [N, 0, U, 0, D, 1]` is again zeros = 2 — still document-level (per IsDocument, ASN-0045) — but with one additional terminal component. The consultation evidence identifies this form as a *document version*: Nelson describes versions as new documents whose addresses indicate ancestry (LM 4/29, conflating version and sub-document), and Gregory's `docreatenewversion` (do1.c:271) realises it as `makehint(DOCUMENT, DOCUMENT, 0, ...)` with depth = 1 — a k = 1 tumbler-depth extension from an existing document address. In purely structural terms, the present ASN admits this allocation: K.δ's k = 1 sub-case (TA5(d) appending the terminal `.1`, TA5(b) preserving the prefix) applies, with t = the originating document. *Semantically*, however, the relationship between a version `[N, 0, U, 0, D, k]` and its base document `[N, 0, U, 0, D]` is richer than the entity-hierarchy parent(·) operation captures — parent(·) is defined to truncate the last field together with its preceding zero separator, and a version's terminal field has no preceding zero. The version-to-base relationship therefore lies outside the entity-hierarchy spine that P8 governs, and a full account of version lineage — what arrangement-transition invariants must hold between successive versions, whether content allocators of base and version are linked, how provenance flows between them — is deferred to the open question "What relationship must hold between a document's version lineage and its sequence of arrangement transitions?" below. The present ASN treats `[N, 0, U, 0, D, k]` for k ≥ 1 as an admissible K.δ output and leaves its version-specific contract to a subsequent version-management ASN.
...
*Precondition:*
- `d ∈ E_doc`.
- *Admissible removal (per-subspace suffix or full-subspace clearance).* For each non-empty subspace S in M(d), the input arrangement has a contiguous, minimum-anchored, uniform-depth range — a structural property established self-containedly in this precondition by the conjunction of D-CTG, D-MIN, S8-depth, S8-fin, and S8a (all properties at the pre-state):

  *(restated self-containedly:)* for each non-empty subspace S, there exists `n_S ≥ 1` and a uniform inner depth m_S ≥ 2 such that `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` with each tuple of depth m_S.

  This structural form is named D-SEQ★ in the *Amendments to existing transitions* section below, where its derivation from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a is given in full (Step 1: an infinite-cardinality contradiction against S8-fin forces every inner position to 1; Step 2: D-CTG★ applied to terminal-varying tuples in lex order yields the {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S} form). The forward reference to the *name* D-SEQ★ is intentional — K.μ⁻'s admissibility contract must be stated at the operator's definition site, before its postconditions can be discharged — but the structural form itself is self-contained at this point: it appeals only to the five base properties listed above (all per-state invariants of ASN-0036 + this ASN's D-CTG/D-MIN amendments), without any forward dependency on a result not derivable from them. Reading order: the precondition here uses the structural form as a self-contained statement; the Amendments section formalises and derives that same form (under the name D-SEQ★) for the extended state; and the K.μ⁻ amendment paragraph below confirms that the admissibility contract carries over unchanged under D-SEQ★.

  Given the input structure, the removed positions, partitioned by subspace, form a suffix of each subspace's range:
...
(b) *Interior removal (forbidden by D-CTG★).* Suppose K.μ⁻ were to remove some interior position `[S, 1, ..., 1, k₀]` with `1 ≤ k₀ < n_S` while retaining at least one position `[S, 1, ..., 1, k']` with `k' > k₀`. Then `V_S(d')` contains `[S, 1, ..., 1, k']` but not `[S, 1, ..., 1, k₀]`, while `k₀ < k'`. This is a gap in the V-ordering on subspace S at depth m_S: under the lex order on terminal-varying tuples, `[S, 1, ..., 1, k₀]` lies strictly between `min(V_S(d'))` (some `[S, 1, ..., 1, k_min]` with k_min ≤ k₀) and `[S, 1, ..., 1, k']`, yet is absent from V_S(d'). This violates D-CTG★'s requirement that V_S(d') be contiguous under the V-ordering. (The "interior" case where `1 < k₀ < n_S` and a position with `k < k₀` is also retained is the special form; the more general statement is that any removal leaving any position above the removed position constitutes an interior removal in this sense.)
...
K.μ~ is a distinguished composite, not a primitive transition. When π ≠ id (which entails dom_C(M(d)) ≠ ∅, as shown below), it decomposes into K.μ⁻ (removing content-subspace mappings) followed by K.μ⁺ (re-adding content-subspace mappings at new positions). When π = id — including the case dom(M(d)) = ∅ (empty bijection) and the case dom(M(d)) ≠ ∅ ∧ dom_C(M(d)) = ∅ (link-subspace-only arrangements; π = id follows directly from the precondition as shown immediately below) — K.μ~ expands into zero elementary steps producing M'(d) = M(d).

*Direct argument for π = id when dom_C(M(d)) = ∅.* The K.μ~ precondition stipulates `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` — the identity clause on link-subspace V-positions. By S3★-aux (subspace exhaustiveness, established for every reachable state), every v ∈ dom(M(d)) has `subspace(v) ∈ {s_C, s_L}`. When dom_C(M(d)) = ∅, no v has subspace s_C, so every v ∈ dom(M(d)) has subspace s_L; the link-subspace identity clause then applies to every v, forcing `π(v) = v` throughout — i.e., π = id on the whole domain. Hence π = id whenever dom_C(M(d)) = ∅, by the precondition alone, with no appeal to the K.μ⁻ + K.μ⁺ decomposition. (S3★-aux is established as an elementary-class invariant in ExtendedReachableStateInvariants below; its inductive proof — base M₀ = ∅ vacuous, step preserves subspace ∈ {s_C, s_L} under every transition — does not depend on the present argument, so this reasoning is non-circular.)

*Consistency: the K.μ⁻ + K.μ⁺ decomposition produces zero steps when dom_C(M(d)) = ∅.* The cardinality argument below verifies that any attempt to decompose K.μ~ into nonzero elementary steps in this case fails — confirming that the zero-step expansion is the unique admissible decomposition and is consistent with the direct precondition conclusion above. This argument assumes S3★ at the pre-state (established inductively in ExtendedReachableStateInvariants below) and the K.μ⁺ content-subspace amendment (§Amendments below). Suppose for contradiction that K.μ~ decomposes into nonzero steps. K.μ⁻ removes r ≥ 1 positions; since dom_C(M(d)) = ∅, all positions are link-subspace, so K.μ⁻ removes r link-subspace positions. K.μ⁺ (amended) then adds positions to restore the bijection's cardinality. Since K.μ⁺ is restricted to content-subspace V-positions (by its amendment), it adds r content-subspace positions. For each such new content-subspace position π(v), the K.μ~ definition gives M'(d)(π(v)) = M(d)(v), and S3★ at the *pre*-state gives M(d)(v) ∈ dom(L) (since v is link-subspace). But K.μ⁺'s referential integrity precondition requires M'(d)(π(v)) ∈ dom(C), and dom(C) ∩ dom(L) = ∅ (L14) — contradiction. Therefore r = 0; K.μ⁻ has no positions to remove, and K.μ⁺ has nothing to re-add. Zero elementary steps; M'(d) = M(d), consistent with the precondition-derived π = id.

In the extended state when dom_C(M(d)) ≠ ∅, link-subspace mappings are preserved through the K.μ⁻ + K.μ⁺ decomposition: the K.μ⁺ amendment (below) restricts K.μ⁺ to content-subspace V-positions, so link-subspace mappings removed by K.μ⁻ could not be restored — forcing K.μ⁻ to retain them. The full argument establishing link-subspace fixity under K.μ~ is developed in the S3★ analysis and ExtendedReachableStateInvariants theorem below. We verify the intermediate-state preconditions and derive the frame for the non-trivial case (dom_C(M(d)) ≠ ∅).
...
**K.μ⁻ amendment (PerSubspaceContiguity).** K.μ⁻'s D-CTG and D-MIN postconditions extend naturally to the two-subspace case under the per-subspace amendment above: contraction must satisfy D-CTG★ and D-MIN★ for each subspace independently. The structural consequence is unchanged from the pre-extension analysis — by D-SEQ★ at the input state (per-subspace, derived below), each non-empty subspace has V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n}, constraining valid contractions to removal from the maximum end or removal of all positions within each subspace.
...
The content- and link-subspace allocators are organized as sibling element-field sub-allocators rooted at each document. We formalize this structure to underwrite the K.λ first-link case's chained inc derivation and to make T10a-conformance precise for the multi-subspace state.
...
These anchors are *virtual allocator predecessors*: they are not themselves in `dom(C) ∪ dom(L)` (content addresses have `#E ≥ 2` by S7c; link addresses have `#E ≥ 2` by L1b), but they are the formal starting points enabling the first inc step under each subspace. Successive allocations advance each anchor's frontier:

- *Content sub-allocator:* the first content address under d is `[d.0.s_C.1] = inc(b_C(d), 1)` (TA5(b)+TA5(d) with `k = 1`); subsequent siblings are `[d.0.s_C.k+1] = inc([d.0.s_C.k], 0)` (TA5(c)).
- *Link sub-allocator:* the first link address under d is `[d.0.s_L.1] = inc(b_L(d), 1)`; subsequent siblings advance by `inc(ℓ_prev, 0)` over the link frontier.

The two sub-allocators share the prefix `[d.0]` and differ only in the first element-field component — they are siblings in the tumbler-algebra sense, related by `b_L(d) = inc(b_C(d), 0)`-style sibling chains within the element-field range under d. Their frontiers advance independently: content allocation does not perturb the link sub-allocator's state and vice versa, because each inc step operates locally on the receiving allocator's own frontier under its subspace prefix.

**T10a-conformance in the hierarchy.** T10a's GlobalUniqueness applies to each sub-allocator's frontier separately. For the content sub-allocator at d, T10a gives `a ∉ dom(C)` for `a = inc(t, k)` over the content frontier under d; for the link sub-allocator at d, T10a gives `ℓ ∉ dom(L)` for `ℓ = inc(t, k)` over the link frontier under d. Cross-document collisions are prevented by T10 (PartitionIndependence): allocators under distinct documents have disjoint prefixes. Cross-subspace collisions are prevented by L14 (StoreDisjointness), itself derived from L0 and SC-NEQ via T7 (SubspaceDisjointness, ASN-0034): every content address has `fields(a).E₁ = s_C`, every link address has `fields(ℓ).E₁ = s_L`, and `s_C ≠ s_L`, so no allocation in one subspace can produce an address inhabiting the other.

The chained inc applications underwriting L1c (from d to `b_L(d)` via `k₁ = 2`, then `k₂ = 0` sibling steps shifting the first element-field component from the content side to s_L) are the explicit construction of the link sub-allocator's anchor from the document-level address. Equivalently, `b_L(d)` is reachable from `b_C(d)` by sibling steps within the element-field range under d. This construction is part of T10a-conformance for the K.λ first-link case: the allocator chain starts at the virtual anchor `b_L(d)` and proceeds by TA5(b)+TA5(d), with GlobalUniqueness covering the chain's endpoint `[d.0.s_L.1]` regardless of whether `b_L(d)` is itself in `dom(L)`.
...
- ℓ is produced by a T10a-conforming allocation event (TA5, ASN-0034) on the link allocator's current frontier under d's link prefix, with the inc operator depending on the case:
  - *First link case.* If `V_{s_L}(d) = ∅` and `dom(L) ∩ {a : origin(a) = d} = ∅` (no link yet allocated under d), the link-prefix base is `t = b_L(d) = [d.0.s_L]` — the link sub-allocator anchor formalized in the Allocator hierarchy section above (a virtual allocator predecessor: not itself in `dom(L)`, but the formal starting point for the link allocator under d). The first link is then `ℓ = inc(b_L(d), 1) = [d.0.s_L.1]` — tumbler-depth extension into the element subspace by TA5(b)+TA5(d): TA5(d) with `k = 1` appends the terminal `.1` (no new zero separator, since the range of zero-positions `#t + 1, ..., #t` is empty), and TA5(b) supplies the agreement on the preserved prefix `[d.0.s_L]`. The resulting ℓ has `#E(ℓ) = 2` with `fields(ℓ).E₁ = s_L`. The k = 1 step is necessary: TA5(c)'s sibling step inc(t, 0) would shift t's terminal component, altering the subspace identifier rather than extending into a child element-field component.
...
The address ℓ is produced by the same T10a-conforming forward-allocation discipline as content addresses: within each document's link subspace, addresses are monotonically increasing, produced by inc(·, 0) on the allocator's current frontier (TA5(c), ASN-0034). The structural requirement closes the GlobalUniqueness chain: T10a guarantees `ℓ ∉ dom(L) ∪ dom(C)` only for inc-conforming allocations, so the precondition explicitly pins ℓ to the frontier rather than merely ordering it against existing addresses. By T7 (SubspaceDisjointness, ASN-0034) and SC-NEQ, the link subspace s_L is disjoint from the content subspace s_C, so ℓ cannot collide with any content address. By T10 (PartitionIndependence, ASN-0034), link addresses in different documents cannot collide either.
...
1. *Transition preconditions:* each step `Σᵢ → Σᵢ₊₁` satisfies the precondition of its transition kind, evaluated at the intermediate state `Σᵢ`. K.μ~ appearing in the sequence is shorthand for its decomposition (per its definition above): if π = id (including the degenerate cases `dom(M(d)) = ∅` and `dom_C(M(d)) = ∅`, in which link-subspace fixity forces π = id), K.μ~ expands into *zero* elementary steps, producing M'(d) = M(d); otherwise (π ≠ id, which requires `dom_C(M(d)) ≠ ∅`) it expands into two consecutive elementary steps (K.μ⁻ + K.μ⁺), each satisfying its own precondition at the respective intermediate state.
2. *Coupling constraints:* J0, J1★, and J1'★ hold for the composite — evaluated between the initial state Σ and the final state Σ'.
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the transitions K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~ (shorthand for its K.μ⁻ + K.μ⁺ decomposition), and K.ρ — satisfies:
  ...
  S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG ∧ D-MIN ∧ D-SEQ★ ∧ P0 ∧ P1 ∧ P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L12 ∧ L14 ∧ CL-OWN

This supersedes the earlier ReachableStateInvariants theorem by replacing S3 with S3★, P4 with P4★, P3 with P3★, P5 with P5★, adding S3★-aux (subspace exhaustiveness), CL-OWN (link-subspace ownership), L3 (triple endset structure), the per-subspace amendment D-SEQ★, and the foundation invariants previously inherited tacitly from ASN-0036 (S4, S7a–S7d, S9) and from ASN-0043 (L1b); covering the extended transition set including K.λ and K.μ⁺_L.
...
**Class (a): Elementary invariants** — preserved by each elementary transition individually. These are all invariants except P4★ and P7a: S0, S1, S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8, S9, D-CTG, D-MIN, D-SEQ★, P0, P1, P2, P3★, P5★, P6, P7, P8, L0, L1, L1a, L1b, L3, L12, L14, CL-OWN.
...
- *L1b (Link element-field depth)* — `#E(ℓ) ≥ 2`: K.λ's address construction via `inc(t, k)` chains (k₁ = 2 followed by k₂ = 0 sibling extensions in the first-link case, or k = 1 in subsequent cases) guarantees the link-prefix base `t` has `#E(t) ≥ 1` and the link allocation step (`inc(t, 1)` for the first link, `inc(prev, 0)` for siblings) produces `#E(ℓ) ≥ 2`. Preserved by L12 thereafter.
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG ∧ D-MIN ∧ D-SEQ★ ∧ P0–P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L12 ∧ L14 ∧ CL-OWN; supersedes ReachableStateInvariants | introduced |
