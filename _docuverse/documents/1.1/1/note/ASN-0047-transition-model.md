...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases, with the freshness conjunct taking the form `inc(t, 0) ∉ E` at k = 0 and `e ∉ E` at k ∈ {1, 2}. *Per-k freshness mechanism (stated once here).* The freshness conjunct is discharged by a k-dependent uniqueness fact: at k = 0 the frontier check `inc(t, 0) ∉ E` is a caller-checked guard, characterized by FrontierEquivalence (the *derived* form of T10a's chain-advancement uniqueness at `(t, 0)`) as the condition that `t` is the frontier of its chain; at k ∈ {1, 2} the conjunct `e ∉ E` is discharged by T10a's *direct* per-`(t, k')` uniqueness axiom, which fires each `(t, k')` child-spawn at most once. For all three sub-cases k ∈ {0, 1, 2}, the parent entity-level sub-allocator on which each step acts and the allocator-discipline properties the guard maintains are discharged in §*K.δ case (ii) discharge and parent-allocator activation*. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
...
*Modeling choice (provisional, in tension with the design intent).* We state plainly what this strengthening costs before adopting it. Nelson's design intent is **tombstoning**: link addresses are permanent serial positions assigned in arrival order, and withdrawal makes a link "not currently addressable" while retaining it for historical backtrack (LM 4/9), so withdrawing an interior link `0.2.5` leaves a gap at position 5 rather than renumbering `0.2.6 … 0.2.N`. D-CTG★/D-MIN★, by requiring `V_{s_L}(d)` to be dense and minimum-anchored, are *incompatible* with that intent: they admit interior gaps only by forbidding interior withdrawal, forcing K.μ⁻ to remove a link only by removing every link allocated after it (worked-example Step 5; Open Questions, tombstoning reconciliation). The strengthening is therefore not justified against Nelson's design — it contradicts it.

What the strengthening *can* lean on is the implementation's *arrangement* layer. In udanax-green the V-space POOM is kept gap-free: `DELETEVSPAN` physically removes the deleted crum and shifts every subsequent link V-address down (`edit.c` case 1 `disown`/`subtreefree`, case 2 `tumblersub`, then `recombine`), leaving no tombstone and no interior gap in the POOM. This ASN models exactly that arrangement layer in `M(d)`, so a gap-free link subspace is faithful to the POOM's deletion behaviour — but only partially: the implementation attains contiguity by compaction-with-renumbering of surviving V-positions, whereas this ASN's K.μ⁻ attains it by suffix removal alone and never renumbers survivors. Tombstoning itself is not lost in the model — it survives in `dom(L)` (I-space), where withdrawn link *addresses* persist permanently under L12 (LinkImmutability); D-CTG★/D-MIN★ constrain only the arrangement `M(d)`, never `dom(L)`.

We adopt D-CTG★/D-MIN★ on the link subspace as a provisional contract for the arrangement layer, and record that the reconciliation with Nelson's tombstoning — whether via a separate withdrawal mechanism, a renumbering-aware contraction, or an interior-gap-admitting arrangement invariant — is **unresolved at the point this invariant is introduced**, not merely a downstream open question (see the tombstoning-reconciliation entry under Open Questions).
