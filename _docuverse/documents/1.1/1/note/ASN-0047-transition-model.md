...
Every link in the link store has exactly three endsets. This is a local extension of ASN-0043's L3 (NEndsetStructure: `|Σ.L(a)| ≥ 3`) — a stronger property than the foundation's, applied within this ASN's extended state — narrowing the foundation's `N ≥ 3` to a fixed-three arity in two respects. First, arity is fixed at three rather than admitting N > 3: the udanax-green realisation places exactly three endsets per link — from, to, and type — at three fixed positions, and Nelson's design uses the same three uniformly ("a package of connecting or marking information" with from/to/type — LM 4/12). Second, ASN-0043's non-empty type-endset clause (`Σ.L(a).e₃ ≠ ∅`) is dropped — the abstract model admits empty Θ as a well-formed link value carrying the semantics of an *untyped link*: F and G remain populated (the connection's endpoints — the link still "does the work of connecting"), while `Θ = ∅` asserts the link carries no further classification beyond the from/to relation itself. Nelson's design treats type assignment as an optional adornment of the underlying connection (LM 4/12: links connect, with type as a classifier *when supplied*); the abstract model formalises this by admitting `Θ = ∅` as the un-typed degenerate case rather than forbidding it. The Endset definition above (`𝒫_fin(Span)`) admits `∅` as a well-formed endset, so `(F, G, ∅) ∈ Link` is well-formed by construction. The udanax-green implementation accepts empty Θ at the storage layer (no runtime guard forbids it); Gregory documents protocol-level asymmetries — for instance `followlink` with `whichend = 3` returns an error when Θ = ∅, treating the type slot as not-currently-traversable, and `endsetqueries` are asymmetric across the three slots — but these are presentational/discovery-layer behaviours, not constraints on the abstract link's well-formedness. The abstract model treats `Θ = ∅` as a valid link value with a defined meaning (an un-typed link), and defers the protocol's handling of type-endset queries to a discovery-layer ASN. The N ≥ 3 generality of ASN-0043 is preserved in principle for future extensions; this ASN's transition model is closed under the realised arity. Consequently, K.λ's precondition `(F, G, Θ) ∈ Link` permits empty Θ — a link with no type classification, distinct from the absence of a link.

*Downstream consequence for L8 (TypeByAddress).* Admitting `Θ = ∅` as a well-formed link value requires this ASN to fix the domain of `same_type`, the type-matching relation introduced by ASN-0043's L8 (TypeByAddress) clause. L8 states `(A ℓ₁, ℓ₂ ∈ dom(Σ.L) :: same_type(ℓ₁, ℓ₂) ⟺ Σ.L(ℓ₁).e₃ = Σ.L(ℓ₂).e₃)` — under the abstract Endset universe (`𝒫_fin(Span)`), naive instantiation makes every empty-Θ link `same_type`-related to every other empty-Θ link, since `∅ = ∅`. Two readings of this fact are coherent with the udanax-green realisation; this ASN selects the one that matches Nelson's design intent. *(Option a, selected.)* The domain of `same_type` is restricted to typed links: `dom(same_type) = {ℓ ∈ dom(Σ.L) : Σ.L(ℓ).e₃ ≠ ∅}`. Under this restriction, `same_type` is a partial relation defined only on links carrying a non-empty type endset, and empty-Θ links stand outside the type-matching apparatus entirely — they are neither type-matched to one another nor to typed links. This aligns with Nelson's LM 4/58 satisfaction rule (type matching is invoked only when a type is supplied) and with udanax-green's `LINKTHREESPAN` indexing (the type-side index is populated only for links whose e₃ is non-empty; empty-Θ links are simply absent from the type-discovery surface). *(Option b, rejected.)* Treating `Θ = ∅` as a sentinel type — making all empty-Θ links a single equivalence class under `same_type` — was rejected because it manufactures a synthetic "untyped" type class with no grounding in either Nelson's design (where untyped links are an *absence* of classification, not a type called "none") or the udanax-green storage layer (which omits empty-Θ links from type discovery rather than indexing them under a sentinel). The restricted-domain reading also keeps L8's biconditional faithful to its intent — `same_type(ℓ₁, ℓ₂)` reads "ℓ₁ and ℓ₂ share a type," which presupposes that each *has* a type. This consequence is a local sharpening of L8's domain induced by this ASN's L3, and is consistent with the ASN-0043 foundation: ASN-0043 does not state L8's domain explicitly because its L3 forbids empty Θ outright; under the present ASN's relaxed L3, the domain restriction must be made explicit to preserve L8's intended semantics. No transition in this ASN's transition system reads `same_type`, so this consequence imposes no proof obligation on K.λ, K.μ⁺_L, K.μ⁻, or any other elementary transition — the domain restriction is a typing constraint on a discovery-layer predicate, deferred to the layer that uses it.
...
*From function equality to π-identity (closure via CL-UNIQ).* Function equality on dom_L tells us that the link-subspace map of M'(d) coincides pointwise with that of M(d) — but the K.μ~ definition gives `M'(d)(π(v)) = M(d)(v)`, which says π(v) is *some* V-position in M'(d) mapping to `M(d)(v)`, not necessarily v itself. To compel `π(v) = v` we need the link-subspace map to be injective: if M'(d)(π(v)) = ℓ and M'(d)(v) = ℓ, then π(v) = v iff ℓ appears at a unique V-position in M'(d). This is exactly CL-UNIQ at the output state. Combining: for each `v ∈ dom_L(M(d))`, let `ℓ := M(d)(v)`. By the function equality just established, `M'(d)(v) = ℓ`. By K.μ~'s definition, `M'(d)(π(v)) = M(d)(v) = ℓ`. By S3★ at M'(d), `subspace(π(v)) = s_L` (eliminated as above), so `π(v) ∈ dom_L(M'(d))`. CL-UNIQ at M'(d) — invoked here as an inductively-established invariant of every reachable state, with the K.μ~ post-state being one such — gives: any two link-subspace V-positions in M'(d) mapping to the same link address are equal. Applied with v₁ = v, v₂ = π(v), both in dom_L(M'(d)), both mapping to ℓ: v = π(v), i.e., `π(v) = v`. The link-subspace identity clause therefore follows.

The redundancy argument now closes properly: S3★ + the K.μ⁺ amendment + the cardinality squeeze deliver function equality on dom_L, and CL-UNIQ at the output state lifts function equality to π-identity. The definition-site identity precondition is consistent with — and structurally compelled by — the rest of the model. (The argument's status as a *redundancy check* rather than a derivation is preserved: K.μ~'s precondition stipulates π = id on dom_L directly, and any K.μ~ instance failing the precondition would be inadmissible at its own definition site; the redundancy argument establishes that the precondition is not an additional restriction but a faithful restatement of what CL-UNIQ already compels at any state where K.μ~ is invoked.)
...
- *K.μ⁺_L.* Adds one mapping `(v_ℓ, ℓ)` with `subspace(v_ℓ) = s_L` and `v_ℓ ∉ dom(M(d))` (verified at K.μ⁺_L's definition site under the empty-link-subspace and non-empty cases respectively). Existing link-subspace mappings are preserved by frame. We must show the new mapping does not collide with any existing link-subspace mapping for `ℓ` itself: by SubAllocatorAxiom's link-namespace property and K.λ's `ℓ ∉ dom(L) ∪ dom(C)` precondition at the K.λ event that introduced ℓ into dom(L), no link-subspace V-position in M(d) before that K.λ could map to ℓ (S3★'s link clause requires the target to be in dom(L) at the state where the mapping holds, and ℓ entered dom(L) only at its K.λ event). Between the K.λ and the present K.μ⁺_L, ℓ may appear in at most one link-subspace V-position of M(d): the only transition that creates link-subspace V-positions mapping to ℓ is K.μ⁺_L itself (K.μ⁺'s amendment forbids link-subspace V-positions; K.μ⁻ removes mappings; K.μ~ preserves the multiset of mappings). The inductive hypothesis CL-UNIQ at the pre-state therefore gives at most one link-subspace V-position in M(d) mapping to ℓ; the present K.μ⁺_L adds at most one more, and that one targets `v_ℓ ∉ dom(M(d))`. Hence the post-state has at most one (in fact exactly one, since either the pre-state had zero and the post-state has one, or — if the pre-state had one and the present K.μ⁺_L attempts to add a duplicate — CL-UNIQ would have been violated at the present step, contradicting our inductive aim; this duplicate-add scenario is precluded by the K.μ⁺_L admissibility on `v_ℓ ∉ dom(M(d))` and is not what the present step performs). CL-UNIQ holds at the post-state.
...
Effect after both transitions: `L = {ℓ ↦ (F, G, Θ), ℓ₂ ↦ (F', G', Θ')}`, `M''(d) = {[1,1] ↦ a₂, [1,2] ↦ a₁, [2,1] ↦ ℓ, [2,2] ↦ ℓ₂}`. Link-subspace V-positions: `V_{s_L}(d) = {[2,1], [2,2]}` — contiguous (D-CTG★), minimum at `[2,1] = [s_L, 1]` (D-MIN★), depth 2 (S8-depth), structural form `{[s_L, 1, 1, k] : 1 ≤ k ≤ 2}` (D-SEQ★ with `n_{s_L} = 2`).
...
**ExtendedReachableStateInvariants (per-state).** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the transitions K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~ (shorthand for its K.μ⁻ + K.μ⁺ decomposition), and K.ρ — satisfies:
  ...
  S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ
...
**Class (a): Elementary per-state invariants** — preserved by each elementary transition individually. These are all per-state invariants except P4★ and P7a: S0, S1, S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8, S9, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, L0, L1, L1a, L1b, L3, L14, L-fin, CL-OWN, CL-UNIQ.
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P0–P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L12 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ; supersedes ReachableStateInvariants | introduced |
