...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | By the induction-along-the-witnessing-trace mechanism of the P4a definition box. | Transiently fails if K.ρ precedes the matching K.μ⁺ within the composite; the `R' \ R` entry is restored at Σ', which carries its witness. |
...
P4a (`(A (a, d) ∈ R :: (E Σ_k ∈ {Σ₀, ..., Σ_n} : (E v ∈ dom(M_k(d)) : subspace(v) = s_C ∧ M_k(d)(v) = a)))`, where `{Σ₀, ..., Σ_n}` is the transition history of the trace reaching Σ', as defined in the P4a definition box): discharged by the induction-along-the-witnessing-trace mechanism stated in its definition box, which establishes both the inductive witness and its robustness to the internal ordering of K.ρ and K.μ⁺ (all other transitions hold R in frame).
