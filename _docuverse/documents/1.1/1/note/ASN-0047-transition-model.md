...
**Link-subspace fixity under K.μ~.** We make two distinct claims about link-subspace fixity under K.μ~, kept logically separate to avoid conflating the definitional content of one with the inductive content of the other.

*Claim A (link-subspace fixity by stipulated precondition).* K.μ~'s definition includes the clause `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)`. Any well-formed K.μ~ instance therefore leaves link-subspace mappings pointwise unchanged by definitional fiat: this holds at the precondition gate without derivation and without appeal to surrounding invariants. Candidates failing the identity clause are inadmissible as K.μ~ at their own definition site. Claim A requires no further argument beyond reference to K.μ~'s definition.

*Claim B (link-subspace fixity as an inductive consistency check).* Independently of Claim A — and addressing the separate question of whether the precondition of Claim A is structurally necessary or freely imposed — under S3★ + the K.μ⁺ amendment + CL-UNIQ (LinkSubspacePositionUniqueness, established as a per-state invariant in the Link-subspace ownership section below; this is a forward reference, not a circular dependency, since CL-UNIQ's own inductive proof does not appeal to the present argument), any candidate transition satisfying only the *weaker* subspace-preservation clause would already be forced into link-subspace identity. Claim B is therefore an inductive consistency check on the rest of the model, not a derivation in lieu of Claim A: it confirms that the precondition of Claim A is not a restrictive addition but a faithful restatement of what surrounding invariants jointly compel at any reachable state where K.μ~ is invoked.

The remainder of this subsection discharges Claim B in two steps: function equality on dom_L (step 1, the cardinality squeeze), and closure from function equality to π-identity via CL-UNIQ (step 2).

*Discharge of Claim B, step 1 — function equality on dom_L.* Since K.μ⁺ (amended) requires `subspace(v) = s_C` for new V-positions, K.μ⁺ cannot create link-subspace V-positions. Let `dom_L(M(d)) = {v ∈ dom(M(d)) : subspace(v) = s_L}` denote the link-subspace V-positions. Assume only the weaker subspace-preservation clause on π (drop the identity clause for the sake of argument). With S3★ established for M'(d), π must map link-subspace positions to link-subspace positions: if `v ∈ dom_L(M(d))` then `M(d)(v) ∈ dom(L)`, and `M'(d)(π(v)) = M(d)(v) ∈ dom(L)`, so `subspace(π(v)) = s_L`: by S3★-aux, `subspace(π(v)) ∈ {s_C, s_L}`; the case `subspace(π(v)) = s_C` is eliminated because a content-subspace position mapping to dom(L) would violate S3★'s content clause, since `M'(d)(π(v)) ∈ dom(L)` and `dom(L) ∩ dom(C) = ∅` (L14, which depends on SC-NEQ). Thus `π` restricted to `dom_L(M(d))` is an injection into `dom_L(M'(d))`. Since K.μ⁺ cannot create link-subspace V-positions, `|dom_L(M'(d))| ≤ |dom_L(M(d))|`. If K.μ⁻ removed `r ≥ 1` link-subspace positions, then `|dom_L(M'(d))| ≤ |dom_L(M(d))| − r`, and the injection from `dom_L(M(d))` (size N) into `dom_L(M'(d))` (size at most N − r) cannot exist. Therefore `r = 0` — no link-subspace positions are removed. It follows that `M'(d)` restricted to `dom_L(M(d))` equals `M(d)` restricted to `dom_L(M(d))` *as a function from V-positions to link addresses*: i.e., for each `v ∈ dom_L(M(d))`, `v ∈ dom_L(M'(d))` and `M'(d)(v) = M(d)(v)`. Let `M_int(d)` denote the intermediate arrangement after K.μ⁻ but before K.μ⁺. K.μ⁻ removes none of the link-subspace positions (`r = 0`) and preserves the values of all surviving positions, so `M_int(d)|_{dom_L} = M(d)|_{dom_L}`. K.μ⁺ (amended) operates on `M_int(d)`: its frame preserves pre-existing mappings (`(A v : v ∈ dom(M_int(d)) : M'(d)(v) = M_int(d)(v))`), and its subspace restriction prevents creating new link-subspace positions. Chaining: `M'(d)|_{dom_L} = M_int(d)|_{dom_L} = M(d)|_{dom_L}` as functions.

*Discharge of Claim B, step 2 — from function equality to π-identity (closure via CL-UNIQ).* Function equality on dom_L tells us that the link-subspace map of M'(d) coincides pointwise with that of M(d) — but the K.μ~ definition gives `M'(d)(π(v)) = M(d)(v)`, which says π(v) is *some* V-position in M'(d) mapping to `M(d)(v)`, not necessarily v itself. To compel `π(v) = v` we need the link-subspace map to be injective: if M'(d)(π(v)) = ℓ and M'(d)(v) = ℓ, then π(v) = v iff ℓ appears at a unique V-position in M'(d). This is exactly CL-UNIQ at the output state. Combining: for each `v ∈ dom_L(M(d))`, let `ℓ := M(d)(v)`. By the function equality just established, `M'(d)(v) = ℓ`. By K.μ~'s definition, `M'(d)(π(v)) = M(d)(v) = ℓ`. By S3★ at M'(d), `subspace(π(v)) = s_L` (eliminated as above), so `π(v) ∈ dom_L(M'(d))`. CL-UNIQ at M'(d) — invoked here as an inductively-established invariant of every reachable state, with the K.μ~ post-state being one such — gives: any two link-subspace V-positions in M'(d) mapping to the same link address are equal. Applied with v₁ = v, v₂ = π(v), both in dom_L(M'(d)), both mapping to ℓ: v = π(v), i.e., `π(v) = v`. The link-subspace identity clause therefore follows.

*Claim B established.* Step 1 + step 2 jointly discharge Claim B: S3★ + the K.μ⁺ amendment + the cardinality squeeze deliver function equality on dom_L, and CL-UNIQ at the output state lifts function equality to π-identity. The link-subspace identity behavior of K.μ~ is therefore overdetermined — it holds both by Claim A (stipulated precondition at K.μ~'s definition site) and by Claim B (a consequence of S3★ + K.μ⁺ amendment + CL-UNIQ at every reachable state). The two claims do not interfere: Claim A excludes inadmissible candidates at the precondition gate, while Claim B confirms that the precondition excludes no instance that the surrounding invariants would have admitted. The arguments retain their distinct status — Claim A is definitional and requires no proof; Claim B is inductive and proven above — and neither subsumes the other.
...
**ExtendedReachableStateInvariants (per-state).** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the transitions K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~ (shorthand for its K.μ⁻ + K.μ⁺ decomposition), and K.ρ — satisfies:
  ...
  S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ

S0 (ContentImmutability) and S1 (StoreMonotonicity) of ASN-0036 are *per-transition* properties quantified over `Σ → Σ'`, not per-state properties; they appear instead in ExtendedTransitionInvariants below (subsumed by P0, which restates both clauses).
...
**ExtendedTransitionInvariants (per-transition).** Every valid composite transition `Σ → Σ'` between reachable states satisfies:
  ...
  S0 ∧ S1 ∧ P0 ∧ P1 ∧ P2 ∧ P3★ ∧ P5★ ∧ L12

Each conjunct is formally stated with the quantifier `(A Σ → Σ' :: ...)` — they are properties of the *step*, not of the endpoint Σ' alone. S0 (ContentImmutability) and S1 (StoreMonotonicity) are listed here because they are quantified over transitions in their original ASN-0036 statements; both are subsumed by P0 (which combines append-only domain growth with value preservation for existing content). The first theorem governs what is true *at* each reachable state; the second governs what is true *across* each reachable step.
...
**Class (a): Elementary per-state invariants** — preserved by each elementary transition individually. These are all per-state invariants except P4★ and P7a: S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8, S9, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, L0, L1, L1a, L1b, L3, L14, L-fin, CL-OWN, CL-UNIQ.
...
**Per-transition invariants** (ExtendedTransitionInvariants: S0, S1, P0, P1, P2, P3★, P5★, L12). These are properties of `Σ → Σ'`; we discharge each by elementary case analysis, observing that every valid composite is a finite sequence of elementary steps and each per-transition invariant is closed under composition (extension and value-preservation compose transitively).

- *S0 (ContentImmutability) and S1 (StoreMonotonicity)* are subsumed by P0. S0 asserts `(A a ∈ dom(C) :: C'(a) = C(a))` — the value-preservation clause of P0. S1 asserts `dom(C) ⊆ dom(C')` — the domain-monotonicity clause of P0. The derivation of P0 below discharges both simultaneously.
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ (per-state). S0, S1, P0, P1, P2, P3★, P5★, L12 are *per-transition*: see ExtendedTransitionInvariants. Together supersedes ReachableStateInvariants | introduced |
...
| ExtendedTransitionInvariants | Every valid composite transition Σ → Σ' between reachable states satisfies S0 ∧ S1 ∧ P0 ∧ P1 ∧ P2 ∧ P3★ ∧ P5★ ∧ L12 (per-transition). Conjuncts are properties of the step, not of the endpoint alone; S0 and S1 are subsumed by P0 | introduced |
