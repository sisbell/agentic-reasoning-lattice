...
(†) holds by the lockstep K.δ effect (its `Document`-case registration grows `dom(M)` and `E_doc` together by the same `{e}`) together with the default-value convention that, for `d ∉ E_doc`, fixes `M(d) = ∅`; the two sets therefore have identical membership at every reachable state.
...
The constructive precondition is *equivalent* to the post-state characterization "M'(d) satisfies D-CTG★/D-MIN★/D-SEQ★, `dom(M'(d)) ⊂ dom(M(d))` (strict, proper subset), with value preservation on survivors"; this equivalence is proved in *K.μ⁻ admissible contraction shape* below. The strict-subset conjunct in the characterization is essential: it is the post-state image of the constructive precondition's strict-contraction clause (`(E S :: n'_S < n_S)`), and without it the identity restriction `M'(d) = M(d)` would satisfy the invariant conjuncts yet fail K.μ⁻'s effect clause. The value-preservation clause `(A v : v ∈ dom(M'(d)) : M'(d)(v) = M(d)(v))` (effect clause) is satisfied automatically by the restriction definition `M'(d) = M(d) ↾ R`, ruling out modification of surviving values.
...
*Concrete trace (m = 3).* Take `S = s_C` and depth `m = 3`, so V-positions have the form `[s_C, v_2, v_3]` and the sole inner position is `j = 2`. Suppose a candidate arrangement places the interior value `v = [s_C, 2, 1]` alongside `v_min = [s_C, 1, 1]`. Step 1's construction instantiates `u_M = [s_C, 1, M]` for each `M ≥ 2` (S at position 1, the inner `1` at position `j = 2`, and `M` at the terminal position `j + 1 = m = 3`). Each `u_M` brackets strictly: `v_min < u_M` (agree at positions 1–2, first differ at position 3 where `v_min[3] = 1 < M`) and `u_M < v` (agree at position 1, first differ at position 2 where `u_M[2] = 1 < 2 = v_2`). D-CTG★'s closed-interval membership then forces every `u_M ∈ V_{s_C}(d)`, exhibiting the infinite family `{[s_C, 1, M] : M ≥ 2}` and contradicting S8-fin. So `[s_C, 2, 1]` cannot survive; the inner position is pinned to 1, and `V_{s_C}(d)` collapses to `{[s_C, 1, k] : 1 ≤ k ≤ n}` — Step 1 verified against this specific depth-3 arrangement.
...
**Lemma (Cross-document entity distinctness — account-level partition).** Distinct K.δ document-allocation events under distinct parent accounts produce distinct document addresses. We abbreviate this lemma **CrossDocEntityDisjoint**; it is the entity-level analogue of CrossDocDisjoint, and it subsumes the node-nesting sub-argument **CrossNodeAccountBase** stated within.
...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. Under case (ii)'s k-labels we give the *parent-allocator activation* (which allocator each step acts on) and, for the child-spawn regimes, the spawnPt premise, consuming the level→allocator-base recovery from ParentAllocatorDispatch (*Sub-allocator names*) that names the owning allocator from an address's zero-count stratum. The spawn-admissibility conjuncts — the `k' ∈ {1, 2}` membership and T10a's zero-count side condition — are stated once at the K.δ box and are not repeated here. By k:
...
**ValidComposite★ (skeleton, stated at first need).** Several statements in this section appeal to the validity of a composite transition before its full definition is reached. We fix the skeleton here, once, and refer to it locally thereafter. A composite transition `Σ →* Σ'` — a finite sequence of atomic transitions `Σ = Σ₀ → Σ₁ → ... → Σₙ = Σ'` drawn from the elementary set (K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.μ~, K.ρ) — is *valid* iff:
...
2. *Coupling constraints (initial-to-final).* The couplings J0, J1★, and J1'★ hold for the composite as a whole, evaluated only between the initial state Σ and the final state Σ'.

The two content-subspace provenance couplings J1★ and J1'★ named in clause (2) are stated and motivated by wp derivation in *Scoped coupling constraints* below, where the full ValidComposite★ paragraph also records the K.μ~-as-shorthand convention and the intermediate-state sequencing detail. Every reference to "ValidComposite★" or "valid composite" below this point uses this skeleton.

The K.ρ/K.μ⁺ coupling trigger is range-based, not unconditional: K.ρ must co-occur with K.μ⁺ exactly when K.μ⁺ adds an I-address `a` that is *new to the content-subspace range* of the document — `a ∈ ran(M'(d)|_{s_C}) \ ran(M(d)|_{s_C})` — and not when it merely reuses a V-position or re-adds an already-ranged address. This is the coupling J1★ named in clause (2) of the ValidComposite★ skeleton above; its full statement and wp derivation appear in *Scoped coupling constraints* below.
...
P4★ bounds provenance by the *content-subspace* restriction of containment (scoped to the content subspace so it coexists with P7). P4★ is a Class (b) composite-boundary property.

Validity of a composite transition `Σ →* Σ'` is ValidComposite★, whose two-clause skeleton is fixed in the preamble above (full statement in *Scoped coupling constraints* below). P4★ is discharged at composite boundaries by clause (2)'s couplings.
...
**ValidComposite★ (ValidComposite, amended).** This is the full statement of the two-clause skeleton fixed in the *Coupling and isolation* preamble above, now that J1★, J1'★, and the K.μ~ decomposition convention are in hand. A composite transition `Σ →* Σ'` in the extended state Σ = (C, L, E, M, R) is *valid* iff it is a finite sequence of atomic transitions `Σ = Σ₀ → Σ₁ → ... → Σₙ = Σ'` — drawn from K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~, and K.ρ — satisfying:
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ by a finite sequence of *elementary* transitions drawn from valid composites satisfies the *per-state invariants* below; every state at a composite boundary additionally satisfies the *composite-boundary properties* below (temporal scopes per the section preamble).
...
The *Composite-boundary verification matrix* below records, for each Class (b) property, where it transiently fails and the coupling that restores it (per the preamble's temporal-scope distinction). P4★ and P7a are per-state properties holding at boundaries; P4a is grouped with them as a trace property whose temporal scope is defined authoritatively in its definition box.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | K.μ~-S3★ (Decomposition, Step (B)) | frame |
...
*S8★ (Per-subspace span decomposition).* The two-route construction (content subspace via ASN-0036's S8; link subspace via the trivial length-1 decomposition) is given at S8★'s definition; the constituent invariants S2/S7b/C1b/S8a/S8-depth/S8-fin are elementary-preserved as the matrix shows. Only the per-transition preservation deltas remain. *K.μ⁺* extends the content-subspace projection `M'(d)|_{V_{s_C}(d')}`, re-establishing S8★(s_C) by reapplying ASN-0036's S8 to it. *K.μ⁺_L* extends the link-subspace projection, re-establishing S8★(s_L) by the trivial length-1 decomposition on the new survivor. *K.μ⁻* restricts both projections, re-applying each route to the contracted projection — the content route reapplies ASN-0036's S8 (preconditions S2, S3★, S8a, S8-depth, S8-fin preserved by restriction) rather than the length-1 fall-back, since the content subspace owes condition (c); the link route reuses the always-available length-1 decomposition on survivors. All other transitions hold M in frame.

*D-CTG★ / D-MIN★.* K.μ⁺'s original precondition list (stated at the K.μ⁺ definition in *Elementary transitions*) requires the resulting `M'(d)` to satisfy D-CTG and D-MIN, strengthened to D-CTG★ / D-MIN★ in the extended state — contiguity and minimum-position preservation on the extended content subspace is therefore discharged by precondition, not by the K.μ⁺ amendment (which adds only the `subspace(v) = s_C` restriction). K.μ⁺_L preconditions cover the D-CTG case (non-empty link subspace) and the D-MIN case (empty); K.μ⁻'s constructive precondition specifies the per-subspace retention count `n'_S` and produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` directly, satisfying D-CTG★ and D-MIN★ at Σ' by construction; all others hold M in frame.
