...
- *K' not contiguous over `[k_min, k_max]`.* Then some `k₀` satisfies `k_min < k₀ < k_max` with `k₀ ∉ K'`; since `k₀ ∈ (k_min, k_max) ⊆ [1, n_S] = K`, we have `k₀ ∈ K \ K'`. Witness (b) at `k_lo := k_min`, `k_hi := k_max`. This branch absorbs every non-contiguous K' with a present minimum (`1 ∈ K'`) as well as every non-contiguous K' with an absent minimum (`1 ∉ K'`): in either configuration the routing is via (b)'s interior hole at `k₀`, *not* via (c) — (c)'s contiguity clause excludes any K' carrying an interior hole, so a K' such as `{2, 4} ⊆ {1, 2, 3, 4}` (minimum absent, interior hole at 3) falls into (b) at `k₀ = 3`, not (c).

The three cases are mutually exclusive: (a) requires K' to be a downward-closed initial segment, so no interior hole and the minimum (when present) is at 1; (b) and (c) each exhibit a removed index that prevents that initial-segment shape, and they are themselves disjoint because (c) requires K' to be contiguous over `[k_min, k_max]` whereas (b) exhibits an interior hole `k₀ ∈ (k_min, k_max) ∩ (K \ K')` violating contiguity. Exhaustiveness over (a)/(b)/(c) is therefore the case analysis "K' = ∅ vs. K' contiguous vs. K' not contiguous", with the contiguous sub-case further split on `k_min = 1` vs. `k_min ≥ 2`. ∎
...
*Known gaps from the catalogue.* The sufficiency claim is bounded by the per-component enumeration above; one identified shortfall — link withdrawal at *interior* link-subspace positions, not expressible at the elementary level under D-CTG★/D-MIN★ — is treated in *Link-withdrawal gap under D-CTG★/D-MIN★* below.
...
*Trade-off acknowledgement.* D-CTG★/D-MIN★ trade interior link-withdrawal expressibility (K.μ⁻ admits only suffix-removal patterns in the link subspace) for uniform contiguity; L12's address-level permanence carries Nelson's load-bearing tombstoning requirement (LM 4/9), and the gap is treated in *Link-withdrawal gap under D-CTG★/D-MIN★* below.
...
## Freshness-discharge summary

The freshness obligations imposed by K.α, K.δ, and K.λ are closed by three distinct discharge axioms or T10a results, partitioned by the allocation event's allocator regime. The table below catalogues the discharge route for each obligation in this ASN.

| Transition | Sub-case | Freshness obligation | Discharged by |
|------------|----------|----------------------|---------------|
| K.δ | IsNode(e) | `e ∉ E` | NodeUniqueAllocation (axiom, this ASN) |
| K.δ | ¬IsNode(e), InEntityAllocatorDomain(t) | `e ∉ E` | T10a GlobalUniqueness on parent allocator's tracked domain |
| K.δ | k = 1 ghost-base (¬InEntityAllocatorDomain(t)) | `e ∉ E` | Direct E-inspection at the K.δ event (SequentialTransitionAxiom atomic inspection/commit) |
| K.α | First emission of A_C(d) | `a ∉ dom(C) ∪ dom(L)` | SubAllocatorAxiom.FirstEmission (axiom, this ASN) |
| K.α | Subsequent emission of A_C(d) | `a ∉ dom(C)` | T10a GlobalUniqueness on A_C(d)'s inc(·, 0) chain |
| K.α | Any emission | `a ∉ dom(L)` | SC-NEQ + T7 (cross-subspace distinctness via L14) |
| K.λ | First emission of A_L(d) | `ℓ ∉ dom(L) ∪ dom(C)` | SubAllocatorAxiom.FirstEmission |
| K.λ | Subsequent emission of A_L(d) | `ℓ ∉ dom(L)` | T10a GlobalUniqueness on A_L(d)'s inc(·, 0) chain |
...
*Discharge of arrangement-side invariants.* Step (ii)'s K.μ⁺ creates only content-subspace V-positions (by the K.μ⁺ amendment) targeting addresses in `ran(M(d_src)|_{V_{s_C}(d_src)}) ⊆ dom(C)` (by S3★'s content clause at the pre-state, which is the only S3★ clause supplying a dom(C) containment — the unrestricted `ran(M(d_src))` includes link-subspace targets in dom(L), which are excluded from dom(C) by L14); C is frame-preserved across the composite (none of K.δ, K.μ⁺, K.ρ modify C), so S3★'s content clause holds at the post-state. The link subspace `V_{s_L}(d_new)` is empty (step (ii) creates only content-subspace V-positions; step (i) initialised `M(d_new) = ∅`), so D-CTG★, D-MIN★, S8-depth, S8-fin, and S8a hold vacuously on d_new's link subspace. Step (ii)'s K.μ⁺ must establish D-CTG★, D-MIN★, S8a, S8-depth, S8-fin on `V_{s_C}(d_new)` by its postconditions — the choice of V-positions in step (ii) must be invariant-discharging, but the specific V-positions are operation-specific.
...
- *S7d★ (Document allocation discipline, relaxed).* The S7d clause of ExtendedReachableStateInvariants is the relaxed form, widening ASN-0036's S7d to admit ghost-operand discharge: every `d ∈ E_doc` is T4-valid with `zeros(d) = 2`, and was placed in E_doc by a K.δ event satisfying K.δ's `e ∉ E` precondition — discharged either by T10a GlobalUniqueness (in the non-ghost case where `InEntityAllocatorDomain(t)` holds at the K.δ event) or by direct E-inspection at the allocation event (in the ghost-base k = 1 sub-case where `¬InEntityAllocatorDomain(t)`). Distinct K.δ events on documents produce distinct addresses in both regimes, the first by T10a's per-`(t, k')` discipline and the second by the K.δ precondition. Preserved by P1 thereafter.
