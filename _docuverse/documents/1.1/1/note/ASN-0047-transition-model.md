...
**NodeRegistryBootstrap (Axiom, BootstrapRegistrySeeding).** At the initial state `Σ₀`, `n₀` is committed to the node-allocation protocol's tracked domain. *Scope note (external commitment, not state component).* The "node-allocation registry" is *not* a component of the system state `Σ = (C, L, E, M, R)`; it is an *external* protocol-layer construct (Nelson's hierarchical baptism / Gregory's granfilade; see NodeUniqueAllocation above), and NodeRegistryBootstrap, like NodeUniqueAllocation, is an *external* commitment about that construct rather than a predicate over a state-bearing component. The membership claim "n₀ ∈ dom(node-allocation registry) at Σ₀" is shorthand for an axiomatic premise of the K.δ discharge — it supplies the spawnPt-membership obligation in the form that T10a's T2-spawn rule expects, without requiring the registry to be a state component subject to per-state invariants. (Future work that lifts the registry into `Σ` — see *Open Questions* — would replace this external commitment with a state predicate over an extended state model; the present model deliberately leaves the registry outside Σ to match the consultation evidence that the node layer is a protocol concern, not a docuverse-layer state component.) Every entity `t ∈ E` other than `n₀` reaches the registry's tracked domain via the K.δ event that minted it (NodeUniqueAllocation clause (a) at that prior event); `n₀` enters at `Σ₀` with no prior K.δ, so its registry membership is supplied by axiom rather than derived from a non-existent predecessor. The K.δ case (ii) k = 2 discharge with operand `t = n₀` (creating the first account under `n₀`) cites NodeRegistryBootstrap as the external commitment satisfying the spawnPt obligation on the registry side — the T10a T2-spawn rule is applied here against the external registry's commitment, not against any state component query.
...
The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` in K.μ⁻'s effect forces `dom(M(d)) ≠ ∅` and also forces at least one V-position to be removed; the non-emptiness obligation does not need a separate precondition. The per-subspace suffix pattern of the K.μ⁻ amendment supplies the *shape* constraint on each subspace's contraction; the effect clause `dom(M'(d)) ⊂ dom(M(d))` supplies the *non-trivial contraction* requirement at the whole-arrangement level and, combined with the per-subspace pattern, forces strict contraction on at least one subspace as a derived consequence.
...
**K.μ⁻ admissible contraction shape (per-subspace, derivation of the per-subspace suffix pattern).** Fix a subspace S with D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`. Under D-CTG★, D-MIN★, and D-SEQ★ at the post-state, the post-state takes the suffix-removal form `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `0 ≤ n'_S ≤ n_S`.

*Proof.* If `V_S(d') = ∅`, the conclusion holds with `n'_S = 0`. Otherwise D-SEQ★ applied at the post-state gives `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `n'_S ≥ 1` directly (D-SEQ★ is the post-state invariant K.μ⁻ must discharge per the per-subspace suffix pattern of its precondition); `V_S(d') ⊆ V_S(d)` (from K.μ⁻'s contraction effect) forces `n'_S ≤ n_S`. ∎
...
- *k = 1 (version under existing document allocator):* The step `e = inc(t, 1)` is the T10a T2 spawn step that activates `A_v(t)`, t's version sub-allocator (per the Sub-allocator-names definition). *Parent-allocator relationship in T10a's allocator tree, case-split by t's own provenance:* (a) If `t` is an *original* document — minted by a prior K.δ case (ii) k = 2 step on operand `parent(t) ∈ E_account` — then `A_v(t)` is a child of `A_doc(parent(t))` (the document sub-allocator under t's account that minted t), and T2 admissibility's spawnPt premise `t ∈ dom(A_doc(parent(t)))` is supplied by the prior K.δ k = 2 event that placed t into that allocator's tracked domain. (b) If `t` is a *version* — `t = inc(t', 1)` minted by a prior K.δ case (ii) k = 1 event on some predecessor `t' ∈ E_doc` — then `A_v(t)` is a child of `A_v(t')` (the version sub-allocator of t's predecessor that minted t), and T2 admissibility's spawnPt premise `t ∈ dom(A_v(t'))` is supplied by the prior K.δ k = 1 event that placed t into `A_v(t')` as its first emission. T10a.6 (DomainDisjointness, ASN-0034) makes the case split mandatory: every `t ∈ E_doc` inhabits *exactly one* allocator's tracked domain, identifying the parent allocator unambiguously, and in case (b) specifically T10a.6 forbids `t ∈ dom(A_doc(parent(t)))` — `dom(A_v(t')) ∩ dom(A_doc(parent(t))) = ∅` since the two are distinct T10a sub-allocators. The earlier framing — "every `t ∈ E_doc` was minted by a prior K.δ event that placed t into `A_doc(parent(t))`'s tracked domain" — is *only* correct in case (a); in case (b), t inhabits `A_v(t')` instead, and the chain of version-of-version K.δ k = 1 events produces an arbitrarily deep nesting of `A_v` allocators along the version lineage. In both cases the precondition `t ∈ E_doc` discharges the membership obligation against whichever parent allocator T10a.6 identifies; P1 preserves that membership across subsequent transitions. T2 admissibility: `k' = 1 ∈ {1, 2}` and no zero-count side condition fires at `k' = 1`. Under T10a's per-`(t, k')` uniqueness this k = 1 step fires at most once per `t`, so it is always the T2 spawn step — never a T1 sibling. Subsequent versions of t arise from K.δ k = 0 events whose operand is a prior version of t (`inc(prev_version, 0)`); those are T1 sibling-increments on `A_v(t)`'s frontier and are dispatched by the k = 0 case above, not by k = 1. T10a GlobalUniqueness on the newly activated `A_v(t)` — viewed as a child of whichever parent allocator T10a.6 identifies — delivers `e ∉ E`.
...
- *k = 2 (descent producing the first child under a node or account):* this is the activation case, and care is required to identify *which* allocator is being spawned and *which* allocator is its parent in T10a's allocator tree. We name the participants explicitly. Let `t` be the K.δ operand (a node when creating an account; an account when creating a document) and `e = inc(t, 2)` the spawn output. The K.δ event itself spawns a *new entity-level sub-allocator* under `t` — call it `A_↓(t)`: when t is a node, `A_↓(t)` is "t's account sub-allocator" emitting account-level children of t; when t is an account, `A_↓(t)` is "t's document sub-allocator" emitting document-level children of t. The K.δ event is the T10a T2 spawn step that *creates* `A_↓(t)`, with `t` as spawnPt and `e` as the first emission.
  ...
  T10a T2 admissibility requires the spawnPt `t` to inhabit `dom(parent_allocator(A_↓(t)))` at the spawn event — i.e., t must already lie in the tracked domain of whatever allocator sits *above* the newly-spawned `A_↓(t)` in the allocator tree. The parent allocator above `A_↓(t)` is identified by `t`'s own provenance: it is precisely the allocator that minted `t`. *(i) t is an account.* The minting allocator is `A_↓(parent(t))` — the account sub-allocator under t's node — which was itself activated by an earlier K.δ event of this same k = 2 form; `t ∈ dom(A_↓(parent(t)))` follows from that earlier event by induction. T2 admissibility against this state-tracked parent allocator discharges in the standard T10a form. *(ii) t is a node, with prior K.δ event.* The minting "allocator" is the *external* node-allocation registry (Nelson's hierarchical baptism / Gregory's granfilade; see NodeUniqueAllocation above), which is *not* a state component of `Σ` — it is a protocol-layer construct external to the docuverse-layer state model (see NodeRegistryBootstrap's scope note). The T2 spawnPt obligation here is therefore discharged not by a state-component query but by NodeUniqueAllocation's external commitment that every baptised node `t` was placed into the registry's tracked domain by a prior K.δ node-allocation event; this is the protocol-layer counterpart to T10a's T2 admissibility, and the T2 discharge form is applied here against the external commitment rather than against any state-bearing component. NodeLineage's `n₀ ≼ t` is a structural consequence of that protocol-layer commitment, not the discharge itself. *(iii) Bootstrap special case t = n₀, no prior K.δ.* `n₀` enters at `Σ₀` with no prior K.δ event whatsoever, so neither the inductive route of case (i) nor NodeUniqueAllocation's prior-event clause of case (ii) applies. The T2-style spawnPt obligation for `A_↓(n₀)` (the account sub-allocator under the bootstrap node) is instead discharged by NodeRegistryBootstrap's external commitment that `n₀` inhabits the node-allocation registry's tracked domain at `Σ₀` — supplied by initial-state axiom rather than by any state query. As with case (ii), the T2 discharge form is applied here against an external protocol-layer commitment, not against a state-component query; the external registry is *not* a component of `Σ`, and no per-state invariant of this ASN constrains its content.

  In all three sub-cases the K.δ precondition `t = parent(e) ∈ E` discharges the T2 spawnPt requirement against whichever allocator (or external commitment) is the minting source of t. The spawn step's `k' = 2 ∈ {1, 2}` is admissible by T10a; K.δ's case-level zeros bound `zeros(t) ≤ 1` discharges T10a's zero-count side condition (T10a admits k' = 2 when `zeros(spawnPt) ≤ 2`, satisfied a fortiori). T10a GlobalUniqueness on the parent allocator's tracked domain — now extended to include `e` as `A_↓(t)`'s first emission — then delivers `e ∉ E`. For the cases involving the external node-allocation registry (ii) and (iii), T10a GlobalUniqueness's application to the freshly-emitted account `e` proceeds *within* the docuverse-layer allocator tree (since `A_↓(t)` *is* a T10a sub-allocator with `t` as its base, sitting inside the docuverse state) — the external registry's role is exhausted at the spawnPt premise, after which the docuverse-layer T10a discipline takes over uniformly.
...
A π that disturbs any position below the smallest candidate cut admits only the full-clearance form (or partial-suffix forms with smaller `k₀` whose admissibility condition the disturbance still respects). K.μ⁻ must retain link-subspace mappings under every expansion — K.μ⁺ (amended) is content-only and K.μ⁺_L only places at the contiguous min or max, so any removed link-subspace position could not be restored.
...
| D-CTG★ / D-MIN★ | frame | frame | frame | amendment postcondition (content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | amendment postcondition (per-subspace suffix shape) | inherits via decomposition | frame |
...
*D-CTG★ / D-MIN★.* K.μ⁺ amendment postcondition (contiguity and minimum-position preservation on the extended content subspace); K.μ⁺_L preconditions (D-CTG case for non-empty link subspace, D-MIN case for empty); K.μ⁻ amendment postcondition (suffix-removal shape forced by D-CTG★/D-MIN★/D-SEQ★ at the post-state); K.μ~ inherits via decomposition; all others hold M in frame.
