...
The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` in K.μ⁻'s effect forces `dom(M(d)) ≠ ∅`; the non-emptiness obligation does not need a separate precondition.
...
- The maximum case is *full content-subspace clearance and rebuild*: at `n'_{s_C} = 0`, K.μ⁻ removes V_{s_C}(d) entirely (maximal-suffix removal, with link-subspace retained) and K.μ⁺ then adds `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` in one step. This expansion works for *every* admissible π — it is the universal default and the form referenced in the K.μ~ verification arguments below.
