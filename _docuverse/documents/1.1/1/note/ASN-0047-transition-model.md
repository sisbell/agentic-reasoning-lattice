...
**Bootstrap genesis vs. K.δ allocation.** The bootstrap node n₀ is established in E₀ by system genesis, not by a K.δ event. NodeUniqueAllocation's discharge — both the freshness clause `e ∉ E` and the bootstrap-lineage clause `n₀ ≼ e` — applies only to subsequent K.δ node-allocation events; n₀ itself satisfies `n₀ ≼ n₀` by reflexivity and falls outside the scope of any allocation-event freshness obligation. Likewise, the S4 invariant ("distinct allocation events produce distinct addresses") quantifies over allocation events; n₀'s presence at Σ₀ as a genesis seed is outside that quantifier.
...
**NodeAllocationRegistry (Discussion).** The freshness and lineage conditions for node allocation are discharged entirely by NodeUniqueAllocation, stated next. The informal narrative is that some deterministic discipline external to T10a's allocator-state machinery issues node addresses — a single allocation authority maintaining a monotonically extended record of issued nodes, consulted at every K.δ event with `IsNode(e)`. This narrative carries no formal content of its own; it names what NodeUniqueAllocation abstracts over. The realisation (issuing protocol, persistence model, concurrency discipline) lies outside this ASN's discharge layer.

**NodeUniqueAllocation (Axiom, FreshNodeAddress).** Every K.δ node-allocation event — every elementary transition of K.δ whose effect places an entity `e` with `IsNode(e)` into E — produces an address satisfying two conditions: (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation; (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order. These are the conditions the node-allocation registry must satisfy; details of the registry mechanism (issuing protocol, persistence model, concurrency discipline) lie outside this ASN's discharge layer. Clause (b) supplies K.δ case (i)'s `n₀ ≼ e` precondition directly from the registry's issuing discipline, so the NodeLineage invariant — `(A e ∈ E : IsNode(e) : n₀ ≼ e)` — is inductively preserved by every K.δ node-allocation event.
...
- *Admissible removal pattern.* For each subspace S, the removed positions in `V_S(d)` form a suffix of `V_S(d)` under the per-subspace enumeration `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` for some `n_S ≥ 1`, with inner positions of uniform depth m_S — admitting the empty suffix (no change), a proper suffix (partial truncation), and the full set (clearance, the maximal suffix); *and* at least one subspace contracts strictly. (The per-subspace enumeration is the D-SEQ★ shape established in *Amendments to existing transitions* below; the K.μ⁻ precondition stated here applies in the extended state where D-SEQ★ holds across both subspaces.) Under that pre-state shape, there exists `0 ≤ n'_S ≤ n_S` per subspace such that the post-state subspace satisfies `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` — partial suffix removal when `1 ≤ n'_S < n_S`, full clearance when `n'_S = 0`, no change when `n'_S = n_S` — and the strict-contraction conjunct `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` requires some subspace to actually shrink. The full-clearance case is the maximal-suffix instance of the suffix pattern (at `n'_S = 0`), not a separate alternative. The per-subspace patterns are independent across `s_C` and `s_L`; the strict-contraction conjunct excludes the "no change in either subspace" assignment that per-subspace patterns alone would admit, closing the effect-clause requirement `dom(M'(d)) ⊂ dom(M(d))` at the whole-arrangement level.

Contraction preserves S2, S3 (since C' = C), S8a, S8-depth, and S8-fin by restriction of M(d). Under the D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` with uniform depth `m_S ≥ 2`, D-CTG★ and D-MIN★ admit exactly per-subspace suffix removals `{[S, 1, ..., 1, k] : n'_S < k ≤ n_S}` for some `0 ≤ n'_S ≤ n_S`, established by the exhaustiveness lemma below.
...
Mutual exclusion and exhaustiveness follow by construction of the case-analysis tree above: the top-level split is "K' = ∅ vs. K' non-empty"; on the non-empty branch, "K' contiguous over `[k_min, k_max]` vs. K' not contiguous"; on the contiguous sub-branch, "`k_min = 1` vs. `k_min ≥ 2`". The leaves (a-via-empty), (a-via-`k_min=1`), (c-via-`k_min≥2`), and (b-via-not-contiguous) are the four assignments to the case labels, with (a) absorbing two leaves and (b), (c) absorbing one each. No leaf is reached twice and every configuration of K' reaches some leaf. ∎

Only case (a) is admissible: (b) violates D-CTG★ (interior `[S, 1, ..., 1, k₀]` lex-between two surviving members but absent from V_S(d')); (c) violates D-MIN★ (`min(V_S(d'))` not at `[S, 1, ..., 1, 1]`). The value-preservation clause additionally pins K.μ⁻ as removal-only, ruling out modification of surviving values.
...
**Link-subspace replacement asymmetry.** The K.μ⁻ + K.μ⁺ replacement decomposition above applies only to the content subspace. Link-subspace "replacement" is structurally different and does *not* compose as K.μ⁻ + K.μ⁺_L at the same V-position. Two structural barriers preclude this: (i) K.μ⁺_L's first-arrangement precondition `ℓ ∉ ran(M(d))` forbids re-adding the same link `ℓ` once it has been withdrawn — a link removed from the link-subspace arrangement and re-added by K.μ⁺_L is treated as a fresh placement, not a recovery; (ii) K.μ⁺_L's position rule forces placement at the contiguous min (when `V_{s_L}(d) = ∅`) or at `shift(max(V_{s_L}(d)), 1)` (when `V_{s_L}(d) ≠ ∅`) — never at an interior or arbitrary V-position, so K.μ⁺_L cannot re-target the original slot of a withdrawn link even with a different link value. Substituting a "new" link for a withdrawn one at the user level therefore composes as K.μ⁻ + K.λ + K.μ⁺_L with a *fresh* link address allocated by K.λ; the new link inherits no V-position from the withdrawn one and is appended at the link-subspace maximum frontier. Repositioning at the original V-position is not expressible in the transition vocabulary of this ASN.
...
*Activation outside T10a's standard T2 spawning step.* SubAllocatorAxiom activates `A_C(d)` and `A_L(d)` at the K.δ event placing `d` into E_doc, but the activation does *not* follow T10a's T2 spawning premise. T10a's T2 step requires the new allocator's spawning point to inhabit its parent allocator's tracked domain (`spawnPt(A) ∈ dom_s(parent(A))` at the activation event), and the anchors `b_C(d), b_L(d)` are not elements of any predecessor allocator's tracked domain — they are structurally producible via inc steps from `d` (and inhabit `T`) but inhabit no state component of Σ. The activation is therefore axiomatic at this layer rather than derivable from T10a's standard discipline. The bypass does not violate T10a.6 (DomainDisjointness): the disjointness of `A_C(d)`'s outputs from `A_L(d)`'s outputs follows from subspace identifier distinctness at the anchor level (`subspace_I(b_C(d)) = s_C ≠ s_L = subspace_I(b_L(d))` by SubspaceConventionAxiom plus T7), not from T10a.6's domain-tracking argument; cross-document disjointness is supplied by the Cross-document disjointness chain lemma below; and within-allocator disjointness for each sub-allocator's own inc chain is closed by T10a.7 (EnumerationInjectivity) once the activation is granted.
...
| K.δ | ¬IsNode(e), ¬InEntityAllocatorDomain(t) | `e ∉ E` | Direct E-inspection at the K.δ event (SequentialTransitionAxiom atomic inspection/commit) — covers the k = 1 ghost-base step and every subsequent k = 0 chain step rooted at it |
...
**Decomposition.** When π = id (including the empty bijection and the degenerate case `dom_C(M(d)) = ∅`, which forces π = id by link-subspace fixity), K.μ~ expands into zero elementary steps: M'(d) = M(d) and no K.μ⁻ + K.μ⁺ round-trip is invoked. When π ≠ id (which requires `dom_C(M(d)) ≠ ∅`), the canonical expansion is *full content-subspace clearance and rebuild*: K.μ⁻ removes V_{s_C}(d) entirely (case-(a) maximal-suffix removal, with link-subspace retained) and K.μ⁺ then adds `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}`. K.μ⁻ must retain link-subspace mappings — K.μ⁺ (amended) is content-only and K.μ⁺_L only places at the contiguous min or max, so any removed link-subspace position could not be restored.
...
*(iii) Link addresses.* `(A ℓ ∈ dom(L) :: n₀ ≼ ℓ)`. By L1a, `origin(ℓ) ∈ E_doc ⊆ E`, so (i) gives `n₀ ≼ origin(ℓ)`. By L1c, ℓ is reachable from `origin(ℓ)` by a T10a-conforming inc chain, and every step in such a chain preserves the operand's prefix (TA5(b) for k > 0, TA5(c) for k = 0), so `origin(ℓ) ≼ ℓ`. Transitivity closes: `n₀ ≼ origin(ℓ) ≼ ℓ`. ∎
...
| NodeAllocationRegistry | Discussion (not a formal item): the narrative name for the abstract obligation discharged formally by NodeUniqueAllocation alone — the requirements are NodeUniqueAllocation's two clauses (freshness against E and bootstrap lineage n₀ ≼ e); realisation unspecified at this layer |
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7c ∧ S7d★ ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8★ ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L1c ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ (per-state). P3 (which packages P0, P1, P2, L12) and S9 are *per-transition*: see ExtendedTransitionInvariants. Together supersedes ReachableStateInvariants | This ASN's own Reachable-state invariants synthesis extended to the two-subspace state |
