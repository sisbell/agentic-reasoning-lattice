...
- *ASN-0093 K.σ effect `dom(M') = dom(M) ∪ {d}`.* Read as the entry of `d` into E_doc. ASN-0047 subsumes K.σ into K.δ for `IsDocument(e)` (*Subsumption of ASN-0093's K.σ*), whose effect `E' = E ∪ {e}` places `e` into E_doc with `M'(e) = ∅` by totality — matching the translated effect.
...
Each non-node K.δ event observes the caller-checked guard `e ∉ E` before firing. This section records, per sub-case, the parent entity-level sub-allocator on which the step `e = inc(t, k)` acts, and which allocator-discipline properties always applying the guard maintains. The guard is a precondition, not a conclusion: at k ∈ {1, 2}, GlobalUniqueness (ASN-0034) establishes only that *distinct* allocation events stay distinct (invariant preservation) — it does not by itself yield `e ∉ E`, since that additionally requires the premise that the producing event of `e = inc(t, k)` has not already fired, which the guard supplies. The at-most-once firing of each `(t, k')` child-spawn is itself *maintained by* always checking the guard; it is not the source of the guard. The regime differs by k:
...
**J1 (Extension records provenance).** The operative coupling is J1★ (*Scoped coupling constraints*) — arrangement extension K.μ⁺ must co-occur with provenance recording K.ρ; over the link-free fragment (`dom(L) = ∅`, every V-position content-subspace) J1★ reads `(A Σ →* Σ', d ∈ E'_doc, a : a ∈ ran(M'(d)) \ ran(M(d)) : (a, d) ∈ R')`, and for a freshly created document `M(d) = ∅` by totality so this fires on every I-address placed.

**J1' (Provenance requires extension).** The operative converse is J1'★ (*Scoped coupling constraints*) — every new provenance entry corresponds to a content-subspace range change; over the link-free fragment (`dom(L) = ∅`) it reads `(A Σ →* Σ', a, d ∈ E'_doc : (a, d) ∈ R' \ R : a ∈ ran(M'(d)) \ ran(M(d)))`, so provenance recording for `(a, d)` occurs only within a composite where K.μ⁺ introduces `a` into `ran(M'(d))` and `(a, d) ∉ R` (a re-introduction of an already-recorded `a` requires no new K.ρ, since P2 persists the prior entry).
...
P7a is a composite-boundary property (Class (b)), not preserved by each elementary transition; its proof is given once under Class (b) in the *Extended reachable-state invariants* section.
...
| TrackedEmission | Per-state invariant (EntityEmissionTracking): `(A e ∈ Σ.E : ¬IsNode(e) : (E A : A a tracked entity-level sub-allocator : e ∈ dom(A)))` — every non-node entity inhabits some tracked sub-allocator domain. Holds vacuously at Σ₀; preserved by K.δ (each non-node entity enters E only via a T10a inc-step on a tracked sub-allocator, per §K.δ case (ii) discharge) and frame on all other transitions. Supplies the *existence* of the owning allocator A that FrontierEquivalence then resolves to uniqueness via T10a.6 |
