...
The two predicates are *not* equivalent: ghosts satisfy InTumblerUniverse but not InEntityAllocatorDomain. The three discharge paths named in the table — Path 1 (T10a GlobalUniqueness at the entity-allocator layer, combined with cross-allocator disjointness T10a.{2,5,6} → T10), Path 2 (TA5 determinism + K.δ precondition `e ∉ E` at the tumbler layer), Path 3 (NodeUniqueAllocation for node events) — discharge the freshness obligation `e ∉ E` at the pre-state. For ghost-chain emissions downstream of a ghost-base K.δ, the displacement to Path 2 propagates chain-wide because T10a's T2 spawning requires the operand to inhabit its parent allocator's tracked domain, which fails at the ghost-base root.
...
Downstream of each sub-allocator's first emission, the frontier is a T10a-conforming `inc(·, 0)` chain whose subsequent emissions inherit T10a's GlobalUniqueness in full; SubAllocatorAxiom underwrites only the bootstrap.
