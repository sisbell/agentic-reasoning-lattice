...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | both clauses preserved via K.μ⁻ restriction + K.μ⁺ amendment alone (link-subspace fixity is downstream, not prerequisite) |  frame |
...
| D-SEQ★ | frame | frame | frame | derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a | derived | derived | derived at Σ' from the K.μ~-chain post-state values of D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a (each established by the K.μ⁻ + K.μ⁺ decomposition above) | frame |
