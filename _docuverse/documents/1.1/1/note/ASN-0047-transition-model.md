...
— every allocated document is a document entity (K.δ's `IsDocument` registration places it in `E_doc` and initialises `M(d) = ∅`), and conversely every `d ∈ E_doc` is an allocated document. Because the typing is unchanged, every inherited `dom(M)`-phrased foundation result applies *verbatim*; where this ASN names the document set it writes `E_doc`, and (†) is the sole bridge connecting the two notations. The trivial total reading `{d : M(d) defined}` is never used: `dom(M)` always denotes allocated documents, so the notation has one meaning throughout.

*Notational convention (default value).* For a tumbler `d ∉ dom(M)`, the expression `M(d)` abbreviates the empty partial function `∅`. This is a writing convenience that keeps range expressions such as `ran(M(d))` defined on all of `T` (yielding `∅` off `E_doc`); it is *not* a change of typing, and `d ∉ dom(M)` continues to mean `d` is unallocated. In particular `M(d) = ∅` does not signal allocation status — a freshly registered document also has `M(d) = ∅` — so `E_doc`-membership, not the test `M(d) = ∅`, is the discriminating predicate for allocation.
...
*Provenance of inherited foundation results.* The foundation axioms and invariants this ASN restates for self-containment — SequentialTransitionAxiom and SubspaceConventionAxiom here, and L0, L3, C-fin, L1c, L14 below — are taken from ASN-0093 directly; we restate them for reading continuity and do not re-derive them. We do not repeat this provenance per result.
...
*Frame:* C' = C; L' = L; R' = R; M' = M (uniform across the IsNode, IsAccount, and IsDocument cases). The sole effect of K.δ is `E' = E ∪ {e}`.
...
The content- and link-subspace allocators are organized as sibling element-field sub-allocators rooted at each document. The anchor and sub-allocator notation used throughout this ASN — `b_C(d)`, `b_L(d)`, `A_C(d)`, `A_L(d)` — follows ASN-0093 (the anchors and chains of its FirstEmission and ChainDiscipline lemmas); we summarise it here for reading continuity.
...
**SubAllocatorBundle (introduced here).** This is a *bundling lemma* assembled in this ASN, not an axiom and not a result posited by ASN-0093. It collects under one name the sub-allocator facts the discharge chains below rely on, so a call site can cite one bundle rather than five separate ASN-0093 lemmas. Each sub-clause is *proved* by a named ASN-0093 lemma; none is taken axiomatically. For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]` — anchors `b_C(d) = inc(d, 2)`, `b_L(d) = inc(b_C(d), 0)`, per ASN-0093's FirstEmission. The five sub-clauses, each with its discharging ASN-0093 lemma:
...
*TrackedEmission.* Established by the self-contained induction in its definition box (the *Preservation* paragraph under TrackedEmission): vacuous at Σ₀, preserved by K.δ via §*K.δ case (ii) discharge* (each non-node entity enters E only by a T10a inc-step on a tracked sub-allocator, joining that allocator's tracked domain at the placing event), and held in frame by every non-K.δ transition. That induction reaches every reachable state, so TrackedEmission holds wherever FrontierEquivalence consumes it.
...
| ExtendedTransitionInvariants | Every valid composite transition Σ →* Σ' satisfies P3, the conjunction P0 ∧ P1 ∧ P2 ∧ L12 (which subsumes ASN-0036's S0 and S1 via P0 and extends ASN-0043's L12). Content-store invariance under arrangement mutation follows from P0 via the arrangement frames (`C' = C` on every M-mutating transition leaves the content store untouched) | This ASN's own per-transition synthesis |
