...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements (operand-admissibility conjuncts only; structural identities between `e` and `t` are catalogued separately below and not repeated here):
  ...
  - Structural identities (consequences of TA5 + T4b's parent projection on `e = inc(t, k)`; catalogued here as an aid to verification, *not* repeated as per-sub-case preconditions): `zeros(e) = zeros(t)` for k ∈ {0, 1} (TA5(c) preserves zeros for k = 0; TA5(d) at k = 1 appends a final 1 with no new zero, so zeros is preserved); `zeros(e) = zeros(t) + 1` for k = 2 (TA5(d) at k = 2 appends one zero separator and a final 1); `parent(e) = parent(t)` for k ∈ {0, 1} (k = 0 leaves the trailing-component position unchanged, k = 1 extends by one non-zero component without crossing a zero separator, so T4b's truncation past the last separator yields the same prefix); `parent(e) = t` for k = 2 (k = 2 introduces a new zero separator immediately after t, making t itself the parent prefix under T4b). These identities discharge the case-level requirement `parent(e) ∈ E` against the operand's own membership (combined with P8 at `t` for the k ∈ {0, 1} cases; directly from `t ∈ E` for the k = 2 case).
...
*Precondition:*
- `d ∈ E_doc`.
- `dom(M(d)) ≠ ∅` — the arrangement must be non-empty. This is required for the effect clause `dom(M'(d)) ⊂ dom(M(d))` to be satisfiable (no proper subset of `∅` exists), so it appears as an explicit precondition rather than as an implicit consequence of effect-clause satisfiability.
- The contracted arrangement `M'(d)` must satisfy the per-state arrangement invariants S2, S3★, S8a, S8-depth, S8-fin, D-CTG★, and D-MIN★.

The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` in K.μ⁻'s effect — under the explicit `dom(M(d)) ≠ ∅` precondition above — also forces at least one V-position to be removed. The per-subspace suffix-removal *shape* on each subspace is forced by D-CTG★ + D-MIN★ + D-SEQ★ at the post-state (derived in the *K.μ⁻ admissible contraction shape* paragraph below); the effect clause `dom(M'(d)) ⊂ dom(M(d))` supplies the *non-trivial contraction* requirement at the whole-arrangement level and, combined with the derived per-subspace shape, forces strict contraction on at least one subspace as a derived consequence.

Contraction preserves S2, S3★ (since C' = C, L' = L), S8a, S8-depth, and S8-fin by restriction of M(d). Under D-CTG★ / D-MIN★, valid contractions are restricted to suffix-removal patterns on the lex-sequential range of the arrangement. The per-subspace refinement of the contraction pattern — including the explicit suffix shape and the derivation of the suffix-removal shape from D-CTG★ / D-MIN★ / D-SEQ★ — is the K.μ⁻ amendment. The value-preservation clause additionally pins K.μ⁻ as removal-only, ruling out modification of surviving values.
...
**K.μ⁻ amendment (PerSubspaceScope).** In the extended state, K.μ⁻'s D-CTG / D-MIN postconditions read as D-CTG★ / D-MIN★ (the per-subspace forms introduced above), and the admissible removal pattern applies *per-subspace* under the D-SEQ★ enumeration `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` (also introduced above). There is no separate per-subspace shape precondition: D-CTG★ + D-MIN★ + D-SEQ★ at the post-state — together with K.μ⁻'s contractive effect — force the suffix-removal shape on each subspace independently, as derived in the *K.μ⁻ admissible contraction shape* paragraph below.

*Per-subspace consequence of the effect clause (not a precondition).* `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` — at least one subspace shrinks strictly. *Empty-arrangement boundary.* K.μ⁻'s explicit precondition `dom(M(d)) ≠ ∅` (stated at the K.μ⁻ definition in *Elementary transitions* above) is what makes the effect clause `dom(M'(d)) ⊂ dom(M(d))` satisfiable — without it, no proper subset of the empty set would exist. K.μ⁻ cannot fire on a document whose arrangement is already empty. Since `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)`, `dom(M(d)) ≠ ∅` forces at least one `V_S(d) ≠ ∅`. The per-subspace strict-contraction conclusion then follows: the derived suffix shape (from D-CTG★ + D-MIN★ + D-SEQ★ at the post-state) supplies `V_S(d') ⊆ V_S(d)` for each S, and the effect clause's proper-subset requirement on `dom(M'(d)) = V_{s_C}(d') ∪ V_{s_L}(d')` against `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)` rules out simultaneous equality on both subspaces, forcing strict contraction on at least one non-empty `V_S(d)`. The operation discharges this consequence by construction whenever its effect clause is satisfied; it is recorded here only to locate, at the per-subspace level, which subspace(s) carry the contraction.
...
**K.μ⁻ admissible contraction shape (per-subspace, derivation of the per-subspace suffix shape).** Fix a subspace S with D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`. Under D-CTG★, D-MIN★, and D-SEQ★ at the post-state — invariants K.μ⁻ must satisfy as a per-state condition on Σ' — the post-state takes the suffix-removal form `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `0 ≤ n'_S ≤ n_S`.

*Proof.* If `V_S(d') = ∅`, the conclusion holds with `n'_S = 0`. Otherwise D-SEQ★ applied at the post-state gives `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `n'_S ≥ 1` directly. (*Non-circularity.* D-SEQ★ at Σ' here is the *local* D-SEQ★ derivation at the K.μ⁻ post-state, fired from D-CTG★, D-MIN★, S8-depth, S8-fin, and S8a at Σ' — all of which are K.μ⁻ preconditions or restriction-preserved at Σ', not from a prior inductive D-SEQ★ assumption. The dependence chain is: K.μ⁻'s preconditions establish D-CTG★ and D-MIN★ at Σ'; restriction of M(d) preserves S8-depth, S8-fin, and S8a at Σ'; the D-SEQ★ definitional derivation then fires at Σ' using only these Class-(a)-elementary-preserved invariants.) S8-depth at Σ' inherits `m_S` from the surviving V-positions of Σ — restriction of M(d) cannot alter the depth of any surviving position, so the post-state depth of subspace S equals the pre-state depth — making `V_S(d')` and `V_S(d)` share the same canonical D-SEQ★ shape (same subspace identifier `S`, same depth `m_S`, same inner "1, ..., 1" segment of length `m_S − 2`); set inclusion `V_S(d') ⊆ V_S(d)` then reduces to comparison of the trailing-component bound. With both `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` and `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` sharing every component except the terminal range, `V_S(d') ⊆ V_S(d)` (from K.μ⁻'s contraction effect) forces `n'_S ≤ n_S`. ∎

*Excluded shapes (side remark).* Configurations with an interior hole — `{[S, 1, ..., 1, k_lo], [S, 1, ..., 1, k_hi]} ⊆ V_S(d')` with some `k_lo < k₀ < k_hi` and `[S, 1, ..., 1, k₀] ∉ V_S(d')` — violate D-CTG★ (the interior tuple lex-between two surviving members but absent). Configurations with a missing minimum — `[S, 1, ..., 1, 1] ∉ V_S(d')` while `V_S(d') ≠ ∅` — violate D-MIN★. Both are therefore excluded by the per-state invariants D-CTG★ and D-MIN★ that the post-state of K.μ⁻ must satisfy.
...
- *k = 2 (descent producing the first child under a node or account):* this is the activation case, and care is required to identify *which* allocator is being spawned and *which* allocator is its parent in T10a's allocator tree. We name the participants explicitly. Let `t` be the K.δ operand (a node when creating an account; an account when creating a document) and `e = inc(t, 2)` the spawn output. The K.δ event itself spawns a *new entity-level sub-allocator* under `t`; the two cases admit specific names — when t is a node, the spawned allocator is `A_account(t)`, t's *account sub-allocator*, emitting account-level children of t; when t is an account, the spawned allocator is `A_doc(t)`, t's *document sub-allocator*, emitting document-level children of t. (The `A_doc(·)` notation matches its use in the *Sub-allocator names* section above, where `A_doc(parent(d))` was named as `A_v(d)`'s parent allocator when d is an original document; `A_account(·)` is the analogous notation one level up. The Sub-allocator names section listed only the three sub-allocators rooted at a document — `A_C(d)`, `A_L(d)`, `A_v(d)` — but `A_doc(·)` and `A_account(·)` are first-class members of the same naming family, rooted one and two entity-hierarchy levels higher respectively.) The K.δ event is the T10a T2 spawn step that *creates* the spawned allocator, with `t` as spawnPt and `e` as the first emission.
...
*Dependency chain at a glance.* The argument structure below has a fixed order of derivation, in which each step's input is the previous step's output. The chain is non-circular and load-bearing for both the K.μ~ link-subspace fixity result and the existence condition `|dom_C(M(d))| ≥ 2`:
  ...
  S3★(Σ') established via K.μ⁻ + K.μ⁺ decomposition (independent route); admissibility clause (ii) is a redundant safety check satisfied by the same decomposition
...
Each step appears below in the same order; the prose substantiates each link explicitly. The S3★(Σ') step is dispatched by the K.μ⁻ + K.μ⁺ decomposition directly (matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~: "preserved via K.μ⁻ restriction + K.μ⁺ amendment alone"); admissibility clause (ii) of K.μ~ then carries no derivational weight at this step — it functions as a verification obligation that the chosen decomposition must satisfy, and the decomposition discharges it in passing.

*Preconditions of K.μ~.* The operation has two explicit preconditions:
- `d ∈ E_doc`
- `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (iii) `π ≠ id` to admit a witness (the singleton case `|dom_C(M(d))| = 1` admits only the identity permutation; the empty case `dom_C(M(d)) = ∅` is doubly excluded — see *Decomposition* below for the full derivation). Stated here at the definition site as a caller-checked precondition: a transition with `|dom_C(M(d))| ≤ 1` does not fire, and the caller is responsible for verifying this condition before invoking K.μ~. Equivalently, the operation's discharge of admissibility clause (iii) is a sufficiency obligation that the operation realises by exhibiting an admissible `π`; the precondition makes that obligation discharge.
...
π is admissible iff (i) every `π(v)` satisfies S8a, (ii) the induced post-state `M'(d)` would satisfy S8-depth, D-CTG★, D-MIN★, and S3★, and (iii) `π ≠ id`. Clause (iii) makes K.μ~ a real reordering: a permutation whose net effect is the identity is not a K.μ~ transition (the system simply does not change). **Verifier-facing notice — clause (i) is provably redundant and need not be discharged independently.** Clause (i) carries *no independent verification burden*: it is satisfied as a derived consequence of clause (ii), K.μ~-FIX, and the inductive hypothesis on S8a at the pre-state Σ — readers verifying a K.μ~ transition must establish clauses (ii) and (iii), and S8a at every `π(v)` follows automatically. The redundancy is structural, not aesthetic; verifiers who reproduce a clause-(i) check duplicate work already done by the inductive hypothesis. The derivation: S8a is a per-state invariant in ExtendedReachableStateInvariants, so every `v ∈ dom(M(d))` satisfies S8a at the pre-state Σ; K.μ~-FIX (derived below) gives `dom(M'(d)) = dom(M(d))`, so π's range is `dom(M(d))`, and every `π(v) ∈ dom(M(d))` inherits S8a from the inductive hypothesis unconditionally. The K.μ~-FIX derivation in turn rests on subspace preservation (a downstream consequence of S3★ at both endpoints together with L14 and the bijection equation), not on clause (i), so the redundancy is non-circular. Clause (i) is retained in the admissibility statement only for *expository* clarity — marking each step's S8a obligation explicitly rather than leaving it as a derived consequence — and does not extend the operational obligations beyond clauses (ii) and (iii). The verification matrix entries for K.μ~ in ExtendedReachableStateInvariants discharge S8a through the K.μ⁻ + K.μ⁺ decomposition's restriction-and-extension chain, not through clause (i) as an independent precondition. Subspace preservation — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — is a *derived consequence* of S3★ at both endpoints together with L14 and the bijection equation; it is *not* a prerequisite for establishing S3★(Σ'). The independent route to S3★(Σ') is by direct K.μ⁻ + K.μ⁺ decomposition (matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~: "preserved via K.μ⁻ restriction + K.μ⁺ amendment alone (link-subspace fixity is downstream, not prerequisite)"), so S3★(Σ') is available without circular dependence on subspace preservation. The derivation of subspace preservation then consumes *both* S3★(Σ) (the inductive hypothesis at the pre-state) and S3★(Σ') (just-established at the post-state): suppose π maps a content-subspace `v` to s_L. By S3★(Σ) at `v`, `M(d)(v) ∈ dom(C)`; the bijection equation gives `M'(d)(π(v)) = M(d)(v) ∈ dom(C)`; by S3★(Σ') at `π(v)` (with `subspace(π(v)) = s_L`), `M'(d)(π(v)) ∈ dom(L)`. Combining, `M'(d)(π(v)) ∈ dom(C) ∩ dom(L) = ∅` by L14 — contradiction. The link-subspace case is symmetric, with the roles of dom(C) and dom(L) interchanged.

**Non-determinism of K.μ~ realisations.** The admissibility conditions (i)–(iii) above identify a *family* of valid (π, K.μ⁻ + K.μ⁺) realisations for any given pre-state — *not* a single canonical decomposition. Once an admissible π is fixed, multiple K.μ⁻ + K.μ⁺ pairs can produce the same net post-state by choosing different K.μ⁻ removal cardinalities and corresponding K.μ⁺ rebuilds. Two named realisation forms partition the choice space:
...
|-----------|-----|-----|-----|------|--------|------|------|-----|
| S2 | frame | frame (M(e)=∅ on new entity disjoint) | frame | new positions disjoint from dom(M(d)) by value-preservation clause | new v_ℓ ∉ dom(M(d)) (verified at K.μ⁺_L) | restriction of M(d) | inherits via K.μ⁻ + K.μ⁺ decomposition | frame |
...
| S3★-aux | frame | frame | frame | amendment: new positions have subspace s_C | precondition: new v_ℓ has subspace s_L | restriction (subspaces of survivors unchanged) | decomposition (each step preserves) | frame |
...
| S8a, S8-depth, S8-fin | frame | new doc has M(d)=∅ (vacuous) | frame | precondition: positivity, depth, finite extension | precondition: positivity, m_L=2, finite | restriction preserves all three | inherits via decomposition | frame |
...
| D-CTG★ / D-MIN★ | frame | frame | frame | precondition discharge (K.μ⁺'s original precondition list, in *Elementary transitions*, requires the resulting M'(d) to satisfy D-CTG / D-MIN — strengthened to D-CTG★ / D-MIN★ in the extended state: content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | precondition discharge (K.μ⁻'s original precondition list requires the contracted M'(d) to satisfy D-CTG★ / D-MIN★; per-subspace suffix shape forced by D-CTG★ + D-MIN★ + D-SEQ★ at the post-state, with D-SEQ★ at Σ' obtained by *local re-derivation* at the post-state from D-CTG★/D-MIN★/S8-depth/S8-fin/S8a — not assumed as an independent post-state premise; see *K.μ⁻ admissible contraction shape* and its *Non-circularity* note for the firing chain at Σ') | inherits via decomposition | frame |
| D-SEQ★ | frame | frame | frame | derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a | derived | derived | derived | frame |
