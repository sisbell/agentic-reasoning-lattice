...
*Set-inclusion notation.* Throughout this ASN, `⊂` and `⊃` denote *proper* (strict) subset and superset respectively — `A ⊂ B ≡ A ⊆ B ∧ A ≠ B`, and `A ⊃ B ≡ B ⊂ A`. The non-strict relations are written `⊆` and `⊇`. The convention is load-bearing for K.μ⁺'s effect clause `dom(M'(d)) ⊃ dom(M(d))` (strict extension) and K.μ⁻'s effect clause `dom(M'(d)) ⊂ dom(M(d))` (strict contraction) — both require strict inequality on the domain to qualify as actual mutations.
...
where C : T ⇀ Val is the content store (per ASN-0036), L : T ⇀ Link is the link store (introduced in the *Link store and extended system state* section below), and M : T → (T ⇀ T) is total, satisfying M(d) = ∅ for d ∉ E_doc. The link store L is named in the state tuple here so the initial state Σ₀ and its invariant verification — both stated next — can be given once in their final five-component form, rather than restated after L is introduced. The four-component preamble preceding the link store's introduction reads Σ with `L = ∅`, under which L-related conjuncts are vacuous.

**Definition (Initial state).** The initial state Σ₀ = (C₀, L₀, E₀, M₀, R₀) is:

- C₀ = ∅ (no content allocated)
- L₀ = ∅ (no links allocated; L is the link store formally introduced in *Link store and extended system state* below)
...
- *S4, S7a, S7b, S7c (content invariants)*: `dom(C₀) = ∅`, vacuous.
...
**System state and initial state (recap).** The full system state Σ = (C, L, E, M, R) and the initial state Σ₀ = (C₀, L₀, E₀, M₀, R₀) with L₀ = ∅ were defined above in *The state model*, where L was named as a state component with the link store's substantive characterisation deferred to this section. The link store L : T ⇀ Link is the only state component whose substantive definition appears here; the remaining four were specified by ASN-0036 / ASN-0093 and recapitulated above. The consolidated verification of every per-state invariant (and every composite-boundary property) of ExtendedReachableStateInvariants at Σ₀ — including L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, and CL-UNIQ — is the *Initial state invariant verification* paragraph at the original definition site, which already covers the link-store invariants on the strength of `L₀ = ∅`.
...
*Frame:* `L' = L; E' = E; (A d :: M'(d) = M(d)); R' = R`. The `L' = L` conjunct comes from ASN-0093's K.α frame directly. The `E' = E` and `R' = R` conjuncts are added by this ASN to extend ASN-0093's K.α frame with the entity and provenance components (ASN-0093 has no E or R components, so these are not "inherited" — they are local additions establishing that K.α modifies neither E nor R).
...
**NodeUniqueAllocation (Axiom, FreshNodeAddress).** Every K.δ node-allocation event — every elementary transition of K.δ whose effect places an entity `e` with `IsNode(e)` into E — produces an address satisfying three conditions: (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation; (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order; (c) *Registry tracking:* for every reachable state Σ and every `t ∈ Σ.E_node`, `t` inhabits the external node-allocation registry's tracked domain. Clauses (a) and (b) are conditions on the address produced at the moment of allocation; clause (c) is the registry-side closure that downstream T2-spawn discharge invokes — specifically at K.δ case (ii) k = 2 sub-case B, where the spawnPt premise must be discharged against the external registry rather than against a state component. These are the conditions the node-allocation registry must satisfy at every K.δ node-allocation event; details of the registry mechanism (issuing protocol, persistence model, concurrency discipline) lie outside this ASN's discharge layer.

**NodeRegistryBootstrap (Axiom, BootstrapRegistrySeeding).** At the initial state `Σ₀`, `n₀` is committed to the node-allocation protocol's tracked domain. The node-allocation registry is external to `Σ`; `n₀` enters at `Σ₀` rather than via a prior K.δ event.

*Relationship to NodeUniqueAllocation clause (c).* NodeUniqueAllocation clause (c) — "for every reachable state Σ and every `t ∈ Σ.E_node`, `t` inhabits the external node-allocation registry's tracked domain" — is the *inductive closure* covering every E_node entry that arrived via a K.δ case (i) event after Σ₀. NodeRegistryBootstrap is the *base case* for n₀ specifically, where no prior K.δ event exists to ground the registry-tracking property: n₀ enters E at Σ₀ by initial-state stipulation, not by a node-allocation event, so clause (c)'s implicit derivation chain ("E_node entry → prior K.δ case (i) → registry tracking") cannot apply. The two axioms together form the standard induction structure — NodeRegistryBootstrap supplies n₀ ∈ registry at Σ₀, and clause (c) supplies the inductive step covering every subsequent K.δ case (i) addition. Downstream sites cite NodeRegistryBootstrap for the n₀ base case (in particular K.δ case (ii) k = 2 sub-case C, where operand t = n₀ has no prior K.δ event of its own to anchor clause (c) against) and clause (c) for every non-bootstrap node.
...
— i.e., the operational predicate "the `(t, 0)` increment has not yet been consumed" is logically equivalent to "no prior K.δ event has fired `(t, 0)` on `t`'s own sub-allocator chain."

*Three load-bearing premises.* (i) *T10a chain-advancement uniqueness at `(t, 0)`* (derived, not axiomatic): within `t`'s own sub-allocator chain, the `(t, 0)` pair fires at most once across the system history. The derivation rests on three independent ingredients: TA5(c) supplies the *functional determinism* of `inc(·, 0)` — `inc(tₙ, 0)` is a determinate single address (the unique sibling-increment of `tₙ`), not a choice among candidates; P1's E-monotonicity (premise (ii)) preserves any output once committed to E; and the operational precondition `inc(t, 0) ∉ E` enforced by the elementary transition that fires `(t, 0)` blocks every subsequent attempt at the same chain position. T10a.7 (EnumerationInjectivity, ASN-0034) plays only the framing role of identifying which address occupies which chain index — the map `n ↦ tₙ` is injective, so different chain positions have different addresses and the notion of "frontier" is well-defined — but it does *not* establish the determinism of inc; that is TA5(c)'s contribution. Once `(t, 0)` fires from operand `t = tₙ` and places `tₙ₊₁` in E, P1 holds it there, and the precondition `inc(t, 0) ∉ E` fails on every subsequent attempt to fire `(t, 0)` from the same chain position. T10a's *direct* per-`(t, k')` uniqueness axiom is stated for child-spawning `k' ∈ {1, 2}` only and does not cover the `k = 0` sibling-increment regime; the at-most-once property for `(t, 0)` is the derived consequence cited here under the named handle "T10a chain-advancement uniqueness at `(t, 0)`," with the derivation chain TA5(c) + P1 + precondition supplying the substantive content. (ii) *P1 (E-monotonicity):* any prior firing of `(t, 0)` would have placed its output permanently in `E`, so absence from the present `E` implies no prior firing. (iii) *T10a GlobalUniqueness (via T10a.6 cross-allocator domain-disjointness):* no allocator *other than* `t`'s own sub-allocator can produce the address `inc(t, 0)`, since independent allocator chains have disjoint tracked domains.

*Proof.* The biconditional decomposes into two implications, each proved separately.

*Forward direction (⟹).* Assume `inc(t, 0) ∉ Σ.E`. We show `t` is the frontier of its sub-allocator's `(t, 0)`-branch — i.e., no prior K.δ event has fired `(t, 0)` on `t`'s own sub-allocator chain. Argue by contrapositive: suppose for contradiction that some prior K.δ event *had* fired `(t, 0)`, producing the address `inc(t, 0)` and placing it into `E` at that earlier state. By premise (i)'s ingredient TA5(c), `inc(t, 0)` is a single determinate address (functional determinism — the same operand-and-parameter pair always produces the same output). By premise (ii) P1 (E-monotonicity), any address once placed into E persists across every subsequent transition. So `inc(t, 0)` is in E at the present state Σ — contradicting the assumed `inc(t, 0) ∉ Σ.E`. Hence no prior firing of `(t, 0)` occurred on t's sub-allocator chain; t is the frontier.

*Reverse direction (⟸).* Assume `t` is the frontier of its sub-allocator's `(t, 0)`-branch — no prior K.δ event has fired `(t, 0)` on t's own sub-allocator chain. We show `inc(t, 0) ∉ Σ.E`. Suppose for contradiction `inc(t, 0) ∈ Σ.E`. Then some allocation event in the system history placed `inc(t, 0)` into E. By premise (iii) (T10a GlobalUniqueness via T10a.6 cross-allocator domain-disjointness), the address `inc(t, 0)` can be produced by exactly one allocator's tracked chain — namely t's own sub-allocator (since `inc(t, 0)` is, by TA5(c), the sibling-increment of t on its own chain, and T10a.6 forbids any other allocator's tracked domain from containing it). So the placing event was a K.δ event firing `(t, 0)` on t's own sub-allocator chain — contradicting the frontier assumption. Hence `inc(t, 0) ∉ Σ.E`.

Together, the two implications yield the biconditional `inc(t, 0) ∉ Σ.E ⟺ t is the frontier of its sub-allocator's (t, 0)-branch`. ∎

*Counterexample to T4b-based identification.* T4b's `parent`/`zeros`/length stratification does *not* in general identify `t` as the frontier of its own sub-allocator. T4b groups entities by entity-hierarchy spine, not by allocator-tree provenance. Under a single account `A`, the document sub-allocator `A_doc(A)` emits documents (`zeros = 2`, `parent(·) = A` under T4b) at one length, and each existing document `d` under `A` spawns a version sub-allocator `A_v(d)` whose own emissions are also documents under T4b's `parent(·) = A` — and two such version sub-allocators `A_v(d₁), A_v(d₂)` can coexist with outputs at the same length (e.g., `v₁ = inc(d₁, 1)` and `v₂ = inc(d₂, 1)`, both of length `#A + 3`). T10a tree-siblings need not differ in length: T10a.1 fixes length uniformly *within* each allocator's sibling chain, and T10a.3 separates ancestor-descendant pairs by length, but neither separates tree-siblings (such as `A_v(d₁)` and `A_v(d₂)`) from one another. A maximality clause over T4b's `parent`/`zeros`/length stratification therefore cannot in general identify `t` as the frontier of its own sub-allocator.

*Significance.* The direct freshness predicate `inc(t, 0) ∉ E` sidesteps allocator-structural identification entirely: its operational truth at the K.δ event is what selects `t` (equivalently, what places `t` on the frontier), and any freshness obligation `e ∉ E` for `e = inc(t, 0)` discharges from the precondition itself. Downstream K.δ case (ii) `k = 0` discharge and the S4 row of the verification matrix cite this lemma rather than re-deriving the three-premise chain in place.
...
*Direct vs derived T10a uniqueness across sub-cases (gloss).* The case-level "where"-clause conjuncts `e ∉ E ∧ ValidAddress(e) ∧ ¬IsElement(e)` apply uniformly across both cases. The freshness conjunct `e ∉ E` is discharged by two structurally distinct T10a forms, depending on the sub-case's k value: (a) *Direct T10a per-`(t, k')` uniqueness axiom*, stated for child-spawning `k' ∈ {1, 2}` only — applied at K.δ case (ii) k = 1 (T2 spawn of a version sub-allocator) and K.δ case (ii) k = 2 (T2 spawn of a child entity-level sub-allocator). (b) *T10a chain-advancement uniqueness at `(t, 0)` (derived form)* — derived from TA5(c) (functional determinism of `inc(·, 0)`) + P1 (E-monotonicity) + the operational precondition `inc(t, 0) ∉ E`; routed through FrontierEquivalence — applied at K.δ case (ii) k = 0 (T1 sibling-increment on an activated allocator's frontier). T10a.7 (EnumerationInjectivity) plays only the framing role of identifying chain enumeration structure (distinct indices → distinct addresses, so "frontier" is well-defined) and is not the determinism source. The direct axiom is not available at k = 0 because T10a's per-`(t, k')` axiom does not cover the `k' = 0` sibling regime; the derived form fills the gap, and FrontierEquivalence packages the three-premise chain into a single citable lemma. Case (i) is outside T10a's discharge layer entirely — its freshness is supplied by NodeUniqueAllocation, the protocol-established axiom. Downstream prose tags T10a citations as "direct per-`(t, k')` uniqueness" (k ∈ {1, 2}) or "T10a chain-advancement uniqueness at `(t, 0)` (derived form)" (k = 0) to keep the two routes distinguishable at every invocation site.
...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ ValidAddress(e) ∧ ¬IsElement(e)` apply uniformly to all three sub-cases; in particular, the freshness conjunct `e ∉ E` is discharged per sub-case by the discharge route named in the gloss above and substantiated in *Freshness discharge* below (T10a chain-advancement uniqueness at `(t, 0)` — derived form, via FrontierEquivalence — at k = 0; T10a direct per-`(t, k')` uniqueness at k = 1 and k = 2). Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements (operand-admissibility conjuncts only; structural identities between `e` and `t` are catalogued separately below and not repeated here):
  - *k = 0 (sibling):* `t ∈ E ∧ ¬IsNode(t) ∧ inc(t, 0) ∉ E`. The substantive operand-admissibility conjuncts are `t ∈ E` (the operand must be an allocated entity), `¬IsNode(t)` (so `parent(t)` is well-defined under T4b), and `inc(t, 0) ∉ E` (the operational frontier check, discharged below). The companion structural identities `parent(t) = parent(e)` and `zeros(t) = zeros(e)` hold by TA5(c) on `e = inc(t, 0)` and are not separate caller obligations — they appear in the structural-identities catalogue below.
  - *k = 1 (version):* `t ∈ E_doc`. The operand must be an allocated document — only an existing document can be versioned. Nelson's CREATENEWVERSION operates on `<doc id>`, an allocated document (LM 4/66); Gregory's `docreatenewversion` retrieves the source's vspan via `doretrievedocvspanfoo`, which fails on a source not present in the granfilade. (`IsDocument(t)` follows from `t ∈ E_doc` by the definition of E_doc.) *Operational uniformity across operand provenance.* The K.δ k = 1 operation is uniform across both classes of operand admitted by the precondition `t ∈ E_doc`: original documents (case (a) of the *Sub-allocator names* definition above — t was minted by an earlier K.δ k = 2 event under an account) and version documents (case (b) — t was minted by a prior K.δ k = 1 or k = 0 event on the version chain of another document). The operation produces the same output `inc(t, 1)` and applies the same effect on E and M regardless of which class t inhabits; no case-split appears in the operation itself. The case-split surfaces only in the *verification* of the T10a T2 spawn step against the operand's actual parent allocator — `A_doc(parent(t))` in case (a), `A_v(t')` in case (b) — where T10a.6 (DomainDisjointness, ASN-0034) dispatches the verification through t's actual provenance at the present state, identifying which allocator's tracked domain hosts t and therefore which parent allocator the T2 spawn premise must be discharged against. Callers may therefore invoke K.δ k = 1 without observing t's allocator-tree provenance — only the surface predicate `t ∈ E_doc` is checked at firing time.
  - *k = 2 (descent):* `t ∈ E ∧ zeros(t) ≤ 1` (equivalently, `IsNode(t) ∨ IsAccount(t)`). The zeros bound follows from the case-level precondition `¬IsElement(e)` (`zeros(e) ≤ 2`) combined with the structural identity `zeros(e) = zeros(t) + 1` recorded below; stated explicitly here so the operand admissibility is visible at the definition site rather than left as a derivation. The companion structural identity `parent(e) = t` holds by TA5(d) on `e = inc(t, 2)` together with T4b's parent projection; it appears in the structural-identities catalogue below rather than being repeated as a separate precondition.
  - Structural identities on `e = inc(t, k)` (consequences of TA5 + T4b's parent projection; named here for direct citation, *not* repeated as per-sub-case preconditions):
    ...
    These four identities discharge the case-level requirement `parent(e) ∈ E` against the operand's own membership: combined with P8 at `t` for the k ∈ {0, 1} cases (giving `parent(e) = parent(t) ∈ E` by K.δ-ID.parent-0/1), and directly from `t ∈ E` for the k = 2 case (giving `parent(e) = t ∈ E` by K.δ-ID.parent-2). Downstream prose cites these identities by name (K.δ-ID.zeros-0/1, K.δ-ID.zeros-2, K.δ-ID.parent-0/1, K.δ-ID.parent-2) rather than unpacking the TA5/T4b derivation chain at each invocation site.

*Rationale (k = 0 conjuncts).* The `¬IsNode(t)` conjunct is required for `parent(t)` to be defined — T4b's parent projection is partial on T and undefined when `IsNode(t)`. Although `¬IsNode(t)` follows from the structural identity `zeros(t) = zeros(e)` together with the case-level `¬IsNode(e)` (`zeros(e) ≥ 1`), it is stated explicitly so the operand admissibility is visible at the definition site rather than left as a consequence of partial-function evaluation. The freshness conjunct `inc(t, 0) ∉ E` is the case-level `e ∉ E` specialised to `e = inc(t, 0)` — stated locally to record that the caller's operand selection must observe it. Operationally, the conjunction `t ∈ E ∧ inc(t, 0) ∉ E` IS the frontier identification: this equivalence is established by FrontierEquivalence above, whose three load-bearing premises (T10a chain-advancement uniqueness at `(t, 0)` — derived from TA5(c) + P1 + precondition, not directly axiomatic; P1 E-monotonicity; T10a GlobalUniqueness via T10a.6) collectively show that `inc(t, 0) ∉ E` holds iff `(t, 0)` has not yet fired on t's own sub-allocator chain — i.e., t is the chain's current frontier along the (t, 0) branch. The frontier identification is therefore *derived* from the precondition pair, not an independent obligation; FrontierEquivalence also records why T4b's `parent`/`zeros`/length stratification cannot in general identify t as the frontier of its own sub-allocator, so the direct freshness predicate `inc(t, 0) ∉ E` is the load-bearing operand selector.

*Freshness discharge.* The `e ∉ E` precondition is discharged by case on the K.δ form: case (i) is closed by NodeUniqueAllocation (the node-allocation protocol's axiomatic uniqueness clause); case (ii) is closed by T10a's GlobalUniqueness on the parent allocator's tracked domain. Every K.δ case (ii) event has its operand `t ∈ E` (sub-cases k = 0 and k = 2 require this directly; sub-case k = 1 requires `t ∈ E_doc ⊆ E`), placed there by a prior K.δ event whose allocator chain is T10a-tracked from activation onward. For sub-case k = 0 the precondition `inc(t, 0) ∉ E` discharges `e ∉ E` directly, since `e = inc(t, 0)`; FrontierEquivalence above establishes that this precondition is achievable and stable — `inc(t, 0) ∉ E` at the pre-state holds iff `(t, 0)` has not yet fired on t's sub-allocator chain (the lemma's three premises chain T10a chain-advancement uniqueness at `(t, 0)` — itself derived from TA5(c) + P1 + precondition — together with P1 E-monotonicity and T10a GlobalUniqueness). Once the current K.δ event fires `(t, 0)`, T10a chain-advancement uniqueness at `(t, 0)` (the derived form) prevents any future K.δ event from re-producing `inc(t, 0)`. The caller's responsibility is to verify the precondition (operationally, by selecting `t` from its sub-allocator's current frontier); the K.δ event then closes `e ∉ E` by precondition and FrontierEquivalence's forward-direction premises close forward-uniqueness for subsequent events.
...
See *Worked example: interior content replacement* for the concrete fresh-content trace; the formal decomposition of K.μ⁻ + K.μ⁺ for the first two forms (and the K.μ⁻ + K.μ⁺ skeleton of the four-step form) is stated in *Decomposition of K.μ~* below (interior replacement is the same shape with `n'_S = k₀ − 1` rather than `n'_S = 0`).
...
**K.α (no local amendment in extended state).** ASN-0093's K.α encodes the content-subspace restriction `E(a)₁ = s_C` (equivalently `subspace_I(a) = s_C`) in its precondition directly, and its frame already includes `L' = L`. No locally-introduced amendment of K.α is required in the extended state. Downstream sites in this ASN — verification matrix, body prose, and the *Properties Introduced* tables — cite this inherited precondition by name as **K.α's `E(a)₁ = s_C` precondition** (or **K.α's content-subspace precondition**), explicitly attributing it to ASN-0093 rather than to a local addition. Earlier drafts of this ASN used the shorthand "K.α amendment" for this inherited precondition; the term has been retired to avoid the false reading of a local strengthening.

*Frame (extended state).* `C' = C ∪ {a ↦ v}` (effect); `L' = L; E' = E; (A d :: M'(d) = M(d)); R' = R`. The `L' = L` conjunct comes from ASN-0093's K.α frame directly, discharging P3's L-monotonicity clause `dom(L) ⊆ dom(L')` explicitly; the `E' = E` and `R' = R` conjuncts come from the K.α frame as introduced in *Elementary transitions* above, where this ASN added them to extend ASN-0093's K.α frame with the entity and provenance components (ASN-0093 has no E or R components, so these are local additions, not inheritance).
...
*Frame (extended state).* `C' = C; L' = L; E' = E; (A d' : d' ≠ d : M'(d') = M(d')); R' = R`. The `L' = L` conjunct extends the original K.μ⁺ frame (which predated the link store) into the extended state, discharging P3's L-monotonicity clause `dom(L) ⊆ dom(L')` explicitly.
...
*Justification (uniform contiguity, no link-subspace exemption).* The strengthening can read as a deviation from Nelson's tombstoning design (LM 4/9), where "deleted links" are diagrammed as a named state of document content "awaiting historical backtrack functions [and which] may remain included in other versions." It is not. The byte and link subspaces have natively different disciplines — bytes are positional and rearrangeable, links are creation-ordered and permanent — and the V-position-level invariant strengthened here constrains *arrangement* (which positions are populated and how they sit on the V-ordering), not link existence or discoverability. Nelson's tombstoning intent decomposes into two architectural commitments, each discharged by a separate piece of this ASN's model. (i) *Eternal record.* Every link, once allocated, persists in dom(L) with its endsets fixed (L12, LinkImmutability) and remains discoverable through reverse-index search regardless of arrangement state. The historical-backtrack and cross-version-inclusion clauses of LM 4/9 are properties of dom(L), not of any document's V-arrangement; they are unaffected by what M(d) does or does not contain. (ii) *Current-version exclusion.* K.μ⁻ removes the V-position mapping `v_ℓ ↦ ℓ` from M(d) while ℓ stays in dom(L) — the *orphan-link state* `dom(L) ∖ ran(M(d))` established and named in *Orphan links and coupling flexibility* below, which the section cross-references LM 4/9 as the realisation of Nelson's "not currently addressable" status. The two commitments together — persistent record (i) plus presentational withdrawal (ii) — capture the tombstoning intent without invoking an exemption to V-position arrangement. The architectural cost is suffix-only contraction on each subspace: withdrawing an interior link forces withdrawal of every link allocated after it. *Interior link withdrawal* — preserving the V-positions of trailing links while marking the interior link as withdrawn — is outside K.μ⁻'s presentational-removal contract under D-CTG★/D-MIN★/D-SEQ★ and would require a separate mechanism (a status flag on the link record, an explicit retraction-link convention, or version-scoped membership semantics, all admitted by Nelson as alternative encodings of "withdrawn but extant"); the Open Questions section catalogues this as future work. The implementation evidence is consistent with both halves of this design: udanax-green offers no `DELETELINK` operation in the FEBE protocol at all, `findnextlinkvsa` (do2.c:151–167) restricts link allocation to strict suffix-append (no insertion path exists), and removal via DELETEVSPAN on a link's V-position decouples the V-arrangement from the link's persistent ISA and spanfilade entries — the link object survives the V-arrangement change. Granting the link subspace a contiguity exemption inside K.μ⁻ would conflate presentational removal (a V-arrangement change) with existential annotation (a per-link withdrawal marker), which the present formulation deliberately keeps orthogonal — V-arrangement invariants constrain only what M(d) looks like, while any "withdrawn" semantics belong to a separate mechanism over dom(L). Both decomposition rules — K.μ⁻'s admissible shape and the verification matrix downstream — follow from this orthogonality without further special-casing.
...
*Per-subspace consequence of the strict-contraction clause.* `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` — at least one subspace shrinks strictly. *Empty-arrangement boundary.* K.μ⁻'s explicit precondition `dom(M(d)) ≠ ∅` (stated at the K.μ⁻ definition in *Elementary transitions* above) is what makes strict contraction satisfiable — without it, no proper subset of the empty set would exist. K.μ⁻ cannot fire on a document whose arrangement is already empty. Since `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)`, `dom(M(d)) ≠ ∅` forces at least one `V_S(d) ≠ ∅`, and the constructive precondition requires at least one S with `n'_S < n_S` — placing the strict contraction on a non-empty subspace.
...
*Frame (extended state).* `C' = C; L' = L; E' = E; R' = R; (A d' : d' ≠ d : M'(d') = M(d'))`. The `L' = L` conjunct extends the original K.μ⁻ frame (which predated the link store) into the extended state, discharging P3's L-monotonicity clause `dom(L) ⊆ dom(L')` explicitly.
...
- `A_doc(A)` — account `A`'s **document sub-allocator** (introduced here), defined for each `A ∈ E_account`. Its first emission is `inc(A, 2)` — equivalently, the determinate tumbler `[A.0.1]` — an entity-level address with `zeros = 2` and `parent(·) = A` under T4b. Subsequent emissions are T1 sibling-increments `inc(prev_doc, 0)` on its frontier. Its outputs are original (non-version) documents under `A` and inhabit `E_doc`. `A_doc(·)` is the family member named in the case-(a') discussion of `A_v(d)`'s parent allocator: when `d ∈ dom(A_doc(parent(d)))`, `A_v(d)` is a child of `A_doc(parent(d))` in T10a's tree.
- `A_account(N)` — node `N`'s **account sub-allocator** (introduced here), defined for each `N ∈ E_node`. Its first emission is `inc(N, 2)` — equivalently, the determinate tumbler `[N.0.1]` — an entity-level address with `zeros = 1` and `parent(·) = N` under T4b. Subsequent emissions are T1 sibling-increments `inc(prev_account, 0)` on its frontier. Its outputs are accounts under `N` and inhabit `E_account`. `A_account(·)` is the family member cited in the K.δ case (ii) k = 2 sub-case A discharge below: when t is an account, the T2 spawnPt obligation `t ∈ dom(A_account(parent(t)))` is discharged against the account sub-allocator under t's node.
...
- **SubAllocatorAxiom.T10aConformance.** `A_C(d)` and `A_L(d)` are T10a-conforming allocators within d's allocator subtree. Each is activated by the entity-allocation event placing d into E_doc — the K.δ event for `d` is the joint T2-spawn step that simultaneously establishes both sub-allocators with their anchors as virtual roots and the first emissions `[d.0.s_C.1]` and `[d.0.s_L.1]` as their respective base addresses. From the second emission onward, each sub-allocator's frontier is governed by T10a's GlobalUniqueness on the sibling-increment chain: subsequent emissions are produced by `inc(·, 0)` on the prior frontier under TA5(c), with T10a chain-advancement uniqueness at `(t, 0)` (derived from TA5(c) + P1 + precondition; see FrontierEquivalence) ensuring each step is fresh against the sub-allocator's tracked domain. The first emission itself is committed outside `dom(C) ∪ dom(L)` by SubAllocatorAxiom.FirstEmission (a separate route, since the anchor traversal `d → b_C(d) → [d.0.s_C.1]` traverses the virtual anchor `b_C(d) ∉ dom(C) ∪ dom(L)`); T10a's discipline takes over from the second emission and applies uniformly thereafter.
...
*Frame:* `C' = C; E' = E; (A d' :: M'(d') = M(d')); R' = R`. The `dom(M)`-preserving conjuncts come from ASN-0093's K.λ frame directly. The `E' = E` and `R' = R` conjuncts are added by this ASN to extend ASN-0093's K.λ frame with the entity and provenance components (ASN-0093 has no E or R components, so these are not "inherited" — they are local additions establishing that K.λ modifies neither E nor R).
...
- *k = 0 (sibling under existing allocator):* operand `t ∈ E` already inhabits the parent's tracked domain by K.δ's `parent(t) = parent(e)` precondition; the step `e = inc(t, 0)` is a T10a T1 sibling-increment on the activated parent allocator. The frontier conjunct on `t` (the maximality clause in K.δ k = 0's precondition) ensures `t` is the parent allocator's current frontier, so `(t, 0)` has not been previously consumed by any earlier K.δ event; T10a chain-advancement uniqueness at `(t, 0)` (the derived form, TA5(c) + P1 + precondition; see FrontierEquivalence) then forces `e = inc(t, 0) ∉ E`. T10a GlobalUniqueness on the inc-chain delivers `e ∉ E` directly.

- *k = 1 (version under existing document allocator):* The step `e = inc(t, 1)` is the T10a T2 spawn step that activates `A_v(t)`, t's version sub-allocator. By the Sub-allocator-names definition of `A_v(d)`, T10a.6 (DomainDisjointness, ASN-0034) identifies the unique parent allocator of `A_v(t)` in T10a's tree from t's owning allocator at the present state — `A_doc(parent(t))` when `t ∈ dom(A_doc(parent(t)))` (case (a') of Sub-allocator names: t inhabits the account's document sub-allocator), `A_v(t')` when `t ∈ dom(A_v(t'))` for some `t'` (case (b'): t inhabits a version sub-allocator) — and the minting K.δ event that placed t into its owning allocator supplies the T2 spawnPt premise against whichever parent allocator T10a.6 identifies. The precondition `t ∈ E_doc` therefore discharges the membership obligation; P1 preserves that membership across subsequent transitions. T2 admissibility: `k' = 1 ∈ {1, 2}` and no zero-count side condition fires at `k' = 1`. Under T10a's *direct* per-`(t, k')` uniqueness axiom (instantiated at `k' = 1`, within the axiom's stated `k' ∈ {1, 2}` regime — see the *Direct vs derived T10a uniqueness across sub-cases (gloss)* paragraph above), this k = 1 step fires at most once per `t`, so it is always the T2 spawn step — never a T1 sibling. Subsequent versions of t arise from K.δ k = 0 events whose operand is a prior version of t (`inc(prev_version, 0)`); those are T1 sibling-increments on `A_v(t)`'s frontier and are dispatched by the k = 0 case above, not by k = 1. *Operational uniformity of K.δ k = 0 reconciled with allocator-tree provenance.* K.δ k = 0's precondition `t ∈ E ∧ ¬IsNode(t) ∧ inc(t, 0) ∉ E` is allocator-agnostic — it makes no commitment about which T10a sub-allocator the operand inhabits — yet the same K.δ k = 0 operation dispatches to different sub-allocators depending on its operand's actual provenance: with operand a sibling document inhabiting `A_doc(account)`'s tracked domain, K.δ k = 0 produces a T1 sibling-increment on `A_doc(account)`'s frontier; with operand a prior version of d inhabiting `A_v(d)`'s tracked domain, K.δ k = 0 produces a T1 sibling-increment on `A_v(d)`'s frontier. T10a.6 (DomainDisjointness, ASN-0034) identifies the specific sub-allocator from the operand's provenance (the operand inhabits exactly one allocator's tracked domain, and that allocator hosts the sibling-increment); FrontierEquivalence's three-premise chain (T10a chain-advancement uniqueness at `(t, 0)` — derived from TA5(c) + P1 + precondition, not directly axiomatic; P1 E-monotonicity; T10a GlobalUniqueness via T10a.6) then makes the operational predicate `inc(t, 0) ∉ E` equivalent to "t is the frontier of *its* sub-allocator's (t, 0)-branch" — with "its" resolved by T10a.6 against the operand's actual provenance, not by any structural property derivable from t alone. T10a GlobalUniqueness on the newly activated `A_v(t)` delivers `e ∉ E`.

  *Multi-version invariant chain.* Let `t = d` be an original document with versions `v₁, v₂, …, vₙ`. The chain has a single k = 1 step at its head and k = 0 sibling-increments thereafter: `v₁ = inc(d, 1)` via K.δ k = 1 (the T2 spawn step that activates `A_v(d)`, with operand `t = d`), and `vᵢ₊₁ = inc(vᵢ, 0)` via K.δ k = 0 (T1 sibling-increments on `A_v(d)`'s frontier, with operand `t = vᵢ`) for every `i ≥ 1`. Each `vᵢ` is itself in `E_doc` (zeros = 2, document-level — by TA5(d) at k = 1 for v₁, then by TA5(c) inductively for `vᵢ₊₁`). The K.δ k = 0 preconditions at step `i ≥ 1` read on `vᵢ` as the operand: `vᵢ ∈ E` (placed there by the immediately preceding step in the chain — k = 1 for v₁'s admission, k = 0 for v₂..vₙ — and preserved by P1), `¬IsNode(vᵢ)` (since `zeros(vᵢ) = 2 ≥ 1`), and `inc(vᵢ, 0) ∉ E` (the frontier check on `A_v(d)` discharged by FrontierEquivalence at the moment of the (i + 1)-th step). P8 reads consistently across the chain: `parent(vᵢ₊₁) = parent(vᵢ)` by the k = 0 structural identity, and `parent(v₁) = parent(d)` by the k = 1 structural identity, so every `vᵢ` has `parent(vᵢ) = parent(d)` — the account that originally hosted `d`. The parent relation does *not* track the version-chain itself: `vᵢ`'s parent in the entity hierarchy is d's account, not `vᵢ₋₁`. The allocator-tree relationship `{vᵢ : i ≥ 1} ⊆ dom(A_v(d))` (every version after v₁ inhabits `A_v(d)`'s tracked domain) is orthogonal to the T4b parent projection that P8 quantifies over.
...
- *k = 2 (descent producing the first child under a node or account):* this is the activation case, and care is required to identify *which* allocator is being spawned and *which* allocator is its parent in T10a's allocator tree. We name the participants explicitly. Let `t` be the K.δ operand (a node when creating an account; an account when creating a document) and `e = inc(t, 2)` the spawn output. The K.δ event itself spawns a *new entity-level sub-allocator* under `t`; the two cases admit specific names — when t is a node, the spawned allocator is `A_account(t)`, t's *account sub-allocator* (catalogued in *Sub-allocator names* above), emitting account-level children of t; when t is an account, the spawned allocator is `A_doc(t)`, t's *document sub-allocator* (also catalogued above), emitting document-level children of t. The K.δ event is the T10a T2 spawn step that *creates* the spawned allocator, with `t` as spawnPt and `e` as the first emission.
  ...
  T10a T2 admissibility requires the spawnPt `t` to inhabit `dom(parent_allocator)` at the spawn event — i.e., t must already lie in the tracked domain of whatever allocator sits *above* the newly-spawned allocator in the allocator tree. The parent allocator above is identified by `t`'s own provenance: it is precisely the allocator that minted `t`. *Sub-case A: t is an account.* Here the spawned allocator is `A_doc(t)`, and its parent allocator in T10a's tree is `A_account(parent(t))` — the account sub-allocator under t's node. The membership obligation `t ∈ dom(A_account(parent(t)))` is discharged by case-split on whether `t` is the first account under `parent(t)` or a subsequent sibling — the same allocator `A_account(parent(t))` emits both, but via different K.δ event kinds (T2 spawn versus T1 sibling-increment). *Sub-case A1: t is the first account under parent(t).* The K.δ event that minted `t` was a K.δ case (ii) k = 2 event whose operand was `parent(t)` (necessarily a node, since k = 2's structural identity `zeros(t) = zeros(parent(t)) + 1` at that prior event, combined with `zeros(t) = 1`, fixes `zeros(parent(t)) = 0` — IsNode(parent(t))). That minting event was dispatched via sub-case B (when `parent(t)` is a non-bootstrap node) or sub-case C (when `parent(t) = n₀`), and in either case its T2 spawn step activated `A_account(parent(t))` with `t` as the first emission, placing `t` into `dom(A_account(parent(t)))`. *Sub-case A2: t is a subsequent (non-first) account under parent(t).* The K.δ event that minted `t` was a K.δ case (ii) k = 0 event with operand a prior sibling account `t_prev` and `t = inc(t_prev, 0)`; the k = 0 structural identity `zeros(t) = zeros(t_prev)` combined with `zeros(t) = 1` gives `zeros(t_prev) = 1`, so `t_prev` is itself an account, and the companion identity `parent(t) = parent(t_prev)` (also k = 0 — see the structural-identities catalogue at K.δ's definition) makes `t` and `t_prev` siblings under the same node. That K.δ k = 0 event was the T10a T1 sibling-increment on `A_account(parent(t_prev)) = A_account(parent(t))`, placing `t` into `dom(A_account(parent(t)))`.

*Termination of the A2 dispatch by induction on chain position.* The recursive dispatch from sub-case A2 back through A2 (and ultimately to A1) is well-founded under an explicit induction principle. Let `n(t) ∈ ℕ` denote the *chain position* of `t` within the emission sequence of `A_account(parent(t))` — i.e., the index `n` such that `t` is the `(n + 1)`-th address emitted by `A_account(parent(t))`, with `n = 0` for the first emission, `n = 1` for the second, and so on. By T10a.7 (EnumerationInjectivity, ASN-0034), distinct chain indices correspond to distinct addresses, so `n(t)` is well-defined. The induction is on `n(t)`:
  - *Base (n(t) = 0).* `t` is the first emission of `A_account(parent(t))`, placed by a K.δ k = 2 event (the T2 spawn step that activated `A_account(parent(t))`); dispatched by sub-case A1.
  - *Inductive step (n(t) ≥ 1).* `t = inc(t_prev, 0)` where `t_prev` is the prior emission of `A_account(parent(t))`, with `n(t_prev) = n(t) − 1 < n(t)`. By the inductive hypothesis applied to `t_prev`, the dispatch on `t_prev` terminates (reaching A1 in at most `n(t_prev) + 1` steps); the present A2 step adds one more, so dispatch on `t` terminates in at most `n(t) + 1` steps.
The induction is well-founded because `n(t) ∈ ℕ` and the dispatch reduces `n(·)` strictly at each A2 step. The chain position is bounded above by the number of K.δ k ∈ {0, 2} events in the system history that emitted onto `A_account(parent(t))` — a finite quantity by SequentialTransitionAxiom's total ordering of transitions and the finite age of the present state. In both A1 and A2, the resulting `t ∈ dom(A_account(parent(t)))` is preserved by T10a's per-allocator-chain monotonicity (allocators do not lose emissions: by AllocatedSet's tracked-domain monotonicity from ASN-0034, `domₛ(A_account(parent(t)))` grows monotonically across transitions) across every subsequent transition, so the present sub-case A event reads it directly. The discharge collapses to a single prior K.δ event in A1, and to a finite chain of K.δ k = 0 events terminating at A1 in A2 — sub-case A itself makes no recursive appeal (sub-case A spawns documents from account operands, not accounts from account operands, so no chain of sub-case A events leads back to a base; the A2 induction runs through K.δ k = 0 events, dispatched by the k = 0 case above). T2 admissibility against the state-tracked `A_account(parent(t))` then discharges in the standard T10a form. (Alternative direct discharge: by T10a.6 (DomainDisjointness, ASN-0034), every emission of `A_account(parent(t))` — whether placed by the activating T2 spawn or by a subsequent T1 sibling-increment — inhabits its tracked domain, and `t ∈ E` with `parent(t)` a node forces `t` to be such an emission independent of which K.δ event placed it. The case-split form above is preserved because it parallels the K.δ k ∈ {0, 1} discharges and makes the inductive structure explicit.) *Sub-case B: t is a non-bootstrap node.* Here the spawned allocator is `A_account(t)`, and the minting "allocator" of t is the *external* node-allocation registry (Nelson's hierarchical baptism / Gregory's granfilade; see NodeUniqueAllocation above), which is *not* a state component of `Σ`. The T2 spawnPt obligation here is therefore discharged not by a state-component query but by NodeUniqueAllocation clause (c) (Registry tracking) directly — for every reachable state Σ and every `t ∈ Σ.E_node`, `t` inhabits the registry's tracked domain by external commitment, which supplies the T2 spawnPt premise at the moment of the K.δ case (ii) k = 2 event with `t ∈ Σ.E_node` as operand. (The structural derivation through "every E_node entry arrived via prior K.δ case (i) → that event obtained t from the registry → therefore t is in the registry" is the *content* of clause (c); the clause names the closure explicitly so the discharge cites the axiom directly without reconstructing the chain at each invocation.) This is the protocol-layer counterpart to T10a's T2 admissibility, and the T2 discharge form is applied here against the external commitment rather than against any state-bearing component. NodeLineage's `n₀ ≼ t` is a structural consequence of NodeUniqueAllocation clause (b), not the discharge itself. *Sub-case C: t = n₀ bootstrap, no prior K.δ.* Here the spawned allocator is `A_account(n₀)`. `n₀` enters at `Σ₀` with no prior K.δ event whatsoever, so neither the inductive route of sub-case A nor NodeUniqueAllocation's prior-event clause of sub-case B applies. The T2-style spawnPt obligation for `A_account(n₀)` (the account sub-allocator under the bootstrap node) is instead discharged by NodeRegistryBootstrap's external commitment that `n₀` inhabits the node-allocation registry's tracked domain at `Σ₀` — supplied by initial-state axiom rather than by any state query. As with sub-case B, the T2 discharge form is applied here against an external protocol-layer commitment, not against a state-component query.

  In all three sub-cases (A, B, C) the K.δ precondition `t = parent(e) ∈ E` discharges the T2 spawnPt requirement against whichever allocator (or external commitment) is the minting source of t. The spawn step's `k' = 2 ∈ {1, 2}` is admissible by T10a's *direct* per-`(t, k')` uniqueness axiom (instantiated at `k' = 2`, within the axiom's stated `k' ∈ {1, 2}` regime — see the *Direct vs derived T10a uniqueness across sub-cases (gloss)* paragraph above); K.δ's case-level zeros bound `zeros(t) ≤ 1` discharges T10a's zero-count side condition (T10a admits k' = 2 when `zeros(spawnPt) ≤ 2`, satisfied a fortiori). T10a GlobalUniqueness on the parent allocator's tracked domain — now extended to include `e` as the spawned allocator's first emission — then delivers `e ∉ E`. For sub-cases B and C (the external node-allocation registry cases), T10a GlobalUniqueness's application to the freshly-emitted account `e` proceeds *within* the docuverse-layer allocator tree (since `A_account(t)` *is* a T10a sub-allocator with `t` as its base, sitting inside the docuverse state) — the external registry's role is exhausted at the spawnPt premise, after which the docuverse-layer T10a discipline takes over uniformly.

In all three K.δ case (ii) sub-cases, the row's "parent allocator's tracked domain" denotes the parent allocator at the moment of the K.δ event — which is either pre-activated (k = 0; k = 1 after first version) or activated by the K.δ event itself (k = 2; first k = 1 emission), with the parent allocator named explicitly above for each sub-case. T10a GlobalUniqueness then closes the freshness obligation directly. The per-sub-case uniqueness route — *direct* per-`(t, k')` uniqueness axiom for k ∈ {1, 2}, *derived* chain-advancement uniqueness at `(t, 0)` via FrontierEquivalence for k = 0 — is the load-bearing distinction summarised in the gloss above.
...
K.μ⁺_L adds exactly one mapping per firing, so the pairwise-distinctness clause that K.μ⁺ states for its newly added V-positions has no work to do here — the singleton `{v_ℓ ↦ ℓ}` is trivially in bijective correspondence with itself, and there are no "two distinct new mappings" to disagree on a shared V-position. We verify `v_ℓ ∉ dom(M(d))`, as required for M'(d) to be a proper extension preserving S2 (ArrangementFunctionality). The verification decomposes by subspace: by S3★-aux (SubspaceExhaustiveness), every `v ∈ dom(M(d))` satisfies `subspace(v) ∈ {s_C, s_L}`, so `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)`. We discharge `v_ℓ ∉ V_{s_L}(d)` and `v_ℓ ∉ V_{s_C}(d)` separately.
...
*Dependency chain at a glance.* The argument structure below has a fixed order of derivation, in which each step's input is the previous step's output. The chain is non-circular and load-bearing for both the K.μ~ link-subspace fixity result and the existence condition `|dom_C(M(d))| ≥ 2`. Each step lists the lemmas/properties it *consumes* (its inputs) and the property it *produces* (its output), making non-circularity verifiable in isolation:

  *Step (A) — Subspace preservation under π (from admissibility (i) + L14).*
    Consumes: admissibility (i) of K.μ~ [stipulates S3★(Σ') as part of the post-state invariant package — admissibility filters which π's K.μ~ admits; the mechanical realisation argument in Step (B) below confirms the K.μ⁻ + K.μ⁺ decomposition produces what admissibility stipulates], S3★(Σ) (inductive hypothesis at the pre-state), the bijection equation `M'(d)(π(v)) = M(d)(v)`, L14 at both states (`dom(C) ∩ dom(L) = ∅`), S3★-aux at both states.
    Produces: `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — bidirectional subspace preservation, derived in a single step from the admissibility-stipulated S3★(Σ') + S3★(Σ) + L14 (case analysis on `s_C → s_L` and `s_L → s_C` mismatches each reaches contradiction via L14, as detailed in the *Case `s_C → s_L`* and *Case `s_L → s_C`* paragraphs in the body below).
    *Does not consume:* link-subspace fixity, CL-UNIQ. Subspace preservation is the *root* of the K.μ~ dependency chain — every downstream step (B)–(E) builds on it.

  *Step (B) — Mechanical realisation of admissibility clause (i) by the K.μ⁻ + K.μ⁺ decomposition.*
    Consumes: Step (A)'s subspace preservation (specifically the content-side restriction `(A v ∈ V_{s_C}(d) :: subspace(π(v)) = s_C)` — the K.μ⁺ amendment in the full-clearance form requires the write set `{π(v) : v ∈ V_{s_C}(d)}` to have `subspace = s_C`, satisfied precisely when π preserves content-side subspace), S3★(Σ) (inductive hypothesis at the pre-state), K.μ⁻'s restriction effect (values unchanged on survivors), K.μ⁺ amendment (new positions have `subspace = s_C`), K.μ⁺'s precondition `M'(d)(v) ∈ dom(C)` for each new content-subspace mapping, S3★(Σ_int) at the intermediate state (link-subspace mappings retained by K.μ⁻'s restriction still target dom(L) since L is frame-preserved).
    Produces: mechanical confirmation that the K.μ⁻ + K.μ⁺ (full-clearance) decomposition realises the admissibility-stipulated S3★(Σ') for any π satisfying Step (A)'s subspace preservation. Together with the per-state invariant rows in the verification matrix, confirms admissibility (i)'s post-state invariant package is mechanically realisable by the decomposition.
    *Does not consume:* link-subspace fixity, CL-UNIQ. The route through K.μ⁻ + K.μ⁺ decomposition argues from each elementary step's effect/frame clauses applied independently to the link- and content-subspace partition of M(d), under Step (A)'s subspace preservation.

  *Step (C) — Link-subspace functional identity (Steps 1–3 below).*
    Consumes: Step (A)'s subspace preservation, K.μ~-FIX's per-subspace domain equality (which follows from D-SEQ★ at both endpoints plus subspace preservation — see K.μ~-FIX below), K.μ⁺ amendment (no link-subspace V-positions created), K.μ⁻'s value-preservation clause on survivors.
    Produces: `M'(d)|_{dom_L} = M(d)|_{dom_L}` as functions.
    *Does not consume:* CL-UNIQ. The functional identity is established from subspace preservation, the bijection's restriction to dom_L (a bijection between sets of equal cardinality both equal to `dom_L(M(d))`), and the K.μ⁻ + K.μ⁺ pointwise framing — no injectivity hypothesis is invoked.

  *Step (D) — Pointwise link-subspace fixity (Step 4 below).*
    Consumes: Step (C)'s functional identity, CL-UNIQ at the pre-state Σ (`M(d)|_{dom_L}` injective — the *inductive hypothesis* on Σ from the ExtendedReachableStateInvariants induction; Σ is reachable by hypothesis of the induction).
    Produces: `π(v) = v` for every `v ∈ dom_L(M(d))`.
    This is the *first* step in the chain that consumes CL-UNIQ; Steps (A)–(C) are independent of CL-UNIQ by construction.

  *Step (E) — Admissibility clause (ii) and the existence condition.*
    Consumes: Step (D)'s pointwise fixity on dom_L (so `π|_{dom_L} = id`), K.μ~-FIX (so `π` is a permutation of a fixed `dom(M(d))`), Step (A)'s subspace preservation (so `π` restricts to a bijection on `dom_C(M(d))`), admissibility clause (ii) (`π ≠ id`).
    Produces: existence condition `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (ii) to admit a witness `π` once link-subspace fixity has reduced (ii) to the existence of a non-trivial permutation on `dom_C(M(d))`.

The chain reads (A) → (B) → (C) → (D) → (E), with no back-references. Step (A) derives subspace preservation in a single step from admissibility (i)'s stipulated S3★(Σ') together with S3★(Σ) and L14; Step (B) then mechanically confirms the K.μ⁻ + K.μ⁺ decomposition produces what admissibility stipulates, consuming Step (A)'s subspace preservation as input (the K.μ⁺ amendment in the full-clearance form requires content-side subspace preservation to satisfy its write-set constraint, which Step (A) supplies). Steps (C)–(D) establish link-subspace fixity (Step (C)'s functional identity, Step (D)'s pointwise fixity via CL-UNIQ); Step (E) closes admissibility (ii)'s existence condition. The matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~ corresponds to Step (B)'s mechanical-realisation conclusion; the chain makes explicit that admissibility (i) is the stipulation source, not a derived consequence of preconditions, and that the decomposition mechanically *realises* the stipulation rather than deriving it from first principles. Each step appears below in the same order; the prose substantiates each link explicitly.

*Proof of Step (A) — Subspace preservation under π from admissibility (i) + L14.* The *Case `s_C → s_L`* and *Case `s_L → s_C`* paragraphs below the *Preconditions of K.μ~* paragraph supply the full case analysis; we summarise the derivation here. Admissibility clause (i) of K.μ~ stipulates that the post-state `M'(d)` satisfies S3★(Σ') as part of its post-state invariant package — the stipulation is part of the admissibility filter on π, not a derived consequence of preconditions. From S3★(Σ) (inductive hypothesis) and the admissibility-stipulated S3★(Σ'), the bijection equation `M'(d)(π(v)) = M(d)(v)`, and L14 at both states (`dom(C) ∩ dom(L) = ∅`), subspace preservation under π follows in a single step by case analysis on the two possible mismatch forms (S3★-aux constrains `subspace(v) ∈ {s_C, s_L}` everywhere). Each mismatch — `s_C → s_L` (content-subspace `v` mapped to link-subspace `π(v)`) and `s_L → s_C` (link-subspace `v` mapped to content-subspace `π(v)`) — reaches contradiction at `dom(C) ∩ dom(L) = ∅` by L14: the bijection equation routes the value through the wrong-subspace clause of S3★, and L14's disjointness rules out the resulting joint membership. The derivation is symmetric under the L14 intersection's commutativity, and S3★-aux's two-subspace exhaustion supplies no third case. The full case-by-case proof is in the body's *Case `s_C → s_L`* and *Case `s_L → s_C`* paragraphs. ∎ (Step (A))

*Proof of Step (B) — Mechanical realisation of admissibility (i)'s stipulated S3★(Σ') by the K.μ⁻ + K.μ⁺ decomposition.* We show that the K.μ⁻ + K.μ⁺ (full-clearance) decomposition mechanically realises the admissibility-stipulated S3★(Σ') at the K.μ~ post-state `Σ' = (C, L, E, M', R)`, consuming Step (A)'s subspace preservation as the input that lets K.μ⁺'s amendment fire on the full-clearance write set. Let `Σ_int = (C, L, E, M_int, R)` denote the intermediate state between the K.μ⁻ and K.μ⁺ atomic steps of the decomposition. Three sub-claims, dispatched in order:
  ...
  *(B.3) S3★(Σ').* The post-state arrangement `M'(d)` is the union of two disjoint contributions: (i) survivors of K.μ⁻ that K.μ⁺ frames unchanged — these inherit S3★ from S3★(Σ_int) at B.1; (ii) newly written content-subspace positions from K.μ⁺ — these satisfy S3★'s content clause directly by B.2. `dom(C') = dom(C)` and `dom(L') = dom(L)` by the composite frame. Combining (i) and (ii) over every `v ∈ dom(M'(d))` discharges S3★(Σ') for the affected document `d`; the framed arrangements `M'(d') = M(d')` for `d' ≠ d` inherit S3★ from S3★(Σ) at the pre-state directly. ∎ (Step (B))

The argument consumes Step (A)'s subspace preservation (specifically the content-side restriction at B.2) but does not consume link-subspace fixity (Steps (C)/(D)) or CL-UNIQ. Step (B)'s mechanical realisation produces what admissibility (i) stipulates — confirming the decomposition is a valid realisation mechanism — without circular dependence on the downstream fixity arguments. Steps (C), (D), (E) follow in the same order in the prose below — the *Case `s_C → s_L`* / *Case `s_L → s_C`* case analysis that Step (A) summarises appears immediately after the bijection equation; Steps (C) and (D) appear as Steps (1)–(3) and Step (4) of the *Link-subspace fixity* proof; Step (E) appears in the *Decomposition* paragraph below.

*Preconditions of K.μ~.* The operation has two explicit preconditions:
- `d ∈ E_doc`
- `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (ii) `π ≠ id` to admit a witness. The two directions enter the argument independently and are kept separate here so the reader sees what each consumes:
  - *Necessity.* `|dom_C(M(d))| ≤ 1` rules out clause (ii) directly. The singleton case `|dom_C(M(d))| = 1` admits only the identity permutation on `dom_C`, and link-subspace fixity forces `π|_{dom_L} = id`, so π = id, violating (ii). The empty case `dom_C(M(d)) = ∅` is doubly excluded — see *Decomposition* below for the full derivation. Necessity uses only link-subspace fixity (established earlier in the dependency chain at Step (D)) plus elementary counting on bijections of singletons and the empty set; no forward result is invoked.
  - *Sufficiency (explicit construction).* When `|dom_C(M(d))| ≥ 2`, an admissible π always exists; we exhibit a concrete witness. Pick any two distinct elements `v₁, v₂ ∈ dom_C(M(d))` (existence supplied by the cardinality bound). Define the *transposition witness*
    ...
    - *Clause (ii) (`π ≠ id`).* `π_swap(v₁) = v₂ ≠ v₁` (the two elements are distinct by construction), so `π_swap` differs from the identity at `v₁`.
    ...
    - *Clause (i) (post-state invariants on `M'(d)`).* By K.μ~-FIX (derived below in *Decomposition of K.μ~*), `dom(M'(d)) = dom(M(d))`, so `S8a`, `S8-depth`, `S8-fin`, `D-CTG★`, `D-MIN★`, `D-SEQ★` at the post-state inherit unchanged from the pre-state's per-state hypothesis (the V-position domain is fixed). `S3★` at the post-state: at every `v ∈ dom(M(d))`, `M'(d)(π_swap(v)) = M(d)(v)` by the bijection equation; subspace preservation routes content-subspace mappings to `dom(C')` and link-subspace mappings to `dom(L')` exactly as at the pre-state, so `S3★` carries forward.
    Hence `π_swap` is admissible, and the full-clearance form (`n'_{s_C} = 0`) of the K.μ⁻ + K.μ⁺ decomposition realises it (the full-clearance form is admissible for *every* admissible π without per-π precondition checks — established at *Decomposition of K.μ~* below, under the *Non-determinism of K.μ~ realisations* paragraph). The transposition witness is the simplest one; many other admissible π exist, but a single witness suffices for the sufficiency obligation.

  Stated here at the definition site as a caller-checked precondition: a transition with `|dom_C(M(d))| ≤ 1` does not fire, and the caller is responsible for verifying this condition before invoking K.μ~. Equivalently, the operation's discharge of admissibility clause (ii) is a sufficiency obligation that the operation realises by exhibiting an admissible `π`; the precondition makes that obligation discharge via the full-clearance form.
...
π is admissible iff (i) the induced post-state `M'(d)` would satisfy S8a, S8-depth, D-CTG★, D-MIN★, and S3★, and (ii) `π ≠ id`. Clause (ii) makes K.μ~ a real reordering: a permutation whose net effect is the identity is not a K.μ~ transition (the system simply does not change). S8a at every `π(v)` is part of clause (i)'s post-state invariant package on M'(d); K.μ~-FIX (derived below) gives `dom(M'(d)) = dom(M(d))`, so every `π(v) ∈ dom(M(d))` inherits S8a from the inductive hypothesis at Σ unconditionally — no separate per-`π(v)` check is needed. Subspace preservation — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — is the content of Step (A) of the dependency chain above: admissibility (i) stipulates S3★(Σ') as part of its post-state invariant package (admissibility *filters* which π's K.μ~ admits, with the stipulation that the post-state must satisfy S3★, S8a, S8-depth, D-CTG★, D-MIN★ on M'(d)); under that stipulation, S3★(Σ') together with S3★(Σ) (inductive hypothesis), L14, and the bijection equation force subspace preservation in a single step. The K.μ⁻ + K.μ⁺ decomposition then mechanically realises the admissibility-stipulated S3★(Σ') (Step (B)) by consuming Step (A)'s subspace preservation as input — the K.μ⁺ amendment in the full-clearance form requires its write set `{π(v) : v ∈ V_{s_C}(d)}` to have `subspace = s_C`, satisfied precisely when π preserves content-side subspace. The matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~ corresponds to Step (B)'s mechanical-realisation conclusion. S3★-aux constrains `subspace(v) ∈ {s_C, s_L}` and `subspace(π(v)) ∈ {s_C, s_L}` at every reachable state, so subspace mismatch under π takes exactly one of two complementary forms: a content-subspace `v` mapped to a link-subspace `π(v)` ("`s_C → s_L`"), or a link-subspace `v` mapped to a content-subspace `π(v)` ("`s_L → s_C`"). We discharge both cases by contradiction.
...
Both cases close by the same L14-driven disjointness; the cases are mechanically symmetric under the L14 intersection's commutativity (`dom(C) ∩ dom(L) = dom(L) ∩ dom(C)`), and S3★-aux's two-subspace exhaustion supplies no third case to consider.

**Non-determinism of K.μ~ realisations.** The admissibility conditions (i)–(ii) above identify a *family* of valid (π, K.μ⁻ + K.μ⁺) realisations for any given pre-state — *not* a single canonical decomposition. Once an admissible π is fixed, multiple K.μ⁻ + K.μ⁺ pairs can produce the same net post-state by choosing different K.μ⁻ removal cardinalities and corresponding K.μ⁺ rebuilds. Two named realisation forms partition the choice space:

- *Full-clearance form* (`n'_{s_C} = 0`): K.μ⁻ removes the entire content subspace `V_{s_C}(d)` (retaining the link subspace pointwise via link-subspace fixity); K.μ⁺ then rebuilds the full content arrangement under π in a single step. This form is admissible for *every* admissible π — the K.μ⁻ step's per-subspace suffix-removal precondition is satisfied vacuously (the full-subspace suffix), and K.μ⁺ writes `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` at fresh positions disjoint from the post-K.μ⁻ content domain.

- *Partial-suffix forms* (`n'_{s_C} = k₀ − 1` for some `k₀ ≥ 1`, i.e., retaining the prefix `{[s_C, 1, ..., 1, k] : 1 ≤ k < k₀}` and rebuilding the suffix `{[s_C, 1, ..., 1, k] : k₀ ≤ k ≤ n_{s_C}}`): admissible only when π satisfies the *below-cut value-preservation condition* `(A u ∈ V_{s_C}(d) : u < [s_C, 1, ..., 1, k₀] under the V-ordering on s_C : M(d)(u) = M(d)(π⁻¹(u)))`. K.μ⁻ retains the prefix with original values; K.μ⁺ rebuilds the suffix with π applied. When π violates the condition at every candidate cut `k₀ ≥ 2`, only the `k₀ = 1` specialisation — the full-clearance form — remains admissible.

The full-clearance form serves as the *universally-applicable realisation*: every K.μ~ verification argument in this ASN that does not name a specific cut point invokes the full-clearance form, sidestepping per-π admissibility analysis. Partial-suffix forms are an optimisation available when π preserves a contiguous suffix of below-cut values; the *Decomposition of K.μ~* paragraph below substantiates both the admissibility condition and its relation to π's pointwise action on `dom_C`. The verification matrix entries for K.μ~ (under ExtendedReachableStateInvariants below) cite "the K.μ⁻ + K.μ⁺ decomposition" without qualifying the cut — under the convention adopted here, these entries should be read as invoking the full-clearance form, which is always available regardless of π's structure on `dom_C`.
...
(4) *Identity via CL-UNIQ at the pre-state.* From (3), `M'(d)|_{dom_L} = M(d)|_{dom_L}`, so for the V-position `v ∈ dom_L(M(d))` under consideration, `M(d)(v) = ℓ`. Subspace preservation places `π(v) ∈ dom_L(M(d))` (using (1)'s cardinality equality `dom_L(M'(d)) = dom_L(M(d))`), and the bijection equation gives `M(d)(π(v)) = M'(d)(π(v)) = M(d)(v) = ℓ` (the first equality by (3) applied at `π(v) ∈ dom_L`). Both `v` and `π(v)` are link-subspace V-positions in `dom(M(d))` mapping to the same link `ℓ`. CL-UNIQ at Σ — link-subspace injectivity of `M(d)|_{dom_L}` — forces `π(v) = v`. The CL-UNIQ premise at Σ is supplied by the *per-state hypothesis of the ExtendedReachableStateInvariants induction* (which establishes the Class (a) invariants — including CL-UNIQ — at every reachable state): Σ is reachable by the outer induction's framing (the K.μ~ event under consideration is a transition `Σ →* Σ'`, so Σ is the pre-state of a valid composite from `Σ₀`), and CL-UNIQ is preserved at every reachable state including Σ. The reference to "the inductive hypothesis" in this proof denotes precisely this per-state hypothesis on Σ, not a self-referential appeal within the K.μ~ derivation. ∎

*Dual consequence — post-state CL-UNIQ preservation.* The functional identity `M'(d)|_{dom_L} = M(d)|_{dom_L}` established in Step (3) discharges *two* downstream obligations, not only K.μ~'s link-subspace fixity postcondition. (a) *Pointwise fixity:* Step (4) consumes (3) together with CL-UNIQ at the pre-state Σ to force `π(v) = v` pointwise. (b) *Post-state CL-UNIQ preservation:* equal functions share injectivity profiles directly, so `M(d)|_{dom_L}` injective (CL-UNIQ at Σ — the inductive hypothesis) and `M'(d)|_{dom_L} = M(d)|_{dom_L}` (Steps 1–3) jointly give `M'(d)|_{dom_L}` injective, i.e., CL-UNIQ at Σ'. The CL-UNIQ row of the Class (a) verification matrix in ExtendedReachableStateInvariants cites Steps 1–3 specifically — the functional identity is what carries CL-UNIQ forward, with no recourse to Step (4)'s pointwise identity needed for the CL-UNIQ obligation itself.

**Decomposition.** Admissibility clause (ii) requires `π ≠ id`. Combined with link-subspace fixity (which forces `π|_{dom_L} = id`) and subspace preservation (which forces `π` to map `dom_C(M(d))` bijectively onto itself, with cardinality `|dom_C(M(d))|`), `π ≠ id` requires a non-trivial permutation of `dom_C(M(d))` to exist. A set of cardinality ≤ 1 admits only the identity permutation, so the necessary-and-sufficient existence condition is `|dom_C(M(d))| ≥ 2`. The weaker statement `dom_C(M(d)) ≠ ∅` is necessary but not sufficient: the singleton case `|dom_C(M(d))| = 1` admits no non-identity bijection (K.μ~-FIX forces `dom_C(M(d')) = dom_C(M(d))` of the same cardinality, and any bijection on a singleton maps the unique element to itself, yielding `π = id` against (ii)). *The empty case* `dom_C(M(d)) = ∅` *is doubly excluded:* (a) the only bijection on `∅` is the empty function `∅ → ∅`, which IS the identity (the empty restriction of `id` to the empty domain), violating (ii); (b) when `dom_C(M(d)) = ∅ ∧ dom_L(M(d)) = ∅` — i.e., `dom(M(d)) = ∅` — the K.μ⁻ + K.μ⁺ decomposition is itself blocked, because K.μ⁻'s effect clause `dom(M_int(d)) ⊂ dom(M(d))` is unsatisfiable on an empty pre-state domain (no proper subset of `∅` exists). So even if a non-trivial decomposition were available structurally, K.μ⁻ would not fire. *Mixed case* (`dom_C(M(d)) = ∅` with `dom_L(M(d)) ≠ ∅`): excluded by (a) alone — π must be a bijection on `dom(M(d)) = dom_L(M(d))`; link-subspace fixity forces `π|_{dom_L} = id`; hence π = id (the bijection is identity on every element of its domain), violating (ii). K.μ~ cannot reorder link-subspace V-positions even when content is empty: link-subspace fixity is uniform across the existence condition, not contingent on content-subspace non-emptiness, so a non-empty link subspace alone cannot supply the non-identity permutation that (ii) demands. K.μ~ therefore cannot fire when `|dom_C(M(d))| ≤ 1` (which subsumes the empty case both unmixed and mixed, and the singleton case on the content subspace); the existence of a non-identity bijection at `|dom_C(M(d))| ≥ 2` is itself a sufficiency obligation the operation discharges by exhibiting an admissible `π`. When the existence condition holds, K.μ~ is realised as *any* valid K.μ⁻ + K.μ⁺ pair on `V_{s_C}(d)` whose net effect achieves the bijection equation for π, subject to K.μ⁻'s admissibility (per-subspace suffix removal under D-CTG★/D-MIN★) and K.μ⁺'s preconditions at the intermediate state. The cardinality of the K.μ⁻ removal — equivalently, the choice of `n'_{s_C} ∈ {0, 1, ..., n_{s_C} − 1}` — depends on which content-subspace positions π actually moves:
...
- `inc(t, 0) = 1.2.0.1.0.2 ∉ E₃`: discharged via FrontierEquivalence at the present pre-state `Σ₃ = (C₃, L₃, E₃, M₃, R₃)`. The three load-bearing premises of FrontierEquivalence chain as follows. (i) *T10a chain-advancement uniqueness at `(t, 0)` (derived form)*: TA5(c) gives `inc(t, 0) = 1.2.0.1.0.2` as a determinate single address, and Step 3 is the only prior K.δ event on `A_doc(1.2.0.1)`'s chain — it activated `A_doc(1.2.0.1)` with `t = 1.2.0.1.0.1` as the first emission, but no subsequent K.δ k = 0 event has fired on `A_doc(1.2.0.1)`'s frontier. (ii) *P1 (E-monotonicity)*: had any prior K.δ event fired `(t, 0)`, P1 would have placed `1.2.0.1.0.2` permanently in E; since `1.2.0.1.0.2 ∉ E₃` operationally, no prior firing occurred. (iii) *T10a GlobalUniqueness via T10a.6*: no allocator *other than* `A_doc(1.2.0.1)` can produce the address `1.2.0.1.0.2` — by T10a.6 (DomainDisjointness), distinct allocators have disjoint tracked domains, and `1.2.0.1.0.2` extends the prefix `[1, 2, 0, 1]` under T10a.6's prefix-incomparability constraint between non-lineage allocators. FrontierEquivalence's biconditional then confirms `inc(t, 0) ∉ E₃` iff `t` is the frontier of `A_doc(1.2.0.1)`'s (t, 0)-branch — which it is, since `t` is the most recent (and sole prior) emission on that chain.
...
**Class (a): Per-state invariants** — preserved by each elementary transition individually, holding at every reachable state including intermediate states within composites. These are all per-state properties except P4★, P4a, and P7a: S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8★, C-fin, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, NodeLineage, L0, L1, L1a, L1b, L3, L14, L-fin, CL-OWN, CL-UNIQ.
...
*Frame entries against link-store invariants under amended transitions.* `frame` entries against the link-store invariants (L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, CL-UNIQ) for the transitions K.α, K.μ⁺, K.μ⁻ refer to the amended forms introduced in the *Amendments to existing transitions* section above (the K.μ⁺ amendment and K.μ⁻ amendment defined there as local strengthenings, and the inherited K.α `E(a)₁ = s_C` precondition documented under *K.α (no local amendment in extended state)*). These amendments extend each original four-component-state frame with the explicit `L' = L` conjunct; the L-related rows discharge to "frame" on the strength of that amendment clause (or, in K.α's case, on the strength of ASN-0093's K.α frame supplying `L' = L` directly), not on the original (pre-link-store) transition's frame, which made no commitment about L. The matrix labels these uniformly as "frame" without footnote markers; readers cross-referencing against the original (pre-amendment) definitions should consult the amended forms instead. This note is the single normative discharge gloss for every link-row "frame" cell under K.α, K.μ⁺, and K.μ⁻ in the matrix below.

*K.μ~ entries invoke the full-clearance form of the K.μ⁻ + K.μ⁺ decomposition.* Every K.μ~ matrix cell below — whether it names "K.μ⁻ (full-clearance)" explicitly or uses an unqualified variant like "K.μ⁻ restriction + K.μ⁺ amendment alone" or "inherits via decomposition" — invokes the *full-clearance form* (`n'_{s_C} = 0`) introduced in *Decomposition of K.μ~* above. This is the universally-applicable realisation in which K.μ⁻ removes the entire content subspace `V_{s_C}(d)` (retaining the link subspace pointwise via link-subspace fixity) and K.μ⁺ then rebuilds the full content arrangement under π in a single step. The full-clearance form is admissible for *every* admissible π without per-π analysis: K.μ⁻'s per-subspace suffix-removal precondition holds vacuously at the full-subspace suffix (`k₀ = 1`), and K.μ⁺ writes the rebuild at fresh positions. The body convention adopted at *Decomposition of K.μ~* — "every K.μ~ verification argument in this ASN that does not name a specific cut point invokes the full-clearance form" — applies uniformly across the matrix. Partial-suffix forms (`n'_{s_C} = k₀ − 1` for some `k₀ ≥ 2`) are an optimisation available when π satisfies the below-cut value-preservation condition (see *Decomposition of K.μ~* for the precise statement); they are not used in any matrix entry below, since the full-clearance form discharges every entry without requiring per-π admissibility analysis. This note is the single normative discharge gloss for every K.μ~ cell in the matrix below.

*K.μ~ admissibility-stipulated invariants vs. mechanically-derived invariants.* Within the K.μ~ column, the post-state invariant package listed in admissibility clause (i) of K.μ~ — `S3★, S8a, S8-depth, D-CTG★, D-MIN★` on `M'(d)` — is *stipulated* by admissibility (the admissibility predicate filters which π's K.μ~ admits, requiring the induced post-state to satisfy these), not *derived* from preconditions by mechanical preservation as the K.α / K.δ / K.μ⁺ / K.ρ columns derive them. Cells in those rows carry the label "stipulated by admissibility (i)" together with the mechanical-realisation argument confirming the K.μ⁻ + K.μ⁺ decomposition produces what admissibility stipulates. Other K.μ~ cells (S2, S3★-aux, S8★, S8-fin, CL-OWN, CL-UNIQ, and the derived D-SEQ★) are *not* in admissibility (i)'s stipulation list and are genuinely derived from the decomposition's elementary effects under the inductive hypothesis and (where named) link-subspace fixity. The distinction matters: for admissibility-stipulated invariants, the K.μ⁻ + K.μ⁺ decomposition's role is to *realise* the stipulation (Step (B) of the dependency chain), not to derive it from elementary preconditions; the realisation argument may itself consume Step (A)'s subspace preservation (which is derived from admissibility (i) + L14 in a single step). For mechanically-derived invariants, the decomposition's elementary effects directly produce the post-state property without admissibility's filter playing a role beyond selecting which π's are admissible candidates.

*Entity-distinctness and Link-distinctness rows are derived allocator-discipline obligations, not separately-named Class (a) invariants.* The matrix tracks three distinctness obligations: S4 (the ASN-0036 content-distinctness invariant, restricted strictly to `dom(C)`), Entity distinctness (the per-K.δ uniqueness obligation `e ∉ E`), and Link distinctness (the per-K.λ uniqueness obligation `ℓ ∉ dom(L) ∪ dom(C)`, equivalent to ASN-0043's L11a in the extended state). Only S4 appears in the Class (a) per-state invariant conjunction above; Entity distinctness and Link distinctness appear in the matrix to record each transition's discharge of the corresponding allocator-discipline freshness premise, but they are *derived* consequences of the K.δ and K.λ preconditions plus the T10a / SubAllocatorAxiom / NodeUniqueAllocation discharge chain rather than standalone named invariants of this ASN. They share the matrix's verification framework — per-elementary discharge with `frame` cells for non-affecting transitions — because their preservation argument has the same shape as the Class (a) invariants' preservation arguments.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | stipulated by admissibility (i); the K.μ⁻ (full-clearance) + K.μ⁺ decomposition mechanically realises both clauses under Step (A)'s subspace preservation (Step (B) of the dependency chain at *Decomposition of K.μ~*); link-subspace fixity is downstream of both Steps (A) and (B), not prerequisite to either |  frame |
...
| S4 (content distinctness, per ASN-0036) | T10a GlobalUniqueness on A_C(origin(a)) (subsequent) or SubAllocatorAxiom.FirstEmission (first) | frame (does not add to dom(C)) | frame (does not add to dom(C)) | frame (no new addresses) | frame | frame | frame | frame |
...
| Link distinctness (derived; distinct K.λ events produce distinct link addresses — the L11a obligation in the extended state) | frame (does not add to dom(L)) | frame (does not add to dom(L)) | SubAllocatorAxiom.FirstEmission (first) or T10a GlobalUniqueness on A_L(d) (subsequent) | frame | frame | frame | frame | frame |
...
The matrix is a navigational index; each cell summarises the load-bearing argument. The detailed per-invariant prose below substantiates each matrix entry, in particular for invariants whose discharge mechanism is non-trivial. (The matrix-preamble note above — *Frame entries against link-store invariants under amended transitions* — covers the link-row "frame" cells under K.α, K.μ⁺, and K.μ⁻ uniformly; the prose below does not repeat its content per row.)
...
*S7c (Element-field depth).* `#E(a) ≥ 2`: K.α's allocator chain produces `E(a) = [s_C, k]` with `k ≥ 1`, so `#E(a) ≥ 2`; preserved by P0.
...
*S8a, S8-depth, S8-fin.* Established at arrangement-extending transitions: K.μ⁺ amendment's preconditions on new V-positions (positivity, depth, finiteness preserved by finite extension); K.μ⁺_L preconditions (positivity, `m_L = 2`, finite extension); K.μ⁻ restriction preserves all three; K.μ~ inherits via decomposition; all others hold M in frame.

*S8★ (Per-subspace span decomposition).* Established per-subspace by the two routes specified at S8★'s definition. *Content subspace:* S8★(s_C) follows from ASN-0036's S8 applied to the projection `M(d')|_{V_{s_C}(d')} : V_{s_C}(d') → dom(C')` (S3★'s content clause is exactly S3 restricted to V_{s_C}(d'); S2/S7b/S7c/S8a/S8-depth/S8-fin are elementary-preserved as above). *Link subspace:* S8★(s_L) is discharged by the *trivial length-1 decomposition* `{(v, M(d')(v), 1) : v ∈ V_{s_L}(d')}` directly, not by ASN-0036's S8 — its S3 precondition (and S7b/S7c) does not hold on the link-subspace projection because the target is `dom(L)` not `dom(C)`. Every link-subspace V-position constitutes its own singleton correspondence run, satisfying S8's conditions (a) and (b) on singletons by construction; finiteness of the decomposition follows from S8-fin. Depth-2-specific (LinkVPositionDepthAxiom, `m_L = 2`) shift-aligned decompositions are admissible — for any `v ∈ V_{s_L}(d')` of form `[s_L, k]`, a sequence `{[s_L, k], [s_L, k+1], …, [s_L, k+n-1]}` mapping shift-aligned to `{M(d')([s_L, k+i]) : 0 ≤ i < n}` constitutes an n-element correspondence run — but they are not required: the trivial form suffices for the existence postcondition under K.μ⁺_L and every other transition. Each arrangement-extending or contracting transition discharges S8★ at its post-state by these per-subspace projections; others hold M in frame.

*D-CTG★ / D-MIN★.* K.μ⁺'s original precondition list (stated at the K.μ⁺ definition in *Elementary transitions*) requires the resulting `M'(d)` to satisfy D-CTG and D-MIN, strengthened to D-CTG★ / D-MIN★ in the extended state — contiguity and minimum-position preservation on the extended content subspace is therefore discharged by precondition, not by the K.μ⁺ amendment (which adds only the `subspace(v) = s_C` restriction). K.μ⁺_L preconditions cover the D-CTG case (non-empty link subspace) and the D-MIN case (empty); K.μ⁻'s constructive precondition specifies the per-subspace retention count `n'_S` and produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` directly, satisfying D-CTG★ and D-MIN★ at Σ' by construction (equivalence with the post-state invariant characterization proved in *K.μ⁻ admissible contraction shape*); K.μ~ inherits via decomposition; all others hold M in frame.
...
| P4a (every R-entry witnessed by past content-subspace containment) | J1'★ at boundary supplies content-subspace witness `v` with `M'(d)(v) = a` at the post-state Σ' itself | May transiently fail at any intermediate state where K.ρ has fired before a matching K.μ⁺ has fired within the same composite. ValidComposite★ does *not* enforce an ordering K.μ⁺ ⤳ K.ρ — orderings such as K.α → K.ρ → K.μ⁺ satisfy every elementary precondition (K.ρ requires only `a ∈ dom(C) ∧ d ∈ E_doc`, both established by the preceding K.α), and produce an intermediate state at which the fresh `(a, d) ∈ R` has no witnessing content-subspace V-position in any current arrangement. Restoration at the composite boundary is *not* by re-deriving from history but by the post-state itself: at Σ', J1'★ requires `(E v ∈ dom(M'(d)) : subspace(v) = s_C ∧ M'(d)(v) = a)`, so Σ' itself is the witnessing state for the P4a quantifier `(E Σ_k in the transition history : ...)` — the K.μ⁺ that ultimately fires in the composite supplies the witness in `M'(d)` directly. The "transient failure" thus describes a real observable mid-composite state; restoration is structural (Σ' carries the witness), not temporal (no earlier state need carry it). |
...
| NodeUniqueAllocation | Axiom: every K.δ node-allocation event produces (a) e ∉ E (freshness, closing the GlobalUniqueness chain for nodes where T10a does not apply) and (b) n₀ ≼ e (bootstrap lineage, supplying K.δ case (i)'s lineage precondition and inductively preserving NodeLineage); and (c) registry tracking — every `t ∈ Σ.E_node` inhabits the external registry's tracked domain at every reachable state, supplying the T2 spawnPt premise at K.δ case (ii) k = 2 sub-case B. The three conditions the node-allocation registry must satisfy at every node-allocation event |
| NodeRegistryBootstrap | Axiom (BootstrapRegistrySeeding): at Σ₀, n₀ is committed to the *external* node-allocation registry's tracked domain. Initial-state commitment grounding the T2-style spawnPt discharge in K.δ case (ii) k = 2 with operand t = n₀ (first account under n₀), where no prior K.δ event for n₀ exists to supply registry membership via NodeUniqueAllocation. The registry is *not* a component of Σ — it is an external protocol-layer construct, and the axiom is an external commitment rather than a predicate over a state component; the spawnPt obligation is discharged against this external commitment, not against a state-component query |
| FrontierEquivalence | Derived lemma: `inc(t, 0) ∉ Σ.E ⟺ t is the frontier of its sub-allocator's (t, 0)-branch`, for every reachable Σ and every `t ∈ Σ.E` with `¬IsNode(t)`. Three load-bearing premises: T10a chain-advancement uniqueness at `(t, 0)` (itself a derived consequence of TA5(c) functional determinism + P1 + operational precondition, since T10a's direct per-`(t, k')` uniqueness axiom is stated for child-spawning `k' ∈ {1, 2}` only; T10a.7 (EnumerationInjectivity) plays only the framing role of identifying chain enumeration structure, not the determinism source), P1 E-monotonicity, and T10a GlobalUniqueness (via T10a.6). Counterexample to T4b-based identification: T4b's `parent`/`zeros`/length stratification does not in general identify t as the frontier of its own sub-allocator. Cited by the K.δ case (ii) k = 0 rationale and freshness discharge, and by the S4 row of the Class (a) verification matrix |
...
The K.δ-ID identities below are *derived consequences* of TA5 (HierarchicalIncrement, ASN-0034) and T4b (UniqueParse, ASN-0034) applied to `e = inc(t, k)`, *not* primitive specifications introduced by this ASN. They are listed under separate naming so downstream sites — verification matrix entries, the multi-version invariant chain, P8 preservation arguments — can cite them by name (K.δ-ID.zeros-0/1, K.δ-ID.zeros-2, K.δ-ID.parent-0/1, K.δ-ID.parent-2) without unpacking the TA5 / T4b derivation chain at each invocation. The "Derivation" column names the load-bearing foundation property; the "Statement" column gives the identity itself.

| Label | Statement | Derivation |
|-------|-----------|------------|
| K.δ-ID.zeros-0/1 | `zeros(e) = zeros(t)` for k ∈ {0, 1} on `e = inc(t, k)` | TA5(c) preserves zeros for k = 0 (sibling-increment at the rightmost nonzero component); TA5(d) at k = 1 appends a final `1` with no new zero, so the zero count is unchanged. Cited by the multi-version invariant chain in K.δ k = 1 discharge and by P8 preservation under K.δ k ∈ {0, 1} |
| K.δ-ID.zeros-2 | `zeros(e) = zeros(t) + 1` for k = 2 on `e = inc(t, 2)` | TA5(d) at k = 2 appends one zero separator and a final `1`. Discharges the K.δ k = 2 zeros bound `zeros(t) ≤ 1` from the case-level `¬IsElement(e)` and provides the increment used in S7d preservation |
| K.δ-ID.parent-0/1 | `parent(e) = parent(t)` for k ∈ {0, 1} on `e = inc(t, k)` | T4b's parent projection's truncation past the last separator is preserved by both k = 0 and k = 1 increments — k = 0 leaves the trailing-component position unchanged; k = 1 extends by one non-zero component without crossing a zero separator. Discharges K.δ's case-level `parent(e) ∈ E` requirement when combined with P8 at `t` |
| K.δ-ID.parent-2 | `parent(e) = t` for k = 2 on `e = inc(t, 2)` | TA5(d) at k = 2 introduces a new zero separator immediately after t, and T4b's parent projection truncates past that new separator, making t itself the parent prefix. Discharges K.δ's case-level `parent(e) ∈ E` requirement directly from `t ∈ E` |
...
| K.α's `E(a)₁ = s_C` precondition (inherited) | Inherited from ASN-0093's K.α directly — not a local amendment. The precondition pins `subspace_I(a) = s_C`, which downstream sites cite to preserve L0's C-clause and L14 in the extended state. The *K.α (no local amendment in extended state)* paragraph at the head of *Amendments to existing transitions* notes that this ASN intentionally introduces no local change; downstream citations refer to the inherited precondition by name | Inherited from ASN-0093's K.α (ContentAllocation) precondition |
