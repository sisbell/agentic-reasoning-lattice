...
The per-state arrangement invariants S2, S3★, S8a, S8-depth, S8-fin, D-CTG★, D-MIN★, and D-SEQ★ at M'(d) are *derived consequences* of the restriction form, not separate preconditions; they are established in *K.μ⁻ admissible contraction shape* below. The strict-contraction constraint `(E S :: n'_S < n_S)` combined with the constructive form yields `dom(M'(d)) ⊂ dom(M(d))` (proper subset at the whole-arrangement level), discharging the effect clause.
