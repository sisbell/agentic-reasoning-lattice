...
Admissibility clause (i) *stipulates* S3★(Σ') as a filter on which π the operation admits — it is an assumed condition on π, not yet a discharged one. The substantive verification that this filter is realisable is Step (B.3) below, which discharges S3★(Σ') by exhibiting the K.μ⁻ + K.μ⁺ post-state. Step (B) as a whole shows the decomposition realises an admissible π, making K.μ~ non-vacuous.
