...
This is a derived quantity of the state — it captures what each document currently displays. The unscoped relation is used below only in the reordering-isolation argument (J3) and the staleness discussion; the provenance bound is carried not by `Contains(Σ)` but by its content-subspace restriction `Contains_C` (defined later, *Content-scoped containment and provenance*), which P4★ constrains. The unscoped relation is explicitly *not* bounded by R once any link is arranged — `Contains(Σ) ⊆ R` is unsatisfiable in the extended state (shown below) — so it is a derived quantity, not a state invariant.
...
| FrontierEquivalence | Derived lemma: `inc(t, 0) ∉ Σ.E ⟺ t is the frontier of A's (t, 0)-branch` (where A is the allocator whose tracked chain contains t, unique by T10a.6), for every reachable Σ and every `t ∈ Σ.E` with `¬IsNode(t)`. Proved from TA5(c) functional determinism and P1 E-monotonicity (forward direction) and GlobalUniqueness (ASN-0034) via T10a.6 (reverse direction), each cited at the consuming step; "frontier" is well-defined by T10a.7. A T4b parse of `t` alone cannot discharge the operational check: whether `inc(t, 0) ∈ Σ.E` turns on prior K.δ emission history — which siblings of `t`'s allocator have already been allocated into Σ.E — and that history is recorded in Σ.E, not in `t`'s component structure, so the frontier predicate must be evaluated against the reachable state rather than read off a structural identification of `t` |
