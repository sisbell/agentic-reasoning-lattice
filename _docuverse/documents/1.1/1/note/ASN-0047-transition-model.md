...
*Modeling choice (layer separation).* D-CTG★/D-MIN★ constrain only the arrangement layer `M(d)`, while link permanence is discharged independently on `dom(L)` by L12 (LinkImmutability), so the strengthening does not contradict tombstoning.
