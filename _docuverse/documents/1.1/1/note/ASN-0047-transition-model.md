...
The per-state arrangement invariants (S2, S3★, S8a, S8-depth, S8-fin, D-CTG★, D-MIN★, D-SEQ★ at M'(d)) and the equivalence of this constructive precondition with the post-state characterization are derived consequences of the restriction form `M'(d) = M(d) ↾ R`, proved once in *K.μ⁻ admissible contraction shape* below; that proof is the sole site stating the equivalence, and every later mention cites it rather than restating it.
