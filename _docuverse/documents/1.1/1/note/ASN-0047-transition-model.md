...
**Reconciliation with ASN-0043's L1c.** SubAllocatorAxiom is operational; L1c (LinkAllocatorConformance) is a structural-producibility existential over T10a-admissible inc-chains. The chain `d → inc(d, 2) = b_C(d) → inc(b_C(d), 0) = b_L(d) → inc(b_L(d), 1) = ℓ_{first}` witnesses both — the intermediates `b_C(d)` and `b_L(d)` inhabit the foundation carrier set `T` via structurally valid inc steps without entering any state component.
...
CL-UNIQ is what lets K.μ~'s link-subspace identity property (`π(v) = v` for v link-subspace) be derived from the surrounding invariants rather than stating it as a precondition. The derivation appears as *Link-subspace fixity under K.μ~* in the *Decomposition of K.μ~* section below, which appeals to CL-UNIQ at the output state to conclude π = id on dom_L from the function-equality established by the cardinality squeeze.
