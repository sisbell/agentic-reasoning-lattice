...
*Subspace-position correspondence.* For `v ∈ dom(M(d))` with `M(d)(v) = a`, `subspace(v) = subspace_I(a)` (S3★ + L0). The equality is a two-step chain: S3★ routes the value to the correct store by subspace — `subspace(v) = s_C ⟹ a ∈ dom(C)` and `subspace(v) = s_L ⟹ a ∈ dom(L)` — and L0 fixes that store's I-address identifier, `subspace_I(a) = s_C` for `a ∈ dom(C)` and `subspace_I(a) = s_L` for `a ∈ dom(L)`. (S3★ alone yields only store *membership*, not equality of subspace identifiers; L0 supplies the second step.) The two projections apply at different state-component levels — `subspace(v)` projects V-positions, `subspace_I(a)` projects I-addresses.
...
*Step (A) — Subspace preservation under π.* Admissibility clause (i) hands us both `S3★(Σ')` and `S3★-aux(Σ')` as hypotheses on the candidate π. From these we derive subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` from S3★(Σ) (inductive hypothesis), the admissibility-stipulated S3★(Σ'), the bijection equation `M'(d)(π(v)) = M(d)(v)`, and L14 at both states (`dom(C) ∩ dom(L) = ∅`). S3★-aux(Σ) constrains the source `subspace(v) ∈ {s_C, s_L}`, and S3★-aux(Σ') constrains the image `subspace(π(v)) ∈ {s_C, s_L}` (this is exactly why S3★-aux is stipulated by clause (i): without it a candidate image such as `π(v) = [7, 1, 1]` would satisfy S8a yet have first component `7 ∉ {s_C, s_L}`, leaving S3★(Σ')'s two implications vacuous at `π(v)` and the exhaustiveness below unestablished). With both source and image subspaces confined to `{s_C, s_L}`, a mismatch under π takes exactly one of two complementary forms — `s_C → s_L` or `s_L → s_C`; there is no third case. We close both by contradiction.
...
*Proof of Step (B) — the K.μ⁻ + K.μ⁺ full-clearance decomposition realises any admissible π.* This step shows mechanical realisability: the K.μ⁻ + K.μ⁺ (full-clearance) decomposition actually realises an admissible π and produces a post-state `Σ' = (C, L, E, M', R)` *consistent with* the filter-stipulated `S3★(Σ')`, consuming Step (A)'s subspace preservation as the input that lets K.μ⁺'s amendment fire on the full-clearance write set. Let `Σ_int = (C, L, E, M_int, R)` denote the intermediate state between the K.μ⁻ and K.μ⁺ atomic steps of the decomposition. Three sub-claims, dispatched in order:
