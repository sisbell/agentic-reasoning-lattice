...
- *Partial-suffix expansion* at `n'_{s_C} = k₀ − 1` for some `k₀ ≥ 1`: K.μ⁻ removes the suffix `{[s_C, 1, ..., 1, k] : k₀ ≤ k ≤ n_{s_C}}`, then K.μ⁺ rebuilds it with π applied. This expansion is admissible iff π preserves M(d)-values below the cut pointwise: `(A v ∈ V_{s_C}(d) : v < [s_C, 1, ..., 1, k₀] under the V-ordering on s_C : M(d)(π(v)) = M(d)(v))`. The "iff" follows from K.μ⁺'s value-preservation clause on the surviving positions — positions below `k₀` retain `M_int(d)(v) = M(d)(v)` through K.μ⁻ and pass through K.μ⁺ unchanged, so any v below `k₀` with `M(d)(π(v)) ≠ M(d)(v)` would require K.μ⁺ to overwrite `M_int(d)(v)`, which K.μ⁺'s value-preservation clause forbids. Conversely, when π preserves M(d)-values below `k₀`, the K.μ⁻ + K.μ⁺ pair at `n'_{s_C} = k₀ − 1` realises π exactly: K.μ⁻ retains positions below `k₀` with their original M(d) values, and K.μ⁺ rebuilds the suffix `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d), v ≥ [s_C, 1, ..., 1, k₀]}`. Below-cut positions pass through with their original M(d) values, which is consistent with whatever permutation π applies among them as long as the values agree. *Relation to pointwise fixity.* The value-preservation form is weaker than pointwise fixity `(A v < [s_C, 1, ..., 1, k₀] : π(v) = v)`: under S5 (UnrestrictedSharing, ASN-0036), distinct V-positions may share the same M(d) value (transclusion), so π may permute below-cut positions among themselves whenever the permuted positions share their M(d) value, without violating value-preservation. On arrangements injective on `dom_C(M(d))` the two conditions coincide, and the simpler "π fixes every v below the cut" form is then sufficient.
