...
**K.μ~ (Arrangement reordering, distinguished composite).** K.μ~ is the transition kind whose contract is the *bijection equation*

`(E π : π is a bijection dom(M(d)) → dom(M'(d)) : (A v ∈ dom(M(d)) :: M'(d)(π(v)) = M(d)(v)))`

for some `d ∈ E_doc`, subject to the *admissibility constraints* that π is subspace-preserving (`(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))`), the resulting M'(d) satisfies S8a, S8-depth, D-CTG★, and D-MIN★, and the *frame* `C' = C; E' = E; R' = R; L' = L; (A d' : d' ≠ d : M'(d') = M(d'))` holds. The link-subspace identity property `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is *not* a primitive admissibility constraint; it is a derived consequence of S3★ + the K.μ⁺ amendment + CL-UNIQ, discharged in *Link-subspace fixity under K.μ~* below. This statement is the *primary* contract of K.μ~. Every admissible K.μ⁻ + K.μ⁺ pair satisfying it is called a *K.μ~ instance*, and the consolidated treatment in *Decomposition of K.μ~* below (placed after S3★-aux and CL-UNIQ on which the link-subspace fixity derivation depends) proves: (i) the link-subspace identity property as a corollary of the contract plus surrounding invariants, (ii) the existence of at least one admissible K.μ⁻ + K.μ⁺ realisation for every contract instance (Case 3 full-clearance witness), and (iii) the resulting frame matches the composite K.μ⁻ ∘ K.μ⁺ frame clause for clause. The presentation order is structurally non-circular: per-state invariants used in the link-subspace fixity derivation are inductively established without appeal to K.μ~'s contract. Wherever K.μ~ appears as a transition kind in the *Elementary transitions* section and its sequels — in composite-transition lemmas, in the structural-sufficiency claim, in worked examples, and in the Frame extension catalogue — the meaning is "any K.μ⁻ + K.μ⁺ realisation of the K.μ~ contract stated above; existence and uniqueness-of-frame are established in *Decomposition of K.μ~* below".
