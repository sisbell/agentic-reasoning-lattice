...
**SubAllocatorBundle (introduced here).** Each sub-clause below is proved by a named ASN-0093 lemma. For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]`, the anchors constructed as in *Allocator hierarchy under documents* above. The five sub-clauses, each with its discharging ASN-0093 lemma:
