...
- `fields(a).E₁` (local notation; equals `E(a)₁` of ASN-0034, T4b): the first component of `E(a)` — the subspace identifier at the I-address level. This ASN uses `fields(a).E₁` exclusively for the I-address subspace identifier; we *do not* employ an alternative `subspace_I(a)` notation. Where `fields(a).E₁` appears in a predicate, it should be read as "the subspace identifier of the I-address a" — i.e., `E(a)₁`.
...
- *Admissible removal pattern (per-subspace suffix or full clearance).* For each subspace S, the removed positions in `V_S(d)` form either a suffix of `V_S(d)` under the D-SEQ★-shaped enumeration or all of `V_S(d)`. The shape D-SEQ★ — stated inline here for self-contained reading and elevated to a system-wide per-state invariant below in *Amendments to existing transitions* under its own heading — asserts that for each non-empty subspace S, `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` for some `n_S ≥ 1`, with the inner positions of uniform depth m_S; the property is derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a at its definition site below, and the K.μ⁻ precondition treats the D-SEQ★-shaped pre-state as a structural input assumption discharged inductively over the elementary set. Concretely: under the D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`, there exists `0 ≤ n'_S ≤ n_S` per subspace such that the post-state subspace satisfies `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` — partial suffix removal when `1 ≤ n'_S < n_S`, full clearance when `n'_S = 0`, and no change in S when `n'_S = n_S`. The per-subspace patterns are independent: a composite K.μ⁻ may mix suffix removal in one subspace with full clearance in another and no change in a third, provided at least one subspace contracts strictly so the effect-clause requirement `dom(M'(d)) ⊂ dom(M(d))` is satisfied at the whole-arrangement level.
...
**K.μ~ (Arrangement reordering, named composite).** K.μ~ is *not an elementary transition*. It is a *named composite* — analogous to J0/J1★/J2/J3/J4 — whose contract is the *bijection equation*

`(E π : π is a bijection dom(M(d)) → dom(M'(d)) : (A v ∈ dom(M(d)) :: M'(d)(π(v)) = M(d)(v)))`

for some `d ∈ E_doc`, subject to the *admissibility constraints* that π is subspace-preserving (`(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))`), the resulting M'(d) satisfies S8a, S8-depth, D-CTG★, and D-MIN★, and the *derived frame* `C' = C; E' = E; R' = R; L' = L; (A d' : d' ≠ d : M'(d') = M(d'))` — derived clause-by-clause from the K.μ⁻ + K.μ⁺ decomposition under the amended frames, not stipulated as a primitive K.μ~ frame. The link-subspace identity property `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is *not* a primitive admissibility constraint; it is a derived consequence of S3★ + the K.μ⁺ amendment + CL-UNIQ, discharged in *Link-subspace fixity under K.μ~* below. The bijection equation, the admissibility constraints, and the derived frame jointly constitute the K.μ~ *contract*. Every admissible K.μ⁻ + K.μ⁺ pair satisfying the contract is called a *K.μ~ realisation*, and the consolidated treatment in *Decomposition of K.μ~* below (placed after S3★-aux and CL-UNIQ on which the link-subspace fixity derivation depends) proves: (i) the link-subspace identity property as a corollary of the contract plus surrounding invariants, (ii) the existence of at least one admissible K.μ⁻ + K.μ⁺ realisation for every contract instance (Case 3 full-clearance witness), and (iii) the resulting frame matches the composite K.μ⁻ ∘ K.μ⁺ frame clause for clause. The presentation order is structurally non-circular: per-state invariants used in the link-subspace fixity derivation are inductively established without appeal to K.μ~'s contract.

*Status as a named composite, not an elementary transition.* K.μ~ has no elementary status: it is not in the seven-element elementary set {K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.ρ}, has no primitive contract beyond its decomposition, and contributes no fresh wp-calculus obligation that the underlying K.μ⁻ + K.μ⁺ pair does not already discharge. Throughout this ASN, every reference to "K.μ~" — in the Frame extension catalogue, in the elementary case analysis of *ExtendedReachableStateInvariants*, in the structural-sufficiency claim, in worked examples, in the ValidComposite★ definition, and in the J3 isolation discussion — denotes "any K.μ⁻ + K.μ⁺ realisation of the K.μ~ contract stated above," with invariant preservation, frame, and coupling all routed through the underlying elementary steps. The convenience of treating K.μ~ as a "case" in proofs lies in arguing at the bijection-equation level (a structural pattern that recurs across many realisations) rather than re-deriving the same conclusion for every K.μ⁻ + K.μ⁺ sequence; the soundness of doing so is the substance of the *Decomposition of K.μ~* section, which establishes that every per-state and per-transition invariant is preserved by every realisation, with frame and admissibility derived from the elementary K.μ⁻ + K.μ⁺ contracts. The structural-sufficiency claim "seven elementary transitions ... plus the named composite K.μ~" should therefore be read as: the seven elementary transitions cover the modification-kind landscape, and K.μ~ is the one *named composite* (alongside J4, etc.) elevated to a named status because its contract structure — the bijection equation — recurs widely enough to warrant a label. The fact that proofs of invariants list a "K.μ~ case" is a presentational shorthand for "K.μ⁻ + K.μ⁺ realisation"; the elementary inventory is unchanged.
...
This section provides the consolidated realisation account for the distinguished composite K.μ~ introduced in *Elementary transitions* above. The K.μ~ *contract* — the bijection equation, admissibility constraints, and frame — was stated as primary at the definition site; this section's job is to prove (i) link-subspace fixity as a corollary of the contract plus S3★ + the K.μ⁺ amendment + CL-UNIQ, (ii) the existence of at least one admissible K.μ⁻ + K.μ⁺ realisation for every contract instance, and (iii) frame consistency between the composite signature and the K.μ⁻ ∘ K.μ⁺ trace. The placement here — after S3★-aux (*Generalized referential integrity*) and CL-UNIQ (*Link-subspace ownership*) — discharges the forward references flagged at K.μ~'s definition site: every per-state invariant cited below has already been established, so the argument proceeds without circularity. The contract is restated below for self-contained reference; link-subspace fixity is then derived from the contract plus the surrounding invariants; the realisation cases follow.

**K.μ~ — contract (restated for reference).** The primary contract stated at the K.μ~ definition site is, for some `d ∈ E_doc`:

`(E π : π is a bijection dom(M(d)) → dom(M'(d)) : (A v ∈ dom(M(d)) :: M'(d)(π(v)) = M(d)(v)))`

Downstream proofs that appeal to K.μ~'s contract (the K.μ~-FIX domain-fixity argument; the link-subspace fixity argument; J3 isolation) discharge through the elementary K.μ⁻ + K.μ⁺ frames of any realisation, not through a primitive K.μ~ frame.

*Admissibility constraints (carried over from the definition site).* π is subspace-preserving — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — so each V-position is permuted within its own subspace (a π violating this would yield M'(d)(π(v)) = M(d)(v) ∈ dom(L) at a content-subspace position, contradicting S3★); π produces V-positions satisfying S8a (all components strictly positive); and the resulting arrangement M'(d) satisfies S8-depth (uniform depth within each subspace), D-CTG★ (contiguity within each subspace), and D-MIN★ (minimum position in each non-empty subspace). The link-subspace identity property `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is *not* among these admissibility constraints — it is a *corollary* of the contract plus S3★ + the K.μ⁺ amendment + CL-UNIQ, established in *Link-subspace fixity under K.μ~* immediately below.
