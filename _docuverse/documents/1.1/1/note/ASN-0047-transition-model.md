...
*Freshness discharge (scope note).* Every K.δ case-(ii) sub-case discharges its freshness conjunct `e ∉ E` as a single live-state read against the present Σ; no structural fact forces it, since GlobalUniqueness (ASN-0034) establishes distinctness only *across distinct allocation events* and so supplies cross-event distinctness once `e ∉ E` grants the event is new, never freshness itself. The sub-cases differ solely in *which* state fact the guard `e ∉ E` encodes. At **k = 0** (sibling advance) it reads the current frontier index: by the biconditional above, `inc(t, 0) ∉ Σ.E` iff `t` is the frontier of `A`'s chain — nothing forces it, since a sibling of an already-allocated `t` could otherwise collide with a previously advanced sibling. At **k ∈ {1, 2}** (child-spawn) the same conjunct *is* the enforcement of T10a's at-most-once-per-`(t, k')` discipline: it reads whether the spawn `(t, k')` has already been performed, a fact logically equivalent to `inc(t, k') ∈ Σ.E`. The K.δ bullets, the S7d discharge, and the worked examples below invoke this note by name rather than re-deriving the contrast.
...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases. The freshness conjunct `e ∉ E` is discharged as a single live-state read against Σ in every sub-case, per FrontierEquivalence's *Freshness discharge* scope note — which states once how that guard encodes the current frontier index at k = 0 versus the at-most-once-per-`(t, k')` occurrence at k ∈ {1, 2}, and why GlobalUniqueness (ASN-0034) supplies only cross-event distinctness rather than freshness itself. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
  - *k = 0 (sibling):* `t ∈ E ∧ ¬Node(t)`. These are the only sub-case-specific operand-admissibility conjuncts: `t ∈ E` (the operand must be an allocated entity) and `¬Node(t)` (so `parent(t)` is well-defined under T4b). No additional freshness conjunct is imposed here — the case-level `e ∉ E` *is* the k = 0 frontier check (FrontierEquivalence's *Freshness discharge* note), since `e = inc(t, 0)`. The structural identities `parent(t) = parent(e)` and `zeros(t) = zeros(e)` hold by TA5(c) on `e = inc(t, 0)` (K.δ-ID.parent-0/1, K.δ-ID.zeros-0/1).
...
**Step 4: K.δ case (ii) k = 0 — allocate sibling document `1.2.0.1.0.2 = inc(1.2.0.1.0.1, 0)` under the same account.** Operand `t = 1.2.0.1.0.1`, the document placed by Step 3. The k = 0 dispatch differs from k ∈ {1, 2} in two distinctive features: (a) freshness `inc(t, 0) ∉ E` is discharged as the k = 0 frontier read of FrontierEquivalence's *Freshness discharge* note, rather than the at-most-once-per-`(t, k')` read used at k ∈ {1, 2}; and (b) the dispatch through T10a.6 (DomainDisjointness) routes the verification against t's actual provenance — for this t, `A_doc(parent(t)) = A_doc(1.2.0.1)`, the account's document sub-allocator activated by Step 3.
...
| S8a, S8-depth, S8-fin | frame | new doc has M(d)=∅ (vacuous) | frame | precondition: positivity, depth, finite extension | precondition: positivity, depth m_L(d), finite | restriction preserves all three | S8a/S8-depth: K.μ~ admissibility (i) on M'(d); S8-fin: K.μ⁻ restricts + K.μ⁺ finite-extends | frame |
| S8★ | frame | new doc M(d)=∅ (vacuous) | frame | per-subspace projection via ASN-0036's S8 on M(d')\|_{V_{s_C}(d')} | trivial length-1 decomposition on M(d')\|_{V_{s_L}(d')} | s_C: per-subspace projection via ASN-0036's S8 on contracted M'(d)\|_{V_{s_C}(d)}; s_L: trivial length-1 fall-back on survivors; see *S8★* prose below | not an admissibility-(i) clause: established at Σ' by K.μ⁻ + K.μ⁺ decomposition (ASN-0036 S8 on rebuilt content projection; length-1 on link survivors) | frame |
| D-CTG★ / D-MIN★ | frame | frame | frame | precondition discharge (K.μ⁺'s original precondition list, in *Elementary transitions*, requires the resulting M'(d) to satisfy D-CTG / D-MIN — strengthened to D-CTG★ / D-MIN★ in the extended state: content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | constructive precondition discharge — the per-subspace retention count `n'_S` directly produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` which satisfies D-CTG★ and D-MIN★ at Σ' by construction | stipulated on M'(d) by K.μ~ admissibility (i) | frame |
| D-SEQ★ | frame | frame | frame | derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a | derived | derived | derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a (all discharged in their own K.μ~ cells) | frame |
