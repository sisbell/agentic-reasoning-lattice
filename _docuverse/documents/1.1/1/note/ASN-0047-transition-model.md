...
The same vacuity afflicts the content subspace — when `V_{s_C}(d) = ∅`, S8-depth is equally vacuous, and K.μ⁺'s preconditions (S8a + S8-depth + D-MIN★) do not pin the depth of the first content insertion. We introduce no `ContentVPositionDepthAxiom`, however, because the content subspace already carries a foundation predicate governing its first insertion: ASN-0036's `ValidFirstInsertionPosition(d, v, m)`, which defines the well-formed first content V-position at any chosen depth `m ≥ 2` and (postcondition (c)) fixes a unique such `v` for each `(d, m)`. The content subspace's first-insertion discipline is thus inherited from that foundation predicate; K.μ⁺ realises it directly. The link subspace has no analogous foundation predicate in ASN-0036 — a deliberate omission, since Nelson left link positional depth open to subdivision "by further digits" (LM 4/31) while keeping the byte/content position to a single Vstream-managed coordinate (LM 4/30). LinkVPositionDepthAxiom supplies, for the link subspace, exactly the first-insertion-depth role that `ValidFirstInsertionPosition` plays for content; the asymmetry between the two subspaces is therefore one of *where the discipline is stated* (inherited foundation predicate vs. local axiom), not a gap left open for content. Like `ValidFirstInsertionPosition`, the axiom leaves the actual depth value free, fixing only that it be determined once per document.
...
*Proof of Step (A) — Subspace preservation under π from admissibility (i) + L14.* Admissibility clause (i) of K.μ~ stipulates that the post-state `M'(d)` satisfies S3★(Σ') as part of its post-state invariant package — the stipulation is part of the admissibility filter on π, not a derived consequence of preconditions. From S3★(Σ) (inductive hypothesis), the admissibility-stipulated S3★(Σ'), the bijection equation `M'(d)(π(v)) = M(d)(v)`, and L14 at both states (`dom(C) ∩ dom(L) = ∅`), subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` follows by case analysis on the two possible mismatch forms `s_C → s_L` and `s_L → s_C` (S3★-aux constrains `subspace(v) ∈ {s_C, s_L}` everywhere, so there is no third case). The full case-by-case derivation is in the *Case `s_C → s_L`* and *Case `s_L → s_C`* paragraphs below the *Preconditions of K.μ~* paragraph. ∎ (Step (A))
