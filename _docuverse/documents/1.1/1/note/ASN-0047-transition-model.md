...
*Scope and base-liveness.* K.δ as stated above requires, for k = 1 events from a document, that the immediate inc operand `t` be in E_doc — equivalently, that the corresponding base `[N, 0, U, 0, D]` be a live document at the moment of the event. We verify that even the *weaker* form (with only `t ∈ T` and no entity-membership clause) would not have broken any invariant introduced in this ASN: K.δ frames every state component except E (and, for documents, M(e), where it installs `M(e) = ∅`), preserves the entity-hierarchy spine P8 through `parent(·)` (the version step `[N,0,U,0,D] → [N,0,U,0,D,k]` crosses no zero separator, so parent(e) = parent(t) ∈ E by P8 at the pre-state), and renders every layer invariant trivial on the new entity: arrangement clauses (S2, S3/S3★, S8a, S8-depth, S8-fin, D-CTG/D-MIN/D-SEQ and their starred forms) hold vacuously on `M(e) = ∅` and by frame elsewhere; origin coherence (P6), provenance grounding (P7, P7a), link-layer invariants (L0–L14), and the uniqueness axioms (GlobalUniqueness, NodeUniqueAllocation, SubAllocatorAxiom) all anchor through `e` itself for any subsequent K.α + K.μ⁺ + K.ρ activity, never through the possibly-absent base; and the couplings J0–J4 are vacuously satisfied at the K.δ step (dom(C') = dom(C), ran(M'(d)) = ran(M(d)) for every d) and discharge through `e` for any J4-style composite. The companion question of intermediate-version liveness — whether `[N,0,U,0,D,k]` with k ≥ 2 (allocated as an inc-k=0 sibling of the prior version) requires `[N,0,U,0,D,k−1] ∈ E_doc` — is decided affirmatively as an immediate consequence of `t ∈ E`: every k ≥ 2 version event has the prior version as its inc operand, which must therefore be in E.

*Relation to the foundation's ghost-element doctrine — substantive scope choice.* The per-invariant verification above establishes that the *weaker* precondition (omitting `t ∈ E_doc`) would have left every clause of P0–P8, S0–S9, L0–L14, J0–J4, and their starred forms invariant-safe. The strengthening to `t ∈ E_doc` is therefore not invariant-forced; it is a substantive scope choice of the present ASN. Three positions were available — (a) identify an invariant that demands the strengthening (none was found), (b) relax to the weaker precondition and admit ghost-base versioning at the abstract level, or (c) adopt the stronger precondition as a deliberate scope decision that future ASNs may revisit. The present ASN takes position (c). The rationale: ghost-element semantics (LM 4/23 — "no specific element need be stored in tumbler-space to correspond to them") remain in force for *non-immediate* ancestors — an account address `[N, 0, U]`, for example, need not be an explicit content store entry to host a document below it, and the entity-hierarchy spine P8 governs that ancestor relation through the parent spine rather than through inc operands. What this ASN newly forbids is using a ghost as the *immediate* inc operand `t`, so version chains must be rooted in a live base and longer chains require every prior version to remain live (preserved automatically by NoDeallocation once allocated). This aligns the abstract specification with Gregory's `docreatenewversion` implementation (consultation evidence, ST-VERSION-CREATE/Finding 0032), which establishes the source ISA as a live granfilade orgl via `doretrievedocvspanfoo` before issuing a version address. A subsequent version-management ASN may revisit this scope choice — either by introducing an invariant that forces the strengthening (e.g., a versioning-specific lineage axiom that takes `t ∈ E_doc` as a hypothesis), or by relaxing K.δ to the weaker form and recovering the implementation's contract at a downstream FEBE-layer specification. The choice is flagged here as deliberate rather than load-bearing on the present invariant set.
...
**K.μ~ (Arrangement reordering, distinguished composite).** K.μ~ is *not* a primitive transition; it is a *distinguished composite* whose semantics are given by its K.μ⁻ + K.μ⁺ decomposition (below). The signature stated here describes what the composite achieves — the contract that any decomposition realising a reordering must discharge — and is derived from the constituent operators' contracts, not stated independently.

*Composite signature.* For some d ∈ E_doc, there exists a bijection π : dom(M(d)) → dom(M'(d)) such that:

`(A v : v ∈ dom(M(d)) : M'(d)(π(v)) = M(d)(v))`

*Preconditions on the realising decomposition.* `d ∈ E_doc`; π is subspace-preserving — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — so each V-position is permuted within its own subspace (a π violating this would yield M'(d)(π(v)) = M(d)(v) ∈ dom(L) at a content-subspace position, contradicting S3★); π produces V-positions satisfying S8a (all components strictly positive); and the resulting arrangement M'(d) satisfies S8-depth (uniform depth within each subspace), D-CTG (contiguity within each subspace), and D-MIN (minimum position in each non-empty subspace). The link-subspace identity property `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is *not* stated as a precondition: it is derived from the realising decomposition under S3★ + the K.μ⁺ amendment + CL-UNIQ (see *Link-subspace fixity under K.μ~* below), and stating it at the definition site would duplicate a consequence.
