...
The strict-subset clause `dom(M'(d)) ⊂ dom(M(d))` is unsatisfiable when `dom(M(d)) = ∅`: the empty set has no proper subset, so K.μ⁻ is definitionally undefined at empty-arrangement pre-states — `dom(M(d)) ≠ ∅` is enforced by the effect clause itself, not a separately-stated precondition.
...
The amendment trades ASN-0036's link-subspace tombstoning provision for uniform structural simplicity across subspaces; the consequences for link withdrawal under the amended forms are deferred to the canonical open question on withdrawal invariants below. All subsequent references to D-CTG and D-MIN in this ASN denote the amended (per-subspace) forms D-CTG★ and D-MIN★ — including the K.μ⁺, K.μ⁻, K.μ⁺_L, and K.μ~ postconditions and the per-subspace arrangement invariants below.
...
*Consequence for link withdrawal.* Under D-CTG★, a user cannot withdraw a single link at a non-maximum link-subspace position while leaving subsequent links in place; the consequences and Nelson's tombstoning design are catalogued at the canonical open question on withdrawal invariants below.
...
**Dispatch of freshness obligations across K.α and K.λ.** The following table records which clause of SubAllocatorAxiom (or T10a's GlobalUniqueness) discharges the freshness obligation at each allocation site. SubAllocatorAxiom.Exists supplies the structural precondition that A_C(d) and A_L(d) are activated frontiers whenever `d ∈ E_doc`, underwriting the "produced by d's sub-allocator" clause of K.α and K.λ across all four rows. SubAllocatorAxiom.Namespace and T10a's GlobalUniqueness are the per-emission dispatch premises:
...
Link withdrawal via K.μ⁻ applied to the link subspace would in principle produce the same state — a link present in L but absent from all current arrangements — but is constrained by D-CTG to suffix truncations (per K.μ⁻'s case analysis above); the reconciliation with Nelson's tombstoning design is catalogued at the canonical open question on withdrawal invariants below.
...
- *Frame-preserved invariants:* P0/P1/P2/P3★/P5★/P6/P7/P7a (vacuous on empty C), all L-invariants (vacuous on empty L), all S-invariants (M, C unchanged), all D-invariants (vacuous on empty arrangements), J0/J1★/J1'★ (K.δ adds no content, arrangement extension, or provenance — all couplings vacuous). ✓
...
- *Frame-preserved invariants:* P0/P1/P2/P3★/P5★ (extension on E only), P6/P7/P7a (vacuous on empty C), all L-invariants (vacuous on empty L), J0/J1★/J1'★ (vacuous). ✓
...
Nelson's tombstoning design — preserving the withdrawn link's arrangement position rather than removing it — is therefore not expressible as a K.μ⁻ transition; the reconciliation is catalogued at the canonical open question on withdrawal invariants below.
...
- *S7d (Document allocation discipline)* — every K.δ on `IsDocument(e)` that proceeds via Path 1 allocates under the T10a discipline within the owning account's document sub-allocator, and GlobalUniqueness produces distinct documents across distinct K.δ events. Preserved by P1 thereafter. *Ghost-base relaxation at k = 1.* The k = 1 ghost-base sub-case (Path 2 discharge) admits an inc operand `t ∉ E_doc`, so the resulting document address `e = inc(t, 1)` is not emitted from any T10a-tracked frontier — the literal foundation form of S7d (every document tumbler is the result of a T10a allocation event) does not hold pointwise for ghost-base emissions. The present ASN therefore commits to a weakened reading of S7d: every document tumbler is structurally T4-valid with `zeros = 2`, distinct K.δ events produce distinct documents (closed at the entity-allocation layer by K.δ's `e ∉ E` precondition, regardless of which path discharges freshness), and the *T10a-conformance* clause of foundation S7d is downgraded for ghost-base emissions to a tumbler-layer structural-validity clause. Reconciliation with the literal foundation form — including the question of when a downstream operation may "upgrade" a ghost-base emission to a T10a-tracked status — is part of the deferred version-management ASN flagged at K.δ's *Ghost-base versioning (k = 1)* paragraph.
...
- *Withdrawal-mechanism gap (canonical reference for the cross-site deferrals).* What invariants must link withdrawal maintain — must withdrawn links remain arranged, or does withdrawal remove them from M(d)? Under D-CTG★/D-MIN★, K.μ⁻'s admissible link-subspace contractions are suffix truncations only (per K.μ⁻'s case analysis above), so withdrawing one interior link requires withdrawing every link allocated after it as well. Nelson's tombstoning design at LM 4/9 — withdrawn links transition to "not currently addressable" status while retaining their arrangement position and permanent serial address — is therefore not expressible as a K.μ⁻ contraction in this ASN. Reconciling that design with D-CTG★ requires a separate withdrawal mechanism (status flag, tombstone marker, or explicit retraction link) operating outside K.μ⁻'s presentational-removal contract. Other sites in this ASN — the D-CTG★ amendment's *Consequence for link withdrawal*, the *Extended structural sufficiency* "tombstoning gap" enumeration, the *Orphan links and coupling flexibility* discussion, and the worked-example Step 5 counterfactual — defer to this open question rather than restating the gap.
