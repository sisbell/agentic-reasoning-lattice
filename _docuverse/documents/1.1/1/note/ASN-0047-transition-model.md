...
**P4 (Provenance bounds, link-free fragment).** P4 is a property of the four-component scaffold — the transition fragment that admits no link-subspace mappings, i.e. no K.λ and no K.μ⁺_L. We state and prove it only over that fragment; in the full extended state it is superseded by P4★ (proved in *Content-scoped containment and provenance* below), because once link-subspace mappings exist `Contains(Σ)` acquires pairs `(ℓ, d)` with `ℓ ∈ dom(L)` that no provenance entry can record (P7 grounds R in `dom(C)`, and `dom(L) ∩ dom(C) = ∅` by L14). The link-free scoping is exactly what keeps `Contains(Σ)` content-only, so that P4 holds.

In any reachable state of the link-free fragment where J1 has been satisfied for all prior transitions:

`Contains(Σ) ⊆ R`

*Base case.* In Σ₀, (E₀)_doc = ∅ (E₀ contains only the bootstrap node), so Contains(Σ₀) = ∅ ⊆ ∅ = R₀. The bound holds vacuously.

*Inductive step.* Assume Contains(Σ) ⊆ R at a reachable state Σ, and let `Σ →* Σ'` be a valid composite transition *of the link-free fragment* (no K.λ, no K.μ⁺_L — the transitions that would introduce link-subspace range entries and break the bound are excluded by the fragment's scoping). Every (a, d) ∈ Contains(Σ') falls into exactly one of two cases:
...
The per-elementary analysis over the link-free fragment confirms *why* only K.μ⁺ introduces new containment and why the other fragment transitions are harmless:
...
The fragment excludes exactly the two transitions that would falsify the bound: K.μ⁺_L adds a link-subspace pair `(ℓ, d)` to `Contains(Σ')` with `ℓ ∈ dom(L)`, which by P7 and L14 cannot lie in R; K.λ allocates the link addresses such positions target. These are accounted for not here but by P4★, which scopes containment to the content subspace and is the form that survives into the full extended state.
