...
*L14 premise convention (used throughout this section).* Every reference below to "L14" or "L14 at both states" denotes the per-state set-equality `dom(C) ∩ dom(L) = ∅` evaluated at both endpoints: `dom(C) ∩ dom(L) = ∅` at Σ and `dom(C') ∩ dom(L') = ∅` at Σ', both Class (a) per-state invariants by the ExtendedReachableStateInvariants induction (Σ reachable by hypothesis of the induction; Σ' reachable as the post-state of a valid composite from Σ). The dependency chain and case analysis below cite L14 ten times across Steps (A)–(E); each invocation consumes this set-equality form at one or both endpoints, not L14's full content. Subsequent references read "L14" / "L14 at both states" without restating this convention.
