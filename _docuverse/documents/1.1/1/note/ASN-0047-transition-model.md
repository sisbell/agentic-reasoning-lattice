...
**K.μ~ (Arrangement reordering, distinguished composite).** K.μ~ is *not* a primitive transition. It is a *distinguished composite* whose semantics are defined by its K.μ⁻ + K.μ⁺ decomposition (see *Decomposition of K.μ~* below). The contract stated immediately below is a *theorem* describing what any such decomposition achieves — it is derived from the constituent operators' contracts, not stipulated independently. Downstream proofs that appeal to K.μ~'s contract (the K.μ~-FIX domain-fixity argument; the link-subspace fixity argument; J3 isolation) discharge through the elementary K.μ⁻ + K.μ⁺ frames, not through a primitive K.μ~ frame.
...
*Frame (derived).* C' = C; E' = E; R' = R; (A d' : d' ≠ d : M'(d') = M(d')). Each clause is the composition of the constituent operators' frames: K.μ⁻ and K.μ⁺ both hold C, E, R fixed and act on the single document d, so the composite holds the same. In the extended state Σ = (C, L, E, M, R), L is also held fixed (`L' = L`) by the same composition; this clause appears uniformly under *Frame extension (all existing transitions)* below.
...
- *`e₁ ∉ E`:* `e₁ = 1.0.1.0.5.1` is fresh to E₆ by T10a's GlobalUniqueness on the `(t, 1)` inc event (this is its first use). ✓
