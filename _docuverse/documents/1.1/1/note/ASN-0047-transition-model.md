...
*Two scopes of "T10a's domain" — named predicates (definitions used by K.δ and downstream).* The phrase "T10a's domain" carries two distinct meanings in this ASN, and we pin them with two named predicates introduced here and used by K.δ's discharge analysis, the ghost-base discharge derivations, and the freshness-path discrimination across worked examples:

- **InTumblerUniverse(t, s) := t ∈ allocated(s)** — the *tumbler-allocation layer*, evaluated at state `s`: `allocated(s)` is ASN-0034's state-indexed set of tumblers issued at the namespace-allocation layer up to state `s` (per ASN-0034's AllocatedSet derivation, with T8's AllocationPermanence supplying monotonic growth). Inclusion in `allocated(s)` is a fact about whether the tumbler-namespace allocator has ever minted the address at the prefix level by state `s`, *independent* of whether an entity record was created for it in E at state `s`. A ghost document tumbler `t = [N, 0, U, 0, D]` may have `InTumblerUniverse(t, s)` (the address has been issued by the account's document namespace at the tumbler layer by state `s`) without having a corresponding entity in E at state `s`. *Note on naming.* This ASN reserves the symbol `T` for ASN-0034's foundation T0 (CarrierSetDefinition) — the fixed carrier set of all finite tumbler sequences over ℕ — and writes `allocated(s)` for the state-indexed allocated set; earlier informal usage of `T` to mean "the universe of allocated tumblers" conflated the two and is replaced throughout by `allocated(s)`.
- **Entity-level allocator (definition, used by InEntityAllocatorDomain below).** An *entity-level allocator* is a T10a-tracked sub-allocator whose output addresses are themselves entity addresses (i.e., `zeros(·) ≤ 2`, so non-element). The entity-level allocators relevant to this ASN are: (E.i) the *node-level allocator(s)* — the protocol-established allocators underwriting NodeUniqueAllocation (Nelson's hierarchical-baptism allocator under each parent node, or Gregory's single global granfilade under the bootstrap root), whose outputs are node tumblers; (E.ii) the *account-level sub-allocator under each node* — the T10a-conforming `inc(·, 0)` frontier whose outputs are account tumblers under a given node (zeros = 1); (E.iii) the *document-level sub-allocator under each account*, written `A_doc(a)` for `a ∈ E_account` — the T10a-conforming frontier whose outputs are top-level documents (the k = 2 descent from `a` and downstream k = 0 sibling emissions), all at zeros = 2; (E.iv) the *version sub-allocator under each live document*, written `A_v(t)` for `t ∈ E_doc` where T2's spawning premises are met — the T10a-conforming frontier whose outputs are version documents `t.1, t.2, ...` at zeros = 2. The *content sub-allocator* `A_C(d)` and the *link sub-allocator* `A_L(d)` introduced under SubAllocatorAxiom are *not* entity-level allocators: their outputs are element-level addresses (zeros = 3, IsElement), inhabit `dom(C)` or `dom(L)` rather than `E`, and lie outside the entity stratification. The qualifier "entity-level" is therefore exactly the level partition `zeros(·) ≤ 2` lifted from output addresses to the allocators that produce them.
- **InEntityAllocatorDomain(t, s) := t ∈ allocated(s) ∧ (E A ∈ Act(s) : A is an entity-level allocator (per the enumeration above) ∧ t ∈ dom_s(A)) ∧ t ∈ E** — the *entity-allocation layer*, evaluated at the state `s` under consideration (typically the pre-state of the K.δ event whose `e ∉ E` precondition is being discharged): t is not only in `allocated(s)` (the state-indexed allocated set) but is tracked by some entity-level T10a allocator from the enumeration above *with a corresponding entity record in E at state s*. The `Act(s)` reference is to T10a's state-indexed active-allocator set at `s` (ASN-0034); `dom_s(A)` is allocator A's tracked emission history at `s` per T10a. This is the scope at which K.δ's `e ∉ E` precondition is discharged via T10a's GlobalUniqueness: GlobalUniqueness gives "inc(t, k) is distinct from every previously allocated address *within that allocator's domain*," and "within that allocator's domain" requires the operand to inhabit some entity allocator's tracked frontier in `Act(s)`, which a ghost operand by stipulation does not. Content/link sub-allocators are excluded from the qualifier because their outputs are non-entity (element-level), so a putative content/link operand-tracked condition does not bear on K.δ's `e ∉ E` discharge — K.δ allocates entity addresses, not element-level addresses, and the discharge layer must match the output layer.

*Shorthand convention.* When the evaluation state `s` is clear from context — typically the pre-state of the K.δ event whose precondition or discharge is under discussion — we write `InTumblerUniverse(t)` and `InEntityAllocatorDomain(t)` as shorthand for `InTumblerUniverse(t, s)` and `InEntityAllocatorDomain(t, s)`. Downstream proofs and worked examples below use the shorthand throughout; the full state-indexed form is restored only when the state is itself a variable in the argument or when two distinct states are being compared.

The two predicates are *not* equivalent: ghosts satisfy InTumblerUniverse but not InEntityAllocatorDomain. They are the discharge-layer pivot for the freshness discrimination across K.δ's three discharge paths (next section), the K.λ first-link / subsequent-link distinction, and the ghost-base versioning analysis in the worked examples.
...
**Structural form of n₀.** The bootstrap node is fixed as `[1]` — a one-element tumbler with `zeros(n₀) = 0`, satisfying `IsNode(n₀)` and `ValidAddress(n₀)`. The choice of `1` as the singular root value is a definitional convention of this ASN; the design rationale and consultation evidence are recorded in *Design provenance* below.
...
*Precondition.* The precondition splits on IsNode(e), reflecting the two distinct allocation disciplines — protocol-established node baptism versus T10a-conforming inc-allocation under a parent entity. The full routing across cases — operand requirement, freshness discharge path, and the operational T10a allocator at which the inc step is taken — is summarised in the following table. One row per precondition signature; within case (ii), the discharge path varies operationally with the path-discriminating premise `InEntityAllocatorDomain(t)` (defined in *Two scopes of "T10a's domain" — named predicates* below): when the premise holds, freshness routes through Path 1 (T10a GlobalUniqueness at the entity-allocator layer); when it fails, through Path 2 (K.δ precondition + TA5 determinism at the tumbler layer).
...
*Two scopes of "T10a's domain" — named predicates.* This ASN's discharge analysis depends on distinguishing the *tumbler-allocation layer* (InTumblerUniverse) from the *entity-allocation layer* (InEntityAllocatorDomain). Both predicates are defined in full at the head of this ASN in the *Two scopes of "T10a's domain" — named predicates (definitions used by K.δ and downstream)* paragraph in the Notation section; downstream uses, including the discharge table above and the ghost-base derivations in the worked examples, appeal to the shorthand forms `InTumblerUniverse(t)` and `InEntityAllocatorDomain(t)` defined there.

**Three discharge paths for `e ∉ E` — named rules.** The table above routes the K.δ freshness obligation through three named paths, one per allocation discipline:

- *Path 1 (T10a GlobalUniqueness at the entity-allocator layer).* When `InEntityAllocatorDomain(t)` holds, the operand inhabits some entity-level T10a allocator's tracked frontier in `Act(s)`, and T10a's GlobalUniqueness applies directly: `inc(t, k)` is distinct from every previously allocated address within that allocator's domain. Cross-allocator disjointness is supplied by T10a.{2,5,6} → T10. Path 1 governs k = 0 sibling, k = 1 live-base versioning, and k = 2 descent cases when the operand satisfies the entity-allocator-tracked condition.
- *Path 2 (TA5 determinism + K.δ precondition `e ∉ E` at the tumbler layer).* When `¬InEntityAllocatorDomain(t)` — i.e., the operand is a ghost or otherwise outside the entity allocator's tracked frontier — T10a's GlobalUniqueness is unavailable. TA5(c)/(d) supplies the unique structural identity of the candidate `inc(t, k)` at the tumbler layer (the foundation carrier set `T`), and K.δ's `e ∉ E` precondition is verified by direct inspection of E at the pre-state. Path 2 governs ghost-base versioning (k = 1 with `t ∉ E_doc`) and downstream chain emissions rooted at a ghost base.
- *Path 3 (NodeUniqueAllocation for node events).* Node baptism (K.δ case (i)) does not pass through `inc(·, k)`; the protocol-established node-allocation discipline supplies `e ∉ E` directly via NodeUniqueAllocation. Path 3 governs every K.δ event with `IsNode(e)`.

The two predicates are *not* equivalent: ghosts satisfy InTumblerUniverse but not InEntityAllocatorDomain. For ghost-chain emissions downstream of a ghost-base K.δ, the displacement to Path 2 propagates chain-wide because T10a's T2 spawning requires the operand to inhabit its parent allocator's tracked domain, which fails at the ghost-base root.
...
D-SEQ★ is the per-state invariant referenced by K.μ⁻'s precondition clause (A) in the Elementary transitions section above (where it appears as the pre-state shape "V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}" cited by the admissible-removal pattern) and re-established in full detail here from the amended D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a. The derivation is a single-state implication consuming the per-state invariants on its left and producing the per-state shape on its right; it is reusable at every reachable state without re-invoking the outer induction. Downstream sections (K.μ⁻ admissibility, the K.μ~-FIX domain-fixity argument, the link-subspace fixity proof, and the ExtendedReachableStateInvariants induction) appeal to D-SEQ★ by name at each state where its premises hold.
...
**SubspaceConventionAxiom (Axiom, FixedSubspaceIdentifiers).** `s_C = 1 ∧ s_L = 2`. The commitment is structurally load-bearing: under these values the chain `d → inc(d, 2) = b_C(d) → inc(b_C(d), 0) = b_L(d) → inc(b_L(d), 1) = [d.0.s_L.1]` is producible as a single-inc-per-step witness for L1c's structural-producibility existential. SC-NEQ (`s_C ≠ s_L`) is the immediate consequence. Design rationale and consultation evidence (Nelson LM 4/30–4/31; Gregory `xanadu.h:144–146`, `granf2.c:162`, `do2.c:94`) are recorded in *Design provenance* below.

These anchors are *structurally producible* via T10a inc steps from `d`: `b_C(d) = inc(d, 2)` (TA5(d) with k = 2) and `b_L(d) = inc(b_C(d), 0)` (TA5(c)). The anchors are not themselves in `dom(C) ∪ dom(L)` — content addresses have `#E ≥ 2` (S7c), link addresses have `#E ≥ 2` (L1b), and the anchors have `#E = 1` — so they inhabit the foundation carrier set `T` but no state component of Σ.

**Reconciliation with ASN-0043's L1c.** ASN-0043's L1c (LinkAllocatorConformance) requires, for every `ℓ ∈ dom(Σ.L)`, the existence of a T10a-admissible inc-chain `s = t₀ → t₁ → ... → tₙ = ℓ` from a T4-valid document-level seed `s` to `ℓ`. L1c is a *structural-producibility* existential, not a state predicate: it asks whether such a chain *exists* under T10a's per-step inc-admissibility rules, not whether each intermediate tumbler inhabits some state component. The witnessing chain for the first emission of d's link sub-allocator — instantiating SubspaceConventionAxiom's `s_L = 2` — is `t₀ = d`, `t₁ = inc(d, 2) = [d.0.1] = b_C(d)`, `t₂ = inc(t₁, 0) = [d.0.2] = b_L(d)`, `t₃ = inc(t₂, 1) = [d.0.2.1] = ℓ_{first}` (length 3, first step `k₁ = 2`, subsequent k_i ∈ {0, 1, 2} as L1c requires). The intermediates `b_C(d)` and `b_L(d)` are admissible tumblers under T10a's per-step inc rules — i.e., elements of the foundation carrier set `T` produced by structurally valid inc steps — but are not entries in any state component (not in `dom(C)`, `dom(L)`, or `E`). L1c is discharged by the chain's *existence* under T10a step-admissibility, not by an operational allocation event at each intermediate. This reconciles SubAllocatorAxiom's axiomatic activation of the sub-allocator anchors with L1c's chain-existence requirement: SubAllocatorAxiom packages the operational claim (the sub-allocator frontier is active with anchor `b_L(d)`), while L1c reads the same chain as a structural-producibility existential; both are satisfied by the witnessing chain just exhibited.

What the inc derivations do *not* supply is the *operational* claim that these anchors head two distinct simultaneously-active sub-allocator frontiers under `d`. T10a's at-most-once spawning constraint forbids minting `b_C(d)` and `b_L(d)` as two forward-frontier roots from a single operational spawning event at `d`. The sub-allocator frontiers are admitted by **SubAllocatorAxiom** below, which packages the operational claim: sub-allocator existence, cross-subspace disjointness, and the namespace property at each first emission. Once each anchor heads a frontier, the sub-allocator behaves as a T10a-conforming chain — SubAllocatorAxiom supplies only the bootstrap. Successive allocations advance each anchor's frontier:
...
Downstream of each sub-allocator's first emission, the frontier is a T10a-conforming `inc(·, 0)` chain whose subsequent emissions inherit T10a's GlobalUniqueness in full; SubAllocatorAxiom.Namespace underwrites only the bootstrap.
...
**LinkVPositionDepthAxiom (Axiom, FixedLinkVPositionDepth).** `(A d ∈ E_doc :: m_L = 2)` — every link-subspace V-position has depth 2. The lower bound `m_L ≥ 2` is structural (ordinal shift at depth 1 alters the subspace identifier, violating TA7a); LinkVPositionDepthAxiom instantiates the lower bound at 2. The axiom is load-bearing in the empty-subspace case at K.μ⁺_L (where S8-depth is vacuous and supplies no constraint). Design rationale and consultation evidence (Nelson LM 4/31; Gregory `do2.c:151–167`, `xanadu.h:144–146`, `do2.c:169–183`) are recorded in *Design provenance* below.
...
- V-position v_ℓ satisfies:
  - subspace(v_ℓ) = s_L
  - m_L = 2, supplied by **LinkVPositionDepthAxiom** (FixedLinkVPositionDepth, stated above at the head of this section). When V_{s_L}(d) ≠ ∅, m_L = 2 is consistent with S8-depth on existing link-subspace positions; when V_{s_L}(d) = ∅, m_L = 2 is the load-bearing axiomatic commitment rather than a parameter left open at the transition (S8-depth is vacuous on an empty subspace and supplies no constraint there). The full rationale, evidence (Nelson LM 4/31; Gregory do2.c:151–167, xanadu.h:144–146, do2.c:169–183), structural lower-bound argument (`m_L ≥ 2` via TA7a), and deferred-extension note are stated at the axiom site above
...
Every freshly allocated I-address appears in some arrangement in the post-state — the containing document may itself have been freshly created by K.δ in the same composite transition. This is an axiom of the state transition model, not a theorem of ASN-0036. S7a tells us that the prefix of a identifies the creating document, but it does not tell us that the creating document's arrangement must contain a — an address could be allocated into dom(C) with the correct prefix while appearing in no arrangement. The justification for J0 is design intent: in Nelson's model, content enters the docuverse by being placed in a document. There is no mechanism for creating "orphan" content that exists in Istream without any document displaying it. Gregory confirms: allocation always occurs in the context of a document operation that inserts the new content. J0 is *axiomatic* in this ASN, standing alongside SC-NEQ (this ASN), NodeUniqueAllocation, SubAllocatorAxiom, NoDeallocation (ASN-0034), and S0 (ASN-0036) as a load-bearing axiomatic premise: P7a's derivation, the ExtendedReachableStateInvariants J1★/J1'★ coupling proofs, and the fresh-content branch of P4★ all depend on J0, and the per-state invariant J1★ is *derived* from J0 by the wp analysis above (not axiomatic), so the distinction must be tracked. Every load-bearing-axiom enumeration in subsequent sections names J0 explicitly under this status.
...
The *ghost* document address `t = 1.0.1.0.5` is a valid document tumbler (IsDocument(t): zeros(t) = 2, ValidAddress(t)) that has never been allocated as an entity, so `t ∉ E₆`. We take `allocated(Σ₆)` (the state-indexed set of tumblers issued at the namespace-allocation layer up to Σ₆, per ASN-0034's AllocatedSet) to include `t` — `InTumblerUniverse(t, Σ₆)` holds (per the *Two scopes of "T10a's domain"* clarification at K.δ) but `¬InEntityAllocatorDomain(t, Σ₆)`. The layering is load-bearing: T10a's GlobalUniqueness at the entity-allocator layer is unavailable; TA5's tumbler-layer machinery does apply to the candidate `inc(t, 1)`. (Note: `allocated(s)` is distinct from the foundation carrier set `T` of ASN-0034's T0; the carrier `T` is the fixed set of all finite tumbler sequences over ℕ and is not state-indexed.)
...
- *`e₁ = inc(t, k)` with `t ∈ allocated(s)`, `k ∈ {0, 1, 2}`:* `t ∈ allocated(Σ₆)`, `k = 1`. ✓ (K.δ's quantifier is over `allocated(s)` (the state-indexed allocated set), not over `E`; `T` denotes the foundation carrier set (ASN-0034 T0) and is reserved for that role.)
...
| SC-NEQ | Axiom: `s_C ≠ s_L` — subspace identifiers are distinct |
...
| LinkVPositionDepthAxiom | Axiom (FixedLinkVPositionDepth): `(A d ∈ E_doc :: m_L = 2)` — every link-subspace V-position has depth 2. Parallel in form and provenance to SubspaceConventionAxiom; pins the link-subspace V-position depth to the value used by Nelson (LM 4/31: `[N.0.U.0.D.V.0.2.k]` shape, where the terminal pair `[2, k]` is the depth-2 link V-position) and reproduced in udanax-green (do2.c:151–167 `findnextlinkvsa` constructing the first link V-position via two successive `tumblerincrement` calls yielding `[2, 1]` of depth 2). Load-bearing in the empty-subspace case of K.μ⁺_L, where S8-depth is vacuous and supplies no constraint; consistent with S8-depth in the non-empty case. Stands alongside SC-NEQ, NodeUniqueAllocation, SubAllocatorAxiom, SubspaceConventionAxiom, NoDeallocation, and S0 as a load-bearing axiomatic premise |
...
| NodeLineage | Derived per-state invariant: `(A e ∈ E : IsNode(e) : n₀ ≼ e)` — every node in E descends structurally from the bootstrap node n₀ by tumbler-prefix relation. Discharged inductively from the base case `E₀ = {n₀}` (reflexivity) and the K.δ case (i) precondition `n₀ ≼ e`, which is the operative load-bearing premise |
...
This section consolidates the consultation evidence underwriting the definitional commitments of this ASN. Citations remain in the body where strictly needed; the full rationale for each commitment is recorded here.

**Bootstrap node `n₀ = [1]`.** The choice of `1` as the singular root value is pinned as a convention. Two interlocking Nelsonian readings underwrite it (consultation-68/answer-06):
- *Ancestral root of the docuverse* (LM 4/28): "the server address always begins with the digit 1, since all other servers are descended from it... it permits referring to the entire docuverse by '1' on the first position." `[1]` is the singular root from which all other node addresses descend by prefix extension.
- *"1-position" as wildcard for "all of this level"* (LM 4/38): "A digit of 'one' may be used to designate all of a given version, all versions of a given document, all works of a given author, all documents in a given project, all documents on a given server — or the entire docuverse." Patterns like `0.0.0.1`, `1.2.0.0.1` rely on `1` carrying "all of" semantics.

Choosing `[c]` with `c ≠ 1` would preserve the structure but lose Nelson's designed semantic overloading. Gregory's implementation realises the same shape: the granfilade has a single canonical root (`granf` fullcrum at `xanadu.h:13–14`, `corediskout.c:21`), and every node address is allocated as a descendant of that single root by query-and-increment.

**SubspaceConventionAxiom (`s_C = 1, s_L = 2`).**
- *Nelson (LM 4/30–4/31).* Nelson speaks of "subdivision of links by further digits (after '2' …)" and "the digit after the one indicates the byte position" — referencing the specific digit values `1` and `2` as concrete, named coordinates rather than as parameters standing for "any pair of distinct positive subspace identifiers." The design instantiation pins the digits.
- *Gregory (`xanadu.h:144–146` with comment "atom types — don't change these").* `TEXTATOM=1` and `LINKATOM=2` are not configurable constants: the values are fed directly into tumbler arithmetic at `granf2.c:162` (`tumblerincrement(&hintptr->hintisa, 2, hintptr->atomtype + 1, &upperbound)` — `atomtype + 1` evaluates to `3` for LINKATOM=2, bounding link allocation to element field `2`) and `granf2.c:166` (`tumblerincrement(&lowerbound, 2, hintptr->atomtype, isaptr)`, embedding the atom-type integer in the resulting ISA mantissa). The literal `2` recurs unguarded at `granf2.c:171` and `do2.c:94`. Swapping the values would shift every existing on-disk ISA address — the constants are encoded in the data format.

A hypothetical generalisation to `s_C = n, s_L = m` with `(n, m) ≠ (1, 2)` would require longer sibling-increment padding in the L1c witnessing chain; this ASN does not develop those generalisations.

**LinkVPositionDepthAxiom (`m_L = 2`).**
- *Nelson (LM 4/31).* The design fixes the form `[N.0.U.0.D.V.0.2.k]` for the k-th link of a document at depth 2 — subspace identifier `s_L = 2` plus a single positional component. Further subdivision of the link subspace by additional digits beyond the single position component is identified as a possible extension ("The subdivision of links by further digits (after '2' and the position) is a distinct possibility, and several possible uses have been discussed") but deferred to a future ASN.
- *Gregory (`do2.c:151–167`).* `findnextlinkvsa` constructs the first link V-position via two successive `tumblerincrement` calls on a zero tumbler — `tumblerincrement(&firstlink, 0, 2)` followed by `tumblerincrement(&firstlink, 1, 1)` — yielding `[2, 1]` of depth 2. Lockstep coupling to `LINKATOM=2` (`xanadu.h:144–146`) and `setlinkvsas` (`do2.c:169–183`, mirroring mantissa[0] discriminants 1, 2, 3 for FROM/TO/THREE endpoints) fixes the convention structurally.

**SubAllocatorAxiom (sub-allocator existence and namespace property at `b_C(d), b_L(d)`).** Nelson treats the content and link sub-spaces under each document as independent sub-allocators sharing a document-level root (LM 4/30–4/31, with link subspace "established by convention at document creation, not by a separate FEBE operation"). Gregory's implementation tracks content and link allocation as two separately-managed allocator records — `granf` for content, `spanf` for spans/links (`xanadu.h:13–16`, `entexit.c:44–45`) — with no parent struct pairing them under a single dual-frontier state and no T2-style spawning event linking the two. Both sources confirm the axiom's operational claim: the two sub-allocators are siblings-by-convention under each document, structurally inc-witnessed at document-creation time. udanax-green's first link VSA `2.1` is a hardcoded constant from `findnextlinkvsa` (`do2.c:151–167`), not derived from the document's own ISA.

**NodeUniqueAllocation (every K.δ node event produces a fresh address).** Two equally admissible protocol realisations underwrite the axiom:
- *Nelson's hierarchical baptism* (LM 4/19–4/20). A parent node ceremonially issues a child node's identifier as a top-level sibling under its own root. Nodes `1.1`, `1.2`, `1.3` are forked from a root node `1`.
- *Gregory's single global granfilade with query-and-increment dispatch* (`granf2.c:209`). All node identifiers descend from a single granfilade tree by the same query-and-increment that governs accounts and documents; the implementation allocates NODE→NODE with depth = 1, producing forms such as `1.1.0.1.1`.

T10a alone does not underwrite node-namespace uniqueness (the `inc(·, k)` operator extends within a single ownership domain rather than minting top-level identifiers across independent actors), so the axiom is needed regardless of protocol mechanism. The spec leaves the mechanism unspecified — any allocator satisfying the namespace property suffices.

**K.δ k = 1 ghost-base versioning.** Nelson's ghost-element doctrine (LM 4/23: "no specific element need be stored in tumbler-space to correspond to them") extends explicitly to documents, and version addressing inherits the relaxation (LM 4/29; LM 2/19). Consultation-54/answer-05 confirms from both directions: Nelson states "documents are explicitly named, not just servers and accounts" and that version addressing places "no requirement that an immediate parent be a live entity"; Gregory confirms `docreatenewversion` does not enforce a bert-style liveness check (uses `NOBERTREQUIRED`, falls through to `fetchorglgr` checking only structural validity, not open-list membership), and documents an ordering bug at `do1.c:277` vs `do1.c:281` (version ISA allocated before source check, so non-existent source causes allocated-but-leaked granfilade slot). Inheriting the implementation's structural-existence check at the abstract layer would inherit the leak; the source-liveness requirement is therefore a FEBE-layer concern.

**Link element-field depth (L1b: `#E(ℓ) ≥ 2`).** Nelson's `[N.0.U.0.D.V.0.2.k]` shape (LM 4/31) requires both the subspace identifier `s_L = 2` and at least one positional component `k`, giving element field `[s_L, k]` of depth ≥ 2. Gregory's implementation realises this via SubAllocatorAxiom's first emission `[d.0.s_L.1]` and subsequent siblings by `inc(prev, 0)` (length-preserving under TA5(c)).

**Versioning under T10a's standard spawning machinery (live-operand k = 1 only).** Nelson describes versions as continuations of the Document field using the same digit-forking mechanism rather than as a separate namespace (LM 4/29). Gregory's `docreatenewversion` (`do1.c:271`) dispatches through the same `granf` granfilade record as `docreatenewdocument`. The implementation conflates the physical record across the spawn event; the abstract T10a structure registers a fresh sub-allocator with distinct spawnPt/spawnParam for the live-operand case. This is principled rather than an asymmetry with SubAllocatorAxiom: content and link sub-allocators require axiomatic activation because the pair-shape is outside T10a's at-most-once spawning, while the live-operand version sub-allocator produces a single output per spawn event and lies within T10a's ordinary admissibility.

**K.μ⁺_L origin restriction (`origin(ℓ) = d`).** Link transclusion is excluded by design. Nelson: "A document includes only the links of which it is the home document" (LM 4/31). Byte streams admit transclusion (LM 4/10); links do not. Links maintain "permanent order of arrival" in their home document, and home document determines ownership (LM 4/12: "A link need not point anywhere in its home document. Its home document indicates who owns it"). Gregory's implementation achieves origin matching by procedural atomicity — `docreatelink` both allocates the link ISA under the document's address and places it in the document's arrangement in a single operation — but no runtime guard exists; `acceptablevsa` unconditionally returns TRUE and `docopy` performs no origin check. K.μ⁺_L's `origin(ℓ) = d` precondition formalises the structural guarantee that the implementation achieves by construction.

**Link-subspace correspondence-run structure — out of scope.** Link addresses have no V/I distinction at the design layer (Nelson LM 4/24, 4/31): a link is addressed `N.0.U.0.D.V.0.2.k` with the final position `k` serving simultaneously as identity, V-position, and serial creation number. "The links designated by a tumbler address are in their permanent order of arrival" (LM 4/31). The arrangement-order/creation-order distinction does not arise in the link subspace, so batch operations enumerate over a single permanent order rather than join across rearranged orderings. Gregory's implementation exposes no operation that traverses a contiguous run of link V-positions as a structural unit: `followlink` (`fns.c:114–127`, `sporgl.c:67–95`) takes a single link ISA, `retrieve_endsets` and `find_links` (`fns.c:189–234, 350–362`, `spanf1.c:56–103, 190–235`) take content specsets as input, and pagination uses link-ISA tumblers as cursors. The per-subspace D-SEQ★(s_L) shape suffices as the link-subspace structural invariant at this ASN.
