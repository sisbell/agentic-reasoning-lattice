...
*Staging of the link store.* The link store `L` and everything that depends on it — the link-allocation transition K.λ, the link-subspace arrangement transition K.μ⁺_L, and the L-invariants (L0, L1, L1a, L1b, L1c, L3, L12, L14, L-fin, CL-OWN, CL-UNIQ) — are characterised once, in the *Link store and extended system state* section below. Until that section, every Σ is read with `L = ∅`, under which all L-related conjuncts are vacuous; no result preceding that section depends on the link store's contents.
...
*Why link depth is permanent across an empty subspace while content depth is not.* The asymmetry tracks where each entity's permanent identity resides. A link is *named* by its link-subspace V-position — its permanent order of arrival in its home document, never reassigned (Nelson: "the links designated by a tumbler address are in their permanent order of arrival", LM 4/31). The V-position depth is therefore part of that permanent name, so it is a standing structural commitment of the document: were an emptied link subspace re-baptised at a different depth, the addresses that link names once occupied would no longer parse under the new depth, violating tumbler address permanence (T0(b), ASN-0034: "new items may be continually inserted while the other addresses remain valid"). Content carries no analogous commitment because content's permanent identity is its I-address (ASN-0036 S0/S4), not its V-position; the content V-position is disposable arrangement, freely renumbered by editing, so its depth is supplied fresh per insertion by ValidFirstInsertionPosition (ASN-0036, any `m ≥ 2`) and need not persist across an empty content subspace. Thus link depth is fixed cross-state by the same address-permanence principle that leaves content depth free.
...
S3★(Σ') for a K.μ~ event holds *by the admissibility filter* — admissibility clause (i) admits only those π whose induced post-state satisfies S3★(Σ'), so the verification-matrix cell for S3★ under K.μ~ is true by construction, not derived from the decomposition. What the dependency chain supplies is the *realisability* that makes K.μ~ a non-vacuous operation: that an admissible π exists and that the K.μ⁻ + K.μ⁺ decomposition actually realises it.

The argument proceeds in five steps, (A) → (B) → (C) → (D) → (E), each building only on its predecessors. (A) derives subspace preservation under π — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — from admissibility (i)'s stipulated S3★(Σ'), S3★(Σ) (inductive hypothesis), the bijection equation, and L14. (B) shows the K.μ⁻ + K.μ⁺ full-clearance decomposition realises any admissible π — its mechanical post-state being faithful to (not violating) the admissibility-stipulated S3★(Σ') — consuming Step (A)'s subspace preservation. (C) establishes the link-subspace functional identity `M'(d)|_{dom_L} = M(d)|_{dom_L}` (Steps 1–3 of *Link-subspace fixity* below). (D) derives pointwise fixity `π(v) = v` on `dom_L` (Step 4 below; premise: CL-UNIQ at the pre-state Σ, the only step consuming it). (E) closes admissibility clause (ii)'s existence condition — `M(d)|_{dom_C}` takes at least two distinct values (*Decomposition* below).
...
*Temporal scoping of J0 (composite-boundary, not per-state).* J0 binds the initial state Σ and the final state Σ' of a composite, not each intermediate atomic state; its existential may transiently fail mid-composite (after K.α places `a` into `dom(C)`, before the trailing K.μ⁺ installs the witnessing `v ↦ a` mapping) and is restored at Σ'. The composite-boundary verification matrix in *Class (b)* below catalogues this scoping (P7a row).
