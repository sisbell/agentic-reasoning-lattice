...
**L-fin (LinkStoreFiniteness, restated from ASN-0043).**

  `|dom(Σ.L)| < ∞`

The link store is finite at every reachable state. This restates ASN-0043's L-fin for self-contained use in this ASN's invariant theorem. ASN-0043 establishes L-fin by induction over K.λ (the only link-allocating elementary transition): the initial state has dom(L₀) = ∅ (finite); K.λ extends dom(L) by one element (preserving finiteness); all other transitions hold L in frame (L' = L preserves finiteness trivially). L-fin underwrites the well-definedness of `max{ℓ' ∈ dom(L) : origin(ℓ') = d}` in K.λ's subsequent-link case and bounds the number of orphan links produced by repeated K.λ-alone composites at any finite point in the transition sequence.
...
*Scope and harmlessness.* K.δ as stated in this ASN does *not* require, for k = 1 events from a document, that the corresponding base `[N, 0, U, 0, D]` (or any prior version `[N, 0, U, 0, D, k']` with `k' < k`) be in E_doc. The precondition reads only `t ∈ T`, `e = inc(t, k)`, T10a's zeros-count constraints, and the structural agreement clauses; it does not constrain the document-level neighbourhood of e beyond `parent(e) ∈ E`, which T10a satisfies through t alone. We claim that this omission is *harmless within this ASN's scope*: no invariant introduced in this ASN — P0–P8, S0–S9, L0–L14, J0–J4, D-CTG, D-MIN, D-SEQ, P3★, P7a, S3★, GlobalUniqueness, NodeUniqueAllocation, SubAllocatorAxiom — refers to a base-to-version relationship or to the existence of intermediate version-shaped addresses below `e`. Specifically: P8 (entity-hierarchy spine) follows the parent(·) chain, which (as noted above) does not relate a version to its base — parent traverses zero-separated levels, and the version-base step crosses no zero separator, so P8 imposes no constraint between e and its "base." P1 (entity-store finiteness) and S2/S3 (functionality/referential integrity of M, L) treat each entity independently; whether a sibling at `[N, 0, U, 0, D]` exists is irrelevant to whether e's own arrangement is well-formed. T10a's GlobalUniqueness guarantees `e ∉ E` from t = a previously allocated document address — this property is purely about address distinctness, not lineage. Hence the K.δ rule for k = 1 admits structurally well-formed events even when the conceptual "base" is absent, and every invariant in this ASN remains satisfiable post-K.δ.
...
*Precondition:*
- `d ∈ E_doc`.
- *Admissible removal (per-subspace suffix or full-subspace clearance).* For each non-empty subspace S in M(d), the input arrangement has a contiguous, minimum-anchored, uniform-depth range. We state and derive this structural form locally — relying only on the per-state invariants D-CTG, D-MIN, S8-depth, S8-fin, and S8a — so that K.μ⁻'s admissibility precondition appeals to no forward result.
...
Steps 1–3 of this example exercise — explicitly or via the frame-preserved annotations — every invariant in ExtendedReachableStateInvariants's conjunction, providing a worked confirmation of the inductive step for the K.λ, K.μ⁺_L, and K.μ~ transition kinds on a concrete two-subspace state.
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the transitions K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~ (shorthand for its K.μ⁻ + K.μ⁺ decomposition), and K.ρ — satisfies:
  ...
  S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG ∧ D-MIN ∧ D-SEQ★ ∧ P0 ∧ P1 ∧ P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L12 ∧ L14 ∧ L-fin ∧ CL-OWN

This supersedes the earlier ReachableStateInvariants theorem by replacing S3 with S3★, P4 with P4★, P3 with P3★, P5 with P5★, adding S3★-aux (subspace exhaustiveness), CL-OWN (link-subspace ownership), L3 (triple endset structure), the per-subspace amendment D-SEQ★, and the foundation invariants previously inherited tacitly from ASN-0036 (S4, S7a–S7d, S9) and from ASN-0043 (L1b, L-fin); covering the extended transition set including K.λ and K.μ⁺_L.
...
**Class (a): Elementary invariants** — preserved by each elementary transition individually. These are all invariants except P4★ and P7a: S0, S1, S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8, S9, D-CTG, D-MIN, D-SEQ★, P0, P1, P2, P3★, P5★, P6, P7, P8, L0, L1, L1a, L1b, L3, L12, L14, L-fin, CL-OWN.
