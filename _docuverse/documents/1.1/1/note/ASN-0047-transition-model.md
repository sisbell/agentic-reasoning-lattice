...
(†) holds by the lockstep K.δ effect (its `Document`-case registration grows `dom(M)` and `E_doc` together by the same `{e}`, placing each allocated document into `E_doc` and initialising `M(d) = ∅`) together with the default-value convention that, for `d ∉ E_doc`, fixes `M(d) = ∅`; the two sets therefore have identical membership at every reachable state. Inherited `dom(M)`-phrased foundation results apply under (†), with `E_doc` naming the document set.
...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases. The freshness conjunct `e ∉ E` is discharged against the present state Σ in every case; the disciplines differ only in *which* state fact the guard reads. At **k = 0** (sibling advance) the check `inc(t, 0) ∉ E` reads the current frontier index: FrontierEquivalence characterizes it as the condition that `t` is the frontier of its chain — there is no structural fact that forces it, since a sibling of an already-allocated `t` could collide with a previously advanced sibling. At **k ∈ {1, 2}** (child-spawn) the same `e ∉ E` conjunct *is* the enforcement of T10a's at-most-once-per-`(t, k')` discipline: it reads whether the spawn `(t, k')` has already been performed — a fact logically equivalent to `inc(t, k') ∈ E`, detectable only against the live state. GlobalUniqueness (ASN-0034) does not supply this freshness on its own; it establishes distinctness only *across distinct allocation events*. Once `e ∉ E` grants that this `(t, k')` spawn is a new event (rather than a repeat the at-most-once rule forbids), GlobalUniqueness supplies `e`'s distinctness from every address produced by every prior event. So both disciplines rest on the same live-state `e ∉ E` conjunct (this is the discharge the worked examples and S7d invoke); the genuine difference is the state fact each encodes — current frontier index for k = 0 versus single-spawn occurrence for k ∈ {1, 2}. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
  - *k = 0 (sibling):* `t ∈ E ∧ ¬Node(t)`. These are the only sub-case-specific operand-admissibility conjuncts: `t ∈ E` (the operand must be an allocated entity) and `¬Node(t)` (so `parent(t)` is well-defined under T4b). No additional freshness conjunct is imposed here — the case-level `e ∉ E` *is* the k = 0 frontier check, since `e = inc(t, 0)`, and FrontierEquivalence characterizes it as the condition that `t` is the frontier of its chain. The structural identities `parent(t) = parent(e)` and `zeros(t) = zeros(e)` hold by TA5(c) on `e = inc(t, 0)` (K.δ-ID.parent-0/1, K.δ-ID.zeros-0/1).
...
**Step 2: K.δ case (ii) k = 2 — allocate account `1.2.0.1 = inc(1.2, 2)`.** TA5(d) gives `zeros = 1`, `parent = 1.2`. Preconditions: `parent(e) = 1.2 ∈ E₁`; `zeros(1.2) = 0 ≤ 2`; `1.2.0.1 ∉ E₁` holds because this is the first `(1.2, 2)` spawn — a state fact read against `E₁`, and the live-state enforcement of T10a's at-most-once-per-`(t, k')` discipline — with GlobalUniqueness (ASN-0034) supplying only `1.2.0.1`'s distinctness from every prior allocation event once freshness is granted. Effect: `E₂ = E₁ ∪ {1.2.0.1}`.

**Step 3: K.δ case (ii) k = 2 — allocate document `1.2.0.1.0.1 = inc(1.2.0.1, 2)`.** TA5(d) gives `zeros = 2`, `parent = 1.2.0.1`. Preconditions analogous to Step 2; `d ∉ E₂` holds because this is the first `(1.2.0.1, 2)` spawn (read against `E₂`), with GlobalUniqueness (ASN-0034) at the document sub-allocator `A_doc(1.2.0.1)` supplying only cross-event distinctness once freshness is granted. Effect: `E₃ = E₂ ∪ {1.2.0.1.0.1}`, with `M₃(1.2.0.1.0.1) = ∅` and SubAllocatorBundle activating the content and link sub-allocators (anchors `b_C(d) = [d.0.1]`, `b_L(d) = [d.0.2]`).

**Step 4: K.δ case (ii) k = 0 — allocate sibling document `1.2.0.1.0.2 = inc(1.2.0.1.0.1, 0)` under the same account.** Operand `t = 1.2.0.1.0.1`, the document placed by Step 3. The k = 0 dispatch differs from k ∈ {1, 2} in two distinctive features: (a) freshness `inc(t, 0) ∉ E` is discharged by the k = 0 limb of the K.δ definition's per-k freshness mechanism, which reads the current frontier index (FrontierEquivalence), rather than the at-most-once-per-`(t, k')` live-state check used at k ∈ {1, 2} (both are live-state `e ∉ E` reads against Σ; they differ only in which state fact the guard encodes); and (b) the dispatch through T10a.6 (DomainDisjointness) routes the verification against t's actual provenance — for this t, `A_doc(parent(t)) = A_doc(1.2.0.1)`, the account's document sub-allocator activated by Step 3.
