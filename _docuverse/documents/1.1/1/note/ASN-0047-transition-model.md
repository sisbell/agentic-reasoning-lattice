...
Each non-node K.δ event's freshness conjunct is discharged by the per-k mechanism stated in the K.δ definition, with GlobalUniqueness (ASN-0034) preserving the resulting distinctness; this section does not re-walk that regime. What follows is the parent-allocator-activation material the definition does not contain: the regime differs by k in its operand, the parent entity-level sub-allocator on which the step `e = inc(t, k)` acts, and the applicable allocator-discipline clause:

- *k = 0 (sibling under existing allocator):* operand `t ∈ E` already inhabits the parent's tracked domain by K.δ's `parent(t) = parent(e)` precondition; the step `e = inc(t, 0)` is a T10a sibling-advance on the activated parent allocator.
