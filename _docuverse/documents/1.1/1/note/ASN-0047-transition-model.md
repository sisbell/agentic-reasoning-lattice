...
**Sub-allocator names** (per ASN-0093 for d-rooted sub-allocators; A_doc and A_account introduced here as the entity-hierarchy generalisations). The naming family `A_·(·)` covers every T10a sub-allocator in the docuverse layer's allocator tree, indexed by the entity at the parent's level. Five members of the family are named in this ASN, three rooted at a document and two rooted at higher entity-hierarchy levels:
...
- *Content-subspace pattern.* `V_{s_C}(d') = V_{s_C}(d)` — empty removal, `n'_{s_C} = n_{s_C} = 2`. This is the zero-suffix admissible pattern.
- *Link-subspace pattern.* `V_{s_L}(d) = {[2,1], [2,2]}`, `V_{s_L}(d') = {[2,1]}` — a 1-element suffix removal with `n'_{s_L} = 1` (admissible).
...
| D-CTG★ / D-MIN★ | frame | frame | frame | precondition discharge (K.μ⁺'s original precondition list, in *Elementary transitions*, requires the resulting M'(d) to satisfy D-CTG / D-MIN — strengthened to D-CTG★ / D-MIN★ in the extended state: content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | constructive precondition discharge — the per-subspace retention count `n'_S` directly produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` which satisfies D-CTG★ and D-MIN★ at Σ' by construction; see *K.μ⁻ admissible contraction shape* for the equivalence proof | K.μ⁻ (full-clearance) leaves link subspace contiguous via fixity and content subspace empty (vacuous); K.μ⁺ preconditions re-establish D-CTG★ / D-MIN★ on the rebuilt content arrangement (the K.μ⁺ cell's discharge above applies at the rebuild step) | frame |
