...
**Typing note (M total — overrides foundation).** ASN-0036 and ASN-0093 type the arrangement family as *partial*, `M(d) : T ⇀ T`, with `dom(M)` serving as the set of allocated documents. This ASN deliberately *overrides* that typing: here M is *total* on T, with `M(d) = ∅` (the empty partial function) for every `d ∉ E_doc`. The document-set role that `dom(M)` played in the foundations is now carried by E_doc, under the identity `d ∈ dom(M) ⟺ d ∈ E_doc`. This single identity translates every inherited foundation precondition and effect: wherever ASN-0093 writes `d ∈ dom(M)` (in K.α, K.λ, K.σ), read `d ∈ E_doc`; ASN-0093's K.σ effect `dom(M') = dom(M) ∪ {d}` becomes the entry of `d` into E_doc.
...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ ValidAddress(e) ∧ ¬IsElement(e)` apply uniformly to all three sub-cases; in particular, the freshness conjunct `e ∉ E` is discharged per sub-case by the discharge route detailed in §*K.δ case (ii) discharge and parent-allocator activation* below: T10a chain-advancement uniqueness at `(t, 0)` (FrontierEquivalence) at k = 0; GlobalUniqueness (ASN-0034) on the parent allocator's tracked domain at k = 1 and k = 2. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
...
**SubAllocatorAxiom (per ASN-0093, ContentLinkSubAllocatorExistence).** The axiom is taken from ASN-0093 directly. For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]`. Four of the five sub-clauses (Subspace, FirstEmission, Namespace, T10aConformance) are inherited from ASN-0093 without modification. The fifth, Disjointness, is *not* inherited verbatim — it is discharged here as a **local lemma**, because its conclusion is stated over ASN-0047's own anchor constructs `b_C(d) = [d.0.s_C]`, `b_L(d) = [d.0.s_L]` and its proof leans on the CrossDocDisjoint lemma (below) and T7, both invoked over those anchors:
...
- *k = 2 (descent off an account, `zeros = 1 → 2`).* The operand `t` is an account (`zeros(t) = 1`); K.δ-ID.zeros-2 gives `zeros(d) = zeros(t) + 1 = 2`. Freshness (`e ∉ E`) is discharged by GlobalUniqueness (ASN-0034) on the parent account's document sub-allocator.
- *k = 1 (version off a document, `zeros = 2` preserved).* The operand `t` is a document (`zeros(t) = 2`); K.δ-ID.zeros-0/1 preserves `zeros(d) = zeros(t) = 2`. Freshness is discharged by GlobalUniqueness (ASN-0034) at k = 1.
- *k = 0 (sibling off a document, `zeros = 2` preserved).* The operand `t` is a document (`zeros(t) = 2`); K.δ-ID.zeros-0/1 preserves `zeros(d) = zeros(t) = 2`. Freshness is discharged by FrontierEquivalence (`inc(t, 0) ∉ E`).
