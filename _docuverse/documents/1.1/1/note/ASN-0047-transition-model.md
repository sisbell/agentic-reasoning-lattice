...
*Subspace-position correspondence.* The two projections `subspace(v)` and `subspace_I(a)` apply at different state-component levels — `subspace(v)` projects V-positions, `subspace_I(a)` projects I-addresses. Their correspondence `subspace(v) = subspace_I(a)` for `M(d)(v) = a` is not notation but a derived consequence; it is established at S3★ (*Generalized referential integrity*) from S3★ + L0 + S3★-aux.
...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases. The freshness conjunct is a caller-checked guard — `inc(t, 0) ∉ E` at k = 0 and `e ∉ E` at k ∈ {1, 2} — checked against the present state Σ, not derived from a structural fact; at k = 0, FrontierEquivalence (the *derived* form of T10a's chain-advancement uniqueness at `(t, 0)`) characterizes it as the condition that `t` is the frontier of its chain. The parent entity-level sub-allocator on which each step acts, the spawn-admissibility conjuncts (for the child-spawn regimes k ∈ {1, 2}), and the allocator-discipline properties the guard maintains are discharged uniformly in §*K.δ case (ii) discharge and parent-allocator activation*. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
...
*Modeling choice (layer separation).* D-CTG★/D-MIN★ constrain the *arrangement* layer `M(d)` — a presentational mapping from V-positions to I-addresses — never `dom(L)`, the permanent link address space. The strengthening is admissible because link permanence is discharged independently on `dom(L)` by L12 (LinkImmutability): every withdrawn link address persists forever, addressable or not. Nelson locates that permanence in I-space — a withdrawn link keeps its permanent order-of-arrival address, retained for historical backtrack (LM 4/9) — an I-space fact, not an arrangement-layer obligation to hold a positional gap. D-CTG★/D-MIN★, acting only on `M(d)`, thus do not contradict tombstoning.
...
- *Subsequent emission* (predicate: `{ℓ' ∈ dom(L) : origin(ℓ') = d} ≠ ∅`): `ℓ = inc(max{ℓ' ∈ dom(L) : origin(ℓ') = d}, 0)` (TA5(c)), the next sibling on `A_L(d)`'s inc chain. The `max` is well-defined by the same argument as K.α's subsequent emission (with L-fin in place of C-fin).
...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. Below, under case (ii)'s k-labels, we give the *parent-allocator activation* (which allocator each step acts on), the spawn-admissibility conjuncts, and, for the child-spawn regimes, the spawnPt premise. By k:
...
**Link-subspace fixity and realisation.** Clause (v) supplies pointwise fixity `π(v) = v` for every `v ∈ dom_L(M(d))` on the admissible class directly, as a hypothesis on that class. The sub-steps below establish what the full-clearance decomposition contributes: the K.μ⁻ + K.μ⁺ realisation preserves the link subspace functionally, `M'(d)|_{dom_L} = M(d)|_{dom_L}` (sub-steps (1)–(3)), and this functional identity carries CL-UNIQ to the post-state Σ' and confirms the realised π agrees with clause (v) pointwise (sub-step (4); premise: CL-UNIQ at the pre-state Σ). Proof:
...
**Link V-position permanence.** Clause (v) bars a *single* K.μ~ from reordering the link subspace, but it does not pin a link's V-position across its lifetime, and the invariant set as a whole imposes no such global non-rearrangeability guarantee. A withdraw-and-re-add composite re-seats a link without violating any invariant: K.μ⁻ removes a link's V-position (suffix-only, so removing `[s_L, …, 1]` forces removal of `[s_L, …, 2]` as well), and two subsequent K.μ⁺_L steps re-append the two links in the opposite order, yielding a genuine re-seating (`ℓ₂ ↦ [s_L, …, 1]`, `ℓ₁ ↦ [s_L, …, 2]`) that preserves both CL-OWN and CL-UNIQ. The K.μ⁺_L first-arrangement guard `ℓ ∉ ran(M(d))` tests only the *current* arrangement, not history, so a previously-withdrawn link passes it. Hence the presentational order of arrival in the arrangement layer is mutable (P3): the model permits link re-positioning and does not detect it. What is permanent is the link's *I-address*, allocated once by serial order of creation and never recycled — Gregory's implementation places each link by append-at-end (`findnextlinkvsa`) and, on removal, leaves a reverse-orphan at the I-space layer while a re-added link is always a *new* object at the current tail; order-of-arrival is encoded once at creation and is not recoverable from the V-position after withdrawal. Nelson's "permanent order of arrival" (LM 4/12) is therefore carried by the I-address subspace ordinal, not by the rearrangeable V-position, and this model enforces it only at the I-address layer. The same V-stream reading underwrites clause (iii): REARRANGE transposes contiguous regions within a document's flat, dense V-stream (Nelson 4/67), reordering byte positions the document already holds rather than relocating content to a new structural depth — the V-stream has no internal depth levels for content to be moved into.

**K.μ~ range-invariance.** K.μ~ does *not* preserve per-position values on dom_C — reassigning which V-position carries which I-address is the whole point of reordering, `M'(d)(π(v)) = M(d)(v)`; what it preserves is the *range*, the set of content I-addresses. Because π bijects dom(M(d)) onto dom(M'(d)) (K.μ~-FIX) carrying dom_L onto dom_L (Link-subspace fixity above), it carries the complement dom_C onto dom_C, so applying the bijection equation `M'(d)(π(v)) = M(d)(v)` over the content-subspace positions gives the *content-range* equality `{a : (E u ∈ dom_C(M'(d)) : M'(d)(u) = a)} = {a : (E v ∈ dom_C(M(d)) : M(d)(v) = a)}` — i.e. `ran(M'(d)|_{dom_C}) = ran(M(d)|_{dom_C})` — and hence `Contains_C(Σ') = Contains_C(Σ)`. Full-range invariance `ran(M'(d)) = ran(M(d))` is not immediate from this content half alone, since `ran(M'(d)) = ran(M'(d)|_{dom_C}) ∪ ran(M'(d)|_{dom_L})`; we close the link half from the functional identity `M'(d)|_{dom_L} = M(d)|_{dom_L}` (Link-subspace fixity, sub-step (3) above), which gives `ran(M'(d)|_{dom_L}) = ran(M(d)|_{dom_L})`. Taking the union of the two subspace equalities yields `ran(M'(d)) = ran(M(d))`. We label this conclusion **K.μ~ range-invariance**. In particular ran(M'(d)) \ ran(M(d)) is empty, so the J1★ coupling has no new containment pairs to record.
...
Validity of a composite transition `Σ →* Σ'` is governed by **ValidComposite★**, defined in *Scoped coupling constraints*.
...
By K.μ~ range-invariance (derived in the *Decomposition of K.μ~* section above), Contains(Σ') = Contains(Σ). All invariants are trivially maintained; no co-occurring transition is needed.

**J4 (Fork composite).** Nelson's forking creation mode — version creation with ancestry indication (LM 4/66, CREATENEWVERSION) — is a composite whose elementary steps are exactly K.δ + K.μ⁺ + K.ρ, all serving the new document d_new. Fork is version creation on d_src's version chain `A_v(d_src)`; its K.δ allocation discipline and content-source operand-tracking rule are stated once in Definition (Fork) below. In every case d_new extends d_src's document-field prefix (d_src ≼ d_new), so d_new carries the ancestry-by-address indication regardless of whether the emitting step is k = 1 or k = 0. This matches Gregory's `docreatenewversion`, which performs the identical `findisatoinsertnonmolecule` allocation for every version (the first allocation produces `d_src.1`, subsequent allocations advance the last component to `d_src.k+1`), special-casing neither. By contrast, the k = 0 *sibling-document* allocation under the source's account (`docreatenewdocument`) and the k = 2 hierarchical descent are *not* forks: their outputs do not extend d_src's prefix and carry no ancestry-by-address indication. The precise checkable discriminator separating a k = 0 fork from an independent sibling-document allocation — which allocator's frontier the k = 0 operand sits on — is stated once in Definition (Fork) below, which the intro here invokes by reference.

**Definition (Fork).** A *fork* of d_src to d_new is a composite transition `Σ →* Σ'`. Write `d_op` for the *content source operand* of the fork — the operand `t` of the K.δ step (i), whose current content subspace is transcluded into d_new. **Allocation and operand-tracking rule.** Fork is version creation on d_src's version chain `A_v(d_src)`, with K.δ address allocation uniform across first and subsequent versions; the content source operand tracks the same K.δ sub-case. This is the sole statement of both the allocation discipline and the operand-tracking rule; the J4 intro above and steps (i)–(ii) below invoke it by reference rather than re-derive the k-split. The two sub-cases are:
...
The *precondition* is d_src ∈ E_doc ∧ d_op ∈ E_doc ∧ V_{s_C}(d_op) ≠ ∅, together with the per-sub-case activation condition on the K.δ step stated above, which also fixes `d_op` and supplies the structural discriminator separating a fork from an independent sibling-document creation.
...
Provenance coupling must be scoped to content-subspace arrangement extensions, scoped per P4★ above. This motivates the content-subspace forms J1★ and J1'★ below.
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ by a finite sequence of *elementary* transitions drawn from valid composites — i.e. every elementary-transition target, including the intermediate states reached partway through a composite — satisfies the *per-state invariants* below. Every state at a composite boundary (the initial Σ or final Σ' of a valid composite) additionally satisfies the *composite-boundary properties* below.
...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | By the induction-along-the-witnessing-trace mechanism of the P4a definition box. | Not evaluated at intermediate states — P4a is a trace property whose witnessing existential ranges over composite boundaries `{Σ₀, ..., Σ_n}`, not per-state, so it has no per-state value to fail; it is discharged at the boundary Σ', which carries the witness placed by K.μ⁺. |
...
P4★ (`Contains_C(Σ) ⊆ R`): For each `(a, d) ∈ Contains_C(Σ') \ Contains_C(Σ)`, J1★ at the composite boundary requires `(a, d) ∈ R'`. K.α, K.δ, K.ρ hold M in frame; K.μ⁺_L adds only link-subspace V-positions (excluded from Contains_C); K.μ⁻ can only shrink Contains_C; K.μ~ preserves Contains_C exactly by **K.μ~ range-invariance** (`Contains_C(Σ') = Contains_C(Σ)`, derived in the *Decomposition of K.μ~* section). Only K.μ⁺ may transiently violate P4★ (it adds a new content-subspace containment to Contains_C while framing R); J1★ supplies the co-occurring K.ρ that places `(a, d) ∈ R'`, restoring the bound at the composite boundary.
...
These properties are foundation invariants of ASN-0093 (or earlier foundation ASNs). Their preservation under this ASN's new transition (K.μ⁺_L) and amended transitions (K.λ, K.μ⁺, K.μ⁻, K.μ~) is verified locally in the Class (a) matrix (e.g. the L0/L1c link-preservation arguments there).
...
| L14a | Inapplicable in the extended state (see *L14a inapplicability* prose above) | ASN-0043's L14a (NonTranscludability) |
