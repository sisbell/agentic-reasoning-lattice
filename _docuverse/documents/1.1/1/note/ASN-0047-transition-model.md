...
Freshness `a ∉ dom(C) ∪ dom(L)` is discharged in three parts, self-contained at this point of statement: *seed* (first emission) — the determinate `[d.0.s_C.1]` is fresh against `dom(C) ∪ dom(L)` by FirstEmissionFreshness (ASN-0093); *frontier* (subsequent emission) — `a = inc(·, 0)` advances strictly past every prior sibling on the tracked `A_C(d)` chain, fresh against `dom(C)` by GlobalUniqueness (ASN-0034); *cross-subspace* — freshness against `dom(L)` is SC-NEQ + T7 (SubspaceDisjointness, ASN-0034). These three parts are collected, with the supporting SubAllocatorBundle machinery, as SubAllocFresh at `x = C` (*Allocator hierarchy under documents*, below).
...
Under a K.μ⁻ contraction, S8★ is discharged per subspace by the same two routes used to establish it, applied to the contracted projections. *Content subspace:* S8★(s_C) — which requires condition (c), uniqueness of the maximal-run decomposition — is re-established by reapplying ASN-0036's S8 to the contracted content-subspace projection `M'(d)|_{V_{s_C}(d)}`. Its preconditions (S2, S3★, S8a, S8-depth, S8-fin) are preserved by restriction, as the rest of the verification matrix shows; the trivial length-1 fall-back is *not* used here, since it would establish only a run-cover (conditions (a) and (b)) and not condition (c). *Link subspace:* S8★(s_L) omits (c) by definition, so its discharge reuses the trivial length-1 decomposition — every surviving V-position constituting its own length-1 run — which is always available after contraction.
...
Freshness `ℓ ∉ dom(L) ∪ dom(C)` is discharged in three parts, self-contained at this point of statement: *seed* (first emission) — the determinate `[d.0.s_L.1]` is fresh against `dom(C) ∪ dom(L)` by FirstEmissionFreshness (ASN-0093); *frontier* (subsequent emission) — `ℓ = inc(·, 0)` advances strictly past every prior sibling on the tracked `A_L(d)` chain, fresh against `dom(L)` by GlobalUniqueness (ASN-0034); *cross-subspace* — freshness against `dom(C)` is SC-NEQ + T7 (SubspaceDisjointness, ASN-0034). These three parts are collected, with the supporting SubAllocatorBundle machinery, as SubAllocFresh at `x = L` (*Allocator hierarchy under documents*).
...
*Step (A) — Admissible π coincide with realisable π.* Subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` holds for every *admissible* π directly by admissibility clause (iv). What this step establishes is the matching realisability direction: every subspace-preserving (hence admissible) π is *realisable* — produced by the K.μ⁻ + K.μ⁺ decomposition in the full-clearance form (canonical statement at the head of this section) — and conversely every realisable π is subspace-preserving, so the two classes coincide and the decomposition realises exactly the admissible π. S3★-aux(Σ) confines every source subspace to `{s_C, s_L}`, so the two cases below are exhaustive.
...
*Necessity and sufficiency of the precondition.* The precondition is stated against the pre-state `Σ` of a K.μ~ event; its necessity direction consumes CL-UNIQ at `Σ`, which holds by the ExtendedReachableStateInvariants inductive hypothesis on the per-state invariants at the reachable pre-state.
  ...
  - *Sufficiency (explicit construction).* When `M(d)|_{dom_C}` takes at least two distinct values, an admissible π always exists; we exhibit a concrete witness. Pick two elements `v₁, v₂ ∈ dom_C(M(d))` with `M(d)(v₁) ≠ M(d)(v₂)` — such a pair exists precisely because `M(d)|_{dom_C}` is non-constant, and the value-distinctness forces `v₁ ≠ v₂`. Define the *transposition witness*
    ...
    Hence `π_swap` is admissible, and the full-clearance form (`n'_{s_C} = 0`, canonical statement at the head of this section) of the K.μ⁻ + K.μ⁺ decomposition realises it. The transposition witness is the simplest one; many other admissible π exist, but a single witness suffices for the sufficiency obligation.
...
- *Full content-subspace clearance and rebuild* at `n'_{s_C} = 0` (the full-clearance form, canonical statement at the head of this section): K.μ⁻ removes V_{s_C}(d) entirely (maximal-suffix removal, with link-subspace retained) and K.μ⁺ then adds `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` in one step — this is the concrete K.μ⁻ + K.μ⁺ write set realising the head form.
...
**Definition (Fork).** A *fork* of d_src to d_new is a composite transition `Σ →* Σ'`, with *precondition* d_src ∈ E_doc ∧ V_{s_C}(d_src) ≠ ∅, together with the per-sub-case activation condition on the K.δ step: the k = 1 sub-case fires when `A_v(d_src)` has no prior emission (no version yet exists), and the k = 0 sub-case fires when `A_v(d_src)` already has a frontier (a prior version exists, supplying `prev_version`). It consists of:
