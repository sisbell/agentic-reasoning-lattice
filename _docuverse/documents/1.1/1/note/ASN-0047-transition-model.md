...
*Significance.* The direct freshness predicate `inc(t, 0) ∉ E` sidesteps allocator-structural identification entirely: its operational truth at the K.δ event is what selects `t` (equivalently, what places `t` on the frontier), and any freshness obligation `e ∉ E` for `e = inc(t, 0)` discharges from the precondition itself.
...
**K.α (no local amendment in extended state).** ASN-0093's K.α encodes the content-subspace restriction `E(a)₁ = s_C` (equivalently `subspace_I(a) = s_C`) in its precondition directly, and its frame already includes `L' = L`. No locally-introduced amendment of K.α is required in the extended state. Downstream sites in this ASN — verification matrix, body prose, and the *Properties Introduced* tables — cite this inherited precondition by name as **K.α's `E(a)₁ = s_C` precondition** (or **K.α's content-subspace precondition**), explicitly attributing it to ASN-0093 rather than to a local addition.
...
**LinkVPositionDepthAxiom (Axiom, FixedLinkVPositionDepth).** `(A d ∈ E_doc :: m_L = 2)` — every link-subspace V-position has depth 2. The lower bound `m_L ≥ 2` is inherited from S8a (ASN-0036, carried as a Class (a) per-state invariant here), which requires `#v ≥ 2` for every `v ∈ dom(M(d))`; LinkVPositionDepthAxiom fixes it at exactly 2.
...
K.μ⁺_L adds exactly one mapping per firing, so the pairwise-distinctness clause that K.μ⁺ states for its newly added V-positions has no work to do here — the singleton `{v_ℓ ↦ ℓ}` is trivially in bijective correspondence with itself, and there are no "two distinct new mappings" to disagree on a shared V-position. We verify `v_ℓ ∉ dom(M(d))`, as required for M'(d) to be a proper extension preserving S2 (ArrangementFunctionality) — and equivalently to discharge the strict-extension `⊃` in the effect clause above. The verification decomposes by subspace: by S3★-aux (SubspaceExhaustiveness), every `v ∈ dom(M(d))` satisfies `subspace(v) ∈ {s_C, s_L}`, so `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)`. We discharge `v_ℓ ∉ V_{s_L}(d)` and `v_ℓ ∉ V_{s_C}(d)` separately.
...
The argument proceeds in five steps, (A) → (B) → (C) → (D) → (E), each building only on its predecessors. (A) derives subspace preservation under π — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — from admissibility (i)'s stipulated S3★(Σ'), S3★(Σ) (inductive hypothesis), the bijection equation, and L14. (B) shows the K.μ⁻ + K.μ⁺ decomposition mechanically realises admissibility clause (i)'s stipulated S3★(Σ'), consuming Step (A)'s subspace preservation. (C) establishes the link-subspace functional identity `M'(d)|_{dom_L} = M(d)|_{dom_L}` (Steps 1–3 of *Link-subspace fixity* below). (D) derives pointwise fixity `π(v) = v` on `dom_L` (Step 4 below). (E) closes admissibility clause (ii)'s existence condition `|dom_C(M(d))| ≥ 2` (*Decomposition* below). The chain is non-circular: CL-UNIQ at the pre-state is consumed only at Step (D), so Steps (A)–(C) are independent of it. Steps (A) and (B) are proved immediately below; Steps (C)–(E) at the cited locations.
...
- `d ∈ E_doc`
- `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (ii) `π ≠ id` to admit a witness.

  *Well-formedness of the precondition under the outer induction.* The precondition is stated against the pre-state `Σ` of a K.μ~ event, and its necessity direction below consumes CL-UNIQ at `Σ` (via Step (D) of the dependency chain, link-subspace fixity). This is well-formed because K.μ~ is defined for invocation at reachable states `Σ` of the ExtendedReachableStateInvariants induction, where CL-UNIQ holds at `Σ` by inductive hypothesis on the per-state invariants. The induction's separation is structural: each K.μ~ event under consideration is the *current* transition `Σ →* Σ'`, with the per-state invariants at `Σ` (including CL-UNIQ) supplied by the induction's *prior* state, and the per-state invariants at `Σ'` (including post-state CL-UNIQ) being what the K.μ~ verification establishes in the *current* step. The reader who encounters the precondition's necessity derivation should read "CL-UNIQ at the pre-state" as the inductive hypothesis on `Σ`, not as a property whose preservation is being established by the current K.μ~ event.

  The two directions enter the argument independently and are kept separate here so the reader sees what each consumes:
  - *Necessity.* Necessity of `|dom_C(M(d))| ≥ 2` is proved in *Decomposition* below.
...
**Decomposition.** We derive the necessary-and-sufficient existence condition `|dom_C(M(d))| ≥ 2` by closing the quantifier structure of admissibility clauses (i) and (ii) cleanly. Necessity is what we prove: for any candidate `π` satisfying both clauses, `|dom_C(M(d))| ≥ 2` must hold.

*Necessity argument (universal closure over admissible π).* Suppose K.μ~ admits some π. Then π satisfies admissibility (i) (the post-state invariant package on M'(d), in particular S3★(Σ')) and admissibility (ii) (`π ≠ id`). From admissibility (i)'s S3★(Σ') stipulation, Step (A) of the dependency chain at *Decomposition of K.μ~* above derives subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` in a single step from S3★(Σ) (inductive hypothesis at the pre-state) + L14 — without consuming link-subspace fixity. From admissibility (i) again plus K.μ~-FIX (`dom(M'(d)) = dom(M(d))`), π is a bijection of a fixed domain. Subspace preservation then forces π to map `dom_L(M(d))` bijectively onto itself (link-subspace closure) and `dom_C(M(d))` bijectively onto itself (content-subspace closure). Now Step (D)'s derivation — which requires CL-UNIQ at the inductive pre-state Σ (supplied by the outer induction's per-state hypothesis on reachable Σ) — gives `π|_{dom_L} = id` pointwise. Combined with admissibility (ii) `π ≠ id` and the just-established content-subspace closure, π must differ from the identity on at least one element of `dom_C(M(d))` — i.e., π must be a non-identity permutation of `dom_C(M(d))`. Such a permutation exists only on a set of cardinality at least 2: the empty set admits only the empty function (which equals `id|_∅`), and a singleton admits only the unique self-map (which equals `id|_singleton`); neither is a non-identity permutation. Therefore `|dom_C(M(d))| ≥ 2`. ∎
...
*Temporal scoping of J0 (composite-boundary, not per-state).* J0 is a coupling constraint over the composite `Σ →* Σ'`, binding the initial state Σ and the final state Σ' — it does *not* bind each intermediate atomic state within the composite. At an intermediate state immediately after K.α has placed `a` into `dom(C)` but before the trailing K.μ⁺ has installed a V-position mapping to `a`, J0's existential is transiently unsatisfied: `a ∈ dom(C_int)` without `v ∈ dom(M_int(d)) : M_int(d)(v) = a`. This transient failure is admissible under ValidComposite★, exactly parallel to P4★'s transient violation between K.μ⁺ and K.ρ in the analogous J1★ case; restoration occurs when K.μ⁺ fires within the same composite and places the witnessing `v ↦ a` mapping into M'(d). The composite-boundary verification matrix in *Class (b)* below catalogues J0 in this scoping uniformly with J1★ and J1'★ — J0 is a composite-boundary coupling whose discharge is the restored existential at Σ', not a per-state invariant of every reachable atomic state.
...
(The `L' = L` conjunct extends J2's frame into the link-store-bearing state.) The wp analysis confirms this. For P0: K.μ⁻ does not touch C. For P1: does not touch E. For P2: does not touch R. For L12: does not touch L. For the provenance bound Contains(Σ) ⊆ R: contraction can only remove pairs from Contains, so Contains(Σ') ⊆ Contains(Σ) ⊆ R = R'. No co-occurring transition is needed to maintain any system invariant.
...
(As with J2, the `L' = L` conjunct extends J3's frame into the link-store-bearing state.) Reordering preserves ran(M(d)), so Contains(Σ') = Contains(Σ). All invariants are trivially maintained; no co-occurring transition is needed.
...
*Derivation of J1★ from preserving P4★ (re-running the J1 wp computation in the extended state).* J1's original derivation in *Coupling and isolation* asked: given that the K.μ⁺ frame holds `R' = R`, what coupling is required to preserve the design choice `Contains(Σ) ⊆ R` (P4) across the post-state? In the extended state the analogous question replaces P4 with P4★ (`Contains_C(Σ) ⊆ R`) — which is precisely the design choice surviving the supersession (P4 is unsatisfiable for the unscoped `Contains(Σ)` once link-subspace mappings exist; P4★ is the form that retains Nelson's "every recorded containment is captured in the reverse index" intent in the two-subspace state). Computing wp backward from `Contains_C(Σ') ⊆ R'` under the K.μ⁺ amendment (which adds only content-subspace V-positions) and the K.μ⁺ frame `R' = R`:

`wp(K.μ⁺ (amended), Contains_C(Σ') ⊆ R') = (A a : a ∈ ran(M'(d)|_{s_C}) \ ran(M(d)|_{s_C}) : (a, d) ∈ R)`

The right-hand side collapses to R because K.μ⁺ frames R, and the difference set on the left is content-subspace-scoped because the K.μ⁺ amendment introduces only content-subspace V-positions, so any new entry in `Contains_C(Σ') \ Contains_C(Σ)` is captured by `ran(M'(d)|_{s_C}) \ ran(M(d)|_{s_C})`. K.μ⁺_L extends only the link subspace, leaving `ran(M(d)|_{s_C})` unchanged and contributing nothing to the difference — the wp computation is vacuous for K.μ⁺_L on P4★. As in the four-component derivation, the requirement "every new content-subspace range entry already in R" is not generally true for fresh content; K.μ⁺ (amended) in isolation cannot maintain P4★. Therefore, to maintain P4★, K.ρ must co-occur within the composite, extending R so that the composite post-state satisfies `(A a : a ∈ ran(M'(d)|_{s_C}) \ ran(M(d)|_{s_C}) : (a, d) ∈ R')` — which is J1★ above. J1★ is the coupling constraint that the design choice P4★ forces upon the elementary transition set in the extended state, derived by the same wp computation as J1 with P4 replaced by P4★ and the K.μ⁺ amendment scoping the difference set to the content subspace. The matrix entry P4★ under K.μ⁺ in *Class (b)* below corresponds to this derivation's discharge: at the composite boundary, K.ρ (under J1★) supplies the missing `(a, d) ∈ R'` for every new content-subspace range entry, restoring P4★. Symmetrically, J1'★ is the converse coupling — every new entry in `R' \ R` corresponds to a content-subspace range change — derived in the same manner from the requirement that R have no extraneous entries unanchored in content-subspace arrangement.
...
- *L11a (Link distinctness for this K.λ event)*: this is the *subsequent-link case* for `d` — the K.λ precondition predicate `{ℓ' ∈ dom(L) : origin(ℓ') = d} ≠ ∅` holds because Step 1's `ℓ ∈ dom(L)` has `origin(ℓ) = d`, so the subsequent-emission path of K.λ applies. The emitted address `ℓ₂ = inc(max{ℓ' ∈ dom(L) : origin(ℓ') = d}, 0) = inc(ℓ, 0) = 1.0.1.0.1.0.2.2` advances `A_L(d)`'s frontier by one sibling-increment via TA5(c). **T10a GlobalUniqueness on A_L(d)'s inc chain** (the direct per-`(t, k')` uniqueness axiom is not stated for `k = 0`; T10a chain-advancement uniqueness at `(ℓ, 0)` — the derived form, TA5(c) + L12 + precondition — discharges freshness against the sub-allocator's own prior emissions, while T10a.6 cross-allocator domain-disjointness rules out any other allocator producing `ℓ₂`) supplies `ℓ₂ ∉ dom(L)` and hence `ℓ₂ ≠ ℓ` — the L11a obligation that this K.λ event produce an address distinct from every prior K.λ event under d (here, the single prior event at Step 1 producing ℓ). Cross-store distinctness `ℓ₂ ∉ dom(C)` follows from SC-NEQ + L14 at the pre-state. SubAllocatorAxiom.FirstEmission is *not* invoked for ℓ₂ — that route is reserved for the first emission; subsequent emissions discharge through T10a's inc-chain discipline. ✓
...
**Class (a): Per-state invariants** — preserved by each elementary transition individually, holding at every reachable state including intermediate states within composites. These are all per-state properties except P4★, P4a, and P7a: S2, S3★, S3★-aux, S4, S7a, S7b, C1b, S7d, S8a, S8-fin, S8-depth, S8★, C-fin, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, NodeLineage, L0, L1, L1a, L1b, L3, L14, L-fin, CL-OWN, CL-UNIQ.
...
|-------|-----------|------------|
| K.δ-ID.zeros-0/1 | `zeros(e) = zeros(t)` for k ∈ {0, 1} on `e = inc(t, k)` | TA5(c) preserves zeros for k = 0 (sibling-increment at the rightmost nonzero component); TA5(d) at k = 1 appends a final `1` with no new zero, so the zero count is unchanged. |
| K.δ-ID.zeros-2 | `zeros(e) = zeros(t) + 1` for k = 2 on `e = inc(t, 2)` | TA5(d) at k = 2 appends one zero separator and a final `1`. Discharges the K.δ k = 2 zeros bound `zeros(t) ≤ 1` from the case-level `¬IsElement(e)` |
