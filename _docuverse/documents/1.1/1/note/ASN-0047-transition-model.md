...
The *Composite-boundary verification matrix* below records, for each Class (b) property, where it transiently fails and the coupling that restores it. P4★ and P7a are *composite-boundary state-predicates* — state-predicates guaranteed only at composite boundaries, not at every reachable state; P4a is grouped with them as a trace property, with its witnessing domain made formal in its definition box.
