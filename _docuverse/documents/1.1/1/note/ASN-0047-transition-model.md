...
*Subspace-position correspondence.* For `v ∈ dom(M(d))` with `M(d)(v) = a`, `subspace(v) = fields(a).E₁` (S3★). The two notations apply at different state-component levels — `subspace(v)` projects V-positions, `fields(a).E₁` projects I-addresses — and are used accordingly.
...
K.μ~ — *arrangement reordering* — is a named composite of K.μ⁻ + K.μ⁺ (analogous to J0/J1★/J2/J3/J4), not a primitive transition. For `d ∈ E_doc`, K.μ~ realises the *bijection equation* `(E π : π is a bijection dom(M(d)) → dom(M'(d)) : (A v ∈ dom(M(d)) :: M'(d)(π(v)) = M(d)(v)))` together with the admissibility constraints and derived frame catalogued in §*Decomposition of K.μ~* below.
...
The sufficiency claim above is bounded — it is structural, not exhaustive over the admissible-state-difference lattice. The known gap is Nelson's tombstone-style interior link withdrawal (see the canonical *Link-withdrawal gap under D-CTG★/D-MIN★* site below).
...
**Link-withdrawal gap under D-CTG★ / D-MIN★ (canonical site).** Trading the link-subspace tombstoning provision for uniform contiguity has a load-bearing consequence: under D-CTG★/D-MIN★, K.μ⁻'s admissible link-subspace contractions are suffix truncations only (per K.μ⁻'s case analysis above). A user cannot withdraw a single link at an interior link-subspace position while leaving subsequent links in place — doing so would violate D-CTG★ at the post-state — so withdrawing one interior link requires withdrawing every link allocated after it as well. Nelson's tombstoning design (LM 4/9) preserves the withdrawn link's arrangement position rather than removing it: withdrawn links transition to "not currently addressable, awaiting historical backtrack functions" status while retaining their permanent arrangement slot. That design is *not* expressible as any K.μ⁻ transition or composite under the amended D-CTG★/D-MIN★. Reconciling it requires a separate withdrawal mechanism — status flag, tombstone marker, or explicit retraction link — operating outside K.μ⁻'s presentational-removal contract. The mechanism is not specified in the present ASN; what to require of it is recorded as an open question below. All subsequent cross-references in this ASN to "link withdrawal under D-CTG★/D-MIN★" or to "Nelson's tombstoning design" defer back to this site.
...
- *Namespace property (SubAllocatorAxiom.Namespace).* The *first emission* of each sub-allocator is the determinate tumbler `[d.0.s_C.1]` for content (resp. `[d.0.s_L.1]` for links), satisfying an at-allocation-state freshness property that closes the uniqueness chain without appeal to T10a's GlobalUniqueness:
  - *Content sub-allocator first emission:* the first address produced by d's content sub-allocator is `a = [d.0.s_C.1]` — the unique sibling indexed `.1` under the content anchor `b_C(d)`. The address satisfies `a ∉ dom(Σ.C) ∪ dom(Σ.L)` at the state of allocation and has `fields(a).E₁ = s_C`, `fields(a).E₂ = 1`, `origin(a) = d`, `#E(a) = 2`.
  - *Link sub-allocator first emission:* the first address produced by d's link sub-allocator is `ℓ = [d.0.s_L.1]` — the unique sibling indexed `.1` under the link anchor `b_L(d)`. The address satisfies `ℓ ∉ dom(Σ.L) ∪ dom(Σ.C)` at the state of allocation and has `fields(ℓ).E₁ = s_L`, `fields(ℓ).E₂ = 1`, `origin(ℓ) = d`, `#E(ℓ) = 2`.
...
Link withdrawal via K.μ⁻ applied to the link subspace would in principle produce the same orphan-link state but is constrained to suffix truncations under D-CTG★ — see the *Link-withdrawal gap* canonical site above.
...
Nelson's tombstoning design is not expressible as a K.μ⁻ transition — see the canonical *Link-withdrawal gap under D-CTG★ / D-MIN★* site above.
...
- What invariants must a separate link-withdrawal mechanism (status flag, tombstone marker, or explicit retraction link) maintain in order to reconcile Nelson's tombstoning design (LM 4/9) with D-CTG★ / D-MIN★? See the canonical *Link-withdrawal gap under D-CTG★ / D-MIN★* site above for the gap statement.
