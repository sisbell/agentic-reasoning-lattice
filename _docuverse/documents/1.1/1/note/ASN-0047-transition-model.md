...
- *L-invariants vacuously satisfied:* `dom(L₂) = dom(L₁) = ∅` (K.δ, K.μ⁺, K.ρ each hold L in frame), so L0 (L-clause), L1, L1a, L1b, L1c, L3, L-fin, CL-OWN, and CL-UNIQ are vacuous at Σ₂; L14 is `dom(C₂) ∩ dom(L₂) = dom(C₂) ∩ ∅ = ∅`. L0 (C-clause) and the remaining per-state invariants (S2, S3★-aux, S4, S7a–S7d, S8a, S8-fin, S8-depth, S8★, C-fin, D-CTG★, D-MIN★, D-SEQ★, P6, P7, NodeLineage) are inherited unchanged from Σ₁ across the K.δ + K.μ⁺ + K.ρ frame, with the exceptions noted in the explicit rows above (J4-specific reasoning for the new document; P4★/P7a/P8 above). ✓
...
| S8★ | frame | frame | frame | per-subspace projection via ASN-0036's S8 on M(d')\|_{V_{s_C}(d')} | trivial length-1 decomposition on M(d')\|_{V_{s_L}(d')} | restriction of decomposition | inherits via decomposition | frame |
