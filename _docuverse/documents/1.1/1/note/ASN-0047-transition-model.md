...
**NodeBaptism (Axiom, boundary input — node provisioning).** Node addresses are not minted by any docuverse transition; the operation set provides no `inc`-rule producing a node. This axiom records, over `Σ` alone, what the provisioning boundary commits at every K.δ node-allocation event — every elementary K.δ transition placing an entity `e` with `IsNode(e)` into E:

- (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation;
- (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order.

The bootstrap node `n₀ ∈ E₀` is itself the boundary baptism performed at `Σ₀`.
...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | J1'★ at boundary supplies content-subspace witness `v` with `M'(d)(v) = a` at the post-state Σ' itself | Transiently fails if K.ρ precedes the matching K.μ⁺ within the composite; restored at Σ', which carries the witness. |
...
| NodeBaptism | Axiom (boundary input — node provisioning): node addresses are baptised at the network-provisioning boundary, not by any docuverse transition; every K.δ node-allocation event commits, over Σ alone, (a) e ∉ Σ.E (freshness) and (b) n₀ ≼ e (bootstrap lineage); n₀ ∈ E₀ is the boundary baptism of Σ₀. (For its role as the k'=2 spawnPt-premise source, see the K.δ case (ii) k = 2 dispatch table.) |
