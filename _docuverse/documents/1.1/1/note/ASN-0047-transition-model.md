...
Every link in the link store has exactly three endsets. This is a local extension of ASN-0043's L3 (NEndsetStructure: `|Σ.L(a)| ≥ 3`) — a stronger property than the foundation's, applied within this ASN's extended state — narrowing the foundation's `N ≥ 3` to a fixed-three arity in two respects. First, arity is fixed at three rather than admitting N > 3: the udanax-green realisation places exactly three endsets per link — from, to, and type — at three fixed positions, and Nelson's design uses the same three uniformly ("a package of connecting or marking information" with from/to/type — LM 4/12). Second, ASN-0043's non-empty type-endset clause (`Σ.L(a).e₃ ≠ ∅`) is dropped — the abstract model admits empty Θ as a well-formed link value carrying the semantics of an *untyped link*: F and G remain populated (the connection's endpoints — the link still "does the work of connecting"), while `Θ = ∅` asserts the link carries no further classification beyond the from/to relation itself. Nelson's design treats type assignment as an optional adornment of the underlying connection (LM 4/12: links connect, with type as a classifier *when supplied*); the abstract model formalises this by admitting `Θ = ∅` as the un-typed degenerate case rather than forbidding it. The Endset definition above (`𝒫_fin(Span)`) admits `∅` as a well-formed endset, so `(F, G, ∅) ∈ Link` is well-formed by construction. The udanax-green implementation accepts empty Θ at the storage layer (no runtime guard forbids it); Gregory documents protocol-level asymmetries — for instance `followlink` with `whichend = 3` returns an error when Θ = ∅, treating the type slot as not-currently-traversable, and `endsetqueries` are asymmetric across the three slots — but these are presentational/discovery-layer behaviours, not constraints on the abstract link's well-formedness. The abstract model treats `Θ = ∅` as a valid link value with a defined meaning (an un-typed link), and defers the protocol's handling of type-endset queries to a discovery-layer ASN. The N ≥ 3 generality of ASN-0043 is preserved in principle for future extensions; this ASN's transition model is closed under the realised arity. Consequently, K.λ's precondition `(F, G, Θ) ∈ Link` permits empty Θ — a link with no type classification, distinct from the absence of a link. Downstream consequence for same_type: under ASN-0043's L8, same_type is defined via shared type-endset coverage, and `coverage(∅) = ∅`, so all empty-Θ links share a single equivalence class — the trivial "untyped" class — under same_type. This ASN's transition contracts are unaffected: K.μ⁺_L treats `L(ℓ)` as opaque (it consumes only the address ℓ and a target subspace position), and the link-subspace placement does not consult Θ. Discovery-layer behaviour over same_type — in particular whether type-endset queries should partition the untyped class or treat it as a single bucket — is deferred to a discovery-layer ASN, consistent with the abstract model's broader stance of admitting empty Θ as well-formed and leaving its protocol-level treatment to higher layers.
...
*Precondition:* `IsElement(a)` (S7b, ASN-0036) ∧ `origin(a) ∈ E_doc` ∧ `a` is produced by origin(a)'s content sub-allocator (established by SubAllocatorAxiom, defined in the Allocator hierarchy section below). The first content address under a document is `[d.0.s_C.1]`, supplied by the axiom's content namespace property; subsequent content addresses are inc-produced over the content sub-allocator's frontier (TA5(c), ASN-0034), and T10a's GlobalUniqueness then gives `a ∉ dom(C)` directly. The axiom underwrites the first emission (where T10a alone cannot, since the content sub-allocator's anchor `b_C(d)` is a virtual predecessor with no inc-history), and T10a underwrites every subsequent emission. Disjointness from the link sub-allocator is supplied by SubAllocatorAxiom's disjointness clause and by SC-NEQ + T7 + L14. Without these conditions, weaker phrasings ("a is allocated under origin(a)'s prefix") would admit non-conforming allocations that break the uniqueness chain. By the axiom or by GlobalUniqueness (depending on case), a is distinct from every previously allocated content address.
...
**(ii) When ¬IsNode(e)** (accounts and documents). parent(e) ∈ E — the parent entity must already exist. The address is produced by a T10a-conforming allocation event:
  ...
  - *Tumbler-depth extension cases (k ∈ {1, 2}, TA5(b)+TA5(d)).* For k > 0, the constructive clause is TA5(d): `#e = #t + k`, with positions `#t + 1, ..., #t + k − 1` set to `0` (the `k − 1` zero separators) and position `#t + k` set to `1` (the appended terminal). TA5(b) supplies the agreement on the preserved prefix (positions `1..#t`). t is at parent(e)'s level — most directly t = parent(e) itself when extending from parent(e) for the first child of e's level. The two sub-cases differ structurally in `zeros(e)` and in the hierarchical-level relationship between e and t:
    - *Sibling-at-deeper-tumbler-depth (k = 1).* TA5(d) with `k = 1` appends `.1` and introduces no new zero separator (the range `#t + 1, ..., #t` of zero positions is empty for `k = 1`), so `zeros(e) = zeros(t)`. Hierarchically e remains at t's level (same zero count); only the tumbler length grows by one component. The "k = 1" form is the structural shape Nelson identifies as a document version when t is a document — see the note on version semantics below.
...
*Scope and harmlessness (per-invariant verification).* K.δ as stated in this ASN does *not* require, for k = 1 events from a document, that the corresponding base `[N, 0, U, 0, D]` (or any prior version `[N, 0, U, 0, D, k']` with `k' < k`) be in E_doc. The precondition reads only `t ∈ T`, `e = inc(t, k)`, T10a's zeros-count constraints, and the structural agreement clauses; it does not constrain the document-level neighbourhood of e beyond `parent(e) ∈ E`, which T10a satisfies through t alone. We claim that this omission is *harmless* under every invariant introduced in this ASN, and verify the claim layer by layer:
...
The verification therefore covers every invariant in P0–P8, S0–S9, L0–L14, J0–J4, D-CTG/D-MIN/D-SEQ, the starred forms (P3★, P5★, P7a, S3★, S3★-aux, D-CTG★, D-MIN★, D-SEQ★, J1★, J1'★), and the three uniqueness axioms. The k = 1 admissibility is harmless across the entity-hierarchy, arrangement, link, provenance, and coupling layers; the orthogonal question — whether the *runtime allocation protocol* (`docreatenewversion`) chooses to require a live base — is addressed below under *Consistency with the foundation* and deferred under *Deferred semantics*.

*Consistency with the foundation.* This admissibility aligns with the design's *ghost-element* doctrine (Nelson, LM 4/23): "While servers, accounts and documents logically occupy positions on the developing tumbler line, no specific element need be stored in tumbler-space to correspond to them. Hence we may call them ghost elements." Conceptually there is no privileged "basic" version (LM 2/19); a version address `[N, 0, U, 0, D, k]` may exist with `[N, 0, U, 0, D]` as a ghost. The orthogonal constraint that Gregory's `docreatenewversion` implementation enforces — that the source ISA must exist as a live granfilade orgl, established by `doretrievedocvspanfoo`'s read of the source's V-span (consultation evidence, ST-VERSION-CREATE/Finding 0032) — is a protocol-level requirement of the runtime's allocation pathway, not an axiom of the abstract transition model: it expresses *how* a particular implementation chooses to allocate a version (by copying from a live source), not *what structural constraints* the transition system imposes on the resulting state. The abstract specification declines to legislate this; an implementation may equally well allocate version-shaped addresses from ghost bases (consistent with the design intent) or refuse to do so (consistent with `docreatenewversion`'s implementation), and either policy yields a transition trace that satisfies this ASN's invariants.

*Deferred semantics.* The richer version contract — what arrangement-transition invariants must hold between successive versions, whether content allocators of base and version are linked, how provenance flows between them, whether `[N, 0, U, 0, D, k]` may be allocated when `k > 1` without `[N, 0, U, 0, D, k − 1]` ∈ E_doc, and whether version lineage must be acyclic — lies outside the entity-hierarchy spine that P8 governs (parent(·) truncates by zero separators; a version's terminal field has no preceding zero, so the version-to-base relationship is not on the parent spine). These questions are deferred to the open question "What relationship must hold between a document's version lineage and its sequence of arrangement transitions?" below, and a comprehensive resolution belongs to a subsequent version-management ASN. The present ASN treats `[N, 0, U, 0, D, k]` for k ≥ 1 as an admissible K.δ output under T10a's structural discipline, and the absence of base-existence and sequential-lineage preconditions reflects a deliberate scope boundary — not an oversight.

*K.δ k = 1 from non-document t.* The structural analysis and harmlessness verification above are presented with t a document (`zeros(t) = 2`), the case Nelson and Gregory identify as document versioning. K.δ's case (ii) precondition admits k = 1 symmetrically when t is an account (`zeros(t) = 1`, producing e = inc(t, 1) at account-level — an "account-shaped sibling at deeper tumbler depth") or a node (`zeros(t) = 0`, producing e = inc(t, 1) at node-level). The structural admissibility under T10a is identical: TA5(d) with k = 1 appends `.1` without introducing a zero separator, so `zeros(e) = zeros(t)` whatever t's level, and GlobalUniqueness yields `e ∉ E`. Every clause of the harmlessness verification above is per-state and per-entity with no dependence on the new entity's level — P8 uses `parent(e) = parent(t)` from T10a's k = 1 case (level-independent); the arrangement-layer invariants apply only to E_doc and are vacuous for e ∉ E_doc; the monotonic-store, origin-coherence, provenance, coupling, link-layer, and uniqueness verifications are all level-independent — so the same conclusion holds: K.δ k = 1 from t with `IsNode(t)` or `IsAccount(t)` preserves every invariant in P0–P8, S0–S9, L0–L14, J0–J4, D-CTG/D-MIN/D-SEQ, the starred forms, and the three uniqueness axioms. Whether such allocations carry account-version or node-version semantics is silent in the consultation evidence — Nelson's versioning discussion (LM 4/29) is document-specific, and Gregory's `docreatenewversion` (do1.c:271) has no account or node analog. The structural admissibility holds; the design intent for account-/node-level "versions" is deferred to the open questions below.
...
*Precondition:*
- `d ∈ E_doc`.
- *Admissible removal (per-subspace suffix or full-subspace clearance).* For each non-empty subspace S in M(d), the input arrangement has a contiguous, minimum-anchored, uniform-depth range. We state and derive this structural form locally — relying on the per-state invariants D-CTG★, D-MIN★, S8-depth, S8-fin, and S8a (D-CTG★ and D-MIN★ are the strengthened, per-subspace forms established below in the *Amendments to existing transitions* section; their proofs there proceed by induction over the elementary transitions of the extended state and do not depend on K.μ⁻'s admissibility derivation, so the cross-reference is a citation of statement, not a circular use of result). The strengthening is essential: K.μ⁻'s precondition quantifies over every non-empty subspace, including the link subspace `s_L`, but ASN-0036's D-CTG and D-MIN explicitly exempt `s_L`. D-CTG★ and D-MIN★ apply uniformly to every non-empty subspace, so the quantifier `(A S : V_S(d) ≠ ∅ : ...)` is meaningful across both `s_C` and `s_L` at K.μ⁻'s definition site in the extended state.

  *Structural form (per-subspace).* For each non-empty subspace S in M(d), there exists `n_S ≥ 1` and a uniform inner depth m_S ≥ 2 such that

  `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` with each tuple of depth m_S.

  *Local derivation.* Fix d and a non-empty subspace S; abbreviate `m := m_S` and `n := |V_S(d)|`. By S8-depth, every `v ∈ V_S(d)` has `#v = m`; by S8a, every component of every `v ∈ V_S(d)` is strictly positive; by D-MIN★, `v_min = [S, 1, ..., 1]` of depth m belongs to V_S(d); by S8-fin, n is finite. We show that under the lex order on positive depth-m tuples sharing first component S, D-CTG★ combined with S8-fin force every element of V_S(d) into the all-1-inner form `[S, 1, ..., 1, k]`. (Step 1) If some `v ∈ V_S(d)` had `v_j ≥ 2` at the minimal inner position `2 ≤ j ≤ m − 1` (vacuous when m = 2), then for each integer `M ≥ 2` the tuple `u_M = [S, 1, ..., 1, 1, M, 1, ..., 1]` (with `S` at position 1, `1` everywhere through position j, `M` at position j + 1, and `1` at every remaining position) is positive-component, depth-m, and strictly between `v_min` and v under lex order — so by D-CTG★ it lies in `V_S(d)`; the family `{u_M : M ≥ 2}` is infinite and pairwise distinct, contradicting `|V_S(d)| = n < ∞`. (Step 2) Therefore every `v ∈ V_S(d)` has form `[S, 1, ..., 1, k]` for some `k ∈ ℕ⁺`; the lex order restricted to terminal-varying tuples coincides with the natural order on k, and D-CTG★ forces the set to be `{[S, 1, ..., 1, k] : 1 ≤ k ≤ k_max}` with `k_max = n`. This establishes the structural form uniformly across both `s_C` and `s_L`, with the forward citation of D-CTG★/D-MIN★ acknowledged above.

  *(Note.* This local derivation is reproduced in fuller detail in the *Amendments to existing transitions* section below, where the same structural form is named *D-SEQ★* and re-derived in the extended state's notation. The K.μ⁻ admissibility precondition here refers only to the structural form, not to that name; the Amendments section's role is to name and re-establish it as a system-wide invariant of the extended state, not to license the precondition retroactively.)

  Given this structure, the removed positions, partitioned by subspace, form a suffix of each subspace's range:
...
*Case 1: π = id (zero elementary steps).* When π is the identity on dom(M(d)) — whether because dom(M(d)) = ∅ (empty bijection) or because π = id on a non-empty domain — K.μ~ produces M'(d) = M(d) and expands into *zero elementary steps*. K.μ⁻'s strict-contraction precondition `dom(M'(d)) ⊂ dom(M(d))` would not be satisfied by a vacuous K.μ⁻, so a "vacuous round-trip" is not a valid elementary path; the correct expansion is the empty sequence. All invariants are trivially preserved.
...
*Case 3: π ≠ id with dom_C(M(d)) ≠ ∅ (the non-trivial decomposition).* This is the only case requiring genuine elementary steps. The decomposition has the explicit form:
...
*Frame derivation.* The frame follows by composition. K.μ⁻ gives C_int = C, E_int = E, R_int = R, L_int = L, and M_int(d') = M(d') for d' ≠ d. K.μ⁺ gives C' = C_int = C, E' = E_int = E, R' = R_int = R, L' = L_int = L, and M'(d') = M_int(d') for d' ≠ d. Composing: C' = C, E' = E, R' = R, L' = L, (A d' : d' ≠ d : M'(d') = M(d')) — matching K.μ~'s frame.
...
This is the system-wide named statement of the same structural form already used locally in the K.μ⁻ admissibility precondition (where it is derived from D-CTG + D-MIN + S8-depth + S8-fin + S8a). The K.μ⁻ precondition stands on the local derivation; D-SEQ★ here promotes that form to a system-wide invariant of the extended state by attaching a name and re-establishing it from the amended D-CTG★ + D-MIN★ in full detail. Downstream sections (the K.μ~-FIX domain-fixity argument, the link-subspace fixity proof, and the ExtendedReachableStateInvariants induction) appeal to D-SEQ★ by name; the precondition at K.μ⁻'s definition site does not.
...
The address ℓ is produced by d's link sub-allocator. In the first-link case the axiom (SubAllocatorAxiom) supplies `ℓ = [d.0.s_L.1]` with the namespace property `ℓ ∉ dom(L) ∪ dom(C)`; in the subsequent case `ℓ = inc(prev, 0)` is a T10a-conforming inc step within the sub-allocator's frontier, with GlobalUniqueness giving `ℓ ∉ dom(L)` and the axiom's disjointness giving `ℓ ∉ dom(C)`. The structural requirement closes the uniqueness chain across both cases: the axiom underwrites the first emission, and T10a underwrites every subsequent emission. By T7 (SubspaceDisjointness, ASN-0034) and SC-NEQ, the link subspace s_L is disjoint from the content subspace s_C, so ℓ cannot collide with any content address. For cross-document disjointness: T10a.6 (DomainDisjointness) gives `d₁ ⋠ d₂ ∧ d₂ ⋠ d₁` for distinct documents d₁, d₂ (whether under the same account or under different accounts, since K.δ allocates documents via independent inc-conformance under distinct prefixes); T10 (PartitionIndependence, ASN-0034) instantiated at p₁ = b_L(d₁), p₂ = b_L(d₂) — which inherit the non-nesting from their respective document prefixes — then gives disjointness of any addresses extending the two anchors, so link addresses originating under different documents cannot collide.
...
**Link-subspace fixity under K.μ~ (redundancy remark).** K.μ~'s precondition already enforces link-subspace fixity directly: the identity-on-link-subspace clause `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is stipulated at the definition site, so any well-formed K.μ~ leaves link-subspace mappings pointwise unchanged by precondition. The argument that follows is *not* a derivation in lieu of the precondition; it is a redundancy/consistency check showing that the precondition selects a canonical representative within the class of value-preserving permutations of link-subspace V-positions that S3★ + the K.μ⁺ amendment already admit. Under those weaker constraints alone, the *function* M'(d) restricted to link-subspace V-positions is forced to equal M(d) restricted to those positions (M'(d)|_{dom_L} = M(d)|_{dom_L}); the *bijection* π itself, however, may still permute link-subspace positions sharing a value — all such permutations yield the same M'(d). The definition-site identity clause picks π = id on the link subspace as the canonical representative among these value-preserving permutations. The check thus confirms that the stronger precondition is a consistent canonical-representative selection — strictly stronger than what S3★ alone derives, but yielding the same observable arrangement — not a restrictive addition.

*Redundancy argument.* Since K.μ⁺ (amended) requires `subspace(v) = s_C` for new V-positions, K.μ⁺ cannot create link-subspace V-positions. Let `dom_L(M(d)) = {v ∈ dom(M(d)) : subspace(v) = s_L}` denote the link-subspace V-positions. Assume only the weaker subspace-preservation clause on π (drop the identity clause for the sake of argument). With S3★ established for M'(d), π must map link-subspace positions to link-subspace positions: if `v ∈ dom_L(M(d))` then `M(d)(v) ∈ dom(L)`, and `M'(d)(π(v)) = M(d)(v) ∈ dom(L)`, so `subspace(π(v)) = s_L`: by S3★-aux, `subspace(π(v)) ∈ {s_C, s_L}`; the case `subspace(π(v)) = s_C` is eliminated because a content-subspace position mapping to dom(L) would violate S3★'s content clause, since `M'(d)(π(v)) ∈ dom(L)` and `dom(L) ∩ dom(C) = ∅` (L14, which depends on SC-NEQ). Thus `π` restricted to `dom_L(M(d))` is an injection into `dom_L(M'(d))`. Since K.μ⁺ cannot create link-subspace V-positions, `|dom_L(M'(d))| ≤ |dom_L(M(d))|`. If K.μ⁻ removed `r ≥ 1` link-subspace positions, then `|dom_L(M'(d))| ≤ |dom_L(M(d))| − r`, and the injection from `dom_L(M(d))` (size N) into `dom_L(M'(d))` (size at most N − r) cannot exist. Therefore `r = 0` — no link-subspace positions are removed. It follows that `M'(d)` restricted to `dom_L(M(d))` equals `M(d)` restricted to `dom_L(M(d))`. Let `M_int(d)` denote the intermediate arrangement after K.μ⁻ but before K.μ⁺. K.μ⁻ removes none of the link-subspace positions (`r = 0`) and preserves the values of all surviving positions, so `M_int(d)|_{dom_L} = M(d)|_{dom_L}`. K.μ⁺ (amended) operates on `M_int(d)`: its frame preserves pre-existing mappings (`(A v : v ∈ dom(M_int(d)) : M'(d)(v) = M_int(d)(v))`), and its subspace restriction prevents creating new link-subspace positions. Chaining: `M'(d)|_{dom_L} = M_int(d)|_{dom_L} = M(d)|_{dom_L}`. Each surviving link-subspace mapping retains its value in dom(L). The redundancy argument therefore compels the *function* `M'(d)` restricted to `dom_L(M(d))` to equal `M(d)` restricted to `dom_L(M(d))` — the link-subspace mappings, viewed pointwise as a partial function from V-positions to ℓ-values, are unchanged in aggregate. The stronger claim that π itself is the identity on `dom_L(M(d))` (rather than merely value-preserving on that domain) requires a uniqueness invariant on link values within `dom_L(M(d))` — i.e., `(A v₁, v₂ ∈ dom_L(M(d)) : v₁ ≠ v₂ : M(d)(v₁) ≠ M(d)(v₂))`. K.μ⁺_L's preconditions place each fresh link at a strictly increasing V-position but do not forbid the same `ℓ` from being placed at two distinct V-positions in the same arrangement (no precondition checks `ℓ ∉ ran(M(d))`). Absent such a uniqueness invariant, a non-identity π on `dom_L(M(d))` that swaps two positions sharing a value would also satisfy both the K.μ~ defining equation and the chained function-level equality. The definition-site identity clause `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` therefore selects a canonical representative among the value-preserving permutations of `dom_L(M(d))`; every such permutation yields the same `M'(d)`, and the precondition is consistent with — though strictly stronger than — what S3★ + the K.μ⁺ amendment alone derive.
...
**Step 4: K.μ⁻ on the link subspace — admissibility cases.** We extend the example to demonstrate the link-subspace specialisation of K.μ⁻'s admissibility precondition. Rename the link allocated in Step 1 as `ℓ₁ = 1.0.1.0.1.0.2.1` for clarity. Pre-state for Step 4 is the post-state of Step 3: `M''(d) = {[1,1] ↦ a₂, [1,2] ↦ a₁, [2,1] ↦ ℓ₁}` with `dom(L) = {ℓ₁}`.

*Step 4a: K.λ — allocate a second link.* By the subsequent-link case (since `dom(L) ∩ {a : origin(a) = d} = {ℓ₁} ≠ ∅`), set `t = max{ℓ' ∈ dom(L) : origin(ℓ') = d} = ℓ₁` and `ℓ₂ = inc(ℓ₁, 0) = 1.0.1.0.1.0.2.2` (TA5(c)). Precondition verification mirrors Step 1: `ℓ₂ ∉ dom(L) ∪ dom(C)` (by T10a GlobalUniqueness within the sub-allocator frontier and L14); `zeros(ℓ₂) = 3`; `fields(ℓ₂).E₁ = 2 = s_L`; `origin(ℓ₂) = d`; forward allocation `ℓ₁ < ℓ₂` (by the inc(·, 0) step at the frontier). Effect: `L''' = {ℓ₁ ↦ (F, G, Θ), ℓ₂ ↦ (F', G', Θ')}` for some second link value. Frame: C, E, M, R unchanged.

*Step 4b: K.μ⁺_L — place ℓ₂ at the next link V-position.* `V_{s_L}(d) = {[2,1]}`, so `v_{ℓ₂} = [s_L, 2] = [2, 2]` (the unique D-CTG-extending position at depth m_L = 2). Preconditions: `d ∈ E_doc`; `ℓ₂ ∈ dom(L''')`; `origin(ℓ₂) = d`; `subspace(v_{ℓ₂}) = 2 = s_L`; `#v_{ℓ₂} = 2 = m_L`. Effect: `M'''(d) = {[1,1] ↦ a₂, [1,2] ↦ a₁, [2,1] ↦ ℓ₁, [2,2] ↦ ℓ₂}`. Post-state verification: S3★ — both link positions map into dom(L''') (`ℓ₁, ℓ₂ ∈ dom(L''')`); CL-OWN — `origin(ℓ₁) = origin(ℓ₂) = d`; D-CTG★ — `V_{s_L}(d) = {[2,1], [2,2]}` contiguous under the lex order on terminal-varying tuples; D-MIN★ — `min(V_{s_L}(d)) = [2,1] = [s_L, 1]`; D-SEQ★ — `V_{s_L}(d) = {[s_L, 1, ..., 1, k] : 1 ≤ k ≤ 2}` with n_{s_L} = 2.

*Step 4c: K.μ⁻ — suffix removal at the link subspace (admissible).* Remove the maximum link-subspace position `[2,2]`, leaving the content subspace and `[2,1]` intact. Admissibility (per the K.μ⁻ amendment, case (a)): the removed set within the link subspace is `{[s_L, 1, ..., 1, k] : 1 < k ≤ 2} = {[2,2]}` — a suffix of `V_{s_L}(d)` with `n'_{s_L} = 1`; the content subspace is untouched (`n'_{s_C} = n_{s_C} = 2`). Effect: `M''''(d) = {[1,1] ↦ a₂, [1,2] ↦ a₁, [2,1] ↦ ℓ₁}`. Post-state verification: S3★ preserved by restriction; S3★-aux preserved (surviving positions retain their subspaces); D-CTG★ — `V_{s_L}(d) = {[2,1]}` is a singleton, contiguous; D-MIN★ — `min(V_{s_L}(d)) = [2,1] = [s_L, 1]`; CL-OWN preserved pointwise on the surviving link position; L12 — L''' held in frame, so `ℓ₂ ∈ dom(L'''')` retains its value even though no V-position now maps to it (an orphan link, as discussed in the *Orphan links and coupling flexibility* section).

*Step 4d: K.μ⁻ on `{[2,1]}` alone — inadmissible.* From the Step 4b post-state `M'''(d) = {[1,1] ↦ a₂, [1,2] ↦ a₁, [2,1] ↦ ℓ₁, [2,2] ↦ ℓ₂}`, consider a hypothetical K.μ⁻ that removes only the older link position `[2,1]` while retaining `[2,2]`. The hypothetical post-state link subspace would be `V_{s_L}(d_hyp) = {[2,2]}` with the smallest surviving terminal index `k_min = 2 ≥ 2`. Per the K.μ⁻ amendment's case (c) (*Prefix removal (forbidden by D-MIN★)*): removing `[s_L, 1, ..., 1, 1] = [2, 1]` while retaining `[s_L, 1, ..., 1, k]` for some `k ≥ 2` gives `min(V_{s_L}(d_hyp)) = [2, 2] ≠ [s_L, 1] = [2, 1]` of depth m_{s_L} = 2, violating D-MIN★. The proposed K.μ⁻ therefore fails its admissibility precondition and is inadmissible as a single elementary step. To withdraw `ℓ₁`'s arrangement, the user must withdraw `ℓ₂`'s arrangement first (Step 4c) and then withdraw `[2,1]` as a (now-maximum) suffix — a two-step composite, mirroring the consequence stated in the *Consequence for link withdrawal* section.

Steps 1–4 of this example exercise — explicitly or via the frame-preserved annotations — every invariant in the conjunction of ExtendedReachableStateInvariants (per-state) and ExtendedTransitionInvariants (per-transition), and demonstrate both the admissible (suffix) and inadmissible (prefix) modes of K.μ⁻ on the link subspace, providing a worked confirmation of the inductive step for the K.λ, K.μ⁺_L, K.μ⁻, and K.μ~ transition kinds on a concrete two-subspace state.
...
**ExtendedReachableStateInvariants (per-state).** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the transitions K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~ (shorthand for its K.μ⁻ + K.μ⁺ decomposition), and K.ρ — satisfies:
  ...
  S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN
...
Each conjunct is formally stated with the quantifier `(A Σ → Σ' :: ...)` — they are properties of the *step*, not of the endpoint Σ' alone. The first theorem governs what is true *at* each reachable state; the second governs what is true *across* each reachable step.
...
- *S9 (Two-stream separation)* — direct corollary of S0: no transition modifying M(d) alters dom(C) or C-values. Every K.μ⁺/K.μ⁺_L/K.μ⁻/K.μ~ holds C in frame; K.α extends C but does not modify M; K.δ, K.λ, K.ρ hold C in frame. S0 supplies the universal guarantee specialised to arrangement-modifying transitions, and the extended state's frame discipline confirms it.
...
**Per-transition invariants** (ExtendedTransitionInvariants: P0, P1, P2, P3★, P5★, L12). These are properties of `Σ → Σ'`; we discharge each by elementary case analysis, observing that every valid composite is a finite sequence of elementary steps and each per-transition invariant is closed under composition (extension and value-preservation compose transitively).

- *P0 (`dom(C) ⊆ dom(C') ∧ (A a ∈ dom(C) : C'(a) = C(a))`).* K.α extends dom(C) by `{a}` and assigns `C'(a)` without modifying existing entries (extension is at `a ∉ dom(C)`). All other elementary transitions hold C in frame: `C' = C`, so both clauses hold by equality.
...
| L3 | TripleEndsetStructure: `(A a ∈ dom(L) :: L(a) = (F, G, Θ) where F, G, Θ ∈ Endset)` | introduced |
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ S9 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P0–P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L3 ∧ L12 ∧ L14 ∧ L-fin ∧ CL-OWN; supersedes ReachableStateInvariants | introduced |
...
- What semantics, if any, attach to K.δ k = 1 allocations from account- or node-level t? The transition model admits such allocations structurally (T10a's k = 1 case is level-independent, and every invariant in P0–P8, S0–S9, L0–L14, J0–J4, D-CTG/D-MIN/D-SEQ, the starred forms, and the three uniqueness axioms is preserved), but Nelson's versioning design (LM 4/29) and Gregory's `docreatenewversion` (do1.c:271) are document-specific. Whether account- or node-shaped "siblings at deeper tumbler depth" carry distinguished version semantics, or should be permitted at all by higher-layer protocols, is unspecified.
