...
We adopt D-CTG★/D-MIN★ across both subspaces as a justified arrangement-layer contract, with link permanence discharged independently on `dom(L)` by L12. One genuine modeling limitation remains, and it concerns an *operation*, not the invariant: this ASN's K.μ⁻ realizes link-arrangement contraction by suffix removal alone and never renumbers survivors, whereas the implementation removes an interior link by compaction-with-renumbering. Modeling interior link withdrawal therefore requires a renumbering-aware contraction operation that the present K.μ⁻ does not provide (see the interior-withdrawal entry under Open Questions) — a missing operation, not an unresolved contradiction in D-CTG★/D-MIN★. The downstream results built on the link-subspace shape (D-SEQ★, the K.μ⁻ contraction, CL-UNIQ, and worked-example Step 5) rest on the now-discharged invariant, not on the deferred operation.
...
*Case `subspace(v) = s_L`.* K.μ⁺ writes only content-subspace positions, so the link-subspace positions of `M'(d)` are exactly those K.μ⁻ retains — namely the link-subspace positions of `M(d)`, unchanged — while the content-subspace positions of `M'(d)` are exactly K.μ⁺'s write set, which the content sources of the preceding case already exhaust bijectively. A realisable bijection π therefore maps each link source onto a retained link position, giving `subspace(π(v)) = s_L = subspace(v)`.

Hence every realisable π preserves subspace; combined with clause (iv) (every admissible π preserves subspace) and the full-clearance decomposition realising every subspace-preserving π, the admissible and realisable classes coincide. ∎ (Step (A))
...
**Link-subspace fixity.** `π(v) = v` for every `v ∈ dom_L(M(d))`. Sub-steps (1)–(3) establish the link-subspace functional identity `M'(d)|_{dom_L} = M(d)|_{dom_L}`; sub-step (4) derives pointwise fixity from it (premise: CL-UNIQ at the pre-state Σ). Proof:
...
- *Full content-subspace clearance and rebuild* at `n'_{s_C} = 0` (the full-clearance form): K.μ⁻ removes V_{s_C}(d) entirely (maximal-suffix removal, with link-subspace retained) and K.μ⁺ then adds `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` in one step — this is the concrete K.μ⁻ + K.μ⁺ write set realising the head form.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | elementary decomposition K.μ⁻+K.μ⁺ (this row's restriction + amendment Class (a) cells); realisable π are exactly the subspace-preserving ones (K.μ⁺ content-subspace precondition); §*Decomposition of K.μ~* |  frame |
...
*Composite-boundary verification matrix.* For each Class (b) property, the matrix records the discharge mechanism at the composite boundary and the elementary step at which it transiently fails (per the preamble's temporal-scope distinction).
