...
- *k = 2 (descent producing the first child under a node or account):* this is the activation case. The K.δ event itself is interpreted as a T10a T2 spawn step on the parent allocator (the parent node's account sub-allocator, or the parent account's document sub-allocator), with the operand `t` as spawnPt. Per T10a's T2 admissibility, spawnPt must inhabit `dom(parent_allocator(spawned))` at the K.δ event — discharged by K.δ's `parent(e) = t ∈ E` precondition, which places `t` in the activated parent allocator's tracked domain (every entity in E has been activated by a prior K.δ event). The spawn step's `k' = 2 ∈ {1, 2}` is admissible by T10a; T4's `zeros(t) ≤ 1` (case-level precondition) discharges the zero-count side condition. T10a GlobalUniqueness on the parent allocator's tracked domain (including the new spawn output) then delivers `e ∉ E` — first emissions of entity-level sub-allocators are *not* axiomatised separately (no "EntityAllocatorAxiom" is required) because the K.δ event itself supplies the T10a T2 spawn step structure that activates the sub-allocator and commits the first output in one step. This contrasts with content/link sub-allocators (A_C(d), A_L(d)), whose first emissions cross from `d` to an anchor outside `dom(C) ∪ dom(L)` and require SubAllocatorAxiom.FirstEmission for activation discharge.

In all three K.δ case (ii) sub-cases, the row's "parent allocator's tracked domain" denotes the parent's entity-level sub-allocator at the moment of the K.δ event — which is either pre-activated (k = 0, k = 1 after first version) or activated by the K.δ event itself (k = 2; first k = 1 emission). T10a GlobalUniqueness then closes the freshness obligation directly.
