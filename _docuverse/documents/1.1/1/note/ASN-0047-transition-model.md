...
**Typing note (M total — overrides foundation).** ASN-0036 and ASN-0093 type the arrangement family as *partial*, `M(d) : T ⇀ T`, with `dom(M)` serving as the set of allocated documents. This ASN *overrides* that typing: here M is *total* on T, with `M(d) = ∅` (the empty partial function) for every `d ∉ E_doc`. The document-set role that `dom(M)` played in the foundations is carried by E_doc, under the identity
...
Every `dom(M)`-phrased foundation result reads under (†) with `dom(M) ↦ E_doc`.
...
**SubAllocatorBundle (introduced here).** This is a *bundling lemma* assembled in this ASN, not an axiom and not a result posited by ASN-0093. It collects under one name the sub-allocator facts the discharge chains below rely on, so a call site can cite one bundle rather than five separate ASN-0093 lemmas. Each sub-clause is *proved* by a named ASN-0093 lemma (which ASN-0093 in turn derives from ASN-0040's `SiblingStream`); none is taken axiomatically. For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]` — anchors `b_C(d) = inc(d, 2)`, `b_L(d) = inc(b_C(d), 0)`, per ASN-0093's FirstEmission. The five sub-clauses, each with its discharging ASN-0093 lemma:
...
- **SubAllocatorBundle.T10aConformance.** `A_C(d)` and `A_L(d)` are T10a-conforming allocators within d's allocator subtree, each activated by the entity-allocation event placing d into E_doc. *Discharge:* ChainDiscipline (ASN-0093) identifies each chain as an ASN-0040 `SiblingStream` (hence a T10a-conforming `inc(·, 0)` allocator); ChainEnumerationInjectivity (ASN-0093) gives the strictly increasing (injective) enumeration on which same-subspace freshness rests.
