...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | S3★(Σ') discharged by the K.μ⁻+K.μ⁺ decomposition: survivors via B.1, new content positions via B.2; non-vacuity by the π_swap witness (see *Decomposition of K.μ~*) |  frame |
