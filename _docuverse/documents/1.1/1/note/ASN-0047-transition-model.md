...
*Proof.* If `V_S(d') = ∅`, the conclusion holds with `n'_S = 0`. Otherwise D-SEQ★ applied at the post-state gives `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` for some `n'_S ≥ 1` directly (D-SEQ★ holds at Σ' as a per-state invariant of ExtendedReachableStateInvariants); `V_S(d') ⊆ V_S(d)` (from K.μ⁻'s contraction effect) forces `n'_S ≤ n_S`. ∎
...
| S7b | K.α's `E(a)₁ = s_C` precondition (per ASN-0093) + content sub-allocator chain ⟹ zeros(a)=3 | frame | frame (link addresses not in dom(C)) | frame | frame | frame | frame | frame |
...
*S7b (Element-level I-addresses).* `zeros(a) = 3`: K.α's `E(a)₁ = s_C` precondition (per ASN-0093) fixes `subspace_I(a) = s_C` and the inc chain under a document-level prefix gives `zeros(a) = 3`; preserved by P0.
...
| P4a (every R-entry witnessed by past content-subspace containment) | J1'★ at boundary supplies content-subspace witness `v` with `M'(d)(v) = a` | After K.ρ before its companion K.μ⁺: vacuous (J1'★ rules out spurious K.ρ); within a composite that completes both, never violated |
