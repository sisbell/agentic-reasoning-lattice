...
*Necessity and sufficiency of the precondition.* The precondition is stated against the pre-state `Σ` of a K.μ~ event. Necessity assumes π admissible, so subspace-preservation (iv) enters as a hypothesis; link-fixity (v) is then available as the *derived* property of admissible π established by Step (A), Case `s_L`, not an independent assumption. CL-UNIQ at `Σ` holds by the ExtendedReachableStateInvariants inductive hypothesis on the per-state invariants at the reachable pre-state.
