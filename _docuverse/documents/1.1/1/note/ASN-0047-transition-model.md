...
*Subtree-scoped GlobalUniqueness (SSGU).* For any `inc`-output `a` with `N ≼ a` for a baptised node `N`: cross-node distinctness (T10, ASN-0034, discharged through CrossNodeAccountBase) excludes every event under a distinct baptised node `N' ≠ N`, including a nested node `N'` with `N ≼ N'` or `N' ≼ N` (NodeLineage), whose base — prefix-nesting (NodeLineage) being not `inc`-descent — remains an independent `inc`-root. Scoped to the node-rooted subtree at `N`, GlobalUniqueness (ASN-0034, via T10a.6 cross-allocator domain-disjointness, with `N` discharging the strong-induction base case) assigns `a` to exactly one allocation event within that subtree.
...
Note `inc(t, k')` is non-node: for `k' = 2`, K.δ-ID.zeros-2 gives `zeros(inc(t, 2)) = zeros(t) + 1 ≥ 1`; for `k' = 1`, admissibility requires `Document(t)` (`zeros(t) = 2`), so K.δ-ID.zeros-0/1 gives `zeros(inc(t, 1)) = 2`. Unlike FrontierEquivalence, no `¬Node(t)` precondition on the operand is needed — a `k' = 2` descent may be spawned off a node operand.
...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. This section supplies only what the K.δ box omits: the *parent-allocator activation* (which allocator each step acts on) and, for the child-spawn regimes, the spawnPt premise — consuming the level→allocator-base recovery from ParentAllocatorDispatch (*Sub-allocator names*) that names the owning allocator from an address's zero-count stratum. The per-sub-case freshness reading (FrontierEquivalence / ChildSpawnFreshness) and the spawn-admissibility conjuncts (`k' ∈ {1, 2}` membership and T10a's zero-count side condition) are discharged at the K.δ box and are not repeated here. By k, the parent-allocator activation:
...
**Decomposition.** When `M(d)|_{dom_C(M(d))}` takes at least two distinct values (the operation's precondition), K.μ~ is realised as *any* valid K.μ⁻ + K.μ⁺ pair on `V_{s_C}(d)` whose net effect achieves the bijection equation for π, subject to K.μ⁻'s admissibility (per-subspace suffix removal under D-CTG★/D-MIN★) and K.μ⁺'s preconditions at the intermediate state. By Steps (A)–(B) above, the full-clearance form (`n'_{s_C} = 0`) covers every admissible π; for a given π its concrete realisation is the write set `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}`. The retention of link-subspace mappings under the clearance is the clause-(v) discharge (Step (A), Case `s_L`).
