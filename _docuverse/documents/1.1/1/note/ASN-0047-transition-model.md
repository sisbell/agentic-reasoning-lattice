...
Modeling interior link withdrawal requires a renumbering-aware contraction that the present K.μ⁻ — which contracts the link arrangement by suffix removal alone, never renumbering survivors — does not provide (see the interior-withdrawal entry under Open Questions). The downstream results built on the link-subspace shape (D-SEQ★, the K.μ⁻ contraction, CL-UNIQ, and worked-example Step 5) rest on the discharged invariant.
...
By K.μ~ range-invariance (established in the P4★ Class (b) discharge under *Extended reachable-state invariants*), ran(M'(d)) \ ran(M(d)) is empty, so the J1★ coupling has no new containment pairs to record.
...
By K.μ~ range-invariance (established in the P4★ Class (b) discharge under *Extended reachable-state invariants*), Contains(Σ') = Contains(Σ). All invariants are trivially maintained; no co-occurring transition is needed.
