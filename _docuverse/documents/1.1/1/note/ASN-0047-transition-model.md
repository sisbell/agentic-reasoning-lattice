...
- `d ∈ E_doc`
- `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (ii) `π ≠ id` to admit a witness (the singleton case `|dom_C(M(d))| = 1` admits only the identity permutation; the empty case `dom_C(M(d)) = ∅` is doubly excluded — see *Decomposition* below for the full derivation). Stated here at the definition site as a caller-checked precondition: a transition with `|dom_C(M(d))| ≤ 1` does not fire, and the caller is responsible for verifying this condition before invoking K.μ~. Equivalently, the operation's discharge of admissibility clause (ii) is a sufficiency obligation that the operation realises by exhibiting an admissible `π`; the precondition makes that obligation discharge.
