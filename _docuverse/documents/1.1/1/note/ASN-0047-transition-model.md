...
*Frame (extended state).* `C' = C ∪ {a ↦ v}` (effect); `L' = L; E' = E; (A d :: M'(d) = M(d)); R' = R`. The `L' = L` conjunct discharges P3's L-monotonicity clause `dom(L) ⊆ dom(L')`.
...
**Decomposition.** We derive the necessary-and-sufficient existence condition — `M(d)|_{dom_C}` takes at least two distinct values — by closing the quantifier structure of admissibility clauses (i) and (ii) cleanly.
