...
- *Type-only marker (both F and G empty).* The link references no source or target span, only a type designation. This case is a *design extension beyond the consultation evidence*: Nelson endorsed one-sided links (LM 4/48) but did not consider links with both F and G empty, leaving the case unspecified — neither blessed nor excluded. We admit it on two independent grounds.
  - *Operational consistency with udanax-green.* The implementation accepts the case as a fully reachable code path, not an unreachable defect of a missing runtime guard: `acceptablevsa` (`do2.c:111-112`) returns TRUE unconditionally; `docreatelink` (`do1.c:195-221`) commits the link ISA to granf before any endpoint validation; `specset2sporglset` (`sporgl.c:14-33`) silently accepts NULL specsets returning NULL sporglsets; `insertendsetsinorgl` (`do2.c:130-149`) unconditionally calls `insertpm` for the F and G slots (only Θ is guarded by a `threevsa && threesporglset` check); and `insertpm` with NULL sporglset (`orglinks.c:75-134`) is a silent no-op that still calls `logbertmodified` and returns TRUE. The resulting link is fully allocated, fully committed, and indexed in the spanfilade by Θ.
  - *Structural compatibility with the link-store invariants.* L0/L1/L1a/L1b/L12 and the link-subspace arrangement invariants CL-OWN/CL-UNIQ are properties of `ℓ ∈ dom(L)` and its position in M(d), not of the contents of (F, G, Θ); admitting empty F and G affects none of them. Subsequent `followlink` on F or G of such a link returns request-failed by construction (no POOM entries for the empty endset); follow on Θ proceeds normally.

  We flag this as *design-uncertain*: a future operations ASN that took a stricter reading of Nelson's design — for instance, requiring at least one of F or G to be populated by adding a `F ∪ G ≠ ∅` precondition to K.λ — would not contradict any per-state invariant of this ASN, but would narrow the admissible link values away from this extension.

Coverage of empty endsets in any endset-iterating consumer (L8's `same_type`, link-search predicates over the link's slots, and the discovery-set union over `(F, G, Θ)`) reduces by the natural inductive form: a link's discovery set is the union of its endsets' coverages (ASN-0043, L4 EndsetGenerality treats each slot's endset symmetrically), so an empty F or G simply contributes ∅. CL-OWN and CL-UNIQ apply only to link-subspace V-positions and are unaffected by the contents of the link's endsets.
...
*Freshness discharge.* Three named routes close the `e ∉ E` precondition, selected by the case structure above:

- **Path 0 (NodeUniqueAllocation).** Case (i) — `IsNode(e)`. NodeUniqueAllocation closes `e ∉ E` directly: the node-allocation protocol is axiomatised to produce addresses fresh to E at every event.
- **Path 1 (T10a GlobalUniqueness).** Case (ii) with `InEntityAllocatorDomain(t)` — the inc operand inhabits its parent allocator's tracked domain, so T10a's per-`(t, k')` discipline applies and GlobalUniqueness closes `e ∉ E` by the standard allocator-state argument.
- **Path 2 (Direct inspection at the allocation event).** Case (ii) ghost operands (`¬InEntityAllocatorDomain(t)`) — the K.δ precondition `e ∉ E` is verified against E directly at the allocation event; TA5 supplies only the candidate's structural identity (`e = inc(t, k)` is well-typed). The ghost-routing propagates chain-wide because T10a's T2 spawning requires the operand to inhabit its parent allocator's tracked domain, so an entire chain rooted at a ghost-base step inhabits no T10a-tracked frontier and discharges freshness by Path 2 at every step. *Sequentiality contingency.* Path 2's discharge of `e ∉ E` is *contingent* on the single-event sequential semantics this ASN assumes — the inspection of E and the commit of `E' = E ∪ {e}` are an atomic, uninterruptible pair. Under concurrent or multi-protocol entity allocation, the inspection-to-commit gap admits a window in which another allocator could emit the same candidate, breaking S4 along Path 2 unless an additional discipline (per-allocator serialization, transactional commit, or a global pre-commit uniqueness check) supplies the missing guard. Paths 0 (NodeUniqueAllocation) and 1 (T10a GlobalUniqueness) discharge `e ∉ E` from their respective axiomatic disciplines and are not subject to this contingency. The Open Questions section records the choice of an explicit concurrency discipline as a deferred decision; the present ASN's S4 conclusion for Path 2 transitions holds *under the single-event sequential semantics assumed throughout*.
...
**D-CTG★ / D-MIN★.** ASN-0036's D-CTG and D-MIN have link-subspace exemptions for tombstoning; this ASN introduces strengthened forms D-CTG★ and D-MIN★ that apply uniformly across both subspaces:
...
**Link-subspace fixity.** `π(v) = v` for every `v ∈ dom_L(M(d))`. Proof: subspace preservation puts `π(v) ∈ dom_L(M'(d))` with `M'(d)(π(v)) = M(d)(v) = ℓ`. Since K.μ⁺ (amended) cannot create link-subspace V-positions and K.μ⁻ leaves no room to remove any (by the bijection count above), `dom_L(M'(d)) = dom_L(M(d))` as sets and `M'(d)|_{dom_L} = M(d)|_{dom_L}` as functions; in particular `M'(d)(v) = ℓ`. CL-UNIQ at Σ' then gives `π(v) = v`. ∎
...
**ExtendedReachableStateInvariants (per-state).** Every state reachable from Σ₀ by a finite sequence of valid composite transitions satisfies:
  ...
  S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ S7c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 (over both subspace projections — content-subspace projection by ASN-0036's S8, link-subspace projection by the trivial length-1 decomposition; both supplied per-subspace in the *Foundation invariants* block below) ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P4a ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ NodeLineage ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L1c ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ
...
*S4 scope.* The S4 conjunct of ExtendedReachableStateInvariants holds unconditionally for transitions discharging freshness via Path 0 (NodeUniqueAllocation) or Path 1 (T10a GlobalUniqueness); for transitions discharging via Path 2 (K.δ ghost-base chains, per the *Freshness discharge* paragraph at K.δ), the conjunct holds *under the single-event sequential semantics this ASN assumes throughout*. The concurrency discipline required to sustain S4 along Path 2 under concurrent or multi-protocol allocation is deferred to a subsequent ASN — see Open Questions.
...
S8 in the extended state is established per-subspace. *Content subspace:* by ASN-0036's S8 applied to the projection `M(d')|_{V_{s_C}(d')} : V_{s_C}(d') → dom(C')` (S3★'s content clause is exactly S3 restricted to V_{s_C}(d'), and S2/S7b/S7c/S8a/S8-depth/S8-fin are elementary-preserved). *Link subspace:* directly, by exhibiting an S8-shaped correspondence-run decomposition rather than relying on D-SEQ★(s_L) (which gives the per-subspace structural shape but not the correspondence-run structure S8 requires). The trivial length-1 decomposition discharges S8's existence postcondition for every finite arrangement: for each `v ∈ V_{s_L}(d')`, the singleton `(v, M(d')(v), 1)` is a correspondence run satisfying conditions (a) and (b) trivially at the unique singleton index k = 0 (S8's index range `0 ≤ k < n_j` reduces to k = 0 only; under S8's `shift(·, 0) = identity` convention, condition (b) `M(d')(shift(v, 0)) = shift(M(d')(v), 0)` reduces to `M(d')(v) = M(d')(v)`, and condition (a) holds at v = v_j). S8-fin gives the finiteness of the decomposition `{(v, M(d')(v), 1) : v ∈ V_{s_L}(d')}`. The naïve extension of ASN-0036's S8 to link-subspace V-positions through the unprojected `M(d')` fails because link-subspace V-positions target dom(L), violating S3 on the unprojected domain; the per-subspace projection `M(d')|_{V_{s_L}(d')} : V_{s_L}(d') → dom(L')` restores S3-style discharge (range in dom(L') by S3★'s link clause), and the length-1 decomposition supplies S8 on that projection. (A more structured decomposition with longer runs arises when K.λ + K.μ⁺_L are invoked sequentially without interleaving — the K.μ⁺_L `v_ℓ = shift(max(V_{s_L}(d)), 1)` precondition and the K.λ `inc(max{ℓ : origin(ℓ) = d}, 0)` allocator produce shift-related V-positions and inc-related I-addresses on a single allocator frontier — but the trivial length-1 decomposition already suffices for S8's existence postcondition.)
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a–S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P4★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L1c ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ (per-state). P0, P1, P2, P3★, S9, L12 are *per-transition*: see ExtendedTransitionInvariants. Together supersedes ReachableStateInvariants | This ASN's own Reachable-state invariants synthesis extended to the two-subspace state |
