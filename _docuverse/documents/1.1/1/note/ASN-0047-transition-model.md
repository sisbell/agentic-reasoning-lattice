...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ by a finite sequence of *elementary* transitions drawn from valid composites — i.e. every elementary-transition target, including the intermediate states reached partway through a composite — satisfies the *per-state invariants* below. Every state at a composite boundary (the initial Σ or final Σ' of a valid composite) additionally satisfies the *composite-boundary properties* below.
  ...
  *Per-state invariants* (Class (a) of the proof below — preserved step-by-step by each elementary transition):
  ...
  S2 ∧ S3★ ∧ S3★-aux ∧ S4 ∧ S7a ∧ S7b ∧ C1b ∧ C1c ∧ S7d ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8★ ∧ C-fin ∧ D-CTG★ ∧ D-MIN★ ∧ D-SEQ★ ∧ P6 ∧ P7 ∧ P8 ∧ NodeLineage ∧ L0 ∧ L1 ∧ L1a ∧ L1b ∧ L1c ∧ L3 ∧ L14 ∧ L-fin ∧ CL-OWN ∧ CL-UNIQ
...
**Class (a): Per-state invariants.** These are the *per-state invariants* enumerated in the ExtendedReachableStateInvariants definition above — every reachable-state property except the Class (b) composite-boundary triple P4★, P4a, P7a.
