...
- **Case (ii) ¬IsNode(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
  - *k = 0 (sibling):* `t ∈ E ∧ ¬IsNode(t) ∧ parent(t) = parent(e) ∧ zeros(t) = zeros(e) ∧ t = max{t' ∈ E : parent(t') = parent(e) ∧ zeros(t') = zeros(t)}`. The `¬IsNode(t)` conjunct is implied by the structural identity `zeros(t) = zeros(e)` together with the case-level `¬IsNode(e)` (which gives `zeros(e) ≥ 1`), and is required for `parent(t)` to be defined (T4b's parent projection is partial on T and undefined when `IsNode(t)`); it is stated explicitly so the operand admissibility is visible at the definition site rather than left as a consequence of partial-function evaluation. The maximality conjunct identifies `t` as the *frontier* of the parent allocator's sibling stream at state Σ — the largest sibling already in `E` under the parent allocator that minted t and e's shared parent. Frontier-ness is what makes `inc(t, 0)` fresh against `E`: a non-frontier `t` would have some `t'' ∈ E` with `t'' > t` under the same parent allocator, so `t'' = inc(t, 0)` would already be in `E` and the K.δ event would violate freshness. By contrast, when `t` is the frontier, T10a's per-`(t, 0)` uniqueness guarantees `inc(t, 0) ∉ E` because no prior K.δ event has fired at this `(t, 0)` pair, and T10a's GlobalUniqueness on the sibling-increment chain closes the freshness obligation.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | both clauses preserved via decomposition + link-subspace fixity |  frame |
