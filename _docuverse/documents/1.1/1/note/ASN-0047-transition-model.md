...
*Subspace projections.* `subspace(v) = v₁` (ASN-0036) projects the first component of a V-position; `subspace_I(a) = E(a)₁` (ASN-0043) projects the first component of an I-address's element field. The two apply at different state-component levels — V-positions and I-addresses respectively.
...
*Necessity and sufficiency of the precondition.* The precondition is stated against the pre-state `Σ` of a K.μ~ event. Necessity does not consume CL-UNIQ directly: it assumes π admissible, so subspace-preservation (iv) and link-fixity (v) enter as hypotheses. Necessity relies on link-subspace fixity (*Link-subspace fixity and realisation*); CL-UNIQ at `Σ` holds by the ExtendedReachableStateInvariants inductive hypothesis on the per-state invariants at the reachable pre-state.
