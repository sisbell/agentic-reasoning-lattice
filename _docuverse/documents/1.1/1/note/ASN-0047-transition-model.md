...
*Subspace-position correspondence.* For `v ∈ dom(M(d))` with `M(d)(v) = a`, `subspace(v) = subspace_I(a)`; see S3★ + L0 + S3★-aux (*Generalized referential integrity*). The two projections apply at different state-component levels — `subspace(v)` projects V-positions, `subspace_I(a)` projects I-addresses.
...
Freshness `a ∉ dom(C) ∪ dom(L)` is SubAllocFresh at `x = C` (Lemma SubAllocatorFreshness, *Allocator hierarchy under documents*, below).
...
*Reverse direction (⟸).* Assume `t` is the frontier of `A`'s `(t, 0)`-branch — no prior K.δ event has fired `(t, 0)` on `A`'s chain. We show `inc(t, 0) ∉ Σ.E`. Suppose for contradiction `inc(t, 0) ∈ Σ.E`. Then some allocation event in the system history placed `inc(t, 0)` into E. E is populated by two disjoint mechanisms — NodeBaptism (which places only node addresses) and T10a `inc`-events; we first exclude the former. By TA5(c), `zeros(inc(t, 0)) = zeros(t)`, and since `¬Node(t)` gives `zeros(t) ≥ 1`, we have `zeros(inc(t, 0)) ≥ 1`, so `¬Node(inc(t, 0))` — `inc(t, 0)` is non-node and therefore cannot have been a NodeBaptism output. Hence the placing event was a T10a `inc`-event, to which GlobalUniqueness applies. By GlobalUniqueness (ASN-0034, via T10a.6 cross-allocator domain-disjointness), the address `inc(t, 0)` can be produced by exactly one allocator's tracked chain — namely `A`, the allocator whose tracked chain contains t (since `inc(t, 0)` is, by TA5(c), the next sibling-increment of t on A's chain, and T10a.6 forbids any other allocator's tracked domain from containing it). So the placing event was a K.δ event firing `(t, 0)` on `A`'s chain — contradicting the frontier assumption. Hence `inc(t, 0) ∉ Σ.E`.
...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases, with the freshness conjunct taking the form `inc(t, 0) ∉ E` at k = 0 and `e ∉ E` at k ∈ {1, 2}. *Per-k freshness mechanism (stated once here).* The freshness conjunct is a caller-checked guard in every sub-case — `inc(t, 0) ∉ E` at k = 0 and `e ∉ E` at k ∈ {1, 2} — checked against the present state Σ, not derived from a structural fact. At k = 0, FrontierEquivalence (the *derived* form of T10a's chain-advancement uniqueness at `(t, 0)`) characterizes that guard as the condition that `t` is the frontier of its chain. For all three sub-cases k ∈ {0, 1, 2}, the parent entity-level sub-allocator on which each step acts and the allocator-discipline properties the guard maintains are discharged in §*K.δ case (ii) discharge and parent-allocator activation*. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
  - *k = 0 (sibling):* `t ∈ E ∧ ¬Node(t) ∧ inc(t, 0) ∉ E`. The operand-admissibility conjuncts are `t ∈ E` (the operand must be an allocated entity), `¬Node(t)` (so `parent(t)` is well-defined under T4b), and `inc(t, 0) ∉ E` (the operational frontier check; discharged per the per-k freshness mechanism above). The structural identities `parent(t) = parent(e)` and `zeros(t) = zeros(e)` hold by TA5(c) on `e = inc(t, 0)` (K.δ-ID.parent-0/1, K.δ-ID.zeros-0/1).
...
**V-position depth (operational).** For a subspace `S ∈ {s_C, s_L}`, we write `m_S(d)` for the depth of document `d`'s *current* `S`-subspace arrangement — the common depth that S8-depth (uniform depth within a subspace, ASN-0036) fixes on `V_S(d)` whenever that set is non-empty, bounded below by the S8a lower bound `m_S(d) ≥ 2`. We write `m_C(d)` and `m_L(d)` for the content- and link-subspace instances. This live-depth rule governs both subspaces uniformly. `m_S(d)` is well-defined only while `V_S(d) ≠ ∅`, and is constant within a contiguous non-empty stretch. K.μ⁻ admits full clearance of either subspace (`V_S(d) = ∅`), after which S8-depth is again vacuous and the next insertion into that subspace (K.μ⁺ for `s_C`, K.μ⁺_L for `s_L`) re-pins `m_S(d)` from scratch — at any value `≥ 2` by S8a, not necessarily the prior depth.
...
The constructive precondition is *equivalent* to the post-state characterization "M'(d) satisfies D-CTG★/D-MIN★/D-SEQ★ with value preservation on survivors"; this equivalence is proved in *K.μ⁻ admissible contraction shape* below. The value-preservation clause `(A v : v ∈ dom(M'(d)) : M'(d)(v) = M(d)(v))` (effect clause) is satisfied automatically by the restriction definition `M'(d) = M(d) ↾ R`, ruling out modification of surviving values.
...
*Modeling choice (layer separation).* D-CTG★/D-MIN★ constrain the *arrangement* layer `M(d)` — a presentational mapping from V-positions to I-addresses — and never `dom(L)`, the permanent link address space. This separation is what reconciles the strengthening with Nelson's tombstoning design. Nelson locates link permanence in I-space: link addresses are permanent serial positions assigned in arrival order, and withdrawal makes a link "not currently addressable" while retaining it for historical backtrack (LM 4/9). That retention is an I-space fact — the withdrawn link keeps its permanent order-of-arrival address — not an arrangement-layer obligation to hold a positional gap. Nelson does not give links a dense, rearrangeable arrangement layer the way he gives bytes one: the Vstream renumbers only the byte stream (LM 4/11, 4/66), whereas links are addressed by permanent order of arrival in the `2.x` subspace, so deletion of an interior link does not renumber subsequent links. A link's permanent address therefore already carries its position, and tombstoning never required an interior gap in any separate arrangement. In this model that permanence is discharged entirely by L12 (LinkImmutability) on `dom(L)`: every withdrawn link address persists forever, addressable or not. D-CTG★/D-MIN★, acting only on `M(d)`, thus do not contradict tombstoning — they constrain a different layer.

The arrangement layer that D-CTG★/D-MIN★ *do* constrain is faithful to the implementation. In udanax-green the V-space POOM is kept gap-free: an interior `DELETEVSPAN` physically removes the deleted crum and shifts every subsequent link V-address down (compaction-with-renumbering), leaving no interior gap in the arrangement. A gap-free, minimum-anchored link subspace in `M(d)` is therefore the arrangement-layer invariant the implementation already maintains, and D-CTG★/D-MIN★ record it.

We adopt D-CTG★/D-MIN★ across both subspaces as a justified arrangement-layer contract, with link permanence discharged independently on `dom(L)` by L12. Modeling interior link withdrawal requires a renumbering-aware contraction operation that the present K.μ⁻ — which contracts the link arrangement by suffix removal alone, never renumbering survivors — does not provide; the implementation instead removes an interior link by compaction-with-renumbering (see the interior-withdrawal entry under Open Questions). The downstream results built on the link-subspace shape (D-SEQ★, the K.μ⁻ contraction, CL-UNIQ, and worked-example Step 5) rest on the discharged invariant.

**S8★ (per-subspace span decomposition).** S8★ states the per-subspace analogue of ASN-0036's S8 (SpanDecomposition):

For each `d ∈ E_doc` and each subspace `S ∈ {s_C, s_L}`, the per-subspace arrangement `M(d)|_{V_S(d)}` decomposes into a finite set of correspondence runs `{(v_j, a_j, n_j)}`: every `v ∈ V_S(d)` lies in exactly one run, and within each run the V-positions and I-addresses advance by shift in lockstep. S8★ retains the finite-run *partition* (run-cover: every V-position lies in exactly one run) — the theorem-level statement of ASN-0036's S8 — together with ASN-0036's S8 condition (a) (lockstep displacement: `shift(vⱼ, k) ∈ dom(M(d))`, `M(d)(shift(vⱼ, k)) = shift(aⱼ, k)`, and `shift(aⱼ, k) ∈ dom(Σ.C)`) and condition (b) (label well-definedness: `aⱼ = M(d)(vⱼ)` well-defined by S2, with `aⱼ ∈ dom(Σ.C)`). The treatment of ASN-0036's S8 condition (c) — uniqueness of the maximal-run decomposition — differs by subspace. For the content subspace, the partition together with conditions (a) and (b) *and* condition (c) is exactly ASN-0036's S8: the content-subspace projection's range is `dom(C)` and S3 holds there (S3★ restricted to `V_{s_C}(d)` is S3), so the full ASN-0036 S8 — including the uniqueness of the maximal-run decomposition — applies unchanged. Only the link-subspace projection omits (c): it is discharged by the trivial length-1 decomposition (below), for which uniqueness of a maximal-run decomposition is not asserted. For the link subspace, the membership conjuncts of (a) and (b) carry `dom(Σ.C)`, which fails for link-subspace labels; the *Link subspace* route below derives the substitution `dom(C) → dom(L)` that repairs them. The decomposition is established by two distinct routes, one per subspace:
...
**SubAllocatorBundle (introduced here).** Each sub-clause below is proved by a named ASN-0093 lemma. For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]` — anchors `b_C(d) = inc(d, 2)`, `b_L(d) = inc(b_C(d), 0)`, per ASN-0093's FirstEmission. The five sub-clauses, each with its discharging ASN-0093 lemma:

- **SubAllocatorBundle.Subspace.** Outputs of the two sub-allocators inhabit `s_C` and `s_L` respectively: every `a` emitted by `A_C(d)` has `subspace_I(a) = s_C`, every `ℓ` emitted by `A_L(d)` has `subspace_I(ℓ) = s_L`. *Discharge:* DisjointSubAllocatorChains (ASN-0093), which reads `E(·)₁ = s_C` along every element of `A_C(d)` and `E(·)₁ = s_L` along every element of `A_L(d)`.
- **SubAllocatorBundle.FirstEmission.** The first emission of each is the determinate tumbler `[d.0.s_C.1]` (resp. `[d.0.s_L.1]`), satisfying `a ∉ dom(Σ.C) ∪ dom(Σ.L)` at the state of allocation with `origin(a) = d` and `#E(a) = 2`. *Discharge:* FirstEmission (ASN-0093) supplies the determinate form (`origin = d`, `#E = 2`, `zeros = 3`, T4-valid); FirstEmissionFreshness (ASN-0093) supplies the freshness against `dom(Σ.C) ∪ dom(Σ.L)`.
- **SubAllocatorBundle.Namespace.** Every output of d's sub-allocators is T4-valid with `zeros(·) = 3`. *Discharge:* ChainElementT4Validity (ASN-0093) gives T4-validity of every chain element; `zeros(·) = 3` is the structural form of FirstEmission (ASN-0093) propagated along the chain by ChainPrefixExtension.
- **SubAllocatorBundle.T10aConformance.** `A_C(d)` and `A_L(d)` are T10a-conforming allocators within d's allocator subtree, each activated by the entity-allocation event placing d into E_doc. *Discharge:* ChainDiscipline (ASN-0093) establishes each chain as a T10a-conforming `inc(·, 0)` sibling-advance allocator; ChainEnumerationInjectivity (ASN-0093) gives the strictly increasing (injective) enumeration on which same-subspace freshness rests.
- **SubAllocatorBundle.Disjointness.** `dom(A_C(d)) ∩ dom(A_L(d)) = ∅`, and for any d ≠ d', `dom(A_C(d)) ∩ dom(A_C(d')) = ∅`, `dom(A_L(d)) ∩ dom(A_L(d')) = ∅`, `dom(A_C(d)) ∩ dom(A_L(d')) = ∅`. *Within-document discharge:* DisjointSubAllocatorChains (ASN-0093) supplies `dom(A_C(d)) ∩ dom(A_L(d)) = ∅` directly; equivalently, every output of `A_C(d)` has `E(·)₁ = s_C` and every output of `A_L(d)` has `E(·)₁ = s_L` (SubAllocatorBundle.Subspace), SC-NEQ (SubspaceConventionAxiom) gives `s_C ≠ s_L`, and T7 (SubspaceDisjointness, ASN-0034) — whose element-level hypothesis `zeros(·) = 3` is supplied by SubAllocatorBundle.Namespace — makes any `A_C(d)` output distinct from any `A_L(d)` output. *Cross-document discharge:* the same-subspace clauses `dom(A_C(d)) ∩ dom(A_C(d')) = ∅` and `dom(A_L(d)) ∩ dom(A_L(d')) = ∅` are CrossDocumentDisjointness (ASN-0093) at anchor pairs `(b_C(d), b_C(d'))` and `(b_L(d), b_L(d'))`. The cross-subspace fourth clause `dom(A_C(d)) ∩ dom(A_L(d')) = ∅` (anchors `[d.0.s_C]`, `[d'.0.s_L]`) is the genuine delta, dispatched by CrossDocDisjoint (below) at `(s₁, s₂) = (s_C, s_L)`.
...
- *Seed* (first emission, predicate `{a' ∈ dom(x-store) : origin(a') = d} = ∅`): `a` is the determinate first emission `[d.0.s_x.1]`, fresh against `dom(C) ∪ dom(L)` by SubAllocatorBundle.FirstEmission (discharged by FirstEmissionFreshness, ASN-0093).
- *Frontier advance* (subsequent emission, predicate non-empty): `a = inc(max{a' ∈ dom(x-store) : origin(a') = d}, 0)` (TA5(c)) is the next sibling on `A_x(d)`'s inc chain; freshness against the same-subspace store is GlobalUniqueness (ASN-0034) on `A_x(d)`'s tracked chain (SubAllocatorBundle.T10aConformance).
...
Freshness `ℓ ∉ dom(L) ∪ dom(C)` is SubAllocFresh at `x = L` (Lemma SubAllocatorFreshness, *Allocator hierarchy under documents*).
...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. The operand-admissibility conjuncts, freshness mechanism, and structural identities are discharged in the K.δ definition's case (ii); this section adds only the *parent-allocator activation* (which allocator each step acts on) and, for the child-spawn regimes, the spawnPt premise. The three regimes:
...
- *k = 2 (descent producing the first child under a node or account):* the activation case. Let `t` be the K.δ operand (a node when creating an account; an account when creating a document) and `e = inc(t, 2)` the spawn output. The K.δ event spawns a *new entity-level sub-allocator* under `t`: when t is a node, `A_account(t)` (t's *account sub-allocator*, catalogued in *Sub-allocator names* above); when t is an account, `A_doc(t)` (t's *document sub-allocator*, also catalogued above). The K.δ event is the T10a child-spawn step that *creates* the spawned allocator, with `t` as spawnPt and `e` as the first emission.
  ...
  T10a child-spawn admissibility requires the spawnPt `t` to inhabit `dom(parent_allocator)` at the spawn event — t must already lie in the tracked domain of whatever sits *above* the newly-spawned allocator in the allocator tree, namely the source that minted `t`. The spawnPt premise `t ∈ dom(parent_allocator)` is supplied by one of three sources according to `t`'s level; the remainder of the discharge is uniform across all three:

  | `t`'s level | spawned allocator (parent above) | spawnPt-premise source |
  |-------------|----------------------------------|------------------------|
  | account (`zeros(t) = 1`) | `A_doc(t)` (parent `A_account(parent(t))`) | ParentAllocatorDispatch's account-level identification: `t` is the unique emission of `A_account(parent(t))` (`parent(t)` a node since `zeros(t) = 1`), uniqueness by T10a.6, membership preserved to the present event by AllocatedSet monotonicity |
...
Combined with L0 and S3★-aux, S3★ yields the subspace-position correspondence `subspace(v) = subspace_I(a)` for `M(d)(v) = a` (used in the *Notation*). The equality is a two-step chain: S3★ routes the value to the correct store by subspace — `subspace(v) = s_C ⟹ a ∈ dom(C)` and `subspace(v) = s_L ⟹ a ∈ dom(L)` — and L0 fixes that store's I-address identifier, `subspace_I(a) = s_C` for `a ∈ dom(C)` and `subspace_I(a) = s_L` for `a ∈ dom(L)`. S3★ alone yields only store *membership*, not equality of subspace identifiers; L0 supplies the second step. Because S3★ is conditional — silent on any V-position whose first component is neither `s_C` nor `s_L` — the per-branch equality promotes to the universal correspondence over all `v ∈ dom(M(d))` only via S3★-aux (SubspaceExhaustiveness), which guarantees every V-position falls into the `s_C` or `s_L` branch.
...
π is admissible iff (i) the induced post-state `M'(d)` would satisfy the arrangement-*shape* invariant package on `M'(d)` — S8a, S8-depth, S8-fin, D-CTG★, D-MIN★, from which the derived D-SEQ★ follows (D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a) — (ii) the net effect is non-trivial, `M'(d) ≠ M(d)`; (iii) π is *length-preserving*: `(A v ∈ dom(M(d)) :: #π(v) = #v)`; and (iv) π is *subspace-preserving*: `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))`; and (v) π is *link-subspace fixing*: `(A v ∈ dom_L(M(d)) :: π(v) = v)`. Clause (iv) is a genuine independent hypothesis, not a consequence of (i)–(iii): S8-depth permits two subspaces to share a depth (`m_{s_C} = m_{s_L}` — the worked examples do exactly this), and at a coinciding depth a transposition swapping a content position `[s_C, …, k]` with a link position `[s_L, …, j]` is length-preserving (iii), non-trivial (ii), and leaves the per-subspace V-position *sets* satisfying the shape package (i) is a constraint on V-position *domains*, not on which I-address each carries) — yet it relocates a `dom(C)` value to an `s_L` position, breaking S3★. Clause (iv) excludes exactly such cross-subspace permutations. Clause (v) is likewise independent of (i)–(iv): at a depth shared by two distinct link positions, a transposition *within* the link subspace — swapping `[s_L, …, 1] ↔ [s_L, …, 2]` carrying distinct links `ℓ₁ ≠ ℓ₂` — is length-preserving (iii), subspace-preserving (iv), non-trivial (ii), and leaves the V-position domains satisfying the shape package (i), yet it re-seats a link at a different V-position. The full-clearance decomposition cannot realise such a re-seating: K.μ⁺ writes only content-subspace positions and K.μ⁻ removes link positions only by suffix, so links are retained pointwise — every realisable π fixes the link subspace (Step (A), Case `s_L`). Clause (v) excludes exactly these within-`s_L` permutations, reflecting the design intent that links carry permanent order of arrival in their home document and are not rearrangeable (Nelson, LM 4/12). Clauses (iv) and (v) together are what make the realisable π (those the K.μ⁻ + K.μ⁺ full-clearance decomposition produces) coincide with the admissible π. Clause (iii) confines K.μ~ to a permutation of the document's existing V-positions *at fixed depth*: REARRANGE transposes contiguous regions within a document's flat, dense V-stream (Nelson 4/67), reordering byte positions the document already holds rather than relocating content to a new structural depth — the V-stream has no internal depth levels for content to be moved into. Clause (iii) thus yields per-subspace depth fixity, from which K.μ~-FIX (Domain fixity) below follows. Clause (ii) makes K.μ~ a real reordering: a permutation whose net effect is the identity arrangement is not a K.μ~ transition (the system simply does not change). Note that `M'(d) ≠ M(d)` is strictly stronger than the map-level `π ≠ id`: under S5 (UnrestrictedSharing, ASN-0036) two distinct content V-positions may carry the same I-address (transclusion), so the swap of two such equal-valued positions is a non-identity *map* with net-identity *effect* — clause (ii)'s net-effect form excludes it.

*Step (A) — Admissible π coincide with realisable π.* Subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` and link-subspace fixity `(A v ∈ dom_L(M(d)) :: π(v) = v)` hold for every *admissible* π directly by admissibility clauses (iv) and (v). What this step establishes is the matching realisability direction: every admissible π is *realisable* — produced by the K.μ⁻ + K.μ⁺ decomposition in the full-clearance form — and conversely every realisable π is subspace-preserving and link-subspace fixing (hence admissible), so the two classes coincide and the decomposition realises exactly the admissible π. S3★-aux(Σ) confines every source subspace to `{s_C, s_L}`, so the two cases below are exhaustive.
...
*Case `subspace(v) = s_L`.* K.μ⁺ writes only content-subspace positions, so the link-subspace positions of `M'(d)` are exactly those K.μ⁻ retains — namely the link-subspace positions of `M(d)`, unchanged, with `M'(d)|_{dom_L} = M(d)|_{dom_L}` — while the content-subspace positions of `M'(d)` are exactly K.μ⁺'s write set, which the content sources of the preceding case already exhaust bijectively. For a link source `v` with `M(d)(v) = ℓ`, the bijection equation forces `M'(d)(π(v)) = ℓ` with `π(v)` a retained link position; CL-UNIQ at Σ (link-subspace injectivity of `M(d)|_{dom_L}`) then forces `π(v) = v`. Hence every realisable π is link-subspace fixing (clause (v)) and `subspace(π(v)) = s_L = subspace(v)`.

Hence every realisable π preserves subspace and fixes the link subspace pointwise; combined with clauses (iv) and (v) (every admissible π does likewise) and the full-clearance decomposition realising every admissible π, the admissible and realisable classes coincide. ∎ (Step (A))

*Proof of Step (B) — the K.μ⁻ + K.μ⁺ full-clearance decomposition realises any admissible π.* This step shows mechanical realisability: the K.μ⁻ + K.μ⁺ (full-clearance) decomposition actually realises π and *establishes* `S3★(Σ')` at the post-state `Σ' = (C, L, E, M', R)` from the two Class (a) cells. The realisable π are precisely those whose content images satisfy K.μ⁺'s content-subspace amendment precondition — equivalently (Step (A)), the subspace-preserving ones — so K.μ⁺'s amendment fires on the full-clearance write set by realisability. Let `Σ_int = (C, L, E, M_int, R)` denote the intermediate state between the K.μ⁻ and K.μ⁺ atomic steps of the decomposition. Three sub-claims, dispatched in order:
...
**Link-subspace fixity.** `π(v) = v` for every `v ∈ dom_L(M(d))`. For an admissible π this is admissibility clause (v) directly — fixity is now a *hypothesis* on the admissible class, not a theorem derived over it. The sub-steps below confirm the K.μ⁻ + K.μ⁺ full-clearance realisation is consistent with clause (v): it preserves the link subspace functionally, `M'(d)|_{dom_L} = M(d)|_{dom_L}` (sub-steps (1)–(3)), and this functional identity carries CL-UNIQ to the post-state and re-derives the pointwise fixity that clause (v) demands (sub-step (4); premise: CL-UNIQ at the pre-state Σ). Proof:

(1) *Link-subspace domain fixity.* K.μ~-FIX establishes `V_S(d') = V_S(d)` for every subspace S; at `S = s_L` this is `dom_L(M'(d)) = dom_L(M(d))` as sets.

(2) *K.μ⁻'s removal set lies in dom_C.* K.μ⁺ (amended) cannot create link-subspace V-positions, so any link-subspace V-position present in dom_L(M'(d)) must have been present in dom_L(M(d)) — the K.μ⁻ + K.μ⁺ pipeline through K.μ~'s decomposition cannot introduce new link-subspace positions. Combined with the set equality from (1), K.μ⁻'s removal set X = dom(M(d)) \ dom(M_int(d)) must satisfy `X ∩ dom_L(M(d)) = ∅` — every removed position is content-subspace. (Were a link-subspace position removed by K.μ⁻, the subsequent K.μ⁺ could not restore it, leaving `|dom_L(M'(d))| < |dom_L(M(d))|` and contradicting (1).)
...
*Necessity and sufficiency of the precondition.* The precondition is stated against the pre-state `Σ` of a K.μ~ event; its necessity direction consumes CL-UNIQ at `Σ`, which holds by the ExtendedReachableStateInvariants inductive hypothesis on the per-state invariants at the reachable pre-state.
  - *Necessity (universal closure over admissible π).* Suppose K.μ~ admits some π. Then π satisfies admissibility (i) (the arrangement-shape package on M'(d)) and admissibility (ii) (net effect `M'(d) ≠ M(d)`); S3★(Σ') holds as a derived consequence (Step (B)). By Step (A) and Link-subspace fixity together with K.μ~-FIX (`dom(M'(d)) = dom(M(d))`), π fixes `dom_L(M(d))` pointwise and preserves each subspace, so it maps `dom_C(M(d))` bijectively onto itself; the net change `M'(d) ≠ M(d)` therefore cannot lie in the link subspace and must lie in the content subspace, `M'(d)|_{dom_C} ≠ M(d)|_{dom_C}`. The remaining step is new: suppose for contradiction `M(d)|_{dom_C}` were constant with common value `a`. Content-subspace closure places `π⁻¹(u) ∈ dom_C(M(d))` for every `u ∈ dom_C(M(d))`, so the bijection equation gives `M'(d)(u) = M(d)(π⁻¹(u)) = a = M(d)(u)` — whence `M'(d)|_{dom_C} = M(d)|_{dom_C}`, contradicting clause (ii). Therefore `M(d)|_{dom_C}` is non-constant: it takes at least two distinct values. ∎
  - *Sufficiency (explicit construction).* When `M(d)|_{dom_C}` takes at least two distinct values, an admissible π always exists; we exhibit a concrete witness. Pick two elements `v₁, v₂ ∈ dom_C(M(d))` with `M(d)(v₁) ≠ M(d)(v₂)` — such a pair exists precisely because `M(d)|_{dom_C}` is non-constant, and the value-distinctness forces `v₁ ≠ v₂`. Define the *transposition witness*
    ...
    - *Clause (v) (link-subspace fixing).* `π_swap` fixes every position outside `{v₁, v₂}`, and `v₁, v₂ ∈ dom_C(M(d))`; since `dom_L(M(d))` is disjoint from `{v₁, v₂}`, `π_swap(v) = v` for every `v ∈ dom_L(M(d))`.
    - *Clause (i) (post-state invariants on `M'(d)`).* Domain fixity holds *by construction*: `π_swap` is a permutation of `dom(M(d))` (a transposition fixing all but `v₁, v₂`), so the bijection equation gives `dom(M'(d)) = π_swap(dom(M(d))) = dom(M(d))` directly, independent of any post-state invariant. With the V-position domain thus fixed, `S8a`, `S8-depth`, `S8-fin`, `D-CTG★`, `D-MIN★`, `D-SEQ★` at the post-state inherit unchanged from the pre-state's per-state hypothesis. (Recall S3★ is *not* a clause-(i) constraint — clause (i) is the arrangement-*shape* package only — so it is discharged separately, immediately below.)
    Since clauses (ii), (iv), (iii), (v), and (i) all hold, `π_swap` is admissible, and the full-clearance form of the K.μ⁻ + K.μ⁺ decomposition realises it. The separately-derived post-state obligation S3★(Σ') then follows from the general Step (B) derivation, which establishes S3★ at the post-state for *every* realisable π; no inline re-derivation specific to `π_swap` is needed. The transposition witness is the simplest one; many other admissible π exist, but a single witness suffices for the sufficiency obligation.
...
**Decomposition.** When `M(d)|_{dom_C(M(d))}` takes at least two distinct values (the operation's precondition), K.μ~ is realised as *any* valid K.μ⁻ + K.μ⁺ pair on `V_{s_C}(d)` whose net effect achieves the bijection equation for π, subject to K.μ⁻'s admissibility (per-subspace suffix removal under D-CTG★/D-MIN★) and K.μ⁺'s preconditions at the intermediate state. The verification arguments realise every admissible π by the full-clearance form stated canonically above (`n'_{s_C} = 0`); for a given π its concrete realisation is the write set `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}`. The retention of link-subspace mappings under the clearance is forced rather than incidental: K.μ⁺ (amended) is content-only and K.μ⁺_L only places at the contiguous min or max, so any link-subspace position removed by K.μ⁻ could not be restored.
...
The K.ρ/K.μ⁺ coupling trigger is range-based, not unconditional; the precise firing condition is J1★ (Scoped coupling constraints, below).
...
P4★ bounds provenance by the *content-subspace* restriction of containment rather than by the unscoped `Contains(Σ) ⊆ R`. The unscoped bound cannot hold once any link-subspace mapping exists: `Contains(Σ)` would then include `(ℓ, d)` for every link `ℓ` mapped in d's arrangement, yet provenance entries satisfy P7 (ProvenanceGrounding, stated below) — `(A (a, d) ∈ R :: a ∈ dom(C))` — and since `ℓ ∈ dom(L)` with `dom(L) ∩ dom(C) = ∅` (L14), `(ℓ, d) ∉ R`. Restricting the bound to the content subspace drops exactly the link-subspace pairs that P7 forbids. In pre-extension states (no link-subspace mappings) `Contains_C(Σ) = Contains(Σ)`, so the scoped bound coincides with the unscoped one there. P4★ is a Class (b) composite-boundary property: K.μ⁺ alone may transiently violate it (it adds to Contains_C but does not extend R), with restoration at the composite boundary governed by J1★.
...
The witnessing existential ranges over the finite set `{Σ₀, ..., Σ_n}` of trace states, so the property is well-typed as a trace property even though it is not well-typed as a per-state invariant. Provenance rides on the permanent I-address and survives deletion from the current arrangement, so the witness is whatever historical version contained the content, not a live moment-of-recording check (Nelson, LM 4/9, 4/11 — included content "may remain included in other versions" after deletion from the current one). The content-subspace qualification is essential: J1'★ scopes provenance recording to content-subspace range changes (link-subspace mappings target `dom(L)`, which is disjoint from `dom(C)` by L14, so no link-subspace V-position can witness provenance under P7's `a ∈ dom(C)` requirement). P4a therefore reads as "every provenance entry corresponds to a content-subspace arrangement in some trace state," consistent with both P7's grounding in `dom(C)` and J1'★'s content-scoped coupling. *Discharge mechanism.* P4a is discharged by induction along the witnessing trace, not by a per-state check: a freshly recorded entry `(a, d) ∈ R' \ R` is witnessed by the post-state Σ' itself — the K.μ⁺ that fires somewhere in the composite places a content-subspace V-position `v` with `M'(d)(v) = a`. ValidComposite★ requires this witness only at the composite endpoint Σ' (J1'★), not at the moment of recording, so the discharge is robust to the internal ordering of K.ρ and K.μ⁺: an ordering such as K.α → K.ρ → K.μ⁺ (each step satisfying its elementary precondition, since K.ρ requires only `a ∈ dom(C) ∧ d ∈ E_doc`) passes through an intermediate state with no live witnessing V-position, yet Σ' carries the witness placed by the eventual K.μ⁺. A persisted entry `(a, d) ∈ R` inherits a witnessing trace state from the inductive hypothesis, carried forward by P2.
...
The wp analysis confirms this. For P0: K.μ⁻ does not touch C. For P1: does not touch E. For P2: does not touch R. For L12: does not touch L. For the operative provenance bound P4★: contraction can only remove pairs from Contains_C, so Contains_C(Σ') ⊆ Contains_C(Σ) ⊆ R = R'. No co-occurring transition is needed to maintain any system invariant.
...
**J4 (Fork composite).** Nelson's forking creation mode — version creation with ancestry indication (LM 4/66, CREATENEWVERSION) — is a composite whose elementary steps are exactly K.δ + K.μ⁺ + K.ρ, all serving the new document d_new. *Fork is version creation on d_src's version chain `A_v(d_src)`, with K.δ address allocation uniform across first and subsequent versions:* the first version is d_new = inc(d_src, 1) — the K.δ k = 1 step that activates `A_v(d_src)` — and each subsequent version is d_new = inc(prev_version, 0), a K.δ k = 0 step on `A_v(d_src)`'s frontier whose operand prev_version is a prior version of d_src. The transcluded content source tracks the K.δ operand (step (ii) of the Definition below establishes this and its consequence for subsequent versions). In every case d_new extends d_src's document-field prefix (d_src ≼ d_new), so d_new carries the ancestry-by-address indication regardless of whether the emitting step is k = 1 or k = 0. The uniformity is necessary: K.δ's per-`(t, 1)` uniqueness fires `inc(d_src, 1)` at most once, so versions 2+ *must* arise as k = 0 frontier-advances on `A_v(d_src)` — restricting fork to k = 1 would capture only the first version. This matches Gregory's `docreatenewversion`, which performs the identical `findisatoinsertnonmolecule` allocation for every version (the first allocation produces `d_src.1`, subsequent allocations advance the last component to `d_src.k+1`), special-casing neither. By contrast, the k = 0 *sibling-document* allocation under the source's account (`docreatenewdocument`) and the k = 2 hierarchical descent are *not* forks: their outputs do not extend d_src's prefix and carry no ancestry-by-address indication. The discriminating fact is which allocator's frontier the k = 0 operand sits on — `A_v(d_src)`'s frontier (a version of d_src, a fork) versus `A_doc(parent(d_src))`'s frontier (an independent sibling document, not a fork).

**Definition (Fork).** A *fork* of d_src to d_new is a composite transition `Σ →* Σ'`. Write `d_op` for the *content source operand* of the fork — the operand `t` of the K.δ step (i), whose current content subspace is transcluded into d_new (the per-sub-case value of `d_op` is spelled out in step (i), and its load-bearing reading in step (ii)). The *precondition* is d_src ∈ E_doc ∧ d_op ∈ E_doc ∧ V_{s_C}(d_op) ≠ ∅, together with the per-sub-case activation condition on the K.δ step: the k = 1 sub-case fires when `A_v(d_src)` has no prior emission (no version yet exists), and the k = 0 sub-case fires when `A_v(d_src)` already has a frontier (a prior version exists, supplying `prev_version`). It consists of:

(i) a K.δ case (ii) step producing d_new on d_src's version chain `A_v(d_src)`, with d_new ∉ E_doc — either k = 1 with t = d_src, producing the first version d_new = inc(d_src, 1) when `A_v(d_src)` has no prior emission, or k = 0 with t = prev_version the current frontier of `A_v(d_src)`, producing the next version d_new = inc(prev_version, 0); in both cases d_src ≼ d_new,

(ii) K.μ⁺ populating M'(d_new) from `d_op`'s content subspace under transclusion: `ran(M'(d_new)) ⊆ ran(M(d_op)|_{V_{s_C}(d_op)})` — no new content addresses are introduced, every target lies in the pre-existing content store. The content source is the K.δ operand `d_op`, not invariably `d_src`: a subsequent version (k = 0) inherits the *current edited content* of the prior version `prev_version = d_op` it is forked from, capturing edits absent from the original base d_src. This matches Nelson's CREATENEWVERSION, which copies the contents of whatever document it is invoked on (LM 4/66, "the contents of document `<doc id>`"), and Gregory's `docreatenewversion`, which reads the source document's live POOM — including all prior INSERT/DELETE edits — rather than re-deriving from any ancestor,
...
The new document d_new is created empty (K.δ), its arrangement extended with mappings to the content source operand's I-addresses (K.μ⁺, with `ran(M'(d_new)) ⊆ ran(M(d_op)|_{V_{s_C}(d_op)})`), and the new associations recorded (K.ρ). The precondition V_{s_C}(d_op) ≠ ∅ ensures K.μ⁺ is well-formed. Since K.μ⁺ (amended) creates only content-subspace V-positions, the I-addresses it maps to must lie in dom(C) (by S3★'s content clause). Only content-subspace V-positions in d_op have I-addresses in dom(C) — link-subspace V-positions map to dom(L), and dom(L) ∩ dom(C) = ∅ (L14). With V_{s_C}(d_op) ≠ ∅, there is at least one content I-address to transclude, so the strict domain extension dom(M'(d_new)) ⊃ dom(M(d_new)) = ∅ is satisfiable. The weaker condition M(d_op) ≠ ∅ is insufficient: a document with only link-subspace positions has ran(M(d_op)) ⊆ dom(L), and no address in dom(L) can serve as the target of a content-subspace V-position — so when d_op's content subspace is empty (V_{s_C}(d_op) = ∅) the fork definition does not apply, and creation from such a source is ex nihilo (K.δ alone), not a fork. Nelson: "the new document's id will indicate its ancestry."
...
Provenance coupling must be scoped to content-subspace arrangement extensions, for the same reason the unscoped bound `Contains(Σ) ⊆ R` fails against P7 (see P4★ above): an unscoped coupling cannot coexist with P7 once any link-subspace mapping exists. This motivates the content-subspace forms J1★ and J1'★ below.
...
**Step 3: K.δ case (ii) k = 2 — allocate document `1.2.0.1.0.1 = inc(1.2.0.1, 2)`.** TA5(d) gives `zeros = 2`, `parent = 1.2.0.1`. Preconditions analogous to Step 2; GlobalUniqueness (ASN-0034) at the document sub-allocator `A_doc(1.2.0.1)` discharges `d ∉ E₂`. Effect: `E₃ = E₂ ∪ {1.2.0.1.0.1}`, with `M₃(1.2.0.1.0.1) = ∅` and SubAllocatorBundle activating the content and link sub-allocators (anchors `b_C(d) = [d.0.1]`, `b_L(d) = [d.0.2]`).

**Step 4: K.δ case (ii) k = 0 — allocate sibling document `1.2.0.1.0.2 = inc(1.2.0.1.0.1, 0)` under the same account.** Operand `t = 1.2.0.1.0.1`, the document placed by Step 3. The k = 0 dispatch differs from k ∈ {1, 2} in two distinctive features: (a) freshness `inc(t, 0) ∉ E` is discharged by the k = 0 limb of the K.δ definition's per-k freshness mechanism (FrontierEquivalence), not the direct per-`(t, k')` axiom used at k ∈ {1, 2}; and (b) the dispatch through T10a.6 (DomainDisjointness) routes the verification against t's actual provenance — for this t, `A_doc(parent(t)) = A_doc(1.2.0.1)`, the account's document sub-allocator activated by Step 3.
...
*A_v(d₁) frontier-advance discharge.* The operand t = d₂ = 1.0.1.0.1.1 is the current frontier of `A_v(d₁)` (placed as its first emission by the k = 1 fork above, with no later sibling-advance). Freshness `inc(d₂, 0) = 1.0.1.0.1.2 ∉ E₃` is discharged via FrontierEquivalence (the derived form of T10a's chain-advancement uniqueness at (d₂, 0)), not the direct per-(t, k') axiom. The output d₃ is the second emission of `A_v(d₁)`, and d₁ ≼ d₃ (1.0.1.0.1 ≼ 1.0.1.0.1.2), so d₃ carries the ancestry-by-address indication.

*K.μ⁺:* M'(d₃) = {[1,1] ↦ a₁, [1,2] ↦ a₂, [1,3] ↦ a₃} — the same I-addresses as the *edited* d₂, transcluding d_op = d₂'s current content subspace. The range bound `ran(M'(d₃)) ⊆ ran(M(d_op)|_{V_{s_C}(d_op)}) = ran(M₃(d₂)|_{s_C}) = {a₁, a₂, a₃}` holds with equality; crucially a₃ — d₂'s edit, absent from d₁ — is inherited. No new content enters C (C' = C₃). The three content-subspace V-positions satisfy S8a (positive components, zeros = 0) and S8-depth (uniform depth 2).
...
- *J4 (range bound):* d₃ ∈ E'_doc \ E₃_doc, and ran(M'(d₃)) = {a₁, a₂, a₃} ⊆ ran(M(d_op)|_{V_{s_C}(d_op)}) = {a₁, a₂, a₃} with d_op = d₂. The bound is against the *operand* d₂. It would *fail* against the original base d₁: ran(M₃(d₁)|_{s_C}) = {a₁, a₂} does not contain a₃, so the (incorrect) d_src-fixed bound `ran(M'(d₃)) ⊆ ran(M(d₁)|_{V_{s_C}(d₁)})` would be violated by a₃ — precisely the silent edit-loss the operand-tracking source prevents. ✓
...
The k = 0 fork inherits a₃ — the prior version's edit — because the transclusion source is the operand d₂ (the chain frontier), not the base d₁. This is Nelson's CREATENEWVERSION copying the contents of the document it is invoked on (LM 4/66, here d₂) and Gregory's `docreatenewversion` reading that document's live POOM including all prior edits. The address allocation, by contrast, is uniform with the k = 1 case — a frontier-advance on `A_v(d₁)` — so the two sub-cases share an allocation discipline while differing only in which document supplies the transcluded content.
...
**P7a (Provenance coverage).** Every I-address in the content store has at least one provenance record, `(A a ∈ dom(C) :: (E d :: (a, d) ∈ R))`. It is a composite-boundary property (Class (b) of ExtendedReachableStateInvariants).
...
*(iii) Link addresses.* `(A ℓ ∈ dom(L) :: n₀ ≼ ℓ)`. By L1a, `origin(ℓ) ∈ E_doc ⊆ E`, so (i) gives `n₀ ≼ origin(ℓ)`. By L1, `zeros(ℓ) = 3`, so T4b's parse makes `origin(ℓ) = N(ℓ).0.U(ℓ).0.D(ℓ)` the prefix of ℓ up to (but not including) ℓ's third zero separator — hence `origin(ℓ) ≼ ℓ`, by the same document-level-prefix truncation used for content in part (ii). Transitivity of ≼ closes: `n₀ ≼ origin(ℓ) ≼ ℓ`. ∎
...
The *Composite-boundary verification matrix* below records, for each Class (b) property, where it transiently fails and the coupling that restores it (per the preamble's temporal-scope distinction). P4★ and P7a are per-state properties holding at boundaries; P4a is a *trace property* (defined and discharged in its definition box), grouped here because, like the other two, it is established by the boundary couplings rather than preserved step-by-step.
...
**Class (a): Per-state invariants.** These are all reachable-state properties except P4★, P4a, and P7a: S2, S3★, S3★-aux, S4, S7a, S7b, C1b, S7d, S8a, S8-fin, S8-depth, S8★, C-fin, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, NodeLineage, L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, CL-UNIQ.

*Verification matrix.* Each cell names the load-bearing discharge for that (invariant, transition) pair; `frame` indicates the transition holds the relevant state component unchanged and so trivially preserves the invariant; `n/a` indicates the invariant's scope does not intersect the transition's effect (e.g., L0's L-clause is `n/a` for transitions that frame both L and C). For link-store invariant rows (L0, L1, L1a, L1b, L1c, L3, L14, L-fin, CL-OWN, CL-UNIQ), `frame` under K.α, K.μ⁺, K.μ⁻ rests on the amended forms of *Amendments to existing transitions* above, which add the explicit `L' = L` conjunct (or, for K.α, ASN-0093's frame supplying it directly), not on the original pre-link transitions, which made no commitment about L. K.μ~ cells invoke the full-clearance form (`n'_{s_C} = 0`) of the K.μ⁻ + K.μ⁺ decomposition (any cell not naming a cut point reads as full-clearance, which is admissible for every admissible π).

| Invariant | K.α | K.δ | K.λ | K.μ⁺ | K.μ⁺_L | K.μ⁻ | K.μ~ | K.ρ |
|-----------|-----|-----|-----|------|--------|------|------|-----|
| S2 | frame | frame (M(e)=∅ on new entity disjoint) | frame | new positions disjoint from dom(M(d)) by value-preservation clause | new v_ℓ ∉ dom(M(d)) (verified at K.μ⁺_L) | restriction of M(d) | π bijection ⟹ M'(d) single-valued | frame |
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | elementary decomposition K.μ⁻+K.μ⁺ (this row's restriction + amendment Class (a) cells); realisable π are exactly the admissible ones — subspace-preserving (K.μ⁺ content-subspace precondition) and link-subspace fixing (clause (v)); §*Decomposition of K.μ~* |  frame |
...
| S8a, S8-depth, S8-fin | frame | new doc has M(d)=∅ (vacuous) | frame | precondition: positivity, depth, finite extension | precondition: positivity, depth m_L(d), finite | restriction preserves all three | see *S8a, S8-depth, S8-fin* prose below | frame |
| S8★ | frame | new doc M(d)=∅ (vacuous) | frame | per-subspace projection via ASN-0036's S8 on M(d')\|_{V_{s_C}(d')} | trivial length-1 decomposition on M(d')\|_{V_{s_L}(d')} | s_C: per-subspace projection via ASN-0036's S8 on contracted M'(d)\|_{V_{s_C}(d)}; s_L: trivial length-1 fall-back on survivors; see *S8★* prose below | see *S8★* prose below | frame |
| D-CTG★ / D-MIN★ | frame | frame | frame | precondition discharge (K.μ⁺'s original precondition list, in *Elementary transitions*, requires the resulting M'(d) to satisfy D-CTG / D-MIN — strengthened to D-CTG★ / D-MIN★ in the extended state: content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | constructive precondition discharge — the per-subspace retention count `n'_S` directly produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` which satisfies D-CTG★ and D-MIN★ at Σ' by construction | see *D-CTG★ / D-MIN★* prose below | frame |
...
| L1c | frame | frame | structural inc-chain established per K.λ allocation (first via SubAllocatorBundle, subsequent via TA5(c)); preserved by L12 | frame | frame | frame | frame | frame |
...
**Derived distinctness corollaries.** Two distinctness properties follow as corollaries of allocator discipline (GlobalUniqueness (ASN-0034) / NodeBaptism) at the single transition that touches the relevant store; each is discharged in one step below.

*Entity distinctness (derived; distinct K.δ events produce distinct entity addresses).* Only K.δ touches `E`; every other transition holds E in frame and preserves entity-distinctness trivially. For K.δ on non-node entities, the freshness guard `e ∉ E` and its GlobalUniqueness distinctness preservation are the case-(ii) preconditions discharged at the K.δ definition; for K.δ on nodes, NodeBaptism (a) supplies `e ∉ E` directly. *Cross-document distinctness for K.δ documents* (parent(d₁) ≠ parent(d₂)): we apply T10 (PartitionIndependence, ASN-0034) directly at the account level with `p₁ = A₁, p₂ = A₂`. The non-nesting precondition `A₁ ⋠ A₂ ∧ A₂ ⋠ A₁` follows from T10a's discipline on accounts as a binary case-split: two distinct accounts emitted by the same account sub-allocator (siblings under a common parent node) are prefix-incomparable by T10a.2 (NonNestingSiblingPrefixes); two distinct accounts emitted by different account sub-allocators (under distinct parent nodes `N₁ ≠ N₂`) are prefix-incomparable by T10a.5 (CrossAllocatorIncomparability), whose non-ancestor-descendant precondition we discharge by **(CrossNodeAccountBase)**: the two account sub-allocator bases are prefix-incomparable even under node nesting. Each account sub-allocator of a node `N` is based at `b_account(N) = [N.0.1]` (the `inc(N, 2)` descent, `zeros = 1`, account-level — *Sub-allocator names*). We show `b_account(N₁) ⋠ b_account(N₂) ∧ b_account(N₂) ⋠ b_account(N₁)` for any `N₁ ≠ N₂`. If neither node is a prefix of the other, they diverge at some position `k ≤ min(#N₁, #N₂)` and both bases inherit that divergence at `k`, so neither is a prefix of the other. If one node nests in the other — say `N₁ ≼ N₂` with `#N₁ < #N₂`, the case NodeBaptism does *not* forbid, since multi-component node tumblers are T4-legal (`zeros = 0`, `t₁ ≠ 0`, `t_{#t} ≠ 0`, so e.g. `N₁ = [1,2] ≼ [1,2,3] = N₂`) — then `b_account(N₁)` carries the field-separating zero at position `#N₁ + 1`, whereas `b_account(N₂)` carries `(N₂)_{#N₁+1}` there, which is nonzero because `N₂` is a node (`zeros(N₂) = 0`, so every component is nonzero). The two bases thus diverge at position `#N₁ + 1`, and since `#b_account(N₂) > #b_account(N₁)` neither is a prefix of the other. Either way the bases are prefix-incomparable, so the two account sub-allocators are non-ancestor-descendant (their domains extend their respective bases by prefix, per ChainPrefixExtension-style TA5(b)/(c)), discharging T10a.5's precondition; T10a.5 then yields `A₁ ⋠ A₂ ∧ A₂ ⋠ A₁`. With `A₁ ⋠ A₂ ∧ A₂ ⋠ A₁` established, T10 yields every address extending `A₁` differs from every address extending `A₂`. Every document `[A₁.0.k]` minted under `A₁` by its document sub-allocator `A_doc(A₁)` extends `A₁` (since `A₁ ≼ [A₁.0.k]` for every `k ≥ 1` by the emission rule `inc(A, 2)` for the first emission and `inc(·, 0)` for subsequent siblings, both of which preserve the parent's prefix per TA5(b)/(c)); symmetrically for `A₂`. T10 therefore yields `d₁ ≠ d₂` for documents minted under each account, across all sibling positions `k, k' ≥ 1` — not only the first emissions. Within a single parent account, two distinct documents need not inhabit the same sub-allocator chain: a direct document `[A.0.1]` lies on `A_doc(A)`, whereas a version `[A.0.1.1]` lies on `A_v([A.0.1])`, yet both have parent account `A` (a version preserves `parent(d_new) = parent(d_src)` by K.δ-ID.parent-0/1). Distinctness across such cross-chain same-parent pairs is discharged by plain GlobalUniqueness (ASN-0034) across distinct K.δ allocation events — which covers cross-chain pairs without requiring co-residence on one chain — or, equivalently for the document/version pair, by T10a.6 (DomainDisjointness) between `A_doc(A)` and the relevant `A_v(·)`. The single-chain sub-case (two siblings on the same `A_doc(A)`) is the special case where GlobalUniqueness reduces to within-chain enumeration injectivity.
...
*S7d (Document allocation discipline).* Every `d ∈ E_doc` is T4-valid with `zeros(d) = 2`, placed in E_doc by a K.δ event on the parent allocator's tracked domain. Documents enter `E_doc` by three K.δ routes, each discharging `zeros(d) = 2` via the relevant K.δ-ID.zeros identity and freshness via the corresponding discharge:
...
- *k = 0 (sibling off a document, `zeros = 2` preserved).* The operand `t` is a document (`zeros(t) = 2`); K.δ-ID.zeros-0/1 preserves `zeros(d) = zeros(t) = 2`. Freshness is the caller-checked guard `inc(t, 0) ∉ E`, characterized by FrontierEquivalence.

Preserved by P1.

*K.μ~ discharge for the arrangement-shape package (uniform argument).* The shape invariants — S8a, S8-depth, S8★, D-CTG★, D-MIN★, D-SEQ★ — share one K.μ~ discharge: each is stipulated on `M'(d)` by K.μ~ admissibility (i), and the K.μ⁻ (full-clearance) + K.μ⁺ decomposition mechanically realises it — K.μ⁻ restriction preserves it on the link-subspace survivors, and K.μ⁺'s preconditions re-establish it on the rebuilt content arrangement. Each per-property block below adds only its genuine delta over this shared discharge.

*S8a, S8-depth, S8-fin.* Established at arrangement-extending transitions: K.μ⁺ amendment's preconditions on new V-positions (positivity, depth, finiteness preserved by finite extension); K.μ⁺_L preconditions (positivity, fixed per-document depth `m_L(d)`, finite extension); K.μ⁻ restriction preserves all three; all others hold M in frame. *K.μ~:* S8a and S8-depth carry no per-property delta — the uniform shape-package discharge above closes them in full. The sole delta is S8-fin(Σ'), discharged independently of admissibility (i) and K.μ~-FIX: K.μ⁻ restricts dom(M(d)) (a subset of a finite set is finite) and K.μ⁺ adds finitely many positions (finite + finite = finite).

*S8★ (Per-subspace span decomposition).* Established per-subspace by the two routes specified at S8★'s definition (content subspace via ASN-0036's S8 on the projection `M(d')|_{V_{s_C}(d')} : V_{s_C}(d') → dom(C')`; link subspace via the trivial length-1 decomposition on `M(d')|_{V_{s_L}(d')}`). The constituent invariants S2/S7b/C1b/S8a/S8-depth/S8-fin are elementary-preserved as above. Each arrangement-extending or contracting transition discharges S8★ at its post-state by these per-subspace projections; others hold M in frame. *K.μ⁻:* under a K.μ⁻ contraction, S8★ is discharged per subspace by the same two routes, applied to the contracted projections. *Content subspace:* S8★(s_C) — which requires condition (c), uniqueness of the maximal-run decomposition — is re-established by reapplying ASN-0036's S8 to the contracted content-subspace projection `M'(d)|_{V_{s_C}(d)}`; its preconditions (S2, S3★, S8a, S8-depth, S8-fin) are preserved by restriction, as the rest of the matrix shows, and the trivial length-1 fall-back is *not* used here, since it would establish only a run-cover (conditions (a) and (b)) and not condition (c). *Link subspace:* S8★(s_L) omits (c) by definition, so its discharge reuses the trivial length-1 decomposition — every surviving V-position constituting its own length-1 run — which is always available after contraction. *K.μ~:* by the uniform shape-package discharge above, with the two routes split per S8★'s structure — the link subspace by fixity (the trivial length-1 decomposition preserved pointwise, `M'(d)|_{dom_L} = M(d)|_{dom_L}` from Steps 1–3 of the link-subspace fixity proof), the content subspace by the K.μ⁺ rebuild (ASN-0036's S8 on the rebuilt content-subspace projection).

*D-CTG★ / D-MIN★.* K.μ⁺'s original precondition list (stated at the K.μ⁺ definition in *Elementary transitions*) requires the resulting `M'(d)` to satisfy D-CTG and D-MIN, strengthened to D-CTG★ / D-MIN★ in the extended state — contiguity and minimum-position preservation on the extended content subspace is therefore discharged by precondition, not by the K.μ⁺ amendment (which adds only the `subspace(v) = s_C` restriction). K.μ⁺_L preconditions cover the D-CTG case (non-empty link subspace) and the D-MIN case (empty); K.μ⁻'s constructive precondition specifies the per-subspace retention count `n'_S` and produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` directly, satisfying D-CTG★ and D-MIN★ at Σ' by construction; all others hold M in frame. *K.μ~:* by the uniform shape-package discharge above (admissibility (i) stipulates; the full-clearance decomposition realises — link subspace retained pointwise from Σ, content subspace empty after clearance then rebuilt by K.μ⁺ preconditions at Σ').

*D-SEQ★.* Derived at each reachable state from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a per the D-SEQ★ definition paragraph in the *Amendments to existing transitions* section above. D-SEQ★ at each reachable state follows by the same derivation applied at that state. *K.μ~:* derived at Σ' from its constituents D-CTG★ + D-MIN★ + S8-depth + S8a (by the uniform shape-package discharge above) and S8-fin(Σ') (per the S8-fin delta above), then the standard derivation.
...
**Class (b): Composite-boundary properties**, discharged at composite boundaries by the J0/J1★/J1'★ couplings of ValidComposite★. These are: P4★, P4a, and P7a.
...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | By the induction-along-the-witnessing-trace mechanism of the P4a definition box. | Transiently fails if K.ρ precedes the matching K.μ⁺ within the composite (restoration per the definition box). |
...
P4★ (`Contains_C(Σ) ⊆ R`): For each `(a, d) ∈ Contains_C(Σ') \ Contains_C(Σ)`, J1★ at the composite boundary requires `(a, d) ∈ R'`. K.α, K.δ, K.ρ hold M in frame; K.μ⁺_L adds only link-subspace V-positions (excluded from Contains_C); K.μ⁻ can only shrink Contains_C; K.μ~ preserves Contains_C exactly — its content-subspace range is unchanged. K.μ~ does *not* preserve per-position values on dom_C (reassigning which V-position carries which I-address is the whole point of reordering, `M'(d)(π(v)) = M(d)(v)`); what it preserves is the *range* — the set of content I-addresses. Because π bijects dom(M(d)) onto dom(M'(d)) carrying dom_L onto dom_L (link-subspace fixity, established in the *Decomposition of K.μ~* section), it carries the complement dom_C onto dom_C, so applying the bijection equation `M'(d)(π(v)) = M(d)(v)` over the content-subspace positions gives the range equality `{a : (E u ∈ dom_C(M'(d)) : M'(d)(u) = a)} = {a : (E v ∈ dom_C(M(d)) : M(d)(v) = a)}` and hence `Contains_C(Σ') = Contains_C(Σ)`. Only K.μ⁺ may transiently violate P4★ (it adds a new content-subspace containment to Contains_C while framing R); J1★ supplies the co-occurring K.ρ that places `(a, d) ∈ R'`, restoring the bound at the composite boundary.
...
| J4 | Fork composite: K.δ + K.μ⁺ + K.ρ (no other steps); content source is the K.δ operand d_op (d_src for k=1, prev_version for k=0); precondition V_{s_C}(d_op) ≠ ∅; range bound ran(M'(d_new)) ⊆ ran(M(d_op)|_{V_{s_C}(d_op)}); dom(C') = dom(C) follows from frames; provenance from J1★/J1'★ (extended-state couplings); content-subspace-empty source is ex nihilo (K.δ), not fork |
...
The four structural identities `K.δ-ID.zeros-0/1`, `K.δ-ID.zeros-2`, `K.δ-ID.parent-0/1`, and `K.δ-ID.parent-2` are stated and derived inline at *Elementary transitions*, K.δ case (ii); that catalogue is their single source.
...
|-------|-----------|--------------------|
| SequentialTransitionAxiom | Axiom: transitions `Σ → Σ'` are atomic, uninterruptible, and totally ordered (full statement in §The state model). | ASN-0093 (SequentialAtomicTransitions) |
...
| L14a | Inapplicable in the extended state: L14a's `s_C`-resident hypothesis (L0a) is unmet, since K.μ⁺_L maps link-subspace V-positions into dom(L); where such mappings occur, S3★ routes them to dom(L) and CL-OWN forces home-document ownership | ASN-0043's L14a (NonTranscludability — "In any `s_C`-resident system (L0a): `(A d, v : v ∈ dom(Σ.M(d)) : Σ.M(d)(v) ∉ dom(Σ.L))`"). The extended state is not `s_C`-resident, so L14a's precondition fails and the invariant does not apply — paralleling the S3/S3★ treatment rather than negating a foundation invariant |
...
| P4★ | `Contains_C(Σ) ⊆ R` — provenance bounds scoped to the content subspace | This ASN's own provenance bound, scoped to the content subspace so it coexists with P7 once link-subspace mappings exist |
