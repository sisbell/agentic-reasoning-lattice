...
- V-position v_ℓ satisfies:
  - subspace(v_ℓ) = s_L
  - depth m_L(d), the current link-subspace depth governed by the live-depth re-pinning rule stated under *V-position depth (operational)*; when V_{s_L}(d) = ∅ this insertion re-pins m_L(d) at any value ≥ 2 (S8a).
...
**Framing: admissibility is a filter; Step (B) is non-vacuity.** Two distinct roles run through this section, and keeping them apart removes an apparent circularity between Step (A) and Step (B.3). *Admissibility is a filter.* The admissibility clauses (i)–(iii) below restrict *which* bijections π the operation considers; clause (i) in particular treats `S3★(Σ')` — post-state generalized referential integrity — as a *hypothesis on the candidate π*, not as a consequence of the preconditions. Every argument phrased "by the admissibility filter" or "admissibility-stipulated `S3★(Σ')`" is consuming this hypothesis on the π in hand; Step (A) is one such consumer, deriving subspace preservation from the hypothesised `S3★(Σ')` together with `S3★(Σ)` and L14. *Step (B) establishes non-vacuity.* Separately, the K.μ⁻ + K.μ⁺ full-clearance decomposition is shown to *realise* an admissible π and to produce a post-state that genuinely satisfies `S3★(Σ')` — so the filter is non-empty and the operation is not vacuous. There is no circularity: Step (A) reasons *within* the filter (assuming the hypothesis), while Step (B) discharges the *existence* obligation (exhibiting a π and a post-state meeting the hypothesis). The *Necessity and sufficiency* argument closes the loop — necessity reasons over every admissible π via Steps (A)/(C)/(D); sufficiency exhibits the transposition witness realised by the full-clearance decomposition. The ExtendedReachableStateInvariants `S3★` matrix entry for K.μ~ is exactly this filter stipulation, discharged constructively at Step (B.3). All cross-references below cite this *Framing* rather than re-deriving the distinction.
