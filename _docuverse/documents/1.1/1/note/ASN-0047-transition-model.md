...
A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. The operand-admissibility conjuncts, freshness mechanism, and structural identities are the contract of the K.δ definition's case (ii) and are not restated here; this section adds only the *parent-allocator activation* (which allocator each step acts on), the spawn-admissibility conjuncts, and, for the child-spawn regimes, the spawnPt premise — keyed to case (ii)'s k-labels. By k:
