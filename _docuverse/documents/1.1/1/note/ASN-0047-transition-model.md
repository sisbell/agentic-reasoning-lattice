...
**LRP (full-clearance preserves the link subspace pointwise).** A direct consequence of the full-clearance form, isolated here as a single named fact so it is proved once and cited rather than re-derived: K.μ⁻'s content-only clearance leaves every link-subspace V-position in place with its value, and K.μ⁺ writes only content-subspace V-positions (its content-subspace amendment), so no link-subspace position is added, removed, or altered. Hence
...
**K.μ~-RANGE (range-invariance).** `ran(M'(d)) = ran(M(d))`, and consequently `Contains_C(Σ') = Contains_C(Σ)` and `Contains(Σ') = Contains(Σ)`. *Proof.* By K.μ~-FIX, π is a bijection `dom(M(d)) → dom(M'(d))` with `dom(M'(d)) = dom(M(d))`, and the bijection equation `M'(d)(π(v)) = M(d)(v)` holds for every `v ∈ dom(M(d))`; as π ranges over the full domain, the image set `{M'(d)(π(v)) : v} = {M(d)(v) : v}` gives `ran(M'(d)) = ran(M(d))`. Subspace preservation (Step (A)) likewise preserves the per-subspace ranges, so the content-subspace range is unchanged. K.μ~ frames `C`, `E`, and every arrangement other than `d`'s (Frame, below), so the document-content containment sets `Contains_C` and `Contains` are unchanged at the boundary. ∎

**Link-subspace fixity and realisation.** Clause (v) supplies pointwise fixity `π(v) = v` for every `v ∈ dom_L(M(d))` on the admissible class directly, as a hypothesis on that class. The functional and domain identities the full-clearance decomposition contributes — `dom_L(M'(d)) = dom_L(M(d))` and `M'(d)|_{dom_L} = M(d)|_{dom_L}` — are exactly **LRP** (stated and proved once above), and the sub-step below shows this functional identity carries CL-UNIQ to the post-state Σ' and confirms the realised π agrees with clause (v) pointwise (premise: CL-UNIQ at the pre-state Σ). Proof:

(4) *Identity via CL-UNIQ at the pre-state.* By LRP, `M'(d)|_{dom_L} = M(d)|_{dom_L}`, so for the V-position `v ∈ dom_L(M(d))` under consideration, `M(d)(v) = ℓ`. Subspace preservation places `π(v) ∈ dom_L(M(d))` (using LRP's domain identity `dom_L(M'(d)) = dom_L(M(d))`), and the bijection equation gives `M(d)(π(v)) = M'(d)(π(v)) = M(d)(v) = ℓ` (the first equality by LRP applied at `π(v) ∈ dom_L`). Both `v` and `π(v)` are link-subspace V-positions in `dom(M(d))` mapping to the same link `ℓ`. CL-UNIQ at Σ — link-subspace injectivity of `M(d)|_{dom_L}`, supplied as the ExtendedReachableStateInvariants per-state hypothesis at the reachable pre-state Σ — forces `π(v) = v`. LRP's functional identity also gives post-state CL-UNIQ preservation directly, without passing through the pointwise identity: equal functions share injectivity profiles, so `M(d)|_{dom_L}` injective (CL-UNIQ at Σ) together with `M'(d)|_{dom_L} = M(d)|_{dom_L}` gives `M'(d)|_{dom_L}` injective, i.e., CL-UNIQ at Σ'. ∎
...
By **K.μ~-RANGE** (range-invariance), Contains(Σ') = Contains(Σ). All invariants are trivially maintained; no co-occurring transition is needed.
...
**Step 3: K.μ~ — reorder text, verify link fixity.** Swap text: `π([1,1]) = [1,2]`, `π([1,2]) = [1,1]`. Clause (v) link-subspace fixity (realised via **LRP** + CL-UNIQ, sub-step (4) of *Link-subspace fixity and realisation*) forces `π([2,1]) = [2,1]`.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | elementary decomposition K.μ⁻+K.μ⁺ (this row's restriction + amendment Class (a) cells); realisable π are exactly the admissible ones — subspace-preserving (K.μ⁺ content-subspace precondition) and link-subspace fixing (clause (v)); the realisation establishes S3★(Σ') by **K.μ~-S3★** |  frame |
