...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | by admissibility (i) (carried on π); realisability and non-vacuity in §*Decomposition of K.μ~* |  frame |
...
| P4a (every R-entry witnessed by content-subspace containment in some trace state) | Two routes: for `R' \ R`, J1'★ supplies a content-subspace witness `v` with `M'(d)(v) = a` at Σ' itself; for the carry-forward `R`-slice (including stale entries with no Σ'-witness), the IH supplies a trace-state witness Σ_k propagated into R' by P2. | Transiently fails if K.ρ precedes the matching K.μ⁺ within the composite; the `R' \ R` entry is restored at Σ', which carries its witness. |
