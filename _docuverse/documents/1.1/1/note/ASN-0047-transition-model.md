...
- *Admissible removal pattern (per-subspace suffix or full clearance).* For each subspace S, the removed positions in `V_S(d)` form either a suffix of `V_S(d)` under the D-SEQ★-shaped enumeration or all of `V_S(d)`. The shape is the per-state D-SEQ★ invariant stated in *Per-state arrangement shape (D-SEQ★)* immediately above and derived in full below: `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` for some `n_S ≥ 1`, with inner positions of uniform depth m_S. Under that pre-state shape, there exists `0 ≤ n'_S ≤ n_S` per subspace such that the post-state subspace satisfies `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` — partial suffix removal when `1 ≤ n'_S < n_S`, full clearance when `n'_S = 0`, and no change in S when `n'_S = n_S`. The per-subspace patterns are independent: a composite K.μ⁻ may mix suffix removal in one subspace with full clearance in another and no change in a third, provided at least one subspace contracts strictly so the effect-clause requirement `dom(M'(d)) ⊂ dom(M(d))` is satisfied at the whole-arrangement level.
...
**Link-subspace fixity under K.μ~.** The K.μ~ contract (stated above) drops the link-subspace identity clause `π(v) = v` for v link-subspace from the admissibility constraints, treating it as a derived property rather than a stated precondition. We discharge the derivation here, *before* the decomposition cases below, so that Case 2 of the decomposition can cite the result without forward reference: any candidate transition satisfying only the *weaker* subspace-preservation clause on π is forced into link-subspace identity under S3★ + the K.μ⁺ amendment + CL-UNIQ. The link-subspace identity is therefore a *consequence* of the surrounding invariants, recoverable at every reachable state where K.μ~ is invoked.
