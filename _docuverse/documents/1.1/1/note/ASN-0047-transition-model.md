...
The admissibility of any particular contraction (which subset of dom(M(d)) may be removed) is *not* stated as a precondition. It is recovered from the post-state requirement that M'(d) satisfy D-CTG★ and D-MIN★ (postconditions inherited from the amendments below), combined with the D-SEQ★-shaped pre-state guaranteed by induction over the elementary set. The case analysis that follows is a *verification* — it works out which per-subspace removal patterns are compatible with D-CTG★/D-MIN★ at the post-state — not a derivation of additional precondition content.
...
Together, (a) is compatible with the D-CTG★/D-MIN★ postconditions, (b) and (c) are not, and any admissible per-subspace pattern is therefore suffix removal or full clearance. The per-subspace clauses are independent: a composite K.μ⁻ may mix patterns across subspaces (e.g., suffix in one S, full clearance in another). The case analysis is exhaustive across the D-SEQ★-shaped pre-state, so the postconditions D-CTG★/D-MIN★ implicitly constrain K.μ⁻ to (a)-class behaviour without requiring a separate admissibility precondition.
...
**Link-subspace fixity under K.μ~.** The K.μ~ definition (Findings 3/4 above) drops the link-subspace identity clause `π(v) = v` for v link-subspace from the precondition catalogue, treating it as a derived property rather than a stated precondition. We discharge the derivation here: any candidate transition satisfying only the *weaker* subspace-preservation clause on π is forced into link-subspace identity under S3★ + the K.μ⁺ amendment + CL-UNIQ. The link-subspace identity is therefore a *consequence* of the surrounding invariants, recoverable at every reachable state where K.μ~ is invoked.
