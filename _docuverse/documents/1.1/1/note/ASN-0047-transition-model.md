...
ASN-0036 established two components of system state — a permanent content store C and mutable document arrangements M(d) — and proved content, once stored, is immutable (S0). That arrangement mutation never reaches the stored content — the separation this ASN generalises to the full extended state in *Destruction confinement* — is the property we extend here. These are properties of the invariants. We have not yet classified the transitions. In what primitive ways can the state change, and what must each change preserve?
...
**NodeBaptism (Axiom, boundary input — node provisioning).** Node addresses are not minted by any docuverse transition. A node is a *ghost element* (Nelson, LM 4/23) — it occupies a position on the tumbler line but stores no docuverse state, and the operation set provides no `inc`-rule producing a node. Bringing a node into existence is a network-provisioning act (Nelson's BEBE inter-server protocol, LM 4/70) performed at the boundary of the docuverse transition model rather than within it. This single axiom records, over `Σ` alone, what the provisioning boundary commits at every K.δ node-allocation event — every elementary K.δ transition placing an entity `e` with `IsNode(e)` into E:

- (a) *Freshness:* `e ∉ Σ.E` at the state Σ of allocation;
- (b) *Bootstrap lineage:* `n₀ ≼ e` under the tumbler-prefix order.

The bootstrap node `n₀ ∈ E₀` is itself the boundary baptism performed at `Σ₀`. As a base for downstream allocation, a baptised node `t ∈ Σ.E_node` serves directly as the base of its account sub-allocator `A_account(t)`: the boundary fact `t ∈ Σ.E_node` is exactly the commitment discharging the spawnPt premise for the `k' = 2` step that creates `A_account(t)` off `t` — the role a docuverse-internal parent allocator plays for non-node parents. No state component beyond `E_node`, and no external registry, is consulted; node provenance is the boundary membership `t ∈ Σ.E_node` together with (a)–(b).
...
The witnessing existential now ranges over the finite set `{Σ₀, ..., Σ_n}` of trace states — the previously-undefined "transition history" reference — so the property is well-typed as a trace property even though it is not well-typed as a per-state invariant. The content-subspace qualification is essential: J1'★ scopes provenance recording to content-subspace range changes (link-subspace mappings target `dom(L)`, which is disjoint from `dom(C)` by L14, so no link-subspace V-position can witness provenance under P7's `a ∈ dom(C)` requirement). P4a therefore reads as "every provenance entry corresponds to a content-subspace arrangement at its recording boundary," consistent with both P7's grounding in `dom(C)` and J1'★'s content-scoped coupling.
...
| NodeBaptism | Axiom (boundary input — node provisioning): node addresses are baptised at the network-provisioning boundary, not by any docuverse transition; every K.δ node-allocation event commits, over Σ alone, (a) e ∉ Σ.E (freshness) and (b) n₀ ≼ e (bootstrap lineage). A baptised node `t ∈ Σ.E_node` is the base of its account sub-allocator `A_account(t)`, the membership `t ∈ Σ.E_node` discharging the k'=2 spawnPt premise; n₀ ∈ E₀ is the boundary baptism of Σ₀ |
