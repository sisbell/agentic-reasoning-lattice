...
**Sub-allocator activation (SubAllocatorBundle).** For each `d ∈ E_doc`, the entity-allocation event placing d into E_doc activates a content sub-allocator `A_C(d)` with anchor `b_C(d) = [d.0.s_C]` and a link sub-allocator `A_L(d)` with anchor `b_L(d) = [d.0.s_L]`, the anchors constructed as in *Allocator hierarchy under documents* above. The standing properties of these chains are foundation facts, cited at their use sites: each chain is a T10a-conforming `inc(·, 0)` sibling-advance allocator with strictly increasing enumeration (ChainDiscipline, ChainEnumerationInjectivity, ASN-0093); its first emission is the determinate tumbler `[d.0.s_C.1]` (resp. `[d.0.s_L.1]`), with `origin = d`, `#E = 2`, `zeros = 3`, T4-valid (FirstEmission, ASN-0093), and fresh against `dom(Σ.C) ∪ dom(Σ.L)` at the allocating state (FirstEmissionFreshness, ASN-0093); every chain element is T4-valid with `zeros(·) = 3` (ChainElementT4Validity, ASN-0093, with `zeros = 3` propagated by ChainPrefixExtension) and inhabits its subspace — `subspace_I(·) = s_C` along `A_C(d)`, `s_L` along `A_L(d)` (DisjointSubAllocatorChains, ASN-0093).

The one fact not supplied directly by a foundation lemma is the cross-subspace disjointness delta. `dom(A_C(d)) ∩ dom(A_L(d)) = ∅`, and for any d ≠ d', `dom(A_C(d)) ∩ dom(A_C(d')) = ∅`, `dom(A_L(d)) ∩ dom(A_L(d')) = ∅`, `dom(A_C(d)) ∩ dom(A_L(d')) = ∅`. The within-document disjointness is DisjointSubAllocatorChains (ASN-0093) directly; the cross-document same-subspace clauses are CrossDocumentDisjointness (ASN-0093) at anchor pairs `(b_C(d), b_C(d'))` and `(b_L(d), b_L(d'))`. The cross-subspace fourth clause `dom(A_C(d)) ∩ dom(A_L(d')) = ∅` (anchors `[d.0.s_C]`, `[d'.0.s_L]`) is the genuine delta, dispatched by CrossDocDisjoint (below) at `(s₁, s₂) = (s_C, s_L)`.
