...
*Scope and base-liveness.* K.δ as stated above admits, for k = 1 events from a document address, an inc operand `t` that need not be in E_doc — i.e., the version base `[N, 0, U, 0, D]` may be a ghost document (a valid document address that was never allocated, or whose entity record is otherwise absent from E). This is the abstract-spec reflection of Nelson's ghost-element doctrine (LM 4/23: "no specific element need be stored in tumbler-space to correspond to them"), which is general — Nelson explicitly extends it to documents — and version addressing inherits it (LM 4/29; LM 2/19). The weaker precondition is invariant-safe by a routine frame argument: K.δ frames every state component except E (and `M(e) = ∅` for documents), so every per-state invariant beyond the entity layer holds by frame at the K.δ step. The entity-hierarchy spine P8 is the only invariant that could touch the new entity's relationship to t, and it is preserved through `parent(·)` rather than through the inc operand — the version step `[N,0,U,0,D] → [N,0,U,0,D,k]` crosses no zero separator, so `parent(e) = parent(t)`, and `parent(e) ∈ E` is itself a K.δ precondition (independent of whether t is in E). Any subsequent K.α + K.μ⁺ + K.ρ activity under the new document anchors through `e` itself, never through t. The companion question of intermediate-version liveness — whether `[N,0,U,0,D,k]` with k ≥ 2 (allocated as an inc-k=0 sibling of the prior version) requires `[N,0,U,0,D,k−1] ∈ E_doc` — is decided affirmatively as an immediate consequence of the k = 0 sub-case's `t ∈ E` requirement: every k ≥ 2 version event proceeds via k = 0 sibling allocation with the prior version as inc operand, which must therefore be in E. Thus chains of versions allocated within this ASN's transition history exhibit live intermediates by construction; the relaxation applies only to the *initial* version step, where the version base may be a ghost.

*Discharge of `e ∉ E` in the ghost-operand case.* When `t ∉ E_doc` at k = 1, T10a's GlobalUniqueness does not underwrite `inc(t, 1) ∉ E` directly: T10a's freshness guarantee is conditional on the operand inhabiting some T10a allocator's domain (the conclusion is that the resulting address is distinct from every prior allocation *within that domain*), but a ghost operand by stipulation lies outside every entity allocator's domain. The freshness premise `e ∉ E` is therefore the operative *K.δ precondition* in this sub-case rather than a T10a consequence — the caller of K.δ at the k = 1 ghost-operand sub-case must verify `inc(t, 1) ∉ E` directly against the current entity set, by inspection of E rather than by appeal to T10a. TA5 (ASN-0034) supplies the structural fact that `inc(t, 1) = t.1` is deterministically determined by t (TA5(b) preserves the prefix `1..#t`, TA5(d) at k = 1 appends `.1` with no intervening zero separator), so the candidate address is exhibitable from t alone, and the precondition reduces to checking that this exhibited address is not already in E. This discharge pattern parallels NodeUniqueAllocation for case (i): in both, an external uniqueness premise — NodeUniqueAllocation for nodes, the K.δ precondition for ghost-operand k = 1 — underwrites `e ∉ E` at K.δ entry in regions where T10a's GlobalUniqueness does not reach. The three discharge paths (T10a for live-operand case (ii); K.δ precondition + TA5 for ghost-operand k = 1; NodeUniqueAllocation for case (i)) close the same `e ∉ E` chain through different premises, and no path silently substitutes for another.

*Relation to the foundation's ghost-element doctrine and the implementation contract.* The frame argument above — K.δ frames every state component except E (and sets `M(e) = ∅` for documents), and the entity-hierarchy spine P8 is preserved through parent-spine independence (`parent(e) = parent(t)` at k = 1 with no zero separator crossed, and `parent(e) ∈ E` is itself a K.δ precondition independent of `t ∈ E`) rather than through the inc operand — establishes that omitting `t ∈ E_doc` for k = 1 is invariant-safe at the level of K.δ itself. Every per-state invariant beyond the entity layer (S0–S9, L0–L14, J0–J4 and their starred forms) is preserved by frame on its state component; the entity-layer invariants P0–P8 are preserved by frame on every component K.δ does not touch, plus P8's parent-spine discharge for the one component (E) it does. Detailed per-invariant verification on a concrete state is exhibited in the *Worked example: ghost-base document versioning* below, where each step enumerates the check on E, M, C, L, R against the named invariant set (P8 by parent chain, NodeUniqueAllocation and NodeLineage vacuously on non-node entities, S0–S9 and L0–L14 by frame, J0–J4 vacuously at K.δ with empty arrangement). Three positions were available — (a) identify an invariant that demands the strengthening (none was found), (b) relax to the weaker precondition and admit ghost-base versioning at the abstract level, or (c) adopt the stronger precondition as a deliberate scope decision. The present ASN takes position (b), on the grounds that the abstract specification should constrain implementations rather than inherit implementation choices that have no invariant-level grounding. Consultation evidence (consultation-54, answer-05) underwrites this choice from two directions: Nelson confirms the ghost-element doctrine is general — "documents are explicitly named, not just servers and accounts" — and version addressing places "no requirement that an immediate parent be a live entity"; Gregory confirms that `docreatenewversion` does *not* enforce a bert-style liveness check on the source (it uses `NOBERTREQUIRED` and falls through to `fetchorglgr`, which checks only that the source ISA is a structurally valid granfilade leaf, not that the document is in the open list), and further documents an ordering bug (the version ISA is allocated at `do1.c:277` *before* the source check at `do1.c:281`, so a non-existent source causes an allocated-but-leaked granfilade slot). Inheriting the implementation's structural-existence check at the abstract layer would also inherit the leak as an admissible state; the abstract specification should constrain valid behavior, not document implementation defects. The implementation's source-liveness requirement is therefore a FEBE-layer concern, not an abstract invariant: a subsequent version-management ASN may introduce a versioning-specific lineage axiom that takes `t ∈ E_doc` as a hypothesis for *specific composite operations* (such as "version-with-content-copy"), but the elementary K.δ transition admits the bare ghost-base case.
...
- *Admissible removal pattern (per-subspace suffix or full clearance).* For each subspace S, the removed positions in `V_S(d)` form either a suffix of `V_S(d)` under the D-SEQ★-shaped enumeration or all of `V_S(d)`. Concretely: under the D-SEQ★-shaped pre-state `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` (guaranteed by induction over the elementary set; established at D-SEQ★'s definition site below), there exists `0 ≤ n'_S ≤ n_S` per subspace such that the post-state subspace satisfies `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` — partial suffix removal when `1 ≤ n'_S < n_S`, full clearance when `n'_S = 0`, and no change in S when `n'_S = n_S`. The per-subspace patterns are independent: a composite K.μ⁻ may mix suffix removal in one subspace with full clearance in another and no change in a third, provided at least one subspace contracts strictly so the effect-clause requirement `dom(M'(d)) ⊂ dom(M(d))` is satisfied at the whole-arrangement level.
...
**K.μ~ (Arrangement reordering, distinguished composite).** K.μ~ denotes a *distinguished composite* — a K.μ⁻ followed by a K.μ⁺ whose joint effect is a subspace-preserving permutation of `dom(M(d))` for some `d ∈ E_doc`. K.μ~ is reserved for this composite pattern so downstream invariant theorems can quantify over it uniformly. The formal contract (the bijection equation and admissibility constraints), the link-subspace fixity argument, and the frame are all derived from the K.μ⁻ + K.μ⁺ decomposition; their statement and discharge are consolidated in *Decomposition of K.μ~* below, placed after S3★-aux (*Generalized referential integrity*) and CL-UNIQ (*Link-subspace ownership*) on which the derivation depends. This presentation order avoids forward references: per-state invariants are inductively established without appeal to K.μ~'s contract, so the deferral is structurally non-circular. Wherever K.μ~ appears as a transition kind in the *Elementary transitions* section and its sequels — in composite-transition lemmas, in the structural-sufficiency claim, in worked examples, and in the Frame extension catalogue — the meaning is "the K.μ⁻ + K.μ⁺ composite admissible as a K.μ~ instance per the *Decomposition of K.μ~* contract below".
...
K.μ~ is a distinguished composite, not a primitive transition. The decomposition account — including the K.μ~-FIX domain-fixity argument and the degenerate-case analysis — is deferred to the dedicated *Decomposition of K.μ~* section below, placed after the per-state invariants S3★-aux (defined in *Generalized referential integrity*) and CL-UNIQ (defined in *Link-subspace ownership*) on which it depends. This presentation order avoids forward references; the per-state invariants are inductively established without appeal to the decomposition account or its corollaries, so the resulting placement is structurally non-circular.
...
*Known gap — interior link withdrawal in Nelson's tombstoning sense is not expressible.* The structural sufficiency claim above admits one *named* exception, separate from the general open-completeness caveat. Under the amended D-CTG★/D-MIN★ (which drop ASN-0036's link-subspace tombstoning exemption — see *D-CTG★ / D-MIN★* below), K.μ⁻ admits only per-subspace suffix removal and full clearance. Withdrawing a single link at a non-maximum link-subspace position while leaving subsequent links in place is *not* a K.μ⁻ contraction — it would leave a gap in the link subspace that violates D-CTG★. Nelson's tombstoning mechanism (LM 4/9), under which a withdrawn link transitions to "not currently addressable" status while retaining its position and permanent serial address, is the design's principal mechanism for selective link withdrawal; the consultation responses confirm it is essentially the only model Nelson contemplates for link withdrawal. The elementary set of this ASN cannot express it. Two consequences follow: (i) the structural sufficiency claim is bounded — it covers Nelson's *named* modification kinds (creation, allocation, arrangement extension and contraction, reordering, provenance recording) and their composites, but excludes tombstoning as a distinct modification kind; (ii) any tombstoning mechanism — status flag, tombstone marker, explicit retraction link, or per-link liveness predicate — must be supplied by a separate operator that operates outside K.μ⁻'s presentational-removal contract. The precise mechanism is deferred to the open question on link-withdrawal invariants below; this paragraph identifies the gap at the point where the structural-sufficiency claim is made, rather than reaching it as an inference from the D-CTG★/D-MIN★ consequence paragraph alone.
...
These anchors are *virtual allocator predecessors*: they are not themselves in `dom(C) ∪ dom(L)` (content addresses have `#E ≥ 2` by S7c; link addresses have `#E ≥ 2` by L1b), and they are *outside* T10a's per-owner inc tree rooted at `d` — they have no inc-history, no `inc(d, k)` step produces them, and they are not endpoints of any sequential T10a allocator chain emanating from `d`. The relationship to T10a is the following: T10a (ASN-0034) governs sequential allocation *within* a single ownership domain by inc-extension, and its at-most-once spawning constraint forbids the single allocator at `d` from spawning two distinct sibling sub-allocators via one inc operation. The anchors `b_C(d)` and `b_L(d)` are therefore admitted by a separate axiom (SubAllocatorAxiom below) — they sit at depth `[d.0.s_C]` and `[d.0.s_L]` respectively as *named entry points* fixed by document-creation convention, not as outputs of T10a's inc discipline applied to `d`. (The convention is Nelson's "subspaces by position-digit reservation" (LM 4/30–4/31) and Gregory's atomtype-prefix encoding (udanax-green, `xanadu.h:144`, `TEXTATOM=1`/`LINKATOM=2` as load-bearing constants); consultation-54 confirms that the content and link sub-allocators are siblings-by-convention under each document, structurally established at document-creation time rather than spawned by an inc event.) This is the same pattern as NodeUniqueAllocation: T10a's inc discipline does not by itself underwrite node baptism either, and a separate axiom supplies the uniqueness premise. Once each anchor is established, the sub-allocator it heads is *itself* a T10a-conforming allocator: subsequent allocations within that frontier are inc(·, 0) sibling steps and inherit T10a's machinery in full. So T10a applies to *each* sub-allocator independently from its first inc-produced output onward, and SubAllocatorAxiom supplies only the bootstrap — the namespace property at the first emission (`a ∉ dom(C)`, `ℓ ∉ dom(L) ∪ dom(C)`) — that T10a cannot itself produce from `d` alone. Successive allocations advance each anchor's frontier:
...
*Relationship to T10a's allocator tree.* The sub-allocator anchors `b_C(d)` and `b_L(d)` sit *outside* T10a's inc-history tree rooted at `d`: no `inc(d, k)` step produces either anchor, and T10a's at-most-once spawning constraint forbids `d` from emitting both anchors via a single inc operation. The anchors are not in any T10a allocator's domain — they are admitted by SubAllocatorAxiom alone as named entry points fixed by document-creation convention. Three properties of T10a remain in force downstream:
...
- *Cross-document disjointness.* The cross-document chain below is instantiated at the **document level** — invoked on `d₁, d₂` themselves, which are T10a-allocated under their owning accounts by S7d (ASN-0036). The operative sub-claims are T10a.2 (NonNestingSiblingPrefixes) when `d₁` and `d₂` share a parent account (same document sub-allocator, sibling outputs) and T10a.5 (CrossAllocatorIncomparability) when they belong to different parent accounts (distinct document sub-allocators not in ancestor-descendant relationship); T10a.6 (DomainDisjointness) is the packaging theorem combining both cases but is not the operative citation for prefix-incomparability. Neither sub-claim is invoked directly on sub-allocator anchors. The prefix incomparability `d₁ ⋠ d₂ ∧ d₂ ⋠ d₁` lifts to `b_*(d₁) ⋠ b_*(d₂) ∧ b_*(d₂) ⋠ b_*(d₁)` by suffix extension (a divergent stem remains divergent after appending suffixes), at which point T10 (PartitionIndependence) closes the chain for addresses extending the respective anchors.
...
This section provides the consolidated decomposition account for the distinguished composite K.μ~ introduced in *Elementary transitions* above. The placement here — after S3★-aux (*Generalized referential integrity*) and CL-UNIQ (*Link-subspace ownership*) — discharges the forward references previously flagged at K.μ~'s definition site: every per-state invariant cited below has already been established, so the argument proceeds without circularity. The contract (bijection equation, admissibility constraints, and frame) is stated first; link-subspace fixity is then derived from the contract plus the surrounding invariants; the decomposition cases follow.

**K.μ~ — formal contract.** A K.μ⁻ + K.μ⁺ composite qualifies as a *K.μ~ instance* exactly when the following derived contract is satisfied at the composite's pre- and post-states. Downstream proofs that appeal to K.μ~'s contract (the K.μ~-FIX domain-fixity argument; the link-subspace fixity argument; J3 isolation) discharge through the elementary K.μ⁻ + K.μ⁺ frames, not through a primitive K.μ~ frame.

*Derived contract (theorem from the decomposition).* For some d ∈ E_doc, there exists a bijection π : dom(M(d)) → dom(M'(d)) such that:

`(A v : v ∈ dom(M(d)) : M'(d)(π(v)) = M(d)(v))`

*Admissibility constraints (when a K.μ⁻ + K.μ⁺ composite qualifies as a K.μ~ instance).* The composite is invoked as a K.μ~ exactly when: `d ∈ E_doc`; π is subspace-preserving — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — so each V-position is permuted within its own subspace (a π violating this would yield M'(d)(π(v)) = M(d)(v) ∈ dom(L) at a content-subspace position, contradicting S3★); π produces V-positions satisfying S8a (all components strictly positive); and the resulting arrangement M'(d) satisfies S8-depth (uniform depth within each subspace), D-CTG (contiguity within each subspace), and D-MIN (minimum position in each non-empty subspace). The link-subspace identity property `(A v ∈ dom(M(d)) : subspace(v) = s_L : π(v) = v)` is *not* listed among the admissibility constraints: it is a *further* derived consequence of S3★ + the K.μ⁺ amendment + CL-UNIQ at the realising decomposition's output state (see *Link-subspace fixity under K.μ~* immediately below). Listing it as an admissibility constraint would duplicate a consequence already forced by the surrounding invariants.
...
**Link-subspace fixity under K.μ~.** The K.μ~ definition (in *Elementary transitions* above) drops the link-subspace identity clause `π(v) = v` for v link-subspace from the precondition catalogue, treating it as a derived property rather than a stated precondition. We discharge the derivation here, *before* the decomposition cases below, so that Case 2 of the decomposition can cite the result without forward reference: any candidate transition satisfying only the *weaker* subspace-preservation clause on π is forced into link-subspace identity under S3★ + the K.μ⁺ amendment + CL-UNIQ. The link-subspace identity is therefore a *consequence* of the surrounding invariants, recoverable at every reachable state where K.μ~ is invoked.
...
*Known gap: Nelson's tombstone-style link withdrawal is not expressible in the present transition set.* The sufficiency claim above covers each *admissible direction of change* recognised by this ASN's elementary set, but one specific modification kind that Nelson identifies — withdrawal of a single link at an interior link-subspace position while leaving subsequent links in place (LM 4/9: a withdrawn link transitions to "not currently addressable" status while retaining its permanent serial address and arrangement position) — is *not* realisable as any K.μ⁻ contraction or composite of the present elementary transitions. The K.μ⁻ amendment's D-CTG★/D-MIN★ postconditions restrict link-subspace contractions to suffix truncations or full clearance, and no other elementary transition deactivates an existing dom(L) entry or marks an arrangement position as inactive without removing it. The K.μ⁻-counterfactual worked example below (in the K.λ/K.μ⁺_L examples section) exhibits this gap concretely. Closing it would require a separate withdrawal mechanism — a status flag on dom(L), a tombstone marker on M(d), or an explicit retraction-link convention — none of which are part of the present five-component state model. This is a specific, known structural gap, distinct from the general open-completeness caveat above: the elementary set is structurally sufficient for *every modification kind expressible in the present state model*, but Nelson's tombstoning mechanism requires a state-model extension and is deferred to the open question on withdrawal invariants below.
...
*What is covered (the elementary set is structurally sufficient for these modification kinds).* The seven elementary transitions K.α, K.δ, K.λ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.ρ — together with the distinguished composite K.μ~ (K.μ⁻ + K.μ⁺) and the named composites J0/J1★/J2/J3/J4 — realise every modification kind in Nelson's design enumeration and Gregory's protocol-command catalogue:
...
Because K.μ~ is a distinguished composite (K.μ⁻ followed by K.μ⁺), we trace it through its intermediate state explicitly.
...
- **Foundation restatements** reproduce a foundation property verbatim or near-verbatim for self-contained reference; the formal content is owned by the cited foundation ASN.
...
| J3 | K.μ~ as distinguished composite requires no coupling: C' = C ∧ E' = E ∧ R' = R |
...
| K.μ~ | Arrangement reordering — distinguished composite (K.μ⁻ + K.μ⁺), bijection on V-positions preserving I-address multiset, D-CTG/D-MIN postcondition | ASN-0036 (notation coined here; semantics from the strand model's arrangement-reordering step) |
