...
- *Admissible removal pattern* (three clauses):

  (1) *Per-subspace suffix pattern.* For each subspace `S ∈ {s_C, s_L}` with `V_S(d) ≠ ∅`, there exists `0 ≤ n'_S ≤ n_S` such that the post-state subspace satisfies
  `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` —
  partial suffix removal when `1 ≤ n'_S < n_S`, full clearance when `n'_S = 0`, no change when `n'_S = n_S`. The full-clearance case is the maximal-suffix instance of the suffix pattern (at `n'_S = 0`), not a separate alternative. The per-subspace patterns are independent across `s_C` and `s_L`.

  (2) *Strict contraction.* `(E S ∈ {s_C, s_L} : V_S(d) ≠ ∅ : n'_S < n_S)` — at least one subspace shrinks strictly.

  (3) *Whole-arrangement effect clause.* Per-subspace independence under clause (1) would by itself admit the "no change in either subspace" assignment `n'_S = n_S` everywhere; clause (2) excludes it. Together clauses (1) and (2) deliver the whole-arrangement effect clause `dom(M'(d)) ⊂ dom(M(d))`: the per-subspace patterns of (1) supply `V_S(d') ⊆ V_S(d)` for each S, and the strict-contraction conjunct of (2) supplies at least one strict inclusion `V_S(d') ⊊ V_S(d)`, so the union `dom(M'(d)) = V_{s_C}(d') ∪ V_{s_L}(d')` is a proper subset of `dom(M(d)) = V_{s_C}(d) ∪ V_{s_L}(d)`.

  (The per-subspace enumeration `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}` with inner positions of uniform depth `m_S` is the D-SEQ★ shape established in *Amendments to existing transitions* below; the K.μ⁻ precondition stated here applies in the extended state where D-SEQ★ holds across both subspaces.)
...
- **Replacement at the maximum position of a subspace.** When the replaced V-position is `max(V_S(d))` for its subspace S, the M-touching pair is K.μ⁻ + K.μ⁺: K.μ⁻ removes that single position (a 1-element suffix of V_S(d)) and K.μ⁺ then re-adds it with the new value. *Co-occurring K.α and K.ρ when the new value is fresh content.* When the replacement value is freshly allocated content (the typical user-level editing operation), the full composite is K.α + K.μ⁻ + K.μ⁺ + K.ρ — four elementary steps: K.α allocates the new I-address (discharging K.μ⁺'s referential-integrity precondition), K.μ⁻ removes the old mapping, K.μ⁺ places the new mapping, and K.ρ records provenance for the freshly-introduced content-subspace range entry (required by J1★). When the replacement value is an *existing* I-address from `dom(C)` — Nelson's transclusion — the K.α and K.ρ steps drop out and the M-touching pair K.μ⁻ + K.μ⁺ alone suffices (no fresh content is allocated, and J1★ is vacuous at d whenever the transcluded I-address already lies in d's content-subspace range, or is restored by P2 carrying through any pre-existing `(a, d) ∈ R`).

- **Replacement at an interior position of a subspace.** When the replaced V-position is `[S, 1, ..., 1, k₀]` with `k₀ < n_S` (some positions above it remain), the D-CTG★ postcondition does *not* admit removing position k₀ alone — that would leave a gap above k₀ within the subspace, violating D-CTG★ at the intermediate state. Replacement at an interior position therefore decomposes the M-touching pair as: K.μ⁻ removes the suffix `{[S, 1, ..., 1, k] : k₀ ≤ k ≤ n_S}` (every position from k₀ to the maximum), and K.μ⁺ then re-adds the entire suffix `{[S, 1, ..., 1, k] : k₀ ≤ k ≤ n_S}` with the replaced position k₀ now carrying the new value and all other positions k ∈ {k₀+1, ..., n_S} carrying their previously mapped values. This is a multi-position K.μ⁻ + K.μ⁺ pair; the count of positions removed and re-added equals `n_S − k₀ + 1`. *Co-occurring K.α and K.ρ when the new value at k₀ is fresh content.* As in the maximum case above, when the replacement value at k₀ is freshly allocated content the full composite is K.α + K.μ⁻ + K.μ⁺ + K.ρ — the same four elementary steps surround the multi-position M-touching pair. K.μ⁺ now re-introduces `n_S − k₀` *previously-allocated* I-addresses (at positions `k₀+1, …, n_S`) alongside the single fresh I-address at k₀; the re-introduced addresses already carry their own provenance entries `(a, d) ∈ R` from the original placements (P2 preserves them), so J1★ requires only the *one* new K.ρ for the freshly-allocated I-address at k₀ — see the *Worked example: interior content replacement* below for the full four-step trace and the J1★/J1'★ verification confined to the single new content-subspace range entry.

A *worked decomposition* of the interior case appears in the K.μ~ subsection below (Decomposition of K.μ~ into K.μ⁻ + K.μ⁺), where full-clearance K.μ⁻ + content-rebuild K.μ⁺ is treated in full detail; interior replacement is the same shape with `n'_S = k₀ − 1` rather than `n'_S = 0`. The simple "K.μ⁻ followed by K.μ⁺" gloss is correct when read as the M-touching pair, but the *full* composite includes K.α before and K.ρ after whenever the replacement value is fresh content; the *cardinality* of each M-touching operation depends on position: replacement at the maximum is one position; replacement at the interior is the suffix from the replaced position to the maximum, all rebuilt in one K.μ⁻ + K.μ⁺ pair.
...
**K.μ⁻ amendment (PerSubspaceScope).** In the extended state, K.μ⁻'s D-CTG / D-MIN postconditions read as D-CTG★ / D-MIN★ (the per-subspace forms introduced immediately below) and the admissible removal pattern stated at K.μ⁻'s definition in *Elementary transitions* applies *per-subspace*: for each subspace `S ∈ {s_C, s_L}`, the removed positions in `V_S(d)` form a suffix of `V_S(d)` under the D-SEQ★ enumeration `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n_S}`, admitting the empty suffix (no change), a proper suffix (partial truncation), or the full set (clearance); at least one subspace must contract strictly. The per-subspace patterns are independent across `s_C` and `s_L`; the strict-contraction conjunct delivers the effect clause `dom(M'(d)) ⊂ dom(M(d))` at the whole-arrangement level. The K.μ⁻ exhaustiveness lemma stated at K.μ⁻'s definition site partitions per-subspace contractions into case (a) suffix removal, case (b) interior hole, and case (c) minimum hole, with only case (a) admissible under D-CTG★ / D-MIN★. This amendment was already foreshadowed at K.μ⁻'s definition (its per-subspace precondition referenced D-SEQ★ explicitly), and is re-stated here for parity with the K.α and K.μ⁺ amendments — every transition whose contract changes in the extended state is now flagged with an explicit amendment paragraph.
...
*Statement.* For any two distinct documents `d₁, d₂ ∈ E_doc` with `d₁ ≠ d₂`, the link sub-allocator anchors `p₁ := b_L(d₁) = [d₁.0.s_L]` and `p₂ := b_L(d₂) = [d₂.0.s_L]` satisfy `p₁ ⋠ p₂ ∧ p₂ ⋠ p₁`. By T10 (PartitionIndependence, ASN-0034), every address extending `p₁` differs from every address extending `p₂`. The same statement holds with `b_C` in place of `b_L` for content allocations.
...
Downstream sites cite this lemma as **CrossDocDisjoint** (or "the Cross-document disjointness chain lemma").
...
| K.δ | ¬IsNode(e) | `e ∉ E` | T10a GlobalUniqueness on parent allocator's tracked domain |
...
| K.α | Cross-document distinctness | `a₁ ≠ a₂` for K.α events under distinct d₁ ≠ d₂ | Cross-document disjointness chain lemma (T10a.{2,5} → T10) applied at content-allocator anchors `b_C(d₁), b_C(d₂)` |
...
Nodes are baptised by an external authority outside T10a's per-tumbler-spawn machinery (Nelson's hierarchical-baptism design). Document and account allocation operates within T10a via inc-chains under their parent allocator's frontier. Content/link sub-allocator activation, in contrast, requires an axiomatic first-emission commitment as an abstraction over a multi-step T10a chain.

*K.δ case (ii) discharge and parent-allocator activation.* The row "K.δ | ¬IsNode(e) | `e ∉ E` | T10a GlobalUniqueness on parent allocator's tracked domain" closes the freshness obligation by interpreting each non-node K.δ event as a T10a transition step on the parent's entity-level sub-allocator. The discharge applies uniformly across k ∈ {0, 1, 2}, but the parent-allocator regime differs by k:
...
- *k = 1 (version under existing document allocator):* operand `t ∈ E_doc` was placed in E_doc by a prior K.δ event whose effect activated d's version sub-allocator `A_v(d)` (per the Sub-allocator-names definition's clause: "Activation requires d's spawning point to inhabit d's parent allocator's tracked domain at the K.δ event"). The step `e = inc(t, 1)` is a T10a T1 sibling-increment (after the first version) or T2 spawn step (for the first version) on `A_v(d)`. T10a GlobalUniqueness delivers `e ∉ E`.
...
- *k = 2 (descent producing the first child under a node or account):* this is the activation case, and care is required to identify *which* allocator is being spawned and *which* allocator is its parent in T10a's allocator tree. We name the participants explicitly. Let `t` be the K.δ operand (a node when creating an account; an account when creating a document) and `e = inc(t, 2)` the spawn output. The K.δ event itself spawns a *new entity-level sub-allocator* under `t` — call it `A_↓(t)`: when t is a node, `A_↓(t)` is "t's account sub-allocator" emitting account-level children of t; when t is an account, `A_↓(t)` is "t's document sub-allocator" emitting document-level children of t. The K.δ event is the T10a T2 spawn step that *creates* `A_↓(t)`, with `t` as spawnPt and `e` as the first emission.
  ...
  *Connection to ownership semantics.* This T10a-mediated discharge is the formal counterpart to Nelson's design intent: the right to baptise sub-addresses under `t` is conferred by ownership of `t`, not by a separate registration event ("Whoever owns a specific node, account, document or version may in turn designate (respectively) new nodes, accounts, documents and versions, by forking their integers" — LM 4/17; cf. Gregory's `findisatoinsertnonmolecule`, which handles the first-child case inline without any prerequisite registration step). The K.δ event for k = 2 is the moment at which the "inherent right of ownership" — already implicit when t entered E — is *exercised* via the T10a T2 spawn step. No separate "EntityAllocatorAxiom" is required because the K.δ event itself supplies the T2 spawn step structure that activates `A_↓(t)` and commits its first emission in one indivisible step (SequentialTransitionAxiom).

  This contrasts with content/link sub-allocators `A_C(d), A_L(d)`, whose first emissions cross from `d` to an anchor outside `dom(C) ∪ dom(L)` and require SubAllocatorAxiom.FirstEmission for activation discharge — there the spawned allocator emits into a *non-entity-level* domain (`zeros = 3` element-level), and the K.δ event placing `d` into E_doc does not itself perform that emission; the SubAllocatorAxiom is needed to bridge the gap. For entity-level descent (k = 2 under K.δ), no such bridge is needed: the entity emission *is* the K.δ event.
...
**Notation disambiguation: atomic vs. composite `Σ → Σ'`.** The notation `Σ → Σ'` is overloaded across this ASN and must be read by context. Four points fix the semantics:

(a) *Elementary (atomic) interpretation.* Within an elementary transition's specification — its precondition, effect, and frame paragraphs (the K.α, K.δ, K.λ, K.μ⁺, K.μ⁻, K.μ⁺_L, K.ρ definitions) — and within SequentialTransitionAxiom, `Σ → Σ'` denotes a single atomic step. Σ is the pre-state, Σ' is the immediate post-state, and no intermediate state intervenes.

(b) *Composite-boundary interpretation.* In the coupling constraints J0, J1, J1★, J1'★, in the per-transition invariants P0, P1, P2, P3, L12, and in the *per-transition* portion of ExtendedReachableStateInvariants and ExtendedTransitionInvariants, `Σ → Σ'` denotes the *initial-to-final* boundary of a valid composite — Σ is the composite's pre-state, Σ' is its post-state, and intermediate states `Σ₁, ..., Σₙ₋₁` are not constrained by these predicates. ValidComposite★ clause (2) makes this explicit: coupling predicates are evaluated "*only* between the initial state Σ and the final state Σ'."

(c) *Intermediate states may transiently violate composite invariants.* A composite of the form K.α + K.μ⁺ + K.ρ has intermediate states Σ_after_K.α and Σ_after_K.μ⁺ at which the composite-boundary invariants P4★, P7a, and the couplings J0/J1★/J1'★ need not hold. For example: after K.α alone, dom(C) has grown by `{a}` but no V-position references `a` and no provenance entry exists for `(a, d)` — P7a and J0 are violated at Σ_after_K.α. Only after the composite completes (K.μ⁺ places `a`, K.ρ records `(a, d)`) do the boundary invariants hold at Σ_final. The per-state elementary invariants (D-CTG★, D-MIN★, S8a, S8-depth, etc., and the per-state portion of ExtendedReachableStateInvariants) *do* hold at every intermediate state, since each elementary step's postcondition discharges them.

(d) *Operations layer responsibility.* This ASN does not specify *which* finite sequences of elementary transitions constitute admissible composites for user-level operations — that is the role of a future operations ASN. The present ASN guarantees only that *if* a finite sequence satisfies ValidComposite★'s clauses (1) and (2), *then* every per-state invariant holds at every intermediate state and every per-composite invariant holds at the composite boundary. Ensuring that user-level operations (insertion, deletion, fork, link creation) decompose into valid composites — and in particular that no operation terminates at an intermediate state where boundary invariants are violated — is the operations layer's obligation, not the elementary transition model's.

A notational distinction (`Σ → Σ'` atomic vs. `Σ ⇒ Σ'` composite) was considered but not adopted, to preserve compatibility with ASN-0036 and ASN-0043's existing usage. Disambiguation by context, fixed by points (a)–(d) above, is the convention this ASN follows.
