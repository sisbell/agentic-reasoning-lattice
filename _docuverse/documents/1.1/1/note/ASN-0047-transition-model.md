...
**NodeRegistryBootstrap (Axiom, BootstrapRegistrySeeding).** At the initial state `Σ₀`, `n₀` inhabits the node-allocation registry's tracked domain. This is an initial-state commitment, independent of NodeUniqueAllocation — every other entity `t ∈ E` reaches the registry's tracked domain via the K.δ event that minted it (NodeUniqueAllocation clause (a) at that prior event), but `n₀` enters at `Σ₀` with no prior K.δ, so its registry membership is supplied by axiom rather than derived from a non-existent predecessor. The T2-spawn discharge in K.δ case (ii) k = 2 with operand `t = n₀` (creating the first account under `n₀`) cites NodeRegistryBootstrap to satisfy the spawnPt premise on the registry side.
...
*Step 2: terminal contiguity.* Restricted to terminal-varying tuples `[S, 1, ..., 1, k]`, the V-ordering coincides with the natural order on `k`. The notation `[S, 1, ..., 1, k]` should be read uniformly across both Step-1 cases: at m ≥ 3 the inner "1, ..., 1" segment has length m − 2 ≥ 1 and carries the inner positions Step 1 fixed at 1, while at m = 2 that segment has length m − 2 = 0 and the notation degenerates to `[S, k]` (subspace identifier and terminal component only); the remainder of Step 2 applies uniformly, with v_min and v_max simply read as `[S, 1]` and `[S, k_max]` in the degenerate case. By S8-fin, n < ∞; let `v_max = max(V_S(d)) = [S, 1, ..., 1, k_max]` for some k_max ∈ ℕ⁺ (well-defined since V_S(d) is finite and non-empty). By D-CTG★'s closed-interval-membership content, every depth-m positive tuple z with subspace identifier S satisfying `v_min ≤ z ≤ v_max` is in V_S(d) (v_min and v_max are both in V_S(d), bracketing a closed interval admissible to the D-CTG★ premise); restricted to terminal-varying tuples `[S, 1, ..., 1, k]`, this gives `{[S, 1, ..., 1, k] : 1 ≤ k ≤ k_max} ⊆ V_S(d)`. The reverse inclusion follows from v_max being the maximum: any `[S, 1, ..., 1, k]` with `k > k_max` would exceed v_max in lex order. Hence `V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ k_max}` — `{[S, k] : 1 ≤ k ≤ k_max}` at m = 2 — and counting gives `k_max = n`. ∎
...
| NodeRegistryBootstrap | Axiom (BootstrapRegistrySeeding): n₀ ∈ dom(node-allocation registry) at Σ₀. Initial-state commitment grounding the T2-spawn discharge in K.δ case (ii) k = 2 with operand t = n₀ (first account under n₀), where no prior K.δ event for n₀ exists to supply registry membership via NodeUniqueAllocation |
