...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | elementary decomposition K.μ⁻+K.μ⁺ (this row's restriction + amendment Class (a) cells); realisable π are exactly the subspace-preserving ones (K.μ⁺ content-subspace precondition), no appeal to S3★(Σ'); §*Decomposition of K.μ~* |  frame |
