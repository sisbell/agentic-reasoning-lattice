...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases. The freshness conjunct is a caller-checked guard — `inc(t, 0) ∉ E` at k = 0 and `e ∉ E` at k ∈ {1, 2} — checked against the present state Σ, not derived from a structural fact; at k = 0, FrontierEquivalence characterizes it as the condition that `t` is the frontier of its chain. Identification of the parent allocator each step acts on, and the spawnPt premise — for every k including the k = 0 freshness dispatch: §*K.δ case (ii) discharge and parent-allocator activation*. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
...
This section is the single owner of the per-k parent-allocator identification: every other site (the K.δ box, the worked examples) cites here. A non-node K.δ event's step `e = inc(t, k)` acts on a parent entity-level sub-allocator that depends on k. Below, under case (ii)'s k-labels, we give the *parent-allocator activation* (which allocator each step acts on) and, for the child-spawn regimes, the spawnPt premise. We consume one reusable primitive from ParentAllocatorDispatch (*Sub-allocator names*) — the level→allocator-base recovery that names the owning allocator from an address's zero-count stratum — but the per-k organisation itself lives only here. The spawn-admissibility conjuncts — the `k' ∈ {1, 2}` membership and T10a's zero-count side condition — are stated once at the K.δ box and are not repeated here. By k:
...
P4★ bounds provenance by the *content-subspace* restriction of containment, scoped so it coexists with P7 (ProvenanceGrounding, stated below): the link-subspace pairs that an unscoped `Contains(Σ) ⊆ R` would demand are exactly the ones P7 forbids (J-LV, *Scoped coupling constraints* below). P4★ is a Class (b) composite-boundary property: K.μ⁺ alone may transiently violate it (it adds to Contains_C but does not extend R), with restoration at the composite boundary governed by J1★.
...
(ii) K.μ⁺ populating M'(d_new) by a *complete* transclusion of `d_op`'s content subspace: `ran(M'(d_new)) = ran(M(d_op)|_{V_{s_C}(d_op)})` — every content I-address of the source is inherited (range equality, not mere containment), and no new content addresses are introduced, so every target lies in the pre-existing content store. The content source operand `d_op` is fixed by Definition (Fork)'s operand-tracking rule, so for a subsequent (k = 0) version the edit-inheritance noted there holds. The equality — not a one-sided bound — is what "copies the contents" demands: Nelson's CREATENEWVERSION "creates a new document with the contents of document `<doc id>`" (LM 4/66) — the *whole* current arrangement, never a deliberate proper subset (partial inclusion is a separate mechanism: COPY/quote-links/versioning-by-inclusion, LM 4/67); and Gregory's `docreatenewversion` reads the source document's full live POOM via `doretrievedocvspanfoo` — the entire `cwid.dsas[V]` content-subspace width, including all prior INSERT/DELETE edits — with no code path that filters or selects a content-subspace subset. (Only the *link* subspace is excluded — `retrievedocumentpartofvspanpm` anchors at the text-subspace start, so `2.x` link orgls fall outside the copied vspan; J4 reconstructs that exclusion as `V_{s_L}(d_new) = ∅` below.)

(iii) K.ρ recording provenance for each a ∈ ran(M'(d_new)),

and no other elementary steps. Step (ii) must produce a content-subspace arrangement on d_new whose range equals d_op's content-subspace range and discharges the per-state arrangement invariants (S2, S3★, S8a, S8-depth, S8-fin, D-CTG★, D-MIN★) at the post-state.
...
- *J4:* d₂ ∈ E₂_doc \ E₁_doc, ran(M₂(d₂)) = {a₁, a₂} = ran(M₁(d₁)|_{s_C}) (complete content-subspace copy, d_op = d₁). ✓
...
*K.μ⁺:* M'(d₃) = {[1,1] ↦ a₁, [1,2] ↦ a₂, [1,3] ↦ a₃} — the same I-addresses as the *edited* d₂, transcluding d_op = d₂'s current content subspace. The complete-copy range equality `ran(M'(d₃)) = ran(M(d_op)|_{V_{s_C}(d_op)}) = ran(M₃(d₂)|_{s_C}) = {a₁, a₂, a₃}` holds; crucially a₃ — d₂'s edit, absent from d₁ — is inherited. No new content enters C (C' = C₃). The three content-subspace V-positions satisfy S8a (positive components, zeros = 0) and S8-depth (uniform depth 2).
...
- *J4 (range equality):* d₃ ∈ E'_doc \ E₃_doc, and ran(M'(d₃)) = {a₁, a₂, a₃} = ran(M(d_op)|_{V_{s_C}(d_op)}) = {a₁, a₂, a₃} with d_op = d₂ — the complete content-subspace copy. The equality is against the *operand* d₂. It would *fail* against the original base d₁: ran(M₃(d₁)|_{s_C}) = {a₁, a₂} omits a₃, so the (incorrect) d_src-fixed source `ran(M'(d₃)) = ran(M(d₁)|_{V_{s_C}(d₁)})` would lose a₃ — precisely the silent edit-loss the operand-tracking source prevents. ✓
...
| J4 | Fork composite: K.δ + K.μ⁺ + K.ρ (no other steps); content source is the K.δ operand d_op (d_src for k=1, prev_version for k=0); precondition V_{s_C}(d_op) ≠ ∅; complete content-subspace copy ran(M'(d_new)) = ran(M(d_op)|_{V_{s_C}(d_op)}) (range equality, per CREATENEWVERSION full-copy semantics); dom(C') = dom(C) follows from frames; provenance from J1★/J1'★ (extended-state couplings) |
