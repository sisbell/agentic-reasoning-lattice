...
**J4 (Fork composite).** Nelson's forking creation mode — version creation with ancestry indication (LM 4/66, CREATENEWVERSION) — is a composite whose elementary steps are exactly K.δ + K.μ⁺ + K.ρ, all serving the new document d_new. *Fork is version creation on d_src's version chain `A_v(d_src)`, with K.δ address allocation uniform across first and subsequent versions:* the first version is d_new = inc(d_src, 1) — the K.δ k = 1 step that activates `A_v(d_src)` — and each subsequent version is d_new = inc(prev_version, 0), a K.δ k = 0 step on `A_v(d_src)`'s frontier whose operand prev_version is a prior version of d_src (K.δ's per-`(t, 1)` uniqueness fires `inc(d_src, 1)` at most once, so versions 2+ necessarily arise as k = 0 frontier-advances). In every case d_new extends d_src's document-field prefix (d_src ≼ d_new), so d_new carries the ancestry-by-address indication regardless of whether the emitting step is k = 1 or k = 0. The transcluded content source operand `d_op` is fixed by the operand-tracking rule of Definition (Fork) below. This matches Gregory's `docreatenewversion`, which performs the identical `findisatoinsertnonmolecule` allocation for every version (the first allocation produces `d_src.1`, subsequent allocations advance the last component to `d_src.k+1`), special-casing neither. By contrast, the k = 0 *sibling-document* allocation under the source's account (`docreatenewdocument`) and the k = 2 hierarchical descent are *not* forks: their outputs do not extend d_src's prefix and carry no ancestry-by-address indication. The discriminating fact is which allocator's frontier the k = 0 operand sits on — `A_v(d_src)`'s frontier (a version of d_src, a fork) versus `A_doc(parent(d_src))`'s frontier (an independent sibling document, not a fork).

**Definition (Fork).** A *fork* of d_src to d_new is a composite transition `Σ →* Σ'`. Write `d_op` for the *content source operand* of the fork — the operand `t` of the K.δ step (i), whose current content subspace is transcluded into d_new. **Operand-tracking rule.** The content source tracks the K.δ sub-case: `d_op = d_src` for k = 1 and `d_op = prev_version` for k = 0, so a subsequent (k = 0) version inherits the *current edited content* of the prior version it forks from, capturing edits absent from the base d_src. This is the sole statement of the rule; steps (i) and (ii) below invoke it rather than re-derive it. The *precondition* is d_src ∈ E_doc ∧ d_op ∈ E_doc ∧ V_{s_C}(d_op) ≠ ∅, together with the per-sub-case activation condition on the K.δ step, which also fixes `d_op` and supplies the structural discriminator separating a fork from an independent sibling-document creation:
...
(i) a K.δ case (ii) step producing d_new on d_src's version chain `A_v(d_src)`, with d_new ∉ E_doc, taking the K.δ operand `t = d_op` as fixed by Definition (Fork)'s operand-tracking rule — k = 1 producing the first version d_new = inc(d_src, 1) when `A_v(d_src)` has no prior emission, or k = 0 producing the next version d_new = inc(prev_version, 0) on `A_v(d_src)`'s current frontier; in both cases d_src ≼ d_new,
