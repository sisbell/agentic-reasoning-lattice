...
The constructive precondition is *equivalent* to the post-state characterization "M'(d) satisfies D-CTG★/D-MIN★/D-SEQ★, `dom(M'(d)) ⊂ dom(M(d))` (strict, proper subset), with value preservation on survivors"; this equivalence is proved in *K.μ⁻ admissible contraction shape* below. The effect clause `dom(M'(d)) ⊂ dom(M(d))` and the strict-contraction clause `(E S :: n'_S < n_S)` carry the proper-subset requirement on their own. The value-preservation clause `(A v : v ∈ dom(M'(d)) : M'(d)(v) = M(d)(v))` (effect clause) is satisfied automatically by the restriction definition `M'(d) = M(d) ↾ R`, ruling out modification of surviving values.
...
Validity of a composite transition `Σ →* Σ'` is ValidComposite★. P4★ is discharged at composite boundaries by clause (2)'s couplings.
...
Every freshly allocated I-address appears in some arrangement in the post-state — the containing document may itself have been freshly created by K.δ in the same composite transition. J0 is an imposed coupling constraint (not derived): it is a clause-(2) validity condition of ValidComposite★, not an axiom of the elementary transition system. A clause-(1)-satisfying elementary sequence in which K.α executes alone — every elementary precondition holding at each intermediate state — does exist; J0 is precisely what excludes it from the *valid* composites. The motivation is Nelson's design intent (LM: content enters the docuverse only by placement in a document), which justifies imposing J0 rather than establishing it as ground truth of the transition vocabulary.
