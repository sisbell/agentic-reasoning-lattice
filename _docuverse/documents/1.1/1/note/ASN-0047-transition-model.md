...
*The conjuncts are mutually independent.* A transposition that swaps two link positions `[s_L, …, 1] ↦ ℓ₁`, `[s_L, …, 2] ↦ ℓ₂` with `ℓ₁ ≠ ℓ₂` (identity on every content position) leaves the V-position *domain* `{[s_L, …, 1], [s_L, …, 2]}` fixed, so it satisfies (i) (the shape package on the unchanged domain), (ii) (`M'(d) ≠ M(d)`, since `ℓ₁` and `ℓ₂` are exchanged), (iii) (length-preserving), (iv) (subspace-preserving), and preserves CL-UNIQ (the link map stays injective) — yet it is *not* link-fixing, so it fails admissibility on clause (v) alone. Symmetrically, since S8-depth permits two subspaces to share a depth (`m_{s_C} = m_{s_L}` — the worked examples do exactly this), at a coinciding depth a transposition swapping a content position `[s_C, …, k]` with a link position `[s_L, …, j]` is length-preserving (iii), non-trivial (ii), and leaves the per-subspace V-position *sets* satisfying the shape package (clause (i)'s scope) — yet it relocates a `dom(C)` value to an `s_L` position, breaking S3★, so it fails admissibility on clause (iv) alone.
