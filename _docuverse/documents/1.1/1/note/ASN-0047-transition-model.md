...
*Dependency chain at a glance.* The argument structure below has a fixed order of derivation, in which each step's input is the previous step's output. The chain is non-circular and load-bearing for both the K.μ~ link-subspace fixity result and the existence condition `|dom_C(M(d))| ≥ 2`. Each step lists the lemmas/properties it *consumes* (its inputs) and the property it *produces* (its output), making non-circularity verifiable in isolation:
  ...
  *Step (A) — S3★(Σ') and admissibility clause (i).*
    Consumes: S3★(Σ) (inductive hypothesis at the pre-state), K.μ⁻'s restriction effect (values unchanged on survivors), K.μ⁺ amendment (new positions have `subspace = s_C`), K.μ⁺'s precondition `M'(d)(v) ∈ dom(C)` for each new content-subspace mapping, S3★(Σ_int) at the intermediate state (link-subspace mappings retained by K.μ⁻'s restriction still target dom(L) since L is frame-preserved).
    Produces: S3★(Σ'); admissibility clause (i) of K.μ~ (post-state invariant package) discharged by the same decomposition.
    *Does not consume:* subspace preservation under π, link-subspace fixity, CL-UNIQ. The route through K.μ⁻ + K.μ⁺ decomposition uses neither of these — it argues purely from each elementary step's effect/frame clauses applied independently to the link- and content-subspace partition of M(d).

  *Step (B) — Subspace preservation under π.*
    Consumes: S3★(Σ) (Step (A)'s pre-state hypothesis), S3★(Σ') (output of Step (A)), the bijection equation `M'(d)(π(v)) = M(d)(v)`, L14 at Σ' (`dom(C) ∩ dom(L) = ∅`), S3★-aux at both states.
    Produces: `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))`.
    *Does not consume:* link-subspace fixity (which is downstream), CL-UNIQ.

  *Step (C) — Link-subspace functional identity (Steps 1–3 below).*
    Consumes: Step (B)'s subspace preservation, K.μ~-FIX's per-subspace domain equality (which follows from D-SEQ★ at both endpoints plus subspace preservation — see K.μ~-FIX below), K.μ⁺ amendment (no link-subspace V-positions created), K.μ⁻'s value-preservation clause on survivors.
  ...
  *Step (E) — Admissibility clause (ii) and the existence condition.*
    Consumes: Step (D)'s pointwise fixity on dom_L (so `π|_{dom_L} = id`), K.μ~-FIX (so `π` is a permutation of a fixed `dom(M(d))`), Step (B)'s subspace preservation (so `π` restricts to a bijection on `dom_C(M(d))`), admissibility clause (ii) (`π ≠ id`).
...
The chain reads (A) → (B) → (C) → (D) → (E), with no back-references. Step (A) and Step (B) jointly produce S3★(Σ') and subspace preservation; the matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~ ("preserved via K.μ⁻ restriction + K.μ⁺ amendment alone (link-subspace fixity is downstream, not prerequisite)") corresponds to Step (A) specifically, and the parenthetical "link-subspace fixity is downstream" makes explicit that Step (C)–Step (D) do not feed back into Step (A). Each step appears below in the same order; the prose substantiates each link explicitly.

*Proof of Step (A) — S3★(Σ') from K.μ⁻ restriction + K.μ⁺ amendment alone.* We show that S3★ holds at the K.μ~ post-state `Σ' = (C, L, E, M', R)`, deriving the property directly from the elementary decomposition without invoking subspace preservation under π (Step (B)) or link-subspace fixity (Steps (C)–(D)), so as to confirm the chain's claim that Step (A) precedes Step (B). Let `Σ_int = (C, L, E, M_int, R)` denote the intermediate state between the K.μ⁻ and K.μ⁺ atomic steps of the decomposition. Three sub-claims, dispatched in order:

  *(A.1) S3★(Σ_int).* K.μ⁻'s effect is `M_int(d) = M(d) ↾ R'` (restriction to a subset of `dom(M(d))`) with values unchanged on survivors, and `M_int(d') = M(d')` for every `d' ≠ d`; K.μ⁻'s frame on `C` and `L` gives `dom(C_int) = dom(C)` and `dom(L_int) = dom(L)`. For every `v ∈ dom(M_int(d))`: `v ∈ dom(M(d))` (survivors lie in the pre-state domain), and `M_int(d)(v) = M(d)(v)` (value-preservation on survivors); the subspace `subspace(v)` is a property of the V-position tumbler itself, unchanged by the restriction. By S3★(Σ) (inductive hypothesis at the pre-state) at `v`: when `subspace(v) = s_C`, `M(d)(v) ∈ dom(C)`; when `subspace(v) = s_L`, `M(d)(v) ∈ dom(L)`. Substituting `M_int(d)(v) = M(d)(v)` and `dom(C) = dom(C_int)`, `dom(L) = dom(L_int)`: when `subspace(v) = s_C`, `M_int(d)(v) ∈ dom(C_int)`; when `subspace(v) = s_L`, `M_int(d)(v) ∈ dom(L_int)`. The same dispatch on every untouched arrangement `M_int(d')` for `d' ≠ d` gives S3★(Σ_int) globally.

  *(A.2) K.μ⁺'s new content-subspace positions target dom(C).* The K.μ⁺ step in the K.μ~ decomposition (full-clearance form) writes `{π(v) ↦ M(d)(v) : v ∈ V_{s_C}(d)}` at fresh V-positions disjoint from `dom(M_int(d))`. By the K.μ⁺ amendment, every new V-position satisfies `subspace(·) = s_C`. By K.μ⁺'s referential-integrity precondition `M'(d)(v) ∈ dom(C')` for every new content-subspace mapping (evaluated at the post-K.μ⁻ intermediate state, which `Σ_int` makes available): each newly written value lies in `dom(C_int) = dom(C')` (`C' = C_int = C` by frame across both atomic steps).

  *(A.3) S3★(Σ').* The post-state arrangement `M'(d)` is the union of two disjoint contributions: (i) survivors of K.μ⁻ that K.μ⁺ frames unchanged — these inherit S3★ from S3★(Σ_int) at A.1; (ii) newly written content-subspace positions from K.μ⁺ — these satisfy S3★'s content clause directly by A.2. `dom(C') = dom(C)` and `dom(L') = dom(L)` by the composite frame. Combining (i) and (ii) over every `v ∈ dom(M'(d))` discharges S3★(Σ') for the affected document `d`; the framed arrangements `M'(d') = M(d')` for `d' ≠ d` inherit S3★ from S3★(Σ) at the pre-state directly. ∎ (Step (A))

The argument consumed neither subspace preservation under π (Step (B)) nor link-subspace fixity (Steps (C)/(D)) nor CL-UNIQ (Step (D)); only S3★(Σ) (inductive hypothesis) and the effect/frame clauses of the two atomic decomposition steps entered. Step (A)'s independence from downstream steps is now verifiable from the proof above. Steps (B), (C), (D), (E) follow in the same order in the prose below — Step (B) appears immediately after the bijection equation as the `s_C → s_L` / `s_L → s_C` case analysis; Steps (C) and (D) appear as Steps (1)–(3) and Step (4) of the *Link-subspace fixity* proof; Step (E) appears in the *Decomposition* paragraph below.
...
π is admissible iff (i) the induced post-state `M'(d)` would satisfy S8a, S8-depth, D-CTG★, D-MIN★, and S3★, and (ii) `π ≠ id`. Clause (ii) makes K.μ~ a real reordering: a permutation whose net effect is the identity is not a K.μ~ transition (the system simply does not change). S8a at every `π(v)` is part of clause (i)'s post-state invariant package on M'(d); K.μ~-FIX (derived below) gives `dom(M'(d)) = dom(M(d))`, so every `π(v) ∈ dom(M(d))` inherits S8a from the inductive hypothesis at Σ unconditionally — no separate per-`π(v)` check is needed. Subspace preservation — `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` — is a *derived consequence* of S3★ at both endpoints together with L14 and the bijection equation; it is *not* a prerequisite for establishing S3★(Σ'). The independent route to S3★(Σ') is by direct K.μ⁻ + K.μ⁺ decomposition (matrix entry in ExtendedReachableStateInvariants for S3★ under K.μ~: "preserved via K.μ⁻ restriction + K.μ⁺ amendment alone (link-subspace fixity is downstream, not prerequisite)"), so S3★(Σ') is available without circular dependence on subspace preservation. The derivation of subspace preservation then consumes *both* S3★(Σ) (the inductive hypothesis at the pre-state) and S3★(Σ') (just-established at the post-state). S3★-aux constrains `subspace(v) ∈ {s_C, s_L}` and `subspace(π(v)) ∈ {s_C, s_L}` at every reachable state, so subspace mismatch under π takes exactly one of two complementary forms: a content-subspace `v` mapped to a link-subspace `π(v)` ("`s_C → s_L`"), or a link-subspace `v` mapped to a content-subspace `π(v)` ("`s_L → s_C`"). We discharge both cases by contradiction.
...
| S3★ | frame | frame | frame | amendment: subspace(v)=s_C ⟹ M'(d)(v)∈dom(C); link clause framed | precondition: ℓ∈dom(L) and subspace(v_ℓ)=s_L | restriction (values unchanged) | both clauses preserved via K.μ⁻ (full-clearance) restriction + K.μ⁺ amendment alone (link-subspace fixity is downstream, not prerequisite) |  frame |
...
| S8a, S8-depth, S8-fin | frame | new doc has M(d)=∅ (vacuous) | frame | precondition: positivity, depth, finite extension | precondition: positivity, m_L=2, finite | restriction preserves all three | K.μ⁻ (full-clearance) restriction preserves all three on link-subspace survivors; K.μ⁺ preconditions re-establish them on the rebuilt content arrangement (positivity / uniform depth / finite extension at the rebuild positions) | frame |
...
| D-CTG★ / D-MIN★ | frame | frame | frame | precondition discharge (K.μ⁺'s original precondition list, in *Elementary transitions*, requires the resulting M'(d) to satisfy D-CTG / D-MIN — strengthened to D-CTG★ / D-MIN★ in the extended state: content-subspace contiguity + minimum preserved) | preconditions cover D-MIN★ (empty case) and D-CTG★ (shift case) | constructive precondition discharge — the per-subspace retention count `n'_S` directly produces the canonical suffix-prefix shape `V_S(d') = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n'_S}` which satisfies D-CTG★ and D-MIN★ at Σ' by construction; see *K.μ⁻ admissible contraction shape* for the equivalence proof | K.μ⁻ (full-clearance) by construction retains `V_{s_L}(d)` pointwise (the form clears only the content subspace), so D-CTG★ / D-MIN★ on the link subspace carry forward from Σ via the inductive hypothesis; content subspace empty after the clearance (D-CTG★ / D-MIN★ vacuous). K.μ⁺ preconditions re-establish D-CTG★ / D-MIN★ on the rebuilt content arrangement (the K.μ⁺ cell's discharge above applies at the rebuild step) | frame |
| D-SEQ★ | frame | frame | frame | derived from D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a | derived | derived | derived at Σ' from the K.μ~-chain post-state values of D-CTG★ + D-MIN★ + S8-depth + S8-fin + S8a (each established by the K.μ⁻ (full-clearance) + K.μ⁺ decomposition above) | frame |
...
| K.δ-ID.zeros-0/1 | Structural identity on `e = inc(t, k)` for k ∈ {0, 1}: `zeros(e) = zeros(t)` (TA5(c)/(d) preserve zeros). Promoted to a named identity for direct citation in K.δ-related verification — in particular by the multi-version invariant chain in K.δ k = 1 discharge and by P8 preservation under K.δ k ∈ {0, 1} |
| K.δ-ID.zeros-2 | Structural identity on `e = inc(t, 2)`: `zeros(e) = zeros(t) + 1` (TA5(d) at k = 2). Discharges the K.δ k = 2 zeros bound `zeros(t) ≤ 1` from the case-level `¬IsElement(e)` and provides the increment used in S7d preservation |
| K.δ-ID.parent-0/1 | Structural identity on `e = inc(t, k)` for k ∈ {0, 1}: `parent(e) = parent(t)` (T4b parent projection's truncation past the last separator is preserved by both k = 0 and k = 1 increments). Discharges K.δ's case-level `parent(e) ∈ E` requirement when combined with P8 at `t` |
| K.δ-ID.parent-2 | Structural identity on `e = inc(t, 2)`: `parent(e) = t` (k = 2 introduces a new zero separator immediately after t, making t the parent prefix). Discharges K.δ's case-level `parent(e) ∈ E` requirement directly from `t ∈ E` |
