...
*K.μ~ discharge for the arrangement-shape package (uniform argument).* The shape invariants — S8a, S8-depth, S8★, D-CTG★, D-MIN★, D-SEQ★ — share one K.μ~ discharge: each is stipulated on `M'(d)` by K.μ~ admissibility (i), and the K.μ⁻ (full-clearance) + K.μ⁺ decomposition mechanically realises it — K.μ⁻ restriction preserves it on the link-subspace survivors, and K.μ⁺'s preconditions re-establish it on the rebuilt content arrangement. Per-property K.μ~ discharge: this paragraph.
