...
- `A_v(d)` — d's **version sub-allocator**, with no element-field anchor (it emits at the entity-hierarchy level, not the element-field level). *Parent allocator in T10a's tree, case-split by d's own provenance:* version sub-allocators nest along the version chain rather than all rooting in the same account-level document sub-allocator. (a) If `d` is an *original* document — minted by a prior K.δ case (ii) k = 2 step with operand `parent(d) ∈ E_account` — then `A_v(d)` is a child of `A_doc(parent(d))` (the document sub-allocator under d's account that minted d), with spawnPt `d` and spawnParam `1`. (b) If `d` is a *version* — `d = inc(d', 1)` minted by a prior K.δ case (ii) k = 1 step on some predecessor `d' ∈ E_doc` — then `A_v(d)` is a child of `A_v(d')` (the version sub-allocator of d's predecessor that minted d), with spawnPt `d` and spawnParam `1`. T10a.6 (DomainDisjointness, ASN-0034) makes (a) and (b) mutually exclusive: every `d ∈ E_doc` inhabits exactly one allocator's tracked domain, and that allocator is the unique parent allocator of `A_v(d)` in T10a's tree. In case (b) specifically, T10a.6 forbids `d ∈ dom(A_doc(parent(d)))`: d was minted by `A_v(d')`, so `d ∈ dom(A_v(d'))`, and the cross-allocator disjointness `dom(A_v(d')) ∩ dom(A_doc(parent(d))) = ∅` rules out the case-(a) parent. Its outputs are versions of d under T10a's discipline: the first emission is `inc(d, 1)` (a new IsDocument tumbler with `zeros = 2`), produced by the K.δ k = 1 step whose operand is d itself — that step IS the T10a T2 spawn step that activates `A_v(d)`. Subsequent emissions are T1 sibling-increments on `A_v(d)`'s frontier, produced by K.δ k = 0 steps whose operand is a prior version of d (`inc(prev_version, 0)`); the k = 1 case fires at most once per d under T10a's per-`(t, k')` uniqueness. T2 admissibility requires the spawnPt `d` to inhabit the parent allocator's tracked domain at the spawn event: in case (a) the prior K.δ k = 2 event placed d into `A_doc(parent(d))`, supplying `d ∈ dom(A_doc(parent(d)))`; in case (b) the prior K.δ k = 1 event placed d into `A_v(d')` as that allocator's first emission, supplying `d ∈ dom(A_v(d'))`. Either way the membership obligation discharges against the parent allocator identified by T10a.6.
...
- *k = 1 (version under existing document allocator):* The step `e = inc(t, 1)` is the T10a T2 spawn step that activates `A_v(t)`, t's version sub-allocator. By the Sub-allocator-names definition of `A_v(d)`, T10a.6 (DomainDisjointness, ASN-0034) identifies the unique parent allocator of `A_v(t)` in T10a's tree from t's own provenance — `A_doc(parent(t))` when t is an original document, `A_v(t')` when t is a version minted from a predecessor `t'` — and the prior K.δ event that minted t supplies the T2 spawnPt premise against whichever parent allocator T10a.6 identifies. The precondition `t ∈ E_doc` therefore discharges the membership obligation; P1 preserves that membership across subsequent transitions. T2 admissibility: `k' = 1 ∈ {1, 2}` and no zero-count side condition fires at `k' = 1`. Under T10a's per-`(t, k')` uniqueness this k = 1 step fires at most once per `t`, so it is always the T2 spawn step — never a T1 sibling. Subsequent versions of t arise from K.δ k = 0 events whose operand is a prior version of t (`inc(prev_version, 0)`); those are T1 sibling-increments on `A_v(t)`'s frontier and are dispatched by the k = 0 case above, not by k = 1. T10a GlobalUniqueness on the newly activated `A_v(t)` delivers `e ∉ E`.
...
- *Content subspace.* `V_{s_C}(d) = {[1,1], [1,2], [1,3], [1,4]}` shrinks to `V_{s_C}(d_int) = {[1,1]}` — partial suffix removal with `n'_{s_C} = 1`; the removed set `{[s_C, k] : 1 < k ≤ 4}` is exactly the n'_{s_C} = 1 suffix in the D-SEQ★-shaped pre-state.
- *Link subspace.* `V_{s_L}(d) = V_{s_L}(d_int) = ∅` — vacuous (zero-suffix at `n'_{s_L} = n_{s_L} = 0`).
