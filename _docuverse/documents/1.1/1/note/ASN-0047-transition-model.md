...
*Step (A) — Admissible π coincide with realisable π.* Subspace preservation `(A v ∈ dom(M(d)) :: subspace(π(v)) = subspace(v))` holds for every *admissible* π directly by admissibility clause (iv). What this step establishes is the matching realisability direction: every subspace-preserving (hence admissible) π is *realisable* — produced by the K.μ⁻ + K.μ⁺ decomposition in the full-clearance form whose canonical statement is given in §*Necessity and sufficiency of the precondition* below — and conversely every realisable π is subspace-preserving, so the two classes coincide and the decomposition realises exactly the admissible π. S3★-aux(Σ) confines every source subspace to `{s_C, s_L}`, so the two cases below are exhaustive.
