...
*Rationale (k = 0 conjuncts).* The `¬IsNode(t)` conjunct is required for `parent(t)` to be defined — T4b's parent projection is partial on T and undefined when `IsNode(t)`. Although `¬IsNode(t)` follows from the structural identity `zeros(t) = zeros(e)` together with the case-level `¬IsNode(e)` (`zeros(e) ≥ 1`), it is stated explicitly so the operand admissibility is visible at the definition site rather than left as a consequence of partial-function evaluation. The freshness conjunct `inc(t, 0) ∉ E` is the case-level `e ∉ E` specialised to `e = inc(t, 0)` — stated locally to record that the caller's operand selection must observe it. Operationally, the conjunction `t ∈ E ∧ inc(t, 0) ∉ E` IS the frontier identification: three premises combine to make this work. (i) T10a's per-`(t, 0)` uniqueness — the `(t, 0)` pair fires at most once across the system, so within `t`'s own sub-allocator chain no prior firing has consumed `(t, 0)` if its output is absent. (ii) P1 (E-monotonicity) — any prior firing of `(t, 0)` (anywhere in the system) would have placed its output permanently in E, so absence from the present E implies no prior firing. (iii) T10a's GlobalUniqueness — no allocator *other than* `t`'s own sub-allocator can produce the address `inc(t, 0)`, since independent allocator chains have disjoint tracked domains (T10a.6 cross-allocator domain-disjointness). Together, (i)–(iii) make `inc(t, 0) ∉ E` logically equivalent to "(t, 0) has not yet fired on t's sub-allocator chain," which means `t` is the most recent emission (the frontier) of that chain along the (t, 0) branch. The frontier identification is therefore *derived* from the precondition pair, not an independent obligation. T4b's `parent` projection does *not* by itself isolate t's sub-allocator: T4b groups entities by entity-hierarchy spine, not by allocator-tree provenance. Under a single account `A`, the document sub-allocator `A_doc(A)` emits documents (`zeros = 2`, `parent(·) = A` under T4b) at one length, and each existing document `d` under A spawns a version sub-allocator `A_v(d)` whose own emissions are also documents under T4b's `parent(·) = A` — and two such version sub-allocators `A_v(d₁), A_v(d₂)` can coexist with outputs at the same length (e.g., `v₁ = inc(d₁, 1)` and `v₂ = inc(d₂, 1)`, both of length `#A + 3`). T10a tree-siblings need not differ in length: T10a.1 fixes length uniformly *within* each allocator's sibling chain, and T10a.3 separates ancestor-descendant pairs by length, but neither separates tree-siblings (such as `A_v(d₁)` and `A_v(d₂)`) from one another. A maximality clause over T4b's `parent`/`zeros`/length stratification therefore cannot in general identify `t` as the frontier of its own sub-allocator. The direct freshness predicate `inc(t, 0) ∉ E` sidesteps allocator-structural identification entirely — its operational truth at the K.δ event is what selects `t` (equivalently, what places `t` on the frontier), and the freshness obligation `e ∉ E` then discharges from the precondition itself.

*Freshness discharge.* The `e ∉ E` precondition is discharged by case on the K.δ form: case (i) is closed by NodeUniqueAllocation (the node-allocation protocol's axiomatic uniqueness clause); case (ii) is closed by T10a's GlobalUniqueness on the parent allocator's tracked domain. Every K.δ case (ii) event has its operand `t ∈ E` (sub-cases k = 0 and k = 2 require this directly; sub-case k = 1 requires `t ∈ E_doc ⊆ E`), placed there by a prior K.δ event whose allocator chain is T10a-tracked from activation onward. For sub-case k = 0 the precondition `inc(t, 0) ∉ E` discharges `e ∉ E` directly, since `e = inc(t, 0)`; T10a's role is to certify that this precondition is achievable and stable. Specifically: T10a's per-`(t, 0)` uniqueness on t's own sub-allocator chain forbids the `(t, 0)` pair from firing more than once across the system history; P1 (E-monotonicity) forbids E from losing entries. The conjunction of these two means `inc(t, 0) ∉ E` at the pre-state holds *iff* `(t, 0)` has not yet fired — i.e., `t` is on the frontier of its sub-allocator's (t, 0)-branch — and once the current K.δ event fires `(t, 0)`, T10a's per-`(t, 0)` uniqueness prevents any future K.δ event from re-producing `inc(t, 0)`. T10a's GlobalUniqueness further ensures no *other* allocator can produce `inc(t, 0)` independently. The caller's responsibility is to verify the precondition (operationally, by selecting `t` from its sub-allocator's current frontier); the K.δ event then closes `e ∉ E` by precondition and T10a closes forward-uniqueness for subsequent events.
...
**Class (a): Per-state invariants** — preserved by each elementary transition individually, holding at every reachable state including intermediate states within composites. These are all per-state properties except P4★, P4a, and P7a: S2, S3★, S3★-aux, S4, S7a, S7b, S7c, S7d, S8a, S8-fin, S8-depth, S8★, D-CTG★, D-MIN★, D-SEQ★, P6, P7, P8, NodeLineage, L0, L1, L1a, L1b, L3, L14, L-fin, CL-OWN, CL-UNIQ.
...
| L3 | frame | frame | precondition: (F,G,Θ)∈Link ∧ Θ≠∅; preserved by L12 | frame | frame | frame | frame | frame |
...
*L3 (TripleEndsetStructure).* K.λ precondition `(F, G, Θ) ∈ Link ∧ Θ ≠ ∅` establishes L3 at the new entry; L12 preserves it pointwise for existing entries; Σ₀ has `dom(L₀) = ∅` so L3 holds vacuously at the base. All other transitions hold L in frame.
...
| P7a (every `a ∈ dom(C)` has a provenance entry) | J0 + J1★ at boundary: J0 places `a ∈ ran(M'(d))` at content-subspace position; J1★ supplies `(a, d) ∈ R'` | After K.α before K.μ⁺/K.ρ: `a ∈ dom(C')` but no `(·, d)` entry in R' |
