...
- **Case (ii) ¬Node(e).** `e = inc(t, k)` for some operand `t` and `k ∈ {0, 1, 2}`. The case-level "where"-clause conjuncts `e ∉ E ∧ T4-valid(e) ∧ ¬Element(e)` apply uniformly to all three sub-cases. The freshness conjunct `e ∉ E` is a single caller-checked guard — checked against the present state Σ, not derived from a structural fact — read uniformly with `e = inc(t, k)`; at k = 0 it is the frontier check `inc(t, 0) ∉ E`, which FrontierEquivalence characterizes as the condition that `t` is the frontier of its chain. Required uniformly: `parent(e) ∈ E`. Per-sub-case additional requirements:
