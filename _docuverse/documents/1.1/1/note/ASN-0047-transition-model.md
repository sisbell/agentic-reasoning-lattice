...
- `|dom_C(M(d))| ≥ 2` — necessary and sufficient for clause (ii) `π ≠ id` to admit a witness. The two directions enter the argument independently and are kept separate here so the reader sees what each consumes:
  ...
  - *Sufficiency.* When `|dom_C(M(d))| ≥ 2`, an admissible π always exists — concretely, the full-clearance form (`n'_{s_C} = 0`) of the K.μ⁻ + K.μ⁺ decomposition realises *every* admissible π without per-π precondition checks (established at *Decomposition of K.μ~* below, under the *Non-determinism of K.μ~ realisations* paragraph). Sufficiency thus points forward to the full-clearance form's universal admissibility — not to a fresh existence argument here.
...
*L14 (StoreDisjointness).* Derived at each state from L0 + SC-NEQ + T7 per the *Link store and extended system state* section.
