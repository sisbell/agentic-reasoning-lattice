...
> *V9(b), ownership* — `ω'(v) = π`: `pfx(π) = 1.1.0.2` covers `v`, and no account-tier prefix (`zeros ≤ 1`, O1a) covers `v` more specifically (the only longer prefix, `1.1.0.2.0.1 = v` itself, is document-tier), so `π` is the maximal coverer (O2). The fork is the forker's outright, the source's permissions never read (P-src is the whole source-side precondition).
