...
> (iv) *Incumbency* — every principal occupies a baptized entity: `pfx(π) ∈ E` for every `π ∈ Π`. An ASN-0042-conforming structure satisfies PrefixBaptismCoupling (`pfx(π) ∈ Σ.B`); since `pfx(π)` is account-tier (`zeros(pfx(π)) ≤ 1`, O1a) it is entity-level, so `pfx(π)`, a baptized entity-level address, is an entity of `E`. An account-tier principal (`zeros(pfx(π)) = 1`) thereby occupies an `Account` entity carrying a document sub-allocator `A_doc` (ASN-0047) — the namespace its cross-owner forks descend into. Incumbency is the `Π → E` direction; the converse `E → Π` coverage (`ω` total) is derived next.
