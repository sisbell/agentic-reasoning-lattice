...
Identity clause
  ...
  []  ω(d_src) ≠ π  →  v := the document identity π allocates as a single
                        ...
                        namespace by PS incumbency); the node-tier case lies outside
                        the operation's domain (P-tier). Validity and freshness of
                        this K.δ are discharged at V-WF; the stream form
                        [pfx(π), 0, k] and its consequences — Document(v),
                        pfx(π) ≼ v (O5(i)), and the maximality (O5(ii)) — at V9.
