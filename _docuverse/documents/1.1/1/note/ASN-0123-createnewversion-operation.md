...
> (iv) *Incumbency* — every principal occupies a baptized entity: `pfx(π) ∈ E` for every `π ∈ Π`. An ASN-0042-conforming structure satisfies PrefixBaptismCoupling (`pfx(π) ∈ Σ.B`); since `pfx(π)` is account-tier (`zeros(pfx(π)) ≤ 1`, O1a) it is entity-level, and the entity-level `B`=`E` identification (below) places it in `E`. An account-tier principal (`zeros(pfx(π)) = 1`) thereby occupies an `Account` entity carrying a document sub-allocator `A_doc` (ASN-0047) — the namespace its cross-owner forks descend into. Incumbency is the `Π → E` direction; the converse `E → Π` coverage (`ω` total) is derived next.
...
Identity clause
  ...
  []  ω(d_src) ≠ π  →  branch requires zeros(pfx(π)) = 1 — π already holds a
                        document-creation namespace (its account, pfx(π) ∈ E by PS, with
                        ...
                        v := the document identity π allocates in it as a single
                        document-level K.δ: allocated_by(π, v), with v ∈ S(pfx(π), 2),
                        π's document sub-allocator (A_doc, ASN-0047). Every member of
                        that stream has the form [pfx(π), 0, k] (ASN-0040), whence
                        Document(v) and pfx(π) ≼ v (O5(i)), and — proved
                        structurally at V9 from that form with O1a and Z-mono, not
                        imported as a stipulation — the maximality
                        (A π'' ∈ Π : pfx(π'') ≼ v ⟹ #pfx(π'') ≤ #pfx(π)) (O5(ii)).
                        Freshness v ∉ E is not the stream form but the frontier
                        choice: the K.δ allocates A_doc's frontier, fresh by
                        ChildSpawnFreshness (first π-document, v = inc(pfx(π), 2))
                        or FrontierEquivalence (later π-document, v = inc(c, 0) off
                        the prior frontier c), both ASN-0047 — the document-namespace
                        counterpart of the owned branch's nextv/VN-B1. Its operand
                        precondition parent(v) ∈ E holds too: parent(v) = pfx(π) by
                        K.δ-ID.parent-2 with PS incumbency, or parent(c) by
                        K.δ-ID.parent-0/1 with P8. The sibling is thus pinned to the
                        fresh frontier — what v ∉ E rests on; only which document
                        number k it carries stays out of scope, the guarantees
                        turning on the form [pfx(π), 0, k] for any k ≥ 1, not the
                        value                            (fork across ownership)
  fi
...
> *Downward* — the *owned* (address-discoverable) versions of `d` are the registry query `E ∩ S(d, 1) = {c₁, …, c_{hwm}}`, gap-free (VN-B1), so enumeration terminates at the first absentee; the full owned-descendant set `{e ∈ E : d ≺ e}` is T1-contiguous (T5), a single range scan of the ordered registry — and every address-encoded descendant of `d` is owned by `ω(d)`, since no account-tier prefix (`zeros ≤ 1`, O1a) can cover past `d` (`zeros = 2`, Z-mono). Cross-owner versions are *not* recovered here: a `VERSION(π, d)` with `π ≠ ω(d)` yields a first-class version `v` — `derives(v, d)` holds by definition, it issued from a `VERSION(·, d)` call — yet `¬(d ≼ v)` by severance (V9), so `v` falls in neither `S(d, 1)` nor `{e : d ≺ e}` and no address-based descendant scan reaches it. This is the downward face of severance, the standing limit flagged in Open Question 2: downward navigation by address enumerates exactly `ω(d)`'s versions, and a cross-owner fork's derivation is recoverable only through the shared-content witness (V9w), never the registry.
...
> The maximality `π`'s severance theorem consumes is therefore *derived* from ASN-0047's document-allocator structure (`A_doc(pfx(π)) = S(pfx(π), 2)`) together with O1a and Z-mono, not asserted; the placement detail the identity clause left out of scope (which `k`) is genuinely not needed, since the form `[pfx(π), 0, k]` holds across the whole stream and the argument turns only on the separator at `#pfx(π) + 1`. (`allocated_by(π, v)` also makes the O5 axiom applicable, yielding the same O5(i)/(ii); we give the structural proof because it does not lean on PS's stipulation that O5 governs every K.δ.) Then:
...
*Proof.* `L' = L` and each link's slot coverage is transition-invariant (LP2, LP3), so the right-hand side may be read at `Σ`. `ran(Σ'.M(v)) = A` (V2). LP12's per-slot biconditional — `project(a, i, d, Σ') ≠ ∅ ⟺ coverage ∩ ran(Σ'.M(d)) ≠ ∅` — instantiated at `d = v` gives the claim. ∎
...
> *V0 / V1, single mint* — `E' = E ∪ {v}` with `v` fresh, and `C' = C ∧ L' = L`. Because `π`'s account `1.1.0.2 ∈ E` already carries its document sub-allocator (PS incumbency), the fork is one document K.δ: no intermediate account is baptized, and the registry grows by the single identity `v` — exactly as the owned branch grows by one version, never an account-plus-version pair.
...
1. **Bounded fork depth.** `NPLACES = 16` caps the tumbler mantissa; `tumblerincrement` aborts fatally on overflow, bounding fork depth at roughly ten levels above a six-component document. The comment "increased from 11 to support deeper version chains" records that the previous bound was hit in practice — the cap is not hypothetical. This violates V6's unbounded-depth requirement (T0(b)). The defect is the *existence* of a fixed cap, not its current value: raising `NPLACES` (as the `11 → 16` history did) postpones the fatal overflow but does not confer conformance, since the renumber-or-refuse dilemma recurs at the new bound. A conforming implementation must *remove* the cap (variable-length tumblers), not merely enlarge it.
