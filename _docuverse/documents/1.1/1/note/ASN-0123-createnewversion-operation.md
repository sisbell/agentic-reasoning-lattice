...
> Let `π_o := ω(d_src) ≠ π`. By V-WF, the cross-owner fork allocates `v = nextd(E, π)` as a *single* document-level K.δ in `π`'s account document sub-allocator `A_doc(pfx(π)) = S(pfx(π), 2)` (ASN-0047, AllocatorHierarchy) — `allocated_by(π, v)` with `v ∈ S(pfx(π), 2)` — carrying `Document(v)` and `zeros(v) = 2`, so `v ∈ E_doc`. One step from that membership gives the positional form V9's arguments rest on: the depth-2 sibling stream contributes `2 − 1 = 1` separator zero (ASN-0040, `S(p, d)`), so `v = [pfx(π)₁, …, pfx(π)_{#pfx(π)}, 0, k]` with `k ≥ 1` — `pfx(π)` extended by one separator zero at position `#pfx(π) + 1` and one nonzero ordinal. The two facts the severance and ownership claims consume now follow *structurally*, discharging what the identity clause deferred:
