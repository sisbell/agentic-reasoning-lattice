...
**Depends:** ASN-0047 (Transition Model), ASN-0086 (Typed Relations on Address Sets), ASN-0093 (Allocation Substrate), ASN-0126 (Substrate Shape Framework), ASN-0128 (Substrate Type Operational Semantics).

We are handed a substrate whose entire dynamics is a *sequence*. The governing axiom — `SequentialTransitionAxiom`, asserted identically in ASN-0047 and ASN-0093 — says that transitions `Σ → Σ'` are atomic, uninterruptible, and totally ordered, and that `Σ →* Σ'` is the reflexive-transitive closure of single steps. The operation surface of ASN-0128 is defined the same way: `Emit_K`, `Nullify_Binary`, `Observe_K` are specified by what one step does to one state. And yet every layer raised on this substrate presumes a crowd. Predicates are evaluated by *any* observer (ASN-0129). Quiescence must be recognizable *while writers may still be active* (ASN-0133). A definition's content must be allocated as one contiguous run *even as other agents allocate* (ASN-0130). The very corpus this note belongs to was produced by a runner driving many workers against one store. So the question is forced upon us: a definition written in the singular is being *used* in the plural, and we must say exactly what survives the translation.
...
- **The atomic unit is the step.** A single `→_sh` step is the grain of all-or-nothing. A typed emission (`Emit_K` miss, `Nullify_Binary` miss) is *exactly one* step; an idempotent hit and an `Observe` are *zero* steps. A fire/batch (a `retract_stale`, a fork, a definition's content run) is *many* steps and is **not** atomic. (A0–A5.)
...
- **Linearization is by construction, and it is local.** Each state-changing operation appears to take effect at one index — its linearization point. The model is globally serial, but the global order is *necessary only within each home*; cross-home steps commute, so a per-home-serial realization is equivalent to a global one. (A1, A2, H0–H2, G0, G1.)
...
- **A minimal isolation contract** with six clauses, and a theorem that under it no reader sees a torn or duplicated effect and no writer collides with a concurrent writer's allocation. (MIC, M1.)
...
in which each `Σ_i → Σ_{i+1}` is one atomic step `σ_i` drawn from the step vocabulary `K = {K.σ, K.α, K.λ_sh, K.δ, K.μ⁺, K.μ⁺_L, K.μ⁻, K.ρ}`. Write `idx(σ) = i` for a step's position. We index states by their position in this one order; `Σ_i →* Σ_j` holds exactly when `i ≤ j`. This sequence is not a description of how an implementation is built — Gregory's loop builds it one way, a future per-home scheduler might build it another — it is the *meaning* the implementation must present. The agents, the scheduler that places their proposals, the fairness with which it does so: all of that is upstream of `𝔼` and out of scope here (it is ASN-0133's named hypothesis, the implementer's territory). What we reason about is `𝔼` and the states on it.
...
A *batch* (we use the words *fire* and *batch* interchangeably) is a finite collection of operations issued as one logical act by one agent or one rule firing — a `retract_stale` issuing one `Nullify_Binary` per stale event (ASN-0128 BH4, which states outright it is "a sequence of wrapper steps, not an atomic operation"), a fork composite (ASN-0047 J4: a `K.δ` then a `K.μ⁺` then per-address `K.ρ` steps), a definition's content run (ASN-0130: one `K.α` per atom). Its constituent steps land at some indices `i₁ < i₂ < ⋯ < i_m` in `𝔼`. Call the batch **contiguous** when `{i₁,…,i_m} = {i₁, i₁+1, …, i₁+m−1}`, i.e. no foreign step interleaves; otherwise **split**.
...
So the disjunction in the headline question — "only committed state, or can it witness a partial effect?" — resolves to the first horn for any single emission or nullification: **only committed state**. An `Emit_K` "in flight" is, from the snapshot's vantage, either not-yet-linearized (its step has not happened; the reader sees the pre-state) or linearized (its step has happened; the reader sees the full post-effect). "In flight" never means "half-applied," because there is no half-state to be in. This is the formal core of Nelson's intent that "a torn read is impossible": content lives at permanent addresses and is never rewritten in place, so the only thing a step changes is which addresses are *present* and how they are *arranged* — and that change is the step itself, indivisible.

The one honest qualification is A5's: while no *step* is ever torn, a *batch* is not protected. A reader can land between the constituent emissions of a `retract_stale` and see some events retracted and others not; it can land mid-fork and see a new version's arrangement half-populated. We must therefore characterize *what kind of state* a mid-batch snapshot is, so that a reader who lands there is not misled about its validity. Here ASN-0047 gives us exactly the distinction we need. Its `ExtendedReachableStateInvariants` theorem partitions the invariant set into *per-state* invariants — `S3★`, `D-CTG★`, `D-MIN★`, `D-SEQ★`, the partition and structural invariants, the chain conformances — which hold at *every* state reachable by elementary steps, and *composite-boundary* properties — `P4★` (provenance bound), `P4a`, `P7a` (coverage) — which hold *only* at composite boundaries. Let us name the per-state package:

> **A6 (CanonicalState).** Call a state *structurally canonical* iff it satisfies the per-state invariant package of `ExtendedReachableStateInvariants`. *Every* state on `𝔼` is structurally canonical, because each per-state invariant is preserved by every single step. The composite-boundary properties (the coupling that ties provenance to containment) hold only at the boundaries of valid composites.

A6 is the formal reading of Nelson's "canonical order is an internal mandate of the system" — the property that "all changes, once made, left the file remaining in canonical order," with *nothing* canonical in between. There is nothing in between because there is no state in between (A0); and every state there *is* satisfies the structural package (A6). So a reader who lands mid-batch sees a state that is *structurally* sound — addresses partitioned correctly, arrangements contiguous and well-ordered, chains conformant — but that may be *coupling-incomplete*: a freshly allocated content address may not yet have its provenance recorded if the batch's `K.ρ` step has not yet fired. This is the precise and only sense in which an observation can be "incomplete." It is never *corrupt*; it is at worst *mid-coupling*. That is the substrate's actual read-isolation guarantee, and it is exactly the guarantee Nelson left as "a real, correctly-open gap" at the multi-step level while the single-step level was closed by construction.
...
>
> *Proof.* An `S`-allocation to `d` adjoins one fresh address with `origin = d` and subspace `S` (ASN-0093 K.α/K.λ effect), so `|P_S(d, ·)|` rises by one. Any other step either frames `dom_S` (`K.σ`, `K.μ`, `K.ρ`, `K.δ` leave the relevant store fixed) or adjoins an address with `origin = d' ≠ d` or subspace `S' ≠ S` (an allocation to a different home or the sibling subspace), which by `DisjointSubAllocatorChains` and `CrossDocumentDisjointness` is not a member of `P_S(d, ·)`. In every case `P_S(d, ·)` is unchanged. ∎
...
>
> *Proof.* `SubsequentEmissionFreshness` derives a fresh address from `a = inc(a_prev, 0)` with `a_prev = max P_S(d, Σ_pre)`. Two emissions with the same pre-state population read the same `a_prev`, hence compute the same `a`. Freshness `a ∉ dom_S` then fails for the second. ∎
...
We can now state the equivalence that liberates an implementation from global serialization:

> **G1 (PerHomeSerializabilitySuffices).** Let `𝔼` be any realization in which, for every `(d, S)`, the `S`-allocations to `d` are totally ordered among themselves (per-home serialization), each step is applied atomically (A0), and observations read committed states (A3). Then `𝔼` preserves every per-state invariant at every committed state and every allocation invariant in its final state, *as if* globally serialized.
>
> *Proof.* Per-state invariants: each is preserved by every single step (A6), so they hold at every state of `𝔼` regardless of interleaving — global order is never invoked. Allocation invariants (uniqueness, dense chain contiguity): these depend only on each allocation reading the correct frontier, which per-home serialization guarantees by H0 (within `(d,S)` the frontier is single-valued per emission) and which cross-home concurrency cannot disturb by H1 (foreign allocations leave the frontier fixed). Any such `𝔼` can be transformed into a global serial order by repeatedly transposing adjacent cross-home steps (H1, commuting) without changing any committed state's content; the sequential model's invariants hold in that serial order (ASN-0047, ASN-0093) and transfer back unchanged. ∎

G1 is the practical payload. An implementer is *not* obliged to run Gregory's single global loop. They may let agents working in different homes proceed with no coordination whatsoever, and serialize only within a home. The result is observationally indistinguishable, for all of the substrate's invariants, from the global order the axiom postulates. Gregory's loop is *a* faithful realization — the maximally serial one — not the *required* one. And this is exactly Nelson's distributed intent (each server "at all times unified and operational," progress never gated on a global agreement) rescued from the implementation's incidental global lock.
...
> **W0 (MonotonicityStepLocal).** Append-only monotonicity and value-immutability — `P0` (ContentPermanence), `C0`, `L12` (LinkImmutability) — are model-intrinsic. Each step's effect either adjoins a fresh key or frames the store; no step removes a key or rewrites a value. So under any interleaving of atomic steps, `dom` only grows and existing values never change. Needs A0, nothing more.
...
ASN-0130 asks for something the bare frontier discipline does not give: a definition's content must occupy *one contiguous run* of addresses, "even as other agents allocate." We must locate this requirement exactly, because it is stronger than W3 and weaker than a global lock.
...
## 7. Quiescence as a snapshot predicate

We come to the sharpest of the questions, and the one Nelson answered most directly: what does a quiescence verdict mean if it is read while a write is in flight? ASN-0133's Q0 wants quiescence — "is everyone done editing?" — recognizable from inside the predicate language even as writers remain active. We must say when such a verdict is *sound* and when it is a *race*.
...
> **M1 (SafetyUnderMIC).** Under MIC, (a) no reader observes a torn effect, (b) no reader observes a duplicated effect, and (c) no writer collides with a concurrent writer's allocation.
>
> *Proof.* (a) *No torn effect.* Any single emission or nullification is one step; by clause 1 (A0) the reader's state is before or after it, never within (A4). A batch is not a single effect; its constituents are each whole, and clause 4 ensures each read is one canonical snapshot (A6) — structurally sound, at worst mid-coupling, never corrupt. (b) *No duplicated effect.* Each state-changing operation is realized at one unique index (A1), so its effect appears once in the order; idempotent types collapse semantic repeats to zero-step hits (ASN-0128 I1); and sharing is by reference to one permanent address, never by copy (ASN-0086, Nelson's correspondence invariant), so "the same content" is structurally one thing that cannot be counted twice. Clause 3 forbids an at-least-once retry from committing a second time before acknowledgment. (c) *No allocation collision.* Two allocations conflict only when into the same `(d, S)` (H1, H2); clause 2 serializes exactly those, so each reads a distinct frontier (H0) and deposits a fresh, unique address (W2); clause 5 extends this to whole runs (W4). Cross-home writers, by H1, never collide and need no coordination. ∎
...
- **The scheduler and its fairness.** *Which* agent's proposal is placed at *which* index — the placement that turns a crowd of proposals into the order `𝔼` — is the implementer's and protocol layer's concern. ASN-0133 names fairness as a hypothesis for termination; we name it as the source of `𝔼` and reason about `𝔼` itself, not its construction. No fairness, starvation-freedom, or priority property is asserted here.
- **Agent activation and rule bodies.** What makes an agent fire, and what its firing computes, are opaque (ASN-0133's RG leaves bodies opaque). We treat a fire only as a batch of operations with linearization points; its internals are not modelled.
...
- **Predicate evaluation cost.** The cost of evaluating a verdict's predicate — ASN-0129's deferred territory — is not bounded here; we constrain only *when* (one snapshot) it is evaluated, not *how expensively*.
...
| A6 | CanonicalState — every state on `𝔼` is structurally canonical (per-state invariants); coupling-completeness holds only at composite boundaries | introduced |
...
| W0 | MonotonicityStepLocal — `P0`/`C0`/`L12` are model-intrinsic (need only A0) | introduced |
...
| G1 | PerHomeSerializabilitySuffices — a per-home-serial, commit-after realization is equivalent to global serialization for all invariants | introduced |
...
| M1 | SafetyUnderMIC — under MIC: no torn read, no duplicated effect, no same-home allocation collision | introduced |
...
What must an implementation guarantee so that a split batch never leaves a reader at a coupling-incomplete state that a higher layer mistakes for a final one — must coupling-completing steps (provenance recording) share their predecessor's critical section?

When a layer genuinely needs batch atomicity (a fork seen whole, a retraction set applied all-or-nothing), what is the minimal additional contract that makes a multi-step batch appear atomic without reintroducing global serialization?
