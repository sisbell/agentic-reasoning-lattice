...
*At-least-one.* T4's axiom gives the upper bound zeros(t) ≤ 3 (T4, ASN-0034). The complementary lower anchor is `0 ≤ zeros(t)`, and this does not come from any order axiom: none of the cited NAT axioms identifies 0 as the least natural — NAT-wellorder (ASN-0034) asserts that a least element exists without naming it, NAT-closure (ASN-0034) makes 0 the additive identity rather than the order-minimum, and NAT-order (ASN-0034) supplies only trichotomy/transitivity/irreflexivity. We therefore record the anchor as an explicit stipulated premise on the counting function rather than derive it:

> **Z0 (cardinality non-negativity).** zeros(t) is defined (T4, ASN-0034) as the cardinality `|{i : 1 ≤ i ≤ #t ∧ tᵢ = 0}|` of a finite index set. We stipulate, as the defining property of counting, that every such cardinality lands in ℕ with `0 ≤ zeros(t)`.

Z0 is a premise, not a theorem of the foundation, and we are deliberate about why. Deriving `0 ≤ n` for all n ∈ ℕ from the cited axioms would require a well-ordering or induction argument that *identifies* 0 as ℕ's minimum — but NAT-wellorder yields only an unnamed least element, and NAT-closure casts 0 as the additive identity, neither of which pins the minimum to 0. Nor do we route the bound through cardinality monotonicity `∅ ⊆ S ⟹ |∅| ≤ |S|`: that path presupposes the very ℕ-order fact at issue and is circular. Z0 instead stipulates `0 ≤ zeros(t)` directly, as a property of what zeros(t) counts, and the case split below cites Z0 wherever the lower bound is used. We must now conclude zeros(t) ∈ {0, 1, 2, 3}, i.e. that `{n ∈ ℕ : 0 ≤ n ≤ 3} = {0, 1, 2, 3}`. Order plus the two bounds does not by itself suffice: a merely totally-ordered carrier (order-isomorphic to ℚ≥0 restricted to [0, 3]) admits infinitely many elements in range. What excludes intermediate values is NAT-discrete (ASN-0034), `m ≤ n < m + 1 ⟹ n = m`, together with its derived form `m < n ⟹ m + 1 ≤ n` (obtained from NAT-discrete and NAT-order's trichotomy, as in TA-Pos's derivation). We walk the interval by successive case split, anchored at the lower bound `0 ≤ zeros(t)` (Z0). By NAT-order's trichotomy applied to zeros(t) and 1: if `zeros(t) < 1`, then `0 ≤ zeros(t) < 0 + 1` (the left bound from Z0) and NAT-discrete (m = 0) gives zeros(t) = 0; if `zeros(t) = 1`, the case is immediate; if `1 < zeros(t)`, the derived form gives `2 ≤ zeros(t)`, and trichotomy applied to zeros(t) and 2 splits again — `zeros(t) = 2` is immediate, while `2 < zeros(t)` yields `3 ≤ zeros(t)` by the derived form, which with T4's bound `zeros(t) ≤ 3` forces zeros(t) = 3 by antisymmetry of `≤`: were `zeros(t) ≠ 3`, then `3 ≤ zeros(t)` would give `3 < zeros(t)` and `zeros(t) ≤ 3` would give `zeros(t) < 3`, and NAT-order's transitivity (ASN-0034) composes these into `3 < 3`, contradicting NAT-order's irreflexivity (ASN-0034). (The remaining branch `zeros(t) < 2` is excluded, as it contradicts `2 ≤ zeros(t)`.) Every branch lands on one of 0, 1, 2, 3, so zeros(t) ∈ {0, 1, 2, 3}: at least one of the four equalities zeros(t) = k holds, so at least one of the four predicates holds at t.
...
|-------|-----------|--------|
| Z0 | `0 ≤ zeros(t)` — cardinality non-negativity of the T4 zeros count | stipulated premise (not derived from NAT-order) |
