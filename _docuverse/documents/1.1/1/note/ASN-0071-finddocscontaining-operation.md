...
We accept the latter. A **vspec** is a pair `(d_s, σ)` where `d_s` is a document address naming a source document and `σ = (u, ℓ)` is a level-uniform V-span confined to the content subspace — `subspace(u) = s_C`, `Pos(ℓ)`, `actionPoint(ℓ) = #u`, `#ℓ = #u` (level-uniformity, ASN-0053 S6), and `actionPoint(ℓ) ≥ 2` (equivalently `ℓ₁ = 0`: the displacement does not perturb the subspace identifier at position 1; together with `actionPoint(ℓ) = #u` this also forces `#u ≥ 2`). A vspec is exactly ASN-0058's ContentReference `(d_s, σ)` minus three of its clauses: we drop well-formedness (the requirement that every depth-`m` position in the span's range belong to `dom(M(d_s))`), the depth-pinning clause (iii) `#ℓ = #u = m`, which fixes the span depth to the source's content-subspace depth `m_C`, and the non-emptiness clause (i) `V_{s_C}(d_s) ≠ ∅`, so a vspec may name a source whose content subspace is empty. Its denotation `⟦σ⟧` and reach `u ⊕ ℓ` used throughout are ASN-0053's (σ.denotation: `⟦σ⟧ = {t : start(σ) ≤ t < reach(σ)}`; σ.reach: `reach(σ) = start(σ) ⊕ width(σ)`); we apply those definitions rather than restate them below. A **vspec-set** is a finite set `Q = {q₁, q₂, ..., q_k}` of vspecs, possibly drawn from multiple source documents.
...
6. K.δ creates document `d_B = inc(d_A, 0) ∈ E_doc` by case (ii) sibling (`k = 0`), operand `d_A ∈ E` with `¬Node(d_A)`. The *sibling-creation discharge*: `parent(d_B) = parent(d_A) = acct ∈ E` (K.δ-ID.parent-0) discharges P8, and `zeros(d_B) = zeros(d_A) = 2` (K.δ-ID.zeros-0), so `Document(d_B)`.
...
9. K.δ creates a third document `d_C = inc(d_B, 0) ∈ E_doc` by case (ii) sibling (`k = 0`), operand `d_B`; by the same discharge as step 6, `Document(d_C)` (activates `A_C(d_C)`).
...
12. K.δ creates a fourth document `d_D = inc(d_C, 0) ∈ E_doc` by case (ii) sibling (`k = 0`), operand `d_C`; by the same discharge as step 6, `Document(d_D)`.
...
*Reachability.* Each step above is a standard allocate–place–record (and create-document) composite of ASN-0047 — entity creation followed by content allocation, placement, and provenance recording — with its precondition discharged by the prior state. A reachable state extended by such composites remains reachable and continues to satisfy ASN-0047's invariants, so `Σ` is reachable.
...
14. K.δ creates `d_E = inc(d_D, 0) ∈ E_doc` by case (ii) sibling (`k = 0`), operand `d_D`; by the same discharge as step 6, `Document(d_E)` (activates `A_C(d_E)`).
...
Steps 14–15 are again standard allocate–place–record composites with preconditions discharged by the prior state, so `Σ⁺` — like `Σ` — is reachable. The extended state adds `Σ⁺.M(d_E) = {[s_C,1,1] ↦ a₁, [s_C,1,2] ↦ a₂, [s_C,1,3] ↦ a₁}`, leaving every other arrangement as in `Σ`.
...
| F-find | `find : VSpecSet × Σ ⇀ P(E_doc)` with `find(Q)(Σ) = { d ∈ Σ.E_doc : ran(Σ.M(d)) ∩ iaddrs(Q)(Σ) ≠ ∅ }`, defined under the precondition `(A (d_s, σ) ∈ Q :: d_s ∈ Σ.E_doc)` | definition | introduced |
...
| F-DEEP | Deep-anchor empty resolution: for a vspec `(d_s, σ)` with `σ = (u, ℓ)`, `V_{s_C}(d_s) ≠ ∅ ∧ #u > m_C` (the source's content-subspace depth, S8-depth) `⟹ iaddrs_one(d_s, σ)(Σ) = ∅` | PC-RANGE depth guard + S8-depth (`#v = m_C < #u` for every content-subspace `v ∈ dom(M(d_s))`) excludes every position via the `#v < #u` case | introduced |
