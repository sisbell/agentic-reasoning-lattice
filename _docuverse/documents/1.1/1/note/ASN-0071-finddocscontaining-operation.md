...
`find(Q)(Σ)` is defined precisely when this precondition holds at the evaluation state `Σ`; entity permanence (P1, ASN-0047: `Σ.E ⊆ Σ'.E`) preserves `d_s ∈ E_doc` forward, so a source document available when the vspec was formed remains so at any later `Σ`. When the precondition holds, every `Σ.M(d_s)` named in `iaddrs(Q)(Σ)` is a defined arrangement and the resolution of the previous section applies unchanged.
...
- *Home/transcluding recovery.* `origin(a₁) = d_A` (grounded in `E_doc` by ASN-0047 P6, ExistentialCoherence). Of the discovered documents, `d_A = origin(a₁)` is `a₁`'s home, while `d_B` and `d_D` — each with `d ≠ origin(a₁)` — transclude it.
...
This non-distinction is recoverable from the address structure already returned, so `find` need not tag its results: for each `a ∈ iaddrs(Q)`, `origin(a)` (a function of `a`'s tumbler alone, grounded in `E_doc` by ASN-0047 P6) names `a`'s home document, and comparing it against each `d ∈ find(Q)` recovers the relationship — `d = origin(a)` means `d` authored `a`, `d ≠ origin(a)` means `d` transcludes `a`.
...
Recovering the *historically*-containing set — every document that ever referenced `a` — is a separate concern. `find` does not consult ASN-0047's provenance relation `R`, which records `(a, d)` permanently (P2); the two have different semantics — `find` returns currently-containing documents, an `R`-based query returns ever-containing ones, and they coincide only when no arrangement contraction has touched a queried I-address.
