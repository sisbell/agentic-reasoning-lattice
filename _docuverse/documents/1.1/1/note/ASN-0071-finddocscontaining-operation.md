...
The companion restriction `actionPoint(ℓ) ≥ 2` enforces *subspace confinement* of the entire span. Its role is sharp, and worth isolating precisely because the two standing preconditions `actionPoint(ℓ) = #u` and `#ℓ = #u` already do most of the work: under `actionPoint(ℓ) = #u`, an action point of 1 can arise *only* at a depth-1 anchor `#u = 1`, so the sole thing `actionPoint(ℓ) ≥ 2` adds is the floor `#u ≥ 2` — it excludes depth-1 anchors and nothing else. (A displacement with `actionPoint(ℓ) = 1` at a deeper anchor, such as `u = [1, 5]` with `ℓ = [2, 0]` where `actionPoint = 1 ≠ 2 = #u`, is *already* rejected by `actionPoint(ℓ) = #u` and so cannot witness what `actionPoint(ℓ) ≥ 2` contributes.) The depth-1 anchor is exactly the case that perturbs the subspace identifier. Take `u = [s_C] = [1]` with `ℓ = [2]`: it satisfies `Pos(ℓ)`, `#ℓ = #u = 1`, and `actionPoint(ℓ) = 1 = #u`, so it clears every vspec precondition except `actionPoint(ℓ) ≥ 2`, yet its action point falls on position 1 itself. Its reach is `u ⊕ ℓ = [3]` (position 1 is the action point, summing `1 + 2 = 3`), so the span `⟦σ⟧ = {t : [1] ≤ t < [3]}` contains `[2]` — and every `[2, …]` — whose subspace identifier is `2 = s_L`. Such a span straddles the content and link subspaces in `d_s`'s V-space, conflating two transclusion regimes that the operation deliberately separates. Requiring `actionPoint(ℓ) ≥ 2` forces `#u ≥ 2`, placing the action point at position 2 or beyond, so by TumblerAdd's prefix-copy region position 1 of every `t ∈ ⟦σ⟧` equals `u₁ = s_C` — the entire span lives in the content subspace. We rely on this property in the codomain argument below.

A vspec is structurally a relaxation of ASN-0058's `ContentReference`. ContentReference additionally requires well-formedness — every depth-`m` position in `⟦σ⟧` belongs to `dom(M(d_s))` — together with `V_{u₁}(d_s) ≠ ∅` and `#u = m` (the common depth of `d_s`'s text-subspace V-positions per S8-depth). The vspec drops all three: it admits spans whose positions may not all be currently arranged, whose source subspace may be empty in `d_s`, and whose depth may differ from `d_s`'s common depth. The relaxation makes the query total over well-typed inputs; resolution silently filters anything that does not match a current arrangement entry (justified below). What the vspec retains from ContentReference is subspace confinement — well-formedness implies `actionPoint(ℓ) = m` via C0 (the action point equals the common subspace depth), and `m ≥ 2` via the S8a lower bound on subspace depth (the same bound C0a names as a precondition, S8-depth fixing the common depth). C0 establishes only the equality `actionPoint(ℓ) = m`; the `≥ 2` half is the depth bound, not a consequence of C0. We lift both consequences into an explicit precondition since the well-formedness they were derived from is no longer available. C0's equality `actionPoint(ℓ) = m` becomes the precondition `actionPoint(ℓ) = #u` — the action point sits at the span's *own* deepest component, with `#u` standing in for the common depth `m` the vspec no longer references. This is strictly tighter than T12's well-formedness bound `actionPoint(ℓ) ≤ #u`, and the tightening is load-bearing: an action point *interior* to the span (`2 ≤ actionPoint(ℓ) < #u`, a non-empty band whenever `#u ≥ 3`) would let the displacement act on an interior prefix component, so `⟦σ⟧` would range across prefix structure and resolution would collect content positions the user never named — exactly the over-collection C0's `actionPoint = m` exists to prevent. F-FILT, the charitable relaxation, drops *unresolvable* positions; it does nothing about *extra resolvable* positions admitted by a coarse span, so confinement to a single prefix is the precondition's job, not resolution's. Pinning `actionPoint(ℓ) = #u` forecloses it. The claim we need — call it *prefix confinement* — is that every `t ∈ ⟦σ⟧` agrees with `u` on all components `1 ≤ j < #u`. ASN-0058's C0a states this conclusion, but only for a well-formed ContentReference; the vspec relaxation discards exactly that well-formedness (and admits the cross-depth case `#u < m` below), so we cannot borrow C0a and must argue directly. The argument is the position-1 reasoning of the Resolution section, run at every prefix position rather than just position 1: for any `j` with `1 ≤ j < #u = actionPoint(ℓ)`, position `j` lies strictly below the action point, so TumblerAdd's prefix-copy gives `(u ⊕ ℓ)_j = u_j` — `u` and `u ⊕ ℓ` share the whole prefix `1 ≤ j < #u`. Before comparing components we must establish that the prefix claim is even *total* over `1 ≤ j < #u` — that every `t ∈ ⟦σ⟧` has depth `#t ≥ #u`, so each `t_j` exists. This is the T1 *case (ii)* exclusion, distinct from the componentwise case (i) below: were `#t < #u`, then either `t` agrees with `u` on its whole length — making `t` a proper prefix of `u`, hence `t < u` by T1 case (ii), contradicting `u ≤ t` — or `t` first disagrees with `u` at some position `p ≤ #t < #u`, which the case-(i) argument below contradicts; either way `#t < #u` is impossible, so `#t ≥ #u` and `t_j` is defined for all `1 ≤ j < #u`. With the components in hand: were `t` to first disagree with `u` at some prefix position `p < #u`, then since `u_p = (u ⊕ ℓ)_p`, T1 case (i) at `p` would force either `t < u` (if `t_p < u_p`) or `t > u ⊕ ℓ` (if `t_p > u_p`), each contradicting `u ≤ t < u ⊕ ℓ`; by NAT-order trichotomy (T0) no such `p` exists, so `t_j = u_j` throughout `1 ≤ j < #u`. We name this *prefix confinement* (PC); it holds with no appeal to well-formedness, and its position-1 instance `t₁ = u₁` is the base case the Resolution section reuses for subspace confinement. By PC, `⟦σ⟧` varies only at component `#u` and deeper, so resolution reads exactly the prefix the user named.

To see the bound bite, consider a source `d_s` of common depth `m = 3` whose content positions are `V_{s_C}(d_s) = {[s_C, 1, k] : 1 ≤ k ≤ n}` (D-SEQ★). The coarse span `σ' = ([s_C, 1, 2], ℓ')` with `ℓ' = [0, 1, 0]` has action point `2`, interior to `#u = 3`; its reach is `[s_C, 1, 2] ⊕ [0, 1, 0] = [s_C, 2, 2]`, so `⟦σ'⟧ ∩ dom(M(d_s)) = {[s_C, 1, 2], [s_C, 1, 3], ..., [s_C, 1, n]}` — positions `2` through `n`, though the user named one. The `actionPoint(ℓ) = #u` precondition rejects `σ'` outright (`2 ≠ 3`). The well-formed alternative `σ'' = ([s_C, 1, 2], δ(1, 3))` with `ℓ'' = [0, 0, 1]` has action point `3 = #u`, reach `[s_C, 1, 3]`, and `⟦σ''⟧ ∩ dom(M(d_s)) = {[s_C, 1, 2]}` — exactly the named position. This is the `#u ≥ 3` case the depth-2 worked scenarios below cannot exercise: at `#u = 2` the band `2 ≤ actionPoint(ℓ) < #u` is empty, so `actionPoint(ℓ) = 2 = #u` is forced and the defect never arises.

The third relaxation — dropping ContentReference's `#u = m` requirement — admits a *cross-depth* collection that, at first inspection, resembles the over-collection the action-point precondition just foreclosed. Reusing the depth-`3` source above, suppose the user submits a *shallow* vspec `u = [s_C, 1]`, `ℓ = δ(1, 2)`, so `#u = 2 < m = 3` and reach is `[s_C, 2]`. Then `⟦σ⟧ = {t : [s_C, 1] ≤ t < [s_C, 2]}` contains *every* `[s_C, 1, k]`: by T1 case (ii) each is a proper extension of the prefix `[s_C, 1]`, and by T1 case (i) at position 2 each lies below `[s_C, 2]` since `1 < 2`. So `⟦σ⟧ ∩ dom(M(d_s))` is the *entire* depth-`3` subtree hanging under `[s_C, 1]` — `n` positions resolved from a span anchored at a single depth-`2` coordinate. Unlike the action-point case, these positions *are* current arrangement entries, so F-FILT offers no defense; the question is whether collecting them is a defect or the intent.

It is the intent, and the asymmetry with the action-point case is principled rather than an oversight. Nelson defines a span as a subtree whose contents are fixed implicitly by its endpoints: *"there is no choice as to what lies between; this is implicit in the choice of first and last point"* (LM 4/25). A coarse address deliberately stands for everything beneath it — *"a link to or search of an account or node will find any of the documents under it"* (LM 4/23) — and FINDDOCSCONTAINING is specified to return *"any portion of the material specified ... regardless of where the native copies are located"* (LM 4/63). Naming content at a depth-`2` anchor over depth-`3` leaves *embraces all the contents below*; the user who names the coarse coordinate names its whole subtree. Cross-depth collection is the *"prefix names subtree"* semantics, not over-collection.

What separates the two cases is precisely *which* component the displacement perturbs, and the `actionPoint(ℓ) = #u` precondition is what enforces the separation. With the action point pinned at `u`'s *own deepest* component `#u`, prefix confinement (PC, proven above) gives that every `t ∈ ⟦σ⟧` agrees with `u` on all components `1 ≤ j < #u`; the span varies only at position `#u` and *below*, so it captures the *descendants* hanging under the named coordinate range — its subtree. The interior-action-point case (`2 ≤ actionPoint(ℓ) < #u`) is the opposite: the displacement acts on a prefix coordinate the anchor fixes, so `⟦σ⟧` sweeps *sideways* across positions that diverge from `u` in a *middle* component — content at the user's own granularity that the anchor's prefix does not name. Descending into a subtree realizes "a coarse name means the whole subtree"; sweeping siblings across a fixed prefix collects content the user never named. The precondition forbids the breadth-wise sibling sweep while permitting the depth-wise subtree capture — exactly the discrimination Nelson's span semantics demand.
...
*Subspace confinement.* This is the position-1 instance of prefix confinement (PC), spelled out here for the routing argument. Fix `t ∈ ⟦σ⟧`. By the half-open interval definition, `u ≤ t < u ⊕ ℓ`. We argue `t₁ = u₁`. Position 1 lies strictly below the action point: by the vspec precondition `actionPoint(ℓ) ≥ 2`, we have `1 < actionPoint(ℓ)`. By TumblerAdd, the result of `u ⊕ ℓ` at any position `i < actionPoint(ℓ)` is copied from `u`: `(u ⊕ ℓ)₁ = u₁`. Now `u` and `u ⊕ ℓ` agree at position 1, so any `t` lying in the lexicographic interval between them must also agree at position 1 — were `t₁ < u₁`, then `t < u` by T1 case (i) at position 1, contradicting `u ≤ t`; were `t₁ > u₁ = (u ⊕ ℓ)₁`, then `t > u ⊕ ℓ` by T1 case (i) at position 1, contradicting `t < u ⊕ ℓ`. So `t₁ = u₁ = s_C` by NAT-order trichotomy on ℕ applied to the components `t₁, u₁ ∈ ℕ` (T0), hence `subspace(t) = s_C`.

*Routing.* Therefore every `v ∈ ⟦σ⟧ ∩ dom(Σ.M(d_s))` is a content-subspace V-position, and S3★ (ASN-0047) routes it: `Σ.M(d_s)(v) ∈ dom(Σ.C)`. The subset claim `iaddrs(Q)(Σ) ⊆ dom(Σ.C)` is read with `Σ` explicit on both sides — the right-hand side is the input state's content store, not a fixed set. Without the `actionPoint(ℓ) ≥ 2` precondition, a depth-1 anchor would place position 1 *at* the action point, where TumblerAdd's prefix-copy reasoning does not apply — and the counter-example `u = [s_C]`, `ℓ = [2]` exhibited above (the only shape `actionPoint(ℓ) = #u` still admits once `actionPoint(ℓ) ≥ 2` is dropped) would silently include a link-subspace V-position in the resolution if `d_s` arranges one.
...
- **Composite 2 (steps 6–8):** `d_B`'s creation (K.δ) and its transclusion of `a₁` (K.μ⁺, K.ρ). No I-address is freshly allocated — `a₁ ∈ dom(C)` already — so J0 is vacuous; `a₁` is range-new to `M(d_B)`'s content subspace, so J1★ forces `(a₁, d_B) ∈ R'` and J1'★ holds symmetrically.
...
- **Composite 4 (steps 12–13):** `d_D`'s creation (K.δ) and its three transcluding binds (K.μ⁺, K.ρ). No fresh allocation, so J0 is vacuous; both `a₁` and `a₂` are range-new to `M(d_D)`'s content subspace, so J1★ forces `(a₁, d_D), (a₂, d_D) ∈ R'`, and J1'★ matches each new provenance entry to its range-new I-address. The repeated bind of `a₁` at `w₃` adds no further range-new content, so it triggers no additional provenance obligation.

Each composite also satisfies clause (1) of ValidCompositeAmended — every atomic step's elementary precondition holds at the intermediate state, as narrated above. The state `Σ` is therefore reached by a finite sequence of valid composites, hence reachable, so ASN-0047's per-state and composite-boundary invariants hold at `Σ` — the foundation the home/transcluding recovery and F-FIN rely on.
...
These two steps form a fifth valid composite: no fresh allocation, so J0 is vacuous; `a₁` and `a₂` are range-new to `M(d_E)`'s content subspace, so J1★ forces `(a₁, d_E), (a₂, d_E) ∈ R'`, and J1'★ matches each. The extended state adds `Σ⁺.M(d_E) = {[s_C,1,1] ↦ a₁, [s_C,1,2] ↦ a₂, [s_C,1,3] ↦ a₁}`, leaving every other arrangement as in `Σ`.
