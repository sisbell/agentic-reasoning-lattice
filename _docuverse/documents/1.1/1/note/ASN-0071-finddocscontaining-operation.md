...
The third relaxation — dropping ContentReference's `#u = m` requirement — admits a *cross-depth* collection that, at first inspection, resembles the over-collection the action-point precondition just foreclosed. When the anchor depth `#u` is shallower than the source's common depth `m`, the span `⟦σ⟧` captures every arrangement position in the deeper subtree hanging under the named coordinate — many current entries resolved from a span anchored at a single shallow coordinate. Unlike the action-point case, these positions *are* current arrangement entries, so F-FILT offers no defense — but collecting them is the intended semantics.

A coarse anchor names its whole subtree. FINDDOCSCONTAINING is specified to return *"any portion of the material specified ... regardless of where the native copies are located"* (LM 4/63), so naming content at a depth-`2` anchor over a depth-`3` source embraces all the contents below — the user who names the coarse coordinate names its whole subtree. Cross-depth collection is the *"prefix names subtree"* semantics.

The `actionPoint(ℓ) = #u` precondition enforces exactly this discrimination: with the action point pinned at `u`'s own deepest component `#u`, prefix confinement (PC, proven above) confines `⟦σ⟧` to vary only at position `#u` and below, so it captures the depth-wise subtree of the named coordinate (permitted); an interior action point (`2 ≤ actionPoint(ℓ) < #u`) would instead perturb a prefix coordinate the anchor fixes, sweeping breadth-wise across siblings the anchor's prefix does not name (forbidden). The worked scenario below exhibits both against a live arrangement.
...
*Routing.* Therefore every `v ∈ ⟦σ⟧ ∩ dom(Σ.M(d_s))` is a content-subspace V-position, and S3★ (ASN-0047) routes it: `Σ.M(d_s)(v) ∈ dom(Σ.C)`. The subset claim `iaddrs(Q)(Σ) ⊆ dom(Σ.C)` is read with `Σ` explicit on both sides — the right-hand side is the input state's content store, not a fixed set. The `actionPoint(ℓ) ≥ 2` precondition is what licenses this position-1 step: the depth-1 anchor it excludes (exhibited in *The query*) places position 1 at the action point, where the prefix-copy reasoning does not apply.
...
- **Composite 4 (steps 12–13):** `d_D`'s creation (K.δ) and its three transcluding binds (K.μ⁺, K.ρ). No fresh allocation; `a₁` and `a₂` are both range-new to `M(d_D)`'s content subspace, so the transcluding-composite discharge of Composite 2 applies, forcing `(a₁, d_D), (a₂, d_D) ∈ R'`. The repeated bind of `a₁` at `w₃` adds no further range-new content, so it triggers no additional provenance obligation.
...
These two steps form a fifth valid composite: no fresh allocation, with `a₁` and `a₂` range-new to `M(d_E)`'s content subspace, so the Composite 2 discharge forces `(a₁, d_E), (a₂, d_E) ∈ R'`. The extended state adds `Σ⁺.M(d_E) = {[s_C,1,1] ↦ a₁, [s_C,1,2] ↦ a₂, [s_C,1,3] ↦ a₁}`, leaving every other arrangement as in `Σ`.
