...
The canonical shape catalog and its per-shape template families are hand-curated under three rules enforced by catalog-author diligence:

*(1) Per-shape uniformity convention.* Shape-mate rows in the catalog share template-body shape by author convention, not by framework derivation. A new catalog row at an existing shape must hand-curate its bodies against the shape-mate.

*(2) Signature derivation rule.* Template signatures derive mechanically from shape components (input/codomain symbols read from `t_F`/`t_G`); bodies do not.

*(3) Citation convention.* Catalog template bodies cite only: (i) shape-component-derived slot accessors (`from₁`, `to₁`, `from₁⁻`, `to₁⁻`, `from_K^Σ`, `to_K^Σ`); (ii) K's name; (iii) named scaffolding clauses (`chain_index`, `home(·)`, `s_L`); (iv) accessors exported by a registered per-K discipline (e.g., `emission_order` under SHCD); (v) meta-operators (logical, set-theoretic, arithmetic primitives); (vi) framework-internal base-machinery accessors (`A_K^Σ`, `addr(τ)`, `slot_addrs(·)`, `δ(1, #·)`). Categories (i)–(iv) are checked by reading the template body; (v) and (vi) are unrestricted.
...
We walk the canonical shapes and exhibit the predicate templates each generates. Each rejection case is introduced inline at its first appearance; the Comment walkthrough below derives the four bare-Sh-conf patterns (non-canonical slots, unallocated targets, cardinality mismatches, unregistered types) as canonical references.
...
*FDD subsumes Sh4 at FDD-registered K.* At FDD-registered K the layer runs only the FDD clauses (i)–(iii), with Sh4's clauses dormant. The Sh4 conclusion still holds on `A_K^Σ` by direct argument on the relation: take any distinct `τ ≠ τ' ∈ A_K^Σ`. By the FDD-discharged property's contrapositive (`addr(τ) ≠ addr(τ') ⟹ from₁(τ) ≠ from₁(τ')`, with R1 supplying address-injectivity to convert `τ ≠ τ'` into `addr(τ) ≠ addr(τ')`), `from₁(τ) ≠ from₁(τ')`. Hence `slot_addrs(F_τ) ≠ slot_addrs(F_{τ'})`, so their slot-pairs differ, discharging Sh4's off-diagonal conclusion on `A_K^Σ`. The contract-side observation `C ⊆ C_fd` corroborates the same fact at the gate level (FDD's candidate set is at least as inclusive as Sh4's, so whenever FDD admits the emission Sh4 would too), but the load-bearing step is the direct relational argument above.
...
*Worked example.* Register `K = citation.depends` with the BundledDirectedPair shape (`T_cat = {citation.depends, R}`), pre-allocating a citing document `d_cite ∈ A_doc^{Σ_0}`, three cited documents `d_src1, d_src2, d_src3 ∈ A_doc^{Σ_0}`, and a home `home_cite ∈ dom(Σ_0.M)` with `dom(Σ_0.L) = ∅`. The walkthrough runs a single linear timeline `Σ_0 → Σ_1 → Σ_2 → Σ_3` whose three steps exhibit the shape's three G-cardinality regimes — empty-G (BDP0, `|slot_addrs(G)| = 0`), bundled multi-target (BDP1, `|slot_addrs(G)| = 3`), and legacy single-target (BDP2, `|slot_addrs(G)| = 1`) — co-existing in the active subset at Σ_3.
