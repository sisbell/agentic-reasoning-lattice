...
*Behavior at `c_F = 0|1`.* Neither `c_F = 0` nor (typically) `t_F = -` fires at a `c_F = 0|1` row, so both implications on the F-side are vacuously satisfied; `t_F` may take any of `A_doc, A_rel, A`. Thus the Provenance shape `(1, 0|1, A, A, ⊤)` is well-formed: the G-side antecedents `c_G = 0` and `t_G = -` are both false (`0|1 ≠ 0` and `A ≠ -`), so both G-side implications are vacuously satisfied. When the slot is empty at emission time (the `0` branch of `0|1`), clause (d) of Sh-conf reads `∅ ⊆ A^Σ`, vacuously true; when the slot is non-empty (the `1` branch), clause (d) enforces `slot_addrs ⊆ A^Σ` normally. The same reading applies to any future shape carrying `c_X = 0|1` paired with a non-`-` target domain.
...
**Emission 1.** `Emit_K(Σ_0, home_K, F_1, G_1)` with `F_1 = {(d_1, δ(1, #d_1))}` (commenter is d_1) and `G_1 = {(d_2, δ(1, #d_2))}` (target is d_2). Let the result be Σ_1 with new tuple `τ_1`. K.λ's first-emission branch fires at home_K: `{ℓ' ∈ dom(Σ_0.L) : origin(ℓ') = home_K} = ∅` (no comment-tuples homed at home_K have been emitted yet, since we start from Σ_0 with empty `L_K`), so `a_1 := addr(τ_1) = [home_K.0.s_L.1]` (ASN-0086, K.λ first-emission deposit address).
