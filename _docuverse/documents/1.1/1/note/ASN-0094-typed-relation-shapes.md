...
- *Content subspace partition.* Fixed `s_C ∈ ℕ` with `s_C > 0` and `s_C ≠ s_L` such that `E(a).1 = s_C` for every `a ∈ dom(Σ.C)`. (Symmetric to L0 from ASN-0043. Identifies `subspace_I(·) = E(·).1` on content-side element-level addresses.)
- *Link subspace partition.* Fixed `s_L ∈ ℕ` with `s_L > 0` such that `E(a).1 = s_L` for every `a ∈ dom(Σ.L)`. (Local commitment consistent with L0: identifies `subspace_I(·) = E(·).1` on link-side element-level addresses. Not derivable from L0 alone — L0 treats `subspace_I(·)` as uninterpreted. The two partition clauses adopt this identification framework-wide; substrates surfacing `subspace_I` via a different projection lie outside the framework's scope.)
...
(3) For each delimited interval `[e_k, e_{k+1})`, test membership in `coverage(E)` and in `coverage(E')` via the representative point `e_k`: `e_k ∈ coverage(E) ⟺ (∃ i : 1 ≤ i ≤ n : s_i ≤ e_k < s_i ⊕ ℓ_i)`. Because each span is a T1-interval and the partition is finer than every span's boundary, membership is uniform on each delimited interval: `e_k ∈ coverage(E)` iff every point of `[e_k, e_{k+1})` is in `coverage(E)`. `O((n + n')²)` T1 comparisons.
...
*Lifetime constancy of `T_cat`.* `T_cat` is fixed at `Σ_init` and does not change as states evolve.
...
*Caller-side dispatch.* The consolidated *Gate Ordering* below is the canonical statement of which checks fire and in what order. A caller wanting to distinguish rejection causes before issuing `Emit_K` runs the same gates as side-effect-free reads — registry membership `K ∈ T_cat`, single-home equality `d = d_K`, canonical-slot form on `F` and `G`, the discipline candidate-set queries `C_K`/`C_fd_K`, and Sh-conf's cardinality and target-domain tests — halting at the first failure. Splitting Sh-conf's cardinality and target-domain conjuncts into separate caller-side checks is admissible (both index distinct rejection causes); the protocol introduces no `↦`-step.
...
The canonical shape catalog is hand-curated; per-shape template families are written by hand against the catalog under three discipline conventions enforced by catalog-author diligence.

*Per-shape uniformity convention.* The catalog's shape-mate convergences — DirectedPair/Resolution sharing five base templates modulo codomain shift; the two `(0, 1)` rows sharing `is_K`; NonIdempotentDirectedPair/BundledDirectedPair/Retraction/Provenance each adapting the same five-template skeleton from DirectedPair with one or both slot accessors lifted from point-accessor to set-accessor as forced by their cardinality components — are *hand-curated*, not framework-derived. Signature derivation from shape components is mechanical per the *Signature derivation rule* below, but body-shape choice (which set comprehensions and which Boolean compositions appear under each signature) is the catalog author's design decision. The framework supplies no mechanical gate that would enforce body-shape convergence at any pair of shape-mate rows; a future catalog extension at an existing shape may register divergent template bodies without violating any framework gate. Subsequent catalog rows that note "bodies hand-curated against the DirectedPair shape-mate" invoke this convention without restating its content.
...
| Tuple-Classifier          | (0, 1)     | -     | A_rel | ⊤    | *base:* `is_K(τ)` (the body matches Classifier's `is_K` by the catalog's current hand-curation per the *per-shape uniformity convention*; the signature shift from `A_doc → Bool` to `A_rel → Bool` is mechanical per the *Signature derivation rule*) |
...
| Resolution                | (1, 1)     | A_doc | A_rel | ⊤    | *base (hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention):* `pair_K(a, b)`, `from_K(a)`, `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)`; *dominant downstream pattern:* parametric consumption by NonIdempotentDirectedPair's `_via` templates (with K serving as the `K_res` argument). Standalone registration consuming the base templates without any `_via` consumer is also admissible. |
...
*Canonical template family (role-neutral, bodies hand-curated per the per-shape uniformity convention).* Every K registered at this shape inherits the following five templates by the catalog's hand-curation. Signatures derive mechanically per the *Signature derivation rule*; bodies are hand-curated per the per-shape uniformity convention. Each listed template is unconditional under Sh0–Sh4 (Sh0/Sh1 supply canonical-slot form and unit cardinality; Sh2/Sh3 supply `A_doc^Σ` codomains for the slot accessors; Sh4 ensures the returned tuple-sets and address-sets are slot-pair-distinct, not multisets). Codomains are made explicit per the *Codomain convention* for templates:
...
*Worked example (FDD + `K_target_of`).* Register `K = primary_author` at DirectedPair (`T_cat = {primary_author, R}`) with FDD opt-in at `K`, pre-allocating two documents `d_doc, d_other ∈ A_doc^{Σ_0}`, two author documents `d_alice, d_bob ∈ A_doc^{Σ_0}`, and a home `home_K ∈ dom(Σ_0.M)` with `dom(Σ_0.L) = ∅`.
...
**Emission FDD2 (suppress by clause (ii)).** From `Σ_1`, attempt `Emit_K(Σ_1, home_K, {(d_doc, δ(1, #d_doc))}, {(d_bob, δ(1, #d_bob))})` — same from-slot `d_doc`, different to-slot `d_bob`. Sh-conf passes (canonical-slot, cardinality match, both addresses allocated). The FDD contract subsumes Sh4 at FDD-registered K, so only the FDD clauses fire. Clause (i) computes `C_fd({(d_doc, δ(1, #d_doc))}, Σ_1) = {φ_1}` (φ_1 matches on from-slot, even though its to-slot differs). Clause (ii) *suppresses*: `Emit_K` returns `⊥`, state remains `Σ_1`, `A_K^{Σ_1} = {φ_1}` unchanged. This is the per-K-discipline suppression pattern (rejection pattern 6): FDD rejects the second emission outright because the from-slot is already claimed, even though Sh4 alone would have admitted it (the slot-pairs `({d_doc}, {d_alice})` and `({d_doc}, {d_bob})` differ). ✗
...
Tuples have form `slot_addrs(F) = {d}, slot_addrs(G) = {addr(σ)}` where `d ∈ A_doc^Σ` is the resolving document and `σ ∈ A_rel^Σ` is the comment-tuple being resolved. The shape `(1, 1, A_doc, A_rel, ⊤)` carries the same five-template base family as DirectedPair (with `t_G = A_rel` substituted for `A_doc`): `pair_K(a, b)`, `from_K(a)`, `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)`. Bodies hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention.
...
Tuples have form `slot_addrs(F) ⊆ A^Σ` (any finite set, possibly empty) and `slot_addrs(G) = {addr(σ)}` for `σ ∈ A_rel^Σ` the tuple being retracted. The retraction shape is consumed by ASN-0086's `A_K^Σ` Definition and Definition (nullified) directly: the active-subset definition uses `L_R`'s tuples to compute `nullified(Σ)`. Signatures derived mechanically per the *Signature derivation rule*; bodies hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention, re-formulated for the `c_F = *` setting since `from₁` is not defined when `c_F` is not `1`: DirectedPair's `from₁(τ)` point accessor is replaced by `slot_addrs(F_τ)` as a set accessor, with matching predicates lifted from address-equality to set-equality or set-membership as appropriate to each template's role:
...
*Canonical base template family (signatures derived per the *Signature derivation rule*; bodies hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention).* The shape's signature derivation is mechanical; the body adapts DirectedPair's `to₁(τ)` point-accessor uses to `slot_addrs(G_τ)` set-accessor uses, with matching predicates lifted from address-equality to set-equality or set-membership as appropriate to each template's role. The F-side templates retain `from₁` as a point accessor by SlotAccessorTotality at `c_F = 1`:
...
*Canonical base templates (signatures derived per the *Signature derivation rule*; bodies hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention).* Every K registered at this shape inherits the following five templates by the catalog's hand-curation:
...
*Canonical base template family (signatures derived per the *Signature derivation rule*; bodies hand-curated against the DirectedPair shape-mate per the per-shape uniformity convention).* The body adapts DirectedPair's templates with the asymmetry that `c_G = 0|1` requires explicit `⊥`-handling on G-side templates: F-side templates close over the totality of `from₁`; G-side templates must filter out tuples whose `to₁⁻` is undefined before applying the to-side accessor. Codomains follow the *Codomain convention for partial templates* established for partial accessors above:
...
(a) *Adding a new relation inherits its shape-mate's templates by hand-curation.* A new K registered at `shape(K) = DirectedPair` inherits the row's five base templates (`pair_K`, `from_K`, `to_K`, `from_addrs_K`, `to_addrs_K`) by the catalog's current hand-curation against the DirectedPair shape-mate, with signatures mechanically derived per the *Signature derivation rule*. Body-shape convergence with prior shape-mate rows is an aspiration of the present catalog (per the per-shape uniformity convention), not a framework-enforced derivation: registering a divergent template body at the same shape is not blocked by any mechanical gate. The cost of a new relation is one entry in the shape registry plus the author's diligence per the *Signature derivation rule*. Layers consuming an Attribute-style or Citation-style reading further define aliases (`has_K`, `K_sidecars_of`, `cites_K`, `K_incoming`); the singleton-returning `K_target_of` becomes available when the layer additionally registers FunctionalDependencyDiscipline.
