...
**Corollary — EffectiveWpSimplification.** Let Σ be reachable from `Σ_init` under the framework's layer commitment. For every `K ∈ T_admissible`, every `d ∈ dom(Σ.M)`, and every `F, G ∈ Endset`, ASN-0086's `wp_086` for `Emit_K(Σ, d, F, G)` simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible`, and the *effective wp* under Sh-conf simplifies to

`wp_eff(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) = d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`

*Proof.* By Lemma — RetractionTargetNotOnChain just proved, `NoCraftedSpanReachesD(Σ, d)` holds at every reachable Σ and every `d ∈ dom(Σ.M)`. The `(K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))` disjunct of `wp_086` collapses by the same per-home / cross-home case analysis applied to whichever R-tuple (if any) `G` might extend; since the only R-tuples reaching the substrate are Sh-conf-admitted ones, the lemma's coverage rule out applies uniformly. So `wp_086` reduces to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible`. Sh-conf adds the conjuncts `K ∈ T_cat ∧ conf_K^Σ(F, G)`; since `T_cat ⊆ T_admissible`, `K ∈ T_cat` absorbs `K ∈ T_admissible`, yielding the named `wp_eff`. ∎
...
| RetractionTargetNotOnChain | LEMMA | At any state Σ reachable from `Σ_init` under the layer commitment, every prior R-tuple `(b, F', G') ∈ L_R^Σ` has `b' ⋠ a_emit(Σ, d)` (`b'` = unique element of `slot_addrs(G')`); equivalently `NoCraftedSpanReachesD(Σ, d)` holds. Case split: per-home distinctness via the *Uniform link sub-allocator chain length* scaffolding clause + T10a.7 + T3; cross-home distinctness via L1 + L1a's home-from-zero-positions reading, with the *Document address structure* scaffolding clause supplying `zeros(d) = 2`. Discharges `wp_086`'s `NoCraftedSpanReachesD` conjunct in the EffectiveWpSimplification Corollary below, and is cited by the Retraction walkthrough's unit-depth-discipline passage. | introduced |
| EffectiveWpSimplification | COROLLARY | Under the framework's layer commitment, `wp_086` simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` and the effective wp under Sh-conf simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`. Established immediately after Lemma — RetractionTargetNotOnChain so downstream proofs cite a named result rather than the preliminary derivation in the Sh-conf section's preview. | introduced |
