...
*Effective weakest-precondition under Sh-conf.* Under the framework's *Emit_K routing commitment*, ASN-0086's `wp_086` collapses; see Corollary — EffectiveWpSimplification below for the named result that downstream proofs cite.
...
*Step 1 — Discharge of `NoCraftedSpanReachesD(Σ, d)`.* For every prior R-tuple `(b̂, F', G') ∈ L_R^Σ`, the framework's preservation theorems applied at the current state Σ pin down `G'`'s structure: by Sh1 at `K := R`, `G'` is canonical-slot with `match(|slot_addrs(G')|, 1)` (so `|slot_addrs(G')| = 1`); by Sh3 at `K := R`, `slot_addrs(G') ⊆ t_G^Σ = A_rel^Σ = dom(Σ.L)`. *Anticipation note (no circularity).* Sh1 and Sh3 are established by induction below in the Cardinality and Target Domain sections respectively; we anticipate them here so that downstream proofs can cite a named wp-simplification result. The dependency is acyclic: Sh0–Sh3's proofs consume only Sh-conf (and ASN-0086's R2/R3/LinkStoreInvarianceUnderArrangement, which are upstream), never this Corollary. Sh1 and Sh3 apply because R is registered in `T_cat` per the framework's baseline registration requirement, and both are theorems-under-conformance proved by induction on `↦*` and so deliver current-state structure for every τ ∈ L_R^Σ, including tuples emitted in prior states. The argument does not rely on Sh-conf-at-past-emission together with R2 plus monotonicity; the preservation theorems already discharge that bridge once and for all. So `G' = {(b', δ(1, #b'))}` for a unique `b' ∈ dom(Σ.L)`. By PrefixSpanCoverage (ASN-0043), `coverage(G') = {t : b' ≼ t}`. Applying the Lemma at `b := b'` (legal because `b' ∈ dom(Σ.L)`) gives `b' ⋠ a_emit(Σ, d)`, so `a_emit(Σ, d) ∉ coverage(G')`. Quantifying over `(b̂, F', G') ∈ L_R^Σ` discharges `NoCraftedSpanReachesD(Σ, d)` per ASN-0086's Definition.
...
| EffectiveWpSimplification | COROLLARY | Under the framework's *Emit_K routing commitment*, `wp_086` simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` and the effective wp under Sh-conf simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`. Established immediately after Lemma — RetractionTargetNotOnChain so downstream proofs cite a named result rather than the preliminary derivation in the Sh-conf section's reference. Proof body splits into two explicit steps: Step 1 discharges `NoCraftedSpanReachesD` by citing Sh1 and Sh3 at `K := R` to pin down prior R-tuple G-endset structure at the current state (replacing the implicit Sh-conf-at-past-emission + R2 + monotonicity bridge with the named preservation theorems), then applies the Lemma at each prior R-tuple's G-slot address; Step 2 case-splits on `K ≁ R` vs `K ~ R` and, in the `K ~ R` case, applies the Lemma at the *new* emission's G-slot address (legal under the Lemma's generalization). | introduced |
