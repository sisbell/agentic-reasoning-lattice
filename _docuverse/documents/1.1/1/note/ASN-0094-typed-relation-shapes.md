...
*Step II.1 — All zeros of `a` lie at positions ≤ `#b`.* Let `Z_a := {i : 1 ≤ i ≤ #a ∧ aᵢ = 0}` and `Z_b := {i : 1 ≤ i ≤ #b ∧ bᵢ = 0}`. By componentwise agreement on `1..#b`, every `i ∈ Z_b` satisfies `aᵢ = bᵢ = 0`, hence `Z_b ⊆ Z_a`. Both sides have cardinality 3 (preamble), so equal-cardinality subsets of finite ℕ-sets force `Z_b = Z_a`. Hence every zero of `a` sits at some position ≤ `#b`; positions `#b + 1..#a` carry no zero of `a` (vacuously when `#b = #a`). Enumerate the three shared zero positions in strict order as `n_1 < n_2 < n_3`.
...
**Template bodies are author-curated, not framework-derived.** The framework's load-bearing content is Sh-conf + Sh0–Sh4 + the layer-discipline contracts. The catalog table and the per-shape template families below sit atop that load-bearing content as an organizational layer; *template signatures* derive mechanically from shape components (input/codomain symbols read from `t_F`/`t_G`), but *template bodies* do not. Registering a divergent template body at a registered shape is not blocked by any mechanical gate. Consumers depending on body-shape consistency across shape-mates (e.g., expecting Resolution's `pair_K(a, b)` to read like DirectedPair's `pair_K(a, b)`) rely on three *hand-curation conventions* enforced by catalog-author diligence:

(1) the *per-shape uniformity convention* — shape-mate rows share template-body shape by author convention, not by framework derivation;

(2) the *Signature derivation rule* — template signatures derive mechanically from shape components, bodies do not;

(3) the *Citation convention* — catalog template bodies cite only shape-derived slot accessors (`from₁`, `to₁`, `from₁⁻`, `to₁⁻`, `slots_F^Σ`, `slots_G^Σ`), K's name, named scaffolding clauses (`chain_index`, `home(·)`, `s_L`), accessors exported by a registered per-K discipline, meta-operators (logical, set-theoretic, arithmetic), and framework base accessors (`A_K^Σ`, `addr(τ)`, `slot_addrs(·)`, `δ(1, #·)`).

Each canonical shape pairs with a *base* predicate template family. Per-K opt-in and parametric extensions sit atop the base family and require additional registration (per-K disciplines) or additional arguments at evaluation time (type-index parameters).
...
*Coverage class self-identity at R.* Retraction is the `K ~ R` case in the framework's coverage classes.
