...
Gate positions index the six-gate ordering in the Sh-conf section below (0 catalog/home check, 1 SHCD, 2 Sh-conf canonical-form, 3 Sh4/FDD, 4 Sh-conf cardinality/target-domain, 5 K.λ).
...
*Structural gates.* Sh-conf's four clauses (a)–(d) partition into two structural gates in the Gate Ordering's six-gate execution sequence: clauses (a) and (b) jointly form the **canonical-form gate** at gate 2 (one gate, two operands — a non-canonical F (clause (a)) and a non-canonical G (clause (b)) are distinguishable as clause-level failures but both fall under gate 2); clauses (c) (cardinality) and (d) (target-domain) jointly form the **cardinality/target-domain gate** at gate 4. Each Sh-conf clause rejects independently of the others. Walkthroughs below cite Sh-conf failures by clause letter (a)/(b)/(c)/(d) when the failure mode is clause-specific, and by gate number (gate 2 vs gate 4) when the execution-sequence position is the load-bearing reading. The earlier convention referring to "Sh-conf's three independently-checked structural gates" — equating each of canonical-form, cardinality, target-domain to a separate gate — is superseded by this two-gate-four-clause structure, which matches the Gate Ordering above.
...
**Sh-conf — ShapeConformanceAxiom.** The framework restricts `Emit_K` by adding two preconditions: `K ∈ T_cat` and `conf_K^Σ(F, G)`. *Sh-conf admission (necessary sub-condition for `Emit_K` success).* If `Emit_K(Σ, d, F, G)` is Sh-conf-admissible, then `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`. The biconditional defining Sh-conf-admissibility (admission iff the three conjuncts hold) is the substrate's own characterization of the Sh-conf gate's decision procedure; the framework consumes only the forward direction, since Sh-conf admission is necessary for `Emit_K` success but not sufficient. For overall success, the per-K discipline non-suppression conjunct `Π_K(d, F, G, Σ)` must additionally hold (per Gate Ordering below); per-K discipline contracts (Sh4, FDD, SHCD) may suppress an admissible call by returning `⊥` at their own gate. The framework's full success condition — combining Sh-conf admission and per-K-discipline non-suppression — is the `wp_eff` of Corollary — EffectiveWpSimplification below. On any failure (Sh-conf rejection or per-K-discipline suppression), `Emit_K` returns `⊥` and leaves state unchanged; the return type extends from `Σ' × A_rel^{Σ'}` to `(Σ' × A_rel^{Σ'}) ∪ {⊥}`. Sh-conf binds `Emit_K`, not K.λ; chain-discipline and invariant-catalog facts (R0a-Cor1, R0a-Cor2, etc.) flow through the scaffolding clauses.
...
0. **Catalog and home check** (Sh-conf preconditions on `K` and `d`): test `K ∈ T_cat` and `d ∈ dom(Σ.M)`. On failure of either conjunct (`K ∉ T_cat` or `d ∉ dom(Σ.M)`), `Emit_K` returns `⊥` and no subsequent gate fires; the rejecting conjunct identifies the failure mode (rejection pattern 4 names the `K ∉ T_cat` case). The two conjuncts are checked left-to-right; the `K ∈ T_cat` failure is reported uniformly via rejection pattern 4 in Sh-conf Rejection Patterns below. On pass, the call proceeds to gate 1. (Gates 1–4 consume `shape(K)`, which is undefined for `K ∉ T_cat`; gate 5 consumes `d` as K.λ's home parameter. Gate 0 short-circuits before either consumption.)
...
3. Gates 0–4 all admit the `Emit_R(Σ, d, F, G)` call, so the call proceeds to K.λ at home `d` (gate 5) and deposits a fresh tuple τ_new with `addr(τ_new) = a_emit(Σ, d)`, producing result state Σ'. Consequently: under R's registration `shape(R) = (*, 1, A, A_rel, ⊤)` (Precondition 2), R is Sh4-registered via the framework's *Sh4 idempotency contract* (since `shape(R).idem = ⊤` and R is not under FDD), so gate-3 admission entails Sh4's clause (i) check `C(F_{τ_new}, G_{τ_new}, Σ) = ∅` already passed at gate 3 — not as an additional condition imposed by this precondition but as a consequence of gate-3 admission's evaluation procedure. Similarly, gate-2 admission entails Sh-conf clauses (a)/(b) passed, and gate-4 admission entails clauses (c)/(d) passed — the proof below cites these clause-level facts (e.g., "clauses (b)/(c)/(d) force `G_{τ_new} = {(b, δ(1, #b))}`") as inheritances from gate-2/gate-4 admission.
...
*Step 4 — `addr(τ_new) ∉ nullified(Σ')` for the active-subset postcondition.* The postcondition phrases freshness over `A_K^{Σ'}` (active subset), not `L_K^{Σ'}` (audit slice). Concluding `τ_new ∈ A_K^{Σ'}` requires `addr(τ_new) ∉ nullified(Σ')` in addition to `τ_new ∈ L_K^{Σ'}` (which gates 0–5 already deliver). Two sub-cases:
...
*Step (Case A: `L_K^{Σ'} = L_K^Σ`).* `L_K` is unchanged. The property is inherited tuple-by-tuple from the IH (no new τ to check; existing τ retain `home(addr(τ)) = d_K`). The framework's `↦`-vocabulary classifies the case-equation discharge into four classes (paralleling Sh4's and FDD's Case A enumerations):

1. *K.σ-steps and K.α-steps:* preserve `Σ.L` pointwise (ASN-0086's `→` Definition's frame conditions), hence `L_K^{Σ'} = L_K^Σ`.
2. *K.λ-steps at type `K'` with `K' ≁ K` and `K' ≁ R`:* the new tuple enters the disjoint slice `L_{K'}^Σ`, leaving `L_K^Σ` untouched (ASN-0086's `~`-class indexing); hence `L_K^{Σ'} = L_K^Σ`.
3. *K.λ-steps at type `K'` with `K' ≁ K` and `K' ~ R`:* the new tuple enters `L_R^Σ`, which is disjoint from `L_K^Σ` since SHCD-registered K has `shape(K) = (1, 1, A_doc, A_doc, ⊥)` while R has `shape(R) = (*, 1, A, A_rel, ⊤)` (per-class constancy of `shape` with shape-tuple inequality forces `K ≁ R`, so `K' ~ R ⟹ K' ≁ K`); hence `L_K^{Σ'} = L_K^Σ`. Nullification of any prior K-tuple does not affect the homed-set property: SHCD's body quantifies over `L_K^Σ` (not `A_K^Σ`), and `L_K` is monotone non-decreasing by R3, so retraction-extending steps preserve the L_K-side membership and the homed-set invariant.
4. *Arrangement-modifying steps in `↦ \ →`:* LinkStoreInvarianceUnderArrangement (ASN-0086) gives `Σ'.L = Σ.L` pointwise, hence `L_K^{Σ'} = L_K^Σ`.
...
| CoverageEqualityDecidability | LEMMA | `coverage(E) = coverage(E')` is decidable on finite endsets via endpoint-partition membership tests |
...
| slots_F^Σ, slots_G^Σ | DEF | Total set-valued slot accessors on tuples (renamed from `from_K^Σ`/`to_K^Σ` to avoid clashing with the address-to-tuple-set accessors `from_K(a)`/`to_K(b)`) |
...
| R-registration commitment | ASSUMPTION | Every conforming layer registers `R ∈ T_cat` with `shape(R) = (*, 1, A, A_rel, ⊤)`; co-requisite of Emit_K routing for Nullify |
