...
*Step 3.1 — Shared zero positions.* The prefix `x ≼ a` gives `#x ≤ #a` and componentwise agreement `aᵢ = xᵢ` for `1 ≤ i ≤ #x`. *Strictly-increasing enumeration of `x`'s zero positions via NAT-card.* From `zeros(x) = 3` (the Case 3 hypothesis on element-level addresses), the zero-index set `Z_x := {i : 1 ≤ i ≤ #x ∧ xᵢ = 0} ⊆ {1, …, #x}` has cardinality 3 by definition of `zeros(·)`. NAT-card (NatFiniteSetCardinality, ASN-0034) applied at `n = #x` characterises `|Z_x|` via the unique `k ∈ ℕ` such that a strictly increasing `f : {1, …, k} → ℕ` exists with image exactly `Z_x`; at `|Z_x| = 3`, `k = 3` and the strictly increasing enumeration `f : {1, 2, 3} → ℕ` supplies three positions `n_1 := f(1) < n_2 := f(2) < n_3 := f(3)` with `Z_x = {n_1, n_2, n_3}`, `1 ≤ n_1` (from the comprehension's range), and `n_3 ≤ #x` (likewise from the comprehension's range). The decomposition is unique by NAT-card's uniqueness clause on `k` together with strict-monotonicity of `f` (any reordering of `{n_1, n_2, n_3}` would violate strict-increase or duplicate a value, both excluded). Let `x`'s three zero positions be `n_1 < n_2 < n_3` per this enumeration. By componentwise agreement at positions `n_1, n_2, n_3 ≤ #x`, `a` carries zeros at each of those positions: `aₙ₁ = xₙ₁ = 0`, `aₙ₂ = xₙ₂ = 0`, `aₙ₃ = xₙ₃ = 0`. So `a` has at least three zeros, located at `n_1, n_2, n_3`. With `zeros(a) = 3` (the hypothesis), the question reduces to whether `a` has any *additional* zero outside `{n_1, n_2, n_3}`. Suppose, toward contradiction, that `a` has a fourth zero at some position `m ∈ {1, ..., #a} ∖ {n_1, n_2, n_3}`: if `m ≤ #x`, then `aₘ = 0` together with componentwise agreement forces `xₘ = 0`, adding a fourth zero to `x` and contradicting `zeros(x) = 3`; if `m > #x`, then `m` is a zero position in `a` outside `{n_1, n_2, n_3}` (which all lie at positions `≤ #x`), so `zeros(a) ≥ 4`, contradicting `zeros(a) = 3`. Both branches fail, so no such `m` exists; combined with the three zeros already located at `n_1, n_2, n_3`, `a`'s three zero positions are exactly `n_1 < n_2 < n_3`, the same as `x`'s. This step holds in both sub-cases because it consumes only the prefix relation `x ≼ a` and the shared-`zeros = 3` constraint, neither of which references domain membership.
...
*Stratified proof order.* The framework's proofs are organized as an explicit, acyclic stratification — the document order below reflects the stratification directly, so a reader following the document linearly never reaches a proof site before its consumed lemmas: (1) Sh-conf axiom (this section, posited); (2) Sh0–Sh3 inductions (Cardinality and Target Domain sections, each independently induct on `↦*` against Sh-conf and ASN-0086's upstream R2/R3/LinkStoreInvarianceUnderArrangement; no member of Sh0–Sh3 consumes another member of Sh0–Sh3 in its inductive hypothesis); (3) RetractionTargetNotOnChain lemma (its own section below, independent of Sh0–Sh3 — its proof consumes only ASN-0086's R0a-Cor1/FreshEmissionAddress/L1/L1a, T10a.7/T3/Prefix, the scaffolding clauses, and NAT-card); (4) EffectiveWpSimplification corollary (after Sh3, consumes Sh1 and Sh3 at `K := R` together with the RetractionTargetNotOnChain lemma); (5) Sh4 induction (Idempotency section, consumes Sh0–Sh3 and EffectiveWpSimplification as state-indexed lemmas). The Sh4 stratification is documented at the Sh4 *Stratification* sub-clause; the same acyclicity claim repeated here clarifies the cross-section dependencies at the document level. No cycle: each stratum's proofs consume only earlier-numbered strata.
...
*Scope of the per-tuple-conformance relaxation.* The weaker check "every prior `L_K`-tuple satisfies `conf_K^{Σ_init}`" is *equivalent* to the empty-baseline only for the per-tuple lemmas Sh0–Sh3, whose universals quantify over a single tuple at a time and whose body is `conf_K^Σ`. It does *not* suffice for Sh4 (whose body is a pairwise predicate `(slot_addrs(F_τ), slot_addrs(G_τ)) = (slot_addrs(F_{τ'}), slot_addrs(G_{τ'})) ⟹ addr(τ) = addr(τ')`: two distinct prior `L_K`-tuples could each independently satisfy `conf_K^{Σ_init}` while jointly sharing a slot-pair, falsifying Sh4 at the base case), nor for the *FDD functional-dependency contract*'s preservation (whose pairwise universal over from-slot values can be violated by two prior tuples sharing a from-slot value, even though each is per-tuple conformant), nor for the *single-home commitment*'s preservation (whose homed-set universal `(A τ ∈ L_K^Σ :: home(addr(τ)) = d_K)` requires every prior tuple's home to equal `d_K`, a global constraint that `conf_K^{Σ_init}` — which never inspects `home(addr(τ))` — does not enforce). For Sh4, the *FDD functional-dependency contract*, and the *single-home commitment*, the empty-baseline `L_K^{Σ_init} = ∅` is required at every K under those contracts; the per-tuple-conformance relaxation alone is insufficient.


## Lemma — RetractionTargetNotOnChain
...
*Case I — `home(b) = d`.* Both `b` and `a_emit(Σ, d)` lie in `A_L(d)`'s chain enumeration. The case hypothesis `home(b) = d` together with `b ∈ dom(Σ.L)` places `b` in the homed set at `d`, so the homed set is non-empty; by R0a-Cor1 (ContiguousPrefix, ASN-0086) this forces `J_d^Σ ≥ 0` (the `J_d^Σ = -1` sentinel encodes an empty homed set, which the case hypothesis rules out), and `b` is enumerated at some chain index `0 ≤ i ≤ J_d^Σ` via `b = inc^i(d.0.s_L.1, 0)`. *FreshEmissionAddress branch forced under Case I.* Because the homed set at `d` is non-empty under the case hypothesis, FreshEmissionAddress (ASN-0086) selects the *subsequent-emission* branch (the first-emission branch's gating predicate `{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d} = ∅` is *false* in Case I — it contains at least `b`). The subsequent-emission branch sets `a_emit(Σ, d) = inc(ℓ_prev, 0)` where `ℓ_prev = max{ℓ' ∈ dom(Σ.L) : origin(ℓ') = d}` under T1; by R0a-Cor1 this maximum is the chain element at index `J_d^Σ`, so `a_emit(Σ, d) = inc^{J_d^Σ + 1}(d.0.s_L.1, 0)` lies at chain index `J_d^Σ + 1`. By the *Uniform link sub-allocator chain length* scaffolding clause (Scope and Substrate Scaffolding), `#b = #a_emit(Σ, d)`. By T10a.7 (EnumerationInjectivity, ASN-0034) applied to the chain at `A_L(d)`, distinct chain indices yield distinct tumblers; since `b`'s chain index `i ≤ J_d^Σ < J_d^Σ + 1 = a_emit(Σ, d)`'s chain index, `b ≠ a_emit(Σ, d)`. Two equal-length distinct tumblers are prefix-incomparable: if `b ≼ a_emit(Σ, d)`, then by Prefix (ASN-0034) the length clause `#b ≤ #a_emit(Σ, d)` combined with equal lengths forces componentwise agreement on all of `1..#b = 1..#a_emit(Σ, d)`, which by T3 (CanonicalRepresentation, ASN-0034) makes them identical — contradiction. So `b ⋠ a_emit(Σ, d)`.

*Case II — `home(b) ≠ d`.* Suppose toward contradiction `b ≼ a_emit(Σ, d)`. *Local definition of the suffix `w` from the prefix relation.* Set `a := a_emit(Σ, d)`. From Prefix (ASN-0034), `b ≼ a` unfolds to `#b ≤ #a` and componentwise agreement `aᵢ = bᵢ` for `1 ≤ i ≤ #b`. We define the *suffix* `w` of `a` after `b` as the tumbler whose components are read off `a`'s tail: `w` has length `#w := #a − #b` (conditional closure under NAT-sub, NatPartialSubtraction, ASN-0034, since `#b ≤ #a`) with components `wⱼ := a_{#b + j}` for `1 ≤ j ≤ #w`. By T0's comprehension clause (CarrierSetDefinition, ASN-0034), instantiated at `p = #w` and the component map `j ↦ a_{#b + j}` (every `a_{#b + j} ∈ ℕ` by T0's component projection), `w ∈ T` is well-defined whenever `#a > #b`; when `#a = #b` the suffix is empty and the entire argument below reduces to `b = a` by T3 (CanonicalRepresentation, ASN-0034) on componentwise agreement at equal length, which already contradicts the construction below since it would force `home(b) = home(a) = d` against the case hypothesis. We henceforth assume `#a > #b` (i.e., `#w ≥ 1`) and write `a = b · w` as shorthand for the join: `aᵢ = bᵢ` for `1 ≤ i ≤ #b` and `aᵢ = w_{i − #b}` for `#b < i ≤ #a`. By L1 (LinkElementLevel, ASN-0043) applied to `b ∈ dom(Σ.L)`, `zeros(b) = 3`. The fresh emission address `a` has `zeros = 3` by construction: in the first-emission branch `a = [d.0.s_L.1]` has two zeros from `zeros(d) = 2` (by the *Document address structure* scaffolding clause, Scope and Substrate Scaffolding), one zero from the separator at position `#d + 1`, zero zeros from `s_L` at position `#d + 2` (since `s_L > 0` by the *Link subspace partition* scaffolding clause's positivity commitment), and zero zeros from the trailing `1` — total 3; in the subsequent-emission branch `a = inc(ℓ_prev, 0)` preserves `zeros` (TA5(c) with `k = 0`, ASN-0034: the step modifies only position `sig(ℓ_prev)`, and on T4-valid `ℓ_prev` — T4-validity supplied by L1c via T10a.4 on the link side — that position carries a non-zero value whose incremented value remains non-zero, so neither the modified position nor any other contributes a new zero) from `zeros(ℓ_prev) = 3` (by L1 applied to `ℓ_prev ∈ dom(Σ.L)`). *Zero-count additivity over prefix decomposition.* From the join `a = b · w` defined above, the componentwise definitions `aᵢ = bᵢ` for `1 ≤ i ≤ #b` and `aᵢ = w_{i − #b}` for `#b < i ≤ #a` give a partition of `a`'s zero-index set: `Z_a := {i : 1 ≤ i ≤ #a ∧ aᵢ = 0} = Z_b ⊔ Z_w^shift`, where `Z_b := {i : 1 ≤ i ≤ #b ∧ bᵢ = 0}` and `Z_w^shift := {i : #b < i ≤ #a ∧ w_{i − #b} = 0}`. *Derivation of `|Z_a| = |Z_b| + |Z_w^shift|` from NAT-card.* NAT-card axiomatizes `|·|` via the strictly-increasing-enumeration characterisation: every subset `S ⊆ {1, …, n}` has a unique `k ∈ ℕ` and a strictly increasing `f : {1, …, k} → ℕ` with image exactly `S`, and `|S| = k`. Apply at `n = #a` to enumerate `Z_a` as `f_a : {1, …, |Z_a|} → ℕ` strictly increasing with image `Z_a`; at `n = #b` to enumerate `Z_b` as `f_b : {1, …, |Z_b|} → ℕ` strictly increasing with image `Z_b`; and at `n = #a` (with the comprehension restricted to indices `> #b`) to enumerate `Z_w^shift` as `f_w : {1, …, |Z_w^shift|} → ℕ` strictly increasing with image `Z_w^shift`. By disjointness `Z_b ∩ Z_w^shift = ∅` (since every element of `Z_b` satisfies `i ≤ #b` and every element of `Z_w^shift` satisfies `i > #b`), the concatenation `f_a' : {1, …, |Z_b| + |Z_w^shift|} → ℕ` defined by `f_a'(j) = f_b(j)` for `1 ≤ j ≤ |Z_b|` and `f_a'(j) = f_w(j − |Z_b|)` for `|Z_b| < j ≤ |Z_b| + |Z_w^shift|` is itself strictly increasing (the concatenation seam preserves strict monotonicity because every value `f_b(j) ≤ #b` is strictly less than every value `f_w(·) > #b`) with image `Z_b ∪ Z_w^shift = Z_a`. By NAT-card's uniqueness clause for `(k, f)` characterising `Z_a`, `|Z_a| = |Z_b| + |Z_w^shift|`. Now `|Z_a| = zeros(a)` by definition; `|Z_b| = zeros(b)` similarly; and `|Z_w^shift| = |Z_w|` where `Z_w := {j : 1 ≤ j ≤ #w ∧ wⱼ = 0}` (the bijection `i ↦ i − #b` between `Z_w^shift` and `Z_w` is strictly increasing and surjective, so NAT-card's enumeration of `Z_w^shift` via `j ↦ f_w(j) − #b` re-indexes to an enumeration of `Z_w` of the same length). Hence `zeros(a) = zeros(b) + zeros(w)`; rearranging via NAT-sub (NatPartialSubtraction, ASN-0034) under `zeros(b) ≤ zeros(a)` (here `3 ≤ 3`), `zeros(w) = zeros(a) − zeros(b) = 3 − 3 = 0`. By L1a (LinkScopedAllocation, ASN-0043), `home(b)` is determined by `b`'s three-zero prefix structure — specifically, `home(b) = N(b).0.U(b).0.D(b)` reads off the field projections of `b`'s zero positions. Since `a` shares its first `#b` components with `b` (the prefix relation gives this directly) and `w` contributes no additional zeros, `a`'s three zero positions lie within positions `1..#b`, agreeing pointwise with `b`'s three zero positions. Hence `N(a) = N(b)`, `U(a) = U(b)`, `D(a) = D(b)`, so `home(a) = home(b)`. But `a`'s home is `d` by construction (first-emission deposits at `[d.0.s_L.1]` whose home is `d`; subsequent-emission inherits home from the parent chain at `d`). So `d = home(b)`, contradicting `home(b) ≠ d`. So `b ⋠ a_emit(Σ, d)`.
...
With Sh1 and Sh3 now established by the preceding sections (Cardinality and Target Domain respectively), and Lemma — RetractionTargetNotOnChain established earlier (in its own section after Initial-State Baseline), the named wp-simplification result can now be discharged with no forward references. Downstream proofs (notably Sh4 below) cite this Corollary directly rather than re-deriving the simplification.
...
*Proof.* `wp_086` carries two non-trivial conjuncts beyond `d ∈ dom(Σ.M) ∧ K ∈ T_admissible`: `NoCraftedSpanReachesD(Σ, d)` and `(K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))`. We discharge each via Lemma — RetractionTargetNotOnChain (established earlier in its own section) applied at the appropriate `b ∈ dom(Σ.L)`. The proof's two consumed inputs — Sh1/Sh3 at `K := R` (Step 1) and the Lemma (both Steps 1 and 2) — were each established by their own independent argument before the present section; no forward reference enters the proof.
...
(b) *META discipline.* This framework's catalog adheres to the rule that every catalog row's templates depend only on the following four input categories of *data symbols*: (i) the shape components (cardinality, target-domain typing, idempotency flag); (ii) K's name; (iii) named scaffolding clauses surfaced in the *Scope and Substrate Scaffolding* list; and (iv) explicitly named per-K disciplines and per-call type-index parameters registered in the row's opt-in or parametric columns. The criterion is *literal name-citation for data symbols*: a data-symbol reference in a template body must either be one of the shape-component slots, K itself, a scaffolding clause name (e.g., `chain_index`, `home(·)`, `s_L`), an accessor exported by a registered per-K discipline (e.g., `emission_order` under SingleHomeCoverageDiscipline), or a parametric type-index argument; any data symbol falling outside these four categories violates the discipline and the catalog rejects the addition. *Meta-operator carve-out.* Logical and set-theoretic primitives — set comprehensions `{… : …}`, finite-set operations `∪`, `∩`, `\\`, `℘_fin(·)`, `∈`, `⊆`, `=`, quantifiers `(A …)`, `(E …)`, extremal operators (`argmax`, `argmin`, `max`, `min`) over finite ℕ-indexed sets, the Boolean connectives `∧`, `∨`, `¬`, `⟹`, `⟺`, conditional expressions, the partiality token `⊥`, and standard arithmetic on ℕ — are not data symbols and are unrestricted at every template body. Their use does not require registration. The discipline's literal name-citation rule applies only to data-symbol references (substrate accessors, scaffolding accessors, per-K-registered accessors, type-index parameters); it never restricts the logical and set-theoretic vocabulary used to *combine* those references. *Signature derivation rule.* Template signatures derive mechanically from shape components: a template's input domain and codomain symbols are read from the shape's `t_F` and `t_G` values respectively. Two registered K's at the same shape therefore inherit identical signatures for every base template; opt-in disciplines may refine codomains (e.g., FDD's `K_target_of` lands in `A_doc^Σ ∪ {⊥}` by adding partiality), but the underlying target-domain symbol still derives from `t_G` via category (i). Template bodies are otherwise identical for shape-mates. The opt-in column registers per-K disciplines (their names, their preservation contracts, and the accessor symbols they export); the parametric column registers per-call type-index arguments. Scaffolding clauses are catalog-row-independent (every row may cite them by name without per-row registration) because they are uniform substrate-layer commitments listed once in *Scope and Substrate Scaffolding*. This is a META commitment about this catalog's construction, not a theorem provable from R0…R7a plus Sh0–Sh4: a future draft proposing to add a template whose well-formedness depends on K-specific data outside (i)–(iv) would violate the discipline, and the framework would reject the addition. The discipline is what makes Sh5's per-shape organization falsifiable: a catalog row with diverging base templates from a shape-mate row, or a template body referencing a *data* symbol outside (i)–(iv), would visibly violate the discipline.

*Catalog extension is a manual review process — the acknowledged cost of Sh5's META status.* The discipline above is enforced by hand-review against each proposed catalog addition, not by a mechanical derivation procedure. There is no algorithm that takes a candidate row and certifies its templates compliant: every new row requires a human auditor to check each template body against the four input categories (i)–(iv) and the meta-operator carve-out, classifying each cited symbol and verifying the absence of out-of-category references. The catalog-wide citation audit table below is the worked-out form of this review for the current catalog's nine rows; future extensions add new rows to that table, with each row's symbol-by-symbol classification produced by the same hand-curation process. *Mechanical falsifiability without mechanical derivation.* The discipline supports *mechanical falsification* even though it does not support mechanical *derivation*: given a candidate template body, the literal name-citation check against the four categories is a finite, decidable per-symbol classification — once an auditor (or a downstream tool) is willing to walk each symbol against the categorization rule, a violation is detectable in finite time. What is *not* mechanical is template *production*: the auditor cannot start from an arbitrary shape and have the discipline produce a template body. Sh5(a)'s "by analogy with existing entries and by hand-design" is the framing for what the auditor does instead. This split — mechanical falsification, manual derivation — is the explicit cost of Sh5's META status; the framework accepts this cost in exchange for the meta-flexibility of admitting shape-specific design choices that a purely procedural derivation rule could not capture (e.g., Retraction's deliberate set-equality F-side argument vs the alternative membership reading, documented in the Retraction walkthrough). Future drafts may sharpen the discipline into a stricter procedural recipe; the current draft does not.

*Worked check at `latest_K_for_addr`.* The Coverage walkthrough's `latest_K_for_addr` template body `argmax_{τ ∈ S_d} emission_order(τ)` references data symbols and meta-operators that the discipline above sorts into two groups. *Data-symbol references* (subject to literal name-citation under categories (i)–(iv)): the inline `S_d = {τ ∈ A_K^Σ : to₁(τ) = d}` cites the shape-component-derived accessor `to₁` (category (i)) and the base-machinery `A_K^Σ` (a substrate export available at every row); `emission_order(τ)`, which under SingleHomeCoverageDiscipline expands to `chain_index(addr(τ), d_K)`, cites `chain_index` (category (iii), named in the *Link sub-allocator chain-index function* scaffolding clause), `d_K` (category (iv), registered by SingleHomeCoverageDiscipline as a per-K commitment), and `addr` (an R1 export from ASN-0086, a base-machinery accessor on every `L_K`-tuple). All data-symbol references are within the four input categories. *Meta-operator references* (exempt under the carve-out): `argmax`, the set-comprehension braces `{… : …}`, and the membership relation `∈`. These are logical and set-theoretic primitives, unrestricted by the discipline. The template clears the discipline.

*Catalog-wide citation audit.* Beyond the `latest_K_for_addr` worked check, every other catalog row's templates are verified against the same discipline. *Common base-machinery accessors* used uniformly across rows — `A_K^Σ`, `addr(τ)` (R1 export from ASN-0086), `slot_addrs(·)` (defined in *The Address-Set Projection* above, well-defined on canonical-slot endsets by Sh0/Sh1), `δ(1, #·)` (OrdinalDisplacement, ASN-0034) — are framework-internal base-machinery exports available at every row without per-row registration; their use in template bodies is unrestricted by category (i)–(iv) gating (they sit at the substrate-export tier the discipline presupposes). *Common meta-operators* — the Boolean connectives, set-comprehensions, finite-set operations, quantifiers, and the partiality token `⊥` — are exempt under the *Meta-operator carve-out*. The table below cites the per-row template data symbols against the four discipline categories; meta-operators and base-machinery accessors are omitted (uniform across rows). Read each row as: each cited symbol is classified under one of (i)–(iv), and the row's templates clear the discipline iff every symbol is so classifiable.

| Catalog row | Template(s) | Cited data symbols (with category) |
|---|---|---|
| Classifier | `is_K(d) ≡ (E τ ∈ A_K^Σ :: to₁(τ) = d)` | `to₁` (i: shape-component-derived under `c_G = 1`); `K` (ii: K's name, via `A_K^Σ` indexing) |
| Tuple-Classifier | `is_K(τ) ≡ (E σ ∈ A_K^Σ :: to₁(σ) = τ)` | `to₁` (i); `K` (ii). Body identical to Classifier modulo single-letter `d ↝ τ` substitution. |
| DirectedPair (base) | `pair_K(a, b)`, `from_K(a)`, `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)` | `from₁`, `to₁` (i: shape-component-derived under `c_F = c_G = 1`); `K` (ii) |
| DirectedPair + FDD (opt-in) | `K_target_of(a)` | `from_K(a)` (the row's own base template, derived from `K` (ii) plus `from₁` (i)); `to₁` (i); `K` (ii). FDD's discipline registration is what makes the "unique element of `from_K(a)`" reading well-formed (category iv: opt-in column entry "FunctionalDependencyDiscipline"). |
| NonIdempotentDirectedPair (base) | `pair_K`, `from_K`, `to_K`, `from_addrs_K`, `to_addrs_K` | Identical to DirectedPair (base) cited symbols; `K` (ii) plus shape-derived `from₁`, `to₁` (i). Body identical to DirectedPair's modulo absence of Sh4-driven slot-pair distinctness. |
| NonIdempotentDirectedPair (Coverage + SHCD opt-in) | `latest_K_for_addr(d)` | Per the worked check above: `to₁` (i), `K` (ii), `addr` (base-machinery), `chain_index` (iii: *Link sub-allocator chain-index function* scaffolding clause), `d_K` (iv: registered by SingleHomeCoverageDiscipline as a per-K commitment). All within categories. |
| NonIdempotentDirectedPair (Comment, parametric in `K_res`) | `unresolved_K_comments_via(K_res, d)`, `all_K_resolved_via(K_res, d)`; `resolved_by(τ, K_res)` | `to₁` (i: shape-component-derived); `K` (ii); `K_res` (iv: parametric type-index argument, registered with shape precondition `shape(K_res) = (1, 1, A_doc, A_rel, ⊤)`); `addr` (base-machinery). The inner `(E ρ ∈ A_{K_res}^Σ ::)` indexes the parametric type's active subset via `K_res`'s name. |
| Resolution (base, inherited per Sh5(b)) | `pair_K`, `from_K`, `to_K`, `from_addrs_K`, `to_addrs_K` | Identical to DirectedPair (base) cited symbols. Codomain symbol shifts from `A_doc` to `A_rel` per the *Signature derivation rule* applied to `t_G = A_rel`, but the cited data symbols are unchanged. |
| Retraction | `pair_K(F̂, b)`, `from_K(a)`, `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)` | `slot_addrs(F_τ)` (base-machinery, well-defined on canonical-slot F by Sh0 at `c_F = *`); `to₁` (i: shape-component-derived under `c_G = 1`); `K` (ii). The use of `slot_addrs(F_τ)` rather than `from₁(τ)` is forced by `c_F = *`, which does not admit `from₁` per SlotAccessorTotality. |
| Provenance | `outgoing_K(s)`, `pair_K`, `from_K`, `to_K`, `from_addrs_K`, `to_addrs_K` | `from₁` (i: shape-component-derived under `c_F = 1`); `to₁⁻` (i: shape-component-derived under `c_G = 0|1`, with `⊥`-handling per the *Codomain convention for partial templates*); `K` (ii); the literal `⊥` (meta-operator, exempt under the carve-out). |
| Layer composite (`K_is_fresh`, not a base template) | `K_is_fresh(d) ≡ from_K(d) ≠ ∅ ∧ mtime(K_target_of(d)) ≥ mtime(d)` | `from_K`, `K_target_of` (the row's own base + FDD-opt-in templates, derived from `K` (ii)); `mtime` (*layer-supplied accessor*, outside the four discipline categories). This is the documented reason `K_is_fresh` is *not* part of the catalog's template families and is moved to the *Layer Composites* section: its `mtime` reference fails the discipline as a substrate template, but the catalog explicitly carries `mtime` as a layer-side accessor outside the framework's coverage. |

Each row's templates cite only symbols within the four discipline categories (or the framework's uniform base-machinery exports). The layer composite `K_is_fresh` is explicitly *excluded* from the catalog precisely because its `mtime` reference fails the discipline; this exclusion is what the discipline-falsifiability criterion delivers in practice — a template that fails the citation check is rejected from the catalog and (if still needed) relocated to *Layer Composites* with explicit named accessor registration. The audit demonstrates that the catalog's nine catalog rows uniformly clear Sh5(b) without per-row exemptions.
...
The catalog row for Resolution `(1, 1, A_doc, A_rel, ⊤)` flags its *primary consumption* as parametric use by NonIdempotentDirectedPair's `_via` templates, but Sh5(b) mechanically generates the same five-template base family at the shape. To verify the base templates are computed identically to DirectedPair's (modulo the `t_G = A_rel` codomain shift), reuse `K_res` and `ρ_1, ρ_2` from the Comment walkthrough's Emissions 3 and 4. At Σ_4: `A_{K_res}^{Σ_4} = {ρ_1, ρ_2}`, with `ρ_1` having `from₁(ρ_1) = d_2, to₁(ρ_1) = a_1` (the address of τ_1) and `ρ_2` having `from₁(ρ_2) = d_2, to₁(ρ_2) = a_2` (the address of τ_2).

`pair_{K_res}(d_2, a_1) ≡ (E ρ ∈ A_{K_res}^{Σ_4} :: from₁(ρ) = d_2 ∧ to₁(ρ) = a_1) = true` (witnessed by ρ_1). `pair_{K_res}(d_2, a_2) = true` (witnessed by ρ_2). The body is identical to DirectedPair's `pair_K`; the only difference is the codomain of `to₁(ρ)` — under Resolution, `to₁(ρ) ∈ A_rel^Σ` by Sh3 at `t_G = A_rel`, so the equality `to₁(ρ) = a_1` lands in `A_rel^Σ` rather than DirectedPair's `A_doc^Σ`.

`to_addrs_{K_res}(d_2) ≡ {to₁(ρ) : ρ ∈ A_{K_res}^{Σ_4} ∧ from₁(ρ) = d_2} = {a_1, a_2}`. The body is identical to DirectedPair's `to_addrs_K`; the codomain is `℘_fin(A_rel^{Σ_4})` rather than `℘_fin(A_doc^{Σ_4})` per the shape's `t_G = A_rel`.

These two evaluations confirm that Resolution's base templates compute identically to DirectedPair's at the shape-mate level — same body, same set-comprehension structure, distinguished only by the codomain symbol read off `t_G` per the *signature derivation rule* of Sh5(b). The remaining base templates (`from_K`, `to_K`, `from_addrs_K`) compute analogously and are omitted; their bodies are identical to DirectedPair's modulo the same codomain shift.
...
| EffectiveWpSimplification | COROLLARY | Under the framework's *Emit_K routing commitment*, `wp_086` simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible` and the effective wp under Sh-conf simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`. Placed in its own section *after* the Target Domain section (Sh3), so that Sh1 and Sh3 are both available as previously-established inputs and no forward reference enters the proof. Proof body splits into two explicit steps: Step 1 discharges `NoCraftedSpanReachesD` by citing Sh1 and Sh3 at `K := R` to pin down prior R-tuple G-endset structure at the current state (replacing the implicit Sh-conf-at-past-emission + R2 + monotonicity bridge with the named preservation theorems), then applies the Lemma at each prior R-tuple's G-slot address; Step 2 case-splits on `K ≁ R` vs `K ~ R` and, in the `K ~ R` case, applies the Lemma at the *new* emission's G-slot address (legal under the Lemma's generalization). | introduced |
