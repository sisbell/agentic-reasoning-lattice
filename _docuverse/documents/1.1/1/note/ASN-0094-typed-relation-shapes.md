...
*Case 3* (`x` and `a` lie in different domains). WLOG `x ∈ dom(Σ.L), a ∈ dom(Σ.C)`; the sub-case `x ∈ dom(Σ.C), a ∈ dom(Σ.L)` proceeds identically with the subspace identifiers `s_L` and `s_C` exchanged in Step 3.3 (the prefix relation `x ≼ a` is unchanged; what symmetrizes is the subspace assignment at position `E(·).1`). By L1 (LinkElementLevel, ASN-0043) and the element-level content-address assumption, both `x` and `a` are element-level (`zeros(x) = zeros(a) = 3`); Step 3.2 below establishes E-field non-emptiness (`#E(·) ≥ 1`) from T4(iv) alone, so both `E(x).1` and `E(a).1` are well-defined.

*Step 3.1 — Shared zero positions.* The prefix `x ≼ a` gives `#x ≤ #a` and componentwise agreement `aᵢ = xᵢ` for `1 ≤ i ≤ #x`. Let `x`'s three zero positions be `n_1 < n_2 < n_3` with `1 ≤ n_1` and `n_3 ≤ #x` (the three witnesses to `zeros(x) = 3`). By componentwise agreement at positions `n_1, n_2, n_3 ≤ #x`, `a` also has `aₙ₁ = aₙ₂ = aₙ₃ = 0`. Suppose, toward contradiction, that `a` has a fourth zero at some position `m ∈ {1, ..., #a} ∖ {n_1, n_2, n_3}`: if `m ≤ #x`, then `aₘ = 0` together with componentwise agreement forces `xₘ = 0`, adding a fourth zero to `x` and contradicting `zeros(x) = 3`; if `m > #x`, then `m` is a zero position in `a` outside `{n_1, n_2, n_3}` (which all lie at positions `≤ #x`), so `zeros(a) ≥ 4`, contradicting `zeros(a) = 3`. So `a`'s three zero positions are exactly `n_1 < n_2 < n_3`, the same as `x`'s.

*Step 3.2 — E-field prefix relation.* The E-field of `x` is non-empty (we need only `#E(x) ≥ 1`, not the stronger `≥ 2` from L1b or its content-side analog, since Step 3.2's conclusion below uses only the index `j = 1`): T4(iv) applied to `x` gives `x_{#x} ≠ 0`, while `x_{n_3} = 0`, so `n_3 ≠ #x`; combined with `n_3 ≤ #x` (since `n_3` is a position of `x`) this gives `n_3 < #x`, i.e., `#x − n_3 ≥ 1`. By T4b's E-field index range `n_3 + 1 .. #x` with length `#E(x) = #x − n_3`, `#E(x) ≥ 1`. The same holds for `a` by the symmetric T4(iv) application at `a`. By T4b (UniqueParse, ASN-0034), with both `x` and `a` element-level and sharing the same three zero positions `n_1 < n_2 < n_3`, the E-field of `x` occupies positions `n_3 + 1 .. #x` and the E-field of `a` occupies positions `n_3 + 1 .. #a`. Hence `#E(x) = #x − n_3` and `#E(a) = #a − n_3`; with `#x ≤ #a` this gives `#E(x) ≤ #E(a)`. For component agreement: componentwise agreement at every position `n_3 + 1 ≤ i ≤ #x` (a subrange of `1 ≤ i ≤ #x` where `x ≼ a` already gives `xᵢ = aᵢ`) translates, via T4b's index offset, into `E(x)ⱼ = E(a)ⱼ` for every `1 ≤ j ≤ #E(x)` (taking `j = i − n_3`). With `#E(x) ≤ #E(a)` and componentwise agreement on `1 ≤ j ≤ #E(x)`, the Prefix definition (ASN-0034) gives `E(x) ≼ E(a)`. In particular, taking `j = 1`, `E(x).1 = E(a).1`.

*Step 3.3 — Subspace contradiction.* The link subspace partition scaffolding gives `E(x).1 = s_L` directly (`x ∈ dom(Σ.L)`); the content subspace partition scaffolding gives `E(a).1 = s_C` directly (`a ∈ dom(Σ.C)`); both scaffolding clauses fix `s_L ≠ s_C`. But Step 3.2 gives `E(x).1 = E(a).1`. Contradiction. In the alternate sub-case `x ∈ dom(Σ.C), a ∈ dom(Σ.L)`, the subspace assignments are exchanged — `E(x).1 = s_C` and `E(a).1 = s_L` — but `s_C ≠ s_L` is symmetric and yields the same contradiction. Both sub-cases vacuous. ∎
...
with the last two conjuncts collapsing to `⊤` under the relational layer's committed operations (regime (i) of ASN-0086's wp simplification under the unit-depth retraction discipline). Within the shape framework, this regime (i) collapse is secured *by Retraction's shape itself*: Retraction's catalog entry `(*, 1, A, A_rel, ⊤)` together with Sh-conf clauses (a)/(b) (canonical-slot form) and clause (c) at `c_G = 1` (`|slot_addrs(G)| = 1`) forces every shape-conformant `Emit_R` emission's G-endset to a single unit-depth span `{(b, δ(1, #b))}` for some `b ∈ A_rel^Σ` — exactly ASN-0086's unit-depth retraction discipline. (The same point is elaborated under *Unit-depth retraction discipline secured by Retraction's shape* in the Retraction catalog row's walkthrough.) Consequently `NoCraftedSpanReachesD(Σ, d)` holds automatically at every Sh-conf-admitted Retraction call site (by Lemma — RetractionTargetNotOnChain below, which spells out the per-home and cross-home chain-element argument), and the `(K ≁ R ∨ a_emit(Σ, d) ∉ coverage(G))` clause likewise collapses, so `wp_086` reduces to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible`. Sh-conf adds the conjuncts `K ∈ T_cat ∧ conf_K^Σ(F, G)`. The *effective wp* for `Emit_K` under Sh-conf is the conjunction:

`wp_eff(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) ≡ wp_086 ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`

Since `T_cat ⊆ T_admissible` (T_cat definition), `K ∈ T_cat` absorbs `K ∈ T_admissible`, and the effective wp simplifies — using the just-derived collapse of the regime (i) conjuncts to `⊤` via Retraction's shape — to:

`wp_eff = d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`
...
*Initial-state baseline for preservation proofs.* The inductive proofs of Sh0–Sh4 below presuppose an initial state `Σ_init` with `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`. The base cases discharge against this baseline. References to `Σ_0` in the proofs below denote this `Σ_init`. States reached before the framework's layer commitment was honored — e.g., substrate states from a prior process generation in which `Emit_K` did not exist, or any reachable state in which a class-(iii) emission of a type `K ∈ T_cat` bypassed `Emit_K` — are outside the framework's scope; the framework's preservation theorems make no claims about such states. A substrate that wants the framework's guarantees from a given starting point must verify `L_K^{Σ_init} = ∅` (or, equivalently, that every prior `L_K`-tuple satisfies `conf_K^{Σ_init}`) at that point.

**Lemma — RetractionTargetNotOnChain.** Let Σ be reachable from `Σ_init` under the framework's layer commitment (so every prior R-tuple in `L_R^Σ` was emitted via a Sh-conf-admitted `Emit_R` call, with `R` registered in `T_cat` per the *Interaction with Nullify* paragraph above). Then for every `(b, F', G') ∈ L_R^Σ` and every `d ∈ dom(Σ.M)`:

`a_emit(Σ, d) ∉ coverage(G')`

equivalently, `NoCraftedSpanReachesD(Σ, d)` holds (per ASN-0086's Definition — NoCraftedSpanReachesD).

*Proof.* Fix `(b, F', G') ∈ L_R^Σ`. By the lemma's hypothesis, `G'` arose through Sh-conf at past R-emission: Sh-conf clauses (a)/(b) force `G'` canonical-slot, and clause (c) at `c_G = 1` (Retraction's catalog row) forces `|slot_addrs(G')| = 1`. So `G' = {(b', δ(1, #b'))}` for a unique `b' ∈ slot_addrs(G')`; by Sh-conf clause (d) on the G-side with `t_G = A_rel`, `b' ∈ A_rel^Σ = dom(Σ.L)`. By PrefixSpanCoverage (ASN-0043), `coverage(G') = {t : b' ≼ t}`. It suffices to show `b' ⋠ a_emit(Σ, d)`. We case-split on whether `b'` and `a_emit(Σ, d)` share a home.
...
*Case II — `home(b') ≠ d`.* Suppose toward contradiction `b' ≼ a_emit(Σ, d)`. Then `a_emit(Σ, d) = b' · w` for some suffix `w` (Prefix definition, ASN-0034). By L1 (LinkElementLevel, ASN-0043) applied to `b' ∈ dom(Σ.L)`, `zeros(b') = 3`. The fresh emission address `a_emit(Σ, d)` has `zeros = 3` by construction: in the first-emission branch `a_emit(Σ, d) = [d.0.s_L.1]` has two zeros from `zeros(d) = 2` (by the *Document address structure* scaffolding clause, Scope and Substrate Scaffolding) plus one separator zero, total 3; in the subsequent-emission branch `a_emit(Σ, d) = inc(ℓ_prev, 0)` preserves `zeros` (TA5 with `k = 0`, ASN-0034) from `zeros(ℓ_prev) = 3` (by L1 applied to `ℓ_prev ∈ dom(Σ.L)`). Hence `zeros(w) = zeros(a_emit(Σ, d)) − zeros(b') = 0`. By L1a (LinkScopedAllocation, ASN-0043), `home(b')` is determined by `b'`'s three-zero prefix structure — specifically, `home(b') = N(b').0.U(b').0.D(b')` reads off the field projections of `b'`'s zero positions. Since `a_emit(Σ, d)` shares its first `#b'` components with `b'` (Prefix definition gives this directly) and `w` contributes no additional zeros, `a_emit(Σ, d)`'s three zero positions lie within positions `1..#b'`, agreeing pointwise with `b'`'s three zero positions. Hence `N(a_emit(Σ, d)) = N(b')`, `U(a_emit(Σ, d)) = U(b')`, `D(a_emit(Σ, d)) = D(b')`, so `home(a_emit(Σ, d)) = home(b')`. But `a_emit(Σ, d)`'s home is `d` by construction (first-emission deposits at `[d.0.s_L.1]` whose home is `d`; subsequent-emission inherits home from the parent chain at `d`). So `d = home(b')`, contradicting `home(b') ≠ d`. So `b' ⋠ a_emit(Σ, d)`.

Either case yields `b' ⋠ a_emit(Σ, d)`, so `a_emit(Σ, d) ∉ coverage(G') = {t : b' ≼ t}`. Universally quantifying over `(b, F', G') ∈ L_R^Σ` discharges `NoCraftedSpanReachesD(Σ, d)`. ∎

The lemma applies symmetrically in any direction `b'` and `a_emit(Σ, d)` could relate — the proof handles only `b' ≼ a_emit(Σ, d)` because that is what `coverage(G') = {t : b' ≼ t}` would require to contain `a_emit(Σ, d)`; the converse direction `a_emit(Σ, d) ≼ b'` is not what `NoCraftedSpanReachesD` asks about (it asks whether `a_emit(Σ, d)` is in some prior G-coverage, not whether some prior G-coverage's anchor is below `a_emit(Σ, d)`).
...
*Inductive step.* Suppose the property holds at Σ; let `Σ ↦ Σ'` be a single broad transition. We fix a particular `K ∈ T_cat`. By R3 (TypedSliceMonotonicity, ASN-0086), `L_K^Σ ⊆ L_K^{Σ'}` for every transition; `L_K` never contracts. The only effects on `L_K` are unchanged (Case A) or extended by one tuple (Case B). The contraction cases that appear in Sh4 over `A_K` (which filters `L_K` by `nullified(·)` membership) do not arise here because Sh0's universal ranges over `L_K`, not `A_K`. We split on whether the step affects `L_K`.
...
*Base case.* At `Σ_0`, `L_K^{Σ_0} = ∅`; the universal quantifier is vacuous.

*Inductive step.* Suppose the property holds at Σ; let `Σ ↦ Σ'` be a single broad transition. By R3, `L_K` is monotone non-decreasing, so the only effects on `L_K` are Case A (unchanged) or Case B (one new tuple). Split on whether the step affects `L_K`.
...
*Case B* (`L_K^{Σ'} = L_K^Σ ∪ {τ_new}` for a single new tuple). By the layer commitment (Scope and Substrate Scaffolding), this is a K.λ-step originating as an `Emit_K` call at type K (or a `~`-equivalent type `K'`; by T_cat's `~`-closure, `K' ∈ T_cat`, and `shape(K') = shape(K)` by per-class constancy). Sh-conf admitted that call only because `conf_K^Σ(F, G)` held — in particular, by Sh-conf's *clause (b)*, `G` is in canonical-slot form, and by *clause (c)*, `match(|slot_addrs(G)|, shape(K).c_G)`. So τ_new satisfies the G-side property: the first conjunct of Sh1's body is discharged by clause (b); the second conjunct by clause (c). Existing tuples retain their values by R2 and their G-side conformance by the IH.
...
*Proof.* By induction on `↦*` from `Σ_0`. Fix `K ∈ T_cat`. By R3, `L_K` is monotone non-decreasing, so the inductive step splits on whether the transition leaves `L_K` unchanged (Case A) or extends it by one tuple (Case B); no contraction case for `L_K` arises.
...
The layer commits to executing clauses (i)–(iii) atomically with respect to other Sh4-emitters at the same `~`-equivalence class of K — concurrent emission and retraction events at any K' with `K' ~ K` that could split (i)'s observation from (iii)'s emission must be serialized by the layer. `L_K` is `~`-class indexed (ASN-0086, `L_K^Σ = L_{K'}^Σ` whenever `K ~ K'`), so concurrent emitters at distinct-but-`~`-equivalent type indices write to the same active subset; atomicity scoped at the `~`-class is what closes the race.
...
*Step (Case A: `A_K^{Σ'} = A_K^Σ`).* The active subset is unchanged at K. Sh4 is inherited directly. The case is *defined* by the equation `A_K^{Σ'} = A_K^Σ`, not by the enumeration of transitions that achieve it; the prose below lists the principal transition classes that fall into this case, but the case-equation is the operative criterion. Principal transitions: all K.σ-steps, K.α-steps, K.λ-steps emitting a tuple of any type `K'` with `K' ≁ K` and `K' ≁ R` (so `L_K` and `nullified` are both untouched at K), and all arrangement-modifying steps in `↦ \ →` (by LinkStoreInvarianceUnderArrangement, `Σ'.L = Σ.L` pointwise hence `L_K^{Σ'} = L_K^Σ`, `L_R^{Σ'} = L_R^Σ`, and `nullified(Σ') = nullified(Σ)`). One residual scenario falls under Case A by the case-equation but is *not* listed in the principal-transitions enumeration: `K ~ R` together with a τ_new whose G-coverage targets `addr(τ_new)` (self-retraction-only, no prior R-tuple exits), under which `A_R` is simultaneously extended with τ_new and contracted by removing τ_new (since `addr(τ_new) ∈ nullified(Σ')`), yielding `A_R^{Σ'} = A_R^Σ`. This residual scenario is structurally precluded by Sh-conf clause (d) on the G-side: `τ_new`'s G-endset, at the moment of Sh-conf evaluation, would need `slot_addrs(G_{τ_new}) ⊆ A_rel^Σ`; but `addr(τ_new)` is the *fresh* address K.λ will deposit at if the call succeeds, so by R0 (TupleAddressFreshness, ASN-0086) `addr(τ_new) ∉ dom(Σ.L) = A_rel^Σ`. Clause (d) fails, Sh-conf rejects, and the framework's layer commitment routes the rejection through `⊥` without an `↦`-step. So the residual scenario is not reachable under Sh-conf; the principal-transitions enumeration above is exhaustive in practice, with the case-equation acting as the formal closure.

*Step (Case B: `A_K^{Σ'} = A_K^Σ ∪ {τ_new}`, a K.λ-step at type K with no concurrent nullification of any τ ∈ A_K^Σ).* By the layer commitment, this K.λ-step originates as an `Emit_K` call (with K or `~`-equivalent registered type). By contract clause (iii), the emission proceeded only because `C(F, G, Σ) = ∅`. Let `τ_new` be the new tuple. Suppose, toward contradiction, that some prior `τ ∈ A_K^Σ` satisfies `(slot_addrs(F_τ), slot_addrs(G_τ)) = (slot_addrs(F_{τ_new}), slot_addrs(G_{τ_new}))`. Then by definition `τ ∈ C(F, G, Σ)`, contradicting `C(F, G, Σ) = ∅`. So no such `τ` exists, and `A_K^{Σ'}` extends with a slot-pair-unique element. The pairwise condition is preserved: existing pairs were Sh4-distinct by IH; `τ_new` shares no slot-pair with any prior active tuple.
...
(b) *META discipline.* This framework's catalog adheres to the rule that every catalog row's templates depend only on (i) the shape components (cardinality, target-domain typing, idempotency flag), (ii) K's name, and (iii) explicitly named layer-supplied accessors registered in the row's opt-in or parametric columns — never on per-K design freedom beyond those three. This is a META commitment about this catalog's construction, not a theorem provable from R0–R7 plus Sh0–Sh4: a future draft proposing to add a template whose well-formedness depends on K-specific data outside (i)–(iii) would violate the discipline, and the framework would reject the addition. The discipline is what makes Sh5's per-shape organization falsifiable in the sense of (b): a catalog row with diverging base templates from a shape-mate row would visibly violate the discipline.
...
| Resolution                | (1, 1)     | A_doc | A_rel | ⊤    | *base (inherited from `(1, 1, A_doc, A_rel, ⊤)` per Sh5(b)):* `pair_K(a, b)`, `from_K(a)`, `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)`; *primary consumption:* parametrically by NonIdempotentDirectedPair's `_via` templates (column lists only this consumer, not the base family) |
| Retraction                | (\*, 1)    | A     | A_rel | ⊤    | *base (reformulated under `c_F = *`; bodies in the walkthrough):* `pair_K(F̂, b)` (F̂ matched by set equality), `from_K(a)` (membership-based: τ included iff `a ∈ slot_addrs(F_τ)`), `to_K(b)`, `from_addrs_K(b)`, `to_addrs_K(a)`; *primary consumption:* by R6's active-subset definition (`nullified(·)` reads `to_K`'s coverage) |
...
A template is *base* iff its definition is well-formed under exactly (a) the shape tuple's components, (b) K's name, and (c) the framework's Sh0–Sh4 guarantees (under Sh4's standard layer-discipline contract when `shape(K).idem = ⊤`). Every K registered at the shape generates the row's base templates mechanically.
...
`pair_K` returns Boolean. `from_K` and `to_K` return tuple-sets. `to_addrs_K` and `from_addrs_K` return address-sets, obtained by composing slot accessors over the corresponding tuple-set.
...
*Layer-discipline contract for FunctionalDependencyDiscipline.* The discipline is enforced by the layer at Emit time via the same Observe-then-Emit protocol used for Sh4, with the candidate-set restricted to from-slot match alone. For each K with FunctionalDependencyDiscipline registered, on every `Emit_K(Σ, d, F, G)` call site:
...
*Base.* At `Σ_0`, `L_K^{Σ_0} = A_K^{Σ_0} = ∅`; FDD's universal is vacuous.
...
(ii) The chain-index map `τ ↦ emission_order(τ)` is injective on `S_d`. Under SingleHomeCoverageDiscipline every `τ ∈ S_d` has `home(τ) = d_K`, so by the per-document link sub-allocator chain (Scope and Substrate Scaffolding) every such `addr(τ)` lies in the chain enumeration `{inc^n(d_K.0.s_L.1, 0) : n ≥ 0}` whose existence is supplied by the substrate-conforming layer; by T10a.7 (EnumerationInjectivity, ASN-0034) the chain index is the unique `n` with `addr(τ) = inc^n(d_K.0.s_L.1, 0)`, so distinct `τ, τ'` with distinct addresses (R1, AddressInjectivity, ASN-0086) have distinct chain indices.
...
| RetractionTargetNotOnChain | LEMMA | At any state Σ reachable from `Σ_init` under the layer commitment, every prior R-tuple `(b, F', G') ∈ L_R^Σ` has `b' ⋠ a_emit(Σ, d)` (`b'` = unique element of `slot_addrs(G')`); equivalently `NoCraftedSpanReachesD(Σ, d)` holds. Case split: per-home distinctness via the *Uniform link sub-allocator chain length* scaffolding clause + T10a.7 + T3; cross-home distinctness via L1 + L1a's home-from-zero-positions reading, with the *Document address structure* scaffolding clause supplying `zeros(d) = 2`. Discharges `wp_086`'s `NoCraftedSpanReachesD` conjunct in the effective-wp derivation, and is cited by the Retraction walkthrough's unit-depth-discipline passage. | introduced |
...
| Sh4 | LEMMA | IdempotencyDiscipline — at-most-one active duplicate when `idem = ⊤`; theorem under the layer-discipline contract; universal scope clarified (off-diagonal substance, diagonal trivial); proof covers A/B/C plus Case D (K=R simultaneous addition and contraction) | introduced |
...
| SingleHomeCoverageDiscipline | DEF | Per-K commitment securing `emission_order` for Coverage instantiations of NonIdempotentDirectedPair; allows non-K relations at the same home as long as K is single-home | introduced |
...
| layer-discipline contract (Sh4) | DEF | Observe-then-Emit protocol clauses (i)–(iii) the layer commits to for idempotent K; clause (i) uses the two-step procedure (i.a) finite-argument `Observe_K(slot_addrs(F), slot_addrs(G), oper)` over-approximation + (i.b) exact-slot-equality post-filter; atomically scoped at the `~`-equivalence class of K | introduced |
| layer-discipline contract (FunctionalDependencyDiscipline) | DEF | Observe-then-Emit protocol with from-slot-only candidate set `C_fd`; same two-step procedure as Sh4 contract; same atomicity scope as Sh4; structurally exclusive with SingleHomeCoverageDiscipline by `idem` flag | introduced |
| content-side scaffolding | ASSUMPTION | Element-level content addresses, content subspace partition, link subspace partition (`E(a).1 = s_L` for every `a ∈ dom(Σ.L)`; concrete realization of L0's abstract `subspace_I(·)` as `E(·).1`), content-store antichain, content-store monotonicity, content-store finiteness, document address structure (`zeros(d) = 2`), per-document link sub-allocator chains, uniform link sub-allocator chain length — assumed of the substrate-conforming layer; surfaced as named scaffolding clauses for direct citation, with attribution to specific upstream numbered invariants kept inside the layer's interface | introduced |
