...
*Load-bearing semantic departure from ASN-0086.* The framework registers Retraction with `idem = ⊤`, changing ASN-0086's apparent multiset semantics at R to set semantics at the bare `Nullify` alias; see *Nullify Compatibility* below.
...
*Emit_K routing commitment.* Every class-(iii) emission of a type `K ∈ T_cat` routes through `Emit_K`. Sh-conf below binds `Emit_K`, not K.λ. Layers that issue class-(iii) emissions of `K ∈ T_cat` outside `Emit_K` fall outside the framework's scope.

*R-registration commitment.* Every conforming layer registers `R` (ASN-0086, Definition — RetractionType) in `T_cat` with `shape(R) = (*, 1, A, A_rel, ⊤)` at the same point it fixes `T_cat^rep`. The framework's nullification semantics is uniformly `Emit_K`-mediated (the `Nullify` alias of ASN-0086 reduces to `Emit_R`, gated by Sh-conf).

*Empty-baseline commitment.* Every conforming layer ensures `L_K^{Σ_init} = ∅` at every `K ∈ T_cat`.

*Named layer-discipline contracts.* The framework introduces four contracts on top of the *Emit_K routing commitment* and the *R-registration commitment*; each is a theorem under its respective contract, not a substrate-enforced axiom. The unit-depth retraction entry retains its ASN-0086 foundation name *discipline*; within this ASN every named gate is treated as a *contract*.

| Contract | Applicable K's | Gate | Discharged theorem |
|---|---|---|---|
...
| *Single-home contract* | K with `shape(K) = (1, 1, A_doc, A_doc, ⊥)` + per-K SHCD registration | 1 | SHCD's homed-set property; secures `emission_order` |
| *Unit-depth retraction discipline* (ASN-0086) | K with `K ~ R` | derived | `NoCraftedSpanReachesD` discharge |

Gate positions index the five-gate ordering enumerated in *Gate Ordering* under the Conformance Axiom section below; after gate 4 admits, the call invokes K.λ.

*Scaffolding-conforming-layer scaffolding.* This ASN defines a *scaffolding-conforming layer* locally as any layer that satisfies the scaffolding clauses enumerated below. This name is distinct from ASN-0086's foundation `SubstrateConformingLayer` definition (which carries the full L/S/M/C invariant catalogs together with the chain discipline catalog); the local notion is strictly weaker and is consumed only by the lemmas of this ASN. The framework adopts the identification `subspace_I(·) = E(·).1` on element-level addresses framework-wide. The following clauses surface the specific properties this ASN cites by name; we refer to them collectively as *the scaffolding clauses*.
...
**Lemma — AllocatedAddressAntichain.** For every reachable state `Σ` at a scaffolding-conforming layer (which, by the subspace partition scaffolding clauses, honors the `subspace_I(·) = E(·).1` identification), and every `x ∈ A^Σ`:
...
*Step 3.1 — Shared zero positions.* From `x ≼ a` (Prefix): `#x ≤ #a` and `aᵢ = xᵢ` for `1 ≤ i ≤ #x`. Define the zero-index sets `Z_x := {i : 1 ≤ i ≤ #x ∧ xᵢ = 0}` and `Z_a := {i : 1 ≤ i ≤ #a ∧ aᵢ = 0}`. Since `zeros(x) = 3` and the zero positions are distinct natural numbers, `|Z_x| = 3`; symmetric for `|Z_a|`. Enumerate `Z_x` elements as `n_1 < n_2 < n_3` under T0's strict ℕ-order. Componentwise agreement gives `{n_1, n_2, n_3} ⊆ Z_a`, both sides of cardinality 3, so `Z_a = {n_1, n_2, n_3}` (equal-cardinality subsets of finite ℕ-sets are equal).

*Step 3.2 — E-field first-position agreement.* The E-field of `x` is non-empty: T4's last-position non-zero clause `t_{#t} ≠ 0` applied to `x` gives `x_{#x} ≠ 0`, while `x_{n_3} = 0`, so `n_3 ≠ #x`; combined with `n_3 ≤ #x` (since `n_3` is a position of `x`) this gives `n_3 < #x`, i.e., `#x − n_3 ≥ 1`. By T4 (HierarchicalParsing, ASN-0034), zeros are field separators: the maximal non-zero contiguous sub-sequences between successive zeros are the field segments of `t`. With zero positions `n_1 < n_2 < n_3`, these segments occupy positions `1..n_1 − 1`, `n_1 + 1..n_2 − 1`, `n_2 + 1..n_3 − 1`, and `n_3 + 1..#·`. T4c (LevelDetermination, ASN-0034) at `zeros(·) = 3` labels these four segments as the N-, U-, D-, and E-fields respectively, fixing the E-field as the segment after the third zero. T4b (UniqueParse, ASN-0034) makes this `fields(·)` projection uniquely computable from the address alone. So `#E(x) ≥ 1`, and the first E-field position `E(x).1` is defined. The same E-field non-emptiness holds for `a` by the symmetric application of T4's last-position non-zero clause at `a`. Applying T4 + T4c + T4b independently to `x` and `a` — both element-level and sharing the same three zero positions `n_1 < n_2 < n_3` — the E-field of `x` occupies positions `n_3 + 1 .. #x` and the E-field of `a` occupies positions `n_3 + 1 .. #a`; the index offset `E(·).j = ·_{n_3 + j}` for `1 ≤ j ≤ #E(·)` follows from the same identifications. The componentwise agreement `xᵢ = aᵢ` on `1 ≤ i ≤ #x` from `x ≼ a`, instantiated at `i = n_3 + 1` (which satisfies `n_3 + 1 ≤ #x` since `#E(x) ≥ 1`), yields `x_{n_3 + 1} = a_{n_3 + 1}`; substituting the index offset on both sides (taking `j = 1`) gives `E(x).1 = E(a).1`. This step holds in both sub-cases because the T4-validity citations and the E-field structure apply uniformly to element-level addresses without reference to subspace identifier.
...
*Representative list as layer-supplied configuration parameter.* `T_cat` is a layer-supplied configuration parameter, structurally parallel to the shape registry (Definition — ShapeRegistry below). The layer supplies a finite list of representative endsets `T_cat^rep = (K_{rep,1}, …, K_{rep,m})` with `K_{rep,i} ∈ T_admissible` and the representatives pairwise coverage-inequivalent (`K_{rep,i} ≁ K_{rep,j}` for `i ≠ j`). The catalog `T_cat` is then defined as the coverage-equivalence closure of `T_cat^rep`: `T_cat = {K ∈ T_admissible : (E K_{rep,i} ∈ T_cat^rep :: K ~ K_{rep,i})}`.

*Decidable membership.* Because `T_cat` is closed under `~` and finite at the quotient level, the predicate `K ∈ T_cat` is the coverage-class membership test `[K] ∈ T_cat / ~`. The test is decidable on arbitrary `K ∈ T_admissible` as the coverage-equivalence check `coverage(K) = coverage(K_rep)` against each of the finitely many registered representatives `K_rep`, by Lemma — CoverageEqualityDecidability below.

**Lemma — CoverageEqualityDecidability.** For any two finite endsets `E, E' ∈ Endset`, the predicate `coverage(E) = coverage(E')` is decidable, using only T1, T2, T12, TumblerAdd, and TA0 — with no canonical-slot precondition.

*Proof.* For any finite endset `E = {(s_1, ℓ_1), …, (s_n, ℓ_n)}`, `coverage(E)` is a finite union of bounded half-open intervals under T1: each span `(s_i, ℓ_i)` denotes `{t ∈ T : s_i ≤ t < s_i ⊕ ℓ_i}` (T12 well-formedness, ASN-0034), bounded above by the explicit upper endpoint `s_i ⊕ ℓ_i ∈ T` (well-defined by TA0, ASN-0034, since `Pos(ℓ_i)` and `actionPoint(ℓ_i) ≤ #s_i` hold by T12). To test `coverage(E) = coverage(E')`:

(1) Compute both endpoint sets `EP := {s_i, s_i ⊕ ℓ_i : 1 ≤ i ≤ n} ∪ {s_j', s_j' ⊕ ℓ_j' : 1 ≤ j ≤ n'}` (T1, TA0, TumblerAdd) — at most `2(n + n')` tumblers, all in T.

(2) Sort `EP` under T1 into a strictly increasing sequence `e_1 < e_2 < … < e_m` with `m ≤ 2(n + n')` (T2 IntrinsicComparison, ASN-0034), partitioning the range `[e_1, e_m)` into `m − 1` consecutive half-open intervals.

(3) For each delimited interval `[e_k, e_{k+1})`, test membership in `coverage(E)` and in `coverage(E')` via the representative point `e_k`: `e_k ∈ coverage(E) ⟺ (∃ i : 1 ≤ i ≤ n : s_i ≤ e_k < s_i ⊕ ℓ_i)`. Since `EP` contains both `s_i` and `s_i ⊕ ℓ_i` for every span, no span boundary lies strictly inside any `(e_k, e_{k+1})`; hence `[e_k, e_{k+1})` is either entirely contained in `[s_i, s_i ⊕ ℓ_i)` or entirely disjoint from it, so the union over spans is uniform on `[e_k, e_{k+1})` and the representative-point test on `e_k` decides membership for the whole interval.

(4) Equality holds iff every delimited interval has matching outcomes for both coverages. Outside `[e_1, e_m)`, both coverages are empty by construction. Each step performs finitely many T1 comparisons and tumbler additions on finite-depth tumblers (T0, ASN-0034), all decidable by T2 and TumblerAdd. ∎
...
- *Lifetime constancy.* `T_cat^rep` and `shape` are fixed before `Σ_init` and never modified across the substrate's lifetime.

*Registration interface.* The layer's pre-`Σ_init` configuration pairs `T_cat^rep` with a per-representative shape assignment `(shape(K_{rep,1}), …, shape(K_{rep,m}))`; `shape(K)` for any `K ∈ T_cat` is the paired shape of the unique representative `K_{rep,i}` with `K ~ K_{rep,i}` (decidable per the *Decidable membership* paragraph above).

**Lemma — NonRSeparation.** For every `K ∈ T_cat` with `shape(K) ≠ shape(R)`: `K ≁ R`.

*Proof.* By per-class constancy of `shape(·)` (Definition — ShapeRegistry): `K ~ K' ⟹ shape(K) = shape(K')`. Contrapositively, `shape(K) ≠ shape(K') ⟹ K ≁ K'`. Instantiating at `K' := R` gives `shape(K) ≠ shape(R) ⟹ K ≁ R`. ∎
...
*Structural gates.* Sh-conf clauses (a)/(b) fire at gate 2; clauses (c)/(d) fire at gate 4 (see *Gate Ordering*).
...
**Sh-conf — ShapeConformanceAxiom.** `Emit_K(Σ, d, F, G)` is *Sh-conf-admissible* iff `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`. On inadmissibility, `Emit_K` returns `⊥` and leaves state unchanged; the return type extends from `Σ' × A_rel^{Σ'}` to `(Σ' × A_rel^{Σ'}) ∪ {⊥}`. Sh-conf admission is necessary but not sufficient for `Emit_K` success: per-K discipline contracts (Sh4, FDD, SHCD) may suppress an admissible call. Per the *Emit_K routing commitment*, chain-discipline and invariant-catalog facts (R0a-Cor1, R0a-Cor2, etc.) flow through the scaffolding clauses.
...
0. **Catalog and home check** (Sh-conf preconditions on `K` and `d`): test `K ∈ T_cat` and `d ∈ dom(Σ.M)`. On failure of either conjunct (`K ∉ T_cat` or `d ∉ dom(Σ.M)`), `Emit_K` returns `⊥` and no subsequent gate fires; the rejecting conjunct identifies the failure mode (Pattern-0a and Pattern-0b in *Sh-conf Rejection Patterns* below). The two conjuncts are checked left-to-right. On pass, the call proceeds to gate 1. (Gates 1–4 consume `shape(K)`, which is undefined for `K ∉ T_cat`; the post-gate-4 K.λ emission consumes `d` as its home parameter. Gate 0 short-circuits before either consumption.)

1. **Single-home check** (if K registered under SingleHomeCoverageDiscipline): literal-equality test `d ?= d_K`. No `Observe_K` invocation, no state-dependent computation. On mismatch (`d ≠ d_K`), the *single-home contract* clause (i) rejects the call outright with `⊥` and no subsequent gate fires. On match, the call proceeds to gate 2. (Skipped entirely at K not registered under SHCD.)
...
4. **Sh-conf cardinality/target-domain gates** (Sh-conf clauses (c) and (d)): cardinality `match` on `|slot_addrs(F)|` and `|slot_addrs(G)|`; target-domain inclusion `slot_addrs(F) ⊆ t_F^Σ` and `slot_addrs(G) ⊆ t_G^Σ`. On failure of any conjunct, `Emit_K` returns `⊥`. On pass, the call proceeds to the K.λ emission below.

After gate 4 admits, the call invokes the **substrate primitive K.λ** (ASN-0086) at home `d` with value `(F, G, K)`. K.λ's first/subsequent-emission protocol fires, depositing the new tuple at the fresh address `a_emit(Σ, d)`. The call returns `(Σ', addr) ∈ Σ' × A_rel^{Σ'}`. No further rejection is possible at this step.
...
*Sh-conf Rejection Patterns.* Sh-conf's gates reject in six canonical patterns, named by their firing gate. Each is described below over an arbitrary reachable state Σ with `K ∈ T_cat` and arbitrary admissible `F, G ∈ Endset`; the per-shape walkthroughs cite these patterns by reference rather than re-deriving them.

*Pattern-0a — Unregistered type (`K ∉ T_cat`).* If `K_ghost ∈ T_admissible \ T_cat` — no representative `K_rep` in the registered list satisfies `K_ghost ~ K_rep` — gate 0's `K ∈ T_cat` conjunct fails and `Emit_K` returns `⊥` before any subsequent gate fires. The conformance predicate `conf_{K_ghost}^Σ` is unevaluable in this case (`shape(K_ghost)` is undefined), but the gate-0 failure is sufficient to reject. The `K ∈ T_cat` gate protects the framework's invariants against accidental schema drift: only registered relations participate in shape-discipline reasoning.

*Pattern-0b — Unregistered home (`d ∉ dom(Σ.M)`).* If `d` is not an allocated document at Σ, gate 0's `d ∈ dom(Σ.M)` conjunct fails and `Emit_K` returns `⊥` before any subsequent gate fires. The home-existence gate protects K.λ's home parameter from referencing an unregistered document container.

*Pattern-2a — Non-canonical F-endset (clause (a)).* If `F` is not in canonical-slot form — e.g., contains a span `(x, ℓ)` with `ℓ ≠ δ(1, #x)`, so `slot_addrs(F)` is undefined — clause (a) fails at gate 2 and `Emit_K` returns `⊥` before any state transition. Non-canonical endsets are admissible at the substrate by L4 (ASN-0043); the canonical-slot restriction is framework-level.

*Pattern-2b — Non-canonical G-endset (clause (b)).* Symmetric to Pattern-2a on the G-slot: clause (b) fails at gate 2 under the same canonical-slot restriction.

*Pattern-4a — Cardinality mismatch (clause (c)).* If `|slot_addrs(F)|` does not match `c_F` (resp. `|slot_addrs(G)|` against `c_G`) under the `match` predicate, clause (c) fails at gate 4. Concrete failure modes: cardinality 0 against `c_F = 1` (under-population); cardinality 2 against `c_F = 1` (over-population); cardinality 3 against `c_G = 1` (bundled emission at a non-`*` shape). Cardinality is independently rejected on F-side and G-side.

*Pattern-4b — Unallocated slot target (clause (d) on partition or allocation).* If `slot_addrs(F) ⊄ t_F^Σ` (resp. `slot_addrs(G) ⊄ t_G^Σ`) — e.g., the slot references a ghost address `d_ghost ∉ A^Σ`, or an address allocated in the wrong partition (`A_rel` vs `A_doc`, with R4 TupleAddressDisjointness, ASN-0086, separating them) — clause (d) fails at gate 4 and `Emit_K` returns `⊥`. L9 (TypeGhostPermission, ASN-0043) admits ghost addresses in endsets generally; the shape framework forbids ghost addresses in *slot positions* of registered relations, and shapes use `t_F`/`t_G` to bipartition admissible slot targets between content and relation addresses.
...
*Semantic departure from ASN-0086.* The framework's `shape(R).idem = ⊤` registration changes ASN-0086's apparent multiset semantics at R to *set semantics* at the bare `Nullify` alias: two consecutive bare `Nullify(Σ, d_retr, a)` calls at the same target produce *one* tuple in `L_R^Σ`, not two — the second is suppressed by the Sh4 contract. In Nelson's two-layer terms, the framework preserves the *active* layer's content (which addresses are presently nullified) but compresses the *audit* layer's per-event multiplicity. The migration recipe: layers requiring audit-grade multiset semantics switch from the bare alias to *attributed retraction* (`c_F ≥ 1` with a distinguishing from-slot value at each emission), which Sh4 admits as a distinct slot-pair per emission.

Under the *R-registration commitment*, `shape(R) = (*, 1, A, A_rel, ⊤)`. Sh-conf admits every well-formed Nullify call (`F = ∅` matches `c_F = *`; canonical-slot `G = {(a, δ(1, #a))}` matches `c_G = 1` with `{a} ⊆ A_rel^Σ` by Nullify's P1). The shape also admits attributed retractions (`F ≠ ∅`), and the unit-depth `G`-form preserves ASN-0086's automatic `NoCraftedSpanReachesD` discharge.
...
Preservation-proof text names the empty-baseline state (per the *Empty-baseline commitment*) `Σ_init` directly; walkthroughs additionally assume `dom(Σ_init.L) = ∅` globally so K.λ's first-emission predicate fires uniformly at each walkthrough's first emission. The symbol `Σ_0` is reserved for worked-example setup, where it denotes a pre-emission state reached from `Σ_init` by a finite sequence of K.σ/K.α steps (no K.λ-steps).

*Per-walkthrough convention.* Every walkthrough below assumes: `T_cat` declared at `Σ_init`; R registered with `shape(R) = (*, 1, A, A_rel, ⊤)`; `dom(Σ_init.L) = ∅` globally.
...
*Case II — `home(b) ≠ d`.* Suppose toward contradiction `b ≼ a_emit(Σ, d)`. Set `a := a_emit(Σ, d)`. From Prefix (ASN-0034), `b ≼ a` unfolds to `#b ≤ #a` and componentwise agreement `aᵢ = bᵢ` for `1 ≤ i ≤ #b`. Both `b` and `a` are T4-valid with `zeros = 3`: `b` by L1c → T10a.4 (L1c places `b ∈ dom(Σ.L)` in a T10a-conforming chain, T10a.4 propagates T4-validity) and L1 (LinkElementLevel, ASN-0043); `a` by TA5a (IncrementPreservesT4, ASN-0034) applied to K.λ's construction — first-emission `a = [d.0.s_L.1]` satisfies T4 from `zeros(d) = 2` and `s_L > 0` (totaling `zeros(a) = 3`); subsequent-emission `a = inc(ℓ_prev, 0)` preserves T4 and zero count (TA5(c) modifies only position `sig(ℓ_prev)`, which TA5-SigValid places past the third zero on T4-valid `ℓ_prev`). We derive the home contradiction uniformly (the argument runs identically whether `#b = #a` or `#b < #a`) through Steps II.1–II.3.

*Step II.1 — All zeros of `a` lie at positions ≤ `#b`.* Let `Z_a := {i : 1 ≤ i ≤ #a ∧ aᵢ = 0}` and `Z_b := {i : 1 ≤ i ≤ #b ∧ bᵢ = 0}`. Since `zeros(a) = zeros(b) = 3` (preamble) and the zero positions are distinct natural numbers, `|Z_a| = |Z_b| = 3`. By componentwise agreement on `1..#b`, every `i ∈ Z_b` satisfies `aᵢ = bᵢ = 0`, hence `Z_b ⊆ Z_a`. Equal-cardinality subsets of finite ℕ-sets force `Z_b = Z_a`. Hence every zero of `a` sits at some position ≤ `#b`; positions `#b + 1..#a` carry no zero of `a` (vacuously when `#b = #a`). Enumerate the three shared zero positions in strict order as `n_1 < n_2 < n_3`.
...
*Step II.3 — Home contradiction.* `home(·) := N(·).0.U(·).0.D(·)` is a deterministic projection of any T4-valid `zeros = 3` address (T4b). Step II.2's three field equalities give `home(b) = home(a)`. K.λ's construction gives `home(a) = d`. Hence `home(b) = d`, contradicting `home(b) ≠ d`. ∎
...
**Lemma — CaseAClosureForLK.** For every `K ∈ T_cat` and every broad transition `Σ ↦ Σ'` of the framework's `↦`-vocabulary, the framework's `↦`-vocabulary partitions exhaustively into four classes — three Case-A sub-classes producing `L_K^{Σ'} = L_K^Σ` and one Case-B class producing `L_K^{Σ'} = L_K^Σ ∪ {τ_new}` strictly:

*Case A — sub-class 1:* K.σ-steps and K.α-steps. *Case-equation:* `L_K^{Σ'} = L_K^Σ`.

*Case A — sub-class 2:* K.λ-steps at type `K'` with `K' ≁ K`. *Case-equation:* `L_K^{Σ'} = L_K^Σ`.

*Case A — sub-class 3:* arrangement-modifying steps in `↦ \ →`. *Case-equation:* `L_K^{Σ'} = L_K^Σ`.

*Case B:* K.λ-steps at type `K' ~ K`. *Case-equation:* `L_K^{Σ'} = L_K^Σ ∪ {τ_new}` for a single new tuple `τ_new`.

*Proof.* The four-class enumeration is exhaustive because ASN-0086's `→` Definition partitions `→` into `K.σ ∪ K.α ∪ K.λ`, and `↦` is `→ ∪ arrangement-modifying`; sub-classes 1, 2, 3 of Case A plus Case B together cover this partition, with the K.λ class refined by the `K' ~ K` vs `K' ≁ K` dichotomy. Discharge each sub-class:

*Sub-class 1 (K.σ/K.α):* The `→` Definition (ASN-0086) frames `Σ.L` pointwise on K.σ- and K.α-steps; hence `Σ'.L = Σ.L`, so `L_K^{Σ'} = L_K^Σ` for every `K`.

*Sub-class 2 (K.λ at `K' ≁ K`):* The new tuple at the K.λ-step enters the disjoint `~`-class slice `L_{K'}^{Σ'}`; by ASN-0086's `~`-class indexing (`L_K^Σ = L_{K''}^Σ` for every `K'' ~ K`), the slice indexed by K is untouched, so `L_K^{Σ'} = L_K^Σ`.

*Sub-class 3 (arrangement-modifying step):* LinkStoreInvarianceUnderArrangement (ASN-0086) gives `Σ'.L = Σ.L` pointwise across every step in `↦ \ →`; hence `L_K^{Σ'} = L_K^Σ`.

*Case B (K.λ at `K' ~ K`):* By the *Emit_K routing commitment*, the K.λ-step originates as an `Emit_K` call admitted by Sh-conf; the substrate primitive K.λ deposits exactly one fresh tuple `τ_new` at the address `a_emit(Σ, d)`. R3 (ASN-0086, TypedSliceMonotonicity) gives `L_K^Σ ⊆ L_K^{Σ'}`, and the freshness postcondition on `τ_new` together with `~`-class indexing forces `L_K^{Σ'} = L_K^Σ ∪ {τ_new}` strictly. ∎
...
**Lemma — CaseAClosureForAK.** For every `K ∈ T_cat` and every broad transition `Σ ↦ Σ'` that satisfies CaseAClosureForLK's Case A precondition (`L_K^{Σ'} = L_K^Σ`), the framework's `↦`-vocabulary's Case A sub-classes refine to four A_K-level sub-classes:

*A-sub-class 1:* K.σ-steps and K.α-steps. `Σ'.L = Σ.L`, so `L_K^{Σ'} = L_K^Σ`, `L_R^{Σ'} = L_R^Σ`, `nullified(Σ') = nullified(Σ)`, and `A_K^{Σ'} = A_K^Σ`.

*A-sub-class 2:* K.λ-steps at type `K'` with `K' ≁ K` and `K' ≁ R`. The new tuple enters `L_{K'}^{Σ'}`, leaving `L_K^Σ` and `L_R^Σ` untouched; `nullified(Σ') = nullified(Σ)`; `A_K^{Σ'} = A_K^Σ`.

*A-sub-class 3:* K.λ-steps at type `K'` with `K' ≁ K` and `K' ~ R` where no `τ ∈ A_K^Σ` has `addr(τ) ∈ coverage(G_{τ_new})`. `L_K^Σ` is untouched; `nullified(Σ')` extends but does not intersect `addr(·)` for any τ ∈ A_K^Σ; `A_K^{Σ'} = A_K^Σ`. The complementary sub-regime (where some τ ∈ A_K^Σ *is* nullified) routes to the preservation proof's Case C, contracting A_K. (Vacuous when `K ~ R`, since `K' ~ R ∧ K ~ R ⟹ K' ~ K`, contradicting `K' ≁ K`.)

*A-sub-class 4:* arrangement-modifying steps in `↦ \ →`. By LinkStoreInvarianceUnderArrangement, `Σ'.L = Σ.L`; hence `L_K^{Σ'} = L_K^Σ`, `L_R^{Σ'} = L_R^Σ`, `nullified(Σ') = nullified(Σ)`, `A_K^{Σ'} = A_K^Σ`.

*Proof.* A-sub-classes 1, 2, 4 follow from CaseAClosureForLK's sub-classes 1, 2, 3 (with the `K' ≁ R` refinement in A-sub-class 2 forcing `L_R` untouched, hence `nullified` unchanged via ASN-0086's Definition (nullified)). A-sub-class 3 invokes ASN-0086's Definition (nullified): `nullified(Σ') = nullified(Σ) ∪ (coverage(G_{τ_new}) ∩ A_rel^{Σ'})`, so the extension intersects `addr(·)` for some τ ∈ A_K^Σ iff the case hypothesis fails; the included condition gives the case-equation. ∎

The A_K-level enumeration is shared by Sh4, FDD, and SHCD; each preservation proof invokes CaseAClosureForAK at "Case A" with its own value of K and supplies only its Case B argument plus, where applicable, a Case C (retraction contraction) or Case D (Sh4's K ~ R simultaneous case).

**Sh0 — FromSlotCanonicalAndCardinalityFixed.** Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, every reachable state Σ satisfies: for each `K ∈ T_cat`, every tuple in `L_K^Σ` has `F` in canonical-slot form with `|slot_addrs(F)|` matching `c_F`:
...
**Sh1 — ToSlotCanonicalAndCardinalityFixed.** Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, every reachable state Σ satisfies the G-side analog of Sh0:
...
*Case A (`L_K^{Σ'} = L_K^Σ`).* IH applies pointwise to the unchanged set; existing tuples retain values by R2.
...
**Sh2 — FromSlotTargetRestricted.** Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, every reachable state Σ satisfies:
...
*Case A (`L_K^{Σ'} = L_K^Σ`).* Monotone preservation `t_F^Σ ⊆ t_F^{Σ'}` (L12a, ASN-0043, for `A_rel`; content-store monotonicity scaffolding for `A_doc`; their union for `A`) lifts the IH from `slot_addrs(F) ⊆ t_F^Σ` to `slot_addrs(F) ⊆ t_F^{Σ'}` on every existing tuple.
...
**Sh3 — ToSlotTargetRestricted.** Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, every reachable state Σ satisfies the G-side analog:
...
*Case A (`L_K^{Σ'} = L_K^Σ`).* Monotone preservation `t_G^Σ ⊆ t_G^{Σ'}` lifts the IH from `slot_addrs(G) ⊆ t_G^Σ` to `slot_addrs(G) ⊆ t_G^{Σ'}` on every existing tuple.
...
2. *R-registration.* `R ∈ T_cat` per the framework's *R-registration commitment*. This precondition is load-bearing for part (ii) below, which invokes Sh1 and Sh3 at `K := R` — Sh1 and Sh3 quantify over `K ∈ T_cat`, so without R-registration these citations have no licensed source.
3. Gates 0–4 admit the `Emit_R(Σ, d, F, G)` call, producing fresh tuple τ_new at state Σ' with `addr(τ_new) = a_emit(Σ, d)`.
...
2. *R-registration.* `R ∈ T_cat` per the framework's *R-registration commitment*. This precondition is load-bearing for Step 1 below, which invokes Sh1 and Sh3 at `K := R` — Sh1 and Sh3 quantify over `K ∈ T_cat`, so without R-registration the Step 1 discharge is unjustified.

*(Statement.)* At any call site where the framework's full gate stack (gates 0–4) admits `Emit_K(Σ, d, F, G)` — i.e., the call reaches substrate K.λ — ASN-0086's `wp_086` simplifies to `d ∈ dom(Σ.M) ∧ K ∈ T_admissible`. *(Effective wp at the framework's gate.)* Combining with each gate's admit conjunct, the framework's *effective wp* for the postcondition "a fresh `(a, F, G)` is deposited in `A_K^{Σ'}`" simplifies to
...
The three implications are mutually exclusive at any K. `shape(K).idem` is fixed by the registry at registration time (Definition — ShapeRegistry, lifetime constancy), so at any K either `shape(K).idem = ⊤` or `shape(K).idem = ⊥` — never both. The FDD antecedent (`K-under-FDD`) requires `shape(K) = (1, 1, A_doc, A_doc, ⊤)`, hence `idem = ⊤`. The SHCD antecedent (`K-under-SHCD`) requires `shape(K) = (1, 1, A_doc, A_doc, ⊥)`, hence `idem = ⊥`. These two are structurally incompatible by the `idem` divergence. The Sh4 antecedent (`K-with-idem = ⊤ ∧ not-under-FDD`) explicitly excludes FDD by the `not-under-FDD` conjunct and excludes SHCD by `idem = ⊤` (which rules out `idem = ⊥`). Hence at any K, at most one antecedent fires; `Π_K` is well-defined as the single active conjunct (or vacuously true when no antecedent fires — e.g., bare NonIdempotentDirectedPair, which is neither under FDD nor SHCD, so `Π_K = true`). At a rejected call site, the failing gate's conjunct evaluates false, `wp_eff = false`, `Emit_K` returns `⊥`, and no tuple is deposited; only `Π_K` captures discipline-mediated suppression at the wp, since the prior conjuncts `d ∈ dom(Σ.M) ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)` all hold at such a call.
...
*Step 4 — `addr(τ_new) ∉ nullified(Σ')` for the active-subset postcondition.* The postcondition phrases freshness over `A_K^{Σ'}` (active subset), not `L_K^{Σ'}` (audit slice). Concluding `τ_new ∈ A_K^{Σ'}` requires `addr(τ_new) ∉ nullified(Σ')` in addition to `τ_new ∈ L_K^{Σ'}` (which gates 0–4 followed by K.λ emission already deliver). Two sub-cases:
...
*Step 5 — Assembly of `wp_eff`.* Combined with gates 0–4's admit conjuncts (gate 0 contributes `d ∈ dom(Σ.M) ∧ K ∈ T_cat`; Sh-conf clauses (a)–(d) at gates 2 and 4 contribute `conf_K^Σ(F, G)`; gates 1 and 3 contribute the `Π_K` implications), and absorbing `K ∈ T_admissible` into `K ∈ T_cat`, the named `wp_eff` form follows. ∎

*Coverage-class disjointness from R at every non-R catalog row.* For any K registered at a catalog row whose shape tuple differs from R's `(*, 1, A, A_rel, ⊤)` on any component, Lemma — NonRSeparation gives `K ≁ R` immediately, so Step 2 lands in Case A.
...
`slots_F^Σ : L_K^Σ → ℘_fin(shape(K).t_F^Σ)` &nbsp; with &nbsp; `slots_F^Σ(a, F, G) = slot_addrs(F)`

`slots_G^Σ : L_K^Σ → ℘_fin(shape(K).t_G^Σ)` &nbsp; with &nbsp; `slots_G^Σ(a, F, G) = slot_addrs(G)`

These are total on `L_K^Σ` for any shape: Sh0/Sh1 guarantee canonical-slot form (so `slot_addrs` is defined); Sh2/Sh3 restrict the codomain to the registered target domain at the current state. The set accessor names use the slot symbol (F/G) rather than F/to to avoid colliding with the address-to-tuple-set accessors `from_K(a)`, `to_K(b)` introduced by the catalog templates below: those take an address argument and return a tuple-set, whereas `slots_F^Σ`/`slots_G^Σ` take a tuple argument and return a slot-address-set.

**Definition — PointSlotAccessors.** For shapes with `c_F = 1`:

`from₁ : L_K^Σ → shape(K).t_F^Σ` &nbsp; with &nbsp; `from₁(τ) = the unique element of slots_F^Σ(τ)`

For shapes with `c_G = 1`:

`to₁ : L_K^Σ → shape(K).t_G^Σ` &nbsp; with &nbsp; `to₁(τ) = the unique element of slots_G^Σ(τ)`
...
&nbsp;&nbsp;(i.a) Query `Observe_K(slot_addrs(F), slot_addrs(G), oper)` — well-typed because Sh-conf clauses (a)/(b) have gated canonical-form, so `slot_addrs(F)` and `slot_addrs(G)` are finite subsets of `T`. Observe_K's prefix-coverage semantics (ASN-0086) returns active tuples whose slot coverages contain the pattern addresses.

&nbsp;&nbsp;*Contract correctness.* `C(F, G, Σ)` equals the specified set: the post-filter (i.b) tests exact slot-address-set equality on finite slot-address sets, decidable in finite time, and any τ in the specified set passes both (i.a)'s Observe (by Prefix reflexivity on each pattern address) and (i.b)'s filter.
...
*Scope: single-process substrate; multi-process semantics is out of scope (see Open Questions).*

*Preservation under the contract.* Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, Sh4 holds at every reachable state under the contract, by induction on `↦*`. Fix `K ∈ T_cat` with `shape(K).idem = ⊤`.
...
*Step (Case A: `A_K^{Σ'} = A_K^Σ`).* By Lemma — CaseAClosureForAK, the broad transition falls into A-sub-class 1, 2, 3, or 4, and in each `A_K^{Σ'} = A_K^Σ` (with A-sub-class 3's complementary sub-regime routed to Case C below). Sh4 is inherited tuple-by-tuple from the IH on the unchanged active subset.

*Step (Case B: `A_K^{Σ'} = A_K^Σ ∪ {τ_new}`, a K.λ-step at type K with `K ≁ R`).* By Lemma — CaseAClosureForAK, Case B fires only at K.λ-steps at type `K' ~ K` with `K ≁ R`; concurrent nullification at `K ~ R` is routed to Case D below. By the *Emit_K routing commitment*, this K.λ-step originates as an `Emit_K` call (with K or `~`-equivalent registered type). By the *Sh4 idempotency contract* clause (iii), the emission proceeded only because `C(F, G, Σ) = ∅`. Let `τ_new` be the new tuple. Suppose, toward contradiction, that some prior `τ ∈ A_K^Σ` satisfies `(slot_addrs(F_τ), slot_addrs(G_τ)) = (slot_addrs(F_{τ_new}), slot_addrs(G_{τ_new}))`. Then by definition `τ ∈ C(F, G, Σ)`, contradicting `C(F, G, Σ) = ∅`. So no such `τ` exists, and `A_K^{Σ'}` extends with a slot-pair-unique element. The pairwise condition is preserved: existing pairs were Sh4-distinct by IH; `τ_new` shares no slot-pair with any prior active tuple.
...
**Template signatures are derived mechanically from shape components; template bodies are author-curated.** Each canonical shape pairs with a *base* predicate template family. Per-K opt-in and parametric extensions sit atop the base family and require additional registration (per-K disciplines) or additional arguments at evaluation time (type-index parameters).
...
*Per-class registration discipline.* Per-K discipline registrations (FDD, SHCD, and the *Sh4 idempotency contract*) apply at the `~`-equivalence-class level: registering at any `K ∈ T_cat` applies uniformly to every `K' ∈ T_cat` with `K' ~ K`. FDD attaches only to DirectedPair (`idem = ⊤`); SHCD attaches only to NonIdempotentDirectedPair (`idem = ⊥`); the two are structurally mutually exclusive at any single K since `shape(K).idem` is fixed.


## Per-Shape Template Walkthroughs

We walk the canonical shapes and exhibit the predicate templates each generates. Shape-distinctive rejection cases (e.g., G-side partition mismatch under Classifier vs Tuple-Classifier) are introduced inline at their first appearance; the canonical Sh-conf rejection patterns are catalogued at Sh-conf Rejection Patterns in the Conformance Axiom section and cited by reference. Every walkthrough below assumes the *Per-walkthrough convention* established in the Initial-State Baseline (`T_cat` declared at `Σ_init`; R registered with `shape(R) = (*, 1, A, A_rel, ⊤)`; `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`; `dom(Σ_init.L) = ∅` globally) and pre-allocates whatever documents and homes its emissions require — citations are omitted in each walkthrough.
...
*Walkthrough.* Register `K = is_claim` with the Classifier shape (`T_cat = {is_claim, R}`), pre-allocating a target document `d ∈ A_doc^{Σ_0}` and a home document `home_K ∈ dom(Σ_0.M)`. K.λ's first-emission branch fires.
...
*Sh4 suppression (re-emission of the same slot-pair).* From Σ_1, attempt `Emit_K(Σ_1, home_K, ∅, {(d, δ(1, #d))})` — identical slot-pair `(∅, {d})` as the admitted emission. Sh-conf clauses (a)–(d) all pass identically to the admission. Sh4 contract clause (i) computes `C(∅, {(d, δ(1, #d))}, Σ_1) = {τ ∈ A_K^{Σ_1} : slot_addrs(F_τ) = ∅ ∧ slot_addrs(G_τ) = {d}} = {σ_1}` (slot-pair matches σ_1 exactly; the empty-F slot-address set equality is decidable trivially on `∅ = ∅`). Clause (ii) *suppresses*: `Emit_K` returns `⊥`, state remains Σ_1, `A_K^{Σ_1} = {σ_1}` unchanged — the Classifier shape's `idem = ⊤` is precisely what blocks re-classifying the same document. ✗
...
A tuple `τ` is *classified as K* iff there exists an active classifier-tuple in `L_K` whose to-slot is `τ`. The signature changes from `A_doc → Bool` to `A_rel → Bool` per `t_G = A_rel`; the body's bound variable is renamed `τ → σ` to avoid capture against the new parameter name `τ`.
...
*Distinction from Resolution.* Resolution shares `t_G = A_rel` but has `c_F = 1`; Tuple-Classifier has `c_F = 0`.

*Walkthrough.* Register `K_tc = is_endorsed` with the Tuple-Classifier shape (`T_cat = {is_endorsed, K_neighbor, R}` where `K_neighbor` is any DirectedPair-shape relation whose tuples will be the endorsement targets). The endorsement target σ is emitted within the walkthrough (Emission 0) rather than pre-allocated, honoring the baseline's "no K.λ-steps before Σ_0". Pre-allocate two documents `d_src, d_tgt ∈ A_doc^{Σ_0}` (the `K_neighbor` from-slot and to-slot), a home `home_neighbor ∈ dom(Σ_0.M)` for `K_neighbor` emissions, a home `home_K ∈ dom(Σ_0.M)` for `K_tc` emissions, and a separate document `d_doc ∈ A_doc^{Σ_0}` for the G-side partition mismatch probe below.

*Emission 0 (creating the endorsement target).* `Emit_{K_neighbor}(Σ_0, home_neighbor, {(d_src, δ(1, #d_src))}, {(d_tgt, δ(1, #d_tgt))})`. Sh-conf at `K_neighbor`'s DirectedPair shape: F canonical-slot with `|slot_addrs| = 1`, matches `c_F = 1`; G canonical-slot with `|slot_addrs| = 1`, matches `c_G = 1`; both target-domain inclusions hold in `A_doc^{Σ_0}`. Sh4 contract at `K_neighbor`: `C(F, G, Σ_0) = ∅` (no prior `K_neighbor`-tuples). Clause (iii) issues. Admitted. Result Σ_1 with a new `K_neighbor`-tuple σ at fresh address `addr(σ) ∈ A_rel^{Σ_1}`. The endorsement target σ is now allocated.

*Emission 1 — Admission.* `Emit_{K_tc}(Σ_1, home_K, ∅, {(addr(σ), δ(1, #addr(σ)))})`. Sh-conf: F = ∅ canonical-slot, `slot_addrs(F) = ∅`, `match(0, c_F = 0)` ✓; clause (d) on F-side reads `∅ ⊆ -^{Σ_1} = ∅`, vacuously ✓. G canonical-slot, `slot_addrs(G) = {addr(σ)}`, `match(1, c_G = 1)` ✓; clause (d) on G-side reads `{addr(σ)} ⊆ A_rel^{Σ_1}` ✓ (since `addr(σ) ∈ dom(Σ_1.L) = A_rel^{Σ_1}` by Emission 0). Admitted. Result Σ_2 with new tuple ε_1 having `to₁(ε_1) = addr(σ)`.

*Rejection (G-side partition mismatch — Classifier-vs-Tuple-Classifier mirror).* From Σ_2, attempt `Emit_{K_tc}(Σ_2, home_K, ∅, {(d_doc, δ(1, #d_doc))})` — G targets the document address `d_doc ∈ A_doc^{Σ_2}` instead of a tuple address. Clauses (a)/(b)/(c) all pass. Clause (d) on G-side reads `{d_doc} ⊆ A_rel^{Σ_2}`, but `d_doc ∈ A_doc^{Σ_2}` and by R4 (TupleAddressDisjointness, ASN-0086) `A_doc^{Σ_2} ∩ A_rel^{Σ_2} = ∅`, so `{d_doc} ⊄ A_rel^{Σ_2}`. Rejected; state remains Σ_2. The G-side partition rejection mirrors Classifier's symmetrically — Classifier's `t_G = A_doc` blocks tuple-address targets, Tuple-Classifier's `t_G = A_rel` blocks document targets; the two shapes are the bipartite halves of `(0, 1)`. ✗

*Sh4 suppression (re-emission of the same slot-pair).* From Σ_2, attempt `Emit_{K_tc}(Σ_2, home_K, ∅, {(addr(σ), δ(1, #addr(σ)))})` — identical slot-pair `(∅, {addr(σ)})` as Emission 1. Sh-conf clauses (a)–(d) all pass identically to Emission 1. Sh4 contract clause (i) computes `C(∅, {(addr(σ), δ(1, #addr(σ)))}, Σ_2) = {τ ∈ A_{K_tc}^{Σ_2} : slot_addrs(F_τ) = ∅ ∧ slot_addrs(G_τ) = {addr(σ)}} = {ε_1}` (slot-pair matches ε_1 exactly). Clause (ii) *suppresses*: `Emit_{K_tc}` returns `⊥`, state remains Σ_2, `A_{K_tc}^{Σ_2} = {ε_1}` unchanged — the Tuple-Classifier shape's `idem = ⊤` is precisely what blocks re-classifying the same tuple. ✗

*Template evaluation at Σ_2.* `A_{K_tc}^{Σ_2} = {ε_1}`. `is_K(addr(σ)) ≡ (E τ ∈ A_{K_tc}^{Σ_2} :: to₁(τ) = addr(σ))` — witnessed by ε_1, so `is_K(addr(σ)) = true`. For any other `σ' ∈ A_rel^{Σ_2}` with `σ' ≠ addr(σ)`, no active tuple has `to₁(τ) = σ'`, so `is_K(σ') = false`.
...
- *`t_F = A_doc`* — `K_target_of`'s codomain `A_doc^Σ ∪ {⊥}` reads `A_doc^Σ` off `t_F` by the mechanical signature-derivation correspondence between shape components and template codomain symbols; without `t_F = A_doc`, the codomain symbol would shift (e.g., to `A_rel^Σ ∪ {⊥}` at `t_F = A_rel`) and the template's name and intended consumption pattern (filesystem-style "the K-target of this document") would no longer match.
...
*Strictly stronger than Sh4.* FDD's candidate set is broader than Sh4's (matching on from-slot alone rather than on the slot-pair).
...
*Preconditions.* K ∈ T_cat with `shape(K) = (1, 1, A_doc, A_doc, ⊤)`; FunctionalDependencyDiscipline registered at K under the *FDD functional-dependency contract*; Σ reachable from `Σ_init` with FDD's preservation theorem discharged on every step.
...
The layer commits to executing (i)–(iii) atomically with respect to other emitters and retractors at the same `~`-equivalence class of K, under the same scope as Sh4's *Scope: single-process substrate.* clause.

*Preservation under the discipline.* Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, the inductive argument runs three cases: Case A (active subset unchanged), Case B (single new tuple at K), and Case C (retraction-only contraction). Case D (the K=R simultaneous addition-and-contraction case from Sh4) is excluded by shape-tuple structure: FDD requires `shape(K) = (1, 1, A_doc, A_doc, ⊤)`, while Retraction has `shape(R) = (*, 1, A, A_rel, ⊤)`, which differs on `c_F`, `t_F`, and `t_G`; Lemma — NonRSeparation gives `K ≁ R` for every FDD-registered K, so no `Emit_R` step can extend `A_K`.
...
*Step (Case A: `A_K^{Σ'} = A_K^Σ`).* By Lemma — CaseAClosureForAK, the broad transition falls into A-sub-class 1, 2, 3, or 4, and in each `A_K^{Σ'} = A_K^Σ` (with A-sub-class 3's complementary sub-regime routed to Case C below). FDD is inherited from the IH on the unchanged active subset. K.λ at type `K'` with `K' ~ K` is the Case B regime; FDD's `(1, 1, A_doc, A_doc, ⊤)` differs from R's `(*, 1, A, A_rel, ⊤)`, so Lemma — NonRSeparation gives `K ≁ R` for every FDD-registered K, the `K' ~ K` branch never coincides with `K' ~ R`, and Case D from Sh4's enumeration is structurally excluded at FDD-registered K.

*Step (Case B: `A_K^{Σ'} = A_K^Σ ∪ {τ_new}`, a K.λ-step at type K).* Lemma — NonRSeparation gives `K ≁ R` for every FDD-registered K (FDD's shape differs from R's, as noted in the preservation header above). A K.λ-step at FDD-registered K therefore cannot extend `L_R^Σ` and cannot expand `nullified(Σ)`; no concurrent nullification of any τ ∈ A_K^Σ is structurally possible at this step, so the qualifier need not be threaded through Case B's hypotheses. By the *Emit_K routing commitment*, this K.λ-step originates as an `Emit_K` call (with K or `~`-equivalent registered type). By the *FDD functional-dependency contract* clause (iii), the emission proceeded only because `C_fd(F, Σ) = ∅`. Suppose, toward contradiction, that some prior `τ ∈ A_K^Σ` satisfies `from₁(τ) = from₁(τ_new)`. Under FDD's shape `(1, 1, A_doc, A_doc, ⊤)`, `from₁` of any tuple is the unique element of `slot_addrs(F)` (SlotAccessorTotality), so `from₁(τ) = from₁(τ_new)` iff `slot_addrs(F_τ) = slot_addrs(F_{τ_new})`. Hence `τ ∈ C_fd(F_{τ_new}, Σ)`, contradicting `C_fd(F, Σ) = ∅`. So no such `τ` exists, and `τ_new`'s from-slot value is fresh among `A_K^Σ`. Combined with the IH (which gives from-slot uniqueness off-diagonal on `A_K^Σ`) and reflexivity at the diagonal `(τ_new, τ_new)`, FDD holds on `A_K^{Σ'} = A_K^Σ ∪ {τ_new}`.
...
*Worked example (FDD + `K_target_of`).* Register `K = primary_author` at DirectedPair (`T_cat = {primary_author, R}`) with FDD opt-in at `K`, pre-allocating two documents `d_doc, d_other ∈ A_doc^{Σ_0}`, two author documents `d_alice, d_bob ∈ A_doc^{Σ_0}`, and a home `home_K ∈ dom(Σ_0.M)`.
...
*Worked example.* Register `K_neighbor = endorses` at NonIdempotentDirectedPair (`T_cat = {endorses, R}`) to supply retraction targets; R is registered per the *R-registration commitment*. Pre-allocate three commenter documents `d_src1, d_src2, d_src3 ∈ A_doc^{Σ_0}`, a common target document `d_tgt ∈ A_doc^{Σ_0}`, two agent documents `a_agent1, a_agent2 ∈ A_doc^{Σ_0}` (the retracting parties), and homes `home_endorses, home_R ∈ dom(Σ_0.M)`. The walkthrough runs a single linear timeline whose steps exhibit the shape's three F-cardinality regimes — bare (RETR1, `|slot_addrs(F)| = 0`) and two attributed regimes with distinct from-slot values (RETR2 with `{a_agent1}`, RETR3 with `{a_agent2}`) — followed by a Sh4 suppression of a duplicate attributed re-emission (RETR4).

**Pre-emission RETR0 (creating three retraction targets at K_neighbor).** Three NonIdempotentDirectedPair emissions issue from `home_endorses`: `Emit_{K_neighbor}(Σ_0, home_endorses, {(d_src_i, δ(1, #d_src_i))}, {(d_tgt, δ(1, #d_tgt))})` for `i ∈ {1, 2, 3}`. Each Sh-conf check passes (canonical-slot, cardinalities match, target-domain inclusion in `A_doc`). At `K_neighbor`'s `idem = ⊥`, Sh4 does not fire. Result Σ_3 with tuples σ_1, σ_2, σ_3 ∈ A_rel^{Σ_3} at distinct tuple addresses (T10a.7 EnumerationInjectivity, ASN-0034).

**Emission RETR1 (bare retraction of σ_1, `|slot_addrs(F)| = 0`).** Issued via the Nullify alias: `Nullify(Σ_3, home_R, addr(σ_1)) ≡ Emit_R(Σ_3, home_R, ∅, {(addr(σ_1), δ(1, #addr(σ_1)))})`.

*Sh-conf check at Σ_3.* F = ∅ canonical-slot trivially, `slot_addrs(F) = ∅`, `match(0, c_F = *)` ✓ (since `0 ∈ ℕ`). G canonical-slot, `slot_addrs(G) = {addr(σ_1)}`, `match(1, c_G = 1)` ✓. Target-domain: `∅ ⊆ A^{Σ_3}` vacuously ✓; `{addr(σ_1)} ⊆ A_rel^{Σ_3}` ✓ (since `addr(σ_1) ∈ dom(Σ_3.L)`). Sh4 contract clause (i) computes `C(∅, {(addr(σ_1), δ(1, #addr(σ_1)))}, Σ_3) = ∅` (no prior R-tuples). Clause (iii) issues. Admitted. Result Σ_4 with new tuple ρ_0 having slot-pair `(∅, {addr(σ_1)})`; by Definition (nullified, ASN-0086), `addr(σ_1) ∈ nullified(Σ_4)`, so `A_{K_neighbor}^{Σ_4} = {σ_2, σ_3}`.

**Emission RETR2 (attributed retraction of σ_2 by a_agent1).** Direct `Emit_R` with non-empty F: `Emit_R(Σ_4, home_R, {(a_agent1, δ(1, #a_agent1))}, {(addr(σ_2), δ(1, #addr(σ_2)))})`.

*Sh-conf check at Σ_4.* F canonical-slot, `slot_addrs(F) = {a_agent1}`, `match(1, c_F = *)` ✓ (since `1 ∈ ℕ`). G canonical-slot, `slot_addrs(G) = {addr(σ_2)}`, `match(1, c_G = 1)` ✓. Target-domain: `{a_agent1} ⊆ A^{Σ_4}` ✓ (since `a_agent1 ∈ A_doc^{Σ_4} ⊆ A^{Σ_4}`); `{addr(σ_2)} ⊆ A_rel^{Σ_4}` ✓. Sh4 contract: `C({(a_agent1, δ(1, #a_agent1))}, {(addr(σ_2), δ(1, #addr(σ_2)))}, Σ_4) = ∅` — the slot-pair `({a_agent1}, {addr(σ_2)})` differs from ρ_0's `(∅, {addr(σ_1)})` on both components. Clause (iii) issues. Admitted. Result Σ_5 with new tuple ρ_1; `addr(σ_2) ∈ nullified(Σ_5)`, so `A_{K_neighbor}^{Σ_5} = {σ_3}`.

**Emission RETR3 (attributed retraction of σ_3 by a_agent2 — distinct from-slot value).** `Emit_R(Σ_5, home_R, {(a_agent2, δ(1, #a_agent2))}, {(addr(σ_3), δ(1, #addr(σ_3)))})`.

*Sh-conf check at Σ_5.* F canonical-slot, `slot_addrs(F) = {a_agent2}`, `match(1, c_F = *)` ✓. G canonical-slot, `slot_addrs(G) = {addr(σ_3)}`, `match(1, c_G = 1)` ✓. Target-domain ✓. Sh4 contract: `C({(a_agent2, δ(1, #a_agent2))}, {(addr(σ_3), δ(1, #addr(σ_3)))}, Σ_5) = ∅` — the slot-pair differs from both ρ_0 and ρ_1 on at least the G-component (ρ_0, ρ_1 target σ_1, σ_2 respectively). Clause (iii) issues. Admitted. Result Σ_6 with new tuple ρ_2; `addr(σ_3) ∈ nullified(Σ_6)`, so `A_{K_neighbor}^{Σ_6} = ∅`. RETR2 and RETR3 jointly exhibit the migration-recipe payoff: distinct from-slot values are admitted as distinct slot-pairs at Sh4, preserving per-event audit-slice multiplicity that the bare alias collapses.

**Emission RETR4 (Sh4 suppression — duplicate attributed retraction).** From Σ_6, attempt a re-emission of RETR2: `Emit_R(Σ_6, home_R, {(a_agent1, δ(1, #a_agent1))}, {(addr(σ_2), δ(1, #addr(σ_2)))})` with the exact same slot-pair as RETR2.

*Sh-conf check at Σ_6.* All clauses (a)–(d) pass identically to RETR2. Sh4 contract clause (i) computes `C({(a_agent1, δ(1, #a_agent1))}, {(addr(σ_2), δ(1, #addr(σ_2)))}, Σ_6) = {τ ∈ A_R^{Σ_6} : slot_addrs(F_τ) = {a_agent1} ∧ slot_addrs(G_τ) = {addr(σ_2)}} = {ρ_1}` (the slot-pair matches ρ_1 exactly; ρ_0 has empty F and targets σ_1, ρ_2 has `{a_agent2}` and targets σ_3, so both are excluded by exact set-equality). Clause (ii) *suppresses*: `Emit_R` returns `⊥`, state remains Σ_6, `A_R^{Σ_6} = {ρ_0, ρ_1, ρ_2}` unchanged. The audit-slice compression at duplicate slot-pairs is the semantic departure from ASN-0086's apparent multiset semantics flagged in the Nullify Compatibility section. ✗

**Template evaluation at Σ_6.** `A_R^{Σ_6} = {ρ_0, ρ_1, ρ_2}` with slot-pairs `(∅, {addr(σ_1)})`, `({a_agent1}, {addr(σ_2)})`, `({a_agent2}, {addr(σ_3)})` respectively. Per the canonical base templates:

| Template | Evaluation at Σ_6 | Notes |
|---|---|---|
| `pair_R(∅, addr(σ_1))` | `true` | Witnessed by ρ_0; exhibits the bare regime via set-equality `slot_addrs(F_{ρ_0}) = ∅`. |
| `pair_R({a_agent1}, addr(σ_2))` | `true` | Witnessed by ρ_1; exhibits the attributed regime. |
| `pair_R({a_agent1}, addr(σ_1))` | `false` | No τ has slot-pair `({a_agent1}, {addr(σ_1)})`; ρ_0's F is `∅ ≠ {a_agent1}`. |
| `from_R(a_agent1)` | `{ρ_1}` | Only ρ_1 has `a_agent1 ∈ slot_addrs(F_τ)`; ρ_0 (empty F) and ρ_2 (F = `{a_agent2}`) excluded by membership. |
| `from_R(a_agent2)` | `{ρ_2}` | Symmetric. |
| `to_R(addr(σ_1))` | `{ρ_0}` | Only ρ_0 has `to₁(τ) = addr(σ_1)`. |
| `to_R(addr(σ_2))` | `{ρ_1}` | Only ρ_1 targets σ_2. |
| `from_addrs_R(addr(σ_2))` | `{a_agent1}` | The agent who retracted σ_2 (recorded in ρ_1's F). |
| `from_addrs_R(addr(σ_1))` | `∅` | ρ_0's F is empty, contributing no witness. The empty result distinguishes bare from attributed retraction at the audit-slice level. |
| `to_addrs_R(a_agent1)` | `{addr(σ_2)}` | The tuple a_agent1 retracted; collapses to a singleton because RETR4 was suppressed (otherwise this would still be a singleton under Sh4's set semantics). |

The bare (ρ_0) and attributed (ρ_1, ρ_2) regimes co-exist in the active subset under one registration. The `from_addrs_R(b) = ∅` outcome at the bare-retraction target distinguishes "no agent attributed" from "agent X attributed" at the audit-slice level — the central semantic distinction that the `c_F = *` cardinality bound makes accessible.
...
*Coverage class disjointness from R.* The BundledDirectedPair shape tuple `(1, *, A_doc, A_doc, ⊤)` differs from R's `(*, 1, A, A_rel, ⊤)` on `c_F`, `c_G`, `t_F`, and `t_G`; Lemma — NonRSeparation gives `K ≁ R` for every K at this shape, so EffectiveWpSimplification's wp simplification applies uniformly at every Sh-conf-admitted `Emit_K` call site for K at this shape.

*Worked example.* Register `K = citation.depends` with the BundledDirectedPair shape (`T_cat = {citation.depends, R}`), pre-allocating a citing document `d_cite ∈ A_doc^{Σ_0}`, three cited documents `d_src1, d_src2, d_src3 ∈ A_doc^{Σ_0}`, and a home `home_cite ∈ dom(Σ_0.M)`. The walkthrough runs a single linear timeline `Σ_0 → Σ_1 → Σ_2 → Σ_3` whose three steps exhibit the shape's three G-cardinality regimes — empty-G (BDP0, `|slot_addrs(G)| = 0`), bundled multi-target (BDP1, `|slot_addrs(G)| = 3`), and legacy single-target (BDP2, `|slot_addrs(G)| = 1`) — co-existing in the active subset at Σ_3.
...
*Canonical base templates.* Every K registered at this shape inherits the following five templates; the body forms carry over from DirectedPair's shape-mate without shape-specific deviation (the cardinality and target-domain components are identical; only `idem` differs, which does not affect template body form):
...
*Preservation under the single-home contract.* Under the baseline assumption `L_K^{Σ_init} = ∅` for every `K ∈ T_cat`, the single-home property holds at every reachable state under the contract, by induction on `↦*`. Fix `K ∈ T_cat` with SingleHomeCoverageDiscipline registered at fixed home `d_K`.
...
*Step (Case A: `L_K^{Σ'} = L_K^Σ`).* By Lemma — CaseAClosureForLK, the broad transition falls into one of three Case-A sub-classes (K.σ/K.α, K.λ at K' ≁ K, or arrangement-modifying), in each of which `L_K^{Σ'} = L_K^Σ`. The homed-set property is inherited tuple-by-tuple from the IH on the unchanged `L_K`. Nullification of any prior K-tuple does not affect the property: SHCD's body quantifies over `L_K^Σ` (not `A_K^Σ`), and `L_K` is monotone non-decreasing by R3, so retraction-extending steps preserve L_K-side membership.

K.λ at type `K' ~ K` is the Case B regime; SHCD's `(1, 1, A_doc, A_doc, ⊥)` differs from R's `(*, 1, A, A_rel, ⊤)`, so Lemma — NonRSeparation gives `K ≁ R` for every SHCD-registered K, and the `K' ~ K` branch never coincides with `K' ~ R`.

*Step (Case B: `L_K^{Σ'} = L_K^Σ ∪ {τ_new}`).* By the *Emit_K routing commitment*, the K.λ-step originates as an `Emit_K(Σ, d, F, G)` call. The *single-home contract* clause (i) admits the call only if `d = d_K`. Under clause (ii), K.λ's first/subsequent-emission protocol fires at home `d = d_K`, depositing τ_new at an address with `home(addr(τ_new)) = d_K` (ASN-0086, R0a-Cor1 places the deposit address in `d_K`'s link sub-allocator chain, so its home is `d_K` by L1a). So `home(addr(τ_new)) = d_K`. Combined with the IH on the older tuples, every τ ∈ L_K^{Σ'} has `home(addr(τ)) = d_K`.

The induction closes. ∎

*Well-definedness of `latest_K_for_addr`.* Define `emission_order(τ) := chain_index(addr(τ), d_K)`, using the *Link sub-allocator chain-index function* scaffolding clause. Under SHCD, every `τ ∈ S_d` has `home(τ) = d_K` (homed-set property), so `addr(τ)` is a chain element at `d_K` and `chain_index(·, d_K)` is single-valued (T10a.7, ASN-0034). Three ingredients close `argmax_{τ ∈ S_d} emission_order(τ)`:
...
*Gate 1 (SHCD single-home check).* Literal-equality test `d = d_K` reads `d_other = d_K` → `false`. The single-home contract clause (i) *rejects* the call outright with `⊥`; gates 2–4 and the post-gate-4 K.λ emission do not fire, and the candidate-set computation that would have run at gate 3 (had K been under Sh4 or FDD) is also skipped because the call short-circuits at gate 1. State remains Σ_2. ✗
...
The precondition `shape(K_res) = (1, 1, A_doc, A_rel, ⊤)` is what makes the template bodies well-typed: `resolved_by(τ, K_res)` invokes `to₁(ρ)` on `ρ ∈ A_{K_res}^Σ`, which under SlotAccessorTotality requires `K_res`'s `c_G = 1`; the comparison `to₁(ρ) = addr(τ)` lands `to₁(ρ) ∈ A_rel^Σ` opposite `addr(τ) ∈ A_rel^Σ`, which requires `K_res`'s `t_G = A_rel`. The other shape components (`c_F = 1`, `t_F = A_doc`, `idem = ⊤`) are inherited by the parametric column's commitment to *Resolution-shaped* `K_res` but are not invoked by the template bodies below.
...
*Canonical base template family.* One shape-specific deviation from DirectedPair's body form: `c_G = 0|1` requires explicit `⊥`-handling on G-side templates. F-side templates close over the totality of `from₁`; G-side templates must filter out tuples whose `to₁⁻` is undefined before applying the to-side accessor. Codomains follow the *Codomain convention for partial templates* established for partial accessors above:
...
*Worked example.* Register `K = attributed_by` with the Provenance shape (`T_cat = {attributed_by, R}`), pre-allocating an agent address `a_agent ∈ A_doc^{Σ_0}` (the human author of an event), an attributed event address `e_target ∈ A_doc^{Σ_0}` (the document event being attributed), and a home `home_K ∈ dom(Σ_0.M)`. The walkthrough exhibits both G-cardinality regimes (`|slot_addrs(G)| = 0` for agent-attribution-only and `|slot_addrs(G)| = 1` for attributed-to-event) co-existing in the active subset.
...
*Sh4 suppression on duplicate empty-G re-emission.* From Σ_2, attempt a re-emission of PROV0: `Emit_K(Σ_2, home_K, F_PROV0, G_PROV0)` with the same `F_PROV0 = {(a_agent, δ(1, #a_agent))}` and `G_PROV0 = ∅`. Sh-conf clauses (a)–(d) all pass exactly as for PROV0 at Σ_0 (the empty-G slot is canonical-slot trivially and matches `c_G = 0|1` at `0 ∈ {0, 1}`). Sh4 contract clause (i) computes `C(F_PROV0, G_PROV0, Σ_2) = {τ ∈ A_K^{Σ_2} : slot_addrs(F_τ) = {a_agent} ∧ slot_addrs(G_τ) = ∅} = {π_0}` (slot-pair matches π_0; π_1 has G-side `{e_target} ≠ ∅`, excluded by exact set-equality). Clause (ii) *suppresses*: `Emit_K` returns `⊥`, state remains Σ_2, `A_K^{Σ_2} = {π_0, π_1}` unchanged — the empty-G slot-pair behaves identically to non-empty-G slot-pairs under Sh4 at the `c_G = 0|1` mixed-cardinality shape. ✗
...
To verify the framework on a concrete instance, register `K = comment` at NonIdempotentDirectedPair (shape `(1, 1, A_doc, A_doc, ⊥)`) and exercise the parametric `_via` templates with a Resolution-shape `K_res`. Pre-allocate two documents `d_1, d_2 ∈ A_doc^{Σ_0}` and two home documents `home_K, home_R ∈ dom(Σ_0.M)` (`home_K` for Comment emissions, `home_R` for Resolution emissions; single-home not required for either, we use one home per relation for simplicity).

*Registered catalog for this walkthrough.* `T_cat = {comment, K_res, R}` (closure under `~` implicit). Distinctive entries: `comment` (the NonIdempotentDirectedPair K under exercise via the `_via` parametric extension, shape `(1, 1, A_doc, A_doc, ⊥)`, with no SHCD opt-in registered) and `K_res` (a Resolution-shape relation, shape `(1, 1, A_doc, A_rel, ⊤)`; registered at `Σ_init` per lifetime constancy but first exercised at Emission 3 below). The Pattern-0a (unregistered-type) reference under *Sh-conf rejection patterns at the Comment registration* below is verifiable against this explicit `T_cat`: any `K_ghost ∉ {comment, K_res, R}` falls outside the registered representative list.
...
*Explicit wp_eff computation.* By Corollary — EffectiveWpSimplification, `wp_eff(Emit_K(Σ_0, home_K, F_1, G_1), fresh (a_1, F_1, G_1) ∈ A_K^{Σ_1}) = (home_K ∈ dom(Σ_0.M)) ∧ (K ∈ T_cat) ∧ conf_K^{Σ_0}(F_1, G_1) ∧ Π_K(home_K, F_1, G_1, Σ_0)`. Conjunct-by-conjunct: `home_K ∈ dom(Σ_0.M)` ✓ by the pre-allocation; `K = comment ∈ T_cat = {comment, K_res, R}` ✓ by registration; `conf_K^{Σ_0}(F_1, G_1)` ✓ from Sh-conf clauses (a)–(d) verified above; `Π_K = true` vacuously, since `comment` registers as bare non-idempotent (`idem = ⊥`, no FDD, no SHCD), so no per-K antecedent fires per EffectiveWpSimplification's mutual-exclusivity argument. All four conjuncts hold, so `wp_eff = true` and `Emit_K` proceeds to deposit τ_1 at the fresh address `a_1 = [home_K.0.s_L.1]`.
...
*Why no Sh4 suppression fires.* `comment` has `shape(K).idem = ⊥` (NonIdempotentDirectedPair), so K is not registered under the *Sh4 idempotency contract*. The Sh4 contract's candidate-set check `C(F_1, G_1, Σ_1)` is *not executed* because `comment` is not under that contract. Compare against the hypothetical case where `comment` were registered with `idem = ⊤`: then `C(F_1, G_1, Σ_1) = {τ_1}` (slot-pair `({d_1}, {d_2})` matches τ_1 exactly), clause (ii) would suppress, and `Emit_K` would return `⊥`. The defining feature of NonIdempotentDirectedPair is exactly this admission of slot-pair-identical re-emissions as distinct active tuples. After Emission 1': `L_K^{Σ_1'} = A_K^{Σ_1'} = {τ_1, τ_1'}` — two active tuples with the *same* slot-address pair `({d_1}, {d_2})` but distinct tuple addresses `a_1 ≠ a_1'`.
...
**Sh-conf rejection patterns at the Comment registration.** The canonical Sh-conf rejection patterns (Sh-conf Rejection Patterns, in the Conformance Axiom section) instantiate at this registration as follows. Pattern-0a (unregistered type) — `Emit_{K_ghost}` for any `K_ghost ∈ T_admissible \ T_cat`, with the registered `T_cat = {comment, K_res, R}` admitting no such `K_ghost` — fails the `K ∈ T_cat` gate. Pattern-0b (unregistered home) — `Emit_K(Σ_5, d_unreg, ...)` for any `d_unreg ∉ dom(Σ_5.M)` — fails the `d ∈ dom(Σ.M)` gate. Pattern-2a (non-canonical F-endset) — e.g., `F_3 = {(d_1, δ(2, #d_1))}` — fails clause (a). Pattern-4a (cardinality mismatch) — e.g., `F_5 = {(d_1, δ(1, #d_1)), (d_2, δ(1, #d_2))}` against `c_F = 1` — fails clause (c). Pattern-4b (unallocated slot target) — e.g., `G_4 = {(d_ghost, δ(1, #d_ghost))}` with `d_ghost ∉ A^{Σ_5}` — fails clause (d) on the G-side under `t_G = A_doc`. In each case, `Emit_K` returns `⊥` and state remains Σ_5. ✗
...
The framework's vocabulary is the canonical-shape catalog's templates instantiated at registered K's. A new relation requires one shape-registry entry plus author-curated template bodies; shape misregistration produces wrong-signature predicates that Sh-conf cannot catch.


## Properties Introduced

Named claims defined in this ASN, listed for navigation. See each named section in the body for statement and proof.

*Axioms, lemmas, corollaries.* Sh-conf; NonRSeparation; CaseAClosureForLK; CaseAClosureForAK; Sh0; Sh1; Sh2; Sh3; Sh4; Sh4HoldsAtFDDRegisteredK; SlotAccessorTotality; AllocatedAddressAntichain; CoverageEqualityDecidability; LinkAddressNotPrefixOfEmit; RetractionSelfFreshness; EffectiveWpSimplification; NullifyActiveSubsetCompatibility.

*Layer-discipline contracts.* Sh4 idempotency contract; FDD functional-dependency contract; single-home contract.

*Definitions.* cov; cov_allocated; CanonicalSlotForm (slot_addrs); Shape; ShapeWellFormedness; CardinalityMatch (match); TypedRelationCatalog (T_cat); ShapeRegistry (shape); Conformance (conf_K^Σ); SetSlotAccessors (slots_F^Σ, slots_G^Σ); PointSlotAccessors (from₁, to₁, from₁⁻, to₁⁻); SingleHomeCoverageDiscipline; FunctionalDependencyDiscipline.

*Layer commitments/assumptions.* Emit_K routing commitment; R-registration commitment; Empty-baseline commitment; scaffolding-conforming-layer scaffolding (scaffolding clauses on element-level addresses, subspace partitions, content-store properties, document structure, link sub-allocator chains).
...
- **[scope boundary]** Can the empty-link-store baseline be relaxed? The *Empty-baseline commitment* (Scope and Substrate Scaffolding) currently requires `L_K^{Σ_init} = ∅` at every `K ∈ T_cat` for Sh4/FDD/SHCD's preservation to go through. A generalized preservation argument that handles pre-existing tuples — by adding a per-K registration-time conformance check that any prior tuples already satisfy the discipline's pairwise condition — would let conforming layers reuse the framework over existing substrate state without retrofitting.
