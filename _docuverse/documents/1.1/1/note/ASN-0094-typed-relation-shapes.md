...
with the last two conjuncts collapsing to `⊤` under the relational layer's committed operations (regime (i) of ASN-0086's wp simplification under the unit-depth retraction discipline). Sh-conf adds the conjuncts `K ∈ T_cat ∧ conf_K^Σ(F, G)`. The *effective wp* for `Emit_K` under Sh-conf is the conjunction:

`wp_eff(Emit_K(Σ, d, F, G), (a, F, G) ∈ A_K^{Σ'}) ≡ wp_086 ∧ K ∈ T_cat ∧ conf_K^Σ(F, G)`

Since `T_cat ⊆ T_admissible` (T_cat definition), `K ∈ T_cat` absorbs `K ∈ T_admissible`, and the effective wp simplifies under the relational layer's committed operations to:
