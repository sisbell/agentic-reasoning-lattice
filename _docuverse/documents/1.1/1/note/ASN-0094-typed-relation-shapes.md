...
*Coverage of the walkthroughs.* The per-shape walkthroughs do not exhibit uniform depth across the catalog. The deepest treatment is the *Worked Example: K = comment* section below at NonIdempotentDirectedPair (multiple emissions, slot-pair-identical re-emissions, resolution tuples, retraction, parametric `_via` consumption with full template evaluation tables at four states). The DirectedPair walkthrough exhibits the FDD opt-in path with admit-and-suppress emissions and `K_target_of` evaluation. The BundledDirectedPair walkthrough exhibits the three G-cardinality regimes (empty, multi-target, singleton) with template tables. The Classifier walkthrough exhibits a G-side partition-mismatch rejection between Classifier and Tuple-Classifier. The Tuple-Classifier walkthrough is brief because it is structurally identical to Classifier modulo a `t_G` substitution. The remaining shapes (Resolution, Retraction, Provenance) lack standalone admit-and-reject walkthroughs because: (i) Resolution is exercised end-to-end as `K_res` in the Comment walkthrough below (Emissions 3–5 demonstrate Resolution emission, target-domain admission, and parametric consumption); (ii) Retraction is exercised end-to-end as `R` in the Comment walkthrough's *Edge case: retraction of τ_1* (Emission 6) and in the Sh4 preservation theorem's Case D structural-bound argument; (iii) Provenance is structurally simpler than DirectedPair (the partial `to₁⁻` accessor's `⊥` dispatch is exhibited explicitly in the `latest_K_for_addr` and `K_target_of` walkthrough tables under NonIdempotentDirectedPair and DirectedPair, which suffice to verify the partiality machinery uniformly). Sh-conf's preservation theorems (Sh0–Sh4) and the slot-accessor totality lemma apply uniformly across all shapes by their universal quantification over `K ∈ T_cat`; the deeper Comment walkthrough verifies them by direct check at a single shape, and the per-shape *admission* and *rejection* annotations under each shape's section verify gate behavior pointwise without re-running the preservation induction.
