...
where `⟦σ⟧` is the I-coverage of span `σ` (T12, ASN-0034). The coverage is a subset of `T`, fixed at link creation and immutable thereafter (L12, ASN-0043).
...
**Depends.** S3★-aux (SubspaceExhaustiveness, ASN-0047) — `subspace(v) ∈ {s_C, s_L}` for every `v ∈ dom(M(d))`. S3★ (GeneralizedReferentialIntegrity, ASN-0047) — `subspace(v) = s_C ⟹ M(d)(v) ∈ dom(C)`, and `subspace(v) = s_L ⟹ M(d)(v) ∈ dom(L)`. L0 (SubspacePartition, ASN-0047) — `a ∈ dom(C) ⟹ subspace_I(a) = s_C`, and `a ∈ dom(L) ⟹ subspace_I(a) = s_L`.

**Derivation.** By S3★-aux, `subspace(v) ∈ {s_C, s_L}`. In the `s_C` case, S3★ gives `M(d)(v) ∈ dom(C)` and L0 gives `subspace_I(M(d)(v)) = s_C = subspace(v)`; in the `s_L` case, S3★ gives `M(d)(v) ∈ dom(L)` and L0 gives `subspace_I(M(d)(v)) = s_L = subspace(v)`. Either case yields `subspace_I(M(d)(v)) = subspace(v)`.
...
**Vacuous-subspace convention.** When `m_S(d)` is undefined — which occurs for either subspace `S ∈ {s_C, s_L}` when `V_S(d) = ∅` — no depth-`m_S(d)` predicate is available against which to restrict, and no V-position in subspace `S` exists in `dom(M(d))`; hence `R(d, e)|_S = ∅` unconditionally. We adopt the convention that the span-set in this vacuous case is the empty sequence `Σ_V^S = ⟨⟩`, and `⟦⟨⟩⟧_V := ∅`.
...
*Step 4 — Per-subspace uniqueness via the V-restricted ↔ full bridge.*
...
**Second configuration — no reach.** Take `L(ℓ).e₁ = {(b, δ(1, m_a))}`, whose coverage is `coverage(L(ℓ).e₁) = {t ∈ T : b ≤ t < b ⊕ δ(1, m_a)} = subtree(b)` (PrefixSpanCoverage, ASN-0043). Since `b ≼ t` with `#t = #b = m_a` forces `t = b`, the unique depth-`m_a` member of `subtree(b)` is `b` itself, so the precondition is exactly `b ∉ ran(M(d))`; under P-depth no deeper coverage member is reachable. By P-depth, each block's intersection with coverage is empty (no depth-`m_a` coverage member is reached). `Σ_V^{s_C} = ⟨⟩` and `Σ_V^{s_L} = ⟨⟩`. F-empty is exercised. ✓
...
**Fifth configuration — interior-offset block clip (`j > 0`, `c < n`).** This configuration exercises the `(j, c)` machinery of "Computation via Decomposition" (F-contig) in its non-trivial case: coverage clips the *tail or interior* of a multi-position block, recording a V-span whose start is `v + j` for `j > 0`.
...
**Sixth configuration — vacuous link subspace, second slot, second document (F-empty's vacuous branch, F-slot, F-multidoc).** This configuration exercises the Vacuous-subspace convention — an empty `s_L`-component arising because the subspace is *vacuous* (`m_{s_L}` undefined), not because coverage missed populated link material — together with slot-uniformity (F-slot) and document-independence (F-multidoc).
...
**Derivation.** Proved inline in "Computation via Decomposition" above (the Contiguity claim). ∎
