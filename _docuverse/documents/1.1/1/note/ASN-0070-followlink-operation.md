...
The two subspace components are disjoint by `s_C ≠ s_L` (SC-NEQ, from SubspaceConventionAxiom, ASN-0047) together with the V-restriction filter's subspace clause: every `t ∈ ⟦Σ_V^{s_C}⟧_V` satisfies `subspace(t) = s_C` and every `t ∈ ⟦Σ_V^{s_L}⟧_V` satisfies `subspace(t) = s_L`, so no `t` lies in both. This is a property of the filtered denotation sets alone — it does not appeal to S3★-aux's exhaustiveness, nor even to `dom(M(d))` membership (the filter's first-component constraint suffices). Disjointness here matches F0's own partition, where the same single-valued first-component projection (not exhaustiveness) supplies disjointness.
...
*Maximal runs partition `X`.* By the Characterisation, consecutivity is a single-valued successor/predecessor relation: each depth-`m_S(d)` subspace-`S` tumbler `t` has at most one consecutive successor (the tumbler agreeing with `t` on positions `1..m_S(d) − 1` with last component `t_m + 1`) and at most one consecutive predecessor (last component `t_m − 1`, when positive). A relation that is single-valued in both directions partitions any set into disjoint maximal chains: the connected components of such a relation are themselves chains, and distinct components are disjoint, so every element of `X` lies in exactly one maximal run.
