...
*Proof.* By D1, s ⊕ d = p (since s < p, #s = #d = #p, and equal length forces divergence(s, p) ≤ #s: T1 witnesses s < p in case (i) `k ≤ #s` or case (ii) `k = #s + 1 ≤ #p`, and #s = #p excludes case (ii) — it would force #s + 1 ≤ #s, refuted by NAT-addcompat's successor inequality #s < #s + 1 and NAT-order's irreflexivity — leaving case (i), whose witness k ≤ #s Divergence's case-(i) uniqueness identifies as divergence(s, p); this is the discharge WF performs in full). By D1 again, p ⊕ d' = reach(σ) (since p < reach(σ), #p = #d' = #reach, and equal length likewise forces divergence(p, reach(σ)) ≤ #p by the same prefix-case exclusion). Chaining:

  (s ⊕ d) ⊕ d' = reach(σ) = s ⊕ ℓ

We discharge TA-assoc's preconditions (Pos(d), Pos(d'), k_d ≤ #s, k_{d'} ≤ #d, ASN-0034):

- *Pos(d)*: λ is a well-formed span built by S4, so by Span validity d > 0.
- *Pos(d')*: ρ is a well-formed span built by S4, so by Span validity d' > 0.
- *k_d ≤ #s*: λ's Span validity (from S4) bounds the action point of d by #s.
- *k_{d'} ≤ #d*: ρ's Span validity (from S4) gives k_{d'} ≤ #p = #s, and level-uniformity of λ gives #d = #s, so k_{d'} ≤ #d.
...
- *Pos(ℓ)*: σ is a well-formed span by hypothesis, so by Span validity Pos(ℓ).
- *actionPoint(d ⊕ d') ≤ #s*: consequence (iii) gives actionPoint(d ⊕ d') = min(k_d, k_{d'}) ≤ k_d ≤ #s, the last bound being λ's Span validity discharged above.
- *actionPoint(ℓ) ≤ #s*: σ's Span validity (by hypothesis).
