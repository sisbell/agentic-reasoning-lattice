...
For the right interval, when reach(β) < reach(α), define ρ = (reach(β), reach(α) ⊖ reach(β)). WF's length precondition #reach(β) = #reach(α) we discharge through S6: for a level-uniform span σ, S6 gives #reach(σ) = #start(σ) — the result-length identity #(start(σ) ⊕ width(σ)) = #width(σ) composed with the defining #width(σ) = #start(σ) — so #reach(α) = #start(α) and #reach(β) = #start(β); level_compat(start(α), start(β)) supplies #start(α) = #start(β), and chaining gives #reach(β) = #start(β) = #start(α) = #reach(α). With reach(β) < reach(α) already in hand, WF gives a well-formed level-uniform span with reach(ρ) = reach(α).
...
- *Depends:*
  ...
  - S6 (LevelConstraint) — supplies the level-uniformity consequence that a span's start, width, and reach share one tumbler length, i.e. #reach(σ) = #start(σ) via the result-length identity #(start(σ) ⊕ width(σ)) = #width(σ); applied to α and β and combined with level_compat, it discharges WF's length precondition #reach(β) = #reach(α) for ρ (the dual #start(α) = #start(β) for λ being level_compat directly)
