...
For the *forward* orientation of (iv) — start(α) ≤ start(β) and reach(β) ≤ reach(α), the boundary ordering SC supplies for that case — the symmetric argument (exchanging α and β throughout the two-bound derivation just given) yields ⟦β⟧ ⊆ ⟦α⟧. This is exactly the denotation-containment precondition S11 consumes where the table invokes it, so the "By S11" cell rests on a discharged hypothesis, not an assumed one.
