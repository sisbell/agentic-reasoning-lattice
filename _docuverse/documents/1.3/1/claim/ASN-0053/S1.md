...
*Proof.* Define s' = max(start(α), start(β)) and r' = min(reach(α), reach(β)). The forward inclusion holds unconditionally, before any case split, by the total order T1 alone: take any t ∈ ⟦α⟧ ∩ ⟦β⟧, then start(α) ≤ t < reach(α) and start(β) ≤ t < reach(β), so t ≥ max(start(α), start(β)) = s' and t < min(reach(α), reach(β)) = r'; hence s' ≤ t < r'. Thus ⟦α⟧ ∩ ⟦β⟧ ⊆ {t : s' ≤ t < r'}.
