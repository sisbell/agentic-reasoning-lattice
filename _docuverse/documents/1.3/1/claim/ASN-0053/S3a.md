**S3a (MergeCommutativity).** Merging α and β yields the same point set as merging β and α: ⟦α⟧ ∪ ⟦β⟧ = ⟦β⟧ ∪ ⟦α⟧. This follows from set union being commutative. The equality is denotational — it asserts that the two orders denote the same set of positions, not that the merge constructs an identical (start, width) pair in each order (that the constructed pairs coincide is a separate fact, owed to the symmetry of S3's min/max endpoint formula, and is not what this lemma establishes).

We are proving that merge is commutative at the level of denotations: merging α with β produces exactly the same point set as merging β with α, with no dependence on the order of the operands. Formally, ⟦α⟧ ∪ ⟦β⟧ = ⟦β⟧ ∪ ⟦α⟧.
