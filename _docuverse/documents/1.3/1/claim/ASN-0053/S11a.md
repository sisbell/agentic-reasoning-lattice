**S11a (DifferenceSeparated).** For level-uniform spans α and β with level_compat(start(α), start(β)) in SC case (i) (separated) or (ii) (adjacent): ⟦α⟧ \ ⟦β⟧ = ⟦α⟧.

*Proof.* In both cases ⟦α⟧ ∩ ⟦β⟧ = ∅ (SC classifies (i) and (ii) as the disjoint cases). When the intersection is empty, removing β's positions from α removes nothing: ⟦α⟧ \ ⟦β⟧ = ⟦α⟧. The result is a span-set of exactly 1 span.  ∎

*Worked example.* Let α = ([1, 3], [0, 4]) and β = ([1, 10], [0, 2]). Then reach(α) = [1, 7] and start(β) = [1, 10]. Since [1, 7] < [1, 10], the spans are separated (SC case (i)). No position belongs to both spans, so ⟦α⟧ \ ⟦β⟧ = ⟦α⟧ = {t : [1, 3] ≤ t < [1, 7]}, a single span.

*Formal Contract:*

- *Preconditions:* α and β are level-uniform spans; level_compat(start(α), start(β)) holds; SC classifies the pair (α, β) as case (i) (separated) or case (ii) (adjacent).
- *Postconditions:* ⟦α⟧ \ ⟦β⟧ = ⟦α⟧, a span-set of exactly 1 span.
- *Axiom:* SC classifies cases (i) and (ii) as the disjoint cases — i.e. ⟦α⟧ ∩ ⟦β⟧ = ∅ holds by SC's classification of separated and adjacent pairs.
- *Frame:* α is preserved unchanged; β contributes no positions to the result.

- *Depends:*
  - SC (SpanClassification) — supplies the disjoint-case result ⟦α⟧ ∩ ⟦β⟧ = ∅ for cases (i) and (ii), which is the axiom on which the proof that ⟦α⟧ \ ⟦β⟧ = ⟦α⟧ rests