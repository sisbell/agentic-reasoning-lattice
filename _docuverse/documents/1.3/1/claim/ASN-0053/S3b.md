...
*Proof.* Both α and β are well-formed, so each σ ∈ {α, β} satisfies TA-strict's preconditions at (a, w) = (start(σ), width(σ)) — start(σ) ∈ T, width(σ) ∈ T, Pos(width(σ)), and actionPoint(width(σ)) ≤ #start(σ) — whence TA-strict's postcondition a ⊕ w > a gives the two strict advances

  start(α) < reach(α)  and  start(β) < reach(β).   (†)

These are precisely the "α/β is non-empty" facts on which the interiority arguments below rest; we cite them as (†) at each use rather than re-deriving them. Adjacency means reach(α) = start(β) or reach(β) = start(α). We handle each disjunct.

*Case A: reach(α) = start(β).* The merge produces γ = (start(α), r ⊖ start(α)) where r = max(reach(α), reach(β)) = reach(β), since reach(α) = start(β) < reach(β) by (†). So γ = (start(α), reach(β) ⊖ start(α)) with reach(γ) = reach(β). The shared boundary p = start(β) is interior to γ: start(α) < start(β), since by (†) start(α) < reach(α) and reach(α) = start(β); and start(β) < reach(β) = reach(γ), again by (†). Level compatibility holds by assumption.
