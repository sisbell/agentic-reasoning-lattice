...
- *Depends:*
  - Σ.C (ContentStore) — supplies the content store `Σ.C`, its domain `dom(Σ.C)` (the precondition `a₁, a₂ ∈ dom(Σ.C)`), and the value application `Σ.C(a)` the invariant declines to compare (`a₁ ≠ a₂` holds independent of whether `Σ.C(a₁) = Σ.C(a₂)`); grounded here at its defining claim as S7 grounds it
