...
- *Depends:*
  - GlobalUniqueness (ASN-0034) — supplies the allocation-event uniqueness invariant that S4's proof invokes directly to conclude `a₁ ≠ a₂`
  - T10a (AllocatorDiscipline, ASN-0034) — supplies the allocator discipline that is the formal precondition of S4; GlobalUniqueness fires only within a system conforming to T10a
- *Forward References:*
  - T3 (CanonicalRepresentation, ASN-0034) — supplies decidability of address comparison; cited as context for the structural test, not used in S4's proof