...
*Part (b).* Assume `v` satisfies S8a: `zeros(v) = 0`, `#v = m ≥ 2`, and `vᵢ ≥ 1` for every `i`. We are looking for the lower bound `rᵢ ≥ 1` at every position, and we observe that both branches already hold it without any fresh arithmetic. For `1 ≤ i < m` the copy rule gives `rᵢ = vᵢ`, and the S8a hypothesis supplies `vᵢ ≥ 1`, so `rᵢ ≥ 1`. At the action point `i = m` we do not re-derive the bound: OrdinalShift already exports the component lower bound `shift(v, n)_{#v} = v_{#v} + n ≥ 1` as a postcondition of the very `shift` we are studying, which at `#v = m` reads `rₘ = vₘ + n ≥ 1`. Every component of `r` is therefore at least 1, hence positive, so `zeros(r) = 0` and `(A i : 1 ≤ i ≤ #r : rᵢ > 0)`, with `#r = m ≥ 2`. Hence `shift(v, n)` satisfies S8a. ∎
