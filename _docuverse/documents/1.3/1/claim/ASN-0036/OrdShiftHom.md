...
*Part (b).* Assume `v` satisfies S8a: `zeros(v) = 0`, `#v = m ≥ 2`, and `vᵢ ≥ 1` for every `i`. For `1 ≤ i < m` the copy rule gives `rᵢ = vᵢ`, and the S8a hypothesis supplies `vᵢ ≥ 1`, so `rᵢ ≥ 1`. At the action point `i = m`, OrdinalShift's exported component lower bound `shift(v, n)_{#v} = v_{#v} + n ≥ 1` reads `rₘ = vₘ + n ≥ 1`. Every component of `r` is therefore at least 1, hence positive: `(A i : 1 ≤ i ≤ #r : rᵢ > 0)`. To read this back as `zeros(r) = 0` we unfold T4's defining count `zeros(r) = |{i : 1 ≤ i ≤ #r ∧ rᵢ = 0}|` (ASN-0034): since every `rᵢ ≥ 1`, no index meets the filter `rᵢ = 0`, so the counted index set is empty, `{i : 1 ≤ i ≤ #r ∧ rᵢ = 0} = ∅`, and NAT-card's empty-set case `|∅| = 0` (ASN-0034) gives `zeros(r) = |∅| = 0`. With `#r = m ≥ 2` all three S8a conjuncts hold, so `shift(v, n)` satisfies S8a. ∎
...
- *Depends:*
  ...
  - T4 (HierarchicalParsing, ASN-0034) — supplies the symbol `zeros` and its zero-count definition `zeros(t) = |{i : 1 ≤ i ≤ #t ∧ tᵢ = 0}|`, which part (b) unfolds at `t := r` to read the proved component bound `(A i : 1 ≤ i ≤ #r : rᵢ > 0)` back as `zeros(r) = 0`. The ASN convention grounds a symbol's defining claim directly in Depends wherever the symbol is written: S8a cites T4 because `zeros` appears in its axiom, and likewise part (b)'s final step writes `zeros(r)` explicitly, so OrdShiftHom must cite it too.
  - NAT-card (NatFiniteSetCardinality, ASN-0034) — supplies the empty-set cardinality `|∅| = 0` (the `k = 0` case of `|·|`) for the closing step of part (b): with no component zero the counted index set `{i : 1 ≤ i ≤ #r ∧ rᵢ = 0}` is empty, so `zeros(r) = |∅| = 0`.
