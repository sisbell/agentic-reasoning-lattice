...
- *Depends:*
  ...
  - S7a (DocumentScopedAllocation) — supplies the allocation rule that the document-level prefix of `a` equals the allocating document's tumbler, used in the Identification step to ground `origin(a) IS the allocating document's tumbler`
  - S7b (ElementLevelIAddresses) — supplies `zeros(a) = 3` for every `a ∈ dom(Σ.C)`, the precondition that activates T4's field-decomposition machinery in Well-definedness
  - S7d (DocumentAllocationDiscipline) — supplies the postcondition that distinct documents have distinct document-level tumblers, used in the Uniqueness step to establish `origin(a₁) ≠ origin(a₂)`
  - T0 (CarrierSetDefinition, ASN-0034) — supplies the carrier ℕ and the strict-positivity reading of non-separator components (`tᵢ ≠ 0 ⇔ tᵢ > 0`), used in Well-definedness to discharge positivity of each field's components
  ...
  - T4b (UniqueParse, ASN-0034) — supplies the partial projections N(a), U(a), D(a), E(a) used throughout to extract the node, user, document, and element fields from `a`
  - T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — supplies T4-validity of every `a ∈ dom(Σ.C)`, the precondition that licenses T4's field-decomposition machinery in Well-definedness