...
- *Depends:*
  ...
  - S8-depth (FixedDepthVPositions) — supplies the common depth `m` shared by all V-positions in a subspace; used to apply TS2 at a fixed depth when proving `succ` is injective
  ...
  - T1 (StrictTotalOrder, ASN-0034) — supplies irreflexivity (`v < v` is false); used to close the acyclicity argument by contradiction once TS4 gives `shift(v,1)>v`