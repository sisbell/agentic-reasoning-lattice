...
- *Depends:*
  - T4 (HierarchicalParsing, ASN-0034) — supplies the field correspondence used to interpret `zeros(a) = 3` as all four identifying fields being present
  - T4b (UniqueParse, ASN-0034) — supplies the projection definitions `N(a)`, `U(a)`, `D(a)`, `E(a)` asserted well-defined in the postconditions
  - T10a.4 (T4PreservationUnderDiscipline, ASN-0034) — supplies T4-validity (no adjacent zeros, `a₁ ≠ 0 ∧ a_{#a} ≠ 0`) and the bound `zeros(a) ≤ 3` that make the axiom tight