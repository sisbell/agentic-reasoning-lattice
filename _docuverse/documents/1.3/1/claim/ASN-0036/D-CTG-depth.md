...
Suppose otherwise. Let j be the least component in the range 1 ≤ j ≤ m − 1 at which v differs from u; then v agrees with u on components 1 through j − 1 and vⱼ ≠ uⱼ. Because #v = #u = m, the prefix clause T1(ii) — which requires one tumbler to be a proper, hence strictly shorter, prefix of the other — cannot order u and v; clause T1(i) orders them at their first differing component, which is exactly j. The hypothesis u < v therefore yields uⱼ < vⱼ. Now u and x agree across components 1 through m − 1, so xⱼ = uⱼ and hence xⱼ < vⱼ; and v agrees with x on components 1 through j − 1, since it agrees with u there and u agrees with x. Component j is thus the first point of disagreement between x and v as well, and T1(i) — reading off xⱼ < vⱼ with j ≤ m − 1 ≤ min(m, m) — witnesses x < v. But the guard supplies v < x, and T1's trichotomy forbids x < v and v < x at once. The supposition is untenable: v agrees with u and x on every component 1 through m − 1.

With v, u, and x sharing components 1 through m − 1, component m is the only site at which any two of them can differ, so T1(i) reads each surviving comparison off that component: u < v reduces to uₘ < vₘ, and v < x to vₘ < xₘ. D-CTG's guard u < v < x is therefore equivalent to the single ordinal betweenness uₘ < vₘ < xₘ. Contiguity of V_1(d) is determined by component m alone — structurally identical to the depth-2 case, where m − 1 = 1 and the shared prefix is the lone subspace component v₁ = 1. ∎
...
- *Depends:*
  ...
  - subspace (VPositionSubspaceIdentifier) — supplies the projection `subspace(w) = w₁` unfolded in the proof step `subspace(w) = w₁ = u₁ = 1`; the value subspace(w) = 1 it yields is what the constructed witness w must satisfy to meet D-CTG's `subspace(v) = 1` guard
- *Forward References:*
