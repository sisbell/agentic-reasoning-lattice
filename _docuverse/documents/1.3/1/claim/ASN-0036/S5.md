...
- *Depends:*
  - S0 (ContentImmutability) — supplies the content-immutability axiom whose vacuous satisfaction (in the trivial empty-transition system) is required for each witness to be a genuine model of S0–S3
  - S1 (StoreMonotonicity) — supplies the store-monotonicity axiom whose vacuous satisfaction (in the trivial empty-transition system) is required for each witness to be a genuine model of S0–S3
  - S2 (ArrangementFunctionality) — supplies the arrangement-functionality invariant the proof explicitly verifies for each witness construction (single entry / pairwise-distinct domain)
  - S3 (ReferentialIntegrity) — supplies the referential-integrity invariant the proof explicitly verifies for both constructions (sole I-address a lies in dom(C) by construction)
  - T3 (CanonicalRepresentation, ASN-0034) — supplies the component-wise equality biconditional used in both constructions to establish that the document addresses dᵢ and V-positions vₖ are pairwise distinct
- *Forward References:*
  - S4 (OriginBasedIdentity) — named as the companion invariant that together with S5 makes quotation a first-class structural relationship; not used in S5's proof