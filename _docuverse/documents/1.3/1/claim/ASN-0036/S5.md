...
- *Depends:*
  - Σ.M(d) (Arrangement) — supplies the arrangement family `Σ.M` and its domain `dom(Σ.M(d))`, against which the Postcondition's referencing pairs are stated (`vₖ ∈ dom(Σ.M(dₖ)) ∧ Σ.M(dₖ)(vₖ) = a`); grounded here at its defining claim as V-sub grounds it
  - Σ.C (ContentStore) — supplies the content store `Σ.C`, its domain `dom(Σ.C)` (the Postcondition's `a ∈ dom(Σ.C)`), and its value codomain `Val` (the witnesses' arbitrary `w ∈ Val`, which ContentStore names S5 as the consumer of); grounded here at its defining claim as S7 grounds it
