...
LLM-based extraction of claim statements from narrative prose. The ASN has a claim table (Label, Statement, Status) that serves as a roster — the LLM reads the surrounding prose for each claim and extracts just the formal math, stripping narrative, examples, and rationale.

Output: `claim-statements.md` in the ASN's manifests directory.
