"""Stabilize gate — format normalization + name population + deps generation.
...
# ---------------------------------------------------------------------------
# Name column population — inlined from core/format.py
# ---------------------------------------------------------------------------
...
def step_populate_names(asn_num):
    ...
    if has_name:
        ...
        for i in range(table_start + 2, len(new_lines)):
            ...
            if len(parts) < 3:
                continue
            # parts[0] is empty, parts[1] is label cell, etc.
...
def step_stabilize(asn_num):
    """Run format gate + name population + deps generation. Returns True on success.
    ...
    print(f"\n  [FORMAT] Format + Names + Deps", file=sys.stderr)
    ...
    if asn_path:
        ...
        if last_mtime is not None and current_mtime == last_mtime:
            print(f"  [FORMAT] Unchanged — skipping", file=sys.stderr)
            return True
    ...
    # Name column population (after format normalizes headers)
