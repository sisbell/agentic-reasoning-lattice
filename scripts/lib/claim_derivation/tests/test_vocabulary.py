"""Unit tests for the vocabulary plumbing.

Two units under test:
  - `_render_vocabulary` in claim_derivation.transclude — converts
    enrich's vocabulary list into a markdown bullet sidecar string
  - `aggregate_vocabulary` in lib.shared.common — concatenates
...
from lib.claim_derivation.transclude import _render_vocabulary
from lib.shared.common import aggregate_vocabulary


class RenderVocabularyTests(unittest.TestCase):
    def test_empty_list_returns_none(self):
        self.assertIsNone(_render_vocabulary([]))
        self.assertIsNone(_render_vocabulary(None))

    def test_single_entry(self):
        out = _render_vocabulary([
...
class AggregateVocabularyTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_no_sidecars_returns_placeholder(self):
        # Empty claim dir
        self.assertEqual(
            aggregate_vocabulary(self.claim_dir),
            "(no vocabulary)",
        )

    def test_nonexistent_dir_returns_placeholder(self):
        self.assertEqual(
            aggregate_vocabulary(Path("/nonexistent/path")),
            "(no vocabulary)",
        )

    def test_single_sidecar(self):
        (self.claim_dir / "T0.vocabulary.md").write_text(
            "- `T` — carrier set\n- `#a` — length operator\n",
        )
        out = aggregate_vocabulary(self.claim_dir)
    ...
    def test_multiple_sidecars_concatenate(self):
        (self.claim_dir / "T0.vocabulary.md").write_text("- `T` — carrier")
        (self.claim_dir / "Sigma.C.vocabulary.md").write_text("- `Σ.C` — store")
        out = aggregate_vocabulary(self.claim_dir)
        # Both stems present
        self.assertIn("### T0", out)
    ...
    def test_empty_sidecar_skipped(self):
        (self.claim_dir / "T0.vocabulary.md").write_text("")
        (self.claim_dir / "T1.vocabulary.md").write_text("- `T` — carrier")
        out = aggregate_vocabulary(self.claim_dir)
        self.assertNotIn("### T0", out)
        self.assertIn("### T1", out)

    def test_only_vocabulary_sidecars_picked_up(self):
