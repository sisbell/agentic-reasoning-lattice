"""Cone-review orchestrator — focused regional review/revise loop.

Drives one dependency cone (apex + same-ASN deps) through review/
revise cycles until the substrate predicate (`is_claim_converged`)
holds or `max_cycles` is exhausted, with a +1 confirmation cycle.
Same shape as the full-review orchestrator but with narrower scope:
the apex's grounding chain only.

Dispatches:
- claim_review agent (with foundation_labels narrowed to cross-ASN deps)
- claim_finding_override agent (per-finding REVISE/OBSERVE override)
...
def run_cone_review(asn_num, apex_label, dep_labels, max_cycles=3,
    ...
    Same structure as full-review but with narrower context: only the
    apex and its same-ASN dependencies.
    ...
    for label in all_cone_labels:
        ...
        for link in session.active_links(
            ...
            for cited in link.to_set:
                ...
                if (
                    dep_label and dep_label not in asn_labels
                    and dep_label not in cross_asn_deps
                ):
                    cross_asn_deps.append(dep_label)

    print(
        f"  Foundation: {len(cross_asn_deps)} cross-ASN deps",
        file=sys.stderr,
    )
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        if verdict == "ERROR":
            print(
                f"\n  [REGIONAL-REVIEW] FAILED on cycle {cycle} "
                f"(review error). Skipping.", file=sys.stderr,
            )
            break
        ...
        review_link = emit_meta(
            ...
            title=(
                f"Cone Review — {asn_label}/{apex_label} (cycle {cycle})"
            ),
        ...
        emitted_for_filter = [
            f for f in findings if f[0] in emitted_titles
        ]
        ...
        emit_meta(
            ...
            findings_summary=findings_summary(
                emitted_for_filter, revise_count,
            ),
        ...
        if dry_run:
            print(
                f"\n  [DRY RUN] {len(revise_findings)} revise finding(s), "
                f"no fixes.", file=sys.stderr,
            )
            break
        ...
        for title, _cls, finding_text in revise_findings:
            ...
            if emitted is None:
                print(
                    f"  [WARN] orphan revise skipped — finding "
        ...
        for label in [apex_label] + dep_labels:
            ...
            if from_addr is None:
                continue
            changes = sync_claim_citations(
                session.store, from_addr, label_index,
            )
            if changes is None:
                continue
            for direction in ("depends", "forward"):
                for added in changes[direction]["added"]:
                    print(
                        f"  [DRIFT-FIX] {label}: emitted "
                        f"citation.{direction} → {added}",
                        file=sys.stderr,
                    )
                for retracted in changes[direction]["retracted"]:
                    print(
                        f"  [DRIFT-FIX] {label}: retracted "
                        f"citation.{direction} → {retracted}",
                        file=sys.stderr,
                    )
        ...
        if revise_findings or any_changed:
            step_commit_asn(
                asn_num,
                f"cone-review(asn): {asn_label}/{apex_label} — "
                f"cycle {cycle}",
            )
        ...
        if (
            ...
            print(
                f"\n  [REGIONAL-REVIEW] Natural convergence at cycle "
        ...
        if revise_findings and not any_changed:
            print(
                f"  [REGIONAL-REVIEW] Revises filed but no fixes applied "
                f"this cycle. Breaking to confirmation.", file=sys.stderr,
            )
            break
    ...
    if not failed and not dry_run and not naturally_converged:
        ...
        if gate_result != "clean":
            print(
                ...
                f"violations remain ({gate_result})", file=sys.stderr,
            )
            failed = True
        else:
            cone_content = assemble_cone(
                asn_label, apex_label, dep_labels,
            )
            ...
            if confirm_verdict == "ERROR":
                ...
                confirm_raw_dir = (
                    WORKSPACE_DIR / "cone-sweep" / asn_label / "raw"
                )
                confirm_raw_dir.mkdir(parents=True, exist_ok=True)
                confirm_raw_path = (
                    confirm_raw_dir / f"{apex_label}-confirm.txt"
                )
                ...
                confirm_emitted_titles = {
                    e["title"] for e in emitted_findings
                }
                confirm_emitted_for_filter = [
                    f for f in confirm_findings
                    if f[0] in confirm_emitted_titles
                ]
                confirmation_revise_count = len(
                    filter_revise(confirm_emitted_for_filter)
                )
                emit_meta(
                    session, asn_label, review_num,
                    title=(
    ...
    if final_review_path is not None and not failed:
        with open(final_review_path, "a") as rf:
            rf.write("\n## Result\n\n")
            if converged:
                rf.write("Cone review converged.\n")
            else:
                rf.write(
                    f"Cone review not converged after {cycle} cycle(s).\n"
                )
