#!/usr/bin/env python3
"""
Run pipeline on all ASNs in dependency order.
...
Processes ASNs topologically (foundations first). For each ASN:
  ...
  2. If needed → run the unified pipeline
  3. If not → skip

Usage:
    python scripts/run-all.py                       # all active ASNs
    python scripts/run-all.py --only 34 36          # specific ASNs
    python scripts/run-all.py --exclude 40 42       # skip specific
    python scripts/run-all.py --topic foundation    # only a topic
    python scripts/run-all.py --dry-run             # show order + status
    python scripts/run-all.py --force               # ignore cached state
"""
...
def needs_update(asn_num, force=False):
    ...
    Returns True if:
      - force is set
      - ASN has no formal-statements.md
      - ASN reasoning doc is newer than last pipeline run
      - Any dependency's formal-statements is newer than last pipeline run
    """
    if force:
        return True
    ...
    # No formal-statements → needs update
    stmts_path = formal_stmts(asn_num)
    if not stmts_path.exists():
        return True

    # ASN modified since last run
    ...
    if yaml_path.exists():
        ...
        # ASN reasoning doc newer than last run
        if asn_path.stat().st_mtime > last_ts:
            return True

        # Any dependency export newer than last run
...
def main():
    parser = argparse.ArgumentParser(
        description="Run pipeline on all ASNs in dependency order")
    ...
    print(f"\n  Pipeline: {len(ordered)} ASNs in dependency order\n",
          file=sys.stderr)
