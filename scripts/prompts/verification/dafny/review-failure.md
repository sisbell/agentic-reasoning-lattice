...
1. **Genuine spec issue**: The claim is too strong, missing a precondition, or contradictory — Dafny exposed a real problem
2. **Proof artifact**: The claim is correct but the proof strategy is insufficient — needs better decomposition, helper lemmas, or different approach
...
For a proof artifact (claim is correct, proof needs work):
```
...
```
SPEC_ISSUE | <what the failure reveals about the claim>

What needs to change: <specific fix — missing precondition, over-strong claim, etc.>
```

Compare the Dafny declaration against the claim's *Formal Contract:* fields:
