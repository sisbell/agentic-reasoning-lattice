...
properties:
  - label: <short identifier, e.g. S0, Σ.C, vpos(S, o)>
    ...
    body: |
      <full property text: bold header + statement + justification + proof + formal contract>
```
...
5. Each property's body starts at its bold `**Label (Name).**` header and includes everything up to the next property's bold header (or end of section). Include the formal contract if present — it stays in the body.
6. **Bold sub-headers within a property are NOT separate properties.** Case splits, consequences, verification steps, and other internal structure (e.g., `**Case A:**`, `**Consequence 1:**`, `**Necessity.**`) stay in the parent property's body. Only `**Label (Name).**` or `**Definition (Name).**` headers start new properties.
...
9. Some sections contain multiple properties. List each one separately.
