...
You are extracting a narrative definition from a property file into a
...
1. For each definition listed above:
   ...
   - In the source property's narrative, replace the inline definition
     with a citation: "By **Definition (Name)**, ..." or
     "(see **Definition (Name)**)"

2. Do not alter the source property's formal contract.
...
4. Choose a label that fits the naming conventions in the table above.
   For a definition extracted from property LABEL, use a descriptive
   name like `Def-Span` or just the PascalCase name. Check the
   property table to avoid collisions.

## Output

Return the complete rewritten file. The file will contain the source
property (with citations replacing inline definitions) followed by the
