...
1. **Property coverage.** List every property from the ASN's Properties Introduced table. For each, determine:
   - Is it verified non-vacuously in at least one scenario?
...
2. **Operation coverage.** List every operation the ASN defines. Flag any operation that has no scenario exercising it. An unexercised operation is an untested contract.

3. **Boundary cases.** For each property and operation, identify the boundary condition that would stress it most. Has it been tested? Be concrete: "DELETE of the last remaining element" not "edge cases missing." Name the specific scenario that is needed.

## Standards

1. **Every property must be exercised non-vacuously** — vacuous verification is not coverage
2. **Every operation must have a scenario** — an operation without a worked example is an unverified claim
...
5. **Be specific** — cite the scenario name, the property label, and the exact step that is wrong or missing
...
**Needed**: [what kind of scenario would fill the gap — specific enough that the reviser can construct it]
```

Separate correctness issues from coverage gaps. The reviser needs to know which scenarios to fix and which to add.
