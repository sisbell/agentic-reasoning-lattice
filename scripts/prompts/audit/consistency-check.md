...
The ASN defines a property locally (status `introduced`) that already exists in the foundation statements. These should have been promoted to `cited` during a rebase. Properties with status `confirms LABEL (ASN-NNNN)` intentionally re-derive a foundation result — do not flag them.
