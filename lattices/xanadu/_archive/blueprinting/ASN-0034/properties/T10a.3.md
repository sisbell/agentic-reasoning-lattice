...
*Formal Contract:*
- *Precondition:* Parent allocator with sibling length `γ`; child spawned via `inc(t, k')` with `k' > 0`.
- *Postcondition:* All child outputs have length `γ + k' > γ`. Descendant at depth `d` has output length ≥ `γ + d`.
