...
- *Empty subspace.* V_S(d) = ∅. Then v = [S, 1, ..., 1] of depth m — a finite sequence of m naturals with m ≥ 2 ≥ 1, hence v ∈ T by T0 (CarrierSetDefinition, ASN-0034). The lower bound m ≥ 2 is required by S8-vdepth: once this position enters dom(Σ.M(d)), that axiom demands #v ≥ 2, so the parameter must satisfy the same bound. For the given m, this is the unique valid position: the tuple [S, 1, ..., 1] of depth m is fully determined by S and m. This is the canonical minimum position required by AX-6.
...
*Case m = 2.* There are no intermediate components (the range [2, m − 1] = [2, 1] is empty), so every position in V_S(d) has the form [S, k]. By D-MIN-VAL, min(V_S(d)) = [S, 1], giving k = 1 at the minimum. By D-CTG, the k-values are contiguous: for any v₁ = [S, k₁], v₂ = [S, k₂] with k₁ < k₂ in V_S(d), each intermediate [S, k] with k₁ < k < k₂ is a finite sequence of 2 naturals, hence belongs to T by T0 (CarrierSetDefinition, ASN-0034), and D-CTG places it in V_S(d). By S8-fin, V_S(d) is finite, so the k-values form a finite contiguous range starting at 1 — namely {1, 2, …, N}. For 1 ≤ j ≤ N − 1, shift([S, 1], j) = [S, 1 + j] by OrdinalShift (with j ≥ 1), giving positions at k = 2, …, N. Together with the minimum at k = 1, V_S(d) = {min(V_S(d))} ∪ {shift(min(V_S(d)), j) : 1 ≤ j ≤ N − 1}.

*Case m ≥ 3.* By D-CTG-depth (SharedPrefixReduction), all positions in V_S(d) share components 2 through m − 1. By D-MIN-VAL, min(V_S(d)) = [S, 1, …, 1] of depth m, so the shared intermediate components are all 1. Every position therefore has the form [S, 1, …, 1, k] for some k at the m-th component, with k = 1 at the minimum. By D-CTG — reduced to last-component contiguity by D-CTG-depth's postcondition (2) — the k-values are contiguous. By S8-fin, V_S(d) is finite, so the k-values are {1, 2, …, N}. For 1 ≤ j ≤ N − 1, shift([S, 1, …, 1], j) = [S, 1, …, 1 + j] by OrdinalShift (with j ≥ 1), giving positions at k = 2, …, N. Together with the minimum at k = 1, V_S(d) = {min(V_S(d))} ∪ {shift(min(V_S(d)), j) : 1 ≤ j ≤ N − 1}.
...
We verify the structural claims. By D-MIN-VAL, min(V_S(d)) = [S, 1, ..., 1] of depth m. Since `V_S(d) ⊆ dom(M(d)) ⊆ T` (Σ.M(d)), `min(V_S(d)) ∈ T`, satisfying OrdinalShift's precondition. By OrdinalShift, since m ≥ 2, the prefix rule (`shift(v, j)ᵢ = vᵢ` for `i < m`) copies components 1 through m − 1 unchanged, and the last-component increment (`shift(v, j)_m = v_m + j`) sets position m to 1 + j. For j ≥ 1, the explicit form is shift(min(V_S(d)), j) = [S, 1, ..., 1 + j].
...
- *Preconditions:* `V_S(d) ⊆ dom(M(d)) ⊆ T` (Σ.M(d)), providing carrier-set membership for OrdinalShift and T3; T0 (CarrierSetDefinition, ASN-0034) — tuple-constructed positions (empty-case [S, 1, ..., 1] and m = 2 intermediates [S, k]) are finite sequences of naturals with length ≥ 1, discharging T-membership where the Σ.M(d) chain does not reach; d satisfies D-CTG; D-CTG-depth (SharedPrefixReduction — for m ≥ 3, establishes sequential form of V_S(d)); S is a subspace identifier (S ≥ 1); m ≥ 2; S8-fin (dom(M(d)) is finite); in the non-empty case, m equals the common depth from S8-depth and S8-vdepth gives m ≥ 2; D-MIN holds for V_S(d); D-MIN-VAL (MinimumPositionValue — min(V_S(d)) = [S, 1, …, 1]).
...
That gives N + 1 = 4 positions. After an operation places new content at, say, [1, 2] — with whatever displacement mechanism the operation defines — the resulting V₁(d) must satisfy D-CTG and AX-6. Verifying this is the operation's obligation, not the predicate's.

**Empty case.** V₁(d) = ∅. For depth parameter m = 2, the unique valid insertion position is [1, 1]. AX-6 requires [1, 1] ∈ V₁(d) once the subspace becomes non-empty, so the position is exactly the one AX-6 demands. For m = 3, the unique valid position is [1, 1, 1]; by T3, this is a different tumbler — once placed, S8-depth locks the subspace to depth 3 as long as it remains non-empty (S8-depth's invariant constrains positions that coexist in a single reachable state; an operation that empties V₁(d) and repopulates at a different depth m' satisfies S8-depth provided all new positions share depth m'). The depth parameter distinguishes these as different predicates: for each m, exactly one position is valid.
