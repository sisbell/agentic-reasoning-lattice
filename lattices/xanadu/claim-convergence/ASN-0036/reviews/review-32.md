...
S8a establishes that every V-position v ∈ dom(Σ.M(d)) is an element-field tumbler satisfying `zeros(v) = 0 ∧ v₁ ≥ 1 ∧ v > 0`. Two facts underwrite the extraction. First, the element field has at least one component — T4's non-empty field constraint requires δ ≥ 1, as invoked in S8a — so `v₁` exists for every V-position. Second, the conjunct `v₁ ≥ 1` guarantees its value is a positive natural number. The extraction `v₁` is therefore total on dom(Σ.M(d)) — every V-position has a first component to extract, and that component is at least 1.
