...
*Exactness of the precondition.* We have shown that `p ∈ Σ.nodes` is sufficient — the freshness derivation establishes all postconditions, and the paragraph above establishes invariant preservation. We now show it is also necessary, making the precondition exact. The strongest obligation is N3(b): every non-root node in `Σ.nodes` must have its parent in `Σ.nodes`. BAPTIZE adds `n` with `parent(n) = p` by construction. In the `C = ∅` case, `n = inc(p, 1)` — TA5(d) extends `p` by one component, giving `parent(n) = p` directly. In the `C ≠ ∅` case, `n = inc(max(C), 0)` — TA5(c) modifies `max(C)` at its last component while preserving all prior components; since `max(C)` shares prefix `p` (it is a child of `p`), the result `n` also has `parent(n) = p`. In the post-state, N3(b) applied to the new node requires `p ∈ post(Σ.nodes)`. Since `p ≠ n` (they differ in length: `#n = #p + 1` in the first-child case, or `#n = #p + 1` again in the sibling case where `n` extends `p` by one component), and the postcondition gives `post(Σ.nodes) = pre(Σ.nodes) ∪ {n}`, we need `p ∈ pre(Σ.nodes)`. That is: `wp(BAPTIZE, postconditions ∧ N3-preservation) = (p ∈ Σ.nodes)`.
...
The remaining properties — N0 (Ghost Element), N1 (Identity by Assignment), N7 (Forward Reference Admissibility), N11 (Coordination-Free Disjointness), N12 (Local Serialization Sufficiency), N13 (Uniform Node Type), and N14 (No Node-Level Mutable State) — are definitional or derived from the tumbler algebra and do not depend on `Σ.nodes` as mutable state; BAPTIZE, which only adds an element to `Σ.nodes`, cannot violate any of them.
...
- Who is authorized to invoke BAPTIZE for a given parent node? Nelson says "the owner of a given item controls the allocation of the numbers under it" — formalizing this authorization model (actors, ownership, delegation, irrevocability) belongs in Account Ontology.
