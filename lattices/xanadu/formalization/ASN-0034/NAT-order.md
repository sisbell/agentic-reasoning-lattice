...
*Formal Contract:*
- *Axiom:* `< ⊆ ℕ × ℕ` (`<` is a binary relation on ℕ); `(A n ∈ ℕ :: ¬(n < n))` (irreflexivity); `(A m, n, p ∈ ℕ : m < n ∧ n < p : m < p)` (transitivity); `(A m, n ∈ ℕ :: (m < n ∧ ¬(m = n) ∧ ¬(n < m)) ∨ (¬(m < n) ∧ m = n ∧ ¬(n < m)) ∨ (¬(m < n) ∧ ¬(m = n) ∧ n < m))` (trichotomy). The non-strict relation `≤` on ℕ is defined by `m ≤ n ⟺ m < n ∨ m = n`.
