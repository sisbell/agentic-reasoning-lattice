...
- *Depends:*
  ...
  - NAT-closure (NatArithmeticClosureAndIdentity) — posits `1 ∈ ℕ` and closes ℕ under `+`, and fixes `0 + n = n`. Required in scope for the consumed contracts of TA-Pos, ActionPoint, Divergence, and ZPD, each of which cites `1 ∈ ℕ`, the additive identity `0 + n = n`, or closure under `+` in its body.
