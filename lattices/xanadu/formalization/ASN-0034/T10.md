...
Before extracting components, we must establish that `k` lies in the index domains of both `a` and `b`. From `p₁ ≼ a`, Prefix gives `#p₁ ≤ #a`, i.e. `m ≤ #a`; with `k ≤ m` and NAT-order's `≤`-transitivity Consequence, `k ≤ #a`, so `aₖ` is well-defined. Symmetrically, `p₂ ≼ b` gives `n ≤ #b`; with `k ≤ n` and `≤`-transitivity again, `k ≤ #b`, so `bₖ` is well-defined. Now Prefix's component clause applies: from `p₁ ≼ a` and `k ≤ m`, `aₖ = p₁ₖ`; from `p₂ ≼ b` and `k ≤ n`, `bₖ = p₂ₖ`. Hence `aₖ = p₁ₖ ≠ p₂ₖ = bₖ`. Since `k ≤ #a`, `k` lies in T3's index domain `{1, …, #a}`. By the contrapositive of the forward direction of T3 (`a = b ⟹ (A i : 1 ≤ i ≤ #a : aᵢ = bᵢ)`), the disagreement `aₖ ≠ bₖ` forces `a ≠ b`. ∎
...
- *Depends:*
  ...
  - NAT-order (NatStrictTotalOrder) — at-least-one trichotomy (for the `m ≤ n` vs `m > n` case split) and the `≤`-transitivity Consequence (for chaining `k ≤ m ≤ #a` and `k ≤ n ≤ #b`).
