...
Promote `n ≥ 1` to `n ≠ 0`: NAT-addcompat's `(A n ∈ ℕ :: n < n + 1)` at n = 0 gives `0 < 1`. NAT-order's `m ≤ n ⟺ m < n ∨ m = n` unfolds `n ≥ 1` to `1 < n ∨ 1 = n`. In the first disjunct, transitivity of `<` composes `0 < 1` with `1 < n` to yield `0 < n`; in the second, substitution of `n = 1` into `0 < 1` yields `0 < n`. By NAT-order's irreflexivity, `n ≠ 0`.
...
- *Depends:*
  ...
  - NAT-addcompat (NatAdditionOrderAndSuccessor) — strict successor inequality at n = 0 supplies the anchor `0 < 1`.
