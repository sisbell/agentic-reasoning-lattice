...
Promote `n ≥ 1` to `n ≠ 0`: NAT-closure posits `0 < 1` directly as the distinctness clause of its two named constants. NAT-order's `m ≤ n ⟺ m < n ∨ m = n` unfolds `n ≥ 1` to `1 < n ∨ 1 = n`. In the first disjunct, transitivity of `<` composes `0 < 1` with `1 < n` to yield `0 < n`; in the second, substitution of `n = 1` into `0 < 1` yields `0 < n`. By NAT-order's disjointness clause `(A m, n ∈ ℕ : m < n : m ≠ n)` instantiated at `(0, n)`, the hypothesis `0 < n` yields `0 ≠ n`; by symmetry of `=`, `n ≠ 0`.
...
- *Depends:*
  ...
  - NAT-order (NatStrictTotalOrder) — `≤`/`<` unfolding, transitivity of `<`, disjointness of `<` and `=`, used in `n ≥ 1 ⟹ n ≠ 0`.
  - NAT-closure (NatArithmeticClosureAndIdentity) — supplies the axiom `0 < 1` that anchors the `n ≥ 1 ⟹ n ≠ 0` promotion.
