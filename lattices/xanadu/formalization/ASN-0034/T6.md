...
*Ingredient 2 (field presence).* Let `zeros(t) = #{i : tᵢ = 0}`, computable by one scan. By T4b's absence-pattern postcondition: `N(t) ≠ ε` unconditionally; `U(t) ≠ ε` iff `zeros(t) ≥ 1`; `D(t) ≠ ε` iff `zeros(t) ≥ 2`; `E(t) ≠ ε` iff `zeros(t) = 3`. Each presence check reduces to comparing `zeros(t)` against a fixed threshold.
...
- *Depends:*
  - T0 (CarrierSetDefinition) — fixes carrier ℕ and subcarrier ℕ⁺.
