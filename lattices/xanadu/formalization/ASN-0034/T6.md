...
**T6 (DecidableContainment).** For any T3-canonical, T4-valid tumblers `a, b ∈ T`, the following are decidable from the addresses alone:
  ...
  (d) Whether `a` and `b` both carry a document field, their node and user fields match, and `D(a)` is a prefix of `D(b)`.
...
*Ingredient 1 (field extraction).* By T4, valid tumblers have at most three zeros, no two zeros adjacent, and neither leading nor trailing zero; by T0 every component lies in ℕ, and NAT-zero with NAT-discrete (at `m = 0`) promotes non-zero components to ℕ⁺. T4b's partial function `fields : T ⇀ Seq(ℕ⁺)⁴` is defined on the T4-valid subset and, under T3, uniquely determined: `fields(t) = (N(t), U(t), D(t), E(t))`, with absent fields represented by `ε` (unambiguous by T4a, since present segments are non-empty). Extraction is a single finite scan.

*Ingredient 2 (field presence).* Let `zeros(t) = |{i : 1 ≤ i ≤ #t ∧ tᵢ = 0}|` as in T4 (with `|·|` the cardinality of a finite subset of ℕ, distinct from T0's tumbler-length `#·`), computable by one scan. By T4b's absence-pattern postcondition: `N(t) ≠ ε` unconditionally; `U(t) ≠ ε` iff `zeros(t) ≥ 1`; `D(t) ≠ ε` iff `zeros(t) ≥ 2`; `E(t) ≠ ε` iff `zeros(t) = 3`. Each presence check reduces to comparing `zeros(t)` against a fixed threshold.

*Ingredient 3 (finite-sequence equality).* Sequences `S = (s₁, ..., sₘ)` and `R = (r₁, ..., rₙ)` are equal iff `m = n` and `(A i : sᵢ = rᵢ)`. Decidability of equality on ℕ follows from NAT-order's trichotomy together with NAT-discrete. The procedure terminates in at most `m + 1` steps; call this *componentwise comparison*.

*(a) Same node field.* Extract `N(a), N(b)`. Both are present (Ingredient 2). Apply componentwise comparison. Returns *yes* iff `N(a) = N(b)`.

*(b) Matching user and node fields under shared presence.* Extract `N(a), U(a), N(b), U(b)`. Require `zeros(a) ≥ 1 ∧ zeros(b) ≥ 1`; asymmetric absence returns *no*, and symmetric absence returns *no* since `U(a) = U(b) = ε` does not furnish a present shared user field. When both thresholds hold, apply componentwise comparison to `N(a) = N(b)` then `U(a) = U(b)`.
...
*Formal Contract:*
- *Preconditions:* `a, b ∈ T` satisfy T3 (CanonicalRepresentation) and T4 (HierarchicalParsing).
- *Depends:*
  ...
  - T3 (CanonicalRepresentation) — fixes component sequences so `fields(a)`, `fields(b)` are uniquely determined.
  - T4 (HierarchicalParsing) — zero-count bound, no-adjacent-zeros, boundary constraints, role-assignment of zeros as separators.
  - T4a (SyntacticEquivalence) — present field segments non-empty, making `ε` an unambiguous absence marker.
  - T4b (UniqueParse) — partial function `fields : T ⇀ Seq(ℕ⁺)⁴`; absence-pattern postcondition tying presence to `zeros(t)`.
- *Postconditions:*
  ...
  - (d) Terminates, returns YES iff `zeros(a) ≥ 2 ∧ zeros(b) ≥ 2 ∧ N(a) = N(b) ∧ U(a) = U(b) ∧ #D(a) ≤ #D(b) ∧ (A k : 1 ≤ k ≤ #D(a) : D(a)ₖ = D(b)ₖ)`; NO under D-absence or when `N(a) ≠ N(b)` or `U(a) ≠ U(b)`.
  - All decisions use only `fields(a)`, `fields(b)` and componentwise comparison on finite ℕ-sequences.
