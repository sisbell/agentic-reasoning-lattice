...
The Definition invokes two primitives not introduced here. The literal `1` — in the witness constraint `1 ≤ k`, in the agreement range `1 ≤ i < k`, and in the exhaustion clause `k = m + 1 ≤ n` (written `k = #a + 1 ≤ #b` in the Formal Contract) — is the `1 ∈ ℕ` posited by NAT-closure. The binary `+` closing `m + 1` (and every `n + 1` and `#a + 1` appearing in the proof below) is the operation posited by NAT-closure's signature clause `+ : ℕ × ℕ → ℕ`; each sum uses `+` at exactly that arity and re-enters ℕ by the signature's codomain commitment. NAT-closure is therefore declared directly in the Depends slot, matching T0's precedent of naming NAT-closure directly for the single `1` in `1 ≤ #a` and NAT-cancel's articulated rule that a symbol supplied only by NAT-closure is named directly rather than reached transitively through downstream NAT-* axioms that themselves depend on it.
...
- *Depends:*
  ...
  - NAT-closure (NatArithmeticClosureAndIdentity, this ASN) — supplies `1 ∈ ℕ` grounding the literal `1` in the witness constraint `1 ≤ k`, the agreement range `1 ≤ i < k`, and the exhaustion clause `k = #a + 1 ≤ #b`; and posits the signature `+ : ℕ × ℕ → ℕ` closing every successor term `m + 1`, `n + 1`, and `#a + 1` appearing in the Definition and proof. Named directly rather than reached transitively through NAT-addcompat, NAT-cancel, or NAT-discrete, following T0's precedent for the single `1` in `1 ≤ #a` and NAT-cancel's articulated precedent for declaring its direct dependency on NAT-zero.
