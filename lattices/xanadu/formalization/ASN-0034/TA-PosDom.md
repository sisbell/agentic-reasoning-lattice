...
*Proof.* Let `t ∈ T` with `Pos(t)` and `z ∈ T` with `Zero(z)`; we show `z < t`. Before the case analysis we extract a witnessing index `k` from `Pos(t)` directly. TA-Pos unpacks `Pos(t)` to the existential `(E i ∈ ℕ : 1 ≤ i ≤ #t : ¬(tᵢ = 0))`, so the set `S = {i : 1 ≤ i ≤ #t ∧ tᵢ ≠ 0}` is nonempty; T0's commitment that the index domain `{1, …, #t}` is a subset of ℕ gives `S ⊆ ℕ`. NAT-wellorder then supplies some `k ∈ S` with `(A n ∈ S :: k ≤ n)`. Membership `k ∈ S` yields (i) `1 ≤ k ≤ #t` together with `tₖ ≠ 0`. Minimality yields (ii) `tᵢ = 0` for all `1 ≤ i < k`: such an `i` lies in `{1, …, #t}`, and if `tᵢ ≠ 0` it would sit in `S` below `k`, contradicting `(A n ∈ S :: k ≤ n)`. For (iii) `1 ≤ tₖ`: T0 places `tₖ ∈ ℕ`; instantiating NAT-zero's disjunction `(A n ∈ ℕ :: 0 < n ∨ 0 = n)` at `n = tₖ` and excluding the equality branch via `tₖ ≠ 0` leaves `0 < tₖ`; NAT-discrete's forward direction `m < n ⟹ m + 1 ≤ n` at `m = 0, n = tₖ` gives `0 + 1 ≤ tₖ`; NAT-closure's additive identity `(A n ∈ ℕ :: 0 + n = n)` at `n = 1` rewrites `0 + 1 ≤ tₖ` to `1 ≤ tₖ`. Unpacking `Zero(z)` gives `zᵢ = 0` for all `1 ≤ i ≤ #z`. Distinguish two cases by the relationship between `#z` and `k`.

*Case `#z ≥ k`.* For `1 ≤ i < k`, `zᵢ = 0` (from `Zero(z)`, since `i < k ≤ #z`) and `tᵢ = 0` (by (ii)), so `zᵢ = tᵢ`. The case hypothesis gives `k ≤ #z`, and (i) gives `k ≤ #t`. At `i = k`: `zₖ = 0` (from `Zero(z)`), and (iii) supplies `1 ≤ tₖ`. NAT-addcompat's `n < n + 1` at `n = 0` gives `0 < 0 + 1`; NAT-closure's `0 + n = n` at `n = 1` rewrites this to `0 < 1`. NAT-order unfolds `1 ≤ tₖ` to `1 < tₖ ∨ 1 = tₖ`; composing with `0 < 1` via NAT-order transitivity (strict disjunct) or substitution (equality disjunct) yields `0 < tₖ`. By T1 case (i) with witness `k`, `z < t`.
...
- *Depends:*
  ...
  - NAT-closure (NatArithmeticClosureAndIdentity) — additive identity `0 + n = n` at `n = 1`, rewriting `0 + 1 ≤ tₖ` to `1 ≤ tₖ` and `0 < 0 + 1` to `0 < 1`.
  - NAT-discrete (NatDiscreteness) — forward form `m < n ⟹ m + 1 ≤ n`, used at `m = 0, n = tₖ` and at `m = #z, n = #t`.
  - NAT-order (NatStrictTotalOrder) — `≤` unfolding `m ≤ n ⟺ m < n ∨ m = n`, `<`/`≤` transitivity, and irreflexivity used both in the least-element witness and in the case analyses.
