...
Apply TA-strict with a = v and w = δ(n, m). Two preconditions:

*First: Pos(δ(n, m)).* From OrdinalDisplacement's exported postcondition `Pos(δ(n, m))`.

*Second: actionPoint(δ(n, m)) ≤ #v.* From OrdinalDisplacement's exported postcondition `actionPoint(δ(n, m)) = m`. Since #v = m, we have m ≤ m.
...
- *Depends:*
  ...
  - OrdinalDisplacement (OrdinalDisplacement) — supplies exported postconditions `Pos(δ(n, m))` and `actionPoint(δ(n, m)) = m` at the two precondition checks.
