...
For the action point: when `w > 0`, `actionPoint(w)` is the least index i with `wᵢ > 0`. Since `w₁ = 0`, we have `actionPoint(w) ≥ 2`. In `w_ord`, position k holds `w_{k+1}`, so the least k with `(w_ord)_k > 0` is `actionPoint(w) − 1`. Hence `actionPoint(w_ord) = actionPoint(w) − 1`.
