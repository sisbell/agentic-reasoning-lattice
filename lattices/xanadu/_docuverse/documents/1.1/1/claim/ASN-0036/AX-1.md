**AX-1 (Initial empty state).** In the initial state `Σ₀`, the content store is empty and no document possesses any V-mappings: `dom(Σ₀.C) = ∅` and `(A d ∈ D : dom(Σ₀.M(d)) = ∅)`.
...
*Formal Contract:*
- *Axiom:* `dom(Σ₀.C) = ∅` — the content store is empty in the initial state; `(A d ∈ D : dom(Σ₀.M(d)) = ∅)` — no document possesses any V-mappings in the initial state.
