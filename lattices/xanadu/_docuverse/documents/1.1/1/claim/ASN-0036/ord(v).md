...
When v satisfies S8a (zeros(v) = 0 ∧ v₁ ≥ 1), every component of v is strictly positive: (A i : 1 ≤ i ≤ m : vᵢ ≥ 1). The components of ord(v) are v₂, ..., vₘ — a subset of v's components — so every component of ord(v) is strictly positive: (A i : 1 ≤ i ≤ m − 1 : ord(v)ᵢ ≥ 1). Combined with ord(v) ∈ T and #ord(v) = m − 1 ≥ 1, this places ord(v) in S = {o ∈ T : #o ≥ 1 ∧ (A i : 1 ≤ i ≤ #o : oᵢ ≥ 1)} (TA7a, SubspaceClosure, ASN-0034). ∎

*Formal Contract:*
- *Preconditions:* `v ∈ T`, `#v ≥ 2`.
- *Definition:* `ord(v) = [v₂, ..., vₘ]` where `m = #v`.
- *Postconditions:* `ord(v) ∈ T` (length `m - 1 ≥ 1`, satisfying T0). `#ord(v) = #v - 1`. When `v` satisfies S8a (`zeros(v) = 0 ∧ v₁ ≥ 1`), `ord(v) ∈ S` — every component of `[v₂, ..., vₘ]` is positive since every component of `v` is positive by S8a.
