...
*Formal Contract:*
- *Preconditions:* `a₁, a₂ ∈ dom(Σ.C)` produced by distinct allocation events. By the set-containment corollary of AX-2 (allocation-mediated entry), `dom(Σ.C) ⊆ allocated(s)`, so each address was produced by a T10a-conforming allocation event (ASN-0034).
