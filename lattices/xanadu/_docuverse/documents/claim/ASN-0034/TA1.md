...
By T1, `a < b` gives two cases: (i) there exists a least `j ≤ min(#a, #b)` with `aⱼ < bⱼ` and `aᵢ = bᵢ` for `i < j`, or (ii) `#a < #b` and `aᵢ = bᵢ` for `1 ≤ i ≤ #a`.

*Case (ii).* `min(#a, #b) = #a`, so `k ≤ #a`. For `i < k`: `(a ⊕ w)ᵢ = aᵢ = bᵢ = (b ⊕ w)ᵢ`. At `i = k`: `aₖ = bₖ` gives `(a ⊕ w)ₖ = aₖ + wₖ = bₖ + wₖ = (b ⊕ w)ₖ`. For `i > k`: `(a ⊕ w)ᵢ = wᵢ = (b ⊕ w)ᵢ`. Lengths agree by TA0, so `a ⊕ w = b ⊕ w` by T3.
