...
- *Definition:*
  ...
  - *Field segment.* The *field segments* of `t` are the maximal contiguous sub-sequences of field-component positions, delimited by the field separators. The terminology names what a segment *is*; how many segments arise for a given `t` and whether each is non-empty are derived structural facts (proved in T4a), not stipulated here.
