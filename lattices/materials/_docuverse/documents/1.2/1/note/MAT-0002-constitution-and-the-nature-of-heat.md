# MAT-0002: Constitution and the Nature of Heat
*2026-04-21*
...
Third, different substances at the same temperature do not in general hold equal energy per unit mass — the 1819 Dulong–Petit measurements on thirteen elemental solids `[1819-dulong-petit]` record specific heats differing by a factor of six across substances at a common temperature. The theory must therefore distinguish the quantity that equalizes at thermal equilibrium from the quantity that merely rides along. We develop this distinction in the next two sections and return to the 1819 record itself in §"Empirical constraint: the atomic-heat regularity".
...
Before formulating the temperature functional we name a modeling commitment we will rely on; its full formal content is stated as premise (iv) of §"Conservation through encounters" below. Under the *centres-of-force modeling of encounters*, an encounter between two molecules is treated as including a conservative central interaction between the pair's centres of mass, derived from a scalar pair potential that vanishes outside a finite sphere of action; the modeling *articulates* only this COM-COM component of the encounter force. For species with no exchangeable internal DOF (β_k = 1 by constitution) the articulated component is the entirety of the encounter-level coupling. For species carrying internal modes coupled to translation (β_k > 1), the modeling *admits* but does not articulate the additional pair couplings responsible for the per-encounter T ↔ eint exchange whose equilibrium outcome P.enc_ratio_equilibration below pins; the form of those couplings is declared beyond what the molecule-level dynamics can reach in the present state of molecular knowledge, and (iv) commits only to their existence and conservativeness rather than denying them. This is a modeling commitment about inter-molecular interaction during an encounter, distinct from the constitutional use of "centre of force" in §"The constitutional commitment" above, which named one admissible internal structure of a single molecule.
...
One might think equilibration selects translational vis viva because only translational motion is exchanged across the boundary between molecules. Under the articulated COM-COM component of the encounter apparatus stated below — a conservative central force on the centres of mass — this is literally so in the β = 1 sub-case: each encounter leaves m.eint unchanged and only the translational sum moves between molecules. (For β > 1 species the unarticulated internal-mode couplings admitted by the modeling do exchange between T and m.eint per encounter; the observation is exact only under the articulated component alone.) Yet this observation alone does not identify *mean* translational vis viva per molecule as the quantity that equilibrates across a body or across species, and it gives no handle on why the β > 1 case, whose internal-to-translational ratio must still settle to the species-characteristic value, does so at all. Pinning down what equilibrates requires the distributional argument (stationary one-molecule and pair distributions are Maxwellian), and that is why P.temp_functional is postulated at this level rather than derived from the encounter apparatus alone.
...
**[P.beta_unity_if_no_internal]** If species k has no internal compartment — m.eint = 0 for every m of species k by the species' constitution, as in the pure centre-of-force realization — then β_k = 1.

With m.eint = 0 species-wide, the sum (+ m ∈ species_k(B.mols) : m.eint) is zero on every body state, and β_k = 1 follows algebraically from the defining formula, universally and without regime qualifier. The antecedent is deliberately narrow. A broader version — "species k carries internal modes dynamically decoupled from translation by the encounter law" — is not grounded here: a decoupled internal DOF preserves whatever initial Σ m.eint the species happens to carry through encounters (decoupling conserves each mode's energy rather than driving it to any particular value), so β_k = 1 in that sub-case would require the state condition Σ m.eint = 0, a property of the body's state rather than of the species, in tension with P.beta_species_invariant. Restricting the antecedent to the constitutional-absence case lets us drop any regime qualifier: β_k = 1 there follows from the species' constitution alone, not from the equilibrium apportionment of P.enc_ratio_equilibration. Which further realizations of β = 1 obtain depends on the intermolecular force law, which the theory does not prescribe in general.

Only one direction of the implication is grounded in the present apparatus. The converse — β_k = 1 ⇒ species k has no internal compartment — does not follow. The theory imports β_k − 1 as a species-characteristic equilibrium apportionment value following Clausius (see P.enc_ratio_equilibration), and does not commit to a strictly positive lower bound on that value when an internal compartment is present. A species carrying an internal DOF coupled to translation through the encounter law but whose equilibrium share happens to be zero would produce β_k = 1 despite having an internal compartment; the present apparatus contains no derivation that forbids such a case. We therefore state the implication one-way and do not promote it to an iff.
...
Let (m₁, m₂) be an encounter pair, with primed quantities denoting post-encounter states. The theory takes on four premises at the molecule level to ground the encounter-scale dynamics: (i) Newtonian mechanics holds in an inertial frame, so mass-times-acceleration equals net force; (ii) the mutual force between the two molecules during an encounter is internal to the pair and obeys action-reaction, so that the internal forces on the pair sum to zero; (iii) the encounter is isolated — no external impulse acts on the pair during the encounter interval, and configurations in which three or more molecules are simultaneously within each other's spheres of action are negligible in the dilute regime; (iv) the centres-of-force modeling *articulates* a conservative central component of the inter-molecular force in the *centres of mass* — passing through the line joining the two COMs, depending only on the scalar COM separation r, deriving from a scalar potential U(r) that vanishes outside the sphere of sensible action — with the further closure that for β_k = 1 species (no exchangeable internal DOF) the articulated component is the complete encounter-level coupling, while for β_k > 1 species the modeling admits but does not articulate the additional pair couplings to internal DOF, asserting only that any such couplings are themselves conservative and finite-range. These four are taken as postulates of the molecule-level dynamics. Under them the theory obtains:
...
This follows from (i) + (iii) + (iv): by (iv) every encounter-level pair coupling is conservative — the articulated COM-COM coupling deriving from U(r), and any unarticulated internal-mode couplings admitted for β_k > 1 species — so the total mechanical energy of the pair (translational + internal + all pair potentials) is a conserved quantity of its motion under (i) and (iii). Outside the sphere of sensible action — before and after the encounter — every pair potential vanishes (U(r) explicitly, the unarticulated potentials by the same finite-range condition that defines the encounter); their contribution cancels between initial and final states, and the translational-plus-internal sum returns to its initial value. Inside the encounter interval the pair potentials are nonzero and the translational-plus-internal sum alone fluctuates, but the total including those potentials does not.

Under the articulated component of (iv), the potential U(r) depends only on the scalar separation of the pair's centres of mass and therefore acts only in the equations of motion of the two COM coordinates; it does not couple to the internal configurations that carry m.eint. For β_k = 1 species with no exchangeable internal DOF, the articulated component is the complete encounter-level coupling: the encounter apparatus conserves the translational sum T(m₁) + T(m₂) strictly per encounter, m₁.eint and m₂.eint are each separately unchanged, and P.enc_energy is discharged by the separate invariance of its two pieces. For β_k > 1 species, the articulated component alone again leaves m.eint unchanged per encounter, but the unarticulated internal-mode couplings admitted by (iv) do exchange between translational and internal compartments — that exchange is exactly what P.enc_ratio_equilibration relies on. The grand-total invariance asserted by P.enc_energy still holds, secured by the closure clause of (iv) that any unarticulated couplings are themselves conservative; the strict per-encounter separation into translational and internal sums does not, and is not claimed.

For β > 1 the equilibrium apportionment 1 : (β − 1) requires precisely that per-encounter T ↔ eint exchange. The articulated component of (iv) does not furnish a mechanism — U(r) couples only to COM coordinates. The unarticulated internal-mode couplings (iv) admits do furnish one in principle, but their concrete form is declared beyond what (i)–(iv) can reach in the present state of molecular knowledge: (iv) commits to their existence and conservativeness, not to their shape. For β > 1 we therefore record the apportionment as a *separate postulate* at the molecule-level dynamics layer, following Clausius: the ratio of mean internal motion to mean translational agitation tends over many encounters to the species-characteristic value (β − 1), with the per-encounter mechanism left unmodelled. The ratio is imported rather than derived from (i)–(iv); (iv) and the postulate are jointly satisfiable because (iv) admits internal-mode couplings rather than denying them, and the postulate fixes only the equilibrium outcome of the dynamics those couplings drive.

Over many encounters this postulate fixes the aggregate:

**[P.enc_ratio_equilibration]** Molecule-level encounter dynamics drive the aggregate ratio (Σ m.eint) / (Σ T(m)) within each species — equivalently, the ratio of mean internal to mean translational vis viva per molecule — toward a species-characteristic steady-state value, and maintain the ratio at that value in the equilibrium regime. By the definition of β_k in §"The internal compartment and the ratio β" together with P.beta_species_invariant, that steady-state value equals β_k − 1; the numerical identification with β_k − 1 is algebraic and adds no content beyond those prior commitments. The dynamical content of the postulate is the convergence to, and stability at, the equilibrium ratio under continued encounters.

The postulate's relation to the encounter apparatus depends on which realization of β_k obtains. In the sub-case of β_k = 1 with no exchangeable internal DOF — the pure centre-of-force realization — the postulate is discharged by (i)–(iv) automatically: with no internal compartment, there is no apportionment to drive and no T ↔ eint exchange mechanism is needed; the articulated component of (iv) is the entirety of the encounter coupling there, and there is no unarticulated coupling to invoke. In the sub-case of β_k = 1 with an exchangeable DOF present but its equilibrium share equal to zero — admitted in §"The internal compartment and the ratio β" as a case the present apparatus does not forbid — the postulate still adds content beyond what the articulated component of (iv) delivers: driving (Σ m.eint) / (Σ T(m)) to 0, and maintaining it there in the equilibrium regime, must be supplied by the unarticulated internal-mode couplings (iv) admits, since the articulated COM-COM coupling alone leaves Σ m.eint unchanged on every encounter. For β_k > 1 the postulate is likewise an *added* commitment at the molecule-level dynamics layer, beyond what (iv) articulates: the articulated COM-COM coupling furnishes no per-encounter T ↔ eint exchange, and the many-encounter drift toward β_k − 1 is supplied by the unarticulated internal-mode couplings whose form (iv) does not pin down. In every case, no individual encounter under the articulated component of (iv) alone changes the apportionment; wherever the postulate adds content, the equilibrium ensemble is asserted to realise the species-characteristic ratio by means the modeling admits but does not articulate.
...
Like P.temp_functional, this is *postulated* within the scope of this note. The theory derives species-independence — by requiring the stationary cross-species velocity distribution to satisfy detailed balance under encounters for which total momentum and total translational energy are kinematic invariants; the equal-modulus conclusion M₁α₁² = M₂α₂² in the Maxwellian forms flows from the energy invariant M₁a² + M₂b² = M₁a'² + M₂b'² — but the derivation runs through the mathematical form of the velocity distribution, which lies outside the scope of this note. A parallel operational route in the theory defines temperature equality by the vanishing of cross-species energy flux, but that route also depends on distributional apparatus we do not import here.
...
What decides the commitment is the explanatory reach of the dynamical picture in domains the statical picture cannot reach. The theory accounts for the independence of gas viscosity from density and for the proportionality of viscosity to absolute temperature — dynamical predictions confirmed by transpiration experiments and by direct viscosity measurements, and inconsistent with a hard-sphere variant, fixing the encounter force to an inverse-fifth-power law `[1867-maxwell-dynamical-theory-of-gases]`. It accounts for the diffusion of one gas through another and for the conduction of heat through gases. It accounts for the law of equivalent volumes — P.equipartition (developed above) supplies the per-molecule energy equality on which that law turns, with the full derivation running through the pressure–volume–temperature apparatus deferred to a later specification. No statical picture exhibited in the corpus reproduces this family of consequences.

A separate conceptual point constrains the *form* any dynamical picture must take rather than discriminating against the statical picture. A theory of hard-body collision in which the outcome of an impact depends on the *absolute* motions of the bodies is ruled out by the relativity of motion: the outcome of a local interaction cannot depend on the frame in which it is observed. This forces collision laws within the dynamical picture to be framed in relative rather than absolute velocities. It does not bear on the statical picture, which posits no moving bodies for frame-invariance to apply to; we record it as a consistency condition on the form of the dynamical picture, not as a further ground for preferring it to the statical.
...
| P.beta_unity_if_no_internal | If species k has no internal compartment (m.eint = 0 species-wide by the species' constitution, as in the pure centre-of-force realization), then β_k = 1 (implication only; the converse is not grounded in the present apparatus, since the theory imports β_k − 1 as a species-characteristic apportionment value without committing to a strict positive lower bound when an internal compartment is present) | introduced |
...
| P.enc_energy | Each encounter between an isolated pair preserves total energy (translational + internal + all pair potentials), strictly per encounter, secured by (iv)'s closure that any unarticulated couplings are themselves conservative; under the articulated COM-COM component of (iv), the translational and internal sums are each separately conserved per encounter for β_k = 1 species, while for β_k > 1 species the unarticulated internal-mode couplings exchange between the two compartments and only the grand total is conserved | introduced |
| P.enc_ratio_equilibration | Molecule-level encounter dynamics drive the aggregate internal-to-translational vis viva ratio (Σ m.eint)/(Σ T(m)) within each species to a species-characteristic steady-state value equal to β_k − 1 (by β's definition together with P.beta_species_invariant), and maintain it there in the equilibrium regime; discharged trivially by (i)–(iv) only in the sub-case of β_k = 1 with no exchangeable internal DOF, postulated separately at the molecule-level dynamics layer following Clausius in every other case — including β_k = 1 with an exchangeable DOF whose equilibrium share is zero — since the articulated COM-COM component of (iv) couples U(r) only to COM coordinates and therefore furnishes no per-encounter T ↔ eint exchange, so the many-encounter drift is supplied by the unarticulated internal-mode couplings (iv) admits but does not articulate; jointly satisfiable with (iv) precisely because (iv) admits such couplings rather than denying them | introduced |
