...
For β_k = 1 the postulate is discharged by the encounter apparatus (i)–(iv) automatically: with no internal compartment, there is no apportionment to drive and no T ↔ eint exchange mechanism is needed. For β_k > 1 the postulate is an *added* commitment at the molecule-level dynamics layer, beyond (i)–(iv): since premise (iv) couples U(r) only to COM coordinates, the encounter apparatus provides no per-encounter T ↔ eint exchange, and the many-encounter drift toward β_k − 1 must be asserted separately, following Clausius. No individual encounter under (iv) changes the apportionment at all; the equilibrium ensemble is asserted to realise the species-characteristic ratio by means left outside the present apparatus.
