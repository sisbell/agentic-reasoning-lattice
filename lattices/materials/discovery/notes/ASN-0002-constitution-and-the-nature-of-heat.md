...
One might think equilibration selects translational vis viva because only translational motion is exchanged across the boundary between molecules — but the encounter apparatus stated below does not preserve the translational/internal partition per encounter: total energy is conserved, yet energy may flow between a molecule's translational and internal compartments during an encounter. The exchange-across-boundary argument alone does not pin down what equilibrates, which is part of why P.temp_functional is not derivable from the encounter apparatus alone and has to be postulated at this level.
...
P.equipartition is *derived* from P.temp_functional and P.temp_functional_species_independent together with the equilibrium definition (sub-populations at equilibrium share a temperature). It inherits its regime qualifiers from that postulate base. Moving from the single-body statement to the cross-species statement adds species-independence as the load-bearing commitment; the single-body statement alone does not carry it.
...
Two empirical grounds and one conceptual ground force the commitment. Empirically, a statical theory in which elastic response arises from inter-particle forces at rest predicts a fixed ratio between the elasticity of form and the elasticity of volume of any body. That ratio is wrong in two distinct domains. In fluids, form-elasticity is vanishing while volume-elasticity is considerable; the statical ratio is grossly exceeded, and fluidity itself becomes inexplicable on the statical picture. In solids, direct measurement of form-elasticity comes in smaller than the statical ratio predicts — elasticity of form too low relative to elasticity of volume, the same direction of failure as in fluids. Both empirical observations cut against the statical theory on the same structural quantity. Conceptually, a theory of hard-body collision in which the outcome of an impact depends on the *absolute* motions of the bodies is ruled out by the relativity of motion: the outcome of a local interaction cannot depend on the frame in which it is observed.
