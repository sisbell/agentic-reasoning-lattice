...
The postulate's relation to the encounter apparatus depends on which realization of β_k obtains. In the sub-case of β_k = 1 with no exchangeable internal DOF — the pure centre-of-force realization — the postulate is discharged by (i)–(iv) automatically: with no internal compartment, there is no apportionment to drive and no T ↔ eint exchange mechanism is needed. In the sub-case of β_k = 1 with an exchangeable DOF present but its equilibrium share equal to zero — admitted in §"The internal compartment and the ratio β" as a case the present apparatus does not forbid — the postulate still adds content beyond (i)–(iv): driving (Σ m.eint) / (Σ T(m)) to 0, and maintaining it there in the equilibrium regime, is not accomplished by an apparatus in which U(r) couples only to COM coordinates and therefore leaves Σ m.eint unchanged on every encounter. For β_k > 1 the postulate is likewise an *added* commitment at the molecule-level dynamics layer, beyond (i)–(iv): since premise (iv) couples U(r) only to COM coordinates, the encounter apparatus provides no per-encounter T ↔ eint exchange, and the many-encounter drift toward β_k − 1 must be asserted separately, following Clausius. In every case, no individual encounter under (iv) changes the apportionment at all; wherever the postulate adds content beyond (i)–(iv), the equilibrium ensemble is asserted to realise the species-characteristic ratio by means left outside the present apparatus.
