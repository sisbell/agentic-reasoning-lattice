...
A pure centre of force, having no internal structure at all, is the clearest case: there is no internal compartment for m.eint to populate, so β = 1 by construction. We ground the implication on this case. A molecule carrying internal modes dynamically decoupled from translation by the encounter law would preserve whatever initial Σ m.eint the species happens to carry through encounters — decoupling conserves each mode's energy rather than driving it to any particular value — so β_k = 1 in that case would require the state condition Σ m.eint = 0, a property of the body's state rather than of the species, in tension with P.beta_species_invariant. We do not rest the implication on the decoupled-mode route. Which further realizations of β = 1 obtain depends on the intermolecular force law, which the theory does not prescribe in general.
