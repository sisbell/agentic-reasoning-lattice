...
Three principles govern whether the [maturation protocol](protocols/maturation/note-to-claim.md) converges or spirals. See the [principles README](principles/README.md) for the full account.
...
**[Validation](principles/validation.md)** enforces structural integrity. A [validate-before-review](patterns/validate-before-review.md) pass runs before each review cycle: the mechanical validator checks the [Claim Document Contract](design-notes/claim-document-contract.md), and per-invariant fix recipes resolve any violations. The reviewer then sees structurally sound state. Three-quarters of typical non-convergence findings are structural violations a validator catches in one pass — duplicate declarations, dangling references, metadata disagreement, dependency cycles. Getting these out of the reviewer's path is what makes semantic review possible.
