...
The [note decomposition protocol](protocols/note-decomposition-protocol.md) works in layers, each adding detail the previous layer could not see.
...
The vocabulary firewall operates on domain vocabulary — theory channel terms vs evidence channel terms differ by domain, but the firewall mechanism is identical. Note decomposition classifies what the channels produce. Claim convergence verifies it. The lattice organizes it.

## The note as artifact

Discovery produces notes — Dijkstra-style prose with embedded claims. Note decomposition transforms them into structured per-claim files. Claim convergence refines those files into precise contracts. Verification translates the contracts into mechanically verifiable code.
...
These are not bugs. They're the natural result of writing to understand rather than writing to verify. Note decomposition exists because the discovery process is messy and downstream review needs clean inputs. The [Claim File Contract](design-notes/claim-file-contract.md) names what "clean" means — not perfect semantic content, but structurally valid form that claim convergence can operate on.

## Related

- [Note Decomposition Protocol](protocols/note-decomposition-protocol.md) — the formal protocol specification with safety/liveness properties, algorithm, and correctness arguments.
