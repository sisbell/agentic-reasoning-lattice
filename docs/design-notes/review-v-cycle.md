...
*Design note. See [Claim Convergence Guide](../guides/claim-convergence.md) for the practical pipeline reference.*
...
- A dependency cone where one claim can't reconcile its many dependencies is invisible to local (each claim looks correct in isolation) but caught by regional.
- A scope mismatch between a proof and its narrative claim is invisible to regional (not in any cone) but caught by global.

Single-scale iteration wastes cycles: local grinds on issues it can't resolve, global scans dozens of claims to find one issue. Multi-scale cycling routes each problem to the scale that can handle it efficiently.
...
| Low-frequency error | Cross-claim patterns (dependency cones) |
| Restriction | Assembling wider context |
| Prolongation | Propagating corrections to affected claims |
...
The cycle includes a mechanical detection mechanism for regional-scale problems. The [dependency cone](../patterns/dependency-cone.md) pattern identifies claims that keep getting revised while their dependencies are stable — a signal that local review is stalling on a tightly coupled cluster. Cone detection uses git revision frequency and YAML dependency metadata, with no LLM involvement.
