...
Convergence is the goal: all scales of review (claim, cluster, system) agree that the ASN is clean. Equilibrium patterns describe what breaks that.

Each was observed through operation. Resolution is not "try harder" — it is a specific structural fix.

## Patterns

- [Contract Sprawl](contract-sprawl.md) — a claim's formal contract keeps growing across cycles because it is the Genesis Attractor for a concept it introduced. Cycles never converge because the contract never stops moving.
