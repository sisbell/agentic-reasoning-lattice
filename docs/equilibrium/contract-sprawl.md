...
The same claim's contract grows across multiple review cycles.
...
With the split, new facts about the concept are added as new claims, not as clauses to an existing one. Previous citations stay valid because no existing contract changes. Growth is by accretion instead of mutation. The attractor is dissolved — no single claim is the sole home for the concept, so there is no accumulation point.
