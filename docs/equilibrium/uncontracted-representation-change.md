...
A protocol stage introduces a new unit of structure — per-claim files, per-proof files, typed dependencies, any split of what was previously unified — without specifying what well-formed output means. The structure lands on disk. No contract says what must hold. Downstream agents inherit the structure without the rules and spend review cycles on symptoms of violations that have no names.

## Forces

**Producing is easier than specifying.** Splitting a monolithic note into per-claim files is straightforward. Writing down what valid per-claim files look like — one body per file, filename matches label, references resolve, metadata agrees with content, no dependency cycles — is a separate effort that the protocol doesn't force you to do. The output exists before its contract does.
...
**Build the validator from the contract.** The contract gives the validator its checklist. The validator runs at two points: immediately after the transition (catches transition bugs) and before each downstream review cycle (catches drift from manual edits or earlier cycles).
...
The first instance is the claim derivation output contract, documented at [Claim Document Contract](../design-notes/claim-document-contract.md). The operational pattern that runs the validator and applies fixes is [Validate Before Review](../design-notes/validate-before-review.md); the design commitment behind it is the [Validation Principle](../principles/validation.md). The [maturation protocol](../protocols/maturation/note-to-claim.md) specifies this contract as safety property B5.
...
- [Validate Before Review](../design-notes/validate-before-review.md) — the operational pattern that resolves this failure mode once a contract exists. The validator checks the contract; per-invariant revise recipes fix the violations.
- [Maturation Stigmergic Protocol](../protocols/maturation/note-to-claim.md) — the protocol whose structural contract (B5) is the first instance of this resolution.