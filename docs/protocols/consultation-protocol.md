...
| `consultation.questions` | An intermediate. The question set generated by phase 1 — one doc per consultation session, holding all N questions produced by the per-channel decomposers as a coherent set. The artifact of one question-generation act. |
| `consultation.answer` | An intermediate. One Q+A pair per doc — the answer text plus the question it's responding to (embedded for self-containedness). One doc per question-and-answer combo, written by phase 2. Both channels' answers use the same classifier; the role token in the filename and the `[<role>]` header in the doc body distinguish them. |
