...
6. **validate-gate** — comprehensive Claim Document Contract validator with bounded structural-only fix recipes; same gate claim convergence runs before each review cycle

The module returns success when the gate reports clean. On failure, partial output is left in place with diagnostic; downstream stages do not operate on a failed derivation.
...
See the [claim derivation guide](../guides/claim-derivation.md) for how the phases work, output structure, and design decisions. Module spec in the [Maturation Stigmergic Protocol](../protocols/maturation/note-to-claim.md).
