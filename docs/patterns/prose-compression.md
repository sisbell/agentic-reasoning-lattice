...
**Formalization-stage compression.** Compression also happens during formalization's own review cycles, often accompanied by extracting the concept into its own property. Three cases from ASN-0036 cone review:

- **`E₁(a)` (ElementSubspaceProjection)** — created in D-CTG-depth cone cycle 2. Before: proofs used prose like "the element-field projection of a." After: `E₁(a)` as a named function with formal contract. Trigger: cone review found "E₁ undefined" — postcondition 3 in S8-crun was using the concept without naming it.
