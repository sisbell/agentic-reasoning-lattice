...
Agents produce reasoning about a domain. Each unit of reasoning — whether a document or a claim — declares what it depends on and builds on verified foundations below it. Over time, shared concepts emerge: the same definition in two places, the same claim assumed by three different proofs. These shared concepts are extracted into new foundation layers. The units that contained them absorb the shared version and depend on it.
...
**Claim lattice** — nodes are individual claims (theorems, definitions, axioms). Links are declared dependencies between claims. Formalization operates here: review/revise iteration refines each claim, regional review handles tightly coupled clusters, verify the whole checks coherence.

The two are not separate structures. The claim lattice is a finer view of the document lattice. Each document contains many claims. Each document-level dependency decomposes into claim-level dependencies.

**Blueprinting bridges the two.** It is [scope narrowing](scope-narrowing.md) + [representation change](representation-change.md) applied together: a single node in the document lattice (one reasoning document) becomes many nodes in the claim lattice (per-claim file pairs with explicit dependencies). This is how reasoning moves from discovery's document granularity to formalization's claim granularity. The content is the same — the lattice gains resolution.

With [Xanadu protocols](../vision.md), the two granularities collapse into one linked structure. Transclusion lets you view at either scope. A document is a collection of claim links. A claim is a node with links to its dependencies. The distinction between document and claim lattice becomes a viewing choice, not a structural one.
...
**[Review/revise iteration](review-revise-iteration.md)** — refines each unit to internal coherence. Find issues, fix them, re-check. The unit converges to a state where its claims are consistent and its dependencies are explicit. (Both lattices — documents during discovery, claims during formalization.)
...
Claim lattice (within strand model):
...
Rebase operates at both granularities. At the document level: an updated foundation ASN triggers re-verification of dependent ASNs. At the claim level: an updated foundation claim triggers re-verification of dependent claims within the same ASN (through the cone mechanism) or across ASNs (through the rebase).
...
[Dependency cone](dependency-cone.md) — when claim-level review/revise stalls on tightly coupled clusters within the claim lattice.
...
The claim lattice within ASN-0034 contains 62 claims. Within ASN-0036, 32 claims. Each was discovered through blueprinting (representation change from document to claim granularity) and refined through formalization.
...
The document lattice emerged during discovery on the Xanadu formalization — ASNs accumulated, dependencies formed, shared concepts were extracted. The claim lattice emerged during formalization — blueprinting decomposed documents into claims, and formalization revealed the fine-grained dependency structure within each document. The two granularities were recognized when the same patterns (meet, join, rebase) appeared at both scales.