...
**Claim-level** — nodes are individual claims (theorems, definitions, axioms). Edges are claim-level dependencies declared per-claim (`follows_from:`). Claim refinement operates at this granularity: review/revise iteration refines each claim, cone-scoped review handles tightly coupled clusters, comprehensive review checks coherence.
...
**Claim derivation bridges the two.** It is [scope narrowing](scope-narrowing.md) + [representation change](representation-change.md) applied together: a note at the note-level becomes many per-claim file pairs at the claim-level. This is how reasoning moves from discovery's note granularity to claim refinement's claim granularity. The content is the same — the lattice gains resolution.
...
Every growth event is one of these two operations. Discovery connects outward (creating joins). Extraction pushes inward (discovering meets). Claim refinement verifies both — checking that new joins cohere with their foundations and that new meets preserve the reasoning above them.
...
[Review/revise iteration](review-revise-iteration.md) — claim refinement applies review/revise to the lattice's claims, verifying and tightening what discovery produced.
...
At the claim level within ASN-0034 there are 62 claims. Within ASN-0036, 32 claims. Each was discovered through claim derivation (representation change from note to claim granularity) and refined through claim refinement.
...
The note-level view emerged during discovery on the Xanadu formalization — notes accumulated, dependencies formed, shared concepts were extracted. The claim-level view emerged during claim refinement — claim derivation decomposed notes into claims, and claim refinement revealed the fine-grained dependency structure within each note. The two granularities were recognized when the same patterns (meet, join, rebase) appeared at both scales.