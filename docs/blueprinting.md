...
Then, per-claim classification and dependency extraction. Each claim is analyzed independently: what type is it, what does its proof cite, what notation does it introduce.
...
The vocabulary firewall operates on domain vocabulary — theory channel terms vs evidence channel terms differ by domain, but the firewall mechanism is identical. Blueprinting classifies what the channels produce. Formalization verifies it. The lattice organizes it.
