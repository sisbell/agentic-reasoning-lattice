...
- *Related properties (stated separately):* injectivity is O1b; the account-level boundary (`zeros(pfx(π)) ≤ 1`) is O1a. These constrain `pfx` globally but are postconditions of those properties, not of the present axiom.
...
O0 governs the two-place predicate `owns(π, a)`, not the one-place effective-owner function `ω(a)`. The latter ranges over the principal registry to select the longest matching prefix, so its evaluation requires `(a, Π_Σ, pfx)` — the registry is consulted to enumerate candidates. The structural-decidability claim of O0 is therefore tight: a stranger handed the tumbler pair `(pfx(π), a)` can decide ownership; the same stranger handed only `a` cannot identify `ω(a)` without also being handed (a representation of) `Π_Σ`. This distinction is load-bearing for the design — it is why `tumbleraccounteq` accepts both tumblers as arguments and not just the address.
...
*Case `zeros(a) = 0`.* The tumbler `a` contains no zero-valued components, so by T4c (LevelDetermination), `a` is a node-level tumbler: the entire sequence constitutes the node field `N(a)`, and no user, document, or element fields are present. The definition stipulates `acct(a) = a`. This is well-defined: `a` is a valid tumbler by hypothesis, and `zeros(acct(a)) = zeros(a) = 0 ≤ 1`.

*Case `zeros(a) = 1`.* By T4b (UniqueParse), `fields(a)` decomposes `a` uniquely into a node field `N(a)` and a user field `U(a)`, separated by a single zero. No document or element fields are present. The definition stipulates `acct(a) = N(a) ++ [0] ++ U(a)`. But this concatenation reconstructs `a` itself — the entire tumbler is `N(a)` followed by one zero followed by `U(a)`, with no further components. Hence `acct(a) = a`. Validity: `a` satisfies T4 by hypothesis, and `zeros(acct(a)) = zeros(a) = 1`.

*Case `zeros(a) = 2`.* By T4b (UniqueParse), `fields(a)` decomposes `a` uniquely into `N(a)`, `U(a)`, and `D(a)`, with the structure `N(a) ++ [0] ++ U(a) ++ [0] ++ D(a)`. By T4a (SyntacticEquivalence), each field segment is non-empty; by T4's positive-component constraint, every non-separator component is strictly positive. Define `acct(a) = N(a) ++ [0] ++ U(a)`. The node field `N(a)` has `α ≥ 1` components (T4a), all strictly positive (T4); the user field `U(a)` has `β ≥ 1` components, all strictly positive. The constructed tumbler therefore has length `α + 1 + β`, begins and ends with a positive component (no leading or trailing zero, both per T4a), contains exactly one zero (at position `α + 1`), and has no adjacent zeros (T4a; the zero is flanked by the last component of `N(a)` and the first component of `U(a)`, both positive by T4). It satisfies T4 and has `zeros(acct(a)) = 1`.

*Case `zeros(a) = 3`.* By T4b (UniqueParse), `fields(a)` decomposes `a` uniquely into `N(a)`, `U(a)`, `D(a)`, and `E(a)`, with the structure `N(a) ++ [0] ++ U(a) ++ [0] ++ D(a) ++ [0] ++ E(a)`. Define `acct(a) = N(a) ++ [0] ++ U(a)`. The construction depends only on `N(a)` and `U(a)`, which by T4a have at least one component each and by T4 have all strictly positive components — invariants that hold regardless of whether `D(a)` and `E(a)` are present. The constructed tumbler therefore has exactly one zero, no adjacent zeros, no leading or trailing zero (T4a), and all non-separator components positive (T4); it satisfies T4 and has `zeros(acct(a)) = 1`.

The case distinction is exhaustive: T4's zero-count clause constrains `zeros(a) ∈ {0, 1, 2, 3}`, and each value is handled. In every case, `acct(a)` is a valid tumbler satisfying T4, and `zeros(acct(a)) ≤ 1`. The field extraction functions `N(a)` and `U(a)` are uniquely determined by T4b (UniqueParse), and component-wise access is decidable from T3 (CanonicalRepresentation), so the construction is computable from `a` alone. ∎
...
*Notation.* `dom(π)` here applies to a principal and yields the prefix-defined subset of `T`; this is distinct from `dom(A)` of T10a (ASN-0034), which applies to an allocator and enumerates its per-stream chain `{tₙ : n ≥ 0}`. The two are disambiguated by argument kind throughout — principal vs. allocator — and never collide in any expression.

Before developing ownership domains' nesting structure, we extract a structural fact about the prefix relation `≼` that the subsequent proofs invoke repeatedly. The fact is a direct consequence of Prefix (PrefixRelation) of ASN-0034 and is independent of T5 (ContiguousSubtrees), but its repeated use across O2 (Step 2), O7(a), OwnershipDomainPermanence (Step 3), NestingByDelegation, and OwnershipDomainPermanence★ warrants stating it once as a named lemma:
...
The lemma admits an immediate specialization: when `p = pfx(π₁)`, `q = pfx(π₂)`, and `x = a` for some `a ∈ Σ.B`, any two principals covering `a` have nested prefixes. The same specialization with `x = pfx(π')` (a principal's prefix in place of an address) supports the most-specific covering arguments in O7(a) and OwnershipDomainPermanence (Step 3).

**MostSpecificCoveringUnique (derived).** Whenever a principal achieves the maximum prefix length among the covering principals of a tumbler, that principal is unique. Stated as a derived corollary of the covering-chain lemma and O1b:

  `(A Σ reachable, x ∈ T, π₁ π₂ ∈ Π_Σ : pfx(π₁) ≼ x ∧ pfx(π₂) ≼ x ∧ #pfx(π₁) = #pfx(π₂) = max{#pfx(π) : π ∈ Π_Σ ∧ pfx(π) ≼ x} ⟹ π₁ = π₂)`

*Proof.* Both `pfx(π₁)` and `pfx(π₂)` are prefixes of `x`, so by the covering-chain lemma above they are `≼`-comparable: either `pfx(π₁) ≼ pfx(π₂)` or `pfx(π₂) ≼ pfx(π₁)`. With `#pfx(π₁) = #pfx(π₂)` (equal lengths), the Prefix (PrefixRelation) of ASN-0034 collapses each direction to componentwise equality on `1..#pfx(π₁)`, so the two prefixes coincide as tumblers: `pfx(π₁) = pfx(π₂)`. By O1b (PrefixInjectivity), distinct principals have distinct prefixes; the contrapositive yields `π₁ = π₂`. ∎

The corollary is the precise content of condition (ii) of the `delegated` relation when read jointly with O1b: condition (ii) asserts the existence of a most-specific covering principal (`π` with `#pfx(π)` maximal among covering principals), and the corollary asserts this principal is unique. O7(a)'s case analysis, DelegatorAllocatesPrefix's identification `π_a = π_d`, and OwnershipDomainPermanence (Step 3) all rely on this joint reading; we name it once so subsequent citations are explicit. The corollary requires no reachability beyond what is already needed to populate `Π_Σ` — every reachable state has finite `Π_Σ` by FiniteRegistry (below), so the maximum in the statement is well-defined whenever the covering set is non-empty.
...
The ownership model rests on seven transition-discipline axioms (O12, O13, O14, O15, O5, O16, O18) together with the primitive allocation relation `allocated_by_Σ` and two structural constraints on the primitive `pfx` function (O1a, O1b — stated and justified in *Account-Level Boundary* and *Prefix Injectivity* above), from which we derive four further properties (FiniteRegistry, NestingByDelegation, O17, PrefixBaptismCoupling) by induction over the transition sequence from `Σ₀`. The transition-discipline axioms constrain transition-induced changes; the initial state `Σ₀` is governed by O14 (for the principal registry `Π₀`) and by ASN-0040's bootstrap clause B₀ conf. (for the baptismal registry `Σ₀.B`). The constraints O5 (SubdivisionAuthority) and O16 (AllocationClosure) apply to transition-induced allocations only — addresses entering `Σ.B` via a `→` step — and not to bootstrap-seeded addresses, whose well-formedness is the responsibility of ASN-0040. We state them explicitly. O1a and O1b are not transition-discipline axioms but constraints on the signature of `pfx` itself — they restrict the codomain of the primitive `pfx : Π → T` to prefixes with `zeros ≤ 1` (O1a) and require `pfx` to be injective (O1b). The inductive preservation arguments in *Delegation* below verify that the delegation predicate's conditions (iv) and (v) — the obligations on a new delegate's prefix — together with O14's bootstrap clauses suffice to keep every introduced prefix within the signature; without conditions (iv) and (v), O15 could admit a delegate whose prefix violates the signature, contradicting `pfx`'s axiomatic constraints. The preservation arguments are therefore *consistency checks* on the interaction between `pfx`'s signature axioms and the delegation predicate's structural conditions, not derivations that promote O1a/O1b from invariants to axioms.
...
*Reachability convention.* All states `Σ` discussed in this ASN are assumed to be *reachable from the bootstrap state* `Σ₀` — that is, there exists a finite sequence `Σ₀ → Σ_1 → ... → Σ` of state transitions producing `Σ`. This convention is load-bearing for the proofs below: the iterated application of O12 (PrincipalPersistence) requires a finite-length transition sequence from `Σ₀` to `Σ` to conclude `Π₀ ⊆ Π_Σ`, which in turn underpins the bootstrap-exclusion arguments in O3, O8, and OwnershipDomainPermanence. The reachability hypothesis also underpins the induction in O4 (DomainCoverage), and through O4, propagates to O2 (which invokes O4 for non-emptiness of the covering set). Each property's formal contract restates the reachability precondition explicitly whenever the proof relies on it; properties whose derivations are entirely state-local (e.g., the AccountField, AccountPrefix, and PrefixDetermination definitions, which constrain a single tumbler or principal without quantifying over transitions) need no reachability hypothesis.
...
Condition (iii) — `π' ∈ Π_{Σ'} ∖ Π_Σ`, the delegate is newly introduced — restates the outer binder as an explicit conjunct so the labels are contiguous. The reading of the conjuncts: (i) the delegate's prefix strictly extends the delegator's, (ii) the delegator is the most-specific covering principal of `pfx(π')` in `Π_Σ` (the authorization clause — delegation requires subdivision authority), (iii) the delegate is newly introduced, (iv) the delegate's prefix is at node or account level, (v) the delegate's prefix is a valid tumbler, and (vi) no existing principal already occupies a sub-domain of `pfx(π')` (enforcing top-down delegation order). Without this closure, O12 permits arbitrary growth of Π — a mechanism outside the inlined conditions could introduce a principal at document level (violating O1a) or within a sub-domain without the effective owner's consent (circumventing the authorization guarantee of condition (ii)). Nelson's design contains no concept of principals appearing outside the delegation hierarchy, and Gregory's codebase provides no mechanism for it. The at-most-one constraint reflects the atomic nature of a delegation act: one delegator, one delegate, one prefix.

**Definition (delegated_Σ).** We name the conjunction of conditions (i)–(vi) above the *delegation predicate*: `delegated_Σ(π, π')` holds iff `π ∈ Π_Σ`, the successor state `Σ'` satisfies `Σ → Σ'`, and the six conditions are met for the pair `(π, π')`. The notation does double duty: as a witness to an actual delegation transition `Σ → Σ'` introducing `π'` (the historical reading, used when reasoning about a transition that has occurred — e.g., O8's anchoring of `Σ_d^{post}`) and as the satisfaction of the six conditions at state `Σ` (the structural reading, used when verifying that a prospective delegation is admissible). The two readings coincide on actual transitions — condition (iii) fixes the successor state via `π' ∈ Π_{Σ'} ∖ Π_Σ` — so we use a single notation throughout this ASN; context disambiguates. The reflexive-transitive closure `delegated_Σ*` is defined alongside NestingByDelegation below. The named definition makes `delegated_Σ` available to the proofs of O3, NestingByDelegation, and OwnershipDomainPermanence — which precede the *Delegation* section where the relation's consequences for ownership flow are developed — without forward-reference.
...
where `delegated_Σ*(π, π')` is defined as follows. Fix a witnessing transition sequence `Σ_0 → Σ_1 → ... → Σ_n = Σ` (supplied by the reachability hypothesis). Let `R_Σ ⊆ Π_Σ × Π_Σ` be the relation `R_Σ(π, π')` iff `(E k : 0 ≤ k < n : delegated_{Σ_k}(π, π'))` — that is, the principal pair `(π, π')` was admitted by a delegation step at some state along the witnessing path. Then `delegated_Σ*` is the reflexive-transitive closure of `R_Σ` in the standard sense: `delegated_Σ* = ∪_{m ≥ 0} R_Σ^m`, where `R_Σ^0` is the identity relation on `Π_Σ` and `R_Σ^{m+1} = R_Σ^m ∘ R_Σ`. Equivalently, `delegated_Σ*(π, π')` iff `π = π'` or there is a finite chain `π = π^{(0)}, π^{(1)}, ..., π^{(m)} = π'` (`m ≥ 1`) of principals in `Π_Σ` with each consecutive pair `(π^{(j)}, π^{(j+1)})` admitted by some `R_Σ`-step. The base relation `R_Σ` is closed under prefix extension along the path (each step strictly extends prefixes by condition (i) of `delegated`); independence from the choice of witnessing sequence follows from O15 (PrincipalClosure), which fixes each non-bootstrap principal's introducing delegation event uniquely, so the same delegation steps lie on every reachable witnessing path. Equality `pfx(π₁) = pfx(π₂)` is excluded by O1b (preserved across transitions — see below).
...
This is the structural coupling between the principal registry and the baptismal registry: a principal cannot enter `Π` without an allocated prefix, and that prefix is allocated at the delegation transition itself, never pre-claimed by an earlier sub-position allocation. The act of delegation by `π` to a sub-account at slot `k` materially baptizes `pfx(π).0.k` and binds the new principal to it; the registries are two views of the same act, not independent ledgers that happen to agree. The freshness conjunct (`pfx(π') ∉ Σ.B`) records the design commitment that principal prefixes are *reserved* — Gregory's `findpreviousisagr` issues every account slot as a fresh entry in the granfilade tree, never re-purposing a pre-existing baptized tumbler as a new principal's prefix. The base case is supplied by O14's seventh clause (`(A π ∈ Π₀ : pfx(π) ∈ Σ₀.B)`), which establishes the membership conclusion for the bootstrap state (the freshness conjunct has no transition source at bootstrap and is vacuous there). The inductive step is the formula above: when `Σ → Σ'` introduces `π' ∈ Π_{Σ'} ∖ Π_Σ`, the same transition records `pfx(π') ∈ Σ'.B` and the prefix is not in `Σ.B` immediately prior; this is the abstract counterpart to Gregory's implementation invariant that delegated account slots reside in the granfilade tree, where `findpreviousisagr` scans the entire subtree under the parent's prefix without distinguishing delegated from non-delegated entries because the granfilade *is* the global baptismal registry and the principal registry's prefixes occupy it as fresh allocations. The O10 fork analysis cites the named derived property PrefixBaptismCoupling (immediately below) to ensure that every Form B (=length) sub-delegate's prefix `pfx(π).0.U^{(i)}_1` lies in `Σ.B`, which is what bounds the pre-claimed slot set `S'` below the high-water mark `hwm_0` and makes the unilateral trajectory unconditional.
...
*Case 1 — `π ∈ Π_{Σ_n}` (carried forward).* By O13 (PrefixImmutability), `pfx_{Σ_{n+1}}(π) = pfx_{Σ_n}(π)`, and the IH gives `pfx_{Σ_n}(π) ∈ Σ_n.B`. By B0 (Irrevocability) of ASN-0040, `Σ_n.B ⊆ Σ_{n+1}.B`, so `pfx_{Σ_{n+1}}(π) ∈ Σ_{n+1}.B`. (T8 of ASN-0034 establishes allocator-domain monotonicity `allocated(s) ⊆ allocated(s')`, not baptismal-registry monotonicity; the registry monotonicity needed here is B0's distinct claim.)
...
The named derived property collects the four foundation steps — O14's seventh clause (bootstrap base case), O15 (closure of `Π` by bootstrap or delegation, which makes the case split exhaustive), O18 (the delegation-step inductive case), and B0 of ASN-0040 (Irrevocability, carrying baptized prefixes forward across transitions via `Σ_n.B ⊆ Σ_{n+1}.B`) — so subsequent properties may cite the single load-bearing fact rather than reassemble its base/step/closure structure each time. O10's non-coverage analysis and the worked example's self-ownership scenario both rely on the conclusion: every principal's prefix sits in the baptismal registry of every reachable state in which the principal exists.
...
*Notation (state-relativization of `ω` and `Π`).* The formal definition above states `ω_Σ` explicitly with `ω_Σ : Σ.B → Π_Σ` and the quantifier over `Π_Σ`. We abbreviate `ω_Σ(a)` as bare `ω(a)` only when the ambient state is fixed by context — typically inside a paragraph operating at a single state `Σ`, or in informal prose where no inter-state comparison is at stake. We write `ω_Σ(a)` whenever the state must be disambiguated — typically when comparing two states (e.g., `ω_{Σ'}(a) ≠ ω_Σ(a)` in O3) or invoking the function under a specific reachable state. Both forms denote the same function under the relativization; the subscript abbreviation is a notational convenience for readability, not a distinct operator. The same convention applies to bare `Π` versus `Π_Σ`. When a property's formal contract restates the state explicitly, the formal-contract form prevails over the prose's abbreviated form. Whenever the proof body invokes `ω` at a state different from the one currently in scope, the subscript is supplied explicitly.
...
The worked example below exhibits a concrete instance at the boundary `a₆ = pfx(π_A) = [1, 0, 2]` and verifies the longest-match outcome on the running principal set `{π_N, π_M, π_A}`. The general property is established here; the worked example is the verification scenario, not the proof site.
...
*The address is invariant.* By B0 (Irrevocability) of ASN-0040, once `a ∈ Σ.B`, the address `a` persists in the baptismal registry of every subsequent state with unchanged components. (T8 of ASN-0034 establishes allocator-domain monotonicity `allocated(s) ⊆ allocated(s')`, a property formally distinct from the baptismal-registry monotonicity `Σ.B ⊆ Σ'.B` invoked here.)
...
*Inductive step.* Assume the corollary holds for sequences of length `n`; consider a sequence of length `n + 1`: `Σ → Σ_1 → ... → Σ_n → Σ_{n+1}`. By the induction hypothesis applied to the prefix `Σ →⁺ Σ_n`, the chain conclusion holds for every transition with index `0 ≤ i < n` along that prefix. It remains to handle the final transition `Σ_n → Σ_{n+1}`. The single-transition OwnershipDomainPermanence applies provided `Σ_n` reachable from `Σ₀` (discharged above) and `a ∈ dom(π) ∩ Σ_n.B` — we discharge the latter from the original hypotheses. The persistence of `a` follows from B0★ (MultiStepIrrevocability) of ASN-0040 applied along the path `Σ →⁺ Σ_n`: `a ∈ Σ.B ⊆ Σ_n.B` since the baptismal registry is monotone under the reflexive-transitive closure of `→`. (T8 of ASN-0034 governs allocator-domain monotonicity, not baptismal-registry monotonicity.) The persistence of `π ∈ Π_{Σ_n}` follows from iterated O12: `Π_Σ ⊆ Π_{Σ_1} ⊆ ... ⊆ Π_{Σ_n}`, so `π ∈ Π_{Σ_n}`. The persistence of `a ∈ dom(π)` is structural — `dom(π) = {a : pfx(π) ≼ a}` depends only on `pfx(π)` and `a`, both of which are fixed as values (O13 immutability for the prefix; tumbler addresses are immutable values in `Σ.B` under B0 of ASN-0040). With premises discharged, the single-transition statement yields the required `π_d^{(n)}` with `pfx(π) ≼ pfx(π_d^{(n)})` whenever `ω_{Σ_{n+1}}(a) ≠ ω_{Σ_n}(a)`. Combined with the inductive conclusion for the earlier transitions, the chain conclusion holds for all `0 ≤ i ≤ n`. ∎

The corollary's content: every delegator that participates in a chain of changes to `ω(a)` within `dom(π)` has a prefix extending `pfx(π)`. No delegator outside `dom(π)` can induce a change. We record a structural corollary of NestingByDelegation specialized to the canonical anchor case.

*Remark (first-delegator form).* Let `Σ` be reachable from `Σ₀`, `π ∈ Π_Σ` with `S_π(Σ) = ∅` (where `S_π(Σ̃) = { π* ∈ Π_{Σ̃} : pfx(π) ≺ pfx(π*) }` denotes the set of strict sub-delegates of `π` present in `Σ̃`), and let `π'' ∈ Π_{Σ'}` with `Σ →⁺ Σ'` and `pfx(π) ≺ pfx(π'')` be any sub-delegate of `π` introduced after `Σ`. Then the delegation chain from `π` to `π''` guaranteed by NestingByDelegation has every link occurring strictly between `Σ` and `Σ'`. The hypothesis `S_π(Σ) = ∅` is satisfied, for instance, when `Σ` is the state at which `π` itself was introduced, or any state at which `π` has not yet exercised delegation authority. *Justification:* NestingByDelegation applied at `Σ'` to the pair `(π, π'')` with `pfx(π) ≺ pfx(π'')` yields `delegated_{Σ'}*(π, π'')`, i.e., a finite chain `π = π^{(0)}, π^{(1)}, ..., π^{(k)} = π''` with each link `delegated_{Σ^{(j)}}(π^{(j)}, π^{(j+1)})` at some state along the transition path `Σ_0 →⁺ Σ'`. Every intermediate `π^{(j)}` with `j ≥ 1` satisfies `pfx(π) ≺ pfx(π^{(j)})` (each link strictly extends the prefix, by condition (i) of `delegated`), so `π^{(j)} ∈ S_π` of its hosting state. By the empty-`S_π` hypothesis at `Σ` and O12 monotonicity, every such `π^{(j)}` lies in `Π_{Σ'} ∖ Π_Σ`, hence its introducing transition falls strictly between `Σ` and `Σ'`. The chain is finite because each link strictly increases the prefix length, bounded above by `#pfx(π'')`. ✓

The first-delegator form yields Nelson's "forevermore" in its multi-step form — the chain of delegators inducing changes inside `dom(π)` always begins with `π`'s own act, and no external party can ever insert into the chain.
...
*Convention on state subscripts.* Throughout this example, subscript labels `Σ_0, Σ_1, Σ_2, …` denote trajectory milestones, not single atomic transitions: the segment between two consecutive labels may comprise multiple `Bop` calls (allocation, namespace baptism, delegation), and the registry recorded at the next milestone is the cumulative result of all such transitions. Intermediate states are not separately named; where the sub-transitions are material to a verification, they are itemized inline at the milestone. By B0 (Irrevocability) of ASN-0040, the order of intervening transitions within a segment is immaterial to the cumulative `Σ.B` recorded at its endpoint.
...
**Bootstrap seeds (consistency table).** `Σ_0` is the source state of the worked-example trajectory; its baptismal registry is given as a single bootstrap snapshot (not as Bop products of transitions reaching `Σ_0`), consistent with O14, B1 (ContiguousPrefix of ASN-0040), and O18. Beyond `[1], [2]` (O14's seventh clause), we seed four positions into `Σ_0.B`:

| Seed | Sibling stream | Coverage in `Π_0` | B1 obligation discharged |
|------|----------------|-------------------|--------------------------|
| `[1, 0, 1] = c_1` | `S([1], 2)` | `π_N` (only `[1] ≼ ·`) | predecessor of `c_2 = [1, 0, 2]` (baptized at the delegation transition below) |
| `[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3]` | `S([1, 0, 2], 2)` | `π_N` (most-specific in `Π_0`) | contiguous family `{c_k : 1 ≤ k ≤ 3}` within the stream |
| `a_1 = [1, 0, 2, 0, 3, 0, 1]` | `S([1, 0, 2, 0, 3], 2)` | `π_N` | first member; B1 vacuous |
| `a_3 = [1, 0, 7, 0, 1, 0, 1]` | `S([1, 0, 7, 0, 1], 2)` | `π_N` (`[1] ≼ a_3`, `[2] ⋠ a_3`) | first member; B1 vacuous |

*O18 freshness.* The freshness conjunct constrains only the delegated prefix `[1, 0, 2]` itself — and `[1, 0, 2] ∉ Σ_0.B`, since it is baptized exactly at the delegation transition below — not other slots of any sibling stream. All four seeded rows respect this.

*Cross-stream non-interference.* `a_3` does not enter `children(Σ_0.B, [1], 2)` (a length-3 stream) or any stream populated by the other seeds, so contiguity within those streams is unaffected.

*No `π_M` seeds required.* The bootstrap state contains no `[2, 0, k]` addresses (only `[2]`), so when the later delegation `delegated(π_M, π_C)` of `[2, 0, 1] = c_1 ∈ S([2], 2)` is baptized as `pfx(π_C)`, B1 is vacuously satisfied — `[2, 0, 1]` is the first baptism in its stream.
...
**Concrete witness for SelfOwnershipAtPrefix.** This paragraph exhibits a concrete instance of the derived property SelfOwnershipAtPrefix (established in *The Exclusivity Invariant* above from O1b, O2, and PrefixBaptismCoupling); it does not re-derive the general fact. By PrefixBaptismCoupling, `pfx(π_A) = [1, 0, 2] ∈ Σ_1.B` — `π_A`'s entry into `Π` is materially accompanied by the baptism of its prefix, and the named property guarantees this in every reachable state at which `π_A ∈ Π`. We verify SelfOwnershipAtPrefix at the concrete boundary `a₆ = pfx(π_A) = [1, 0, 2]`. The covering set is `C(a₆) = {π ∈ Π_{Σ_1} : pfx(π) ≼ [1, 0, 2]}`. Candidates: `π_N` (prefix `[1]`, `[1] ≼ [1, 0, 2]` since the first component matches and `#[1] ≤ #[1, 0, 2]`), `π_M` (prefix `[2]`, fails: `2 ≠ 1`), `π_A` (prefix `[1, 0, 2]`, `[1, 0, 2] ≼ [1, 0, 2]` reflexively — every component matches and lengths are equal). So `C(a₆) = {π_N, π_A}` with prefix lengths `1` and `3`. The longest match is `π_A`, hence `ω(a₆) = π_A`. ✓ The general statement `(A Σ, π ∈ Π_Σ : ω_Σ(pfx(π)) = π)` is established as the derived property SelfOwnershipAtPrefix in the *Exclusivity Invariant* section above (from O1b, O2, and PrefixBaptismCoupling); the present paragraph exhibits the concrete witness rather than re-derive the general fact.

**Sub-account namespaces (B1 prerequisites for the later delegation).** Between `π_A`'s introduction at `Σ_1` and the delegation of `[1, 0, 2, 3]` to `π_B` traced in *Verifying O8* below, `π_A` baptizes two sub-account positions in `S(pfx(π_A), 1) = S([1, 0, 2], 1)` as organizational namespaces — addresses entered into `Σ.B` without introducing new principals. By O5, `π_A` is the most-specific covering principal of each and is therefore authorized as allocator; by O15, namespace baptism allocates an address without admitting a new principal, so `Π` is unchanged. Specifically, `π_A` invokes `Bop([1, 0, 2], 1)` twice in succession: the first call is a field-opening (`hwm = 0` in `S([1, 0, 2], 1)`), producing `[1, 0, 2, 1] = c_1` — B6 check: `zeros([1, 0, 2]) + (d − 1) = 1 + 0 = 1 ≤ 3` ✓; B1 check within `S([1, 0, 2], 1)`: `c_1` is the first member of its stream, so the contiguous-prefix obligation is vacuous ✓. The second is a sibling-advance (`hwm = 1`), producing `[1, 0, 2, 2] = c_2` — B6 check identical to the first ✓; B1 check: the contiguous-prefix family extends from `{c_1}` to `{c_1, c_2}` ✓. By B0 (Irrevocability) of ASN-0040, both addresses persist in every successor state. We fold these two baptisms into the `Σ_1 →* Σ_2` segment of the running trajectory (alongside `a_2`'s allocation), so `Σ_2.B ⊇ {[1, 0, 2, 1], [1, 0, 2, 2], a_2}`. These supply B1's contiguous-prefix prerequisite for the subsequent baptism of `[1, 0, 2, 3] = c_3 ∈ S([1, 0, 2], 1)` at the `Σ_2 → Σ_3` delegation transition: B1 demands `c_1, c_2 ∈ Σ_2.B` prior to baptizing `c_3`, and the two namespace baptisms exactly establish that.
...
*Trajectory.* `π_A` has `pfx(π_A) = [1, 0, 2]` with `zeros = 1`. The pre-fork state `Σ_pre` is reached from `Σ_0` by the *Delegation* and *Allocation* segments already established above; we summarize the resulting baptismal registry rather than re-derive it. *Verification convention.* We do not trace each intermediate state along the trajectory `Σ_0 → ⋯ → Σ_pre`; instead we record the cumulative baptismal registry and note, in aggregate, which B6/B1 obligations were discharged by which Bop call. The reader who wishes to expand a step-by-step verification can do so by reading each Bop invocation below as a single transition whose obligations are listed alongside it. *Pre-delegation seeds (already in `Σ_0.B` per the bootstrap snapshot of the *Worked Example* introduction):* `{[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3], a_1 = [1, 0, 2, 0, 3, 0, 1]} ⊆ Σ_0.B`. *Delegation:* `Σ_0 → Σ_1` introduces `π_A` with `pfx(π_A) = [1, 0, 2]`, and by O18 also enters `[1, 0, 2] ∈ Σ_1.B`. *Post-delegation baptisms by `π_A` (most-specific covering principal of `[1, 0, 2]` in `Σ_1`, satisfying O5; each Bop call below corresponds to one transition):* (1) `Bop([1, 0, 2], 2)` advancing `hwm` in `S([1, 0, 2], 2)` from `3` to `4` produces `[1, 0, 2, 0, 4]` — B6 check: `zeros([1, 0, 2]) + (d − 1) = 1 + 1 = 2 ≤ 3` ✓; B1 check within `S([1, 0, 2], 2)`: the prior children `{[1, 0, 2, 0, 1], [1, 0, 2, 0, 2], [1, 0, 2, 0, 3]}` form a contiguous-prefix family `{c_k : 1 ≤ k ≤ 3}`, and the new slot `[1, 0, 2, 0, 4] = c_4` extends it contiguously ✓. (2) `Bop([1, 0, 2], 2)` advancing `hwm` from `4` to `5` produces `[1, 0, 2, 0, 5]` — B6 check identical to (1) ✓; B1 check: the contiguous-prefix family extends from `{c_k : 1 ≤ k ≤ 4}` to `{c_k : 1 ≤ k ≤ 5}` ✓. (3) `Bop([1, 0, 2, 0, 5], 2)` produces `a_2 = [1, 0, 2, 0, 5, 0, 1]` — B6 check: `zeros([1, 0, 2, 0, 5]) + (d − 1) = 2 + 1 = 3 ≤ 3` ✓ (at the bound, but satisfied); B1 check within `S([1, 0, 2, 0, 5], 2)`: `a_2` is the first member of its stream, so the contiguous-prefix obligation is vacuous ✓. (The namespace baptisms in *Sub-account namespaces* above — producing `[1, 0, 2, 1]` and `[1, 0, 2, 2]` in `S([1, 0, 2], 1)` — are verified inline in that paragraph with explicit B6 and B1 checks; we cite the cumulative outcome here without repeating those checks.) Cumulatively, `Σ_2.B ⊇ Σ_0.B ∪ {[1, 0, 2], [1, 0, 2, 0, 4], [1, 0, 2, 0, 5], a_2 = [1, 0, 2, 0, 5, 0, 1], [1, 0, 2, 1], [1, 0, 2, 2]}`. The pre-fork state `Σ_pre := Σ_2` therefore has `children(Σ_pre.B, [1, 0, 2], 2) = {[1, 0, 2, 0, k] : 1 ≤ k ≤ 5}`. Hence `hwm(Σ_pre.B, [1, 0, 2], 2) = 5`. Set `m = hwm + 1 = 6`.

The single baptism: `b_1 = next(Σ_pre.B, [1, 0, 2], 2)`. Since `hwm = 5 > 0`, the sibling-advance branch applies: `b_1 = inc([1, 0, 2, 0, 5], 0) = [1, 0, 2, 0, 6]`. This produces a document-level address with `zeros = 2`. *O5 check at `Σ_pre`:* the most-specific covering principal of `[1, 0, 2, 0, 6]` in `Π_{Σ_pre} = {π_N, π_M, π_A}` is `π_A` (matches first three components; `π_N` matches only `[1]`; `π_M` does not cover); no sub-delegate of `π_A` exists in `Π_{Σ_pre}`, and were any to exist they would have `zeros(pfx(π_i)) = 1` with the zero already consumed at `pfx(π_A)`'s user-field separator — they would have positive at position 4, where `b_1` has 0. `π_A` is O5-authorized. *B6 check:* `zeros(pfx(π_A)) + (d − 1) = 1 + 1 = 2 ≤ 3`. ✓ Result: `Σ_pre → Σ'` with `b_1 = a' ∈ Σ'.B`.
...
*Verifying `hwm_0 = 0` at `Σ_3`.* The depth-2 granfilade `S([1, 0, 2, 3], 2)` consists of tumblers of the form `[1, 0, 2, 3, 0, k]` (each parent-anchored at `pfx(π_B)` with one additional zero separator). The delegation transition `Σ_2 → Σ_3` baptizes only `pfx(π_B)` itself (by O18), not any element of `S([1, 0, 2, 3], 2)` — O15 introduces at most one new principal per transition and the baptismal coupling materializes exactly `pfx(π_B)` (a depth-`#pfx(π_A) + 1` entry in `S(pfx(π_A), 1)`, not a depth-2 entry in `S(pfx(π_B), 2)`). The prior transitions on the witnessing path `Σ_0 → Σ_1 → Σ_2 → Σ_3` perform `Bop` calls under different parents (`Bop([1, 0, 2], 2)` for the document stream `S([1, 0, 2], 2)`, `Bop([1, 0, 2, 0, k], 2)` for element streams, and `Bop([1, 0, 2], 1)` for the namespace baptisms) — by ASN-0040's parent-anchoring of granfilade streams, none of these populate `S([1, 0, 2, 3], 2)`. Hence `children(Σ_3.B, [1, 0, 2, 3], 2) = ∅`, and `hwm(Σ_3.B, [1, 0, 2, 3], 2) = 0`. *Scope.* This condition is trajectory-specific, not delegation-intrinsic: along a different trajectory, the most-specific covering principal of `[1, 0, 2, 3]` in `Π_{Σ_2}` — namely `π_A`, since `[1, 0, 2, 3] ∉ Π_{Σ_2}` — could itself invoke `Bop([1, 0, 2, 3], 2)` pre-delegation (B6 check: `zeros([1, 0, 2, 3]) + (d − 1) = 1 + 1 = 2 ≤ 3` ✓; first-member B1 vacuous), populating `S([1, 0, 2, 3], 2)` and lifting `hwm_0` above zero. On such a trajectory `π_B`'s first fork would traverse the sibling-advance branch instead. The field-opening branch is *one possible exhibition* of a fresh delegate's first fork — the one traced here — not a structurally forced consequence of delegation.
...
*O5 check at `Σ_3`:* `π_B` is the most-specific covering principal of `[1, 0, 2, 3, 0, 1]` in `Π_{Σ_3} = {π_N, π_M, π_A, π_B}`. Candidates: `π_N` (prefix `[1]`, covers, length 1); `π_M` (prefix `[2]`, fails); `π_A` (prefix `[1, 0, 2]`, covers, length 3); `π_B` (prefix `[1, 0, 2, 3]`, covers, length 4). Longest match: `π_B`. No sub-delegate of `π_B` exists yet (`Π_{Σ_3} ∖ {π_B} = {π_N, π_M, π_A}` — none has a prefix strictly extending `[1, 0, 2, 3]` by condition (vi) at the moment `π_B` was delegated). ✓ *B6 check:* `d = 2`, `zeros(pfx(π_B)) + (d − 1) = 1 + 1 = 2 ≤ 3`. ✓ *O10(a) at the post-baptism state* `Σ_4`: `pfx(π_B) ≼ a'` and `π_B` has the longest matching prefix in `Π_{Σ_4} = Π_{Σ_3}` (baptism introduces no principals by O15), so `ω_{Σ_4}(a') = π_B`. ✓ *O10(b):* every pre-existing baptized address persists in `Σ_4.B` (B0 Irrevocability of ASN-0040) and `Π_{Σ_4} = Π_{Σ_3}`; in particular, `ω_{Σ_4}(a₃) = ω_{Σ_3}(a₃) = π_N`, unchanged. ✓

The field-opening branch is therefore exhibited via a concrete sub-trajectory of the running scenario rather than a hypothetical scenario substitution: along the witnessing path traced above, `S([1, 0, 2, 3], 2)` is virgin at `Σ_3`, so `π_B`'s first fork — whether immediate or deferred — invokes the field-opening branch with `hwm_0 = 0`. On alternative trajectories where the delegator pre-populates the stream (see *Scope* above), the same fork instead traverses sibling-advance; the boundary case is one possible exhibition of a fresh delegate's first fork, not its structurally forced consequence. Unilateral O10★ is robust to either branch: the user-field-separator argument articulated below for `π_A` transposes to `π_B` by substituting `pfx(π_B)` for `pfx(π_A)`, and applies identically to both branch outputs — `inc(pfx(π_B), 2) = [1, 0, 2, 3, 0, 1]` (field-opening) and `inc([1, 0, 2, 3, 0, k], 0) = [1, 0, 2, 3, 0, k + 1]` (sibling-advance) are both document-level addresses whose separator at position `#pfx(π_B) + 1 = 5` structurally defeats every potential sub-delegate prefix of `π_B`.
...
The Prefix (PrefixRelation) definition of ASN-0034 requires two conditions: `#a ≥ #acct(a)` and `(A i : 1 ≤ i ≤ #acct(a) : acct(a)ᵢ = aᵢ)`. By T3 (CanonicalRepresentation), each component `aᵢ` is a uniquely determined natural number, so component equality is well-defined. By T4c (LevelDetermination), `zeros(a) ∈ {0, 1, 2, 3}`; by T4b (UniqueParse), the field decomposition `fields(a)` is uniquely determined by `a` alone. We proceed by cases on `zeros(a)`.

*Case `zeros(a) = 0`.* The tumbler `a` contains no zero-valued components. By T4c (LevelDetermination), `a` is a node-level tumbler, and by T4b (UniqueParse) the entire tumbler is its node field: `N(a) = a`, and no user, document, or element fields are present. By the definition of `acct` (AccountField), `acct(a) = a`. The prefix relation `a ≼ a` holds: `#a = #a` and `aᵢ = aᵢ` for all `1 ≤ i ≤ #a`.

*Case `zeros(a) = 1`.* By T4b (UniqueParse) for the field decomposition, T4a (SyntacticEquivalence) for non-empty field segments, and T4's positive-component constraint, `a` has the form `N₁. ... .Nα . 0 . U₁. ... .Uβ` with `α ≥ 1`, `β ≥ 1` (T4a), every `Nᵢ > 0`, and every `Uⱼ > 0` (T4). The node field is `N(a) = [N₁, …, Nα]` and the user field is `U(a) = [U₁, …, Uβ]`. By AccountField, `acct(a) = N(a) ++ [0] ++ U(a) = [N₁, …, Nα, 0, U₁, …, Uβ]`. Since `a` has exactly one zero separator and only node and user fields, `a = [N₁, …, Nα, 0, U₁, …, Uβ] = acct(a)`. The prefix relation holds as in the previous case: `acct(a) = a` implies `acct(a) ≼ a`.

*Case `zeros(a) = 2`.* By T4b (UniqueParse), T4a (SyntacticEquivalence), and T4's positive-component constraint, `a = N₁. ... .Nα . 0 . U₁. ... .Uβ . 0 . D₁. ... .Dγ` with `α ≥ 1`, `β ≥ 1`, `γ ≥ 1` (T4a), every `Nᵢ > 0`, every `Uⱼ > 0`, every `Dₖ > 0` (T4). By AccountField, `acct(a) = [N₁, …, Nα, 0, U₁, …, Uβ]` with `#acct(a) = α + 1 + β`. The address has `#a = α + 1 + β + 1 + γ`. Since `γ ≥ 1`, `#a = α + 1 + β + 1 + γ ≥ α + 1 + β + 2 > α + 1 + β = #acct(a)`, satisfying the length condition `#a ≥ #acct(a)`. For the component condition: the first `α + 1 + β` components of `a` are `N₁, …, Nα, 0, U₁, …, Uβ`, which are exactly the components of `acct(a)`. Hence `acct(a)ᵢ = aᵢ` for all `1 ≤ i ≤ #acct(a)`, and `acct(a) ≼ a`.

*Case `zeros(a) = 3`.* By T4b (UniqueParse), T4a (SyntacticEquivalence), and T4's positive-component constraint, `a = N₁. ... .Nα . 0 . U₁. ... .Uβ . 0 . D₁. ... .Dγ . 0 . E₁. ... .Eδ` with `α ≥ 1`, `β ≥ 1`, `γ ≥ 1`, `δ ≥ 1` (T4a), all field components strictly positive (T4). By AccountField, `acct(a) = [N₁, …, Nα, 0, U₁, …, Uβ]` with `#acct(a) = α + 1 + β`. The address has `#a = α + 1 + β + 1 + γ + 1 + δ ≥ α + 1 + β + 4 > #acct(a)`. The first `α + 1 + β` components of `a` are again `N₁, …, Nα, 0, U₁, …, Uβ` — the document and element fields appear strictly after position `α + 1 + β`. Hence `acct(a)ᵢ = aᵢ` for all `1 ≤ i ≤ #acct(a)`, and `acct(a) ≼ a`.
...
Of the rights that ownership confers, one is essential to the ownership model itself: the right to create sub-positions. O5 (SubdivisionAuthority) was stated in *State Axioms* above — the allocator of any newly baptized address must be the most-specific covering principal in `Π_Σ`. We develop here its consequences for the relation between allocation and effective ownership.
...
The delegation predicate `delegated_Σ(π, π')` was defined in *State Axioms* alongside O15 (PrincipalClosure); the definition's historical/structural double reading is stated there. We restate the six conditions here for ready reference:
...
- *Invariant:* The two registries (principal `Π` and baptismal `B`) are coupled by a single allocator at every delegation act: the same `π_d` whose authority condition (ii) admits `π'` into `Π` is the allocator whose O5 authority enters `pfx(π')` into `B`. The delegation act is one event recorded in two views, not two independent acts that happen to coincide.

The derived property makes explicit a coupling that the subsequent proofs of O7 and OwnershipDomainPermanence already rely on tacitly: when a delegation transition fires, the delegator is the unique principal whose O5 authority underwrites the baptism of the new prefix. Gregory's implementation realizes this coupling concretely — `findpreviousisagr` enters the new account slot into the granfilade under the session's own account-tumbler authority, so the delegator and the allocator are necessarily the same process.
...
*Covering-chain lemma (cited).* O2 (OwnershipExclusivity)'s proof — which uses the Prefix (PrefixRelation) definition of ASN-0034 — establishes that any two tumbler prefixes of the same address are linearly ordered by `≼`: given `p ≼ a` and `q ≼ a`, either `p ≼ q` or `q ≼ p`. (Both `p` and `q` agree with `a` on their leading components; whichever is shorter is a prefix of the other. T5 ContiguousSubtrees is not invoked — it concerns the lexicographic contiguity of prefix-defined intervals, not the comparison of two prefixes of the same address.) Applying this lemma to `pfx(π'')` and `pfx(π')` (both prefixes of `a`), they are `≼`-comparable. Three cases exhaust the comparison.
...
Since `π' ∈ Π_{Σ'}`, the delegation relation's conditions are satisfiable with `π'` as delegator for a sub-prefix `p''` with `pfx(π') ≺ p''` *immediately upon entry* — that is, at `Σ'`. Condition (i) holds by the choice of `p''`. Condition (ii) requires that `π'` be the most-specific covering principal of `p''` in `Π_{Σ'}` — equivalently, no `π'' ∈ Π_{Σ'}` with `pfx(π'') ≼ p''` has `#pfx(π'') > #pfx(π')`. We derive this directly from condition (vi) of the original delegation `delegated_Σ(π, π')`: `¬(E π'' ∈ Π_Σ : pfx(π') ≺ pfx(π''))` — no principal in `Π_Σ` has a prefix strictly extending `pfx(π')`. By O15, `Π_{Σ'} ∖ Π_Σ = {π'}`, and `pfx(π')` does not strictly extend itself, so the same non-existence carries over: no `π'' ∈ Π_{Σ'}` has `pfx(π') ≺ pfx(π'')`. Hence no covering principal of `p''` in `Π_{Σ'}` has prefix length exceeding `#pfx(π')`; `π'` achieves the maximum, satisfying condition (ii). Conditions (iv), (v), and (vi) constrain the target prefix `p''`, not the delegator, and are obligations on the choice of delegate prefix. *At later states* `Σ'' ⪰ Σ'`, condition (ii) requires re-checking: if `π'` has itself already delegated some `p* ⪯ p''` to a principal `π*` with `pfx(π') ≺ pfx(π*) ≼ p''`, then `π*` — not `π'` — is the most-specific covering principal for `p''` at `Σ''`, and `π'` cannot delegate `p''`. Postcondition (c) thus asserts the *right* to delegate sub-prefixes, conditional on `π'` retaining most-specific-covering status; it does not assert an absolute right against subsequent sub-delegations by `π'` itself. The recursion may continue indefinitely along any chain whose successive delegates remain consistent with this condition. A bare appeal to T0(b) (UnboundedLength) does not suffice on its own: T0(b) supplies length-`n` witnesses with `zeros = 0`, but condition (iv) requires every delegate's prefix to satisfy `zeros ≤ 1`, which excludes the freely growing witness `[1, 1, ..., 1, 1]` at lengths producing additional zero separators. We exhibit a chain of account-level delegates rooted at a node principal: the root `π_0` is itself a node-level principal (`zeros(pfx(π_0)) = 0`), and every subsequent chain member `π_k` (for `k ≥ 1`) is account-level (`zeros(pfx(π_k)) = 1`). Only the boundary link `π_0 → π_1` crosses the node→account boundary; every later link `π_k → π_{k+1}` remains within the account level by extending the user field. The root principal is `π_0` with `pfx(π_0) = [1]` (zeros = 0, T4 satisfied — a single-component positive tumbler with no zeros, no adjacency violations, no leading/trailing zero); by T4c (LevelDetermination), zero-count-zero classifies `π_0` as node-level, not account-level. The *chain* of delegates is `π_1, π_2, π_3, …`, defined for `k ≥ 1` by `pfx(π_k) = [1, 0, 1, 1, ..., 1]` with `k + 2` total components (one node component, the zero separator, and `k` user-field components, each equal to 1). Every chain member `pfx(π_k)` (for `k ≥ 1`) has `zeros(pfx(π_k)) = 1` (account level by T4c, satisfying (iv)) and satisfies T4 — the single zero sits at position 2, flanked by positive components, with no leading or trailing zero and no adjacent zeros, and all field components are strictly positive (satisfying (v)). *Boundary step `π_0 → π_1` (verified separately as bootstrap-to-first-delegate).* This step is structurally distinct from subsequent extensions: `pfx(π_0) = [1]` has `zeros = 0` and consists of the node field alone, whereas `pfx(π_1) = [1, 0, 1]` has `zeros = 1` and *opens* the user field by appending both the zero separator and the first user-field component (two components added in one step). Condition (i): `pfx(π_0) = [1] ≺ pfx(π_1) = [1, 0, 1]` — both have first component 1, with `#pfx(π_0) = 1 < 3 = #pfx(π_1)`, so the prefix relation holds and the inequality is strict. Conditions (iv)–(v) for `pfx(π_1)`: verified above (`zeros = 1`, T4 satisfied with single zero at position 2). The boundary step admits `π_1` as a valid first-link delegate. *Inductive extension `π_k → π_{k+1}` for `k ≥ 1` (uniform).* For every `k ≥ 1`, `pfx(π_{k+1})` extends `pfx(π_k)` by one additional user-field component (a single `1` appended at the end). Condition (i): `pfx(π_k) ≺ pfx(π_{k+1})` — the first `k + 2` components agree, with `#pfx(π_k) = k + 2 < k + 3 = #pfx(π_{k+1})`. Conditions (iv)–(v) for `pfx(π_{k+1})`: `zeros(pfx(π_{k+1})) = 1` (the appended `1` adds no zero, the existing single zero at position 2 is preserved), and T4 holds for the same reason as `pfx(π_k)` (single zero at position 2, flanked by positives, no adjacency or boundary violations, all user-field components positive). The inductive extension admits `π_{k+1}` as a valid next-link delegate. T0(b) supplies arbitrarily large `k`, so the chain `π_1, π_2, π_3, …` extends indefinitely. Conditions (ii) and (vi) are discharged inductively along the chain. Let `Σ_k` denote the state immediately after `π_0, …, π_k` have been introduced (with `Σ_0` the bootstrap state containing `π_0`, and each `Σ_j → Σ_{j+1}` the delegation transition introducing `π_{j+1}`). We show that the covering set of `pfx(π_{k+1})` in `Π_{Σ_k}` is exactly `{π_0, …, π_k}`, from which conditions (ii) and (vi) follow directly. *Coverage.* By the chain construction, the delegations `delegated_{Σ_j}(π_j, π_{j+1})` hold for `0 ≤ j < k`, each link a strict prefix extension (condition (i)); transitivity of `≺` along `pfx(π_0) ≺ pfx(π_1) ≺ … ≺ pfx(π_k) ≺ pfx(π_{k+1})` gives `pfx(π_j) ≺ pfx(π_{k+1})` for every `0 ≤ j ≤ k`, so every `π_j` covers `pfx(π_{k+1})`. *Exhaustiveness.* For any other principal `π' ∈ Π_{Σ_k} ∖ {π_0, …, π_k}` (necessarily a bootstrap principal distinct from `π_0`, since the chain delegations are the only post-bootstrap introductions), NestingByDelegation at `Σ_k` applied to the pair `(π', π_k)` yields one of three alternatives: (α) `pfx(π'), pfx(π_k)` non-nesting; (β) `delegated_{Σ_k}*(π', π_k)`; (γ) `delegated_{Σ_k}*(π_k, π')`. Option (β) is excluded: by O15 and condition (iii), each `π_{j+1}` has a unique introducer event, so the unique delegation chain ending at `π_k` is `π_0 → π_1 → … → π_k` (terminating at the bootstrap principal `π_0`), and `π' ∉ {π_0, …, π_{k-1}}`. Option (γ) is excluded because `π_k` has not exercised delegation in `Σ_k`. Hence (α) holds: `pfx(π')` is non-nesting with `pfx(π_k)`. If `pfx(π') ≼ pfx(π_{k+1})` held, the covering-chain lemma applied to `pfx(π')` and `pfx(π_k)` as two prefixes of `pfx(π_{k+1})` would force them to be `≼`-comparable, contradicting non-nesting. So `π'` does not cover `pfx(π_{k+1})`, and the covering set is exactly `{π_0, …, π_k}`. *Condition (ii):* the prefix lengths in `{π_0, …, π_k}` are `1, 3, 4, …, k+2`; the maximum is `#pfx(π_k) = k+2`, so `π_k` is the most-specific covering principal. ✓ *Condition (vi):* for each `π_j` (`0 ≤ j ≤ k`), `#pfx(π_j) ≤ k+2 < k+3 = #pfx(π_{k+1})`, ruling out strict extension by length; for any other `π' ∈ Π_{Σ_k} ∖ {π_0, …, π_k}`, non-nesting of `pfx(π')` with `pfx(π_k)` (established above) combined with `pfx(π_k) ≺ pfx(π_{k+1})` excludes `pfx(π_{k+1}) ≺ pfx(π')` (such an extension would yield `pfx(π_k) ≺ pfx(π')` by transitivity, contradicting non-nesting). ✓ The ascending chain `pfx(π_0) ≺ pfx(π_1) ≺ pfx(π_2) ≺ ...` of zero-count-constrained account-level prefixes admits arbitrarily long delegation chains. We do not claim termination; we claim only that the construction extends as far as the chosen sequence of delegates permits.
...
The formulation captures irrevocability without overclaiming. It says the *parent* can never recover the addresses, while permitting the delegate `π'` to sub-delegate (via O7(c)): if `π'` delegates to `π''` with `pfx(π') ≺ pfx(π'')`, then `ω(a) = π''` for `a ∈ dom(π'')` — the address leaves `π'`'s effective ownership but does not return to `π`. The domain restriction `dom(π') ∩ Σ'.B` ensures `ω` is applied only to addresses where it is defined (grounded by O4). The reachability hypothesis on `Σ_d` propagates to every `Σ'` with `Σ_d →⁺ Σ'` (each transition extends the witnessing sequence from `Σ₀`), so the iterated O12 argument used by the proof body remains available throughout the trajectory. The added hypothesis `π' ∈ Π_{Σ'}` constrains the trajectory `Σ_d →⁺ Σ'` to pass through the delegation transition that introduces `π'`: by O15, principals enter `Π` only via bootstrap or delegation, so `π' ∈ Π_{Σ'}` implies the witnessing transition Σ_d → Σ_d^post lies on the path; without this hypothesis, `Σ_d →⁺ Σ'` admits paths bypassing the delegation entirely (along which `π' ∉ Π_{Σ'}` and `ω_{Σ'}(a)` could equal `π`).

*Historical reading of `delegated_{Σ_d}(π, π')`.* In O8's hypothesis, `delegated_{Σ_d}(π, π')` is read *historically*: it asserts that an actual transition `Σ_d → Σ_d^{post}` occurred in which `π'` was introduced into `Π` with `pfx(π') ≼·` — not merely that the six conditions are structurally satisfiable at `Σ_d`. The historical reading is what supports the proof's identification of `π'`'s unique introduction event with `Σ_d → Σ_d^{post}` (rather than with some other delegation transition along `Σ_d →⁺ Σ'`). Both readings coincide on actual transitions — the definition of `delegated_Σ` (above, alongside O15) records this duality — and the proof body invokes the historical reading throughout.
...
*The delegate persists with an unchanged prefix.* Let `Σ_d^{post}` denote the successor state of the delegation transition `Σ_d → Σ_d^{post}` at which `π'` enters `Π`. We argue in three steps that the trajectory `Σ_d →⁺ Σ'` must pass through `Σ_d^{post}`. (1) By condition (iii) of the `delegated` relation, `π' ∈ Π_{Σ_d^{post}} ∖ Π_{Σ_d}` — at the source state `Σ_d`, `π'` is not yet in `Π`, and the transition `Σ_d → Σ_d^{post}` is `π'`'s introduction event. (2) Condition (iii) together with O12 (PrincipalPersistence) forbids re-introduction: once a principal enters `Π` it never leaves (O12 gives `Π_{Σ_i} ⊆ Π_{Σ_{i+1}}` along every transition), so no transition can re-introduce a principal already present. Hence `π'` has exactly one introduction event along its entire history; the hypothesis `π' ∈ Π_{Σ'}` therefore requires that introduction event to lie on the trajectory `Σ_d →⁺ Σ'`. (3) By O15 (PrincipalClosure), each introduction event is either bootstrap or a delegation transition. Bootstrap is excluded: by the reachability of `Σ_d` from `Σ₀`, iterated O12 along the witnessing sequence gives `Π₀ ⊆ Π_{Σ_d}`, and condition (iii) gives `π' ∉ Π_{Σ_d}`, hence `π' ∉ Π₀`. The remaining delegation case identifies the unique introduction event with the transition `Σ_d → Σ_d^{post}` already witnessed by the historical reading of the hypothesis `delegated_{Σ_d}(π, π')` (recorded above): the hypothesis asserts that this *specific* transition occurred and introduced `π'`, and the uniqueness from (1)–(2) forces the introduction event on the trajectory `Σ_d →⁺ Σ'` to coincide with it. Without the historical reading — were `delegated_{Σ_d}(π, π')` interpreted only structurally as "the six conditions are satisfiable at `Σ_d`" — the proof could conclude that *some* delegation transition introduces `π'` but could not identify it with `Σ_d → Σ_d^{post}`, leaving open the possibility of an introduction event by a different delegator `π_d' ≠ π` along the trajectory; the historical reading closes this ambiguity. Combining (1)–(3): the introduction event on any path bringing `π'` into `Π_{Σ'}` is precisely `Σ_d → Σ_d^{post}`, so the trajectory `Σ_d →⁺ Σ'` passes through `Σ_d^{post}`. By O12 iterated along `Σ_d^{post} →^* Σ'`, `Π_{Σ_d^{post}} ⊆ Π_{Σ'}`, consistent with the hypothesis. By O13 (PrefixImmutability) iterated along the same sub-trajectory, `pfx_{Σ'}(π') = pfx_{Σ_d^{post}}(π')`. The delegate is present at `Σ'` with the prefix it received at the delegation transition.

*The delegate covers the address.* Since `a ∈ dom(π')`, the definition of domain gives `pfx(π') ≼ a`. This relation depends only on the components of `pfx(π')` and `a`. By O13, `pfx(π')` is immutable. By B0★ (MultiStepIrrevocability) of ASN-0040 applied along the sub-trajectory `Σ_d^post →* Σ'`, `a` — being baptized — persists in the baptismal registry with unchanged components. Therefore `pfx_{Σ'}(π') ≼ a` holds in `Σ'`. (T8 of ASN-0034 governs allocator-domain monotonicity `allocated(s) ⊆ allocated(s')`, not the baptismal-registry monotonicity `Σ.B ⊆ Σ'.B` that grounds address persistence here.)
...
| MostSpecificCoveringUnique | Whenever two covering principals of a tumbler tie for maximum prefix length, they are equal — uniqueness of the most-specific covering principal; cited by O7(a) (case analysis), DelegatorAllocatesPrefix (allocator identification), OwnershipDomainPermanence (Step 3) | from Covering-chain lemma, O1b, Prefix |
