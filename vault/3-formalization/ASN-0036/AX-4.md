**AX-4 (BaseOrdinal).** For every document d and subspace S, if V_S(d) is non-empty then the minimum value of the last component across all positions in V_S(d) is 1:

`(A d, S : V_S(d) ≠ ∅ : (E v : v ∈ V_S(d) : (v)_{#v} = 1))`
...
*Formal Contract:*
- *Axiom:* `(A d, S : V_S(d) ≠ ∅ : (E v : v ∈ V_S(d) : (v)_{#v} = 1))` — for non-empty V_S(d), there exists a position whose last component is 1.
