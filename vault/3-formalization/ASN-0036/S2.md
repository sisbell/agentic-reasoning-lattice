...
`(A d ∈ D, v : v ∈ dom(Σ.M(d)) : Σ.M(d)(v) is uniquely determined)`
...
The bridge between the two state components is precisely this functionhood: M(d) maps each V-position to a single I-address, so the Vstream is well-defined at every occupied position.

*Formal Contract:*
- *Axiom:* For each d ∈ D, Σ.M(d) is a function — each v ∈ dom(Σ.M(d)) maps to exactly one I-address
