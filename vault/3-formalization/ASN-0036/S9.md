...
- *Preconditions:* S0 (content immutability) — for every transition Σ → Σ': a ∈ dom(Σ.C) ⟹ a ∈ dom(Σ'.C) ∧ Σ'.C(a) = Σ.C(a).
...
- *Frame:* Arrangement-only transitions preserve `Σ.C` exactly; transitions that also create content extend `dom(Σ.C)` while preserving all existing entries.
