...
**S8a satisfaction.** Every v ∈ V_S(d) is, by construction, an element of dom(Σ.M(d)). S8a quantifies universally over dom(Σ.M(d)), so zeros(v) = 0 ∧ v₁ ≥ 1 holds for each such v. The filter condition v₁ = S selects a subset of a domain whose elements already satisfy S8a; it adds no new constraint.
...
- *Preconditions:* S8a (V-position well-formedness) — every v ∈ dom(Σ.M(d)) satisfies zeros(v) = 0 ∧ v₁ ≥ 1; S8-depth (MinimalVPositionDepth) — within any subspace, all V-positions share a common depth.
