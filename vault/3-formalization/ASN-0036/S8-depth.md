...
*Inductive step.* Suppose the constraint holds in state Σ: within each subspace, every occupied V-position has the same tumbler depth. Let Σ → Σ' be any state transition produced by an operation (INSERT, DELETE, COPY, MOVE, REARRANGE, or APPEND). Each such operation either removes positions from a subspace — which cannot introduce a depth mismatch among the remaining positions — or introduces new positions into a subspace, which must share the depth of the positions already there (or, if the subspace was previously empty, must all share a common depth). Therefore `dom(Σ'.M(d))` satisfies fixed-depth.
