...
- *Preconditions:* Postcondition 3 requires S7c (`#fields(a).element ≥ 2`): at element-field depth `δ = 1`, position 1 is the last component (`#a`), and OrdinalShift's last-component increment would alter `E₁` — subspace preservation would be false.
- *Postconditions:* For any correspondence run `(v, a, n)` in document `d`:
  ...
  3. I-address subspace preservation (requires S7c, `δ ≥ 2`): `(A k : 0 ≤ k < n : E₁(a + k) = E₁(a))` — S7c guarantees `δ ≥ 2`, so `E₁` is at a position less than `#a`; OrdinalShift's prefix rule copies it unchanged.
