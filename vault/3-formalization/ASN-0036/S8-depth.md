**S8-depth (Fixed-depth V-positions).** Within a given subspace `s` of document `d ∈ D`, all V-positions share the same tumbler depth:

`(A d ∈ D, p, q : p ∈ dom(Σ.M(d)) ∧ q ∈ dom(Σ.M(d)) ∧ p₁ = q₁ : #p = #q)`
...
*Formal Contract:*
- *Invariant:* `(A d ∈ D, p, q : p ∈ dom(Σ.M(d)) ∧ q ∈ dom(Σ.M(d)) ∧ p₁ = q₁ : #p = #q)` — for every reachable state Σ.
- *Preconditions:* `dom(Σ.M(d)) ⊆ T` (Σ.M(d)).
