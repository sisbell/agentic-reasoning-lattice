...
*Formal Contract:*
- *Preconditions:* `V_S(d) ⊆ dom(M(d)) ⊆ T` (Σ.M(d)), providing carrier-set membership for T1; V_S(d) non-empty; common V-position depth m (S8-depth) with m ≥ 2 (S8-vdepth); D-CTG (VContiguity); D-CTG-depth (SharedPrefixReduction — for m ≥ 3, establishes that all positions share components 2 through m − 1); D-MIN (VMinimumPosition); S8-fin (finite arrangement).
