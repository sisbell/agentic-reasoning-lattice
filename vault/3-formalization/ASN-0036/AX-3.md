**AX-3 (Infinite type domains).** The state model `Σ = (C, M)` operates over two carrier sets drawn from the tumbler space `T` (ASN-0034). The *document identifier space* is `Doc = {d ∈ T : zeros(d) = 2}`, comprising all tumblers at document depth (D-DOC). The *V-position space* within subspace `S` is `VPos(S) = {v ∈ T : zeros(v) = 0 ∧ v₁ = S ∧ v > 0}`, comprising all tumblers satisfying S8a's well-formedness conditions with subspace identifier `S`. Both are countably infinite.
...
Each `VPos(S)` is countably infinite because, for any `S ≥ 1`, the tumblers `[S, 1], [S, 2], [S, 3], …` are pairwise distinct elements satisfying `zeros(v) = 0`, `v₁ = S ≥ 1`, and `v > 0`; since `T` is countable, `|VPos(S)| = ℵ₀`.
...
- *Axiom:* `(A S ≥ 1 :: |VPos(S)| = ℵ₀)` where `VPos(S) = {v ∈ T : zeros(v) = 0 ∧ v₁ = S ∧ v > 0}` — the V-position space within each subspace is countably infinite.
