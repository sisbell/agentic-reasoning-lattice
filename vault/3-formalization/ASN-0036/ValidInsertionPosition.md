...
*Case m = 2.* There are no intermediate components (the range [2, m − 1] = [2, 1] is empty), so every position in V_S(d) has the form [S, k]. By D-MIN, min(V_S(d)) = [S, 1], giving k = 1 at the minimum. By D-CTG, the k-values are contiguous: for any v₁ = [S, k₁], v₂ = [S, k₂] with k₁ < k₂ in V_S(d), each intermediate [S, k] with k₁ < k < k₂ is a finite sequence of 2 naturals, hence belongs to T by T0 (CarrierSetDefinition, ASN-0034), and D-CTG places it in V_S(d). By S8-fin, V_S(d) is finite, so the k-values form a finite contiguous range starting at 1 — namely {1, 2, …, N}. For 1 ≤ j ≤ N − 1, shift([S, 1], j) = [S, 1 + j] by OrdinalShift (with j ≥ 1), giving positions at k = 2, …, N. Together with the minimum at k = 1, V_S(d) = {min(V_S(d))} ∪ {shift(min(V_S(d)), j) : 1 ≤ j ≤ N − 1}.
...
- *Preconditions:* `V_S(d) ⊆ dom(M(d)) ⊆ T` (Σ.M(d)), providing carrier-set membership for OrdinalShift and T3; T0 (CarrierSetDefinition, ASN-0034) — tuple-constructed positions (empty-case [S, 1, ..., 1] and m = 2 intermediates [S, k]) are finite sequences of naturals with length ≥ 1, discharging T-membership where the Σ.M(d) chain does not reach; d satisfies D-CTG; D-CTG-depth (SharedPrefixReduction — for m ≥ 3, establishes sequential form of V_S(d)); S is a subspace identifier (S ≥ 1); m ≥ 2; S8-fin (dom(M(d)) is finite); in the non-empty case, m equals the common depth from S8-depth and S8-vdepth gives m ≥ 2; D-MIN holds for V_S(d).
