...
- *Non-empty subspace.* V_S(d) ≠ ∅. Since d satisfies S8-fin, dom(M(d)) is finite, so V_S(d) is finite; write |V_S(d)| = N. Write m for the common V-position depth in subspace S (S8-depth); m ≥ 2 by S8-vdepth (every V-position has depth at least 2). Then either v = min(V_S(d)) (the j = 0 case) or v = shift(min(V_S(d)), j) for some j with 1 ≤ j ≤ N. In both cases, #v = m.
...
- *Preconditions:* d satisfies D-CTG, D-MIN, S8-depth, S8-fin; S ≥ 1; in the non-empty case, m ≥ 2 (S8-vdepth).
