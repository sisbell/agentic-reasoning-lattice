...
A full element-level I-address has the form `N₁. ... .Nₐ . 0 . U₁. ... .Uᵦ . 0 . D₁. ... .Dᵧ . 0 . E₁. ... .Eδ` where each `Nᵢ, Uⱼ, Dₖ, Eₗ > 0` (T4, positive-component constraint). The arrangement `M(d)` maps each V-position to such an address: by S3 (ReferentialIntegrity), `M(d)(v) ∈ dom(Σ.C)` for every `v ∈ dom(Σ.M(d))`; by S7b (Element-level I-addresses), every address in `dom(Σ.C)` satisfies `zeros(a) = 3`, confirming it is element-level with all four fields present. A V-position `v` is the element field `E` extracted from this element-level address — the fourth field in T4's decomposition. As an isolated field, `v` contains no field separators: the zeros in the full address are inter-field boundaries, not intra-field components. Therefore `zeros(v) = 0`.
...
- *Preconditions:* S3 (ReferentialIntegrity) — every V-reference resolves: `M(d)(v) ∈ dom(Σ.C)`, connecting V-positions to the I-address space; S7b (Element-level I-addresses) — addresses in `dom(Σ.C)` are element-level tumblers with `zeros(a) = 3`, confirming M(d)'s targets have all four fields; T4 (FieldSeparatorConstraint, ASN-0034) — every non-separator component is strictly positive, every present field has at least one component.
