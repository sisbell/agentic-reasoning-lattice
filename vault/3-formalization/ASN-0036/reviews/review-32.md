...
Components `r₁` through `rₖ` are unconditionally positive. S8a requires `zeros(r) = 0`, `r₁ ≥ 1`, and `r > 0`, which together reduce to: every component of `r` is strictly positive. Since the prefix `r₁, ..., rₖ` is established positive, the only components that can fail are `r_{k+1}, ..., r_m = w_{k+1}, ..., w_m` — exactly the tail components of `w`, which are the tail components of `w_ord` after its action point (since `(w_ord)_j = w_{j+1}` and the action point of `w_ord` is `k - 1`, the components after that action point are `(w_ord)_k, ..., (w_ord)_{m-1} = w_{k+1}, ..., w_m`). When `k = m`, the tail region is empty and the condition holds vacuously. Therefore:
