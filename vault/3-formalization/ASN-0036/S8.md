...
*Formal Contract:*
- *Preconditions:* `dom(M(d)) ⊆ T` (Σ.M(d)), providing the carrier-set membership required by T0, T1, T3, T5, T10, and OrdinalShift; `dom(M(d))` finite (S8-fin); `M(d)` a function (S2); `(A v ∈ dom(M(d)) :: zeros(v) = 0 ∧ v₁ ≥ 1)` (S8a); within each subspace, all V-positions share a common depth (S8-depth); `#v ≥ 2` for all V-positions (S8-vdepth, a theorem from S8a, S3, S7c — depth-1 V-position subspaces are excluded by the model, rendering the `m = 1` cross-subspace case vacuously true).
