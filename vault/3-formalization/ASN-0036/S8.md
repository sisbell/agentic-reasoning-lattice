...
*Proof.* We construct a finite decomposition satisfying both conjuncts and prove it partitions `dom(M(d))`. Since `M(d) : T ⇀ T` (Σ.M(d)), every `v ∈ dom(M(d))` belongs to the carrier set `T`. This membership is the precondition for T0, T1, T3, T5, T10, and OrdinalShift, each of which is invoked below on V-positions drawn from `dom(M(d))`.
...
By S8-crun, `v + 1 = shift(v, 1)`. OrdinalShift gives `#(v + 1) = m` (length preservation), `(v + 1)ᵢ = vᵢ` for all `i < m` (prefix preservation), and `(v + 1)_m = v_m + 1` (deepest component advances).
...
*Vacuity note.* S8-vdepth establishes `#v ≥ 2` for every V-position in `dom(M(d))` (an axiom). Depth-1 V-position subspaces are excluded by the model, so the `m = 1` case below is vacuously true. The argument is retained to confirm the proof structure is self-contained at all depths.
...
*Formal Contract:*
- *Preconditions:* `dom(M(d)) ⊆ T` (Σ.M(d)), providing the carrier-set membership required by T0, T1, T3, T5, T10, and OrdinalShift; `dom(M(d))` finite (S8-fin); `M(d)` a function (S2); `(A v ∈ dom(M(d)) :: zeros(v) = 0 ∧ v₁ ≥ 1)` (S8a); within each subspace, all V-positions share a common depth (S8-depth); `#v ≥ 2` for all V-positions (S8-vdepth — depth-1 V-position subspaces are excluded by the model, rendering the `m = 1` cross-subspace case vacuously true).
