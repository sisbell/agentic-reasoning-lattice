...
*Formal Contract:*
- *Preconditions:* `dom(M(d))` finite (S8-fin); `M(d)` a function (S2); every V-reference resolves — `(A v ∈ dom(M(d)) :: M(d)(v) ∈ dom(Σ.C))` (S3); `(A v ∈ dom(M(d)) :: zeros(v) = 0 ∧ v₁ ≥ 1)` (S8a); within each subspace, all V-positions share a common depth (S8-depth); V-positions within each subspace form a contiguous ordinal range — `(A d, S, u, q : u ∈ V_S(d) ∧ q ∈ V_S(d) ∧ u < q : (A v : subspace(v) = S ∧ #v = #u ∧ u < v < q : v ∈ V_S(d)))` (D-CTG); every V-position has depth at least 2 (S8-vdepth).
