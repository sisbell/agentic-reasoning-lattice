...
S1 is a corollary of S0, stated separately for emphasis. It is the content-store specialisation of T8 (allocation permanence, ASN-0034): T8 guarantees that allocated addresses persist in the abstract address space; AX-2 (allocation-mediated entry) establishes `dom(Σ.C) ⊆ allocated(s)`, bridging the content store to the allocated set; S1 ensures that the content at those addresses persists as well.
