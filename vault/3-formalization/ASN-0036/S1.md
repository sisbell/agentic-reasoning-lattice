...
S1 is strictly weaker than S0: it asserts domain persistence without value preservation. We state it separately because it names a distinct architectural commitment — the content store grows monotonically — and because the allocated address space and the content store have different scopes. The allocated set includes addresses that carry no content; `dom(Σ.C)` covers only addresses at which content has actually been stored. That `dom(Σ.C)` is a subset of the allocated set — a containment derived as the set-containment corollary of AX-2 (allocation-mediated entry) — means domain monotonicity could in principle follow from monotonicity of the allocated set together with AX-2. But the derivation from S0 is more direct and reveals the logical relationship: domain monotonicity is a consequence of content immutability, not an independent commitment. ∎
