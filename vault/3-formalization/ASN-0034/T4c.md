...
*Formal Contract:*
- *Preconditions:* `t` satisfies the T4 constraints (at most three zero-valued components, positive-component constraint, field-segment constraint — no two zeros adjacent, `t₁ ≠ 0`, `t_{#t} ≠ 0`). `t` satisfies T4b (UniqueParse): every zero in `t` is a field separator and every separator is a zero, so the separator positions are exactly the zero-valued positions.
