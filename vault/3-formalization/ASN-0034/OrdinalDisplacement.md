...
Since δ(n, m) is a finite sequence of length m ≥ 1 over ℕ, it satisfies the carrier set criterion, so δ(n, m) ∈ T by T0. Since n ≥ 1, the m-th component of δ(n, m) is nonzero, whence δ(n, m) > 0 by PositiveTumbler. By ActionPoint, actionPoint(δ(n, m)) = min({i : 1 ≤ i ≤ m ∧ δ(n, m)ᵢ ≠ 0}); since δ(n, m)ᵢ = 0 for 1 ≤ i < m and δ(n, m)ₘ = n ≥ 1, the minimum is m. ∎
...
- *Postconditions:* δ(n, m) ∈ T (by T0), δ(n, m) > 0 (by PositiveTumbler), actionPoint(δ(n, m)) = m (by ActionPoint)
