...
*Ingredient 1 (field extraction).* By T4, valid address tumblers have at most three zero-valued components, no two zeros are adjacent, and the tumbler neither begins nor ends with zero; by T0, every component lies in ℕ, so every non-zero component is strictly positive. The function `fields(t)` decomposes a tumbler into its node, user, document, and element fields by locating all zero-valued components — which, by T4's definitional role-assignment, are exactly the field separators, leaving the strictly positive components (via T0) as field contents. By T4(b), `fields(t)` is well-defined and uniquely determined by `t` alone. Since `t` is a finite sequence, the extraction terminates: scan `t` once, record the positions of zero-valued components, and partition the remaining components into the corresponding fields. Write `N(t)`, `U(t)`, `D(t)`, `E(t)` for the node, user, document, and element fields of `t` respectively. Each is a finite (possibly absent) sequence of strictly positive natural numbers.
