...
- *Depends:* T1 (LexicographicOrder) — postcondition (a) invokes T1 case (i) at divergence position `sig(t)` for `k = 0`, and T1 case (ii) with the proper-prefix condition `#t + 1 ≤ #t'` for `k > 0`.
