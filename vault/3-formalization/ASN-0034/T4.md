...
T4 is an axiom: it constrains which tumblers the system admits as valid addresses. We verify three consequences that follow from these constraints. The argument uses only T3 (canonical representation) and the T4 constraints themselves; no other properties are required. The syntactic equivalence of the non-empty field constraint appears as T4a (SyntacticEquivalence). The unique-parse theorem appears as T4b (UniqueParse). The level-determination bijection appears as T4c (LevelDetermination). ∎

*Formal Contract:*
- *Axiom:* Valid address tumblers satisfy `zeros(t) ≤ 3`, no adjacent zeros, `t₁ ≠ 0`, and `t_{#t} ≠ 0`.
- *Postconditions:* (T4a) The non-empty field constraint — each present field has `≥ 1` component with all components strictly positive — is equivalent to the three syntactic conditions: no adjacent zeros, `t₁ ≠ 0`, `t_{#t} ≠ 0`. (T4b) For every valid address tumbler `t`, `fields(t)` is well-defined and computable from `t` alone; the decomposition into node, user, document, and element fields is unique. (T4c) `zeros(t) ∈ {0, 1, 2, 3}` determines the hierarchical level bijectively: `0 ↔ node`, `1 ↔ user`, `2 ↔ document`, `3 ↔ element`.
