...
- *Depends:* T3 (CanonicalRepresentation) — postcondition (b) requires the bridge from component-level agreement to tumbler equality: Case 1 invokes T3 to conclude `a = b` from `m = n` and `aᵢ = bᵢ` for all `i`; Case 3 invokes its contrapositive to conclude `a ≠ b` from `m ≠ n`.
