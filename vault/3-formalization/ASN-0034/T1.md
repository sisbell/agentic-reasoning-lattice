...
*Proof.* We show that `<` as defined is a strict total order on T by establishing irreflexivity, trichotomy, and transitivity. The argument relies on the corresponding properties of `<` on ℕ and on T3 (CanonicalRepresentation, stated in § *Canonical form* above): tumblers with the same length and identical components at every position are equal. T3 is a consequence of T0's extensional characterisation of T as a set of finite sequences — two finite sequences are equal exactly when they agree in length and at every position — so it is a derived theorem, not an independent postulate.
