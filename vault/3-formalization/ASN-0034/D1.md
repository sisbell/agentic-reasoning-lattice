...
Define w = b ⊖ a. TumblerSub's case analysis pivots on the zero-padded divergence zpd(b, a), not the formal Divergence. Under our preconditions these coincide: since k ≤ #a ≤ #b, every position i ≤ k falls within both operands' actual lengths, so no zero-padding occurs at or before position k; the padded sequences therefore agree exactly where the unpadded ones do — at all i < k — and first disagree at position k, where bₖ ≠ aₖ. Hence zpd(b, a) = k = divergence(a, b). The subtraction yields:
...
We establish three properties of w needed for TumblerAdd. First, w ∈ T: since a < b implies b ≥ a, TumblerSub's precondition is satisfied for minuend b and subtrahend a, and its postcondition gives w = b ⊖ a ∈ T. Second, w > 0: the component wₖ = bₖ − aₖ ≥ 1 since aₖ < bₖ, so w is not the zero tumbler. Third, the action point of w is k: every component before position k is zero, and wₖ > 0, so k is the first positive component. Since k ≤ #a by hypothesis, the precondition of TumblerAdd (TA0) is satisfied — the action point falls within the start position's length.
