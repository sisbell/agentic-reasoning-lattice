...
**I3-VD** — *PostInsertionDepthUniformity* (POSTCONDITION, introduced). `(A v₁, v₂ ∈ dom(M'(d)) : subspace(v₁) = subspace(v₂) = S ⟹ #v₁ = #v₂ = m)`. V-positions in dom(M'(d)) within subspace S fall into two regions. *Left region* (I3-L): v ∈ dom(M(d)) with subspace(v) = S and v < p; these have depth m by VD on M(d). *Shifted region* (I3): shift(v, n) for v ∈ dom(M(d)) with subspace(v) = S and v ≥ p; #shift(v, n) = #δₙ = m by the result-length identity of TumblerAdd, and #v = m by VD on M(d). Both regions yield depth m. ∎

**I3-VP** — *PostInsertionPositiveSubspace* (POSTCONDITION, introduced). `(A v ∈ dom(M'(d)) : v₁ ≥ 1)`. V-positions in dom(M'(d)) fall into three regions. *Left region* (I3-L): v ∈ dom(M(d)) with subspace(v) = S and v < p; v₁ = S ≥ 1 by VP on M(d). *Shifted region* (I3): shift(v, n) for v ∈ dom(M(d)) with subspace(v) = S and v ≥ p; shift(v, n)₁ = v₁ ≥ 1 since shift copies position 1 from v when m ≥ 2, and VP on M(d) gives v₁ ≥ 1. *Cross-subspace region* (I3-X): v ∈ dom(M(d)) with subspace(v) ≠ S; v₁ ≥ 1 by VP on M(d). ∎
