...
We need two properties of this shift before we can use it in the postcondition. Both are established in the foundation.
...
(ii) *Arrangement reordering* — K.μ~ on document d, *when the shift set is non-empty*. Let V_shift = {v ∈ dom(M(d)) : subspace(v) = S ∧ v ≥ p}. When V_shift ≠ ∅, apply K.μ~ with bijection π : v ↦ shift(v, n) for v ∈ V_shift and π(v) = v otherwise. By TS1 (order preservation, ASN-0034) and TS2 (injectivity, ASN-0034), π is well-behaved. It produces V-positions satisfying S8a (shift preserves positivity), S8-depth (shift preserves depth), and the multiset of I-addresses is preserved (only V-positions change). Precondition: d ∈ E_doc. Satisfied by I8(i). When V_shift = ∅ — inserting into an empty subspace or appending past all existing positions — step (ii) is omitted: K.μ~ requires strict contraction followed by strict extension, which is inapplicable when π is the identity. The composite then reduces to steps (i), (iii), (iv).
...
**Elementary preconditions at intermediate states.** ASN-0047's ValidComposite requires each step's precondition to hold at the intermediate state where it executes. For K.α: each aᵢ must satisfy aᵢ ∉ dom(Cᵢ₋₁) where Cᵢ₋₁ is the content store after the preceding allocations. We have aᵢ ∉ dom(C) by I0(i). For i ≥ 2, we need aᵢ ∉ {a₁, ..., aᵢ₋₁}: I0(ii) gives aⱼ₊₁ = aⱼ + 1 > aⱼ by TA5(a) (ASN-0034), which directly states t' > t for ordinal increment, so a₁ < a₂ < ... < aₙ — all distinct. Therefore aᵢ ∉ dom(C) ∪ {a₁, ..., aᵢ₋₁} = dom(Cᵢ₋₁). For K.μ⁺: the new V-positions {p + k : 0 ≤ k < n} must not already belong to dom(M(d)) at the point K.μ⁺ executes (i.e., after step (ii)). When step (ii) applies, every position in V_shift has been shifted to positions ≥ p + n by I3, and positions in subspace S below p remain below p by I1, so the interval [p, p + (n − 1)] is unoccupied. When step (ii) is omitted (V_shift = ∅), no positions in subspace S exist at or beyond p, so the interval is likewise unoccupied. Positions in other subspaces have different subspace identifiers and cannot collide.

**Coupling constraints.** J0 (AllocationRequiresPlacement): every a ∈ dom(C') \ dom(C) appears in M'(d) via step (iii). J1 (ExtensionRecordsProvenance): the new addresses in ran(M'(d)) \ ran(M(d)) are the a₁...aₙ, each recorded in R' by step (iv). The shifted entries contribute no new I-addresses to ran(M'(d)) — only their V-positions change. J1' (ProvenanceRequiresExtension): each new pair (aᵢ, d) ∈ R' \ R corresponds to aᵢ ∈ ran(M'(d)) \ ran(M(d)). These arguments hold identically whether step (ii) is present or omitted: K.μ~ contributes no new content to dom(C') and no new provenance to R', so the coupling constraints J0, J1, J1' depend only on steps (i), (iii), and (iv).
...
**S2 (ArrangementFunctionality).** M'(d) is a function — each V-position maps to exactly one I-address. The four regions (I1, I2, I3, I4) are pairwise disjoint (verified in Domain Completeness above), so no V-position receives two mappings. Within each region, functionality is inherited (I1, I4), by construction (I2), or by injectivity of the shift (I3 with TS2, ASN-0034).
...
| TS1 | shift preserves strict order: v₁ < v₂ ⟹ shift(v₁, n) < shift(v₂, n) | cited (ASN-0034) |
| TS2 | shift is injective: shift(v₁, n) = shift(v₂, n) ⟹ v₁ = v₂ | cited (ASN-0034) |
