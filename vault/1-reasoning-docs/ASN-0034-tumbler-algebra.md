...
*(b) Same node and user fields.* Extract `N(a), U(a)` and `N(b), U(b)` via `fields` (Fact 1). Both `a` and `b` must possess user fields — that is, `zeros(a) ≥ 1` and `zeros(b) ≥ 1` — which is decidable by Fact 2. If either tumbler lacks a user field, the answer is *no*: two tumblers cannot share a field that one does not possess. When both are present, apply componentwise comparison (Fact 3) to each pair in turn: first `N(a) = N(b)`, checking `#N(a) = #N(b)` and `(A i : 1 ≤ i ≤ #N(a) : N(a)ᵢ = N(b)ᵢ)`; then `U(a) = U(b)`, checking `#U(a) = #U(b)` and `(A j : 1 ≤ j ≤ #U(a) : U(a)ⱼ = U(b)ⱼ)`. The procedure terminates and returns *yes* iff both pairs are identical. The total number of comparisons is at most `#N(a) + #U(a) + 2` — two length checks and at most `#N(a) + #U(a)` componentwise checks.
...
**Sub-partition construction.** Within the partition, the parent allocator spawns a child allocator according to T10a. The child's base address `t₀` is produced by `inc(s, k)` with `k > 0`, where `s` is a parent sibling extending `p`; by TA5(d), `#t₀ = #s + k` and the first `#s` components of `t₀` are those of `s`. Since `p ≼ s` gives `sᵢ = pᵢ` for `1 ≤ i ≤ #p`, and `#s ≥ #p`, these equalities transfer: `t₀ᵢ = sᵢ = pᵢ` for `1 ≤ i ≤ #p`, with `#t₀ > #s ≥ #p`, so `p ≼ t₀`. The child allocator then produces its sibling outputs by repeated application of `inc(·, 0)` (T10a): `t₁ = inc(t₀, 0)`, `t₂ = inc(t₁, 0)`, and so on. Each `tᵢ` serves as the prefix for a sub-partition within the parent partition.

We verify that every sub-partition prefix lies within the parent partition: `p ≼ tₙ` for all `n ≥ 0`. The base case `p ≼ t₀` is established above. For the inductive step, suppose `p ≼ tₙ`. By TA5(c), `tₙ₊₁ = inc(tₙ, 0)` differs from `tₙ` only at position `sig(tₙ)`. By TA5(d), the last component of `t₀` is `1`, so `sig(t₀) = #t₀`; and by TA5(c), each sibling increment modifies only position `sig(tₙ)` where the value increases, so the last nonzero position is preserved: `sig(tₙ) = #t₀` for all `n`. Since `#t₀ = #s + k > #s ≥ #p`, the modification at position `#t₀` does not affect positions `1` through `#p`, giving `tₙ₊₁,ᵢ = tₙ,ᵢ = pᵢ` for `1 ≤ i ≤ #p` and therefore `p ≼ tₙ₊₁`.
...
**Intra-partition ordering.** Within any single sub-partition with prefix `tᵢ`, the child allocator's direct sibling outputs — those produced by successive `inc(·, 0)` applications (T10a) — are ordered by T9: for any two such siblings `a, b` with `allocated_before(a, b)`, we have `a < b`. The sub-partition may also contain addresses from descendant allocators spawned within it; the ordering of these is established recursively in the total ordering argument below.

**Total ordering.** Every address in the partition belongs to exactly one sub-partition — the one whose prefix it extends. This uniqueness follows from non-nesting: for distinct `i ≠ j`, if an address `a` extended both `tᵢ` and `tⱼ`, then `tᵢ ≼ a` and `tⱼ ≼ a` would give `tᵢ,ₗ = aₗ = tⱼ,ₗ` for all `1 ≤ ℓ ≤ #tᵢ = #tⱼ` (uniform length), so `tᵢ = tⱼ` — contradicting distinctness. For any two distinct allocated addresses `a` and `b` within the partition, exactly one of three cases holds.

*Case 1: Same sub-partition.* Both `a` and `b` extend the same prefix `tᵢ`. Since `a, b ∈ T` and `a ≠ b`, T1 trichotomy (T1(b)) gives exactly one of `a < b` or `b < a` — comparability is immediate from the total order on T, without appeal to any allocator relationship. It remains to show consistency with allocation order. If `a` and `b` were both produced by the same allocator (i.e., `same_allocator(a, b)` holds), then T9 gives `allocated_before(a, b) ⟹ a < b` directly. If they were produced by different allocators — as when `a` is a direct output of the child allocator at level `tᵢ` and `b` is produced by a descendant allocator spawned within the `tᵢ` sub-partition, or vice versa — then the sub-partition rooted at `tᵢ` is itself a prefix-delimited partition. The same argument applies recursively: the descendant allocator's outputs form their own sub-sub-partitions, and by PrefixOrderingExtension at the next level, the prefix structure determines the ordering across these deeper allocators. At each level of the recursion, intra-allocator ordering is governed by T9 and inter-allocator ordering by PrefixOrderingExtension, until the recursion bottoms out at pairs sharing a single allocator. In every case, `a` and `b` are comparable under T1, and the ordering is consistent with allocation order at each level of the hierarchy.
...
The length separation extends inductively across nesting levels. Each child-spawning step via `inc(·, k')` with `k' ≥ 1` adds at least one component (TA5(d)). A descendant `d` nesting levels below the parent produces outputs of length at least `γ + d > γ`. Allocators at different nesting depths produce outputs of different lengths, so they cannot collide by the T3 contrapositive. Allocators on different branches have non-nesting prefixes — neither branch's prefix extends the other's — and are handled by Case 2.
...
Let k = divergence(a, b). The preconditions give k ≤ #a and #a ≤ #b, so k ≤ #a = min(#a, #b). This eliminates Divergence case (ii), which requires k = min(#a, #b) + 1 = #a + 1 > #a, contradicting k ≤ #a. We are in case (i): aᵢ = bᵢ for all i < k, and aₖ ≠ bₖ at position k ≤ min(#a, #b). Since a < b, T1 (Lexicographic order) provides a witness j for the strict ordering. T1 case (ii) is excluded — it requires agreement through position #a, but aₖ ≠ bₖ with k ≤ #a. So T1 case (i) holds with some j ≤ min(#a, #b) where aⱼ < bⱼ and aᵢ = bᵢ for i < j. This j must equal k: j < k contradicts the Divergence case (i) agreement (aⱼ = bⱼ for j < k), and j > k places k in the T1 agreement prefix (aₖ = bₖ for k < j), contradicting Divergence case (i). Therefore aₖ < bₖ.
