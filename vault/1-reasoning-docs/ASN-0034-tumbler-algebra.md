...
*Case 1: Same sub-partition.* Both `a` and `b` extend the same prefix `tᵢ`. Since they are produced by the same allocator's sequential stream — T10a constrains each allocator to produce outputs by successive `inc(·, 0)` applications, imposing a total order on its allocation events — one was allocated before the other. By T9, `allocated_before(a, b) ⟹ a < b` (or `allocated_before(b, a) ⟹ b < a`). In either case, `a` and `b` are comparable under T1, and the ordering is consistent with allocation order.
