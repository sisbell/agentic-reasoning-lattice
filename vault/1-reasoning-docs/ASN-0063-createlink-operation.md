...
The coverage *is* tight at element level. Within a single CL0 I-span arising from block β = (v_β, a_β, n) with overlap range [c, c'), the depth-`#a_β` members of the denotation are exactly `{a_β + k : c ≤ k < c'}`. The argument is structurally identical to the V-position case in CL0's proof. Consider any depth-`#a_β` tumbler `t` satisfying `a_β + k ≤ t < a_β + (k + 1)`. First, `t` must agree with `a_β + k` on components 1 through `#a_β − 1`: if `t` differed at position `j < #a_β`, then either `t_j < (a_β + k)_j`, giving `t < a_β + k` by T1(i), or `t_j > (a_β + k)_j`; but `(a_β + k)_j = (a_β + (k + 1))_j` for `j < #a_β` (ordinal increment affects only the last component), so `t_j > (a_β + (k + 1))_j`, giving `t > a_β + (k + 1)` by T1(i) — both contradictions. Second, with the prefix fixed, the last component satisfies `(a_β + k)_{#a_β} ≤ t_{#a_β} < (a_β + (k + 1))_{#a_β} = (a_β + k)_{#a_β} + 1`, so by the integer gap `t_{#a_β} = (a_β + k)_{#a_β}` and thus `t = a_β + k`. These are precisely the I-addresses produced by M(d) for the overlapping V-positions.
