...
**Theorem — ExtendedReachableStateInvariants.** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the elementary transitions K.α, K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻, K.μ~, and K.ρ — satisfies:
  ...
  S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ D-CTG ∧ D-MIN ∧ P0 ∧ P1 ∧ P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6 ∧ P7 ∧ P7a ∧ P8 ∧ L0 ∧ L1 ∧ L1a ∧ L12 ∧ L14

This supersedes the ReachableStateInvariants theorem (ASN-0047) by replacing S3 with S3★, P4 with P4★, P3 with P3★, P5 with P5★, adding the link invariants L0, L1, L1a, L12, L14, and covering the extended transition set including K.λ and K.μ⁺_L.

*Proof.* Base: the extended initial state Σ₀ satisfies all invariants (verified in the Extending the Transition Framework section — L₀ = ∅ satisfies link invariants vacuously; S3★ and P4★ reduce to S3 and P4). Inductive step: each elementary transition preserves the full invariant set. For K.α, K.δ, K.ρ: hold both M and L in frame; content, entity, and provenance invariants preserved per ASN-0047; link invariants preserved since L is unchanged. For K.μ⁺ (amended): holds L in frame; S3★ and P4★ preserved (analyses above); D-CTG, D-MIN preserved by the amended postcondition requirement; S8 follows from D-SEQ, S8-fin, S8a, S2, S8-depth; link invariants preserved since L is unchanged. For K.μ⁻: holds L in frame; S3★ preserved (restriction of M(d) preserves both clauses); P4★ preserved (Contains_C can only shrink under contraction); D-CTG, D-MIN preserved by the postcondition requirement — contraction admits only removal from the maximum end or removal of all positions, per D-SEQ; S8 follows similarly; link invariants preserved since L is unchanged. For K.μ~: holds L in frame; S3★ and P4★ preserved (analyses above); link-subspace positions are fixed (link-subspace fixity). For each content subspace S, D-SEQ gives V_S(d) = {[S, 1, ..., 1, k] : 1 ≤ k ≤ n}; π bijects dom(M(d)) onto dom(M'(d)) preserving S8a, S8-depth, S8-fin (K.μ~ preconditions, ASN-0047), and link-subspace fixity forces π to biject dom_C(M(d)) onto dom_C(M'(d)); D-SEQ at equal cardinality yields V_S(d') = V_S(d), so D-CTG and D-MIN hold; S8 follows; link invariants preserved since L is unchanged. For K.λ: holds M, C, E, R in frame; link invariants verified (orphan link analysis above). For K.μ⁺_L: holds C, L, E, R in frame; arrangement invariants verified in CL11 (per-subspace arrangement invariants); S3★ satisfied by precondition (`ℓ ∈ dom(L)`). Coupling constraints J0, J1★, J1'★ hold for all valid composites by the analysis in the Extending the Transition Framework section. ∎
...
| ExtendedReachableStateInvariants | Every reachable state satisfies S0 ∧ S1 ∧ S2 ∧ S3★ ∧ S8a ∧ S8-fin ∧ S8-depth ∧ S8 ∧ D-CTG ∧ D-MIN ∧ P0–P2 ∧ P3★ ∧ P4★ ∧ P5★ ∧ P6–P8 ∧ L0 ∧ L1 ∧ L1a ∧ L12 ∧ L14; supersedes ASN-0047 ReachableStateInvariants | introduced |
