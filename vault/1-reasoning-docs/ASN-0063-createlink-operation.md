...
**Lemma CL1 — ResolutionExistence.** For any document d with M(d) satisfying the arrangement invariants (S2, S8-fin, ASN-0036) and any V-span-set Ψ whose spans lie in the text subspace — `(A σ : σ ∈ Ψ : subspace(start(σ)) = s_C)` — there exists an endset `E ∈ Endset` with `image(d, Ψ) ⊆ coverage(E)`.
...
**CL2 — ResolutionContainment.** Under the same text-subspace precondition as CL1 — `(A σ : σ ∈ Ψ : subspace(start(σ)) = s_C)` — we have `image(d, Ψ) ⊆ coverage(resolve(d, Ψ))`.

Immediate from CL1: each element of `image(d, Ψ)` is an I-address `M(d)(v)` for some `v ∈ ⟦Ψ⟧ ∩ dom(M(d))`, which falls in exactly one CL0 I-span by B1 and B2. The text-subspace precondition is essential: CL0 projects through text-subspace mapping blocks (ASN-0058's canonical decomposition covers the text subspace), so a V-span crossing into the link subspace would contribute positions falling in no text-subspace block, leaving their images uncovered.
...
| CL1 | For any d and text-subspace Ψ (`subspace(start(σ)) = s_C`), there exists E ∈ Endset with image(d, Ψ) ⊆ coverage(E) | introduced |
| resolve(d, Ψ) | Finite endset of CL0 I-spans from canonical block decomposition | introduced |
| CL2 | image(d, Ψ) ⊆ coverage(resolve(d, Ψ)) for text-subspace Ψ — resolution containment | introduced |
