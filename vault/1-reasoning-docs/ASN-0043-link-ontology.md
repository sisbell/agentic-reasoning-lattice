...
Because link addresses are element-level tumblers (L1) allocated under their creating document's prefix (L1a), the same field-extraction formula that ASN-0036 uses to define `origin` on `dom(Σ.C)` is well-defined for link addresses. T4 (HierarchicalParsing, ASN-0034) defines `fields` for tumblers satisfying its format constraints (no adjacent zeros, no leading/trailing zeros, positive non-separator components). Link addresses satisfy these constraints by L1 (`zeros(a) = 3`), so `fields` is well-defined for them. We define the link analog directly.
...
This is the same formula as `origin` (ASN-0036), applied here to link addresses rather than content addresses. The domain extension is justified: `origin`'s formula relies only on `fields`, which T4 defines for tumblers satisfying its format constraints, and on `zeros(a) = 3` (L1), which guarantees both that all four fields are present and that T4's constraints are met.
...
| GlobalUniqueness | LEMMA | No two allocation events produce the same address — extends S4 (OriginBasedIdentity, ASN-0036) beyond I-addresses via T9, T10, T10a, TA5, T3 (ASN-0034) | introduced |
