...
This architectural property — that transclusion inherits link discoverability without coupling — is *stronger* than SV8 for K.μ⁺/K.μ⁺_L transitions: SV8 gives `⊆` (monotonicity) for arbitrary transitions; SV7 gives `=` (invariance) for arrangement extensions specifically. It is the mechanism by which shared content propagates link relationships. It applies equally to forking (J4): the new version shares I-addresses with the source by the fork's K.μ⁺ step, so it discovers the same links for all shared content without explicit link propagation.
...
| SV7 | TransclusionCouplingAbsence: `discover_s(A) in Σ' = discover_s(A) in Σ` for K.μ⁺/K.μ⁺_L — discovery is invariant, not merely monotonic | introduced |
