...
*(a) The maximal block containing any v ∈ dom(M(d)) is uniquely determined.* Fix v. Define the *forward extent* f(v) = max{k ≥ 0 : (A j : 0 ≤ j ≤ k : v + j ∈ dom(M(d)) ∧ M(d)(v + j) = M(d)(v) + j)}. This maximum exists because dom(M(d)) is finite (S8-fin). Define the *backward extent* r(v) = max{k ≥ 0 : (A j : 0 ≤ j ≤ k : ord(v) − j ≥ 1 ∧ M(d)(v − j) = M(d)(v) − j)}, where v − j denotes [S, ord(v) − j] (well-defined when ord(v) − j ≥ 1, since V-positions have positive ordinals by S8a). Both f(v) and r(v) are determined by M(d) and v alone — M(d) is a function (S2), so for each candidate position the correspondence either holds or does not, with no ambiguity. The maximal block containing v is then (v − r(v), M(d)(v) − r(v), r(v) + 1 + f(v)), and it is uniquely determined by the values of r(v) and f(v).

*(b) Two maximal blocks sharing a V-position are identical.* Let b₁ = (v₁, a₁, n₁) and b₂ = (v₂, a₂, n₂) be maximal blocks with some w ∈ V(b₁) ∩ V(b₂). Since w ∈ V(b₁), we have w = v₁ + k₁ and M(d)(w) = a₁ + k₁ for some 0 ≤ k₁ < n₁; similarly w = v₂ + k₂ and M(d)(w) = a₂ + k₂. Now consider any position u ∈ V(b₂). Write u = v₂ + j for some 0 ≤ j < n₂, so M(d)(u) = a₂ + j. Since M(d)(w) = a₂ + k₂ and M(d)(u) = a₂ + j, the correspondence M(d)(w + (j − k₂)) = M(d)(w) + (j − k₂) holds (both sides reduce to a₂ + j). This means the forward or backward extension from w reaches u while maintaining the correspondence — so u must lie within the maximal block containing w. By (a), the maximal block containing w is unique, so V(b₂) ⊆ V(b₁). By the symmetric argument, V(b₁) ⊆ V(b₂). Since both are maximal blocks with the same V-extent, they have the same start, width, and I-start, hence b₁ = b₂.
