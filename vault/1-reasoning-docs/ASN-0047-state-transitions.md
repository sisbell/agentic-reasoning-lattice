...
K.μ~ is a distinguished composite, not a primitive transition. When dom_C(M(d)) ≠ ∅ (some content-subspace positions exist), it decomposes into K.μ⁻ (removing content-subspace mappings) followed by K.μ⁺ (re-adding content-subspace mappings at new positions). When dom(M(d)) ≠ ∅ but dom_C(M(d)) = ∅ — a document with link-subspace positions only — we claim π is the identity and K.μ~ expands into zero elementary steps. The argument parallels the dom_C ≠ ∅ fixity proof (below) in simplified form. For any v ∈ dom_L(M(d)), the K.μ~ definition gives M'(d)(π(v)) = M(d)(v), and S3★'s link clause gives M(d)(v) ∈ dom(L), so M'(d)(π(v)) ∈ dom(L); by S3★-aux, subspace(π(v)) ∈ {s_C, s_L}, and the case subspace(π(v)) = s_C is eliminated because S3★'s content clause would require M'(d)(π(v)) ∈ dom(C), contradicting dom(C) ∩ dom(L) = ∅ (L14, SC-NEQ). Thus π maps dom_L(M(d)) injectively into dom_L(M'(d)). Since K.μ⁺ (amended) cannot create link-subspace V-positions, |dom_L(M'(d))| ≤ |dom_L(M(d))| − r where r is the count of link-subspace positions removed by K.μ⁻; the injection forces r = 0. Since dom_C(M(d)) = ∅, K.μ⁻ has no content-subspace positions to remove either — it removes nothing, and its strict-subset requirement dom(M_int(d)) ⊂ dom(M(d)) is unsatisfiable, so K.μ⁻ cannot fire. With K.μ⁻ vacuous, K.μ⁺ has nothing to re-add. Therefore zero elementary steps; M'(d) = M(d); π = id. In the extended state when dom_C(M(d)) ≠ ∅, link-subspace mappings are preserved through the K.μ⁻ + K.μ⁺ decomposition: the K.μ⁺ amendment (below) restricts K.μ⁺ to content-subspace V-positions, so link-subspace mappings removed by K.μ⁻ could not be restored — forcing K.μ⁻ to retain them. The full argument establishing link-subspace fixity under K.μ~ is developed in the S3★ analysis and ExtendedReachableStateInvariants theorem below. We verify the intermediate-state preconditions and derive the frame for the non-trivial case (dom_C(M(d)) ≠ ∅).
