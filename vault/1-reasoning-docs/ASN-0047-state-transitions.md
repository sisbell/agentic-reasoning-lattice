...
**ValidComposite★ (ValidComposite, amended).** A composite transition Σ → Σ' in the extended state Σ = (C, L, E, M, R) is *valid* iff it is a finite sequence of elementary transitions `Σ = Σ₀ → Σ₁ → ... → Σₙ = Σ'` — drawn from K.α (amended), K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻ (amended), K.μ~, and K.ρ — satisfying:
...
**ExtendedReachableStateInvariants.** Every state reachable from Σ₀ = (C₀, L₀, E₀, M₀, R₀) by a finite sequence of valid composite transitions — composed from the elementary transitions K.α, K.δ, K.λ, K.μ⁺ (amended), K.μ⁺_L, K.μ⁻, K.μ~, and K.ρ — satisfies:
...
| K.μ⁺ amendment | Content-subspace restriction (`subspace(v) = s_C`) and D-CTG/D-MIN postcondition; partitions arrangement extension by subspace with K.μ⁺_L | introduced |
| K.μ⁻ amendment | D-CTG/D-MIN postcondition: M'(d) must satisfy D-CTG and D-MIN for each subspace; constrains contraction to removal from the maximum end or removal of all positions | introduced |
...
| ValidComposite★ | Valid composite in extended state: elementary preconditions at each step + J0, J1★, J1'★ at composite boundary; supersedes ValidComposite | introduced |
