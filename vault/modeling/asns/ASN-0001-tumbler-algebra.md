...
Now subtract `w` from `r`. The subtraction scans for the first divergence between `r` and `w`. For `i < k`: `rᵢ = aᵢ = 0 = wᵢ` (both are zero — `aᵢ` by the zero-prefix precondition, `wᵢ` by definition of action point). So the subtraction finds no divergence before position `k`. Two sub-cases arise at position `k`.

*Sub-case (i): `aₖ > 0`.* Then `rₖ = aₖ + wₖ > wₖ` (since `aₖ > 0`), and the first divergence is at position `k`. The subtraction produces: positions `i < k` get zero, position `k` gets `rₖ - wₖ = aₖ + wₖ - wₖ = aₖ`, and positions `i > k` copy from `r`, giving `rᵢ = wᵢ`. Since `k = #a` and `#w = k`, there are no trailing components, and the result is `[0, ..., 0, aₖ] = a`. For valid I-space addresses, T4's positive-component constraint guarantees `aₖ > 0`, so this sub-case always applies in the address context.

*Sub-case (ii): `aₖ = 0`.* Then `a` is a zero tumbler (all components zero, since positions before `k` are zero by hypothesis and `aₖ = 0`). The addition gives `rₖ = 0 + wₖ = wₖ`. The result `r` has: `rᵢ = 0 = wᵢ` for `i < k` and `rₖ = wₖ`. Since `#r = #w = k` (no trailing components), `r = w`. The subtraction `r ⊖ w = w ⊖ w` follows the equal-operand path: no divergence exists, and the result is the zero tumbler of length `k`, which is `a`. TA4 holds through this distinct path.
...
We define the *last significant position* of a tumbler `t`. When `t` has at least one nonzero component, `sig(t) = max({i : 1 ≤ i ≤ #t ∧ tᵢ ≠ 0})` — the position of the last nonzero component. When every component is zero, `sig(t) = #t` — the last position. For `[1, 0, 3]`, `sig = 3` (position of the 3). For `[1, 0, 3, 0]`, `sig = 3` (position of the 3 — the trailing zero is not the last nonzero component; however, this trailing-zero case does not arise for valid I-space addresses, as shown below). For `[0, 0]`, `sig = 2` (the last position, since no component is nonzero). Note that T3 makes `[1, 0, 3, 0]` and `[1, 0, 3]` distinct tumblers with the same `sig` value — this is consistent because `sig` identifies where the increment acts, not the identity of the tumbler.
...
**TA7a (Subspace closure).** Let `S₁` and `S₂` be distinct element subspaces within a document. The canonical representation for shift arithmetic is the *ordinal-only* formulation: a V-position in subspace `S` with identifier `N` and ordinal `x` is represented as the single-component tumbler `[x]` for arithmetic purposes, with `N` held as structural context that selects which positions are subject to the shift. An element-local displacement is `w = [n]` with `n > 0`. In this formulation, the shift operations are closed within each subspace:

  `(A [x] ∈ S₁, n > 0 : [x] ⊕ [n] = [x + n] ∈ S₁)`

  `(A [x] ∈ S₁, n > 0, x ≥ n : [x] ⊖ [n] = [x - n] ∈ S₁ ∪ {[0]})`

The subspace identifier is context — it determines which crums are subject to the shift — not an operand to the arithmetic. The ordinal-only formulation is adopted because, as the verification below shows, applying `⊖` to full 2-component V-positions `[N, x]` with displacement `[0, n]` finds the divergence at the subspace identifier (position 1), not at the ordinal (position 2), producing a no-op rather than a genuine shift. Both operations produce genuine shifts in the ordinal-only view; the 2-component view gives a genuine shift for `⊕` but a vacuous closure for `⊖`.

**Verification of TA7a.** We derive TA7a from the constructive definition of `⊕`. A V-space position in subspace `S` has the form `a = [N, x]` where `N` is the subspace identifier and `x` is the ordinal. An element-local displacement has the form `w = [0, n]` with `n > 0`, giving action point `k = 2` — strictly after the subspace-identifier position 1. By the constructive definition's "copy from start" rule, `(a ⊕ w)ᵢ = aᵢ` for `i < k`, so `(a ⊕ w)₁ = a₁ = N`. The subspace identifier is preserved. At position `k = 2`, `(a ⊕ w)₂ = a₂ + w₂ = x + n`. The result `[N, x + n]` remains in subspace `S`. For `⊖`: the divergence scan on `[N, x]` and `[0, n]` finds the first difference at position 1 (where `N ≠ 0`), not at the ordinal position — this is the subtlety noted in the worked example. As established in the subspace confinement analysis above, the implementation resolves this by operating on ordinals directly: the arithmetic receives `[x]` and `[n]` as operands, with the subspace identifier as structural context.

In the ordinal-only formulation, the shift operates on `x` by `w = [n]`. The ordinal is a single-component tumbler with `#x = 1`, and the displacement has `#w = 1` with action point `k = 1`. For `⊕`: `(x ⊕ n)₁ = x + n`, a positive single-component tumbler (since `x ≥ 1` and `n > 0`). For `⊖`: the divergence is at position 1 (since `x ≥ n`); the result is `[x - n]`. When `x = n`, the result is `[0]`, a sentinel (TA6) marking the subspace boundary. In either case the result is a single-component non-negative tumbler — a valid within-subspace ordinal. The subspace identifier, preserved by the copy-from-start rule in the 2-component view and held as context in the ordinal view, is never modified. TA7a holds.
...
A span is a pair `(s, ℓ)` where `s ∈ T` is a start address and `ℓ ∈ T` is a length (a positive tumbler used as a displacement), denoting the contiguous range from `s` up to but not including `s ⊕ ℓ`. The length `ℓ` is a tumbler — the same kind of object as `s` — because `s ⊕ ℓ` is defined by TA0, which requires both operands in T. The form of `ℓ` depends on the hierarchical level at which the span operates, because the action point of `ℓ` must match the level of the start address `s`. For within-subspace ordinals — single-component values `[x]` used in V-space — a span length is a single-component tumbler `[n]` with action point `k = 1`. For full I-space element addresses — 8-component values like `[1, 0, 3, 0, 2, 0, 1, x]` — the span length must be `[0, 0, 0, 0, 0, 0, 0, n]` with action point `k = 8`, so that the advance occurs at the element ordinal, not at a higher hierarchical level. A single-component length `[n]` applied to a full I-space address would have action point `k = 1`, and `s ⊕ [n]` would replace the entire hierarchical structure below position 1 — producing a server-level address, not an element-level displacement. The algebra also admits multi-component lengths at higher hierarchical levels, which arise when a span crosses a boundary (e.g., a span covering all content in multiple documents under a user).
...
**T12 (Span well-definedness).** The addresses `a₂` through `a₄` form a natural span in I-space — three consecutive elements within the text subspace of document 2. We construct the span `(s, ℓ)` with `s = a₂ = 1.0.3.0.2.0.1.2`. The start address has 8 components, so an element-level displacement must have action point `k = 8`: `ℓ = [0, 0, 0, 0, 0, 0, 0, 3]`. By the constructive definition of `⊕`: positions 1–7 copy from `s` (giving `1.0.3.0.2.0.1`), and position 8 advances: `2 + 3 = 5`. So `s ⊕ ℓ = 1.0.3.0.2.0.1.5 = a₅`. The span denotes `{t : a₂ ≤ t < a₅} = {a₂, a₃, a₄}` — exactly the three intended elements. A single-component length `[3]` would give the wrong result: action point `k = 1`, and `[1, 0, 3, 0, 2, 0, 1, 2] ⊕ [3] = [4]` — a node-level address, not an element displacement. The action point of the span length must match the hierarchical level of the start address.
