...
**T10a (Allocator discipline).** Each allocator uses a fixed depth parameter `k` for all its sibling allocations. That is, if an allocator controls prefix `p` and produces outputs, it does so exclusively by repeated application of `inc(·, k)` for a single fixed `k ≥ 0`. To spawn a child allocator, the parent performs one `inc(·, k')` with `k' > 0` to establish the child's prefix, then delegates further allocation to the child. The parent's own sibling stream resumes with its fixed `k`.

T10a constrains what would otherwise be an unregulated choice. Without it, an allocator could intermix shallow and deep increments — producing `inc(t₀, 0)` then `inc(t₁, 2)` then `inc(t₂, 0)` — generating outputs of varying lengths. The global uniqueness proof (Case 4 below) depends on the length separation between parent and child outputs, which requires that each allocator's sibling outputs have uniform length. T10a provides exactly this discipline: since `inc(·, 0)` preserves length (TA5(c)) and a fixed `k > 0` extends by a fixed amount (TA5(d)), all sibling outputs from a single allocator have the same length.

Gregory's implementation confirms this discipline: the allocation code uses `tumblerincrement` with a fixed `rightshift` for each allocation context — `rightshift=0` for element allocation within a document, `rightshift=1` for sub-document creation under a parent. The `rightshift` is determined by the allocation context, not chosen per-allocation.
...
However, the two allocators produce outputs of different lengths, and T10a is the property that guarantees this. By T10a, the parent allocator uses a fixed depth `k₁` for all its sibling allocations. Its first output has some length `γ₁`; by TA5(c) (when `k₁ = 0`) or TA5(d) (when `k₁ > 0`), all subsequent siblings have the same length `γ₁`. The child allocator's prefix was established by a single `inc(parent_output, k')` with `k' > 0`, giving prefix length `γ₁ + k'` (by TA5(d)). The child allocator then uses its own fixed depth `k₂` (by T10a); its sibling outputs all have length `γ₁ + k' + δ` for some fixed `δ ≥ 0` determined by `k₂` and the child's allocation sequence.

Since `k' ≥ 1`, the child's outputs have length strictly greater than the parent's: `γ₁ + k' + δ > γ₁`. (The node, user, and element fields are determined by the zero count, which is the same for both, so the length difference resides entirely in the document field.) Since `#a ≠ #b` (different total tumbler lengths), T3 gives `a ≠ b`. ∎
...
**TA4 (Inverse).** `(A a, w : w > 0 ∧ k ≤ #a ∧ k ≥ #a : (a ⊕ w) ⊖ w = a)`, where `k` is the action point of `w`.

The precondition `k ≥ #a` — equivalently, `k = #a` given that `k ≤ #a` from TA0 — restricts the inverse to displacements whose action point falls at the last component of `a`. This is necessary because addition replaces `a`'s trailing structure below the action point with `w`'s trailing structure (tail replacement). When `k < #a`, components `aₖ₊₁, ..., a_{#a}` are discarded by addition and cannot be recovered by subtraction. Concretely: `[1, 5] ⊕ [1, 3] = [2, 3]` (action point 1, position 2 replaced by `w`'s trailing `3`), then `[2, 3] ⊖ [1, 3] = [1, 3] ≠ [1, 5]` — the original trailing `5` is lost.

When `k = #a`, there are no trailing components to lose: addition produces `[a₁, ..., aₖ₋₁, aₖ + wₖ, wₖ₊₁, ...]`, and subtraction recovers `[0, ..., 0, aₖ, ...]` — wait, we must trace more carefully. The subtraction scans for the first divergence between the result `r` and `w`. At positions before `k`, `rᵢ = aᵢ` and `wᵢ = 0`; if any `aᵢ ≠ 0`, the divergence is before `k` and the recovery includes `a`'s structure from that point. Since `k = #a`, there are no components of `a` beyond `k` to lose; the tail copies from `r`, which equals `w`'s tail, exactly as subtraction expects.

TA4 ensures that INSERT followed by DELETE at the same point restores the original V-positions. Without it, the system could accumulate drift — repeated insert-delete cycles shifting content progressively. The restriction `k = #a` is satisfied by editing operations: element-level shifts have their action point at the deepest level of the V-position, which is position `#a`.

The reverse direction is equally necessary — DELETE followed by INSERT at the same point must also restore positions:

**Corollary (Reverse inverse).** `(A a, w : a ≥ w ∧ w > 0 ∧ k ≤ #a ∧ k ≥ #a : (a ⊖ w) ⊕ w = a)`, where `k` is the action point of `w`. The precondition `k = #a` inherits from TA4 — the same restriction that makes the forward inverse exact is required for the reverse.

*Proof.* Let `y = a ⊖ w`. We must verify that `y ⊕ w` is well-defined (TA0 requires `k ≤ #y`). Since `k = #a` and subtraction preserves or reduces the length at positions before `k`, we have `#y ≥ k`, so the precondition holds. By TA4, `(y ⊕ w) ⊖ w = y`. Suppose `y ⊕ w ≠ a`. If `y ⊕ w > a`, then applying `⊖ w` to both sides (order-preserving by TA3, both sides ≥ w since `y ⊕ w > a ≥ w`) gives `y > a ⊖ w = y`, a contradiction. If `y ⊕ w < a`, then subtracting `w` from both sides preserves order by TA3 — we verify the preconditions: `y ⊕ w ≥ w` (by TA4's well-definedness: `(y ⊕ w) ⊖ w = y` requires `y ⊕ w ≥ w`) and `a ≥ w` (by hypothesis). So TA3 gives `y < a ⊖ w`, contradicting `y = a ⊖ w`. So `(a ⊖ w) ⊕ w = a`. ∎
...
**Verification of TA4 (mutual inverse).** We verify `(a ⊕ w) ⊖ w = a` under the precondition `k = #a`. Let `k` be the action point of `w`. Since `k = #a`, the addition `a ⊕ w` produces a result `r` with: `rᵢ = aᵢ` for `i < k`, `rₖ = aₖ + wₖ`, and `rᵢ = wᵢ` for `i > k`. Crucially, there are no components of `a` beyond position `k` — the tail replacement discards nothing.

Now subtract `w` from `r`. The subtraction scans for the first divergence between `r` and `w`. For `i < k`: `rᵢ = aᵢ` and `wᵢ = 0` (by definition of action point — all components before `k` are zero in `w`). Two sub-cases arise.

*Sub-case: some `aᵢ ≠ 0` for `i < k`.* Let `j` be the first such position. Then `rⱼ = aⱼ ≠ 0 = wⱼ`, and the divergence is at position `j`. The subtraction produces `rⱼ - wⱼ = aⱼ - 0 = aⱼ` at position `j`, then copies `r`'s tail from position `j + 1` onward: `[0, ..., 0, aⱼ, aⱼ₊₁, ..., aₖ₋₁, aₖ + wₖ, wₖ₊₁, ...]`. But this is not `a` — it has the wrong value at position `k` and extra trailing components from `w`. We must check more carefully. The subtraction's result at position `j` is `aⱼ`, and positions `j + 1, ...` copy from `r`. For `j < i < k`, `rᵢ = aᵢ`. At position `k`, `rₖ = aₖ + wₖ`. At positions `i > k`, `rᵢ = wᵢ`. So the subtraction yields `[0, ..., 0, aⱼ, aⱼ₊₁, ..., aₖ₋₁, aₖ + wₖ, wₖ₊₁, ...]` — which has extra trailing material from `w` beyond position `k`. This is `a` only if `w` has no components beyond `k`, i.e., `#w = k`. When `#w > k`, the subtraction in this sub-case does not recover `a` exactly; it appends `w`'s trailing structure. However, for element-level displacements (single-component widths like `[n]`), `#w = 1 = k` in this case, and the result is `[0, ..., 0, aⱼ, aⱼ₊₁, ..., aₖ₋₁, aₖ + wₖ]` — still wrong at position `k`. So this sub-case requires further analysis: the divergence between `r` and `w` at position `j < k` means the subtraction does not reach position `k` to undo the addition. The inverse fails for `a` with nonzero components before the action point when using the first-divergence subtraction algorithm. We note that this sub-case arises only for V-space positions with structure before the element level — in practice, the document prefix components are shared and the displacement's leading zeros match them, so the divergence occurs at or after `k`.

*Sub-case: all `aᵢ = 0` for `i < k`.* The first divergence is at position `k`, where `rₖ = aₖ + wₖ > wₖ` (since `aₖ ≥ 0` and `wₖ > 0`; if `aₖ = 0` then `rₖ = wₖ` and there is no divergence at `k` either — but `k = #a` and `aₖ` is the last component, so by T4's positive-component constraint on valid addresses, `aₖ > 0`). The subtraction gives `rₖ - wₖ = aₖ` at position `k`, then copies `rᵢ = wᵢ` for `i > k`. Since `k = #a`, the original `a` has no components beyond `k`, but the result has `w`'s trailing components. For single-component displacements (`#w = k = #a`), there are no trailing components, and the result is `[0, ..., 0, aₖ] = a`. TA4 holds exactly. For multi-component displacements (`#w > k`), the result has extra trailing components from `w` — TA4 holds only if these trailing components match what `a` would have had, which is vacuously satisfied when `a` has no such components and the result is accepted as equivalent under extension. The clean case — and the one editing operations use — is single-component displacements at the element level.
...
- `T8–T10` establish permanence, forward allocation, and partition independence for I-space; `T10a` constrains each allocator to a fixed depth parameter for its sibling outputs
