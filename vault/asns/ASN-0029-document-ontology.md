...
The existential scopes over both postcondition and frame — the `d` that is freshly created is the same `d` added to `Σ'.D` and quantified over in the frame. The `parent(d) undefined` constraint makes explicit that D0 produces a root document — one whose document field is single-component. The allocator model distinguishes two cases (T10a): when the account has no existing root documents, the account-level allocator spawns the document-level allocator via `inc(a, 2)` (child creation with `k' = 2`), producing the first root document with `D₁ = 1`; when root documents already exist, the document-level allocator's sibling stream via `inc(·, 0)` advances the document component. This is essential for the monotonicity clause: a root document `a.0.K` with `K` exceeding all existing root document numbers also exceeds all children of lower-numbered roots (since `a.0.K'.x < a.0.K` when `K' < K` by T1), so `(A d' : d' ∈ Σ.D ∧ account(d') = a : d' < d)` holds. A non-root document would not satisfy this — see the discussion of per-allocator monotonicity in D1. Only the account owner may create documents under that account — matching D5(d) and the pattern of D10a and D15. No I-space content is allocated. No existing document is disturbed. The new document exists but contains nothing — a position claimed, a container awaiting content. It begins as `private`; the owner must explicitly publish it.

**Worked example for D0.** Let account `a = 1.0.1` have existing root document `d₁ = 1.0.1.0.3` and child `d₂ = 1.0.1.0.3.1` (a version of `d₁` created by D12 Case 1). The root allocator's current maximum is `3`; D0 produces `d = 1.0.1.0.4` via `inc(·, 0)` (sibling increment at the document component). We verify the monotonicity postcondition `(A d' : d' ∈ Σ.D ∧ account(d') = a : d' < d)`:

- `d₁ = 1.0.1.0.3 < 1.0.1.0.4 = d`: divergence at position 5 (the first document component), `3 < 4`, so `d₁ < d` by T1(i). ✓
- `d₂ = 1.0.1.0.3.1 < 1.0.1.0.4 = d`: same divergence at position 5, `3 < 4`, so `d₂ < d` by T1(i) — regardless of `d₂`'s additional components. ✓

The key observation: the root allocator advances the first document component past all existing first document components under the account. Any document `d'` under account `a` — whether root or child — has a first document component `K' ≤ 3 < 4 = K`, so divergence occurs at position 5 with `K' < K`, giving `d' < d` by T1(i). This argument generalizes: it depends only on the root allocator producing a first document component exceeding all existing ones (T9, ForwardAllocation), not on the depth of existing documents in the version tree.
...
- The *root allocator* for account `a` produces root documents (single-component document field). On first invocation, the account-level allocator spawns the document-level allocator via `inc(a, 2)` (T10a child creation, `k' = 2`), producing the first root document; subsequent root documents are produced via `inc(·, 0)` (T10a sibling stream). D0 uses this allocator.
...
Different allocators under the same account are NOT jointly monotonic. If account `1.0.1` creates root documents `1.0.1.0.3`, then `1.0.1.0.5`, then versions `1.0.1.0.3` producing child `1.0.1.0.3.1`, we have `1.0.1.0.3.1 < 1.0.1.0.5` by T1 (diverge at position 5: `3 < 5`), yet `1.0.1.0.3.1` was allocated later. The root allocator and the child allocator for `1.0.1.0.3` are independent — each monotonic in its own stream, but not jointly ordered. Per-account uniqueness requires distinguishing two cases. Between child allocators with distinct parents (e.g., the child allocator for `1.0.1.0.3` vs. the child allocator for `1.0.1.0.5`), T10 (PartitionIndependence) applies: neither parent's prefix nests within the other, so their outputs are guaranteed distinct. Between the root allocator and any child allocator, T10 does not apply — the root's ownership prefix `a` is a prefix of each child allocator's ownership prefix `a.0.D₁` (since `a ≼ a.0.D₁`), violating the non-nesting precondition. Uniqueness here follows from structural length: root allocator outputs have single-component document fields (`#doc = 1`), while child allocator outputs have multi-component document fields (`#doc ≥ 2`), giving different tumbler lengths and therefore distinct tumblers by T3 (CanonicalRepresentation). By T5 (ContiguousSubtrees), all documents sharing a prefix form a contiguous interval in the tumbler order.
...
Verification against each document-creating operation: D0 allocates via the root allocator for the account. *Base case:* when no root documents exist under the account, the account-level allocator spawns the document-level allocator via `inc(a, 2)` (T10a child creation with `k' = 2`); by TA5(d), this appends one intermediate zero (field separator) and a final component of `1`, producing `N.0.U.0.1` with `#D = 1` and `D₁ = 1 > 0`; the monotonicity postcondition is vacuously satisfied (no existing documents under the account). *Inductive case:* when root documents already exist, the document-level allocator uses `inc(·, 0)` (T10a sibling stream), incrementing the document component to produce a single-component document field with `D₁ > 0`. D12 Case 1 allocates via `inc(d_s, 1)`, appending one component to `d_s`'s document field with the new component set to `1` (TA5(d)); since `d_s ∈ Σ.D` satisfies D14a by induction, the result has `#D ≥ 2` with all components positive. D12 Case 2 uses the root allocator for `a_req`, identical to D0. No other operation adds documents to Σ.D.
...
*Case 2: cross-account version.* CREATENEWVERSION(d_s, 2.0.1) — user `2.0.1` versions Alice's document. Since `account(d_s) = 1.0.1 ≠ 2.0.1 = a_req`, D13 gives `account(d_v) = 2.0.1`. The version is allocated under the requester's account via the root allocator for `2.0.1`. Suppose `2.0.1` already has root document `2.0.1.0.2` and child `2.0.1.0.2.1`; the root allocator produces `d_v = 2.0.1.0.3`. Postconditions (a)–(f) hold identically. For (g₂): we must show `(A d' : d' ∈ Σ.D ∧ account(d') = 2.0.1 : d' < d_v)`. The root allocator advances the first document component past all existing ones under `2.0.1` (T9, ForwardAllocation). Any document `d'` under account `2.0.1` — root or child — has first document component `K' ≤ 2 < 3 = K`; divergence occurs at position 5 with `K' < K`, giving `d' < d_v` by T1(i), regardless of `d'`'s depth in the version tree. This is the same argument as D0's monotonicity (see worked example above). But `d_s ⋠ d_v` — there is no structural ancestry in the address. The version relationship exists only through shared I-addresses: both `Σ'.V(d_v)` and `Σ'.V(d_s)` map to `{a₁, a₂}`.
...
The four operations introduced in this ASN — D0, D10a, D12, D17 — must preserve P0 (ISpaceImmutable), P1 (ISpaceMonotone), P2 (ReferentiallyComplete), P3 (MappingExact), P7 (CrossDocVIndependent), and D7b (HomeDocumentMembership). The verifications are straightforward. D0, D10a, and D12 all have `Σ'.I = Σ.I` in their frames, so P0 and P1 hold trivially: `dom(Σ'.I) = dom(Σ.I)` and every address retains its content. D17 is a pure query with `Σ' = Σ`, so all invariants are preserved vacuously. For P2 (ReferentiallyComplete): D0 creates a document with `|Σ'.V(d)| = 0`, so the quantifier `(A p : 1 ≤ p ≤ n_d : ...)` is vacuously satisfied. D10a does not modify any V-space (`Σ'.V(d) = Σ.V(d)` in its frame), so P2 is inherited from the pre-state. D12 creates `d_v` with `Σ'.V(d_v)(p) = Σ.V(d_s)(p)` for all `p` (postcondition (c)); by P2 on Σ, `Σ.V(d_s)(p) ∈ dom(Σ.I) = dom(Σ'.I)`, so P2 holds for `d_v`. For P3 on `d_v`: `Σ'.I(Σ'.V(d_v)(p)) = Σ'.I(Σ.V(d_s)(p))` by (c), `= Σ.I(Σ.V(d_s)(p))` by (e), which satisfies P3 by P3 on Σ for `d_s`. For existing documents, all four operations preserve V-space in their frames, so P2, P3, and P7 carry forward from the pre-state.

For D7b (HomeDocumentMembership): D0 creates a document with `|Σ'.V(d)| = 0`, so D7b is vacuously satisfied for `d`; for existing documents, V-space is unchanged (frame) and `Σ.D ⊆ Σ'.D` (D2), so D7b carries forward. D12 creates `d_v` with `Σ'.V(d_v)(p) = Σ.V(d_s)(p)` (postcondition (c)); by D7b on Σ, `home(Σ.V(d_s)(p)) ∈ Σ.D`; by D2, `Σ.D ⊆ Σ'.D`, so `home(Σ'.V(d_v)(p)) ∈ Σ'.D`. For D10a, V-space is unchanged and `Σ.D ⊆ Σ'.D`, so D7b is inherited. For D17, `Σ' = Σ`.
...
The key insight across all properties: the tumbler address does extraordinary work. It encodes identity (D6), ownership (D3, D4), provenance (D7), and structural subordination (D14's ≺ relation). Because documents are permanent members of Σ.D (D2) and these encodings are pure functions of the address value, they are permanent — no mutable state is consulted, so no state transition can alter them. The `parent` function (D14) requires a separate argument: `parent(d) = max≼ {d' ∈ Σ.D : d' ≺ d}` consults `Σ.D`, which is mutable. But `parent(d)` is nonetheless invariant across all subsequent states. Once `parent(d) = d_s` at creation (D12 Case 1, via `inc(d_s, 1)`), no future document can interpose between `d_s` and `d` in the ≺ ordering: `d` extends `d_s` by exactly one document-level component, so any `d'` with `d_s ≺ d' ≺ d` would need to be a proper extension of `d_s` that is also a proper prefix of `d` — but no such document-level tumbler exists, since `d`'s document field extends `d_s`'s by a single component, leaving no room for an intermediate. Since `Σ.D` only grows (D2), `parent(d)` can only gain candidates; but none can interpose, so the maximum remains `d_s`. A single design decision — permanent addresses — gives the entire lifecycle its character: immutable ownership, structural provenance, non-destructive collaboration, and version forests that grow but never reorganize.
