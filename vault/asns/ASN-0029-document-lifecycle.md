...
**Worked example for D0.** Let account `a = 1.0.1` have existing root document `d₁ = 1.0.1.0.3` and child `d₂ = 1.0.1.0.3.1` (a version of `d₁` created by D12 Case 1). The root allocator's current maximum is `3`; D0 produces `d = 1.0.1.0.5` via `inc(·, 0)` (sibling increment at the document component). We verify the monotonicity postcondition `(A d' : d' ∈ Σ.D ∧ account(d') = a : d' < d)`:

- `d₁ = 1.0.1.0.3 < 1.0.1.0.5 = d`: divergence at position 5 (the first document component), `3 < 5`, so `d₁ < d` by T1(i). ✓
- `d₂ = 1.0.1.0.3.1 < 1.0.1.0.5 = d`: same divergence at position 5, `3 < 5`, so `d₂ < d` by T1(i) — regardless of `d₂`'s additional components. ✓

The key observation: the root allocator advances the first document component past all existing first document components under the account. Any document `d'` under account `a` — whether root or child — has a first document component `K' ≤ 3 < 5 = K`, so divergence occurs at position 5 with `K' < K`, giving `d' < d` by T1(i). This argument generalizes: it depends only on the root allocator producing a first document component exceeding all existing ones (T9, ForwardAllocation), not on the depth of existing documents in the version tree.
