...
*Source: ASN-0034-tumbler-algebra.md (revised 2026-03-21) — Extracted: 2026-03-25*

## T0(a) — Every component value of a tumbler is unbounded (INV, predicate)
...
## T0(b) — Tumblers of arbitrary length exist in T (INV, predicate)
...
## T1 — Tumblers are totally ordered by lexicographic comparison, with the prefix-les... (INV, predicate)
...
## T2 — Tumbler comparison is computable from the two addresses alone, examining at m... (INV, predicate)
...
## T3 — Each tumbler has exactly one canonical representation; component-wise identit... (INV, predicate)
...
## T4 — An address tumbler has at most three zero-valued components as field separato... (INV, predicate)
...
## T5 — The set of tumblers sharing a prefix forms a contiguous interval under T1 (THEOREM, lemma)
...
## T6 — Containment (same node, same account, same document family, structural subord... (LEMMA, lemma)
...
## T7 — Subspaces (text, links) within a document's element field are permanently dis... (LEMMA, lemma)
...
## T8 — Once allocated, an address is never removed from the address space; the set o... (INV, predicate)
...
## T9 — Within a single allocator's sequential stream, new addresses are strictly mon... (LEMMA, lemma)
...
## T10 — Allocators with non-nesting prefixes produce distinct addresses without coord... (INV, predicate)
...
## T10a — Each allocator uses inc(·, 0) for siblings and inc(·, k>0) only for child-spa... (INV, predicate)
...
## PartitionMonotonicity — Per-allocator ordering extends cross-allocator; for non-nesting sibling prefi... (THEOREM, lemma)
...
## GlobalUniqueness — No two distinct allocation events anywhere in the system at any time produce ... (THEOREM, lemma)
...
## T12 — A span (s, ℓ) is well-formed when ℓ > 0 and action point k of ℓ satisfies k ≤... (INV, predicate)
...
## TA0 — Tumbler addition a ⊕ w is well-defined when w > 0 and the action point k sati... (PRE, requires)
...
## TA1 — Addition preserves the total order (weak) (POST, ensures)
...
## Divergence — For a ≠ b (DEFINITION, function)
...
## TA1-strict — Addition preserves the total order (strict) when k ≤ min(#a, #b) ∧ k ≥ diverg... (POST, ensures)
...
## TA-strict — Adding a positive displacement strictly advances (POST, ensures)
...
## TA2 — Tumbler subtraction a ⊖ w is well-defined when a ≥ w (PRE, requires)

## TA3 — Subtraction preserves the total order (weak) (POST, ensures)

## TA3-strict — Subtraction preserves the total order (strict) when additionally #a = #b (POST, ensures)

### Partial inverse

## TA4 — Addition and subtraction are partial inverses (LEMMA, lemma)
...
## ReverseInverse — ReverseInverse (LEMMA, lemma)
...
## TA5 — Hierarchical increment inc(t, k) produces t' > t (POST, ensures)
...
## TA6 — Every all-zero tumbler (any length) is less than every positive tumbler and i... (INV, predicate)
...
## TA7a — For ordinals `o = [o₁, (POST, ensures)
...
The restriction to element-local displacements is necessary. An unrestricted displacement whose action point falls at the subspace-identifier position 
[...truncated...]

## TA-assoc — Addition is associative where both compositions are defined (LEMMA, lemma)
...
## TA-RC — Right cancellation fails (LEMMA, lemma)
...
## D0 — Displacement well-definedness (PRE, requires)
...
## D1 — Displacement round-trip (LEMMA, lemma)
...
## D2 — Displacement uniqueness (LEMMA, lemma)
