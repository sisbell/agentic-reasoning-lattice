-- A4.left — CopyLeftFrame
-- Positions left of the insertion point are unchanged after Copy.
...
-- A4.left: positions left of pt are preserved
assert CopyLeftFrame {
...
-- Non-vacuity: find a Copy where pt > 1 so left frame is non-trivial
run FindCopy {
    some s, sPost: State, ds, dt: Doc, ps, k, pt: Int |
        wellFormed[s] and bounded[ps, k, pt] and
        Copy[s, sPost, ds, dt, ps, k, pt] and pt > 1
} for 5 but exactly 2 State, 6 Int

check CopyLeftFrame for 5 but exactly 2 State, 6 Int
