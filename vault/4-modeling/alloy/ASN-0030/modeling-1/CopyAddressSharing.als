-- ASN-0030 A5(a) — CopyAddressSharing
-- After COPY(d_s, p_s, k, d_t, p_t), target positions share source I-addresses:
...
-- COPY precondition (matches ASN-0030 A5 pre)
...
-- Property: copied positions actually share the source I-addresses
assert CopyAddressSharing {
  all s, s2: State, ds, dt: Document, ps, k, pt: Int |
    Copy[s, s2, ds, dt, ps, k, pt] implies
      (all j: Int | (j >= 0 and j < k) implies
        dt.(s2.vmap)[plus[pt, j]] = ds.(s.vmap)[plus[ps, j]])
}

-- Property: post-state target is well-formed
assert CopyPostWellFormed {
  all s, s2: State, ds, dt: Document, ps, k, pt: Int |
    Copy[s, s2, ds, dt, ps, k, pt] implies wellFormed[s2, dt]
}

-- Property: post-state target length is n_dt + k
assert CopyNewLength {
  all s, s2: State, ds, dt: Document, ps, k, pt: Int |
    Copy[s, s2, ds, dt, ps, k, pt] implies
      docLen[s2, dt] = plus[docLen[s, dt], k]
}

-- Property: source document unchanged when d_s != d_t
...
run FindCopy {
  some s, s2: State, ds, dt: Document, ps, k, pt: Int |
    Copy[s, s2, ds, dt, ps, k, pt]
} for 4 but exactly 2 State, 6 Int

-- Non-vacuity: self-copy permitted (d_s = d_t)
run FindSelfCopy {
  some s, s2: State, d: Document, ps, k, pt: Int |
    Copy[s, s2, d, d, ps, k, pt]
} for 5 but exactly 2 State, 6 Int

check CopyAddressSharing for 5 but exactly 2 State, 6 Int
