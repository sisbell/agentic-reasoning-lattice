...
<parameters>
<file_path>vault/2-blueprints/ASN-0036/properties/S8-depth.md</file_path>
<content>**S8-depth (FixedDepthVPositions).** Within a given subspace `s` of document `d`, all V-positions share the same tumbler depth:
...
A *correspondence run* formalises the notion of a contiguous block where arrangement preserves ordinal displacement — see **Definition (CorrespondenceRun)**.

*Contract:*

- *Label:* S8-depth
- *Name:* FixedDepthVPositions
- *Statement:* `(A d, v₁, v₂ : v₁ ∈ dom(Σ.M(d)) ∧ v₂ ∈ dom(Σ.M(d)) ∧ (v₁)₁ = (v₂)₁ : #v₁ = #v₂)`
- *Status:* design requirement
- *Dependencies:* — (design requirement; no logical antecedents)
- *Dependents:* S8 (FiniteSpanDecomposition), D-CTG-depth (SharedPrefixReduction), D-SEQ (SequentialPositions)
- *Notes:* Parallel to S7a (DocumentScopedAllocation) in being a design-level constraint. Enables Definition (ConsecutiveVPositions), Definition (OrdinalDisplacementExtension), Definition (CorrespondenceRun).
